...
'    End If
    ...
    Select Case UCase(gHOSP.MACHNM)
        ...
        Case "BS200":           Call TCPRcvData_BS200E
        
        
        Case "CYBOW300":        Call Phase_Serial_CYBOW300
...
'                   3 : 체크된것중 제일 위에 것
'-----------------------------------------------------------------------------'
Private Sub GetOrder_BS240E(ByVal pBarno As String, ByVal pType As String) ', _
...
'        Call SetText(.spdOrder, mOrder.Seq, intRow, colSEQNO)
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = GetEquipExamCode_BS240E(gHOSP.MACHCD, pBarno, intRow)
...
Private Sub GetOrder_BS200E(ByVal pBarno As String, ByVal pType As String)
    ...
    GetOrder = ""
    blnLast = False
    mOrder.BS_DtTM = Format(Now, "yyyymmddhhmmss")
    
    '-- 1. 접수정보 조회
    With frmMain
        For i = 1 To .spdOrder.DataRowCnt
            If GetText(frmMain.spdOrder, i, colCHECKBOX) = "1" And Trim(GetText(frmMain.spdOrder, i, colSTATE)) = "" Then
                ...
                mOrder.PID = Trim(GetText(.spdOrder, i, colCHARTNO))
                intRow = i
                'intSndPhase = intSndPhase + 1
                Exit For
            End If
...
'            wSck.SendData strSndBuffer
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = GetEquipExamCode_BS200E(gHOSP.MACHCD, pBarno, intRow)
        ...
        End If
                   ...
                   GetOrder = SB & "MSH|^~\&|||" & mOrder.BS_Maker & "|" & mOrder.BS_MchNm & "|" & mOrder.BS_DtTM & "||DSR^Q03|1|P|2.3.1||||0||ASCII|||" & vbCr
        GetOrder = GetOrder & "MSA|AA|" & CStr(intSndPhase) & "|Message accepted|||0|" & vbCr
        GetOrder = GetOrder & "ERR|0|" & vbCr
        GetOrder = GetOrder & "QAK|SR|OK|" & vbCr
        GetOrder = GetOrder & "QRD|" & mOrder.BS_DtTM & "|R|D|" & mOrder.BS_QryId & "|||RD|" & mOrder.BarNo & "|OTH|||T|" & vbCr
        GetOrder = GetOrder & "QRF|" & mOrder.BS_Model & "|" & mOrder.BS_STime & "|" & mOrder.BS_ETime & "|||RCT|COR|ALL||" & vbCr
        GetOrder = GetOrder & "DSP|1||" & mOrder.BarNo & "|||" & vbCr
        GetOrder = GetOrder & "DSP|2|||||" & vbCr
        'GetOrder = GetOrder & "DSP|3||" & mOrder.PNAME & " " & mOrder.PID & "|||" & vbCr
        GetOrder = GetOrder & "DSP|3||" & mOrder.PNAME & "|||" & vbCr
        GetOrder = GetOrder & "DSP|4|||||" & vbCr
        GetOrder = GetOrder & "DSP|5|||||" & vbCr
        GetOrder = GetOrder & "DSP|6|||||" & vbCr
        GetOrder = GetOrder & "DSP|7|||||" & vbCr
        GetOrder = GetOrder & "DSP|8|||||" & vbCr
        GetOrder = GetOrder & "DSP|9|||||" & vbCr
        GetOrder = GetOrder & "DSP|10|||||" & vbCr
        GetOrder = GetOrder & "DSP|11|||||" & vbCr
        GetOrder = GetOrder & "DSP|12|||||" & vbCr
        GetOrder = GetOrder & "DSP|13|||||" & vbCr
        GetOrder = GetOrder & "DSP|14|||||" & vbCr
        GetOrder = GetOrder & "DSP|15|||||" & vbCr
        GetOrder = GetOrder & "DSP|16|||||" & vbCr
        GetOrder = GetOrder & "DSP|17|||||" & vbCr
        GetOrder = GetOrder & "DSP|18|||||" & vbCr
        GetOrder = GetOrder & "DSP|19|||||" & vbCr
        GetOrder = GetOrder & "DSP|20|||||" & vbCr
        GetOrder = GetOrder & "DSP|21||" & mOrder.BarNo & "|||" & vbCr
        'GetOrder = GetOrder & "DSP|21||" & mOrder.PID & "|||" & vbCr
        GetOrder = GetOrder & "DSP|22||" & CStr(intSndPhase) & "|||" & vbCr
        GetOrder = GetOrder & "DSP|23|||||" & vbCr
        GetOrder = GetOrder & "DSP|24|||||" & vbCr
        GetOrder = GetOrder & "DSP|25|||||" & vbCr
        GetOrder = GetOrder & "DSP|26||serum|||" & vbCr
        GetOrder = GetOrder & "DSP|27|||||" & vbCr
        GetOrder = GetOrder & "DSP|28|||||" & vbCr
        GetOrder = GetOrder & mOrder.Order
        
        If intRow = spdOrder.DataRowCnt Then
            GetOrder = GetOrder & "DSC||" & vbCr
            GetOrder = GetOrder & EB & vbCr
        Else
            GetOrder = GetOrder & "DSC|" & CStr(intSndPhase) & "|" & vbCr
            GetOrder = GetOrder & EB & vbCr
        End If
...
'    strRData = strRecvData
    ...
    Call SetSQLData("RCV", UBound(strRecvData) & ">>>", "A")
...
'
'[Rx]
'MSH|^~\&|Mindray|BS-220E|||20200626121231||ACK^Q03|1|P|2.3.1||||0||ASCII|||
...
'ERR|0|
            ...
            Select Case strType
                Case "MSH"
                        ']MSH|^~\&|Mindray|BS-220E|||20200703121601||ORU^R01|24|P|2.3.1||||0||ASCII|||
                        With mOrder
                            .BS_SEQ = mGetP(strRcvBuf, 10, "|")
                            .BS_MType = mGetP(strRcvBuf, 9, "|")
                            .BS_Maker = mGetP(strRcvBuf, 3, "|")
                            .BS_MchNm = mGetP(strRcvBuf, 4, "|")
                            .BS_DtTM = Format(Now, "yyyymmddhhmmss")
                        End With

                        Select Case mOrder.BS_MType
                            Case "ORU^R01"
                            
                            '-- 오더 준비
                            Case "QRY^Q02"
'                                               strSndBuffer = SB & "MSH|^~\&|||" & mOrder.BS_Maker & "|" & mOrder.BS_MchNm & "|" & mOrder.BS_DtTM & "||QCK^Q02|" & mOrder.BS_MType & "|P|2.3.1||||0||ASCII|||" & vbCr
'                                strSndBuffer = strSndBuffer & "MSA|AA|" & mOrder.BS_MType & "|Message accepted|||0|" & vbCr
'                                strSndBuffer = strSndBuffer & "ERR|0|" & vbCr & EB & vbCr
...
'                                End If
                            ...
                            Case "ACK^Q03"
                                '-- 오더만들기
                                'SetRawData "[2ND]" & strSndBuffer
                                Call GetOrder_BS200E(strBarno, gHOSP.RSTTYPE)

                        End Select
                Case "QRD"
                        mOrder.BS_QryId = mGetP(strRcvBuf, 5, "|")

                Case "QRF"
                        mOrder.BS_Model = mGetP(strRcvBuf, 2, "|")
                        mOrder.BS_STime = mGetP(strRcvBuf, 3, "|")
                        mOrder.BS_ETime = mGetP(strRcvBuf, 4, "|")

                        '-- 오더만들기
                        Call GetOrder_BS200E(strBarno, gHOSP.RSTTYPE)

                Case "PID"
                    mOrder.BS_MType = mGetP(strRcvBuf, 2, "|")


                Case "OBR"
''MSH|^~\&|Mindray|BS-220E|||20200630171918||ORU^R01|29|P|2.3.1||||0||ASCII|||
''PID|28|492694|||전미자||||||||||||||||||||||||||
''OBR|1|492694|3|Mindray^BS-220E|N||20200630162334||||||||Serum|||||||||||||||||||||||||||||||||
''OBX|1|NM|GGT|r-GTP|12.792507|U/L|11.000000-73.000000|Normal|||F||12.792507|20200630162334||||

                    '-- 인터페이스 응답
                                   strSndBuffer = SB & "MSH|^~\&|Mindray|BS-220E|||" & Format(Now, "yyyymmddhhmmss") & "||ACK^R01|" & mOrder.BS_SEQ & "|P|2.3.1||||0||ASCII|||" & vbCr
                    strSndBuffer = strSndBuffer & "MSA|AA|" & mOrder.BS_SEQ & "|Message accepted|||0|" & vbCr
                    strSndBuffer = strSndBuffer & EB & vbCr
...
ErrHandle:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_TCPRcvData_BS200E" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
            ...
            Select Case strType
                Case "MSH"
                    ...
                    Select Case mOrder.MSHMessageType
                        ...
                        Case "ACK^Q03"
                            '-- 최초이후전송
                            Call GetOrder_BS240E(strBarno, gHOSP.RSTTYPE)
...
Private Sub SerialRcvData_CYBOW300()
    Dim RS_L            As ADODB.Recordset
...
RST:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SerialRcvData_CYBOW300" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Private Sub Phase_Serial_CYBOW300()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
            ...
            Case 1      '## Estabilshment Phase
                Select Case BufChar
                    Case ENQ
                        ...
                        Debug.Print "intBufCnt ENQ >>" & intBufCnt
                        '2019-11-04 추가
                        ReDim Preserve strRecvData(intBufCnt)
...
'                        End If
                        ...
                        Debug.Print "intBufCnt >> STX" & intBufCnt
...
Private Sub wSCK_DataArrival(ByVal bytesTotal As Long)
    ...
    pBuffer = strText & vbCr
    
'    Call Sleep(200)
    
    If InStr(strText, EB) > 0 Then
