...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Public Function GetEquipExamCode_BS200E(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim lngIntBase  As Long
    Dim strItems    As String           '전송할 검사항목
    Dim blnISE      As Boolean          'Na, K, Cl 검사여부
    
    GetEquipExamCode_BS200E = ""
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            mOrder.SendCnt = mOrder.SendCnt + 1
            strItems = strItems & "DSP|" & CStr(28 + mOrder.SendCnt) & "||" & Trim(AdoRs_Local.Fields("SENDCHANNEL").Value) & "^^^|||" & vbCr
            AdoRs_Local.MoveNext
        Loop
    End If
    ...
    GetEquipExamCode_BS200E = strItems
...
Function SaveTransData_DASOM(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
...
'jec_regno          integer             PK  - 접수일련번호
'jec_reg_date       char        8       접수일자
'jec_ch_date        char        8       PK  - 처방일자
'jec_ptno           varchar     15      차트번호
'jec_medroom        varchar     3       진료실번호
'jec_kwa            varchar     3       진료과
'jec_act_date       varchar     8       검사일자
'jec_ord_seq        smallint            PK  - 처방오더 일련번호
'jec_seq            smallint            PK  - 처방일련번호
'jec_ord_date       varchar     8       오더내린일자
'jec_gcode          varchar     10      검사 대분류코드
'jec_kcode          Varchar     10      검사 종류코드
'jec_scode          varchar     10      자체코드
'jec_edi_code       varchar     10      청구코드
'jec_name           varchar     100     명칭
'jec_value          varchar     20      결과치
'jec_dnvalue        varchar     20      하위값
'jec_upvalue        varchar     20      상위값
'jec_sutakflag      varchar     10      수탁검사 기관기호
'jec_routine        varchar     1       루틴구분 S:하위검사, O:부
'jec_gumche         varchar     25      검체
'jec_unit           varchar     14      단위
'jec_prn_flag       Varchar     1       출력여부
'jec_exam_uid       Varchar     10      사용자 id
'jec_detail         Varchar     2       지원처방 상세내역 H:헤더정보,T:진료지원Title
    
                
                
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    '-- 결과저장
                          SQL = "Update dba.examch                                  " & vbCrLf
                    SQL = SQL & "   Set jec_value       = '" & sResult & "'     " & vbCrLf
                    'SQL = SQL & "     , jec_exam_uid    = '" & gHOSP.USERID & "'" & vbCrLf
                    SQL = SQL & " Where jec_regno       = '" & strBarcode & "'  " & vbCrLf
                    SQL = SQL & "   AND jec_ch_date     = '" & strHospDate & "' " & vbCrLf
                    SQL = SQL & "   AND jec_scode       = '" & strTestCd & "' " & vbCrLf
                    'SQL = SQL & "   And jec_ord_seq     = '" & strPatID & "'    " & vbCrLf
                    'SQL = SQL & "   And jec_seq         = '" & strOrdCd & "'    " & vbCrLf
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_DASOM" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "DASOM"                        '다솜
                Call GetWorkList_DASOM(pFrom, pTo, SPD)
...
Public Sub GetWorkList_DASOM(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
'
'1.1 waitexam (검사대기자 인적정보)
'Field  Name        Type        Length  Detail
'jew_regno          integer             PK  - 접수일련번호
'jew_kind           char        2       청구구분
'jew_reg_date       char        8       접수일자
'jew_ch_date        char        8       PK  - 처방일자
'jew_ptno           varchar     15      차트번호
'jew_medroom        varchar     3       진료실번호
'jew_kwa            varchar     3       진료과
'jew_ord_seq        smallint            PK  - 처방오더 일련번호
'jew_ord_date       varchar     8       오더내린일자
'jew_act_date       varchar     8       검사일자
'jew_name           varchar     40      이름
'jew_resno          varchar     13      주민번호
'jew_yage           smallint            나이
'jew_mage           smallint            나이
'jew_sex            char        1       성별(M,F)
'jew_jo_gubun       char        2       종별구분
'jew_byungdong      varchar     4       병실구분
'jew_room           varchar     4       병실번호
'jew_reg_time       time                접수시간
'jew_exam_time      varchar     8       완료시간
'jew_ord_uid        varchar     10      사용자 id
'jew_message        varchar     200     진료실전달메세지
'jew_waitgu         char        1       진행구분  0:대기, 1:접수, 2:검사중, 8:환원, 9:검사(판독)완료
'jew_return_time    varchar     16      환원처리 시간(YYYY-MM-DD HH:MM)
'jew_iwgu           varchar     1       외:1,입:2
'jew_gumjin         varchar     1       Y:아름누리
'jew_exmnum         varchar     10      아름누리검진Key번호
...
'jec_detail         Varchar     2       지원처방 상세내역 H:헤더정보,T:진료지원Title
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       a.jec_ch_date           AS HOSPDATE " & vbCrLf
    SQL = SQL & "     , a.jec_regno             AS BARCODE  " & vbCrLf
    SQL = SQL & "     , a.jec_ptno              AS CHARTNO  " & vbCrLf
    SQL = SQL & "     , a.jec_ord_seq           AS PID      " & vbCrLf
    'SQL = SQL & "     , a.jec_seq"
    SQL = SQL & "     , b.jew_name              AS PNAME    " & vbCrLf
    SQL = SQL & "     , COUNT(a.jec_scode)      AS CNT      " & vbCrLf
    SQL = SQL & "   FROM dba.examch a, dba.waitexam b               " & vbCrLf
    SQL = SQL & " WHERE a.jec_ch_date BETWEEN '" & pFrom & "' and '" & pTo & "'" & vbCrLf '접수일자
    SQL = SQL & "   AND a.jec_regno     = b.jew_regno       " & vbCrLf  'PK  - 접수일련번호
    SQL = SQL & "   AND a.jec_ch_date   = b.jew_ch_date     " & vbCrLf  'PK  - 처방일자
    SQL = SQL & "   AND a.jec_ord_seq   = b.jew_ord_seq     " & vbCrLf  'PK  - 처방오더 일련번호
    SQL = SQL & "   AND a.jec_ptno      = B.jew_ptno        " & vbCrLf
    SQL = SQL & "   AND a.jec_scode     IN (" & gAllTestCd & ")     " & vbCrLf
'    SQL = SQL & "   AND a.jec_routine   = 'S'                       " & vbCrLf
    SQL = SQL & "   AND (a.jec_value = '' OR a.jec_value IS NULL)   " & vbCrLf
    SQL = SQL & " GROUP BY a.jec_ch_date, a.jec_regno, a.jec_ptno, a.jec_ord_seq, b.jew_name " & vbCrLf
    SQL = SQL & " ORDER BY a.jec_ch_date, a.jec_regno "
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_DASOM" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As Object) As String
    ...
    Select Case gEMR
        Case "DASOM"
            SQL = ""
            SQL = SQL & "SELECT jec_scode as ITEM                       " & vbCrLf
            SQL = SQL & "  FROM dba.examch                              " & vbCrLf
            SQL = SQL & " WHERE jec_regno     = '" & strBarcode & "'    " & vbCrLf  'PK  - 접수일련번호
            SQL = SQL & "   AND jec_ch_date   = '" & strRegDate & "'    " & vbCrLf  'PK  - 처방일자
            SQL = SQL & "   AND jec_ord_seq   = '" & strPatID & "'      " & vbCrLf  'PK  - 처방오더 일련번호
            SQL = SQL & "   AND jec_ptno      = '" & strChartNo & "'    " & vbCrLf
            SQL = SQL & "   AND (jec_value = '' OR jec_value IS NULL)   " & vbCrLf
            SQL = SQL & "   AND jec_scode in (" & gAllTestCd & ")       " & vbCrLf
            SQL = SQL & "  ORDER BY jec_scode                           " & vbCrLf
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "DASOM"
            SaveTransData = SaveTransData_DASOM(argSpcRow, SPD)
...
Public Sub SetPatInfo(ByVal pBarno As String, ByVal pType As String)
    ...
    intRow = -1
    With frmMain
        If UCase(gHOSP.MACHNM) = "BS200" Then
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    If cn_Server_Flag = True Then
        Select Case gEMR
            Case "DASOM"
                    Call GetSampleInfo_DASOM(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_DASOM(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       a.jec_ch_date            " & vbCrLf
    SQL = SQL & "     , a.jec_regno               " & vbCrLf
    SQL = SQL & "     , a.jec_ptno                " & vbCrLf
    SQL = SQL & "     , a.jec_ord_seq                " & vbCrLf
    SQL = SQL & "     , a.jec_seq                 " & vbCrLf
    SQL = SQL & "     , b.jew_name                  " & vbCrLf
    SQL = SQL & "     , a.jec_scode                  " & vbCrLf
    SQL = SQL & "   FROM dba.examch a, dba.waitexam b               " & vbCrLf
    SQL = SQL & " WHERE a.jec_regno     = b.jew_regno       " & vbCrLf  'PK  - 접수일련번호
    SQL = SQL & "   AND a.jec_ch_date   = b.jew_ch_date     " & vbCrLf  'PK  - 처방일자
    SQL = SQL & "   AND a.jec_ord_seq   = b.jew_ord_seq     " & vbCrLf  'PK  - 처방오더 일련번호
    SQL = SQL & "   AND a.jec_ptno      = B.jew_ptno        " & vbCrLf
    SQL = SQL & "   AND a.jec_ch_date   =  '" & strRegDate & "'     " & vbCrLf
    SQL = SQL & "   AND a.jec_regno     =  '" & strBarcode & "'     " & vbCrLf
    SQL = SQL & "   AND a.jec_ptno      =  '" & strChartNo & "'     " & vbCrLf
    SQL = SQL & "   AND a.jec_ord_seq   =  '" & strPatID & "'       " & vbCrLf
    SQL = SQL & "   AND a.jec_scode     IN (" & gAllTestCd & ")     " & vbCrLf
'    SQL = SQL & "   AND a.jec_routine   = 'S'                       " & vbCrLf
    SQL = SQL & "   AND (a.jec_value = '' OR a.jec_value IS NULL)   " & vbCrLf
    SQL = SQL & " ORDER BY a.jec_scode          "
...
'    Set RS = New ADODB.Recordset
    ...
    Call SetSQLData("바코드조회", RS.RecordCount & ":" & SQL, "A")
...
'                MsgBox RS.Fields("jec_ch_date")
'                MsgBox RS.Fields("jec_regno")
'                MsgBox RS.Fields("jec_ord_seq")
'                MsgBox RS.Fields("jec_ptno")
'                MsgBox RS.Fields("jec_scode")
                
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, RS.Fields("jec_ch_date"), asRow, colHOSPDATE
                SetText SPD, RS.Fields("jec_regno"), asRow, colBARCODE
                SetText SPD, RS.Fields("jec_ord_seq"), asRow, colPID
                SetText SPD, RS.Fields("jec_ptno"), asRow, colCHARTNO
                SetText SPD, RS.Fields("jew_name"), asRow, colPNAME
...
'                오더정보에 저장
                With mOrder
'                    .BarNo = Trim(RS.Fields("jec_regno")) & ""
'                    .PID = Trim(RS.Fields("jec_ptno")) & ""
'                    .PNAME = Trim(RS.Fields("jew_name")) & ""
                    .Count = CStr(intTestCnt)
                    .NoOrder = False
                End With
                ...
                '-- 화면에 표시
                For intCol = colSTATE + 1 To .MaxCols
                    If RS.Fields("jec_scode") = gArrEQP(intCol - colSTATE, 2) Then
                        .Row = asRow
                        .Col = intCol
                        '.BackColor = vbYellow
                        'Call SetText(SPD, "◇", asRow, intCol)
...
'                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("ORDCODE")) & ""
                ...
                gPatOrdCd = gPatOrdCd & "'" & RS.Fields("jec_scode") & "',"
...
'MsgBox "2"
    ...
    GetSampleInfo_DASOM = 1
    
    Call SetSQLData("바코드조회", gPatOrdCd, "A")
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_DASOM1" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
