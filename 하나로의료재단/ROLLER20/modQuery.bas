...
On Error GoTo RST
    ...
    With frmQCMaster.spdHeader
        For i = 1 To .MaxRows
            ...
            End If
            SQL = ""
            SQL = SQL & "Insert Into QCHEADER (LabID,LotID,MachID,InstrumentID) " & vbCr
            SQL = SQL & " Values (" & vbCr
            ...
            SQL = SQL & "'" & GetText(frmQCMaster.spdHeader, i, 4) & "'" & vbCr
            SQL = SQL & ") " & vbCr
...
On Error GoTo RST
    ...
    SQL = ""
    SQL = SQL & "Select LabID,LotID,MachID,InstrumentID "
    SQL = SQL & "  From QCHEADER " & vbCr
    SQL = SQL & " Order By MachID "
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        Do Until RS.EOF
            Call SetText(frmQCMaster.spdHeader, Trim(RS.Fields("LabID")) & "", i, 1)
            Call SetText(frmQCMaster.spdHeader, Trim(RS.Fields("LotID")) & "", i, 2)
            Call SetText(frmQCMaster.spdHeader, Trim(RS.Fields("MachID")) & "", i, 3)
            Call SetText(frmQCMaster.spdHeader, Trim(RS.Fields("InstrumentID")) & "", i, 4)
            i = i + 1
            RS.MoveNext
        Loop
    End If
...
On Error GoTo RST
    ...
    SQL = ""
    SQL = SQL & "SELECT Distinct h.LabID, h.LotID, h.MachID,d.QCLevel " & vbCr
    SQL = SQL & "  FROM QCHEADER h,QCDETAIL d" & vbCr
    SQL = SQL & " WHERE d.ID = '" & pQCChannel & "'" & vbCr
    SQL = SQL & "   AND h.InstrumentID = d.InstrumentID "
    '-- Record Count 가져옴
    ...
    ' 두개 이상의 같은 검사가 있을 경우 나중에 등록한 것을 가지고 온다.
'    SQL = ""
'    SQL = SQL & "SELECT Distinct b.AnalyteID, a.lablottestid,c.name,b.MethodID,b.ReagentID, b.UnitID, b.TemperatureID " & vbCr
'    SQL = SQL & "  FROM LabLotTest a, test b, analyte c" & vbCr
''    SQL = SQL & " WHERE a.Labid = '" & pEqpCD & "'" & vbCr
'    SQL = SQL & " WHERE a.Labid = '" & strLabID & "'" & vbCr
'    SQL = SQL & "   AND a.Lotid = '" & strLotID & "'" & vbCr
'    SQL = SQL & "   AND b.InstrumentID = '" & strMachID & "'" & vbCr
'    SQL = SQL & "   AND b.AnalyteID = '" & pAnalyID & "'" & vbCr
'    SQL = SQL & "   AND a.testid = b.testid " & vbCr
...
'    SQL = SQL & " ORDER BY a.lablottestid"
    ...
    SQL = ""
    SQL = SQL & "SELECT Distinct top 1 b.entereddate,b.AnalyteID, a.lablottestid,c.name,b.MethodID,b.ReagentID, b.UnitID, b.TemperatureID " & vbCr
    SQL = SQL & "  FROM LabLotTest a, test b, analyte c" & vbCr
'    SQL = SQL & " WHERE a.Labid = '" & pEqpCD & "'" & vbCr
    SQL = SQL & " WHERE a.Labid = '" & strLabID & "'" & vbCr
    SQL = SQL & "   AND a.Lotid = '" & strLotID & "'" & vbCr
    ...
    SQL = SQL & "   AND a.testid = b.testid " & vbCr
    SQL = SQL & "   AND b.AnalyteID = c.AnalyteID " & vbCr
    SQL = SQL & " ORDER BY b.entereddate desc,a.lablottestid"
    ...
    If intCnt > 0 Then
        For i = 0 To UBound(strQCTemp)
            ...
            If strQCTemp(i) <> "" Then
                ...
                strQCVal = strQCVal & strLabID & "|"          ' lab           // 447834(병원코드로 대체 가능?)
...
On Error GoTo DBErr
    ...
    Select Case gOCS
        Case "HANARO"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT "
            SQL = SQL & "       SPRDT   AS HOSPDATE             " & vbCrLf
            ...
            SQL = SQL & "     , CODE1   AS ITEM                 " & vbCrLf
            SQL = SQL & "     , CODE2   AS CODE2                " & vbCrLf
            SQL = SQL & "  From GOLIATH.SZDAT01T                " & vbCrLf
            SQL = SQL & " WHERE COMP    = '" & gHOSP.HOSPCD & "'" & vbCrLf  'K
            SQL = SQL & "   AND WORKCD  = '" & gHOSP.MACHCD & "'" & vbCrLf  'ZS1, ZC1
            SQL = SQL & "   AND ACPTNO  = '" & strBarcode & "'  " & vbCrLf
            SQL = SQL & "   AND CODE1  IN (" & gAllTestCd & ")  " & vbCrLf
            SQL = SQL & "   AND STATUS  = '1'                   " & vbCrLf '1:준비, 3:완료
            SQL = SQL & " ORDER BY CODE1                        "
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        mResult.SEX = Trim(RS.Fields("SEX")) & ""
                        ...
                        For intCol = colSTATE + 1 To .MaxCols
                            If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                                ...
                                '-- CODE2
                                gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("CODE2")) & ""
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_SCL(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
...
On Error GoTo DBErr
    ...
    AdoCmd_SCL.CommandTimeout = 15
    AdoCmd_SCL.CommandText = "SLRTRM51P"
    AdoCmd_SCL.CommandType = adCmdStoredProc
    
    Set Prm1 = AdoCmd_SCL.CreateParameter("pbarc", adChar, adParamInput, 12, strBarcode)
    AdoCmd_SCL.Parameters.Append Prm1
    
    Set Prm2 = AdoCmd_SCL.CreateParameter("pmach", adChar, adParamInput, 3, gHOSP.SCLMACHCD)
    AdoCmd_SCL.Parameters.Append Prm2
        
    Set Prm3 = AdoCmd_SCL.CreateParameter("perr", adChar, adParamOutput, 1, "")
    AdoCmd_SCL.Parameters.Append Prm3

    
    Set RS = New ADODB.Recordset
    RS.Open AdoCmd_SCL.Execute
...
'                    .Count = CStr(intTestCnt)
                ...
                gPatOrdCd = gPatOrdCd & "'" & Trim(RS.Fields("ITEMCODE")) & "',"
            End With
            DoEvents
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_SCL" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Public Function CalProcess(ByVal SPDORD As Object, ByVal SPDRST As Object, ByVal pTestCd As String, Optional ByVal pTV As String)
    ...
    Select Case pTestCd
        ...
        ' 2017-12-21 추가
        Case "C3730-2" 'Crea
                strCalTestCd = "C3730-2"   'Crea(24시간뇨)
        ...
        'Case "C3750N3"    'CREA(단회)
        '        strCalTestCd = "PRCR"      'PRCR '코드찾아야함
                
        'Case "C2200-1"    'TP(단회뇨)
        '        strCalTestCd = "PRCR"      'PRCR
...
'        Case "C3750N3"    'CREA(단회) 이면  TP(단회뇨)결과를 가져온다.
'            SQL = "SELECT [dbo].FUN_H7LIS_PRE_RESULT4('" & strPTID & "', '" & Format(Now, "yyyymmdd") & "', '" & Format(Now, "hhmm") & "', 'C3750N3')"
'            AdoCn.CursorLocation = adUseClient
'            Set RS = AdoCn.Execute(SQL, , 1)
'            If Not RS.EOF = True And Not RS.BOF = True Then
