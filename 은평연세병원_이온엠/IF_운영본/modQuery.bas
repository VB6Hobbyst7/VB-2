...
On Error GoTo RST
    ...
    Select Case gOCS
        Case "EONM"
            ...
            SQL = SQL & "   AND O.H141_ODRDAT BETWEEN '" & pFrom & "' AND '" & pTo & "'" & vbCr
...
'                            SetText frmWorkList.spdWork, Trim(RS.Fields("SEX")) & "", intRow, colPSEX
                            ...
                            SetText frmWorkList.spdWork, GetSampleITEM(intRow, SPD), intRow, colITEMS
        ...
        Case "BIGUBCARE"
            ...
            'SQL = SQL & ", i.IntLabNum AS PID "               ' 검사번호"
...
'                            SetText frmWorkList.spdWork, Trim(RS.Fields("SEX")) & "", intRow, colPSEX
        ...
        Case "NAVY"
            ...
            SQL = SQL & "   And (RSLTTEXT = '' or RSLTTEXT is null) " & vbCr    ' 결과확정유무
            'SQL = SQL & "   And PROCSTAT = 'N' " & vbCr   '-- 결과확정유무
...
'
'                            SetText SPD, "1", intRow, colCHECKBOX
'                            SetText SPD, "20180604", intRow, colHOSPDATE
'                            SetText SPD, "180001234" & intRow, intRow, colBARCODE
'                            SetText SPD, "1000" & intRow, intRow, colCHARTNO
'                            SetText SPD, "1234" & intRow, intRow, colPID
'                            SetText SPD, "홍길동" & intRow, intRow, colPNAME
'                            SetText SPD, "M", intRow, colPSEX
'                            SetText SPD, "35", intRow, colPAGE
'                            SetText SPD, frmMain.txtSeq.Text, intRow, colSEQNO
'                            'SetText SPD, GetSampleITEM(intRow, SPD), intRow, colITEMS
'                            SetText SPD, "NA/K/CL/", intRow, colITEMS
'
'                            frmWorkList.txtSeq.Text = frmWorkList.txtSeq.Text + 1
'
'                        Next
...
'                        Exit Sub
        ...
        Case "BIT"
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                ...
                Loop
            Else
                frmMain.lblCommStatus.Caption = ">> 조회 대상자가 없습니다."
            End If
...
RST:
                ...
                strErrMsg = "위    치 : " & gHOSP.MACHNM & "_GetWorkList_Main" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show 'vbModal
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    ...
    Select Case gOCS
        Case "EONM"
            ...
            SQL = SQL & "   AND O.H141_NOTYYN = 'N'                      " & vbCr
            SQL = SQL & "   And O.H141_SUGACD in (" & gAllTestCd & ")" & vbCr
            SQL = SQL & " ORDER BY O.H141_SUGACD "
...
On Error GoTo DBErr
    ...
    Select Case gOCS
        Case "EONM"
            SQL = ""
            SQL = SQL & "SELECT O.H141_ODRDAT       AS HOSPDATE " & vbCr
            SQL = SQL & "      ,O.H141_TSAMPLENO    AS BARCODE  " & vbCr
            SQL = SQL & "      ,O.H141_SEQNO        AS PID      " & vbCr
            SQL = SQL & "      ,P.A110_CHARTNO      AS CHARTNO  " & vbCr
            SQL = SQL & "      ,P.A110_PATNM        AS PNAME    " & vbCr
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        gPatOrdSeq = gPatOrdSeq & Trim(RS.Fields("ITEM")) & ":" & Trim(RS.Fields("PID")) & "/"
                    ...
                    With SPD
                        ...
                        For intCol = colSTATE + 1 To .MaxCols
                            If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                                ...
                                '-- 결과저장용 SEQ
                                gArrEQP(intCol - colSTATE, 7) = Trim(RS.Fields("SUBITEM")) & ""   '결과저장용 번호's
