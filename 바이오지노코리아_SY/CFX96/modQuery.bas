...
'''                    SQL = SQL & " '" & strHospDate & "'"    '조회일 : yyyy-mm-dd
'''                    SQL = SQL & "," & intRegNo              '접수번호 int
'''                    SQL = SQL & ",'" & strOrdCd & "'"       '오더코드 or 공백
'''                    SQL = SQL & ",'" & strTestCd & "'"      '테스트코드 or 공백
'''                    SQL = SQL & ",'" & strTestCdSub & "'"   '테스트서브코드
'''                    SQL = SQL & ",'" & sResult & "'"        '결과 : 문자,숫자
'''                    SQL = SQL & ",''"                       '결과 : 문자
'''                    SQL = SQL & ",'" & strJudge & "'"       '결과 : 판정
'''                    SQL = SQL & ",0"                        '패닉
'''                    SQL = SQL & ",0"                        '델타
'''                    SQL = SQL & ",0"                        '크리티컬
'''                    SQL = SQL & ",'" & gHOSP.MACHCD & "'"
'''                    SQL = SQL & ",'" & strReturn & "'"      '0:실패,1:성공
                    
                                            
                    '-- 2020-09-01 수정
'                    Interface_SetPatientResult03 - 장비의 검사결과를 해당수진자의 검사항목에 저장하는 기능.
'                    색상값을 지정할 수 있고 나머지는 D와 같다.
'                        파라메터설정
'                    @LabRegDate     smalldatetime,  -- 필수입력: 조회일
'                    @LabRegNo       int,        -- 필수입력: 접수번호
'                    @OrderCode      varchar(20),    -- 필수입력: 오더코드or 공백
'                    @TestCode       varchar(20),    -- 필수입력: 테스트코드 or 공백
'                    @TestSubCode        varchar(20),    -- 필수입력: 테스트서브코드
'                    @TestResult01       varchar(50),    -- 결과값1 - 문자,숫자
'                    @TestResult02       varchar(50),    -- 결과값2 - 문자
'                    @TestResultAbn      varchar(50),    -- 결과값3 - 판정(문자)
'                    @IsPanic        bit,        -- 패닉
'                    @IsDelta        bit,        -- 델타
'                    @IsCritical     bit,        -- 크리티컬
'                    @Result         bit             -- 결과값: 0-실패, 1-성공
'                    @ResultFontColor    int -- 폰트색(DECIMAL 값)
'                                        -- 검정 : 0 흰색 : 16777215 빨강 : 16711680
                    
                    SQL = ""
                    SQL = SQL & "Exec Interface_SetPatientResult03 "
                    SQL = SQL & " '" & strHospDate & "'"    '조회일 : yyyy-mm-dd
                    SQL = SQL & "," & intRegNo              '접수번호 int
                    SQL = SQL & ",'" & strOrdCd & "'"       '오더코드 or 공백
                    SQL = SQL & ",'" & strTestCd & "'"      '테스트코드 or 공백
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_SY" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pFromNo As String = "", Optional ByVal pToNo As String = "", Optional ByVal pSave As Boolean = False)
    ...
    Select Case gEMR
        Case "SY"                           'SY
                Call GetWorkList_SY(pFrom, pTo, SPD, pFromNo, pToNo)
...
Public Sub GetWorkList_SY(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread, Optional ByVal pFromNo As String = "", Optional ByVal pToNo As String = "")
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    strTestCds = Replace(strTestCds, "'XXXXX'", "")
    strTestCds = Replace(strTestCds, "'YYYYY'", "")
    strTestCds = Replace(strTestCds, "'ZZZZZ'", "")
    strTestCds = Replace(strTestCds, ",,,", "")
    strTestCds = Replace(strTestCds, "'", "''")
    
    SQL = ""
    SQL = SQL & "Exec interface_GetPatientResultList02 "
    SQL = SQL & "'" & frmInterface.cboChk.ListIndex & "','" & gHOSP.PARTCD & "','" & pFrom & "','" & pTo & "'," & pFromNo & "," & pToNo & ",'" & strTestCds & "'"
        ...
        Do Until RS.EOF
            With SPD
                For i = 1 To SPD.DataRowCnt
                    ...
                    If Trim(RS("LabRegDate")) = strHospDate And Format(Trim(RS.Fields("LabRegDate")), "yymmdd") & PedLeftStr(Trim(RS.Fields("LabRegNo")), 5, "0") = strPID Then
                        blnSame = True
                        strItems = strItems & GetTestNm(Trim(RS.Fields("TestCode"))) & "/"
                        iCnt = iCnt + 1
                    End If
                Next
                ...
                If blnSame = False Then
                    ...
                    'SetText SPD, Trim(RS.Fields("PatientBirthDay")) & "", intRow, colPJUMIN
...
'    Call SetCommStatus("R", pBarNo, frmInterface.lstComStatus)
    ...
    With frmInterface
        Select Case pType
            ...
            Case "1"
                For i = 1 To .spdOrder.DataRowCnt
                    If GetText(frmInterface.spdOrder, i, colSEQNO) = mResult.Seq Then
                        intRow = i
                        Exit For
                    End If
                Next i
...
On Error GoTo DBErr
    ...
    SQL = ""
    SQL = SQL & "Exec Interface_GetPatientResult02 '" & gHOSP.PARTCD & "','" & strRegDate & "','" & strRegno & "'"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                SetText SPD, Format(Trim(RS.Fields("LabRegDate")), "yymmdd") & PedLeftStr(Trim(RS.Fields("LabRegNo")), 5, "0"), asRow, colBARCODE
                ...
                '-- 화면에 표시
                For intCol = colSTATE + 1 To .MaxCols
                    If GetTestNm(Trim(RS.Fields("TestCode")) & "", False) = gArrEQP(intCol - colSTATE, 6) Then
'                        SQL = ""
'                        SQL = SQL & " SELECT OrderCode, TestCode, TestSubCode   " & vbCrLf
'                        SQL = SQL & "   FROM LC13_DIOGENE..LabRegResult         " & vbCrLf
'                        SQL = SQL & "  WHERE LABREGDATE = '" & strRegDate & "'  " & vbCrLf
'                        SQL = SQL & "    AND LABREGNO   = " & strRegno & vbCrLf
'                        SQL = SQL & "    AND ORDERCODE  = '" & Trim(RS.Fields("OrderCode")) & "'"
'
'                        Set RS_T = AdoCn.Execute(SQL, , 1)
'                        If Not RS_T.EOF = True And Not RS_T.BOF = True Then
'                            gArrEQP(intCol - colSTATE, 16) = Trim(RS_T.Fields("TestCode")) & ""
'                            gArrEQP(intCol - colSTATE, 17) = Trim(RS_T.Fields("TestSubCode")) & ""
'                        End If
'                        RS_T.Close
                        ...
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("OrderCode")) & ""
                        gArrEQP(intCol - colSTATE, 17) = Trim(RS.Fields("TestSubCode")) & ""
                        
                        Exit For
                    End If
                Next
                
                gPatOrdCd = gPatOrdCd & "'" & Trim(RS.Fields("TestCode")) & "',"
            End With
            DoEvents
