   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboChk 
            ...
            EndProperty
            Height          =   300
            ItemData        =   "frmInterface.frx":C214
            Left            =   2340
            List            =   "frmInterface.frx":C21E
            TabIndex        =   45
            Top             =   -90
            Visible         =   0   'False
            Width           =   765
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C22E
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":CD34
         End
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String)
    ...
    '-- 검사대상자 가져오기
          ...
          SQL = "SELECT DISTINCT GUMSA013.G13_GMDATE, GUMSA013.G13_SAMPLE, GUMSA013.G13_CHTNO, GUMSA013.G13_SJJNAM " & vbCr
    ...
    '-- 보고여부(부분통보B,전체통보Y)
    'SQL = SQL & "   AND GUMSA013.G13_BOGOYN LIKE 'Y' " & vbCr
    '-- 진료과목(종합검진 = 90)
    SQL = SQL & "   AND SUBSTR(GUMSA013.G13_JINKWA,1,1) <> '9' " & vbCr
    
    ',G13_JINKUL    VARCHAR(    1)   /*  -:접수전,  1:검체접수, 2:검체수거,6:전체통보, 5:부분통보 */
    SQL = SQL & "   AND G13_JINKUL <= '5' " & vbCr
    
    '-- 검사항목 코드(WORKNM)
    'SQL = SQL & "   AND GUMSA015.G15_WORKNM"
    'SQL = SQL & " GROUP BY GUMSA013.G13_GMDATE, GUMSA013.G13_SAMPLE GUMSA013.G13_CHTNO, GUMSA013.G13_SJJNAM "
    ...
    Do Until RS.EOF
        ...
        SetText vasWorkList, Trim(RS.Fields("G13_GMDATE")), intRow, colOrdDate
        SetText vasWorkList, Trim(RS.Fields("G13_SAMPLE")) & "", intRow, colBarcode
        SetText vasWorkList, Trim(RS.Fields("G13_CHTNO")), intRow, colPID
        SetText vasWorkList, Trim(RS.Fields("G13_SJJNAM")), intRow, colPName
...
'        1H|\^&|||D10^07^3.00|||||||||20040223090354
'F5
'2P|1
'06
'3O|1|123456|123456-02-032-20031209-07|^^^4|||||||||||||||||||||||F
'06
'4R|1|^^^Unknown^AREA|0.9|||||||||20031209132023
'06
'5R|2|^^^Unknown^TIME|0.15|||||||||20031209132023
'06
'6R|3|^^^A1b^AREA|3.5|||||||||20031209132023
'06
'7R|4|^^^A1b^TIME|0.28|||||||||20031209132023
'06
'0R|5|^^^F^AREA|0.9|||||||||20031209132023
'06
'1R|6|^^^F^TIME|0.55|||||||||20031209132023
'06
'2R|7|^^^A1c^AREA|4.6|||||||||20031209132023
'06
'3R|8|^^^A1c^TIME|0.82|||||||||20031209132023
'06
'4R|9|^^^A0^AREA|88.8|||||||||20031209132023
'06
'5R|10|^^^A0^TIME|1.44|||||||||20031209132023
'06
'6R|11|^^^TOTAL^AREA|3335057|||||||||20031209132023
'06
'7L|1|N
'06
'
...
'        lblStatus = "작업 대기중.."
    ...
    If Not Connect_Local Then
        MsgBox "연결되지 않았습니다.", vbCritical, Me.Caption
        cn_Local_Flag = False
        Exit Sub
    Else
        cn_Local_Flag = True
...
Rst:
    If Err.Number = "8002" Then
        strPortList = getPortList
        If (MsgBox("포트 번호 COM" & gSetup.gPort & " 가 잘못되었습니다." & vbNewLine & vbNewLine & "계속 진행하시겠습니까?", vbYesNo + vbCritical, Me.Caption)) = vbYes Then
            MsgBox "사용가능 포트는 " & vbNewLine & strPortList & vbNewLine & "입니다.", vbInformation, Me.Caption
            Resume Next
...
Private Function getPortList()
    ...
    For Each objItem In colItems
        'If UCase(objItem.Description) Like "*USB*" Then
        'If UCase(objItem.Description) Like "*COM*" Then
        If UCase(objItem.DeviceID) Like "*COM*" Then
            getPortList = getPortList & objItem.DeviceID & vbNewLine
            ...
            Case 18 To 49 '## Result
                ...
                If strResult <> "" Then
                    ...
                    If Res > 0 Then
                        ...
                        '판정
                        If strResult < strLow Then
                            strFlag = "L"
                        End If
                        
                        If strResult > strHigh Then
                            strFlag = "H"
                        End If
                        ...
                        SetText vasRes, strFlag, lsResRow, colFLAG              'Flag
...
''''
''''                SetText vasWorkList, "2", 1, colSeqNo
''''                SetText vasWorkList, "20140616", 1, colOrdDate
''''                SetText vasWorkList, "12345", 1, colBarcode
'''
'''                For ii = 1 To vasWorkList.DataRowCnt
'''                    If Trim(GetText(vasWorkList, ii, colSeqNo)) = strSeq Then
...
Private Sub EditRcvDataASTM()
   ...
   ' On Error Resume Next
    ...
    strType = Trim(Mid$(strBuffer, 2, 1))
    
    Select Case strType
        Case "H"
        Case "P"
        Case "O"
            strBarno = mGetP(strBuffer, 3, "|")
            strSeq = mGetP(strBuffer, 2, "|")
...
'                strSeq = Val(Trim(strSeq))
        ...
        Case "R"
            Erase tmpField()
            strTemp1 = mGetP(strBuffer, 3, "|")
            strIntBase = mGetP(strTemp1, 4, "^") & "^" & mGetP(strTemp1, 5, "^")
            'If (tmpData(3) = "A1c") And (tmpData(4) = "AREA") Then 'A1c(Area)
            '    strIntBase = "1"
                
                strResult = mGetP(strBuffer, 4, "|")
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    SQL = SQL & "EXAMDATE,EQUIPNO,BARCODE,DISKNO,POSNO," & vbCrLf & _
                ...
                "EQUIPRESULT,RESULT,EXAMNAME,SENDFLAG,REFVALUE, REFFLAG,EXAMUID) " & vbCrLf
    SQL = SQL & "VALUES("
