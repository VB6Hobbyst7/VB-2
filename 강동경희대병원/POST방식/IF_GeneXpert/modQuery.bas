...
'        End If
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            Do Until RS_L.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    strAllResult = strAllResult & strOrdCd & "%17"        'IF코드
                    strAllResult = strAllResult & sResult & "%17"         '결과값
                    strAllResult = strAllResult & sComment & "%17"        '코멘트
                    strAllResult = strAllResult & strDate & "%03"         '검사일시
                End If
                RS_L.MoveNext
            Loop
        End If
        ...
        If strAllResult <> "" Then
            sParam = ""
            sParam = sParam & "&Data1=" & strBarcode & "%03" & strAllResult
            ...
            If UCase(strOrdCd) = "ERCOVID" Then
            ...
            ElseIf UCase(strOrdCd) = "MTB" Or UCase(strOrdCd) = "RIF" Then
                gCFXCmnt.MTBRIF = Replace(gCFXCmnt.MTBRIF, "CHR(10)CHR(13)", "%0D")
                sParam = sParam & "&Data3=" & gCFXCmnt.MTBRIF      '의사소견 내용
            'ElseIf frmMain.optTest(2).Value = True Then
            ElseIf UCase(strOrdCd) = "CARBA" Then
                gCFXCmnt.CARBAR = Replace(gCFXCmnt.CARBAR, "CHR(10)CHR(13)", "%0D")
                sParam = sParam & "&Data3=" & gCFXCmnt.CARBAR       '의사소견 내용
...
'    Kill App.PATH & "\Xml\" & strXmlName
    ...
    If UBound(varRcvData) >= 1 Then
        For i = 1 To UBound(varRcvData)
            ...
            ElseIf frmMain.optTest(1).Value = True Then
                'TB
                If InStr(strGSNO, "MTB") > 0 Then
                    blnAdd = True
                End If
            ElseIf frmMain.optTest(2).Value = True Then
                'CARBA-R
                If InStr(strGSNO, "carba") > 0 Then
                    blnAdd = True
                End If
            End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object, Optional ByVal pEdit As Boolean = False) As Integer
    ...
    'If cn_Server_Flag = True Then
        Select Case gEMR
            Case "KHNMC"
                    Call GetSampleInfo_KHNMC(asRow, SPD, pEdit)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_KHNMC(ByVal asRow As Long, ByVal SPD As vaSpread, Optional ByVal pEdit As Boolean = False) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'sRcvData = ""
'sRcvData = sRcvData & "<?xml version=""1.0"" encoding=""euc-kr"" ?>" & vbCrLf
'sRcvData = sRcvData & "<root>" & vbCrLf
'sRcvData = sRcvData & "</root>" & vbCrLf
'sRcvData = sRcvData & "            <IFordcd>" & vbCrLf
'sRcvData = sRcvData & "<ifordcd>LBG2008180010▦20200924▦08686896▦서유리B▦M▦62▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Carba....유전자PCR▦L91037▦155740"
...
'    Call SetSQLData("바코드조회", "UBound(varRcvData) =" & UBound(varRcvData), "A")
...
Dim strTmpData As String
                ...
                strTmpData = varRcvData(i)
                strTmpData = Replace(strTmpData, vbCr, "")
                varRcvDtl = Split(strTmpData, vbLf)
            
'    Call SetSQLData("바코드조회", "varRcvData(i) =" & varRcvData(i), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl  =" & UBound(varRcvDtl), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl(0)  =" & varRcvDtl(0), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl(1)  =" & varRcvDtl(1), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl(2)  =" & varRcvDtl(2), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl(3)  =" & varRcvDtl(3), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl(4)  =" & varRcvDtl(4), "A")
'    Call SetSQLData("바코드조회", "varRcvDtl(5)  =" & varRcvDtl(5), "A")
                
                With SPD
                    .ReDraw = False
                    SetText SPD, "1", asRow, colCHECKBOX
                    SetText SPD, mGetP(varRcvDtl(1), 2, "▦"), asRow, colHOSPDATE
                    'SetText SPD, mGetP(varRcvDtl(1), 9, "▦"), asRow, colBARCODE
                    SetText SPD, mGetP(varRcvDtl(1), 3, "▦"), asRow, colPID
                    SetText SPD, mGetP(varRcvDtl(1), 4, "▦"), asRow, colPNAME
                    'SetText SPD, mGetP(varRcvDtl(1), 13, "▦"), asRow, colCHARTNO
                    intTestCnt = intTestCnt + 1
                    'For J = 1 To 1
                    
                        
                    varRcvDtl2 = Split(varRcvDtl(1), "▩")
                    For J = 0 To UBound(varRcvDtl2)
                        
                        strTstCode = mGetP(varRcvDtl2(J), 13, "▦")
                        
                        '-- 화면에 표시
                        strTstName = GetTestNm(strTstCode)
                        For intCol = colSTATE + 1 To .MaxCols
                            If strTstName = gArrEQPNm(intCol - colSTATE, 6) Then
                                .Row = asRow
                                .Col = intCol
                                .BackColor = vbYellow
                                If pEdit = False Then
        ...
        If gPatOrdCd <> "" Then
            ...
            If InStr(gPatOrdCd, "L91037") > 0 Then
                gPatOrdCd = gPatOrdCd & "'L91037#01',"
                gPatOrdCd = gPatOrdCd & "'L91037#02',"
                gPatOrdCd = gPatOrdCd & "'L91037#03',"
                gPatOrdCd = gPatOrdCd & "'L91037#04',"
                gPatOrdCd = gPatOrdCd & "'L91037#05',"
            End If
            gPatOrdCd = Mid(gPatOrdCd, 1, Len(gPatOrdCd) - 1)
        End If
'    Else
...
'    End If
    ...
    Call SetSQLData("바코드조회", "gPatOrdCd =" & gPatOrdCd, "A")
