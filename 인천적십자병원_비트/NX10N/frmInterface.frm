   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboChk 
            Height          =   315
            ItemData        =   "frmInterface.frx":B73E
            Left            =   3720
            List            =   "frmInterface.frx":B74B
            TabIndex        =   51
            Top             =   -30
            Visible         =   0   'False
            Width           =   825
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C1DB
         End
         Begin FPSpread.vaSpread vasWorkList 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":FF24
            UserResize      =   2
         End
         Begin VB.Label Label1 
            Appearance      =   0  'Æò¸é
...
'''    Res = GetDBSelectVas(gServer_Bit, SQL, vasWorkList)
                                 ...
                                 'Ã­Æ®¹øÈ£,  È¯ÀÚ¸í,    µî·Ï¹øÈ£ , Á¢¼öÀÏ  , ÀÔ/¿Ü,         ³»¿ø¹øÈ£
          SQL = "SELECT DISTINCT PbsChtNum, PbsPatNam, ResOcmNum, OcmAcpDtm, 'O' As IOGBN, OcmNum,ResSpmNum  " & vbCrLf
...
'    SQL = SQL & "   AND ResSpmNum = '" & sBarcode & "' " & vbCrLf
    SQL = SQL & "   AND OcmAcpDtm Between '" & pFrDt & "' And '" & pToDt & "'" & vbCr

    SQL = SQL & " UNION ALL " & vbCrLf
    SQL = SQL & "SELECT DISTINCT PbsChtNum, PbsPatNam, PbsResNum, IcmAcpDtm As OcmAcpDtm, 'I' As IOGBN, IcmOcmNum As OcmNum,ResSpmNum " & vbCrLf
    ...
    SQL = SQL & "   AND RsbAcpCod = '" & gGumPart & "' " & vbCrLf
'    SQL = SQL & "   AND ResSpmNum = '" & sBarcode & "' " & vbCrLf
    SQL = SQL & "   AND IcmAcpDtm Between '" & pFrDt & "' And '" & pToDt & "'" & vbCr
    SQL = SQL & " ORDER BY OcmAcpDtm,PbsPatNam"
    
    Call SetSQLData("¿öÅ©Á¶È¸", SQL)
    
'''    Res = GetDBSelectColumn(gServer_Bit, SQL)
''''    SetRawData "[SQL]" & SQL
'''
'''    If Res = 1 Then
'''        SetText vasID, "1", asRow, colCheckBox
'''        SetText vasID, sBarcode, asRow, colBarcode
'''        SetText vasID, Trim(gReadBuf(0)), asRow, colChtNum
'''        SetText vasID, Trim(gReadBuf(1)), asRow, colPName
'''        SetText vasID, Trim(gReadBuf(3)), asRow, colOrdDate
'''        SetText vasID, Trim(gReadBuf(4)), asRow, colIO
'''        SetText vasID, Trim(gReadBuf(5)), asRow, colInNum
'''    End If
    ...
    Do Until RS.EOF
        With vasID
            For i = 1 To .DataRowCnt
                'strBarcode = GetText(vasID, i, colOrdDate)
                strBarcode = GetText(vasID, i, colBarcode)

                If Trim(RS("ResSpmNum")) = strBarcode Then
                    SetText vasID, Trim(RS.Fields("ResSpmNum")) & "", i, colBarcode
                    blnSame = True
                End If
            Next
            ...
            If blnSame = False Then
                .MaxRows = .MaxRows + 1
                SetText vasID, "1", .MaxRows, colBarcode
                SetText vasID, Trim(RS.Fields("ResSpmNum")) & "", .MaxRows, colBarcode
                SetText vasID, Trim(RS.Fields("PbsChtNum")) & "", .MaxRows, colChtNum
                SetText vasID, Trim(RS.Fields("PbsPatNam")) & "", .MaxRows, colPName
                SetText vasID, Trim(RS.Fields("OcmAcpDtm")) & "", .MaxRows, colOrdDate
                SetText vasID, Trim(RS.Fields("IOGBN")) & "", .MaxRows, colIO
                SetText vasID, Trim(RS.Fields("OcmNum")) & "", .MaxRows, colInNum
            End If
...
Private Sub GetOrder(ByVal pBarNo As String)
    ...
    SetRawData "[gOrderExam]" & gOrderExam
    ...
    Dim i As Integer
        ...
        Select Case strType
            ...
            Case "N"    '## Order
                ...
                strBarNo = Trim(Mid(strRcvBuf, 23, 12))
                ...
                '## Àåºñ±âÁØ °Ë»ç¸í, °á°ú, Abnormal Flag
                strIntBase = Trim(Mid(strRcvBuf, 51, 5))
                strResult = Trim(Mid(strRcvBuf, 59, 5))
...
'                        End If
                ...
                If Len(strBarNo) >= 10 And MnTransAuto.Checked = True And strState = "R" Then
