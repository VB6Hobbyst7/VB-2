...
On Error GoTo errHandler
    ...
    With frmInterface
        ...
        strChart = Trim(GetText(.vasID, argSpcRow, colChtNum))
        If strChart = "" Then
            Exit Function
        End If
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                '-- 결과업데이트
                      SQL = "SELECT RESLABCOD FROM ResInf "
                SQL = SQL & " WHERE RESLABCOD = '" & strEqpCd & "' " & vbCrLf
                SQL = SQL & "   AND ResSpmNum = '" & lsID & "' " & vbCrLf
                ...
                If Not RS.EOF And Not RS.BOF Then
                    '-- 결과업데이트 쿼리
                          SQL = "Update ResInf Set"
                    SQL = SQL & "       ResMzhMnt  = '" & sResult & "', "
                    SQL = SQL & "       ResRepTyp = 'M', "                      '-- 보고대기(M)
                    SQL = SQL & "       ResMchCod = '" & frmInterface.Caption & "' "
                    SQL = SQL & " Where ResSpmNum = '" & lsID & "'"
        ...
        '-- 상태업데이트
              SQL = "SELECT RsbOcmNum FROM RsbInf "
        SQL = SQL & " WHERE RsbOcmNum = " & strInNum & vbCrLf
        SQL = SQL & "   AND RsbAcpCod = '" & gGumPart & "' " & vbCrLf
        SQL = SQL & "   AND (RsbItfYon <> 'I' AND RsbItfYon <> 'F') "   '최종(F)/중간(I)은 제외
        'SQL = SQL & "   AND RsbMchCod IN ('ADVIA 2120 I','ADVIA 2120_II')"
        SQL = SQL & "   AND RsbSpmCod = 'B01'"  '-- 'B01':whols blood  'B04'는 CA500 검체
...
Function GetSampleInfoW(ByVal asRow As Long) As Integer
    ...
    End If
          ...
          SQL = "SELECT DISTINCT PbsChtNum, PbsPatNam, ResOcmNum, OcmAcpDtm, 'O' As IOGBN, OcmNum  " & vbCrLf
    ...
    Call SetSQLData("RES", Res)
        
    If Res = 1 Then
        SetText frmInterface.vasID, "1", asRow, colCheckBox
        SetText frmInterface.vasID, sBarcode, asRow, colBarcode
        SetText frmInterface.vasID, Trim(gReadBuf(0)), asRow, colChtNum
        SetText frmInterface.vasID, Trim(gReadBuf(1)), asRow, colPName
        SetText frmInterface.vasID, Trim(gReadBuf(3)), asRow, colOrdDate
        SetText frmInterface.vasID, Trim(gReadBuf(4)), asRow, colIO
        SetText frmInterface.vasID, Trim(gReadBuf(5)), asRow, colInNum
