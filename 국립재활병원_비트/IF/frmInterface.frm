...
Begin VB.Form frmInterface 
   ...
   Begin FPSpread.vaSpread spdResult 
      ...
      ScrollBarExtMode=   -1  'True
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmInterface.frx":B4EB
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   ...
   Begin VB.PictureBox picBottom 
      ...
      Begin VB.Image imgReceive 
         Height          =   240
         Left            =   8730
         Picture         =   "frmInterface.frx":10193
         Top             =   180
         Width           =   240
      End
      ...
      Begin VB.Image imgNet1 
         Height          =   240
         Left            =   390
         Picture         =   "frmInterface.frx":1071D
         Top             =   180
         Width           =   240
      End
      Begin VB.Image imgNet2 
         Height          =   240
         Left            =   390
         Picture         =   "frmInterface.frx":10867
         Top             =   180
         Width           =   240
      End
      Begin VB.Image imgNet3 
         Height          =   240
         Left            =   390
         Picture         =   "frmInterface.frx":109B1
         Top             =   180
         Width           =   240
      End
   ...
   Begin FPSpread.vaSpread spdWork 
      ...
      ScrollBarExtMode=   -1  'True
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmInterface.frx":10AFB
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
End
...
Private Sub ReceiveProcess()
    ...
    If gComm.COMTYPE = "1" Then
        ...
        Select Case UCase(gHOSP.MACHNM)
            ...
            Case "URISCAN":         Call Phase_Serial_URINSCAN
            Case "TEST1":           Call Phase_Serial_TEST1
            Case "CS1600":          Call Phase_Serial_CS1600
...
Private Sub GetOrder_CS1600(ByVal pBarNo As String, ByVal pType As String)
    ...
    With frmInterface
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = ""
        strItems = GetEquipExamCode_CS1600(gHOSP.MACHCD, pBarNo, intRow)
...
On Error GoTo RST
    ...
    With frmInterface
        'R01TEST 01        0301010002e
        'R01LATEX01261009   0301010002e
        'R01LATEX01262009   0301010002e
        'R01LATEX01263009   0301010002e
        strRcvBuf = RcvBuffer
        
        Call SetSQLData("RCV", strRcvBuf, "A")
        
        If Mid(strRcvBuf, 1, 1) = "Q" Then
            Exit Sub
        End If
        ...
        If InStr(strBarno, "LATEX") > 0 Then
            mResult.Kind = "QC"
'            If InStr(strBarno, "LATEX01261") > 0 Then
'                mResult.LabNab = "1"
'            ElseIf InStr(strBarno, "LATEX01262") > 0 Then
'                mResult.LabNab = "2"
'            ElseIf InStr(strBarno, "LATEX01263") > 0 Then
'                mResult.LabNab = "3"
'            End If
            
            mResult.LabNab = Mid(strBarno, 10, 1)
            mResult.PatNo = "QC-" & mResult.LabNab
        Else
            mResult.Kind = ""
            mResult.LabNab = ""
        End If
        ...
        If strIntBase <> "" And strResult <> "" Then
            ...
            If ResultProcess(mResult.BarNo, strIntBase, strResult, strIntResult) = True Then
                If mResult.Kind = "QC" Then
                    ...
                    '-- 등록된 QC정보가 없으면 저장하지 않는다.
                    If blnQcVal = True Then
                        '-- 이미 결과가 있는지 확인
                        SQL = ""
                        SQL = SQL & "SELECT LABADPDTE                               " & vbCrLf
                        SQL = SQL & "  FROM LABQCINF                                " & vbCrLf
...
RST:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SerialRcvData_TEST1" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Private Sub SerialRcvData_CS1600()
    Dim RS_Q            As ADODB.Recordset
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                Case "Q"    '## Request Information
                    ...
                    strTypeEntry = mGetP(strTemp1, 4, "^")
                    
                    With mOrder
                        .RackNo = strRackNo
                        .TubePos = strTubePos
                        .BarNo = strBarno
                        .TypeEntry = strTypeEntry
                    End With
                    
                    Call GetOrder_CS1600(strBarno, gHOSP.RSTTYPE)
                ...
                Case "O"
                    ...
                    '-- QC 처리부분
                    If InStr(strBarno, "QC") > 0 Then
                        mResult.Kind = "QC"
                        If InStr(strBarno, "QC01") > 0 Then
                            mResult.LabNab = "1"
                        ElseIf InStr(strBarno, "QC02") > 0 Then
                            mResult.LabNab = "2"
                        'ElseIf InStr(strBarno, "QC03") > 0 Then
                        '    mResult.LabNab = "3"
                        End If
                ...
                Case "R"
                    'R|1|^^^041^PT sec^100.00^9^^^|10.2|sec||N||||||20110328135056<CR>
                    'R|2|^^^042^PT %^100.00^9^^^| 99.4|%||N||||||20110328135056<<CR>
                    strTemp1 = mGetP(strRcvBuf, 3, "|")
                    strIntBase = mGetP(strTemp1, 4, "^")
...
RST:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SerialRcvData_CS1600" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
        ...
        For intCnt = 1 To UBound(strRecvData)
            ...
            Select Case intCnt
                Case 7
                    ...
                    ElseIf InStr(strRecvData(intCnt), "Lot #") > 0 Then
                        strBarno = Trim(mGetP(strRecvData(intCnt), 2, ":"))
...
Private Sub Phase_Serial_CS1600()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
...
Private Sub SendOrder_CS1600()
    Dim strOutput   As String     '송신할 데이터

    Select Case intSndPhase
        Case 1  '## Header
            'H|￥^&|||||||||||E1394-97<CR>
            strOutput = intFrameNo & "H|\^&|||||||||||E1394-97" & vbCr & ETX
            intSndPhase = 2
            intFrameNo = intFrameNo + 1
        Case 2  '## Patient
            'P|1||||^Heisei^Jiro<CR>
        ...
        Case 4  '## Order
            'O|1|000001^01^123456789012345^B||^^^040^^100.00
            '                                \^^^050^^100.00
            '                                \...
            ...
            '## 최초 보낼때
            If mOrder.IsSending = False Then
                strOutput = "O|1|" & mOrder.RackNo & "^" & mOrder.TubePos & "^" & mOrder.BarNo & "^" & mOrder.TypeEntry & "||" & mOrder.Order & "|||||N"
                If Len(strOutput) > 230 Then
                    mOrder.IsSending = True
                    mOrder.Order = Mid$(strOutput, 231)
                    strOutput = intFrameNo & Mid$(strOutput, 1, 230) & vbCr & ETB
...
'            If UCase(Mid(strNewBarNo, 1, 2)) = "QC" Then
'                'QC1....., QC2....., QC3....., QC4.....
'                mResult.PatNo = Mid(strNewBarNo, 1, 3)
'                mResult.LabNab = Mid(strNewBarNo, 3, 1)
'                mResult.Kind = "QC"
'
'                '-- 결과환자정보
'                Call SetPatInfoQC(strBarno, gHOSP.RSTTYPE)
