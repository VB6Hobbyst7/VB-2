...
Public Function GetEquipExamCode_CS1600(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim strSendCH       As String
    
    GetEquipExamCode_CS1600 = ""
    strExamCode = ""
    ...
    '^^^Test code^^Dilution ratio^Option(^Replication)*(^Measurement Time)*
    
    '2Q|1|000001^01^ERR000000000009^A||^^^040^PT\^^^050^APTT|0|20210107155937||||||N
    ...
    If strExamCode <> "" Then
        GetEquipExamCode_CS1600 = Mid(strExamCode, 2)
    End If
...
'        End If
        ...
        If UCase(gHOSP.MACHNM) = "CS1600" Then
            Call SetText(.spdOrder, mResult.BarNo, intRow, colBARCODE)
        End If
...
Public Sub SetPatInfoQC(ByVal pBarNo As String, ByVal pType As String)
    ...
    With frmInterface
        ...
        Call SetText(.spdOrder, mResult.LabNab, intRow, colPNAME)
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_BIT1" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
