...
Public Function GetSysDate() As String
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        'Do Until RS.EOF
        '    GetSysDate = Trim(RS.Fields("ORD_NM")) & ""
        '    RS.MoveNext
        'Loop
    End If
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
        ...
        Case "BIT_KJ"                       '비트(국립재활병원)
                Call GetWorkList_BITKJ(pFrom, pTo, SPD)
        ...
        Case "HMHOSP"                       '혜민병원
                'Call GetWorkList_HMHOSP(pFrom, pTo, SPD)
...
On Error GoTo RST
    ...
    SQL = SQL & "  FROM RESINF R, OCMINF O, PBSINF P, LABMST E      " & vbCr
    SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & pFrom & "' AND '" & pTo & "'   " & vbCr
    ...
    SQL = SQL & "   AND RTRIM(LTRIM(R.RESOCMNUM)) = '" & strBarcode & "'        " & vbCr
    ...
    SQL = SQL & " GROUP BY SUBSTRING(O.OCMACPDTM,1,8)  , R.RESOCMNUM, O.OCMCHTNUM, R.RESOCMNUM, P.PBSPATNAM, P.PBSSEXTYP" & vbCr
    SQL = SQL & " ORDER BY HOSPDATE, BARCODE                                    " & vbCr
...
Public Sub GetWorkList_BITKJ(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo RST
    ...
    'R.RESACPNUM,R.RESACPDTE, R.RESMZHTYP, R.RESLABCOD ,
    'SQL = SQL & "       SUBSTRING(R.RESACPDTE,1,8) AS 접수일시"
    
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       R.RESACPDTE         AS HOSPDATE " & vbCr
    SQL = SQL & "     , R.RESSPMNUM         AS BARCODE  " & vbCr
    SQL = SQL & "     , R.RESCHTNUM         AS CHARTNO  " & vbCr
    SQL = SQL & "     , P.PBSPATNAM         AS PNAME    " & vbCr
    SQL = SQL & "     , P.PBSRESNUM         AS JUMIN    " & vbCr
    SQL = SQL & "     , R.RESACPNUM                     " & vbCr
    SQL = SQL & "     , R.RESMZHTYP                     " & vbCr
    SQL = SQL & "     , COUNT(R.RESLABCOD)  AS CNT      " & vbCr
    SQL = SQL & "  FROM RESINF R, PBSINF P              " & vbCr
    SQL = SQL & " WHERE R.RESACPDTE Between '" & pFrom & "' AND '" & pTo & "'           " & vbCr
    SQL = SQL & "   AND R.RESCHTNUM = P.PBSCHTNUM                                       " & vbCr
    SQL = SQL & "   AND R.RESLABCOD IN (" & gAllTestCd & ")                             " & vbCr
    SQL = SQL & "   AND (R.RESREPTYP IS NULL OR R.RESREPTYP <> 'F')                     " & vbCr         '--  'I':중간 'F' 완료"
    SQL = SQL & "   AND (R.RESMZHMNT = ''  OR R.RESMZHMNT = ' '  OR R.RESMZHMNT IS NULL)" & vbCr
    SQL = SQL & " GROUP BY R.RESACPDTE, R.RESSPMNUM, R.RESCHTNUM, P.PBSPATNAM, P.PBSRESNUM, R.RESACPNUM, R.RESMZHTYP " & vbCr
    SQL = SQL & " ORDER BY R.RESACPDTE,R.RESACPNUM, R.RESMZHTYP ASC " & vbCr
...
'
            ...
            SQL = SQL & "  WHERE R.RESCHTNUM = P.PBSCHTNUM  " & vbCr
            SQL = SQL & "    AND R.RESSPMNUM = '" & strBarcode & "'" & vbCr
            SQL = SQL & "    AND R.RESLABCOD IN (" & gAllTestCd & ") "
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        ...
        Case "BIT_KJ"
                Call GetSampleInfo_BITKJ(asRow, SPD)
        ...
        Case "HMHOSP"
                Call GetSampleInfo_HMHOSP(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_BITKJ(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       R.RESACPDTM         AS HOSPDATE " & vbCr
    SQL = SQL & "     , R.RESSPMNUM         AS BARCODE  " & vbCr
    SQL = SQL & "     , R.RESCHTNUM         AS CHARTNO  " & vbCr
    SQL = SQL & "     , P.PBSPATNAM         AS PNAME    " & vbCr
    SQL = SQL & "     , P.PBSRESNUM         AS JUMIN    " & vbCr
    SQL = SQL & "     , R.RESLABCOD         AS ITEM     " & vbCr
    SQL = SQL & "  FROM RESINF R, PBSINF P              " & vbCr
    SQL = SQL & " WHERE R.RESSPMNUM = '" & strBarcode & "'" & vbCr
    SQL = SQL & "   AND R.RESCHTNUM = P.PBSCHTNUM                                       " & vbCr
    SQL = SQL & "   AND R.RESLABCOD IN (" & gAllTestCd & ")                             " & vbCr
    SQL = SQL & "   AND (R.RESREPTYP IS NULL OR R.RESREPTYP <> 'F')                     " & vbCr         '--  'I':중간 'F' 완료"
    SQL = SQL & "   AND (R.RESMZHMNT = ''  OR R.RESMZHMNT = ' '  OR R.RESMZHMNT IS NULL)" & vbCr
    SQL = SQL & " ORDER BY R.RESACPDTM, R.RESSPMNUM, R.RESCHTNUM  " & vbCr
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_HMHOSP(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = ""
    SQL = SQL & "Select DISTINCT "
    SQL = SQL & "       R.SPECIMEN_SER                                                                  AS BARCODE  " & vbCr
    SQL = SQL & "     , (SELECT DISTINCT BUNHO  FROM CPL2010 WHERE SPECIMEN_SER = '" & strBarcode & "') AS PID      " & vbCr
    SQL = SQL & "     , (SELECT DISTINCT SUNAME FROM CPL2010 WHERE SPECIMEN_SER = '" & strBarcode & "') AS PNAME    " & vbCr
    SQL = SQL & "     , R.HANGMOG_CODE                                                                  AS ORDCD    " & vbCr
    SQL = SQL & "     , E.JANGBI_OUT_CODE                                                               AS ITEM     " & vbCr
    SQL = SQL & "  From CPL3020 R, EQP1010 E                                " & vbCr
    SQL = SQL & " WHERE R.SPECIMEN_SER          = '" & strBarcode & "'      " & vbCr
    SQL = SQL & "   AND NVL(R.CONFIRM_YN, 'N')  = 'N'                       " & vbCr
    SQL = SQL & "   AND E.JANGBI_CODE           = '" & gHOSP.MACHCD & "'    " & vbCr
    SQL = SQL & "   AND R.HANGMOG_CODE          = E.HANGMOG_CODE            " & vbCr
    SQL = SQL & "   And JANGBI_OUT_CODE  IN (" & gAllTestCd & ")            " & vbCr
