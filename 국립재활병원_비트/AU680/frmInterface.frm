...
'워크리스트에 처방명이 들어갈 수 있는가요??
'
'ex) 생화학루틴 16종 ~ AST(sGOT) ~ ALT(sGPT) ~ LDH ~ ALP ~ Total Protein ~ Albumin ~ T-Cholesterol ~ HDL-cholesterol ~ Triglyceride ~ Glucose(FBS) ~ T-Bilirubin  ~ BUN ~ Creatinine ~ Na(Sodium) ~ K(Potassium) ~ Cl(Chloride) ~ 접수시간 : 2016년 11월 16일 08:56
'
'이런식으로 스프레드에 한 줄 더 나타났으면 하는데 칸을 넓게 해서 전부 다 보일 필요는 없고
'
'마우스 커서를 두면 ToolTip으로 보여도 됩니다.
'
'처방명 부분은 RESSHTNAM 컬럼이고, 접수일시는 RESACPDTM   'YYYYMMDDHH24MI'입니다.
    
    
    '바코드번호로 환자정보 불러오기
    'RESODRDTM : 처방일자, R.RESACPDTE : 접수일자
    ...
    Do Until rs.EOF
        With vasID
            ...
            If blnSame = False Then
                ...
                SetText vasID, Trim(rs.Fields("차트번호")) & "", .MaxRows, colPID
                SetText vasID, Trim(rs.Fields("바코드")) & "", .MaxRows, colBarcode
                SetText vasID, Trim(rs.Fields("환자명")) & "", .MaxRows, colPName
            ...
            strTests = strTests & Trim(rs.Fields("검사명")) & "/"
                ...
                Do While Len(strTmp) >= 10
                    ...
                    If strResult <> "" Then
                        ...
                        If Res > 0 Then
                            ...
                            SQL = SQL & "   And labcod     = '" & strLabCod & "'" & vbLf   '검사코드
                            ...
                            If Res = 1 Then
                                ...
                                SQL = SQL & " WHERE LABADPDTE = '" & strDate & "'" & vbLf       '조회일자
                                SQL = SQL & "   And LABQCCOD  = '" & gEquip & "'" & vbLf        '장비명
                                SQL = SQL & "   And LABNAB    = '" & strLabNab & "'" & vbLf     '1,2:QC1,QC2"
                                SQL = SQL & "   And LABCOD    = '" & strLabCod & "'" & vbLf     '검사코드
                                'SQL = SQL & "   And LABLOT    = '" & Trim(gReadBuf(0)) & "'"    'LOT
                                
                                Call SetSQLData("QC조회_INUP구분", SQL)
                                ...
                                If Res = 1 Then
                                    ...
                                    SQL = SQL & "   Set labmzh = '" & strResult & "'" & vbLf            '결과
                                    SQL = SQL & "     , labdtm = '" & strDTM & "'" & vbLf               '시간
                                ...
                                Call SetSQLData("QC결과저장", SQL)
                                Call SetRawData("QC결과저장:" & SQL)
