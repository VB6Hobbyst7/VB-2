...
'                End If
        ...
        Else
            Select Case pType
                ...
                Case "1"
                    For i = 1 To .spdOrder.DataRowCnt
                        If Trim(GetText(frmMain.spdOrder, i, colSEQNO)) = mOrder.Seq Then
...
'                   3 : 체크된것중 제일 위에 것
'-----------------------------------------------------------------------------'
Public Sub SetPatInfo_BITQC(ByVal pBarno As String, ByVal pType As String)
    ...
    With frmMain
        ...
        Call SetText(.spdOrder, "QC", intRow, colPNAME)
        Call SetText(.spdOrder, mResult.LabNab, intRow, colCHARTNO)
...
'''        SQL = SQL & "       LABADPDTE       AS HOSPDATE                     " & vbCr
'''        SQL = SQL & "     , LABCOD          AS ITEM                         " & vbCr
'''        SQL = SQL & "  FROM LABQCINF                                        " & vbCr
'''        SQL = SQL & " WHERE LABADPDTE = '" & Format(Now, "yyyymmdd") & "'   " & vbCr      '조회일자
'''        SQL = SQL & "   And LABQCCOD  = '" & gHOSP.MACHCD & "'              " & vbCr      '장비명
'''        SQL = SQL & "   And LABNAB    = '" & mResult.LabNab & "'            " & vbCr      '1:QC1 ,2:QC2"
'''        SQL = SQL & "   And LABCOD    IN (" & gAllTestCd & ")               " & vbCr      '검사코드
'''
'''        Call SetSQLData("QC조회", SQL)
'''
'''        '-- Record Count 가져옴
'''        AdoCn.CursorLocation = adUseClient
'''        Set RSQ = AdoCn.Execute(SQL, , 1)
'''
'''        '처방이 없을 수도 있으므로..
'''        SetText SPD, "0", asRow, colCHECKBOX
'''
'''        If Not RSQ.EOF = True And Not RSQ.BOF = True Then
'''            Do Until RS.EOF
'''                With SPD
'''                    .ReDraw = False
'''                    intTestCnt = intTestCnt + 1
'''                    SetText SPD, "1", asRow, colCHECKBOX
'''                    SetText SPD, Trim(RSQ.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
'''                    SetText SPD, "QC", asRow, colPNAME
'''                    SetText SPD, mResult.LabNab, asRow, colCHARTNO
'''
'''                    '오더갯수
...
'''                        If Trim(RSQ.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
'''                            .Row = asRow
'''                            .Col = intCol
'''                            .BackColor = vbYellow
...
'''                    Next
'''
'''                    gPatOrdCd = gPatOrdCd & "'" & Trim(RSQ.Fields("ITEM")) & "',"
'''
'''                End With
'''                DoEvents
'''
...
Function SaveTransData_BITKJ(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim blnSave         As Boolean
    Dim strSaveSeq      As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        strPatID = Trim(GetText(.spdOrder, argSpcRow, colPID))
        strChartNo = Trim(GetText(.spdOrder, argSpcRow, colCHARTNO))
        strPatName = Trim(GetText(.spdOrder, argSpcRow, colPNAME))
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 결과저장
                SQL = ""
                SQL = SQL & "UPDATE RESINF                                              " & vbCr
                SQL = SQL & "   SET RESMZHMNT = '" & sResult & "'                       " & vbCr
                SQL = SQL & "     , RESUPDDTM = '" & Format(Now, "yyyymmddhhmm") & "'   " & vbCr
                SQL = SQL & "     , RESREPTYP = 'M'                                     " & vbCr        'M : 보고대기, N : 미결과, F : 보고
                SQL = SQL & " WHERE RESSPMNUM = '" & strBarcode & "'                    " & vbCr
                SQL = SQL & "   AND RESLABCOD = '" & strTestCd & "'                     " & vbCr
                SQL = SQL & "   AND RESREPTYP = 'N'                                     " & vbCr        'N : 미결과
...
Function SaveTransData_BITKJ_QC(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        SQL = ""
        SQL = SQL & "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,EXAMSUBCODE " & vbCr
        SQL = SQL & "     , REFFLAG, REFJUDGE    " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            ...
            Do Until RS_L.EOF
                intRow = intRow + 1
                Call SetText(.vasTemp, RS_L.Fields("EQUIPCODE").Value & "", intRow, 1)
                Call SetText(.vasTemp, RS_L.Fields("ORDERCODE").Value & "", intRow, 2)      'lot
                Call SetText(.vasTemp, RS_L.Fields("EXAMCODE").Value & "", intRow, 3)
                Call SetText(.vasTemp, RS_L.Fields("EXAMSUBCODE").Value & "", intRow, 4)    'ave
                Call SetText(.vasTemp, RS_L.Fields("EQUIPRESULT").Value & "", intRow, 5)
                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 6)
                Call SetText(.vasTemp, RS_L.Fields("REFFLAG").Value & "", intRow, 7)        'low
                Call SetText(.vasTemp, RS_L.Fields("REFJUDGE").Value & "", intRow, 8)       'max
        ...
        '-- 서버로 결과값 저장하기
        For intRow = 1 To .vasTemp.DataRowCnt
            strLABLOT = Trim(GetText(.vasTemp, intRow, 2))      'lot
            strTestCd = Trim(GetText(.vasTemp, intRow, 3))      '검사코드
            strLABAVE = Trim(GetText(.vasTemp, intRow, 4))      'ave
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            strLABLOW = Trim(GetText(.vasTemp, intRow, 7))      'low
            strLABMAX = Trim(GetText(.vasTemp, intRow, 8))      'max
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 이미 결과가 있는지 확인
                SQL = ""
                SQL = SQL & "SELECT LABADPDTE                           " & vbCr
                SQL = SQL & "  FROM LABQCINF                            " & vbCr
                SQL = SQL & " WHERE LABADPDTE = '" & strDate & "'       " & vbCr      '조회일자
                SQL = SQL & "   And LABQCCOD  = '" & gHOSP.MACHCD & "'  " & vbCr      '장비명
                SQL = SQL & "   And LABNAB    = '" & strChartNo & "'    " & vbCr      '1,2:QC1,QC2"
                SQL = SQL & "   And LABCOD    = '" & strTestCd & "'     " & vbCr      '검사코드
                Set RS_Q = AdoCn.Execute(SQL, , 1)
                
                If Not RS_Q.EOF = True And Not RS_Q.BOF = True Then
                    'UPDATE
                    SQL = ""
                    SQL = SQL & "Update LABQCINF                                " & vbCr
                    SQL = SQL & "   Set LABMZH      = '" & sResult & "'         " & vbCr    '결과
                    SQL = SQL & "     , LABDTM      = '" & strDTM & "'          " & vbCr    '시간
                    SQL = SQL & "     , LABAVE      = '" & strLABAVE & "'       " & vbCr    'AVE
                    SQL = SQL & "     , LABLOW      = '" & strLABLOW & "'       " & vbCr    'LOW
                    SQL = SQL & "     , LABMAX      = '" & strLABMAX & "'       " & vbCr    'MAX
                    SQL = SQL & " Where LABADPDTE   = '" & strDate & "'         " & vbCr    '조회일자
                    SQL = SQL & "   And LABQCCOD    = '" & gHOSP.MACHCD & "'    " & vbCr    '장비명
                    SQL = SQL & "   And LABNAB      = '" & strChartNo & "'      " & vbCr    '1,2:QC1,QC2"
                    SQL = SQL & "   And LABCOD      = '" & strTestCd & "'       " & vbCr     '검사코드
...
Function SaveTransData_HMHOSP(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                '-- 서버저장
                SQL = ""
                SQL = SQL & "INSERT INTO EQP3020 (" & vbCr
                SQL = SQL & "  SYS_DATE"
                SQL = SQL & ", USER_ID"
                SQL = SQL & ", UPD_DATE"
                SQL = SQL & ", SPECIMEN_SER"
                SQL = SQL & ", JANGBI_CODE"
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Select Case gEMR
        ...
        Case "BIT_KJ"
            SaveTransData = SaveTransData_BITKJ(argSpcRow)
        ...
        Case "HMHOSP"
            SaveTransData = SaveTransData_HMHOSP(argSpcRow)
