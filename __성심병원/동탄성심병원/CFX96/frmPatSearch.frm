...
Begin VB.Form frmPatSearch 
   ...
   Begin FPSpread.vaSpread vasPrint 
      ...
      ShadowDark      =   13815180
      SpreadDesigner  =   "frmPatSearch.frx":11F6
   End
   Begin FPSpread.vaSpread vasCode 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      SpreadDesigner  =   "frmPatSearch.frx":241F
   End
   Begin FPSpread.vaSpread vasList 
      ...
      ShadowDark      =   13815180
      SpreadDesigner  =   "frmPatSearch.frx":2692
   End
...
'          SQL = "SELECT a.SAMPLE, a.HOSPNO,b.NAME,b.SEX,'',b.JUMIN,a.BSDATE,a.SAMPLE,'',a.INOUT" & vbCr
'    SQL = SQL & "  FROM TL_LABOORDER a, TB_IDMAST b" & vbCr
'    SQL = SQL & " WHERE a.HOSPNO = b.HOSPNO" & vbCr
'    SQL = SQL & "   AND a.BSDATE BETWEEN '" & sSch1 & "' AND '" & sSch2 & "'" & vbCr
'    SQL = SQL & "   AND a.PUMMOK = 'VB8506A' " & vbCr
'    SQL = SQL & " GROUP BY a.SAMPLE,a.HOSPNO, b.NAME,b.SEX,b.JUMIN,a.BSDATE,a.INOUT  "
    ...
    SQL = SQL & "  FROM TL_LABOORDER a, TB_IDMAST b" & vbCr
    SQL = SQL & " Where A.HOSPNO = b.HOSPNO" & vbCr
'    SQL = SQL & "   AND a.pummok = 'VB8506B'" & vbCr
    SQL = SQL & "   AND a.pummok IN ('VB8506A','VB8506B')" & vbCr
    SQL = SQL & "   AND EXISTS (SELECT * FROM TL_WORKHEAD d" & vbCr
    SQL = SQL & "                 WHERE TO_CHAR(d.labodate,'yyyymmdd') BETWEEN '" & sSch1 & "' AND '" & sSch2 & "'" & vbCr
    SQL = SQL & "                   AND d.labpart = '31'" & vbCr
    SQL = SQL & "                   AND  d.bogoyn = 'N'" & vbCr
    SQL = SQL & "                   AND A.SAMPLE = d.sample)" & vbCr
    SQL = SQL & "   AND EXISTS (SELECT 1" & vbCr
    SQL = SQL & "                FROM TL_WORKVALUE c" & vbCr
    SQL = SQL & "               Where A.SAMPLE = C.SAMPLE" & vbCr
    SQL = SQL & "                 AND TRIM(rvalue) IS NULL) "
    
    Res = db_select_Vas(gServer, SQL, vasList, 1, 4)
    
    If Res = -1 Then
        MsgBox "조회 결과가 없습니다", vbOKOnly + vbCritical, Me.Caption
        Exit Sub
    End If
...
'    End If
    ...
    For lRow = 1 To vasList.DataRowCnt
        ...
        If vasList.Value = 1 And Trim(GetText(vasList, lRow, 4)) <> "" Then
            ...
            SetText frmInterface.vasID, Trim(GetText(vasList, lRow, 13)), lDestRow, colINOUT '입외
