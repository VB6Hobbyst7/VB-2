...
Public Function GetEquipExamCode_CA270(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim strSendCH       As String
...
Function SaveTransData_JUNNUNG(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
...
'                End If
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" And strTestCdSub <> "" Then
                    '-- 서버저장
                    SQL = ""
                    SQL = SQL & "Update SPRT_ROOM_PRSC_CNTN"
                    SQL = SQL & "   Set INSP_RSLT     = '" & sResult & "' " & vbCrLf
                    SQL = SQL & " Where CARE_ORG_ID   = '" & strBarcode & "' " & vbCrLf
                    SQL = SQL & "   And CUST_NO       = '" & strChartNo & "' " & vbCrLf
                    SQL = SQL & "   And INSR_SEQNO    = '" & mGetP(strOrdCd, 1, "|") & "' " & vbCrLf
                    SQL = SQL & "   And MDCL_SEQNO    = '" & mGetP(strOrdCd, 2, "|") & "' " & vbCrLf
                    SQL = SQL & "   And PRSC_SEQNO    = '" & mGetP(strOrdCd, 3, "|") & "' " & vbCrLf
                    SQL = SQL & "   And SPRT_ROOM_CD  = '" & strTestCdSub & "' " & vbCrLf
                    SQL = SQL & "   And USE_PRSC_CD   = '" & strTestCd & "' " & vbCrLf
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_JUNNUNG" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "JUNNUNG"                          '전능
                Call GetWorkList_JUNNUNG(pFrom, pTo, SPD)
...
Public Sub GetWorkList_JUNNUNG(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       A.MDCL_DAY      AS HOSPDATE " & vbCrLf
    SQL = SQL & "     , A.MDCL_TIME                 " & vbCrLf
    SQL = SQL & "     , B.CARE_ORG_ID   AS BARCODE  " & vbCrLf
    SQL = SQL & "     , B.CUST_NO       AS CHARTNO  " & vbCrLf
    SQL = SQL & "     , C.NAME          AS PNAME    " & vbCrLf
    SQL = SQL & "     , C.SEX           AS SEX      " & vbCrLf
    SQL = SQL & "     , C.CTZN_NO       AS JUMIN    " & vbCrLf
'    SQL = SQL & "     , B.INSR_SEQNO                " & vbCrLf
'    SQL = SQL & "     , B.MDCL_SEQNO                " & vbCrLf
'    SQL = SQL & "     , B.PRSC_SEQNO                " & vbCrLf
'    SQL = SQL & "     , B.SPRT_ROOM_CD              " & vbCrLf
    SQL = SQL & "     , C.CTZN_NO       AS JUMIN    " & vbCrLf
    SQL = SQL & "  FROM MDCL_INFO A, SPRT_ROOM_PRSC_CNTN B, CUST_INFO C" & vbCrLf
    SQL = SQL & " WHERE A.CARE_ORG_ID   = b.CARE_ORG_ID"
    SQL = SQL & "   AND A.CUST_NO       = B.CUST_NO"
    SQL = SQL & "   AND A.INSR_SEQNO    = B.INSR_SEQNO"
    SQL = SQL & "   AND A.MDCL_SEQNO    = B.MDCL_SEQNO"
    SQL = SQL & "   AND B.CARE_ORG_ID   = C.CARE_ORG_ID"
    SQL = SQL & "   AND B.CUST_NO       = C.CUST_NO"
    SQL = SQL & "   AND A.MDCL_DAY BETWEEN '" & pFrom & "' AND '" & pTo & "'" & vbCrLf
    SQL = SQL & "   AND b.USE_PRSC_CD   IN (" & gAllTestCd & ")             " & vbCrLf
    SQL = SQL & "   AND (b.INSP_RSLT is null or b.INSP_RSLT = '' )          " & vbCrLf
    SQL = SQL & " Order by PNAME, CHARTNO"
        ...
        Do Until RS.EOF
            With SPD
                For i = 1 To SPD.DataRowCnt
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colCHARTNO)
                    If Trim(RS("HOSPDATE")) = strHospDate And Trim(RS("CHARTNO")) = strBarcode Then
                        blnSame = True
                    End If
                Next
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_JUNNUNG" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
'    SQL = SQL & " Order by PNAME, CHARTNO"
    ...
    Select Case gEMR
        Case "JUNNUNG"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT "
            SQL = SQL & "       b.USE_PRSC_CD       AS ITEM  " & vbCrLf
            SQL = SQL & "  FROM MDCL_INFO A, SPRT_ROOM_PRSC_CNTN B  " & vbCrLf
            SQL = SQL & " WHERE A.CARE_ORG_ID   = b.CARE_ORG_ID"
            SQL = SQL & "   AND A.CUST_NO       = B.CUST_NO"
            SQL = SQL & "   AND A.INSR_SEQNO    = B.INSR_SEQNO"
            SQL = SQL & "   AND A.MDCL_SEQNO    = B.MDCL_SEQNO"
            SQL = SQL & "   AND A.MDCL_DAY      = '" & strRegDate & "'" & vbCrLf
            SQL = SQL & "   AND B.CUST_NO       = '" & strChartNo & "'" & vbCrLf
            SQL = SQL & "   AND b.USE_PRSC_CD   IN (" & gAllTestCd & ")             " & vbCrLf
            SQL = SQL & "   AND (b.INSP_RSLT is null or b.INSP_RSLT = '' )          " & vbCrLf
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "JUNNUNG"
            SaveTransData = SaveTransData_JUNNUNG(argSpcRow, SPD)
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    If cn_Server_Flag = True Then
        Select Case gEMR
            Case "JUNNUNG"
                    Call GetSampleInfo_JUNNUNG(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_JUNNUNG(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = SQL & "   AND B.CUST_NO       = C.CUST_NO"
    SQL = SQL & "   AND A.MDCL_DAY      =  '" & strRegDate & "'" & vbCrLf
    SQL = SQL & "   AND B.CUST_NO       =  '" & strChartNo & "'"
    SQL = SQL & "   AND b.USE_PRSC_CD   IN (" & gAllTestCd & ")             " & vbCrLf
    SQL = SQL & "   AND (b.INSP_RSLT is null or b.INSP_RSLT = '' )          " & vbCrLf
    SQL = SQL & " Order by PNAME, CHARTNO"
            ...
            With SPD
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        '저장키
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("INSR_SEQNO")) & "|" & Trim(RS.Fields("MDCL_SEQNO")) & "|" & Trim(RS.Fields("PRSC_SEQNO"))
                        gArrEQP(intCol - colSTATE, 17) = Trim(RS.Fields("SPRT_ROOM_CD")) & ""
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_JUNNUNG" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
