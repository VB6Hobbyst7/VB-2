...
Begin VB.Form frmQCMaster 
   ...
   Begin FPSpread.vaSpread spdDetail 
      ...
      ScrollBars      =   2
      SelectBlockOptions=   0
      SpreadDesigner  =   "frmQCMaster.frx":057C
   End
   Begin FPSpread.vaSpread spdQCID 
      ...
      ScrollBars      =   2
      SelectBlockOptions=   0
      SpreadDesigner  =   "frmQCMaster.frx":0BE9
   End
...
Private Sub cmdQCPathSave_Click()
    ...
    Call WritePrivateProfileString("HOSP", "QCPATH", txtQCPath.Text, App.PATH & "\INI\" & gMACH & ".ini")
...
Private Sub spdHeader_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    strInst = Trim(GetText(spdHeader, Row, 3))
    lblQCID.Caption = "QC ID 리스트," & strInst
    
    '-- 673018,
    Call GetQCList_Detail(gHOSP.LABCD, Trim(GetText(spdHeader, Row, 1)), Trim(GetText(spdHeader, Row, 2)))
...
Private Sub spdHeader_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyDelete Then
        If iRow < 1 Or iRow > spdHeader.DataRowCnt Then
            Exit Sub
        End If
        strLot = Trim(GetText(spdHeader, iRow, 1))
