...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                      SQL = "Update twexam_general_sub " & vbCrLf
                SQL = SQL & "   Set RESULT1     = '" & sResult & "'" & vbCrLf '검사결과
                SQL = SQL & " Where PTNO     = '" & lsID & "'" & vbCrLf        '검체번호
...
'                '-- 상태업데이트
'                         SQL = "Update TW_HSP_OCS.TWEXAM_SPECMST "
'                SQL = SQL & "   Set STATUS     = '4',"                  '검사상태(=결과등록(부분전송)
'                SQL = SQL & "       RESULTDATE = TRUNC(SYSDATE) "
'                SQL = SQL & " Where SPECNO     = '" & lsID & "'"        '검체번호
'                SQL = SQL & "   And STATUS     <= '3' "                 '검사상태(=검체접수)
'
...
'-- 검사자 정보 가져오기
Function GetSampleTestInfo(ByVal asRow As Long) As Integer
    Dim sBarcode            As String
    Dim GetOrderExamCode    As String
    Dim intCol              As Integer
    Dim strTestCd           As String
    ...
    Select Case gOCS
        Case "TWIN"
            '-- 2017.04.05 : 최초작성
                  SQL = "SELECT DISTINCT"
            SQL = SQL & " a.JEOBSUDT AS HOSPDATE" & vbCr
            SQL = SQL & ",a.PTNO     AS PATID" & vbCr
            SQL = SQL & ",a.SLIPNO2  AS CHARTNO" & vbCr
            SQL = SQL & ",b.SNAME    AS PATNM" & vbCr
            SQL = SQL & ",a.SEX      AS PATSEX" & vbCr
            SQL = SQL & ",a.AGEYY    AS PATAGE " & vbCr
            SQL = SQL & ",a.ITEMCD   AS ITEM " & vbCr
            SQL = SQL & "  FROM TWEXAM_GENERAL_SUB a," & vbCr
            SQL = SQL & "       TW_MIS_PMPA.TWBAS_PATIENT b " & vbCr
            SQL = SQL & " Where a.PTNO = '" & sBarcode & "'" & vbCr
            SQL = SQL & "   And a.PTNO = b.PTNO " & vbCr
            SQL = SQL & "   And a.JEOBSUDT = TO_DATE('" & strDate & "', 'yyyy/mm/dd hh24/mi/ss') " & vbCr
            SQL = SQL & "   AND a.ITEMCD IN (" & gAllExam & ")"
        ...
        Case "HWASAN"
            ...
            ' O.AllTestNm   모든 검사명
                  SQL = " SELECT DISTINCT"
            SQL = SQL & " O.OrdDt AS HOSPDATE"
            SQL = SQL & ",O.PtID AS PATID"
            SQL = SQL & ",O.PtNm AS PATNM"
            SQL = SQL & ",O.Sex AS PATSEX"
            SQL = SQL & ",O.Age AS PATAGE"
            SQL = SQL & ",O.StatFg AS INOUT"
            SQL = SQL & ",O.SPCNO as 바코드번호"
        ...
        Case "MCC"
                  SQL = "select a.ptnt_no AS PATID"
            SQL = SQL & ",c.ptnt_nm AS PATNM"
            SQL = SQL & ",a.recept_no AS CHARTNO, a.ord_no, a.ord_seq_no, a.ord_cd," & vbCr
            SQL = SQL & "       (select substr( max( apply_ymd || ord_nm),9) " & vbCr
            SQL = SQL & "          from hz_mst_lab_spc " & vbCr
            SQL = SQL & "         where ord_cd = a.ord_cd " & vbCr
            SQL = SQL & "           and spc_cd = a.spc_cd and apply_ymd <= a.acc_ymd) as ord_nm, " & vbCr
    ...
    Call SetSQLData("바코드조회" & gOCS, SQL)
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, Trim(RS.Fields("CHARTNO")) & "", asRow, colCHARTNO
                SetText .vasID, Trim(RS.Fields("PATID")) & "", asRow, colPID
                SetText .vasID, Trim(RS.Fields("PATNM")) & "", asRow, colPNAME
                SetText .vasID, Trim(RS.Fields("PATSEX")) & "", asRow, colPSEX
                SetText .vasID, Trim(RS.Fields("PATAGE")) & "", asRow, colPAGE
