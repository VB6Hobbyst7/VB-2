...
'RESA_RESULT     NVARCHAR    50          결과        NULL
                ...
                SQL = SQL & " Set "
                SQL = SQL & " RESA_RESULT     = '" & sResult & "'" & vbCrLf '검사결과
'                SQL = SQL & ",RESA_BIGO5     = '1'" & vbCrLf                '검사결과가 인터페이스에서 저장되면 챠트번호란 우측 상단에 빨간 점이 나타나게 하는것..
                SQL = SQL & " Where RESA_CHAM_INDEX     = '" & Val(lsID) & "'" & vbCrLf        '검체번호
                ...
                'LAB_HISTORY2017 테이블에 저장시간, RESA_KEY, TAT, 결과 입력자 등 을 맞춰서 입력해주시면 됩니다. (검사인증에 관련한 사항)
'LAB_HISTORY2017
...
'0   2017-02-08 10:39:00 17020834633330102101    1   33017   최초    0.57    61  62600   31  39  C3720
'
'
...
Function GetSampleInfoW_NEOSOFT(ByVal asRow As Long) As Integer
    ...
    End If
          ...
          SQL = " SELECT DISTINCT a.MEDM_ID,a.CHAM_INDEX,a.WORK_DATE, '입원' as IO,b.CHAM_NAME,b.CHAM_SEX,b.CHAM_YY, a.CODE as ORD_CD " & vbCrLf
    ...
    SQL = SQL & " UNION ALL "
    SQL = SQL & " SELECT DISTINCT a.MEDM_ID,a.CHAM_INDEX,a.WORK_DATE, '외래' as IO,b.CHAM_NAME,b.CHAM_SEX,b.CHAM_YY, a.CODE as ORD_CD " & vbCrLf
    ...
    With frmInterface
        Do Until RS.EOF
            GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("ORD_CD")) & "',"
             ...
             SetText .vasID, "1", .vasID.MaxRows, colCheckBox
             SetText .vasID, Trim(RS.Fields("WORK_DATE")) & "", asRow, colHOSPDATE
             SetText .vasID, Trim(RS.Fields("CHAM_INDEX")) & "", asRow, colBARCODE
             SetText .vasID, Trim(RS.Fields("MEDM_ID")) & "", asRow, colCHARTNO
             SetText .vasID, Trim(RS.Fields("CHAM_INDEX")) & "", asRow, colPID
             SetText .vasID, Trim(RS.Fields("IO")) & "", asRow, colINOUT
             SetText .vasID, Trim(RS.Fields("CHAM_NAME")) & "", asRow, colPNAME
            ' SetText vasID, Trim(RS.Fields("CHAM_YY")) & "", .MaxRows, colPAGE
             SetText .vasID, IIf(Trim(RS.Fields("CHAM_SEX")) & "" = "0", "M", "F"), asRow, colPSEX
...
'
''>> ORDER_IN/ ORDER_OUT
''MEDM_ID        NVARCHAR    10          병원 인덱스     NOT NULL
''WORK_DATE      NVARCHAR    8           처방 일자       NOT NULL
''CHAM_INDEX     NVARCHAR    10          챠트 인덱스     NOT NULL  >> 바코드번호
''GWAM_ID        NVARCHAR    3           진료과목        NOT NULL
''DOC_ID         NVARCHAR    20          진료의사        NOT NULL
''SEQ            INT                     처방 번호(불변) NOT NULL
''CNT            INT                     처방 순서       NOT NULL
''CODE           NVARCHAR    20          병원 코드       NULL
''ID             NVARCHAR    20          심사 코드       NULL
''
''>> HP_CHAM
''CHAM_ID        NVARCHAR    10          환자 아이디     NOT NULL
''CHAM_INDEX     NVARCHAR    10          챠트 번호       NOT NULL
''CHAM_NAME      NVARCHAR    20          환자 성명       NULL
''CHAM_YY        NVARCHAR    2           주민번호  년도  NULL
''CHAM_JUMIN1    NVARCHAR    64          주민번호 1      NULL
''CHAM_JUMIN2    NVARCHAR    64          주민번호 2      NULL
''CHAM_SEX       SMALLINT                성별            NULL ( 0 : 남자 , 1 : 여자)
'
'
'          SQL = " SELECT DISTINCT a.MEDM_ID,a.CHAM_INDEX,a.WORK_DATE, '입원' as IO,b.CHAM_NAME,b.CHAM_SEX,b.CHAM_YY " & vbCrLf
'    SQL = SQL & "  From E_ORDER..ORDER_IN" & Format(Now, "yyyy") & " a, E_BASECODE..HP_CHAM b " & vbCrLf
'    SQL = SQL & " Where a.CHAM_INDEX = '" & sBarcode & "'" & vbCrLf
'    SQL = SQL & "   And a.CHAM_INDEX = b.CHAM_INDEX " & vbCr
'    SQL = SQL & "   AND a.CODE IN (" & gAllExam & ")"
'    SQL = SQL & "   AND a.TRANS = '2' " & vbCr
'    SQL = SQL & " UNION ALL "
'    SQL = SQL & " SELECT DISTINCT a.MEDM_ID,a.CHAM_INDEX,a.WORK_DATE, '외래' as IO,b.CHAM_NAME,b.CHAM_SEX,b.CHAM_YY " & vbCrLf
'    SQL = SQL & "  From E_ORDER..ORDER_OUT" & Format(Now, "yyyy") & " a, E_BASECODE..HP_CHAM b " & vbCrLf
'    SQL = SQL & " Where a.CHAM_INDEX = '" & sBarcode & "'" & vbCrLf
'    SQL = SQL & "   And a.CHAM_INDEX = b.CHAM_INDEX " & vbCr
'    SQL = SQL & "   AND a.CODE IN (" & gAllExam & ")"
