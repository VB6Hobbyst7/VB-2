...
'''    SQL = SQL & "  FROM TB_H141_LISTAKEBODY O, TB_A110_PATINFO P  " & vbCr          'TB_H131_SPPRESULT
'''    SQL = SQL & " Where P.A110_ChartNo      = O.H141_CHARTNO      " & vbCr
'''    SQL = SQL & "   AND O.H141_TSAMPLENO    = '" & strBarcode & "'" & vbCr
'''    'SQL = SQL & "   AND O.H141_NOTYYN       = 'N'                 " & vbCr
''''    SQL = SQL & "   AND O.H141_NOTYYN       IN ('N','T')                 " & vbCr '결과대기:T
...
''''                SQL = SQL & "   AND H131_SEQNO   = '" & strSubCode & "'     " & vbCr
    ...
    SQL = SQL & "  FROM TB_H141_LISTAKEBODY O, TB_A110_PATINFO P, TB_H131_SPPRESULT R  " & vbCrLf
    SQL = SQL & " Where P.A110_ChartNo      = O.H141_CHARTNO        " & vbCrLf
    SQL = SQL & "   AND O.H141_TSAMPLENO    = '" & strBarcode & "'  " & vbCrLf
'    SQL = SQL & "   AND R.H131_SPPTYPE = '" & gHOSP.PARTCD & "'     " & vbCrLf    'L010
    SQL = SQL & "   And O.H141_SUGACD in (" & gAllTestCd & ")       " & vbCrLf
    SQL = SQL & "   And O.H141_SEQNO = R.H131_SEQNO                 " & vbCrLf
    
    '-- 2021.01.27 추가
    SQL = SQL & "   AND O.H141_NOTYYN       IN ('N','T')                 " & vbCr '결과대기(부분통보):T, Y:확정

    If UCase(gHOSP.MACHNM) = "HITACHI7180" Then
        SQL = SQL & "   AND  (R.H131_TRANSDATE = '' OR R.H131_TRANSDATE IS NULL)" & vbCrLf        'OR R.H131_TRANSTIME = '' OR R.H131_TRANSMEN
        'SQL = SQL & "   And (R.H131_RESULT = '결과대기' OR R.H131_RESULT = '' OR R.H131_RESULT IS NULL)" & vbCrLf
    End If
