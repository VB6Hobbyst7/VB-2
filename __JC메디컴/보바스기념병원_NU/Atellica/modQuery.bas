...
Function SaveTransData_NU(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Dim strOrdCd        As String
    Dim strTestCd       As String
    Dim strPrevTestCd   As String
    
    Dim strSubCode      As String
    Dim strEqpcd        As String
    Dim strPrevEqpcd    As String
    Dim sResult         As String
    Dim sResult1        As String
    Dim sResult2        As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        'End If
              ...
              SQL = "SELECT DISTINCT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMCODESUB,EQUIPRESULT,RESULT " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE BARCODE = '" & strBarcode & "' " & vbCr                       '바코드
...
'                Call SetText(.vasTemp, RS_L.Fields("EXAMCODESUB").Value & "", intRow, 4)
...
'                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 6)
                ...
                If strPrevTestCd <> strTestCd Then
                    If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                        strAllResult = strAllResult & strTestCd & "" & sResult & "" & strDate & "" & "1" & ""
                    End If
                End If
                strPrevTestCd = strTestCd
        ...
        If strAllResult <> "" Then
            '보바스
            'strURL += SERVERIP + "/himed2/.live?submit_id=" + TXLII00101 + "&business_id=lis&bcno=" + argBarcode + "&result=";
            ...
            'Call SetSQLData("결과저장1", "Param:" & gHOSP.APIURL & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine)
            
            sRcvData = OpenURLWithIE2(gHOSP.APIURL & sParam, frmMain.Inet1)

            Call SetSQLData("결과저장", "Param:" & gHOSP.APIURL & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine)
...
Function SaveTransDataR_NU(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
...
Function SaveTransDataROne_NU(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
...
'                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 6)
                ...
                If strPrevTestCd <> strTestCd Then
                    If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                        strOneResult = strTestCd & "" & sResult & "" & strDate & "" & "1" & ""
                    End If
                End If
                strPrevTestCd = strTestCd
                
                If strOneResult <> "" Then
                    sParam = ""
                    sParam = sParam & "submit_id=TXLII00101&"
                    sParam = sParam & "business_id=lis&"
                    sParam = sParam & "bcno=" & strBarcode & "&"                                      '검체번호(바코드)
                    sParam = sParam & "result=" & strOneResult & "&"                            '결과
                    sParam = sParam & "eqmtcd=" & gHOSP.MACHCD & "&"                             '장비코드
                    sParam = sParam & "instcd=" & gHOSP.HOSPCD & "&"                            '기관코드
                    sParam = sParam & "userid=" & gHOSP.USERID & "&"                                '사용자ID
                    ...
                    Call SetSQLData("결과저장one", "Param:" & gHOSP.APIURL & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine, "A")
                End If
...
ReSearch:
    ...
    sParam = sParam & "testcd=" & gHOSP.ORDCODE & "&"                                  '검사코드
...
'-- 장비결과 조회
Public Sub GetResultListR(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim i           As Integer
    Dim iCnt        As Long
    Dim intRow      As Long
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT EXAMDATE,EXAMTIME,HOSPDATE,BARCODE,PNAME,SENDFLAG,SENDDATE " & vbCr
    '-- 검사결과
    SQL = SQL & ",SEQNO,EXAMNAME,RESULT,PREVRESULT,REFJUDGE" & vbCr
    ...
    SQL = SQL & " WHERE EXAMDATE Between '" & pFrom & "' AND '" & pTo & "'" & vbCr
'    SQL = SQL & "   AND EXAMCODE IN (" & gAllTestCd & ") " & vbCr
    SQL = SQL & " ORDER BY EXAMDATE,BARCODE,SEQNO"
        ...
        Do Until RS.EOF
            ...
            With SPD
                ...
                If strBarcode <> Trim(RS.Fields("BARCODE")) & "" Or strExamDate <> Trim(RS.Fields("EXAMDATE")) & "" Then
                    ...
                    SetText SPD, "0", intRow, colCHECKBOX
                    'SetText SPD, Trim(RS.Fields("SAVESEQ")) & "", intRow, colSAVESEQ
                    SetText SPD, Trim(RS.Fields("EXAMDATE")) & "", intRow, colEXAMDATE
                    SetText SPD, Trim(RS.Fields("EXAMTIME")) & "", intRow, colEXAMTIME
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
...
Function SaveTransDataR(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "NU"
            SaveTransDataR = SaveTransDataROne_NU(argSpcRow, SPD)
'        Case "AMIS"
...
'-- 검사자 정보 가져오기
Function GetSampleInfoR(ByVal asRow As Long, ByVal SPD As Object) As Integer
   ...
   ' If cn_Server_Flag = True Then
        Select Case gEMR
            Case "NU"
                    Call GetSampleInfoR_NU(asRow, SPD)
...
On Error GoTo DBErr
    ...
    'Call SetSQLData("바코드조회1", gHOSP.APIURL & sParam & vbNewLine)
    
    sRcvData = ""
    sRcvData = OpenURLWithIE2(gHOSP.APIURL & sParam, frmMain.Inet1)

    Call SetSQLData("바코드조회", "Param:" & gHOSP.APIURL & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine)
...
'sRcvData = sRcvData & "<root><worklist>"
'sRcvData = sRcvData & "<bcno><![CDATA[3053400770]]></bcno><patnm><![CDATA[김경문]]></patnm><prgstno><![CDATA[431002-1******]]></prgstno><pid><![CDATA[000076981]]></pid><sex><![CDATA[M]]></sex><age><![CDATA[76]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4280]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20200217I10038]]></workno><testcd><![CDATA[C4280]]></testcd><execprcpuniqno><![CDATA[2012438327]]></execprcpuniqno><spcacptdt><![CDATA[20200217140830]]></spcacptdt><prcpdd><![CDATA[20200217]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist><message><type>info</type><code>info</code><msg>정상적으로 처리되었습니다.</msg><description></description></message>"
...
'sRcvData = sRcvData & "<root>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C3340]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10045]]></workno><testcd><![CDATA[C3340]]></testcd><execprcpuniqno><![CDATA[2018523155]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C3360]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10045]]></workno><testcd><![CDATA[C3360]]></testcd><execprcpuniqno><![CDATA[2018523156]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4212]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10046]]></workno><testcd><![CDATA[C4212]]></testcd><execprcpuniqno><![CDATA[2018523153]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4212]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat></rsltstat><workno><![CDATA[20201105I10046]]></workno><testcd><![CDATA[C4212E]]></testcd><execprcpuniqno><![CDATA[2018523153]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4220]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10046]]></workno><testcd><![CDATA[C4220]]></testcd><execprcpuniqno><![CDATA[2018523150]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4230]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10046]]></workno><testcd><![CDATA[C4230]]></testcd><execprcpuniqno><![CDATA[2018523151]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4240]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10046]]></workno><testcd><![CDATA[C4240]]></testcd><execprcpuniqno><![CDATA[2018523152]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4601]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I20017]]></workno><testcd><![CDATA[C4601]]></testcd><execprcpuniqno><![CDATA[2018523149]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4713]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10045]]></workno><testcd><![CDATA[C4713]]></testcd><execprcpuniqno><![CDATA[2018523106]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4802]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10045]]></workno><testcd><![CDATA[C4802]]></testcd><execprcpuniqno><![CDATA[2018523107]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4812]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10045]]></workno><testcd><![CDATA[C4812]]></testcd><execprcpuniqno><![CDATA[2018523108]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
'sRcvData = sRcvData & "<worklist><bcno><![CDATA[3079500150]]></bcno><patnm><![CDATA[박수연]]></patnm><prgstno><![CDATA[660120-2******]]></prgstno><pid><![CDATA[000143118]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[54]]></age><spcnm><![CDATA[Serum]]></spcnm><spccd><![CDATA[01A]]></spccd><tclscd><![CDATA[C4872]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20201105I10045]]></workno><testcd><![CDATA[C4872]]></testcd><execprcpuniqno><![CDATA[2018523110]]></execprcpuniqno><spcacptdt><![CDATA[20201105113705]]></spcacptdt><prcpdd><![CDATA[20201105]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[HM]]></orddeptcd></worklist>"
    ...
    If UBound(varRcvData) >= 1 Then
        For i = 0 To UBound(varRcvData) - 1
            With SPD
                ...
                '접수상태 : <rsltstat><![CDATA[1]]></rsltstat>  ==> 1:접수 , 4:보고

                '-- 화면에 표시
                For intCol = colSTATE + 1 To .MaxCols
                
                    'If XmlSelectS.RSLTSTAT(i) <> "4" Then
                    If XmlSelectS.RSLTSTAT(i) = "1" Or XmlSelectS.RSLTSTAT(i) = "-" Then
...
'-- 검사자 정보 가져오기
Function GetSampleInfoR_NU(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'        SQL = SQL & " WHERE EQUIPNO     = '" & gHOSP.MACHCD & "' " & vbCrLf
        SQL = SQL & " WHERE EXAMDATE    = '" & Trim(GetText(.spdOrder, asRow1, colEXAMDATE)) & "' " & vbCrLf
