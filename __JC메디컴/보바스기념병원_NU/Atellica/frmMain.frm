...
Begin VB.Form frmMain 
   ...
   Begin VB.PictureBox picHeader 
      ...
      Top             =   0
      Width           =   15960
      Begin VB.CommandButton cmdResultR 
         BackColor       =   &H00FFFFFF&
         Caption         =   "결과조회"
         Height          =   375
         Left            =   18420
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                Case "O"
                    ...
                    'O|1|3053600010^AD031074^4||^^^TSH3UL^^^1|R|20200219134236|||||||||Serum^^^1||||||||||F
...
'                    End If
                ...
                Case "L"
                    '## DB에 결과저장
                    If gHOSP.SAVEAUTO = "Y" And strState = "R" Then
                        Res = SaveTransDataROne_NU(gRow, spdOrder)
                        ...
                        Else
                            ...
                            SetText .spdOrder, "0", gRow, colCHECKBOX
                                  ...
                                  SQL = "Update PATRESULT Set " & vbCrLf
                            SQL = SQL & " sendflag = '2' " & vbCrLf
                            SQL = SQL & " Where MID(EXAMDATE,1,10) = '" & Mid(Trim(GetText(.spdOrder, gRow, colEXAMDATE)), 1, 10) & "'" & vbCr   '검사일
                            SQL = SQL & "   And barcode = '" & Trim(GetText(.spdOrder, gRow, colBARCODE)) & "' " & vbCrLf
                            'SQL = SQL & "   And saveseq = " & Trim(GetText(.spdOrder, gRow, colSAVESEQ)) & vbCrLf
        ...
        Select Case intPhase
            Case 1
                Select Case BufChar
                    ...
                    Case ACK
                        If strState = "Q" Then
                            Call SetRawData("[State1]" & strState)
                            Call SendOrder_ATELLICA_Centaur
                            Call SetRawData("[State1]" & strState)
                        Else
                            Call SetRawData("[State2]" & strState)
                            Call SendWSckData(ACK)
                            Call SetRawData("[State2]" & strState)
                        End If
                End Select
...
Private Sub SendOrder_ATELLICA_One()
    Dim strOutput   As String     '송신할 데이터
    ...
    Select Case intSndPhase
        Case 1  '## Header
            ...
            Else
                strOutput = strOutput & "P|1|||" & mOrder.PID & "||||" & vbCr '& ETX
                'strOutput = strOutput & "O|1|" & mOrder.BarNo & "||" & mOrder.Order & "|R" & "||||||||||Serum||||||||||O\Q" & vbCr
                strOutput = strOutput & "O|1|" & mOrder.BarNo & "||" & mOrder.Order & "|R" & "||||||||||Serum||||||||||O" & vbCr
                strOutput = strOutput & "L|1|N" & vbCr & ETX
...
Private Sub SendOrder_ATELLICA_Centaur()
    Dim strOutput   As String     '송신할 데이터
    ...
    Select Case intSndPhase
        Case 1  '## Header
            ...
            If mOrder.NoOrder = False Then
                ...
                '## Patient
                strOutput = strOutput & "P|1|" & mOrder.PID & vbCr
                '## Order
                strOutput = strOutput & "O|1|" & mOrder.BarNo & "||" & mOrder.Order & "|R||||||N||||Serum||||||||||Q" & vbCr
                '## Termianator
                strOutput = strOutput & "L|1|F" & vbCr & ETX
            Else
                '## Header
                strOutput = "H|\^&||||||||||P|1" & vbCr
                '## Termianator
                strOutput = strOutput & "L|1|I" & vbCr & ETX
            End If
