...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "ACT5DIFF_UNI"
            Call PhaseCfg_Protocol_ACT5diff_UNI  '<2008/11/10    mc
            
        Case "ACT5DIFF_ASTM"
            Call PhaseCfg_Protocol_ACT5diff_ASTM  '<2008/05/22    mc
            
        Case "ACT5DIFF_ASTM_REAL"
            Call PhaseCfg_Protocol_ACT5diff_ASTM_Real   '<2011/04/28    mc
...
Private Sub DataEdit_ACT5diff_UNI()
    On Error GoTo ErrRtn
    ...
    '설정된 장비코드 편집
    m_p_sTIFCd = "WBC|RBC|HGB|HCT|MCV|MCH|MCHC|RDW|PLT|MPV|PDW|PCT|PDW|"
    ...
    For ii = 0 To UBound(sTestCd) - 1
        sIFCd = Trim(sTestCd(ii))
        sRst1 = CStr(Val(Mid(sRowData(ii + 2), 2, 6)))
        sFlag = Trim(Mid(sRowData(ii + 2), 8, 2))
...
Private Sub PhaseCfg_Protocol_ACT5diff_UNI()
...
Private Sub PhaseCfg_Protocol_ACT5diff_ASTM_Real()
    On Error GoTo ErrRtn
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case m_iPhase
            ...
            Case 3
                Select Case Asc(wkDat)
                    Case 6      'ACK
                        Call SendOrder_ACT5diff_ASTM_Real
...
Private Sub SendOrder_ACT5diff_ASTM()
    ...
    Select Case m_iSendPhase
        ...
        Case 1
            'Header Record
            'sSendBuff = m_iFrameN & "H|\^&|||BCI|||||||P|D1394-97|20080522154814" & vbCr
            sSendBuff = m_iFrameN & "H|\^&||||||||||||" & vbCr & Chr(3)
            m_iSendPhase = 2
            
        Case 2
            'Patient Record
            Call Get_OrderString
            sSendBuff = m_iFrameN & "P|1||" & pSampleInfo.ID & "||^|||U|||||Physician||||||||||||Location|||||||||" & vbCr & Chr(3)
            m_iSendPhase = 3
...
Private Sub SendOrder_ACT5diff_ASTM_Real()
    On Error GoTo Err_Rtn
