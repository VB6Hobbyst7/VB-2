...
Private Sub SendOrder_ACT5diff_ASTM()
    ...
    Select Case m_iSendPhase
        ...
        Case 2      'P
            ...
            'sTmp = sTmp & "P|1||" & pSampleInfo.ID & "||^|||U|||||Physician||||||||||||Location|||||||||" & Chr(13) & Chr(3)
            sTmp = sTmp & m_iFrameN & "P|1||" & pSampleInfo.ID & "||^|||U" & Chr(13) & Chr(3)
            
            m_iSendPhase = 3

        Case 3      'O
            '----- 검사항목 조회/편집
            sTmp = sTmp & m_iFrameN & "O|1|" & pSampleInfo.ID & "||" & "^^^" & pSampleInfo.IFCD(1) & "|||" & Format(Now, "yyyyMMddHHmmss") & "||||||||||||||||||O|||||" & Chr(13) & Chr(3)
                                    
            m_iSendPhase = 4

        Case 4      'T
            sTmp = sTmp & m_iFrameN & "L|1|N" & vbCr & Chr(13) & Chr(3)
