...
End Sub
Private Sub PhaseCfg_Protocol_BN2()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            ...
            Case 21        ' NAK
                If m_iPhase = 2 Then
                    msComm.Output = Chr$(2) & msSavOrder & Chr$(13) & Chr$(3) & Chr$(6)
...
Private Sub DataEditResponse_BN2()
    ...
    sFrame = Left$(RcvBuffer, 1)        ' get Record Identifier of RcvBuffer.
    
    Select Case sFrame
        Case "J"        'Joblist Request
            tmpID = Trim(Mid$(RcvBuffer, 2, 29))                   ' Sample ID.
            
            pSampleInfo.ID = tmpID

            Call SendOrder_BN2
            
       Case "D"         'Result Record
            '결과 구조체 초기화
            Call Init_pResultInfo
            
            tmpID = Trim$(Mid$(RcvBuffer, 2, 29))          ' Sample ID.
            
            tmpIFCd = Trim$(Mid$(RcvBuffer, 31, 2))
            tmpFlag = Trim$(Mid$(RcvBuffer, 33, 4))
            If tmpFlag = "0" Then
                tmpFlag = ""
            End If
            tmpRst = Trim$(Mid$(RcvBuffer, 37, 15))
            If Left(tmpRst, 1) = "." Then
                tmpRst = "0" & tmpRst
            End If
            tmpUnit = Trim(Mid(RcvBuffer, 52, 9))
            
            tmpRstDT = Trim(Mid(RcvBuffer, 73, 6))
            tmpRstDT = Mid(tmpRstDT, 5, 2) & Mid(tmpRstDT, 3, 2) & Mid(tmpRstDT, 1, 2)
            tmpRstDT = "20" & tmpRstDT & Trim(Mid(RcvBuffer, 79, 6))
       ...
       Case "R"         ' QC Result Record
            '결과 구조체 초기화
            Call Init_pResultInfo
            
            tmpID = Trim$(Mid$(RcvBuffer, 2, 29))       'Control ID
            ...
            tmpRstDT = Trim(Mid(RcvBuffer, 83, 6))
            tmpRstDT = Mid(tmpRstDT, 5, 2) & Mid(tmpRstDT, 3, 2) & Mid(tmpRstDT, 1, 2)
            tmpRstDT = "20" & tmpRstDT & Trim(Mid(RcvBuffer, 89, 6))
...
Private Sub SendOrder_BN2()
    ...
    'Order Packet 구성
    sSendBuff = "J"
    sSendBuff = sSendBuff & Left(pSampleInfo.ID & Space(29), 29)
    sSendBuff = sSendBuff & Format(Now, "DDMMYYYY")
    sSendBuff = sSendBuff & " "     'SEX
    sSendBuff = sSendBuff & "0"     'EMER
    sSendBuff = sSendBuff & "00"    'REMARK
    sSendBuff = sSendBuff & "0"     'Flag(Not Use)
    
    '검사항목 편집
    sTestBuf = ""
    For i = 1 To pSampleInfo.ORDCNT
        sTestBuf = sTestBuf & Format(pSampleInfo.IFCD(i), "00") & "0"
    Next i
    sSendBuff = sSendBuff & sTestBuf
               
    '<S ***** Check Sum(BCC) Calculate Start *****
    iCS = 0
    For i = 1 To Len(sSendBuff)
        iCS = iCS + Asc(Mid$(sSendBuff, i, 1))
    Next i
    iCS = 64 - (iCS Mod 64)
    If iCS < 32 Then iCS = iCS + 64
    'E> ***** Check Sum(BCC) Calculate End   *****

    sSendBuff = sSendBuff & Chr$(iCS)

    sSendBuff = Chr$(2) & sSendBuff & Chr$(13) & Chr$(3) & Chr$(6)
    msSavOrder = sSendBuff
