...
Private Sub PhaseCfg_Protocol_LX2200()
...
Private Sub DataEditResponse_LX2200()
    On Error GoTo ErrRtn
    
    Dim sFNo    As String
    Dim aData() As String
    Dim tmpBarCd$, tmpKind$
    Dim tmpIFCd$, tmpRst1$, tmpRst2$, tmpFlag$, tmpUnit$
    Dim sRstType$
    
    sFNo = Mid$(RcvBuffer, 1, 3)
    
    Select Case sFNo
        Case "F10"      'ORDER REQUEST
            m_iPhase = 2
            
            aData() = Split(RcvBuffer, ",")
            tmpBarCd = Trim(aData(1))
        ...
        Case Else
            If m_iPhase = 3 Then    'Result Data
                aData() = Split(RcvBuffer, ",")
                
                sRstType = Trim(aData(0))
                If Left(sRstType, 1) = "3" Then
                    tmpKind = "QC"
                End If
                
                tmpBarCd = Trim(aData(2))
                tmpIFCd = Trim(Val(aData(3)))
                tmpRst1 = Trim(aData(4))
                tmpUnit = Trim(aData(5))
...
Private Sub SendOrder_LX2200()
    ...
    For ii = 1 To pSampleInfo.ORDCNT
        If Trim$(pSampleInfo.IFCD(ii)) <> "" Then
            Mid$(sTestCd, Val(pSampleInfo.IFCD(ii)), 1) = "1"
        End If
    Next ii
    
    'Send Message 편집
    SendBuf = Chr$(2)
    SendBuf = SendBuf & Space(4) & "," & Right(Space(14) & pSampleInfo.ID, 14)
