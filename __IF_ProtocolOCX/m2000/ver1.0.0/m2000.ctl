...
Private Sub SendOrder_Architect_Old()
...
'            Else
'                'Order 없는 경우...
'                sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||" & TestDat & "|||||||C||||||||||||||Q" & Chr(13) & Chr(3)
''                '2004/5/4 yk
...
'    End If
End Sub
Private Sub SendOrder_Architect_화순()
'    On Error GoTo ErrRtn
'
'    Dim sTmp    As String
'    Dim ChkS    As String
...
'            sTmp = m_iFrameN & "H|\^&||||||||||P|1" & Chr(13) & Chr(3)
'
'            '### Start Add : Rack Barcode -> Sample Barcode
'            msRackBC = pSampleInfo.ID
'            pSampleInfo.ID = GetTSMBarcode(msRackBC, 0)
'
'             '----- 검사항목 조회
'            msSampBC = pSampleInfo.ID
'
'            RaiseEvent RequestCurOrder(pSampleInfo.ID, "", "")
'
'            Call Get_OrderString
...
'            msPINFO = ""
'
'            msPINFO_1 = Left(Trim(pSampleInfo.PATINFO), 20)
'            msPINFO_2 = Left(Trim(pSampleInfo.SAMPINFO), 20)
'            msPINFO_3 = Mid(Trim(pSampleInfo.SAMPINFO), 21, 12)
'
'            If msPINFO_1 <> "" Then
'                msPINFO = msPINFO_1
'
'                If msPINFO_2 <> "" Then
'                    msPINFO = msPINFO_1 & "^" & msPINFO_2
'
'                    If msPINFO_3 <> "" Then
'                        msPINFO = msPINFO_1 & "^" & msPINFO_2 & "^" & msPINFO_3
'                    End If
'                End If
'            End If
'
'            '장비에서 실제 넘어온 바코드로 변경
'            If pSampleInfo.ID <> "" And pSampleInfo.ID <> "0" Then
'                pSampleInfo.ID = msRackBC
'            End If
'
'            msTestDat = ""
'
'            For i = 1 To pSampleInfo.ORDCNT
'                msTestDat = msTestDat & "^^^" & pSampleInfo.IFCD(i) & "\"
'            Next i
'
'            If pSampleInfo.ORDCNT > 0 Then
'                msTestDat = Left(msTestDat, Len(msTestDat) - 1)       '"\" Cutting
'            End If
'            '### End   Add : Rack Barcode -> Sample Barcode
'
'            If Trim(msTestDat) = "" Then
'                m_iSendPhase = 3
'            Else
'                m_iSendPhase = 2
'            End If
'
'        Case 2      'Patient Record
'            sTmp = m_iFrameN & "P|1||" & Trim(msSampBC) & "||" & msPINFO & "|||" & Chr(13) & Chr(3)
'
'            m_iSendPhase = 3
'
'        Case 3      'Order Record
...
'            If Trim(msTestDat) = "" Then
'                'Order 없는 경우...
'                '>>> EDIT BY BYK (2004-05-04)
'                'sTmp = m_iFrameN & "O|1|" & Trim(msRackBC) & "||" & msTestDat & "|||||||C||||||||||||||Q" & Chr(13) & Chr(3)
'                sTmp = m_iFrameN & "Q|1|^" & Trim(msRackBC) & "||^^^ALL||||||||X" & Chr(13) & Chr(3)
'                '<<< EDIT BY BYK (2004-05-04)
'            Else
'                sTmp = m_iFrameN & "O|1|" & Trim(msRackBC) & "||" & msTestDat & "|||||||N||||||||||||||Q" & Chr(13) & Chr(3)
'            End If
'
'            m_iSendPhase = 4
'
...
End Sub
Private Sub PhaseCfg_Protocol_m2000()
        ...
        Select Case m_iPhase
            ...
            Case 2      '<LF> 대기
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        Call DataEditResponse_m2000     'Data 편집
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_m2000()
    On Error GoTo ErrRtn
...
'''                    Exit Sub
                Case "F"
                    ...
                    If tmpRst = "Not detected" Then
                        If InStr(UCase(tmpIFCd), "HBV 0.2") > 0 Then
'                            tmpRst = "≤ 50"
                            tmpRst = "< 51 , < 15"
                        ElseIf InStr(UCase(tmpIFCd), "HBV 0.5") > 0 Then
'                            tmpRst = "< 50"
                            tmpRst = "< 51 , < 15"
                        Else
                            'tmpRst = "< 30"
                            tmpRst = "< 12"
                        End If
                    ElseIf tmpRst = "< 30" Then
                    
                    ElseIf tmpRst = "< 12" Then
                    
                    ElseIf tmpRst = "< 51" Then
                        If InStr(UCase(tmpIFCd), "HBV") > 0 Then
'                            tmpRst = "≤ 50"
                            tmpRst = "< 51 , < 15"
                        End If
                    ElseIf tmpRst = "< 34" Then
                        If InStr(UCase(tmpIFCd), "HBV") > 0 Then
'                            tmpRst = "< 50"
                            tmpRst = "< 51 , < 15"
                        End If
                    Else
'                        tmpRst = Mid$(tmpRst, 1, 1) & "." & Mid$(tmpRst, 2, 2) & " X 10(" & CStr(Len(tmpRst) - 1) & ")"
                        '<S--- 결과값에 부등호 들어간 경우 처리 추가...2007/7/9 yk
                        sFlag = ""
                        sIML = ""
                        If InStr(UCase(tmpIFCd), "HBV") > 0 Then
                            If Left(Trim(tmpRst), 1) = ">" Or Left(Trim(tmpRst), 1) = "<" Then
                                sFlag = Left(Trim(tmpRst), 1)
                                tmpRst = Trim(Mid(tmpRst, 2))
                            End If
                            
                            sIML = Val(tmpRst) / 3.41
                            If InStr(sIML, ".") > 0 Then
                                sIML = Left(sIML, InStr(sIML, ".") - 1)
                            End If
                            
                            sIML = Format$(Mid$(sIML, 1, 1) & "." & Mid$(sIML, 2, 2), "#0.0") & " X 10(" & CStr(Len(sIML) - 1) & ")"
'                            tmpRst = Format$(Mid$(tmpRst, 1, 1) & "." & Mid$(tmpRst, 2, 2), "#0.0") & " X 10(" & CStr(Len(tmpRst) - 1) & ")"
                            tmpRst = Format$(Mid$(tmpRst, 1, 1) & "." & Mid$(tmpRst, 2, 2), "#0.0") & " X 10(" & CStr(Len(tmpRst) - 1) & ")" & " , " & sIML
                            tmpRst = Trim(sFlag & " " & tmpRst)
                        Else
                            If Left(Trim(tmpRst), 1) = ">" Or Left(Trim(tmpRst), 1) = "<" Then
                                sFlag = Left(Trim(tmpRst), 1)
                                tmpRst = Trim(Mid(tmpRst, 2))
...
'                            tmpRst = Mid$(tmpRst, 1, 1) & "." & Mid$(tmpRst, 2, 2) & " X 10(" & CStr(Len(tmpRst) - 1) & ")"
                    ...
                    With pResultInfo
                        ...
                        '결과값 누적
                        .RSTCNT = 2
                        .IFCD = tmpIFCd & Chr(124) & tmpIFCd & "Q" & Chr(124)
                        .RST1 = tmpRst & Chr(124) & tmpRst & Chr(124)
                        .RST2 = "" & Chr(124) & Chr(124)
                        .UNIT = "" & Chr(124) & Chr(124)
                        .FLAG = "" & Chr(124) & Chr(124)
                    End With
                Case "I"
