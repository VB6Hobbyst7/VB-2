...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "UBIT"
            Call PhaseCfg_Protocol_UBiT_IR300
...
End Sub
Private Sub PhaseCfg_Protocol_UBiT_IR300()
...
End Sub
Private Sub DataEdit_UBiT_IR300()
    ...
    '실제 전송되는 결과값
    'D   12007-11-3013:5715              -0.2- 2.3 2.5 2.500
    ...
    tmpSeq = Trim(Mid(RcvBuffer, 2, 4))
    
    tmpBarCd = Trim(Mid(RcvBuffer, 21, m_iBCLen))
            
    '--- 결과편집 저장
    tmpIFCd = "CO2"
    
    sGbn = Trim(Mid(RcvBuffer, 41, 2))
    
    If sGbn = "-" Then
        tmpRst = "Negative(" & Trim(Mid(RcvBuffer, 36, 5)) & ")"
    ElseIf sGbn = "+" Then
        tmpRst = "Positive(" & Trim(Mid(RcvBuffer, 36, 5)) & ")"
    End If

    tmpRst2 = Trim(Mid(RcvBuffer, 41, 2))
