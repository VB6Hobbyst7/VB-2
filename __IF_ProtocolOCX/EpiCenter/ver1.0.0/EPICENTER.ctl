...
Begin VB.UserControl EPICENTER 
   ClientHeight    =   3150
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   3330
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "EPICENTER"
            Call PhaseCfg_Protocol_EpiCenter
...
Private Sub PhaseCfg_Protocol_EpiCenter()
        ...
        Select Case m_iPhase
            ...
            Case 2
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        If bEndChk = True Then
                            Call DataEditResponse_EpiCenter
                            RcvBuffer = ""
                        End If
                        msComm.Output = Chr(6)
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_EpiCenter()
    On Error GoTo ErrRtn
    ...
    Dim sBottle As String
    Dim sBKind As String
    ...
    Select Case RecType
        Case "H"        'Header Record
            ...
            '1H|\^&|||Becton Dickinson||||||||V1.0|20120328155258
            sState = ""
            
            tmpData() = Split(RcvBuffer, "|")
            
            If UBound(tmpData) > 3 Then
                'Sender, Version 바뀔 수 있음...
                pSampleInfo.SENDER = Trim(tmpData(4))
                pSampleInfo.VERSION = Trim(tmpData(12))
            End If
        ...
        Case "R"        'Result Record
            'R|1|^^^GND^446543288551|INST_NEGATIVE|||||P|||20110702013449|20110707014319|BACTECFX^65^5^01^A-B01
            tmpData() = Split(RcvBuffer, "|")

            tmpField = Split(tmpData(2), "^")
            tmpIFCd = Trim(tmpField(3))
            sBottle = Trim(tmpField(4)) 'Bottle Number
            
            tmpRst1 = Trim(tmpData(3))
            If InStr(tmpRst1, "^") > 0 Then
                tmpField = Split(tmpRst1, "^")
                tmpRst1 = Trim(tmpField(0))
                tmpRst2 = Trim(tmpField(1))
            End If
            
            sSDate = Trim(tmpData(11))  '배양시작일시
            sEDate = Trim(tmpData(12))  '배양완료일시(Negative, Growth)
            
            tmpField = Split(tmpData(13), "^")
            sBKind = Trim(tmpField(1))  'Bottle 종류 (92 : 호기, 65 : 혐기, 94 : 소아)
            
            If UBound(tmpField) > 4 Then    'Position(Location)정보가 안넘어오는 현상 처리..
                tmpPos = Trim(tmpField(4))
            End If
            ...
            With pResultInfo
                ...
                .UNIT = .UNIT & "" & Chr(124)
                .FLAG = .FLAG & "" & Chr(124)
                .RSTDT = .RSTDT & sSDate & "^" & sEDate & Chr(124)  '결과일시(2005/6/10) yk
            End With
...
'   환자 Order 전송
'
Private Sub SendOrder_EpiCenter()
    ...
    Do
        Select Case m_iSendPhase
            Case 1
                'Header Record
                sSendBuff = "1H|\^&|||" & pSampleInfo.SENDER & "||||||||" & pSampleInfo.VERSION & "|" & Format(Now, "yyyyMMddHHmmss") & Chr(13)
            ...
            Case 2
                ...
                If Trim(pSampleInfo.SEX) = "" Then
                    pSampleInfo.SEX = "U"
                End If
                
                If pSampleInfo.DEPT = "" Then
                    sDept = pSampleInfo.WARD
                Else
                    sDept = pSampleInfo.DEPT
                End If
                
                If pSampleInfo.BIRTH <> "" And IsDate(pSampleInfo.BIRTH) Then
                    pSampleInfo.BIRTH = Format(pSampleInfo.BIRTH, "yyyyMMdd")
                End If
                
                'Patient Record
                sSendBuff = sSendBuff & "P|1||" & pSampleInfo.PATNO & "||" & pSampleInfo.NAME & "^^^^||" & pSampleInfo.BIRTH & "|" & pSampleInfo.SEX & "||^^^^||||^^^^|||||^^^^||||" & _
                                        Format(Now, "yyyyMMddHHmmss") & "||" & pSampleInfo.ROOM & "|||||||" & sDept & "|" & Chr(13)
    
                m_iSendPhase = 3
                
            Case 3
                If pSampleInfo.SPCCD = "" Then
                    pSampleInfo.SPCCD = "BLOOD"
                End If
                
                sSendBuff = sSendBuff & "O|1|" & pSampleInfo.ID & "^^^||^^^^|||" & Format(Now, "yyyyMMddHHmmss") & "||||||^^|" & Format(Now, "yyyyMMddHHmmss") & _
                                        "|" & pSampleInfo.SPCCD & "^||^^||||||^|||||" & Chr(13)
                                        
                m_iSendPhase = 4
                       
           Case 4
                sSendBuff = sSendBuff & "L|1|F" + Chr(13) + Chr(3)
