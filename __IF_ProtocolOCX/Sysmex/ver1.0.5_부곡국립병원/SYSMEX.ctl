...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "XT1800I"
            Call PhaseCfg_Protocol_XT1800i  '바코드 사용
            
        Case "KX21"
            Call PhaseCfg_Protocol_KX21     '바코드 사용
...
Private Sub PhaseCfg_Protocol_XT1800i()
...
Private Sub DataEdit_XT1800i()
    ...
    Dim i%
    Dim tmpRst(1 To 32)    As String       '결과 임시 저장
            ...
            Select Case sLC
                Case "U"
                    If Len(RcvBuffer) > 253 Then
                        MsgBox "XT1800i 로부터 전송된 문자열의 길이 (" & Len(RcvBuffer) & ")의 이상이 발생하였습니다!!"
                    End If
        ...
        Case "D2"
            Select Case sLC
                Case "U"
                    ...
                    pResultInfo.ID = Trim(Mid(RcvBuffer, 22, 13))
                    
                    Erase tmpRst
                    
                    '1) gIFItem.s08(인터페이스 장비코드)을 전송순서와 Matching
                    'WBC
                    ...
                    Else
                        ...
                        '''tmpRst(2) = Val(tmpRst(2)) * 100
                    End If
                    
                    'HGB, HCT, MCV, MCH, MCHC
                    For i = 3 To 7
                        tmpRst(i) = Mid(RcvBuffer, 59 + (i - 3) * 5, 4)
                        
                        If tmpRst(i) = Space(4) Then
                            tmpRst(i) = "N"
                        Else
'                            Select Case i
'                            Case 4, 5
'                                tmpRst(i) = Format(Val(Format(tmpRst(i), "@@@.@")), "0")
'                            Case Else
                                tmpRst(i) = Format(Val(Format(tmpRst(i), "@@@.@")), "0.0")
'                            End Select
                        End If
                    Next
                    ...
                    Else
                        ...
                        '''tmpRst(8) = Val(tmpRst(8)) * 1000
                    End If
                    
                    'LYMPH%, MONO%, NEUT%, EO%, BASO%
                    For i = 9 To 13
                        tmpRst(i) = Mid(RcvBuffer, 89 + (i - 9) * 5, 4)
                    ...
                    'LYMPH#, MONO#, NEUT#, EO#, BASO#
                    For i = 14 To 18
                        tmpRst(i) = Mid(RcvBuffer, 114 + (i - 14) * 6, 5)
                        
                        If tmpRst(i) = Space(5) Then
                            tmpRst(i) = "N"
                        Else
                            tmpRst(i) = Format(Val(Format(tmpRst(i), "@@@.@@")), "0.00")
                        End If
                    Next
                                        
                    'RDW-CV(%), RDW-SD(fL), PDW(fL), MPV(fL), P-LCR
                    For i = 19 To 23
                        tmpRst(i) = Mid(RcvBuffer, 144 + (i - 19) * 5, 4)
...
''''                    End If
                    ...
                    'IRF, LFR, MFR, HFR
                    For i = 26 To 29
                        tmpRst(i) = Mid(RcvBuffer, 179 + (i - 26) * 5, 4)
...
''''                    End If
                    ...
                    With pResultInfo
                        ...
                        For i = 1 To 30
                            .IFCD = .IFCD & CStr(i) & Chr(124)
                            .RST1 = .RST1 & tmpRst(i) & Chr(124)
                            .RST2 = .RST2 & Chr(124)
                            .UNIT = .UNIT & Chr(124)
                            .FLAG = .FLAG & Chr(124)
                            .RSTCNT = .RSTCNT + 1
        ...
        Case "D1"
            Select Case sLC
                Case "U"
                    ...
                    tmpBarCd = Trim(Val(Mid(RcvBuffer, 10, 13)))
                    ...
                    If Mid(RcvBuffer, 29, 1) = "S" Then
                        iTmp = 15
                    ElseIf Mid(RcvBuffer, 29, 1) = "C" Then
                        iTmp = 16
                    Else
                        iTmp = 0
                    End If
