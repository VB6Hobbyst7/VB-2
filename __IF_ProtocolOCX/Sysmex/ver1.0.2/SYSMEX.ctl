...
Private Sub PhaseCfg_Protocol_K1000()
        ...
        Case "D1"
            Select Case sLC
                Case "U"
                    ...
                    tmpSeqNo = Trim(Mid(RcvBuffer, iPos, 12))
                    iPos = iPos + 12
                    
                    ReDim tmpRst(12) As String
                    ReDim tmpFlag(12) As String
                    
                    iPos = iPos + 6
                    
                    'RDW Selection Infomation(C:RDW-CV, S:RDW-SD)
                    sRDWInfo = Mid(RcvBuffer, iPos, 1)
                    iPos = iPos + 1
                    
                    
                    'WBC
                    tmpRst(1) = Mid(RcvBuffer, iPos, 4)
                    If tmpRst(1) = Space(4) Then
                        tmpRst(1) = "N"
                    Else
                        tmpRst(1) = Format(Val(Format(tmpRst(1), "@@@.@")), "0.0")
                    End If
                    tmpFlag(1) = Mid(RcvBuffer, iPos + 4, 1)
                    iPos = iPos + 5
                    
                    'RBC
                    tmpRst(2) = Mid(RcvBuffer, iPos, 4)
                    If tmpRst(2) = Space(4) Then
                        tmpRst(2) = "N"
                    Else
                        tmpRst(2) = Format(Val(Format$(tmpRst(2), "@@.@@")), "0.00")
                    End If
                    tmpFlag(2) = Mid(RcvBuffer, iPos + 4, 1)
                    iPos = iPos + 5
                    
                    'HGB, HCT, MCV, MCH, MCHC
                    For ii = 3 To 7
                        tmpRst(ii) = Mid(RcvBuffer, iPos, 4)
                        If tmpRst(ii) = Space(4) Then
                            tmpRst(ii) = "N"
                        Else
                            tmpRst(ii) = Format(Val(Format(tmpRst(ii), "@@@.@")), "0.0")
                        End If
                        tmpFlag(ii) = Mid(RcvBuffer, iPos + 4, 1)
                        iPos = iPos + 5
                    Next ii
                    
                    'PLT
                    tmpRst(8) = Mid(RcvBuffer, iPos, 4)
                    If tmpRst(8) = Space(4) Then
                        tmpRst(8) = "N"
                    Else
                        tmpRst(8) = Trim(Val(Format(tmpRst(8), "@@@@")))
                    End If
                    tmpFlag(8) = Mid(RcvBuffer, iPos + 4, 1)
                    iPos = iPos + 5
                    
                    'W-SCR%...등
                    iPos = iPos + (6 * 5)
                                                            
                    'RDW-CV(%) or RDW-SD(fL), PDW(fL), MPV(fL), P-LCR
                    For ii = 9 To 12
                        tmpRst(ii) = Mid(RcvBuffer, iPos, 4)
                        If tmpRst(ii) = Space(4) Then
                            tmpRst(ii) = ""
                        Else
                            tmpRst(ii) = Format(Val(Format(tmpRst(ii), "@@@.@")), "0.0")
                        End If
                        tmpFlag(ii) = Mid(RcvBuffer, iPos, 1)
                        iPos = iPos + 5
                    Next ii
