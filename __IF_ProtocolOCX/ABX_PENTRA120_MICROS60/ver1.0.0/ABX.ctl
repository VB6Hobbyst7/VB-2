...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "PENTRA120", "MICROS60"
            Call PhaseCfg_Protocol_Pentra120
...
Private Sub PhaseCfg_Protocol_Pentra120()
...
Private Sub PhaseCfg_Protocol_G7()
...
Private Sub DataEdit_A1C22()
    ...
    ReDim tmpData(1 To 5) As String

    iPos = 1

    tmpSeqNo = Trim(Mid(RcvBuffer, iPos, m_iSMPLen))    ' 8))
    '2003/4/15 백병원의 경우...
    If Left(tmpSeqNo, 1) = "?" Then
        iPos = iPos + m_iSMPLen - 1
    Else
...
Private Sub DataEdit_Pentra120()
    On Error GoTo ErrRtn
    ...
    aData() = Split(RcvBuffer, Chr(13))
    
    If Left(aData(0), 1) <> "R" Then    '결과 Type 아님
        Exit Sub
    End If
    
    tmpBarCd = Trim(aData(1))       'Barcode
    
    For ii = 4 To 36
        If ii > UBound(aData()) Then
            Exit For
        End If
        
        tmpData = Trim(aData(ii))
        
        If tmpData <> "" Then
            tmpIFCd = Trim(ii - 3)
            tmpRst1 = Trim(Mid(tmpData, 1, 5))
            If IsNumeric(tmpRst1) Then
                tmpRst1 = Val(tmpRst1)
            End If
            tmpRst2 = Trim(Mid(tmpData, 6))
