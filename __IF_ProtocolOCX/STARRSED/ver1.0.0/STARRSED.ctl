...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "STARRSED"
            Call PhaseCfg_Protocol_STARRSED
...
End Sub
Private Sub PhaseCfg_Protocol_STARRSED()
...
Private Sub DataEdit_STARRSED()
    ...
    '3009249113    3    3 CLEAR       101   60   20                        EDTA
    
    ''-----------
    '    sJNo = "0" & Trim(Mid(RcvBuffer, 1, 10))
    '    sPos = Trim(Mid(RcvBuffer, 34, 3))           'Pipette number
    '
    '    iRstCnt = 5
    '    sTIFRstCd = sTIFRstCd & "1|2|3|4|5|"
    '
    '    sTmp = Trim(Mid(RcvBuffer, 13, 3))
    '    Call JudgeResult1("1", sTmp, sTRst2)
    '    sTRst2 = sTRst2 & "|"
    '
    '    sTRst = sTRst & Trim(sTmp) & "|"  'ESR
    '    sTRst = sTRst & Trim(Mid(RcvBuffer, 18, 3)) & "|"  'Corrected for temp
    '    sTRst = sTRst & Trim(Mid(RcvBuffer, 22, 9)) & "|"  'Aspect
    '    sTRst = sTRst & Trim(Mid(RcvBuffer, 45, 2)) & "|"  'Temp
    '    sTRst = sTRst & Trim(Mid(RcvBuffer, 48, 33)) & "|" 'Message
    ''-----------

    tmpBarCd = Trim(Mid(RcvBuffer, 1, 10))
    
    '--- 결과편집 저장
    tmpIFCd = "ESR"
    tmpRst = Trim(Mid(RcvBuffer, 18, 3))
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("DataEdit_STARRSED 오류발생 - " & Err.Description)
    End If
End Sub
