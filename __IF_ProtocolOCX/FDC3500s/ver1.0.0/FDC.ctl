...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "DRICHEM3500S"
            Call PhaseCfg_Protocol_Dri_Chem3500s
            
        Case "DRICHEM3000"
            Call PhaseCfg_Protocol_Dri_Chem3000
...
Private Sub PhaseCfg_Protocol_Dri_Chem3500s()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            ...
            Case 3      'ETX 수신
                Call DataEditResponse_Dri_Chem3500s
...
Private Sub PhaseCfg_Protocol_Dri_Chem3000()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            Case 12      ' 수신
                Call DataEditResponse_Dri_Chem3000
                RcvBuffer = ""
...
Private Sub DataEditResponse_Dri_Chem3500s()
    On Error GoTo ErrRtn

    Dim ii%, iRstCnt%, iPos%
    Dim tmpSeq$, tmpIFCd$, tmpRst$, tmpUnit$, tmpFlag$, sDemographics$, sTestInfo$
        
    iPos = 23
    
    tmpSeq = Mid(RcvBuffer, iPos, 1)
    pResultInfo.SEQNO = Trim(tmpSeq)
...
Private Sub DataEditResponse_Dri_Chem3000()
    On Error GoTo ErrRtn
    ...
    iPos = InStr(RcvBuffer, "NO.")
    iPos = iPos + 3
        
    tmpSeq = Trim(Mid(RcvBuffer, iPos, 5))
    pResultInfo.SEQNO = Trim(tmpSeq)
    
    iPos = InStr(RcvBuffer, """") + 1
    
    sTestInfo = Mid(RcvBuffer, iPos)
    aData() = Split(sTestInfo, Chr(10))
    aField() = Split(sTestInfo, vbTab)
    
    For ii = 0 To UBound(aData) - 1
        aField() = Split(aData(ii), vbTab)
        
        tmpIFCd = Trim(aField(1))
        tmpRst = Trim(aField(4))
