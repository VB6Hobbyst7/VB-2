...
''Private Sub PhaseCfg_Protocol_GammaPro()
''
''    Dim wkDat   As String
''    Dim ix1     As Integer
''
...
''
''            Case 3      'ETX
''                Call DataEditResponse_GammaPro
''
''                msComm.Output = Chr(6)
''
''            Case 4      'EOT
...
Private Sub DataEditResponse_GammaPro()
    On Error GoTo ErrRtn

    Dim sMark$, sPNo$, sPID$
    Dim sRstData$
    Dim tmpIFCd$, tmpRst1$, tmpRst2$, tmpFlag$, tmpUnit$
    Dim tmpSvrCd$, tmpPatNm$, tmpSex$, tmpAge$
    Dim i%, iRstCnt%, iPos%
    Dim aRstData$()
    
    sPID = MidH(RcvBuffer, 1, 32)
    tmpSvrCd = MidH(RcvBuffer, 33, 32)
    tmpPatNm = MidH(RcvBuffer, 65, 32)
    tmpSex = MidH(RcvBuffer, 97, 1)
    tmpAge = MidH(RcvBuffer, 98, 16)
    
    sRstData = MidH(RcvBuffer, 114, LenH(RcvBuffer) - 114)
    
    iRstCnt = Len(sRstData) / 48
    
    iPos = 1
    
    For i = 0 To iRstCnt - 1
        tmpIFCd = MidH(sRstData, iPos, 16)
        tmpRst1 = MidH(sRstData, iPos + 16, 16)
        tmpUnit = MidH(sRstData, iPos + 16, 16)
        ...
        With pResultInfo
            ...
            .RST2 = .RST2 & "" & Chr(124)
            .UNIT = .UNIT & Trim(tmpUnit) & Chr(124)
            .FLAG = .FLAG & "" & Chr(124)
        End With
...
Private Sub SendOrder_GammaPro()
...
'    Dim sCS$
    ...
    For ii = 1 To pSampleInfo.ORDCNT
        sTestCd = sTestCd & Left(pSampleInfo.IFCD(ii) & Space(16), 16)
    Next ii

    'Send Message 편집
    SendBuf = Chr(2)
    SendBuf = SendBuf & Left(pSampleInfo.ID & Space(32), 32)
    SendBuf = SendBuf & Left(pSampleInfo.SEQNO & Space(32), 32) '등록번호 or 과
    SendBuf = SendBuf & LeftH(pSampleInfo.RACK & Space(32), 32)  '이름
    SendBuf = SendBuf & Left(pSampleInfo.POS & Space(1), 1)   '성별
    SendBuf = SendBuf & Left("" & Space(16), 16)
    SendBuf = SendBuf & sTestCd
    
    msComm.Output = SendBuf & Chr(3)
    ...
    'Log 작성
    If m_sTestMode = "77" Then
        RaiseEvent PrintSendLog(SendBuf & Chr(3))
    End If
...
End Function
Private Function ChkSum_RIAmat_test(ByVal Para As Variant)
'
'    Dim i   As Integer
'    Dim Tmp 'As Integer
'    Dim ChkS1   'As Integer
...
''    Dim bByte() As Byte
''
''    For ix1 = 0 To LenB(Para) - 1
''        Debug.Print Chr(CStr(Para(ix1)))
'''        Select Case Para(ix1)
'''                RcvBuffer2 = RcvBuffer2 + ChrB(vwkBuf(ix1))
'''
'''        End Select
''    Next ix1
'''---------------
...
'
'
''        Tmp = AscB(MidB$(Para, i, 1))
'        Tmp = Tmp + AscB(MidB(Para, i, 1))
...
'
''    Tmp = Right("00" & Tmp, 2)
''    ChkS1 = Hex(AscB(MidB(Tmp, 1, 1)))
''    ChkS2 = Hex(AscB(MidB(Tmp, 3, 1)))
''
''    sC1 = CDec("&H" & ChkS1)
''    sC2 = CDec("&H" & ChkS2)
'
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "GAMMAPRO"
            Call PhaseCfg_Protocol_GammaPro
