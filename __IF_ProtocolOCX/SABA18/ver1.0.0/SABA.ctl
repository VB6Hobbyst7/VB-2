...
Private Sub PhaseCfg_Protocol_SABA18()
...
''    '검사항목 조회
    ...
    If pSampleInfo.ORDCNT > 0 Then
        ...
        'ORDER 전송
        sSendBuff = "P" & Right("00000000" & CStr(Val(Trim(pSampleInfo.POS))), 8) & Space(30)
        sSendBuff = sSendBuff & Mid(sTestDat, 1, 50)
            
        msComm.Output = Chr$(2) & sSendBuff & Chr$(3) & SABA18_CheckSum(sSendBuff)
            
        If m_sTestMode = 77 Then
            RaiseEvent PrintSendLog(Chr$(2) & sSendBuff & Chr$(3) & SABA18_CheckSum(sSendBuff))
        End If
...
Private Sub DataEditResponse_SABA18()
    On Error GoTo ErrRtn
    ...
    Dim tmpIFCd$, tmpRst$, tmpData$, tmpFlag$
    ...
    Select Case sType
        ...
        Case "R"
            ...
            sBarCd = Trim(Mid(RcvBuffer, iPos, 8))
            
            iPos = iPos + 38
            
            Do
                tmpData = Mid(RcvBuffer, iPos, 10)
                
                If tmpData = "" Then
                    Exit Do
                End If
                
                tmpIFCd = Trim(Mid(tmpData, 1, 2))
                tmpRst = Trim(Mid(tmpData, 3, 7))
                tmpFlag = Trim(Mid(tmpData, 10, 1))
                ...
                With pResultInfo
                    .ID = sBarCd
                    .RSTCNT = .RSTCNT + 1
                    .IFCD = .IFCD & CStr(Val(tmpIFCd)) & Chr(124)
...
Private Function SABA18_CheckSum(ByVal sBuf$) As String
    Dim iCnt%
    Dim iSum%
    ...
    'SABA18_CheckSum = Right("0" & CStr(Hex(iSum)), 2)
    SABA18_CheckSum = Right("0" & CStr(Hex(256 - (iSum Mod 256))), 2)
End Function
