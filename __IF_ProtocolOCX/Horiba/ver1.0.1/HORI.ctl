...
End Sub
Private Sub PhaseCfg_Protocol_HORIBA()
...
Private Sub DataEdit_HORIBA()
    On Error GoTo ErrRtn
    
    Dim tmpBarCd$, tmpRack$, tmpPos$, tmpSeq$
    Dim tmpIFCd$, tmpRst$
    Dim iPos    As Integer
    Dim i       As Integer
    ...
    i = 0
    sField = Split(RcvBuffer, Chr(13))
    '! 009.8
    Do
        sTmp = sField(i)
        sTmp = Trim(sTmp)
                
        If UCase(Left(sTmp, 1)) = "S" Then
            tmpSeq = Mid(sTmp, 3, 4)
            
        ElseIf i > 8 Then
            tmpIFCd = Split(sTIFCD, Chr(124))(iRstCnt)
            tmpRst = CStr(Val(Trim(Mid(sTmp, 3, 5))))
