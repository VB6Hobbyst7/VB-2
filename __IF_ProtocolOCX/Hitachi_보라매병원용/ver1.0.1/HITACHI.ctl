...
End Sub
'
'   HITACHI7600 바코드 사용 안하는 경우...(SampleNo 로 오더 전송)
'
        ...
        Case 1
            ...
            For iCnt = 1 To pSampleInfo.ORDCNT
                Select Case Trim$(pSampleInfo.IFCD(iCnt))
                    Case "961"
                        '--- TIBC인 경우 UIBC,Fe의 Order를 준다.(보라매병원용 임시)
                        If Val(InStr(1, sSendBuff, "21/")) = 0 Then  'UIBC
                            sSendBuff = sSendBuff & "^^^21/\"
                        End If
                        If Val(InStr(1, sSendBuff, "20/")) = 0 Then  'Fe
                            sSendBuff = sSendBuff & "^^^20/\"
                        End If
                        '--------------------------------------
                    Case Else
                        '정상 오더
