...
'이벤트 선언:
Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sKind$, sOther1$)
...
Private Sub DataEditResponse_AX4030()
    ...
    Dim tmpIFCd$, tmpRst$, tmpRst2$, tmpUnit$, tmpFlag$, tmpType$, tmpErr$
    ...
    Select Case RecType
        ...
        Case "Q"        'Order Request Record
            'Q|1|45645645645645 ^001-01||||||||||O<CR>
            ...
            tmpData() = Split(tmpField(1), "-")
            tmpRack = Trim(tmpData(0))
            tmpPos = Trim(tmpData(1))
        ...
        Case "O"
            '0   1 2                         3    4...
            '3O|1|a51823100010a     ^001-01|0001|^^^GLU $^^^PRO $^^^BIL $^^^URO $^^^PH  $^^^BLD $^^^KET $^^^NIT $^^^LEU $^^^TURB$^^^S.G.$^^^COLOR|R||||||||||S|||10EA   ^29|^
            '3O|1|------------------^001-08|0001|^^^GLU $^^^PRO $^^^BIL $^^^URO $^^^PH  $^^^BLD $^^^KET $^^^NIT $^^^LEU $^^^TURB$^^^S.G.$^^^COLOR|R||||||||||CNT^L|||10EA   ^29|^
            tmpSeqNo = "": tmpBarCd = "": tmpRack = "": tmpPos = ""
            tmpField() = Split(RcvBuffer, "|")
            
            tmpSeqNo = Trim(tmpField(3))
            
            tmpErr = Trim(tmpField(19))
            If InStr(tmpErr, "^") > 0 Then
                Erase tmpData()
                tmpData() = Split(tmpErr, "^")
                
                tmpErr = Trim(tmpData(0))
            End If
            
            If InStr(1, tmpField(2), "^") > 0 Then
                Erase tmpData()
                tmpData() = Split(tmpField(2), "^")
            
                tmpBarCd = Trim(tmpData(0))
                If InStr(tmpBarCd, "----------") > 0 Then
                    tmpBarCd = ""
                End If
            ...
            tmpType = Trim(tmpField(15))
            If tmpType = "S" Then
                tmpType = ""
            Else
                tmpBarCd = tmpType: tmpType = "QC"
            End If
            ...
            pSampleInfo.RACK = tmpRack
            pSampleInfo.POS = tmpPos
            pSampleInfo.KIND = tmpType
            pSampleInfo.OTHER = tmpErr

        Case "R"        'Result Record
            '4R|1|^^^GLU |-     ^|mg/dL ||||F|||201006141031
            '4R|1|^^^GLU |4+    ^1000|mg/dL ||*||F|||201006141222
            '7R|4|^^^URO |NORMAL^|mg/dL ||||F|||201006141031
            '0R|5|^^^PH  |^5.5|||||F|||201006141031
            ...
            If ii <> 0 Then
                Erase tmpData()
                tmpData() = Split(tmpRst, "^")
            
                Select Case tmpIFCd
                    Case "PH", "S.G.", "COLOR"
                        tmpRst = Trim(tmpData(1))
                        If InStr(tmpRst, "---") > 0 Then
                            tmpRst = ""
                        End If
...
'                        End If
            ...
            With pResultInfo
                ...
                .OTHER = pSampleInfo.OTHER      'Measurement Error
        ...
        Case "L"
            ...
            With pResultInfo
                If .RSTCNT > 0 Then
                    RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .UNIT, .FLAG, .KIND, .OTHER)
                End If
            End With
...
Private Sub SendOrder_AX4030()
    ...
    Select Case m_iSendPhase
        Case 1
            'Header Record
            sSendBuff = m_iFrameN & "H|$^&|||AX-4030^40910037^V01.15    |||||||||" & Format(Now, "yyyymmddHHMMSS") & vbCr & Chr(23)
            m_iSendPhase = 2
            
        Case 2
            'Patient Record
            sSendBuff = m_iFrameN & "P|1" & vbCr & Chr(23)
            m_iSendPhase = 3
        ...
        Case 3
            ...
            sSendBuff = m_iFrameN & "O|1|" & Left(Trim(pSampleInfo.ID) & Space(18), 18) & "||"
                    
            '검사오더는 NO1, NO2, 공백 셋중 하나다.
            If pSampleInfo.ORDCNT = 0 Then
                sSendBuff = sSendBuff & "|S" & vbCr & Chr(23)
            Else
                sSendBuff = sSendBuff & "^^^" & pSampleInfo.IFCD(1) & "|S" & vbCr & Chr(23)
            End If
        ...
        Case 5      'EOT
            ...
            Else
                ...
                Call Init_pResultInfo
                RaiseEvent SendOrderOK("", pSampleInfo.SEQNO, pSampleInfo.RACK, pSampleInfo.POS)
            End If
    ...
    msComm.Output = Chr(2) & sSendBuff & ChkSum & Chr(13) & Chr(10)
    
    m_iFrameN = m_iFrameN + 1

    If m_sTestMode = "77" Then
        RaiseEvent PrintSendLog(Chr(2) & sSendBuff & ChkSum & Chr(13) & Chr(10))
    End If
