...
End Sub
Private Sub PhaseCfg_Protocol_UroMeter720()
...
End Sub
Private Sub PhaseCfg_Protocol_NanoScan700()
...
ErrRtn:
    ...
    End If
End Sub
Private Sub DataEdit_UroMeter720()
    On Error GoTo ErrRtn
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("DataEdit_UroMeter720 Error - " & Err.Description)
    End If
End Sub
Private Sub DataEdit_NanoScan700()
    On Error GoTo ErrRtn
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("DataEdit_NanoScan700 Error - " & Err.Description)
''        MsgBox "DataEdit_NanoScan700 Error - " & Err.Description & vbCrLf & "RcvBuffer:" & RcvBuffer
    End If
End Sub
...
ErrRtn:
    ...
    End If
End Sub
Private Sub DataEdit_UriScanSuper()
    On Error GoTo ErrRtn
    ...
    m_p_sTIFCd = "BLD|BIL|URO|KET|PRO|NIT|GLU|p.H|S.G|LEU|VTC|COL|CLA|"
    ReDim sTestCd(13) As String
    sTestCd() = Split(m_p_sTIFCd, Chr(124))
    ...
    'Error Flag
    sInfo = ""
    iPos = InStr(RcvBuffer, "Ward :")
    If iPos <> 0 Then
        Erase aRowData()
        aRowData() = Split(Mid(RcvBuffer, iPos + 6), Chr(13))
        sInfo = Trim(aRowData(0))
    End If
            ...
            If Trim(sInfo) = "" Then
                ...
                Select Case sIFCd
                    Case "p.H", "S.G"
                        sRst1 = Trim$(Mid$(sRstData, 11, 5))
                    Case "COL", "CLA"
                        sRst1 = Trim$(Mid$(sRstData, 5, 11))
                    Case Else
                        sRst1 = Trim$(Mid$(sRstData, 8, 4))
    '                    sRst1 = Trim$(Mid$(sRstData, 11, 5))
                End Select
            Else
                sRst1 = ""
    ...
    With pResultInfo
        ...
        If .RSTCNT > 0 Then
            RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .UNIT, .FLAG, "", "", "", Trim(sInfo))
        End If
    End With
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "UROMETER720"
            Call PhaseCfg_Protocol_UroMeter720
            
        Case "NANOSCAN700"
            Call PhaseCfg_Protocol_NanoScan700
            
        Case "US2100R", "US3100R"
            Call PhaseCfg_Protocol_US2100R
            
        Case "US1000"
            Call PhaseCfg_Protocol_US1000
...
Private Sub PhaseCfg_Protocol_US1000()
...
Private Sub DataEdit_US1000()
    ...
    Dim sUnit   As String
    Dim iPos    As Integer
    Dim sTmpRstData() As String
    
    '결과정보 구조체 초기화
    Call Init_pResultInfo
    
    '설정된 장비코드 편집
    m_p_sTIFCd = "URO|BLD|BID|KET|GLU|PRO|PH|NIT|LEU|SG|"
    ReDim sTestCd(9) As String
    sTestCd() = Split(m_p_sTIFCd, Chr(124))
    
    ReDim sTmpRstData(15) As String
    sTmpRstData() = Split(RcvBuffer, ",")
    
    tmpSeqNo = Mid(sTmpRstData(2), 2, 8)
    
    Dim iRstCnt As Integer
    iRstCnt = 0
    
    For ii = 8 To 17
        sRstData = sTmpRstData(ii)
        
        '' 측정상태 : 0 :측정있음. 1: 측정 없음.
        If Left(sRstData, 1) = "0" Then
            sIFCd = sTestCd(iRstCnt)
            sRst1 = Trim(Mid(sRstData, 3, 6))
            sRst2 = Trim(Mid(sRstData, 9, 6))
            sUnit = ""
