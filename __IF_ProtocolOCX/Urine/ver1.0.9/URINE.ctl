...
Private Sub PhaseCfg_Protocol_UriScan_NEW()
...
Private Sub DataEdit_UriScan()
    ...
    aRowData = Split(RcvBuffer, Chr(13))
    
    For ii = 0 To UBound(aRowData) - 1
        aRowData(ii) = Replace(aRowData(ii), Chr(10), "")
        
        If Trim(aRowData(ii)) <> "" Then
            If ii = 1 Then
                tmpSeqNo = Trim(Mid(aRowData(ii), 7, 4))
                tmpBarCd = Trim(Mid(aRowData(ii), 12))
                
            ElseIf ii >= 4 Then
                sIFCd = Trim$(Mid$(aRowData(ii), 1, 4))
                sRst1 = Trim$(Mid$(aRowData(ii), 5, 11))
...
Private Sub DataEdit_UriScan_NEW()
    On Error GoTo ErrRtn
    ...
    Dim iPos    As Integer
    Dim sType   As String, sTmp As String
    ...
    For ii = 0 To UBound(aRowData) - 1
        ...
        If Trim(aRowData(ii)) <> "" Then
            If InStr(aRowData(ii), ":") > 0 Then
                sType = Trim(Mid(aRowData(ii), 1, InStr(aRowData(ii), ":") - 1))
                sTmp = Trim(Mid(aRowData(ii), InStr(aRowData(ii), ":") + 1))
                ...
                Select Case UCase(sType)
                    ...
                    Case "ID_NO", "CON.NO"
                        If InStr(sTmp, "-") > 0 Then
                            tmpSeqNo = Trim(Split(sTmp, "-")(0))
                            tmpBarCd = Trim(Split(sTmp, "-")(1))
                        Else
                            tmpSeqNo = Trim(Mid(sTmp, 1, 4))
                            tmpBarCd = ""
                        End If
                        
                        If UCase(sType) = "CON.NO" Then
                            sKind = "Q"
                        Else
                            sKind = ""
                        End If
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "URISCAN_NEW"
            Call PhaseCfg_Protocol_UriScan_NEW
