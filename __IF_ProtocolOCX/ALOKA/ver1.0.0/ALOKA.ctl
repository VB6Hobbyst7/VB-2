...
Begin VB.UserControl ALOKA 
   ...
   ScaleHeight     =   3150
   ScaleWidth      =   3330
   Begin MSWinsockLib.Winsock sckServerSnd 
      Index           =   0
      Left            =   1710
      Top             =   2400
      _ExtentX        =   741
      ...
      _Version        =   393216
   End
   Begin MSWinsockLib.Winsock sckServerRcv 
      Index           =   0
      Left            =   1050
      Top             =   2400
      _ExtentX        =   741
...
Dim miDataLen As Integer
Dim miDataCnt As Integer
Dim msProtocalInfo As String
...
Private Sub PhaseCfg_Protocol_ALOKA()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        miDataCnt = miDataCnt + 1
        
        If sState = "" Then
            If miDataCnt = 16 Then
                miDataLen = Val(Mid(RcvBuffer, 3, 6))
                miDataCnt = 0
                sState = "R"
            End If
        End If
        
        If miDataCnt = miDataLen Then
            ''sckServerSnd.SendData Chr(6)
        
            Call DataEditResponse_ALOKA
            RcvBuffer = ""
            miDataLen = 0
            miDataCnt = 0
             sState = ""
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_ALOKA()
    On Error GoTo ErrRtn
    ...
    Dim sUnitCnt As String
    Dim sMachNo As String
    Dim sRackInfo As String
    Dim sDviceInfo As String
    Dim sHeader As String
   
    RecType = Mid$(RcvBuffer, 1, 2)

    Select Case RecType
''        Case "PQ"
''            msProtocalInfo = Mid(RcvBuffer, 3, 24)
''
''            Call Send_ProtocalMsg(msProtocalInfo)
        
        Case "AD"        'Order Request Record
            sHeader = Mid(RcvBuffer, 3, 16)
            
            sckServerSnd.SendData "AK" & sHeader
            
            sMachNo = Mid(RcvBuffer, 17, 2)
            tmpSeqNo = Mid(RcvBuffer, 19, 6)
            sRackInfo = Mid(RcvBuffer, 25, 2)   '00 : 라인 공급 랙
                                                '01 : 응급 검체 투입 랙
            sDviceInfo = Mid(RcvBuffer, 27, 2)  '00 : 정상
                                                '01 : 분리 중
            tmpBarCd = Mid(RcvBuffer, 29, 20)
            ...
            If sDviceInfo = "00" Then
                Call SendOrder_ALOKA(Mid(RcvBuffer, 3))
            Else
...
Public Sub SocketRcv_Open(ByVal iPort As Integer)
    On Error GoTo ErrRtn

    sckServerRcv.LocalPort = iPort
    sckServerRcv.Listen
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("SocketRcv_Open - " & Err.Description)
    End If
End Sub

Public Sub SocketSnd_Open(ByVal iPort As Integer)
    On Error GoTo ErrRtn
    
    sckServerSnd.LocalPort = iPort
    sckServerSnd.Listen
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("SocketSnd_Open - " & Err.Description)
    End If
End Sub
...
'   환자 Order 전송
'
Private Sub SendOrder_ALOKA(ByVal rsQInfo As String)
    ...
    ''sSendBuff = "AI" & rsQInfo & "01" & "00" & Left(pSampleInfo.ID & Space(20), 20) & "00" & Space(4)
    sSendBuff = "AI" & rsQInfo & "01" & "00" & Left("123456789" & Space(20), 20) & "00" & Space(4)
   
''    ChkSum = ChkSum_ASTM(sSendBuff)
''    sSendBuff = sSendBuff & ChkSum
    sckServerSnd.SendData sSendBuff

    If m_sTestMode = "77" Then
        RaiseEvent PrintSendLog(Now & " : " & sSendBuff & vbCrLf)
    End If
...
Private Sub Send_ProtocalMsg(ByVal rsProtocalInfo As String)
    On Error GoTo Err_Rtn
    ...
    sSendBuff = "PA" & rsProtocalInfo
    
''    ChkSum = ChkSum(sSendBuff)
''    sSendBuff = sSendBuff & ChkSum
...
Public Function ChkSum(ByVal Para As String) As String
...
Private Sub sckServerRcv_Close()
...
Private Sub sckServerRcv_ConnectionRequest(ByVal requestID As Long)
    
    sckServerRcv.Accept requestID
End Sub

Private Sub sckServerRcv_DataArrival(ByVal bytesTotal As Long)
    Dim iSpaceCnt As Integer
    
    sckServerRcv.GetData wkBuf
    
    If m_sTestMode = "77" Then
        RaiseEvent PrintSendLog(Now & " : " & wkBuf & vbCrLf)
    End If
...
Private Sub sckServerRcv_Error(ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)
    RaiseEvent DispMsg("Rcv - " & Number & " : " & HelpFile)
End Sub

Private Sub sckServerSnd_Close()
    On Error GoTo ErrRtn
...
Private Sub sckServerSnd_ConnectionRequest(ByVal requestID As Long)
   
    sckServerSnd.Accept requestID
End Sub

Private Sub sckServerSnd_Error(ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)
    RaiseEvent DispMsg("Snd - " & Number & " : " & HelpFile)
End Sub
