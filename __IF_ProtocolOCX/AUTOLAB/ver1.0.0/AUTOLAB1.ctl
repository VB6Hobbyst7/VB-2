...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "AUTOLAB_SIMPLEX"
            Call PhaseCfg_Protocol_AUTOLAB_SIMPLEX
...
End Sub
Private Sub PhaseCfg_Protocol_AUTOLAB_SIMPLEX()
...
Private Sub DataEdit_AUTOLAB_SIMPLEX()
    ...
    'R20                                    0625      0716      130.8     0212.9    03176     CA
    If Left(RcvBuffer, 1) <> "R" Then
        Exit Sub
    End If
    
    'pResultInfo.ID = Trim(Mid(RcvBuffer, 4, 15))
    
    pResultInfo.POS = Trim(Mid(RcvBuffer, 2, 3))
    
    sTmp = Mid(RcvBuffer, 40)
    pResultInfo.RSTCNT = Len(sTmp) / 10
    ...
    With pResultInfo
        For i = 1 To .RSTCNT
            .IFCD = .IFCD & Trim(Mid(RcvBuffer, iPos, 2)) & Chr(124)
            iPos = iPos + 2
            
            .RST1 = .RST1 & Trim(Mid(RcvBuffer, iPos, 6)) & Chr(124)
            iPos = iPos + 6
            
            .FLAG = .FLAG & Trim(Mid(RcvBuffer, iPos, 2)) & Chr(124)
            iPos = iPos + 2
