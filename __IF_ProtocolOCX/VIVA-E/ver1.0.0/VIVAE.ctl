...
'Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sTAlarmCd$, sRstGbn$)
'Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sTAlarmCd$, sRstGbn$)
Event SendOrderOK(sID$, sSeqNo$, sRack$, sPos$)
'Event SendOrderOK(sID$, sSeqNo$, sRack$, sPos$, sRerunGbn$)
'Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sTAlarmCd$)
...
Event DispMsg(sMsg$)
Event RequestNextOrder(sID$, sRack$, sPos$)
...
Private Sub PhaseCfg_Protocol_VIVAE()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case m_iPhase
            ...
            Case 4      '결과요청
                Select Case Asc(wkDat)
                    Case 6      'ACK
                        If sRequestState = "" Then
                            Call SendRequest_VIVAE(m_p_sTSVol)
                        Else
                            msComm.Output = Chr(4)
                            sRequestState = ""
                            m_iPhase = 1
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_VIVAE()
    ...
    Dim tmpIFCd$, tmpRst$, tmpUnit$, tmpFlag$, tmpAlarmCd$, tmpIFNm$, tmpIFRef$, tmpState$
    ...
    Select Case RecType
        ...
        Case "O"
            ...
            pSampleInfo.ID = Trim(tmpField(2))
            
            'S (Stat)
            'A (Pediatric)
            'R (Normal)
            'All other will be treated as R.
            'Viva-Jr: only S and R are
            'supported.
            tmpState = Trim(tmpField(5))
                                    
        Case "R"        'Result Record
            'R|1|^^^VPA^E2 Valproic Acid|81.1|μg/ml|0.0 to 50.0|H^N||F||||20060126162405
            ...
            '8:Result Status -> F : indicates a final result
            '                   i : indicates a result is pending (Field 10.1.4 will contain BUSY or ------)
            
            tmpData() = Split(RcvBuffer, "|")
            
            tmpIFCd = Split(Trim(tmpData(2)), "^")(3)
            tmpIFNm = Split(Trim(tmpData(2)), "^")(4)
            tmpRst = Trim(tmpData(3))
            tmpUnit = Trim(tmpData(4))
            tmpIFRef = Trim(tmpData(5))
            tmpFlag = Trim(tmpData(6))
            tmpState = Trim(tmpData(8))
...
Private Sub SendOrder_VIVAE()
    ...
    Select Case m_iSendPhase
        Case 1
            ...
            sSendBuff = m_iFrameN & "H|\^&|||LABLIS|||||||P|LIS2-A|" & Format(Now, "yyyyMMddHHmmss") & vbCr
            ...
            'Order Record
            'O|1|12934-A||^^^VPA\^^^+C15|R|||||||||||Dr. Carnegy|||||||||O
            sSendBuff = sSendBuff & "O|1|" & pSampleInfo.ID & "||"
            ...
            sSendBuff = sSendBuff & "|R||||||||||||||||||||O" & vbCr

            'Terminator Record
            sSendBuff = sSendBuff & "L|1"
...
Public Sub SendRequest_VIVAE(ByVal sCondition)
    Dim sSendBuff As String
    Dim ChkSum    As String
    
    sSendBuff = "1H|\^&|||LABLIS|||||||P|LIS2-A|" & Format(Now, "yyyyMMddHHmmss") & Chr(13)
    sSendBuff = sSendBuff & "Q|1|^" & sCondition & "||||||||||" & Chr(13)
    sSendBuff = sSendBuff & "L|1" & Chr(13) & Chr(3)
