...
Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sOther$)
Event EnableExit()
...
'            End Select
        ...
        Case "Q"
            sBarCd = Trim(Mid(RcvBuffer, 3 + 4, 14))
            sDiskID = Trim(Mid(RcvBuffer, 23 + 4, 4))
            sDiskPos = Trim(Mid(RcvBuffer, 27 + 4, 2))

            If Mid(sBarCd, 1, 1) = "0" Then     'For 명지병원
                sBarCd = Mid(sBarCd, 2)
            End If
...
'            End If
        ...
        Case "R"
            ...
            If Len(sBufCnt) Mod 13 = 0 Then
                ...
                End With
            Else
                RaiseEvent DispMsg(pSampleInfo.ID & " 결과 길이 이상...")
            End If
...
Private Sub SendOrder_TBA200FR_Old()
'    On Error GoTo ErrRtn
'
'    Dim sSend   As String
...
'    sSendBuf = Chr(2) & "M     " & Chr(3)
    sSendBuf = Chr(2) & "M" & Space(5) & Chr(3)
...
'    sSendBuf = Chr(2) & "I " & Chr(3)
    sSendBuf = Chr(2) & "I" & Space(1) & Chr(3)
