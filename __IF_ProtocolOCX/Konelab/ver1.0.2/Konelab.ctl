    ...
    Select Case UCase(m_EqName)
        Case "KONELAB"
            If m_bUseBarcode = True Then
                Call PhaseCfg_Protocol_Konelab          '바코드 사용
            Else
                Call PhaseCfg_Protocol_Konelab_Batch    '바코드 미사용
            End If
...
Private Sub PhaseCfg_Protocol_Konelab_Batch()
    Dim wkdat As String
    Dim ix1 As Integer
                ...
                Select Case Asc(wkdat)
                    ...
                    Case 6
                        ...
                        ElseIf sSndstate = "P" Then
                            ...
                            m_iPhase = 3
                        ElseIf sSndstate = "P2" Then
                            msComm.Output = sSndO2
                            
                             If m_sTestMode = 77 Then
                                RaiseEvent PrintSendLog(sSndO2)
                            End If
...
'                            Call DisplayOrderOK("AFTER_ORDER")
                            ...
                            sSndstate = ""
                            sSndH = "": sSndP = "": sSndO = "": sSndO2 = "": sSndL = ""
                            m_iPhase = 1
...
'    sSndO = "3O|1|" & pSampleInfo.ID & "^^" & "^" & "||" & sBuf & "|R||||||N||||1||||||||||O" & vbCr & Chr(3)
   ...
   If Len(sSndO) >= 241 Then
        sSndO2 = "4" & Mid(sSndO, 241)
        sSndO = Left(sSndO, 240)
        
        sSndO = sSndO & vbCr & Chr(23)
        sSndO = Chr(2) & sSndO & ASTM_CheckSum(sSndO) & vbCr & vbLf
        
'        sSndO2 = sSndO2 & vbCr & Chr(3)
        sSndO2 = Chr(2) & sSndO2 & ASTM_CheckSum(sSndO2) & vbCr & vbLf
    Else
        sSndO = Chr(2) & sSndO & ASTM_CheckSum(sSndO) & vbCr & vbLf
    End If
    
    If sSndO2 <> "" Then
        sSndL = "5L|1" & vbCr & Chr(3)
    Else
...
Public Sub SendOrder_Konelab_Batch()
    On Error GoTo ErrHandler
