    ...
    Select Case UCase(m_EqName)
        Case "C311"
            If m_bUseBarcode = True Then
                Call PhaseCfg_Protocol_C311        '바코드사용
            Else
...
'    End If
End Sub
Private Sub PhaseCfg_Protocol_C311()
    On Error GoTo ErrRtn
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_C311()
    On Error GoTo ErrRtn
...
Private Sub SendOrder_C311()
    ...
    Select Case m_iSendPhase
        Case 1
            'Header Record
            sSendBuff = m_iFrameN & "H|\^&|||HOST^2|||||C311^1|TSDWN^REPLY|P|1" & vbCr
...
Private Sub SendOrder_C311_Batch()
    ...
    Select Case m_iSendPhase
        Case 1
            'Header Record
'            sSendBuff = m_iFrameN & "H|\^&|||HOST^2|||||C311^1|TSDWN^REPLY|P|1" & vbCr
            sSendBuff = m_iFrameN & "H|\^&|||HOST^2|||||C311^1|TSDWN^BATCH|P|1" & vbCr
...
'            RaiseEvent RequestCurOrder(pSampleInfo.ID, pSampleInfo.RACK, pSampleInfo.POS, pSampleInfo.KIND)
            ...
            'Order Record
            sSendBuff = sSendBuff & "O|1|" & Right(Space(13) & Trim(pSampleInfo.ID), 13) & Chr(124)
            sSendBuff = sSendBuff & Right(Space(5) & Trim(pSampleInfo.SEQNO), 5) & "^" & Trim(pSampleInfo.RACK) & "^" & Trim(pSampleInfo.POS) & "^^" _
