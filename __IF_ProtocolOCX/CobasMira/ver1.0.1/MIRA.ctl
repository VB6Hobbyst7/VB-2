...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "COBASMIRA"
            Call PhaseCfg_Protocol_CobasMira
...
Private Sub PhaseCfg_Protocol_CobasMira()
    ...
    For ix1 = 1 To Len(WkBuf)
        ...
        Select Case Asc(WkDat)
            ...
            Case 4         ' EOT
                If InStr(RcvBuffer, Chr(13)) > 0 Then
                    RcvBuffer = Replace(RcvBuffer, Chr(13), "")
                End If
                Call EditData_CobasMira
                RcvBuffer = ""
...
End Sub
Private Sub EditData_CobasMira()
...
'<---- COBAS 장비에서 주로 사용 E --->
    ...
    Do
        ...
        Select Case sLC
            Case "20"       'RESULT DATA
                sJNo = Trim(Mid(RcvBuffer, iLCpos + 21, 10))
                sIFRstCd = Trim(Mid(RcvBuffer, iLCpos + 5, 2))
                sSignFlag = Mid(RcvBuffer, iLCpos + 32, 1)
                sRst = Trim(Mid(RcvBuffer, iLCpos + 33, 7))
                sExpFlag = Mid(RcvBuffer, iLCpos + 40, 4)
                
                If sSignFlag = "-" Then
                    If sRst = "9.999999" And Mid(sExpFlag, 3, 2) = "99" Then
                        sRst = "LOWER LIMIT"  '"LOWER LIMIT"
                    Else
                        sRst = "-" & ConvertResult1(Mid(sExpFlag, 2, 1), Mid(sExpFlag, 3, 2), sRst, sIFRstCd)
                    End If
                Else
                    If sRst = "9.999999" And Mid(sExpFlag, 3, 2) = "99" Then
                        sRst = "UPPER LIMIT" '"UPPER LIMIT"
                    Else
                        sRst = ConvertResult1(Mid(sExpFlag, 2, 1), Mid(sExpFlag, 3, 2), sRst, sIFRstCd)
                    End If
                End If
                ...
                If Len(sJNo) > 0 And sIFRstCd <> "" Then
                    If iResultFlag = 1 And sBC = "03" Then
                        ...
                        '결과값 등록/화면 표시 처리...
                        With pResultInfo
            '                RaiseEvent AppendData(sJNo, "", "", "", 1, sIFRstCd, sRst, "", "", "")
                ...
                If Mid$(RcvBuffer, iLCpos + 52, 1) = Chr(10) Then
                    iLCpos = iLCpos + 53
                ElseIf Mid$(RcvBuffer, iLCpos + 53, 1) = Chr(10) Then
                    iLCpos = iLCpos + 54
                ElseIf Mid$(RcvBuffer, iLCpos + 54, 1) = Chr(10) Then
                    iLCpos = iLCpos + 55
                ElseIf Mid$(RcvBuffer, iLCpos + 55, 1) = Chr(10) Then
                    iLCpos = iLCpos + 56
                ElseIf Mid$(RcvBuffer, iLCpos + 56, 1) = Chr(10) Then
                    iLCpos = iLCpos + 57
                ElseIf Mid$(RcvBuffer, iLCpos + 57, 1) = Chr(10) Then
                    iLCpos = iLCpos + 58
                ElseIf Mid$(RcvBuffer, iLCpos + 58, 1) = Chr(10) Then
                    iLCpos = iLCpos + 59
                ElseIf Mid$(RcvBuffer, iLCpos + 59, 1) = Chr(10) Then
                    iLCpos = iLCpos + 60
                ElseIf Mid$(RcvBuffer, iLCpos + 60, 1) = Chr(10) Then
                    iLCpos = iLCpos + 61
                ElseIf Mid$(RcvBuffer, iLCpos + 61, 1) = Chr(10) Then
                    iLCpos = iLCpos + 62
                End If
...
'### NO MORE PENDING SAMPLE #####################################################
    If iPendingFlag = 1 And sBC = "00" Then
        iPendingFlag = 0
    End If
...
Public Sub SendOrder_CobasMira()
    On Error GoTo ErrRtn
    
    Dim sTmp    As String
    Dim i       As Integer
    Dim SendBuff    As String
    Dim SendBuffTmp As String
    ...
    SendBuff = SendBuff & "07 COBAS MIRA HOST" & " " & "10" & Chr(10)     '<LF>
    SendBuff = SendBuff & Chr(2) & Chr(10)      '<STX><LF>
    
    'Sample Type No
    SendBuff = SendBuff & "50" & " " & Right("0000" & pSampleInfo.POS, 4) & " " & Left(pSampleInfo.ID & Space(10), 10) & " " & "R"
        
    'Order 편집
    If m_iMultiStart + 7 < pSampleInfo.ORDCNT Then
        For i = m_iMultiStart To m_iMultiStart + 7
            SendBuffTmp = SendBuffTmp & " " & pSampleInfo.IFCD(i)
        Next
        m_iMultiStart = m_iMultiStart + 8
    Else
        For i = m_iMultiStart To pSampleInfo.ORDCNT
            SendBuffTmp = SendBuffTmp & " " & pSampleInfo.IFCD(i)
        Next
        m_iMultiStart = 1
    End If
    
    SendBuff = SendBuff & SendBuffTmp & Chr(10)
...
'    SendBuff = SendBuff & "09" & " " & "COBAS INTEGRA700" & " " & "09" & Chr(10)
    SendBuff = SendBuff & "07" & " " & "COBAS MIRA HOST " & " " & "09" & Chr(10)
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_iMultiStart = PropBag.ReadProperty("iMultiStart", m_def_iMultiStart)
End Sub
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("iMultiStart", m_iMultiStart, m_def_iMultiStart)
End Sub
...
Private Sub UserControl_InitProperties()
    ...
    m_iMultiStart = m_def_iMultiStart
End Sub
...
'    SendBuff = SendBuff & "09" & " " & "COBAS INTEGRA700" & " " & "00" & Chr(10)
    'Integra 800
    SendBuff = SendBuff & "07" & " " & "COBAS MIRA HOST" & " " & "00" & Chr(10)
...
'경고! 주석으로 되어 있는 다음 줄은 제거하거나 수정하지 마십시오!
'MemberInfo=7,0,0,0
Public Property Get iMultiStart() As Integer
    iMultiStart = m_iMultiStart
End Property

Public Property Let iMultiStart(ByVal New_iMultiStart As Integer)
    m_iMultiStart = New_iMultiStart
    PropertyChanged "iMultiStart"
End Property
