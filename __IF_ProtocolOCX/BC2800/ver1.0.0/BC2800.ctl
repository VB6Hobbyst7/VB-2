...
Private Sub PhaseCfg_Protocol_BC2800()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            Case 5      'ENQ(Handsahke ON)
                msComm.Output = Chr(6)
                RcvBuffer = ""
                
            Case 3      'ETX(Handsahke ON)
                msComm.Output = Chr(6)
                Call DataEditResponse_BC2800
                RcvBuffer = ""
                
            Case 2      'STX(Handsahke OFF)
                RcvBuffer = ""
                
            Case 26     'EOF(Handsahke OFF)
                Call DataEditResponse_BC2800
                RcvBuffer = ""
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_BC2800()
    On Error GoTo ErrRtn
    ...
    tmpBarCd = CStr(Val(Mid(RcvBuffer, 5, 5)))
    pSampleInfo.ID = tmpBarCd
    
    tmpRstDT = CStr(Mid(RcvBuffer, 11, 8))
    tmpRstDT = tmpRstDT & CStr(Mid(RcvBuffer, 19, 4))
    ...
    For ii = 1 To 19
        If ii = 5 Or ii = 6 Or ii = 7 Or ii = 8 Or ii = 9 Or ii = 13 Or ii = 14 Or ii = 16 Or ii = 17 Or ii = 18 Then
            tmpRst = Mid(RcvBuffer, iIdx, 3)
            iIdx = iIdx + 3
        Else
            tmpRst = Mid(RcvBuffer, iIdx, 4)
            iIdx = iIdx + 4
        End If
        
        tmpRst = Trim(tmpRst)
        If IsNumeric(tmpRst) Then
            tmpRst = CStr(Val(tmpRst))
        End If
