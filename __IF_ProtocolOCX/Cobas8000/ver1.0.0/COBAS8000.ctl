...
Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sTDil$, sInstID$, sAlarmCd$, sKind$, sTRstDT$, sOther1$)
...
Private Sub PhaseCfg_Protocol_Cobas8000_HL7_20120927()
'    On Error GoTo ErrPhase
'
'    Dim wkDat   As String
'    Dim ix1%, kk%
'    Dim aOneMsg()   As String
'
'    aOneMsg() = Split(wkBuf & Chr(28), Chr(28))   'FS
'
'    For kk = 0 To UBound(aOneMsg())
'        If Trim(aOneMsg(kk)) = "" Then Exit For
'
'        For ix1 = 1 To Len(aOneMsg(kk))
'            wkDat = Mid(aOneMsg(kk), ix1, 1)
'
'            Select Case Asc(wkDat)
'                Case 10     'LF
'                Case 11     'VT
...
'        With pResultInfo
'            If .RSTCNT > 0 Then
'                RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .UNIT, .FLAG, .DIL, .INSTID, .ALARMCD, .KIND, .RSTDT, .OTHER)
'            End If
'        End With
'
'        Call Init_pResultInfo
...
'ErrPhase:
'    If Err <> 0 Then
'        RaiseEvent DispMsg("PhaseCfg_Protocol_Cobas8000_HL7 - " & Err.Description)
'    End If
End Sub
...
Private Sub DataEdit_Cobas8000_Old()
...
Private Sub DataEdit_Cobas8000()
    ...
    Dim sTmp$
    Dim tmpIFCd$, tmpRst$, tmpUnit$, tmpFlag$, tmpAlarmCd$, tmpInstID$, tmpDil$, TmpModule$, TmpSubModule$
    Dim tmpRstDT$, tmpCmt$, tmpSrcCd$
    ...
    For ii = 0 To UBound(aFrame) - 1
        If Trim(aFrame(ii)) <> "" Then
            ...
            aField = Split(sRcvBuffer & String(13, Chr(124)), Chr(124))
            ...
            Select Case sHeader
                ...
                Case "OBX"          'Observation Result Segment - OBX
                    ...
                    '<S--- 단위가 COI 인 경우 처리...2012/7/4 yk
                    If tmpUnit = "COI" And InStr(tmpRst, "^") > 0 Then
                        aData() = Split(tmpRst, "^")
                        tmpRst = Trim(aData(1))
                    End If
                    '>E---------------------------
                    ...
                    TmpModule = Trim(Split(sTmp, "^")(0))
                    TmpSubModule = Trim(Split(sTmp, "^")(1))
                    ...
                    With pResultInfo
                        ...
                        .FLAG = .FLAG & tmpFlag & Chr(124)
                        .RSTDT = .RSTDT & tmpRstDT & Chr(124)
                        .INSTID = .INSTID & TmpModule & Chr(124)
                    End With

                Case "TCD"          'Test Code Detail Segment - TCD
                    'TCD|989|Dec|[CR]
                    tmpDil = Trim(aField(2))    'Dilution

                    With pResultInfo
                        .DIL = .DIL & tmpDil & Chr(124)
                    End With
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("DataEdit_Cobas8000 - " & Err.Description & " [" & sHeader & "] " & RcvBuffer)
    End If
End Sub

Private Sub DataEdit_Cobas8000_20120927()
'    On Error GoTo ErrRtn
'
'    Dim sHeader     As String   'Record Type
'    Dim sMsgType    As String
...
'    Dim aData()     As String
'
'    Dim tmpIFCd$, tmpRst$, tmpUnit$, tmpFlag$, tmpAlarmCd$, tmpInstID$, tmpDil$
'    Dim tmpRstDT$, tmpCmt$, tmpSrcCd$
'
'    If Trim(RcvBuffer) = "" Then Exit Sub
'
''    aField = Split(RcvBuffer, Chr(124))
'    aField = Split(RcvBuffer & String(13, Chr(124)), Chr(124))      '2012/9/27 yk
'
'    sHeader = Trim(aField(0))
'
'    Select Case sHeader
...
'    sSndBuf = sSndBuf & "PID|1|" & Trim(pSampleInfo.CMT1) & " |||^" & "" & "||" & Trim(pSampleInfo.BIRTH) & "|" & Trim(pSampleInfo.SEX) & "|" & Chr(13)
    ...
    'SPM
    If Trim(pSampleInfo.OTHER) <> "" Then
        '장비 Comment1 란에 환자명 표시...2012/9/3 yk
        sSndBuf = sSndBuf & "SPM||" & pSampleInfo.ID & "||" & pSampleInfo.SPCCD & "||||||||||" & Trim(pSampleInfo.OTHER) & "^^^^|||||||||||||" & Trim(pSampleInfo.CONTAINER) & "|" & Chr(13)
    Else
    ...
    For iCnt = 1 To pSampleInfo.ORDCNT
        ...
        If InStr(pSampleInfo.IFCD(iCnt), "^") = 0 Then
            '일반항목
            'OBR|1|||990^|||||||A[CR]
            If pSampleInfo.CANCELORDER <> "" And InStr(Trim(pSampleInfo.CANCELORDER), Trim(pSampleInfo.IFCD(iCnt)) & Chr(124)) > 0 Then
                sSndBuf = sSndBuf & "OBR|" & Trim(iCnt) & "|||" & Trim(pSampleInfo.IFCD(iCnt)) & "^|||||||C|" & Chr(13)     'Cancel Test...2012/9/26 yk
            Else
    ...
    If pSampleInfo.SINDEX = True Then           'Serum Index
        If InStr(sSndBuf, "|992^") = 0 Then
            sSndBuf = sSndBuf & "TQ1|1||||||||" & IIf(msStatGbn = "", "R", msStatGbn) & "|" & Chr(13)
            sSndBuf = sSndBuf & "OBR|" & Trim(iCnt) & "|||992^|||||||A|" & Chr(13)
        End If
        
        If InStr(sSndBuf, "|993^") = 0 Then
            sSndBuf = sSndBuf & "TQ1|1||||||||" & IIf(msStatGbn = "", "R", msStatGbn) & "|" & Chr(13)
            sSndBuf = sSndBuf & "OBR|" & Trim(iCnt) & "|||993^|||||||A|" & Chr(13)
        End If
        
        If InStr(sSndBuf, "|994^") = 0 Then
            sSndBuf = sSndBuf & "TQ1|1||||||||" & IIf(msStatGbn = "", "R", msStatGbn) & "|" & Chr(13)
            sSndBuf = sSndBuf & "OBR|" & Trim(iCnt) & "|||994^|||||||A|" & Chr(13)
        End If
    End If
...
''    sckServer(miSckMax).SendData UTF8Bytes
    ...
    If pTestMode = "77" Then
        RaiseEvent PrintSendLog(sSndBuf)
    End If
...
''    ReDim tmpDil(m_p_iOrdCnt) As String
''    tmpDil() = Split(m_p_sTDil, Chr(124))
...
'            If Trim(tmpData(ii - 1)) <> "" Then
            If Trim(tmpData(ii - 1)) <> "" And Trim(tmpData(ii - 1)) <> "." Then    '계산식은 '.' 로 표시...2012/7/3 yk
                iCnt = iCnt + 1
                .IFCD(iCnt) = tmpData(ii - 1)
''                .DIL(iCnt) = tmpDil(ii - 1)
            End If
        Next ii
        ...
        .CANCELORDER = m_pCancelOrder   '취소오더...2012/9/26 yk
    End With
...
'경고! 주석으로 되어 있는 다음 줄은 제거하거나 수정하지 마십시오!
'MemberInfo=13,0,0,
Public Property Get p_sTDil() As String
    p_sTDil = m_p_sTDil
End Property

Public Property Let p_sTDil(ByVal New_p_sTDil As String)
    m_p_sTDil = New_p_sTDil
    PropertyChanged "p_sTDil"
End Property
...
Private Sub UserControl_InitProperties()
    ...
    m_p_sTDil = m_def_p_sTDil
    m_pCancelOrder = m_def_pCancelOrder
End Sub
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_p_sTDil = PropBag.ReadProperty("p_sTDil", m_def_p_sTDil)
    m_pCancelOrder = PropBag.ReadProperty("pCancelOrder", m_def_pCancelOrder)
End Sub
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("p_sTDil", m_p_sTDil, m_def_p_sTDil)
    Call PropBag.WriteProperty("pCancelOrder", m_pCancelOrder, m_def_pCancelOrder)
End Sub
...
'경고! 주석으로 되어 있는 다음 줄은 제거하거나 수정하지 마십시오!
'MemberInfo=13,0,0,
Public Property Get pCancelOrder() As String
    pCancelOrder = m_pCancelOrder
End Property

Public Property Let pCancelOrder(ByVal New_pCancelOrder As String)
    m_pCancelOrder = New_pCancelOrder
    PropertyChanged "pCancelOrder"
End Property
