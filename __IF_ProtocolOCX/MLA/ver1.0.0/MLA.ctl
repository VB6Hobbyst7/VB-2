...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "MLA900C"
            Call PhaseCfg_Protocol_MLA900C
...
Private Sub PhaseCfg_Protocol_MLA900C()
...
End Sub
Private Sub DataEdit_MLA900C()
    On Error GoTo ErrRtn
    
    Dim sBuf        As String
    Dim tmpField()  As String
    Dim tmpData()   As String
    Dim tmpRack$, tmpPos$, tmpBarCd$
    Dim tmpIFCd$, tmpIFRstCd$, tmpRst$
    
    
    'H Record 나머지 잘라내기
    Call GetByOneUserSymbol(RcvBuffer, RcvBuffer, vbCr)
    ...
    Do
        ...
        If Left(sBuf, 1) <> "R" Then Exit Do
        
        '결과정보 초기화
        Call Init_pResultInfo
        
        tmpField() = Split(sBuf, Chr(124))
        tmpRack = Left(tmpField(1), 1)
        tmpPos = Trim(Mid(tmpField(1), 2))
        ...
        '특별한 결과처리
        'PT
        If tmpIFCd & "1" = "0011" Then
            If IsNumeric(tmpRst) = True And Val(tmpRst) >= 80 Then
                tmpRst = "80"
            End If
        End If
        
        'APTT
        If tmpIFCd & "1" = "0031" Then
            If IsNumeric(tmpRst) = True And Val(tmpRst) >= 200 Then
                tmpRst = "200"
            End If
        End If
        
        'FIB
        If tmpIFCd & "2" = "0042" Then
            If IsNumeric(tmpRst) = True And Val(tmpRst) <= 50 Then
                tmpRst = "50"
            End If
            If IsNumeric(tmpRst) = True And Val(tmpRst) >= 850 Then
                tmpRst = "850"
            End If
        End If
