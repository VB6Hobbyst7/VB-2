...
'이벤트 선언:
Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTUnit$, sTFlag$, sOther1$, sRstDt$)
...
Private m_iSendOrderMsgPhase As Integer
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "PATHFAST"
            Call PhaseCfg_Protocol_PATHFAST
...
'   PATHFAST 바코드 사용
'
Private Sub PhaseCfg_Protocol_PATHFAST()
        ...
        Select Case m_iPhase
            ...
            Case 2
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        Call DataEditResponse_PATHFAST
            ...
            Case 3
                Select Case Asc(wkDat)
                    Case 6      'ACK
                        If sState = "Q" Then
                            Call SendOrder_PATHFAST      'Order 전송
                        End If
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_PATHFAST()
    ...
    Dim tmpIFCd$, tmpRst$, tmpRst2$, tmpUnit$, tmpRef$, tmpFlag$, tmpRstDt$
    ...
    Select Case RecType
        ...
        Case "P"        'Patient Record
            If pResultInfo.ID <> "" Or pSampleInfo.SEQNO <> "" Then
                If sState <> "Q" Then
                    '결과값 등록/화면 표시 처리...
                    With pResultInfo
                        If .RSTCNT > 0 Then
                            RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .UNIT, .FLAG, .QCGBN, .RSTDT)
                        End If
                    End With
        ...
        Case "O"
            ...
            'O|1|00228411303^1^QC1||^^^2^Myo^00000000001|||||||||||1||||||||||F|||||
            
            
            tmpData() = Split(RcvBuffer, "|")
            tmpBarCd = Split(Trim(tmpData(2)), "^")(0)
            tmpSeqNo = Split(Trim(tmpData(2)), "^")(1)
            
            '< QC 구분
            If (Trim(tmpData(11)) = "Q") Then
                pResultInfo.QCGBN = "QC"
                
                tmpBarCd = Split(Trim(tmpData(2)), "^")(2)
            Else
                tmpBarCd = Split(Trim(tmpData(2)), "^")(0)
            End If
            '> QC 구분
        ...
        Case "R"        'Result Record
            ...
            'R|1|^^^2^Myo^00000000001|14.70^F|ng/dl||>@A||F||OperaterID||20050228105910|
            ...
            If Split(tmpData(3), "^")(1) = "F" Then
                'quantitative
                
                tmpRst = Trim(Split(tmpData(3), "^")(0))
...
ErrRtn:
    ...
    End If
End Sub
Private Sub SendOrder_PATHFAST()
    On Error GoTo ErrRtn
    ...
    Select Case m_iSendPhase
        ...
        Case 1      'H
            sTmp = m_iFrameN & "H|@^\||||||||PATHFAST01||P|1|20050228105347" & Chr(13) & Chr(3)
            ...
            Else
                m_iSendOrderMsgPhase = 1
                
                m_iSendPhase = 2
            End If
            
        Case 2      'P
            'P|1||99999991||Smith^John^M||19980305|M||||||||||||||||||||||||||
            sTmp = m_iFrameN & "P|1||" & pSampleInfo.ID & "||^^||19980305|M||||||||||||||||||||||||||" & Chr(13) & Chr(3)
        ...
        Case 3      'O
            ...
            sTmp = m_iFrameN & "O|" & m_iSendOrderMsgPhase & "|" & Trim(pSampleInfo.ID) & "||" _
                & "^^^" & pSampleInfo.IFCD(m_iSendOrderMsgPhase) _
                & "|||||||||||||||||||||O|||||" & Chr(13) & Chr(3)
             

            If m_iSendOrderMsgPhase = pSampleInfo.ORDCNT Then
                m_iSendPhase = 4
            Else
                m_iSendOrderMsgPhase = m_iSendOrderMsgPhase + 1
        ...
        Case 5      'EOT
            ...
            m_iFrameN = 1: m_iPhase = 1: m_iSendPhase = 1: m_iSendOrderMsgPhase = 1
            sState = ""
...
Private Sub ConvertComment(ByVal rsComment As String, ByRef rsCommentText As String)
    ...
    Select Case rsComment
        Case "'BAD_TEST"
            rsCommentText = "Invalid test code."
        Case "'QC_MA_ID"
            rsCommentText = "The ID is already used in QC database"
        Case "'BAD_S_ID"
            rsCommentText = "Invalid Sample ID."
        Case "'WRONG_ID"
            rsCommentText = "SID sent by the host is already used"
        Case "'PDB_FULL"
            rsCommentText = "Patient data base is full"
        Case "'M_TEST_E"
            rsCommentText = "Too many tests received than supported"
        Case "'UKNOWN_T"
            rsCommentText = "Unknown test requested"
        Case "'NO_TESTS"
            rsCommentText = "No test ordered for patient record"
        Case "'NO_PATIE"
            rsCommentText = "No patient information record received"
        Case "'BAD_RECO"
            rsCommentText = "Incorrect record format"
            
        Case Else
            rsCommentText = "Unknown Message!!!!!"
    End Select
