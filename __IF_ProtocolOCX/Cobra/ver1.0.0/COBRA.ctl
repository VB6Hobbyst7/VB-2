    ...
    Select Case UCase(m_EqName)
        Case "COBRA2"
            If m_bUseBarcode = True Then
                Call PhaseCfg_Protocol_COBRA_BarCd          '바코드사용
            Else
...
Private Sub PhaseCfg_Protocol_COBRA_BarCd()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkdat)
            'LF
            Case 10
                Call DataEditResponse_COBRA_BarCd
                RcvBuffer = ""
            'CR
            Case 13
...
' *=====================================================*
'*****************인하대병원용*****************
Private Sub DataEditResponse_COBRA_BarCd()
    On Error GoTo ErrRtn
    ...
    tmpIFCd = Trim(tmpField(0))
    
    If IsNumeric(tmpIFCd) = False Then
        Exit Sub
    End If
    
    tmpSeqNo = Trim(tmpField(1))
    
    If IsNumeric(Trim(tmpField(2))) = True Then
        tmpBarCd = Trim(tmpField(2))
    Else
        Exit Sub
    End If
    
''    If IsNumeric(Trim(tmpField(3))) = True Then
