...
Private Sub PhaseCfg_Protocol_CHORUS()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        ElseIf msRcvState = "R2" Then
            msRcvBuf = msRcvBuf + wkDat
            miCnt = miCnt + 1

'            If miLen + 1 = miCnt Then
            If miLen = miCnt Then
                Call EditData_CHORUS
...
'    End If
End Sub
Private Sub EditData_CHORUS()
    On Error GoTo ErrRtn
    
    Dim tmpID$, tmpSeq$, tmpRack$, tmpPos$
    Dim tmpIFCd$, tmpRst1$, tmpRst2$, tmpUnit$, tmpFlag$
    ...
    tmpID = Mid(aData2(0), 1, 19)       '', 9)

    tmpIFCd = Trim(aData2(1))

'    tmpRst2 = Trim(Mid(aData2(2), 1, 1))
    tmpRst1 = Trim(aData2(2))

    If UBound(aData2()) > 4 Then
        tmpUnit = Trim(aData2(3))
    End If
...
ErrRtn:
    If Err <> 0 Then
        RaiseEvent DispMsg("EditData Error - " & Err.Description)
    End If
End Sub
