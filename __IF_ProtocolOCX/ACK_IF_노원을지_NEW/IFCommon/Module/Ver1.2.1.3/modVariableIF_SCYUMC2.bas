Attribute VB_Name = "modVariableIF_YUMC2"
'
...
'2004/6/10 for 신촌세브란스병원 LX-20
Public Const 연회색 = &HE0E0E0
'
' 2002-05-24 JJH 추가
...
'
'   QC 결과 등록 처리...(MySQL DB 사용) for 신촌세브란스 LX-20
'
Private Sub RegServerOK_QC(ByVal iCRow%, ByVal iRstCnt%, ByVal sIFRstCd$, ByVal sRst1$)
    On Error GoTo ErrRtn
    
    Dim sBuf$, sCRst1$, sCRstCd$, sCSvrCd$, sTIFSeq$, sTSvrCd$, sTRst1$, sTRst2$, sRetVal$, sTmp$, sIFSeq$
    Dim iTRstCnt%, i%, j%
    ...
    Set objQC = CreateObject("bregqc01.bcregqc0101")
    ...
    'QC 물질 아닌 경우는 제외
    If Len(vJNo) = 0 Or Mid(vJNo, 7, 1) = "Q" Then
        Set objQC = Nothing
        Exit Sub
    End If
    ...
    sUserID = GetUserIdQC
    iPos = InStr(gsMachineNm, "-")
    If iPos > 0 Then
        sEqCd = Trim(Mid(gsMachineNm, 1, iPos - 1))
    Else
        Exit Sub
    End If
    
    '서버등록 실행
    Call objQC.SetMachineInfo(gsMachineCd, gsMachineNm)

'       objRst.RegServerQC(iPersonCnt, sWDate, sTWSeq, sTJDate, sTJGbn, sTJNo, sTRack, sTPos, sTRegNo, sTName, _
                        sTSex, sTEmer, sTReRun, sTOther, sTItemCnt, sTIFSeq, sTServerCd, sTRst1, sTRst2, sID, _
                        iMode, sEqCd, sRegStep, sQCLotNo, sQCLevel, sQCLevelNm, sQCDt, sQCEqCd)
    Call objQC.RegServerQC(1, Format(gfIFDisplayForm.dtpLabDate.Value, "YYYYMMDD"), CStr(vWSeq) & Chr(124), _
                Chr(124), Chr(124), _
                CStr(vJNo) & Chr(124), CStr(vRack) & Chr(124), CStr(vPos) & Chr(124), Chr(124), Chr(124), _
                Chr(124), Chr(124), Chr(124), Chr(3), CStr(iTRstCnt) & Chr(124), _
                sTIFSeq & Chr(3), sTSvrCd & Chr(3), sTRst1 & Chr(3), sTRst2 & Chr(3), sUserID, _
                0, sEqCd, "최종보고", "", "", Trim(vJNo), _
                Format(gfIFDisplayForm.dtpLabDate.Value, "yyyy-MM-dd HH:nn:ss"), sEqCd)
    'RegServerQC(1, "20040829", "0001|", "20040829|", "|", "200406081144|", "0|", "3|", "|", "|", _
                "|", "|", "|", "|", "22|", "004|005|007|008|052|", "LA217|LA208|", "99|7.6||", "||||||||||||||||||||||", "ACK", _
                0, "002", "최종보고", "", "04", "CONT-AN","2004-06-08 11:44:00", "A002")
    
    Set objQC = Nothing
    
ErrRtn:
    If Err <> 0 Then
        ViewMsg "RegServerOK_QC Err - " & Err.Description
    End If
End Sub
Private Function GetUserIdQC()
    On Error GoTo ErrHandler
    ...
    Exit Function
ErrHandler:
    ViewMsg "GetUserIdQC - Err(" & Err.Description & ")"
End Function
...
'                    If gIFPosInfo(j).sRackNo = CStr(vRack) Then
                    If Val(gIFPosInfo(j).sRackNo) = CStr(vRack) Then
...
Public Sub DisplayResultOK(ByVal iMode As Integer, ByVal sWDate As String, ByVal sWSeq As String, _
                ...
                ByVal sSex As String, ByVal sEmer As String, ByVal sReRun As String, ByVal sOther As String, _
                ByVal iRstCnt As Integer, ByVal sIFRstCd As String, ByVal sRst1 As String, ByVal sRst2 As String, _
                ByVal sIFSpcCd As String, ByVal sCurRow As String, Optional ByVal sFlag As String, _
                Optional ByVal sKind As String)
    On Error GoTo ErrHandler
...
'            End If
    ...
    'gsTxMode="0" => Batch, gsTxMode="1" => RealTime(한 항목씩), gsTxMode="2" => RealTime(한 환자씩)
            ...
            ElseIf sRetVal = "MORE" Or sRetVal = "DONE" Then
                ...
                If sRetVal = "DONE" Then
                    ...
                    Else
                        '수작업 결과
                        Call SpdForeBack(.spdIntList, 3, 15, gResultTable(1).iCRow, _
                             gResultTable(1).iCRow, RGB(0, 0, 0), 연회색)
                    End If
                End If
            End If
        ElseIf gsTXMode = "2" Then
        ...
        If Trim(sKind) <> "CO" Then
            ...
            End If
        Else
            'Append MySQL DB QC Result
            Call RegServerOK_QC(gResultTable(1).iCRow, iRstCnt, sIFRstCd, sRst1)
        End If
