Attribute VB_Name = "modVariableIF_YISH"
Option Explicit
...
'        Select Case sDot
'            Case "0"
'                sValue = Trim(Val(sValue) + 0.00001)
'                sTmpVal = Format$(sValue, "0")
'
'            '2003/11/12 YK
'            Case "1", "2", "3", "4", "5", "6", "7", "8", "9"
'                sValue = Trim(Val(sValue) + Val("0." & String(Val(sDot), "0") & "1"))
'                sTmpVal = Format(sValue, "0." & String(Val(sDot), "0"))
'        End Select
        '--- 반올림처리 수정(2004/2/2 yk)
        sTmpVal = RoundOff(sValue, Val(sDot))
...
End Function
'
'   반올림 함수 (sValue: 결과값, iDot: 표시를 원하는 소숫점 자릿수)
'   < 2004/2/2 yk >
'
Public Function RoundOff(ByVal sValue As String, ByVal iDot As Integer) As String
    On Error GoTo ErrRoundOff
    ...
    vTmp = Val(sValue) * (10 ^ iDot)
    
    sTmp = Trim(Int(vTmp + 0.5) / (10 ^ iDot))

    If iDot = 0 Then
        RoundOff = Format(sTmp, "0")
    Else
        RoundOff = Format(sTmp, "0." & String(iDot, "0"))
    End If
        ...
        For i = 1 To CInt(vIFCnt)
            ...
            If i > 15 Then
                ...
                ElseIf sCRst2 = "L" Then
                    Call SpdForeBack(gfIFDisplayForm.spdRst2, 1, 4, i - 15, i - 15, RGB(0, 0, 0), RGB(220, 220, 255))
                ElseIf sCRst2 <> "" And sCRst2 <> "N" Then
...
Public Sub ViewMsgLog(ByVal sMsg$)
    ...
    With gfIFDisplayForm
        ...
        If iExist = 0 Then
            If .listNoOrd.ListCount > 300 Then
                .listNoOrd.RemoveItem 0
            End If
...
Public Sub DisplayResultOK_AU640(ByVal iMode As Integer, ByVal sWDate As String, ByVal sWSeq As String, _
                ByVal sJDate As String, ByVal sJGbn As String, ByVal sJNo As String, _
                ...
                Optional ByVal sErrFlag As String)
    On Error GoTo ErrHandler
...
'            Call SendResultSocket(1, CStr(gResultTable(1).iCRow), iRstCnt, sIFRstCd, sRst1, sRst2, sIFSpcCd)
            '정상 결과인 경우만 자동 등록(2004/1/12 yk)
            If Trim(sErrFlag) = "E" Then
            Else
                '실시간 결과등록
