...
Public Function NewIFListBySex(ByVal sWDate As String, ByVal sWSeq As String, _
    ...
    With gfIFDisplayForm
        ...
        With .spdIntList
            ...
            Call .SetText(1, .MaxRows, sWSeq & "")
            'NewIFListBySex와 OldIFList의 차이 = 체크 O, X
            Call .SetText(2, .MaxRows, "1")     '체크
            '<--- 나중에 데이터 삭제시에 편리함..
            Call .SetText(3, .MaxRows, sJDate & "")
            Call .SetText(4, .MaxRows, sJGbn & "")
...
ErrHandler:
    OldIFListBySex = "NO"
    ViewMsg "OldIFListBySex - Err(" & Err.Description & ")"
End Function
...
Public Function fExcelPath() As String
    fExcelPath = GetKeyValue(HKEY_CURRENT_USER, _
                "Software\Ack_if\Interface Config\" & gsMachineCd, "ExcelPath")
End Function
...
Public Function JudgeResult1(ByVal sIFRstCd As String, ByVal sCompRst As String, _
            ...
            If sCompRst = "LOWER LIMIT" Then
                If sLimit1 <> "" Then
                    Select Case sLimit1Gbn
                        Case "0"
                            JudgeResult1 = sLimit1
                        Case "1"
                            JudgeResult1 = "< " & sLimit1
                        Case "2"
                            JudgeResult1 = sLimit1 & " 이하"
                        Case Else
                    End Select
                End If
            ElseIf sCompRst = "UPPER LIMIT" Then
                If sLimit2 <> "" Then
                    Select Case sLimit2Gbn
                        Case "0"
                            JudgeResult1 = sLimit2
                        Case "1"
                            JudgeResult1 = "> " & sLimit2
                        Case "2"
                            JudgeResult1 = sLimit2 & " 이상"
                        Case Else
                    End Select
                End If
            End If
...
Public Function JudgeResultNew(ByVal sIFRstCd As String, ByVal sCompRst As String, _
                                   sOneRst2 As String, Optional sConvRst As String) As String
    On Error GoTo ErrHandler
    ...
    Select Case sJGbn
        ...
        'QAL N/P
            If Val(sCompRst) <= Val(sRef1) Then
                JudgeResultNew = "Negative"
                sOneRst2 = "Negative"
            ElseIf Val(sCompRst) > Val(sRef2) Then
                JudgeResultNew = "Positive"
                sOneRst2 = "Positive"
            Else
                JudgeResultNew = "GrayZone(+/-)"
                sOneRst2 = "GrayZone(+/-)"
            End If
        Case "3"
        'QAN N/P
        ...
        '이하 / 이상
            If IsNumeric(sCompRst) = True Then
                If Val(sCompRst) <= Val(sRef1) Then
                    JudgeResultNew = "<" & sRef1
                    sOneRst2 = "이하"
                ElseIf Val(sCompRst) > Val(sRef1) And Val(sCompRst) < Val(sRef2) Then
                    JudgeResultNew = sCompRst
                    sOneRst2 = ""
                Else
                    JudgeResultNew = ">" & sRef2
                    sOneRst2 = "이상"
                End If
            Else
                If sCompRst = "LOWER LIMIT" Then
...
ErrHandler:
    ViewMsg "JudgeResultNew - Err(" & Err.Description & ")"
End Function
