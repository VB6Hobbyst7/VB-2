...
Object = "{09ABA8AC-02A8-4F70-83D3-5045D5B649FD}#1.1#0"; "ACKHITA0124.ocx"
Begin VB.Form frmInterface 
   ...
   ScaleHeight     =   10650
   ScaleWidth      =   15240
   Begin ACKHITA01.HITACHI HITACHI1 
      Height          =   2265
      Left            =   6120
      TabIndex        =   46
      Top             =   6840
   ...
   Begin VB.ListBox listNoOrd 
      ...
      BackColor       =   &H00E0E0E0&
      Height          =   2550
      ItemData        =   "INTERFACE.frx":450B
      Left            =   3030
      List            =   "INTERFACE.frx":450D
      TabIndex        =   25
      Top             =   6750
      Width           =   4965
   End
   ...
   Begin FPSpread.vaSpread spdRst2 
      ...
      ProcessTab      =   -1  'True
      ScrollBars      =   2
      SpreadDesigner  =   "INTERFACE.frx":450F
      UserResize      =   0
      VisibleCols     =   4
      VisibleRows     =   85
      TextTip         =   1
   ...
   Begin FPSpread.vaSpread spdRst 
      ...
      ProcessTab      =   -1  'True
      ScrollBars      =   0
      SpreadDesigner  =   "INTERFACE.frx":4E71
      UserResize      =   0
      VisibleCols     =   4
      VisibleRows     =   15
      TextTip         =   1
...
Dim miTimerCnt%
Dim miLeaveCell%
Dim mCRERst As String
Dim mCRECd As String
...
'For 서버연결이 오래 걸리는 병원用
Dim ADOCNSvr As ADODB.Connection
...
Private Sub Get_OrderString(Optional ByVal sSendYN$)
    ...
    If Trim(gOrderTable.sSampID) <> "" Then
        Set objOrd = CreateObject(sBuf)
        Call objOrd.SetMachineInfo(gsMachineCd, gsMachineNm)
        sOrdList = objOrd.FetchOrder(gsMachineCd, ADOCNSvr, "", "", "", gOrderTable.sSampID)
        Set objOrd = Nothing
    End If
    
    'sOrdList = "12345|TEST1|M|4|003|021|061|106|"
    'sOrdList = "12345|TEST1|M|1|106|"
    If sOrdList = "" Then
    ...
    'sOrdList 구성
    '환자번호 | 이름 |  접수일자 | n | IFSeq 1 | IFSeq 2 | ... | IFSeq n |
    gOrderTable.sWDate = Format(dtpLabDate.Value, "YYYYMMDD")
    gOrderTable.sJDate = ""
    gOrderTable.sJGbn = ""
    gOrderTable.sJNo = gOrderTable.sSampID
    ...
    gOrderTable.sReRun = ""
    gOrderTable.sOrdOpt = ""
    gOrderTable.sOther = GetByOne(sOrdList, sOrdList)
    gOrderTable.iOrdCnt = Val(GetByOne(sOrdList, sOrdList))
...
Public Sub Order_Input(Optional ByVal sSendYN$)
    ...
    'sOrdList 구성
    '접수일자|환자번호 | 이름 | SEX | n | IFSeq 1 | IFSeq 2 | ... | IFSeq n |
    gOrderTable.sWDate = Format(dtpLabDate.Value, "YYYYMMDD")
    gOrderTable.sJDate = ""
    gOrderTable.sJGbn = ""
    gOrderTable.sJNo = gOrderTable.sSampID
...
Public Function SpecificProcessResult(ByVal sIFRstCd$, sSpRst1$, sSpRst2$, Optional ByVal sIFSeq$, Optional ByVal sSex$) As Integer
    ...
    sSpRst1 = ConvertResult1("+", "0", sSpRst1, sIFRstCd, sIFSeq)
...
End Function
Private Sub ConvertResultData_H7600(ByRef iCnt As Integer, ByRef sTIFCd As String, _
    ...
    If mCRECd = "CRE" Then
        iCnt = iCnt + 1
        sTIFCd = sTIFCd & "CRE" & Chr(124)
        sTRst1 = sTRst1 & mCRERst & Chr(124)
...
Private Sub HITACHI1_AppendData(sID As String, sSeq As String, sRack As String, sPos As String, iRstCnt As Integer, sTIFCd As String, sTRst1 As String, sTRst2 As String, sTUnit As String, sTFlag As String, sTAlarmcd As String, sKind As String)
...
'    If gOrderTable.sSampID <> "" Then
        '결과값 편집(소숫점 변경 처리)
        Call ConvertResultData_H7600(iRstCnt, sTIFCd, sTRst1, sTRst2, sTFlag)
...
Private Sub HITACHI1_DispMsg(sMsg As String)
    ViewMsg sMsg
End Sub

Private Sub HITACHI1_PrintRcvLog(sLog As String)
    Print #1, sLog;
End Sub

Private Sub HITACHI1_PrintSendLog(sLog As String)
    Print #2, sLog;
End Sub

Private Sub HITACHI1_RaiseError(sError As String)
    MsgBox sError, vbCritical, Me.Caption
    
    Unload Me
End Sub

Private Sub HITACHI1_RequestCurOrder(sID As String, sRack As String, sPos As String, sKind As String)
...
Private Sub HITACHI1_SendOrderOK(sID As String, sSeq As String, sRack As String, sPos As String)
...
'    Call PortOpen
    ...
    If giTestMode = 77 Then
        HITACHI1.Visible = True
    Else
        HITACHI1.Visible = False: HITACHI1.TabStop = False
    End If
    ...
    '--- DB Connect
    Set ADOCNSvr = New ADODB.Connection
    With ADOCNSvr
        .ConnectionString = "DSN=" & gOrdCfg.sPath & ";UID=EJNUSER;PWD=EJNPASS;"
        .Open
    End With
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If ADOCNSvr.State = 1 Then
        ADOCNSvr.Close: Set ADOCNSvr = Nothing
    End If
End Sub
