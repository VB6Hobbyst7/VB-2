...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "HCTS2000"
            Call PhaseCfg_Protocol_HCTS2000          '바코드사용
...
Private Sub PhaseCfg_Protocol_HCTS2000()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            ...
            Case 3  'ETX
                Call DataEditResponse_HCTS2000
                RcvBuffer = ""
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_HCTS2000()
    On Error GoTo ErrRtn
    ...
    sFrameType = Mid(tmpData(0), 6, 1)
    
    If sFrameType = "D" Then
        sDataType = Mid(tmpData(0), 8, 2)
    
        Select Case sDataType
            Case "BI"
                '       Data type|Tube barcode|Sorting rule Cap Color Extension Occurrences Target|Device id|
                '       2        |1 -30       |1            2         3         1           1     |0 - 30   |
                '     D BI       |XY12345678  |11400000                                           |
                pSampleInfo.ID = Trim(tmpData(1))
...
'   환자 Order 전송
'
Private Sub SendOrder_HCTS2000()
    ...
    '     D Data type| Tube barcode|Ext. 1|Ext. 2|Test#1  Test#2     ... Test#n|   '#n : Max 25
    '     D 2        |1 - 30       |0     |0     |8       8              8     |
    '     D DW       |XY12345678   |      |      |HBA1C   COTININETSH....      |
        
    msSendBuf = ""
    msSendBuf = msSendBuf & "     D "   'FL 3 Byte + FN 2 Byte + FI 2 Byte
    msSendBuf = msSendBuf & "DW" & Chr(124)
    msSendBuf = msSendBuf & pSampleInfo.ID & Chr(124)
    msSendBuf = msSendBuf & "" & Chr(124)
    msSendBuf = msSendBuf & "" & Chr(124)
    
    '검사항목 Order코드 추가
    For iCnt = 1 To pSampleInfo.ORDCNT
        If InStr(pSampleInfo.IFCD(iCnt), "^") = 0 Then
            sTOrder = sTOrder & Left(Trim(pSampleInfo.IFCD(iCnt)) & Space(8), 8)
        End If
    Next iCnt
    
    msSendBuf = msSendBuf & sTOrder & Chr(124)
    
    msSendBuf = Chr(2) & msSendBuf
    
    sChkSum = ChkSum_HCTS(msSendBuf)  'CheckSum 계산  STX + DATA
    
    msSendBuf = msSendBuf & sChkSum & Chr(3)
...
Private Function ChkSum_HCTS(ByVal Para As String) As String
