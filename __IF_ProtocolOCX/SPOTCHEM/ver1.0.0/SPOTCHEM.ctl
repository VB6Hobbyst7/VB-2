...
End Sub
Private Sub PhaseCfg_Protocol_SP4420()
    ...
    For ii = 0 To UBound(aRow()) - 1
        ...
        If Trim(aRow(ii)) <> "" Then
            Select Case Left(aRow(ii), 3)
                Case "ID#"
                    tmpSampNo = Trim(Mid(aRow(ii), 4))
                    pResultInfo.ID = tmpSampNo
                    pResultInfo.SEQNO = tmpSampNo
                    
                Case Else
                    If Left(aRow(ii), 6) <> "" And InStr(Left(aRow(ii), 6), "/") = 0 Then
                        tmpIFCd = Trim(Left(aRow(ii), 6))
                        tmpRst1 = Trim(Mid(aRow(ii), 7, 7))
                        tmpUnit = Trim(Mid(aRow(ii), 13))
