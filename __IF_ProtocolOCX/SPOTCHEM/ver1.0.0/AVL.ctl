...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "AVL9180"
            Call PhaseCfg_Protocol_AVL9180
...
End Sub
Private Sub PhaseCfg_Protocol_AVL9180()
...
End Sub
Private Sub DataEdit_AVL9180()
    On Error GoTo ErrRtn
    
    Dim tmpSampNo$, tmpCup$
    Dim tmpIFCd$, tmpRst1$, tmpUnit$
    Dim aRow()  As String
    Dim ii%
    ...
    aRow() = Split(RcvBuffer, Chr(10))
    For ii = 0 To UBound(aRow()) - 1
        aRow(ii) = Replace(aRow(ii), Chr(13), "")
        
        If Trim(aRow(ii)) <> "" Then
            Select Case Left(aRow(ii), 3)
                Case "Cup"
                    tmpCup = Trim(Mid(aRow(ii), 8))
                    pResultInfo.POS = tmpCup
                    
                Case "Sam"
                    If Left(aRow(ii), 10) = "Sample No." Then
                        tmpSampNo = Trim(Mid(aRow(ii), 11))
                        pResultInfo.SEQNO = tmpSampNo
                    End If
                    
                Case "Na=", "K =", "Cl="
                    tmpIFCd = Trim(Left(aRow(ii), 2))
                    tmpRst1 = Trim(Mid(aRow(ii), 4, 5))
                    tmpUnit = Trim(Mid(aRow(ii), 9))
