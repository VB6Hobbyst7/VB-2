...
Dim miEqSeq As Integer

Dim miETXchk As Integer
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "MARS180"
                Call PhaseCfg_Protocol_MARS180
...
Private Sub PhaseCfg_Protocol_MARS180_()
...
Private Sub PhaseCfg_Protocol_MARS180()
    On Error GoTo ErrRtn
        ...
        Else
            Select Case Asc(wkDat)
                ...
                Case 3      'ETX
                    ...
                    'msComm.Output = Chr(6) & Chr(4)
                    msComm.Output = Chr(6)
                    
                    If m_sTestMode = "77" Then
                        RaiseEvent PrintSendLog(Chr(6) & Chr(4))
                    End If
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_MARS180()
    On Error GoTo ErrRtn
    ...
    Dim tmpSampCnt As String
    Dim tmpRstInfo As String
    Dim iIdx       As Integer
    Dim iSampIdx   As Integer
    
    '04010120110421-0000021              m00001               GOT         60    IU/L   n
    
    tmpSampCnt = CStr(Val(Mid(RcvBuffer, 1, 2)))    '샘플갯수
        
    iSampIdx = 3
    
    'For i = 1 To Val(tmpSampCnt)
        pResultInfo.SEQNO = Trim(Mid(RcvBuffer, iSampIdx, 2)): iSampIdx = iSampIdx + 2
        pResultInfo.RSTCNT = Trim(Val(Mid(RcvBuffer, iSampIdx, 2))): iSampIdx = iSampIdx + 2
        pResultInfo.RSTDT = Trim(Mid(RcvBuffer, iSampIdx, 15)): iSampIdx = iSampIdx + 15
        pResultInfo.POS = Trim(Mid(RcvBuffer, iSampIdx, 2)): iSampIdx = iSampIdx + 2
        iSampIdx = iSampIdx + 13
        tmpSex = Trim(Mid(RcvBuffer, iSampIdx, 1)): iSampIdx = iSampIdx + 1
        tmpAge = Trim(Mid(RcvBuffer, iSampIdx, 3)): iSampIdx = iSampIdx + 3
        iSampIdx = iSampIdx + 2
        pSampleInfo.CMT1 = Trim(Mid(RcvBuffer, iSampIdx, 15)): iSampIdx = iSampIdx + 15
        
        tmpRstInfo = Mid(RcvBuffer, iSampIdx, 26 * pResultInfo.RSTCNT): iSampIdx = iSampIdx + 26 * pResultInfo.RSTCNT
        
        iIdx = 1
        For ii = 1 To pResultInfo.RSTCNT
            tmpIFCd = Trim(Mid(tmpRstInfo, iIdx, 7)): iIdx = iIdx + 7
            tmpRst = Trim(Mid(tmpRstInfo, iIdx, 10)): iIdx = iIdx + 10
            tmpUnit = Trim(Mid(tmpRstInfo, iIdx, 8)): iIdx = iIdx + 8
            tmpFlag = Trim(Mid(tmpRstInfo, iIdx, 1)): iIdx = iIdx + 1   'e : 에러, s : 중지, n : 성공
...
Private Sub SendOrder_MARS180()
    ...
    Dim aID() As String
    Dim aEqSeq() As String
    Dim aFOrdCd() As String
    ...
    sSendBuff = ""
    sSendBuff = sSendBuff & Format(CStr(pSampleInfo.ORDCNT), "00")  '2byte
    
    aID = Split(pSampleInfo.ID, Chr(3))
    aEqSeq = Split(pSampleInfo.SEQNO, Chr(3))
    
    For i = 1 To pSampleInfo.ORDCNT
        aFOrdCd = Split(pSampleInfo.IFCD(i), Chr(124))
        
        miEqSeq = miEqSeq + 1
        sEqSeq = Format(CStr(miEqSeq), "00")
        
        sSendBuff = sSendBuff & sEqSeq  '2byte
        sSendBuff = sSendBuff & Format(UBound(aFOrdCd), "00")  '2byte
        sSendBuff = sSendBuff & Space(15)  '2byte
    Next
...
Private Sub Get_OrderString()
    ...
    tmpData() = Split(m_p_sTIFCd, Chr(3))
