        ...
        Select Case m_iPhase
            ...
            Case 2      '<LF> 대기
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        Call DataEditResponse_AxSym_Multi     'Data 편집
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_AxSym_Multi()
...
'            sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||" & TestDat & "|R||||||N||||||||||||||Q" & Chr(13) & Chr(3)
            'ActionCode -> A : 오더 추가 또는 신규 발생의 의미...(2003/6/23)
'            If pSampleInfo.ORDCNT > 0 Then
                sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||" & TestDat & "|||||||A||||||||||||||Q" & Chr(13) & Chr(3)
'            Else
'                'Order 없는 경우...
'                If iTEST = 0 Then
'                    sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "|||||||||Y||||||||||||||Q" & Chr(13) & Chr(3)
'                ElseIf iTEST = 1 Then
'                    sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||^^^|||||||A||||||||||||||Q" & Chr(13) & Chr(3)
'                End If
'            End If
            'Order 없는 경우...
            'AxSym은 오더 없는 경우 별도의 Action Code를 주지 않는다.(Architect만 사용)
