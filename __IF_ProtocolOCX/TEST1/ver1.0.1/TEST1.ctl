...
End Sub
Private Sub PhaseCfg_Protocol_TEST1()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            Case 2      'STX
'                RcvBuffer = ""     '장비 CheckSum이 STX가 나오는 경우가 있음...2006/5/29 yk
...
Private Sub DataEdit_TEST1OLD()
    ...
    '실제 전송되는 결과값
    'R01016451100112   1301010012v
    
    If Left(RcvBuffer, 1) <> "R" Then
        Exit Sub
    End If
    ...
    tmpBarCd = Trim(Mid(RcvBuffer, 4, m_iBCLen))
    
    tmpRack = Trim(Mid(RcvBuffer, m_iBCLen + 9, 2))
    tmpPos = Trim(Mid(RcvBuffer, m_iBCLen + 11, 2))
    
    '--- 결과편집 저장
    tmpIFCd = "ESR"
    tmpRst = Trim(Mid(RcvBuffer, m_iBCLen + 15, 4))
    tmpRst = Trim(Val(tmpRst))
...
Private Sub DataEdit_TEST1()
    ...
    tmpBarCd = Trim(Mid(RcvBuffer, 4, 15))      'm_iBCLen))
    
    tmpRack = Trim(Mid(RcvBuffer, 19, 2))
    tmpPos = Trim(Mid(RcvBuffer, 21, 2))
    
    '--- 결과편집 저장
    tmpIFCd = "ESR"
    tmpRst = Trim(Mid(RcvBuffer, 25, 4))    'm_iBCLen + 10, 4))
    tmpRst = Trim(Val(tmpRst))
