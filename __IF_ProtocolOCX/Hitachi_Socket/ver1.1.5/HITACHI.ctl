...
Private Sub SendOrder_Hitachi7180(ByVal sFunc As String, ByVal sSeqNo As String)
    ...
    ' SPE Send
'    msComm.Output = Chr$(2) & sSendBuff & Chr$(3) & HITACHI_CheckSum(sSendBuff) & Chr$(13)
    Call SendSckData(Chr$(2) & sSendBuff & Chr$(3) & HITACHI_CheckSum(sSendBuff) & Chr$(13))
'    Do
'    '   DoEvents
'    Loop Until msComm.OutBufferCount = 0
...
'    msComm.Output = Chr(2) & ">" & Chr(3) & "3E" & vbCr
    Call SendSckData(Chr(2) & ">" & Chr(3) & "3E" & vbCr)
...
Private Sub sckServer_Close(Index As Integer)
    ...
    RaiseEvent DispMsg(Index & " Sockect Close")
...
Private Sub tcpClient_Error(Index As Integer, ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)
    RaiseEvent DispMsg(Number & ":" & Description)
    Timer1.Enabled = True
End Sub
...
Private Sub SendSckData(ByVal sData As String)
    ...
    tcpClient(1).SendData sData
'    sckServer(miSckMax).SendData sData
            
ErrSck:
    If Err <> 0 Then
        RaiseEvent DispMsg("SendSckData - " & Err.Description & "(State:" & sckServer(miSckMax).State & ")")
...
Private Sub ConnectWinSock()
    ...
    tcpClient(0).RemotePort = Val(m_Port)
    
    Load tcpClient(1)
    tcpClient(1).RemoteHost = m_IPAddress
    ...
    RaiseEvent DispMsg("WinSock State: " & tcpClient(1).State)
...
'경고! 주석으로 되어 있는 다음 줄은 제거하거나 수정하지 마십시오!
'MemberInfo=14
Public Function Listen() As Variant
