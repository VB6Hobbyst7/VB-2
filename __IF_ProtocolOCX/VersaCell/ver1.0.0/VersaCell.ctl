...
Begin VB.UserControl VersaCell 
   ClientHeight    =   3150
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   3330
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "VERSACELL"
            Call PhaseCfg_Protocol_VersaCell
...
Private Sub PhaseCfg_Protocol_VersaCell()
...
' *=====================================================*
Private Sub DataEditResponse_VersaCell()
    On Error GoTo ErrRtn
    ...
    Select Case RecType
        Case "H"        'Header Record
            '1H|\^&||DPC|Sender|111 Canfield Ave^Randolph^NJ^07869||(201)927-2828|N81|Receiver||P|1|20131028081724
            tmpField() = Split(RcvBuffer, Chr(124))
                        
            msMsgID = tmpField(2)
            msAccessPW = tmpField(3)
            msSender = tmpField(4)
            msReceiver = tmpField(9)
            msVersion = tmpField(12)
            
        Case "P"        'Patient Record
            Call Init_pResultInfo

        Case "Q"        'Order Request Record
            '2Q|1|^1321525151||ALL||||||||O
            tmpField() = Split(RcvBuffer, Chr(124))
            
            tmpData() = Split(tmpField(2), "^")
            tmpBarCd = Trim(tmpData(1))
            
            sReqStatusCd = Trim(tmpField(12))    'Requesting orders and demographics
        ...
        Case "R"        'Result Record
            '4R|1|^^^AFP^^^1^DOSE|95.2|ng/mL||||F||||20131028114036|CXP A
            tmpField() = Split(RcvBuffer, "|")

            tmpData() = Split(tmpField(2), "^")
            tmpIFCd = Trim(tmpData(3)) & "^" & Trim(tmpData(7))
            ...
            With pResultInfo
                ...
                .RSTDT = .RSTDT & tmpRstDT & Chr(124)  '결과일시(2005/6/10) yk
            End With
...
Private Sub SendOrder_VersaCell()
    ...
    Select Case m_iSendPhase
        Case 1
            'Header Record
            sSendBuff = m_iFrameN & "H|\^&|" & msMsgID & "|" & msAccessPW & "|" & msReceiver & "|||||" & msSender & "||P|" & msVersion & "|" & Format(Now, "yyyyMMddHHmmss") & Chr(13) & Chr(3)
        ...
        Case 2
            'Patient Record
            'P|1|1322245781||||||U|||||
            sSendBuff = sSendBuff & m_iFrameN & "P|1|" & Trim(pSampleInfo.ID) & "||||||U|||||" & Chr(13) & Chr(3)
            m_iSendPhase = 3
                    
        Case 3
            'Order Record
            'O|1|1322253951||^^^HBsII|R|||||||||||||||||||2Q|1|^1322253951||ALL||||||||O|R
            sSendBuff = sSendBuff & m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||"
            ...
            sSendBuff = sSendBuff & "|" & Priority & "|" & Format(Now, "yyyyMMddHHmmss") & "|" & Format(Now, "yyyyMMddHHmmss") & "||||||" & Chr(13)
        ...
        Case 4
            sSendBuff = m_iFrameN & sNextSend & Chr(3)
            sNextSend = ""
            m_iSendPhase = 5
                                    
        Case 5
            If pSampleInfo.ORDCNT = 0 Then
                sSendBuff = sSendBuff & m_iFrameN & "L|1|I" & Chr(13) & Chr(3)
            Else
                sSendBuff = sSendBuff & m_iFrameN & "L|1|N" & Chr(13) & Chr(3)
            End If
