...
'   E411 바코드 사용 안하는 버전
'
Private Sub PhaseCfg_Protocol_Coda()
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_Coda()
    On Error GoTo ErrRtn
    ...
    Select Case RecType
        ...
        Case "R"        'Result Record
            ...
            '--- 결과값에 "^" 들어갈 경우 편집
            ii = InStr(1, tmpRst, "^")
            If ii <> 0 Then tmpRst = Trim(Mid(tmpRst, ii + 1))
...
Private Sub SendOrder_Coda()
    ...
    Select Case m_iSendPhase
        ...
        Case 1
            'Header Record
            sSendBuff = m_iFrameN & "H|\^&|||Bio-Rad Laboratories^CODA^10250" & vbCr
            
            Call Get_OrderString
            
            'Patient Record
            sSendBuff = sSendBuff & "P|   1||" & pSampleInfo.ID & "||||||||||||||||||||||||||||||" & vbCr
                                
            'Order Record
            sSendBuff = sSendBuff & "O| 1|||" & pSampleInfo.IFCD(1) & "||||||||||||||||||" & Format(Now, "YYYYMMDDHHMMSS") & "||Bio-Rad Laboratories^CODA^10250||||||" & vbCr
                                          
            'Terminator Record
            sSendBuff = sSendBuff & "L|1|N" & vbCr & Chr(3)
