...
'이벤트 선언:
Event RequestCurOrder(sID$, sRack$, sPos$, sSeqNo$)
    ...
    Select Case UCase(m_EqName)
        Case "ACCUTE"
            If bUseBarcode = True Then
                Call PhaseCfg_Protocol_ACCUTE
            Else
                Call PhaseCfg_Protocol_ACCUTE_Batch
            End If
...
Private Sub PhaseCfg_Protocol_ACCUTE()
...
Private Sub PhaseCfg_Protocol_ACCUTE_Batch()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            ...
            Case 3      'ETX 수신
                Call DataEditResponse_ACCUTE_Batch
...
Private Sub SendOrder_ACCUTE()
    ...
    '----- 검사항목 조회
    RaiseEvent RequestCurOrder(pSampleInfo.ID, pSampleInfo.RACK, pSampleInfo.POS, pSampleInfo.SEQNO)

    Call Get_OrderString
    
    If pSampleInfo.ID = "" Then Exit Sub

    sSendBuff = ""
    sSendBuff = Chr(2)
    sSendBuff = sSendBuff & "Y"
    sSendBuff = sSendBuff & " "
    sSendBuff = sSendBuff & " "
    sSendBuff = sSendBuff & pSampleInfo.ID & String(14 - Len(pSampleInfo.ID), " ")
    sSendBuff = sSendBuff & String(5 - Len(pSampleInfo.SEQNO), " ") & pSampleInfo.SEQNO
    sSendBuff = sSendBuff & " "
    sSendBuff = sSendBuff & String(2 - Len(pSampleInfo.RACK), " ") & pSampleInfo.RACK & "/"
    sSendBuff = sSendBuff & String(2 - Len(pSampleInfo.POS), " ") & pSampleInfo.POS & "/0"
    sSendBuff = sSendBuff & " "
    sSendBuff = sSendBuff & "   "
    sSendBuff = sSendBuff & " "

    For i = 1 To pSampleInfo.ORDCNT
        sSendBuff = sSendBuff & String(2 - Len(pSampleInfo.IFCD(i)), " ") & pSampleInfo.IFCD(i) & " "
    Next i
    
    sSendBuff = sSendBuff & vbLf
    sSendBuff = sSendBuff & Format(Now, "MM") & "/" & Format(Now, "dd") & " "
...
Private Sub SendOrder_ACCUTE_Batch()
    On Error GoTo Err_Rtn
...
Private Sub DataEditResponse_ACCUTE()
    ...
    Dim sBarCd$, sDiskID$, sDiskPos$, sQCLevel$, sQCDt$, sSampNo$, sKind$, sTmp$
    Dim i%, iTCnt%, iPos%
    ...
    Select Case sType
        ...
        Case Else
            If sType <> "Q" Then
                ...
                If m_sTestMode = "77" Then
                    RaiseEvent PrintSendLog(Chr(2) & Chr(6) & Chr(3))
                End If
            End If
            
            Select Case sType
                Case "Q"
                    'Q 1234567890123400001 11/22/0    
                    sBarCd = Trim(Mid(RcvBuffer, 3, 14))
                    
                    If InStr(RcvBuffer, "/") > 0 Then
                        sDiskID = Split(Trim(Mid(RcvBuffer, 23, 7)), "/")(0)
                        sDiskPos = Split(Trim(Mid(RcvBuffer, 23, 7)), "/")(1)
                    End If
                    
                    sSampNo = Trim(Mid(RcvBuffer, 17, 5))
                    ...
                    Call SendOrder_ACCUTE
        
                Case "R"
                    'R 12345678901234000013 11/22/0                  01  -1.61 02 123.4  
                    sBarCd = Trim(Mid(RcvBuffer, 3, 14))
                    sSampNo = Trim(Mid(RcvBuffer, 17, 5))
                    sKind = Trim(Mid(RcvBuffer, 22, 1))
                    sDiskID = Split(Trim(Mid(RcvBuffer, 24, 7)), "/")(0)
                    sDiskPos = Split(Trim(Mid(RcvBuffer, 24, 7)), "/")(1)
                    
                    sTmp = Mid(RcvBuffer, 49)
                    iTCnt = Len(sTmp) / 10
                    ...
                    With pResultInfo
                        .ID = sBarCd
                        .SEQNO = sSampNo
                        If sKind = "1" Or sKind = "2" Or sKind = "3" Then
                            .KIND = sKind
                        End If
                        .RACK = sDiskID
                        .POS = sDiskPos
                        ...
                        For i = 1 To iTCnt
                            ...
                            .IFCD = .IFCD & Trim(Mid(sTmp, iPos, 2)) & Chr(124)
                            iPos = iPos + 2
                            .RST1 = .RST1 & Trim(Mid(sTmp, iPos, 6)) & Chr(124)
                            iPos = iPos + 6
                            .RST2 = .RST2 & Chr(124)
                            .UNIT = .UNIT & Chr(124)
                            .FLAG = .FLAG & Trim(Mid(sTmp, iPos, 1)) & Chr(124)
                            iPos = iPos + 2
                            .RSTDT = .RSTDT & Chr(124)
                        Next
...
Private Sub DataEditResponse_ACCUTE_Batch()
    On Error GoTo ErrRtn
    ...
    Dim sBarCd$, sDiskID$, sDiskPos$, sQCLevel$, sQCDt$, sSampNo$, sKind$, sTmp$, sFunc$
    Dim i%, iTCnt%, iPos%
