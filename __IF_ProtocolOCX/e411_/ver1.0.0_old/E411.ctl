...
'            If m_bUseBarcode = True Then
                Call PhaseCfg_Protocol_E411         '바코드사용
'            Else
'                Call PhaseCfg_Protocol_E411_Batch   'Batch Mode
'            End If
...
End Sub
Private Sub PhaseCfg_Protocol_E411_Batch()
...
Private Sub DataEditResponse_E411()
    ...
    Select Case RecType
        ...
        Case "Q"        'Order Request Record
            'Q|1|^000663^32^@7^2^^SAMPLE^NORMAL||ALL||||||||O[CR]
            ...
            tmpSeqNo = Trim(aData(2))
            tmpRack = aData(3)
            tmpPos = aData(4)
            tmpSampType = aData(6)      'SAMPLE TYPE
...
'            pSampleInfo.KIND = Trim(tmpKind)
        ...
        Case "O"
            'O|1|000663|36^0044^2^^SAMPLE^NORMAL|ALL|R|2005070509341 6 | | | | | X | | | | | | | | | | | | ||O [ C R ]
