...
'   e411 바코드 사용 안하는 버전
'
Private Sub PhaseCfg_Protocol_e411()
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_e411()
...
Private Sub SendOrder_e411()
    ...
    Select Case m_iSendPhase
        ...
        Case 1      'H
            sTmp = m_iFrameN & "H|\^&||||||||||P" & Chr(13) & Chr(3)
            m_iSendPhase = 2
        ...
        Case 3      'O
            ...
            For i = 1 To pSampleInfo.ORDCNT
                TestDat = TestDat & "^^^" & Left(pSampleInfo.IFCD(i), Len(pSampleInfo.IFCD(i)) - 1) & "0^\"
            Next i
            If pSampleInfo.ORDCNT > 0 Then
            ...
            ElseIf UCase(m_EqName) = "E411DISK" Then
                '--- e411 (Disk 버전)
                If m_bUseBarcode = True Then
                    sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||" _
                        & TestDat & "|R||||||" _
                ...
                Else
                    sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "|" _
                        & pSampleInfo.SEQNO & "^" & pSampleInfo.RACK & "^" & Val(pSampleInfo.POS) & "^^SAMPLE^NORMAL|" _
                        & TestDat & "|R||||||" _
                        & sActionCd & "||||||||||||||" & sReportType & vbCr & Chr(3)
''                    'test
'                    sTmp = m_iFrameN & "O|1|" & "" & "|" _
'                        & pSampleInfo.SEQNO & "^" & pSampleInfo.RACK & "^" & Val(pSampleInfo.POS) & "^^SAMPLE^NORMAL|" _
'                        & TestDat & "|R||||||" _
'                        & sActionCd & "||||||||||||||" & sReportType & vbCr & Chr(3)
                End If
...
'            Else
'                sTmp = m_iFrameN & "L|1|I" & vbCr & Chr(3)
'            End If
            If m_bUseBarcode = True Then
                If pSampleInfo.ORDCNT > 0 Then
                    sTmp = m_iFrameN & "L|1|F" & vbCr & Chr(3)
