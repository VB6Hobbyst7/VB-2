...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "HM-JACK"
            Call PhaseCfg_Protocol_HM_Jack
...
Private Sub PhaseCfg_Protocol_HM_Jack()
...
Private Sub DataEditResponse_HM_Jack()
    On Error GoTo ErrRtn

    Dim sDataType, sRstDt$, sDisk$, sPos$, sSampNo$, sSampType$, sCellNo$, sDiltCtl$, sAv$, sRst1$, sSign$, sStatus$, sIFCd$
    Dim aField() As String

    aField = Split(RcvBuffer, ",")
    
    sDataType = Trim(aField(0)) 'Data Type
    sRstDt = Trim(aField(2))    'Date and Time
    sDisk = Trim(aField(3))
    sPos = Trim(aField(4))      'Sample positon No
    sSampNo = Trim(aField(5))   'Sample No
    sRst1 = Trim(aField(10))     'Measurement results
    sIFCd = "OB"
