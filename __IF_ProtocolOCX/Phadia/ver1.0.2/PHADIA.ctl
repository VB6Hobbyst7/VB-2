...
Event AppendData(sID$, sSeq$, sRack$, sPos$, iRstCnt%, sTIFCd$, sTRst1$, sTRst2$, sTDil$, sTUnit$, sTFlag$, sKind$, sReqID$)
...
''            End If
        ...
        ElseIf sRecType = "L" Then
            ...
            With pResultInfo
                If .RSTCNT > 0 Then
                    RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .DIL, .UNIT, .FLAG, .Kind, .INSTID)
                End If
            End With
