...
ErrHandler:
    RaiseEvent DispMsg("PhaseCfg_Protocol 에러 - " & Err.Description)
End Sub
...
'            
            ...
            For ii = 1 To UBound(sData) - 1
                ...
                ElseIf UCase(Left(sData(ii), 7)) = "CLARITY" Then
                    'Clarity정보
                    'CL500으로 장비코드변경 - ATLAS는 Clarity안나오게설정됨
                    sMach = "CL500"
...
'                    msFlag = msFlag & Chr(124)
                ...
                Case "C4103"    'Comment Block
                    ...
                    'Review + Cross 에 Comment(FLAG)추가함
...
ErrHandler:
    RaiseEvent DispMsg("PhaseCfg_Protocol_UF100 에러 - " & Err.Description)
End Sub
