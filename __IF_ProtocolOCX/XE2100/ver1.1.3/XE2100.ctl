...
'            End If
            ...
            If InStr(Trim(pSampleInfo.CMT1), "^") > 0 Then
                sPatNo = Trim(Split(pSampleInfo.CMT1, "^")(0))
                sPatNm = Trim(Split(pSampleInfo.CMT1, "^")(1))
                sSex = Trim(Split(pSampleInfo.CMT1, "^")(2))
                sBirth = Trim(Split(pSampleInfo.CMT1, "^")(3))
                
                sSendBuff = m_iFrameN & "P|1|||" & sPatNo & "|^" & sPatNm & "^||" & sBirth & "|" & sSex & "|||||||||||||||||" & Chr(13) & Chr(3)
            Else
