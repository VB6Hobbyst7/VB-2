...
End Sub
Private Sub PhaseCfg_Protocol_PhD()
...
End Sub
Private Sub DataEdit_PhD()
    On Error GoTo ErrRtn

    Dim sRecType    As String
    Dim tmpField()  As String
    Dim tmpData()   As String
    Dim tmpID$, tmpIFCd$, tmpRst1$, tmpRst2$, tmpUnit$

    Dim sPatGbn$, sRstFlag$
    ...
    Select Case sRecType
        ...
        Case "O"
            'Control
            '2O|1|BLANK^LISTEST^||ANA1|||||||Q||||||||||||||F
            'Patient
            '2O|1|Andrew^ABC123^1||Test1|||||||||||||||||||||F
            msActCd = ""
            
            tmpField() = Split(RcvBuffer, Chr(124))
            msActCd = Trim(tmpField(11))        'Action Code
        ...
        Case "R"        'Result Record
            '3R|1|^^^Test1^QUANT|0.04|EU1||||F3E
            '4R|2|^^^Test1^QUAL^^^F|neg|||||FF7
            '--- 결과데이타 편집
            If msActCd = "Q" Then
                Exit Sub
            End If
