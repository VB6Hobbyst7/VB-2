...
Begin VB.UserControl AMPLILINK 
   ClientHeight    =   3150
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   3330
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "AMPLILINK"
'            Call PhaseCfg_Protocol_AmpliLink
            Call PhaseCfg_Protocol_AmpliLink_Batch
...
End Sub
Private Sub PhaseCfg_Protocol_AmpliLink()
        ...
        Select Case m_iPhase
            ...
            Case 2
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        If bEndChk = True Then
                            Call DataEditResponse_AmpliLink
                            RcvBuffer = ""
                        End If
                        msComm.Output = Chr(6)
...
Private Sub PhaseCfg_Protocol_AmpliLink_Batch()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case m_iPhase
            ...
            Case 3
                Select Case Asc(wkDat)
                    Case 6      'ACK
                        Call SendOrder_AmpliLink    'Order 전송
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_AmpliLink()
    On Error GoTo ErrRtn
    ...
    Select Case RecType
        ...
        Case "O"
            'Patient Sample
            '3O|1||3|^^^ALL||20060804104642|||^ml||A
            'QC
            '3O|1||E1360200000|^^^ALL||20040726120320|||^ml||Q\A||||LPC^E13602000007B
            tmpSeqNo = "": tmpBarCd = "": tmpRack = "": tmpPos = ""
            
            tmpField() = Split(RcvBuffer, Chr(10))
            tmpField() = Split(tmpField(0), "|")
            
            tmpBarCd = Trim(tmpField(2))
            tmpReqDT = Trim(tmpField(6))    'RequestedDateTime
            
            If InStr(1, tmpField(3), "^") > 0 Then
            ...
            pSampleInfo.OTHER = tmpReqDT
            
            'QC 결과 체크...2008/10/13 yk
            pSampleInfo.KIND = ""
            If UBound(tmpField()) >= 15 Then
                If InStr(tmpField(11), "Q") > 0 Then
                    pSampleInfo.KIND = "Q"
                    
                    tmpData() = Split(tmpField(15), "^")
                    tmpQCNm = Trim(tmpData(0))      'Coded the type of the Quality Control: HPC, MPC, LPC or NC
                    pSampleInfo.ID = tmpQCNm & "-" & tmpSeqNo
                End If
            End If
            
        Case "R"        'Result Record
            '4R|1|^^^HBMCAP48|2.88178324699402|IU/mL|12^110000000|<||V||JUNG|20060804125620|20060804151242|1831
            ...
            '8:Result Status(X:Result failed, F:Final Result, V, C)
            '12:DateTimeTestCompleted
            ...
            If Trim(tmpFlag) = "N" Then
                tmpFlag = ""
            End If
            
            tmpRstDT = Trim(tmpField(12))       'DateTimeTestCompleted
'            tmpRstDT = Trim(tmpField(11))       'DateTimeTestStarted...2010/8/27 yk
...
'            If tmpAlarmCd = "0" Then        '2005/4/20 yk
            If tmpAlarmCd = "0" Or tmpAlarmCd = "Accepted" Then     '2011/9/8 yk
                tmpAlarmCd = ""
            End If
'            pResultInfo.ALARMCD = pResultInfo.ALARMCD & tmpAlarmCd & Chr(124)
            If tmpAlarmCd <> "" Then
                pResultInfo.ALARMCD = Replace(pResultInfo.ALARMCD, "####" & Chr(124), tmpAlarmCd) & ",####" & Chr(124)
            End If
...
'   환자 Order 전송
'
Private Sub SendOrder_AmpliLink()
    On Error GoTo ErrRtn
...
'                sActionCd = "N"     '2011/12/12 yk
            ...
            sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "|" & Trim(pSampleInfo.SEQNO) & "|" _
                & TestDat & "|||||||" _
                & sActionCd & "||||||||||||||" & sReportType & Chr(13) & Chr(3)
