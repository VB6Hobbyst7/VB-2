...
'   E411 바코드 사용 안하는 버전
'
Private Sub PhaseCfg_Protocol_BIOM()
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_BIOM()
    ...
    Select Case RecType
        ...
        Case "Q"        'Order Request Record
            ...
            '2006/4/4 추가...Abort인 경우는 별도의 응답을 안하도록 수정...
        ...
        Case "O"
            '3O|1|M158072^1|||||||||||||Miscellaneous|1266|||||||||F
            tmpData() = Split(RcvBuffer, "|")
            tmpData() = Split(tmpData(2), "^")
            
            tmpBarCd = Trim(tmpData(0))
            tmpPos = Trim(tmpData(1))   'Isolation
...
''          4R|1|^^^ORGN|901^Escherichia coli|||||F||
''          5R|2|^^^102^NAME|Ampicillin|||||F
''          6R|3|^^^102^MIC|>46.000|||||F
''          7R|4|^^^102^INTERP|R|||||F
''          0R|5|^^^102^ZONE|06|||||F
''          1R|6|^^^102^SERUM_IQ|<   0.761|||||F
''          2R|7|^^^102^COST|A|||||F

            RcvBuffer = Mid(RcvBuffer, 1, InStr(RcvBuffer, Chr(3)) - 2)
            tmpData() = Split(RcvBuffer, "|")
            
            If tmpData(8) = "F" Then   'F : Final Results, R : Previously Transmitted Results
                tmpField = Split(tmpData(2), "^")
                
                If UBound(tmpField) < 4 Then
                    If Trim(tmpField(3)) = "ORGN" Then
                        pResultInfo.ID = pSampleInfo.ID
                        pResultInfo.RACK = Trim(Split(tmpData(3), "^")(0))  '균
                        pResultInfo.POS = pSampleInfo.POS 'Isolation
                    End If
                    
                Else
                    Select Case Trim(tmpField(4))
                        Case "NAME"
                            tmpIFCd = Trim(tmpField(3)) '항생제코드
                            tmpUnit = Trim(tmpData(3))  '항생제명
                            
                            pResultInfo.RSTCNT = pResultInfo.RSTCNT + 1
                            pResultInfo.IFCD = pResultInfo.IFCD & tmpIFCd & Chr(124)
                            pResultInfo.UNIT = pResultInfo.UNIT & tmpUnit & Chr(124)
                            
                        Case "MIC"
                            tmpRst = Trim(tmpData(3))   '감수성
                            pResultInfo.RST1 = pResultInfo.RST1 & tmpRst & Chr(124)
                            
                        Case "INTERP"
                            tmpRst2 = Trim(tmpData(3))  '판정
                            pResultInfo.RST2 = pResultInfo.RST2 & tmpRst2 & Chr(124)
                            pResultInfo.FLAG = pResultInfo.FLAG & tmpFlag & Chr(124)
...
Private Sub SendOrder_BIOM()
    ...
    Select Case m_iSendPhase
        ...
        Case 3      'O
            'O|1|1B140030^5|||||||||N||||er||||||||||F
                                                                          'Isolation                                 '검체
            sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "^" & Trim(pSampleInfo.POS) & "|||||||||N||||" & Trim(pSampleInfo.RACK) & _
                               "||||||||||F" & vbCr & Chr(3)
