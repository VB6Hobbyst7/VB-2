            ...
            If pSampleInfo.ID <> "ALL" Then
                For i = 0 To UBound(maSpcNo)
                    ...
                    If pSampleInfo.ORDCNT > 0 Then
                        ...
                        'Patient Record
                        ''sSendBuff = sSendBuff & "P|" & CStr(iPatCnt) & "||||^||||||||" & Chr(13)
                        sSendBuff = sSendBuff & "P|" & CStr(iPatCnt) & "||" & pSampleInfo.PATNO & "||^||||||||" & Chr(13)
...
Private Sub DataEditResponse_ACUSTAR()
    ...
    Select Case RecType
        ...
        Case "R"        'Result Record
            'R|1|^^^2254|24.7|U/mL||H||F@V||ACL^ACL||20130325154343|ALBA^H^3
            'R|2|^^^2254|Positive|Interpretation||H||F@V||ACL^ACL||20130325154343|ALBA^H^3
            tmpData() = Split(RcvBuffer, "|")

            tmpIFCd = Trim(Split(tmpData(2), "^")(3)) & "^" & tmpData(4)
        ...
        Case "L"        'Msg Terminater Record
            ...
            With pResultInfo
                If .RSTCNT > 0 Then
                    RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .UNIT, .FLAG, .QCGBN, "", sRstDt, .OTHER)
                End If
            End With
