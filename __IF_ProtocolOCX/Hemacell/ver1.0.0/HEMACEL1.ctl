...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "HEMACELL"
            Call PhaseCfg_Protocol_HEMACELL
...
Private Sub PhaseCfg_Protocol_HEMACELL()
    Dim wkDat   As String
    Dim ix1     As Integer
...
'382        5                                                                           6 1 0 0 0 0           0101960221 6.919960101                3.83 8.526.3 69.22.232.414.9666. 6.3 .4256.0 4.1  .5 2.3        59.0 7.034.0        PiPc      9:
    
    tmpIFCd = "WBC|RBC|HGB|HCT|MCV|MCH|MCHC|RDW|PLT|MPV|PCT|PDW|Lym#|Mid#|Grn#|Lyn%|Mid%|Grn%|"
    aIFCd = Split(tmpIFCd, Chr(124))
    
    iPos = 4
    tmpBarCd = Trim(Mid(RcvBuffer, iPos, 10))
    pResultInfo.ID = tmpBarCd
    
    iPos = 120
    sTmp = Mid(RcvBuffer, 120)
    
    tmpRst = Trim(Mid(sTmp, 1, 4))
    pResultInfo.RST1 = pResultInfo.RST1 & tmpRst & Chr(124)
    pResultInfo.RST2 = pResultInfo.RST2 & Chr(124)
    pResultInfo.IFCD = pResultInfo.IFCD & aIFCd(0) & Chr(124)
    pResultInfo.UNIT = pResultInfo.UNIT & Chr(124)
    pResultInfo.FLAG = pResultInfo.FLAG & Chr(124)
    
    iPos = 29
    sTmp = Mid(sTmp, iPos, 84)
    
    iRstCnt = 1
    
    For iPos = 1 To Len(sTmp)
        tmpRst = Trim(Mid(sTmp, iPos, 4))
        ...
        If tmpRst <> "" Then
            ...
            pResultInfo.RST1 = pResultInfo.RST1 & tmpRst & Chr(124)
            pResultInfo.RST2 = pResultInfo.RST2 & Chr(124)
            pResultInfo.IFCD = pResultInfo.IFCD & aIFCd(iRstCnt) & Chr(124)
            pResultInfo.UNIT = pResultInfo.UNIT & Chr(124)
            pResultInfo.FLAG = pResultInfo.FLAG & Chr(124)
