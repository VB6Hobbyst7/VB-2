...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "VSMT210"
            Call PhaseCfg_Protocol_VSMT210
...
End Sub
Private Sub PhaseCfg_Protocol_VSMT210()
...
Private Sub DataEdit_VSMT210()
    On Error GoTo ErrRtn
    
    Dim tmpBarCd$, tmpRack$, tmpPos$
    Dim tmpIFCd$, tmpRst$, sRxData$
    Dim iPos    As Integer
    Dim ii      As Integer
    Dim sTmpData() As String
    Dim bChkFlag As Boolean
    
    If Left(RcvBuffer, 1) = Chr(6) Then
        RcvBuffer = Mid(RcvBuffer, 3, Len(RcvBuffer) - 3)
        RcvBuffer = ""
        
        Exit Sub
    End If
    
    sRxData = Mid(RcvBuffer, 9, Len(RcvBuffer) - 9)
    
    ReDim sTmpData(4) As String
    
    iPos = 1
   
    For ii = 0 To 4
    
        iPos = InStr(iPos, sRxData, ".............")
        sTmpData(ii) = Mid(sRxData, iPos + 15, 1)
        
        tmpPos = CStr(Asc(Mid(sRxData, iPos - 8, 1)))
        tmpRst = CStr(Asc(sTmpData(ii)))
        
        ''' 검사건수가 5개 이상의 경우 이전 데이타를 받기 위한것.
        If bChkFlag = False Then
            If Val(tmpPos) > 5 Then
                bChkFlag = True
            End If
        End If
    ...
    ''' 5개 이상 검사한 경우 이전 1번부터 5번까지의 결과를 요청함.
    If bChkFlag = True Then
        msComm.Output = Chr(62) & "0002009301" & Chr(13) & "00"
        bChkFlag = False
    End If
