...
'            Else
                ...
                With pResultInfo
                    .RACK = Mid$(RcvBuffer, 3, 4)
                    .POS = Mid$(RcvBuffer, 7, 2)
                    sSampType = Trim(Mid(RcvBuffer, 9, 1))  'SampleType(Serum:' ', Urine:'U', Other:'X', Other-1:'Y', Whole blood:'W', Not specified:'N')
                    .SEQNO = Mid$(RcvBuffer, 10, 4)
                ...
                For ii = 1 To m_iTotalItemCnt   '120
                    sTmp = Mid(RcvBuffer, 39 + 17 * (ii - 1), 1)
    
                    If Asc(sTmp) = 3 Then Exit For
    
                    tmpIFCd = Format(Val(Mid(RcvBuffer, 39 + 17 * (ii - 1), 3)), "000")
                    tmpRst = Trim(Mid(RcvBuffer, 39 + 17 * (ii - 1) + 3, 6))
                    tmpFlag = Trim(Mid(RcvBuffer, 39 + 17 * (ii - 1) + 9, 8))       '4 types 적용
    ...
    Select Case sBC
        ...
        Case "D"
            ...
            For ii = 1 To m_iTotalItemCnt   '120
                ...
                tmpFlag = Trim(Mid(RcvBuffer, 39 + 19 * (ii - 1) + 11, 8))       '4 types 적용
...
Private Sub SendOrder_AU5800(ByVal sLC As String)
    ...
    'Send Message 편집
    SendBuf = Chr$(2)
    ''SendBuf = SendBuf & "S" & Space(1) & pSampleInfo.RACK & pSampleInfo.POS & pSampleInfo.SPCCD     'Sample Type 추가
    SendBuf = SendBuf & "S" & sLC & pSampleInfo.RACK & pSampleInfo.POS & pSampleInfo.SPCCD     'Sample Type 추가
    SendBuf = SendBuf & pSampleInfo.SEQNO
    SendBuf = SendBuf & Space(20 - Len(pSampleInfo.ID)) & pSampleInfo.ID & Space$(4) & "E"
    ...
    Dim tmpOrd(1 To 120) As String
    For ii = 1 To pSampleInfo.ORDCNT
        If InStr(pSampleInfo.IFCD(ii), "^") > 0 Then
            tmpOrd(Val(pSampleInfo.IFCD(ii))) = Format(Split(pSampleInfo.IFCD(ii), "^")(0), "000") & Trim(Split(pSampleInfo.IFCD(ii), "^")(1))
        Else
...
Private Sub SendOrder_AU680()
    ...
    'Send Message 편집
    SendBuf = Chr$(2)
    SendBuf = SendBuf & "S" & Space(1) & Format(pSampleInfo.RACK, "0000") & Format(pSampleInfo.POS, "00") & pSampleInfo.SPCCD     'Sample Type 추가
    SendBuf = SendBuf & pSampleInfo.SEQNO
    SendBuf = SendBuf & Space(20 - Len(pSampleInfo.ID)) & pSampleInfo.ID & Space$(4) & "E"
