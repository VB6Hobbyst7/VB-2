...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "NYCOCARD"
            Call PhaseCfg_Protocol_NYCOCARD
...
Private Sub PhaseCfg_Protocol_NYCOCARD()
...
Public Sub DataEdit_NYCOCARD()
    On Error GoTo ErrRtn
    
    Dim tmpRst      As String
    Dim tmpRstCd    As String
    ...
    Do
        ...
        If InStr(RcvBuffer, "Result") Then
            iSPos = InStr(RcvBuffer, "HbA1c")
            iEPos = InStr(RcvBuffer, "Result") + 16
            tmpBuf = Mid(RcvBuffer, iSPos, iEPos - iSPos)
            RcvBuffer = Mid(RcvBuffer, iEPos)
            ''RcvBuffer = Replace(RcvBuffer, tmpBuf, "")
            
            iSPos = InStr(tmpBuf, "HbA1c")
            iEPos = InStr(tmpBuf, "Result") + 16
            tmpRst = Trim(Mid(tmpBuf, iEPos - 9, 5))
            tmpRstCd = Trim(Mid(tmpBuf, iSPos, 7))
            
            With pResultInfo
                .RSTCNT = .RSTCNT + 1
                
                .IFCD = .IFCD & tmpRstCd & Chr(124)
