...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_E170_ORI()
'    On Error GoTo ErrRtn
'
'    Dim RecType     As String   'Record Type
'    Dim Tmp     As String
...
'    Dim tmpPos      As String
'    Dim tmpData()   As String
'    Dim tmpRst$, tmpIFCd$, tmpUnit$
'
'    Dim sRst        As String
'    Dim sRst2       As String
'    Dim sIFRstCd    As String   '인터페이스시 검사항목코드
'
'    Dim bCOLflg     As Boolean
'    Dim sTmpRef     As String
'
'
'    i = InStr(1, RcvBuffer, "|")
'    If i <> 0 Then
...
'            tmpData() = Split(tmpData(2), "/")
'
'            If Trim(tmpData(1)) <> "" Then    'BarCode ID가 잘 넘어왔는지 검사
'                state = "Q"
'                gOrderTable.sSampID = Trim(tmpData(1))
'            Else
'                state = ""
'                gOrderTable.sSampID = ""
'            End If
...
'                tmpSeqNo = Trim(tmpData(2))
'
'                gOrderTable.sSeqNo = tmpSeqNo
'                gOrderTable.sRack = tmpRack
'                gOrderTable.sPos = tmpPos
'            End If
'
'        Case "O"
'            '--- Sample ID 편집
...
'            End If
'
'            gOrderTable.sSampID = tmpBarCd      '바코드
'            gOrderTable.sJDate = Edit_BarCode(0, gOrderTable.sSampID)
'            gOrderTable.sJNo = Edit_BarCode(1, gOrderTable.sSampID)
'            gOrderTable.sRack = tmpRack
'            gOrderTable.sPos = tmpPos
'
'        Case "R"        'Result Record
...
'            tmpUnit = Trim(tmpData(4))
'
'            'UNIT가 COI인 경우 결과값 편집(For Elecsys2010 & E170)
'            bCOLflg = False
'            If tmpUnit = "COI" Then
'                i = InStr(tmpRst, "^")
'                If i <> 0 Then
'                    sTmpRef = Mid(tmpRst, 1, i - 1)
'                    If sTmpRef = "1" Then
'                        sRst2 = "Pos"
'                    ElseIf sTmpRef = "-1" Then
'                        sRst2 = "Neg"
'                    End If
'
...
'            End If
'
'            'E170의 경우 AHBS 항목에서 Limit값 벗어난 결과값에 대한 편집...(2002/10/14 yk)
'            If tmpIFCd = "64" Then
'                If Val(tmpRst) <> 0 Then
'                    Select Case Val(tmpRst)
'                        Case Is >= 1000
'                            tmpRst = ">1000.0": sRst2 = "Pos"
'                        Case Is <= 2
'                            tmpRst = "<2.0": sRst2 = "Neg"
'                        Case Else
'                    End Select
'                End If
'            End If
...
'            '--- 결과값에 "^" 들어갈 경우 편집
'            i = InStr(1, tmpRst, "^")
'            If i <> 0 Then tmpRst = Mid(tmpRst, i + 1)
'
'            sIFRstCd = tmpIFCd      '검사코드
'            sRst = tmpRst           '결과
'            If Left$(sRst, 1) = "." Then
'                sRst = "0" & sRst
'            End If
'
'            '참고치 판정
'            If bCOLflg <> True Then
'                sRst = JudgeResult1(sIFRstCd, sRst, sRst2)
'            End If
'
'            '검사결과 누적
'            sTotIFCd = sTotIFCd & sIFRstCd & Chr(124)
...
'
'        Case "L"
'            If iTestCnt = 0 Then Exit Sub
'
'            '----- 결과 저장 및 화면표시
'            Call DisplayResultOK(3, Format(dtpLabDate.Value, "YYYYMMDD"), "", _
'                                gOrderTable.sJDate, "", gOrderTable.sSampID, gOrderTable.sRack, gOrderTable.sPos, _
'                                "", "", "", "", "", "", _
'                                iTestCnt, sTotIFCd, sTotRst, sTotRst2, _
'                                "", "")
'
'            iTestCnt = 0
'            sTotIFCd = "": sTotRst = "": sTotRst2 = ""
...
'ErrRtn:
'    If Err <> 0 Then
'        ViewMsg "DataEditResponse_E170 오류발생 - " & Err.Description
'    End If
End Sub
