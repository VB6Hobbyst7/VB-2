...
Private Sub PhaseCfg_Protocol_MGIT()
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_MGIT()
    On Error GoTo ErrRtn
    ...
    Select Case RecType
        ...
        Case "R"        'Result Record
            ...
            If InStr(tmpData(3), "^") > 0 Then
                tmpRst = Trim(Split(tmpData(3), "^")(0))
                tmpRst2 = Trim(Split(tmpData(3), "^")(1))   'Growth Units
            Else
                tmpRst = Trim(tmpData(3))
                tmpRst2 = ""
            End If

            sSDate = Trim(tmpData(11))
            sEDate = Trim(tmpData(12))
            tmpRstDT = sSDate & "^" & sEDate   '시작일시 | 종료일시

            tmpSeqNo = Trim(Split(tmpData(2), "^")(4))
            tmpRack = Trim(Split(tmpData(13), "^")(2))  '혐기, 호기, 소아 구분
            tmpPos = Trim(Split(tmpData(13), "^")(4))   'Location
            
            tmpInstID = Trim(Split(tmpData(13), "^")(3))    '장비번호
            
            pSampleInfo.SEQNO = tmpSeqNo
            pSampleInfo.RACK = tmpRack
            pSampleInfo.POS = tmpPos
            pSampleInfo.RSTDT = tmpRstDT
