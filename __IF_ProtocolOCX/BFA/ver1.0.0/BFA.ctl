...
Private Sub PhaseCfg_Protocol_BFA()
...
Private Sub DataEdit_BFA()
    On Error GoTo ErrRtn
    
    Dim tmpBarCd$
    Dim tmpIFCd$, tmpRst$, tmpErr$
    Dim sType   As String
    
    sType = Trim(Mid(RcvBuffer, 1, 4 - 1))
    
    If sType <> "DT" Then   'DT : Result
        Exit Sub
    End If
    
    tmpBarCd = Trim(Mid(RcvBuffer, 4, 27 - 4))
    tmpRst = Trim(Mid(RcvBuffer, 48, 55 - 48))
    tmpIFCd = Trim(Mid(RcvBuffer, 55, 64 - 55))
    
    tmpErr = Trim(Mid(RcvBuffer, 80, 83 - 80))
    
    If tmpErr <> "0" Then
        RaiseEvent DispMsg("Err Code : " & tmpErr)
    End If
