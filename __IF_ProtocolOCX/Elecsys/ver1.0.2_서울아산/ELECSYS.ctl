...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "ELECSYS2010", "ELECSYS2010TRAY"
            If m_bUseBarcode = True Then
                '바코드 사용
                Call PhaseCfg_Protocol_Elecsys2010_BarcodeMode
            Else
                '바코드 사용 안함
                Call PhaseCfg_Protocol_Elecsys2010
            End If
        
        Case "ELECSYS1010"
            If m_bUseBarcode = True Then
                '바코드 사용
                Call PhaseCfg_Protocol_Elecsys1010_BarcodeMode
            Else
'                Call PhaseCfg_Protocol_Elecsys1010
            End If
...
'   Elecsys1010 바코드 사용
'
Private Sub PhaseCfg_Protocol_Elecsys1010_BarcodeMode()
        ...
        Select Case m_iPhase
            ...
            Case 2
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        Call DataEditResponse_Elecsys1010
            ...
            Case 3
                Select Case Asc(wkDat)
                    Case 6      'ACK
                        If sState = "Q" Then
                            Call SendOrder_Elecsys1010      'Order 전송
                        End If
...
'
'   ELECSYS2010 바코드 사용 안하는 버전
'
Private Sub PhaseCfg_Protocol_Elecsys2010()
        ...
        Select Case m_iPhase
            ...
            Case 2
                Select Case Asc(wkDat)
                    ...
                    Case 10     '<LF>
                        Call DataEditResponse_Elecsys2010
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_Elecsys2010()
    On Error GoTo ErrRtn
...
'   Elecsys2010 바코드 사용
'
Private Sub PhaseCfg_Protocol_Elecsys2010_BarcodeMode()
...
Private Sub SendOrder_Elecsys2010()
    On Error GoTo ErrRtn
    ...
    Select Case m_iSendPhase
        ...
        Case 3      'O
            ...
            For i = 1 To pSampleInfo.ORDCNT
                If Trim(pSampleInfo.IFCD(i)) = "760" Then
                    'b-HCG인 경우 Auto Dilution
                    Select Case bHCG_AutoDil
                        Case "10"
                            sAutoDil = "3"
                        Case "50"
                            sAutoDil = "7"
                    ...
                    TestDat = TestDat & "^^^" & Left(pSampleInfo.IFCD(i), Len(pSampleInfo.IFCD(i)) - 1) _
                                    & "0^" & sAutoDil & "\"
                Else
                    TestDat = TestDat & "^^^" & Left(pSampleInfo.IFCD(i), Len(pSampleInfo.IFCD(i)) - 1) & "0^0\"
                End If
            ...
            If UCase(m_EqName) = "ELECSYS2010" Then
                '--- ELECSYS2010 (RACK 버전)
                sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "|" _
                    & pSampleInfo.SEQNO & "^" & pSampleInfo.RACK & "^" & pSampleInfo.POS & "^^SAMPLE^NORMAL|" _
                    & TestDat & "|R||||||" _
                    & sActionCd & "||||||||||||||" & sReportType & vbCr & Chr(3)
            ElseIf UCase(m_EqName) = "ELECSYS2010TRAY" Then
                '--- ELECSYS2010 (TRAY 버전)
                sTmp = m_iFrameN & "O|1|" & Trim(pSampleInfo.ID) & "||" _
                    & TestDat & "|R||||||" _
                    & sActionCd & "||||||||||||||" & sReportType & vbCr & Chr(3)
        ...
        Case 4      'T
            If m_bUseBarcode = True Then
                sTmp = m_iFrameN & "L|1|F" & vbCr & Chr(3)
            Else
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_bHCG_AutoDil = PropBag.ReadProperty("bHCG_AutoDil", m_def_bHCG_AutoDil)
End Sub
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("bHCG_AutoDil", m_bHCG_AutoDil, m_def_bHCG_AutoDil)
End Sub
...
Private Sub UserControl_InitProperties()
    ...
    m_bHCG_AutoDil = m_def_bHCG_AutoDil
End Sub
...
'경고! 주석으로 되어 있는 다음 줄은 제거하거나 수정하지 마십시오!
'MemberInfo=14,0,0,0
Public Property Get bHCG_AutoDil() As Variant
    bHCG_AutoDil = m_bHCG_AutoDil
End Property

Public Property Let bHCG_AutoDil(ByVal New_bHCG_AutoDil As Variant)
    m_bHCG_AutoDil = New_bHCG_AutoDil
    PropertyChanged "bHCG_AutoDil"
End Property
