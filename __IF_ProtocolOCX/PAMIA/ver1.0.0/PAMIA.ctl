...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "PAMIA"
            Call PhaseCfg_Protocol_PAMIA    '바코드 사용
...
End Sub
Private Sub DataEditResponse_PAMIA()
    ...
    sLC = Mid(RcvBuffer, 5, 2)      'Block Number
    
    Select Case sBC
        Case "RR", "RI"         'Inquiry
            With pSampleInfo
                .RACK = Mid(RcvBuffer, 22, 6)
                .POS = Mid(RcvBuffer, 28, 2)
                .ID = Trim(Mid(RcvBuffer, 30, 15))
            End With
            
            Call Sleep(200)
            
            'Order Request 요청 받은 후
            Call SendOrder_PAMIA
            
            Exit Sub
            
        Case "DS"
            Select Case sLC
                Case "01"       'Sample ID Sub Text
                    '결과정보 초기화
                ...
                Case Else       'Concentration Sub Text
                    tmpIFCd = Mid(RcvBuffer, 9, 3)
                    tmpRst = Trim(Mid(RcvBuffer, 12, 11))
                    tmpFlag = Trim(Mid(RcvBuffer, 51, 2))
        ...
        Case Else
            RaiseEvent DispMsg("Data 수신오류 - (" & sBC & "/" & sLC & ")")
    End Select
...
Private Sub SendOrder_PAMIA()
    ...
    SendBuf = SendBuf & "0"     '숫자
'    SendBuf = SendBuf & "O"    '영문자
    
    SendBuf = SendBuf & "33"
    SendBuf = SendBuf & "01"
    SendBuf = SendBuf & "01"
    SendBuf = SendBuf & " "
    SendBuf = SendBuf & Format$(Date, "YYYYMMDD")
    ...
    With pSampleInfo
        For ii = 1 To pSampleInfo.ORDCNT
            If Trim(.IFCD(ii)) <> "" Then
                sTestCd = sTestCd & Format(.IFCD(ii), "000") & "    1" & "  "
            End If
        Next ii
    End With
...
Private Sub PhaseCfg_Protocol_PAMIA()
