...
'이벤트 선언:
Event SendOrderOK(sID$, sSeq$, sRack$, sPos$, iOrdCnt%)
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "XS"       'XS-Series ASTM Protocol
            Call PhaseCfg_Protocol_XS
...
Private Sub PhaseCfg_Protocol_XS()
...
' *               Data편집 & 응답처리                   *
' *=====================================================*
Private Sub DataEditResponse_XS()
    ...
    Select Case RecType
        ...
        Case "O"        'Test Order Record
            'O|1||^^123-4567-890123^B|^^^^WBC\^^^^RBC\····^^^^NEUT#|||||||N||||||||||||||F[CR]
            tmpData() = Split(RcvBuffer, "|")
        ...
        Case "R"        'Result Record
            'R|1|^^^^WBC^1|7.80|10*3/μL||N||||||20011116101000[CR]
            '--- 결과데이타 편집
            tmpData() = Split(RcvBuffer, "|")
        ...
        Case "C"        'Comment Record(Not Used)
