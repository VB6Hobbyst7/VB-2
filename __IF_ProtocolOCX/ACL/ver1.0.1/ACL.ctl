...
''            If msAllBarCd = "ALL" Or msAllBarCd = "" Then
''                msSendBuff = msSendBuff & "H|@^\|<0_0><1025080549_50>||HOSTTOP|||||ACLTOP||P|1394-97|" & Format(Now, "YYYYMMDDHHMMSS") & Chr(13)
''                msSendBuff = msSendBuff & "L|1|N"
''
''                ChkS = ChkSum_ASTM("1" & msSendBuff & Chr(3))
''
''                msSendBuff = msSendBuff & Chr(2) & "1" & msSendBuff & Chr(3) & ChkS & Chr(13) & Chr(10)
''
''                msComm.Output (msSendBuff)
''                msSendBuff = ""
''                Exit Sub
''            End If
...
Private Sub SendOrder_ACLTOP(ByVal iCurOrd As Integer, ByVal iMaxOrd)
    ...
    If f_typSampleInfo.ORDCNT > 0 Then
        ...
        msSendBuff = msSendBuff & "O|" & "1" & "|" & f_typSampleInfo.ID & "|" & "|" & sBuf & "|R||||||A||||P||||||||||Q" & Chr(13)
    End If
