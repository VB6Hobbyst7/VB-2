...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        Case "MYTHIC18"
            Call PhaseCfg_Protocol_Mythic18
...
Private Sub PhaseCfg_Protocol_Mythic18()
...
Private Sub DataEdit_Mythic18()
    On Error GoTo ErrRtn

    Dim sIFCd$, sRst1$, sRst2$
    Dim tmpMsgType$, tmpSpcType$
    Dim tmpID$, tmpSeqNo$, tmpBarCd$, tmpTolIFCd$, tmpFlag$, tmpRstDt$
    Dim ii%
    Dim tmpData()   As String
    Dim aTolIFCd()  As String
    ...
    If UBound(tmpData) > 1 Then
        If UCase(Trim(tmpData(2))) = "RESULT_READY" Then
            msComm.Output = "ACK_RESULT_READY" & Chr(13)
            Exit Sub
...
''    If UCase(Trim(tmpData(0))) = "ACK_RESULT" Then
''        Exit Sub
        
    ''ElseIf UCase(Trim(tmpData(2))) = "RESULT" Then
    If sState = "R" Then
        tmpMsgType = UCase(Trim(tmpData(0)))
        
        Select Case tmpMsgType
            Case "DATE"
                tmpRstDt = Format(Trim(tmpData(1)), "yyyyMMdd")
            ...
            Case "END_RESULT"
                ...
                With pResultInfo
                    ...
                    msComm.Output = "ACK_RESULT" & Chr(13)
            ...
            Case Else
                tmpTolIFCd = "WBC|RBC|HGB|HCT|MCV|MCH|MCHC|RDW|PLT|MPV|THT|PDW|LYM%|MON%|GRA%|LYM|MON|GRA|"
                aTolIFCd = Split(tmpTolIFCd, Chr(124))
                        
                For ii = 0 To UBound(aTolIFCd) - 1
                    If aTolIFCd(ii) = tmpMsgType Then
                        sIFCd = Trim(tmpData(0))
                        sRst1 = Trim(tmpData(1))
                        tmpFlag = Trim(tmpData(2))
                        sRst2 = Trim(tmpData(3))
