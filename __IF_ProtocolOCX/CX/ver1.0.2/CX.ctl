        ...
        Case "701", "801"
            Select Case sFunction
                Case "02"
                    Select Case sRetCd
                        Case "1"
                            ...
                            End If
                        Case "2"
                            RaiseEvent DispMsg("[" & sStream & ",02,02] BUSY")
                        Case "3"
                            If sStream = "701" Then
                                RaiseEvent DispMsg("[701,02,03] INVALID CHEMISTRY REQUESTED")
                            Else
                        ...
                        Case "5"
                            ...
                            End If
                        Case "6"
                            RaiseEvent DispMsg("[" & sStream & ",02,06] CONTROL NOT CONFIGURED")
                        Case "7"
                            RaiseEvent DispMsg("[" & sStream & ",02,07] CALIBRATOR Rack(Sector) ONLY")
                        Case "8"
                            If sStream = "701" Then
                                RaiseEvent DispMsg("[701,02,08] MODE MISMATCH")
                            Else
                            ...
                            End If
                        Case "9"
                            RaiseEvent DispMsg("[" & sStream & ",02,09] LX20/CX7 ERROR")
                        Case "10"
                            RaiseEvent DispMsg("[" & sStream & ",02,10] Completed Sample")
                        Case "11"
                            RaiseEvent DispMsg("[" & sStream & ",02,11] Incompatible Fluid Types")
                        Case "12"
                            RaiseEvent DispMsg("[" & sStream & ",02,12] Incompatible Test Types")
                        Case "13"
                            RaiseEvent DispMsg("[" & sStream & ",02,13] Incompatible Patient Name")
                        Case "14"
                ...
                Case "04"
                    Select Case sRetCd
                        Case "1"
                            RaiseEvent DispMsg("[" & sStream & ",04,01] BAD MESSAGE")
                        Case "2"
                            RaiseEvent DispMsg("[" & sStream & ",04,02] BUSY")
                        Case "3"
                            RaiseEvent DispMsg("[" & sStream & ",04,03] LX20/CX7 ERROR")
                        Case "4"
                            RaiseEvent DispMsg("[" & sStream & ",04,04] Non-existent ERROR")
                        Case "5"
...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "CX3DELTA"
            Call PhaseCfg_Protocol_CX3Delta
...
End Sub
Private Sub PhaseCfg_Protocol_CX3Delta()
            ...
            Case 4      '===== EOT 대기
                Select Case Asc(wkDat)
                    Case 4      '----- EOT 수신
                        ...
                        Call DataEditResponse_CX3Delta
...
Private Sub DataEditResponse_CX3Delta()
    On Error GoTo ErrRtn
            ...
            Case "702"
                Select Case sFC
                    ...
                    Case "03"
                        ...
                        tmpRstErr = ""
                        For ii = 27 To 42   'Result error code
...
ErrRtn:
    ...
    End If
End Sub
Private Sub SendNextOrder_CX3Delta()
    On Error GoTo ErrRtn
...
Private Sub SendOrder_CX3Delta()
    On Error GoTo ErrRtn
...
'    sSendBuff = sSendBuff & ",00"   'RACK
    sSendBuff = sSendBuff & "," & Format(pSampleInfo.RACK, "00")    'RACK
'    sSendBuff = sSendBuff & ",00"   'POS
    sSendBuff = sSendBuff & "," & Format(pSampleInfo.POS, "00")     'POS
...
'경고! 주석으로 되어 있는 다음 줄은 제거하거나 수정하지 마십시오!
'MemberInfo=14
Public Function SendInitMsg() As Variant
