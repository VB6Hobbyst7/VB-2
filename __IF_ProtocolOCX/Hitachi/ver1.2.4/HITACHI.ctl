...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "HITACHI747"
            Call PhaseCfg_Protocol_HITACHI747
        
        Case "HITACHI7080"
            If bUseBarcode = True Then
                Call PhaseCfg_Protocol_HITACHI7080
            Else
                Call PhaseCfg_Protocol_HITACHI7080_Batcd
            End If
...
End Sub
Private Sub PhaseCfg_Protocol_HITACHI7080()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
           ...
           Case 13, 10  'CR,LF
              Call DataEditResponse_Hitachi7080
...
Private Sub PhaseCfg_Protocol_HITACHI7080_Batcd()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
           ...
           Case 13, 10  'CR,LF
              Call DataEditResponse_Hitachi7080_Batch
...
Private Sub DataEditResponse_Hitachi7080()
    ...
    Select Case sFrame
        ...
        Case ";"
            ...
            If Len(tmpID) > 0 Then
                '--- ORDER 조회/전송
                Call SendOrder_Hitachi7080(sFunc, pSampleInfo.SEQNO)
            Else
                Call Send_MOR
            End If
...
'            If sFunc <> "@" And sFunc <> "N" And sFunc <> "M" Then
            If sFunc <> "@" And sFunc <> "M" Then
                ...
                iTestCnt = Val(Trim(Mid(RcvBuffer, 47, 2)))
...
'            If sFunc <> "@" And sFunc <> "N" And sFunc <> "M" Then
            If sFunc <> "@" And sFunc <> "M" Then
                ...
                iTestCnt = Val(Trim(Mid(RcvBuffer, 47, 2)))     'Test Count
                StartPtr = 49

                For i = 1 To iTestCnt
                    sRstData = Mid(RcvBuffer, StartPtr, 9)
                    
                    tmpIFCd = CStr(Val(Trim(Mid(sRstData, 1, 2))))  'TEST NUMBER
                    tmpRst = Trim(Mid(sRstData, 3, 6))              'RESULT
                    tmpFlag = Trim(Mid(sRstData, 9, 1))             'FLAG
...
Private Sub DataEditResponse_Hitachi7080_Batch()
    On Error GoTo ErrRtn
    ...
    Select Case sFrame
        ...
        Case ";"
            '<SPE> 전송
            ';A1     0 15  31117000902     1202031630
            sFunc = Mid(RcvBuffer, 2, 39)
            tmpSeqNo = Mid(RcvBuffer, 4, 5)
            tmpRack = Mid(RcvBuffer, 9, 1)
            tmpPos = Mid(RcvBuffer, 10, 2)
            ...
            'If Len(tmpID) > 0 Then
                '--- ORDER 조회/전송
                Call SendOrder_Hitachi7080_Batch(sFunc, pSampleInfo.SEQNO)
            'Else
            '    Call Send_MOR
            'End If
...
Private Sub SendOrder_Hitachi7080(ByVal sFunc As String, ByVal sSeqNo As String)
    On Error GoTo ErrRtn
...
Private Sub SendOrder_Hitachi7080_Batch(ByVal sFunc As String, ByVal sSeqNo As String)
    On Error GoTo ErrRtn
    ...
    '검사항목 조회
    RaiseEvent RequestNextOrder(pSampleInfo.ID, pSampleInfo.RACK, pSampleInfo.POS)
    ...
    'A1     0 15                  1202031630
    Mid(sFunc, 12, 13) = pSampleInfo.ID
