...
'   Dim SampIf   As String              ' Sample Information
...
'
'
'   Frame = Left$(RcvBuffer1, 1)        ' get Frame of RcvBuffer.
'   Select Case Frame
'      Case ">"                        ' ANY
...
'
'      Case ";"                        ' SPE
'         Func = Mid$(RcvBuffer1, 2, 1)     ' Function
'         SampNo = Mid$(RcvBuffer1, 4, 5)     ' Sample No.
'         RackNo = Mid$(RcvBuffer1, 9, 1)     ' Disk No.  ?????????(9 ??????)
'         PosiNo = Mid$(RcvBuffer1, 10, 3)    ' Position No.
'         Id_Num = Mid$(RcvBuffer1, 13, 13)   ' Id No.
'         '--- ID 저장
'         sTRID1 = Trim(Id_Num)
'         '===========
'         SampIf = Mid$(RcvBuffer1, 4, 37)    ' Sample Information
'
''         '--- 해당 BarCode의  Order 조회/전송  (-> 바코드 사용안함)
'         'Test(삼광) 2001-03-20
'         Call Order_Input_Hitachi7170(Id_Num, SampNo, RackNo, PosiNo, Func, SampIf)
'
'
...
'
'         Func = Mid$(RcvBuffer1, 2, 1)     ' Function
'         If Func = "K" Or Func = "L" Or Func = "G" Or Func = "H" Then
'            GoTo Nxt2
'         End If
'         If Func <> "@" Then              ' TNNA ?
'            TstCount = Val(Trim$(Mid$(RcvBuffer1, 41, 3))) ' Test Count
'            StartPtr = 44
'            For i = 1 To TstCount
'               TablePtr1 = TablePtr1 + 1      ' Table Pointer Increament
'               ResData = Mid$(RcvBuffer1, StartPtr, 10)
'               TestResultTable(TablePtr1).TestNo = Mid$(ResData, 1, 3)   ' Test Number
'               TestResultTable(TablePtr1).RESULT = Mid$(ResData, 4, 6)   ' Result
'               '--- Flag 추가 (2001/3/7 yk)
'               TestResultTable(TablePtr1).FLAG = Mid(ResData, 10, 1)     ' Flag
...
'      Case ":"                        ' END
'         Func = Mid$(RcvBuffer1, 2, 1)     ' Function
'         'Calibration 값이 넘어올 경우 예외처리...(2000/11/2)
'         If Func = "K" Or Func = "L" Or Func = "G" Or Func = "H" Then   'If Func = "K" Or Func = "L" Then
'            GoTo Nxt2
'         End If
'         Dim tmpID$, tmpidcnt%
'         If Func <> "@" Then              ' TNNA
'            With p_TestSample1
'                .RACK = Trim$(Mid$(RcvBuffer1, 9, 1))   ' Rack No.
''                .EQID = Right$("0000" & Trim$(Mid$(Rcvbuffer, 4, 5)), 5) ' Sample No.
'                 .EQID = "1"    ' Sample No
'                .POS = Trim$(Mid$(RcvBuffer1, 10, 3))  ' Position No.
'                tmpID = Trim$(Mid$(RcvBuffer1, 13, 13))
'                tmpidcnt = Len(Trim(tmpID))
''                If tmpidcnt = 0 Then
'                If tmpidcnt <> 11 Then
'                    GoTo Nxt2
'                End If
''                tmpid = Mid(tmpid, 1, 8) & "0" & Mid(tmpid, 9, 4)
'                .ID = tmpID     ' Id No.
'            End With
'
'            TstCount = Val(Trim$(Mid$(RcvBuffer1, 41, 3))) ' Test Count
...
'                TablePtr1 = TablePtr1 + 1     ' Table Pointer Increament
'                ResData = Mid$(RcvBuffer1, StartPtr, 10)
'                TestResultTable(TablePtr1).TestNo = Trim(Mid$(ResData, 1, 3))   ' Test Number
'                TestResultTable(TablePtr1).RESULT = Mid$(ResData, 4, 6) ' Result
'                '--- Flag 추가 (2001/3/7 yk)
''                TestResultTable(TablePtr1).Flag = Mid(ResData, 10, 1)    ' Flag
'                'Flag에 성별 입력(삼광...)
'                TestResultTable(TablePtr1).FLAG = Get_Sex(Me, p_TestSample1.ID, "F")    'M/F/""
'
'                StartPtr = StartPtr + 10    ' Start Pointer Increament
'            Next i
'
'            '=== 결과 Local DB에 등록
'            Call Append_LocalDB_7170(p_TestSample1.ID, p_TestSample1.EQID, TablePtr1)
'
'            '--- spdList에 결과 표시/spdBarCode에서 해당 ID 삭제
'            For i = 1 To TablePtr1
'                Call Disp_spdList(0, p_TestSample1.ID, TestResultTable(i).TestNo, TestResultTable(i).RESULT)
'            Next i
'
'            TablePtr1 = 0
'         End If
...
'
'Err_Rtn:
'    pnlStatus.Caption = "[Hitachi 7170] INTERFACE 오류발생...(" & Error(Err()) & ")"
'    TablePtr1 = 0       '2002/3/7 yk
End Sub
