...
Private Sub PhaseCfg_Protocol()
    ...
    Select Case UCase(m_EqName)
        ...
        Case "HITACHI7020"
            If m_bUseBarcode = True Then
                Call PhaseCfg_Protocol_HITACHI7020_BarCd '바코드 사용함
            Else
...
Private Sub DataEditResponse_Hitachi7170()
    ...
    Select Case sFrame
        ...
        Case ";"                        ' SPE
            ...
            Else
                '--- ORDER 조회/전송(배치방식)
                Call SendOrder_Hitachi7170_Batch(sFunc, pSampleInfo.SEQNO)
            End If
...
Private Sub SendOrder_Hitachi7170_Batch(ByVal sFunc As String, ByVal sSeqNo As String)
    On Error GoTo ErrRtn
    ...
    Call SetProperty_Hitachi7170_Batch
    '검사항목 편집
    Call Get_OrderString
...
'    pSampleInfo.SEQNO = sSeqNo
    ...
    ' 2007-03-16 분당메디피아 add
    '<<<<<<<<<<<<<<<<<<<<<<<<<<<
    sWkFunc = Left(sFunc, 11) & pSampleInfo.ID & Mid(sFunc, 25, Len(sFunc) - 24)
    sFunc = sWkFunc
    ' >>>>>>>>>>>>>>>>>>>>>>>>>>
            
    'sTestDat = String$(88, "0")
    sTestDat = String$(89, "0")
...
Private Sub SetProperty_Hitachi7170_Batch()

    m_p_sID = pSampleInfo.ID
    m_p_sRack = pSampleInfo.RACK
    m_p_sPos = pSampleInfo.POS
...
Private Sub SendOrder_Hitachi7180(ByVal sFunc As String, ByVal sSeqNo As String)
    ...
    For iCnt = 1 To pSampleInfo.ORDCNT
        If Trim$(pSampleInfo.IFCD(iCnt)) <> "" Then
            If Val(pSampleInfo.IFCD(iCnt)) >= 88 Then    '2006/7/28 yk
                If pSampleInfo.IFCD(iCnt) = "88" Or pSampleInfo.IFCD(iCnt) = "89" Then
                    Mid$(sTestDat, 87, 1) = "1"
                    Mid$(sTestDat, 88, 1) = "1"
                End If
            Else
...
'   HITACHI7020 Barcode
'
Private Sub PhaseCfg_Protocol_HITACHI7020_BarCd()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            ...
            Case 10, 13     '
                Call DataEditResponse_Hitachi7020_BarCd       'Data 편집
                RcvBuffer = ""
...
End Sub
Private Sub DataEditResponse_Hitachi7020_BarCd()
    On Error GoTo ErrRtn
    ...
    Select Case sFrame
        ...
        Case ";"                        ' SPE
            ...
            '--- Order 전송
            Call SendOrder_Hitachi7020_BarCd(sFunc)
...
Private Sub SendOrder_Hitachi7020_BarCd(ByVal sFunc As String)
    On Error GoTo ErrRtn
