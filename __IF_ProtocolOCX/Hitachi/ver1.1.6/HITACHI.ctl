...
Private Sub DataEditResponse_Hitachi7180()
    ...
    Dim i       As Integer
    Dim StartPtr    As Integer
    Dim tmpSeqNo$, tmpID$, tmpRack$, tmpPos$, tmpKind$
    Dim tmpIFCd$, tmpRst$, tmpUnit$, tmpFlag$, tmpAlarm$, tmpRstDT$
    Dim iTestCnt    As Integer
    Dim sRstData    As String
...
'            If sFunc <> "@" And sFunc <> "M" Then
                ...
                If tmpID = "" Then tmpID = tmpSeqNo
                
                If UCase(sFunc) = "F" Then      'QC 구분 추가...2009/1/13 yk
                    tmpKind = "QC"
                Else
                    tmpKind = ""
                End If
