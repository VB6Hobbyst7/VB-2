...
Private Sub PhaseCfg_Protocol_HITACHI7020_Batch()
    ...
    For ix1 = 1 To Len(wkBuf)
        ...
        Select Case Asc(wkDat)
            ...
            Case 10, 13     '
                Call DataEditResponse_Hitachi7020_Batch       'Data 편집
                RcvBuffer = ""
...
Private Sub DataEditResponse_Hitachi7020_Batch()
    On Error GoTo ErrRtn
    ...
    Select Case sFrame
        ...
        Case ";"                        ' SPE
            ...
            '--- Order 전송
            Call SendOrder_Hitachi7020_Batch(sFunc)
...
Private Sub DataEditResponse_Hitachi7600()
    ...
    Select Case RecType
        Case "H"        'Header Record
            ...
            tmpData() = Split(tmpField(10), "^")
            tmpRstGbn = Trim(tmpData(1))
            If Trim(tmpData(0)) = "RSUPL" Then
                pResultInfo.RSTGBN = tmpRstGbn
            End If
        ...
        Case "L"
            ...
            With pResultInfo
                If .RSTCNT > 0 Then
                    RaiseEvent AppendData(.ID, .SEQNO, .RACK, .POS, .RSTCNT, .IFCD, .RST1, .RST2, .UNIT, .FLAG, .ALARMCD, .RSTGBN)
                End If
            End With
...
Private Sub SendOrder_Hitachi7020_Batch(ByVal sFunc As String)
    On Error GoTo ErrRtn
...
Private Sub SendOrder_Hitachi7600()
    ...
    Select Case m_iSendPhase
        Case 1
            'Header Record
            If m_p_sRerunGbn = "R" Then
                sSendBuff = m_iFrameN & "H|\^&|||HOST^2|||||H7600^1|TSDWN^REAL|P|1" & vbCr
            Else
            ...
            If m_p_sRerunGbn = "R" Then
                sSendBuff = sSendBuff & "O|1|" & pSampleInfo.SEQNO & "^" & Left(Trim(pSampleInfo.ID) & Space(13), 13) & "^1^" _
                        & Trim(pSampleInfo.RACK) & "^" & Trim(pSampleInfo.POS) & "|R2|"
            Else
                sSendBuff = sSendBuff & "O|1|" & pSampleInfo.SEQNO & "^" & Left(Trim(pSampleInfo.ID) & Space(13), 13) & "^1^" _
                    ...
                    Select Case Trim$(pSampleInfo.IFCD(iCnt))
                        Case "989", "990", "991"
                            ...
                            If Val(InStr(1, sSendBuff, "989" & "/")) = 0 Then
                                sSendBuff = sSendBuff & "^^^989/" & Trim$(pSampleInfo.SVOL(iCnt)) & "\"
                            End If
                            If Val(InStr(1, sSendBuff, "990" & "/")) = 0 Then
                                sSendBuff = sSendBuff & "^^^990/" & Trim$(pSampleInfo.SVOL(iCnt)) & "\"
                            End If
                            If Val(InStr(1, sSendBuff, "991" & "/")) = 0 Then
                                sSendBuff = sSendBuff & "^^^991/" & Trim$(pSampleInfo.SVOL(iCnt)) & "\"
                            End If
                            
                        Case Else
                            '일반항목
                            sSendBuff = sSendBuff & "^^^" & Trim$(pSampleInfo.IFCD(iCnt)) & "/" _
                                    & Trim$(pSampleInfo.SVOL(iCnt)) & "\"
                    End Select
            ...
            If pSampleInfo.SINDEX = True And m_p_sRerunGbn <> "R" Then       'Serum Index
...
Send_Terminate:
    ...
    If pSampleInfo.ORDCNT > 0 And sReqStatusCd = "O" Then
        If Trim(sNextSend) = "" And m_iSendPhase <> 2 Then
            If m_p_sRerunGbn = "R" Then
                RaiseEvent SendOrderOK(pSampleInfo.ID, pSampleInfo.SEQNO, "R", pSampleInfo.POS)
            Else
                RaiseEvent SendOrderOK(pSampleInfo.ID, pSampleInfo.SEQNO, pSampleInfo.RACK, pSampleInfo.POS)
            End If
        End If
...
Private Sub Get_OrderString()
    ...
    If m_p_sRerunGbn <> "R" Then
        m_p_sTSVol = String(m_p_iOrdCnt, "|")
    End If
    ReDim tmpData2(m_p_iOrdCnt) As String
    tmpData2() = Split(m_p_sTSVol, Chr(124))
