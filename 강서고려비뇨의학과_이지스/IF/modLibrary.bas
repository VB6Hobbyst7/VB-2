...
'-- 생년월일로 나이를 찾아온다.
'-- 인수 :  생년월일(YYYYMMDD),현재날자
Public Function CalAge(ByRef asPNRN As String, ByVal asCurDate As String) As String
    Dim sBirth As String
    'Dim sStart As String
    
    CalAge = ""
    
    sBirth = Mid(asPNRN, 1, 4) & "/" & Mid(asPNRN, 5, 2) & "/" & Mid(asPNRN, 7, 2)
    
    CalAge = DateDiff("yyyy", sBirth, asCurDate) + 1
    CalAge = CalAge - 1
