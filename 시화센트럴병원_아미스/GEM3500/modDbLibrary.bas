...
'                SQL = ""
'                SQL = SQL & "UPDATE RESULTOFNUM R SET "
'                SQL = SQL & " R.NUMRESULTVAL    = '" & sResult & "'"
'                SQL = SQL & ",R.TEXTRESULTVAL   = '" & sResult & "'"
'                SQL = SQL & ",R.RESULTINDATE    = TO_CHAR(SYSDATE,'YYYYMMDD'),"
'                SQL = SQL & ",R.RESULTINTIME    = TO_CHAR(SYSDATE,'HH24MI'),"
'                SQL = SQL & ",R.RESULTINID      = ?,"
'                'SQL = SQL & ",R.DELTAFLAG       = ?,"
'                SQL = SQL & ",R.PRINTFLAG = 5"
'                SQL = SQL & ",R.RESULTFLAG = 1"
'                SQL = SQL & " WHERE R.SPCMNO        = '" & lsID & "'"
'                SQL = SQL & "   AND R.RESULTFLAG      < 3"
'                SQL = SQL & "   AND UPPER(R.RESULTITEMCODE) = '" & UCase(strEqpCd) & "'"
                
'                SQL = SQL & "   AND UPPER(R.RESULTITEMCODE) IN (SELECT L.RESULTITEMCODE"
'                SQL = SQL & "                                     FROM LABRSLTITEMBASES L"
'                SQL = SQL & "                                    WHERE TRIM(UPPER(NVL(L.GRPRESULTITEMCODE, L.RESULTITEMCODE))) = UPPER())"
                
'1)  R.NUMRESULTVAL : 수치형 결과
'2)  R.TEXTRESULTVAL: 텍스트형 결과
'3)  R.RESULTINID: 진행샅태 : 결과입력자ID
'4)  R.DELTAFLAG: 델타여부(0:델타아님, 1:델타)
'5)  R.SPCMNO : 검체번호
'6)  L.RESULTITEMCODE : 결과코드 (LABRSLTITEMBASES)
                ...
                If Res = 1 Then
                    ...
                    If InStr(gOrderCD, ",") > 0 Then
                        SQL = SQL & "   AND ORDERCODE IN " & gOrderCD & vbCr
                    Else
                        SQL = SQL & "   AND ORDERCODE = '" & gOrderCD & "'" & vbCr
                    End If
...
'2. 결과값 update 후, Orderinfos 의 Orderstepstate = 4 로 업데이트 합니다.
'3. 결과값 update 후, Registinfos 의 RsvAcptState = 4, ResultState = 1 로 업데이트 합니다.
...
'    SQL = SQL & "   AND O.ORDERCODE = '" & gOrderCD & "'"
    If InStr(gOrderCD, ",") > 0 Then
        SQL = SQL & "   AND O.ORDERCODE IN " & gOrderCD & vbCr
    Else
