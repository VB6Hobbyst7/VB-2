...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If lsID <> "" And strRegDate <> "" And sResult <> "" Then
                '-- 결과업데이트
                cn_Ser.Execute "Exec Interface_SetPatientResult02 '" & strRegDate & "'," & _
                                                                       lsPid & ",'" & _
                                                                       mGetP(strSubCodes, 1, "|") & "','" & _
                                                                       mGetP(strSubCodes, 2, "|") & "','" & _
                                                                       mGetP(strSubCodes, 3, "|") & "','" & _
                                                                       sResult & "','','',0,0,0,'M010','" & strReturn & "'"


                'cn_Ser.Execute "Exec Interface_SetPatientResult02 '" & strRegDate & "'," & lsPid & ",'" & mGetP(strSubCodes, 1, "|") & "','" & mGetP(strSubCodes, 2, "|") & "','" & mGetP(strSubCodes, 3, "|") & "',
                '                                                   '" & sResult & "','','',0,0,0,'M010','" & strReturn & "'"
...
'    Set RS = cn_Ser.Execute("Exec Interface_GetPatientResult02 '" & gWKCD & "','" & strRegDate & "'," & lngRegNo)
    ...
    SetRawData "[환자조회]" & RS.RecordCount
    ...
    With frmInterface
        If Not RS.EOF = True And Not RS.BOF = True Then
            Do Until RS.EOF
                ...
                SQL = SQL & "   FROM LC26_SQLAB..LabRegResult " & vbCrLf
                ...
                If Not RS1.EOF = True And Not RS1.BOF = True Then
                    Do Until RS1.EOF
                        '-- 화면에 표시
                        For intCol = colState + 1 To .vasID.MaxCols
                            If Trim(RS1.Fields("TestSubCode")) = gArrEquip(intCol - colState, 3) Then
                                .vasID.Row = asRow
                                .vasID.Col = intCol
                                .vasID.BackColor = vbYellow
                                '-- 결과저장용 SEQ
