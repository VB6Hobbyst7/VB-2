...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
'                SQL = ""
'                SQL = SQL & "Update twexam_general_sub              " & vbCrLf
'                SQL = SQL & "   Set RESULT1     = '" & sResult & "' " & vbCrLf  '검사결과
'                SQL = SQL & " Where PTNO        = '" & lsPid & "'   " & vbCrLf  '환자번호
...
'                SQL = SQL & "   and verify      <> 'Y'"
                ...
                SQL = SQL & " Where PTNO        =  (select ptno                                             " & vbCrLf
                SQL = SQL & "                         From twexam_order                                     " & vbCrLf
                SQL = SQL & "                        where geomch_no    = '" & lsID & "'                    " & vbCrLf
'                SQL = SQL & "                          and jeobsudt     = to_date('" & strHospDate & "', 'yyyy/mm/dd hh24/mi/ss') " & vbCrLf
                SQL = SQL & "                          and itemcd       = '" & strEqpCd & "')               " & vbCrLf
'                SQL = SQL & "   and jeobsudt    = to_date('" & strHospDate & "', 'yyyy/mm/dd hh24/mi/ss')   " & vbCrLf
                SQL = SQL & "   and itemcd      = '" & strEqpCd & "'                                        " & vbCrLf
                SQL = SQL & "   and verify      <> 'Y'                                                      " & vbCrLf
...
Function GetSampleInfoW_TWIN(ByVal asRow As Long) As Integer
    ...
    SQL = SQL & "  From twexam_general_sub a, tw_mis_pmpa.twbas_patient b, twexam_order c       " & vbCrLf
    SQL = SQL & " Where c.geomch_no = '" & sBarcode & "'                                        " & vbCrLf   ' 검체번호 10 자리 : 2106230748
    SQL = SQL & "   And a.ptno      = b.ptno                                                    " & vbCrLf
    SQL = SQL & "   And a.ptno      = c.ptno                                                    " & vbCrLf
'    SQL = SQL & "   And a.jeobsudt  = c.jeobsudt                                                " & vbCrLf
    SQL = SQL & "   And a.itemcd    = c.itemcd                                                  " & vbCrLf
'    SQL = SQL & "   And a.jeobsudt  = to_date('" & strDate & "', 'yyyy/mm/dd hh24/mi/ss')       " & vbCrLf
    SQL = SQL & "   And a.itemcd    in (" & gAllExam & ")                                       " & vbCrLf
    SQL = SQL & "   And a.verify <> 'Y'                                                         " & vbCrLf
