...
On Error GoTo ErrHandle
    ...
    End If
'    SQL = SQL & "   AND a.SUTAK_CD   = 'LAB'            " & vbCrLf
...
'-- 검사자 정보 가져오기
Sub GetSampleCode_EASYS(ByVal asBarcode As String)
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = SQL & "     , H1OPDIN b               " & vbCrLf
    SQL = SQL & "     , HZ_MST_PTNT c           " & vbCrLf
    SQL = SQL & " WHERE a.RECEPT_NO = '" & asBarcode & "'  " & vbCrLf
    If gAllTestCd <> "" Then
        SQL = SQL & "   AND a.ORD_CD IN (" & gAllTestCd & ")" & vbCrLf
    End If
    SQL = SQL & "   AND a.RECEPT_NO = b.RECEPT_NO           " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            strTestNM = GetTestNm(Trim(RS.Fields("ITEM")))
            ...
            Call SetText(frmInterface.spdPatOrder, Trim(RS.Fields("ITEM")), frmInterface.spdPatOrder.MaxRows, 2)
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleCode_EASYS" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
