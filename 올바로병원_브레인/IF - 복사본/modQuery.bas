...
'                    SQL = ""
'                    SQL = SQL & "UPDATE oslabws " & vbCrLf
'                    SQL = SQL & "   SET slabws_result = '" & sResult & "'" & vbCrLf
'                    SQL = SQL & "  FROM oslabws INNER JOIN oslabs       ON slabws_slab    = slabs_key               " & vbCrLf
'                    SQL = SQL & "                                      AND slabws_scnt    = slabs_cnt               " & vbCrLf
'                    SQL = SQL & "               INNER JOIN ospecislab   ON specis_date    = slabws_date             " & vbCrLf
'                    SQL = SQL & "                                      AND specis_cnt     = slabws_cnt              " & vbCrLf
'                    SQL = SQL & "               INNER JOIN Ospecimen    ON speci_date     = specis_date             " & vbCrLf
'                    SQL = SQL & "                                      AND speci_seqno    = specis_seqno            " & vbCrLf
'                    SQL = SQL & "                                      AND speci_date     = '" & strOrdCd & "'      " & vbCrLf
'                    SQL = SQL & "                                      AND speci_seqno    = '" & strTestCdSub & "'  " & vbCrLf
'                    SQL = SQL & "                                      AND slabs_rcode    IN ('" & strTestCd & "')  " & vbCrLf
                
                    SQL = ""
                    SQL = SQL & "Update OSLABWS "
                    'SQL = SQL & "   Set slabws_cresult      = '" & sResult & "'     " & vbCrLf
                    SQL = SQL & "   Set slabws_result      = '" & sResult & "'     " & vbCrLf
                    SQL = SQL & " Where slabws_date         = '" & strHospDate & "' " & vbCrLf
                    SQL = SQL & "   And slabws_cnt          = (Select slabw_cnt                                     " & vbCrLf
                    SQL = SQL & "                                From Oslabw                                        " & vbCrLf
                    SQL = SQL & "                               Where slabw_cham            = '" & strBarcode & "'  " & vbCrLf
                    SQL = SQL & "                                 And slabw_date            = '" & strHospDate & "' " & vbCrLf
                    SQL = SQL & "                                 And RTRIM([slabw_time])   = '" & strPatID & "'    " & vbCrLf
                    ...
                    SQL = SQL & "   And RTRIM(slabws_scnt)  = " & strTestCdSub & "  " & vbCrLf
                    SQL = SQL & "   And RTRIM(slabws_momu)  = '" & mGetP(strTestCd, 1, "|") & "'   " & vbCrLf
                    
'    SQL = ""
'    SQL = SQL & "SELECT DISTINCT "
'    SQL = SQL & " B.slabw_date as HOSPDATE "
'    SQL = SQL & ",B.slabw_cham as BARCODE "
'    SQL = SQL & "     , A.CHAM_WHANJA             AS PNAME    " & vbCrLf
'    SQL = SQL & "     , A.CHAM_JUMIN1             AS JUMIN1   " & vbCrLf
'    SQL = SQL & "     , A.CHAM_JUMIN2             AS JUMIN2   " & vbCrLf
'    SQL = SQL & "     , B.SLABW_INOUT             AS INOUT    " & vbCrLf
'    SQL = SQL & "  FROM BRWONMU..WCHAM  A, OSLABW B, OSLABWS C" & vbCrLf
'    SQL = SQL & " Where B.SLABW_DATE = C.SLABWS_DATE" & vbCrLf
'    SQL = SQL & "   AND A.CHAM_KEY      = '" & strBarcode & "'  " & vbCrLf
'    SQL = SQL & "  and A.CHAM_KEY          = B.slabw_cham" & vbCrLf
...
'    SQL = SQL & "  AND C.SLABWS_DATE   = '" & strRegDate & "'  " & vbCrLf
'    SQL = SQL & "  and C.slabws_momu IN (" & gAllTestCd & ")" & vbCrLf
'
...
'    SQL = SQL & "   AND R.RESULTFLAG    = 0                                 " & vbCrLf
    ...
    SQL = SQL & "   AND O.RESULTSTATE  = '0'                                " & vbCrLf '결과상태
...
Public Sub GetWorkList_BRAIN(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
''    SQL = SQL & "     , CHAM_JUMIN1             AS JUMIN1   " & vbCrLf
''    SQL = SQL & "     , CHAM_JUMIN2             AS JUMIN2   " & vbCrLf
''    SQL = SQL & "     , SLABW_INOUT             AS INOUT    " & vbCrLf
''    SQL = SQL & "     , SPECI_DATE              AS CHARTNO  " & vbCrLf
''    SQL = SQL & "     , SPECI_SEQNO             AS PID      " & vbCrLf '결과저장 Key
''    'SQL = SQL & "     , COUNT(E.slabs_rcode)    AS CNT      " & vbCrlf
''    SQL = SQL & "  FROM BRWONMU..WCHAM  A   INNER JOIN OSLABW       B   ON  A.CHAM_KEY       = B.SLABW_CHAM         " & vbCrLf
''    SQL = SQL & "                                                       AND B.slabw_status   = '1'                  " & vbCrLf    '(0:대기,1:접수,2:완료)
''
...
'''    SQL = SQL & "                           INNER JOIN Ospecislab   F   ON  B.SLABS_cnt      = F.SPECIS_CNT         " & vbCrLf
...
''    SQL = SQL & "                               AND F.specis_seqno = S.speci_seqno      " & vbCrLf
''
''    'SQL = SQL & " GROUP BY SLABW_INOUT, SLABWS_DATE, CHAM_KEY, CHAM_WHANJA, SPECI_DATE, SPECI_SEQNO " & vbCrlf
''    SQL = SQL & " ORDER BY SPECI_DATE, SPECI_SEQNO          " & vbCrLf
''
''
''    SQL = ""
''    SQL = SQL & "SELECT slabs_cnt, slabs_momu, slabs_name, slabs_rcode, slabs_scode, slabs_unit "
''    SQL = SQL & "  FROM [Oslabs]"
''    SQL = SQL & " where slabs_key = 'B3341' "
''    SQL = SQL & "   And [Oslabs].[slabs_use] ='1'"
''    SQL = SQL & "   And [Oslabs].[slabs_edate] ='99991231'ORDER BY  [slabs_unit]"
''
''
''    SQL = ""
''    SQL = SQL & "UPDATE [Oslabws] SET slabws_cresult = '-' "
''    SQL = SQL & "WHERE (slabws_date = '20200316') "
''    SQL = SQL & "AND (slabws_cnt = (select slabw_cnt "
''    SQL = SQL & "                     from Oslabw "
''    SQL = SQL & "                    where slabw_cham= '0000008336' "
''    SQL = SQL & "                      and slabw_date ='20200316' "
''    SQL = SQL & "                      AND RTRIM([slabw_time]) =  '2028' "
''    SQL = SQL & "                      and slabws_date = slabw_date and slabws_dept = slabw_dept and slabws_cnt = slabw_cnt and slabws_slab = slabw_slab AND slabw_status <>'2')) "
''    SQL = SQL & " AND (RTRIM(slabws_slab) = 'UA 7종') "
''    SQL = SQL & " AND (RTRIM(slabws_scnt) = 2) "
''    SQL = SQL & " AND (RTRIM(slabws_momu) = 'D2252')"
''
''2020-03-17 오후 5:49:39 : [저장문] : UPDATE [Oslabw] SET slabw_cdate = '20200317', slabw_ctime = '1749', slabw_cuser = '정예주', slabw_status = '1' WHERE (slabw_date = '20200316') AND (slabw_cnt = (select slabws_cnt from Oslabws where slabw_cham= '0000008336' and slabw_date ='20200316' AND RTRIM([slabw_time]) =  '2028' AND RTRIM([slabws_scnt]) = 2  AND RTRIM([slabws_momu]) = 'D2252' and slabws_date = slabw_date and slabws_dept = slabw_dept and slabws_cnt = slabw_cnt and slabws_slab = slabw_slab)) AND (slabw_slab = 'UA 7종') AND (slabw_cham = '0000008336') AND  slabw_status <>'2'
''2020-03-17 오후 5:49:39 : [저장문] : UP
''
        
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "  B.SLABW_DATE             AS HOSPDATE         " & vbCrLf
    SQL = SQL & ", B.SLABW_CHAM             AS BARCODE          " & vbCrLf
    SQL = SQL & ", A.CHAM_WHANJA            AS PNAME            " & vbCrLf
    SQL = SQL & ", A.CHAM_JUMIN1            AS JUMIN1           " & vbCrLf
    SQL = SQL & ", A.CHAM_JUMIN2            AS JUMIN2           " & vbCrLf
    SQL = SQL & ", B.SLABW_INOUT            AS INOUT            " & vbCrLf
    SQL = SQL & "  FROM BRWONMU..WCHAM A, OSLABW B, OSLABWS C   " & vbCrLf
    SQL = SQL & " Where A.CHAM_KEY      = B.SLABW_CHAM          " & vbCrLf
    SQL = SQL & "   And B.SLABW_STATUS  = '1'                   " & vbCrLf  '(0,1,2)
    SQL = SQL & "   And B.SLABW_DATE    = C.SLABWS_DATE         " & vbCrLf
    SQL = SQL & "   And B.SLABW_DEPT    = C.SLABWS_DEPT         " & vbCrLf
    SQL = SQL & "   And B.SLABW_CNT     = C.SLABWS_CNT          " & vbCrLf
    SQL = SQL & "   And B.SLABW_SLAB    = C.SLABWS_SLAB         " & vbCrLf
    SQL = SQL & "   And C.SLABWS_DATE   >= '" & pFrom & "'      " & vbCrLf
    SQL = SQL & "   And C.SLABWS_DATE   <= '" & pTo & "'        " & vbCrLf
    'SQL = SQL & "   And C.SLABWS_MOMU+C.SLABWS_SCNT IN (" & gAllTestCd & ") " & vbCrLf
    SQL = SQL & "   And CONCAT(RTRIM(LTRIM(C.SLABWS_MOMU)),'|',RTRIM(LTRIM(C.SLABWS_SCNT)))      IN (" & gAllTestCd & ") " & vbCrLf
    SQL = SQL & " Order By B.SLABW_DATE, B.SLABW_CHAM   "
...
'    End If
    ...
    SQL = SQL & " Order by HOSPDATE,PID,PNAME,BARCODE                               " & vbCr
...
'''    SQL = SQL & "  FROM RESINF AS R, PBSINF P                                   " & vbCrLf
...
'''    SQL = SQL & " GROUP BY R.RESACPDTM,R.RESSPMNUM,R.RESCHTNUM,P.PBSPATNAM      " & vbCrLf
    ...
    SQL = ""
    SQL = SQL & "SELECT Distinct " 'R.RESACPNUM,R.RESACPDTE, R.RESMZHTYP, R.RESLABCOD "
'    SQL = SQL & "     , SUBSTRING(R.RESACPDTE,1,8)  AS HOSPDATE"
    SQL = SQL & "       R.RESACPDTM                 AS HOSPDATE"
    SQL = SQL & "     , R.RESSPMNUM                 AS BARCODE "
    SQL = SQL & "     , R.RESCHTNUM                 AS CHARTNO"
    SQL = SQL & "     , P.PBSPATNAM                 AS PNAME"
'    SQL = SQL & "     , P.PBSRESNUM                 AS JUMIN"
'    SQL = SQL & "     , R.RESSHTNAM                 AS 검사명 "
    SQL = SQL & "     , COUNT(R.RESLABCOD)          AS CNT            " & vbCrLf
    ...
    'End If
    SQL = SQL & " GROUP BY R.RESACPDTM, R.RESSPMNUM, R.RESCHTNUM, P.PBSPATNAM "
    SQL = SQL & " ORDER BY R.RESACPDTM, R.RESSPMNUM, R.RESCHTNUM, P.PBSPATNAM "
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As Object) As String
    ...
    Select Case gEMR
        Case "BRAIN"
'            SQL = ""
'            SQL = SQL & "SELECT DISTINCT E.SLABS_RCODE    AS ITEM      " & vbCrLf
'            SQL = SQL & "  FROM BRWONMU..WCHAM A    INNER JOIN OSLABW   B    ON A.CHAM_KEY      = B.SLABW_CHAM          " & vbCrLf
'            SQL = SQL & "                                                   AND A.CHAM_KEY      = '" & strBarcode & "'  " & vbCrLf
'            SQL = SQL & "                                                   AND B.SLABW_STATUS  = '1'                   " & vbCrLf
'            SQL = SQL & "                           INNER JOIN OSLABWS  C    ON B.SLABW_DATE    = C.SLABWS_DATE         " & vbCrLf
...
'            SQL = SQL & "                                                   AND E.SLABS_RCODE   IN (" & gAllTestCd & ") " & vbCrLf
'            SQL = SQL & "                                                   AND E.SLABS_USE     = 1                     " & vbCrLf
'            SQL = SQL & " ORDER BY E.SLABS_RCODE    " & vbCrLf
        
            SQL = ""
            'SQL = SQL & "SELECT DISTINCT C.slabws_momu  AS ITEM         " & vbCrLf
...
'    SQL = SQL & "     , CHAM_JUMIN2             AS JUMIN2   " & vbCrLf
'    SQL = SQL & "     , SLABW_INOUT             AS INOUT    " & vbCrLf
'    SQL = SQL & "     , SPECI_DATE              AS ORDCODE  " & vbCrLf  '결과저장 Key
'    SQL = SQL & "     , SPECI_SEQNO             AS ORDSEQ   " & vbCrLf  '결과저장 Key
'    SQL = SQL & "     , E.SLABS_RCODE           AS ITEM     " & vbCrLf
'    SQL = SQL & "  FROM BRWONMU..WCHAM  A   INNER JOIN OSLABW       B   ON  A.CHAM_KEY       = B.SLABW_CHAM         " & vbCrLf
'    SQL = SQL & "                                                       AND B.SLABW_STATUS   = '1'                  " & vbCrLf    '(0:대기,1:접수,2:완료)
'    SQL = SQL & "                                                       AND A.CHAM_KEY       = '" & strBarcode & "' " & vbCrLf
'    SQL = SQL & "       INNER JOIN OSLABWS C    ON B.SLABW_DATE = C.SLABWS_DATE         " & vbCrLf
...
'    SQL = SQL & "                               AND F.specis_seqno = S.speci_seqno      " & vbCrLf
    ...
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "  B.SLABW_DATE             AS HOSPDATE         " & vbCrLf
    SQL = SQL & ", B.SLABW_TIME             AS PID              " & vbCrLf
    SQL = SQL & ", B.SLABW_CHAM             AS BARCODE          " & vbCrLf
    SQL = SQL & ", A.CHAM_WHANJA            AS PNAME            " & vbCrLf
    SQL = SQL & ", A.CHAM_JUMIN1            AS JUMIN1           " & vbCrLf
    SQL = SQL & ", A.CHAM_JUMIN2            AS JUMIN2           " & vbCrLf
    SQL = SQL & ", B.SLABW_INOUT            AS INOUT            " & vbCrLf
    'SQL = SQL & ", C.SLABWS_MOMU            AS ITEM             " & vbCrLf
    SQL = SQL & ", CONCAT(RTRIM(LTRIM(C.SLABWS_MOMU)),'|',RTRIM(LTRIM(C.SLABWS_SCNT))) AS ITEM             "
    SQL = SQL & ", C.SLABWS_SLAB            AS ORDCODE          " & vbCrLf
    SQL = SQL & ", C.SLABWS_SCNT            AS ORDSEQ           " & vbCrLf
    SQL = SQL & "  FROM BRWONMU..WCHAM  A, OSLABW B, OSLABWS C  " & vbCrLf
    SQL = SQL & " Where A.CHAM_KEY      = B.SLABW_CHAM          " & vbCrLf
    SQL = SQL & "   And A.CHAM_KEY      = '" & strBarcode & "'  " & vbCrLf
    SQL = SQL & "   And B.SLABW_STATUS  = '1'                   " & vbCrLf  '(0,1,2)
