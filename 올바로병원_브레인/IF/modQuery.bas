    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            ...
            If Trim(AdoRs_Local.Fields("TESTCODE").Value) <> "" Then
                ...
                Else
                    gAllTestCd = gAllTestCd & ",'" & AdoRs_Local.Fields("TESTCODE").Value & "'"
                    If strPrevTestCd <> mGetP(AdoRs_Local.Fields("TESTCODE").Value, 1, "|") Then
                        gAllTestCd_F = gAllTestCd_F & ",'" & mGetP(AdoRs_Local.Fields("TESTCODE").Value, 1, "|") & "'"
                    End If
                End If
                strPrevTestCd = mGetP(AdoRs_Local.Fields("TESTCODE").Value, 1, "|")
            End If
            AdoRs_Local.MoveNext
        Loop
    End If
...
'    SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
    If frmInterface.optSch(0).Value = True Then
'        SQL = SQL & "                               AND B.slabw_status = '0'                " & vbCr
    ElseIf frmInterface.optSch(1).Value = True Then
        SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
    ElseIf frmInterface.optSch(2).Value = True Then
...
'        SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
    ...
    SQL = SQL & "                               AND C.slabws_date <= '" & pTo & "'      " & vbCrLf
    SQL = SQL & "                               And CONCAT(RTRIM(LTRIM(C.SLABWS_MOMU)),'|',RTRIM(LTRIM(C.SLABWS_SCNT)))      IN (" & gAllTestCd & ") " & vbCrLf
    SQL = SQL & "                               And (C.slabws_result = '' OR C.slabws_result IS NULL)" & vbCrLf
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    'SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & Format(Trim(RS.Fields("PID")), "0000"), intRow, colBARCODE
...
'    SQL = SQL & " Order By ITEM "
    ...
    SQL = SQL & "       SLABW_INOUT             AS INOUT    " & vbCrLf
    SQL = SQL & "     , SLABWS_DATE             AS HOSPDATE " & vbCrLf
    SQL = SQL & "     , SLABW_TIME              AS TIME     " & vbCrLf
    SQL = SQL & "     , CHAM_KEY                AS BARCODE  " & vbCrLf
    SQL = SQL & "     , CHAM_WHANJA             AS PNAME    " & vbCrLf
    SQL = SQL & "     , CHAM_JUMIN1             AS JUMIN1   " & vbCrLf
    SQL = SQL & "     , CHAM_JUMIN2             AS JUMIN2   " & vbCrLf
    SQL = SQL & "     , SPECI_DATE              AS CHARTNO  " & vbCrLf
    SQL = SQL & "     , SPECI_SEQNO             AS PID      " & vbCrLf
...
'    SQL = SQL & "                               AND E.slabs_rcode IN (" & gAllTestCd & ")" & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
                'SetText SPD, Trim(RS.Fields("BARCODE")) & "", asRow, colBARCODE
                SetText SPD, Trim(RS.Fields("CHARTNO")) & Format(Trim(RS.Fields("PID")), "0000"), asRow, colBARCODE
                
                SetText SPD, Trim(RS.Fields("BARCODE")) & "", asRow, colCHARTNO
                
                SetText SPD, Trim(RS.Fields("TIME")) & "", asRow, colPID
                SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                SetText SPD, IIf(Trim(RS.Fields("INOUT")) & "" = "O", "외래", "입원"), asRow, colINOUT
                SetText SPD, mPatient.AGE, asRow, colPAGE
                ...
                '오더정보에 저장
                With mOrder
                    .BarNo = Trim(RS.Fields("CHARTNO")) & Format(Trim(RS.Fields("PID")), "0000")
                    .PNAME = Trim(RS.Fields("PNAME")) & ""
                    .Count = CStr(intTestCnt)
                    .NoOrder = False
                End With
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If GetTestNm(Trim(RS.Fields("ITEM")), False) = gArrEQP(intCol - colSTATE, 6) Then
                        ...
                        '-- 처방코드
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("CHARTNO")) & ""
                        '-- 처방일련번호
                        gArrEQP(intCol - colSTATE, 17) = Trim(RS.Fields("TIME")) & ""
    ...
    Call SetSQLData("바코드조회", mOrder.PNAME & ":" & gPatOrdCd & vbCrLf, "A")
