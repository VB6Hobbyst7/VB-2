...
Function SaveTransData_NPH(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
    Dim RsCmnt          As ADODB.Recordset
    ...
    Dim strPosNo        As String
    Dim strPosCmnt      As String
    Dim strIntCmnt      As String
    Dim strEVal         As String
    Dim strRdRpVal      As String
    Dim strNVal         As String
    Dim strICVal        As String
    Dim strCmnt         As String
    Dim strCmntOrg      As String
    Dim strInUp         As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        strPatNm = Trim(GetText(SPD, argSpcRow, colPNAME))
        strChartNo = Trim(GetText(SPD, argSpcRow, colCHARTNO))
        strRackNo = Trim(GetText(SPD, argSpcRow, colRACKNO))
        strPosNo = Trim(GetText(SPD, argSpcRow, colPOSNO))
        
        If Trim(strBarcode) = "" Then
            Exit Function
        End If
        
        If Len(strBarcode) < 3 Then
            Exit Function
        End If
        
        '-- Local에서 환자별로 결과값 가져오기
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMCODESUB,EQUIPRESULT,RESULT,REFJUDGE,SEQNO    " & vbCrLf
        SQL = SQL & "  FROM PATRESULT                                                               " & vbCrLf
        SQL = SQL & " WHERE EXAMDATE    = '" & strExamDate & "'                                     " & vbCrLf
        SQL = SQL & "   AND SAVESEQ     = " & strSaveSeq & vbCrLf
        SQL = SQL & "   AND BARCODE     = '" & strBarcode & "'                                      " & vbCrLf
        SQL = SQL & "   AND EXAMCODE    <> ''                                                       " & vbCrLf
        SQL = SQL & "   AND (RESULT <> '' OR RESULT IS NOT NULL)                                    " & vbCrLf
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                Debug.Print RsLocal.Fields("SEQNO").Value & ""
                strEqpcd = RsLocal.Fields("EQUIPCODE").Value & ""
                strOrdCd = RsLocal.Fields("ORDERCODE").Value & ""
                strTestCd = RsLocal.Fields("EXAMCODE").Value & ""
                strTestCdSub = RsLocal.Fields("EXAMCODESUB").Value & ""
                ...
                Else
                    If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                        ...
                        ElseIf strTestCd = "NNNNN" Then
                        ElseIf strTestCd = "Comment" Then
                        Else
                            '-- QC결과저장
                            If Mid(strBarcode, 2, 1) = "9" Then
                                SQL = ""
                                SQL = SQL & "Update SPSLHQRST                           " & vbCrLf
                                SQL = SQL & "   Set RSLT_VALU   = '" & sResult & "'     " & vbCrLf
                                SQL = SQL & "      ,RSLT_DT     = SYSDATE               " & vbCrLf
                                SQL = SQL & "      ,UPDT_DT     = SYSDATE               " & vbCrLf
                                SQL = SQL & " Where SPCM_NO     = '" & strBarcode & "'  " & vbCrLf
                                SQL = SQL & "   AND SBSN_CD     = '" & strPatID & "'    " & vbCrLf
                                SQL = SQL & "   And EXMN_CD     = '" & strTestCd & "'   " & vbCrLf
                                
                                Call SetSQLData("QC결과저장", SQL, "A")
                                AdoCn.Execute SQL
                                
                            End If
                            '-- 결과저장
                            SQL = ""
                            SQL = SQL & "UPDATE SPSLHRRST SET                           " & vbCrLf
                            SQL = SQL & "  REAL_RSLT    = '" & sResult & "'             " & vbCrLf
                            SQL = SQL & ", VIEW_RSLT    = '" & sResult & "'             " & vbCrLf
                            SQL = SQL & ", RSLT_INPS_ID = '" & gHOSP.USERID & "'        " & vbCrLf
                            SQL = SQL & ", AMEN_ID      = '" & gHOSP.USERID & "'        " & vbCrLf
                            SQL = SQL & ", RSLT_INPT_DT = SYSDATE                       " & vbCrLf
                            SQL = SQL & ", EXMN_EQPM    = '" & gHOSP.MACHCD & "'        " & vbCrLf
                            SQL = SQL & ", EXMN_RCPN_NO = '" & strRackNo & strPosNo & "'" & vbCrLf
                            SQL = SQL & ", UPDT_DT      = SYSDATE                       " & vbCrLf
                            SQL = SQL & ", RSLT_NO      = 1                             " & vbCrLf
                            SQL = SQL & ", RSLT_STAT    = '1'                           " & vbCrLf
                            SQL = SQL & ", REEXAMYN     = 'N'                           " & vbCrLf
                            If strCmntOrg <> "" Then
                                '검사별 결과등록화면 아래에 있는 코멘트
                                SQL = SQL & ", EXMN_PER_OPNN     = '" & strCmntOrg & "' " & vbCrLf
                            End If
                            SQL = SQL & " Where SPCM_NO = FN_LABCVTBCNO(NVL('" & strBarcode & "', '-'))" & vbCrLf
                            SQL = SQL & "   AND EXMN_CD = '" & Trim(strTestCd) & "'     " & vbCrLf
                            SQL = SQL & "   AND RSLT_STAT IN ('0','1','4')              " & vbCrLf
                            ...
                            If strCmntOrg <> "" Then
                                ...
                                SQL = ""
                                SQL = SQL & "Select EXMN_CD,RSLT_NO,OPNN_CD     " & vbCrLf
                                SQL = SQL & "  From SPSLMRRMK                   " & vbCrLf
                                SQL = SQL & " Where SPCM_NO = FN_LABCVTBCNO(NVL('" & strBarcode & "', '-'))"
                                Call SetSQLData("소견저장", SQL, "A")
                                Set RsCmnt = New ADODB.Recordset
                                Set RsCmnt = AdoCn.Execute(SQL, , 1)
                                If Not RsCmnt.EOF = True And Not RsCmnt.BOF = True Then
                                    Do Until RsCmnt.EOF
                                        strInUp = "UPDATE"
                                        RsCmnt.MoveNext
                                    Loop
                                ...
                                If strInUp = "UPDATE" Then
                                    SQL = ""
                                    SQL = SQL & "UPDATE SPSLMRRMK SET                           " & vbCrLf
                                    SQL = SQL & "  OPNN    = '" & strCmntOrg & "'               " & vbCrLf
                                    'SQL = SQL & ", AMEN_ID      = '" & gHOSP.USERID & "'        " & vbCrLf
                                    'SQL = SQL & ", UPDT_DT      = SYSDATE                       " & vbCrLf
                                    SQL = SQL & " Where SPCM_NO = FN_LABCVTBCNO(NVL('" & strBarcode & "', '-'))" & vbCrLf
                                    Call SetSQLData("소견저장", SQL, "A")
                                ...
                                ElseIf strInUp = "INSERT" Then
'                                    SQL = ""
'                                    SQL = SQL & "delete from SPSLMRRMK "
'                                    SQL = SQL & " WHERE SPCM_NO= FN_LABCVTBCNO(NVL('" & strBarcode & "', '-')) "
'                                    SQL = SQL & "   And EXMN_CD = 'I44' "
'
'                                    Call SetSQLData("소견저장", SQL, "A")
'                                    AdoCn.Execute SQL
                                    ...
                                    SQL = ""
                                    SQL = SQL & "INSERT INTO SPSLMRRMK                          " & vbCrLf
                                    SQL = SQL & "(SPCM_NO, EXMN_CD,RSLT_NO,OPNN_CD,OPNN,REGI_ID,RGST_DT,AMEN_ID,UPDT_DT) " & vbCrLf
                                    SQL = SQL & " Values                                        " & vbCrLf
                                    SQL = SQL & "(FN_LABCVTBCNO(NVL('" & strBarcode & "', '-')) " & vbCrLf
                                    SQL = SQL & ", 'I44',1,'FLAG','" & strCmntOrg & "'          " & vbCrLf
                            ...
                            '-- 상태변경
                            SQL = ""
                            SQL = SQL & "UPDATE SPSLMJBBI SET                       " & vbCrLf
                            SQL = SQL & "  RSLT_STAT    = 1                         " & vbCrLf
                            SQL = SQL & ", AMEN_ID      = '" & gHOSP.USERID & "'    " & vbCrLf
                            SQL = SQL & ", UPDT_DT      = SYSDATE                   " & vbCrLf
                            SQL = SQL & " Where SPCM_NO = FN_LABCVTBCNO(NVL('" & strBarcode & "', '-'))" & vbCrLf
                            SQL = SQL & "   AND RSLT_STAT <> '3'                    " & vbCrLf
                            SQL = SQL & "   AND SPCM_STAT <= '3'                    " & vbCrLf
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_NPH" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
'        End If
        ...
        If strAllResult <> "" Then
            ...
            sParam = sParam & "ex_interface=" & frmMain.txtTestID.Text & "|" & gHOSP.HOSPCD & "&"       '사용자ID|기관코드
            ...
            sParam = sParam & "userid=" & frmMain.txtTestID.Text & "&"                                '사용자ID
            
            sRcvData = OpenURLWithIE2(gHOSP.APIURL & sParam, frmMain.Inet1)

            Call SetSQLData("결과저장", "Param:" & gHOSP.APIURL & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine, "A")
...
'        End If
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            Do Until RS_L.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    strAllResult = strAllResult & strOrdCd & ""        'IF코드
                    strAllResult = strAllResult & sResult & ""         '결과값
                    strAllResult = strAllResult & sComment & ""        '코멘트
                    strAllResult = strAllResult & strDate & ""         '검사일시
                End If
                RS_L.MoveNext
            Loop
        End If
        ...
        If strAllResult <> "" Then
            ...
            Call SetSQLData("결과저장", "Param>>" & "http://" & gHOSP.APIURL & "/lis/jindangeomsaweb/GyeolGwaIF.live?Mode=reqGetIFGyeolGwaDR" & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine, "A")
            ...
            '소견저장
            sParam = ""
            sParam = sParam & "&Data1=" & strPatID              '등록번호
            sParam = sParam & "&Data2=" & strBarcode            '바코드번호
            If frmMain.optTest(0).Value = True Then
                gCFXCmnt.SGCOVID = Replace(gCFXCmnt.SGCOVID, "CHR(10)CHR(13)", "%0D")
                sParam = sParam & "&Data3=" & gCFXCmnt.SGCOVID  '의사소견 내용
            ElseIf frmMain.optTest(1).Value = True Then
                gCFXCmnt.RV16 = Replace(gCFXCmnt.RV16, "CHR(10)CHR(13)", "%0D")
                sParam = sParam & "&Data3=" & gCFXCmnt.RV16     '의사소견 내용
            ElseIf frmMain.optTest(2).Value = True Then
                gCFXCmnt.PB5 = Replace(gCFXCmnt.PB5, "CHR(10)CHR(13)", "%0D")
                sParam = sParam & "&Data3=" & gCFXCmnt.PB5      '의사소견 내용
            ElseIf frmMain.optTest(3).Value = True Then
                gCFXCmnt.SDCOVID = Replace(gCFXCmnt.SDCOVID, "CHR(10)CHR(13)", "%0D")
                sParam = sParam & "&Data3=" & gCFXCmnt.SDCOVID  '의사소견 내용
            End If
            
            sParam = sParam & "&Data4=" & gHOSP.MACHCD          '장비코드
            sParam = sParam & "&Data5=" & "91"                  '슬립코드
            
            sRcvData = OpenURLWithIE2("http://" & gHOSP.APIURL & "/lis/jindangeomsaweb/GyeolGwaIF.live?Mode=reqSetCLOBrstIF&" & sParam, frmMain.Inet1)

            Call SetSQLData("소견저장", "Param>>" & "http://" & gHOSP.APIURL & "/lis/jindangeomsaweb/GyeolGwaIF.live?Mode=reqSetCLOBrstIF&" & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine, "A")
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_KHNMC" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
'                        .CommandType = adCmdStoredProc
'
'                        Set Prm1 = .CreateParameter("IV_RSLT_VALUE", adInteger, adParamInput, 30, dblBarno)      '바코드번호
'                        .Parameters.Append Prm1
'
'                        Set Prm2 = .CreateParameter("IV_RSLT_DD", adVarChar, adParamInput, 10, strTestCd)       '처방코드
'                        .Parameters.Append Prm2
'
'                        Set Prm3 = .CreateParameter("RESULT_NM", adVarChar, adParamInput, 4000, sResult)    '결과값
'                        .Parameters.Append Prm3
...
'                    End With
                    ...
                    With AdoCmd
                        ...
                        .CommandType = adCmdStoredProc
                        .Parameters.Refresh
                        .Parameters("IV_RSLT_VALUE").Value = sResult            '검사결과
                        .Parameters("IV_RSLT_DD").Value = strDate               '입력일자
                        .Parameters("IV_RSLT_TIME").Value = strTime             '입력시간
                        .Parameters("IV_RSLT_INPS_ID").Value = gHOSP.USERID     '결과입력자
                        .Parameters("IV_SPCM_RCPN_NO").Value = strBarcode       '바코드
                        .Parameters("IV_EXMN_CD").Value = strTestCd             '오더코드
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_HCMS" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
        ...
        Do Until AdoRs_Local.EOF
            ...
            If Trim(AdoRs_Local.Fields("TESTCODE").Value) <> "" Then
                If intRow = 1 Or gAllTestCd = "" Then
                    ...
                    gAllTestCd6 = "'" & Mid(AdoRs_Local.Fields("TESTCODE").Value, 1, 6) & "'"
                Else
                    gAllTestCd = gAllTestCd & ",'" & AdoRs_Local.Fields("TESTCODE").Value & "'"
                    gAllTestCd = gAllTestCd & ",'" & Mid(AdoRs_Local.Fields("TESTCODE").Value, 1, 6) & "'"
                End If
            End If
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "NPH"
                Call GetWorkList_NPH(pFrom, pTo, SPD)
...
Public Sub GetWorkList_NPH(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    SQL = SQL & "  J2.BRCD_LABL_NO  AS BARCODE  " & vbCrLf
    SQL = SQL & ", J1.PID           AS PID      " & vbCrLf
    SQL = SQL & ", J1.SPCM_NO       AS CHARTNO  " & vbCrLf
    SQL = SQL & ", J1.PT_NM         AS PNAME    " & vbCrLf
    SQL = SQL & ", J1.SEX           AS SEX      " & vbCrLf
    SQL = SQL & ", J1.AGE           AS AGE      " & vbCrLf
    SQL = SQL & ", J1.DOBR          AS BIRTH    " & vbCrLf
    SQL = SQL & ", R1.RCPN_DT       AS HOSPDATE " & vbCrLf
    SQL = SQL & ", R1.RSLT_NO       AS WrkSeq   " & vbCrLf
    SQL = SQL & ", J1.SPCM_CD       AS SpcNm    " & vbCrLf
    SQL = SQL & ", R1.PRSC_CD       AS ITEM     " & vbCrLf
    SQL = SQL & ", D1.ENGL_CD       AS DEPTCD   " & vbCrLf
    SQL = SQL & "  FROM SPSLMJBBI J1            " & vbCrLf
    SQL = SQL & "     , SPSLMJBDI J2            " & vbCrLf
    SQL = SQL & "     , SPSLHRRST R1            " & vbCrLf
    SQL = SQL & "     , SPSLMFBIF F1            " & vbCrLf
    SQL = SQL & "     , AZCMMDEPT D1            " & vbCrLf
    SQL = SQL & " Where J1.SPCM_NO      = J2.SPCM_NO    " & vbCrLf
    SQL = SQL & "   AND R1.RSLT_STAT    <> '3'          " & vbCrLf
    SQL = SQL & "   AND J1.MED_DP       = D1.DEPT_CD    " & vbCrLf
    SQL = SQL & "   AND J2.SPCM_NO      = R1.SPCM_NO    " & vbCrLf
    SQL = SQL & "   AND R1.EXMN_CD      = F1.EXMN_CD (+)" & vbCrLf
    SQL = SQL & "   AND R1.RCPN_DT      >= F1.USE_STR_DY (+)" & vbCrLf
    SQL = SQL & "   AND R1.RCPN_DT      < F1.USE_END_DY (+) " & vbCrLf
    SQL = SQL & "   AND R1.RCPN_DT BETWEEN to_date('" & pFrom & "' , 'YYYYMMDDHH24MISS') AND to_date ('" & pTo & "', 'YYYYMMDDHH24MISS')" & vbCrLf
    SQL = SQL & "   AND F1.MNDT_YN      <> 'Y'" & vbCrLf
    SQL = SQL & "   AND J1.SLIP_CD in ('" & gHOSP.PARTCD & "')  " & vbCrLf
    SQL = SQL & "   AND SUBSTR(R1.PRSC_CD, 1, 6) IN (" & gAllTestCd & ")" & vbCrLf
    SQL = SQL & " ORDER BY R1.RCPN_DT, J1.PT_NM  " & vbCrLf
        ...
        Do Until RS.EOF
            With SPD
                For i = 1 To SPD.DataRowCnt
                    ...
                    For intCol = colSTATE + 1 To .MaxCols
                        If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                            ...
                            .Col = intCol
                            .BackColor = vbYellow
                            Call SetText(SPD, "◇", intRow, intCol)
                            Exit For
                        End If
                    Next
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    SetText SPD, Trim(RS.Fields("SEX")) & "", intRow, colPAGE
                    SetText SPD, Trim(RS.Fields("AGE")) & "", intRow, colPSEX
                    'SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
                    'SetText SPD, GetSampleITEM(intRow, SPD), intRow, colITEMS
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_NPH" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_HCMS" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
RST:
                ...
                strErrMsg = "위    치 : " & gHOSP.MACHNM & "_GetWorkList_NU" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show 'vbModal
...
'Data2: 접수일자
    ...
    sParam = ""
    sParam = sParam & "Data1=" & gHOSP.MACHCD & "&"     '장비코드
    sParam = sParam & "Data2=" & pFrom & "&"            '접수일자
    
    sRcvData = OpenURLWithIE2("http://" & gHOSP.APIURL & "/lis/jindangeomsaweb/GyeolGwaIF.live?Mode=reqGetIFPCR&" & sParam, frmMain.Inet1)
    
    Call SetSQLData("워크조회", "Return==>" & sRcvData & vbNewLine, "A")
    ...
    'Call DisplayNode_InfoS_Worklist_KHNMC("D:\프로젝트\VB\강동경희대병원\IF\XML\worklist.xml", 10)
...
'    Kill App.PATH & "\Xml\" & strXmlName
    ...
    If UBound(varRcvData) >= 1 Then
        For i = 1 To UBound(varRcvData)
            ...
            'MsgBox strIFCODE
            'MsgBox UBound(varRcvDtl)
            If UBound(varRcvDtl) >= 6 Then
                strRstFlag = Trim(mGetP(mGetP(varRcvDtl(6), 2, ">"), 1, "<"))
            End If
            'MsgBox strRstFlag
            '- = 접수
            '1 = 결과저장
                ...
                With SPD
                    ...
                    For J = 1 To SPD.DataRowCnt
                        ...
                        If strBCNO = strBarcode Then
                            blnSame = True
                            strNames = GetText(SPD, intRow, colITEMS)
                            strNames = strNames & "|" & GetTestNm_ORD(strIFCODE)
'MsgBox strNames
                    ...
                    If blnSame = False Then
                        If strRstFlag = "-" Or strRstFlag = "" Then
                            .MaxRows = .MaxRows + 1
                            intRow = .MaxRows
'                            If strRstFlag = "" Or strRstFlag = "-" Then
'                                .Row = intRow
'                                .Col = colBARCODE
'                                .BackColor = vbWhite
'                            ElseIf strRstFlag = "1" Or strRstFlag = "2" Then
'                                .Row = intRow
'                                .Col = colBARCODE
'                                .BackColor = vbCyan
'                            Else
...
RST:
                ...
                strErrMsg = "위    치 : " & gHOSP.MACHNM & "_GetWorkList_KHNMC" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show 'vbModal
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "NPH"
            SaveTransData = SaveTransData_NPH(argSpcRow, SPD)
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    'If cn_Server_Flag = True Then
        Select Case gEMR
            Case "NPH"
                    Call GetSampleInfo_NPH(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_NPH(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    If Mid(strBarcode, 2, 1) = "9" Then
        ...
        SQL = SQL & "SELECT"
        SQL = SQL & "   '' SPCM_NO                          "
        SQL = SQL & "   , SPCM_NO              AS BARCODE   "
        SQL = SQL & "   , SBSN_CD              AS PID       "
        SQL = SQL & "   , LOT_NO               AS PNAME     "
        SQL = SQL & "   , ''                   AS SEX       "
        SQL = SQL & "   , ''                   AS AGE       "
        SQL = SQL & "   , TO_CHAR(SYSDATE,'yyyyMMdd')   AS HOSPDATE "
        SQL = SQL & "   , RSLT_SQNO                     AS CHARTNO  "
        SQL = SQL & "   , '' AS SLIP_CD "
        SQL = SQL & "   , '' AS SPCM_CD "
        SQL = SQL & "   , EXMN_CD               AS ITEM"
        SQL = SQL & "   , ''                    AS DEPTCD"
        SQL = SQL & "  From SPSLHQRST"
        SQL = SQL & " WHERE SPCM_NO = '" & strBarcode & "'"
        SQL = SQL & "   AND DEL_YN  = 'N'"
        SQL = SQL & "   AND (RSLT_VALU is null OR RSLT_VALU = '')"
    Else
        SQL = ""
        SQL = SQL & "SELECT DISTINCT                    "
        SQL = SQL & "     J1.SPCM_NO        AS CHARTNO            "
        SQL = SQL & "   , J2.BRCD_LABL_NO   AS BARCODE  "
        SQL = SQL & "   , J1.PID            AS PID      "
        SQL = SQL & "   , J1.PT_NM          AS PNAME    "
        SQL = SQL & "   , J1.SEX            AS SEX      "
        SQL = SQL & "   , J1.AGE            AS AGE      "
        SQL = SQL & "   , R1.RCPN_DT                    "
        SQL = SQL & "   , J1.DOBR AS BIRTH              "
        SQL = SQL & "   , J2.BRCD_LABL_NO   AS BARCODE  "
        SQL = SQL & "   , TO_CHAR(R1.RCPN_DT,'yyyyMMdd')    AS HOSPDATE"
        SQL = SQL & "   , R1.RSLT_NO                        "
        SQL = SQL & "   , J1.SLIP_CD"
        SQL = SQL & "   , J1.SPCM_CD"
        SQL = SQL & "   , J1.PID"
        ...
        SQL = SQL & "   , R1.RCPN_DT"
        SQL = SQL & "   , J1.DOBR AS BIRTH"
        SQL = SQL & "   , TO_CHAR(R1.RCPN_DT,'yyyymmdd') AS ACPT_DT"
        SQL = SQL & "   , R1.RSLT_NO AS ACPT_NO"
        SQL = SQL & "   , J1.SLIP_CD"
        SQL = SQL & "   , J1.SPCM_CD"
        SQL = SQL & "   , R1.EXMN_CD        AS ITEM"
        SQL = SQL & "   , D1.ENGL_CD        AS DEPTCD"
        SQL = SQL & "  From"
        SQL = SQL & "      SPSLMJBDI J2"
        SQL = SQL & "     ,SPSLHRRST R1"
        SQL = SQL & "     ,SPSLMJBBI J1"
        SQL = SQL & "     ,AZCMMDEPT D1"
        SQL = SQL & "     ,SPSLMFBIF F1"
        SQL = SQL & " Where"
        SQL = SQL & " J2.BRCD_LABL_NO   = '" & strBarcode & "'"
        SQL = SQL & " AND J2.SPCM_NO    = R1.SPCM_NO"
        SQL = SQL & " AND J1.SPCM_NO    = J2.SPCM_NO"
        SQL = SQL & " AND J1.MED_DP     = D1.DEPT_CD"
        SQL = SQL & " AND R1.EXMN_CD    = F1.EXMN_CD  (+)"
        SQL = SQL & " AND R1.RCPN_DT    >= F1.USE_STR_DY  (+)"
        SQL = SQL & " AND R1.RCPN_DT    < F1.USE_END_DY  (+)"
        SQL = SQL & " AND F1.MNDT_YN    <> 'Y' "
        If gAllTestCd <> "" Then
            SQL = SQL & " AND R1.EXMN_CD IN (" & gAllTestCd & ")"
        End If
    End If
    ...
    If gPatOrdCd <> "" Then
        ...
        gPatOrdCd = gPatOrdCd & "'NNNNN',"
        gPatOrdCd = gPatOrdCd & "'Comment',"
        gPatOrdCd = Mid(gPatOrdCd, 1, Len(gPatOrdCd) - 1)
    End If
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_NPH" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
'    sParam = sParam & "instcd=" & gHOSP.HOSPCD & "&"                            '기관코드
'    sParam = sParam & "bcno=" & strBarcode                                      '검사코드
...
'    sParam = sParam & "eqmtcd=" & gHOSP.MACHCD & "&"                           '장비코드
    ...
    Call SetSQLData("바코드조회", "Param:" & gHOSP.APIURL & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine, "A")
    ...
    Call DisplayNode_InfoS(App.PATH & "\Xml\" & strXmlName, UBound(varRcvData))
    'Call DisplayNode_InfoS("D:\프로젝트\VB\__NU\인천성모병원_NU\참고\patlist2.xml", 1)
...
On Error GoTo DBErr
    ...
    sParam = ""
    sParam = sParam & "Data1=" & strBarcode     '바코드
    ...
    Call SetSQLData("바코드조회", "http://" & gHOSP.APIURL & "/lis/jindangeomsaweb/GyeolGwaIF.live?Mode=reqGetMWLBCDJoHoi&" & sParam & vbNewLine & "Return:" & sRcvData & vbNewLine, "A")
...
'''sRcvData = sRcvData & "<ifordcd>LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Enterovirus▦L91546▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦호흡기 바이러스 PCR 16종▦L91530▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Coronavirus NL63▦L91544▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Coronavirus 229E▦L91543▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Metapneumovirus▦L91542▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Bocavirus▦L91541▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦RSV B▦L91540▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦RSV A▦L91539▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Rhinovirus▦L91538▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Parainfluenza 4▦L91537▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Parainfluenza 3▦L91536▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Parainfluenza 2▦L91535▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Parainfluenza 1▦L91534▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Influenza B▦L91533▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Influenza A▦L91532▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Adenovirus▦L91531▦155740"
'''sRcvData = sRcvData & "▩LBG2008180010▦20200818▦08686896▦서유리B▦M▦0▦소아청소년과▦신생아중환자실/NI01▦253430060▦Throat swab▦105▦Coronavirus OC43▦L91545▦155740"
'''sRcvData = sRcvData & "                </ifordcd>"
'''sRcvData = sRcvData & "            </IFordcd>"
    ...
    Else
        If UBound(varRcvData) >= 1 Then
            For i = 1 To UBound(varRcvData)
                'varRcvData(i) = Replace(varRcvData(i), vbLf, "")
                varRcvDtl = Split(varRcvData(i), "▩")
                With SPD
                    .ReDraw = False
                    For k = 0 To UBound(varRcvDtl)
                        varRcvDtl2 = Split(varRcvDtl(k), "▦")
                        For J = 1 To 1
                            strTstCode = mGetP(varRcvDtl(J), 13, "▦")
                            intTestCnt = intTestCnt + 1
                            '-- 화면에 표시
                            For intCol = colSTATE + 1 To .MaxCols
                                If GetTestNm(varRcvDtl2(12)) = gArrEQPNm(intCol - colSTATE, 6) Then
                                    .Row = asRow
                                    .Col = intCol
                                    .BackColor = vbYellow
                                    Call SetText(SPD, "◇", asRow, intCol)
                                ...
                                End If
                            Next
                            gPatOrdCd = gPatOrdCd & "'" & Trim(varRcvDtl2(12)) & "',"
                        Next
                    Next
                End With
            Next
                    ...
                    For i = 0 To xNode.childNodes.Item(intNodeLen).childNodes.Length - 1
                        For k = 0 To xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Length - 1
                            ...
                            Select Case UCase(xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).baseName)
                                Case "AGE":             XmlSelectS.AGE(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                  '나이           [78]
                                Case "BCNO":            XmlSelectS.BCNO(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                 '검체번호       [3010700030]
                                Case "EXECprcpuniqno":  XmlSelectS.EXECprcpuniqno(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue       '챠트번호?      [2002638354]
                                Case "ORDDEPTCD":       XmlSelectS.ORDDEPTCD(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue            '처방부서코드?  [NU]
                                
                                Case "PATNM":           XmlSelectS.PATNM(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                '환자명         [박성일]
                                Case "PID":             XmlSelectS.PID(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                  '환자번호       [000132623]
                                Case "PRCPDD":          XmlSelectS.PRCPDD(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue               '처방일?        [20181217]
                                Case "PRGSTNO":         XmlSelectS.PRGSTNO(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue              '주민번호       [400321-1******]
                                
                                Case "RETESTYN":        XmlSelectS.RETESTYN(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue             '재검여부       [N]
                                Case "RSLTSTAT":        XmlSelectS.RSLTSTAT(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue             '결과상태       [-]
                                
                                Case "SEX":             XmlSelectS.SEX(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                  '성별           [M]
                                
                                Case "SPCACPTDT":       XmlSelectS.SPCACPTDT(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue           '검체채취시간?  [20181217094414]
                                Case "SPCCD":           XmlSelectS.SPCCD(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                '검체코드       [023]
                                Case "SPCNM":           XmlSelectS.SPCNM(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue                '검체명         [Throat swab]
                                Case "SPCSTAT":         XmlSelectS.SPCSTAT(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue              '검체상태       [4]
                                
                                Case "TCLSCD":          XmlSelectS.TCLSCD(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue               '처방코드       [VB6012A]
                                Case "TCLSCDLIST":      XmlSelectS.TCLSCDLIST(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue           '검사코드       [CDATA[LPD329▦]
                                ...
                                Case "TESTCD":          XmlSelectS.TestCd(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue               '검사코드       [VB6012A]
                                
                                Case "TESTLRGCD":       XmlSelectS.TESTLRGCD(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue            '결과그룹코드?  [I]
                                Case "WORKNO":          XmlSelectS.WORKNO(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).childNodes.Item(k).nodeTypedValue               '워크번호       [20181217I20002]
            ...
            If xNode.nodeType = NODE_ELEMENT Then
                For intNodeLen = 0 To xNode.childNodes.Length - 1
                    For i = 0 To xNode.childNodes.Item(intNodeLen).childNodes.Length - 1
                        ...
                        Select Case UCase(xNode.childNodes.Item(intNodeLen).childNodes.Item(i).baseName)
                            Case "PID":         XmlSelectS.PID(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue                 '환자번호       [000132623]
                            Case "KORNM":       XmlSelectS.PATNM(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue               '환자명         [박성일]
                            Case "GSNO":        XmlSelectS.PRGSTNO(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue             '
                            Case "BCNO":        XmlSelectS.BCNO(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue                '검체번호       [3010700030]
                            Case "IFCODE":      XmlSelectS.TestCd(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue              '검사코드       [VB6012A]
                        End Select
                    Next
                    J = J + 1
                Next
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_HCMS" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
