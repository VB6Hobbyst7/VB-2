...
Private Sub cmdAdd_Click()
    ...
    If Trim(txtKitCode) = "" Then
        Call ShowMessage("장비검사코드와 연결할 KIT코드가 없습니다. 코드를 선택 하시오.   ")
        Exit Sub
    End If
    ...
    If Trim(txtQcLCode) = "" Then
        Call ShowMessage("장비검사코드와 연결할 QC Low연동코드가 없습니다. 코드를 선택 하시오.   ")
        Exit Sub
    End If
    
    If Trim(txtQcHCode) = "" Then
        Call ShowMessage("장비검사코드와 연결할 QC High연동코드가 없습니다. 코드를 선택 하시오.   ")
        Exit Sub
    End If
    
    If Trim(txtQCLlot) = "" Then
        Call ShowMessage("장비검사코드와 연결할 QC Lot 번호가 없습니다. 번호를 입력 하시오.   ")
        Exit Sub
    End If
...
Private Sub lvwTestListLab_Click()
    ...
    If Not itemX Is Nothing Then
        With itemX
            ...
            lblTstnmEqp = .SubItems(3)      '장비검사 이름
            txtTestCD = .SubItems(2)        '임상검사 코드
            txtKitCode = .SubItems(1)       'KIT 코드
...
Private Sub lvwTstListEqp_Click()
    ...
    Else
        ...
        txtKitCode = itemX.SubItems(1)       'KIT 코드
...
Private Sub f_subSet_ListView()
    ...
    With lvwTstListEqp
        ...
        With .ColumnHeaders
            .Clear
            Call .Add(1, "KCode", "장비코드", lvwWidth * 0.4)
            Call .Add(2, "Code", "KIT코드", lvwWidth * 0.4)
            Call .Add(3, "Name", "검사명", lvwWidth * 0.6)
        End With
    End With
    ...
    With lvwTestListLab
        ...
        With .ColumnHeaders
            .Clear
            Call .Add(1, "KitCode", "장비코드", lvwWidth * 0.15)
            Call .Add(2, "CodeEqp", "KIT코드", lvwWidth * 0.12)
            Call .Add(3, "Code", "검사코드", lvwWidth * 0.14)
            Call .Add(4, "Name", "검사명", lvwWidth * 0.12)
            Call .Add(5, "RefL", "참고치(L)", lvwWidth * 0, lvwColumnCenter)
            Call .Add(6, "RefH", "참고치(H)", lvwWidth * 0, lvwColumnCenter)
            Call .Add(7, "Prtno", "순서", lvwWidth * 0.05, lvwColumnCenter)
