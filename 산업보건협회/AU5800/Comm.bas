...
'    gSetup.gStartBit = Trim(frmInterface.txtTemp)
    ...
    'ID_Par
    db_tmp = ""
    Call GetPrivateProfileString("GLOBAL", "BarcodeID", "", db_tmp, 20, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gID_Par.BARCID = Trim(frmInterface.txtTemp)

    db_tmp = ""
    Call GetPrivateProfileString("GLOBAL", "MachineID", "", db_tmp, 20, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gID_Par.MACHID = Trim(frmInterface.txtTemp)
