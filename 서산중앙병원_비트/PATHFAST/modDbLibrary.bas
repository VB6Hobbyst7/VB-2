...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        lsPNm = Trim(GetText(.vasID, argSpcRow, colPNAME))
...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                '-- 결과업데이트 쿼리
                SQL = ""
                SQL = SQL & "UPDATE ResInf Set" & vbCr
                SQL = SQL & "       ResMzhMnt  = '" & sResult & "', " & vbCr
                SQL = SQL & "       ResRepTyp = 'I' " & vbCr                      '-- 보고대기(M)
                'SQL = SQL & "       ResMchCod = '" & frmInterface.Caption & "' "
                'SQL = SQL & "       ResTstDtm  = TO_CHAR(sysdate, 'YYYYMMDDHH24MI'), " & vbCr
                'SQL = SQL & "       ResUpdDtm  = TO_CHAR(sysdate, 'YYYYMMDDHH24MI') " & vbCr
                SQL = SQL & " Where ResSpmNum = '" & lsID & "'" & vbCr
                SQL = SQL & "   And ResLabCod = '" & strExamCd & "'" & vbCr
                'SQL = SQL & "   And (ResRepTyp  = '' OR ResRepTyp = 'M') "  '-- 최종(F)/중간(I)/보고대기(M)
                SQL = SQL & "   And ResRepTyp <> 'F' "  '-- 최종(F)/중간(I)/보고대기(M)
        ...
        If Res = 1 Then
            '-- 상태변경
            SQL = ""
            SQL = SQL & "UPDATE RsbInf Set " & vbCr
            SQL = SQL & "       RsbItfYon  = 'M' " & vbCr
            SQL = SQL & " WHERE RSBSPMNUM = '" & lsID & "'" & vbCr
            SQL = SQL & "   AND (RsbItfYon IS NULL OR RsbItfYon <> 'F') "
...
'    SQL = SQL & "       M.RESULTENDYN, R.SUBMCD, R.TESTITEMSEQ as ITEM, R.RESULTDATE, R.RESULTTIME, R.ROUTINECD, R.MACHINECD, R.RESULT, R.DELTAGBN, R.REFGBN, R.PANICGBN, R.DELTAMARK, R.REFMARK, R.PANICMARK, R.CMCODE " & vbCr
                                 ...
                                 '챠트번호,  환자명,    등록번호 , 접수일  , 입/외,         내원번호
'          SQL = "SELECT DISTINCT PbsChtNum, PbsPatNam, ResOcmNum, OcmAcpDtm as 접수일자, 'O' As IOGBN, OcmNum  " & vbCrLf
'    SQL = SQL & "  FROM PbsInf P, RsbInf M, ResInf R" & vbCrLf
...
'    SQL = SQL & "   AND RsbAcpCod = '" & gAllExam & "' " & vbCrLf
'    SQL = SQL & "   AND ResSpmNum = '" & sBarcode & "' " & vbCrLf
'    SQL = SQL & "   AND RSBACPDTE = RESACPDTE        " & vbCrLf
'    SQL = SQL & "   AND (RESMZHMNT IS NULL OR RESMZHMNT = '' or R.RESMZHMNT = ' ') "
    
    SQL = ""
    SQL = SQL & "SELECT M.RsbSpmNum 바코드번호, M.RSBACPDTE 접수일자, M.RSBACPCOD 접수코드, M.RSBACPNUM 접수번호, M.RSBSPMCOD 검체코드, S.LABSPMNAM 검체명, M.RsbChtNum 등록번호, M.RsbItfYon 결과상태, R.RESLABCOD ITEM," & vbCr
    SQL = SQL & "       R.RESACPCOD 검사파트, R.RESMZHMNT 검사결과, R.RESCMTTXT 결과코멘트, R.RESTSTDTM 검사일시, R.RESTSTUID 검사자아이디, " & vbCr
    SQL = SQL & "       R.RESUPDDTM 결과최종수정일시, R.RESUPDUID 결과최종수정아이디, R.RESCONLVL 결과신뢰도, P.PBSPATNAM 이름, P.PBSRESNUM 주민번호 " & vbCr
    SQL = SQL & "  FROM RsbInf M, ResInf R, PbsInf P, labspmmst S " & vbCr
    SQL = SQL & " WHERE M.RsbSpmNum = '" & sBarcode & "' " & vbCr
    'SQL = SQL & "   AND (M.RsbItfYon is Null or M.RsbItfYon <> 'F')" & vbCr
    SQL = SQL & "   AND (R.RESMZHMNT is Null or R.RESMZHMNT = '' or R.RESMZHMNT = ' ') " & vbCr
    SQL = SQL & "   AND M.RSBACPDTE = R.RESACPDTE " & vbCr
    SQL = SQL & "   AND M.RSBACPCOD = R.RESACPCOD " & vbCr
    SQL = SQL & "   AND M.RSBACPNUM = R.RESACPNUM " & vbCr
    'SQL = SQL & "   AND M.RSBSPMCOD = R.RESSPMCOD " & vbCr
    SQL = SQL & "   AND M.RSBSPMNUM = R.RESSPMNUM " & vbCr
    SQL = SQL & "   AND M.RSBCHTNUM = P.PBSCHTNUM " & vbCr
    SQL = SQL & "   AND M.RSBSPMCOD = S.LABSPMCOD "
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, Trim(RS.Fields("접수일자")) & "", asRow, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("바코드번호")) & "", asRow, colBARCODE
                SetText .vasID, Trim(RS.Fields("접수번호")) & "", asRow, colCHARTNO
                SetText .vasID, Trim(RS.Fields("등록번호")) & "", asRow, colPID
                SetText .vasID, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                'SetText .vasID, Trim(RS.Fields("SPECIMENCD")) & "", asRow, colINOUT
                'SetText .vasID, Trim(RS.Fields("나이")) & "", asRow, colPAGE
                'SetText .vasID, Trim(RS.Fields("JUMIN")) & "", asRow, colDISKNO
...
'                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
'                SetText .vasID, "20160802", asRow, colHOSPDATE
'                SetText .vasID, sBarcode, asRow, colBARCODE
'                SetText .vasID, "12345", asRow, colCHARTNO
'                SetText .vasID, "67890", asRow, colPID
'                SetText .vasID, "이허리", asRow, colPNAME
