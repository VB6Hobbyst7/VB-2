...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                'If strORQN <> "" Then
                    '-- 서버저장
                          SQL = "INSERT INTO EQP3020(SYS_DATE, USER_ID, UPD_DATE, SPECIMEN_SER, JANGBI_CODE, JANGBI_OUT_CODE, CPL_RESULT, RESULT_DATE)" & vbCr
                    SQL = SQL & " Values (SYSDATE, '', SYSDATE, '" & lsID & "',  '" & gEquipCode & "', '" & strExamCd & "', '" & sResult & "', SYSDATE ) "
