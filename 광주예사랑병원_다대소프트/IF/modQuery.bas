...
Public Function GetEquipExamCode_INDIKO(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    ...
    If strExamCode <> "" Then
        GetEquipExamCode_INDIKO = Mid(strExamCode, 1, Len(strExamCode) - 1)
    End If
...
Public Function GetEquipExamCode_CENTAURXPT(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim strSendCH       As String
    
    GetEquipExamCode_CENTAURXPT = ""
    strExamCode = ""
    ...
    If strExamCode <> "" Then
        GetEquipExamCode_CENTAURXPT = Mid(strExamCode, 2)
    End If
...
'MsgBox SQL
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
           ...
           ' MsgBox AdoRs_Local.Fields("SENDCHANNEL").Value & ""
...
'                SQL = SQL & "   And O.H141_SUGACD     = '" & strTestCd & "'    " & vbCrLf
                ...
                SQL = SQL & "   AND L.처방코드 + L.서브코드 = '" & strTestCd & "'               " & vbCrLf
                                    
                Set RS = AdoCn.Execute(SQL, , 1)
                If Not RS.EOF = True And Not RS.BOF = True Then
                    Do Until RS.EOF
                        strOrdCd = Trim(RS.Fields("TESTID")) & ""
                        strTestCdSub = Trim(RS.Fields("SPRTID")) & ""
                        RS.MoveNext
                    Loop
                End If
...
On Error GoTo ErrHandle
    ...
    SQL = SQL & "                                AND  J.챠트번호   = A.챠트번호      " & vbCrLf
    SQL = SQL & "                                AND  J.진료번호   = A.진료번호)     " & vbCrLf
    SQL = SQL & " Where J.접수일자 BETWEEN '" & Format(pFrom, "####-##-##") & "' and '" & Format(pTo, "####-##-##") & "'" & vbCrLf
    SQL = SQL & "   AND L.검사상태 < 5                                     " & vbCrLf
...
Public Sub GetWorkList_HANARO(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo DBErr
    ...
    SQL = SQL & "   AND L.검사상태 < 5                                     " & vbCrLf
    SQL = SQL & "   AND L.처방코드 + L.서브코드 IN (" & gAllTestCd & ")    " & vbCrLf
    SQL = SQL & " ORDER BY J.접수일자, J.접수번호                          " & vbCrLf
