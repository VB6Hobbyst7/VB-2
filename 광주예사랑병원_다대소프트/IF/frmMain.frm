...
Begin VB.Form frmMain 
   ...
   Begin FPSpread.vaSpread spdResult 
      ...
      ScrollBarExtMode=   -1  'True
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":43DF
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdOrder 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":4FD4
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdWork 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":6D28
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
...
Private Sub cmdRcv_Click()
    ...
    Select Case UCase(gHOSP.MACHNM)
        Case "CENTAURXPT":      Call Phase_Serial_CENTAURXPT
        Case "URISCAN":         Call Phase_Serial_URISCAN
    End Select
...
Private Sub GetOrder_CENTAURXPT(ByVal pBarNo As String, ByVal pType As String)
    ...
    With frmMain
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = GetEquipExamCode_CENTAURXPT(gHOSP.MACHCD, pBarNo, intRow)
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                Case "Q"    '## Request Information
                    ...
                    Call GetOrder_CENTAURXPT(strBarno, gHOSP.RSTTYPE)
...
Private Sub SerialRcvData_CENTAURXPT()
    Dim RS_L            As ADODB.Recordset
    ...
    Dim strResult       As String   '수신한 결과(정성)
    Dim strIntResult    As String   '수신한 결과(정량)
    Dim strPNResult     As String   '수신한 결과(정량)
    Dim strQCResult     As String   '수신한 결과(QC)
    Dim strFlag         As String   '수신한 Abnormal Flag
    Dim strComm         As String   '수신한 Comment
...
On Error GoTo ErrHandle
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                Case "Q"    '## Request Information
                    ...
                    strRackNo = Trim$(mGetP(strTemp1, 4, "^"))
                    strTubePos = Trim$(mGetP(strTemp1, 5, "^"))
    ...
    '                strQCResult = strIntResult
                    ...
                    Case "HBs", "HBsAgD", "HBsII"
                        Select Case strAspect
                            ...
                            Case "INTR"     '## 정성
                                ...
                                End Select
                                strResult = strResult & "(" & strPNResult & ")"
                                strPNResult = ""
                        End Select
                    ...
                    Case "aHBs2" 'Anti-HBs
                        'R|1|^^^aHBs2^^^1^RLU|667131|||||F||||20190617185650|IRL12831627
                        'R|2|^^^aHBs2^^^1^COFF|1.00|mIU/mL||||F||||20190617185650|33
                        'R|3|^^^aHBs2^^^1^DOSE|881.77|mIU/mL||||F||||20190617185650|IRL12831627
                        'R|4|^^^aHBs2^^^1^INTR|React|||||F||||20190617185650|IRL12831627
...
Private Sub SerialRcvData_URISCAN()
    Dim RS_L            As ADODB.Recordset
...
'   기능 : 오더정보 전송
'-----------------------------------------------------------------------------'
Private Sub SendOrder_CENTAYRXPT()
    Dim strOutput   As String     '송신할 데이터
...
'   기능 : 오더정보 전송
'-----------------------------------------------------------------------------'
Private Sub SendOrder_CENTAURXPT()
''    Dim strOutput   As String     '송신할 데이터
''
''    Select Case intSndPhase
''        Case 1  '## Header
...
''    Call SendData(strOutput)
    ...
    With mOrder
        Select Case intSndPhase
            Case 1  '## Header
                ...
                strOutput = strOutput & "O|1|" & mOrder.BarNo & "^||" & mOrder.Order & "|||||||||||||||||||||O\Q" & vbCr
            ...
            Case 2  '## Patient
                strOutput = intFrameNo & mOrder.OtherOrder
                intFrameNo = intFrameNo + 1
    ...
    If Len(strOutput) > 230 Then
        mOrder.OtherOrder = Mid$(strOutput, 231)
...
Private Sub Phase_Serial_CENTAURXPT()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
...
Private Sub Phase_Serial_URISCAN()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
