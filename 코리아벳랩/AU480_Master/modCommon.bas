...
Public Const colCHARTNO = 5     '챠트번호
Public Const colPNAME = 6
Public Const colINOUT = 7      '입/외
Public Const colBARCODE = 8     '바코드
Public Const colDISKNO = 9      '
Public Const colPOSNO = 10       '
Public Const colPID = 11         '내원번호
Public Const colSPCNM = 12      '검체명
Public Const colSN = 13         'Serial No (AU의 경우 장비 Seq)
Public Const colPSEX = 14
Public Const colPAGE = 15
Public Const colOCNT = 16
Public Const colRCNT = 17
Public Const colState = 18
...
'    strEqpCd = frmInterface.txtTemp
    ...
    db_tmp = ""
    Call GetPrivateProfileString("config", "ORDERPATH", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gORDERPATH = Trim(frmInterface.txtTemp)

    db_tmp = ""
    Call GetPrivateProfileString("config", "RESULTPATH", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gRESULTPATH = Trim(frmInterface.txtTemp)

    db_tmp = ""
    Call GetPrivateProfileString("config", "LOGPATH", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gLOGPATH = Trim(frmInterface.txtTemp)

    db_tmp = ""
    Call GetPrivateProfileString("config", "READINTERVAL", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gREADINTERVAL = Trim(frmInterface.txtTemp)
...
Public Sub SetMediLABData(ByVal strName As String, ByVal argSQL As String)
'argSQL의 내용을 파일로 저장
    Dim FilNum
    Dim sFileName As String
