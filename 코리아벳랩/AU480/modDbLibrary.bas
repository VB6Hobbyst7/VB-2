...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        If InStr(strBarcode, "오더없음") > 0 Then
            Exit Function
        End If
        ...
        If Mid(strBarcode, 1, 1) = "E" Then
            Open gRESULTPATH & "\AU480_" & Mid(strBarcode, 2) & ".EMS" For Output As FilNum
        Else
            Open gRESULTPATH & "\AU480_" & strBarcode & ".RST" For Output As FilNum
        End If
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            sResult1 = Trim(GetText(.vasTemp, iRow, 3)) '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, iRow, 4)) '결과(수정결과)
            ...
            If sResult <> "" Then
                ...
                If iRow = 1 Then
                    'strRstBuf = strExamDate & Space(14 - Len(Trim(strExamDate))) & .vasTemp.DataRowCnt & CStr(Trim(.vasTemp.DataRowCnt)) & vbCrLf
                    strRstBuf = strExamDate & Space(14 - Len(Trim(strExamDate))) & intCnt & Space(10 - Len(CStr(intCnt))) & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                mResult.Sex = ""
                mResult.Age = ""
                strSex = Trim(RS.Fields("주민번호")) & ""
                If Trim(strSex) & "" <> "" Then
                    strSex = Mid(mGetP(Trim(RS.Fields("주민번호")) & "", 2, "-"), 1, 1)
                    'MsgBox "1:" & strSex
                    strAge = Mid(Trim(RS.Fields("주민번호")) & "", 1, 2)
                    'MsgBox "2:" & strAge
                    If strSex <> "" Then
                        Select Case strSex
                        Case "1"
                ...
                If mResult.Age <> "" Then
                   ...
                   ' Call SetSQLData("AGE", mResult.Age & "," & Trim(RS.Fields("주민번호")) & "")
