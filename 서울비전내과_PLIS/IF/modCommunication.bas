...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,PANICVALUE,CHARTNO " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            ...
            Do Until RS_L.EOF
                ...
                Call SetText(.vasTemp, RS_L.Fields("CHARTNO").Value & "", intRow, 8) 'ORDDT
                RS_L.MoveNext
            Loop
        End If
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            strOrdDT = Trim(GetText(.vasTemp, intRow, 8))       'ORDDT
            '-- 장비결과적용
            If .optSaveResult(0).Value = True Then
                sResult = sResult1
            Else
...
'                    SQL = ""
'                    SQL = SQL & " UPDATE plis..s2ord102 SET " & vbCr
'                    SQL = SQL & "    stscd   = '5'" & vbCr
'                    SQL = SQL & " WHERE workarea = '" & mOrder.WA & "'" & vbCr
'                    SQL = SQL & "   AND accdt    = '" & strHospDate & "'" & vbCr
'                    SQL = SQL & "   AND accseq   = '" & lngAccSeq & "'" & vbCr
'                    SQL = SQL & "   AND ordcd    = '" & strTestCd & "'" & vbCr
'
'                    Call SetSQLData("결과저장", SQL, "A")
'                    AdoCn.Execute SQL
                    
                    lngInsCnt = lngInsCnt + 1
                    
                    '검진저장 안함
                    If frmMain.chkGum.Value = "0" Then
                        '-- 결과저장 for LIS
                        SQL = ""
                        ...
                        SQL = SQL & "  , rstcd   = '" & sResult & "'" & vbCr
                        SQL = SQL & "  , rsttype = 'N' " & vbCr
    '                    SQL = SQL & "  , vfyid  = '" & gHOSP.USERID & "' " & vbCr
    '                    SQL = SQL & "  , vfydt  = '" & Format(Now, "yyyymmdd") & "' " & vbCr
    '                    SQL = SQL & "  , vfytm  = '" & Format(Now, "hhmmdd") & "' " & vbCr
                        SQL = SQL & " WHERE workarea = '" & mOrder.WA & "'" & vbCr
                        SQL = SQL & "   AND accdt    = '" & strHospDate & "'" & vbCr
                        SQL = SQL & "   AND accseq   = '" & lngAccSeq & "'" & vbCr
                        SQL = SQL & "   AND testcd   = '" & strTestCd & "'" & vbCr
                    ...
                    Else
                        ...
                        If Len(strOrdDT) = 8 Then
                            '-- 결과저장 for 검진
                            SQL = ""
                            SQL = SQL & "Update MEDICHART..TB_검사항목 " & vbCr
                            SQL = SQL & "   Set 검사결과 = '" & sResult & "' ," & vbCr
                            SQL = SQL & "       진료지원상태 = 5 " & vbCr
                            SQL = SQL & " Where 진료년 = '" & Mid(strOrdDT, 1, 4) & "' " & vbCr
                            SQL = SQL & "   AND 진료월 = '" & Mid(strOrdDT, 5, 2) & "' " & vbCr
                            SQL = SQL & "   AND 진료일 = '" & Mid(strOrdDT, 7, 2) & "' " & vbCr
                            SQL = SQL & "   AND 챠트번호 = '" & strPatID & "' " & vbCr
                            SQL = SQL & "   AND 진료지원상태 < 5" & vbCr
                            SQL = SQL & "   AND 진료지원상태 <> 5" & vbCr
        ...
        '검진저장
        If frmMain.chkGum.Value = "1" Then
            SQL = ""
            SQL = SQL & "select reqtotcnt,reqinputcnt                       " & vbCr
            SQL = SQL & "  from plis..s2lab201                  " & vbCr
            SQL = SQL & " WHERE workarea = '" & mOrder.WA & "'" & vbCr
            SQL = SQL & "   AND accdt    = '" & strHospDate & "'" & vbCr
            SQL = SQL & "   AND accseq   = '" & lngAccSeq & "'" & vbCr
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                lngTotCnt = RS.Fields("reqtotcnt")
                lngInsCnt = lngInsCnt + RS.Fields("reqinputcnt")
            End If
            
            RS.Close
            
            If lngTotCnt >= lngInsCnt Then
                SQL = ""
                SQL = SQL & " UPDATE plis..s2lab201 SET " & vbCr
                SQL = SQL & "    stscd   = '5'" & vbCr
                SQL = SQL & " WHERE workarea = '" & mOrder.WA & "'" & vbCr
                SQL = SQL & "   AND accdt    = '" & strHospDate & "'" & vbCr
                SQL = SQL & "   AND accseq   = '" & lngAccSeq & "'" & vbCr
...
Function SaveTransDataR_PLIS(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim RS As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
...
'                    AdoCn.Execute SQL
                    ...
                    '검진저장 안함
                    If frmMain.chkRGum.Value = "0" Then
                        '-- 결과저장 for LIS
                        SQL = ""
                        SQL = SQL & " UPDATE plis..s2lab302 SET " & vbCr
                        SQL = SQL & "    eqpcd   = '" & gHOSP.MACHCD & "'" & vbCr
    ...
    '                    SQL = SQL & "  , vfytm  = '" & Format(Now, "hhmmdd") & "' " & vbCr
        ...
        '검진저장
        If frmMain.chkRGum.Value = "1" Then
            SQL = ""
            SQL = SQL & "select reqtotcnt,reqinputcnt                       " & vbCr
            SQL = SQL & "  from plis..s2lab201                  " & vbCr
            SQL = SQL & " WHERE workarea = '" & mOrder.WA & "'" & vbCr
