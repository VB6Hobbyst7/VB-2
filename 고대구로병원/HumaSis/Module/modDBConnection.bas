...
Global Const REG_UID        As String = "UID"
Global Const REG_PWD        As String = "PWD"

Global Const REG_SAVEPW    As String = "SAVEPW"
...
Public Function TmaxConnect() As Boolean
    Dim ret     As Integer
    Dim lsndbuf As Long
    ...
    '-- Tmax Read Env(IP)
'    If tmaxreadenv("C:\RMX\ENV\RMX.env", "190.65") = -1 Then   '평촌
'    If tmaxreadenv("C:\RMX\ENV\RMX.env", "190.50") = -1 Then    '강남
'    If tmaxreadenv("C:\RMX\ENV\RMX.env", "190.20") = -1 Then    '강남
    If tmaxreadenv("C:\TMAX\ENV\TMAX.env", "01") = -1 Then    '고대안암_개발
        ErrMsg = tmaxerrdesc(gettperrno())
        MsgBox "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg, vbOKOnly + vbCritical, "TMAX 에러"
        'Print #1, "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg & Format(Now, "yyyymmddhhmmss")
        TmaxConnect = False
        Exit Function
    End If
    
'ret = TuxedoInit_V2("", "", "01")
