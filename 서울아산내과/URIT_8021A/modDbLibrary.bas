...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        strDate = Trim(GetText(.vasID, argSpcRow, 2))   '접수일자
        strType = Trim(GetText(.vasID, argSpcRow, 3))   '구분
        lsID = Trim(GetText(.vasID, argSpcRow, 4))      '접수번호
        lsPid = Trim(GetText(.vasID, argSpcRow, 5))     '챠트번호

        '-- Local에서 환자별로 결과값 가져오기
        ClearSpread .vasTemp
        
              SQL = "SELECT EQUIPCODE,EXAMCODE,RESULT,EQUIPRESULT,REFFLAG,PANICVALUE,DELTAVALUE,PSEX,BARCODE " & vbCrLf
        SQL = SQL & "  FROM PATRESULT " & vbCrLf
        SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf                               '장비코드
        SQL = SQL & "   AND EXAMDATE = '" & Format(Now, "yyyymmdd") & "'  " & vbCrLf            '검사일
        SQL = SQL & "   AND PID = '" & Trim(GetText(.vasID, argSpcRow, colPID)) & "' " & vbCrLf '챠트번호
        SQL = SQL & "   AND SAMPLETYPE = '" & strType & "' " & vbCrLf                           '구분
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            strExamNo = Trim(GetText(.vasTemp, iRow, 9))    '접수번호
...
'서버의 데이타 베이스에 저장
    ...
    With frmInterface
        ...
        strDate = Format(CDate(.dtpExamDate.Value), "yyyymmdd") '접수일자
        strType = Trim(GetText(.vasRID, argSpcRow, 2))          '구분
        lsID = Trim(GetText(.vasRID, argSpcRow, 3))             '접수번호
        lsPid = Trim(GetText(.vasRID, argSpcRow, 6))            '챠트번호
        ...
        SQL = SQL & "   AND PID = '" & lsPid & "' " & vbCrLf            '챠트번호
        SQL = SQL & "   AND SAMPLETYPE = '" & strType & "' "            '구분
...
Function GetSampleInfoW(ByVal asRow As Long) As Integer
    ...
    '-- 바코드번호로 환자정보 불러오기
          SQL = "SELECT Distinct "
    SQL = SQL & IIf(gDBCOLUMN_Parm.PID = "", "''", gDBCOLUMN_Parm.PID) & ","
    SQL = SQL & IIf(gDBCOLUMN_Parm.PNAME = "", "''", gDBCOLUMN_Parm.PNAME) & ","
    SQL = SQL & IIf(gDBCOLUMN_Parm.PSEX = "", "''", gDBCOLUMN_Parm.PSEX) & ","
    SQL = SQL & IIf(gDBCOLUMN_Parm.PAGE = "", "''", gDBCOLUMN_Parm.PAGE) & vbCrLf
    ...
    If Res = 1 Then
        SetText frmInterface.vasID, Trim(gReadBuf(0)), asRow, colPID    '5  검사자번호(=병록번호)
        SetText frmInterface.vasID, Trim(gReadBuf(1)), asRow, colPName  '6  검사자명
        '-- 성별이 없을경우 주민번호로 찾기
...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Function GetOrderExamCode(argEquipCode As String, argPID As String, Optional argRow As Long) As String
    Dim i           As Integer
    Dim sExamCode   As String
    Dim strExamCode As String
    Dim sExamCd     As String
    ...
    frmInterface.vasID.Row = argRow
    frmInterface.vasID.Col = 3
    If frmInterface.vasID.Text = "검진" Then
        '-- 검진
        SQL = "select a.exam_code " & vbCrLf & _
              "from totres a, total b " & vbCrLf & _
              "where a.request_date = b.request_date and a.exam_no = b.exam_no " & vbCrLf & _
              "  and b.chart_no = '" & argPID & "' "
    ...
    Else
        '--진료
              SQL = " Select ResLabCod"
        SQL = SQL & "   From drbitpack..ResInf AS R, drbitpack..OcmInf AS O"
        SQL = SQL & " where O.OcmChtNum Not In ('CN', 'CR', 'VC')"
        SQL = SQL & "   and LTRIM(O.OcmChtNum) = '" & argPID & "' "
