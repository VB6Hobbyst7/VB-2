...
'''            SQL = "select max(seqno) from qc_res where examdate = '" & Format(Date, "yyyymmdd") & "'  and equipno = '" & gEquip & "' " & vbCrLf & _
'''                  "and levelname = '" & sSpecID & "'"
'''            res = db_select_Col(gLocal, SQL)
'''            If Trim(gReadBuf(0)) = "" Then
''''''                SetText vasID, "1", glRow, colReExam
'''            Else
''''''                SetText vasID, CCur(gReadBuf(0)) + 1, glRow, colReExam
'''            End If
...
'''                    If aCnt = 3 Then
'''                        slen = InStr(i + 1, argData, "|")
'''                        sCommentNum = Trim(Mid(argData, i + 1, slen - i - 1))
'''                        slen = InStr(1, sCommentNum, "^")
'''                        If slen > 0 Then
'''                            sCommentNum = Mid(sCommentNum, 1, slen - 1)
'''                        End If
'''
'''                        Select Case sCommentNum
'''                        Case "5", "6", "7", "8", "9", "26"
'''                            If gtestid = "459" Or gtestid = "120" Or gtestid = "186" Or gtestid = "250" Or gtestid = "111" Or gtestid = "127" Or gtestid = "205" Or gtestid = "110" Or gtestid = "24" Then
'''                                sResValue = ">" & gResultRes
'''                                sCmtProc = True
'''                            Else
'''                                sResValue = ""
'''                                sCmtProc = True
...
'''                                  "where ExamCode = '" & Trim(gReadBuf(0)) & "' and equipno = '" & Trim(gReadBuf(1)) & "' "
'''
'''                            res = db_select_Col(gLocal, SQL)
'''
...
'''
'''
'''                        SQL = "select examcode from equipexam where equipcode = '" & gtestid & "'"
...
'''                        For j = 1 To res
'''                            If strSCode = "" Then
'''                                strSCode = "'" & Trim(gReadBuf(j - 1)) & "'"
'''                            Else
'''                                strSCode = strSCode & ", '" & Trim(gReadBuf(j - 1)) & "'"
'''                            End If
'''
'''                        Next
'''
'''                        If strSCode <> "" Then
'''                            strDBBar = Trim(GetText(vasID, glRow, colBarCode))
'''
...
'''                                SQL = SQL & vbCrLf & "    AND ORD_CD IN (" & strSCode & ")"
'''
...
'''                        Else
'''                            SQL = SQL & vbCrLf & " where ExamCode = '" & sExamCode & "'"
'''                        End If
...
'''                        SQL = "SELECT AMIN, AMAX FROM LRESULT WHERE SPNO = '" & Trim(GetText(vasID, glRow, colBarCode)) & "' AND ORCD = '" & sExamCode & "'"
'''                        res = db_select_Col(gServer, SQL)
'''
'''                        sMin = Trim(gReadBuf(0))
'''                        sMax = Trim(gReadBuf(1))
'''
'''                        If IsNumeric(sResValue) = True Then
'''                            If IsNumeric(sMin) = True Then
'''                                If CCur(sMin) > CCur(sResValue) Then
'''                                    sRefFlag = "L"
'''                                End If
'''                            End If
'''                        End If
'''
'''                        If IsNumeric(sResValue) = True Then
'''                            If IsNumeric(sMax) = True Then
'''                                If CCur(sMax) < CCur(sResValue) Then
'''                                    sRefFlag = "H"
'''                                End If
'''                            End If
'''                        End If
...
'''                                If CCur(sResValue) > CCur(sMin) And CCur(sResValue) < CCur(sMax) Then
'''                                    sRefFlag = " "
'''                                End If
'''                            End If
'''                        End If
...
'''                                sExamDate = Mid(sResDateTime, 1, 8)
'''                            End If
'''                        End If
                    End Select
                End If
...
'''    vasID.Visible = True
    ...
    '-- 일주일 이전 데이터 지우기
    strDate = Format(Now - 7, "yyyymmdd")
    SQL = "delete from pifrsltm where pifrendt < '" & strDate & "'"
