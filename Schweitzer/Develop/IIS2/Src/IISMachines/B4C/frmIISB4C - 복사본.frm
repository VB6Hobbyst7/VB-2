...
Begin VB.Form frmIISB4C 
   ...
   Begin FPSpread.vaSpread vasFDPrint 
      ...
      MaxRows         =   65
      Protect         =   0   'False
      SpreadDesigner  =   "frmIISB4C.frx":0000
   End
   Begin FPSpread.vaSpread vasINPrint 
      ...
      MaxRows         =   65
      Protect         =   0   'False
      SpreadDesigner  =   "frmIISB4C.frx":887A
   End
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISB4C.frx":110F4
   End
   ...
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISB4C.frx":11661
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISB4C.frx":11F07
   End
   ...
   Begin FPSpread.vaSpread tblexcel 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      SpreadDesigner  =   "frmIISB4C.frx":125FA
   End
   ...
   Begin FPSpread.vaSpread vasExcel 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      SpreadDesigner  =   "frmIISB4C.frx":12843
   End
...
Private Sub OpenExcel()
    ...
    AlloFile.InitDir = GetB4CConfig("ExportPath")
    AlloFile.ShowOpen
...
'    Dim strOldInFo  As String
    ...
    With AlloFile
        .CancelError = True
        .FileName = GetB4CConfig("ImportPath") & "\" & Trim(txtFileNm.Text) & ".csv"
...
'4,MEDIWISS / RIDA Panel 2 KO / Rev. 004,2,5,,,,,,,,,,,,,,,,,,,,,,,,,,,PatientStrip,,IgE,,0,,,,,,,,,,,,,,,,,
        ...
        For iCnt = 1 To tblReady.DataRowCnt
            ...
            If varTmp = "IN" Then
                strInFo = strInFo & CStr(intCnt) & "," & "MEDIWISS / RIDA Panel 1 KO / Rev. 006," & strBarNo & "," & strLabNo & "," & strLNm & "," & strFNm & ",," & strSex & "," & strAge & vbCr '& vbLf
                If iCnt = tblReady.DataRowCnt Then
                    strInFo = strInFo & CStr(intCnt + 1) & "," & "MEDIWISS / RIDA Panel 2 KO / Rev. 004," & strBarNo & "," & strLabNo & "," & strLNm & "," & strFNm & ",," & strSex & "," & strAge
                Else
                    strInFo = strInFo & CStr(intCnt + 1) & "," & "MEDIWISS / RIDA Panel 2 KO / Rev. 004," & strBarNo & "," & strLabNo & "," & strLNm & "," & strFNm & ",," & strSex & "," & strAge & vbCr '& vbLf
                End If
            ElseIf varTmp = "FD" Then
                strInFo = strInFo & CStr(intCnt) & "," & "MEDIWISS / RIDA Panel 3 KO / Rev. 004," & strBarNo & "," & strLabNo & "," & strLNm & "," & strFNm & ",," & strSex & "," & strAge & vbCr '& vbLf
                If iCnt = tblReady.DataRowCnt Then
                    strInFo = strInFo & CStr(intCnt + 1) & "," & "MEDIWISS / RIDA Panel 4 KO / Rev. 003," & strBarNo & "," & strLabNo & "," & strLNm & "," & strFNm & ",," & strSex & "," & strAge
                Else
                    strInFo = strInFo & CStr(intCnt + 1) & "," & "MEDIWISS / RIDA Panel 4 KO / Rev. 003," & strBarNo & "," & strLabNo & "," & strLNm & "," & strFNm & ",," & strSex & "," & strAge & vbCr '& vbLf
                End If
            End If
...
Private Function GetB4CConfig(ByVal strConfigNm As String) As String

Dim strFileName As String
Dim strReturnedString As String

    strFileName = App.Path & "\b4c.ini"
    
    strReturnedString = String(1024, " ")
    GetPrivateProfileString "B4C", strConfigNm, "", strReturnedString, Len(strReturnedString), strFileName
    strReturnedString = Trim(Replace(strReturnedString, Chr(0), Chr(32), 1, -1, vbBinaryCompare))
    GetB4CConfig = strReturnedString
                        ...
                        With vasINPrint
                            For i = TCompleteEnum.ccResult To tblComplete.DataColCnt
                                ...
                                Select Case mGetP(strTemp, TResultEnum.ccIntBase, DIV)
                                    ' 1KO
                                    Case "IN|IgE":  iDestRow = 12
                                    Case "IN|F14":  iDestRow = 13
                                    Case "IN|F2":   iDestRow = 14
                                    Case "IN|F1":   iDestRow = 15
                                    Case "IN|F23":  iDestRow = 16
                                    Case "IN|F24":  iDestRow = 17
                                    Case "IN|F53":  iDestRow = 18
                                    Case "IN|T36":  iDestRow = 19
                                    Case "IN|T15":  iDestRow = 20
                                    Case "IN|TXKO": iDestRow = 21
                                    Case "IN|T12":  iDestRow = 22
                                    Case "IN|F17":  iDestRow = 23
                                    Case "IN|T24":  iDestRow = 24
                                    Case "IN|T7":   iDestRow = 25
                                    Case "IN|T14":  iDestRow = 26
                                    Case "IN|T1":   iDestRow = 27
                                    Case "IN|G2":   iDestRow = 28
                                    Case "IN|G3":   iDestRow = 29
                                    Case "IN|G6":   iDestRow = 30
                                    Case "IN|G12":  iDestRow = 31
                                    ' 2KO
                                    Case "IN|W12":  iDestRow = 35
                                ...
                                Call .SetText(14, iDestRow, strClass)
...
'    2015-08-21_11-46-00
    ...
    FileB4C.Path = GetB4CConfig("ExportPath") & "\"
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        For intCnt = 0 To UBound(varTmp) - 1
            If intCnt = 1 Then
                ...
                If strPart = "RIDA Panel 1 KO" Then
                    strPart = "IN|"
                ElseIf strPart = "RIDA Panel 2 KO" Then
                    strPart = "IN|"
                ElseIf strPart = "RIDA Panel 3 KO" Then
                    strPart = "FD|"
                ElseIf strPart = "RIDA Panel 4 KO" Then
                    strPart = "FD|"
                End If
            End If
