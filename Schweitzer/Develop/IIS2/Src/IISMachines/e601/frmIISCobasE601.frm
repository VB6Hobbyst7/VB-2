...
Begin VB.Form frmIISCobasE601 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISCobasE601.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISCobasE601.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISCobasE601.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISCobasE601"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISCobasE601.frm
'   작성자  : 오세원
'   내  용  : CobasE601 장비폼
'   작성일  : 2014-02-04
'   버  전  :
...
Private Sub Command1_Click()
             ...
             strBuf = "1H|\^&|||cobas6000^1|||||host|TSREQ^REAL|P|1" & vbCr
    strBuf = strBuf & "Q|1|^^          140000390395^0^5001^3^^S1^SC^R1||ALL||||||||A" & vbCr
    strBuf = strBuf & "L|1|N" & vbCr
    strBuf = strBuf & "E5" & vbCr
...
'Q|1|^^          140000390401^0^5001^2^^S1^SC^R1||ALL||||||||A
'L|1|N
'D8


strBuf = ""
strBuf = strBuf & "1H|\^&|||cobas6000^1|||||host|RSUPL^REAL|P|1" & vbCr
strBuf = strBuf & "P|1|||||||U||||||^" & vbCr
strBuf = strBuf & "O|1|          140000390432|0^5002^3^^S1^SC|^^^126^|R||20140218104713||||N||||1|||||||20140218104718|||F" & vbCr
strBuf = strBuf & "C|1|I|                              ^^^^|G" & vbCr
strBuf = strBuf & "R|1|^^^126//not|45.08|pg/mL||N2A" & vbCr
strBuf = strBuf & "2||F||      |20140218104907||E12" & vbCr
strBuf = strBuf & "C|1|I|0|I" & vbCr
strBuf = strBuf & "L|1|N" & vbCr
strBuf = strBuf & "1E" & vbCr
strBuf = strBuf & ""
        ...
        Select Case strType
            ...
            Case "Q"    '## Request Information
                ...
                With mOrder
                    ...
                    .RackNo = mGetP(strTemp1, 5, "^")
                    .TubePos = mGetP(strTemp1, 6, "^")
                End With
            ...
            Case "R"    '## Result
                ...
                Else
                    ...
                    '## 1.0.3: 이상대(2005-08-09)
                    '   - 결과값이 없으면 컬렉션에 저장하지 않도록 수정
                    If Trim(strResult) <> "" Then
                        If objIntNms.ExistIntBase(strIntBase) Then
                            Call objIntInfo.IntResults.Add(strIntBase, objIntNms.GetIntNm(strIntBase), _
                                 strResult, strResult)
    ...
    With mOrder
        Select Case mIntLib.SndPhase
            ...
            Case 1
                ...
                strOutput = "H|\^&|||ASTM-Host^||||||TSDWN^REPLY" & vbCr
    
                strOutput = strOutput & "P|1|||||||U||||||^" & vbCr
    
                'Orderoutput = "O|1|" & Trim(PatientID) & "|" & PatientSeq & "^" & PatientDisk & "^" & PatientPos & "^^S1^SC|" & OutPutData & "|R||" & Format(Now, "YYYYMMDDHHMMSS") & "||||N||||1||||||||||O"
                '& .Seq & "^" & .RackNo & "^" & .TubePos &
                
                If .NoOrder = True Then
                    '## 접수정보가 없을경우
                    strOutput = strOutput & "O|1|" & .BarNo & "|||R||||||C||||||||||||||Q" & vbCr
                Else
                    '## 접수정보가 있을경우
                    'strOutput = strOutput & "O|1|" & .BarNo & "||" & .GetOrder & "|R||||||||||S||||||||||Q" & vbCr
                    strOutput = strOutput & "O|1|" & .BarNo & "|" & .Seq & "^" & .RackNo & "^" & .TubePos & "^^S1^SC|" & .GetOrder & "|R||" & Format(Now, "YYYYMMDDHHMMSS") & "||||N||||1||||||||||O" & vbCr
                End If
