...
Begin VB.Form frmIISHitachi7180_1 
   BackColor       =   &H00DBE6E6&
   Caption         =   "Hitachi 7180"
   ClientHeight    =   9180
   ClientLeft      =   165
   ClientTop       =   450
   ClientWidth     =   15240
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISHitachi7180_1.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISHitachi7180_1.frx":0508
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISHitachi7180_1.frx":0D58
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISHitachi7180_1"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISHitachi7180.frm
'   작성자  : 오세원
'   내  용  : Hitachi 7180 장비폼
'   작성일  : 2008-07-09
'   버  전  :
'   병  원  :
'       1. 안동성소병원
...
Private strFe   As String
Private strUIBC As String
Private strTIBC As String
Private blnTIBC As Boolean
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Set frmIISHitachi7180_1 = Nothing
End Sub
...
Private Sub EditRcvData()
    ...
    Dim strType     As String   '수신한 Frame Character
    Dim strBarNo    As String   '수신한 바코드번호
    Dim strFu       As String   '수신한 Function Character
    Dim strDiskNo   As String   '수신한 Disk No
    Dim strPos      As String   '수신한 Position No
    Dim strIntBase  As String   '수신한 장비기준 검사명
    Dim strResult   As String   '수신한 결과
    Dim strTemp     As String
    Dim i           As Long

    strRcvBuf = mIntLib.Buffers(mIntLib.Buffers.Count).Buffers
    ...
    Select Case strType
        ...
        Case ";"    '## TS inquiry
            '## Function Char, Sample No, Disk No, Position, 바코드번호 조회
            ...
            With mOrder
                .ClsClear
                .BarNo = strBarNo
                .Fu = Mid$(strRcvBuf, 2, 2)
                .SampleData = Mid$(strRcvBuf, 4, 38)
                .DiskNo = Mid$(strRcvBuf, 9, 1)
                .Pos = Mid$(strRcvBuf, 10, 3)
            End With
            Call GetOrder(Trim$(strBarNo))
            
        Case "1", "2", "3", "4", "5"    '## Result
            mIntLib.BufCnt = mIntLib.BufCnt + 1
            mIntLib.State = "R"
            Call SendMorMessage
            
        Case ":"    '## End
            '## Control, Calibration 데이터는 무시하도록 수정
            strFu = Mid$(strRcvBuf, 2, 1)
            If UCase(strFu) = "H" Or UCase(strFu) = "G" Or UCase(strFu) = "F" Then
                Call SendMorMessage
                mIntLib.State = ""
                Exit Sub
            End If
            ...
            strDiskNo = Mid$(strRcvBuf, 9, 1)
            strPos = Trim$(Mid$(strRcvBuf, 10, 3))
            ...
            For Each objBuffer In mIntLib.Buffers
                ...
                For i = 1 To Len(strTemp) Step 10
                    strIntBase = Trim$(Mid$(strTemp, i, 3))
                    strResult = Trim$(Mid$(strTemp, i + 3, 6))
                    
                    'TIBC 결과 저장용
                    If Val(strIntBase) = 30 Then strFe = strResult
                    If Val(strIntBase) = 31 Then strUIBC = strResult
                    If Trim(strFe) <> "" And Trim(strUIBC) <> "" And IsNumeric(strFe) And IsNumeric(strUIBC) Then
                        strTIBC = Val(strFe) + Val(strUIBC)
                        strUIBC = "": strFe = ""
                        blnTIBC = True
                    End If
                    ...
                    If objIntNms.ExistIntBase(strIntBase) Then
                        Call objIntInfo.IntResults.Add(strIntBase, objIntNms.GetIntNm(strIntBase), _
                             strResult, strResult, strPos & "/" & strDiskNo)
                    End If
                Next i
            ...
            'TIBC 결과 저장용
            If blnTIBC And Trim(strTIBC) <> "" Then
                strIntBase = "93"
                If objIntNms.ExistIntBase(strIntBase) Then
                    Call objIntInfo.IntResults.Add(strIntBase, objIntNms.GetIntNm(strIntBase), _
                         strTIBC, strTIBC, strPos & "/" & strDiskNo)
                End If
                strTIBC = ""
                strIntBase = ""
                blnTIBC = False
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    Dim strPos As String    '검체위치
    
    strPos = Trim$(mOrder.Pos) & "/" & Trim$(mOrder.DiskNo)
    With tblReady
        ...
        .Col = TReadyEnum.ccNo:     .Text = strPos
