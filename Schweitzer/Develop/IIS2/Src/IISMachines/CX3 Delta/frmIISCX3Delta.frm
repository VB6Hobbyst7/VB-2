...
Begin VB.Form frmIISCX3Delta 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISCX3Delta.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISCX3Delta.frx":053D
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISCX3Delta.frx":0D7F
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISCX3Delta"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISCX3Delta.frm
'   작성자  : 이상대
'   내  용  : CX3 Delta 장비폼
'   작성일  : 2004-10-14
'   버  전  :
'       1. 1.0.2: 이상대(2005-06-24)
'   병  원  :
'       1. 예수병원
'       2. 단국대병원
'-----------------------------------------------------------------------------'
                    ...
                    Case 2  '## Message 대기
                        Select Case BufChar
                            ...
                            Case vbLf
                                '## ACK,ETX 구분하여 전송!
                                mIntLib.Phase = 3
                                strOutput = IIf(mIntLib.ACKFg = 1, ETX, ACK)
                                MSComm.Output = strOutput
                                Call mIntLib.WriteLog(strOutput, ccPCLog)
                                mIntLib.ACKFg = IIf(strOutput = ACK, 1, 0)
                            Case Else
                    ...
                    Case 3  '## EOT 대기
                        Select Case BufChar
                            Case EOT
                                ...
                                If mIntLib.State = "Q" Then
                                    mIntLib.SndPhase = 0
                                    MSComm.Output = EOT & SOH
                                    Call mIntLib.WriteLog(EOT & SOH, ccPCLog)
                                End If
                                mIntLib.Phase = 1
                            Case Else
...
Private Sub EditRcvData()
    ...
    Dim strStream    As String   '수신한 Stream
    Dim strFunction  As String   '수신한 Function
    Dim strRtnCd     As String   '수신한 Return Code
    Dim strBarNo     As String   '수신한 바코드번호
    Dim strError     As String   '수신한 에러
    Dim strSectorNo  As String   '수신한 Sector No
    Dim strCupNo     As String   '수신한 Cup No
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        '## CheckSum, "[", "]" 제거
        strRcvBuf = Mid$(strRcvBuf, 1, Len(strRcvBuf) - 2)
        strRcvBuf = Replace$(Replace$(strRcvBuf, "[", ""), "]", "")
        strStream = Trim$(mGetP(strRcvBuf, 2, ","))
        strFunction = Trim$(mGetP(strRcvBuf, 3, ","))
        
        Select Case strStream
            Case "701"  '## SampleCup Program(Request Order)
                Select Case strFunction
                    Case "02"   '## SampleCup return status
                        strRtnCd = Trim$(mGetP(strRcvBuf, 4, ","))
                        Select Case strRtnCd
                            Case "0"   '## OK
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                For i = 1 To tblReady.DataRowCnt
                                    Call tblReady.GetText(TReadyEnum.ccBarNo, i, vBarNo)
                                    If CStr(vBarNo) = strBarNo Then
                                        Call tblReady.SetText(TReadyEnum.ccSendFg, i, "1")
                                        Exit For
                                    End If
                                Next i
                            Case "1"    '## Syntax Error
                                ...
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "2"    '## Busy
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "BUSY"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "3"    '## Invalid Chemistry Requested
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "INVALID CHEMISTRY REQUESTED"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "4"    '## Invalid ORDAC Requested
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "INVALID ORDAC REQUESTED"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "5"    '## Invalid Chemistry Combination Programmed
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "INVALID CHEMISTRY COMBINATION PROGRAMMED"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "6"    '## Control Not Configured
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "CONTROL NOT CONFIGURED"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "7"    '## Calibrator Sector Only
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "CALIBRATOR SECTOR ONLY"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "8"    '## Mode Mismatch
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "MODE MISMATCH"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "9"    '## Synchron CX Error
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "SYNCHRON CX Error"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "10"   '## Completed Sample
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "COMPLETED SAMPLE"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "11"   '## Incompatible Fluid Types
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "Incompatible Fluid Types"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "12"   '## Incompatible Test Types
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "Incompatible Test Types"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                            Case "13"   '## Incompatible Patient Name
                                strBarNo = mGetP(strRcvBuf, 8, ",")
                                strError = "Incompatible Patient Name"
                                cmdAlarm.BackColor = vbRed
                                Call mIntErrors.AddX("X001", mEqpCd, mEqpKey, strBarNo, strError)
                        End Select
                        
                        '## 전송할 오더유무 판단
                        mIntLib.State = IIf(mOrder.SendCnt = 0, "", "Q")
                    Case "04"   '## Clear sector/sample IDs
                    Case "06"   '## Host Query sector/sample IDs Auto Clear Queue
                        mOrder.DeviceId = mGetP(strRcvBuf, 1, ",")
                        mOrder.SendCnt = 0
                        strRcvBuf = Mid$(strRcvBuf, 11, Len(strRcvBuf) - 12)
                        For i = 0 To 6
                            strBarNo = Trim$(Mid$(strRcvBuf, (i * 12) + 1, 11))
            ...
            Case "702"  '## Result
                Select Case strFunction
                    Case "01"   '## Cup header
                        '## Sector No, Cup No, 바코드번호 조회
                        strSectorNo = Trim$(mGetP(strRcvBuf, 8, ","))
                        strCupNo = Trim$(mGetP(strRcvBuf, 9, ","))
                        strBarNo = Trim$(mGetP(strRcvBuf, 13, ","))
                        
                        Set objIntInfo = New clsIISIntInfo
                        With objIntInfo
                            .BarNo = strBarNo
                            .SpcPos = strCupNo & "/" & strSectorNo
                        End With
                        
                    Case "03"   '## Test result
                        strIntBase = Trim$(mGetP(strRcvBuf, 11, ","))
                        strResult = Trim$(mGetP(strRcvBuf, 16, ","))
...
Private Sub SendOrder()
    Dim objAccinfo As clsIISAccInfo '접수내역 클래스
    Dim vSendFg    As Variant    'Spread의 전송여부(0:미전송,1:전송)
    Dim vBarNo     As Variant    'Spread의 바코드번호
    ...
    With tblReady
        For i = 1 To .DataRowCnt
            Call .GetText(TReadyEnum.ccSendFg, i, vSendFg)
            If CStr(vSendFg) = "0" Then
                Call .GetText(TReadyEnum.ccBarNo, i, vBarNo)
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    With tblReady
        ...
        .Col = TReadyEnum.ccSendFg: .Text = "0"
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISCX3Delta", "GetEqpComm", strErrMsg, Now
