...
Begin VB.Form frmIISAxsym 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISAxsym.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISAxsym.frx":0509
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISAxsym.frx":0D59
      TextTip         =   2
   End
...
Private Sub Form_Load()
    ...
    DoEvents
    'RackNo = "A": PosNum = 1
    'txtRackNo.Text = "A": txtPosNum.Text = "1"
...
'            strOutput = mIntLib.GetFrameNo & "P|1||" & mOrder.AccInfo.PtId & "|" & vbCr & ETX
            strOutput = "2P|1||" & mOrder.AccInfo.PtId & "|" & vbCr & ETX
            mIntLib.SndPhase = 4
        ...
        Case 4  '## Order
            With mOrder
                If .IsSending = False Then  '## 최초 보낼때
                    'strOutput = "O|1|" & .BarNo & "^0001^A||" & .GetOrder & "|R||||||N||||||||||||||Q"
                    'strOutput = "O|1|" & .BarNo & "||" & .GetOrder & "|||||||A||||||||||||||O"
...
'                        strOutput = mIntLib.GetFrameNo & Mid$(strOutput, 1, 230) & vbCr & ETB
                        strOutput = "3" & Mid$(strOutput, 1, 230) & vbCr & ETB
                        mIntLib.SndPhase = 4
                    Else
'                        strOutput = mIntLib.GetFrameNo & strOutput & vbCr & ETX
                        strOutput = "3" & strOutput & vbCr & ETX
                        mIntLib.SndPhase = 5
                    End If
                Else                        '## 남은 문자열이 있을때
                    strOutput = .Order
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    With tblReady
        ...
        .Col = TReadyEnum.ccBarNo:  .Text = pAccInfo.GetBarNo: mOrder.BarNo = .Text
        ...
        End If
'        .Col = 1: .Text = txtRackNo.Text & Format(txtPosNum.Text, "0000")
'        txtPosNum.Text = txtPosNum.Text + 1
'
'        If txtPosNum.Text >= 15 Then
'            txtRackNo.Text = Chr(Asc(txtRackNo.Text) + 1)
'        End If
        
        'txtRackNo.Text = RackNo
        'txtPosNum.Text = PosNum
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISAxsym", "GetEqpComm", strErrMsg, Now
...
'Dim strtmpBarno As String

    '## 해당 바코드번호에 대한 오더정보 조회
    If KeyCode = vbKeyReturn Then
        Me.MousePointer = vbHourglass
        'strtmpBarno = Trim(txtBarNo.Text)
