...
Begin VB.Form frmIISGemini 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISGemini.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISGemini.frx":04FA
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISGemini.frx":0D3C
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISGemini"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISGemini.frm
'   작성자  : 이상대
'   내  용  : Hitachi 7600 장비폼
'   작성일  : 2004-10-07
'   버  전  :
'       1. 1.0.1: 이상대(2005-07-12)
'   병  원  :
'       1. 예수병원
'       2. 성모자애병원
'   메  모  :
'       1. Serum Index 오더 전송유무를 병원별로 설정하도록 할것!
'-----------------------------------------------------------------------------'
...
''                mIntLib.State = "Q"
''
''            'O|1|||^^^RBio Norovirus||20130220145532
''            Case "O"    '## Order
''
''
''            'R|1|^^^RBio Norovirus|pos||
''            Case "R"    '## Result
''                strIntBase = mGetP(mGetP(strRcvBuf, 3, "|"), 2, "^^^")
...
Private Sub EditRcvData()
    ...
    Dim strPos       As String   '수신한 Sample Position
    Dim strIntBase   As String   '수신한 장비기준 검사명
    Dim strResult, strIntResult   As String   '수신한 결과
    Dim strTemp      As String
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        ...
        Case 0      '## 최초 오더 전송인 경우
'                    strOutput = "1H|\^&|||" & vbCr
'        strOutput = strOutput & "P|1||" & strBarNo & vbCr
'        If gblCnt = 0 Then
'            strOutput = strOutput & "O|1|" & strBarNo & "||^^^Chla|||||||||||S||||||||||X" & vbCr
'        ElseIf gblCnt = 1 Then
'            strOutput = strOutput & "O|1|" & strBarNo & "||^^^Clos|||||||||||S||||||||||X" & vbCr
'        ElseIf gblCnt = 2 Then
'            strOutput = strOutput & "O|1|" & strBarNo & "||^^^Noro|||||||||||S||||||||||X" & vbCr
'            strOutput = strOutput & "O|1|" & strBarNo & "||^^^Rota|||||||||||S||||||||||X" & vbCr
'        Else
'            strOutput = strOutput & "O|1|" & strBarNo & "||^^^Rota|||||||||||S||||||||||X" & vbCr
'        End If
'
...
'        strOutput = strOutput & ETX
            ...
            If mOrder.NoOrder = False Then
                ...
                strOutput = strOutput & "O|1|" & Format$(mOrder.BarNo, String$(12, "@")) & "||" & mOrder.GetOrder & "|||||||||||S||||||||||X" & vbCr
            Else
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISGemini", "GetEqpComm", strErrMsg, Now
