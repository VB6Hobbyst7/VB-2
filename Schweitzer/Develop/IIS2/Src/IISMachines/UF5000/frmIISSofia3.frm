...
Begin VB.Form frmIISSofia3 
   ...
   Begin IISSofia3.sckStringData sck 
      Height          =   300
      Left            =   15420
      TabIndex        =   26
      Top             =   8550
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISSofia3.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISSofia3.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISSofia3.frx":0D31
      TextTip         =   2
   End
   ...
   End
End
Attribute VB_Name = "frmIISSofia3"
...
'    TxtIP = Winsock1.LocalIP
    ...
    gOrderPort = GetPCXConfig("OrderPort")
    gResultPort = GetPCXConfig("ResultPort")
    
    Call WinSock_Listen(wSckOrder)
    Call WinSock_Listen(wSckResult)
...
Dim strReturnedString As String
    ...
    GetPrivateProfileString "IT1000", strConfigNm, "", strReturnedString, Len(strReturnedString), strFileName
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        Select Case strType
            ...
            Case "R"    '## Result
                ...
                strFlag = mGetP(mGetP(strRcvBuf, 4, "|"), 2, "^")   'RAW,  MAINFORMAT
                                
                If strTemp1 = "1+" Then
                    strResult = "+"
                ElseIf strTemp1 = "2+" Then
                    strResult = "++"
                ElseIf strTemp1 = "3+" Then
                    strResult = "+++"
                ElseIf strTemp1 = "4+" Then
                    strResult = "++++"
                ElseIf UCase(strTemp1) = "NORMAL" Then
                    strResult = "+/-"
                ElseIf strTemp1 = "+-" Then
                    strResult = "+/-"
                End If
                                
                If strResult <> "" And UCase(strFlag) = "RAW" Then
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        Case 1  '## Header
            strOutput = mIntLib.GetFrameNo & "1H|\^&&|||HOST||||||||LIS2-A2|" & vbCr & ETX
            mIntLib.SndPhase = 2
        ...
        Case 4  '## Order
            With mOrder
                If .NoOrder = True Then
                    '## 접수정보가 없을경우
                    strOutput = mIntLib.GetFrameNo & "O|1|" & .BarNo & "||R||||||C||||||||||||||Q" & vbCr & ETX
                    mIntLib.SndPhase = 5
                Else
                    If .IsSending = False Then  '## 최초 보낼때
                                                                         '|R||20170131012521                       ||||N||||||||||||||O|||||
                        strOutput = "O|1|" & .BarNo & "||" & .GetOrder & "|R||" & Format(Now, "yyyymmddhhmmss") & "||||N||||||||||||||O|||||"
...
Private Sub wSckOrder_Error(ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)
...
Private Sub wSckResult_Error(ByVal Number As Integer, Description As String, ByVal Scode As Long, ByVal Source As String, ByVal HelpFile As String, ByVal HelpContext As Long, CancelDisplay As Boolean)
