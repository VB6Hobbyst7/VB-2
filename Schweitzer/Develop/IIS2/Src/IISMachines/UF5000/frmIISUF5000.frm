...
Begin VB.Form frmIISUF5000 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISUF5000.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISUF5000.frx":0477
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISUF5000.frx":0C26
      TextTip         =   2
   End
   ...
   End
End
Attribute VB_Name = "frmIISUF5000"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISTriage.frm
'   작성자  : 오세원
'   내  용  : Triage 장비폼
'   작성일  : 2014-12-29
'   버  전  :
...
Private gOrderPort    As String
Private gResultPort   As String
...
'    TxtIP = Winsock1.LocalIP
    gOrderPort = GetUF5000Config("OrderPort")
    gResultPort = GetUF5000Config("ResultPort")
    
    Call WinSock_Listen(wSckOrder, gOrderPort)
    Call WinSock_Listen(wSckResult, gResultPort)
...
Public Sub WinSock_Listen(argWinSock As Winsock, pPort As String)

    On Error GoTo ErrorCheck
    
    If pPort = "0" Or IsNumeric(pPort) = False Then
        Exit Sub
    End If
    ...
    argWinSock.LocalPort = CInt(pPort)
    argWinSock.Listen
...
ErrorCheck:
    ...
    MsgBox pPort & " 포트에 정상적으로 연결되지 않았습니다. " & vbCrLf & "다시 한번 실행해 주세요." & vbCrLf & "ERROR : " & Err.Number & " - " & Err.Description
    
End Sub

Private Function GetUF5000Config(ByVal strConfigNm As String) As String
    Dim strFileName As String
    Dim strReturnedString As String
    
    GetUF5000Config = ""
    
    strFileName = App.Path & "\UF5000.ini"
    
    strReturnedString = String(1024, " ")
    GetPrivateProfileString "UF5000", strConfigNm, "", strReturnedString, Len(strReturnedString), strFileName
    strReturnedString = Trim(Replace(strReturnedString, Chr(0), Chr(32), 1, -1, vbBinaryCompare))
    GetUF5000Config = strReturnedString
...
Private Sub EditRcvData()
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        Select Case strType
            ...
            Case "P"    '## Patient
            Case "O"    '## Order
                '3O|1|138857||^^^RBC\^^^NL RBC\^^^WBC\^^^WBC Clumps\^^^EC\^^^Squa.EC\^^^Non SEC\^^^Tran.EC\^^^RTEC\^^^CAST\^^^Hy.CAST\^^^Path.CAST\^^^BACT\^^^X'TAL\^^^YLC\^^^SPERM\^^^MUCUS\|A||20160929201113||||N||||||||||||||O|||||
                '3O|1|QC-UK0056LL||^^^RBC\^^^WBC\^^^EC\^^^CAST\^^^BACT\^^^Cond.\                                                                                                             | ||              ||||Q||||||||||||||F
                
                '3O|1|ERR01000000001||^^^RBC\^^^NL RBC\^^^WBC\^^^WBC Clumps\^^^EC\^^^Squa.EC\^^^Non SEC\^^^Tran.EC\^^^RTEC\^^^CAST\^^^Hy.CAST\^^^Path.CAST\^^^BACT\^^^X'TAL\^^^YLC\^^^SPERM\^^^MUCUS\^^^RBC-Info.\^^^UTI-Info.\^^^BACT-Info.\^^^SF_DSS_PxSF_FSC_P\53
                
                If mGetP(strRcvBuf, 12, "|") = "Q" Then
                    strQC = "QC"
                Else
                    strQC = ""
                End If
                ...
                '바코드 리딩에러는 ERR은 제외하고 표시한다.
                If Mid(strBarNo, 1, 3) = "ERR" Then
                    strBarNo = Mid(strBarNo, 4)
                ElseIf Mid(strBarNo, 1, 3) = "End" Then
                    strBarNo = Format$(Now, "yymmddhhmmss")
                    strBarNo = Format$(strBarNo, String$(SPCLEN, "#"))
                'POSITIVEIDERR||
                ElseIf Mid(strBarNo, 1, 3) = "POS" Then
                    strBarNo = Format$(Now, "yymmddhhmmss")
                    strBarNo = Format$(strBarNo, String$(SPCLEN, "#"))
                Else
            ...
            Case "R"    '## Result
                '5R|1|^^^RBC^A^1^S^000019^1|----^RAW|/μl||A||||^^device||20160929184605|UF_4000
                strIntBase = Trim(mGetP(mGetP(strRcvBuf, 3, "|"), 4, "^"))
                strIntResult = Trim(mGetP(mGetP(strRcvBuf, 4, "|"), 1, "^"))
                
                strComm = Trim(mGetP(mGetP(strRcvBuf, 4, "|"), 2, "^"))   'RAW,  MAINFORMAT
                strFlag = Trim(mGetP(strRcvBuf, 7, "|"))   'RAW,  MAINFORMAT
...
'                    strFlag = "(+)"
'                End If
                strIntBase = Replace(strIntBase, "'", "")
                
                
'5R|1|^^^RBC^A^1^S^  E001^04|293.9^RAW|/μl||W||||^^device||20201020082506|UF-5000 ==> *
'5R|9| ^^^EC^A^1^S^  E001^04|540.4^RAW|/μl||>||||^^device||20201020082506|UF-5000   ==> +
                
                
                If strQC = "QC" Then
                    'strIntResult = Mid(strTemp, 5, 8)
                    If strIntBase = "RBC" Or strIntBase = "WBC" Or strIntBase = "EC" Or strIntBase = "CAST" Or strIntBase = "BACT" Then
                        strIntResult = Format(CCur(strIntResult), "#0.00")
                        strResult = strIntResult
                    End If
                    
                    If strResult <> "" And UCase(strComm) = "RAW" Then
                ...
                Else
                    If UCase(strComm) = "RAW" Then
                        '-- RBC, WBC, EC(Ep Cell)
                        If strIntBase = "RBC" Or strIntBase = "WBC" Or strIntBase = "EC" Then
                            If IsNumeric(strIntResult) Then
                                If CCur(strIntResult) <= 5.6 Then
                                     strResult = "1개미만"
                                '-- 2019.12.19 수정
                                'ElseIf CCur(strIntResult) <= 22.2 Then
                                ElseIf CCur(strIntResult) <= 27.8 Then
                                     strResult = "1-4개"
                                ElseIf CCur(strIntResult) <= 55.6 Then
                                     strResult = "5-9개"
                                ElseIf CCur(strIntResult) <= 111.1 Then
                                     strResult = "10-19개"
                                ElseIf CCur(strIntResult) <= 166.7 Then
                                     strResult = "20-29개"
                                '-- 2019.12.19 수정
                                'ElseIf CCur(strIntResult) <= 222.2 Then
                                ElseIf CCur(strIntResult) <= 277.8 Then
                                     strResult = "30-49개"
                                ElseIf CCur(strIntResult) <= 555.6 Then
                                     strResult = "50-99개"
                                Else
                                     strResult = "100개이상"
                                End If
                                
                                If strIntBase = "RBC" Or strIntBase = "EC" Then
                                    If strFlag = "W" Then
                                        strFlag = "(*)"
                                    ElseIf strFlag = ">" Then
                                        strFlag = "(+)"
                        ...
                        '-- BACT
                        If strIntBase = "BACT" Then
                            If strIntResult = "*" Then
                                strIntResult = "99,999.0"
                            End If
                            
                            '2018.12.04 : BACT 가 "++++" 일 경우 "VERY MANY" 로 표시
                            If strIntResult = "++++" Then
                                strResult = "VERY MANY"
                            Else
                                If IsNumeric(strIntResult) Then
                                    If CCur(strIntResult) <= 100 Then
                                         strResult = ""
                                    ElseIf CCur(strIntResult) <= 500 Then
                                         strResult = "FEW"
                                    ElseIf CCur(strIntResult) <= 1000 Then
                                         strResult = "SOME"
                                    ElseIf CCur(strIntResult) <= 5000 Then
                                         strResult = "MANY"
                                    Else
                                         strResult = "VERY MANY"
                                    End If
                        ...
                        '-- Flag 데이터
                        '3R|31|^^^SPERM^A^1^S^  E001^2|0.0   ^RAW|/μl||N||||^^device||20181114153616|UF-5000
                        '3R|31|^^^SPERM^A^1^S^  E002^6|1622.0^RAW|/μl||>||||^^device||20181115103903|UF-5000
                        If strIntBase = "XTAL" Then
                            If IsNumeric(strIntResult) Then
                                If CCur(strIntResult) <= 5.6 Then
                                     strResult = ""
                                ElseIf CCur(strIntResult) <= 27.8 Then
                        ...
                        If strIntBase = "YLC" Or strIntBase = "MUCUS" Then
                            If IsNumeric(strIntResult) Then
                        ...
                        '3R|31|^^^SPERM^A^1^S^  E014^05|47.1^RAW|/μl||>||||^^device||20200529105943|UF-5000
                        '4R|32|^^^SPERM^A^1^S^  E014^05|47.1^MAINFORMAT|/μl||>||||^^device||20200529105943|UF-5000
                        
                        '-- 2020-05-29 : 인터페이스 채널에 SPM 을 SPERM으로 수정
                        If strIntBase = "SPERM" Then
                            If IsNumeric(strIntResult) Then
                                If CCur(strIntResult) <= 5.6 Then
                                     strResult = ""
                                ElseIf CCur(strIntResult) <= 27.8 Then
                        ...
                        If strIntBase = "RTEC" Then
                            If IsNumeric(strIntResult) Then
                                ...
                                If strFlag = ">" Then
                                    strResult = strResult & "(+)"
                                Else
                                    '2018.11.27 : RTEC 가 10 이상일 경우
                                    If strIntResult >= 10 Then
                                        strResult = strResult & "(+)"
                                    Else
                                        '2018.12.03 : RTEC 가 10 이하일 경우 결과저장을 하지 않는다.
                                        strResult = ""
                                    End If
                                End If
                            End If
                        ...
                        If strIntBase = "CAST" Then
                            If IsNumeric(strIntResult) Then
                                If CCur(strIntResult) < 2.1 Then
                                     strResult = ""
                                ElseIf CCur(strIntResult) <= 3.8 Then
                                     strResult = "6-10개"
                                ElseIf CCur(strIntResult) <= 6.9 Then
                                     strResult = "11-20개"
                                Else
                                     strResult = "20개이상"
                                End If
                        ...
                        If strIntBase = "Hy.CAST" Then
                            If IsNumeric(strIntResult) Then
                                ...
                                If strFlag = ">" Then
                                    strResult = strResult & "(+)"
                                Else
                                    '2018.12.03 : Hy.CAST 가 1.5 이하일 경우 결과저장을 하지 않는다.
                                    If CCur(strIntResult) < 1.5 Then
                                        strResult = ""
                                    End If
                                End If
                            End If
                        ...
                        If strIntBase = "Path.CAST" Then
                            If IsNumeric(strIntResult) Then
                                If CCur(strIntResult) < 0.3 Then
                                     strResult = ""
                                ElseIf CCur(strIntResult) <= 1 Then
                                     strResult = "1-2개"
                                ElseIf CCur(strIntResult) <= 2.1 Then
                                     strResult = "3-5개"
                                ElseIf CCur(strIntResult) <= 3.8 Then
                                     strResult = "6-10개"
                                ElseIf CCur(strIntResult) <= 6.9 Then
                                ...
                                If strFlag = ">" Then
                                    strResult = strResult & "(+)"
                                Else
                                    '2018.12.03 : Path.CAST 가 1.5 이하일 경우 결과저장을 하지 않는다.
                                    If CCur(strIntResult) < 1.5 Then
                                        strResult = ""
                                    End If
                                End If
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        Case 1  '## Header
            strOutput = mIntLib.GetFrameNo & "H|\^&&|||HOST||||||||LIS2-A2|" & vbCr & ETX
            mIntLib.SndPhase = 2
            
        Case 2  '## Patient
            'P|1||||^^^^^||||||||||||||||||||||||||||||
            strOutput = mIntLib.GetFrameNo & "P|1||" & mOrder.Seq & "||^^^^^||||||||||||||||||||||||||||||" & vbCr & ETX
            mIntLib.SndPhase = 4
        ...
        Case 4  '## Order
            With mOrder
                If .NoOrder = True Then
                    '## 접수정보가 없을경우
                    strOutput = mIntLib.GetFrameNo & "O|1|" & .BarNo & "||R||" & Format(Now, "yyyymmddhhmmss") & "||||C||||||||||||||Q" & vbCr & ETX
                    mIntLib.SndPhase = 5
                Else
                    If .IsSending = False Then  '## 최초 보낼때
                                                                         '|A||20160929201113                       ||||N||||||||||||||O|||||
                        strOutput = "O|1|" & .BarNo & "||" & .GetOrder & "|A||" & Format(Now, "yyyymmddhhmmss") & "||||N||||||||||||||O|||||"
                        If Len(strOutput) > 240 Then
                            .IsSending = True
                            .Order = Mid$(strOutput, 241)
                            strOutput = mIntLib.GetFrameNo & Mid$(strOutput, 1, 240) & vbCr & ETB
                            mIntLib.SndPhase = 4
                        Else
...
'End Sub
'
Public Sub RcvSocketData(ByVal lsData As String, argWinSock As Winsock)
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
...
Private Sub wSckOrder_Close()
    ...
    If wSckOrder.State <> sckClosed Then
        wSckOrder.Close
    End If
    wSckOrder.LocalPort = CInt(gOrderPort)
    wSckOrder.Listen
...
Private Sub wSckOrder_DataArrival(ByVal bytesTotal As Long)
    ...
    wSckOrder.GetData strRcvBuffer

    Call RcvSocketData(strRcvBuffer, wSckOrder)
...
Private Sub wSckResult_Close()
    ...
    If wSckResult.State <> sckClosed Then
        wSckResult.Close
    End If
    wSckResult.LocalPort = CInt(gResultPort)
    wSckResult.Listen
...
Private Sub wSckResult_DataArrival(ByVal bytesTotal As Long)
    ...
    wSckResult.GetData strRcvBuffer

    Call RcvSocketData(strRcvBuffer, wSckResult)
