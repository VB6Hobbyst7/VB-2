...
Begin VB.Form frmIISBEP2000 
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SpreadDesigner  =   "frmIISBEPIII.frx":0000
      TextTip         =   2
   End
   Begin FPSpread.vaSpread tblReady 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISBEPIII.frx":0697
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISBEPIII.frx":0B7B
   End
   Begin MedControls1.LisLabel LisLabel3 
      ...
      Appearance      =   0
   End
   Begin MSComDlg.CommonDialog dlgResult 
      Left            =   7035
      Top             =   8490
      _ExtentX        =   847
      _ExtentY        =   847
   ...
   End
End
Attribute VB_Name = "frmIISBEP2000"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISBEP2000.frm
'   작성자  : 오세원
'   내  용  : BEP2000 장비폼
'   작성일  : 2007-09-04
'   버  전  :
...
Private Sub cmdOrder_Click()
    ...
    '## 오더파일 생성경로가 존재하지 않으면 폴더를 생성
    Call CreateFolder(mOrderPath)
    
    '## 기존에 JobList 파일이 있으면 삭제
    strFileNm = mOrderPath & "\" & mOrderFileNm
    ...
    '## 파일오픈
    lngFileNo = FreeFile
    Open strFileNm For Output As #lngFileNo
        ...
        For i = 1 To .DataRowCnt
            ...
            If CStr(vOrdChk) = "" Then
                ...
                If Not (objAccInfo Is Nothing) Then
                    ...
                    mOrder.BarNo = AddCheckDigit(CStr(vBarNo)) 'check digit 추가 수정
                    
                    If strRecord = "" Then
                        Print #lngFileNo, "Patient ID,Patient name,Test name,Test name,Birthdate,Sex,Collection date"
                    End If
                    
                    strRecord = mOrder.GetOrder(objAccInfo)
                    Print #lngFileNo, strRecord
...
Private Sub cmdResult_Click()
    Dim objPro      As clsIISProgress   'Progressbar
    Dim strFileNm   As String           '결과파일명(경로포함)
    Dim strRecord   As String           '결과파일의 Record
    Dim strIntNm    As String           '결과파일의 검사명
    Dim lngFileNo   As Long             'File No
    Dim i           As Long
    ...
    With dlgResult
        .InitDir = mResultPath
        .Filter = "BEP2000 Result (*." & mResultFileNm & ")|*." & mResultFileNm
        .DefaultExt = "*." & mResultFileNm
        .ShowOpen
    ...
    '## 파일오픈
    lngFileNo = FreeFile
    Open strFileNm For Input As #lngFileNo
    ...
    Do Until EOF(lngFileNo)
        Line Input #lngFileNo, strRecord
        
        If InStr(strRecord, "[Results]") > 0 Then
            i = i + 1
            blnDef = False
        End If
            
        If i = 2 Then
            If InStr(strRecord, "Patient ID") > 0 Then
                blnDef = True
                i = i + 1
            End If
            GoTo Skip
        ...
        If i = 3 And blnDef = True Then
            Call EditRcvData(strRecord, strIntNm)
        End If
Skip:
        objPro.Value = i
        DoEvents
        ...
        For i = TCompleteEnum.ccResult To .DataColCnt
            ...
            tblResult.Col = TResultEnum.ccInfo:
                ...
                If Trim$(strInfo) <> "" Then
                    tblResult.Text = vbCrLf & Space(2) & "[Result Comment]" & vbCrLf & strInfo
                End If
...
'   기능 : 장비로부터 수신한 데이터 편집
'   인수 :
'       - pRcvBuf : 장비로부터 수신한 데이터
'       - pIntNm  : 검사명
'-----------------------------------------------------------------------------'
Private Sub EditRcvData(ByVal pRcvBuf As String, ByVal pIntNm As String)
    Dim objIntInfo  As clsIISIntInfo    '인터페이스 검체정보 클래스
    Dim objIntNms   As clsIISIntNms     '장비별 검사항목 컬렉션 클래스
    Dim strType     As String   '수신한 Sample Type
    Dim strBarNo    As String   '수신한 BarNo
    Dim strNo       As String   '수신한 No
    ...
    '## 검체유형 조회
    pRcvBuf = Replace(pRcvBuf, """", "")
    
    '## 바코드번호, Position 조회
    strBarNo = mGetP(pRcvBuf, 1, ";")
    strBarNo = Mid(strBarNo, 1, 11)
    If Not IsNumeric(strBarNo) Then
        Exit Sub
    End If
    
    If strBarNo = "" Or Len(strBarNo) <> 11 Then
        Exit Sub
    End If
    
    pIntNm = mGetP(pRcvBuf, 2, ";")
    pIntNm = Mid(pIntNm, 4)
    
    If Len(pIntNm) > 10 Then
        pIntNm = Mid(pIntNm, 1, 7)
    End If
    ...
    strIntBase = objIntNms.GetIntBase(pIntNm)
    
    'AntiGen,HIV 와 AntiBidy의 데이터 포맷이 틀리다.
    'AntiGen,HIV = 4번째칸 : mGetP(pRcvBuf, 4, ";")
    'AntiBody    = 6번째칸 : mGetP(pRcvBuf, 6, ";")
    Select Case mGetP(strIntBase, 2, "|")
        Case "C"
            strResult = mGetP(pRcvBuf, 4, ";")
            Select Case strResult
                Case "-": strResult = IISNEGATIVE
                Case "+": strResult = IISPOSITIVE
            End Select
        Case "N"
            strResult = mGetP(pRcvBuf, 6, ";")
            Select Case strResult
                Case "<5.000":   strResult = IISLMTLOW
                Case ">100.000": strResult = IISLMTHIGH
                Case "overflow"
                    'A-HBsII|N  안티바디
                    If UCase(strIntBase) = "A-HBSII|N" Then
                        strResult = ">270"
                    End If
                Case "*****"
                    'A-HBsII|N  안티바디
    ...
    '## 결과저장
    If strIntBase <> "" Then
        Call objIntInfo.IntResults.Add(strIntBase, pIntNm, strResult, strResult)
    End If
...
Private Sub GetEqpComm()
    ...
    '## 오더파일 생성경로 체크
    If mOrderPath = "" Then
        strErrMsg = mEqpCd & " 장비의 오더파일 생성경로 정보가 없습니다."
        Error.SetLog App.EXEName, "frmIISBEP2000", "GetEqpComm", strErrMsg, Now
        Call mIntLib_EqpError("E007")
    End If
    
    '## 결과파일 생성경로 체크
    If mResultPath = "" Then
        strErrMsg = mEqpCd & " 장비의 결과파일 생성경로 정보가 없습니다."
        Error.SetLog App.EXEName, "frmIISBEP2000", "GetEqpComm", strErrMsg, Now
        Call mIntLib_EqpError("E008")
    End If
    
    '## 오더파일명 체크
    If mOrderFileNm = "" Then
        strErrMsg = mEqpCd & " 장비의 오더파일명 정보가 없습니다."
        Error.SetLog App.EXEName, "frmIISBEP2000", "GetEqpComm", strErrMsg, Now
        Call mIntLib_EqpError("E009")
    End If
    
    '## 결과파일 체크
    If mResultFileNm = "" Then
        strErrMsg = mEqpCd & " 장비의 결과파일명 확장자 정보가 없습니다."
        Error.SetLog App.EXEName, "frmIISBEP2000", "GetEqpComm", strErrMsg, Now
        Call mIntLib_EqpError("E010")
    End If
End Sub

'-----------------------------------------------------------------------------'
'   기능 : 지정한 폴더가 존재하지 않으면 생성
'   인수 :
'       - pPath : 경로
'-----------------------------------------------------------------------------'
Private Sub CreateFolder(ByVal pPath As String)
    Dim aryTemp()   As String
    Dim strTemp     As String
    Dim i           As Long
