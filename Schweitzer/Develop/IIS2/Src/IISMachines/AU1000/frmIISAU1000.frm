...
Begin VB.Form frmIISAU1000 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISAU1000.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISAU1000.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISAU1000.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISAU1000"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISAU1000.frm
'   작성자  : 이상대
'   내  용  : AU1000 장비폼
'   작성일  : 2004-06-15
'   버  전  :
...
Private Sub EditRcvData()
    ...
    Dim strCode     As String   '수신한 Classification Code
    Dim strBarNo    As String   '수신한 바코드번호
    Dim strRackNo   As String   '수신한 RackNo
    Dim strCupPos   As String   '수신한 Cup Position
    Dim strIntBase  As String   '수신한 장비기준 검사명
    Dim strResult   As String   '수신한 결과
    Dim strFlags    As String   '수신한 결과 Flags
    Dim strInfo     As String   '추가정보
    Dim strOutput   As String
    Dim i           As Long
    
    strRcvBuf = mIntLib.Buffers(1).Buffers
    strCode = Trim$(Mid$(strRcvBuf, 1, 2))
    
    Select Case strCode
        Case "RB"   '## Start Inquiry Text
            MSComm.Output = ACK
            Call mIntLib.WriteLog(ACK, ccPCLog)
            
        Case "R"    '## TRI Inquiry Text
            MSComm.Output = ACK
            Call mIntLib.WriteLog(ACK, ccPCLog)
            
            '## Rack No, Cup Position, 바코드번호 조회
            strBarNo = Trim$(Mid$(strRcvBuf, 13, SPCLEN))
            strRackNo = Mid$(strRcvBuf, 3, RACKNOLEN)
            strCupPos = Mid$(strRcvBuf, 7, CUPPOSLEN)
            ...
            With mOrder
                ...
                .CupPos = strCupPos
                .BarNo = strBarNo
                .SampleNo = Mid$(strRcvBuf, 9, SAMPLENOLEN)
            End With
        ...
        Case "DB"   '## Start Test Result Text
            MSComm.Output = ACK
            Call mIntLib.WriteLog(ACK, ccPCLog)
            
        Case "D"    '## Test Result Text
            '## Rack No, Cup Position, 바코드번호 조회
            strRackNo = Format$(Mid$(strRcvBuf, 3, RACKNOLEN), "####")
            strCupPos = Format$(Mid$(strRcvBuf, 7, CUPPOSLEN), "##")
            strBarNo = Mid$(strRcvBuf, 9, SPCLEN)
            
            Set objIntInfo = New clsIISIntInfo
            With objIntInfo
                .BarNo = strBarNo
                .SpcPos = strCupPos & "/" & strRackNo
            End With
            
            '## 결과저장
            Set objIntNms = mIntLib.IntNms
            For i = 24 To Len(strRcvBuf) - 1 Step 10
                strIntBase = Mid$(strRcvBuf, i, TESTCDLEN)
                strResult = Trim$(Mid$(strRcvBuf, i + 2, RESULTLEN))
                strFlags = Trim$(Mid$(strRcvBuf, i + 8, FLAGSLEN))
                strInfo = GetContent(strFlags)
...
Private Sub GetOrder(ByVal pBarNo As String)
    ...
    '## 장비가 바코드리딩에 실패한경우
    If Mid$(pBarNo, 1, 3) = "ERR" Then
        cmdAlarm.BackColor = vbRed
        Call mIntErrors.AddX("X002", mEqpCd, mEqpKey, Format$(mOrder.CupPos, "##") & _
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    ...
    With tblReady
        ...
        .Col = TReadyEnum.ccNo:     .Text = Format$(mOrder.CupPos, "##") & "/" & Format$(mOrder.RackNo, "####")
...
Errors:
    ...
    If Err.Number = 8005 Then
        ...
        Error.SetLog App.EXEName, "frmIISAU1000", "GetEqpComm", strErrMsg, Now
