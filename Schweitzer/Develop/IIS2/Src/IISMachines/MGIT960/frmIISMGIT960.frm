...
'                strTubePos = Format$(mGetP(strTemp2, 3, "^"), "##")
                ...
                '-- 결과유형 : INST_ONGOING INST_POSITIVE INST_NEGATIVE INST_COMPLETE INST_REMOVED (for Ongoing Removed only) INST_ERROR
                If strTemp1 <> "" Then
                    strIntBase = "AFB"
...
Private Sub SaveServer(ByVal pIntInfo As clsIISIntInfo, Optional ByVal pIntResult As String, Optional pResult As String)
    ...
    Dim lngSpcNo    As Long    '검체번호
    Dim i           As Long
    Dim objMGIT     As clsIISIntInfo   '장비에서 수신한 결과저장 클래스
...
'                                                         접수일 + 14 < 결과받은날짜
            If DateAdd("d", 14, Format(objAccInfo.ColDt, "yyyymmdd")) < Format(Now, "yyyymmdd") Then
                strResult = "01"    '2 week 접수일로부터 2주 이내
            Else
                strResult = "04"    '6 week 접수일로부터 2주 이상 경과
            End If
