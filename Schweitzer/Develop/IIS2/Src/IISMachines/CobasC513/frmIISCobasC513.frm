...
Begin VB.Form frmIISCobasC513 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISCobasC513.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISCobasC513.frx":04FA
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISCobasC513.frx":0D3C
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISCobasC513"
...
'   파일명  : frmIISC513.frm
'   작성자  :
'   내  용  : Modular C513 장비폼
'   작성일  : 2016-11-29
'   버  전  :
'   병  원  :
'   메  모  :
'       1. 테스트 해야함! (V1.0, V1.3 두가지 모드에서 테스트 해야함)
'       2. NOTE 2가지 꼭 확인할것!!!
'-----------------------------------------------------------------------------'
...
'Dim strBUFFF As String

Private mSpcPos  As String
...
'    strBUFFF = "1H|\^&|||cobasc513^1|||||HOST|TSREQ^REAL|P|1|20161219132807" & vbCr
'    strBUFFF = strBUFFF & "Q|1|^^002^0^00003^4^^S1^R1||ALL||||||||O" & vbCr
'    strBUFFF = strBUFFF & "L|1|N" & vbCr
'    strBUFFF = strBUFFF & "1A" & vbCrLf
'    strBUFFF = strBUFFF & ""
'
'    strBUFFF = ACK
'
        ...
        Select Case strType
            ...
            Case "Q"    '## Request Information
                ...
                With mOrder
                    ...
                    .Pos = mGetP(strTemp, 6, "^")
                    .RackType = mGetP(strTemp, 8, "^")  'S1
                    .ConType = mGetP(strTemp, 9, "^")   'R1
                End With
            ...
            Case "O"    '## Order
                ...
                With objIntInfo
                    .BarNo = strBarNo
                    .SpcPos = strPos & "/" & strRackNo 'mOrder.Pos & "/" & mOrder.RackNo
                End With
                
                mSpcPos = strPos & "/" & strRackNo


            Case "R"    '## Result
                '## NOTE: 정성결과가 나가는지 확인할것
                'strIntBase = mGetP(Replace$(mGetP(strRcvBuf, 3, "|"), "^^^", ""), 1, "/")
                'R|1|^^^29162/|4.93|%||N||F||bmserv|20161129161034|20161129162144|P1^1609-02
                '
                'strIntBase = mGetP(mGetP(strRcvBuf, 3, "|"), 1, "/")
                strIntBase = mGetP(strRcvBuf, 3, "|")
                strIntBase = mGetP(strIntBase, 4, "^")
                ...
                If objIntNms.ExistIntBase(strIntBase) Then
                    Call objIntInfo.IntResults.Add(strIntBase, objIntNms.GetIntNm(strIntBase), _
                         strResult, strResult, objIntInfo.SpcPos)
                End If
                mIntLib.State = "R"
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        ...
        Case 0      '## 최초 오더 전송인 경우
            '## Header
            strOutput = "H|\^&|||HOST^1|||||cobasc513|TSDWN^REPLY|P|1|" & Format(Now, "yyyymmddhhmmss") & vbCr
            ...
            If mOrder.NoOrder = False Then
                ...
                strOutput = strOutput & "O|1|" & mOrder.BarNo & "|" & mOrder.GetInstSpcId & _
                            "|" & mOrder.GetOrder & "|" & mOrder.GetPriority & "||" & Format(Now, "yyyymmddhhmmss") & "||||A||||" & _
                            mOrder.GetSampleType & "|||||||" & Format(Now, "yyyymmddhhmmss") & "|||O" & vbCr
            Else
                '## 접수정보가 없는경우: Default 검사항목[HbA1c : ^^^29162^] 을 보냄!
                strOutput = strOutput & "O|1|" & mOrder.BarNo & "|" & mOrder.GetInstSpcId & _
                            "|^^^29162^|R||" & Format(Now, "yyyymmddhhmmss") & "||||A||||" & _
                            mOrder.GetSampleType & "|||||||" & Format(Now, "yyyymmddhhmmss") & "|||O" & vbCr
            End If
...
Private Sub SetComplete2(ByVal pAccInfo As clsIISAccInfo)
    ...
    With tblComplete
        ...
        .Col = TCompleteEnum.ccNo:      .Text = mSpcPos 'pAccInfo.SpcPos
