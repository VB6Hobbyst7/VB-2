...
Begin VB.Form frmIISNS1000 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISNS1000.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISNS1000.frx":04E4
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISNS1000.frx":0D38
   End
   ...
   End
End
Attribute VB_Name = "frmIISNS1000"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISmNS1000.frm
'   작성자  : 오세원
'   내  용  : mNS1000 장비폼
'   작성일  : 2010-06-29
'   버  전  :
'       1. 1.0.0: 오세원(2010-06-29)
'   병  원  :
'       1. 전주예수병원
'-----------------------------------------------------------------------------'
...
'DU5160001 11              0.012    0(-/-/-)2010/ 6/2915:08:27 00AF1
        

'DC9001S01 41Low           0.230    98       10/03/201612:30:1200  1 10C8F
    
    Pos = InStr(strRcvBuf, "DU")
    If Pos > 0 Then
        strWorkNo = Trim(Mid(strRcvBuf, 3, 4))
        strRackNo = Trim(Mid(strRcvBuf, 7, 3))
    ...
    For Each objIntNm In mIntLib.IntNms
        ...
        If strIntBase <> strTemp2 Then
                Select Case strIntBase
                    Case "OBqn"  '## 정량
                        strResult = Trim$(Mid$(strRcvBuf, 32, 5))
                    Case "OBst"  '## 정성
                        strResult = Trim$(Mid$(strRcvBuf, 37, 7))
                    Case Else
                        strResult = Trim$(Mid$(strRcvBuf, 32, 5))
                End Select
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    ...
    Dim lngRackNo As Long
    Dim lngPosNo  As Long
...
'        End If
        ...
        If .DataRowCnt = 0 Then
            If Trim(txtPosNo.Text) <> "" Then
                lngPosNo = CLng(txtPosNo.Text)
                txtPosNo.Text = CStr(lngPosNo + 1)
            Else
                lngPosNo = 1
                txtPosNo.Text = CStr(lngPosNo + 1)
            End If
        ...
        '## Rack 구하기
        If txtPosNo.Text = "11" Then
            If .DataRowCnt = 0 Then
                If Trim(txtRackNo.Text) <> "" Then
                    lngRackNo = CLng(txtRackNo.Text)
                    txtRackNo.Text = CStr(lngRackNo + 1)
                Else
                    lngRackNo = 1
                    txtRackNo.Text = CStr(lngRackNo + 1)
                End If
        ...
        If lngRackNo = 0 Then lngRackNo = CLng(txtRackNo.Text)

        
'        .Col = TReadyEnum.ccNo:     .Text = CStr(lngWorkNo)
        .Col = TReadyEnum.ccNo:     .Text = lngRackNo & "-" & lngPosNo
