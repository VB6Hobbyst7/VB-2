Attribute VB_Name = "modAlloStation"
Option Explicit
...
Function CheckDisit(ByVal sCommoCode As String) As String
    ...
    scode = Mid(sCommoCode, 1, 11)
    scode = scode + Space(1)
    
    iAdd = 0
    iOld = 0

    For gi = Len(scode) To 1 Step -1
        Select Case gi
        Case 2, 4, 6, 8, 10, 12    '짝수
            iOld = iOld + Mid(scode, gi - 1, 1)
        Case 3, 5, 7, 9, 11  '홀수
            iAdd = iAdd + Mid(scode, gi - 1, 1)
        End Select
    Next
    
    iOld = iOld * 3
    iTot = iAdd + iOld              '짝수에 X 3

    CheckDisit = Right(10 - (iTot Mod 10), 1)  '10으로 나눗고 나머지를 10으로 뺀 값
