...
Begin VB.Form frmIISIT1000 
   ...
   Begin VB.FileListBox FileIT1000 
      Height          =   450
      Left            =   120
      Pattern         =   "*.hl7"
      TabIndex        =   24
   ...
   Begin FPSpread.vaSpread tblComplete 
      ...
      SpreadDesigner  =   "frmIISIT1000.frx":0000
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISIT1000.frx":08B4
      TextTip         =   2
   End
   Begin VB.Frame framConf 
   ...
   End
End
Attribute VB_Name = "frmIISIT1000"
...
Private PCXEqpCd(100)    As String
Private IT1000EqpCd(100) As String
...
'    Call GetEqpComm
    ...
    '-- 장비SN(Serial Number) Read
    For intCnt = 0 To 100
        If GetPCXConfig("IT1000-" & CStr(intCnt + 1)) <> "" Then
            PCXEqpCd(intCnt) = GetPCXConfig("IT1000-" & CStr(intCnt + 1))
        Else
            Exit For
        End If
    Next
    
    'FilePCX.Path = "\\172.16.10.179\DataForwardingDir\"
    FileIT1000.Path = GetPCXConfig("ServerPath")
    
    'Timer1.Interval = 1000
    Timer1.Interval = CLng(GetPCXConfig("Interval"))
    Timer1.Enabled = True
...
Private Sub lblGeneral_DblClick(Index As Integer)
    
    If framConf.Visible = True Then
        framConf.Visible = False
    Else
        framConf.Visible = True
    End If
...
Private Sub tblComplete_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    With tblComplete
        ...
        For i = TCompleteEnum.ccResult To .DataColCnt
            ...
            tblResult.Col = TResultEnum.ccInfo:         tblResult.Text = mGetP(strTemp, TResultEnum.ccInfo, DIV)
...
Private Sub EditRcvData()
    ...
    Dim strResult   As String   '수신한 결과값
    Dim strRackNo   As String   '수신한 Rack No
    Dim strPos      As String   '수신한 Tube Position
    Dim strRslt(13) As String
    Dim varRslt     As Variant
    Dim intCnt      As Integer
    
    'mIntLib.Buffers(1).Buffers = "201302210001|GLU2|173"
    '## 바코드번호
    strBarNo = mGetP(mIntLib.Buffers(1).Buffers, 1, "|")
    '## 검사채널
    strIntBase = mGetP(mIntLib.Buffers(1).Buffers, 2, "|")
    '## 검사결과
    strResult = mGetP(mIntLib.Buffers(1).Buffers, 3, "|")
    strPos = ""
    strRackNo = ""
    ...
    With objIntInfo
        .BarNo = strBarNo
        .SpcPos = mEqpKey '렉/포지션번호 보이는곳에 장비명 보이게함
    End With
    

    Set objIntNms = mIntLib.IntNms
    If objIntNms.ExistIntBase(UCase(strIntBase)) Then
        Call objIntInfo.IntResults.Add(UCase(strIntBase), objIntNms.GetIntNm(UCase(strIntBase)), _
             strResult, strResult)
    End If
        
    If GetPCXConfig("IT1000SaveFlag") = "1" Then
        '## DB에 결과저장
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISPCX", "GetEqpComm", strErrMsg, Now
...
On Error GoTo ErrRoutine
    ...
    For intIdx = 0 To FileIT1000.ListCount - 1
        ...
        '대상 파일 이름을 정의
        strDestFile = App.Path & "\Log\" & FileIT1000.FileName
        '원본을 대상에 복사
...
'        FileIT1000.Refresh
        ...
        For i = 0 To UBound(varBuffer)
            BufChar = Mid$(varBuffer(i), 1, 3)

            Select Case BufChar
                Case "MSH"
                    Call mIntLib.ClearBuffer
                    strtmpBuf = ""
                    If mGetP(varBuffer(i), 11, "|") = "Q" Then
                        Exit For
                    Else
                        Kill strSrcfile
                    End If
                ...
                Case "PID"
                    strtmpBuf = mGetP(varBuffer(i), 4, "|")
                    
                Case "OBX"
                    '-- 검사기기이름
                    If UCase(mGetP(varBuffer(i), 4, "|")) = "ANALYZERNAME" Then
                        mEqpCd = "": mEqpKey = ""
                        'ini파일에서 장비코드,장비키 읽어오기
                        For intCnt = 0 To UBound(IT1000EqpCd)
                            If PCXEqpCd(intCnt) = "" Then Exit For
                            
                            If mGetP(varBuffer(i), 6, "|") = PCXEqpCd(intCnt) Then
                                mEqpCd = PCXEqpCd(intCnt)
                                mEqpCd = GetPCXConfig(mEqpCd)
                                mEqpKey = "IT1000-" & CStr(intCnt + 1)
                                
                                mIntLib.mEqpCd = mEqpCd
                                mIntLib.mEqpKey = mEqpKey
                    ...
                    '-- 검사결과
                    ElseIf UCase(mGetP(varBuffer(i), 4, "|")) = "GLU2" Then
                        strtmpBuf = strtmpBuf & "|" & mGetP(varBuffer(i), 4, "|") & "|" & mGetP(varBuffer(i), 6, "|")
                        'strtmpBuf = "201302210001|GLU2|173"
...
Private Function GetPCXConfig(ByVal strConfigNm As String) As String

Dim strFileName As String
Dim strReturnedString As String

    strFileName = App.Path & "\IT1000.ini"
    ...
    GetPCXConfig = strReturnedString
