...
Begin VB.Form frmIISTriage 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISTriage.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISTriage.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISTriage.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISTriage"
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        Select Case strType
            ...
            Case "P"    '## Patient
                ...
                End With
            Case "O"    '## Order
                '5O|1||00071292^00055|NGAL^57835|S|||||||||||||||PASS    |RESULT APPROVED|20150113152800|||Q
            Case "R"    '## Result
                '6R|1|NGAL|> 1300|ng/mL|     0 to    153|H^09B7|N|F||9999999999
                '## 장비기준 검사명, 결과, Abnormal Flag
                strTemp1 = Trim(mGetP(strRcvBuf, 3, "|"))
                strTemp2 = Trim(mGetP(strRcvBuf, 4, "|"))
                'strUnit = mGetP(strRcvBuf, 5, "|")
                
                strIntBase = strTemp1
                strResult = strTemp2
                
                If strIntBase = "NGAL" Then
                    If IsNumeric(strResult) Then
                        If strResult > 1300 Then
                            strResult = ">1300"
                        End If
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        Case 0
            ...
            Exit Sub
        Case 1  '## Header
            strOutput = mIntLib.GetFrameNo & "H|\^&|||1234567890|||||||P||" & Format(Now, "YYYYMMDDhhmmss") & vbCr & ETX
            mIntLib.SndPhase = 2
        Case 2  '## Q
            strOutput = mIntLib.GetFrameNo & "Q|1|All||||" & Format(Now, "YYYYMMDDhhmmss") & "|" & Format(Now, "YYYYMMDDhhmmss") & "|||D||F" & vbCr & ETX
            mIntLib.SndPhase = 3
        Case 3  '## Termianator
