...
Begin VB.Form frmIISLpiaNv7 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISLpiaNv7.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISLpiaNv7.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISLpiaNv7.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISLpiaNv7"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISLpiaNv7.frm
'   작성자  : 이상대
'   내  용  : LPIA-NV7 장비폼
'   작성일  : 2004-10-15
'   버  전  :
'       1. 1.0.1: 이상대(2005-05-31)
'       2. 1.0.2: 이상대(2005-07-12)
'   병  원  :
'       1. 예수병원
'   메  모  : NOTE 확인할것!
'-----------------------------------------------------------------------------'
                    ...
                    Case 2      '## ETX대기
                        Select Case BufChar
                            Case ETX
                                ...
                                '## Reception Complete 메시지 전송
                                Call mSleep(500)
                                strOutput = mOrder.GetControlInfo
...
Private Sub EditRcvData()
    ...
    Dim strType      As String   '수신한 Message Type
    Dim strSpcType   As String   '수신한 Sample Type
    Dim strWorkNo    As String   '수신한 Worksheet No
    Dim strIntBase   As String   '수신한 장비기준 검사명
    Dim strIntResult As String   '수신한 정성결과
    Dim strResult    As String   '수신한 결과
    Dim strCheck     As String   '수신한 Data Check (<,>)
    Dim lngCnt       As Long     '수신한 결과수
    Dim i            As Long
    ...
    Select Case strType
        Case "8"    '## User ID specification item selection request
            strBarNo = Trim$(Mid$(strRcvBuf, 3, 13))
            With mOrder
                .ClsClear
                .BarNo = strBarNo
                .MsgType = Mid$(strRcvBuf, 2, 1)
            End With
            Call GetOrder(Mid$(strBarNo, 1, SPCLEN))
            
        Case "4"    '## Realtime measurement data
            '## Routine, Emergency Type만 결과수신
            strSpcType = Trim$(Mid$(strRcvBuf, 2, 1))
            If strSpcType <> "1" And strSpcType = "4" Then Exit Sub
            
            strWorkNo = Format$(Trim$(Mid$(strRcvBuf, 3, 4)), "####")
            strBarNo = Trim$(Mid$(strRcvBuf, 7, 13))
            ...
            strIntBase = Format$(Trim$(Mid$(strRcvBuf, 23, 3)), "000")
            If objIntNms.ExistIntBase(strIntBase) Then
                '## 등호가 있으면 결과에 붙힘
                strCheck = Mid$(strRcvBuf, 37, 1)
                If strCheck <> "" Then
                    strResult = strCheck & Trim$(Mid$(strRcvBuf, 30, 6))
                Else
                    strResult = Trim$(Mid$(strRcvBuf, 30, 6))
                End If
...
Private Sub SaveServer(ByVal pIntInfo As clsIISIntInfo)
    ...
    strBarNo = Mid$(pIntInfo.BarNo, 1, SPCLEN)
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISLpiaNv7", "GetEqpComm", strErrMsg, Now
