...
Begin VB.Form frmIISUriscanPro3 
   BackColor       =   &H00DBE6E6&
   Caption         =   "Uriscan Pro+ 3"
   ClientHeight    =   9180
   ClientLeft      =   225
   ClientTop       =   555
   ClientWidth     =   15120
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISUriscanPro3.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISUriscanPro3.frx":04E4
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISUriscanPro3.frx":0D26
   End
   ...
   End
End
Attribute VB_Name = "frmIISUriscanPro3"
...
'   작성일  : 2004-06-02
'   버  전  :
'       1. 1.0.1: 이상대(2005-02-22)
'       2. 1.0.4: 이상대(2005-07-12)
'   병  원  :
'       1. 동국대 경주병원
'       2. 예수병원
'       3. 안동성소병원 : 오세원(2008-07-09)
'-----------------------------------------------------------------------------'
...
'Buffer = "Date :May-28-2010 14:21:46"
'Buffer = Buffer & "ID_NO:0069-                "
'Buffer = Buffer & "Ward: "
'Buffer = Buffer & "Name: "
'Buffer = Buffer & "BLD    ++    50   RBC/ul "
'Buffer = Buffer & "BIL    -    neg          "
'Buffer = Buffer & "URO    -    neg          "
'Buffer = Buffer & "KET    -    neg          "
'Buffer = Buffer & "PRO    -    neg          "
'Buffer = Buffer & "NIT    +    pos          "
'Buffer = Buffer & "GLU    ++++2000   mg/dL  "
'Buffer = Buffer & "p.H         5.5          "
'Buffer = Buffer & "S.G       1.025          "
'Buffer = Buffer & "LEU    +     25   WBC/ul "
'Buffer = Buffer & "COL LT.Yellow  "
'Buffer = Buffer & "CLA Clear      "
'Buffer = Buffer & ""
...
'Normal : ID_NO:0001-140007408499
    ...
    If Pos > 0 Then
        ...
        'Call mIntLib.WriteLog("바코드0" & strBarNo, ccPC)
        
        strBarNo = Format(strBarNo, "#")
        
        'Call mIntLib.WriteLog("바코드1" & strBarNo, ccPC)
        
        '-- QC
        If Not IsNumeric(strBarNo) Then
            strWorkNo = CStr(Val(Trim$(Mid$(strRcvBuf, Pos + 6, 4))))
            
            'Call mIntLib.WriteLog("워크번호" & strWorkNo, ccPC)
            ...
            With tblReady
                For i = 1 To .DataRowCnt
                    ...
                    If CStr(vWorkNo) = strWorkNo Then
                        ...
                        Call mIntLib.WriteLog("바코드" & strBarNo, ccPC)
