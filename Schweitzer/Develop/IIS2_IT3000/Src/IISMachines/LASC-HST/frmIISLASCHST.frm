...
Private Sub Form_Load()
    ...
    Call GetEqpComm1    'XE-2100 통신설정
    Call GetEqpComm2    'LASC-HST 통신설정
    DoEvents
    ...
    With tblReady
        ...
        If Not (objAccInfo Is Nothing) Then
            If objAccInfo.QcFg = "0" Then
                ...
                Call .SetText(TReadyEnum.ccNo, .Row, "전송")
                Set objAccInfo = Nothing
            Else
                mIntLib.State = ""
                MSComm2.Output = EOT
...
Private Sub GetEqpComm2()
    ...
    If mBaudRate = "" Or mDataBit = "" Or mStopBit = "" Or mParityBit = "" Then
        strErrMsg = "LASC-HST 장비에 대한 통신설정이 되어 있지 않습니다."
        Error.SetLog App.EXEName, "frmIISLASCHST", "GetEqpComm", strErrMsg, Now
...
On Error GoTo Errors
    ...
    With MSComm2
        ...
        '## 이미 포트가 열린경우
        If .PortOpen Then
            strErrMsg = "LASC-HST 장비의 통신포트가 이미 열려있습니다."
            Error.SetLog App.EXEName, "frmIISLASCHST", "GetEqpComm", strErrMsg, Now
...
Errors:
    '## 다른 장치에서 포트를 사용하는 경우
    If Err.Number = 8005 Then
        strErrMsg = "LASC-HST 장비에 설정된 포트가 이미 사용중입니다."
