...
Begin VB.Form frmIISOCSensorII 
   BackColor       =   &H00DBE6E6&
   Caption         =   "OC-Sensor II"
   ClientHeight    =   9180
   ClientLeft      =   165
   ClientTop       =   450
   ClientWidth     =   15240
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISOCSensorII.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISOCSensorII.frx":04E4
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISOCSensorII.frx":0D26
      TextTip         =   2
   End
   ...
   End
End
Attribute VB_Name = "frmIISOCSensorII"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISOCSensorII.frm
'   작성자  : 이상대
'   내  용  : OC-SensorII 장비폼
'   작성일  : 2005-07-14
'   버  전  :
...
Private Sub EditRcvData()
    ...
    Dim strRstType  As String   '정성(C), 정량(N) 구분
    ...
    If mIntLib.BarPos = ccEqp Then
        strWorkNo = Format$(Mid$(strRcvBuf, 10, 4))
        strBarNo = Trim$(Mid$(strRcvBuf, 16, 14))
    Else
        strWorkNo = Format$(Mid$(strRcvBuf, 10, 4), "#")
    ...
    For Each objIntNm In mIntLib.IntNms
        strTemp1 = objIntNm.IntBase
        
        If strTemp1 <> strTemp2 Then
            strIntBase = Mid$(strTemp1, 1, Len(strTemp1) - 1)
            strRstType = Mid$(strTemp1, Len(strTemp1))
    
            Select Case strRstType
                Case "N"    '## 정량
                    strResult = Trim$(Mid$(strRcvBuf, 30, 4))
                Case "C"    '## 정성
                    strResult = Trim$(Mid$(strRcvBuf, 34, 1))
                    strResult = IIf(strResult = "-", IISNEGATIVE, IISPOSITIVE)
            End Select
            
            strInfo = GetInfo(Trim$(Mid$(strRcvBuf, 35, 1)))
            Call objIntInfo.IntResults.Add(strTemp1, objIntNm.IntNm, strResult, strResult, strInfo)
...
'-----------------------------------------------------------------------------'
'   기능 : 수신한 에러 Flag에 대한 상세설명 표시
'   인수 :
'       - pInfoCd : 에러코드
'   반환 : 에러코드에 대한 정보
'-----------------------------------------------------------------------------'
Private Function GetInfo(ByVal pInfoCd As String) As String
    Dim strInfo As String   '에러정보
    
    If pInfoCd = "" Then Exit Function
    
    Select Case pInfoCd
        Case "R": strInfo = "시약 없음"
        Case "S": strInfo = "샘플 없음"
        Case "E": strInfo = "Barcode reader NG"
        Case "D": strInfo = "Sample 적하 NG"
        Case "B": strInfo = "시약 Blank NG"
        Case "T": strInfo = "적하 Sensor Error"
        Case "#": strInfo = "Sample 다량적하"
        Case "V": strInfo = "Over Range"
        Case "U": strInfo = "Buffer 없음"
    End Select
    
    GetInfo = vbCrLf & Space(2) & "[Error Information]" & vbCrLf & Space(2) & strInfo & vbCrLf
End Function
        ...
        If pAccInfo.QcFg = "0" Then         '## 일반검체
            ...
            For Each objResult In pAccInfo.Results
                ...
                .Text = objResult.IntNm.IntNm & DIV & objResult.IntResult & DIV & objResult.RstCd & _
                        ...
                        IIf(objResult.Ref.RefFg = "1", mGetRef(objResult.Ref.RefFrVal, objResult.Ref.RefToVal), "") & _
                        objResult.IntInfo
                i = i + 1
            Next
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISOCSensorII", "GetEqpComm", strErrMsg, Now
