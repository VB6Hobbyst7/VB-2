...
'   파일명  : frmIISLH750.frm
'   작성자  : 이상대
'   내  용  : Coulter LH750 장비폼
'   작성일  : 2005-05-16
'   버  전  :
'       1. 1.0.1: 이상대(2005-06-02)
'   병  원  :
'       1. 성모자애병원
'-----------------------------------------------------------------------------'
...
Private Sub cmdSend_Click()
    Dim vOrdChk     As Variant  'Spread의 오더전송 여부
...
Private Sub EditRcvData()
    ...
    If Pos1 > 0 Then
        ...
        End If
    Else
        '## 1.0.1: 이상대(2005-06-02)
        '   - LH750 Format에서도 프로그램을 사용할수 있도록 수정
        Pos1 = 0
...
'   기능 : 장비로 오더정보 전송
'   인수 :
'       - pFlag: 장비로 부터 수신한 Flag
'-----------------------------------------------------------------------------'
Private Sub SendOrder(Optional ByVal pFlag As String = "")
    Dim objAccInfo  As clsIISAccInfo    '접수정보 객체
    Dim strOutput   As String           '송신할 데이터
        ...
        Case 3  '## Data Block 전송1
            Select Case pFlag
                ...
                Case NAK        'Abort
                    ...
                    mIntLib.SndPhase = 1
            End Select
        Case 4  '## Data Block 전송2, All Done ENQ 전송
            Select Case pFlag
                Case ACK        'Accept
                    '## 모든 전송이 완료시
                    strOutput = ENQ
...
'-----------------------------------------------------------------------------'
'   기능 : tblReady Spread에 전송한 검체에 표시
'   인수 :
'       - pBarNo : 바코드번호
'       - pFlag  : 수신 Flag
'-----------------------------------------------------------------------------'
