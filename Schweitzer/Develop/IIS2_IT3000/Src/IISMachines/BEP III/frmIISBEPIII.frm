...
Begin VB.Form frmIISBEPIII 
   ...
   End
End
Attribute VB_Name = "frmIISBEPIII"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISBEPIII.frm
'   작성자  : 이상대
'   내  용  : BEP III 장비폼
'   작성일  : 2005-09-12
'   버  전  :
...
Private Sub cmdResult_Click()
    ...
    With dlgResult
        .InitDir = mResultPath
        .Filter = "BEP III Result (*." & mResultFileNm & ")|*." & mResultFileNm
...
Private Sub EditRcvData(ByVal pRcvBuf As String, ByVal pIntNm As String)
    ...
    '## 검체유형 조회
    strType = mGetP(pRcvBuf, 2, ",")
    If strType <> "P" Then Exit Sub
    
    '## 바코드번호, Position 조회
    strBarNo = mGetP(pRcvBuf, 4, ",")
    strNo = mGetP(pRcvBuf, 1, ",")
    ...
    Select Case mGetP(strIntBase, 2, "|")
        Case "C"
            strResult = mGetP(pRcvBuf, 13, ",")
            Select Case strResult
                Case "-": strResult = IISNEGATIVE
                Case "+": strResult = IISPOSITIVE
            End Select
        Case "N"
            strResult = mGetP(pRcvBuf, 12, ",")
            
            '-- 2007.11.05 osw : AntiB채널 101 >> 8 으로 변경
            'If mGetP(strIntBase, 1, "|") = "101" Then
            If mGetP(strIntBase, 1, "|") = "8" Then
                Select Case strResult
...
Private Sub GetEqpComm()
    ...
    If mOrderPath = "" Then
        ...
        Error.SetLog App.EXEName, "frmIISBEPIII", "GetEqpComm", strErrMsg, Now
