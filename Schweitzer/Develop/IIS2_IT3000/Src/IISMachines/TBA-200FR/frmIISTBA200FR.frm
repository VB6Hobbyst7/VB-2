...
Begin VB.Form frmIISTBA200FR 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISTBA200FR.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISTBA200FR.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISTBA200FR.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISTBA200FR"
        ...
        For i = TCompleteEnum.ccResult To .DataColCnt
            ...
            tblResult.Col = TResultEnum.ccInfo:
                ...
                If Trim$(strInfo) <> "" Then
                    tblResult.Text = vbCrLf & Space(2) & "[Error]" & vbCrLf & strInfo
                End If
...
Private Sub EditRcvData()
    ...
    Dim strFlag      As String  '수신한 Error Flag
    Dim strError     As String  '수신한 Error
    Dim strTemp      As String
    Dim i            As Long
    ...
    Select Case strFunc
        Case "Q"    '## Order Request Message
            ...
            strBarNo = Trim$(Mid$(strRcvBuf, 7, 20))
            strDiskNo = Mid$(strRcvBuf, 27, 4)
            strPos = Mid$(strRcvBuf, 31, 2)
        ...
        Case "R"    '## Result Message
            ...
            strPos = Trim$(Mid$(strRcvBuf, 31, 2))
            ...
            '## 장비기준 검사명, 검사결과 조회
            strTemp = Mid$(strRcvBuf, 48, InStr(strRcvBuf, ETB) - 48)
            For i = 1 To Len(strTemp) Step 13
                strIntBase = Format$(Trim$(Mid$(strTemp, i, 4)), "00")
                strResult = Trim$(Mid$(strTemp, i + 4, 6))
                strFlag = Trim$(Mid$(strTemp, i + 11, 1))
                strError = GetError(strFlag)
...
'-----------------------------------------------------------------------------'
'   기능 : 수신한 Error Flag에 대한 상세설명 조회
'-----------------------------------------------------------------------------'
Private Function GetError(ByVal pFlag As String)
    Dim strInfo     As String

    If pFlag = "" Then Exit Function

    Select Case UCase$(pFlag)
        Case "H": strInfo = "Hardware error."
        Case "W", "S", "R", "G", "D", "B", "C"
            strInfo = "Error in the sample aspiration/dispensing system."
        Case "1", "A"
            strInfo = "Error in the photometric/ISE system."
        Case "K", "G", "X", "Y", "Z", "R", "L", "P", "C", "T", "N", "U", "O", "F", "I", "S", _
             "M", "E", "U"
            strInfo = "Error in the concentration calculation system."
    End Select

    GetError = Space(2) & strInfo & vbCrLf
End Function
