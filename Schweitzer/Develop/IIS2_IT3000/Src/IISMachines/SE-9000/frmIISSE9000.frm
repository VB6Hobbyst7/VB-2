...
Begin VB.Form frmIISSE9000 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISSE9000.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISSE9000.frx":04FA
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISSE9000.frx":0D3C
   End
End
Attribute VB_Name = "frmIISSE9000"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISSE9000.frm
'   작성자  : 이상대
'   내  용  : SE-9000 장비폼
'   작성일  : 2004-10-13
'   버  전  :
...
Private Sub EditRcvData()
    ...
    Dim strDicCd     As String   '수신한 Distinction Code
    ...
    Dim lngStart     As Long     '수신한 Data중 각 결과위치
    Dim lngLen       As Long     '수신한 Data중 가 결과길이
    Dim strTemp1     As String
    Dim strTemp2     As String

    strRcvBuf = mIntLib.Buffers(1).Buffers
    strDicCd = Mid$(strRcvBuf, 1, 1)
    
    Select Case strDicCd
        Case "R"    '## Inquiry Data Format
            '## 바코드번호, Rack No, Tube Postion 조회
            strBarNo = Format$(Trim$(Mid$(strRcvBuf, 3, 13)), String$(13, "#"))
            With mOrder
                .ClsClear
                .BarNo = strBarNo
                .RackNo = Mid$(strRcvBuf, 16, 4)
                .Pos = Mid$(strRcvBuf, 20, 2)
            End With
            
            '## 접수정보 조회, 오더전송
            Call GetOrder(strBarNo)
            
        Case "D"    '## Result Data Format
            If Mid$(strRcvBuf, 2, 1) = "B" Then Exit Sub    '## ?
            If Mid$(strRcvBuf, 3, 1) = "C" Then Exit Sub    '## QC Result
            
            '## 바코드번호, Rack No, Tube Position 조회
            strBarNo = Format$(Mid$(strRcvBuf, 22, 13), String$(13, "#"))
            strRackNo = Format$(Mid$(strRcvBuf, 10, 4), "####")
            strPos = Format$(Mid$(strRcvBuf, 14, 2), "##")
            
            '## 바코드리딩에 실패한 경우
            If Mid$(strBarNo, 1, 3) = "ERR" Then
                cmdAlarm.BackColor = vbRed
                Call mIntErrors.AddX("X002", mEqpCd, mEqpKey, strRackNo & "/" & strPos, _
            ...
            For Each objIntNm In mIntLib.IntNms
                ...
                If strTemp1 <> strTemp2 Then
                    ...
                    lngStart = CLng(mGetP(strTemp1, 2, "|"))
                    lngLen = CLng(mGetP(strTemp1, 3, "|"))
                    strResult = Trim$(Mid$(strRcvBuf, lngStart, lngLen - 1))
                    
                    If strResult <> "" Then
                        Select Case strIntBase
                            Case "02"   '## RBC
                                strResult = Format$(Format$(strResult, "@@.@@"), "#0.##")
                            Case "08"   '## PLT
                                strResult = Format$(Format$(strResult, "@@@@"), "####")
                            Case Else
                                If lngLen = 5 Then
                                    strResult = Format$(Format$(strResult, "@@@.@"), "##0.#")
                                ElseIf lngLen = 6 Then
                                    strResult = Format$(Format$(strResult, "@@@.@@"), "##0.##")
                                End If
                        End Select
                        ...
                        If Not (IsNumeric(strResult)) Then
                            strResult = IISERROR
                        End If
                        Call objIntInfo.IntResults.Add(strTemp1, objIntNm.IntNm, strResult, strResult)
                    End If
                    strTemp2 = strTemp1
                End If
            Next
...
Private Sub GetOrder(ByVal pBarNo As String)
    ...
    If Not (objAccInfo Is Nothing) Then
        ...
        Set objAccInfo = Nothing
    Else
        '## No Order 전송: 접수정보가 없더라도 장비는 기본검사를 진행하고 결과를 전송함
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISSE9000", "GetEqpComm", strErrMsg, Now
