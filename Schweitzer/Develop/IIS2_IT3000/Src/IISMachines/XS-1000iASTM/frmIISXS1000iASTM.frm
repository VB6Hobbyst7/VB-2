...
Begin VB.Form frmIISXS1000iASTM 
   BackColor       =   &H00DBE6E6&
   Caption         =   "XS1000i"
   ClientHeight    =   9180
   ClientLeft      =   165
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISXS1000iASTM.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISXS1000iASTM.frx":0508
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISXS1000iASTM.frx":0D58
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISXS1000iASTM"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISXS1000iASTM.frm
'   작성자  : 오세원
'   내  용  : XS1000iASTM 장비폼
'   작성일  : 2008-02-20
'   버  전  :
        ...
        Select Case strType
            ...
            Case "Q"    '## Request Information
                ...
                With mOrder
                    ...
                    .RackNo = Trim$(mGetP(strTemp, 1, "^"))
                    .Pos = Trim$(mGetP(strTemp, 2, "^"))
            ...
            Case "O"    '## Order
                ...
                strRackNo = Trim$(mGetP(strTemp, 1, "^"))
                strPos = Trim$(mGetP(strTemp, 2, "^"))
            ...
            Case "R"    '## Result
                ...
                strIntBase = mGetP(mGetP(Replace(strRcvBuf, "^^^^", ""), 3, "|"), 1, "^")
                ...
                If objIntNms.ExistIntBase(strIntBase) Then
                    Call objIntInfo.IntResults.Add(strIntBase, objIntNms.GetIntNm(strIntBase), _
                         strResult, strResult, strPos & "/" & strRackNo)
                End If
                mIntLib.State = "R"
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        Case 0  '## Header
            strOutput = mIntLib.GetFrameNo & "H|\^&|||||||||||E1394-97" & vbCr & ETX
            mIntLib.SndPhase = 1
            
        Case 1  '## Patient
            '-- manual
            'P|1|||100|^Carol^Thomas||20010820|F|||||^Dr. N||||||||||||^^^East[CR]
            '-- real
            strOutput = mIntLib.GetFrameNo & "P|1" & vbCr & ETX
            mIntLib.SndPhase = 2
            
        Case 2  '## Patient Comment
            strOutput = mIntLib.GetFrameNo & "C|1" & vbCr & ETX
            mIntLib.SndPhase = 3
            
        Case 3  '## Order
            '-- manual
            'O|1|^^123-4567-890123^C||^^^^WBC\^^^^RBC\····^^^^NRBC||20010807101000|||||N||||||||||||||Q[CR]
            '-- real
            '4O|1||2^2^    18000091918^B|^^^^WBC\^^^^RBC\^^^^HGB\^^^^HCT\^^^^MCV\^^^^MCH\^^^^MCHC\^^^^PLT\^^^^NEUT%\^^^^LYMPH%\^^^^MONO%\^^^^EO%\^^^^BASO%\^^^^NEUT#\^^^^LYMPH#\^^^^MONO#\^^^^EO#\^^^^BASO#\^^^^IG#\^^^^IG%\^^^^RDW-SD\^^^^RDW-CV\^^^^PDW\^^^^6F
            '5MPV\^^^^P-LCR\^^^^PCT\^^^^RET%\^^^^RET#\^^^^IRF\^^^^LFR\^^^^MFR\^^^^HFR\^^^^IPF|||||||N||||||||||||||F
            
            With mOrder
                If .NoOrder = True Then
                    '-- manual
                    '<STX>3O|1|2^1^ 1234567890^B||||20010910101000|||||||||||||||||||Y<CR><ETX><CHK1><CHK2><CR><LF>
                    
                    '## 접수정보가 없을경우
                    strOutput = mIntLib.GetFrameNo & "O|1|" & .RackNo & "^" & .Pos & "^" & Space(4) & .BarNo & "^C||||" & Format(Now, "yyyymmddhhmmss") & _
                                                     "|||||||||||||||||||Y" & vbCr & ETX
                    mIntLib.SndPhase = 4
                Else
                    If .IsSending = False Then  '## 최초 보낼때
                        strOutput = "O|1|" & .RackNo & "^" & .Pos & "^" & Space(4) & .BarNo & "^C||" & .GetOrder & _
                                    "||" & Format(Now, "yyyymmddhhmmss") & "|||||N||||||||||||||Q"
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISXS1000iASTM", "GetEqpComm", strErrMsg, Now
