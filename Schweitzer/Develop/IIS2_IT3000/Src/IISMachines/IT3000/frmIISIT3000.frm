...
Begin VB.Form frmIISIT3000 
   ...
   Begin VB.Frame fraHidden 
      ...
      Begin VB.CommandButton Command1 
         ...
         Width           =   1245
      End
      Begin VB.FileListBox FileIT3000 
         Height          =   270
         Left            =   135
         Pattern         =   "*.dat"
         TabIndex        =   13
      ...
      Begin FPSpread.vaSpread tblResult 
         ...
         SpreadDesigner  =   "frmIISIT3000.frx":0000
         TextTip         =   2
      End
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISIT3000.frx":0697
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISIT3000.frx":0BB7
   End
   ...
   End
End
Attribute VB_Name = "frmIISIT3000"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISIT3000.frm
'   작성자  : 오세원
'   내  용  : IT3000 장비폼
'   작성일  : 2014-07-28
'   버  전  :
    ...
    With tblReady
        ...
        For i = 1 To .DataRowCnt
            ...
            If IsNumeric(vOrdChk) Then
                ...
                'Set objAccInfo = mIntLib.GetAccInfo_New_Temp1(CStr(vBarNo), chkPOC.Value)
                
                strRecord = Get_TestList(CStr(vAccNo))
                
                '-- 등록된 검사항목일 경우
                If strRecord <> "" Then
                    varRecord = Split(strRecord, "|")
                    '## 기존에 Dat 파일이 있으면 삭제
...
'                        If strSPC = "1A" Or strSPC = "1H" Then
                    ...
                    If strTestCd <> "" Then
                                 strSql = "UPDATE S2LAB320 SET STSCD = '1' " & vbCr
                        strSql = strSql & " WHERE WORKAREA = '" & strWA & "'" & vbCr
                        strSql = strSql & "   AND ACCDT = '" & strAccDt & "'" & vbCr
                        strSql = strSql & "   AND ACCSEQ = " & strAccseq & vbCr
...
On Error GoTo ErrRoutine
    ...
    For intIdx = 0 To FileIT3000.ListCount - 1

        FileIT3000.ListIndex = intIdx

'        strSrcfile = FileIT3000.Path & "\" & FileIT3000.FileName    ' 원본 파일 이름을 정의합니다.
'        If Right(FileIT3000.Path, 1) <> "\" Then
'            strSrcfile = FileIT3000.Path & "\" & FileIT3000.FileName    ' 원본 파일 이름을 정의합니다.
'        Else
            strSrcfile = FileIT3000.Path & FileIT3000.FileName     ' 원본 파일 이름을 정의합니다.
'        End If
        ...
        '대상 파일 이름을 정의
        strDestFile = App.Path & "\Log\" & FileIT3000.FileName
        '원본을 대상에 복사
...
Public Function Get_NewResult() As ADODB.Recordset
    ...
    'strSql = strSql & " AND a.ptid = '01016767' "
...
Public Function Get_SendFlag(ByVal pAccNo As String) As Boolean
    ...
    strAccseq = mGetP(pAccNo, 3, "-")
             ...
             strSql = "SELECT STSCD " & vbCr
    strSql = strSql & "  FROM S2LAB320 " & vbCr
    strSql = strSql & " WHERE WORKAREA = '" & strWA & "'" & vbCr
    strSql = strSql & "   AND ACCDT = '" & strAccDt & "'" & vbCr
    strSql = strSql & "   AND ACCSEQ = " & strAccseq & vbCr
    strSql = strSql & "   AND TESTCD IN (SELECT INTBASE FROM S2LAB702 WHERE EQPCD = '" & mEqpCd & "')"
    strSql = strSql & "   AND SPCCD IN ('1A','1H','3A','3I','3C','4L','4E','1J','4B','S1','S2','S3','S4','S5','S6','S7','S8','S9','30','40','4P','4V','4T','T3','U1','3D','1X') "
    strSql = strSql & " ORDER BY TESTCD  "
...
'    Dim lngSpcNo    As Long
    ...
    strAccseq = mGetP(pAccNo, 3, "-")
             ...
             strSql = "SELECT DISTINCT SPCCD, TESTCD " & vbCr
    strSql = strSql & "  FROM S2LAB320 " & vbCr
    strSql = strSql & " WHERE WORKAREA = '" & strWA & "'" & vbCr
    strSql = strSql & "   AND ACCDT = '" & strAccDt & "'" & vbCr
    strSql = strSql & "   AND ACCSEQ = " & strAccseq & vbCr
...
Private Sub Get_SearchList()
    ...
    If blnRS = False Then
        MsgBox "IT3000 조회 자료가 없습니다.", vbOKOnly + vbInformation, Me.Caption
        Exit Sub
    End If
...
Function Get_TestList(ByVal pAccNo As String) As String
    ...
    If Not AdoRS.BOF Then
        AdoRS.MoveFirst
        Do Until AdoRS.EOF
            Get_TestList = Get_TestList & Trim(AdoRS.Fields("SPCCD").Value) & "#" & Trim(AdoRS.Fields("TESTCD").Value) & "|"
            AdoRS.MoveNext
        Loop
    End If
    
    If Get_TestList <> "" Then
        Get_TestList = Mid(Get_TestList, 1, Len(Get_TestList) - 1)
    End If
...
'    FileIT3000.Path = mReadINI(strFileNm, pEqpKey, "ResultPath")
'
...
'    FileIT3000.Path = mOrderPath '& "\"
    FileIT3000.Path = mResultPath '& "\"
...
Private Sub EditRcvData()
    ...
    'varTmp = Split(strRcvBuf, vbLf)
    
    'MSH||\^&|PACMAN^PACMAN-LAB^PSM-LAB|PACMAN_PACMAN-
    'LAB|||20140728153137||SSU^U03|101627360|P|2.4|||NE|NE||8859/1|
    'EQU||^^^RSA3^^SAMPLEEVENT^SEEN|20140728153133|
    'SAC|||04207585422|||01|20140728153137|P|INSTRUMENT|RSA3||
    ...
    For i = 0 To UBound(varTmp)
        ...
        Case "LAB"
        Case "EQU"
            strResult = mGetP(strTemp1, 3, "|")
            strResult = mGetP(strResult, 7, "^")
        Case "SAC"
            strBarNo = mGetP(strTemp1, 4, "|")
        End Select
    Next
    
    'If strBarNo <> "" And IsNumeric(strBarNo <> "") And Len(strBarNo) = "11" And UCase(strResult) = "SEEN" Then
