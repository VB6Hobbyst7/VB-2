...
Begin VB.Form frmIISPCX 
   ...
   Begin FPSpread.vaSpread tblComplete 
      ...
      SpreadDesigner  =   "frmIISPCX.frx":0000
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISPCX.frx":07AF
      TextTip         =   2
   End
...
'    Call GetEqpComm
    ...
    For intCnt = 0 To 100
        If GetPCXConfig("PCX" & CStr(intCnt + 1)) <> "" Then
            PCXEqpCd(intCnt) = GetPCXConfig("PCX" & CStr(intCnt + 1))
        Else
            Exit For
        End If
    Next
    ...
    FilePCX.Path = GetPCXConfig("ServerPath")
...
Private Sub EditRcvData()
    ...
    '가공전 : " 0   0 "9"          "67ek2e"    8   0   0  99   0    0     0  -1 1122258750 "05043529911"
    strResult = mIntLib.Buffers(1).Buffers
    
    For intCnt = 0 To Len(strResult)
        If InStr(strResult, "  ") > 0 Then
            strResult = Replace(strResult, "  ", " ")
        Else
            strResult = Replace(strResult, """", "")
            varRslt = Split(strResult, " ")
            '가공후 : 0 0 9 67ek2e 8 1 0 225 0 0 0 -1 1122259116 170002344713
            Exit For
        End If
    Next
        
    '## 바코드번호 조회
    strBarNo = Trim$(Mid$(varRslt(13), 1, SPCLEN))
    strPos = ""
    strRackNo = ""
    ...
    If GetPCXConfig("PCXSaveFlag") = "1" Then
        '## DB에 결과저장
...
On Error GoTo ErrRoutine
    ...
    For intIdx = 0 To FilePCX.ListCount - 1
        FilePCX.ListIndex = intIdx
                
        strSrcfile = FilePCX.Path & FilePCX.FileName    ' 원본 파일 이름을 정의합니다.
        ...
        '대상 파일 이름을 정의
        strDestFile = App.Path & "\Log\" & FilePCX.FileName
        '원본을 대상에 복사
        ...
        For i = 1 To lngBufLen
            ...
            Select Case mIntLib.Phase
                Case 1
                    Select Case BufChar
                        Case vbLf
                            If InStr(strtmpBuf, "MeteorRecordID") > 0 Then
                                mIntLib.Phase = 2
                            End If
                ...
                Case 2
                    Select Case BufChar
                        Case vbLf
                            ...
                            For intCnt = 0 To UBound(PCXEqpCd)
                                If PCXEqpCd(intCnt) = "" Then Exit For
                                If InStr(strtmpBuf, PCXEqpCd(intCnt)) > 0 Then
                                    mEqpCd = PCXEqpCd(intCnt)
                                    mEqpCd = GetPCXConfig(mEqpCd)
                                    mEqpKey = "PCX" & CStr(intCnt + 1)
                ...
                Case 3
                    Select Case BufChar
                        Case vbLf
                            If InStr(strtmpBuf, "MeteorRecordPatientTest") > 0 Then
                                mIntLib.Phase = 4
                            End If
                            Call mIntLib.ClearBuffer
                            strtmpBuf = ""
                ...
                Case 4
                    Select Case BufChar
                        Case vbLf
                            ' 0   0 "9"          "67ek2e"    8   1   0  225   0    0     0  -1 1122259116 "05043531411" & vbCr & vbLf
                            strtmpBuf = Mid(strtmpBuf, InStr(strtmpBuf, "0"))
                            strtmpBuf = Replace(strtmpBuf, vbCr, "")
                            strtmpBuf = Replace(strtmpBuf, vbLf, "")
