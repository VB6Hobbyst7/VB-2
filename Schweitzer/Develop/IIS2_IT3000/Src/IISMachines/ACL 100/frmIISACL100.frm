...
Begin VB.Form frmIISACL100 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISACL100.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISACL100.frx":04E4
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISACL100.frx":0D26
   End
   ...
   End
End
Attribute VB_Name = "frmIISACL100"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISACL100.frm
'   작성자  : 이상대
'   내  용  : ACL 100 장비폼
'   작성일  : 2004-08-26
'   버  전  :
...
Private mType       As String   'Protocol, Message Type 구분(P,M)
Private mAck        As String   'ACK String
Private mChkCnt     As Long     'Check Sum Count
Private mPrevWorkNo As Long     '이전 WorkNo
            ...
            For i = 1 To lngBufLen
                ...
                Select Case mIntLib.Phase
                    Case 1      '## Protocol Header 수신대기
                        ...
                        End Select
                    Case 2      '## Message Header, Content 수신대기
                        Select Case BufChar
                            Case vbCr
                            Case vbLf
'                                If mType = "P" Then
...
'                                End If
                            Case Else
                                Call mIntLib.AddBuffer(BufChar)
                        End Select
                    Case 3      '## Protocol Tail 수신대기
                        mChkCnt = mChkCnt + 1
                        If mChkCnt = 2 Then
                            Call EditRcvData
                            MSComm.Output = mAck
                            Call mIntLib.WriteLog(mAck, ccPCLog)
                            mChkCnt = 0
                            mIntLib.Phase = 1
                        End If
                End Select
...
Private Sub EditRcvData()
    ...
    Dim lngWorkNo   As Long     '수신한 Message ID
    Dim strTemp     As String
    Dim i           As Long
    
    strRcvBuf = mIntLib.Buffers(1).Buffers
    If mType = "P" Then     '## Protocol Type
        mAck = "!" & Mid$(strRcvBuf, 1, 4)
        Exit Sub
    End If
    
    '## Message Header인 경우
    If Len(strRcvBuf) > 26 Then
        mPrevWorkNo = 0
        Exit Sub
    End If
    
    lngWorkNo = CLng(Right(strRcvBuf, 2))
    If lngWorkNo >= 1 And lngWorkNo <= 18 Then
        If mPrevWorkNo <> lngWorkNo Then
            mPrevWorkNo = lngWorkNo
            
            '## 바코드번호 조회
            With tblReady
                For i = 1 To .DataRowCnt
                    Call .GetText(TReadyEnum.ccNo, i, vWorkNo)
                    If CLng(vWorkNo) = lngWorkNo Then
            ...
            For Each objIntNm In mIntLib.IntNms
                ...
                If strIntBase <> strTemp Then
                    Select Case strIntBase
                        Case "PT(S)":   strResult = Trim$(Mid$(strRcvBuf, 1, 4))
                        Case "PT(%)":   strResult = Trim$(Mid$(strRcvBuf, 5, 5))
                        Case "PT(INR)": strResult = Trim$(Mid$(strRcvBuf, 11, 5))
                        Case "FIB":     strResult = Trim$(Mid$(strRcvBuf, 15, 5))
                    End Select
                    
                    If strResult <> "" Then
                        Call objIntInfo.IntResults.Add(strIntBase, objIntNm.IntNm, strResult, _
                             strResult)
                        strResult = ""
                    End If
                    strTemp = strIntBase
        ...
        Else
            ...
            With tblComplete
                For i = .DataRowCnt To 1 Step -1
                    Call .GetText(TCompleteEnum.ccNo, i, vWorkNo)
                    If CLng(vWorkNo) = lngWorkNo Then
            ...
            For Each objIntNm In mIntLib.IntNms
                ...
                If strIntBase <> strTemp Then
                    Select Case strIntBase
                        Case "APTT(S)": strResult = Trim$(Mid$(strRcvBuf, 1, 4))
                        Case "APTT(R)": strResult = Trim$(Mid$(strRcvBuf, 5, 4))
                    End Select
...
Private Sub CtlClear(Optional ByVal pFlag As ClearEnum = ccAll)
    ...
    If pFlag = ccAll Then
        txtBarNo.Text = "":         txtWorkNo.Text = ""
        Call mTblClear(tblResult):  Call mTblClear(tblReady)
