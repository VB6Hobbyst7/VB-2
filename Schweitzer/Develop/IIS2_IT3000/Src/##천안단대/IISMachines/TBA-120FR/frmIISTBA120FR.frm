...
Begin VB.Form frmIISTBA120FR 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISTBA120FR.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISTBA120FR.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISTBA120FR.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISTBA120FR"
...
'   메  모  :
'       1. 테스트 해야함!
'       3. 결과수신시 추가정보에 대한 표시 프로그램 할것!
'          - 에러에 대한 설명이 사용자 메뉴얼에 있음.
'-----------------------------------------------------------------------------'
...
Private Sub EditRcvData()
    ...
    Dim strFunc      As String  '수신한 Function
    ...
    '## NOTE: 결과수신시 추가정보에 대한 표시 프로그램 할것!
'    Dim strFlag      As String  '수신한 Result Status
'    Dim strInfo      As String  '추가정보

    strRcvBuf = mIntLib.Buffers(1).Buffers
    strFunc = Trim$(Mid$(strRcvBuf, 1, 2))

    Select Case strFunc
        Case "Q"    '## Order Request Message
            '## ACK 전송
            strOutput = STX & ACK & ETX
            MSComm.Output = strOutput
            Call mIntLib.WriteLog(strOutput, ccPCLog)
        
            '## 바코드번호, Disk No, Position 조회
            strBarNo = Mid$(strRcvBuf, 7, 20)
            strDiskNo = Mid$(strRcvBuf, 27, 4)
            strPos = Mid$(strRcvBuf, 31, 3)
            ...
            With mOrder
                ...
                .Pos = strPos
            End With
            Call GetOrder(Format$(Trim$(strBarNo), "#"))
            mIntLib.State = "Q"
                        
        Case "R"    '## Result Message
            '## 바코드번호, Disk No, Position 조회
            strBarNo = Format$(Trim$(Mid$(strRcvBuf, 7, 20)), "#")
            strDiskNo = Trim$(Mid$(strRcvBuf, 27, 4))
            strPos = Trim$(Mid$(strRcvBuf, 31, 3))
            ...
            '## 장비기준 검사명, 검사결과 조회
            strTemp = Mid$(strRcvBuf, 49, InStr(strRcvBuf, ETB) - 49)
            For i = 1 To Len(strTemp) Step 15
                strIntBase = Trim$(Mid$(strRcvBuf, i, 4))
                strResult = Trim$(Mid$(strRcvBuf, i + 4, 6))
...
'   기능 : 오더전송후 NAK수신시 처리
'-----------------------------------------------------------------------------'
Private Sub ReSendOrder()
    Dim strOutput As String '송신할 데이터
    
    If mOrder.SendCnt < 4 Then
        strOutput = mOrder.Order
        MSComm.Output = strOutput
        Call mIntLib.WriteLog(strOutput, ccPCLog)
        mOrder.SendCnt = mOrder.SendCnt + 1
    Else
        '## NAK를 4회이상 반복수신하면 초기상태로
        mIntLib.Phase = 1
    End If
End Sub
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    With tblReady
        ...
        .Col = TReadyEnum.ccNo:     .Text = Trim$(mOrder.Pos) & "/" & Trim$(mOrder.DiskNo)
