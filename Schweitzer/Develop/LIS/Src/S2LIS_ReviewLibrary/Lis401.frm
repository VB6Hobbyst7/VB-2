...
Begin VB.Form frm401ResultView 
   ...
   Begin FPSpread.vaSpread tblOrdSheet 
      ...
      ShadowText      =   0
      SpreadDesigner  =   "Lis401.frx":0949
      TextTip         =   4
   End
   ...
   Begin VB.PictureBox picFootNote 
      ...
      Begin RichTextLib.RichTextBox txtSamCmt 
         ...
         ScrollBars      =   2
         Appearance      =   0
         TextRTF         =   $"Lis401.frx":66E7
         MouseIcon       =   "Lis401.frx":678C
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "돋움체"
            Size            =   9
            Charset         =   129
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      ShadowDark      =   12632256
      ShadowText      =   0
      SpreadDesigner  =   "Lis401.frx":6993
      UnitType        =   0
      UserResize      =   0
      VisibleCols     =   8
      VisibleRows     =   22
...
'        .Value = 1
    ...
    Else
        ...
        ElseIf optOrdDiv(3).Value = True Then   '미생물만
            strSelDiv = "7"
        ElseIf optOrdDiv(2).Value = True Then
            strSelDiv = "4"
        ElseIf optOrdDiv(1).Value = True Then
            strSelDiv = "8"
        ElseIf optOrdDiv(4).Value = True Then
            strSelDiv = "9"
        End If
    End If
        
    SqlStmt = objSQL.SqlQueryOrders(txtPtId.Text, strKeyFld, Format(dtpFromDate.Value, CS_DateDbFormat), _
    ...
    With tblOrdSheet
        ...
        Do Until tmpRs.EOF
            ...
            If SvDoctNm <> Trim("" & tmpRs.Fields("DoctNm").Value) Then
                ...
                SvDoctNm = Trim("" & tmpRs.Fields("DoctNm").Value)
            End If
             .Col = enREVIEW1.tcTESTNM:
             '.Value = Trim("" & tmpRs.Fields("TestNm").Value)        '검사명
             
            Select Case tmpRs.Fields("OrdCd").Value & ""
                Case "NCOVGCRL", "2019NCOV", "2019COV1", "2019COV2", "2019COVP", "COVGCRL1", "COVGCRL2", "NCOVPMC2"
                    .ForeColor = vbRed
                    .Value = Trim("" & tmpRs.Fields("TestNm").Value)        '검사명
                Case Else
                    .ForeColor = vbBlack
                    .Value = Trim("" & tmpRs.Fields("TestNm").Value)
            End Select
            ...
            If UCase(.Value) = "CREPCR15" Then
                .Col = 3:  .BackColor = vbGreen
            End If
...
Private Sub optOrdDiv_Click(Index As Integer)
    ...
    optOrdDiv(4).ForeColor = &H404040
    optOrdDiv(Index).ForeColor = DCM_LightRed
    ChkDivAll.Value = 0
    If optOrdDiv(2).Value = True Then optQueryKey(0).Value = True
    
'    If optOrdDiv(1).Value = True Then
'        dtpFromDate.Value = DateAdd("d", -365, dtpFromDate.Value)     '외래
'    End If
End Sub
...
'08.09.25 양성현 수납일시 추가
       ...
       .Col = enREVIEW1.tcTESTCD: strTestcd = .Value
        ...
        Else
            If Trim(strTestcd) = "B4054G" Then
            '검사항목별 미생물 장비명 조회
                Set RS1 = New Recordset
                
                strSQL = " select machcd from " & T_LAB404 & _
                         "  where workarea = " & DBS(strWorkArea) & _
                         "    and accdt = " & DBS(strAccDt) & _
                         "    and accseq = " & DBN(strAccSeq) & _
                         "    and testcd = 'B4055' "  '" & DBS(strTestcd)
                
                RS1.Open strSQL, DBConn
                
                If RS1.EOF = False Then
                    tmpToolTip = tmpToolTip & vbCrLf & "  미생물장비코드 : " & RS1.Fields("machcd").Value & ""
                End If
