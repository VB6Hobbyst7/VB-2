...
Begin VB.Form frm424SpcStatics 
   BackColor       =   &H00DBE6E6&
   ClientHeight    =   11100
   ClientLeft      =   60
   ClientTop       =   75
   ...
   Begin FPSpread.vaSpread tblCnt 
      ...
      ShadowDark      =   12632256
      ShadowText      =   0
      SpreadDesigner  =   "frm424.frx":0000
      StartingColNumber=   2
      VirtualRows     =   24
      VisibleCols     =   5
      VisibleRows     =   20
   ...
   Begin FPSpread.vaSpread tblexcel 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      SpreadDesigner  =   "frm424.frx":0A1C
   End
End
Attribute VB_Name = "frm424SpcStatics"
...
Private Function GetWorkAccount(ByVal FRcvDt As String, ByVal TRcvDt As String)
    ...
    SelectIDX = "0"
    SelectIDX = IIf(Selected_IDX = 0, "0,1,2,3,4,5,6,7", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 1, "1,2,3,4,5,6", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 2, "2,3,4,5,6", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 3, "3,4,5,6", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 4, "4,5,6", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 5, "5,6", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 6, "6", SelectIDX)
    SelectIDX = IIf(Selected_IDX = 7, "7", SelectIDX)
        
        
'    SQL = " SELECT a.rcvdt,a.workarea, Count(*) AS Cnt from " & T_LAB032 & " b, " & T_LAB201 & " a " & _
'          " where" & DBW("a.rcvdt>=", FRcvDt) & _
'          " and " & DBW("a.rcvdt<=", TRcvDt) & _
'          " and a.stscd >'1' and a.stscd<'7'" & _
'          " and " & DBW("b.cdindex=", LC3_WorkArea) & _
'          " and a.workarea=b.cdval1 "
'
'    SQL = SQL & "GROUP BY a.rcvdt,a.workarea"
'
'
