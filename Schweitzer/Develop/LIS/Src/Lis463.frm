...
Begin VB.Form frm463EmmaList 
   ...
   Begin FPSpread.vaSpread tblexcel 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      SpreadDesigner  =   "Lis463.frx":1BAB
   End
...
'            If strWorkArea <> "" Then
            strSQL = ""
            strSQL = " SELECT * FROM MDNOTIFT WHERE  notidate between '" & strSDate & "' and '" & strEDate & "' AND notitype = '7' and workarea = '" & strWorkArea & "' "
            ...
            Call DspSpd_New1("" & .Fields("TRANSDT").Value, "" & .Fields("REMARK").Value, strTransNm, "" & .Fields("DOCTID").Value & " / " & "" & .Fields("DEPTNM").Value & " (" & "" & .Fields("TELNO").Value & ")", _
                        "" & .Fields("TRANSMSG").Value, strTransFg, "" & .Fields("RCVSTAT").Value, strTATDt, "" & .Fields("DOCTID").Value, "" & .Fields("DOCTNM").Value, "" & .Fields("TELNO").Value, "" & .Fields("DEPTNM").Value, "", i)
        End With
      rsGetinfo.MoveNext
    Next i
