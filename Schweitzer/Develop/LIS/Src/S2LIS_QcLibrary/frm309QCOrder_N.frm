...
Begin VB.Form frm309QCOrder_N 
   ...
   Begin FPSpread.vaSpread tblTestList 
      ...
      SpreadDesigner  =   "frm309QCOrder_N.frx":0000
      VisibleCols     =   4
      VisibleRows     =   24
   End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdPopCtrl 
         ...
         Height          =   360
         Left            =   3915
         Picture         =   "frm309QCOrder_N.frx":3005
         Style           =   1  '그래픽
         TabIndex        =   3
         Top             =   165
         Width           =   330
   ...
   Begin FPSpread.vaSpread tblWorkList 
      ...
      MaxRows         =   3
      ScrollBars      =   0
      SpreadDesigner  =   "frm309QCOrder_N.frx":30B7
   End
   ...
   End
End
Attribute VB_Name = "frm309QCOrder_N"
...
Private Sub cmdDelete_Click()
    ...
    strSpcYY = lblSpcYY.Caption
    strSpcNo = lblSpcNo.Caption
    If Trim(strSpcYY) = "" Or strSpcNo = "" Then Exit Sub

    MousePointer = vbHourglass

'    If currBarStat > "3" Then cmdDelete.Enabled = False: Exit Sub

    strSql = " Select distinct b.workarea, b.accdt, b.accseq from " & T_LAB201 & " b  Where " & DBW("b.spcyy =", Trim(strSpcYY)) & " and " & DBW("b.spcno =", Val(Trim(strSpcNo)))
...
DBConn.BeginTrans
        strSql = " Delete " & T_LAB028 & " Where " & DBW("workarea =", Trim(strWorkArea)) & " and " & DBW("accdt =", Val(Trim(strAccDt))) & " and " & DBW("accseq =", Val(Trim(strAccSeq)))
        DBConn.Execute strSql
        
        strSql = " Delete " & T_LAB027 & " Where " & DBW("workarea =", Trim(strWorkArea)) & " and " & DBW("accdt =", Val(Trim(strAccDt))) & " and " & DBW("accseq =", Val(Trim(strAccSeq)))
        DBConn.Execute strSql

        strSql = " Delete " & T_LAB026 & " Where " & DBW("workarea =", Trim(strWorkArea)) & " and " & DBW("accdt =", Val(Trim(strAccDt))) & " and " & DBW("accseq =", Val(Trim(strAccSeq)))
        DBConn.Execute strSql

        strSql = " Delete " & T_LAB201 & " Where " & DBW("workarea =", Trim(strWorkArea)) & " and " & DBW("accdt =", Val(Trim(strAccDt))) & " and " & DBW("accseq =", Val(Trim(strAccSeq)))
        DBConn.Execute strSql
...
Private Function GetControlInfo_Wm(Optional ByVal pCtrlCd As String = "", _
                                Optional ByVal pLevelCd As String = "") As Recordset
    Dim strSql As String
    ...
    strSql = " select distinct a.ctrlcd,a.ctrlnm,a.levelcd,a.ctrldiv,a.eqpcd,c.eqpnm, a.buildcd,d.field1 as buildnm, " & _
            ...
            " and a.sectcd=e.cdval1 " & _
            " and " & DBW("f.cdindex=", lc3_workarea) & _
            " and a.workarea=f.cdval1 and a.ctrlcd = g.ctrlcd and  g.expdt >= to_char(sysdate,'YYYYMMDD') " & _
            " and a.workarea = '14' "
...
'    strSQL = strSQL & " order by a.ctrlcd,ctrlnm,levelcd"
    strSql = strSql & " and a.eqpcd NOT IN ('E067','E068','E084','E070','E071','E072','E073','E074','E083','E069') order by a.ctrlcd, eqpcd, levelcd,ctrlnm"
...
'                                    Format(GetSystemDate, CS_DateDbFormat))
    ...
    strSql = " select distinct a.workarea" & FUNC_CONCAT & "'-'" & FUNC_CONCAT & FUNC_SUBSTR & "( a.accdt,3)" & FUNC_CONCAT & "'-'" & FUNC_CONCAT & FUNC_CONVERT("CHAR", "a.accseq") & " as accno, " & _
            " b.spcyy" & FUNC_CONCAT & "'-'" & FUNC_CONCAT & FUNC_CONVERT("CHAR", "b.spcno") & " as barno,b.coldt ,b.coltm, b.stscd from " & T_LAB026 & " a," & T_LAB201 & " b " & _
            " where " & DBW("a.ctrlcd=", Trim(txtCtrlCd.Text)) & _
            " and " & DBW("a.levelcd=", IIf(optLevelCd(0).Value, "L", IIf(optLevelCd(1).Value, "N", "H"))) & _
    ...
    With objPop
        ...
        .ColumnHeaderText = "접수번호바코드번호채취일자채취시간상태"
        .ColumnHeaderWidth = "1214.9291319.8111124.7871110.0471110.047"
        .FormWidth = 5220
        .LoadPopUp
        ...
        currBarStat = medGetP(.SelectedString, 5, .Delimiter)   'Rs.Fields("stscd").Value & ""  '
    End With
    ...
    If tblTestList.DataRowCnt > 0 Then
        ...
        cmdSave.Enabled = False
    End If
'    If currBarStat > "3" Then cmdDelete.Enabled = False
Nodata:
    Set Rs = Nothing
    Set objPop = Nothing
    Set objRep = Nothing
