...
Begin VB.Form frm456SuscTrand 
   ...
   Begin VB.Frame Frame4 
      ...
      Begin FPSpread.vaSpread tblSus 
         ...
         ShadowDark      =   14737632
         SpreadDesigner  =   "Lis456.frx":089E
         UserResize      =   0
      End
   ...
   Begin VB.Frame Frame3 
      ...
      Begin FPSpread.vaSpread ssResult 
         ...
         ShadowColor     =   14737632
         ShadowDark      =   14737632
         SpreadDesigner  =   "Lis456.frx":1109
         UserResize      =   0
      End
...
Dim aMic(0 To 800) As String
Dim aAnti(0 To 200) As String
    ...
    Do Until dsRst.EOF
        ...
        If sRCnt > 0 Then
            For i = 1 To sRCnt
                ...
                If objDic.Exists(sMnmCD & COL_DIV & sSusCd) = False Then
                    objDic.AddNew sMnmCD & COL_DIV & sSusCd, _
                                  Join(Array("0", "0", "0", "0"), COL_DIV)
                End If
...
'                    .Col = 6: .Value = lngTotal & " 건": .FontBold = True
            ...
            .Col = 2: .Value = GetName("C221", objDic.Fields("suscd"), "S")
...
Private Function GetName(ByVal Cdindex As String, ByVal CdVal1 As String, ByVal Sus As String) As String
    ...
    If Not RS.EOF Then
        If Sus <> "" Then
            GetName = CdVal1 & Space(8 - Len(CdVal1)) & " " & RS.Fields("text1").Value & ""
        Else
            GetName = CdVal1 & Space(8 - Len(CdVal1)) & " " & RS.Fields("field1").Value & ""
        End If
    Else
        GetName = CdVal1
    End If
