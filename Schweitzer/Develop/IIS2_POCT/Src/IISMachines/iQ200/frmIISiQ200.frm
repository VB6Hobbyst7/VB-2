...
Begin VB.Form frmIISiQ200 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISiQ200.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISiQ200.frx":04E4
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISiQ200.frx":0D26
   End
...
Private Sub EditRcvData()
    ...
    Dim tmpRcvBuf   As String
    
    strRcvBuf = mIntLib.Buffers(1).Buffers
    strRcvBuf = Replace(strRcvBuf, STX, "")
    strRcvBuf = Replace(strRcvBuf, ETB, "")
    varRcvBuf = Split(strRcvBuf, vbCrLf)
    tmpRcvBuf = ""
    
    For i = 0 To UBound(varRcvBuf)
        tmpRcvBuf = tmpRcvBuf & Mid(varRcvBuf(i), 2, 240)
    Next
    
    '-- 유니코드 String(XML)변환
    strRcvBuf = UniToStr(tmpRcvBuf)
    
    '-- XML 로그기록
    Call mIntLib.WriteLog(strRcvBuf, ccEqp)
    
    '-- XML 변환오류(미해결 => "&lt;" 수동으로 변경)
    ...
    '-- 줄바꿈행으로 Split
    varRcvBuf = Split(strRcvBuf, vbCrLf)
    strRcvBuf = varRcvBuf(2)
    
    Pos = InStr(strRcvBuf, "ID=")
    If Pos > 0 Then
        If mIntLib.BarPos = ccPC Then
            strBarNo = CStr(Trim$(Mid$(strRcvBuf, Pos + 3)))
            strBarNo = Mid(strBarNo, 1, InStr(strBarNo, " ") - 1)
            If Len(strBarNo) > 11 Then
                strBarNo = Mid(strBarNo, 1, 11)
            ElseIf Len(strBarNo) < 11 Then
                'Rack에 붙어있는 바코드번호 8자리이며 끝에 2자리 사용
                'strWorkNo = Right(strBarNo, 2)
                'Rack에 붙어있는 바코드번호 8자리에서 9자리로 수정 (끝에 3자리 사용)
                strWorkNo = Right(strBarNo, 3)
            End If
            '## 바코드번호 조회
            With tblReady
                For i = 1 To .DataRowCnt
                    ...
                    If strWorkNo <> "" Then
                        If CLng(vWorkNo) = CLng(strWorkNo) Then
                            Call .GetText(TReadyEnum.ccBarNo, i, vBarNo)
                            strBarNo = CStr(vBarNo)
                            Exit For
                        End If
'                    Else
'                        Call .GetText(TReadyEnum.ccBarNo, i, strBarNo)
'                        strBarNo = CStr(vBarNo)
'                        Exit For
                    End If
                Next i
            End With
        Else
            strWorkNo = CStr(Trim$(Mid$(strRcvBuf, Pos + 4, 8)))
            strTemp1 = Trim$(Mid$(strRcvBuf, Pos + 11, 16))
            strBarNo = Format$(strTemp1, String$(SPCLEN, "#"))
        End If
    Else
        Exit Sub
    End If
    ...
    strRcvBuf = Replace(strRcvBuf, "</AR>", "")
    varRcvBuf = Split(strRcvBuf, "<AR Key=")
    
    For Each objIntNm In mIntLib.IntNms
        strIntBase = objIntNm.IntBase:  Pos = 0
        
        If strIntBase <> strTemp2 Then
            For i = 1 To UBound(varRcvBuf)
                Pos = InStr(varRcvBuf(i), strIntBase)
                If Pos > 0 Then
                    strResult = Trim$(Mid$(varRcvBuf(i), InStr(varRcvBuf(i), ">") + 1))
                    
                    '-- 조건12)경우의수 발생 "AR Key=" 끝에 Chemistry의 끝 값인</AC> 있음
                    '-- 따라서 strResult값이 Yellow</AR></AC> 이렇게 들어가는 현상 발생
                    If InStr(strResult, "<") > 0 Then
                        strResult = Trim$(Mid$(strResult, 1, InStr(strResult, "<") - 1))
                    End If
                    
                    '-- 검사항목이 (WBC or RBC or SQEP) 이면서 결과값이 "none"인 것은 "<1"로 치환
                    If strIntBase = "WBC" Or strIntBase = "RBC" Or strIntBase = "SQEP" Then
                        If InStr(LCase(strResult), "none") > 0 Then strResult = "<1"
                        '위의 조건12)때문에 빈칸으로 나올 수 있다.
                        If Trim(strResult) = "" Then strResult = "<1"
                    Else
                        If InStr(LCase(strResult), "none") > 0 Then strResult = ""
                    End If
                    
                    '-- 결과값이 null  인 것은 "<1"로 치환
                    'If Trim(strResult) = "" Then strResult = "<1"
                    
                    '-- 결과값이 "-\+" 인 것은 "-/+"로 치환  = "Trace"
                    'If Trim(strResult) = "-\+" Then strResult = "-/+"
                    
                    Call objIntInfo.IntResults.Add(strIntBase, objIntNm.IntNm, strResult, strResult)
                    strTemp2 = strIntBase
                    Exit For
                End If
            Next
