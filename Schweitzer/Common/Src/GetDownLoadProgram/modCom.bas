...
'Public Function AppExitRtn() As VbMsgBoxResult
'
'   AppExitRtn = MsgBox("Schweitzer-LIS 를 종료하시겠습니까?", vbYesNo + vbQuestion, "Main")
'   If AppExitRtn = vbNo Then Exit Function
'   medAbout.cmdOK.Visible = False
'   Call UnloadForms(medAbout)
'   medAbout.Show
'   medAbout.ZOrder 0
'   DoEvents
'   Call DbClose
...
'*-----------------------------------------------------------------
'Public Sub medMsg(ByVal MSG As String, Optional BeepOk)
'
'    medMain.stsBar.Panels(2) = MSG
'    '
...
Dim lRet As Long, lHandle As Long
    ...
    lHandle = FindWindowEx(objToolBar.hwnd, 0&, "ToolbarWindow32", vbNullString)
    lRet = SendMessage(lHandle, 1081&, 0&, 0&)
    lRet = lRet Or 2048&
