...
Global Const gNetDriveChar = "R:"
'Global Const gNetDrive = "\\Mikyung\medcom"
'Global Const gNetDrive = "\\jmk\ghil_lis"
'Global Const gcServerPath = "R:\ghil_lis\package\support\"
'Global Const gcServerPath = "R:\"
Global gsAppName As String  '= "Ghil_lis.exe"
...
Public Const RESOURCETYPE_IPC = &H3
...
'Public gNetDriveChar As String

Public Function AttachNetDir(strPass As String, strUser As String, strNetDir As String, strDrLetter As String) As String
    Dim errinfo As Long
    ...
    NetR.lpLocalName = strDrLetter
    NetR.lpRemoteName = strNetDir
    errinfo = WNetAddConnection2(NetR, strPass, strUser, CONNECT_UPDATE_PROFILE)
    If errinfo = NO_ERROR Then
'        MsgBox "네트워크 드라이브 연결이 성공적으로 이루어졌습니다!", vbInformation, "네트워크 드라이브 연결 성공"
        AttachNetDir = strDrLetter
    Else
        MsgBox "에러: " & errinfo & " - 네트워크 드라이브 연결이 실패하였습니다!", vbExclamation, "네트워크 드라이브 연결 실패"
        AttachNetDir = ""
    End If
End Function

Public Function FastFileCopy(strFrom As String, strTo As String) As Boolean
    Dim lRet As Long
    FastFileCopy = False
    lRet = CopyFile(strFrom, strTo, False)
    FastFileCopy = (lRet <> 0)
End Function

Public Function ReleaseNetDir(strDrLetter As String) As Boolean
    Dim errinfo As Long
    errinfo = WNetCancelConnection2(strDrLetter, CONNECT_UPDATE_PROFILE, True)
    ReleaseNetDir = (errinfo = 0)
End Function
...
Public Function NetConnect(Index As Integer, Optional ByVal NetPath As String) As Boolean

    Dim X As Long
    
    NetConnect = True
    If Index = 0 Then
        X = WNetAddConnection(NetPath, "", gNetDriveChar)
        If X <> 0 Then NetConnect = False
    Else
        X = WNetCancelConnection(gNetDriveChar, 1)
        If X <> 0 Then NetConnect = False
    End If
