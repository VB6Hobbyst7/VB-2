...
Public Function Get_WorkList() As String
    ...
    Call SetSQLData("¿öÅ©¸®½ºÆ®ÀÎ¼ö", sParam)
...
ErrHandle:
    ...
    If oSOAP.FaultString <> "" Then
        ...
        Call SetSQLData("¿öÅ©¸®½ºÆ®¿¡·¯", "Get_WorkList:" & Format(Time, "hh:nn:ss") & "[SOAP]" & oSOAP.FaultString & vbCrLf & oSOAP.Detail & vbCrLf)
    End If
    
    If Trim(Err.Description) <> "" Then
        Debug.Print Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf
        'SetRawData "Get_WorkList:" & Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf
        Call SetSQLData("¿öÅ©¸®½ºÆ®¿¡·¯", "Get_WorkList:" & Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf)
    End If
