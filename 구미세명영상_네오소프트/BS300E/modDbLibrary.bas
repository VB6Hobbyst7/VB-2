...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                If strGubun = "진료" Then
                          SQL = "Select RESA_SEQ, RESA_TIME "
                    SQL = SQL & "  From Month..RESA" & strYM & vbCr
                    SQL = SQL & " Where RESA_CHAM_ID = '" & lsID & "'" & vbCr
                    'SQL = SQL & "   And RESA_GWAM_ID = '01' "       '-- 내과
                    SQL = SQL & "   And RESA_KIND = 0 " & vbCr
                    SQL = SQL & "   And RESA_DATE = '" & strDay & "'" & vbCr
                    'SQL = SQL & "   And RESA_SLIP_ID = '" & lsID & "'" & vbCr
                    SQL = SQL & "   And RESA_LABM_ID = '" & strEqpCd & "'" & vbCr
                    'SQL = SQL & "   And (RESA_RESULTS is null or RESA_RESULTS = '') "
                    ...
                    If Not RS.EOF = True And Not RS.BOF = True Then
                        strResaSeq = RS.Fields("RESA_SEQ").Value & ""
                        strResaTime = RS.Fields("RESA_TIME").Value & ""
                        
                        '-- Update
                              SQL = "Update Month..RESA" & strYM & " Set " & vbCr
                        SQL = SQL & " RESA_RESULTS = '" & sResult & "'" & vbCr
                        'SQL = SQL & ",RESA_SLIP_ID = '" & sResult & "'" & vbCr
                        SQL = SQL & " ,RESA_GWAM_ID = '16'" & vbCr
                        
                        SQL = SQL & " Where RESA_CHAM_ID = '" & lsID & "'" & vbCr
                        'SQL = SQL & "   And RESA_GWAM_ID = '01'" & vbCr
                        SQL = SQL & "   And RESA_KIND = 0" & vbCr
                        SQL = SQL & "   And RESA_DATE = '" & strDay & "'" & vbCr
                        'SQL = SQL & "   And RESA_SLIP_ID = '" & lsID & "'" & vbCr
                        SQL = SQL & "   And RESA_LABM_ID = '" & strEqpCd & "'" & vbCr
                        SQL = SQL & "   And RESA_SEQ     = '" & strResaSeq & "'" & vbCr
                        SQL = SQL & "   And RESA_TIME    = '" & strResaTime & "'" & vbCr
                    ...
                    Else
                        '-- Insert
                              SQL = "Insert Into Month..RESA" & strYM & " ( RESA_CHAM_ID " & vbCr
                        SQL = SQL & " , RESA_GWAM_ID         " & vbCr
                        SQL = SQL & " , RESA_KIND            " & vbCr
                        SQL = SQL & " , RESA_DATE            " & vbCr
                        SQL = SQL & " , RESA_SLIP_ID         " & vbCr
                        SQL = SQL & " , RESA_SEQ             " & vbCr
                        SQL = SQL & " , RESA_TIME            " & vbCr
                        SQL = SQL & " , RESA_GUBUN           " & vbCr
                        SQL = SQL & " , RESA_RNO             " & vbCr
                        SQL = SQL & " , RESA_LABM_ID         " & vbCr
                        SQL = SQL & " , RESA_AMT             " & vbCr
                        SQL = SQL & " , RESA_DAY             " & vbCr
                        SQL = SQL & " , RESA_BIGUB           " & vbCr
                        SQL = SQL & " , RESA_RESULTN         " & vbCr
                        SQL = SQL & " , RESA_RESULTS         " & vbCr
                        SQL = SQL & " , RESA_USGM_ID         " & vbCr
                        SQL = SQL & " , RESA_RESULT_FLAG     " & vbCr
                        SQL = SQL & " , RESA_FLAG            " & vbCr
                        SQL = SQL & " , RESA_MEMO            " & vbCr
                        SQL = SQL & " , RESA_BIGO1           " & vbCr
                        SQL = SQL & " , RESA_BIGO2           " & vbCr
                        SQL = SQL & " , RESA_BIGO3 )         " & vbCr
                        SQL = SQL & " Values " & vbCr
                        SQL = SQL & " (  '" & lsID & "'" & vbCr
                        SQL = SQL & "  , '16' " & vbCr
                        SQL = SQL & "  , 0 " & vbCr
    ...
    ''                Parameters.ParamByName('RESA_BIGO3').Value:= '';
                    ...
                    'RS.Close
                    'Set RS = Nothing
                ElseIf strGubun = "종검" Then
                    If strEqpCd = "1101" Or strEqpCd = "1102" Then
                              SQL = "UPDATE HIS_CHART03 SET "
                        SQL = SQL & "C" & strEqpCd & " = '" & sResult & "'"
                        SQL = SQL & " WHERE CDATE = '" & Format(strRegDate, "yyyy.mm.dd") & "'"
                        SQL = SQL & "   AND CPNO = '" & lsID & "'"
                    Else
                              SQL = "UPDATE HIS_CHART02 SET "
                        SQL = SQL & "C" & strEqpCd & " = '" & sResult & "'"
                        SQL = SQL & " WHERE CDATE = '" & Format(strRegDate, "yyyy.mm.dd") & "'"
                        SQL = SQL & "   AND CPNO = '" & lsID & "'"
                    End If
...
Function GetSampleInfoW_NEOSOFT(ByVal asRow As Long) As Integer
    ...
    End If
          ...
          SQL = "SELECT distinct B.OENT_MOMM_ID AS ITEM     " & vbCrLf
    SQL = SQL & "  FROM Month..SPRT" & strYM & " A          " & vbCrLf
    SQL = SQL & "     , Month..OENT" & strYM & " B          " & vbCrLf
    SQL = SQL & "     , Neosoft..CC_CHAM P                  " & vbCrLf
    SQL = SQL & " WHERE A.SPRT_CHAM_ID  = '" & sBarcode & "'" & vbCrLf
    SQL = SQL & "   AND B.OENT_MOMM_ID IN (" & gAllExam & ")" & vbCrLf
    SQL = SQL & "   AND A.SPRT_DATE    = B.OENT_DATE        " & vbCrLf
    SQL = SQL & "   AND A.SPRT_CHAM_ID = B.OENT_CHAM_ID     " & vbCrLf
    SQL = SQL & "   AND A.SPRT_CHAM_ID = P.CHAM_ID          " & vbCrLf
