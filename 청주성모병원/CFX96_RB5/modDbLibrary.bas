...
'''' values ('''+TGlobal.FICode+''', sysdate, '''+TMaster.FBarCode+''', '''+TMaster.FResult+''', '''+TMaster.FIfCode+''', 1) ';
                ...
                SQL = ""
                SQL = SQL & " Update tl_workvalue " & vbCr
                SQL = SQL & "   Set rvalue = '" & sResult & "'," & vbCr
                SQL = SQL & "       autocode = '" & gEquipCode & "'" & vbCr
                SQL = SQL & " Where SAMPLE = '" & lsID & "'" & vbCr
                SQL = SQL & "   And RTNAME = '" & strEqpCd & "'" & vbCr
                SQL = SQL & "   And RTRIM(LTRIM(WORKNAME)) = '" & strChannel & "'"
            
                SetRawData "[Update]" & SQL
                ...
                SQL = ""
                SQL = SQL & " Insert Into TL_ACSDATA (AUTOCODE, LABDATE, SERIAL, RVALUE, SOKET, EVALUECHK) " & vbCr
                SQL = SQL & "  values ('" & gEquipCode & "',sysdate, '" & lsID & "', '" & sResult & "', '" & strChannel & "','1')"
