...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" And strSeqS <> "" Then
                 ...
                 SQL = ""
                 SQL = SQL & "Insert Into TC206 "
                 SQL = SQL & " (SpcNo, EquipCd, TestCd, TransDt, TransTm, RstVal, SpcDiv) "
                 SQL = SQL & " values "
                 SQL = SQL & " ('" & lsID & "'" & vbCr
                 SQL = SQL & " , '" & gEquipCode & "'" & vbCr
                 SQL = SQL & " , '" & strEqpCd & "'" & vbCr
                 SQL = SQL & " , '" & strDate & "'" & vbCr
                 SQL = SQL & " , '" & strTime & "'" & vbCr
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_HWASAN(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
...
'''    SQL = SQL & "   And a.OKFL <> 'Y' "   '-- 결과확정유무
    ...
    ' O.AllTestNm   모든 검사명
    SQL = ""
    SQL = SQL & "SELECT O.OrdDt as 접수일자, O.PtID as 내원번호, O.PtNm as 이름, O.Sex as 성별, O.Age as 나이, O.SpcCd, O.SpcNm, O.AllTestNm, O.StatFg, O.SPCNO as 바코드번호, T.TESTCD as ITEM" & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                SetText .vasID, Trim(RS.Fields("접수일자")) & "", asRow, colHOSPDATE
                'SetText .vasID, Format(Now, "yyyy-mm-dd"), asRow, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("바코드번호")) & "", asRow, colBARCODE
                SetText .vasID, Trim(RS.Fields("내원번호")) & "", asRow, colPID
                SetText .vasID, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                SetText .vasID, Trim(RS.Fields("성별")) & "", asRow, colPSEX
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_UJIN(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
...
'    SQL = SQL & "   And T.TESTCD in (" & gAllExam & ")" & vbCr
    ...
    SQL = SQL & "  FROM polly.GUMSA013, polly.GUMSA011, polly.GUMSA015 " & vbCr
    ...
    SQL = SQL & "   AND GUMSA013.G13_JINKUL <= '5' " & vbCr
    SQL = SQL & "   AND GUMSA013.G13_SAMPLE = GUMSA015.G15_SAMPLE"
