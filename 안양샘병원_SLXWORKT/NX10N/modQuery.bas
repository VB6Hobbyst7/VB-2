...
Function SaveTransData_SAM(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
    ...
    With frmInterface
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                strTemp = GetTestSeq_GSAM(strBarcode, strTestCd)
                strSeqNo = mGetP(strTemp, 1, "|")
...
'''                    AdoCn.BeginTrans
                    ...
                    iRow = iRow + 1
'''                    If iRow = 1 Then
'''                              SQL = "Update SLSPCMDT Set procstat = 'E'" & vbCrLf
'''                        SQL = SQL & " WHERE spcid    = '" & strBarcode & "'" & vbCrLf
'''                        SQL = SQL & "   AND procstat IN ('A','B','C')" & vbCrLf
'''                        Call SetSQLData("결과저장", SQL, "A")
'''                        AdoCn.Execute SQL
'''
'''                              SQL = "Update SLSPEXDT Set procstat = 'E'" & vbCrLf
'''                        SQL = SQL & " where spcid    = '" & strBarcode & "'" & vbCrLf
'''                        SQL = SQL & "   AND procstat IN ('A','B','C')" & vbCrLf
'''                        Call SetSQLData("결과저장", SQL, "A")
'''                        AdoCn.Execute SQL
'''
'''                              SQL = "Update SLXWORKT Set procstat = 'E'" & vbCrLf
'''                        SQL = SQL & " ,repttech = '" & gHOSP.USERID & "'" & vbCrLf  '검사자ID
'''                        SQL = SQL & " where spcid    = '" & strBarcode & "'" & vbCrLf
'''                        SQL = SQL & "   AND procstat IN ('A','B','C')" & vbCrLf
'''                        Call SetSQLData("결과저장", SQL, "A")
'''                        AdoCn.Execute SQL
'''
'''                              SQL = "Update MDEXMORT Set procstat = 'E'" & vbCrLf
'''                        SQL = SQL & " where spcid1 = '" & strBarcode & "'" & vbCrLf
'''                        SQL = SQL & "   AND procstat IN ('A','B','C')" & vbCrLf
'''                        Call SetSQLData("결과저장", SQL, "A")
'''                        AdoCn.Execute SQL
'''                    End If
                    ...
                    With AdoCmd
                        .CommandTimeout = 15
                        .CommandText = "PR_SL_RESLTEXT_INTERFACE_PROC"
                        .CommandType = adCmdStoredProc
                        
                        Set Prm1 = .CreateParameter("IN_SPCID", adVarChar, adParamInput, 4000, strBarcode)
                        .Parameters.Append Prm1
                        Set Prm2 = .CreateParameter("IN_EXAMCODE", adVarChar, adParamInput, 4000, strTestCd)
                        .Parameters.Append Prm2
                        Set Prm3 = .CreateParameter("IN_RSLT", adVarChar, adParamInput, 4000, sResult)
                        .Parameters.Append Prm3
                        Set prm4 = .CreateParameter("IN_ROOT", adVarChar, adParamInput, 4000, sResult)
                        .Parameters.Append prm4
                        Set prm5 = .CreateParameter("IN_LABRMK", adVarChar, adParamInput, 4000, "")
                        .Parameters.Append prm5
                        Set prm6 = .CreateParameter("IN_EQIPCODE", adVarChar, adParamInput, 4000, gHOSP.MACHCD)
                        .Parameters.Append prm6
                        Set prm7 = .CreateParameter("IN_EDITID", adVarChar, adParamInput, 4000, gHOSP.USERID)
                        .Parameters.Append prm7
                        Set prm8 = .CreateParameter("IN_EDITIP", adVarChar, adParamInput, 4000, CStr(frmInterface.wSck.LocalIP))
                        .Parameters.Append prm8
                        Set prm9 = .CreateParameter("OUT_ERR", adVarChar, adParamOutput, 4000, strErr)
                        .Parameters.Append prm9
                        
                        .Execute
                        Set AdoCmd = Nothing
                        
                        'strRet = .Parameters("OUT_ERR").Value
                        
                        
                        SQL = ""
                        SQL = SQL & strBarcode & "," & vbCrLf
...
'''                        If strSeqNo <> "" And strTemp <> "" And mGetP(strTemp, 7, "|") <> "" Then
'''                                  SQL = "Insert Into MDRESULT (PATNO,ORDDATE,ORDSEQNO,EXAMCODE,SLIPCD,RSLT1,RSLTUPP,RSLTLOW,EXECDATE,"
'''                            SQL = SQL & "                      RSLTDATE,REPTDR,READDR,CHNGDATE,RSLTTYPE,EDITID,EDITIP,EDITDATE)"
'''                            SQL = SQL & " Values ("
'''                            SQL = SQL & "'" & strPatID & "',"
'''                            SQL = SQL & "'" & mGetP(strTemp, 7, "|") & "',"
'''                            SQL = SQL & strSeqNo & ","
'''                            SQL = SQL & "'" & strTestCd & "',"
'''                            'SQL = SQL & "'" & mGetP(strTemp, 4, "|") & "',"
'''                            SQL = SQL & "'" & gHOSP.PARTCD & "',"           '안양샘병원 L24
'''                            SQL = SQL & "'" & sResult & "',"
'''                            SQL = SQL & "'" & mGetP(strTemp, 3, "|") & "',"
'''                            SQL = SQL & "'" & mGetP(strTemp, 2, "|") & "',"
'''                            SQL = SQL & "'" & mGetP(strTemp, 5, "|") & "',"
'''
'''                            SQL = SQL & "sysdate,"
'''                            SQL = SQL & "'" & gHOSP.USERID & "',"
'''                            SQL = SQL & "'" & gHOSP.USERID & "',"
'''                            SQL = SQL & "sysdate,"
'''                            SQL = SQL & "'" & mGetP(strTemp, 6, "|") & "',"
'''                            SQL = SQL & "'" & gHOSP.USERID & "',"
'''                            SQL = SQL & "'" & CStr(frmInterface.wSck.LocalIP) & "',"
'''                            SQL = SQL & "sysdate)"
'''                            Call SetSQLData("결과저장", SQL, "A")
'''                            AdoCn.Execute SQL
'''                        End If
...
'''              SQL = "Update SLSPCMDT Set procstat = 'N'" & vbCrLf
'''        SQL = SQL & " WHERE spcid    = '" & strBarcode & "'" & vbCrLf
'''        SQL = SQL & "   AND procstat = 'E' "
'''        Call SetSQLData("결과저장", SQL, "A")
'''        AdoCn.Execute SQL
'''
'''              SQL = "Update SLSPEXDT Set   procstat = 'N'" & vbCrLf
'''        SQL = SQL & " where spcid    = '" & strBarcode & "'" & vbCrLf
'''        SQL = SQL & "   AND procstat = 'E' "
'''        Call SetSQLData("결과저장", SQL, "A")
'''        AdoCn.Execute SQL
'''
'''              SQL = "Update SLXWORKT Set " & vbCrLf
'''        SQL = SQL & "   procstat     = 'N' " & vbCrLf
'''        SQL = SQL & " , reptdate     = sysdate " & vbCrLf
'''        SQL = SQL & " , repttech     = '" & gHOSP.USERID & "'" & vbCrLf  '검사자ID
'''        SQL = SQL & " where spcid    = '" & strBarcode & "'" & vbCrLf
'''        SQL = SQL & "   AND procstat = 'E' "
'''        Call SetSQLData("결과저장", SQL, "A")
'''        AdoCn.Execute SQL
'''
'''              SQL = "Update MDEXMORT Set " & vbCrLf
'''        SQL = SQL & "   procstat = 'N'" & vbCrLf
'''        SQL = SQL & " , rsltdate = sysdate " & vbCrLf
'''        SQL = SQL & " where spcid1   = '" & strBarcode & "'" & vbCrLf
'''        SQL = SQL & "   AND procstat = 'E' "
'''        Call SetSQLData("결과저장", SQL, "A")
'''        AdoCn.Execute SQL
...
'''    AdoCn.RollbackTrans
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_SAM" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
'-- 검사자 정보 가져오기
Function GetTestSeq_GSAM(ByVal asBarcode As String, ByVal asTestcode As String) As String
    
    GetTestSeq_GSAM = ""
  
          SQL = "SELECT ORDSEQNO,NORLOW,NORHIGH,WORKCODE,EXECDATE,RSLTTYPE,ORDDATE  " & vbCrLf
    SQL = SQL & " FROM SLXWORKT                                                     " & vbCrLf
    SQL = SQL & "WHERE SPCID    = '" & asBarcode & "'                               " & vbCrLf
    'SQL = SQL & "  AND PROCSTAT = 'E'                                               " & vbCrLf
    SQL = SQL & "  AND EXAMCODE = '" & asTestcode & "'                              " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            GetTestSeq_GSAM = Trim(RS.Fields("ORDSEQNO")) & "|" & Trim(RS.Fields("NORLOW")) & "|" & Trim(RS.Fields("NORHIGH")) & "|" & Trim(RS.Fields("WORKCODE")) & "|" & Mid(Trim(RS.Fields("EXECDATE")), 1, 10) & "|" & Mid(Trim(RS.Fields("RSLTTYPE")), 1, 10) & "|" & Trim(RS.Fields("ORDDATE"))
            RS.MoveNext
        Loop
    End If
    RS.Close
...
'
''-- 워크리스트 조회
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pFromNo As String = "", Optional ByVal pToNo As String = "")

    Select Case gEMR
        Case "SAM"
                Call GetWorkList_SAM(pFrom, pTo, SPD)
...
Public Sub GetWorkList_EASYS(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
    Dim strHospDate As String
    Dim strBarcode  As String
...
Public Sub GetWorkList_SAM(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
    Dim strHospDate As String
    Dim strBarcode  As String
...
'''    Select Case Mid(strBarcode, 1, 1)
'''        Case "1", "2", "3", "4", "5" '안양
'''            strTblName = "ORAA1.APPATBAT P"
'''        Case Else
'''            '지샘
'''            strTblName = "ORAA1.APPATBAT @goraa1 P"
'''    End Select
    
    strTblName = "ORAA1.APPATBAT P"
    
    SQL = ""
    SQL = SQL & "SELECT W.SPCID                         AS BARCODE  " & vbCrLf
    SQL = SQL & "     , to_char(W.ORDDATE, 'yyyymmdd')  AS HOSPDATE " & vbCrLf
    SQL = SQL & "     , W.ORDSEQNO                                  " & vbCrLf
    SQL = SQL & "     , W.EXAMCODE                                  " & vbCrLf
    SQL = SQL & "     , to_char(W.WORKNO)                           " & vbCrLf
    SQL = SQL & "     , to_char(W.AREANO)                           " & vbCrLf
    SQL = SQL & "     , W.RSLTTEXT                                  " & vbCrLf
    SQL = SQL & "     , S.SPCCODE                                   " & vbCrLf
    SQL = SQL & "     , W.MEDDEPT                                   " & vbCrLf
    SQL = SQL & "     , S.PATNO                         AS PID      " & vbCrLf
    SQL = SQL & "     , P.PATNAME                       AS PNAME    " & vbCrLf
    SQL = SQL & "     , P.SEX                           AS SEX      " & vbCrLf
    SQL = SQL & "     , to_char(P.BIRTdate, 'yyyymmdd') AS AGE      " & vbCrLf
    SQL = SQL & "     , W.EXECID                                    " & vbCrLf '접수자 ID
    SQL = SQL & "     , W.EXAMCODE                      AS ITEM     " & vbCrLf '접수자 ID
    SQL = SQL & " FROM SLXWORKT W,                                  " & vbCrLf
    SQL = SQL & "      SLSPCMDT S,                                  " & vbCrLf
    SQL = SQL & strTblName & vbCrLf                            '샘병원 DB링크
    SQL = SQL & " WHERE W.ORDDATE BETWEEN '" & pFrom & "' and '" & pTo & "'    " & vbCrLf
    SQL = SQL & "   AND W.EXAMCODE IN (" & gAllTestCd & ")                    " & vbCrLf
    'SQL = SQL & "WHERE W.SPCID    = '" & sBarcode & "'            " & vbCrLf
    'If frmInterface.chkSave.Value <> "1" Then
        SQL = SQL & "  AND W.PROCSTAT = 'E'                       " & vbCrLf
    'End If
    SQL = SQL & "  AND S.SPCID    = W.SPCID                       " & vbCrLf
    SQL = SQL & "  AND W.PATNO    = P.PATNO                       " & vbCrLf
    SQL = SQL & "ORDER BY W.WORKNO, W.SPCID, W.EXAMCODE           " & vbCrLf
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_SAM" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "SAM"
            SaveTransData = SaveTransData_SAM(argSpcRow, SPD)
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    If cn_Server_Flag = True Then
        Select Case gEMR
            Case "SAM"
                    Call GetSampleInfo_SAM(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_SAM(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = SQL & "      SLSPCMDT S,                                  " & vbCrLf
    SQL = SQL & strTblName & vbCrLf                            '샘병원 DB링크
    SQL = SQL & "WHERE W.SPCID    = '" & strBarcode & "'            " & vbCrLf
    'If frmInterface.chkSave.Value <> "1" Then
        SQL = SQL & "  AND W.PROCSTAT = 'E'                         " & vbCrLf
    'End If
    SQL = SQL & "  AND S.SPCID    = W.SPCID                         " & vbCrLf
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_SAM" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
