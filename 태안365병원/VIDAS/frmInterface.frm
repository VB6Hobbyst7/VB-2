   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":9465
         End
...
Private Sub cmdSearch_Click()
    ...
    Call GetWorkList(Format(dtpStartDt.Value, "yyyy-mm-dd"), Format(dtpStopDt.Value, "yyyy-mm-dd"))
...
'''
'''    SQL = ""
'''    SQL = SQL & " SELECT Distinct R.RESACPNUM,R.RESACPDTE, R.RESMZHTYP, R.RESLABCOD ,SUBSTRING(R.RESACPDTE,1,8) AS 접수일시, R.RESCHTNUM AS 차트번호, P.PBSPATNAM AS 환자명, P.PBSRESNUM AS 주민, R.RESSPMNUM AS 바코드" & vbLf
'''    SQL = SQL & "                ,R.RESACPDTM AS 접수일자, R.RESSHTNAM AS 검사명 "
...
'''    SQL = SQL & " ORDER BY R.RESACPDTE,R.RESACPNUM, R.RESMZHTYP ASC, R.RESLABCOD ASC"
    ...
    SQL = ""
    SQL = SQL & "SELECT a.접수일자,a.검체번호,a.챠트번호,b.환자이름,b.환자성별,a.처방코드,a.서브코드,b.생년월일,b.환자성별 " & vbCrLf
    SQL = SQL & "  FROM SEE_TREATEXAM a,SEE_CHART b" & vbCrLf
    SQL = SQL & " WHERE a.접수일자 Between '" & pFrDt & "' AND '" & pToDt & "'" & vbCrLf
    SQL = SQL & "   AND a.챠트번호 = b.챠트번호" & vbCrLf
    SQL = SQL & "   AND a.처방코드 IN (" & gAllExam & ")" & vbCrLf
    'SQL = SQL & "   AND a.진료처방ID > 0 " & vbCrLf
    If chkSaveAll.Value = "1" Then
    '    SQL = SQL & "   AND a.연동구분 IN ('1','2') " & vbCrLf
    Else
    '    SQL = SQL & "   AND a.연동구분 IN ('1') " & vbCrLf
    End If
    SQL = SQL & " ORDER BY a.접수일자,a.챠트번호"
        ...
        With vasID
            ...
            If blnSame = False Then
                If strDTM <> "" Then
                    ...
                    SetText vasID, strTests, .MaxRows, colState + 1
                End If
                .MaxRows = .MaxRows + 1
            
                SetText vasID, "1", .MaxRows, colCheckBox
                SetText vasID, Trim(RS.Fields("챠트번호")) & "", .MaxRows, colPID
                SetText vasID, Trim(RS.Fields("검체번호")) & "", .MaxRows, colBarcode
                SetText vasID, Trim(RS.Fields("환자이름")) & "", .MaxRows, colPName
                SetText vasID, Trim(RS.Fields("환자성별")) & "", .MaxRows, colSex
                SetText vasID, Trim(RS.Fields("생년월일")) & "", .MaxRows, colAge
            ...
            'strDTM = Trim(rs.Fields("접수일자")) & ""
            'strDTM = "접수시간 : " & Format(strDTM, "####년 ##월 ##일 ##시 ##분")
            
            strTests = strTests & Trim(RS.Fields("처방코드")) & "/"
...
'    varTests = Split(strTests, "/")
'
'    SetText vasID, UBound(varTests) + 1, vasID.MaxRows, colTCNT
'    SetText vasID, "0", vasID.MaxRows, colSCNT
'    SetText vasID, "0", vasID.MaxRows, colRCNT
'
'    For i = 0 To UBound(varTests)
'        If colINTB + i > vasID.MaxCols Then
'            vasID.MaxCols = colINTB + i
'        End If
'        vasID.ColWidth(vasID.MaxCols) = 6
'        vasID.TypeHAlign = TypeHAlignCenter
'        vasID.TypeVAlign = TypeVAlignCenter
'
'        SetText vasID, varTests(i), vasID.MaxRows, colINTB + i
'    Next
...
Private Sub cmdSend_Click()
    ...
    Dim vPtId   As Variant  'Spread의 환자Id
    Dim i       As Long
    ...
    With vasID
        For i = 1 To .DataRowCnt
            Call .GetText(colCheckBox, i, vOrdChk)
            
            If Trim$(vOrdChk) = "1" Or Trim$(vOrdChk) = "전송중" Then
                Call .GetText(colBarcode, i, vBarNo)
                Call .GetText(colPID, i, vPtId)
                ...
                comEqp.Output = ENQ
                'SetRawData "[cmdSend_Click Tx]" & ENQ
...
Private Sub SendOrder()
    ...
    Dim strOutput   As String   '전송할 데이터
    Dim lngTotCnt   As Long     '전송할 총 개수
    Dim lngSndCnt   As Long     '전송한 개수
    
    With vasID
        .Row = mOrder.Row
        .Col = colTCNT:                  lngTotCnt = CLng(.Text)
        .Col = colSCNT:                  lngSndCnt = CLng(.Text)
        .Col = colINTB + lngSndCnt:      strIntBase = CStr(.Text)
        
        strOutput = GetOrderX(strIntBase)
        ...
        .Col = colSCNT: .Text = CStr(lngSndCnt)
        
        If lngTotCnt = lngSndCnt Then
            .Col = colState: .Text = "전송"
            ...
            End If
        Else
            .Col = colState: .Text = "전송중"
            intSndPhase = 1
        End If
        
        'NOTE: 오더전송시 이상이 있으면 DoEvents를 삭제할것!
        DoEvents
    End With
...
Public Function GetOrderX(ByVal pIntBase As String) As String
    ...
    strOrder = Chr(30) & "mtmpr|pi" & mOrder.PtId & "||si|ci" & mOrder.BarNo & "|" & "rt" & pIntBase & "|" & Chr(29)
...
'                                    strState = ""
'
'                                    '## intSndPhase = 1 : 전송할 오더가 있음
'                                    '## intSndPhase = -1: 전송할 검체 없음
...
Private Sub EditRcvData()
    ...
    Dim i, j As Integer
    Dim strIntBaseC  As String   '수신한 장비기준 검사명
    Dim strIntBaseN  As String   '수신한 장비기준 검사명
    
    strRcvBuf = strBuffer 'strRecvData(1)
...
'                'strIntBaseC = strIntBase & "C"
            
            Case "qn"   '## 정량결과
                'strIntBaseN = strIntBase & "N"
                strResult = Trim$(UCase$(Mid$(aryTemp(i), 3, 1)))
            
                If strResult <> "" Then
                    strState = "R"
                    If Mid$(aryTemp(i), 3, 1) = "<" Or Mid$(aryTemp(i), 3, 1) = ">" Then
                        strResult = Mid$(aryTemp(i), 3, 1) & Trim$(mGetP(aryTemp(i), 2, Space(1)))
                    Else
                        strResult = Mid$(mGetP(aryTemp(i), 1, Space(1)), 3)
                    End If
