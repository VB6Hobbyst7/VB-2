...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
        ...
        Case "HANSEN"                       '한센
                Call GetWorkList_HANSEN(pFrom, pTo, SPD)
...
Public Sub GetWorkList_HANSEN(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo RST
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       Hcode" & vbCr ':       병록번호"
    SQL = SQL & "     , PtName" & vbCr ':      성명"
    SQL = SQL & "     , BirthSex" & vbCr '   : 생일-성별(1,3,5,7:남 / 2,4,6,8:여)"
    SQL = SQL & "     , DeptCode" & vbCr ':    과코드"
    SQL = SQL & "     , DeptKname" & vbCr ':   진료과명"
    SQL = SQL & "     , WardNum" & vbCr ':     병동"
    SQL = SQL & "     , Room" & vbCr ':        병실"
    SQL = SQL & "     , DrSign" & vbCr '     : 처방의 사인                ================ 수정"
    SQL = SQL & "     , KName" & vbCr ':       처방의명"
    SQL = SQL & "     , Coda" & vbCr ':        구분코드 (내부코드)"
    SQL = SQL & "     , Bcode" & vbCr ':       청구코드"
    SQL = SQL & "     , BName" & vbCr ':       청구명칭"
    SQL = SQL & "     , FDate" & vbCr ':       처방일"
    SQL = SQL & "     , FTime" & vbCr ':       처방시간"
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        ...
        Case "HANSEN"
                Call GetSampleInfo_HANSEN(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_HANSEN(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
