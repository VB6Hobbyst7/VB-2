...
''''결과 LABADRKEY
'''select * from S_LabInf
''' where LabBarNum ='1700000113'
...
'''select *
'''From S_PcpInf
'''Where pcpacpkey = 162026

'''select a.labadrkey, b.adracpkey
'''from S_LabInf a, S_AdrInf b
''' where a.LabBarNum IN ('1700000113')
'''  and  a.labadrkey = b.adrkey
                
                    '-- 결과업데이트
                    '                                         결과상태    결과상태     최초 중간보고자    최초 중간보고일시     결과값
                    '[중간 보고]일 경우 Update 해야 할 컬럼들 PcpStaTyp,  AdrStaTyp,   LabMidUid,         LabMidDtf,            LabRst
                    
                    '                                         결과상태    결과상태     최종승인ID    최종승인일시    최초최종보고자    최초최종보고일시     결과값
                    '[최종 보고]일 경우 Update 해야 할 컬럼들 PcpStaTyp,  AdrStaTyp,   LabLstUid,    LabLstDtf,      LabOkUid,         LabOkDtf,            LabRst
                    '                                                                                                                  LABOKDTF
...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    Dim strLabAdrKey As String
    Dim strAdrAcpKey As String
...
On Error GoTo errHandler
    ...
    With frmInterface
        ...
        strInNum = Trim(GetText(.vasID, argSpcRow, colInNum))
        strDate = Format(CDate(.dtpToday.Value), "yyyymmdd")

        '-- Local에서 환자별로 결과값 가져오기
        ClearSpread .vasTemp
        
              SQL = "SELECT EQUIPCODE,EXAMCODE,RESULT,EQUIPRESULT,REFFLAG,PANICVALUE,DELTAVALUE,PSEX,SEQNO,PAGE,PID " & vbCrLf
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                '-- 결과테이블에서 LABADRKEY 가져오기
                      SQL = "SELECT LABADRKEY "
                SQL = SQL & "  FROM S_LabInf "
                SQL = SQL & " Where LabBarNum = '" & lsID & "'"
                SQL = SQL & "   And LabCod = '" & strEqpCd & "'"
                Set RS = cn_Ser_Bit.Execute(SQL, , 1)
                If Not RS.EOF And Not RS.BOF Then
                    '-- 결과 업데이트 [중간보고]
                    If frmInterface.optRstType(0).Value = True Then
                              SQL = "Update S_LabInf Set"
                        SQL = SQL & " LABRST  = '" & sResult & "'"
                        SQL = SQL & ",LabMidUid = '" & gUserID & "'"
                        SQL = SQL & ",LabMidDtf = SYSDATE "
                        SQL = SQL & " Where LabBarNum = '" & lsID & "'"
                        SQL = SQL & "   And LabCod = '" & strEqpCd & "'"
                    Else
                    '-- 결과 업데이트 [최종보고]
                              SQL = "Update S_LabInf Set"
                        SQL = SQL & " LABRST  = '" & sResult & "'"
                        SQL = SQL & ",LabLstUid = '" & gUserID & "'"
                        SQL = SQL & ",LabLstDtf = SYSDATE "
                        SQL = SQL & ",LabOkDtf = SYSDATE "
                        SQL = SQL & " Where LabBarNum = '" & lsID & "'"
                        SQL = SQL & "   And LabCod = '" & strEqpCd & "'"
                    End If
                    
                    SetSQLData "결과저장", SQL
                    cn_Ser_Bit.Execute SQL
                    
                    strLabAdrKey = Trim(RS.Fields("LABADRKEY").Value & "")
                End If
                RS.Close
                
                If strLabAdrKey <> "" Then
                          SQL = " Select ADRACPKEY "
                    SQL = SQL & "  From S_AdrInf"
                    SQL = SQL & " Where ADRKEY = " & strLabAdrKey
                    Set RS = cn_Ser_Bit.Execute(SQL, , 1)
                    If Not RS.EOF And Not RS.BOF Then
                        '-- 바디 상태 업데이트
                        If frmInterface.optRstType(0).Value = True Then
                                  SQL = "Update S_AdrInf Set"
                            SQL = SQL & " AdrStaTyp  = 'I'" '중간보고
                            SQL = SQL & " Where ADRKEY = " & strLabAdrKey
                        Else
                                  SQL = "Update S_AdrInf Set"
                            SQL = SQL & " AdrStaTyp  = 'F'" '최종보고
                            SQL = SQL & " Where ADRKEY = " & strLabAdrKey
                        End If
                        SetSQLData "상태변경_D", SQL
                        cn_Ser_Bit.Execute SQL
                        
                        strAdrAcpKey = Trim(RS.Fields("ADRACPKEY").Value & "")
                    End If
                End If
            End If
        Next iRow
        
        If strAdrAcpKey <> "" Then
            '-- 헤더 상태 업데이트
            If frmInterface.optRstType(0).Value = True Then
                      SQL = "Update S_PcpInf Set"
                SQL = SQL & " PcpStaTyp  = 'I'" '중간보고
                SQL = SQL & " Where PCPACPKEY = " & strAdrAcpKey
            Else
                      SQL = "Update S_PcpInf Set"
                SQL = SQL & " PcpStaTyp  = 'F'" '최종보고
                SQL = SQL & " Where PCPACPKEY = " & strAdrAcpKey
            End If
            SetSQLData "상태변경_H", SQL
            cn_Ser_Bit.Execute SQL
            ...
            SQL = SQL & ",LabMidDtf = SYSDATE "
            SQL = SQL & " Where LabBarNum = '" & lsID & "'"
            SQL = SQL & "   And LabCod = 'ADM14'"   'D-Chemi

            SetSQLData "ADM14", SQL
            cn_Ser_Bit.Execute SQL
...
Function GetSampleInfoW(ByVal asRow As Long) As Integer
    ...
    End If
          ...
          SQL = "SELECT PcpAcpDte, PatChtNum, PatNam" & vbCrLf
    SQL = SQL & "  From S_PcpInf" & vbCrLf
    SQL = SQL & "   INNER JOIN S_AcpInf ON PcpAcpKey = AcpKey" & vbCrLf
    SQL = SQL & "   INNER JOIN P_PatInf ON AcpChtNum = PatChtNum" & vbCrLf
    SQL = SQL & " WHERE  PcpBarNum = '" & sBarcode & "'" & vbCrLf
    ...
    If Res = 1 Then
        ...
        SetText frmInterface.vasID, Trim(gReadBuf(1)), asRow, colChtNum
...
'          SQL = "SELECT ResLabCod AS EXAMCODE " & vbCrLf
...
'    SQL = SQL & "   AND ResSpmNum = '" & argPID & "' " & vbCrLf
'    SQL = SQL & "   AND ResLabCod IN (" & gAllExam & ") " & vbCrLf
'    SQL = SQL & " UNION ALL " & vbCrLf
'    SQL = SQL & "SELECT ResLabCod AS EXAMCODE " & vbCrLf
...
'    SQL = SQL & "   AND RsbAcpCod = '" & gGumPart & "' " & vbCrLf
'    SQL = SQL & "   AND ResSpmNum = '" & argPID & "' " & vbCrLf
'    SQL = SQL & "   AND ResLabCod IN (" & gAllExam & ") "
    
          SQL = "SELECT DISTINCT adrcod as EXAMCODE" & vbCrLf
    SQL = SQL & " From S_PcpInf, S_AcpInf, P_PatInf, S_AdrInf" & vbCrLf
    SQL = SQL & " Where PcpAcpKey = AcpKey" & vbCrLf
    SQL = SQL & "   AND AcpChtNum = PatChtNum" & vbCrLf
    SQL = SQL & "   AND PcpBarNum = '" & argPID & "' " & vbCrLf
    SQL = SQL & "   AND PCPACPKEY = ADRACPKEY" & vbCrLf
    SQL = SQL & "   AND ADRCOD IN (" & gAllExam & ") "


'SELECT d.adrcod as EXAMCODE
'From S_PcpInf a, S_AcpInf b, P_PatInf c, S_AdrInf d
'Where a.PcpAcpKey = b.AcpKey
'AND b.AcpChtNum = c.PatChtNum
'and PcpBarNum = '1700000120'
'and a.PCPACPKEY = d.adracpkey
' and d.adrcod IN ('B2570','B2580','B2602','B
    
    SetSQLData "검사조회", SQL
    Set RS = cn_Ser_Bit.Execute(SQL)
