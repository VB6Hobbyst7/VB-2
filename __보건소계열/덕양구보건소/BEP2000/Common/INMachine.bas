...
'
'   Bar-Code로부터 입력된 ID의 검사항목 조회 및 편집,송신
'
'Sub Send_Order()
'
'    Dim Tmp   As String
'    Dim ChkS  As String
'    Dim TestDat As String       '검사항목
'    Dim i   As Integer
'    Dim K   As Integer
'    Dim tData() As String
'    Dim sStr    As String
...
'    End If
'
'    '더이상 보낼 Order가 없으면 ENQ 대기상태(결과받기 준비상태)로...
...
'            iRet = spdWork.GetText(4, K, spDate)    '작업일자
'            iRet = spdWork.GetText(9, K, spSlip)    'Slip Code
'            iRet = spdWork.GetText(5, K, spID)      '작업번호
...
'                '--- Q.C 또는 일반 Sample인 경우
'                If Mid(Trim(spSampNo), 5, 1) = "9" Then        'Q.C 인 경우
...
'
'        Case 5      'EOT
'            '----- Order 전송완료된 Sample의 BackColor변경
