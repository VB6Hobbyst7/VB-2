...
Sub Create_Code_DB()
    ...
    Dim idx(1 To 2) As New Index
    Dim Fd(1 To 4)  As New Field
    ...
    Set Tb = Db.CreateTableDef("cdtable")
            ...
            Set Fd(1) = Tb.CreateField("eqipNo", dbText, 3)
            Tb.Fields.Append Fd(1)
            
            Set Fd(2) = Tb.CreateField("Name", dbText, 20)
            Tb.Fields.Append Fd(2)
            
            Set Fd(3) = Tb.CreateField("Code", dbText, 20)
            Tb.Fields.Append Fd(3)
            
            Set Fd(4) = Tb.CreateField("Mname", dbText, 30)
            Tb.Fields.Append Fd(4)
                                   
            Set idx(1) = Tb.CreateIndex("Primarykey")
            Set Fd(1) = idx(1).CreateField("eqipNo")
            idx(1).Primary = True
            idx(1).Fields.Append Fd(1)
            Tb.Indexes.Append idx(1)
