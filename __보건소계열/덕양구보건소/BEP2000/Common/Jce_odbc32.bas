...
Function Qsqlclose(Index As Long, Optional finish As Integer) As Integer
   Dim i%
   i = Index
   If ODBCConnect(i).useflag = False Then
      'MsgBox "닫혀진 INDEX CLOSE" & Chr(13) & Chr(10) & " 에 연락주세요"
      Exit Function
   End If
...
'        MsgBox "hstmt is not 0 " & Chr(13) & Chr(10) & "JCE에 연락주세요"
        OdbcErrorMsg "hstmt is not 0 " & Chr(13) & Chr(10) & "JCE에 연락주세요.", "ODBC Error"
'        Exit Function
   End If
...
Function QSqlOpen(Server As String, hWnd As Integer, Index As Long) As Integer
