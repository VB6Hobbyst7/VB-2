...
Dim fHitachi7080(100) As String
Dim fHitachi7080_1(100) As String
...
Dim fHitachi7080Cfg(100) As Integer
Dim fHitachi7080Size(100, 1) As Integer
...
Private Sub psDataDefine(ByVal strdata As String, ByRef brChannel() As String, ByVal brspread As Object) ', ByVal brOst As String) ' ByRef brItemdeci() As String)
    ...
    sTemp = strdata
    Erase fHitachi7080
    '------------------------------<<< fHitachi7080() 배열 Clear 한다.         >>>----------
    For Loop_count = 1 To 100: fHitachi7080(Loop_count) = "": Next Loop_count
    '------------------------------<<< fHitachi7080() 배열에 구분하여 넣는다.  >>>----------

    sDecnt = (Len(sTemp) - 50) / 9                  ' 총 검사항목 갯수를 찿는다.
    fHitachi7080(0) = Str$(sDecnt)                  ' 총 검사항목 갯수를 넣는다. 나중에 사용한다.
    fHitachi7080(1) = Mid$(sTemp, 2, 3)             ' 항목 1 ":s"
    fHitachi7080(2) = Mid$(sTemp, 5, 5)             ' 항목 2 "1"   일련번호
    fHitachi7080(4) = Trim(Mid$(sTemp, 14, 16))     ' 항목 4 접수번호,환자 번호

    Dim ssTemp1 As Integer
    Dim ssTemp2 As String
    For pDoCount = 1 To sDecnt
        ssTemp1 = (pDoCount - 1) * 9 + 50           ' 첫번째 Channel 및 검사결과 위치 확인
        ssTemp2 = Mid$(sTemp, ssTemp1, 9)
        fHitachi7080(((pDoCount - 1) * 2) + 4 + 1) = Trim(Mid$(ssTemp2, 1, 2))   ' channel
        fHitachi7080(((pDoCount - 1) * 2) + 4 + 2) = Trim(Mid$(ssTemp2, 3, 6))   ' result
    Next pDoCount

    pGrid_Point = 0
    strTmp = ""
    If Len(fHitachi7080(2)) > 0 Then
        intRow = 0
        With spdResult1

            sSeq = Trim(fHitachi7080(2))
            sCol = 2
                ...
                For intCol = 5 To .MaxCols
                    ...
                    If Not itemX Is Nothing Then
                        For intIdx = 1 To .MaxCols
                            If Len(fHitachi7080(((intIdx - 1) * 2) + 4 + 1)) > 0 Then
'                                chrChk = IsNumeric(fHitachi7080(((intIdx - 1) * 2) + 4 + 1))
'                                If chrChk = False Then
'                                    Channel_No = Mid(Trim(fHitachi7080(((intIdx - 1) * 2) + 4 + 1)), 1, Len(Trim(fHitachi7080(((intIdx - 1) * 2) + 4 + 1))) - 1)
'                                Else
'                                    Channel_No = Trim(fHitachi7080(((intIdx - 1) * 2) + 4 + 1))
'                                End If
                                Channel_No = Trim(fHitachi7080(((intIdx - 1) * 2) + 4 + 1))
                                If Channel_No = itemX.tag Then
                                    strRstval = Trim(fHitachi7080((intIdx - 1) * 2 + 4 + 2))
...
strTmp = strTmp & vbCrLf
strTmp = strTmp & "--------------"
strTmp = strTmp & "Date 4 / 4 / 17"
strTmp = strTmp & "TEST 2"
strTmp = strTmp & "ID"
strTmp = strTmp & "LAB"
strTmp = strTmp & "WBC 7.3"
strTmp = strTmp & "RBC 3.83"
strTmp = strTmp & "HGB 11.9"
strTmp = strTmp & "HCT 36.4"
strTmp = strTmp & "MCV 95#"
strTmp = strTmp & "MCH 31.1"
strTmp = strTmp & "MCHC 32.7"
strTmp = strTmp & "RDW 13.8"
strTmp = strTmp & "PLT 353#"
strTmp = strTmp & "PCT"
strTmp = strTmp & "MPV 6.7"
strTmp = strTmp & "PDW"
strTmp = strTmp & "LY    54.1 R2"
strTmp = strTmp & "MO     9.7 R2"
strTmp = strTmp & "GR    36.2 RM"
strTmp = strTmp & "LY #   3.9 R2"
strTmp = strTmp & "MO #    .7 R2"
strTmp = strTmp & "GR #   2.6 RM"
strTmp = strTmp & "--------------"
strTmp = strTmp & "9C"
