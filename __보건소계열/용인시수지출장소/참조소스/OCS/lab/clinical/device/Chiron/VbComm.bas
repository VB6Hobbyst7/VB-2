...
Type Selectra_ReceiveRecord
    StrKeyCode              As String
    StrFullName             As String
    strRecord               As String
End Type

Public nReciveData()        As Selectra_ReceiveRecord

'Type Selectra_ConstData
'    StrKeyCode              As String
'    StrFullName             As String
'End Type
'
'Public NConstData()           As Selectra_ConstData
...
Public Sub GetBarCodeNumber(ArgPtno As String, Ndate As String, ArgJangBi As String)
    ArgPtno = Format(Val(ArgPtno), "00000000")
    strSQL = ""
    strSQL = strSQL & " SELECT UNIQUE SLIPNO1, SLIPNO2                              " & vbLf
    strSQL = strSQL & " FROM TWEXAM_GENERAL_SUB                                     " & vbLf
    strSQL = strSQL & " WHERE ITEMCD IN (SELECT CODEKY                              " & vbLf
    strSQL = strSQL & "                  FROM TWEXAM_ITEMML                         " & vbLf
    strSQL = strSQL & "                  WHERE GEOMJAN1 = '" & ArgJangBi & "')      " & vbLf
    strSQL = strSQL & "     AND JEOBSUDT = TO_DATE('" & Ndate & "', 'YYYY-MM-DD')   " & vbLf
    strSQL = strSQL & "     AND PTNO     = '" & ArgPtno & "'                        " & vbLf
    strSQL = strSQL & " ORDER BY  SLIPNO1, SLIPNO2                                  " & vbLf
...
Public Sub vaSpread_Clear(Spr As vaSpread, SColumn, SRow, EColumn, ERow)

    Spr.Col = SColumn:       Spr.Col2 = EColumn
    Spr.Row = SRow:          Spr.Row2 = ERow
    ...
    Spr.Col = 1:     Spr.Row = 1
    Spr.Action = SS_ACTION_ACTIVE_CELL
