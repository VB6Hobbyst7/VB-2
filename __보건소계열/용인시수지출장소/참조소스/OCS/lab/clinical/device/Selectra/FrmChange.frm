...
Begin VB.Form FrmChange 
   ...
   Begin FPSpread.vaSpread SS 
      ...
      GridColor       =   8421504
      MaxCols         =   4
      SpreadDesigner  =   "FrmChange.frx":2039
      UserResize      =   1
      VisibleCols     =   4
      VisibleRows     =   120
   End
...
'    strSQL = strSQL & "  WHERE TG.JEOBSUDT = TO_DATE('" & GeomDate & "', 'YYYY-MM-DD')      " & vbLf
'    strSQL = strSQL & "      AND PT.PTNO = TG.PTNO                                          " & vbLf
'    strSQL = strSQL & "      AND TG.SLIPNO1 = 21                                            " & vbLf
'    strSQL = strSQL & "      AND TG.STATUS <> 'C'                                           " & vbLf
'    strSQL = strSQL & "  ORDER BY TG.SLIPNO2,TG.PTNO                                        " & vbLf
    Result = adoSQL(strSQL)
...
Private Sub Form_Load()
    ...
    Me.Top = Selectra.Top + (Selectra.ScaleHeight - Me.ScaleHeight)
    Me.Left = Selectra.Left + (Selectra.ScaleWidth - Me.ScaleWidth)
End Sub
