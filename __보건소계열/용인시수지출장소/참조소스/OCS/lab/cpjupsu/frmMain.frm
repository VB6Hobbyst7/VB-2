...
Private Sub cmdEnrolIPd_Click()
    ...
    If ssOrder.DataRowCnt = 0 Then
        MsgBox "접수할 Data 가 조회되지 않았습니다. Order조회부터하십시오!"
        Exit Sub
    End If
    ...
    MsgBox "재원환자 임상병리검사 접수가 원하시는 조건대로 접수되었습니다!..", _
            vbInformation, _
           "재원환자 접수"
...
Private Sub cmdEnrolOk_Click()
    ...
    iMatchno = Get_MatchLabno      'Order, General, General_Sub 에 입력할때 동일한 Key를 만들기 위하여
                                   '접수 Button 을 누를때 번호를 변수에 Setting
...
General_Vinding_Sub:
    ...
    If Trim(General.DeptCode) = "ER" Then
        General.GbCh = "1"   '응급실 검체는 임상병리과에서 검체확인 한댑니다.
    Else
        General.GbCh = "Y"
    End If
    ...
    For i = nRow(0) To nRow(1)
        ...
        If ssOrder.Value = True Then      '검체확인 Check Box
            ...
            If Trim(sCheckDeptCode) = "ER" Then
                strSql = strSql & "        GBCH      = '1' "        '병동추가Order=1, 응급실=1, 정규Order=2
            Else
                strSql = strSql & "        GBCH      = 'Y' "        'ER 이 아닌 외래는 Y
            End If
...
Get_Order_MainProcess:
    ...
    strSql = strSql & "        TO_CHAR(a.JeobsuDt, 'YYYY-MM-DD') Jeobsudt1," & vbLf '접수일자
    strSql = strSql & "        TO_CHAR(a.Indate,   'YYYY-MM-DD') Indate1,  " & vbLf '내원일자
    strSql = strSql & "        TO_CHAR(a.OrderDt,  'YYYY-MM-DD') Orderdt1, " & vbLf 'order 일자
    strSql = strSql & "        TO_CHAR(a.CollDate, 'YYYY-MM-DD') CollDate1," & vbLf '검체접수일자
    ...
    strSql = strSql & " AND    a.Gbio      = 'O'" & vbLf         '외래환자만
...
Private Sub Form_Load()
    ...
    frmMain.Caption = "검체도착확인(접수)" & GstrPassName
...
Public Sub ssOrder_ButtonClicked(ByVal Col As Long, ByVal Row As Long, ByVal ButtonDown As Integer)
    ...
    GoSub DrComment_Display  '의사 Comment
    GoSub Get_iLLname        '외래상병
