...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Begin FPSpread.vaSpread spdResult2 
         ...
         SpreadDesigner  =   "frmComm_YEJ.frx":7BB4
      End
...
Dim Clinitek500 As typeClinitek500
Dim flgETB As Boolean
Dim fClinitek500(100) As String
Dim fClinitek500Dtl(100) As String
Dim fClinitek500Cfg(100) As Integer
Dim fClinitek500Size(100, 1) As Integer
Dim fChannel() As String
Dim RecordChk As Boolean
Dim fRcvString As String
Dim ReceiveData      As String
Dim fClinitek500Order(100) As String
...
On Error GoTo ErrorTrap
    ...
    gSql = "select a.RE_RCID,b.JU_NAME,b.JU_PERID from EXAM_TOC A,JUMN_TMA B,RECE_TJU C" _
            ...
            & " And a.IN_CODE like 'UR%' And a.EX_INST < '2' " _
            & " And a.IN_CODE <> 'UR00190' " _
...
Private Sub psDataDefine(ByVal strdata As String, ByRef brChannel() As String, ByVal brSpread As Object) ', ByVal brOst As String) ' ByRef brItemdeci() As String)
    ...
    Dim ii, intldx, jj        As Integer
    Dim Loop_Count, pDoCount As Integer
    
    Dim strOrdLst() As String, strPid() As String, strPnm() As String
    Dim intRet      As Integer
    Dim Channel_No, SampleNo As String       ' 문자형 변수
    ...
    For Loop_Count = 1 To 100: fClinitek500(Loop_Count) = "": Next Loop_Count
    Erase fClinitek500Dtl
    pDoCount = 0
    
    ReceiveData = Text_Change(ReceiveData, STX, "")
    
'    Select Case Mid(ReceiveData, 2, 1)
        If InStr(Mid(ReceiveData, 1, 3), "H|") > 0 Then
'        Case "H|"
            ReceiveData = ""
            Exit Sub
'        ElseIf InStr(Mid(ReceiveData, 1, 3), "P|") > 0 Then
''        Case "P|"
'            ReceiveData = Mid$(ReceiveData, InStr(ReceiveData, "|") + 1)
'            ReceiveData = Mid$(ReceiveData, InStr(ReceiveData, "|") + 1)
'            If IsNumeric(Mid(ReceiveData, 1, InStr(ReceiveData, "|") - 1)) Then
'                Patiant_Recevid = Mid(ReceiveData, 1, InStr(ReceiveData, "|") - 1)
'            End If
'            ReceiveData = ""
'            Exit Sub
'        ElseIf InStr(Mid(ReceiveData, 1, 3), "R|") > 0 Then
        Else
'        Case "R|"
            Do While InStr(ReceiveData, Chr(13)) > 0
                pDoCount = pDoCount + 1
                fClinitek500(pDoCount) = Text_Redefine(ReceiveData, Chr(13))
                ReceiveData = Mid$(ReceiveData, InStr(ReceiveData, Chr(13)) + 1)
                If pDoCount > 99 Then
                    ReceiveData = ""
                    Exit Do
                End If
            Loop
'            Do While InStr(ReceiveData, "|") > 0
'                pDoCount = pDoCount + 1
'                fClinitek500(pDoCount) = Text_Redefine(ReceiveData, "|")
'                ReceiveData = Mid$(ReceiveData, InStr(ReceiveData, "|") + 1)
'                If pDoCount > 99 Then
'                    ReceiveData = ""
'                    Exit Do
...
'            Loop
            ...
            'SampleNo = Mid(fClinitek500(2), 4, 3)
            
            sSeq = Mid(fClinitek500(2), 5, 3) 'Patiant_Recevid
            sCol = 0
                    
            intRow = SeqSearch(spdResult1, sSeq, sCol)
                    
            If Len(sSeq) > 0 And intRow > 0 Then
                With spdResult1
                    ...
                    If sSeq > 0 Then
                        For Int_i = 1 To pDoCount
                            'fClinitek500Dtl(pDoCount) = fClinitek500(Int_i)
                            Dim pDoCounttl As Integer
                            pDoCounttl = 0
'                            Do While InStr(fClinitek500(Int_i), "|") > 0
'                                pDoCounttl = pDoCounttl + 1
'                                fClinitek500Dtl(pDoCounttl) = Text_Redefine(fClinitek500(Int_i), "|")
'                                fClinitek500(Int_i) = Mid$(fClinitek500(Int_i), InStr(fClinitek500(Int_i), "|") + 1)
'                                Debug.Print fClinitek500Dtl(pDoCounttl)
'                            Loop
                            ...
                            For intCol = 5 To .MaxCols
                                ...
                                If Not itemX Is Nothing Then
                                    For intIdx = 1 To .MaxCols
                                        Channel_No = Mid(Trim(fClinitek500(intIdx + 2)), 2, 3)
                                        Result = Trim(Mid(fClinitek500(intIdx + 2), 6))
                                        If Trim(Result) = "POSITIVE" Then
                                            Result = "+"
                                        ElseIf Trim(Result) = "NEGATIVE" Then
                                            Result = "-"
                                        ElseIf Trim(Result) = "TRACE" Then
                                            Result = "+/-"
                                        End If
...
'    Dim strData(1 To 5)         As Byte
   ...
   strDta = "1H|\^&|||BAYER^6470^^02.04/02.00||||||020|P|1|20040423190200" & vbCr & vbLf
   strDta = strDta & "08" & vbCr & vbLf
   strDta = strDta & "2P|1|02941|20040423190200" & vbCr & vbLf
   strDta = strDta & "P|2|C|P|MULTISTIX 10 SG|F|0000" & vbCr & vbLf
   strDta = strDta & "24" & vbCr & vbLf
   strDta = strDta & "3R|1|N|GLU|1|NEGATIVE|1|0|A" & vbCr & vbLf
   strDta = strDta & "R|2|N|BIL|2|NEGATIVE|1|0|A" & vbCr & vbLf
   strDta = strDta & "R|3|A|KET|3|1.5^mmol/L|3|0|A" & vbCr & vbLf
   strDta = strDta & "R|4|N|SG|4|>=1.030|6|0|A" & vbCr & vbLf
   strDta = strDta & "R|5|A|BLD|9|TRACE-LYSED|2|0|A" & vbCr & vbLf
   strDta = strDta & "R|6|N|pH|5|6.0|3|0|A" & vbCr & vbLf
   strDta = strDta & "6E" & vbCr & vbLf
   strDta = strDta & "4R|7|A|PRO|6|0.3^g/L|3|0|A" & vbCr & vbLf
   strDta = strDta & "R|8|N|UBG|7|3.2^umol/L|1|0|A" & vbCr & vbLf
   strDta = strDta & "R|9|A|NIT|8|POSITIVE|2|0|A" & vbCr & vbLf
   strDta = strDta & "R|10|A|LEU|10|Ca 70^Leu/uL|3|0|A" & vbCr & vbLf
   strDta = strDta & "R|11|A|COL|11|DK. RED|3|0|A" & vbCr & vbLf
   strDta = strDta & "1E" & vbCr & vbLf
   strDta = strDta & "5L|1" & vbCr & vbLf
   strDta = strDta & "3E" & vbCr & vbLf
   strDta = strDta & "" & vbCr & vbLf
