...
begin
  if ARow = 0 then begin
      ...
      if ACol in [0..C_STA] then begin
          ...
          if (sState = '미전송') or (sState = '결과수신') then
              ABrush.Color:= clYellow
          else
          if sState = '오더전송' then
...
begin
  ...
  if Bcd='' then begin
      ...
      try
          ...
          //7020은 받은거에 스페이스 한자리 붙는다(개고생한다ㅡㅡ;)
          sSender:= Copy(DataStr,1,41)+' 37';
          ...
          //TGlobal.DataLog:= 'IF_PC-> '+STX + sSender + ETX + CR + LF; //Log
          //SendStr(STX + sSender + ETX + CR + LF);
          TGlobal.DataLog:= 'IF_PC-> '+STX + sSender + ETX; //Log
...
begin
  ...
  s:= STX + '>' + ETX;// + CR + LF; //H747CheckSum('>') + CR;
  Result:= s;
...
begin
  ...
  //이미 전송완료인것은 변경하지 말자.
  if gdIf.Cells[C_STA, nRow] <> '전송완료' then begin
      if UpState = 'Y' then
...
begin
          ...
          try
              ...
              if DM.DownLoadOrder_Result(TMaster) then begin
                  ...
                  if DM.UpLoadResult(TMaster.BarCode, ErrMsg) then begin
                      ...
                      if ErrMsg <> '' then
                          AddViewLog('바코드:'+TMaster.BarCode+' 전송 에러 -> MSG['+ErrMsg+']');
                  end;
              end
              else begin
                  AddViewLog('바코드:'+TMaster.BarCode+' -> 오더가 없습니다! 바코드를 확인하세요');
                  Continue;
              end;
          finally
              TMaster.Free;
  ...
  ShowMessage(IntToStr(UpCount)+' 건 전송완료!');
end;
...
begin
  ...
  try
      ...
      ResCnt := StrToIntDef(Trim(Copy(sData,42,2)),0);
      ...
      ResStr:=Copy(sData,45, Length(sData)-45);

      for i:=0 to ResCnt -1 do begin
          sIfCode:= Trim((Copy(ResStr,i*10+1,2)));

          h7180.slResIfCode.Add(sIfCode);
          h7180.slResult.Add(Trim(Copy(ResStr,i*10+3,8)));
      ...
      if ResCnt > 0 then begin
          ...
          if DM.UpLoadResult(h7180.BarCode, SvrMsg) then begin
              ...
              if SvrMsg <> '' then begin
                  AddViewLog('바코드:'+H7180.BarCode+' 전송 에러 -> MSG['+SvrMsg+']');
                  TGlobal.LogMsg:= '바코드:'+H7180.BarCode+' 전송 에러 -> MSG['+SvrMsg+']';  //Err.log
              end;
          end;
...
begin
  ...
  if nRow > 0 then begin
      ...
      with TMaster do begin
          ...
          //gdIf.Cells[C_ADT, nRow] := FAcptDt;
          //gdIf.Cells[C_ANO, nRow] := FAcptNo;
          if gdIf.Cells[C_STA, nRow] <> '전송완료' then
              gdIf.Cells[C_STA, nRow] := '결과수신'
      end;
  end;
...
begin
  ...
  if OldBcd = NewBcd then begin
      ShowMessage('동일한 번호로 변경할수 없습니다!');
      exit;
  end;

  if NewBcd = '' then begin
      ShowMessage('변경하실 번호를 입력하세요!');
      edNew.SetFocus;
      exit;
  end;
