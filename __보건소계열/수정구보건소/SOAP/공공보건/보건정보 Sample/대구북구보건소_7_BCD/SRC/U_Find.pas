...
begin
  ...
  if nRow = 0 then begin
      ShowMessage('해당 데이터를 선택하세요!');
      exit;
  end;

  if MessageDlg('해당 데이터를 변경하시겠습니까?', mtConfirmation, mbOKCancel, 1) <> mrOk then
      exit;
...
begin
  ...
  if iCount > 0 then begin
      if MessageDlg('삭제후 되돌릴수 없습니다! '+#13#10+ ' 선택하신 검사 결과를 삭제 하시겠습니까? ',
      ...
      ShowMessage('삭제하실 검체를 선택 하셔야 합니다!');
  end;
  ...
  try
      with TMaster do begin
          ...
          if UpSucc = True then begin
              ...
              pnUpload.Color:= Color_Load;
              //메인화면 state도 바꿔달라고.. 2008.08.18
              F_Main.DispOneState(TMaster.ExamDate, 'C', '전송완료', TMaster.ExamSeq, True);
  ...
  end;
{
  ...
  if iCount > 0 then begin
      ...
      begin
          ...
          ShowMessage(IntToStr(nCount)+'건 전송!');
      end;
  end
  else begin
...
begin
  ...
  if Length(PatNo) <> PatNoLen then begin
      ShowMessage('등록번호를 확인하세요! 자릿수가 맞지 않습니다.');
      exit;
  end;
