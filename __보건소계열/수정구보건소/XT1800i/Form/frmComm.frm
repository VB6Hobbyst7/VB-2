...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Tab(0).Control(28)=   "spdResult1"
      Tab(0).Control(28).Enabled=   0   'False
      Tab(0).Control(29)=   "tmrOrder"
      Tab(0).Control(29).Enabled=   0   'False
      Tab(0).Control(30)=   "cmdWorkList"
      ...
      Begin BHButton.BHImageButton cmdNext 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         TransparentPicture=   "frmComm.frx":9D43
         ForeColor       =   16711680
         BackColor       =   255
         AlphaColor      =   255
         ImgOutLineSize  =   3
      ...
      Begin VB.ListBox List1 
         Height          =   2220
         ItemData        =   "frmComm.frx":AAA5
         Left            =   7950
         List            =   "frmComm.frx":AAA7
         TabIndex        =   46
         Top             =   6000
         Width           =   7215
      End
      Begin FPSpread.vaSpread spdResult2 
         ...
         SpreadDesigner  =   "frmComm.frx":AAA9
         UserResize      =   0
      End
      ...
      Begin FPSpread.vaSpread spdRstview 
         ...
         ScrollBars      =   0
         ShadowColor     =   14735310
         SpreadDesigner  =   "frmComm.frx":AF20
         UserResize      =   0
      End
      ...
      Begin FPSpread.vaSpread spdWorklist 
         ...
         ScrollBarMaxAlign=   0   'False
         ShadowColor     =   14735310
         SpreadDesigner  =   "frmComm.frx":B7A1
         UserResize      =   2
      End
      ...
      Begin Threed.SSPanel SSPanel1 
         ...
         Begin VB.ComboBox cboChk 
            Height          =   300
            ItemData        =   "frmComm.frx":BCD5
            Left            =   5160
            List            =   "frmComm.frx":BCDF
            TabIndex        =   64
            Top             =   90
            Width           =   1095
         End
         Begin VB.ComboBox cboComNm 
            Height          =   300
            ItemData        =   "frmComm.frx":BCF3
            Left            =   4590
            List            =   "frmComm.frx":BCF5
            TabIndex        =   63
            Top             =   480
            Visible         =   0   'False
            Width           =   1725
      ...
      Begin Threed.SSPanel SSPanel 
         ...
         Begin VB.ComboBox cboRstgbn 
            Height          =   300
            Index           =   1
            ItemData        =   "frmComm.frx":BCF7
            Left            =   2640
            List            =   "frmComm.frx":BD04
            Style           =   2  '드롭다운 목록
            TabIndex        =   77
            Top             =   105
            Width           =   1770
         ...
         Begin VB.ComboBox Combo2 
            Height          =   300
            ItemData        =   "frmComm.frx":BD2E
            Left            =   4590
            List            =   "frmComm.frx":BD30
            TabIndex        =   76
            Top             =   480
            Visible         =   0   'False
            Width           =   1725
      ...
      Begin FPSpread.vaSpread tblexcel 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         SpreadDesigner  =   "frmComm.frx":BD32
      End
...
On Error GoTo ErrorTrap
    CallForm = "clsCommon - Public Function f_subSet_WorkList() As ADODB.Recordset"
        ...
        If Mid(strName, 1, 2) = "검진" Then
            ...
            '-- 검진
            sqlDoc = ""
            sqlDoc = sqlDoc & " Select Rtrim(PRSNCODE) + lTrim(PRSNSUBC) as 처방코드 From MediEHE..PRSNUMBM"
            sqlDoc = sqlDoc & " Where PRSNVSDT = '" & strDate & "'"
            sqlDoc = sqlDoc & "   And PRSNMRNO = '" & strStatus & "'"
            sqlDoc = sqlDoc & "   And PRSNRSLT = ''"
            sqlDoc = sqlDoc & "   And PRSNCODE in('21AC','21AN','21AD','21AE','21AF','21AK','21AF1')"
            sqlDoc = sqlDoc & "   And PRSNSUBC In('','001','002','003','004','005')"
        Else
            ...
            sqlDoc = "       Select 처방코드 + 서브코드 as 처방코드 from TB_검사항목 "
            sqlDoc = sqlDoc & " Where 진료년 = '" & stryy & "'"
            sqlDoc = sqlDoc & "   and 진료월 = '" & strmm & "'"
            sqlDoc = sqlDoc & "   and 진료일 = '" & strdd & "'"
            sqlDoc = sqlDoc & "   and 챠트번호 = '" & strStatus & "'"
            sqlDoc = sqlDoc & "   and 처방코드 in('C4802','C4812','C2243') "
            sqlDoc = sqlDoc & "   and 서브코드 in('','001','002','003','004','005','006','007') "
            sqlDoc = sqlDoc & "   and 진료지원상태 < 5 "
...
On Error GoTo ErrorRoutine
    ...
    With objSpd
        For intRow = 1 To .maxrows
            .GetText 2, intRow, varTmp:    strDate = Trim$(varTmp)      '-- 처방일자
            .GetText 3, intRow, varTmp:    strBarNo = Mid(Trim$(varTmp), 8)    '-- Work No
            .GetText 4, intRow, varTmp:    strSPnm = Trim$(varTmp)      '-- 이름
            .GetText 5, intRow, varTmp:    strRack = Trim$(varTmp)      '-- Rack
            .GetText 6, intRow, varTmp:    strPos = Trim$(varTmp)       '-- Pos
            '.GetText 7, intRow, varTmp:    strChartNo = Trim$(varTmp)   '-- 접수정보
                    ...
                    If Trim$(strResult) <> "" Then         '-- 결과값이 있으면
                        ...
                        If Not itemX Is Nothing Then
                           ...
                           strEqpCd = ""
                            ...
                            If cboChk.ListIndex = 0 Then
                                If strTestCd <> "" Then
                                              sqlDoc = " Update T_LABB42 "
                                     sqlDoc = sqlDoc & "    Set RESULT = '" & strResult & "', "
                                     sqlDoc = sqlDoc & "        SIGN_YN = '0' "
                                     sqlDoc = sqlDoc & "  Where HEALTH_YEAR = '" & Mid(strDate, 1, 4) & "' "
                                     sqlDoc = sqlDoc & "    And HEALTH_DATE = '" & strDate & "' "
                                     sqlDoc = sqlDoc & "    And WORK_NO = " & strBarNo
                                     sqlDoc = sqlDoc & "    And CODE = '" & strTestCd & "' "
                                End If
...
'    Call AddPopup(strSPnm, strBarno)

    'MsgBox "▒ OCSSERVER에 결과를 Upload 완료되었습니다. ▒      " & vbCrLf & vbCrLf & "     LIS 결과조회 화면에서 결과를 확인 하십시요..  ", vbInformation, App.Title
...
Private Sub SetSoapOpen()
Dim oSOAP As SoapClient30
Dim send As Long
...
Private Sub cmdSearch_Click()
    ...
    Call GetHL7_WorkListOrder(mBarNo)
...
Private Sub cmdRstQuery_Click()
    ...
    Do While Not adoRS.EOF
        ...
        If strSpcno <> Trim$(adoRS("TRANSDT") & "") + Trim$(adoRS("SPCNO") & "") Then
                ...
                If InStr(Trim$(adoRS("PNO") & ""), "-") > 0 Then
                    strRackNo = Mid(Trim$(adoRS("PNO") & ""), 1, InStr(Trim$(adoRS("PNO") & ""), "-") - 1)
                    strPos = Mid(Trim$(adoRS("PNO") & ""), InStr(Trim$(adoRS("PNO") & ""), "-") + 1)
                Else
                    strRackNo = ""
                    strPos = ""
                End If
                ...
                .SetText 4, intRow, strRackNo
                .SetText 5, intRow, strPos
                .SetText 6, intRow, Trim$(adoRS("NAME") & "")
            ...
            If Trim$(varTmp) = "1" Then
                ...
                If intRow2 < 1 Then
                    ...
                    If blnFlag = True Then
                        ...
                        spdResult1.SetText 5, intRow2, Format(intRow2, "####")
                        spdResult1.SetText 6, intRow2, intRow2
                        spdResult1.SetText 7, intRow2, intRow2
...
Public Function GetOrder1(ByVal pChannel As Variant) As String
    ...
    For i = 0 To UBound(pChannel) - 1
        strIntBase = pChannel(i)
    ...
    With cInterface
        ...
        Select Case strDisCode1
            Case "R"    '## Informat Inquiry Format
                ...
                strBarNo = Mid$(strRcvBuf, 6, 15):  mBarNo = strBarNo
                strRackNo = Mid$(strRcvBuf, 23, 6): mRackNo = strRackNo
                strPos = Mid$(strRcvBuf, 29, 2):    mPos = strPos
            ...
            Case "D"
                ...
                Select Case strDisCode2
                    Case "1"    '## Analysis Data Format1
                        ...
                        strRackNo = Format$(Mid$(strRcvBuf, 62, 6), "#"): mRackNo = strRackNo
                        strPos = Format$(Mid$(strRcvBuf, 68, 2), "#"):    mPos = strPos
                        ...
                        End With
                    Case "2"
                        pGrid_Point = SeqSearch(spdResult1, mBarNo, 3)
                        If pGrid_Point = 0 Then
                            pGrid_Point = SeqNullSearch(spdResult1, mBarNo, 3)
                            If pGrid_Point = 0 Then
                                '-- 바코드에 해당하는 오더 가져와서 보여주기
'                                Call GetHL7_BarOrder(mBarNo)
                        ...
                        spdResult1.Col = 3: spdResult1.text = mBarNo
                        spdResult1.Col = 5: spdResult1.text = mRackNo
                        spdResult1.Col = 6: spdResult1.text = mPos
                        
                        spdResult1.Col = pDocount + 7
                        For intCol = 8 To spdResult1.MaxCols
                            ...
                            If Not itemX Is Nothing Then
                                ...
                                strIntBase = itemX.tag
                                lngStart = itemX.SubItems(8)
                                lngLen = itemX.SubItems(9)
                                ...
                                'spdResult1.Col = 5:      spdResult1.text = strRackNo
                                'spdResult1.Col = 6:      spdResult1.text = strPos
                                
                                '-- 임시 : rackno와 pos를 저장할때가 없다는......ㅜㅜ
                                pNo = strRackNo & "-" & strPos
                                ...
                                spdResult1.GetText 4, pGrid_Point, strName
                                ...
                                If cboChk.ListIndex = 0 Then
                                    sqlDoc = "insert into INTERFACE003(" & _
                                             ...
                                             "    values( '" & strBarNo & "', '" & itemX & "', '" & itemX.tag & "'," & _
                                             "            '" & Format$(Now, "YYYYMMDD") & "', '" & Format$(Now, "MMSS") & intCol & "'," & _
                                             "            '" & strResult & "', ''," & _
                                             "            '" & INS_CODE & "', '', '" & strName & "', '" & pNo & "')"
                                Else
                                    sqlDoc = "insert into INTERFACE003(" & _
                                             "            SPCNO, TESTCD, EQPNUM, TRANSDT, TRANSTM, RSTVAL, REFVAL, EQUIPCD, SERVERGBN, NAME, PNO)" & _
                                             "    values( '" & strBarNo & "', '" & itemX & "', '" & itemX.tag & "'," & _
...
Private Function GetHL7_BarOrder(ByVal strBarNo As String) As String
...
Set oSOAP = New SoapClient30
                ...
                strHL7Msg = "MSH|^~\&|HL7|MMS|||1||ORU^R01|1a082e2:10e59b48c04:-2cf9:27695009|P|2.3||||||8859/1" & Chr(13)
    strHL7Msg = strHL7Msg & "PID|||" & Format(Now, "yyyymmddhhmm") & "^" & INS_CODE & "^DefaultDomain^PI" & Chr(13)
    strHL7Msg = strHL7Msg & "PV1||E|" & HOS_CODE & " & Chr(13)"
    strHL7Msg = strHL7Msg & "OBR|1||||||1"

                            'MSH|^~\&|HL7|MMS|||1||ORU^R01|1a082e2:10e59b48c04:-2cf9:27695009|P|2.3||||||8859/1
                            'PID|||200812300001^C1^^DefaultDomain^PI
                            'PV1||E|A1301
                            'OBR|1||||||1

    GetHL7_BarOrder = B64Encode(strHL7Msg)
    
'    oSOAP.MSSoapInit ("http://10.47.14.52:8009/HL7IFWebService/WebService.asmx?wsdl")
    oSOAP.MSSoapInit ("C:\Users\Administrator\Desktop\SOAP\공공보건\보건정보 Sample\WebService.wsdl")
    
'    result = oSOAP.MSSoapInit2("C:\Users\Administrator\Desktop\SOAP\공공보건\보건정보 Sample\WebService.wsdl", "C:\Users\Administrator\Desktop\SOAP\공공보건\보건정보 Sample\WebService.wsdl", "New_SelectOrder", "", "")


    Result = oSOAP.New_SelectOrder(GetHL7_BarOrder)
    
    

End Function

Private Function GetHL7_WorkListOrder(ByVal strBarNo As String) As String

Dim SoapClientA As SoapClient.clsSoap
    
    On Error GoTo SOAPError
    
    Set SoapClientA = New SoapClient.clsSoap
    
    GetHL7_WorkListOrder = SoapClientA.MdbOrderList(INS_CODE, HOS_CODE)

    Set SoapClientA = Nothing

Exit Function

SOAPError:
  MsgBox Err.Description
  GetHL7_WorkListOrder = ""

End Function

Private Function SetHL7_Result(ByVal strBarNo As String) As String
Dim strHL7Msg As String

                strHL7Msg = "MSH|^~\&|HL7|MMS|||1||ORU^R01|1a082e2:10e59b48c04:-2cf9:27695009|P|2.3||||||8859/1" & Chr(13)
    strHL7Msg = strHL7Msg & "PID|||" & strBarNo & "^" & INS_CODE & "^^DefaultDomain^PI" & Chr(13)
    strHL7Msg = strHL7Msg & "PV1||E|" & HOS_CODE & " & Chr(13)"
    strHL7Msg = strHL7Msg & "OBR|1||||||1"
    
    SetHL7_Result = B64Decode(strHL7Msg)

End Function

Function B64Decode(ByVal EncodeData As String) As String
Dim objEncode       As Base64

    Set objDecode = New Base64

    If IsNull(EncodeData) Or Len(Trim$(EncodeData)) < 10 Then
        B64Decode = ""
    Else
        B64Decode = objDecode.DecodeArr(EncodeData)
    End If
...
Dim objEncode       As Base64
    ...
    If IsNull(DecodeData) Or Len(Trim$(DecodeData)) < 10 Then
        B64Encode = ""
    Else
        B64Encode = objEncode.EncodeArr(Data)
    End If
