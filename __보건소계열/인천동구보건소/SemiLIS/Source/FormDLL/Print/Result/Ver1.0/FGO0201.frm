   ...
   Begin Threed.SSPanel pnlMain 
      ...
      Begin Threed.SSPanel pnlOption 
         ...
         Begin Threed.SSPanel SSPanel3 
            ...
            EndProperty
            BevelOuter      =   0
            Begin Threed.SSOption optprtkind 
               Height          =   315
               Index           =   0
               Left            =   1170
               TabIndex        =   20
               ToolTipText     =   "검사항목옆에 검사결과를 출력"
               Top             =   60
               Width           =   945
               _Version        =   65536
               _ExtentX        =   1667
            ...
            Begin Threed.SSOption optprtkind 
               ...
               TabIndex        =   21
               TabStop         =   0   'False
               ToolTipText     =   "검사항목을 첫 TITLE에만 출력하고, 계속 결과만 출력"
               Top             =   60
               Width           =   945
               _Version        =   65536
               _ExtentX        =   1667
...
' 기본 변수 체크
    ...
    If optprtkind(0).Value = True Then
        PrtKind = "0"
    Else
        PrtKind = "1"
    End If
...
'
'            For k = 1 To j
                'iRCnt = DCO0101.Print_ResultSheet(Format(dtpLabDateS.Value, "YYYYMMDD"), gCodeHlpTable(k).sCode, txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn)
                iRCnt = DCO0101.Print_ResultSheet(Format(dtpLabDateS.Value, "YYYYMMDD"), "All", txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn, PrtKind)
'            Next
        Else
            iRCnt = DCO0101.Print_ResultSheet(Format(dtpLabDateS.Value, "YYYYMMDD"), txtslipcd.Text, txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn, PrtKind)
        End If
    Set DCO0101 = Nothing
    
    If iRCnt = 0 Then
        ViewMsg "출력할 결과대장이 없습니다."
    Else
        ViewMsg Trim(Str(iRCnt)) & " 건의 결과대장이 출력되었습니다."
    End If
...
Private Sub dtpLabDateE_KeyDown(KeyCode As Integer, Shift As Integer)
