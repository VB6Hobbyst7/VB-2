   ...
   Begin Threed.SSPanel pnlMain 
      ...
      Begin Threed.SSPanel pnlButton 
         ...
         Begin Threed.SSCommand cmdExit 
            ...
            EndProperty
            BevelWidth      =   3
            Picture         =   "FGO0101.frx":0000
         End
         Begin Threed.SSCommand cmdPrint 
            Height          =   1005
            Left            =   2520
            TabIndex        =   7
            ToolTipText     =   "검사 보고서 출력"
            Top             =   60
            Width           =   1125
            _Version        =   65536
            _ExtentX        =   1984
...
Private Sub chkAll_Click()
    ...
    If chkAll.Value = 1 Then
        ...
        cmdsliph.Enabled = False
        lblSlipNm.Enabled = False
    Else
        txtslipcd.Enabled = True
        cmdsliph.Enabled = True
        lblSlipNm.Enabled = True
    End If
...
'            For k = 1 To j
'                iRCnt = DCO0101.Print_Result(Format(dtpLabDateS.Value, "YYYYMMDD"), gCodeHlpTable(k).sCode, txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn)
'            Next

            iRCnt = DCO0101.Print_AllResult(Format(dtpLabDateS.Value, "YYYYMMDD"), "All", txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn)

'            For i = 1 To 2
'                iRCnt = DCO0101.Print_Result(Format(dtpLabDateS.Value, "YYYYMMDD"), Val(i), txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn)
'            Next
        Else
            iRCnt = DCO0101.Print_Result(Format(dtpLabDateS.Value, "YYYYMMDD"), txtslipcd.Text, txtLabSeqS.Text, txtLabSeqE.Text, PrtGbn)
        End If
    Set DCO0101 = Nothing
    
    If iRCnt = 0 Then
        ViewMsg "출력할 결과가 없습니다."
    Else
        ViewMsg Trim(Str(iRCnt)) & " 건의 결과가 출력되었습니다."
    End If
...
Private Sub dtpLabDateS_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        If txtslipcd.Enabled = True Then
            txtslipcd.SetFocus
        End If
        KeyCode = 0
    End If
...
Private Sub txtLabSeqE_LostFocus()
    ...
    If IsNumeric(txtLabSeqS) = False Then
        ViewMsg "입력된 작업번호가 유효하지 않습니다"
    End If
...
Private Sub txtLabSeqS_LostFocus()
    ...
    If IsNumeric(txtLabSeqS) = False And Trim(txtLabSeqS) <> "" Then
        ViewMsg "입력된 작업번호가 유효하지 않습니다"
    End If
