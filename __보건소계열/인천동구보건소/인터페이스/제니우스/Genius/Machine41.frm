...
Sub PhaseCfg_Protocol()
    ...
    Call spdface.GetText(1, SampleCnt + 1, no_tmp)
    ...
    On Error GoTo ErrProtocol
    
'    Open App.Path & "\giorn.dat" For Input As #3
    '-- 절대경로
    Open "C:\Program Files\Semilis\giorn.dat" For Input As #3
    Test_OpenFlag = 2
            
    '-- 2001.12.18수정(osw)
    ReDim s_RcvBuffer(0) As String
    'dumplen = 0
    Bfr = 0
    RcvBuffer = ""
    
    Do While Not EOF(3)   ' 파일의 끝을 만날 때까지 반복합니다.
        wkdat = Input(1, #3)   ' 한 문자를 가져옵니다.
        ...
        Case 2
            Select Case Asc(wkdat)
                Case 1
                    phase = 2
                    If Len(RcvBuffer) <> 0 Then
                        If Len(RcvBuffer) > 10 Then
                            For il = 1 To Len(RcvBuffer)
                                If IsDate(Mid(RcvBuffer, il, 10)) And InStr(Mid(RcvBuffer, il, 10), "/") > 0 Then
                                    RcvBuffer = Mid(RcvBuffer, InStr(RcvBuffer, "/") - 2)
                                    Debug.Print RcvBuffer
                                    ReDim Preserve s_RcvBuffer(Bfr) As String
                                    s_RcvBuffer(Bfr) = RcvBuffer
                                    'dumplen = dumplen + 1
                                    Bfr = Bfr + 1
                                    'Call Edit_data
                                    phase = 1
    ...
    If MsgBox("샘플 " & Bfr & "개를 수신받았습니다" & Chr(13) & _
            "범위를 지정하시겠습니까?", vbInformation + vbYesNo, Me.Caption) = vbYes Then
        
        Stnum = InputBox("총 " & Bfr & " 개의 샘플이 있습니다" & Chr(13) & _
                "수신받을 샘플의 시작번호를 입력하십시요", Me.Caption, "1")
    
        If Stnum = "" Then Stnum = "1"
        If CInt(Stnum) > Bfr Then
            MsgBox "샘플번호 입력 오류입니다." & "샘플번호를 확인 하십시요", vbInformation, Me.Caption
            Close #3
            Exit Sub
        End If
    Else
    ...
    For ix1 = 0 To Bfr
       ...
       Call spdface.GetText(1, ix1, no_tmp)
       
       If Trim(no_tmp) = "" Then Exit For
       If CInt(Stnum) <= ix1 + 1 Then
           If ix1 < Bfr Then
               RcvBuffer = s_RcvBuffer(ix1)
               Call Edit_data
           End If
       Else
           wkbuf = Input(loglen, #3)
       End If
    Next
    Close #3
...
ErrProtocol:
    ...
    If Err.Number <> 0 Then
        MsgBox "에러번호 : " & Err.Number & Chr(13) & Err.Description, vbExclamation, "혈액학 장비"
    End If
...
'    '-- sampleNo.얻기->slipno에 해당
    ...
    For j = 1 To 25
        Select Case j
            Case 1 To 18
                ...
                If j = 9 Then tresult(j) = Format(tresult(j), "0.###")
                StNm = StNm + 4
            Case 19 To 25
    ...
    '-- DB에 등록이 되었으므로 결과 등록이 가능한 조건이 되었음을 나타내는 키
    identbOpenKey = True
...
Sub Test()
    ...
    Open App.Path & "\dump_g.dat" For Input As #3
    Test_OpenFlag = 2
    wkbuf = ""
    Do While Not EOF(3)
...
Private Sub cmdLoad_Click()
    ...
    '-- 장비로부터 파일받기(현재 Giorn.ris로 만들어져 있슴)
    'Kill App.Path & "\Giorn.dat"
    Call Shell(App.Path & "\convertN.exe", vbNormalFocus)
        
'    HndUse = App_GetWindowHendle("convertN")
    
    Timer1.Enabled = True
    Timer1.Interval = 2000
    
    '-- 컨버터시킨 파일을 스프레드에 나타낸다.
    'Call PhaseCfg_Protocol
