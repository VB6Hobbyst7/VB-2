...
/*******************
	time_t	ltime;
	struct	tm *today;
	char	TmpDate[8],TodayDateTime[16];

	TmpDate[8]='\0';

	time(&ltime);
	today = localtime(&ltime);
	strftime(TodayDateTime, 16, "%Y%m%d%H%M%S", today);
-------------------------
	struct date today;
	struct time now;
	char kim[1];
	  ...
	  for (i=1;i<=2;i++){
        if (BufEmpty(i)!=TRUE) {
		  ...
		  if(ch == SOH){
			...
			if(datasure == TRUE){
				for( k=1 ; k < (j-1) ; k++)	fputc(Data[k], fp2);
				fputc(0x0d, fp2);
			}
		  }else{
...
/*************************************************
#define	TIMER_IntV 0x1c				Timer Interrupt Vector 
#define TimedOut() (tic==0)
#define SetTimer(m_sec) tic=m_sec*182/10+1
