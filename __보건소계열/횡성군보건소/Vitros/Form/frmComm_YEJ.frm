...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Begin VB.ComboBox cboRstgbn 
         Height          =   300
         Index           =   1
         ItemData        =   "frmComm_YEJ.frx":5462
         Left            =   -72570
         List            =   "frmComm_YEJ.frx":546F
         Style           =   2  '드롭다운 목록
         TabIndex        =   16
         Top             =   495
         Visible         =   0   'False
      ...
      Begin VB.ComboBox cboRstgbn 
         Height          =   300
         Index           =   0
         ItemData        =   "frmComm_YEJ.frx":5499
         Left            =   3930
         List            =   "frmComm_YEJ.frx":54A6
         Style           =   2  '드롭다운 목록
         TabIndex        =   12
         Top             =   495
         Visible         =   0   'False
      ...
      Begin FPSpread.vaSpread spdResult1 
         ...
         SpreadDesigner  =   "frmComm_YEJ.frx":54D0
         UserResize      =   0
      End
      ...
      Begin FPSpread.vaSpread spdWorkList 
         ...
         ScrollBarMaxAlign=   0   'False
         SelectBlockOptions=   0
         SpreadDesigner  =   "frmComm_YEJ.frx":5CC5
         UserResize      =   0
      End
      ...
      Begin FPSpread.vaSpread spdResult2 
         ...
         ScrollBarMaxAlign=   0   'False
         SelectBlockOptions=   0
         SpreadDesigner  =   "frmComm_YEJ.frx":6903
      End
...
On Error GoTo ErrorTrap
    ...
    gSql = "select a.RE_RCID,b.JU_NAME,b.JU_PERID from EXAM_TOC A,JUMN_TMA B,RECE_TJU C" _
            & " Where c.RE_DATE = '" & strDate & "' And a.IN_CODE like 'BC%' And a.EX_INST < '2' And a.RE_RCID = c.RE_RCID And b.JU_PERID = c.JU_PERID"
...
'        pDoCount = pDoCount + 1
'        fVitors(pDoCount) = Text_Redefine(sTemp, " ")
'        sTemp = Mid$(sTemp, InStr(sTemp, " ") + 1)   'pdocount = 18 --seq
'        If pDoCount > 99 Then
'            sTemp = ""
'            Exit Do
'        End If
'        fVitors(pDoCount) = Text_Change(fVitors(pDoCount), " ", "")
'    Loop
    ...
    If Len(fVitors(1)) > 0 Then
        ...
        With spdResult1
            ...
            .GetText 2, 2, varTmp:   strBarno = Trim$(varTmp)
...
''                    Channel_No = Trim(fVitors(((Loop_Count - 1) * 2) + 4 + 1))          ' channel
''                       ssTemp2 = fVitors(((Loop_Count - 1) * 2) + 4 + 2)               ' result
''                    If Channel_No > 0 And Channel_No = Val(brChannel(pDoCount)) Then
''                        If Channel_No = 4 Then
''                            Float_rate2 = Val(ssTemp2)
''                            Float_rate = Val(ssTemp2) * 2.14                            ' BUN값을 UREA값으로 치환
''                            Float_rate1 = Fix((Val(Float_rate) + Val(0.05)) * Val(10)) / Val(10)
''                            .Text = Val(Float_rate1)
''                            FunStr = Val(Float_rate1)
''                            .SetText 14, pGrid_Point, Float_rate2                       ' 실제로는 BUN값이지만 보건소 사정상 UREA값으로 넘김
''                            Exit For
''                        Else
''                            .Text = Val(ssTemp2)
''                            FunStr = Val(ssTemp2)
...
''
''                            sqlDoc = "delete from INTERFACE003 " & _
...
''                            sqlDoc = "insert into INTERFACE003(" & _
''                                     "            SPCNO, TESTCD, EQPNUM, TRANSDT, TRANSTM, RSTVAL, REFVAL, EQUIPCD, SERVERGBN)" & _
''                                     "    values( '" & CStr(sSeq) & "', '" & tmpTstCd & "', '" & Channel_No & "'," & _
