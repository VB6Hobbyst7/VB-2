...
'    Dim intCnt As Integer, intCnt1   As Integer, intCnt2   As Integer
...
Dim sDecnt As Integer
    ...
    sTemp = strdata
    Erase fToshiba
    '------------------------------<<< fToshiba() 배열 Clear 한다.         >>>----------
    For Loop_Count = 1 To 100: fToshiba(Loop_Count) = "": Next Loop_Count
    '------------------------------<<< fToshiba() 배열에 구분하여 넣는다.  >>>----------
    
    sDecnt = (Len(sTemp) - 45) / 9                  ' 총 검사항목 갯수를 찿는다.
    fToshiba(0) = Str$(sDecnt)                      ' 총 검사항목 갯수를 넣는다. 나중에 사용한다.
    fToshiba(1) = Mid$(sTemp, 2, 1)                 ' 항목 1 ":E"
    fToshiba(2) = Mid$(sTemp, 16, 5)                ' 항목 2 "1"   일련번호
    fToshiba(4) = Trim(Mid$(sTemp, 21, 14))         ' 항목 4 접수번호,환자 번호
    
    Dim ssTemp1 As Integer
    Dim ssTemp2 As String
    For pDoCount = 1 To sDecnt
        ssTemp1 = (pDoCount - 1) * 9 + 45           ' 첫번째 Channel 및 검사결과 위치 확인
...
'                                If InStr(itemX.ListSubItems(1), "L20") = 1 Then
'                                    tmpTstCd = Mid(itemX.ListSubItems(1), 1, InStr(itemX.ListSubItems(1), ",") - 1)
'                                Else
