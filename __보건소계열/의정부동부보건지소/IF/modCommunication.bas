...
Function SaveTransData_HEALTH(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim j               As Integer
    ...
    Dim sParamHeader    As String
    Dim sParamDetail    As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        sParamHeader = ""
        sParamHeader = sParamHeader & "MSH|^~\&|HL7|MMS|||" & Format(Now, "yyyymmddhhmmss") & "||ORU^R01|1a082e2:10e59b48c04:-2cf9:27695009|P|2.3||||||8859/1 " & Chr(13)
        sParamHeader = sParamHeader & "PID|||" & strBarcode & "^" & gHOSP.MACHCD & "^" & gHOSP.USERID & "^^^DefaultDomain^PI" & Chr(13)
        sParamHeader = sParamHeader & "PV1||E|" & gHOSP.HOSPCD & Chr(13)
        sParamHeader = sParamHeader & "OBR|1||||||" & Format(Now, "yyyymmddhhmmss") & Chr(13)
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                sParamDetail = sParamDetail & "OBX|" & CStr(j) & "|" & lsSpcCode & "|" & strTestCd & "||" & sResult & "||||||R" & Chr(13)
            End If
        Next intRow
               
        sParam = Chr(11) & sParamHeader & sParamDetail
        
        If gWebSvcCon = True Then
            If SendResult(sParam) = 1 Then
                SaveTransData_HEALTH = 1
            Else
                SaveTransData_HEALTH = -1
            End If
