...
Public Function GetEquipExamCode_INDIKO(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim i As Integer
    Dim sExamCode As String
    Dim strExamCode As String
    Dim sSpecNo     As String
    ...
    If strExamCode <> "" Then
        GetEquipExamCode_INDIKO = Mid(strExamCode, 2)
    End If
...
'        Call SetSQLData("결과저장", strBarcode & ":" & strPatNm, "A")
    ...
    Else
        ...
        With SPD
            ...
            For i = 0 To UBound(strHData)
                blnSame = False
                If Trim(mGetP(strHData(i), 1, "|")) >= pFrom And Trim(mGetP(strHData(i), 1, "|")) <= pTo Then
...
On Error GoTo DBErr
    ...
    Call SetSQLData("바코드조회", strParam, "A")
    strParam = MakeB64(strParam)
    strReturn = oSOAP.New_SelectOrder(strParam)
    strReturn = MakeUB64(strReturn)
    
'strReturn = Chr(11) & "MSH|^~\&|HL7|MMS|||20200907175054||ORU^R01|1a082e2:10e59b48c04:-2cf9:27695009|P|2.3||||||8859/1" & vbCr
'strReturn = strReturn & "PID|||" & strBarcode & "^F^000000^0^20200907^20200907^DefaultDomain^PI" & vbCr
'strReturn = strReturn & "PV1||E|L0601" & vbCr
'strReturn = strReturn & "OBR|1||||||20200907175054" & vbCr
'strReturn = strReturn & "OBX|1|ST|WB2570||||||||R" & vbCr
'strReturn = strReturn & "OBX|2|ST|WB2580||||||||R" & vbCr
'strReturn = strReturn & "OBX|3|ST|WC2411||||||||R" & vbCr
'strReturn = strReturn & "OBX|4|ST|WC3730||||||||R" & vbCr
'strReturn = strReturn & "OBX|5|ST|WC3780||||||||R" & vbCr
'strReturn = strReturn & "OBX|6|ST|WHA0000016||||||||R" & vbCr
'strReturn = strReturn & "  "
            ...
            For i = 0 To UBound(strHData)
                Select Case mGetP(strHData(i), 1, "|")
                    ...
                    Case "OBX"
                        ...
                        For intCol = colSTATE + 1 To .MaxCols
                            'If Trim(strExmnCd) = gArrEQP(intCol - colSTATE, 2) Then
                            If GetTestNm(strExmnCd, False) = gArrEQP(intCol - colSTATE, 6) Then
