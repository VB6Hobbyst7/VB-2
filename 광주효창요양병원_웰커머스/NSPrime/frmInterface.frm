   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboChk 
            Height          =   315
            ItemData        =   "frmInterface.frx":8A6F
            Left            =   3720
            List            =   "frmInterface.frx":8A7C
            TabIndex        =   51
            Top             =   -30
            Visible         =   0   'False
            Width           =   825
         ...
         Begin FPSpread.vaSpread vasWorkList 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":8A92
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":9408
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":9D75
         End
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String)
    ...
    Dim strFDay     As String
    Dim strTYear    As String
    Dim strTMonth   As String
    Dim strTDay     As String
    
    Dim strDate     As String
    
    strFYear = Mid(pFrDt, 1, 4)
    strFMonth = Mid(pFrDt, 5, 2)
    strFDay = Mid(pFrDt, 7, 2)
    
    strTYear = Mid(pToDt, 1, 4)
    strTMonth = Mid(pToDt, 5, 2)
    strTDay = Mid(pToDt, 7, 2)
    ...
    'Â÷Æ®¹øÈ£,°Ë»çÀÏ,°ËÃ¼¹øÈ£,¼º¸í
          SQL = "SELECT a.per1_mem_id, a.per1_date, a.per1_bun_no, a.per1_name "
    SQL = SQL & "  FROM M01002TB1 a, M01007TB1 b"
    SQL = SQL & " WHERE a.KEYFIELD = b.GUL_KEY"
    SQL = SQL & "   AND a.per1_date Between '" & pFrDt & "' And '" & pToDt & "'" & vbCr
    SQL = SQL & "   AND b.gul_gum_code = 'M006' " & vbCr
    SQL = SQL & "   AND a.per1_date = b.gul_date"
    SQL = SQL & " Order By a.per1_bun_no"
        
        
'M01002TB1.KEYFIELD = M01007TB1.GUL_KEY
'M01002TB1.KEYFIELD2 = M01007TB3.GUL_KEY
'
'1 Â÷°á°ú¸¶½ºÅÍ (M01007TB1)
'gul_date    °Ë»çÀÏ  datetime(8)
'gul_bun_no  °ËÃ¼¹øÈ£    smallint(2)
'gul_gum_code    °Ë»çÇ×¸ñÄÚµå    nvarchar(4)
'gul_value °á°ú°ª
'
'2 Â÷°á°ú¸¶½ºÅÍ (M01007TB3)
'gul2_date °Ë»çÀÏ
'gul2_bun_no °ËÃ¼¹øÈ£
'gul2_gum_code °Ë»çÇ×¸ñÄÚµå
'gul2_value °á°ú°ª
...
'        strDate = Trim(RS.Fields(1)) + Trim(RS.Fields(2)) + Trim(RS.Fields(3))
        txtNum.Text = txtNum.Text + 1
        
        SetText vasWorkList, 1, intRow, colCheckBox
        SetText vasWorkList, txtNum, intRow, colSeqNo
        SetText vasWorkList, Trim(RS.Fields(1)) & "", intRow, colOrdDate
        SetText vasWorkList, Trim(RS.Fields(2)) & "", intRow, colBarcode
        SetText vasWorkList, Trim(RS.Fields(3)) & "", intRow, colPName
        
        
        RS.MoveNext
    Loop
    
'    Call SpreadSheetSort(vasWorkList, 4)
...
Private Function GetWorkList_Barcode(ByVal pDate As String, ByVal pBunNo As String, ByVal pintRow As Long) As Boolean
    Dim i           As Integer
    Dim intRow      As Long
    Dim strItems    As String
    Dim strExamCode As String
    ...
    SQL = SQL & "  FROM M01002TB1 a, M01007TB1 b"
    SQL = SQL & " WHERE a.KEYFIELD = b.GUL_KEY"
    SQL = SQL & "   AND a.per1_date = '" & pDate & "'" & vbCr
    SQL = SQL & "   AND b.gul_gum_code = 'M006' " & vbCr
    SQL = SQL & "   AND a.per1_bun_no = '" & pBunNo & "'" & vbCr
    ...
    Do Until RS.EOF
        ...
        SetText vasID, 1, pintRow, colCheckBox
        SetText vasID, txtNum, pintRow, colSeqNo
        SetText vasID, Trim(RS.Fields(1)) & "", pintRow, colOrdDate
        SetText vasID, Trim(RS.Fields(2)) & "", pintRow, colBarcode
        SetText vasID, Trim(RS.Fields(3)) & "", pintRow, colPName
...
Private Sub SetPatInfo(ByVal pBarNo As String)
    ...
    For i = 1 To vasID.DataRowCnt
        If Trim(GetText(vasID, i, colBarcode)) = pBarNo Then
            intRow = i
            strOrdDt = Trim(GetText(vasID, i, colOrdDate))
            Exit For
        End If
    Next i
...
'Debug.Print strBuffer
    If Mid$(strTemp1, 3, 1) <> "U" Then '"U" = °ËÃ¼ "C" = ÄÁÆ®·Ñ
        Exit Sub
    End If
...
'    intRow = 0
    ...
    strRackNo = Trim(Mid(strTemp1, 8, 3))
    strTubePos = Trim(Mid(strTemp1, 11, 2))
    strMethode = IIf(Mid(strTemp1, 13, 1) = "1", "1ÀÏº°°ËÃ¼", "2ÀÏº°°ËÃ¼")
    strPatID = Trim(Mid(strTemp1, 14, 14))
'        strAbs = Trim(Mid(strTemp1, 28, 5))
    strResult = Trim(Mid(strTemp1, 33, 13))
'        strConc = Trim(Mid(strTemp1, 38, 7))
'        strDtTm = Trim(Mid(strTemp1, 45, 10)) & " " & Trim(Mid(strTemp1, 55, 8))
'        strErr = Mid(strTemp1, 63, 2)
...
'        Channel_No = "1"
    ...
    For i = 1 To vasID.DataRowCnt
        ...
        If vasID.Text = strSeq Then
            ...
            '-- 2015.09.03 Ã¼Å©µÇ¾î ÀÖÁö ¾ÊÀ¸¸é °á°úÀúÀåÀ» ÇÏÁö ¾Ê´Â´Ù.
    ...
    If strResult <> "" Then
        SQL = ""
        SQL = SQL & "SELECT EXAMCODE,EXAMNAME,SEQNO, SUBCODE "
        SQL = SQL & "  FROM EQPMASTER"
        SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' "
        SQL = SQL & "   AND EQUIPCODE = '" & strIntBase & "' "
        SQL = SQL & "   AND EXAMCODE = 'M006' "
        ...
        Else
                  ...
                  SQL = "Select EXAMCODE,EXAMNAME,SEQNO,SUBCODE "
            SQL = SQL & "  From EQPMASTER"
            SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' "
            SQL = SQL & "   AND EQUIPCODE = '" & strIntBase & "' "
            Res = GetDBSelectColumn(gLocal, SQL)
...
'    SQL = SQL & "'" & Trim(Format(dtpExamDate.Value, "YYYYMMDD")) & "', "
    SQL = SQL & "'" & Format(Trim(GetText(vasID, asRow1, colOrdDate)), "YYYYMMDD") & "', "
    SQL = SQL & "'" & gEquip & "', "
...
Dim lngNum As Long
    ...
    If KeyAscii = 13 Then
        With vasID
            If vasID.ActiveCol = colSeqNo Then
            ...
            ElseIf vasID.ActiveCol = colBarcode Then
                vasID.Row = vasID.ActiveRow
                vasID.Col = colBarcode
                If Not IsNumeric(vasID.Text) Then
                ...
                If Len(vasID.Text) > 4 Then
                    Exit Sub
                End If
                
                If GetWorkList_Barcode(Format(dtpDate.Value, "yyyymmdd"), vasID.Text, .ActiveRow) Then
                    '-- ·ÎÄÃ ÀúÀå
                    'Call SetLocalDB(gRow, .ActiveRow, "1")
                End If
