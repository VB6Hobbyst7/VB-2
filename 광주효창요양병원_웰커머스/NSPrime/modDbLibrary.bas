...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
       ...
       'VallsID = Val(lsID)
        ...
        strDate = Format(Trim(GetText(.vasID, argSpcRow, colOrdDate)), "yyyymmdd")
...
'gul_value 결과값
                      ...
                      SQL = "Update M01007TB1 "
                SQL = SQL & "   Set gul_value = '" & sResult & "'"
                SQL = SQL & " Where gul_date   = '" & strDate & "'" & vbCr
                SQL = SQL & "   and gul_bun_no   = '" & lsID & "'" & vbCr
                SQL = SQL & "   and gul_gum_code   = 'M006'" & vbCr
