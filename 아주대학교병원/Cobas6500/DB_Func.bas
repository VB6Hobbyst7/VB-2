...
Public gReadBuf(2000) As String
...
'            .ConnectionString = "driver={SQL Server};" & _
'                            "server= 61.252.130.66;" & _
'                            "uid= sa;" & _
'                            "pwd= \\\\\;" & _
'                            "database= shinbase"
    '.ConnectionString = "DSN=1;Uid=sa;Pwd=mate; "
    '.ConnectionString = "DSN=SINWON;Uid=sa;Pwd=\\\\\; "
        .Open
    End With
...
errFind:
    MsgBox "서버 연결 오류:(" & Err.Number & ")" & Err.Description
...
Public Function db_select_VasS(argServer As Integer, argSQL As String, ByVal argSpread As vaSpread, Optional argRow As Integer = 1, Optional argcol As Integer = 1) As Integer
'쿼리 실행 내용을 스프레드쉬트에 Display
    Dim i, j As Integer
    ...
    While Not rs.EOF
        ...
        For j = 0 To rs.Fields.Count - 1
            ...
            Else
                ...
                argSpread.Row = i
                argSpread.Col = j + argcol
                If IsNull(rs.Fields.Item(j).Value) = True Then
                    argSpread.Text = ""
                Else
                    argSpread.Text = Trim(CStr(rs.Fields.Item(j).Value)) & ""
                End If
...
Public Function db_select_ColS(argServer As Integer, argSQL As String) As Integer
'쿼리 실행 내용을 gReadbuf()의 Array에 저장
    Dim i, j As Integer
...
ErrHandle:
     'MsgBox Err.Number & " : " & Error(Err.Number), vbCritical
     Save_Raw_Data "ERR - SENDQUERY : " & vbCrLf & SQL
     SendQuery = -1
