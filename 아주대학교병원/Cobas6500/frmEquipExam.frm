...
'''    SQL = "SELECT EQUIPNO, EQUIPCODE, EXAMCODE, EXAMNAME, RESGUBUN, RESPREC, SEQNO, REPLOW, REPHIGH, " & vbCrLf & _
'''          "       REFLOW, LEQUIL, LSTRING, REFHIGH, HEQUIL, HSTRING, MSTRING" & vbCrLf & _
'''          "  FROM EQUIPEXAM " & vbCrLf & _
'''          " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf & _
'''          " GROUP BY EQUIPNO, SEQNO, EQUIPCODE, EXAMCODE, EXAMNAME, RESGUBUN, RESPREC, REPLOW, REPHIGH, REFLOW, LEQUIL, LSTRING, REFHIGH, HEQUIL, HSTRING, MSTRING"
    
    SQL = "SELECT EQUIPNO, EQUIPCODE, EXAMCODE, EXAMNAME, RESGUBUN, RESPREC, SEQNO, REPLOW, REPHIGH, " & vbCrLf & _
          "       REFLOW, LEQUIL, LSTRING, REFHIGH, HEQUIL, HSTRING, MSTRING, examtype, deltavalue, RESSEQNO" & vbCrLf & _
          "  FROM EQUIPEXAM " & vbCrLf & _
          " GROUP BY EQUIPNO, EQUIPCODE, SEQNO, EXAMCODE, EXAMNAME, RESGUBUN, RESPREC, REPLOW, REPHIGH, REFLOW, LEQUIL, LSTRING, REFHIGH, HEQUIL, HSTRING, MSTRING, examtype, deltavalue, RESSEQNO"
...
Function ExistOfEquipCode(asEquip As String, asEQUIPCODE As String, Optional asExamCode As String = "") As Integer
'장비코드와 검사코드에 해당하는 데이타 존재 확인 하는 procedure
    ...
    SQL = "SELECT EquipCode, ExamCode, ExamName " & vbCrLf & _
          "  From EquipExam " & vbCrLf & _
          " WHERE Equipno = '" & asEquip & "' and EquipCode = '" & asEQUIPCODE & "' "
...
'    db_BeginTran gServer
    ...
    SQL = "Delete from EquipExam " & vbCrLf & _
          "Where Equipno = '" & Trim(txtEquip) & "' " & vbCrLf & _
...
'        Exit Sub
    ...
    res = ExistOfEquipCode(Trim(txtEquip), Trim(txtEquipCode), Trim(txtExamCode))
    If res = 1 Then
        'update
        SQL = " Update EquipExam " & vbCrLf & _
              ...
              "     examtype = '" & chkORDER.Value & "', " & vbCrLf & _
              "     Mstring = '" & txtMidStr & "', " & vbCrLf & _
              "     RESSEQNO = '" & txtResSeq & "', " & vbCrLf & _
              "     deltavalue = '" & chkNaf.Value & "' " & vbCrLf & _
...
'
    ElseIf res = 0 Then
        ...
        SQL = " Insert Into EquipExam(Equipno, EquipCode, ExamCode, ExamName,  resgubun, resprec, seqno, replow, rephigh, " & vbCrLf & _
              ...
              "         '" & txtRefHigh & "', '" & intHEquil & "', '" & txtRefHighStr & "', '" & txtMidStr & "','" & chkORDER.Value & "','" & chkNaf.Value & "','" & txtResSeq & "') "
    End If
...
Private Sub vasList_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    If Trim(GetText(vasList, Row, 5)) = "" Then
        cboGubun.ListIndex = -1
    Else
        cboGubun.ListIndex = Trim(GetText(vasList, Row, 5))
    End If
    txtRang = Trim(GetText(vasList, Row, 6))
    txtSeq.Text = Trim(GetText(vasList, Row, 7))
    txtRepLow = Trim(GetText(vasList, Row, 8))
    txtRepHigh = Trim(GetText(vasList, Row, 9))
    
    txtRefLow = Trim(GetText(vasList, Row, 10))
    If GetText(vasList, Row, 11) = "1" Then
        optRefLow(1).Value = True
    Else
        optRefLow(0).Value = True
    End If
    txtRefLowStr = Trim(GetText(vasList, Row, 12))
    
    txtRefHigh = Trim(GetText(vasList, Row, 13))
    If GetText(vasList, Row, 14) = "1" Then
        optRefHigh(1).Value = True
    Else
        optRefHigh(0).Value = True
    End If
    txtRefHighStr = Trim(GetText(vasList, Row, 15))
    
    txtMidStr = Trim(GetText(vasList, Row, 16))
    ...
    txtResSeq = Trim(GetText(vasList, Row, 19))
