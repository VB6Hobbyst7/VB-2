...
Public Sub Worker_Info_Parsing(ByRef Nodes As MSXML.IXMLDOMNodeList, _
...
'''    If xDoc.Load(App.Path & "\Res" & "\userid.xml") Then
        ' 문서가 성공적으로 로드되었습니다.
        ' 이제 재미있는 작업을 수행합니다.
...
Public Sub Display_QCWorkList_Parsing(ByRef Nodes As MSXML.IXMLDOMNodeList, _
    ByVal Indent As Integer)
...
'''    Save_Raw_Data "[Save Result]" & sParam
    
    send = oSOAP.wsLISInterface(strDiv, sParam)
    
'''    Save_Raw_Data "[Save Result => Return]" & send
...
Public Function Get_Order(asSID) As Integer
    ...
    If Len(asSID) = 12 Then '연속검사인경우
        sRetStr = Get_Qry_OrderList_M(asSID)
    Else
        sRetStr = Get_Qry_OrderList(asSID)
    End If
...
'''    If xDoc.Load(App.Path & "\Res" & "\order.xml") Then
    If xDoc.loadXML(sRetStr) Then
        ...
        ' 이제 재미있는 작업을 수행합니다.
        If Len(asSID) = 12 Then '연속검사인경우
            Display_Order_Parsing_M xDoc.childNodes, 0
        Else
            Display_Order_Parsing xDoc.childNodes, 0
        End If
...
Public Function Get_Order1(asSID) As Integer
    Dim sRetStr As String
    ...
    'SaveXMLFile sRetStr
'''    Save_Xml_Data sRetStr, "order1"
    
    Dim xDoc As MSXML.DOMDocument
    
    Set xDoc = New MSXML.DOMDocument
    
'''    If xDoc.Load(App.Path & "\Res" & "\order1.xml") Then
    If xDoc.loadXML(sRetStr) Then
        ' 문서가 성공적으로 로드되었습니다.
        ' 이제 재미있는 작업을 수행합니다.
...
'XML File Parsing
Public Sub Display_Order_Parsing_0(ByRef Nodes As MSXML.IXMLDOMNodeList, _
    ByVal Indent As Integer)
...
Public Function Get_PatInfo(asSID) As Integer
    ...
    'SaveXMLFile sRetStr
    Save_Xml_Data sRetStr, "pat"
    
    Dim xDoc As MSXML.DOMDocument
    
    Set xDoc = New MSXML.DOMDocument
    
'''    If xDoc.Load(App.Path & "\Res" & "\pat.xml") Then
    If xDoc.loadXML(sRetStr) Then
        ' 문서가 성공적으로 로드되었습니다.
        ' 이제 재미있는 작업을 수행합니다.
...
Public Sub Display_PatInfo_Parsing(ByRef Nodes As MSXML.IXMLDOMNodeList, _
    ...
    For Each xNode In Nodes
        ...
        'If xNode.nodeType = NODE_ELEMENT Then
            Select Case xNode.parentNode.nodeName
            Case "PTNO":    gPatient_Info.PTNO = xNode.nodeValue
