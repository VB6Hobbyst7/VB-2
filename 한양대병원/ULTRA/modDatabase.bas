...
Public Function TmaxConnect() As Boolean
    ...
    '-- Tmax Read Env(IP)
    If tuxreadenv(ByVal envfile, ByVal svrcon) = -1 Then
        'ErrMsg = tuxerrdesc(gettperrno())
        'MsgBox "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg, vbOKOnly + vbCritical, "TMAX 에러"
        MsgBox svrcon & "read error ->" & tpstrerror(gettperrno)
        'MsgBox envfile & "read error ->" & tpstrerror(gettperrno)
        TmaxConnect = False
        Exit Function
    End If
    ...
    If tpinit(ByVal 0&) = -1 Then
        'ErrMsg = tuxerrdesc(gettperrno())
        'MsgBox "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg, vbOKOnly + vbCritical, "TMAX 에러"
        MsgBox envfile & "init error ->" & tpstrerror(gettperrno)
        TmaxConnect = False
        Exit Function
    End If
