...
Function SaveTransData_SY(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            Do Until RS_L.EOF
                ...
                If sResult <> "" Then
                    ...
                    'SQL = "Exec Interface_SetPatientResult02 '" & strHospDate & "'," & strBarcode & ",'" & mGetP(strOrdCd, 1, "|") & "','" & mGetP(strOrdCd, 2, "|") & "','" & mGetP(strOrdCd, 3, "|") & "','" & sResult & "','','',0,0,0,'IMM0001','" & strReturn & "'"
                    SQL = "Exec Interface_SetPatientResult02 '" & strHospDate & "'," & strBarcode & ",'" & mGetP(strOrdCd, 1, "|") & "','" & mGetP(strOrdCd, 2, "|") & "','" & mGetP(strOrdCd, 3, "|") & "','" & sResult & "','','',0,0,0,'" & gHOSP.MACHCD & "','" & sRcvData & "'"
                    AdoCn.Execute SQL
...
'
''-- 워크리스트 조회
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pFNo As String, Optional ByVal pTNo As String)

    Select Case gEMR
        Case "SY"
                Call GetWorkList_SY(pFrom, pTo, SPD, pFNo, pTNo)
...
Public Sub GetWorkList_SY(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pFNo As String, Optional ByVal pTNo As String)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    If pFNo = "" Or Not IsNumeric(pFNo) Then
        pFNo = "0"
    End If
    
    If pTNo = "" Or Not IsNumeric(pTNo) Then
        pTNo = "0"
    End If
    ...
    '   @TestCodes         varchar(200)         '+#13#10+    // 선택입력: 검사코드      :   gAllTestCd
    
    
    SQL = "Exec interface_GetPatientResultList02 '" & frmMain.cboChk.ListIndex & "','" & gHOSP.PARTCD & "','" & pFrom & "','" & pTo & "'," & Val(pFNo) & "," & Val(pTNo) & ",''"
    Call SetSQLData("워크조회", SQL, "")
    
    '-- Record Count 가져옴
    AdoCn.CursorLocation = adUseClient
    Set RS = AdoCn.Execute(SQL, sqlRet)
        ...
        Do Until RS.EOF
            With SPD
                For i = 1 To SPD.DataRowCnt
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colBARCODE)
                    If Trim(RS("LabRegDate")) = strHospDate And strBarcode = Trim(RS.Fields("LabRegDate")) & PedLeftStr(Trim(RS.Fields("LabRegNo")), 5, "0") Then
                        blnSame = True
                        strItems = GetText(SPD, intRow, colITEMS)
                        strItems = strItems & GetTestNmS(Trim(RS.Fields("OrderCode")))
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, Trim(RS.Fields("LabRegDate")) & PedLeftStr(Trim(RS.Fields("LabRegNo")), 5, "0"), intRow, colBARCODE
                    ...
                    SetText SPD, Trim(RS.Fields("CompanyCode")) & "", intRow, colDEPT
                    SetText SPD, Trim(RS.Fields("PatientSex")) & "", intRow, colPSEX
                    SetText SPD, Trim(RS.Fields("PatientAge")) & "", intRow, colPAGE
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_SY" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "SY"
            SaveTransData = SaveTransData_SY(argSpcRow, SPD)
...
On Error GoTo ErrHandle
    ...
    SQL = "Exec Interface_GetPatientResult02 '" & gHOSP.PARTCD & "','" & strRegDate & "','" & lngRegNo & "'"
    Call SetSQLData("바코드조회", SQL, "")
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        Do Until RS.EOF
            With SPD
                ...
                SetText SPD, Trim(RS.Fields("LabRegDate")) & PedLeftStr(Trim(RS.Fields("LabRegNo")), 5, "0"), asRow, colBARCODE
                ...
                SetText SPD, Trim(RS.Fields("CompanyCode")) & "", asRow, colDEPT
                ...
                '환자 성별/나이
                With mPatient
                    .AGE = Trim(RS.Fields("PatientAge")) & ""
                    .SEX = Trim(RS.Fields("PatientSex")) & ""
                End With
                
                      SQL = " SELECT OrderCode, TestCode, TestSubCode " & vbCrLf
                SQL = SQL & "   FROM LC13_DIOGENE..LabRegResult " & vbCrLf
                ...
                If Not RS1.EOF = True And Not RS1.BOF = True Then
                    Do Until RS1.EOF
                        '-- 화면에 표시
                        For intCol = colSTATE + 1 To .MaxCols
                            If GetTestNm(Trim(RS.Fields("TestSubCode"))) = gArrEQPNm(intCol - colSTATE, 6) Then
                                .Row = asRow
                                .Col = intCol
                                .BackColor = vbYellow
                                Call SetText(SPD, "◇", asRow, intCol)
                                ...
                                '-- 처방코드
                                gArrEQP(intCol - colSTATE, 16) = Trim(RS1.Fields("OrderCode")) & "|" & Trim(RS1.Fields("TestCode")) & "|" & Trim(RS1.Fields("TestSubCode"))
                                
                                Exit For
                            End If
                        Next
                        gPatOrdCd = gPatOrdCd & "'" & Trim(RS.Fields("TestSubCode")) & "',"
                    Loop
                End If
            End With
            DoEvents
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_SY" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
