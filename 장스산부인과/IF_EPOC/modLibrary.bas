...
Public Sub SetRawData(argSQL As String)
    ...
    If Dir(App.PATH & "\Log\Temp\", vbDirectory) <> "Log" Then
        MkDir (App.PATH & "\Log")
    End If
    
    sFileName = gHOSP.MACHNM & "_" & Format(CDate(frmMain.dtpToday), "yyyy-mm-dd")
    
    Open App.PATH & "\Log\Temp\" & sFileName & ".txt" For Append As FilNum
    Print #FilNum, argSQL
    Close FilNum
...
Public Sub SetSQLData(ByVal strName As String, ByVal argSQL As String, Optional ByVal argMode As String)
    ...
    If argMode = "A" Then
        Open App.PATH & "\Log\Temp\" & sFileName & ".txt" For Append As FilNum
    Else
        Open App.PATH & "\Log\Temp\" & sFileName & ".txt" For Output As FilNum
    End If
    Print #FilNum, argSQL
    Close FilNum
