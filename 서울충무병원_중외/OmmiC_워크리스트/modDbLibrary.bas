...
'                SQL = "Update " & gDB_Parm.DB & "..jun370_resulttb" & _
                ...
                SQL = SQL & vbCrLf & "   And LabCode = 'BLOODGAS' " 'LABCODE = 'BLOODGAS'
...
'''
'''        If Trim(gReadBuf(3)) = "62" Then
'''            SetText frmInterface.vasID, "검진", asRow, colSex
'''        Else
'''            SetText frmInterface.vasID, "진료", asRow, colSex
'''        End If
'''        GetSampleInfoW = 1
'''    Else
'''        GetSampleInfoW = -1
'''    End If
    ...
    SQL = SQL & vbCrLf & "   AND ORDERCODE IN (" & gAllExam & ") " & vbLf
...
''    End If
    ...
    If Res = 1 Then
        ...
        SetText frmInterface.vasID, Trim(RS.Fields("RECEIPTNO")), asRow, colOrdDate
        SetText frmInterface.vasID, Trim(RS.Fields("RECEIPTDATE")) & "", asRow, colBarcode
        SetText frmInterface.vasID, Trim(RS.Fields("PTNO")), asRow, colPID
        SetText frmInterface.vasID, Trim(RS.Fields("SPECIMENNUM")), asRow, colPName
        SetText frmInterface.vasID, Trim(RS.Fields("SNAME")), asRow, colPName
        SetText frmInterface.vasID, IIf(Trim(RS.Fields("IPDOPD")) = 1, "입원", "외래"), asRow, colPName
        SetText frmInterface.vasID, Trim(RS.Fields("AGE")), asRow, colPName
