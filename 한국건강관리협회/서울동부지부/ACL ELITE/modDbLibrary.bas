...
'        Call SetSQLData("결과저장조회", SQL)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                '-- 결과업데이트 쿼리
                SQL = ""
                SQL = SQL & "Update TWMED_RESULT1 Set"
                SQL = SQL & " Result    = '" & sResult & "'" & vbCr         '검사결과
                SQL = SQL & ",ACTSTATUS = '7'" & vbCr                       '상태
                SQL = SQL & " Where SAMPLENO = '" & lsID & "'" & vbCr       '바코드
                SQL = SQL & "   And ITEMCODE = '" & strExamCd & "'" & vbCr  '검사코드
                SQL = SQL & "   And ACTSTATUS < '8' "
...
'    End If
    ...
    Call SetSQLData("gOrderExam", gOrderExam)
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_KAHP(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
...
'On Error Resume Next
    ...
    '-- 테이블 사용
    SQL = ""
    SQL = SQL & "Select DISTINCT R.SEQNO,  R.ITEMCODE as ITEMCODE,  R.JUMIN as 주민번호,  R.SAMPLENO as 바코드번호,  R.JDATE as 접수일자,  J.Name as 이름,  J.AGE as 나이,  J.SEX as 성별" & vbCr
    SQL = SQL & "  From TWMED_RESULT1 R, TWMED_KEYTBL  J" & vbCr
    SQL = SQL & "  where R.JDATE    = J.JDATE " & vbCr
    ...
    SQL = SQL & " From TWMED_RESULT1 R"
    SQL = SQL & "    where R.SampleNo = '" & sBarcode & "'" & vbCr
    SQL = SQL & "    and R.ITEMCODE in (" & gAllExam & ")" & vbCr

'Select DISTINCT J.Name as 이름,  J.AGE as 나이,  J.SEX as 성별
'  From TWMED_KEYTBL  J
'  where J.CORPCODE = 'AIN00001'
'    and J.SEQNO =168
'    and J.JUMIN ='6009122167424'
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("ITEMCODE")) & "" & "',"
                
                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                'MsgBox Trim(RS.Fields("접수일자")) & ""
                SetText .vasID, Trim(RS.Fields("JDATE")) & "", asRow, colHOSPDATE
                'MsgBox Trim(RS.Fields("이름")) & ""
               ' SetText .vasID, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                SetText .vasID, Trim(RS.Fields("SAMPLENO")) & "", asRow, colBARCODE
                'SetText .vasID, Trim(RS.Fields("성별")) & "", asRow, colPSEX
                'SetText .vasID, Trim(RS.Fields("나이")) & "", asRow, colPAGE
               ...
               ' SetText .vasID, Trim(RS.Fields("챠트번호")) & "", asRow, colCHARTNO
                'SetText .vasID, Trim(RS.Fields("내원번호")) & "", asRow, colPID
               ' SetText .vasID, Trim(RS.Fields("검체명")) & "", asRow, colSPCNM
                SetText .vasID, Trim(RS.Fields("SEQNO")) & "", asRow, colPAGE
                ...
                '-- 화면에 표시
                For intCol = colState + 1 To .vasID.MaxCols
                    If Trim(RS.Fields("ITEMCODE") & "") = gArrEquip(intCol - colState, 3) Then
                        .vasID.Row = asRow
                        .vasID.Col = intCol
                        .vasID.BackColor = vbYellow
                        Exit For
                ...
                Next
                      ...
                      SQL = " Select J.* " 'DISTINCT J.Name as 이름,  J.AGE as 나이,  J.SEX as 성별"
                SQL = SQL & "   From TWMED_KEYTBL  J"
                SQL = SQL & "  where J.CORPCODE = '" & Trim(RS.Fields("CORPCODE")) & "'"
                SQL = SQL & "    and J.SEQNO    = " & Trim(RS.Fields("SEQNO"))
                SQL = SQL & "    and J.JUMIN    = '" & Trim(RS.Fields("JUMIN")) & "'"
                ...
                If Not RS1.EOF = True And Not RS1.BOF = True Then
                    Do Until RS1.EOF
                        SetText .vasID, Trim(RS1.Fields("Name")) & "", asRow, colPNAME
                        SetText .vasID, Trim(RS1.Fields("Sex")) & "", asRow, colPSEX
                        SetText .vasID, Trim(RS1.Fields("Age")) & "", asRow, colPAGE
                        RS1.MoveNext
                    Loop
                End If
                RS1.Close
