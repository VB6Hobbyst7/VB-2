...
Public Function sl_sel_acptdte_acptno_all&(i_s_acpt_dte$, i_e_acpt_dte$, i_tst_frct_cd$, i_tst_cd$, i_s_acptno$, i_e_acptno$, i_equip_cd$, i_spc_no$, v_spc_no$(), v_pt_no$(), v_pt_nm$(), v_tst_frct_cd$(), v_tst_frct_nm$(), v_acpt_dte$(), v_acpt_no$(), v_sex$(), v_birth$(), v_age$(), v_spc_cd$(), v_spc_nm$(), v_tst_cd$(), v_tst_nm$())
    Dim dce_table As Long, Socket As Integer

    Call dce_checkver(2, 0)
    Socket = dce_findserver("sl_d_m01")
    If (Socket > -1) Then
        rv& = dce_push_string(Socket, "i_s_acpt_dte", i_s_acpt_dte)
        rv& = dce_push_string(Socket, "i_e_acpt_dte", i_e_acpt_dte)
        rv& = dce_push_string(Socket, "i_tst_frct_cd", i_tst_frct_cd)
        rv& = dce_push_string(Socket, "i_tst_cd", i_tst_cd)
        rv& = dce_push_string(Socket, "i_s_acptno", i_s_acptno)
        rv& = dce_push_string(Socket, "i_e_acptno", i_e_acptno)
        rv& = dce_push_string(Socket, "i_equip_cd", i_equip_cd)
        rv& = dce_push_string(Socket, "i_spc_no", i_spc_no)
        dce_table = dce_submit("sl_d_m01", "sl_sel_acptdte_acptno_all", Socket)
    End If
    sl_sel_acptdte_acptno_all = dce_pop_long(dce_table, "dce_result")
    rv& = dce_pop_array(dce_table, "v_spc_no", v_spc_no())
    rv& = dce_pop_array(dce_table, "v_pt_no", v_pt_no())
    rv& = dce_pop_array(dce_table, "v_pt_nm", v_pt_nm())
    rv& = dce_pop_array(dce_table, "v_tst_frct_cd", v_tst_frct_cd())
    ...
    rv& = dce_pop_array(dce_table, "v_acpt_no", v_acpt_no())
    rv& = dce_pop_array(dce_table, "v_sex", v_sex())
    rv& = dce_pop_array(dce_table, "v_birth", v_birth())
    rv& = dce_pop_array(dce_table, "v_age", v_age())
    rv& = dce_pop_array(dce_table, "v_spc_cd", v_spc_cd())
    rv& = dce_pop_array(dce_table, "v_spc_nm", v_spc_nm())
    rv& = dce_pop_array(dce_table, "v_tst_cd", v_tst_cd())
...
Public Function sl_sel_slip_list&(v_slip_cd$(), v_slip_nm$())
    Dim dce_table As Long, Socket As Integer

    Call dce_checkver(2, 0)
    Socket = dce_findserver("sl_d_m01")
    If (Socket > -1) Then
        dce_table = dce_submit("sl_d_m01", "sl_sel_slip_list", Socket)
    End If
    sl_sel_slip_list = dce_pop_long(dce_table, "dce_result")
    rv& = dce_pop_array(dce_table, "v_slip_cd", v_slip_cd())
    rv& = dce_pop_array(dce_table, "v_slip_nm", v_slip_nm())
    Call dce_table_destroy(dce_table)
End Function


Public Function sl_sel_slip_tstcd_list&(i_slip_cd$, v_tst_cd$(), v_tst_snm$(), v_tst_nm$(), v_dis_seq$())
    Dim dce_table As Long, Socket As Integer

    Call dce_checkver(2, 0)
    Socket = dce_findserver("sl_d_m01")
    If (Socket > -1) Then
        rv& = dce_push_string(Socket, "i_slip_cd", i_slip_cd)
        dce_table = dce_submit("sl_d_m01", "sl_sel_slip_tstcd_list", Socket)
    End If
    sl_sel_slip_tstcd_list = dce_pop_long(dce_table, "dce_result")
    rv& = dce_pop_array(dce_table, "v_tst_cd", v_tst_cd())
    rv& = dce_pop_array(dce_table, "v_tst_snm", v_tst_snm())
    rv& = dce_pop_array(dce_table, "v_tst_nm", v_tst_nm())
    rv& = dce_pop_array(dce_table, "v_dis_seq", v_dis_seq())
    Call dce_table_destroy(dce_table)
End Function
