...
Type PatInfo_Select
    ...
    Age         As String
    CAUTION_YN  As String       '2011.10.31 이상은 추가
    OK          As Integer
    PATSECT     As String       '2012.04.25 오세원 추가
End Type
...
'        SaveXML_Data strErrText
    ...
    Else
        ...
        If gTLA_Info_Select(0).OK = 1 Then
            Online_TLA = 1
        Else
            Online_TLA = 0
        End If
...
Public Sub display_online_parsing_PatInfo(ByRef Nodes As MSXML.IXMLDOMNodeList, _
    ...
    For Each xNode In Nodes
        ...
        If xNode.nodeType = 4 Then
            ...
            Case "CAUTION_YN"
                gPat_Info_Select.CAUTION_YN = xNode.nodeValue
            Case "PATSECT"
...
ErrHandle:
    If oSOAP.FaultString <> "" Then
        SaveXML_Data "[Error Result]" & Format(Time, "hh:nn:ss") & "[SOAP]" & oSOAP.FaultString & vbCrLf & oSOAP.Detail & vbCrLf
    End If
    If Trim(Err.Description) <> "" Then
        SaveXML_Data "[Error Result]" & Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf
    End If
End Function
