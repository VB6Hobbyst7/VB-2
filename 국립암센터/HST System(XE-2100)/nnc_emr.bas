...
Public Const gXml_S08 = "PG_SRL.SLP91_S08"      '처방정보-검증된 항목도 볼 수 있음
...
Public Function Online_XML_Qry(ByVal asStrDiv As String, ByVal asParam As String) As String
    ...
    SaveXML_Data "[Use Proc => " & strDiv & " ]" & sParam, "IPU1_Info"
    send = oSOAP.wsLISInterface(strDiv, sParam)
    SaveXML_Data "[Return Proc => " & strDiv & " ]" & send, "IPU1_Info"
    Online_XML_Qry = send
    Set oSOAP = Nothing
    DoEvents
    Exit Function
...
Public Function Online_Result_Qry(ByVal asParam As String) As String
    ...
    SaveXML_Data "[Save Result]" & sParam, "IPU1_Res"
    send = oSOAP.wsLISInterface(strDiv, sParam)
    SaveXML_Data "[Save Result => Return]" & send, "IPU1_Res"
    Online_Result_Qry = send
    Set oSOAP = Nothing
    DoEvents
    Exit Function
...
ErrHandle:
    If oSOAP.FaultString <> "" Then
        SaveXML_Data "[Error Result]" & Format(Time, "hh:nn:ss") & "[SOAP]" & oSOAP.FaultString & vbCrLf & oSOAP.Detail & vbCrLf, "IPU1_ERR"
    End If
    If Trim(Err.Description) <> "" Then
        SaveXML_Data "[Error Result]" & Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf, "IPU1_ERR"
    End If
End Function

Public Function Online_TLA_Qry(ByVal asParam As String) As String
    Dim oSOAP As MSSOAPLib30.SoapClient30
    Dim strDiv As String
    Dim send As String
    Dim sParam As String
    ...
    strDiv = "PG_SRL.SLP91_U02"
    
    sParam = asParam
    
    SaveXML_Data "[Save Result]" & sParam, "IPU1_Update"
    send = oSOAP.wsLISInterface(strDiv, sParam)
    SaveXML_Data "[Save Result => Return]" & send, "IPU1_Update"
    Online_TLA_Qry = send
    Set oSOAP = Nothing
    DoEvents
    Exit Function
...
ErrHandle:
    If oSOAP.FaultString <> "" Then
        SaveXML_Data "[Error Result]" & Format(Time, "hh:nn:ss") & "[SOAP]" & oSOAP.FaultString & vbCrLf & oSOAP.Detail & vbCrLf, "IPU1_Update_Err"
    End If
    If Trim(Err.Description) <> "" Then
        SaveXML_Data "[Error Result]" & Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf, "IPU1_Update_Err"
    End If
End Function
...
'데이터 저장=================================================================================================================
Public Sub SaveXML_Data(argSQL As String, argFileName As String)
'argSQL의 내용을 파일로 저장
    Dim FilNum
    ...
    'Open App.Path & "\XML" & "\" & argFileName & ".log" For Append As FilNum
