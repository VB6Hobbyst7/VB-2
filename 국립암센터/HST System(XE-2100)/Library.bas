...
Public Sub SaveQuery(argSQL As String)
'argSQLÀÇ ³»¿ëÀ» ÆÄÀÏ·Î ÀúÀå
    Dim FilNum
    
    FilNum = FreeFile
    
    Open App.Path & "\Log\qry" & Date & ".txt" For Append As FilNum
    Print #FilNum, argSQL
    Close FilNum
End Sub
...
'argSQLÀÇ ³»¿ëÀ» ÆÄÀÏ·Î ÀúÀå
    ...
    Open App.Path & "\Log\Res" & Date & ".log" For Append As FilNum
...
Public Sub CalSexAge(ByRef asPNRN As String, ByRef asCurDate As String)
    ...
    If IsNumeric(Mid(asPNRN, 3, 2)) = True Then
        If CInt(Mid(asPNRN, 3, 2)) > 0 And CInt(Mid(asPNRN, 3, 2)) < 13 Then
    ...
    Else
        sBirth = sBirth & "/01"
    End If
    If IsNumeric(Mid(asPNRN, 5, 2)) = True Then
        If CInt(Mid(asPNRN, 5, 2)) > 0 And CInt(Mid(asPNRN, 5, 2)) < 32 Then
...
Public Sub ClearGraph(ByRef ChartFX As Object)
'ChartFXÀÇ Data¸¦ ClearÇÑ´Ù
    
    ChartFX.OpenDataEx COD_VALUES, 1, 1
    ChartFX.Axis(AXIS_Y).Max = 1
    ChartFX.Axis(AXIS_Y).Min = 0
    ChartFX.ThisSerie = 0
    ChartFX.Value(1) = 1E+308
...
Public Function Conv_Kor_Eng(ByVal asName As String) As String
    ...
    For i = 1 To Len(asName)
        sSubName = hantoeng(Mid(asName, i, 1))
        
        If Trim(sName) = "" Then
            sName = sName & sSubName
        Else
            sName = sName & " " & sSubName
        End If
