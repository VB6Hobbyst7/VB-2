...
Private Sub vasWorkList_DblClick(ByVal Col As Long, ByVal Row As Long)
    ...
    With vasWorkList
        ...
        If pGrid_Point = 0 Then
            pGrid_Point = SeqNullSearch(frmInterface.vasID, Trim(.Text), colCHARTNO)
            If pGrid_Point = 0 Then
                frmInterface.vasID.MaxRows = frmInterface.vasID.MaxRows + 1
            ...
            Else
                ...
                lsID = Trim(GetText(frmInterface.vasID, pGrid_Point, colBARCODE))
                
                
                '-- 바코드 번호가 이전과 틀리다면 업데이트
                      SQL = "UPDATE PATRESULT SET"
                SQL = SQL & " HOSPDATE = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colHOSPDATE)) & "' " & vbCrLf
                SQL = SQL & ",BARCODE = '" & lsID & "' " & vbCrLf
                SQL = SQL & ",CHARTNO = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colCHARTNO)) & "' " & vbCrLf
                SQL = SQL & ",PID = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colPID)) & "' " & vbCrLf
                SQL = SQL & ",PNAME = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colPNAME)) & "' " & vbCrLf
                SQL = SQL & ",INOUT = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colINOUT)) & "' " & vbCrLf
                SQL = SQL & ",PSEX = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colPSEX)) & "' " & vbCrLf
                SQL = SQL & ",PAGE = '" & Trim(GetText(frmInterface.vasID, pGrid_Point, colPAGE)) & "' " & vbCrLf
                SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf
                SQL = SQL & "   AND SAVESEQ = " & Trim(GetText(frmInterface.vasID, pGrid_Point, colSAVESEQ)) & vbCrLf
