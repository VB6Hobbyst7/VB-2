...
'    SQL = SQL & "     , O.StatFg                        " & vbCr
    ...
    SQL = SQL & "   AND ISNULL(O.CANCELFG, '0') = '0'"
    SQL = SQL & "   AND (O.VFYDT = '' OR O.VFYDT IS NULL)                       " & vbCr
    SQL = SQL & "   AND T.TESTCD NOT IN (SELECT R.TESTCD                          " & vbCr
    SQL = SQL & "                         FROM TC206 R                          " & vbCr
    SQL = SQL & "                        WHERE R.SPCNO  = O.SPCNO)              " & vbCr
    'SQL = SQL & "                          AND R.TESTCD IN (" & gAllTestCd & ")) " & vbCr
    SQL = SQL & "   AND O.STSCD > 1                                             " & vbCr
    SQL = SQL & "   AND T.STSCD < 5                                             " & vbCr
...
'    SQL = SQL & "      , COUNT(R.RESSHTNAM)         AS CNT                      " & vbCr
...
'    SQL = SQL & " GROUP BY R.RESACPDTM, R.RESCHTNUM, P.PBSPATNAM, R.RESSPMNUM   "
