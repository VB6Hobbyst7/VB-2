    ...
    If KeyAscii = vbKeyReturn Then
        If txtReelBarcode.Text <> "" Then
            ...
            If AdoRs.RecordCount = 0 Then
                ...
                Else
                    'strBarData = ""
                    lblLotNo2.Visible = True
                    txtLotNo2.Visible = True
                    Do Until pAdoRS1.EOF
                        txtLotNo2.Text = pAdoRS1.Fields("PROD_LOT_NO").Value & ""
                        txtMsg.Text = txtMsg.Text & txtReelBarcode.Text & "는 Lot No가 다릅니다." & vbCrLf

                        spdScan.MaxRows = spdScan.MaxRows + 1
                        Call SetText(spdScan, txtReelBarcode.Text, spdScan.MaxRows, 1)
                        Call SetText(spdScan, pAdoRS1.Fields("PROD_REEL_BAR").Value & "", spdScan.MaxRows, 2)
                        Call SetText(spdScan, pAdoRS1.Fields("PROD_PP_BAR").Value & "", spdScan.MaxRows, 3)
                        Call SetText(spdScan, pAdoRS1.Fields("PROD_ICE_BAR").Value & "", spdScan.MaxRows, 4)
                        Call SetText(spdScan, pAdoRS1.Fields("PROD_PP_BAR_IN").Value & "", spdScan.MaxRows, 5)
                        Call SetText(spdScan, pAdoRS1.Fields("PROD_ICE_BAR_IN").Value & "", spdScan.MaxRows, 6)
                ...
                Select Case lblstrPrtLabelName.Caption
                    Case "P0001"
                        ...
                        If GetText(spdPrtReelDetail, i, 1) = "Total Quantity/Length" Then
                            ...
                            End If
                            strContents = CCur(strASReelCnt) & "Reels/" & CCur(strASReelCnt) * 100 & "cm"
                        ...
                        If GetText(spdPrtReelDetail, i, 1) = "바코드" Then
                            ...
                            strContents = strProdLen & Format(CCur(strASReelCnt) * 100, "00000")
                            Call SetText(spdPrtReelDetail, strContents, i, 3)
                        End If
                    Case "P0002"
                        '제품수량/길이
                        If GetText(spdPrtReelDetail, i, 1) = "Total Quantity/Length" Then
                            ...
                            Else
                                strASReelCnt = txtPPMaxTot.Text
                            End If
                            strContents = CCur(strASReelCnt) & "Reels/" & CCur(strASReelCnt) * 200 & "cm"
                            Call SetText(spdPrtReelDetail, strContents, i, 3)
                        End If
                        '바코드
                        If GetText(spdPrtReelDetail, i, 1) = "바코드" Then
                            strProdLen = GetText(spdPrtReelDetail, i, 3)
                            strProdLen = Mid(strProdLen, 1, 17)
                            strContents = strProdLen & Format(CCur(strASReelCnt) * 200, "00000")
                            Call SetText(spdPrtReelDetail, strContents, i, 3)
                        End If
                    Case "P0003"
                        '제품수량/길이
