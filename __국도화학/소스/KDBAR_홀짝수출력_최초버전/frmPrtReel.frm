    ...
    Select Case lblstrPrtLabelName.Caption
        Case "P0001", "P0002"
            ...
            For intPRow = 1 To spdRegOrderDetail.MaxRows
                ...
                'intPrintOddCnt = intPRow
                'intPrintEvenCnt = intPRow
                    ...
                    With spdPrtReelDetail
                        For intRow = 1 To .MaxRows
                            ...
                            If strType = "바코드" Then
                                ...
                                strData = Mid(strData, 1, 12) & strLotSub & Mid(strData, 17, 5)
...
'                    intPrtCnt = intPrtCnt + 1
                ...
                '마지막번호가 홀수일 경우 다음 번호를 맞추기 위해 짝수를 하나 더 찍는다.
                If intPrtTNo Mod 2 <> 0 Then
'                    ReDim Preserve strPrint(intPrtCnt) As Variant
'                    strPrint(intPrtCnt) = strHeader & "^XZ" & vbLf
'                    intPrtCnt = intPrtCnt + 1
                End If
                    ...
                    With spdPrtReelDetail
                        For intRow = 1 To .MaxRows
                            ...
                            If strType = "바코드" Then
                                ...
                                strLotSub = "0" & Mid(strPrtFNo, 2, 1) & Format(intPrt, "00")
...
'                For intPrtCnt = 0 To UBound(strPrint)
'                    comEqp.Output = strPrint(intPrtCnt)
'                    Call SetPrtData("REEL" & "_" & txtProdNm.Text & "_" & txtProdLen.Text, "[출력시간 : " & Format(Now, "yyyy-mm-dd hh:mm:ss.nnn") & "]" & vbCrLf & strPrint(intPrtCnt), "A")
'                Next
'            End If
...
'            End If
    ...
    If UBound(strPrint) > 0 Then
        ...
        For intCount = 0 To UBound(strPrint) + 2 'Step 2
            ...
            frmProgress.lblProgress.Caption = " 출력중 [전체수량 / 현재수량]  " & CCur(txtReelQTY.Text) & " / " & intCount + 1
            DoEvents
...
'''        '홀수출력
'''        For intCount = 0 To UBound(strPrint) Step 2
'''            If intCount + 1 > CCur(txtReelQTY.Text) Then
'''                Exit For
'''            End If
'''            comEqp.Output = strPrint(intCount)
...
'''            '-- 프로그레스바 진행
'''            frmProgress.Xprog.Value = intCount
'''            frmProgress.lblProgress.Caption = "홀수번호 출력중 " & CCur(txtReelQTY.Text) & " / " & intCount + 1
'''            DoEvents
'''
'''        Next
'''
'''        '짝수출력
'''        For intCount = 1 To UBound(strPrint) Step 2
'''            If intCount + 1 > CCur(txtReelQTY.Text) Then
'''                Exit For
'''            End If
'''            comEqp.Output = strPrint(intCount)
...
'''            '-- 프로그레스바 진행
'''            frmProgress.Xprog.Value = intCount
'''            frmProgress.lblProgress.Caption = "짝수번호 출력중 " & CCur(txtReelQTY.Text) & " / " & intCount + 1
'''            DoEvents
'''
'''        Next
'''        'End If
