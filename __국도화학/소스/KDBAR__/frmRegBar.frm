...
Begin VB.Form frmRegBar 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Frame Frame3 
         ...
         Top             =   300
         Width           =   13245
         Begin VB.TextBox txtTempMasterGubun 
            Height          =   375
            Left            =   6810
            TabIndex        =   43
            Top             =   3870
      ...
      Begin FPSpread.vaSpread spdRegBar 
         ...
         SpreadDesigner  =   "frmRegBar.frx":0D26
         ScrollBarTrack  =   3
         ShowScrollTips  =   3
      End
   End
...
Private Sub SetBar()
    ...
    Else
        gBarMaster.YN = "N"
    End If
    gBarMaster.TEMPGU = mGetP(cboGubun.Text, 2, "|")
    ...
    Else
        Do Until AdoRs.EOF
            With spdRegBar
                ...
                Call SetText(spdRegBar, AdoRs.Fields("TEMP_MASTER_GU").Value & "", .MaxRows, 8)
                Call SetText(spdRegBar, AdoRs.Fields("BAR_TYPE").Value & "", .MaxRows, 9)
...
' 바코드 리스트 가져옴
Private Sub GetBarDetail(ByVal pProdBarCd As String, ByVal pTempGu As String)
    Dim strLabelType    As String
    ...
    Else
        Do Until AdoRs.EOF
            With spdRegBarDetail
                ...
                strItemName = Get_TempMaster_Name1(pTempGu, AdoRs.Fields("BAR_ITEM_NO").Value & "")
...
Private Sub Form_Load()
    ...
    If UCase(gKUKDO.USERID) = "ADMIN" Then
...
Private Sub spdRegBar_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    For i = 0 To cboProdLabelCd.ListCount
        If Mid(cboProdLabelCd.List(i), 1, 5) = Mid(GetText(spdRegBar, Row, 7), 1, 5) Then
            cboProdLabelCd.ListIndex = i
            Exit For
        End If
    Next
    ...
    strTempMasterGu = GetText(spdRegBar, Row, 8)
    
    For i = 0 To cboGubun.ListCount
        If Trim(mGetP(cboGubun.List(i), 2, "|")) = GetText(spdRegBar, Row, 8) Then
            cboGubun.ListIndex = i
            Exit For
        End If
    Next
    
    For i = 0 To cboBarType.ListCount
        If Mid(cboBarType.List(i), 1, 1) = GetText(spdRegBar, Row, 9) Then
            cboBarType.ListIndex = i
            Exit For
        End If
    Next
    ...
    Call GetBarDetail(strBarCd, strTempMasterGu)
