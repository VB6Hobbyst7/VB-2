...
'      -- 테이블 사용
          SQL = "SELECT DISTINCT a.RECEIPTDATE as 접수일자, a.SPECIMENNUM as 바코드번호, a.IPDOPD, a.RECEIPTNO as 챠트번호, a.PTNO as 내원번호, a.SNAME as 이름, b.LABCODE as ITEM, a.ORDERCODE, a.STAT "
...
'                If strER = "1" Then
'                    SetText .vasID, "1", .vasID.MaxRows, colER
'                    mOrder.STAT = "S"
'                Else
'                    SetText .vasID, "0", .vasID.MaxRows, colER
'                    mOrder.STAT = "R"
'                End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfoWorkList_JWINFO(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    ...
    GetSampleInfoWorkList_JWINFO = -1
    strORQN = ""
    gOrderExam = ""
    
    sBarcode = Trim(GetText(frmWorkList.vasID, asRow, colWBARCODE))
...
'                strER = Trim(RS.Fields("STAT")) & ""
'                If strER = "1" Then
'                    SetText .vasID, "1", .vasID.MaxRows, colWER
'                    mOrder.STAT = "S"
'                Else
'                    SetText .vasID, "0", .vasID.MaxRows, colWER
'                    mOrder.STAT = "R"
'                End If
                                    
'                SetText .vasID, "1", .vasID.MaxRows, colWCheckBox
'                SetText .vasID, Trim(RS.Fields("접수일자")) & "", asRow, colWHOSPDATE
'                SetText .vasID, Trim(RS.Fields("바코드번호")) & "", asRow, colWBARCODE
'                SetText .vasID, Trim(RS.Fields("챠트번호")) & "", asRow, colWCHARTNO
'                SetText .vasID, Trim(RS.Fields("내원번호")) & "", asRow, colWPID
'                SetText .vasID, Trim(RS.Fields("이름")) & "", asRow, colWPNAME
'                SetText .vasID, Trim(RS.Fields("ORDERCODE")) & "", asRow, colWPSEX   'ORDERCODE
'                SetText .vasID, IIf(Trim(RS.Fields("IPDOPD")) = 1, "입원", "외래"), asRow, colWINOUT
...
'                Next
    ...
    frmWorkList.vasID.RowHeight(-1) = 12
