...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        strDate = Trim(GetText(.vasID, argSpcRow, colHOSPDATE))
...
'                    SQL = SQL & "   And OKFL <> 'Y' "   '-- 결과확정유무
                    '-- 결과입력
                          SQL = "Update HIB70..ME_LABDAT Set"
                    ...
                    SQL = SQL & " Where LABATTEND = '" & lsPid & "'"
                    SQL = SQL & "   And LABCHTNUM = '" & strChtNum & "'"
                    SQL = SQL & "   And LABODRDTE = '" & strDate & "'"
                    SQL = SQL & "   And LABENDDEP <= '3'"
                    SQL = SQL & "   And LABODRCOD = '" & strEqpCd & "'"
        ...
        SQL = SQL & "       DATENDDEP = '3'"
        SQL = SQL & " Where DATATTEND = '" & lsPid & "'"
        SQL = SQL & "   And DATCHTNUM = '" & strChtNum & "'"
        SQL = SQL & "   And DATODRDTE = '" & strDate & "'"
        SQL = SQL & "   And DATODRCOD = '" & strEqpCd & "'"
        SQL = SQL & "   And DATENDDEP <= '3'"
                    
        Call SetSQLData("상태변경", SQL)
        Res = SendQuery(gServer, SQL)
