...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                '-- 결과입력
                      SQL = "Update HIB70..ME_LAB Set"
                SQL = SQL & "       LABRESULT = '" & sResult & "', "
                SQL = SQL & "       LABENDDEP = '3',"
                SQL = SQL & "       LABRSTDTE ='" & Format(Now, "YYYY-MM-DD") & "', "
                SQL = SQL & "       LABRSTTIM ='" & Format(Now, "hh:mm:ss") & "', "
                SQL = SQL & "       LABRSTUID ='" & gUserID & "', "
                SQL = SQL & "       LABRSTCOM ='" & gComName & "' "
                SQL = SQL & " Where LABATTEND = '" & strInNum & "'"
                SQL = SQL & "   And LABCHTNUM = '" & lsID & "'"
                SQL = SQL & "   And LABODRDTE = '" & strDate & "'"
                SQL = SQL & "   And LABORDDEP <= '3'"
                ...
                '-- 상태업데이트
                      SQL = "Update HIB70..ME_DAT Set"
                SQL = SQL & "       DATENDDEP = '3'"
                SQL = SQL & " Where DATCHTNUM = '" & lsID & "'"
                SQL = SQL & "   And DATODRDTE = '" & strDate & "'"
                SQL = SQL & "   And DATKEYNUM = '" & strGumNum & "'"
...
Function GetOrderExamCode(argEquipCode As String, argPID As String, _
    ...
    Else
              SQL = "SELECT Distinct B.EXAM_CODE AS EXAMCODE"
        SQL = SQL & "  FROM MEDITOLISS..TOTAL A, MEDITOLISS..TOTRES B"
        SQL = SQL + " WHERE A.REQUEST_DATE = '" & argOrdDt & "'" & vbCr
        SQL = SQL & "   And B.EXAM_CODE in (" & gAllExam & ") " & vbCrLf
        SQL = SQL & "   And A.CHART_NO = '" & argPID & "'"
        SQL = SQL & "   And A.EXAM_NO = '" & argGumNum & "'"
        SQL = SQL & "   AND B.EXAM_PART = 'C'"
        '-- 저장미포함
        If frmInterface.chkSaveAll.Value = "0" Then
            SQL = SQL & "   AND B.RESULT_VALUE = ''"
