   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C26A
         End
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":FFB3
            UserResize      =   2
         End
         Begin VB.Label Label1 
            Appearance      =   0  'Æò¸é
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String, Optional pBarNo As String)
    ...
    '-- BIT
          SQL = " SELECT DISTINCT '1', a.LABODRDTE AS Á¢¼öÀÏ½Ã, a.LABCHTNUM AS Â÷Æ®¹øÈ£, a.LABATTEND AS ³»¿ø¹øÈ£, a.LABKEYNUM, '','','', b.MANPATNAM AS È¯ÀÚ¸í, '' AS ¼ºº°" & vbCrLf
    ...
    'If chkSaveAll.Value = "0" Then
        SQL = SQL & "   AND a.LABENDDEP = '2' " & vbCrLf         '--  '2':Á¢¼ö '3' °á°úÀÔ·Â"
    '    SQL = SQL & "   AND (a.LABRESULT = ''  OR a.LABRESULT IS NULL)" & vbCrLf
    'End If
...
'    strBuffer=strBuffer & "Y           #);KVW^aqs€|†…?†‚?uufciaaPLDGDC;;6610,-.210,,))&'''&&#$"$&&&$$""&&$! !!#"" !!! !""! !"!!!!""#!!!! """   !""""""$""!!
    ...
    strBuffer = ":N1    80 81                 00620141422      15 1   7.0  2   4.1  3   0.5  4   4.5  5    34  6    20  7   417  8   239  9    97 14    85 15    14 16   0.7 18    93 19      T54     1 " & vbCr
...
Private Sub EditRcvDataASTM()
    ...
    Dim strBC       As String
    Dim strLC       As String
    'Dim strIFRstCd  As String   'ÀÎÅÍÆäÀÌ½º½Ã °Ë»çÇ×¸ñÄÚµå
'    Dim strRst      As String
    Dim strRst2     As String
    Dim intPos      As Integer
    Dim intNotCoag  As Integer
    Dim strFRcvBuffer As String
    
    'On Error Resume Next
        
    strFRcvBuffer = strBuffer   'strRecvData(intCnt)
    strBC = Mid$(strFRcvBuffer, 23, 1)

    '===== HEADER¸¦ ÀÎ½ÄÇØ¼­ °Ë»çÇ×¸ñÀ» Ã£´Â´Ù
    If Trim(strBC) = ":" Then
        intFMulti = 1
        strFExamInfo = ""
        ...
        Select Case strLC
            ...
            Case Else
                'MsgBox strLC & " ¶õ ¼ýÀÚ¿Í °Ë»çÇ×¸ñÀ» ´ã´çÀÚ´Ô²² ¾Ë·ÁÁÖ¼¼¿ä!"
        End Select
        Exit Sub
    End If
    
    '===== ½ÇÁ¦°á°úÀÌ¸é °á°ú¸¦ µî·ÏÇÑ´Ù
    ...
    If intPos > 0 And intPos < 19 Then
        Select Case strFExamInfo
            Case "PT-FIB"
                strIntBase = "PT(s)"
                strResult = Trim(Mid(strFRcvBuffer, 1, 4))
                Call LocalSave(strIntBase, strResult)
                
                strIntBase = "PT(%)"
                strResult = Trim(Mid(strFRcvBuffer, 6, 3))
                Call LocalSave(strIntBase, strResult)
                                
                strIntBase = "PT(R)"
                strResult = Trim(Mid(strFRcvBuffer, 10, 4))
                Call LocalSave(strIntBase, strResult)
                
                strIntBase = "FIB"
                strResult = Trim(Mid(strFRcvBuffer, 15, 3))
                Call LocalSave(strIntBase, strResult)
            ...
            Case "TT/APTT"
                ...
                '====== APTT ======
                strIntBase = "APTT"
                strResult = Trim(Mid(strFRcvBuffer, 6, 4))
                Call LocalSave(strIntBase, strResult)
...
Private Sub LocalSave(ByVal strIntBase As String, ByVal strResult As String)
    Dim strRcvBuf    As String   '¼ö½ÅÇÑ Data
    Dim strType      As String   '¼ö½ÅÇÑ Record Type
    Dim strBarno     As String   '¼ö½ÅÇÑ ¹ÙÄÚµå¹øÈ£
    Dim strSeq       As String   '¼ö½ÅÇÑ Sequence
