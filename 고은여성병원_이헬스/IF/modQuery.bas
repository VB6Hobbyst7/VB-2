...
'                End If
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" Then
                    ...
                    SQL = SQL & "   And OBSUCODE = '" & strTestCd & "'" & vbCrLf
...
'    SQL = SQL & "   AND RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) IN (" & gAllTestCd & ") " & vbCrLf  '검사코드 + '|' + OBSUSUBC
    SQL = SQL & "   AND LTRIM(RTRIM(b.OBODCODE)) IN (" & gAllTestCd & ") " & vbCrLf  '검사코드 + '|' + OBSUSUBC
...
'''    SQL = ""
'''    SQL = SQL & "SELECT DISTINCT"
'''    SQL = SQL & "       A.APATMRNO                      as PTID"
'''    SQL = SQL & "     , A.APATMRNO                      as SPCNO"
'''    SQL = SQL & "     , A.APATNAME                      as PTNM"
'''    SQL = SQL & "     , (CASE WHEN SUBSTRING(APATJMN1,1,2) > 19 THEN"
'''    SQL = SQL & "                 (FLOOR(CAST(DATEDIFF(DAY,'19' +SUBSTRING(APATJMN1,1,2) +'-' +SUBSTRING(APATJMN1,3,2) + '-' + SUBSTRING(APATJMN1,5,2) ,GETDATE()) AS INTEGER)/365.2422))"
'''    SQL = SQL & "             ELSE"
'''    SQL = SQL & "                 (FLOOR(CAST(DATEDIFF(DAY,'20' +SUBSTRING(APATJMN1,1,2) +'-' +SUBSTRING(APATJMN1,3,2) + '-' + SUBSTRING(APATJMN1,5,2) ,GETDATE()) AS INTEGER)/365.2422))"
'''    SQL = SQL & "        END)                           as AGE"
'''    SQL = SQL & "     , A.APATJMN1                      as BIRTH"
'''    SQL = SQL & "     , A.APATPSEX                      as SEX"
'''    SQL = SQL & "     , B.OBODSDTE                      as REQDATE"
'''    SQL = SQL & "     , RTRIM(B.OBODSLIP)               as SLIPCODE"
'''    SQL = SQL & "       (CASE WHEN B.OBODSTAT = 'OE' THEN '미접'"
'''    SQL = SQL & "             WHEN B.OBODSTAT = 'AC' THEN '접수'"
'''    SQL = SQL & "             ELSE B.OBODSTAT"
'''    SQL = SQL & "        END)                           as STATE1"
'''    SQL = SQL & "     , B.OBODSTAT                      as STATE"
'''    SQL = SQL & "     , (Select TOP 1 RTRIM(BADRDESC)"
'''    SQL = SQL & "          From dbo.BABORDRM"
'''    SQL = SQL & "         Where BADRGUBN = 'OBSLIP' "
'''    SQL = SQL & "           And BADRITEM = B.OBODSLIP)  as SLIPNM"
'''    SQL = SQL & "     , (Select TOP 1 RTRIM(BASUEDSC)"
'''    SQL = SQL & "          From BABSUGAM"
'''    SQL = SQL & "         Where BASUITEM = OBODCODE)    as TESTNM"
'''    SQL = SQL & "     , RTRIM(OBODCODE)                 as TESTCD"
'''    SQL = SQL & "  FROM ABPATMST A, OBODRMTM B"
'''    SQL = SQL & " Where A.APATMRNO  = B.OBODMRNO"
'''    SQL = SQL & "   And B.OBODSDTE  = '20190313'"
'''    'SQL = SQL & "   And B.OBODMRNO = ' & strPID & '"
'''    SQL = SQL & "   And B.OBODSLIP  IS NOT NULL"
'''    SQL = SQL & "   AND B.OBODLABR  = 'CP'"
'''    SQL = SQL & "   AND B.OBODSTAT  IN ('OE', 'AC')"          'AC 접수 OE 미접"
'''    SQL = SQL & " Order By A.APATNAME, A.APATMRNO, SLIPCODEsss"
...
'            SQL = SQL & "SELECT DISTINCT RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) AS ITEM" & vbCrLf
            SQL = SQL & "SELECT LTRIM(RTRIM(b.OBODCODE)) AS ITEM " & vbCrLf
            SQL = SQL & "  FROM ABPATMST a"             '환자기본정보
            SQL = SQL & "      ,OBODRMTM b"            '처방내역 Table
            SQL = SQL & "      ,OBSURSTM c " & vbCrLf     '검사결과(수치결과) Table
            SQL = SQL & " WHERE a.APATMRNO = b.OBODMRNO " & vbCrLf                                '등록번호,고객번호
            ...
            'SQL = SQL & "   AND RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) IN (" & gAllTestCd & ") " & vbCrLf    '검사코드 + '|' + OBSUSUBC
            SQL = SQL & "   AND LTRIM(RTRIM(b.OBODCODE)) IN (" & gAllTestCd & ") " & vbCrLf
...
'    SQL = SQL & "     , RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) AS ITEM" & vbCrLf
    SQL = SQL & "     ,  LTRIM(RTRIM(b.OBODCODE)) AS ITEM" & vbCrLf
    SQL = SQL & "  FROM ABPATMST a"                 '환자기본정보
    SQL = SQL & "      ,OBODRMTM b"                 '처방내역 Table
    SQL = SQL & "      ,OBSURSTM c " & vbCrLf         '검사결과(수치결과) Table
    SQL = SQL & " WHERE a.APATMRNO = b.OBODMRNO " & vbCrLf                                '등록번호,고객번호
