    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            If Trim(AdoRs_Local.Fields("SENDCHANNEL").Value) & "" <> "" Then
                strExamCode = strExamCode & "DSP|" & CStr(18 + i) & "||" & CStr(i + 1) & "^" & Trim(AdoRs_Local.Fields("SENDCHANNEL").Value) & "" & "^^^^|||" & vbCr
                i = i + 1
            End If
...
Public Sub GetWorkList_UBCARE(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Dim strJumin    As String
    Dim blnTC       As Boolean
    Dim blnTG       As Boolean
    Dim blnHDL      As Boolean
            ...
            For i = 0 To UBound(varXML) - 1
                ...
                If Not RS_L.EOF = True And Not RS_L.BOF = True Then
                    ...
                    Else
                        ...
                        '퇴계원 한내과
'                        If XMLInData.ComExamID = "CHOL" Then
'                            blnTC = True
'                        ElseIf XMLInData.ComExamID = "HDL-C" Then
'                            blnHDL = True
'                        ElseIf XMLInData.ComExamID = "TG" Then
'                            blnTG = True
'                        ElseIf XMLInData.ComExamID = "CREA" Then
'                            blnCREA = True
'                        End If
                    End If
...
'            'LDL
'            If blnTC = True And blnHDL = True And blnTG = True Then
'                blnTC = False
'                blnHDL = False
'                blnTG = False
'
...
'            End If
    ...
    SQL = SQL & " Order by HOSPDATE,BARCODE,PID,PNAME                               " & vbCr
...
Public Sub GetResultList(ByVal pFrom As String, ByVal pTo As String, ByVal pDateType As Integer, ByVal pOpt As Integer)
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT SAVESEQ,EXAMDATE,HOSPDATE,EQUIPNO,BARCODE,SAMPLETYPE,DISKNO,POSNO,SEQNO" & vbCr
    SQL = SQL & ",CHARTNO,INOUT,PID,PNAME,PSEX,PAGE,PJUMIN,SENDFLAG,SENDDATE,EXAMUID,HOSPITAL,QCRun " & vbCr
    '-- 검사결과
...
'                End If
                ...
                strBarcode = GetText(frmMain.spdROrder, intRow, colBARCODE)
                ...
                If strBarcode <> Trim(RS.Fields("BARCODE")) & "" Or strExamDate <> Trim(RS.Fields("EXAMDATE")) & "" Then
                    ...
                    SetText frmMain.spdROrder, Trim(RS.Fields("QCRun")) & "", intRow, colSEQNO
...
Public Function getMaxTestNum(ByVal strDate As String) As Long
    ...
    getMaxTestNum = 1
          ...
          SQL = "SELECT MAX(SAVESEQ) as SEQ FROM UB_PATRESULT  "
...
'-- 오늘 검사한 날짜의 Max + 1 번호를 가져온다
Public Function getMaxTestNum_UBCARE(ByVal strDate As String) As Long

    getMaxTestNum_UBCARE = 1
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        If Trim(RS.Fields("SEQ") & "") = "" Then
            getMaxTestNum_UBCARE = 1
        Else
            getMaxTestNum_UBCARE = Trim(RS.Fields("SEQ")) + 1
        End If
    Else
        getMaxTestNum_UBCARE = 1
    End If
    
    If getMaxTestNum_UBCARE >= 99999 Then
        getMaxTestNum_UBCARE = 99999
    End If
