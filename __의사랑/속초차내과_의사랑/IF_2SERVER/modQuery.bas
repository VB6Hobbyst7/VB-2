...
Public Sub GetDestList()
    Dim intRow          As Long
    
    frmMain.vasDest.MaxRows = 0
    intRow = 0
    
    SQL = ""
    SQL = SQL & "SELECT  SYS_DT,SYS_TM,SYS_SEQ,EQP_TST_CD,RACK_NO,SPC_NO,PT_NO,PT_NM,RSLT,TRANS_YN,TRANS_DT,TRANS_TM " & vbCr
    SQL = SQL & "  FROM ELIMAS301 " & vbCr
    SQL = SQL & " WHERE RSLT <> '' " & vbCr
    SQL = SQL & "   AND PT_NO <> '' " & vbCr
    SQL = SQL & "   AND PT_NM <> '' " & vbCr
    SQL = SQL & "   AND TRANS_YN = '1' " & vbCr
    'SQL = SQL & " ORDER BY  SYS_DT,SYS_TM,(SYS_SEQ * 10),(EQP_TST_CD * 10),EQP_TST_CD"
'    SQL = SQL & " ORDER BY SPC_NO,SYS_DT,SYS_TM,(SYS_SEQ * 10),(EQP_TST_CD * 10),EQP_TST_CD"
    SQL = SQL & " ORDER BY SYS_DT,SYS_TM,SPC_NO,(SYS_SEQ * 10),(EQP_TST_CD * 10),EQP_TST_CD"
    
    '-- Record Count 가져옴
    AdoCn_Local.CursorLocation = adUseClient
    Set AdoRs_Local = AdoCn_Local.Execute(SQL, , 1)
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        With frmMain.vasDest
            .MaxRows = AdoRs_Local.RecordCount
            frmMain.Command4.Caption = AdoRs_Local.RecordCount
            .MaxCols = 11
            '처방코드, SUB코드용 추가 16,17
            '여자상한,하한 추가  18,19
            'ReDim Preserve gArrEQP(AdoRs_Local.RecordCount, 19)
            ...
            Do Until AdoRs_Local.EOF
                intRow = intRow + 1
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("SYS_DT").Value & "", intRow, 1)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("SYS_TM").Value & "", intRow, 2)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("SYS_SEQ").Value & "", intRow, 3)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("EQP_TST_CD").Value & "", intRow, 4)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("SPC_NO").Value & "", intRow, 5)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("PT_NO").Value & "", intRow, 6)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("PT_NM").Value & "", intRow, 7)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("RSLT").Value & "", intRow, 8)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("TRANS_YN").Value & "", intRow, 9)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("TRANS_DT").Value & "", intRow, 10)
                Call SetText(frmMain.vasDest, AdoRs_Local.Fields("TRANS_TM").Value & "", intRow, 11)
...
Public Sub GetMasterList()
    ...
    frmMain.vasMaster.MaxRows = 0
    intRow = 0
    
    SQL = ""
    SQL = SQL & "SELECT EQP_TST_CD,LIS_TST_CD,LIS_TST_NM " & vbCr
    SQL = SQL & "  FROM ELIMAS002 " & vbCr
    SQL = SQL & " ORDER BY (EQP_TST_CD * 10)"
    
    '-- Record Count 가져옴
    AdoCn_Local.CursorLocation = adUseClient
    Set AdoRs_Local = AdoCn_Local.Execute(SQL, , 1)
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        With frmMain.vasMaster
            .MaxRows = AdoRs_Local.RecordCount
            'frmMain.Command4.Caption = AdoRs_Local.RecordCount
            .MaxCols = 3
            '처방코드, SUB코드용 추가 16,17
            ...
            Do Until AdoRs_Local.EOF
                intRow = intRow + 1
                Call SetText(frmMain.vasMaster, AdoRs_Local.Fields("EQP_TST_CD").Value & "", intRow, 1)
                Call SetText(frmMain.vasMaster, AdoRs_Local.Fields("LIS_TST_CD").Value & "", intRow, 2)
                Call SetText(frmMain.vasMaster, AdoRs_Local.Fields("LIS_TST_NM").Value & "", intRow, 3)
...
Public Sub GetTargetList()
    ...
    frmMain.vasTarget.MaxRows = 0
    intRow = 0
    
    SQL = ""
    SQL = SQL & "SELECT SAVESEQ,EXAMDATE,HOSPDATE,EQUIPNO,BARCODE,EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EXAMNAME,SEQNO,SAMPLETYPE,DISKNO,POSNO,EQUIPRESULT,RESULT,REFJUDGE,REFFLAG,REFVALUE,CHARTNO,INOUT,PID,PNAME,PSEX,PAGE,PJUMIN,PANICVALUE,DELTAVALUE,SENDFLAG,SENDDATE,EXAMUID,HOSPITAL,EXAMTYPE,EXAMNO " & vbCr
    SQL = SQL & "  FROM UB_PATRESULT " & vbCr
'    SQL = SQL & " WHERE RSLT <> '' " & vbCr
'    SQL = SQL & "   AND PT_NO <> '' " & vbCr
'    SQL = SQL & "   AND PT_NM <> '' " & vbCr
'    SQL = SQL & "   AND TRANS_YN = '1' " & vbCr
'    SQL = SQL & " ORDER BY  SYS_DT,SYS_TM,(SYS_SEQ * 10),(EQP_TST_CD * 10)"
    
    '-- Record Count 가져옴
    AdoCn_Local2.CursorLocation = adUseClient
    Set AdoRs_Local = AdoCn_Local2.Execute(SQL, , 1)
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        With frmMain.vasTarget
            .MaxRows = AdoRs_Local.RecordCount
            frmMain.Command5.Caption = AdoRs_Local.RecordCount
            .MaxCols = 34
            '처방코드, SUB코드용 추가 16,17
            '여자상한,하한 추가  18,19
            'ReDim Preserve gArrEQP(AdoRs_Local.RecordCount, 19)
            ...
            Do Until AdoRs_Local.EOF
                intRow = intRow + 1
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(0).Value & "", intRow, 1)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(1).Value & "", intRow, 2)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(2).Value & "", intRow, 3)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(3).Value & "", intRow, 4)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(4).Value & "", intRow, 5)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(5).Value & "", intRow, 6)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(6).Value & "", intRow, 7)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(7).Value & "", intRow, 8)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(8).Value & "", intRow, 9)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(9).Value & "", intRow, 10)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(10).Value & "", intRow, 11)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(11).Value & "", intRow, 12)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(12).Value & "", intRow, 13)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(13).Value & "", intRow, 14)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(14).Value & "", intRow, 15)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(15).Value & "", intRow, 16)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(16).Value & "", intRow, 17)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(17).Value & "", intRow, 18)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(18).Value & "", intRow, 19)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(19).Value & "", intRow, 20)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(20).Value & "", intRow, 21)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(21).Value & "", intRow, 22)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(22).Value & "", intRow, 23)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(23).Value & "", intRow, 24)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(24).Value & "", intRow, 25)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(25).Value & "", intRow, 26)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(26).Value & "", intRow, 27)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(27).Value & "", intRow, 28)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(28).Value & "", intRow, 29)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(29).Value & "", intRow, 30)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(30).Value & "", intRow, 31)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(31).Value & "", intRow, 32)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(32).Value & "", intRow, 33)
                Call SetText(frmMain.vasTarget, AdoRs_Local.Fields(33).Value & "", intRow, 34)
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
        Case "AMIS"                         '아미스
            Call GetWorkList_AMIS_AMIS(pFrom, pTo, SPD)
        Case "JWINFO"                       '중외정보
            Call GetWorkList_JWINFO_JWINFO(pFrom, pTo, SPD)
    
    End Select
    
'    If gEMR2 <> "" Then
'        Call GetWorkList_AMIS_JWINFO(pFrom, pTo, SPD)
'        Call GetWorkList_AMIS_AMIS(pFrom, pTo, SPD)
'        Exit Sub
'    End If
...
Public Sub GetWorkList_AMIS_JWINFO(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, "AMIS", intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        Do Until RS.EOF
            With SPD
                ...
                For i = 1 To SPD.DataRowCnt
                    strInOut = GetText(SPD, i, colINOUT)
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colBARCODE)
                    If "JW" = strInOut And Trim(RS("HOSPDATE")) = strHospDate And Trim(RS("BARCODE")) = strBarcode Then
                        blnSame = True
                    End If
                Next
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, "JW", intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
                    SetText SPD, GetSampleITEM2(intRow, SPD), intRow, colITEMS
                    If gWORKPOS = "P" Then
                        SetText SPD, frmWorkList.txtSeqNo.Text, intRow, colSEQNO
                        frmWorkList.txtSeqNo.Text = frmWorkList.txtSeqNo.Text + 1
                    Else
...
Public Sub GetWorkList_JWINFO_JWINFO(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, "OL", intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    SetText SPD, Trim(RS.Fields("SEX")) & "", intRow, colPSEX
                    SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
                    SetText SPD, GetSampleITEM_JWINFO_JWINFO(intRow, SPD), intRow, colITEMS
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, "OB", intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
...
Public Sub GetWorkList_AMIS_AMIS(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, "소아과", intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    SetText SPD, Trim(RS.Fields("SEX")) & "", intRow, colPSEX
                    SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
                    SetText SPD, GetSampleITEM_AMIS_AMIS(intRow, SPD), intRow, colITEMS
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, "내과", intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
...
On Error GoTo RST
    ...
    SQL = SQL & " GROUP BY L.LABODRDTE, L.LABBARCOD,L.LABATTEND,L.LABCHTNUM,M.MANADMFOR,M.MANPATNAM, L.LABINSNUM " & vbCr
...
'-- 검사자 ITEM 가져오기
Function GetSampleITEM_AMIS_AMIS(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strRegDate      As String
    Dim strChartNo      As String
    ...
    If strInOut = "소아과" Then
        ...
        GetSampleITEM_AMIS_AMIS = strItems
...
'-- 검사자 ITEM 가져오기
Function GetSampleITEM_JWINFO_JWINFO(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strRegDate      As String
    Dim strChartNo      As String
    ...
    If strInOut = "OL" Then
        ...
        GetSampleITEM_JWINFO_JWINFO = strItems
...
'-- 검사자 ITEM 가져오기
Function GetSampleITEM2(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strRegDate      As String
    Dim strChartNo      As String
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        Case "AMIS"
            Call GetSampleInfo_AMIS_AMIS(asRow, SPD)
        Case "JWINFO"
            Call GetSampleInfo_JWINFO_JWINFO(asRow, SPD)
    End Select
...
'            SQL = SQL & ", L.LABSMPNAM as 검체명"
    ...
    If gHOSP.BARUSE = "Y" Then
        SQL = SQL & " WHERE L.LABBARCOD = '" & strBarcode & Space(20) & "' " & vbCrLf
    Else
        SQL = SQL & " WHERE L.LABCHTNUM = '" & strChartNo & "'          " & vbCr
        SQL = SQL & "   AND L.LABODRDTE = '" & strRegDate & "'          " & vbCr
    End If
...
'                Next
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        Call SetSQLData("SUB", Trim(RS.Fields("SEQ")) & "", "A")
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_AMIS_JWINFO(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strInOut        As String
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
        ...
        If Not RS.EOF = True And Not RS.BOF = True Then
            Do Until RS.EOF
                With SPD
                    ...
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")), asRow, colBARCODE
                    SetText SPD, "AMIS", asRow, colINOUT
                    SetText SPD, Trim(RS.Fields("PID")) & "", asRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                    SetText SPD, Trim(RS.Fields("SEX")) & "", asRow, colPSEX
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_JWINFO_JWINFO(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strInOut        As String
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_AMIS_AMIS(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strInOut        As String
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
        ...
        If Not RS.EOF = True And Not RS.BOF = True Then
            Do Until RS.EOF
                With SPD
                    ...
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
                    SetText SPD, Trim(RS.Fields("BARCODE")), asRow, colBARCODE
                    SetText SPD, "소아과", asRow, colINOUT
                    SetText SPD, Trim(RS.Fields("PID")) & "", asRow, colPID
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                    SetText SPD, Trim(RS.Fields("SEX")) & "", asRow, colPSEX
...
'                                            SetText .spdResult, strTestCodeSub, strRstRow, colRSUBCD          '검사코드SUB
                ...
                If DBExec(AdoCn_Local, SQL) Then
                    ...
                    Call SetSQLData("SUB", Trim(GetText(.spdResult, asRow2, colRSUBCD)), "A")
