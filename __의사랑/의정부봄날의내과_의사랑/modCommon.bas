...
'    gDBTYPE = Trim(strSetUp1)
    ...
    If gComm.COMTYPE <> "" Then
        ...
        strSetup = "":    strSetUp1 = ""
        Call GetPrivateProfileString("COMM", "FILEPATH", "", strSetup, 100, App.PATH & "\INI\" & gMACH & ".ini")
        strSetUp1 = Trim(strSetup)
        strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
        gComm.FILEPATH = Trim(strSetUp1)
...
'    gArrEQP(intRow, 15) = AdoRs_Local.Fields("COHOUT").Value & ""
    ...
    With frmMain.spdOrder
        ...
        For i = 0 To UBound(gArrEQPNm) - 1
            ...
            .TypeHAlign = TypeHAlignCenter
            .TypeVAlign = TypeVAlignCenter
            Call SetText(frmMain.spdOrder, Trim(gArrEQPNm(i + 1, 6)), 0, colSTATE + (i + 1))   '-- 5 : 약어명
            .ColWidth(colSTATE + (i + 1)) = gCOLWIDTH
        Next
    End With
    ...
    With frmMain.spdROrder
        ...
        For i = 0 To UBound(gArrEQPNm) - 1
            ...
            .TypeHAlign = TypeHAlignCenter
            .TypeVAlign = TypeVAlignCenter
            Call SetText(frmMain.spdROrder, Trim(gArrEQPNm(i + 1, 6)), 0, colSTATE + (i + 1))   '-- 5 : 약어명
            .ColWidth(colSTATE + (i + 1)) = gCOLWIDTH
        Next
    End With
