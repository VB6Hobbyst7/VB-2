...
'    gCENXPCD = Trim(strSetUp1)
    ...
    '-- 배치오더
    strSetup = "":    strSetUp1 = ""
    Call GetPrivateProfileString("HOSP", "BATCHORDER", "", strSetup, 100, App.PATH & "\INI\" & gMACH & ".ini")
    strSetUp1 = Trim(strSetup)
    strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
    gHOSP.BATCHORDER = strSetUp1

    '-- 결과요청
    strSetup = "":    strSetUp1 = ""
    Call GetPrivateProfileString("HOSP", "REQRESULT", "", strSetup, 100, App.PATH & "\INI\" & gMACH & ".ini")
    strSetUp1 = Trim(strSetup)
    strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
    gHOSP.REQRESULT = strSetUp1
