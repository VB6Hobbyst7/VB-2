...
Private Sub ReceiveProcess()
    ...
    ElseIf gComm.COMTYPE = "2" Then
        Select Case UCase(gHOSP.MACHNM)
            Case "BS360":           Call TCPRcvData_BS360S
...
Private Sub GetOrder_BS360S(ByVal pBarno As String, ByVal pType As String) ', _
    ...
    With frmInterface
        ...
        '-- 스프레드에서 못찾았음..
        If intRow < 0 Then
            If UCase(gHOSP.MACHNM) = "BS200" Or UCase(gHOSP.MACHNM) = "BS220" Or UCase(gHOSP.MACHNM) = "BS360S" Or UCase(gHOSP.MACHNM) = "BS360" Then
               Exit Sub
            Else
                intRow = .spdOrder.DataRowCnt + 1
                If .spdOrder.MaxRows < intRow Then
        ...
        Else
            ...
            strSend = strSend & "DSP|1||" & mOrder.PID & "|||" & vbCr       'Admission Number(=PID)
            strSend = strSend & "DSP|2|||||" & vbCr                         'Bed Number
            strSend = strSend & "DSP|3||" & mOrder.PNAME & "|||" & vbCr     'Patient Name(Maximum of 32 characters)
            strSend = strSend & "DSP|4|||||" & vbCr                         'Birth Date(The format is YYYYMMDDHHmmSS,such as 20061122130540)
            strSend = strSend & "DSP|5||" & mPatient.SEX & "|||" & vbCr     'SEX(M,F,O)
            strSend = strSend & "DSP|6|||||" & vbCr                         'Blood Type(O, A, B, and AB)
            strSend = strSend & "DSP|7|||||" & vbCr
            strSend = strSend & "DSP|8|||||" & vbCr
            strSend = strSend & "DSP|9|||||" & vbCr
            strSend = strSend & "DSP|10|||||" & vbCr
            strSend = strSend & "DSP|11|||||" & vbCr                        'Sample Position
            strSend = strSend & "DSP|12|||||" & vbCr                        'Sample Collection Time
            strSend = strSend & "DSP|13|||||" & vbCr
            strSend = strSend & "DSP|14|||||" & vbCr
            strSend = strSend & "DSP|15|||||" & vbCr                        'Patient Type(Outpatient, inpatient, and other)
            strSend = strSend & "DSP|16|||||" & vbCr
            strSend = strSend & "DSP|17|||||" & vbCr                        'Charge type (Pay type)
            ...
            strSend = strSend & "DSP|22||" & mOrder.Seq & "|||" & vbCr      'Sample id (Integer)
            strSend = strSend & "DSP|23||" & strNow & "|||" & vbCr          'Send time
            strSend = strSend & "DSP|24||N|||" & vbCr                       'STAT (응급여부) Y, N
            strSend = strSend & "DSP|25|||||" & vbCr
            strSend = strSend & "DSP|26||serum|||" & vbCr                   'sample type (It includes serum, plasma, urine , CSF, and other.)
