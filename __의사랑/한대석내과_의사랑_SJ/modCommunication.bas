...
'                   3 : 체크된것중 제일 위에 것
'-----------------------------------------------------------------------------'
Public Sub SetPatInfo(ByVal pBarno As String, ByVal pType As String, Optional ByVal pSemi As String = "")
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                If mResult.Kind = "QC" Then
                    ...
                    SQL = SQL & " WHERE workarea = '" & gHOSP.LABCD & "'" & vbCr
...
'        strKey1 = Trim(GetText(.spdOrder, argSpcRow, colKEY1))
        ...
        strSaveSeq = Trim(GetText(.spdOrder, argSpcRow, colSAVESEQ))
        strExamDate = Mid(Trim(GetText(.spdOrder, argSpcRow, colEXAMDATE)), 1, 8)
        strHospDate = Trim(GetText(.spdOrder, argSpcRow, colHOSPDATE))
        
        strChartNo = Trim(GetText(.spdOrder, argSpcRow, colCHARTNO))
        strPName = Trim(GetText(.spdOrder, argSpcRow, colPNAME))
        strPJumin = Trim(GetText(.spdOrder, argSpcRow, colPJUMIN))
        strBarcode = Trim(GetText(.spdOrder, argSpcRow, colBARCODE))
        strPatID = Trim(GetText(.spdOrder, argSpcRow, colPID))
'        strIO = Trim(GetText(.spdOrder, argSpcRow, colINOUT))
'        strSpc = Trim(GetText(.spdOrder, argSpcRow, colINOUT))
...
'        strTime = Format(Now, "hh:mm:ss")
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,PANICVALUE,EXAMNO,SAMPLETYPE,REFVALUE,INOUT " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            ...
            Do Until RS_L.EOF
                ...
                Call SetText(.vasTemp, RS_L.Fields("EQUIPRESULT").Value & "", intRow, 5)
                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 6)
                Call SetText(.vasTemp, RS_L.Fields("PANICVALUE").Value & "", intRow, 7)
                Call SetText(.vasTemp, RS_L.Fields("EXAMNO").Value & "", intRow, 8)
                Call SetText(.vasTemp, RS_L.Fields("SAMPLETYPE").Value & "", intRow, 9)
                Call SetText(.vasTemp, RS_L.Fields("REFVALUE").Value & "", intRow, 10)
                Call SetText(.vasTemp, RS_L.Fields("INOUT").Value & "", intRow, 11)
                RS_L.MoveNext
            Loop
        End If
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            strExamNo = Trim(GetText(.vasTemp, intRow, 8))
            strSpcType = Trim(GetText(.vasTemp, intRow, 9))
            strRefValue = Trim(GetText(.vasTemp, intRow, 10))
            strIO = Trim(GetText(.vasTemp, intRow, 11))
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                XMLAllTxt = XMLAllTxt & "<업체>" & gHOSP.LABCD & "</업체>"
                XMLAllTxt = XMLAllTxt & "<요양기관번호>" & gHOSP.HOSPCD & "</요양기관번호>"
                XMLAllTxt = XMLAllTxt & "<차트번호>" & strChartNo & "</차트번호>"
                XMLAllTxt = XMLAllTxt & "<수진자명>" & strPName & "</수진자명>"
                XMLAllTxt = XMLAllTxt & "<주민등록번호>" & strPJumin & "</주민등록번호>"
                XMLAllTxt = XMLAllTxt & "<내원번호>" & strPatID & "</내원번호>"
                XMLAllTxt = XMLAllTxt & "<의뢰일>" & strHospDate & "</의뢰일>"
                XMLAllTxt = XMLAllTxt & "<검사번호>" & strExamNo & "</검사번호>"
                XMLAllTxt = XMLAllTxt & "<검사ID>" & strTestCd & "</검사ID>"
                XMLAllTxt = XMLAllTxt & "<업체검사ID>" & strEqpcd & "</업체검사ID>"
                XMLAllTxt = XMLAllTxt & "<검체>" & strSpcType & "</검체>"
                XMLAllTxt = XMLAllTxt & "<결과치>" & sResult & "</결과치>"
                XMLAllTxt = XMLAllTxt & "<참조치>" & strRefValue & "</참조치>"
                XMLAllTxt = XMLAllTxt & "<소견></소견>"
                XMLAllTxt = XMLAllTxt & "<결과일>" & strExamDate & "</결과일>"
                XMLAllTxt = XMLAllTxt & "<입원외래구분>" & strIO & "</입원외래구분>"
                XMLAllTxt = XMLAllTxt & "</검사>"
...
'        strKey1 = Trim(GetText(.spdrOrder, argSpcRow, colKEY1))
        ...
        strExamDate = Mid(Trim(GetText(.spdROrder, argSpcRow, colEXAMDATE)), 1, 8)
        strHospDate = Trim(GetText(.spdROrder, argSpcRow, colHOSPDATE))
        
        strChartNo = Trim(GetText(.spdROrder, argSpcRow, colCHARTNO))
        strPName = Trim(GetText(.spdROrder, argSpcRow, colPNAME))
        strPJumin = Trim(GetText(.spdROrder, argSpcRow, colPJUMIN))
...
'        strSpc = Trim(GetText(.spdrOrder, argSpcRow, colINOUT))
        
        'strWorkArea = Trim(GetText(.spdrOrder, argSpcRow, colRACKNO))
        'strAccSeq = Trim(GetText(.spdrOrder, argSpcRow, colPOSNO))
