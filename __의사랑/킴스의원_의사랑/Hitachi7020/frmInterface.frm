...
Begin VB.Form frmInterface 
   Caption         =   " Hitachi 7020 Interface "
   ClientHeight    =   10680
   ClientLeft      =   345
   ClientTop       =   840
   ClientWidth     =   25560
   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C58C
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasWorkList 
            ...
            Protect         =   0   'False
            RetainSelBlock  =   0   'False
            SpreadDesigner  =   "frmInterface.frx":D0E6
         End
...
'                            .clearRcvbuf
                            intBufCnt = 1
                        End If
'                        Call addRcvbuf(strRcvBuf & Fld_Div)
                End Select
            Case ":"    '¹ÞÀº µ¥ÀÌÅÍ ÆíÁý
                Call sndRec
                
                mResult.BarNo = Trim(Mid(strRcvBuf, 8, 3))
                mResult.SpcPos = Trim(Mid(strRcvBuf, 11, 2))
                mResult.SpcmNo = Trim(Mid(strRcvBuf, 15, 6))
                
                gRow = SeqNullSearch(vasID, 4, 4)
                vasID.Row = gRow
                vasID.Col = 2
                strBarNo = vasID.Text
                Call GetOrder(strBarNo)

                Call vasID.SetText(4, gRow, mResult.SpcPos)
                ...
                'strBuffer = ":n    17  17                              8 "
                ' 1   100
                ' 4   266   5   232   6    54   9  1.05  11    32  12    35  14    11 "
                    
                    strIntBase = Trim(Mid$(strTmp, 1, 4))
                    strResult = Trim(Mid$(strTmp, 5, 6))
Rst:
                    If strResult <> "" Then
                        ...
                        If Res > 0 Then
'''                            '-- TG °á°úÀúÀå(==>12 ·Îº¯°æ)
'''                            If Val(strIntBase) = 5 Then
'''                                strTGResult = Trim(strResult)
'''                            End If
'''                            '-- Chol °á°úÀúÀå(==>9 ·Îº¯°æ)
'''                            If Val(strIntBase) = 4 Then
'''                                strCHOLResult = Trim(strResult)
'''                            End If
'''                            '-- HDL °á°úÀúÀå(==>14 ·Îº¯°æ)
'''                            If Val(strIntBase) = 6 Then
...
Private Sub vasWorkList_DblClick(ByVal Col As Long, ByVal Row As Long)
    ...
    With vasWorkList
        ...
        Call vasID.SetText(2, pGrid_Point, .Text)
'        .Row = Row: .Col = 5
