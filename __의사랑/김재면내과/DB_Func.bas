...
'Public Connect As ADODB.Recordset
...
Public Function db_select_Vas_Work(argServer As Integer, argSQL As String, ByVal argSpread As vaSpread, Optional argRow As Integer = 1, Optional argcol As Integer = 1) As Integer
'쿼리 실행 내용을 스프레드쉬트에 Display
    Dim i, j As Integer
    
On Error GoTo ErrHandle
       
    db_select_Vas_Work = -1
    ...
    While Not RS.EOF
        If argSpread.MaxRows < i Then
            argSpread.MaxRows = i
        End If
        If Trim(CStr(RS.Fields.Item(10).Value)) > 1 Then
