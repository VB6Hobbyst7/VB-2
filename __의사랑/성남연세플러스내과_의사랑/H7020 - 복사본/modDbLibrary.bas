...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        strDate = Trim(GetText(.vasID, argSpcRow, 2))
        ...
        SQL = "SELECT DISTINCT Company,HospCode,ChartNo,PatName,PatSex," & _
              ...
              "   AND CHARTNO = '" & Trim(GetText(.vasID, argSpcRow, 5)) & "' " & _
              "   AND COMMDATE = '" & strDate & "' "
...
Function SaveTransDataR(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        lsID = Trim(GetText(.vasRID, argSpcRow, 4))
        ...
        SQL = "SELECT DISTINCT Company,HospCode,ChartNo,PatName,PatSex," & _
              ...
              "  FROM PAT_RES " & vbCrLf & _
              " WHERE EXAMTYPE = 'C' " & vbCrLf & _
              "   AND CHARTNO = '" & Trim(GetText(.vasRID, argSpcRow, 4)) & "' " & _
              "   AND BARCODE = '" & Trim(GetText(.vasRID, argSpcRow, 3)) & "' "
        SQL = SQL & " AND (Result <> '' )"
'        SQL = SQL & "  AND (TRANSYN = '' OR TRANSYN IS NULL) "
'        SQL = SQL & "  AND TRANSYN <> '2' "
        ...
        '서버로 결과값 저장하기
        For iRow = 1 To .vasTemp.DataRowCnt
            strEqpCd = Trim(GetText(.vasTemp, iRow, 11))
            sResult1 = Trim(GetText(.vasTemp, iRow, 14)) '결과
            '-- 장비결과 치환
            'sResult1 = Replace(sResult1, "<", "")
            'sResult1 = Replace(sResult1, ">", "")
        ...
        For vasResRow = 1 To .vasTemp.DataRowCnt
            ...
            If .vasRID.Value = 1 Then
                ...
                If liRet = 1 Then
                    ...
                    .vasRID.Row = vasIDRow: .vasRID.Col = 2
...
Dim rs_svr As ADODB.Recordset
    ...
    SQL = SQL & vbCrLf & "  FROM PAT_RES "
    SQL = SQL & vbCrLf & " WHERE EXAMTYPE = 'C' "
    SQL = SQL & vbCrLf & "   AND CHARTNO = '" & argPID & "'"
