...
On Error GoTo errFind
    ...
    With cn
        .ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;" & _
                            ...
                            "Persist Security Info=True"
'        .ConnectionString = "Provider=MSDASQL;DSN=ddif;DATABASES=;ddif=sa;PWD=sa1004;"
...
Public Function GetDBSelectVas_Ref(argServer As Integer, argSQL As String, ByVal argSpread As vaSpread, Optional argRow As Long = 1, Optional argCol As Long = 1) As Integer
'쿼리 실행 내용을 스프레드쉬트에 Display
    Dim i, j As Integer
...
On Error GoTo ErrHandle
    ...
    While Not RS.EOF
        ...
        For j = 0 To RS.Fields.Count - 1
            ...
            Else
                If j = 3 Then
                    strResult = Trim(CStr(RS.Fields.Item(j).Value))
                End If
'                If j = 6 Then
'                    If strResult <= RS.Fields.Item(j).Value Then
'                        argSpread.Col = 4
'                        argSpread.Row = i
'                        argSpread.ForeColor = vbRed
'                        blnLow = True
'                    Else
'                        argSpread.Col = 4
'                        argSpread.Row = i
'                        argSpread.ForeColor = vbBlack
'                        blnLow = False
'                    End If
'                ElseIf j = 7 Then
'                    If strResult > RS.Fields.Item(j).Value Then
...
'                End If
                ...
                If j = 6 Then
                    '--             Low                                     High
                    If strResult <= RS.Fields.Item(j).Value And strResult > RS.Fields.Item(j + 1).Value Then
                        argSpread.Col = 4
                        argSpread.Row = i
                        argSpread.ForeColor = vbRed
                        blnLow = True
