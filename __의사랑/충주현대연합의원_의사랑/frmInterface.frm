...
Private Sub cmdMatch_Click()
    ...
    If blnSame = True Then
        '기존 결과를 갖고있는다.
        strData(0) = GetText(spdOrder, intOSrcRow, colITEMS)     'Na
        strData(1) = GetText(spdOrder, intOSrcRow, colITEMS + 1)    'K
        strData(2) = GetText(spdOrder, intOSrcRow, colITEMS + 2)    'Cl
        
        Call GetSampleInfo_UBCARE_Match(intWSrcRow, spdWork, intOSrcRow, spdOrder)
        ...
        'gPatOrdCd = 'C3791','C3792','C3793'

        
        'Call SetText(spdOrder, strData(0), intOSrcRow, colITEMS + 1) 'Na
        'Call SetText(spdOrder, strData(1), intOSrcRow, colITEMS + 2) 'K
        'Call SetText(spdOrder, strData(2), intOSrcRow, colITEMS + 3) 'Cl
...
'    SQL = SQL & " ORDER BY SEQNO "
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT SEQNO, EQUIPCODE,EXAMCODE, EXAMNAME, EQUIPRESULT, RESULT,REFFLAG,REFJUDGE" & vbCr    ' SQL = SQL & "SELECT DISTINCT SEQNO, EQUIPCODE,EXAMCODE, EXAMNAME, EQUIPRESULT, RESULT,REFFLAG,REFJUDGE" & vbCr
...
'''    End With
    ...
    For intORow = 1 To spdOrder.MaxRows
        If GetText(spdOrder, intORow, colCHECKBOX) = "0" Then
            'intCnt = intCnt + 1
            intOSrcRow = intORow
            blnSame = True
            'If intCnt >= 2 Then
    ...
    If intWSrcRow < 1 Then
        MsgBox "검사리스트에서 하나의 검체만 선택하세요", vbOKOnly + vbCritical, Me.Caption
        Exit Sub
    End If
