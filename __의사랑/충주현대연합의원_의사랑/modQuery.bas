...
'    SQL = SQL & " ORDER BY EXAMDATE,SAVESEQ,BARCODE,SEQNO"
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT * " & vbCr    ' SQL = SQL & "SELECT DISTINCT SEQNO, EQUIPCODE,EXAMCODE, EXAMNAME, EQUIPRESULT, RESULT,REFFLAG,REFJUDGE" & vbCr
    SQL = SQL & "  FROM UB_PATRESULT " & vbCr
    SQL = SQL & " WHERE EXAMDATE Between '" & pFrom & "' AND '" & pTo & "'" & vbCr
    SQL = SQL & " ORDER BY EXAMDATE,BARCODE"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        Do Until RS.EOF
            With SPD
                ...
                If strBarcode <> Trim(RS.Fields("BARCODE")) & "" Then
                    .MaxRows = .MaxRows + 1
                    intRow = .MaxRows
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_UBCARE_Match(ByVal asRow As Long, ByVal SPD As fpSpread, ByVal asDestRow As Long, ByVal SPDDest As fpSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    'SetText SPDDest, "0", asDestRow, colCHECKBOX
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPDDest
                .ReDraw = False
                intTestCnt = intTestCnt + 1
                SetText SPDDest, "1", asDestRow, colCHECKBOX
                SetText SPDDest, Trim(RS.Fields("SAVESEQ")) & "", asDestRow, colSAVESEQ
                SetText SPDDest, Trim(RS.Fields("HOSPDATE")) & "", asDestRow, colHOSPDATE
                Select Case Trim(Trim(RS.Fields("INOUT")) & "")
                    Case "0":   SetText SPDDest, "외래", asDestRow, colINOUT
                    Case "1":   SetText SPDDest, "입원", asDestRow, colINOUT
                    Case Else:  SetText SPDDest, Trim(Trim(RS.Fields("INOUT")) & ""), asDestRow, colINOUT
                End Select
                SetText SPDDest, Trim(RS.Fields("BARCODE")), asDestRow, colBARCODE
                SetText SPDDest, Trim(RS.Fields("PID")) & "", asDestRow, colPID
                SetText SPDDest, Trim(RS.Fields("CHARTNO")), asDestRow, colCHARTNO
                SetText SPDDest, Trim(RS.Fields("PNAME")) & "", asDestRow, colPNAME
                SetText SPDDest, Trim(RS.Fields("PJUMIN")) & "", asDestRow, colPJUMIN
                SetText SPDDest, Trim(RS.Fields("PSEX")) & "", asDestRow, colPSEX
                SetText SPDDest, Trim(RS.Fields("PAGE")) & "", asDestRow, colPAGE
                
                '오더갯수
                SetText SPDDest, CStr(intTestCnt), asDestRow, colOCNT
                                                 
                mPatient.AGE = Trim(RS.Fields("PAGE")) & ""
                mPatient.SEX = Trim(RS.Fields("PSEX")) & ""
                
                '오더정보에 저장
                mResult.BarNo = Trim(RS.Fields("BARCODE")) & ""
                With mOrder
                    .BarNo = Trim(RS.Fields("BARCODE")) & ""
'                    .PID = Trim(RS.Fields("PID")) & ""
'                    .PNAME = Trim(RS.Fields("PNAME")) & ""
...
'                    .NoOrder = False
                ...
                '-- 화면에 표시
                'For intCol = colSTATE + 1 To .MaxCols
                'GetTestNm(Trim(RS1.Fields("itemcd")))
                    'If Trim(RS.Fields("ITEM")) & "" = gArrEQP(intCol - colSTATE, 6) Then
                        '.Row = asDestRow
                        '.Col = intCol
                        '.BackColor = vbYellow
                    '    Call SetText(SPDDest, "◇", asDestRow, intCol)
                    '    Exit For
                    'End If
                'Next
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_UBCARE_Match" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
