...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" And strHospDate <> "" And strPartGbn <> "" And strLabSeq <> "" And strSpcCd <> "" And strJumin <> "" And Len(strJumin) = 13 And strExamCd <> "" Then
                '-- 결과저장
                SQL = ""
                Select Case gTblNm
                    Case "H": SQL = SQL & "UPDATE H_RESULT "
                    Case "J": SQL = SQL & "UPDATE J_RESULT "
                    Case "M": SQL = SQL & "UPDATE M_RESULT "
                    Case "S": SQL = SQL & "UPDATE S_RESULT "
                    Case "U": SQL = SQL & "UPDATE U_RESULT "
                End Select
                SQL = SQL & " SET RESULT = '" & sResult & "'" & vbCr
                'SQL = SQL & " , MACHINECD = '10' " & vbCr
                SQL = SQL & " , RESULTDATE = '" & strDate & "'" & vbCr
                SQL = SQL & " , RESULTTIME = '" & strTime & "'" & vbCr
                SQL = SQL & " WHERE TESTITEMSEQ = '" & strExamCd & "'" & vbCr
                SQL = SQL & "   AND IDLEFT  = '" & Mid(strJumin, 1, 6) & "'" & vbCr
                SQL = SQL & "   AND IDRIGHT = '" & Mid(strJumin, 7, 7) & "'" & vbCr
                SQL = SQL & "   AND SPECIMENCD = '" & strSpcCd & "'" & vbCr
                SQL = SQL & "   AND LABSEQ = '" & strLabSeq & "'" & vbCr
...
'                    SQL = SQL & "   And OKFL <> 'Y' "   '-- 결과확정유무
        ...
        If Res = 1 And strHospDate <> "" And strPartGbn <> "" And strLabSeq <> "" And strSpcCd <> "" And strJumin <> "" And Len(strJumin) = 13 And strExamCd <> "" Then
            '-- 상태변경
            SQL = ""
            'SQL = SQL & "UPDATE J_JUBSU " & vbCr
            Select Case gTblNm
                Case "H": SQL = SQL & "UPDATE H_JUBSU "
                Case "J": SQL = SQL & "UPDATE J_JUBSU "
                Case "M": SQL = SQL & "UPDATE M_JUBSU "
                Case "S": SQL = SQL & "UPDATE S_JUBSU "
                Case "U": SQL = SQL & "UPDATE U_JUBSU "
            End Select
            
            
            SQL = SQL & " SET JUBSUGBN = '1'" & vbCr
            SQL = SQL & " , RESULTENDYN = '1' " & vbCr
            SQL = SQL & " , RESULTENDDATE = '" & strDate & "'" & vbCr
            SQL = SQL & " , RESULTENDTIME = '" & strTime & "'" & vbCr
            SQL = SQL & " WHERE IDLEFT  = '" & Mid(strJumin, 1, 6) & "'" & vbCr
            SQL = SQL & "   AND IDRIGHT = '" & Mid(strJumin, 7, 7) & "'" & vbCr
            SQL = SQL & "   AND SPECIMENCD = '" & strSpcCd & "'" & vbCr
            SQL = SQL & "   AND LABSEQ = '" & strLabSeq & "'" & vbCr
            SQL = SQL & "   AND PARTGBN = '" & strPartGbn & "'" & vbCr
            SQL = SQL & "   AND LABDATE = '" & strHospDate & "'" & vbCr
            SQL = SQL & "   AND RESULTENDYN = '0' "
            
    'SQL.Text:=' update H_JUBSU set '+CRLF+
    '' JUBSUGBN = ''1'' '+CRLF+
    '' , RESULTENDYN = ''1'' '+CRLF+
    '' , RESULTENDDATE = '''+Copy(sNow,1,8)+''' '+CRLF+
    '' , RESULTENDTIME = '''+Copy(sNow,9,6)+''' '+CRLF+
    '' where IDLEFT = '''+Copy(PID,1,6)+''' '+CRLF+
    '' and IDRIGHT = '''+Copy(PID,7,7)+''' '+CRLF+
    '' and SPECIMENCD = '''+SCD+''' '+CRLF+
    '' and LABSEQ = '''+ANO+''' '+CRLF+
    '' and PARTGBN = '''+SLP+''' '+CRLF+
    '' and LABDATE = '''+ADT+''' '+CRLF+
    '' and RESULTENDYN = ''0'' ';
