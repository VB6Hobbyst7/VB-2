...
Function SaveTransData_BOBATH(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 결과업데이트 쿼리
                SQL = ""
                SQL = SQL & "UPDATE bobath.LRESULT Set" & vbCr
                SQL = SQL & "  RSLT = '" & sResult & "', " & vbCr           '검사결과
                SQL = SQL & "  RSFL = 'Y', " & vbCr      '결과입력자
                SQL = SQL & "  RSID = '" & gHOSP.USERID & "', " & vbCr      '결과입력자
                SQL = SQL & "  RSDT = sysdate " & vbCr                     '결과입력일
                'SQL = SQL & " WHERE SENO = '" & strPatID & "'" & vbCr      '일련번호
                'SQL = SQL & " Where OIFL = '" & strIO & "'" & vbCr          '입/외
                'SQL = SQL & "   And ORQN = '" & strKey1 & "'" & vbCr        '처방일련번호
                'SQL = SQL & "   And ORDT = '" & strHospDate & "'" & vbCr    '처방일자
                'SQL = SQL & "   And NWNO = '" & strChartNo & "'" & vbCr     '내원번호
                'SQL = SQL & "   And PAID = '" & strPatID & "'"              '환자번호
                SQL = SQL & " Where SPNO = '" & strBarcode & "'" & vbCr     '검체번호
                SQL = SQL & "   And ORCD = '" & strTestCd & "'" & vbCr      '처방코드
        ...
        If blnSave = True Then
            For intRow = 1 To 10
                Select Case intRow
                    Case 1:  strTestCd = "L-ME01"    'WBC
                    Case 2:  strTestCd = "L-ME02"    'RBC
                    Case 3:  strTestCd = "L-ME03"    'S cell
                    Case 4:  strTestCd = ""
                    Case 5:  strTestCd = "L-ME05"    'Cast
                    Case 6:  strTestCd = "L-ME06"    'Crystal
                    Case 7:  strTestCd = "L-ME07"    'Bacteria
                    Case 8:  strTestCd = "L-ME08"    'Remark
                    Case 9:  strTestCd = "L-UA01"    'Color
                    Case 10: strTestCd = "L-UA02"    'Turbidity
                End Select
