...
Public Function GetTestNm(ByVal pItem As String, Optional pFull As Boolean) As String
    ...
    SQL = ""
    SQL = SQL & "SELECT ABBRNAME as ITEMNM  "
    SQL = SQL & "  FROM EQPMASTER " & vbCr
...
'-- 검사코드로 검사명 조회
Public Function GetTestInfos(ByVal pItem As String, Optional pFull As Boolean) As String
    Dim RS2             As ADODB.Recordset
    Dim intRow          As Long
    ...
    SQL = ""
    SQL = SQL & "SELECT SENDCHANNEL,RSLTCHANNEL,TESTCODE,ABBRNAME as ITEMNM,SEQNO "
    SQL = SQL & "  FROM EQPMASTER " & vbCr
    SQL = SQL & " WHERE TESTCODE = '" & pItem & "'"
    ...
    If Not RS2.EOF = True And Not RS2.BOF = True Then
        Do Until RS2.EOF
            GetTestInfos = RS2.Fields("SENDCHANNEL").Value & "|" & RS2.Fields("RSLTCHANNEL").Value & "|" & RS2.Fields("TESTCODE").Value & "|" & RS2.Fields("ITEMNM").Value & "|" & RS2.Fields("SEQNO").Value & "|"
            RS2.MoveNext
        Loop
    End If
...
Public Function GetTestCd(ByVal pItem As String, Optional pFull As Boolean) As String
    ...
    If Not RS2.EOF = True And Not RS2.BOF = True Then
        Do Until RS2.EOF
            GetTestCd = RS2.Fields("ITEMNM").Value & ""
            RS2.MoveNext
        Loop
    End If
    ...
    Select Case gOCS
        ...
        Case "KOMAIN"
         ...
         Case "BIT"
            ...
            SQL = ""
            SQL = SQL & "SELECT DISTINCT L.LABATTEND as 내원번호, L.LABCHTNUM as 챠트번호, L.LABODRDTE as 접수일자, M.MANADMFOR as 입외," & vbCrLf
            SQL = SQL & "       M.MANRESNUM as 주민번호, M.MANPATNAM as 이름, L.LABINSNUM as 처방순서,L.LABSMPNAM as 검체명, L.LABBARCOD as 바코드번호, L.LABODRSTP as SEQ " & vbCrLf
            ...
            '    SQL = SQL & "   AND L.LABENDDEP <= '3' " & vbCrLf                       '-- 처리상태 (2:접수, 3:결과입력)
            'End If
            SQL = SQL & " ORDER BY L.LABODRDTE, L.LABCHTNUM, L.LABBARCOD, L.LABINSNUM "
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                ...
                Do Until RS.EOF
                    ...
                    With frmWorkList.spdWork
                        ...
                        For i = 1 To frmWorkList.spdWork.DataRowCnt
                            strDate = GetText(frmWorkList.spdWork, i, colHOSPDATE)
                            strChart = GetText(frmWorkList.spdWork, i, colCHARTNO)
                        ...
                        If blnSame = False Then
                            .MaxRows = .MaxRows + 1
                            intRow = .MaxRows
                            SetText frmWorkList.spdWork, "1", .MaxRows, colCHECKBOX
                            SetText frmWorkList.spdWork, Trim(RS.Fields("접수일자")) & "", .MaxRows, colHOSPDATE
'                            SetText frmWorkList.spdWork, Trim(RS.Fields("바코드번호")) & "", .MaxRows, colBARCODE
                            SetText frmWorkList.spdWork, Trim(RS.Fields("챠트번호")) & "", .MaxRows, colCHARTNO
                            SetText frmWorkList.spdWork, Trim(RS.Fields("내원번호")) & "", .MaxRows, colPID
                            SetText frmWorkList.spdWork, Trim(RS.Fields("이름")) & "", .MaxRows, colPNAME
'                            SetText frmWorkList.spdWork, Trim(RS.Fields("검체명")) & "", .MaxRows, colSPCNM
                            SetText frmWorkList.spdWork, Trim(RS.Fields("SEQ")) & "", .MaxRows, colPAGE
                            
                            Select Case Trim(Trim(RS.Fields("입외")) & "")
                                Case "A":   SetText frmWorkList.spdWork, "외래", .MaxRows, colINOUT
                                Case "F":   SetText frmWorkList.spdWork, "입원", .MaxRows, colINOUT
                                Case Else:  SetText frmWorkList.spdWork, "", .MaxRows, colINOUT
                            End Select
                            
                            SetText frmWorkList.spdWork, frmWorkList.txtSeq.Text, .MaxRows, colSEQNO
                            'SetText frmWorkList.spdWork, RS.Fields("CNT"), .MaxRows, colOCNT
                            SetText frmWorkList.spdWork, GetSampleITEM(intRow), .MaxRows, colITEMS
         ...
         Case "BOBATH"
            ...
            SQL = ""
            SQL = SQL & "SELECT DISTINCT PAID as 환자번호, NWNO as 내원번호, SPNO as 바코드번호, ORDT as 접수일자, OIFL as 입외," & vbCrLf
            SQL = SQL & "       '' as 주민번호, PANM as 이름, '' as 처방순번,'' as 검체명, '' as SEQ " & vbCrLf
            SQL = SQL & "  FROM bobath.V_LAB_ORDER_CP " & vbCrLf
            SQL = SQL & " WHERE ORDT between  '" & pFrom & "' AND '" & pTo & "'" & vbCrLf
            SQL = SQL & "   AND ORCD IN (" & gAllTestCd & ")" & vbCrLf
            SQL = SQL & " ORDER BY ORDT, SPNO "
                ...
                Do Until RS.EOF
                    ...
                    With frmWorkList.spdWork
                        ...
                        If blnSame = False Then
                            ...
                            SetText frmWorkList.spdWork, Trim(RS.Fields("접수일자")) & "", .MaxRows, colHOSPDATE
                            SetText frmWorkList.spdWork, Trim(RS.Fields("바코드번호")) & "", .MaxRows, colBARCODE
                            SetText frmWorkList.spdWork, Trim(RS.Fields("환자번호")) & "", .MaxRows, colPID
                            SetText frmWorkList.spdWork, Trim(RS.Fields("내원번호")) & "", .MaxRows, colCHARTNO
                            SetText frmWorkList.spdWork, Trim(RS.Fields("처방순번")) & "", .MaxRows, colKEY1
                            SetText frmWorkList.spdWork, Trim(RS.Fields("이름")) & "", .MaxRows, colPNAME
                            SetText frmWorkList.spdWork, Trim(RS.Fields("입외")) & "", .MaxRows, colINOUT
         ...
         Case "HWASAN"
            ...
            SQL = ""
            SQL = SQL & "SELECT O.OrdDt as 접수일자, O.PtID as 내원번호, O.PtNm as 이름, O.Sex as 성별, O.Age as 나이, O.SpcCd, O.SpcNm, O.AllTestNm, O.StatFg, O.SPCNO as 바코드번호" & vbCr
            SQL = SQL & "  FROM TC201 O, TC301 T " & vbCr
            SQL = SQL & " WHERE O.OrdDt between  '" & pFrom & "' AND '" & pTo & "'" & vbCrLf
            SQL = SQL & "   AND O.SPCNO = T.SPCNO " & vbCr
                ...
                Do Until RS.EOF
                    ...
                    With frmWorkList.spdWork
                        ...
                        If blnSame = False Then
                            ...
                            SetText frmWorkList.spdWork, Trim(RS.Fields("내원번호")) & "", .MaxRows, colCHARTNO
                            SetText frmWorkList.spdWork, Trim(RS.Fields("이름")) & "", .MaxRows, colPNAME
                            SetText frmWorkList.spdWork, Trim(RS.Fields("성별")) & "", .MaxRows, colPSEX
                            SetText frmWorkList.spdWork, Trim(RS.Fields("나이")) & "", .MaxRows, colPAGE
...
'            SQL = SQL & "   AND a.STS_CD    = b.STS_CD " & vbCr
         ...
         Case "BIT"
            ...
            'SQL = SQL & "   AND L.LABBARCOD = '" & sBarcode & "' " & vbCrLf
            SQL = SQL & "   AND L.LABKEYNUM = D.DATKEYNUM " & vbCrLf                    '-- 테이블연결키값
            SQL = SQL & "   AND L.LABATTEND = D.DATATTEND " & vbCrLf                    '-- 내원번호
            SQL = SQL & "   AND L.LABATTEND = M.MANATTEND " & vbCrLf                    '-- 내원번호
            SQL = SQL & "   AND L.LABCHTNUM = D.DATCHTNUM " & vbCrLf                    '-- 챠트번호
            ...
            SQL = SQL & "   AND (L.LABRESULT = ''  OR L.LABRESULT IS NULL)" & vbCrLf
            SQL = SQL & "   AND L.LABENDDEP < '3' " & vbCrLf                            '-- 처리상태 (2:접수, 3:결과입력)
           ' SQL = SQL & " ORDER BY L.LABODRDTE, L.LABATTEND, L.LABBARCOD Desc, L.LABINSNUM, L.LABODRCOD "
         
         Case "BOBATH"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT ORCD as ITEM " & vbCr
            SQL = SQL & "  FROM bobath.V_LAB_ORDER_CP " & vbCr
            SQL = SQL & " WHERE SPNO = '" & strBarcode & "'" & vbCr
            SQL = SQL & "   AND ORCD IN (" & gAllTestCd & ") " & vbCr
            SQL = SQL & " ORDER BY ORCD"
                    
        Case "HWASAN"
            SQL = ""
            SQL = SQL & "SELECT T.TESTCD as ITEM" & vbCr
            SQL = SQL & "  FROM TC201 O, TC301 T " & vbCr
...
Function GetSampleInfos(ByVal asRow As Long) As String
    Dim RS1             As ADODB.Recordset
    Dim strBarcode      As String
    Dim strRegDate      As String
    Dim strChartNo      As String
...
'검사일련번호"
    ...
    If Not RS1.EOF = True And Not RS1.BOF = True Then
        Do Until RS1.EOF
            If strItems = "" Then
                strItems = GetTestInfos(Trim(RS1.Fields("ITEM")) & "", False)
            Else
                strItems = strItems & "/" & GetTestNm(Trim(RS1.Fields("ITEM")), False)
            End If
...
On Error GoTo DBErr
    ...
    Select Case gOCS
        ...
        Case "BIT"
            ...
            SQL = SQL & "SELECT DISTINCT "
            SQL = SQL & "  L.LABSERIAL "
            SQL = SQL & " ,L.LABATTEND as PID" '내원번호
            SQL = SQL & ", L.LABCHTNUM as CHARTNO" '챠트번호"
            SQL = SQL & ", L.LABODRDTE as HOSPDATE " '접수일자"
            SQL = SQL & ", M.MANADMFOR as IO" & vbCrLf
            SQL = SQL & " ,M.MANRESNUM as JUMIN"
            SQL = SQL & ", M.MANPATNAM as PNAME "
'            SQL = SQL & ", L.LABINSNUM as 처방순서"
'            SQL = SQL & ", L.LABSMPNAM as 검체명"
            SQL = SQL & ", L.LABBARCOD as BARCODE"
            SQL = SQL & ", L.LABODRSTP as SEQ " & vbCrLf
            'SQL = SQL & ", COUNT(L.LABODRCOD) as CNT"
            SQL = SQL & ", L.LABODRCOD as ITEM"
            SQL = SQL & "  FROM ME_LABDAT L, ME_DAT D, ME_MAN M" & vbCrLf
            SQL = SQL & " WHERE L.LABCHTNUM  = '" & strChartNo & "'" & vbCr
            SQL = SQL & "   AND L.LABODRDTE = '" & strRegDate & "'" & vbCr
            SQL = SQL & "   AND L.LABKEYNUM = D.DATKEYNUM " & vbCrLf                    '-- 테이블연결키값
            ...
            SQL = SQL & "   AND (L.LABRESULT = ''  OR L.LABRESULT IS NULL)" & vbCrLf
            SQL = SQL & "   AND L.LABENDDEP < '3' " & vbCrLf                            '-- 처리상태 (2:접수, 3:결과입력)
            'SQL = SQL & " GROUP BY L.LABSERIAL,L.LABATTEND,L.LABCHTNUM,L.LABODRDTE,M.MANADMFOR,M.MANRESNUM,M.MANPATNAM,L.LABBARCOD,L.LABODRSTP " & vbCr
            'SQL = SQL & " ORDER BY L.LABODRDTE, L.LABATTEND, L.LABBARCOD Desc, L.LABINSNUM, L.LABODRCOD "
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        SetText SPD, Trim(RS.Fields("PID")) & "", asRow, colPID
                        SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                        SetText SPD, Trim(RS.Fields("SEQ")) & "", asRow, colPAGE
                        SetText SPD, CStr(intTestCnt), asRow, colOCNT
         ...
         Case "BOBATH"
            ...
            SQL = ""
            SQL = SQL & "SELECT DISTINCT PAID as 환자번호, NWNO as 내원번호, SPNO as 바코드번호, '' as 챠트번호, ORDT as 접수일자, OIFL as 입외," & vbCrLf
            SQL = SQL & "       '' as 주민번호, PANM as 이름, '' as 처방순번,'' as 검체명, '' as SEQ, ORCD as ITEM " & vbCrLf
            SQL = SQL & "  FROM bobath.V_LAB_ORDER_CP " & vbCrLf
            SQL = SQL & " WHERE SPNO  = '" & strBarcode & "'" & vbCr
            SQL = SQL & "   AND ORCD IN (" & gAllTestCd & ")" & vbCrLf
            SQL = SQL & " ORDER BY ORDT, SPNO "
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        intTestCnt = intTestCnt + 1
                        SetText SPD, "1", asRow, colCHECKBOX
                        SetText SPD, Trim(RS.Fields("접수일자")) & "", asRow, colHOSPDATE
                        SetText SPD, Trim(RS.Fields("바코드번호")), asRow, colBARCODE
                        SetText SPD, Trim(RS.Fields("환자번호")) & "", asRow, colPID
                        SetText SPD, Trim(RS.Fields("내원번호")) & "", asRow, colCHARTNO
                        SetText SPD, Trim(RS.Fields("처방순번")) & "", asRow, colKEY1
                        SetText SPD, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                        SetText SPD, Trim(RS.Fields("입외")) & "", asRow, colINOUT
                        SetText SPD, CStr(intTestCnt), asRow, colOCNT
         ...
         Case "HWASAN"
            ...
            SQL = ""
            SQL = SQL & "SELECT O.OrdDt as 접수일자, O.PtID as 내원번호, O.PtNm as 이름, O.Sex as 성별, O.Age as 나이, O.SPCNO as 바코드번호,  T.TESTCD as ITEM" & vbCr
            SQL = SQL & "  FROM TC201 O, TC301 T " & vbCr
            SQL = SQL & " WHERE O.SPCNO = T.SPCNO " & vbCr
            SQL = SQL & "   AND O.SPCNO = '" & strBarcode & "'"
            SQL = SQL & "   And T.TESTCD in (" & gAllTestCd & ")" & vbCr
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        SetText SPD, Trim(RS.Fields("내원번호")) & "", asRow, colCHARTNO
                        SetText SPD, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                        SetText SPD, Trim(RS.Fields("성별")) & "", asRow, colPSEX
                        SetText SPD, Trim(RS.Fields("나이")) & "", asRow, colPAGE
                        SetText SPD, Format(frmMain.txtSeqNo.Text, "#0"), asRow, colSEQNO
                        frmMain.txtSeqNo.Text = Val(frmMain.txtSeqNo.Text) + 1
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    With frmMain
        ...
        If DBExec(AdoCn_Local, SQL) Then
            ...
            SQL = SQL & ",'" & Trim(GetText(.spdOrder, asRow1, colKEY1)) & "'"
...
'''
'''    SQL = ""
'''    SQL = SQL & "DELETE FROM PATRESULT "
'''
'''    If DBExec(AdoCn_Local_Send, SQL) Then
'''        With frmWorkList
'''
'''            For i = 1 To .spdWork.MaxRows
'''                If GetText(.spdWork, i, colCHECKBOX) = "1" Then
'''                    sExamDate = Format(Now, "yyyymmddhhmmss")
'''                    strSaveSeq = getMaxTestNum_Send(Format(frmMain.dtpToday, "yyyymmdd"))
'''
''''                    GetSampleInfos (intRow)
'''
'''                    SQL = ""
'''                    SQL = SQL & "INSERT INTO PATRESULT (" & vbCr
'''                    SQL = SQL & "SAVESEQ"                           '저장순번(날짜별)
...
'-- 오늘 검사한 날짜의 Max + 1 번호를 가져온다
Public Function getMaxTestNum_Send(ByVal strDate As String) As Long

    getMaxTestNum_Send = 1
    
          SQL = "SELECT MAX(SAVESEQ) as SEQ FROM PATRESULT  "
    SQL = SQL & " WHERE MID(EXAMDATE,1,8) = '" & strDate & "' " & vbCrLf
    
    Set RS = AdoCn_Local_Send.Execute(SQL, , 1)
    If Not RS.EOF = True And Not RS.BOF = True Then
        If Trim(RS.Fields("SEQ") & "") = "" Then
            getMaxTestNum_Send = 1
        Else
            getMaxTestNum_Send = Trim(RS.Fields("SEQ")) + 1
        End If
    Else
        getMaxTestNum_Send = 1
    End If
    
    If getMaxTestNum_Send >= 99999 Then
        getMaxTestNum_Send = 99999
    End If
