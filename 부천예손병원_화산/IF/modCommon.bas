...
'    '-- 로컬 DB 접속
'    If Not DbConnect_Local_Send Then
'        MsgBox "워크리스트 데이터베이스가 없습니다"
'        End
'    Else
'        cn_Local_Flag = True
'    End If
...
Public Sub GetSetup()
    ...
    strSetup = "":    strSetUp1 = ""
    Call GetPrivateProfileString("VIEW", "BARLENGTH", "", strSetup, 100, App.PATH & "\INI\" & gHOSP.APPNM & ".ini")
    strSetUp1 = Trim(strSetup)
    strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
    gBARLENGTH = Trim(strSetUp1)
