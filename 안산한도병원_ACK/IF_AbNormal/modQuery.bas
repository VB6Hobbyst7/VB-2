...
On Error GoTo RST
    ...
    If sGubun = 1 Then
        SQL = ""
        SQL = SQL & "select r.labdate as stadate,r.testitemseq,t.printnm,count(r.result) as cnt" & vbCr
        SQL = SQL & "  from c_result r, testitem t" & vbCr
        SQL = SQL & " where r.labdate between '" & pFrom & "' and '" & pTo & "'" & vbCr
        SQL = SQL & "   and r.machinecd = '!'" & vbCr
        SQL = SQL & "   and r.result <> ''" & vbCr
        SQL = SQL & "   and r.partgbn = '01'" & vbCr
        SQL = SQL & "   and r.SPECIMENCD = '002'" & vbCr
        SQL = SQL & "   and t.partcd = 'C'" & vbCr
        SQL = SQL & "   and r.partgbn = t.partgbn" & vbCr
        SQL = SQL & "   and r.SPECIMENCD = t.SPECIMENCD" & vbCr
        SQL = SQL & "   and r.testitemseq = t.testitemseq" & vbCr
        SQL = SQL & " group by r.labdate, r.testitemseq,t.printnm" & vbCr
        'SQL = SQL & "  having substring(r.labdate,1,6) = '" & strFromMon & "'"
        SQL = SQL & " order by r.labdate, r.testitemseq,t.printnm" & vbCr
...
'        SQL = SQL & " group by labdate,testitemseq" & vbCr
        ...
        SQL = ""
        SQL = SQL & "select substring(r.labdate,1,6) as stadate,r.testitemseq,t.printnm,count(r.result) as cnt" & vbCr
        SQL = SQL & "  from c_result r, testitem t" & vbCr
        SQL = SQL & " where r.labdate between '" & strFromMon & "' and '" & strToMon & "'" & vbCr
        SQL = SQL & "   and r.machinecd = '!'" & vbCr
        SQL = SQL & "   and r.result <> ''" & vbCr
        SQL = SQL & "   and r.partgbn = '01'" & vbCr
        SQL = SQL & "   and r.SPECIMENCD = '002'" & vbCr
        ...
        SQL = SQL & "   and r.SPECIMENCD = t.SPECIMENCD" & vbCr
        SQL = SQL & "   and r.testitemseq = t.testitemseq" & vbCr
        SQL = SQL & " group by substring(r.labdate,1,6), t.regservercd,t.printnm,r.testitemseq" & vbCr
'        SQL = SQL & "  having substring(r.labdate,1,6) = '" & strFromMon & "'" & vbCr
        SQL = SQL & " order by substring(r.labdate,1,6), r.testitemseq,t.printnm" & vbCr
        
    ElseIf sGubun = 3 Then
        strFromMon = Mid(pFrom, 1, 4) & "0101"
        strToMon = Mid(pTo, 1, 4) & "1231"
        
        SQL = ""
        SQL = SQL & "select substring(r.labdate,1,4) as stadate,r.testitemseq,t.printnm,count(r.result) as cnt" & vbCr
        SQL = SQL & "  from c_result r, testitem t" & vbCr
        SQL = SQL & " where r.labdate between '" & strFromMon & "' and '" & strToMon & "'" & vbCr
        SQL = SQL & "   and r.machinecd = '!'" & vbCr
        SQL = SQL & "   and r.result <> ''" & vbCr
        ...
        SQL = SQL & "   and r.SPECIMENCD = t.SPECIMENCD" & vbCr
        SQL = SQL & "   and r.testitemseq = t.testitemseq" & vbCr
        SQL = SQL & " group by substring(r.labdate,1,4), t.regservercd,t.printnm,r.testitemseq" & vbCr
        SQL = SQL & " order by substring(r.labdate,1,4), r.testitemseq,t.printnm" & vbCr
    End If
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
            ...
            With SPD
                ...
                If sGubun = 1 Then
                    If intRow = 0 Then
                        ...
                        .MaxCols = intCol
                        .ColWidth(intCol) = 10
                        SetText SPD, Format(Trim(RS.Fields("STADATE")), "####/##/##"), 0, intCol
                    Else
                        blnSame = False
                        For iCnt = 3 To .MaxCols
                            If Format(Trim(RS.Fields("STADATE")), "####/##/##") = GetText(SPD, 0, iCnt) Then
                                blnSame = True
                                Exit For
                            End If
                        Next
                    ...
                    For iCnt = 1 To .MaxRows
                        If Trim(RS.Fields("PRINTNM")) & "" = GetText(SPD, iCnt, 2) Then
                            blnSame = True
                            Exit For
                        End If
                    Next
                    ...
                    Else
                        SetText SPD, intRow, intRow, 1
                        SetText SPD, Trim(RS.Fields("CNT")) & "", iCnt, intCol
                    End If
                '월계
                ElseIf sGubun = 2 Then
                    If intRow = 0 Then
        ...
        SetText SPD, "소계", 0, SPD.MaxCols
        SPD.ColWidth(intCol) = 12
        dblSumTot = 0
        
        For intRow = 1 To SPD.MaxRows
            dblSum = 0
            For intCol = 3 To SPD.MaxCols - 1
                dblSum = dblSum + IIf(GetText(SPD, intRow, intCol) = "", 0, GetText(SPD, intRow, intCol))
            Next
            SetText SPD, dblSum, intRow, SPD.MaxCols
            dblSumTot = dblSumTot + dblSum
        Next
            
        SPD.MaxRows = SPD.MaxRows + 1
        SetText SPD, "합게", SPD.MaxRows, 2
        SetText SPD, dblSumTot, SPD.MaxRows, SPD.MaxCols
