...
Public Sub GetTestList()
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        With frmMain.spdTest
            ...
            ReDim Preserve gArrEQP(AdoRs_Local.RecordCount + 2, 28)
            ...
            Do Until AdoRs_Local.EOF
               ...
               ' Call SetText(frmMain.spdTest, IIf(AdoRs_Local.Fields("QCTemp").Value & "" = "1", "사용", "미사용"), intRow, colLUseResSpec)
                ...
                gArrEQP(intRow, 21) = ""    '이전LABNO로  사용 (ACK : BEFORELABNO)
                
                gArrEQP(intRow, 22) = ""    ' 남 상
                gArrEQP(intRow, 23) = ""    ' 남 하
                gArrEQP(intRow, 24) = ""    ' 여 상
                gArrEQP(intRow, 25) = ""    ' 여 하
                gArrEQP(intRow, 26) = ""    ' 패닉 하
                gArrEQP(intRow, 27) = ""    ' 패닉 상
                gArrEQP(intRow, 28) = ""    ' 델타
...
'''    SQL = ""
'''    SQL = SQL & "SELECT DISTINCT"
'''    SQL = SQL & "       M.LABDATE   as HOSPDATE             " & vbCrLf
'''    SQL = SQL & "     , M.PARTGBN,  M.LABSEQ, M.SPECIMENCD  " & vbCrLf
'''    SQL = SQL & "     , M.NAME      as PNAME                " & vbCrLf
'''    SQL = SQL & "     , M.REGNO     as PID                  " & vbCrLf
'''    SQL = SQL & "     , M.AGE       as AGE                  " & vbCrLf
'''    SQL = SQL & "     , M.SEX       as SEX                  " & vbCrLf
'''    SQL = SQL & "     , M.IDLEFT + M.IDRIGHT AS PJUMIN      " & vbCrLf
'''    SQL = SQL & "     , M.MEDICALREMARK as BARCODE          " & vbCrLf
'''    SQL = SQL & "     , M.RESULTENDYN, R.SUBMCD             " & vbCrLf
'''    SQL = SQL & "     , R.TESTITEMSEQ as ITEM               " & vbCrLf
'''    SQL = SQL & "     , R.DELTAGBN                          " & vbCrLf
'''    SQL = SQL & "     , R.BEFORERESULT                      " & vbCrLf
'''    SQL = SQL & "     , R.BEFORELABNO                       " & vbCrLf
'''    '2019-04-29 추가 c_Abnormal 테이블 용
'''    SQL = SQL & "     , M.DEPTCD                            " & vbCrLf
'''    SQL = SQL & "     , M.WARD                              " & vbCrLf
'''    SQL = SQL & "     , M.DOCTOR                            " & vbCrLf
'''    SQL = SQL & "     , M.JUBSUGBN                          " & vbCrLf
'''
'''
'''    Select Case gHOSP.PARTCD
...
'''    SQL = SQL & "   AND (R.RESULT = '' OR R.RESULT IS NULL) " & vbCr
    ...
    SQL = SQL & "     , S.REFHIGHM                          " & vbCrLf
    SQL = SQL & "     , S.REFLOWM                           " & vbCrLf
    SQL = SQL & "     , S.REFHIGHF                          " & vbCrLf
    SQL = SQL & "     , S.REFLOWF                           " & vbCrLf
    SQL = SQL & "     , S.PANICLOW                          " & vbCrLf
    SQL = SQL & "     , S.PANICHIGH                         " & vbCrLf
    SQL = SQL & "     , S.DELTAVAL                          " & vbCrLf


    Select Case gHOSP.PARTCD
        Case "H": SQL = SQL & "  FROM H_JUBSU M, H_RESULT R, TESTITEM S " & vbCrLf   '혈액학
        Case "C": SQL = SQL & "  FROM C_JUBSU M, C_RESULT R, TESTITEM S " & vbCrLf   '생화학
        Case "J": SQL = SQL & "  FROM J_JUBSU M, J_RESULT R, TESTITEM S " & vbCrLf   '수탁
        Case "M": SQL = SQL & "  FROM M_JUBSU M, M_RESULT R, TESTITEM S " & vbCrLf   '미생물
        Case "S": SQL = SQL & "  FROM S_JUBSU M, S_RESULT R, TESTITEM S " & vbCrLf   '면역학
        Case "U": SQL = SQL & "  FROM U_JUBSU M, U_RESULT R, TESTITEM S " & vbCrLf   '뇨화학
    End Select

    SQL = SQL & " WHERE M.MEDICALREMARK = '" & strBarcode & "'  " & vbCrLf
    SQL = SQL & "   AND M.LABDATE       = R.LABDATE             " & vbCrLf
    SQL = SQL & "   AND M.PARTGBN       = R.PARTGBN             " & vbCrLf
    SQL = SQL & "   AND M.LABSEQ        = R.LABSEQ              " & vbCrLf
    SQL = SQL & "   AND M.SPECIMENCD    = R.SPECIMENCD          " & vbCrLf
    SQL = SQL & "   AND S.PARTCD        = '" & gHOSP.PARTCD & "'" & vbCrLf
    SQL = SQL & "   AND R.PARTGBN       = S.PARTGBN             " & vbCrLf
    SQL = SQL & "   AND R.SPECIMENCD    = S.SPECIMENCD          " & vbCrLf
    SQL = SQL & "   AND R.TESTITEMSEQ   = S.TESTITEMSEQ         " & vbCrLf
    SQL = SQL & "   AND S.SUBMCD        = 'NNNN'                " & vbCrLf
    SQL = SQL & "   And R.TESTITEMSEQ   IN (" & gAllTestCd & ") " & vbCrLf
    SQL = SQL & "   AND R.CONFIRM       <> 'Y'                  " & vbCrLf
    SQL = SQL & "   AND (R.RESULT = '' OR R.RESULT IS NULL)     " & vbCrLf
...
'     , S.PANICHIGH
'     , S.DELTAVAL
'  FROM C_JUBSU M, C_RESULT R ,TESTITEM S
' WHERE M.MEDICALREMARK = '10410356'
'   AND M.LABDATE       = R.LABDATE
'   AND M.PARTGBN       = R.PARTGBN
'   AND M.LABSEQ        = R.LABSEQ
'   AND M.SPECIMENCD    = R.SPECIMENCD
'   AND S.PARTCD = 'C'
'   AND R.PARTGBN = S.PARTGBN
'   AND R.SPECIMENCD = S.SPECIMENCD
'   AND R.TESTITEMSEQ =  S.TESTITEMSEQ
'   AND S.SUBMCD = 'NNNN'
...
'   AND R.CONFIRM       <> 'Y'
'   AND (R.RESULT = '' OR R.RESULT IS NULL)
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                If Trim(RS.Fields("JUBSUGBN")) & "" = "0" Then
                   strJubsuGbn = "입원"
                ElseIf Trim(RS.Fields("JUBSUGBN")) & "" = "1" Then
                    strJubsuGbn = "외래"
                Else
                    strJubsuGbn = Trim(RS.Fields("JUBSUGBN")) & ""
                End If
                ...
                '-- 2019-04-29
                With mResult
                    .DEPTNM = Trim(RS.Fields("DEPTCD")) & ""
                    .WARD = Trim(RS.Fields("WARD")) & ""
                    .DOCTOR = Trim(RS.Fields("DOCTOR")) & ""
                    .JUBSUGBN = strJubsuGbn
                End With
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        '-- BEFORELABNO
                        gArrEQP(intCol - colSTATE, 21) = Trim(RS.Fields("BEFORELABNO")) & ""
                        '남상,남하,여상,여하,패닉하,패닉상,델타
                        gArrEQP(intCol - colSTATE, 22) = Trim(RS.Fields("REFHIGHM")) & ""
                        gArrEQP(intCol - colSTATE, 23) = Trim(RS.Fields("REFLOWM")) & ""
                        gArrEQP(intCol - colSTATE, 24) = Trim(RS.Fields("REFHIGHF")) & ""
                        gArrEQP(intCol - colSTATE, 25) = Trim(RS.Fields("REFLOWF")) & ""
                        gArrEQP(intCol - colSTATE, 26) = Trim(RS.Fields("PANICLOW")) & ""
                        gArrEQP(intCol - colSTATE, 27) = Trim(RS.Fields("PANICHIGH")) & ""
                        gArrEQP(intCol - colSTATE, 28) = Trim(RS.Fields("DELTAVAL")) & ""
        ...
        Select Case gEMR
            ...
            Case Else
                ...
                If DBExec(AdoCn_Local, SQL) Then
                    ...
                    SQL = SQL & ",'" & gHOSP.USERID & "'"
                    '-- 2019-04-29
                    SQL = SQL & ",'" & mResult.DEPTNM & "|" & mResult.WARD & "'"
                    SQL = SQL & ",'" & mResult.DOCTOR & "|" & mResult.JUBSUGBN & "'"
                    SQL = SQL & ",'" & mResult.REFLOW & "|" & mResult.REFHIGH & "'"
                    SQL = SQL & ",'" & mResult.PANICLOW & "|" & mResult.PANICHIGH & "'"
                    SQL = SQL & ",'" & mResult.DELTALOW & "|" & mResult.DELTAHIGH & "|" & mResult.DELTAGBN & "'"
                    SQL = SQL & ",'" & mResult.BeforeRslt & "'"
                    SQL = SQL & ",'" & mResult.BeforeLabNo & "'"
                    SQL = SQL & ",'" & mResult.BeforeDept & "|" & mResult.BeforeWard & "'"
                    SQL = SQL & ",'" & mResult.BeforeDoctor & "|" & mResult.BeforeJubsuGbn & "'"
