...
Type IntfData
    ...
    BeforeLabNo As String
    BeforeDept  As String
    BeforeWard  As String
    BeforeDoctor  As String
    BeforeJubsuGbn  As String
...
'        Call SetSQLData("로컬결과조회", SQL)
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strHospDate <> "" And strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                '상태코드 가져오기
                CriticalCd = ""
                CriticalCd = GetStatusCd(gHOSP.PARTCD, strPartGbn, strSpcCd, strTestCd, sResult)
                ...
                If CriticalCd <> "" Then
                    ...
                    SQL = SQL & "  From FLAG "
                    SQL = SQL & " Where LABDATE     = '" & strHospDate & "' " & vbCrLf
                    SQL = SQL & "   And PARTCD      = '" & gHOSP.PARTCD & "'" & vbCrLf
                    SQL = SQL & "   And PARTGBN     = '" & strPartGbn & "'  " & vbCrLf
                    SQL = SQL & "   And LABSEQ      = '" & strSubCode & "'  " & vbCrLf
                    SQL = SQL & "   And SUBMCD      = 'NNNN'                " & vbCrLf
                    SQL = SQL & "   And SPECIMENCD  = '" & strSpcCd & "'    " & vbCrLf
                    ...
                    If Not RS_S.EOF = True And Not RS_S.BOF = True Then
                        If IsNull(RS_S.Fields("CNT")) Or RS_S.Fields("CNT") = 0 Then
                            ...
                            SQL = SQL & ",'" & strTestCd & "'"
                            SQL = SQL & ",'NNNN'"
                            SQL = SQL & ",'" & CriticalCd & "')" & vbCrLf
                        Else
                            'Update
                            SQL = ""
                            SQL = SQL & "Update FLAG Set " & vbCrLf
                            SQL = SQL & "      STATUSCD         = '" & CriticalCd & "'      " & vbCrLf
                            SQL = SQL & " Where LABDATE         = '" & strHospDate & "'     " & vbCrLf
                            SQL = SQL & "   And PARTCD          = '" & gHOSP.PARTCD & "'    " & vbCrLf
                            SQL = SQL & "   And PARTGBN         = '" & strPartGbn & "'      " & vbCrLf
                            SQL = SQL & "   And LABSEQ          = '" & strSubCode & "'      " & vbCrLf
                        ...
                        Call SetSQLData("Crit결과저장", SQL, "A")
                        AdoCn.Execute SQL
...
'-- 결과에 따른 상태코드 가져오기.
Function GetStatusCd(asPartCD As String, asPartGbn As String, asSpecimenCd As String, asTestItem As String, dRes As String) As String
    Dim RS_S    As ADODB.Recordset
    Dim dLow    As Double
    Dim dHigh   As Double
    Dim sLow    As String
...
On Error GoTo ErrHandle
    ...
    SQL = ""
    SQL = SQL & "Select STATUSCD, COMPOPERATOR, CRITICALVAL "
    SQL = SQL & "  From STATUS                              "
    SQL = SQL & " Where PARTCD      = '" & asPartCD & "'" & vbCrLf
    SQL = SQL & "   And PARTGBN     = '" & asPartGbn & "'" & vbCrLf
    SQL = SQL & "   And SPECIMENCD  = '" & asSpecimenCd & "'" & vbCrLf
    SQL = SQL & "   And TESTITEMSEQ = '" & asTestItem & "'" & vbCrLf

'T-Ca
'STATUSCD    COMPOPERATOR    CRITICALVAL
'1   이상 12
'2   이하 6
    ...
    If Not RS_S.EOF = True And Not RS_S.BOF = True Then
        Do Until RS_S.EOF
            If RS_S.Fields("COMPOPERATOR") & "" = "이하" Then '6 T.ca
                sLow = RS_S.Fields("CRITICALVAL") & ""
                LowCd = RS_S.Fields("STATUSCD") & ""
            ElseIf RS_S.Fields("COMPOPERATOR") & "" = "이상" Then '12
                sHigh = RS_S.Fields("CRITICALVAL") & ""
                HighCd = RS_S.Fields("STATUSCD") & ""
            End If
            RS_S.MoveNext
        Loop
    End If
...
'''
''''상태플래그 저장
'''function SaveHospFlag(asLabDate as String , asPartCD  as String, asPartGbn  as String, asLabSeq  as String, asSpecimenCd  as String, asTestItem:string; dRes:double): boolean;
'''Var
'''  CriticalCd:string;
'''  UpCnt:integer;
'''begin
...
RST:
'                strErrMsg = "위    치 : " & gHOSP.MACHNM & "_GetAbNormal" & vbNewLine & vbNewLine
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Select Case gEMR
        Case "ACK"
            SaveTransData = SaveTransData_ACK(argSpcRow)
...
'    Call SetSQLData("판정", SQL, "A")
    ...
    Do Until RSJ.EOF
        ...
        '-- 2019-04-30
        mResult.REFLOW = RSJ.Fields("MIN")
        mResult.REFHIGH = RSJ.Fields("MAX")
...
Function GetBeforeResult_ACK(asLabDate As String, asPartGbn As String, asSpecimenCd As String, asTestItem As String, asRegNo As String, asLabSeq As String) As String
    Dim RSJ             As ADODB.Recordset
...
On Error GoTo ErrorTrap
    ...
    SQL = SQL & "   AND LABSEQ       = '" & asLabSeq & "'" & vbCr
...
ErrorTrap:
    GetBeforeResult_ACK = ""
                ...
                strErrMsg = "위    치 : " & gHOSP.MACHNM & "_GetBeforeResult_ACK" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show 'vbModal
...
Sub GetBeforePatInfo_ACK(asLabDate As String, asPartGbn As String, asSpecimenCd As String, asRegNo As String, asLabSeq As String)
    Dim RSJ             As ADODB.Recordset
