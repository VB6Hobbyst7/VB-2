...
Private Sub EditRcvDataASTM()
    ...
    strRecvData(1) = Replace(strRecvData(1), vbLf, "")
    varRcvBuf = Split(strRecvData(1), vbCr)
    
    For intCnt = 0 To UBound(varRcvBuf)
        
        strTemp1 = varRcvBuf(intCnt)
        
        Select Case intCnt
        Case 4
        'u 000000000AUTO_SID012               --AUTO가 나오면 끝3자리 이용해서 12로 사용
            If InStr(strTemp1, "AUTO_SID") > 0 Then
                strSeq = Mid(strTemp1, InStr(strTemp1, "AUTO_SID") + 8)
