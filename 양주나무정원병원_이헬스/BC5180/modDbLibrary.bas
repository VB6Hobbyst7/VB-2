...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            strSubCD = Trim(GetText(.vasTemp, iRow, 14))
            ...
            '-- Seq 찾아오기
            strOrd_Seq_No = GetOrdSeq(lsID, strChtNum, lsPid, strEqpCd, strSubCD)
            
            If sResult <> "" And strOrd_Seq_No <> "" Then
                      SQL = "Update OBSURSTM Set "
                SQL = SQL & "  OBSURSLT = '" & sResult & "'" & vbCr '수치형결과
                SQL = SQL & " ,OBSUENDT = '" & strRstDT & "'" & vbCr '(수치형 + 서술형 등 모든결과)
                SQL = SQL & " ,OBSUENTM = '" & strRstTM & "'" & vbCr
                SQL = SQL & " ,OBSUENID = '" & gUserID & "'" & vbCr
                SQL = SQL & " where OBSUMRNO = '" & lsID & "'" & vbCr
                SQL = SQL & "   And OBSUCASE = '" & strChtNum & "'" & vbCr
                SQL = SQL & "   And OBSUORNO = '" & lsPid & "'" & vbCr
                SQL = SQL & "   And OBSUORSQ = '" & strOrd_Seq_No & "'" & vbCr
                SQL = SQL & "   And OBSUCODE = '" & strEqpCd & "'" & vbCr
                SQL = SQL & "   And OBSUSUBC = '" & strSubCD & "'"
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_EHEALTH(ByVal asRow As Long) As Integer
    Dim sBarcode            As String
    Dim GetOrderExamCode    As String
    Dim intCol              As Integer
    Dim strTestCd           As String
    ...
    GetSampleInfoW_EHEALTH = -1
    
    sBarcode = Trim(GetText(frmInterface.vasID, asRow, colBARCODE))
    sChart = Trim(GetText(frmInterface.vasID, asRow, colCHARTNO))
    sPID = Trim(GetText(frmInterface.vasID, asRow, colPID))
...
'          SQL = "SELECT DISTINCT  a.APATMRNO"   '등록번호
...
'    SQL = SQL & "   AND b.APATMRNO = c.OBSUMRNO " & vbCr                                '등록번호,고객번호
...
'    SQL = SQL & "   AND (c.OBSURSLT IS NULL OR c.OBSURSLT = '')"                        '검사결과
...
'    SQL = SQL & "   AND c.OBSUCODE + '|' + c.OBSUSUBC IN (" & gAllExam & ") " & vbCr    '검사코드 + '|' + OBSUSUBC
'    SQL = SQL & "   AND b.OBODSTAT = 'AC' "                                             '필수 기본 = 'OE', 채혈시 = 'AC'
'    SQL = SQL & " Order by c.OBODORDT,a.APATMRNO,b.OBODMRNO,b.OBODORNO,b.OBODORSQ " & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("ORDCD")) & "',"
                RS.MoveNext
            Loop
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_EHEALTH(ByVal asRow As Long) As Integer
    Dim sBarcode            As String
    Dim GetOrderExamCode    As String
    Dim intCol              As Integer
    Dim strTestCd           As String
...
'    SQL = SQL & ", c.OBSUCODE"                  '검사코드
'    SQL = SQL & ", c.OBSUSUBC" & vbCr           '검사코드SUB
    SQL = SQL & ", RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) AS ORDCD"
    SQL = SQL & "  FROM ABPATMST a"             '환자기본정보
    SQL = SQL & "      ,OBODRMTM b"            '처방내역 Table
    SQL = SQL & "      ,OBSURSTM c " & vbCr     '검사결과(수치결과) Table
    SQL = SQL & " WHERE a.APATMRNO = b.OBODMRNO " & vbCr                                '등록번호,고객번호
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText frmInterface.vasID, "1", asRow, colCheckBox
                SetText frmInterface.vasID, Trim(RS.Fields("OBODORDT")) & "", asRow, colHOSPDATE
                SetText frmInterface.vasID, Trim(RS.Fields("APATMRNO")) & "", asRow, colBARCODE
                SetText frmInterface.vasID, Trim(RS.Fields("OBODCASE")) & "", asRow, colCHARTNO
                SetText frmInterface.vasID, Trim(RS.Fields("OBODORNO")) & "", asRow, colPID
                SetText frmInterface.vasID, IIf(Trim(RS.Fields("OBODIOGB")) & "" = "I", "입원", "외래"), asRow, colINOUT
                SetText frmInterface.vasID, Trim(RS.Fields("APATNAME")) & "", asRow, colPNAME
                SetText frmInterface.vasID, Trim(RS.Fields("APATPSEX")) & "", asRow, colPSEX
                SetText frmInterface.vasID, Trim(RS.Fields("APATJMN1")) & "", asRow, colPAGE
                With mOrder
                    .PID = Trim(RS.Fields("OBODORNO")) & ""
                    .PNAME = Trim(RS.Fields("APATNAME")) & ""
                    .BarNo = Trim(RS.Fields("APATMRNO")) & ""
                End With
                
                For intCol = colState + 1 To frmInterface.vasID.MaxCols
                    If Trim(RS.Fields("ORDCD")) = gArrEquip(intCol - colState, 3) Then
                        frmInterface.vasID.Row = asRow
                        frmInterface.vasID.Col = intCol
                        frmInterface.vasID.BackColor = vbYellow
                        Exit For
                    End If
                Next
...
Function GetOrdSeq(ByVal pOBSUMRNO As String, ByVal pOBSUCASE As String, ByVal pOBSUORNO As String, ByVal pOBSUCODE As String, ByVal pOBSUSUBC As String) As String

    GetOrdSeq = ""

          SQL = "Select OBSUORSQ From OBSURSTM " & vbCr
    SQL = SQL & " Where OBSUMRNO = '" & pOBSUMRNO & "'" & vbCr
    SQL = SQL & "   And OBSUCASE = '" & pOBSUCASE & "'" & vbCr
    SQL = SQL & "   And OBSUORNO = '" & pOBSUORNO & "'" & vbCr
    SQL = SQL & "   And OBSUCODE = '" & pOBSUCODE & "'" & vbCr
    SQL = SQL & "   And OBSUSUBC = '" & pOBSUSUBC & "'"
