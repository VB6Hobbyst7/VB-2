...
Function GetOrdSeq(pOBSUMRNO As String, pOBSUCASE As String, pOBSUORNO As String, pOBSUCODE As String, pOBSUSUBC) As String
...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Function GetOrderExamCode(argEquipCode As String, asRow As Long) As String
    ...
    sBarcode = Trim(GetText(frmWorkList.vasWorkList, asRow, colBARCODE))
    ...
    SQL = SQL & "  FROM EQPMASTER "
    SQL = SQL & " WHERE EQUIPNO  = '" & Trim(gEquip) & "' "
    SQL = SQL & "   AND EXAMCODE in (" & Trim(GetOrderExamCode) & ")"
    SQL = SQL & " ORDER BY EQUIPCODE "
