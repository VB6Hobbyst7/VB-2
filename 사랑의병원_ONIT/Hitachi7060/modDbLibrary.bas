...
'                Else
'                    SQL = "Update medicom..jun370_resulttb" & _
                          "   Set Result = '" & sResult & "'" & _
...
'    If Len(ValargPID) > 6 Then
'        SQL = ""
'        SQL = SQL & "SELECT Distinct INTERFACECODE " & vbCrLf
'        SQL = SQL & "  FROM ONIT..GUMJIN_INTERFACE  " & vbCrLf
'        SQL = SQL & " WHERE PER_GUMJIN_DATE = '" & Mid(argPID, 1, 8) & "' " & vbCrLf
'        SQL = SQL & "   AND PER_GUM_NUM = '" & Mid(argPID, 9) & "' " & vbCrLf
'        SQL = SQL & "   AND STATUS = '0' " & vbCrLf
'        SQL = SQL & "   AND RESULT = ''  OR RESULT IS NULL"
'
'    Else
'        SQL = ""
'        SQL = SQL & " SELECT Distinct b.map2seqno "
'        SQL = SQL & "   FROM onit_out..jun370_resulttb b, onit_out..pewprsnp c, onit_out..BAGMAP2PREF d "
'        SQL = SQL & "  WHERE b.orderorder = '" & argPID & "' "
'        SQL = SQL & "    AND d.labno in (4) "
'        SQL = SQL & "    AND b.map2seqno = d.map2seqno "
