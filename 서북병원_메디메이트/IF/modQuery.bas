...
Function SaveTransData_MEDIMATE(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
...
'                End If
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    '-- 서버저장
                    SQL = ""
                    SQL = SQL & "UPDATE EXAMRES SET " & vbCrLf
                    SQL = SQL & "       EXAMUID    =   '" & gHOSP.USERID & "'   " & vbCrLf
                    SQL = SQL & "     , EXAMDATE   =   SYSDATE                  " & vbCrLf '/판독일로 쓰이는데 전산과장이 변경되지 않게 해달라고 함."
'                    SQL = SQL & "     , EXAMSTATE  =   'D'                      " & vbCrLf
                    SQL = SQL & "     , RESULT     =   '" & sResult & "'        " & vbCrLf
                    SQL = SQL & "     , EXAMSTATE = "
                    SQL = SQL & "                 (CASE "
                    SQL = SQL & "                  WHEN NVL(EXAMSTATE,' ') = 'B'"
                    SQL = SQL & "                       THEN 'D'"
                    SQL = SQL & "                  END)                         " & vbCrLf
                    SQL = SQL & " WHERE SPECIMENID =   '" & strBarcode & "'     " & vbCrLf
                    SQL = SQL & "   AND EXAMCODE   =   '" & strTestCd & "'      " & vbCrLf
                    SQL = SQL & "   AND NVL(RESEND,' ') || NVL(EXAMSTATE,' ') <> '1D' " & vbCrLf
                    SQL = SQL & "   AND LABRECYN = 'Y' "
...
'''                                             '결과상태"
     ...
     ' SQL.Text:='   UPDATE EXAMRES                                    '+#13#10+
                '   SET RESULT = '''+TMaster.FResult+'''              '+#13#10+
                '       ,EXAMUID = '''+TGlobal.FUserID+'''            '+#13#10+
                '       ,EXAMDATE = SYSDATE                           '+#13#10+
                '       ,EXAMSTATE =                                  '+#13#10+
                '                 (CASE                               '+#13#10+
                '                  WHEN NVL(EXAMSTATE,'' '')  = ''B'' '+#13#10+
                '                       THEN ''D''                    '+#13#10+
                '                  END)                               '+#13#10+
                ' WHERE SPECIMENID =  '''+PadLeftStr(TMaster.FBarCode, '0', 12)+''' '+#13#10+
                '   AND EXAMCODE = '''+TMaster.FExamCode+''' '+#13#10+
                '   AND NVL(RESEND,'' '') || NVL(EXAMSTATE,'' '') <> ''1D'' '+#13#10+
                '   AND LABRECYN = ''Y'' ';
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_MEDIMATE" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        If Trim(strPatID) = "" Then
            Exit Function
        Else
            strSchBarNo = Space(10 - Len(strPatID)) & strPatID
        End If
...
'                End If
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    SQL = SQL & " WHERE RESOCMNUM = '" & strSchBarNo & "'                           " & vbCrLf
            ...
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    With AdoCmd
                        ...
                        Set prm4 = .CreateParameter("EQP_CD", adVarChar, adParamInput, 15, gHOSP.MACHCD)    '장비코드 'B09' 또는 'B10'
                        .Parameters.Append prm4
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "MEDIMATE"                      'MEDIMATE
                Call GetWorkList_MEDIMATE(pFrom, pTo, SPD)
...
'    SQL = SQL & "    , R.ResLabCod AS ITEM                                   " & vbCrLf
    ...
    SQL = SQL & "      and O.odrdtm Between '" & pFrom & "' and '" & pTo & "'   " & vbCrLf
    If frmWorkList.chkSave.Value = "0" Then
        SQL = SQL & "      and (R.ResRltVal is null or R.ResRltVal = '')            " & vbCrLf
    End If
'    SQL = SQL & " ORDER BY W.OcmAcpDtm , R.ResOcmNum , W.OcmChtNum "
    SQL = SQL & " GROUP BY P.PbsPatNam, P.PbsResNum , W.OcmAcpDtm, R.ResOcmNum, W.OcmChtNum " & vbCrLf
      
      
'select distinct ltrim(rtrim(a.ospchtnum)) as ptno, ltrim(rtrim(b.pbspatnam)) as ptnm, substring(a.ospodrdtm, 1, 8) as orddt, a.ospocmnum, a.ospchtnum
'from dbo.ospinf a, dbo.pbsinf b, dbo.resinf c, dbo.OcmInf d
'where a.ospodrdtm >= '201910190000' and a.ospodrdtm <= '201910192359' and a.ospcsmdep = 'LAB'  and a.ospdelflg = 'N'  and a.ospcanyon = 'N'  and a.OspOcmNum = d.OcmNum  and d.OcmComStt In ('PN', 'TN', 'OE', 'VN','HN','WN')  and d.OcmRsuYon In ('Y', '', 'N')  and ltrim(rtrim(b.pbschtnum)) = ltrim(rtrim(a.ospchtnum))  and c.resocmnum = a.ospocmnum  and c.resodrseq = a.ospodrseq  and c.resseq = a.ospseq  and upper(ltrim(rtrim(c.reslabcod))) in ('','Tprot','alb','ALBUMIN','C3720','TBIL','T-bil','AST','GOT','ALT','GPT','ALK_PHOS','ALK-PHOS','B2602','b2710','B2710_2','B2710-1','G-GTP','RGT','c3711','GLU1','GLUCOS','C2411','T-CHO1','TCHOL','C2420','HDL','C2443','C2443_1','C2443-1','TG1','BUN','CR','CREA','CREATIN','URIC ACID','URIC_ACID','Vit_D3','vit_D3_1','CRP정량','C4903','P','LDH','ldh','cpk','','','')
' and ltrim(rtrim(isnull(c.resrltval, ''))) = ''  order by a.ospchtnum, ptnm
...
Public Sub GetWorkList_MEDIMATE(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
'    SQL = SQL & " ORDER BY READING_YMD,PTNT_NO,BCODE_NO                         " & vbCrLf
    ...
    '-- 검사대상자 가져오기                             '검체번호(접수번호)                             바코드번호(검체번호)
    '      SQL = "Select Distinct a.PID, b.PNAME, b.SEX, a.RECENO, a.SEQNO, a.EXAMCODE, a.SPECIMENCODE,a.SPECIMENID "
    
    SQL = ""
    SQL = SQL & "Select Distinct "
    SQL = SQL & "       a.PRSC_DATE                 AS HOSPDATE     " & vbCrLf
    SQL = SQL & "     , a.PID                       AS PID          " & vbCrLf
    SQL = SQL & "     , b.PNAME                     AS PNAME        " & vbCrLf
    SQL = SQL & "     , b.SEX                       AS SEX          " & vbCrLf
    SQL = SQL & "     , a.RECENO                    AS CHARTNO      " & vbCrLf
    SQL = SQL & "     , a.SEQNO                                     " & vbCrLf
    SQL = SQL & "     , a.EXAMCODE                                  " & vbCrLf
    SQL = SQL & "     , a.SPECIMENCODE                              " & vbCrLf
    SQL = SQL & "     , a.SPECIMENID                AS BARCODE      " & vbCrLf
    SQL = SQL & "  From EXAMRES a, PATIENT b                        " & vbCrLf
    SQL = SQL & " Where a.PRSC_DATE between TO_DATE(" & pFrom & ",'yyyymmdd') + 0.000000 "
    SQL = SQL & "   AND TO_DATE(" & pTo & ",'yyyymmdd') + 0.999999 " & vbCrLf
    'SQL = SQL & "   AND substr(EXAMCODE,1,5) = 'L4109'"
    SQL = SQL & "   AND a.EXAMCODE IN (" & gAllTestCd & ")          " & vbCrLf
    'SQL = SQL & "   AND a.EXAMCODE = '" & gComm.ORDCODE & "'        " & vbCrLf
    SQL = SQL & "   AND a.EXAMSTATE >= 'B'                          " & vbCrLf     '접수
    SQL = SQL & "   AND a.PID = b.PID                               " & vbCrLf
    SQL = SQL & "   AND (NVL(a.RESEND,' ') <> '1'                   " & vbCrLf
    SQL = SQL & "        OR (a.RESEND = '1' AND a.EXAMSTATE = 'E')) " & vbCrLf
    SQL = SQL & " Order By  a.RECENO,a.SEQNO, a.PID                 " & vbCrLf
...
'
'        SetText vasID, Trim(RS.Fields(5) & ""), J, colRack   '5  코드
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_MEDIMATE" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
'            SQL = SQL & "   And 처방코드+서브코드 in (" & gAllExam & ")"
        ...
        Case "MEDIMATE"
            SQL = ""
            SQL = SQL & "Select Distinct "
            SQL = SQL & "       a.EXAMCODE                  AS ITEM         " & vbCrLf
            SQL = SQL & "  From EXAMRES a, PATIENT b                        " & vbCrLf
            SQL = SQL & " Where SPECIMENID = '" & strBarcode & "'           " & vbCrLf
            'SQL = SQL & "   AND a.EXAMCODE IN (" & gAllTestCd & ")          " & vbCrLf
            SQL = SQL & "   AND a.EXAMCODE = '" & gComm.ORDCODE & "'        " & vbCrLf
            ...
            'SQL = SQL & "   AND (NVL(a.RESEND,' ') <> '1'                   " & vbCrLf
            'SQL = SQL & "        OR (a.RESEND = '1' AND a.EXAMSTATE = 'E')) " & vbCrLf
            SQL = SQL & " Order By  a.EXAMCODE                              " & vbCrLf
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "MEDIMATE"
            SaveTransData = SaveTransData_MEDIMATE(argSpcRow, SPD)
...
'    Call SetCommStatus("R", pBarNo, frmMain.lstComStatus)
...
'                   3 : 체크된것중 제일 위에 것
'-----------------------------------------------------------------------------'
Public Sub SetPatInfo_ACCESS2(ByVal pBarNo As String, ByVal pType As String)
...
'    Call SetCommStatus("R", pBarNo, frmMain.lstComStatus)
    ...
    With frmMain
        Select Case pType
            Case "0"
                For i = 1 To .spdOrder.DataRowCnt
                    If GetText(frmMain.spdOrder, i, colCHARTNO) = pBarNo Then
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    If cn_Server_Flag = True Then
        Select Case gEMR
            Case "MEDIMATE"
                    Call GetSampleInfo_MEDIMATE(asRow, SPD)
...
On Error GoTo DBErr
    ...
    SQL = SQL & "      and W.OcmComStt Not In ('CN', 'CR','VC')             " & vbCrLf
    SQL = SQL & "      and (R.ResRltVal is null or R.ResRltVal = '')        " & vbCrLf
    SQL = SQL & "      and R.ResOcmNum = '" & strSchBarNo & "'              " & vbCrLf
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_MEDIMATE(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'      SQL.Text:=' Select a.PID, b.PNAME, b.SEX, a.RECENO, a.SEQNO, a.EXAMCODE, a.SPECIMENCODE,a.SPECIMENID '+#13#10+
                '  From EXAMRES a               '+#13#10+
                '     , PATIENT b               '+#13#10+
                '  Where a.EXAMSTATE >= ''B''   '+#13#10+
                '    AND a.PID = b.PID          '+#13#10+
                '    AND a.SPECIMENID = '''+PadLeftStr(TMaster.FBarCode, '0', 12)+''' ';
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_MEDIMATE" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
