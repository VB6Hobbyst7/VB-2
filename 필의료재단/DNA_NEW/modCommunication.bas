...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            strClass = Trim(GetText(.vasTemp, intRow, 7))
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                If strTestCd = "L-5363" Then
                
                ElseIf strTestCd = "L-5366" Then
                
                Else
                    '-- 결과저장
                          SQL = "Update SLA_LabResult                       " & vbCr
                    SQL = SQL & "   Set Result      = '" & strClass & "'     " & vbCr
                    SQL = SQL & "      ,NormalFlag  = '0'                   " & vbCr
                    SQL = SQL & "      ,PanicFlag   = '0'                   " & vbCr
                    SQL = SQL & "      ,DeltaFlag   = '0'                   " & vbCr
                    SQL = SQL & "      ,TransFlag   = '1'                   " & vbCr
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,REFJUDGE,REFFLAG " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            ...
            Do Until RS_L.EOF
                ...
                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 6)
                Call SetText(.vasTemp, RS_L.Fields("REFJUDGE").Value & "", intRow, 7)
                Call SetText(.vasTemp, RS_L.Fields("REFFLAG").Value & "", intRow, 8)    'Class
                RS_L.MoveNext
            Loop
        End If
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            strClass = Trim(GetText(.vasTemp, intRow, 8))       'Class
            ...
            'sResult = sResult & "(" & strClass & ")"
            
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '묶음처방코드는 제외
                If strTestCd = "L-5363" Or strTestCd = "L-5366" Then
...
Function SaveTransData_PHILL(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "###" Then
                strHL = SetJudge(sResult, strEqpcd)
            End If
            
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 서버저장
'''                SQL = ""
'''                SQL = SQL & "Update trures Set "
'''                If strTestCd = "5397" Then
'''                    SQL = SQL & "    RESULT_VALUE      = '*' "
'''                    SQL = SQL & "   ,RESULT_MEMO       = '" & sResult & "'"
'''                Else
'''                    SQL = SQL & "    RESULT_VALUE      = '" & sResult & "'"
'''                End If
''''                SQL = SQL & "  , RESULT_DECISION    = '" & strHL & "'" & vbCrLf
'''                SQL = SQL & " WHERE request_date    = '" & strHospDate & "'" & vbCrLf
'''                SQL = SQL & "   And exam_no         = '" & strPatID & "'" & vbCrLf
'''                SQL = SQL & "   And exam_code       = '" & strTestCd & "'" & vbCrLf
'''                SQL = SQL & "   And (RESULT_VALUE   = '' or RESULT_VALUE IS NULL) "
'''
'''                Call SetSQLData("결과저장", SQL, "A")
'''                AdoCn.Execute SQL
                '-- 결과저장
...
'            ElseIf strBarcode <> "" And strTestCd <> "" And sResult <> "" And strTestCd = "5397" Then
'                '-- 학부메모저장
'                Set AdoCmd = New ADODB.Command
'                Set AdoCmd.ActiveConnection = AdoCn
'                With AdoCmd
...
'                    Set prmcmt3 = .CreateParameter("labDeptcode", adVarChar, adParamInput, 30, "U2")
'                    .Parameters.Append prmcmt3
'                    Set prmcmt4 = .CreateParameter("labmemo", adVarChar, adParamInput, 2000, sResult)
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Select Case gEMR
        ...
        Case "PHILL"
            SaveTransData = SaveTransData_PHILL(argSpcRow)
