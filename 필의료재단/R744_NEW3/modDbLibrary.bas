...
Function SaveTransDataW(ByVal argSpcRow As Integer, SPD As Object) As Integer
    Dim iRow            As Integer
    Dim lsID            As String
...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        lsID = Trim(GetText(SPD, argSpcRow, colBARCODE))
        lsPid = Trim(GetText(SPD, argSpcRow, colPID))
        'strPartGbn = Trim(GetText(spd, argSpcRow, colCHARTNO))
        strChtNum = Trim(GetText(SPD, argSpcRow, colCHARTNO))
        ...
        strSpcCd = Trim(GetText(SPD, argSpcRow, colINOUT))
        strLabSeq = Trim(GetText(SPD, argSpcRow, colPOSNO))
        strRegDate = Trim(GetText(SPD, argSpcRow, colHOSPDATE))
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "###" Then
                strHL = f_subSet_RefVal(strExamCd, "", sResult)
            End If
            ...
            'K가   > 5.5  ==> 학부메모 코멘트
            strCmnt = ""
            If UCase(Mid(strEqpCd, 1, 1)) = "K" Then
                If IsNumeric(sResult) Then
                    If CCur(sResult) > 5.5 Then
                        '학부메모
                        strCmnt = ""
                        strCmnt = strCmnt & "전혈로 오래 방치될 경우 전해질검사에 영향을 미칩니다." & vbNewLine
                        strCmnt = strCmnt & "필요한 경우 새로운 검체로 재검 확인하시기 바랍니다." & vbNewLine
                    End If
                End If
            End If
            ...
            If sResult <> "" Then
                ...
                With cmdSQL
                    ...
                    Set prm1 = .CreateParameter("receiptdate", adDate, adParamInput, 30, Format(strRegDate, "####-##-##"))
                    .Parameters.Append prm1
                    Set prm2 = .CreateParameter("receiptnum", adVarChar, adParamInput, 30, lsPid)
                    .Parameters.Append prm2
                    Set prm3 = .CreateParameter("labcode", adVarChar, adParamInput, 30, strExamCd)
                    .Parameters.Append prm3
                    Set prm4 = .CreateParameter("resultvalue", adVarChar, adParamInput, 50, sResult)
                    ...
                    Set prm7 = .CreateParameter("abnormal", adVarChar, adParamInput, 30, strHL)
                    ...
                    Set prm12 = .CreateParameter("machinecode", adVarChar, adParamInput, 30, gEquipCode)
                    .Parameters.Append prm12
                    Set prm13 = .CreateParameter("employeecode", adVarChar, adParamInput, 30, gUserID)
                    .Parameters.Append prm13
                    Set prm14 = .CreateParameter("computerid", adVarChar, adParamInput, 30, CStr(frmInterface.Winsock1.LocalIP))
                    ...
                    SQL = ""
                    SQL = SQL & Format(strRegDate, "####-##-##") & vbCrLf
                    SQL = SQL & lsPid & vbCrLf
                    SQL = SQL & strExamCd & vbCrLf
                    SQL = SQL & sResult & vbCrLf
                    SQL = SQL & "" & vbCrLf
                    SQL = SQL & strHL & vbCrLf
                    SQL = SQL & "" & vbCrLf
                    SQL = SQL & "" & vbCrLf
                    SQL = SQL & "" & vbCrLf
                    SQL = SQL & "" & vbCrLf
                    SQL = SQL & gEquipCode & vbCrLf
                    SQL = SQL & gUserID & vbCrLf
                    SQL = SQL & CStr(frmInterface.Winsock1.LocalIP) & vbCrLf
                    SQL = SQL & "" & vbCrLf
                    SQL = SQL & "0" & vbCrLf
                ...
                '-- 학부메모저장
                If frmInterface.chkMemoSave.Value = "1" And strCmnt <> "" And strRet = "1" Then
                    Set cmdSQL = New ADODB.Command
                    Set cmdSQL.ActiveConnection = cn_Ser
                    With cmdSQL
                        .CommandTimeout = 15
                        ...
                        Set prmcmt1 = .CreateParameter("receiptdate", adDate, adParamInput, 30, Format(strRegDate, "####-##-##"))
                        .Parameters.Append prmcmt1
                        Set prmcmt2 = .CreateParameter("receiptnum", adVarChar, adParamInput, 30, lsPid)
                        .Parameters.Append prmcmt2
                        Set prmcmt3 = .CreateParameter("labDeptcode", adVarChar, adParamInput, 30, "C2")
                        ...
                        'Set prmcmt5 = .CreateParameter("machinecode", adVarChar, adParamInput, 30, gEquipCode)
                        '.Parameters.Append prmcmt5
                        Set prmcmt5 = .CreateParameter("employeecode", adVarChar, adParamInput, 30, gUserID)
                        .Parameters.Append prmcmt5
                        Set prmcmt6 = .CreateParameter("computerid", adVarChar, adParamInput, 30, CStr(frmInterface.Winsock1.LocalIP))
...
'    End If
    ...
    If sBarcode = "" Or Len(sBarcode) <> 8 Then
        Exit Function
    End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_PHILL(ByVal asRow As Long, SPD As Object) As Integer
    Dim sBarcode            As String
    ...
    sBarcode = Trim(GetText(SPD, asRow, colBARCODE))
    strRegDate = Mid(sBarcode, 1, 8)
    strRegNo = Mid(sBarcode, 9)
    ...
    SQL = SQL & "       M.성별 AS 성별,M.나이 AS 나이, E.검사코드 AS ITEM " & vbCr
    SQL = SQL & "  FROM VW_검사접수 M, VW_검사결과 R, VW_검사코드 E  " & vbCr
    SQL = SQL & " WHERE M.접수일자 = CONVERT(DATE, '" & strRegDate & "')" & vbCr
    SQL = SQL & "   AND M.접수번호 = '" & strRegNo & "'" & vbCr
    ...
    With frmInterface
        If Not RS.EOF = True And Not RS.BOF = True Then
            Do Until RS.EOF
                ...
                SetText SPD, Trim(RS.Fields("차트번호")) & "", asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("내원번호")) & "" & "", asRow, colPID
                SetText SPD, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                SetText SPD, Trim(RS.Fields("의뢰처")) & "", asRow, colDISKNO
