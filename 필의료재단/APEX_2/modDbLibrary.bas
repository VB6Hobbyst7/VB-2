...
'                strRefVal = "소견"
                ...
                If strEqpCd = "1131" Or strEqpCd = "1132" Or strEqpCd = "1133" Then
                    SQL = SQL & "Update trures Set RESULT_MEMO = '" & sResult & "'" & vbCrLf
                Else
                    SQL = SQL & "Update trures Set RESULT_VALUE = '" & sResult & "'" & vbCrLf
                End If
                
                'SQL = SQL & " WHERE exam_part collate latin1_general_cs_as = 'Z'" & vbCr
                SQL = SQL & " WHERE request_date    = '" & strRegDate & "'          " & vbCrLf
                SQL = SQL & "   And exam_no         = '" & lsPid & "'               " & vbCrLf
                SQL = SQL & "   And exam_code       = '" & strEqpCd & "'            " & vbCrLf
                SQL = SQL & "   And (RESULT_VALUE   = '' or RESULT_VALUE IS NULL)   " & vbCrLf
