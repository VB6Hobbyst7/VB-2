...
Begin VB.Form frmMain 
      ...
      Begin MSComctlLib.ImageList imlStatus 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":2B6B
               Key             =   "LST"
            EndProperty
            BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":33FD
               Key             =   "ITM"
            EndProperty
            BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":3557
               Key             =   "ERR"
            EndProperty
            BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":36B1
               Key             =   "NOF"
            EndProperty
            BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":380B
               Key             =   "ON"
               Object.Tag             =   "OFF"
            EndProperty
            BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":40E5
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   Begin FPSpread.vaSpread spdOrder 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":6FCF
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdWork 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":8ECD
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
    ...
    Select Case comEqp.CommEvent
        Case comEvReceive
            ...
            Select Case UCase(gHOSP.MACHNM)
                        ...
                        Case "MEDONIC":         Call Phase_Serial_MEDONIC           'MEDONIC M Series
...
Private Sub GetOrder(ByVal pBarNo As String, ByVal pType As String)
    ...
    With frmMain
        ...
        '-- 검사채널로 장비오더 만들기
        If Trim(strItems) = "" And mOrder.SendCnt = 0 Then
            mOrder.NoOrder = True
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                Case "O"
                    ...
                    strBarno = Trim(mGetP(strRcvBuf, 3, "|"))
                    strRackNo = Trim(mGetP(mGetP(strRcvBuf, 4, "|"), 2, "^"))
                    strTubePos = Trim(mGetP(mGetP(strRcvBuf, 4, "|"), 3, "^"))
                ...
                Case "R"
                    '4R|1|^^^Myco-M^^DOSE||||N||X||||20200103032414

'
'1H|\^&|||Liaison XL|||||HOST||P|1|
'2P|1||||^|||U|||||
'3O|1|2020010243265||^^^Myco-M^|R||||||||||||||||||||X
'4R|1|^^^Myco-M^^DOSE||||N||X||||20200103032414
'5C|1|I|JOB_FAILED|I
'6L|1|N
'
'1H|\^&|||Liaison XL|||||HOST||P|1|
'2P|1||||^|||U|||||
'3O|1|2020010243265||^^^Myco-M^|R||||||||||||||||||||F
'4R|1|^^^Myco-M^^DOSE|4.0|INDEX||N||F||||20200103081917
'5L|1|N
'

                    '4R|1|^^^ACTH^^DOSE|3.919|pg/mL||L||F||||20210505164033
                        ...
                        If strResult <> "" Then
                            Select Case strIntBase
                                ...
                                Case "MEAS-G" 'Measles IgG
                                        ...
                                        Else
                                            If InStr(strIntResult, "<") > 0 Then
                                                strResult = "N"
                                                strIntResult = "<5.0"
                                            ElseIf InStr(strIntResult, ">") > 0 Then
                                                strResult = "P"
                                                strIntResult = ">300.0"
                                            Else
                                                strResult = "E"
                                ...
                                Case "Myco-M"
                                        ...
                                        If IsNumeric(strIntResult) Then
                                            If strIntResult >= 0.1 And strIntResult <= 9.9 Then
                                                strResult = "N"
...
On Error GoTo RST
    ...
    '-- XML 파일을 열어 ASTM 포멧으로 변경
    Set xmlDoc = New MSXML2.DOMDocument30
    intBufCnt = 0
    
    xmlDoc.async = False
    xmlDoc.Load App.PATH & "\XML\" & gHOSP.MACHNM & "_" & Format(CDate(Now), "yyyy-mm-dd") & ".xml"   '"D:\프로젝트\VB\__의사랑\효창동김내과_의사랑\IF\Log\MEDONIC_2019-06-27.xml"
    ...
    Else
        ...
        For Each xNode In Child_Node
            If xNode.nodeType = NODE_ELEMENT Then
                strType = xNode.nodeName
                If UCase(strType) = "SAMPLE" Then
                    Exit For
                End If
            End If
        Next
        ...
        If UCase(strType) = "SAMPLE" Then
            For i = 0 To xNode.childNodes.Length - 1
                For J = 0 To xNode.childNodes.Item(i).childNodes.Length - 1
                    '챠일드 정보가 있는것만..
                    'Debug.Print xNode.childNodes.Item(i).childNodes.Item(J).childNodes.Length
                    If xNode.childNodes.Item(i).childNodes.Item(J).childNodes.Length > 0 Then
                        Select Case UCase(xNode.childNodes.Item(i).baseName)
                            Case "INSTRINFO"    '-- 장비정보
                                    If intBufCnt = 0 Then
                                        ReDim Preserve strRecvData(intBufCnt)
                                        strRecvData(intBufCnt) = "H|1"
                                        intBufCnt = intBufCnt + 1
                                    End If
                            Case "SMPINFO"      '-- 샘플정보
                                    ReDim Preserve strRecvData(intBufCnt)
                                    strRecvData(intBufCnt) = "O|" & xNode.childNodes.Item(i).childNodes.Item(J).FirstChild.nodeTypedValue & "|" & xNode.childNodes.Item(i).childNodes.Item(J).LastChild.nodeTypedValue
                                    intBufCnt = intBufCnt + 1
                                    
                            Case "SMPRESULTS"   '-- 검사결과
                                    ReDim Preserve strRecvData(intBufCnt)
                                    strRecvData(intBufCnt) = "R|" & xNode.childNodes.Item(i).childNodes.Item(J).FirstChild.nodeTypedValue & "|" & xNode.childNodes.Item(i).childNodes.Item(J).LastChild.nodeTypedValue
                                    intBufCnt = intBufCnt + 1
                            
                            Case "TPARAMS"      '-- 뭔지 모르지만 검사결과 다음에 나옴??
                                    ReDim Preserve strRecvData(intBufCnt)
                                    strRecvData(intBufCnt) = "L|1|N"
                                    Exit For
                                    
                            Case "HGRAMS"       '-- 그래프 INFO?
                            Case "HGRAM"        '-- 그래프 데이터
                        End Select
                    End If
                Next
    ...
    With frmMain
        For intCnt = 0 To UBound(strRecvData)
            ...
            Select Case strType
                ...
                Case "O"
                    strOrderType = UCase(mGetP(strRcvBuf, 2, "|"))
                    Select Case strOrderType
                        Case "ID"
                                If strOrderType <> mGetP(strRcvBuf, 3, "|") Then
        ...
        If intSndPhase <= 3 Then
            For intRow = 1 To .DataRowCnt
                If GetText(spdOrder, intRow, colCHECKBOX) = "1" And GetText(spdOrder, intRow, colSTATE) = "오더준비" Then
                    ...
                    strPtId = Trim(GetText(spdOrder, intRow, colPID))
                    strItems = Trim(GetText(spdOrder, intRow, colINOUT))
                    If intSndPhase = 3 Then
                        .Row = intRow
