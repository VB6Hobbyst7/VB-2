...
'''  ANO:= Copy(BCD, 8, 5);
            ...
            intBcFive = Mid(lsID, 1, 5) '06351
            intBcAdd = intBcFive - intBcNow
            strADT = Format(Now + intBcAdd, "yyyymmdd")
  
            strSlip1 = Mid(lsID, 6, 2)
            strSlip2 = Mid(lsID, 8, 5)
...
''   and status  in (''U'', ''R'', ''P'')' ;
                ...
                If sResult <> "" Then
                    '-- 검사결과업데이트
                          SQL = "UPDATE twexam_general_sub Set " & vbCr
                    SQL = SQL & " result1 = '" & sResult & "'" & vbCr
'                    SQL = SQL & ",outclassno = '" & strEqpCd & "'" & vbCr
                    SQL = SQL & " WHERE jeobsudt = to_date('" & strADT & "', 'yyyymmdd') " & vbCr
                    SQL = SQL & "   AND slipno1 = '" & strSlip1 & "'" & vbCr
                    SQL = SQL & "   AND slipno2 = '" & strSlip2 & "'" & vbCr
                    SQL = SQL & "   AND itemcd = '" & strExamCd & "'" & vbCr
                    SQL = SQL & "   AND ptno = '" & lsPid & "'" & vbCr
                    ...
                    '--상태업데이트
                          SQL = "UPDATE twexam_general Set " & vbCr
                    SQL = SQL & " status = 'U'" & vbCr
                    SQL = SQL & " WHERE jeobsudt = to_date('" & strADT & "', 'yyyymmdd') " & vbCr
                    SQL = SQL & "   AND slipno1 = '" & strSlip1 & "'" & vbCr
                    SQL = SQL & "   AND slipno2 = '" & strSlip2 & "'" & vbCr
                    SQL = SQL & "   AND ptno = '" & lsPid & "'" & vbCr
                    SQL = SQL & "   AND status IN ('U','R','P')"
...
'    Next
    ...
    If (rv = 0) Then
        MsgBox "DB연결 실패", vbOKOnly, Me.Caption
        Exit Sub
    Else
         'rv = dce_error("msg")
...
''    Call WinExec("C:\IF_EPOC\COM\EPOC.exe gSetup.gPort C:\IF_EPOC\LOG", 0)
'    Call WinExec("C:\IF_EPOC\COM\EPOC.exe " & gSetup.gPort & " C:\IF_EPOC\LOG", 0)
...
'    Call GetSampleInfoW_MEDIOUS(intRow)
    ...
    Call GetSampleInfoW_KWUNIV(intRow)
    ...
    For intCnt = 0 To UBound(varRcvBuf)
        ...
        Case 7
            ...
            If InStr(varRcvBuf(intCnt), "Patient ID") > 0 Then
                ...
                If strBarno = "" And Len(strBarno) <> 13 Then
                    strRackNo = Mid(strBarno, 6, 2)
                    strTubePos = Mid(strBarno, 8, 5)
                End If
