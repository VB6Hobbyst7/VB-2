...
Begin VB.Form frmComSetup 
   ...
   Begin VB.Frame Frame5 
      ...
      Begin VB.ComboBox cboPort 
         Height          =   300
         ItemData        =   "frmComSetup.frx":02F9
         Left            =   1695
         List            =   "frmComSetup.frx":02FB
         TabIndex        =   14
         Text            =   "cboPort"
         Top             =   225
         Width           =   1740
...
'        .EndColor = vbWhite
    ...
    With Eqp_Property
        ...
        .Add "COM_IBS", IIf(txtInBuf.Text = "", 1024, txtInBuf.Text)
        .Add "COM_INLEN", IIf(txtInPutLen.Text = "", 0, txtInPutLen.Text)
        .Add "COM_OBS", IIf(txtOutBufferSize.Text = "", 512, txtOutBufferSize.Text)
        .Add "COM_PTR", txtParityReplace.Text
        .Add "COM_RTH", IIf(txtRThreshold.Text = "", 1, txtRThreshold.Text)
        .Add "COM_STH", IIf(txtSThreshold.Text = "", 1, txtSThreshold.Text)
    End With
...
Private Sub cmdEQP_Click()
    ...
    sqlDoc = "select EQP_CD, EQP_NM from INTERFACE001"
    adoRS.Open sqlDoc, AdoCn_Jet
    If adoRS.RecordCount > 0 Then
        adoRS.MoveFirst
        Do Until adoRS.EOF
            Set itemX = PopUp_List.ListItems.Add(, , Trim(adoRS("EQP_CD") & ""))
            itemX.SubItems(1) = Trim(adoRS("EQP_NM") & "")
            adoRS.MoveNext
        Loop
...
'            txtTmp_H = strTmp_LH(1)
            ...
            cboHandshaking.ListIndex = Val(mAdoRs("COM_HANDSHAK") & "")
            cboInputMode.ListIndex = Val(mAdoRs("COM_INPUTMOD") & "")
            DTREnable.Value = Val(mAdoRs("COM_DTR") & "")
            EOFEnable.Value = Val(mAdoRs("COM_EOF") & "")
            NullDiscard.Value = Val(mAdoRs("COM_NULDIS") & "")
            RTSEnable.Value = Val(mAdoRs("COM_RTS") & "")
