...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Tab(0).Control(17)=   "cboTest"
      Tab(0).Control(17).Enabled=   0   'False
      Tab(0).Control(18)=   "chkMan"
      Tab(0).Control(18).Enabled=   0   'False
      Tab(0).Control(19)=   "cmdSel(5)"
      Tab(0).Control(19).Enabled=   0   'False
      Tab(0).Control(20)=   "cmdSel(4)"
      ...
      Begin VB.ComboBox cboTest 
         Height          =   300
         ItemData        =   "frmComm_yej.frx":6B52
         Left            =   5460
         List            =   "frmComm_yej.frx":6B5F
         Style           =   2  '드롭다운 목록
         TabIndex        =   56
         Top             =   480
         Visible         =   0   'False
      ...
      Begin Threed.SSCommand cmdSel 
         ...
         _StockProps     =   78
         BevelWidth      =   1
         Picture         =   "frmComm_yej.frx":6B7F
      End
      Begin VB.ComboBox cboRstgbn 
         Height          =   300
         Index           =   1
         ItemData        =   "frmComm_yej.frx":7001
         Left            =   -72570
         List            =   "frmComm_yej.frx":700E
         Style           =   2  '드롭다운 목록
         TabIndex        =   23
         Top             =   495
         Visible         =   0   'False
      ...
      Begin VB.ComboBox cboRstgbn 
         Height          =   300
         Index           =   0
         ItemData        =   "frmComm_yej.frx":7038
         Left            =   3900
         List            =   "frmComm_yej.frx":7045
         Style           =   2  '드롭다운 목록
         TabIndex        =   21
         Top             =   480
         Visible         =   0   'False
      ...
      Begin FPSpread.vaSpread spdResult1 
         ...
         SpreadDesigner  =   "frmComm_yej.frx":706F
         UserResize      =   1
      End
      ...
      Begin FPSpread.vaSpread spdWorkList 
         ...
         MaxRows         =   14
         ScrollBarMaxAlign=   0   'False
         SpreadDesigner  =   "frmComm_yej.frx":7B1E
         UserResize      =   0
      End
      ...
      Begin FPSpread.vaSpread spdResult2 
         ...
         ScrollBarMaxAlign=   0   'False
         SelectBlockOptions=   0
         SpreadDesigner  =   "frmComm_yej.frx":88BD
         UserResize      =   1
      End
...
Private Sub cmdInit_Click()
    ...
    Call COM_OUTPUT(STX + "I " + ETX)
    Print #1, Chr(13) + Chr(10) + "[HOST]" + STX + "I " + ETX;
...
Private Sub cmdOrder_Click()
    ...
    Call COM_OUTPUT(STX + "M     " + ETX)
    Print #1, Chr(13) + Chr(10) + "[HOST]" + STX + "M     " + ETX;
        ...
        For intRow1 = 1 To .maxrows
            ...
            If Trim$(varTmp) = "1" Then
                ...
                If intRow2 < 1 Then
                    ...
                    End If
                    If chkQC.Value = 1 Then
                        Rev = sl_spcid_tstcd_select_qc1&(INS_CODE, cboLevel.Text, strPid, Test_Cd, strLevel)
                    Else
                    ...
                    If bgetWork = True Then
                        ...
                        Else
                            ...
                            spdResult1.SetText 3, intRow2, Format(Val(intRack), "00")
                            spdResult1.SetText 4, intRow2, Trim(intCup)
                            spdResult1.SetText 5, intRow2, strPid(0)
                            spdResult1.SetText 6, intRow2, "QC검사"
                        End If
...
Private Sub ComReceive(ByRef RecData As String)
    ...
    If f_strJOB_FLAG = "1" Then Timer1.Enabled = False:      f_strJOB_FLAG = ""
 
'    strRec = StrConv(RecData, vbUnicode)
    strRec = RecData
    
    Print #1, Chr(13) + Chr(10) + "[TOSH]" + strRec;
    ...
    For intIdx = 1 To Len(strRec)
        ...
        Select Case Asc(strBuff)
            ...
            Case 3 '-- ETX
                        ...
                        If Mid(f_strBuffer, 1, 3) = STX + NAK + ETX Then
                            Timer2.Enabled = False
                        
                            Call cmdInit_Click
                            If f_intCnt > 0 Then f_intCnt = f_intCnt - 1
                            
'                            lblStatus = "결과대기중..."
                        
                        ElseIf Mid(f_strBuffer, 1, 8) = STX + "M 0001" + ETX Then
                            '-- 오더송신
                            ...
                            If f_intCnt <= UBound(f_strBarno) And f_strBarno(1) <> "" Then
                                Call COM_OUTPUT(STX & ACK & ETX)
                                Print #1, Chr(13) + Chr(10) + "[HOST]" + STX + ACK + ETX;
                                
                                strSend = STX + "O "
                                strSend = strSend + f_strBarno(f_intCnt) + Space$(20 - Len(Trim(f_strBarno(f_intCnt))))
                                strSend = strSend + "  " + f_strRack(f_intCnt)  '-- Rack
                                strSend = strSend + f_strCup(f_intCnt)  '-- cup
                                '-- Manual Dil
                                strSend = strSend + "  1"
                                
                                If Len(f_strTest(f_intCnt)) > 0 Then
                                    strSend = strSend + f_strTest(f_intCnt) + ETB
                                Else
                                    MsgBox f_strBarno(f_intCnt) & "의 해당 검사항목이 없습니다.!", vbCritical
                                    Call COM_OUTPUT(STX + "I " + ETX)
                                    Print #1, Chr(13) + Chr(10) + "[HOST]" + STX + "I " + ETX;
                                    Timer2.Enabled = True
                                    Exit Sub
                                End If
                                '-- Order Date/Time
                                strSend = strSend + Format(Now, "yyyymmddhhmm")
                                '-- Name
                                strSend = strSend + Space$(30)
                                '-- Sex
                                strSend = strSend + Space$(1)
                                '-- Birth Date
                                strSend = strSend + Space$(8)
                                '-- Location
                                strSend = strSend + Space$(20)
                                '-- Doctor
                                strSend = strSend + Space$(20)
                                '-- Comment
                                strSend = strSend + Space$(20)
                                strSend = strSend + ETB + ETX
                                
                                Call COM_OUTPUT(strSend)
                                Print #1, Chr(13) + Chr(10) + "[HOST]" + strSend;
                            ...
                            End If
                        '-- 결과수신
                        ElseIf Mid(f_strBuffer, 1, 3) = STX + "R " Then
                            lblStatus = "결과수신중..."
                            Call psDataDefine(f_strBuffer, fChannel(), spdResult1)
                            
                            Call COM_OUTPUT(STX + ACK + ETX)
                            Print #1, Chr(13) + Chr(10) + "[HOST]" + STX + ACK + ETX;
                        ElseIf Not Mid(f_strBuffer, 1, 3) = STX + ACK + ETX Then
                            Call COM_OUTPUT(STX + "M     " + ETX)
                            Print #1, Chr(13) + Chr(10) + "[HOST]" + STX + "M     " + ETX;
                        End If
                        f_strBuffer = ""
                        'If f_intCnt > 0 Or Len(f_strBarno(1)) > 0 Then
                        Timer2.Enabled = True
...
Dim strTime As String, strDate  As String, sqlRet   As Integer
...
'                                    spdResult1.Row = iRow 'f_intSampleNo
...
'    Me.Show
    ...
    Open App.Path + "\" + "Toshiba.log" For Output As #1
