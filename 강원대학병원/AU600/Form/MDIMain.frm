...
'    If INS_BARCODE = "Y" Then
        Call Load_From(frmComm)
'    Else
'        Call Load_From(frmComm_2)
'    End If
...
On Error GoTo ErrorRouten
    ...
    sqlDoc = "select SAVE_DT, BARCODE from INTERFACE001 where EQP_CD = '" & INS_CODE & "'"
    adoRS.CursorLocation = adUseClient
    adoRS.Open sqlDoc, AdoCn_Jet
    If adoRS.RecordCount > 0 Then adoRS.MoveFirst
    If Not adoRS.EOF Then
        strSaveDt = Trim$(adoRS(0) & "")
        INS_BARCODE = Trim$(adoRS(1) & "")
    End If
    adoRS.Close:    Set adoRS = Nothing

    strTmp = Format$(DateAdd("d", -Val(strSaveDt), Format$(Now, "YYYY-MM-DD")), "YYYYMMDD")
    
    sqlDoc = "select count(*) from INTERFACE003 where TransDt <= '" & strTmp & "'"
    adoRS.CursorLocation = adUseClient
    adoRS.Open sqlDoc, AdoCn_Jet
    If adoRS.RecordCount > 0 Then adoRS.MoveFirst
    ...
    If intCnt > 0 Then
        If MsgBox(strSaveDt + "일 지난 데이타를 삭제하시겠습니까?", vbQuestion + vbYesNo, Me.Caption) = vbYes Then
            ...
            sqlDoc = "delete from INTERFACE003 where TransDt <= '" & strTmp & "'"
