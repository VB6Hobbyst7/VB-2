...
Function SaveTransData_KNUH(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
    ...
    With frmMain
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If sResult = "Presumptive positive" Then
                    sResult = "COVID-19 : " & sResult
                End If
                '-- 결과저장용 키 가져오기
'                If strOrdcd = "" Then
'                    strOrdcd = GetSampleSubITEM(strBarcode, strTestCd)
'                End If
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    Dim strErrFlag()    As String
                    Dim strEqpCode()    As String
                    
                    ReDim Preserve strSampleno(1 To 1) As String
                    ReDim Preserve strOrdCode(1 To 1) As String
                    ReDim Preserve strRstval(1 To 1) As String
                    ReDim Preserve strErrFlag(1 To 1) As String
                    ReDim Preserve strEqpCode(1 To 1) As String
                                        
                    strSampleno(1) = strBarcode
                    strOrdCode(1) = strTestCd
                    strRstval(1) = sResult
                    strErrFlag(1) = ""
                    strEqpCode(1) = gHOSP.MACHCD
                    
                    
                    '-- 서버저장
                    If IsNumeric(Right(strOrdCode(1), 1)) Then
                        Call sl_online_result_ul_4&(strErrMsg, strSampleno, strOrdCode, strRstval, strErrFlag, strEqpCode, Chr(0))
                        
                        Call SetSQLData("결과저장", strErrMsg & "/" & strSampleno(1) & "/" & strOrdCode(1) & "/" & strRstval(1) & "/" & strErrFlag(1) & "/" & strEqpCode(1) & "/" & Chr(0), "A")
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_KNUH" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
    ...
    With frmMain
        ...
        If strPatID <> "" And strCmnt <> "" And (strRet = "0" Or strRet = "1") Then
            ...
            With AdoCmd
                ...
                Set Prm1 = .CreateParameter("receiptdate", adDate, adParamInput, 12, strBarcode)
                .Parameters.Append Prm1
                
                Set Prm2 = .CreateParameter("receiptnum", adVarChar, adParamInput, 3, gHOSP.MACHCD)
                .Parameters.Append Prm2
                
                Set Prm3 = .CreateParameter("labDeptcode", adVarChar, adParamOutput, 1, "")
                .Parameters.Append Prm3
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "KNUH"                                 '강원대학교 병원
                Call GetWorkList_KNUH(pFrom, pTo, SPD)
...
Public Sub GetWorkList_KNUH(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    intIdx = sl_tstcd_spcid_select&(pFrom, "(" + gAllTestCd + ")", strBarno, strPid, strPnm, strOrdcd)
        
    If UBound(strOrdcd) > 0 Then
        ...
        For intIdx = 0 To UBound(strOrdcd) - 1
            With SPD
                For i = 1 To SPD.DataRowCnt
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colBARCODE)
                    If pFrom = strHospDate And strBarno(intIdx) = strBarcode Then
                        blnSame = True
                    End If
                Next
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, strBarno(intIdx), intRow, colBARCODE
                    SetText SPD, strPid(intIdx), intRow, colPID
                    SetText SPD, strPnm(intIdx), intRow, colPNAME
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_KNUH" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As Object) As String
    ...
    Select Case gEMR
        Case "KNUH"
            intIdx = sl_spcid_tstcd_select(strBarcode, strTestCd, strTPid, strTPnm)
            
            gPatOrdCd = ""
                        
            For i = 0 To intIdx
                strEqpCd = Trim(strTestCd(i))
                
                If strItems = "" Then
                    strItems = GetTestNm(strEqpCd, False)
                Else
                    strItems = strItems & "/" & GetTestNm(strEqpCd, False)
                End If
                
                '처방 검사정보를 가져온다.
                gPatOrdCd = gPatOrdCd & "'" & strEqpCd & "',"
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "KNUH"
            SaveTransData = SaveTransData_KNUH(argSpcRow, SPD)
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    If cn_Server_Flag = True Then
        Select Case gEMR
            Case "KNUH"
                    Call GetSampleInfo_KNUH(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_KNUH(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    If strRegDate = "" Then
        strRegDate = Format(Now, "yyyymmdd")
    End If
   ...
   ' intIdx = sl_spcid_tstcd_select(strBarcode, strTestCd, strTPid, strTPnm)
    ...
    intIdx = sl_d_m05_sel_test_info_by_spcno2(strBarcode, strTestCd, strTPid, strTPnm, strACPTNo)
    
'    Call SetSQLData("intIdx ", intIdx)

'    If UBound(strTestCd) > 0 Then
'        For i = 0 To UBound(strTestCd)
'            MsgBox strTestCd(i) & "," & strTPnm(i)
'        Next
'    End If
        ...
        With SPD
            ...
            For i = 0 To intIdx - 1
                With SPD
                    ...
                    SetText SPD, "1", asRow, colCHECKBOX
                    SetText SPD, strRegDate, asRow, colHOSPDATE
                    SetText SPD, strBarcode, asRow, colBARCODE
                    SetText SPD, Trim(strTPid(i)), asRow, colPID
                    SetText SPD, Trim(strTPnm(i)), asRow, colPNAME
                    SetText SPD, Trim(strACPTNo(i)), asRow, colCHARTNO
                    ...
                    '-- 화면에 표시
                    For intCol = colSTATE + 1 To .MaxCols
                        If Trim(strTestCd(i)) = gArrEQP(intCol - colSTATE, 2) Then
                            .Row = asRow
                            .Col = intCol
                            .BackColor = vbYellow
                            Call SetText(SPD, "◇", asRow, intCol)
                    ...
                    gPatOrdCd = gPatOrdCd & "'" & Trim(strTestCd(i)) & "',"
                End With
                DoEvents
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_KNUH" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
