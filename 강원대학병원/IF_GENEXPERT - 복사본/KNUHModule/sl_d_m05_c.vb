...
Function sl_d_m05_sel_test_info_by_spcno2&(spcno$, tst_cd$(), pt_no$(), patname$(), acpt_no$())
        Dim dce_table As Long, Socket As Integer

        Call dce_checkver(2, 0)
        Socket = dce_findserver("sl_d_m05")
        If (Socket > -1) Then
                rv% = dce_push_string(Socket, "in_spc_no", spcno)
                rv% = dce_push_string(Socket, "in_tst_stat_list", "C','R")
                rv% = dce_push_string(Socket, "in_tst_stat_list_x", "0','1','2','3")
                dce_table = dce_submit("sl_d_m05", "sl_d_m05_sel_test_info_by_spcno2", Socket)
        End If
        sl_d_m05_sel_test_info_by_spcno2 = dce_pop_long(dce_table, "dce_result")
        rv% = dce_pop_array(dce_table, "tst_cd", tst_cd())
        rv% = dce_pop_array(dce_table, "pt_no", pt_no())
        rv% = dce_pop_array(dce_table, "pt_nm", patname())
