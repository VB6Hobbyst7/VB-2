...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Begin FPSpread.vaSpread spdWorkList 
         ...
         MaxRows         =   14
         ScrollBarMaxAlign=   0   'False
         SpreadDesigner  =   "frmComm_2.frx":751D
         UserResize      =   0
      End
      Begin Threed.SSCommand cmdSel 
         Height          =   360
      ...
      Begin FPSpread.vaSpread spdResult2 
         ...
         ScrollBarMaxAlign=   0   'False
         SelectBlockOptions=   0
         SpreadDesigner  =   "frmComm_2.frx":82E2
      End
...
Private f_intSampleNo   As String
Private f_blnWorkList   As Boolean
Private f_lngWork_Row   As Long
Private f_spcno_row     As Integer         '검체코드와 결과를 찍기위한 Row
...
Private Sub f_subSet_ItemHeader()
    ...
    With lvwCuData
        ...
        With .ColumnHeaders
            ...
            Call .Add(, "ETC", "비고", (lvwCuData.Width - 310) * 0.5)
        End With
...
Private Sub f_subSet_Result(ByVal strdata As String)
    ...
    strTmp = strdata
    intPos = InStr(strTmp, Chr$(13)): intCnt = 0: Erase strDta
    Do While intPos > 0
        intCnt = intCnt + 1
        strDta(intCnt) = Mid$(strTmp, 1, intPos - 1)
        strTmp = Mid$(strTmp, intPos + 2)
        intPos = InStr(strTmp, Chr$(13))
    Loop
    
    'strRstval()라는 배열에 필요한 결과값만 담는다.
    strRstval(1) = Mid$(strDta(1), 7)            '-- 1. Date
    strRstval(2) = Mid$(strDta(2), 7)            '-- 2. ID_NO
    strIDNO = Split(strRstval(2), "-")
    If Trim(strIDNO(1)) = "" Then                '검체조회시 Seqno OR Barcode를 사용해서 조회
       strSampleno = Trim(strIDNO(0))
    Else
       strSampleno = Trim(strIDNO(1))
    End If
    strRstval(3) = ""                             '-- 3. Ward
    strRstval(4) = ""                             '-- 4. NAME
        
    Dim find_row As Integer               '바코드 Or Seqno 번호가 있는 위치(=Row)
    Dim icnt As Integer                   'For문의 Count를 위한 임시변수
        
    '검사항목별로 결과값을 잘라서 배열(strRstval)에 담는다.
    For icnt = 5 To 14
        Select Case Mid(strDta(icnt), 1, 3)
        Case "S.G"    '7자리
             strRstval(icnt) = Trim(Mid$(strDta(icnt), 9, 7))
        Case "p.H"    '3자리
             strRstval(icnt) = Mid$(strDta(icnt), 13, 3)
        Case Else     '4자리
             strRstval(icnt) = Trim(Mid$(strDta(icnt), 8, 4))
        End Select
    Next
    
    With spdResult1
'        If chkJobgbn.Value <> "1" Then '-- QC가 아니면
            For ii = 1 To .maxrows
                .Col = 2: .Row = ii
                If ii = 1 And Trim(.Text) = "" Then
                    MsgBox "Work List를 작성후에 다시 작업을 하십시요.."
                    f_spcno_row = 0
                    Exit Sub
                End If
                ...
                If Val(Trim(.Text)) = Val(Trim(Mid$(strRstval(2), 1, 4))) Then
                    f_spcno_row = ii
                    Exit For
                End If
                
                If Val((.StartingRowNumber + ii) - 1) = Val(Trim(Mid$(strRstval(2), 1, 4))) Then
                    .Col = 2
                    If Trim(.Text) <> "" Then
                        f_spcno_row = ii
                        Exit For
            ...
            .Col = 2: .Row = f_spcno_row
            strSampleno = .Text
            intRet = sl_spcid_tstcd_select&(strSampleno, strOrdLst, strPid, strPnm)
'        Else '--QC 이면
'            f_spcno_row = SeqNullSearch(spdResult1, "XXX", 2)
'            strSampleno = "QC" + CStr(f_spcno_row)
'            .SetText 2, f_spcno_row, strSampleno
'        End If
        .Row = f_spcno_row: .Col = 2
        .BackColor = vbCyan
    End With
    

    For intIdx = 5 To UBound(strRstval)
        If strRstval(intIdx) <> "" Then
            Set itemX = lvwCuData.FindItem(Mid(strDta(intIdx), 1, 3), lvwSubItem, , lvwWhole)
            If Not itemX Is Nothing Then
                '결과값을 비교해서 정상-"", 비정상-"N"으로 표시해줌
                If itemX.ListSubItems(9) <> "" Then
                   If Trim(strRstval(intIdx)) <> itemX.ListSubItems(9) Then
                      strRefVal(intIdx) = "N"
                   End If
                End If
                If strRstval(intIdx) = "+-" Then strRstval(intIdx) = "trace"
                
                 '바코드를 사용할경우
'                intCol = itemX.Index
'                spdResult1.Col = intCol + 4: spdResult1.Row = find_row: spdResult1.Text = strRstval(intIdx)
'                spdResult1.ForeColor = IIf(strRefVal(intIdx) <> "", vbRed, vbBlack)
                
                'Seq번호를 사용할경우
                intCol = itemX.Index
                
                'Spread Sheet의 Row Header에 Seq번호를 표시를 해줌[요구사항]
                'If chkJobgbn.Value <> "1" Then
                '    spdResult1.SetText 0, f_spcno_row, Mid(strRstval(2), 1, 4)
                'Else
                    spdResult1.SetText 0, f_spcno_row, strSampleno
                'End If
                '결과표시
                spdResult1.Col = intCol + 6: spdResult1.Row = f_spcno_row: spdResult1.Text = strRstval(intIdx)
                spdResult1.ForeColor = IIf(strRefVal(intIdx) <> "", vbRed, vbBlack)
                ...
                End If
                '-- 서버결과등록
                If varTmp > 0 And intRet > 0 And chkAuto.Value = vbChecked And chkQC.Value = 0 Then
                    If f_funAdd_Server(strSampleno, itemX.ListSubItems(2), strRstval(intIdx), strOrdLst) Then
                    ...
                    End If
                Else
                    If f_funAdd_QcServer(strSampleno, itemX.ListSubItems(2), strRstval(intIdx), strOrdLst) Then
                        spdResult1.Row = f_intSampleNo
                        spdResult1.Col = -1:    spdResult1.BackColor = &HFFF8F0
    ...
    If chkSuces = True Then
        spdResult1.SetText 1, f_spcno_row, 0
    End If
...
Private Sub cmdWorkList_Click()
    ...
    Dim varTmp  As Variant
    Dim intRow1 As Integer, intRow2 As Integer, intCol As Integer
    Dim intIdx  As Integer
    
    Dim strBarno    As String, strPid   As String, strPnm   As String
    ...
    With spdWorkList
        For intRow1 = 1 To .maxrows
            ...
            .SetText 1, intRow1, "0"
            If Trim$(varTmp) = "1" Then
                .GetText 3, intRow1, varTmp:    strPnm = Trim$(varTmp)
                .GetText 4, intRow1, varTmp:    strPid = Trim$(varTmp)
                ...
                If intRow2 < 1 Then
                    ...
                    If chkQC.Value = 0 Then
                        ...
                        spdResult1.SetText 3, intRow2, strPnm
                        spdResult1.SetText 4, intRow2, strPid
                    Else
                        spdResult1.SetText 1, intRow2, "1"
                        spdResult1.SetText 2, intRow2, strBarno
                        spdResult1.SetText 3, intRow2, "QC검사"
                        spdResult1.SetText 4, intRow2, "Level : " & strPid
                    End If
                End If
                For intCol = 5 To UBound(strDta)
...
'    strRec = "Date :Feb-14-2003 09:52:31" & Chr(13) & Chr(10) & _
             "ID_NO:0125-03041500007     " & Chr(13) & Chr(10) & _
             "Ward: " & Chr(13) & Chr(10) & _
             "Name: " & Chr(13) & Chr(10) & _
             "BLD    +++  250          " & Chr(13) & Chr(10) & _
             "BIL    -    neg          " & Chr(13) & Chr(10) & _
             "URO    -   norm          " & Chr(13) & Chr(10) & _
             "KET    +++  100          " & Chr(13) & Chr(10) & _
             "PRO    -    neg          " & Chr(13) & Chr(10) & _
             "NIT    -    neg          " & Chr(13) & Chr(10) & _
             "GLU    ++++2000          " & Chr(13) & Chr(10) & _
             "p.H         5.0          " & Chr(13) & Chr(10) & _
             "S.G       1.015          " & Chr(13) & Chr(10) & _
             "LEU    +     25   WBC/ul " & Chr(13) & Chr(10) & _
             ""
             
'    strRec = "Date :Feb-14-2003 09:52:31" & Chr(13) & _
             "ID_NO:0001-03041500006     " & Chr(13) & _
             "Ward: " & Chr(13) & _
             "Name: " & Chr(13) & _
             "BLD    +++  250          " & Chr(13) & _
             "BIL    -    neg          " & Chr(13) & _
...
'    Me.Show
    ...
    spdResult1.SetText 0, 0, "ID_NO"
    mskRstDate.Text = Format$(Now, "YYYYMMDD")
    mskOrdDate.Text = Format$(Now, "YYYYMMDD")
    Open App.Path + "\" + "uriscam.log" For Output As #1

    f_strJOB_FLAG = "1":  f_intSampleNo = 0: f_spcno_row = 0
