...
Private Sub f_subSet_Result(ByVal strdata As String)
    ...
    Do While intPos > 0
        ...
        Debug.Print strDta(intCnt)
        strTmp = Mid$(strTmp, intPos + 2)
        intPos = InStr(strTmp, Chr$(13))
        If intCnt = 12 Then Exit Do
    Loop
    
    'strRstval()라는 배열에 필요한 결과값만 담는다.
    strRstval(1) = Mid$(strDta(1), 1)             '--  1. ==========
    strRstval(2) = Trim(Mid$(strDta(2), 13))      '--  2.  0:18  JAN 22/88
    strRstval(3) = Mid$(strDta(3), 1)             '--  3. 공백
    strRstval(4) = Mid$(strDta(4), 1)             '--  4. SAMPLE No. = 245
    strRstval(5) = Mid$(strDta(5), 7)             '--  5. 공백
    
    strRstval(8) = Mid$(strDta(8), 6, 3)          '--  6. Na = 143 mmol/L
    strRstval(9) = Mid$(strDta(9), 5, 4)          '--  7. K = 4.25 mmol/L
    strRstval(10) = Mid$(strDta(10), 6, 3)          '--  8. Cl = 107 mmol/L
...
'    If chkJobgbn.Value = vbChecked Then
'        strSampleno = "QC" + CStr(f_intSampleNo)
'        spdResult1.SetText 2, f_intSampleNo, strSampleno
'    Else
'        If Trim(strSampleno) = "" Then
...
'                If chkJobgbn.Value = vbUnchecked Then
                    '-- 서버결과등록
'                    If varTmp > 0 And intRet > 0 And chkAuto.Value = vbChecked Then
'                        If f_funAdd_Server(strSampleno, itemX.SubItems(2), strRstval(intIdx), strOrdLst) Then
...
strRec = "   ==========" + vbCrLf
strRec = strRec + "SAMPLE No. = 3" + vbCrLf
strRec = strRec + vbCrLf
strRec = strRec + "BLOOD SAMPLE" + vbCrLf
strRec = strRec + vbCrLf
strRec = strRec + "Na = 140 mmol/L" + vbCrLf
strRec = strRec + "K = 3.56 mmol/L" + vbCrLf
strRec = strRec + "Cl = 110 mmol/L" + vbCrLf
strRec = strRec + vbCrLf
strRec = strRec + "   ==========" + vbCrLf
strRec = strRec + vbCrLf
strRec = strRec + vbCrLf
...
'    Me.Show
    ...
    Open App.Path + "\" + "m644.log" For Output As #1
