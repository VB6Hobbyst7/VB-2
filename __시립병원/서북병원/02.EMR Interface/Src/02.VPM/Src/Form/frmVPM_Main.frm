   ...
   '/----------------------------------------------------------------------------------------------------/
    ...
    If UBound(FtpScanFileName) > 0 Then
        For intX = 1 To UBound(FtpScanFileName)
            ...
            If UBound(strTemp1) = 3 Then
                If Left(strTemp1(3), InStr(strTemp1(3), ".") - 1) > nImageSeq Then
                    nImageSeq = Left(strTemp1(3), InStr(strTemp1(3), ".") - 1)
                End If
            End If
        Next intX
    End If
    ...
    For intX = 1 To spr미전송.MaxRows
        If GET_CELL(spr미전송, 1, intX) = "1" Then
            ...
            rtn = MMSFTP.FTPUploadFile(gtypEQ_INFO.EQIMGFILEPATH & "\" & GET_CELL(spr미전송, 2, intX), strIMGFILEPATH & strFileName & nImageSeq & Mid(GET_CELL(spr미전송, 2, intX), InStr(GET_CELL(spr미전송, 2, intX), ".")))
        End If
    Next intX
...
Private Sub cmdSave_Click()
    ...
    For intRow수진자내역 = 1 To spr수진자내역.MaxRows
        If GET_CELL(spr수진자내역, 1, intRow수진자내역) = "1" Then
            If FUNC_MM_SAVE(intRow수진자내역) = False Then Exit Sub
        End If
    Next intRow수진자내역
