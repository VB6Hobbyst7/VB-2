...
Begin VB.Form frmInterface 
   ...
   Caption         =   " BactAlert 3D Interface "
   ClientHeight    =   10680
   ClientLeft      =   330
   ClientTop       =   825
   ClientWidth     =   15165
...
Dim strExamCode As String
    ...
    strQryDt = Format(strQryDt, "yyyymmdd")
                        ...
                        '  바코드번호,yyyymmdd,hhmmss,rackno
             strSQL = "select BARCODE, TRANSDT, TRANSTM, DISKNO from PAT_RES"
    strSQL = strSQL & " where EQUIPNO = '" & gEquip & "'"
    strSQL = strSQL & "   and TRANSDT >= '" & strQryDt & "'"
    strSQL = strSQL & "   and DISKNO <> '' "
    strSQL = strSQL & " order by BARCODE, TRANSDT, TRANSTM "
    
    Set rs = cn.Execute(strSQL, , adCmdText)
    
    If Not (rs.BOF Or rs.EOF) Then
        Do Until rs.EOF
            If Trim(rs.Fields("BARCODE").Value) <> "" Then
                If strSpcNo <> Trim(rs.Fields("BARCODE").Value) Then
                             Buffer = "1H|\^&|||BACT/ALERT^A.00|||||||P|1|20100611104944" & vbCr
                    Buffer = Buffer & "E5" & vbCr
                    Buffer = Buffer & "2P|1|" & vbCr
                    Buffer = Buffer & "BB" & vbCr
                    Buffer = Buffer & "3O|1|0" & Trim(rs.Fields("BARCODE").Value) & "|0" & Trim(rs.Fields("BARCODE").Value) & "*19486bb5||||||||||||||||||||||I" & vbCr
                    Buffer = Buffer & "0B" & vbCr
                    Buffer = Buffer & "4R|1|^^^BC^BFA^SFJBTVSH|-|||||I|||" & Trim(rs.Fields("TRANSDT").Value) & Trim(rs.Fields("TRANSTM").Value) & "|" & Format(Now, "yyyymmddhhmmss") & "|" & Trim(rs.Fields("DISKNO").Value) & vbCr
                    ...
                    strBuffer = ""
                    intPhase = 1
                    strSpcNo = Trim(rs.Fields("BARCODE").Value)
                End If
...
Private Sub Command16_Click()
    ...
    Open App.Path & "\log\1202.log" For Input As #3
...
Private Sub EditRcvData()
    ...
    Dim strOrdTime, strFOrdTime  As String
    Dim strRstTime, strFRstTime  As String
    ...
    For intCnt = 1 To UBound(strRecvData)
        ...
        Select Case strType
            ...
            Case "R"    '## Result
                ...
                strFlag = mGetP(mGetP(strRcvBuf, 3, "|"), 4, "^")   '-- Channel??
                strIntBase = strFlag
                strRackNo = strTemp2
                ...
                If strTemp1 <> "" Then
                    ...
                    If strResult = "-" Then
                        strFOrdTime = Format(strOrdTime, "####-##-## ##:##:##")
                        strFRstTime = Format(strRstTime, "####-##-## ##:##:##")
                        ...
                        '-- 검사시작 : 검사시작시간 = 장비결과시간
                        If CStr(DateAdd("h", 0, strFOrdTime)) = CStr(DateAdd("h", 0, strFRstTime)) Then
                            strResult = ""
                        '-- 1 Day No Growth : 검사시작시간 + 24시간 <= 장비결과시간 AND 검사시작시간 + 48시간 > 장비결과시간
                        ElseIf CStr(DateAdd("h", 24, strFOrdTime)) <= CStr(DateAdd("h", 0, strFRstTime)) And CStr(DateAdd("h", 48, strFOrdTime)) > CStr(DateAdd("h", 0, strFRstTime)) Then  '-- 1day no growth
                            strResult = "1 Day No Growth"
                        '-- 2 Day No Growth : 검사시작시간 + 48시간 <= 장비결과시간 AND 검사시작시간 + 72시간 > 장비결과시간
                        ElseIf CStr(DateAdd("h", 48, strFOrdTime)) <= CStr(DateAdd("h", 0, strFRstTime)) And CStr(DateAdd("h", 72, strFOrdTime)) > CStr(DateAdd("h", 0, strFRstTime)) Then  '-- 2day no growth
                            strResult = "2 Day No Growth"
                        '-- 3 Day No Growth : 검사시작시간 + 72시간 <= 장비결과시간 AND 검사시작시간 + 96시간 > 장비결과시간
                        ElseIf CStr(DateAdd("h", 72, strFOrdTime)) <= CStr(DateAdd("h", 0, strFRstTime)) And CStr(DateAdd("h", 96, strFOrdTime)) > CStr(DateAdd("h", 0, strFRstTime)) Then  '-- 3day no growth
                            strResult = "3 Day No Growth"
                        '-- 4 Day No Growth : 검사시작시간 + 96시간 <= 장비결과시간 AND 검사시작시간 + 120시간 > 장비결과시간
                        ElseIf CStr(DateAdd("h", 96, strFOrdTime)) <= CStr(DateAdd("h", 0, strFRstTime)) And CStr(DateAdd("h", 120, strFOrdTime)) > CStr(DateAdd("h", 0, strFRstTime)) Then  '-- 4day no growth
                            strResult = "4 Day No Growth"
                        '-- 5 Day No Growth : 검사시작시간 + 96시간 <= 장비결과시간 AND 검사시작시간 + 168시간 > 장비결과시간
                        ElseIf CStr(DateAdd("h", 120, strFOrdTime)) <= CStr(DateAdd("h", 0, strFRstTime)) And CStr(DateAdd("h", 168, strFOrdTime)) > CStr(DateAdd("h", 0, strFRstTime)) Then '-- 5day no growth
                            strResult = "5 Day No Growth"
                        '-- 7 Day No Growth : 검사시작시간 + 168시간 <= 장비결과시간
                        ElseIf CStr(DateAdd("h", 168, strFOrdTime)) <= CStr(DateAdd("h", 0, strFRstTime)) Then  '-- 7day no growth
                            strResult = "7 Day No Growth"     '-- 최종
                        End If
