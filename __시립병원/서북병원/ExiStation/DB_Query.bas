...
Function Get_Sample_Info(ByVal asRow As Long) As Integer
    ...
    sBarcode = Format(sBarcode, "000000000000")
    '환자번호, 환자이름, 주민번호, 성별, 나이
    '-- 검사대상자 가져오기
    ...
    If res = 1 Then
        ...
        SetText frmInterface.vasID, Val(sBarcode), asRow, colBarcode    '3
        SetText frmInterface.vasID, Trim(gReadBuf(5)), asRow, colRack  '4
        SetText frmInterface.vasID, Trim(gReadBuf(0)), asRow, colPID    '6
        SetText frmInterface.vasID, Trim(gReadBuf(2)), asRow, colAge    '9
        Get_Sample_Info = 1
    Else
        Get_Sample_Info = -1
    End If
