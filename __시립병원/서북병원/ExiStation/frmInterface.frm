...
Begin VB.Form frmInterface 
   ...
   Begin VB.Frame Frame4 
      ...
      Begin FPSpread.vaSpread vasPrint 
         ...
         EndProperty
         MaxCols         =   9
         SpreadDesigner  =   "frmInterface.frx":1B23
      End
      Begin FPSpread.vaSpread vasPrintBuf 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         SpreadDesigner  =   "frmInterface.frx":35AA
      End
   End
   Begin TabDlg.SSTab stInterface 
      ...
      EndProperty
      TabCaption(0)   =   "진행상태"
      TabPicture(0)   =   "frmInterface.frx":37D0
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame1"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "결과확인"
      TabPicture(1)   =   "frmInterface.frx":37EC
      ...
      Begin VB.Frame Frame3 
         ...
         Begin FPSpread.vaSpread vasRRes 
            ...
            MaxCols         =   7
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":4226
         End
      End
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            MoveActiveOnFocus=   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":7FCF
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            MaxCols         =   7
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":8A3C
         End
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String)
    ...
    Do Until rs.EOF
        ...
        If sSpecNo <> Trim(rs.Fields(3) & "") & "" And sWorkNo <> Trim(rs.Fields(7) & "") Then
            j = j + 1
            vasID.MaxRows = j
            SetText vasID, "1", j, colCheckBox                  '1  체크박스
            SetText vasID, Trim(rs.Fields(3) & "") & "", j, colSpecNo                 '2  검체번호
            SetText vasID, Val(Trim(rs.Fields(7) & "")), j, colBarcode                '3  바코드번호
...
'    Call Winsock1.Connect(gRemote.ServerIP, gRemote.ServerPort)
            
    If Winsock1.State = sckConnected Then
        lblStatus.Caption = Winsock1.RemoteHost & ":" & Winsock1.RemotePort & " 에 연결성공"
    ...
    cboExam.AddItem "SAM-QL-ES"
    cboExam.AddItem "--Quality"
    cboExam.AddItem "HAV-QL-ES"
    cboExam.AddItem "IFB-QL-ES"
    cboExam.AddItem "MTB-QL-ES"
    cboExam.AddItem "CMV-QL-ES"
    cboExam.AddItem "CPN-QL-ES"
    cboExam.AddItem "EBV-QL-ES"
    cboExam.AddItem "MPN-QL-ES"
    cboExam.AddItem "--2-Quality"
    cboExam.AddItem "CNM-QL-ES"
    cboExam.AddItem "HPV-QL-ES"
    cboExam.AddItem "MTN-QL-ES"
    cboExam.AddItem "--Quantity"
    cboExam.AddItem "CMV-QN-ES"
    cboExam.AddItem "EBV-QN-ES"
    cboExam.AddItem "HBV-QN-ES"
    cboExam.AddItem "HCV-QN-ES"
    cboExam.AddItem "HIV1-QN-ES"
    cboExam.AddItem "HIV2-QN-ES"
    cboExam.AddItem "--2-Quantity"
    cboExam.AddItem "HIM-QN-ES"
...
Private Sub SendOrder()
    ...
    With vasID
        For intCnt = 1 To .DataRowCnt
            ...
            .Row = intCnt
            If .Value = "1" Then
                If intFrameNo = 8 Then intFrameNo = 0
                Select Case intSndPhase
                    ...
                    Case 2  '## Patient   '
                        ...
                        'strOutput = intFrameNo & "P|" & strSeq & "||PID" & Trim(GetText(vasID, intCnt, 2)) & "||" & Han2EngOCX1.HanToEng(Trim(GetText(vasID, intCnt, 7))) & "^^^^^|" & vbCr & ETX
                        strOutput = intFrameNo & "P|" & strSeq & "||PID" & Trim(GetText(vasID, intCnt, 2)) & "||" & Han2EngOCX1.HanToEng(Trim(GetText(vasID, intCnt, 7))) & "^^^^^|" & vbCr & ETX
                        strSeq = strSeq + 1
                        intSndPhase = 3
                        intFrameNo = intFrameNo + 1
                    ...
                    Case 3  '## order
'                      O|1|SID0001^HBV Sample Kit^Lee Chang Yeop blood sample^B^1^11082100555||^^^TID00_HBV^HBV|||||||N|[CR]
''                     O|1|SID0001^HBV Sample Kit^Lee Chang Yeop blood sample^B^1||^^^TID00_HBV^HBV|||||||N|[CR]
                        strOutput = intFrameNo & "O|1|SID" & Trim(GetText(vasID, intCnt, 3)) & "^" & cboExam.Text & "^" & Han2EngOCX1.HanToEng(Trim(GetText(vasID, intCnt, 7))) & _
                                                  "^" & sCol & "^" & sRow & "||^^^" & Trim(GetText(vasID, intCnt, 4)) & "^" & cboExam.Text & "|||||||N|" & vbCr & ETX
...
'
'                            'strOutput = intFrameNo & "P|1||" & Trim(GetText(vasID, intCnt, 2)) & "||" & Han2EngOCX1.HanToEng(Trim(GetText(vasID, intCnt, 7))) & "^^^^^|" & vbCr & ETX
'
'                            strOutput = intFrameNo & "P|1||PID0001||Jeon^Ji Suk1^^^^|" & vbCr & ETX
'
'                            intSndPhase = 3
'                            intFrameNo = intFrameNo + 1
'
...
'
'                            'strOutput = intFrameNo & "O|1|" & Trim(GetText(vasID, intCnt, 3)) & "^" & cboExam.Text & "^" & Han2EngOCX1.HanToEng(Trim(GetText(vasID, intCnt, 7))) & _
'                                                      sCol & "^" & sRow & Trim(GetText(vasID, intCnt, 3)) & "||^^^" & Trim(GetText(vasID, intCnt, 4)) & "^" & cboExam.Text & "|||||||N|" & vbCr & ETX
'
'    '                                                  txtCol & "^" & txtRow & "||^^^" & "TID00_HBV" & "^" & cboExam.Text & "|||||||N|" & vbCr & ETX
'
'                            strOutput = intFrameNo & "O|1|SID0001^CNM-QL-ES^Jeon Ji Suk1 blood sample^B^1||^^^TID00_CNM^CNM|||||||N|" & vbCr & ETX
'
'                            'If intCnt = .DataRowCnt Then
'                                intSndPhase = 4
'                            'Else
...
Private Sub EditRcvData()
    ...
    For intCnt = 1 To UBound(strRecvData)
        ...
        Select Case strType
            ...
            Case "O"    '## Order
                '3O|1|SID0001^CNM-QL-ES^Jeon Ji Suk1 blood sample^B^1||^^^TID00_CNM^CNM|||||||||||||||||||||F||||||8F
...
'                mResult.SpcPos = strTubePos & "/" & strRackNo
            ...
            Case "R"    '## Result
                ...
                '4R|1|^^^TID00_CNM^CNM^IPC CT|Undetermined|||N||F||||||CC

                strTemp1 = mGetP(mGetP(strRcvBuf, 3, "|"), 6, "^")
...
'                End Select
                ...
                If strTemp1 = "NewInfA Result" Then
                    strResult = strResult
                End If
                '6R|3|^^^TID00^HBV^HBV Ct|Undetermined|||N||F||||||
                If strTemp1 = "InfA_Texas Result" Then
                    strResult = strResult
                End If
                ...
                If strResult = "Undetermined" Then
                    strResult = "Undetermined"
                End If
