...
'    ClearSpread frmInterface.vasTemp
'
'    SQL = " Select isocd, equipcode, examcode, antrslt, antsize, EQUIPRESULT, refflag, panicflag, deltaflag,exmncd,isonm,antmachcd " & vbCrLf & _
'          "   From pat_res " & vbCrLf & _
'          " Where equipno = '" & gEquip & "' " & vbCrLf & _
'          "   And examdate = '" & Format(CDate(frmInterface.dtpToday.Value), "yyyymmdd") & "'  " & vbCrLf & _
'          "   And barcode = '" & Trim(GetText(frmInterface.vasResult, argSpcRow, 3)) & "' " & vbCrLf & _
...
'    For iRow = 1 To frmInterface.vasTemp.DataRowCnt
'        If iRow = 1 Then
'            strExmnCD = Trim(GetText(frmInterface.vasTemp, iRow, 3))
...
'            sResult = Trim(GetText(frmInterface.vasTemp, iRow, 11))
'            sCnt = ""
'
'            SQL = ""
'            SQL = SQL & vbCrLf & "SELECT  RES.RECENO"
'            SQL = SQL & vbCrLf & " FROM EXAMRES RES,"
'            SQL = SQL & vbCrLf & "      EXAMMASTER EXAMM"
'            SQL = SQL & vbCrLf & " WHERE RES.HID = '01'"
'            SQL = SQL & vbCrLf & "   AND RES.PID = '" & lsPid & "'"
'            SQL = SQL & vbCrLf & "   AND RES.HID = EXAMM.HID"
'            SQL = SQL & vbCrLf & "   AND RES.EXAMCODE = EXAMM.EXAMCODE"
'            SQL = SQL & vbCrLf & "   AND RES.LABRECYN = 'Y'"
'            SQL = SQL & vbCrLf & "   AND EXAMM.RESCLASSCODE = '4'"
'            SQL = SQL & vbCrLf & "   AND EXAMM.MICGUBUN = '3'"
'            SQL = SQL & vbCrLf & "   AND SUBSTR(RES.RECENO, 1, 8) >= '" & Strings.Left(lsSpecNo, 8) & "'"
'            SQL = SQL & vbCrLf & " GROUP BY RECENO"
'            SQL = SQL & vbCrLf & " ORDER BY RECENO"
'            res = db_select_Col(gServer, SQL)
'
'
'            If res > 0 Then
'                lsCulSpecNo = gReadBuf(0)
'            Else
'
'            End If
'
'            '-- 미생물 세균결과 조회
'            SQL = "SELECT count(*) FROM ISO "
'            SQL = SQL & vbCrLf & " WHERE HID = '01' "
'            SQL = SQL & vbCrLf & "   AND PID = '" & lsPid & "' "
'            SQL = SQL & vbCrLf & "   AND RECENO = '" & lsSpecNo & "' "
'            SQL = SQL & vbCrLf & "   AND SPECIMENCODE = '" & strSpcmCd & "' "
'            SQL = SQL & vbCrLf & "   AND MICCODE = '" & strMICCd & "' "
'
'            res = db_select_Col(gServer, SQL)
'
'
...
'                '-- 미생물 세균결과 저장
'                SQL = "INSERT INTO ISO (HID,PID,RECENO,SPECIMENCODE,MICCODE,RESCODE,INPUT_UID,INPUT_DATETIME,EXAMCODE,REMARK,PRINTCNT,HIGHERFLAG,COUNTGUBUN,ANTIVOL,RESNAME,MGRPCODE,SEQ) "
'                SQL = SQL & vbCrLf & " Values ( "
'                SQL = SQL & vbCrLf & " '01', "                  'HID
'                SQL = SQL & vbCrLf & " '" & lsPid & "', "       'PID
'                SQL = SQL & vbCrLf & " '" & lsSpecNo & "', "    'RECENO
'                SQL = SQL & vbCrLf & " '" & strSpcmCd & "', "   'SPECIMENCODE
'                SQL = SQL & vbCrLf & " '" & UCase(strMICCd) & "', "    'MICCODE
'                SQL = SQL & vbCrLf & " '', "                    'AMOUNT
'                SQL = SQL & vbCrLf & " '" & gUserID & "', "     'INPUT_UID
'                SQL = SQL & vbCrLf & " SYSDATE, "               'INPUT_DATETIME
'                SQL = SQL & vbCrLf & " '" & strExmnCD & "', "   'EXAMCODE
'                SQL = SQL & vbCrLf & " '', "                    'REMARK
'                SQL = SQL & vbCrLf & " '', "                    'PRINTCNT
'                SQL = SQL & vbCrLf & " '', "                    'HIGHERFLAG
'                SQL = SQL & vbCrLf & " '', "                    'COUNTGUBUN
'                SQL = SQL & vbCrLf & " '', "                    'ANTIVOL
'                SQL = SQL & vbCrLf & " '', "                    'RESNAME
'                SQL = SQL & vbCrLf & " '', "                    'MGRPCODE
'                SQL = SQL & vbCrLf & " 0) "                    'SEQ
'            Else
'                '-- 미생물 세균결과 저장
'                SQL = "UPDATE ISO SET INPUT_UID = '" & gUserID & "',"
'                SQL = SQL & vbCrLf & "      INPUT_DATETIME = SYSDATE, "
'                SQL = SQL & vbCrLf & "      EXAMCODE = '" & strExmnCD & "',"
'                SQL = SQL & vbCrLf & "      MICCODE = '" & UCase(strMICCd) & "'"
'                SQL = SQL & vbCrLf & "WHERE HID = '01' "                  'HID
'                SQL = SQL & vbCrLf & "  AND PID = '" & lsPid & "' "       'PID
'                SQL = SQL & vbCrLf & "  AND RECENO = '" & lsSpecNo & "' "    'RECENO
'                SQL = SQL & vbCrLf & "  AND SPECIMENCODE ='" & strSpcmCd & "' "   'SPECIMENCODE
'                'SQL = SQL & vbCrLf & "  AND MICCODE ='" & UCase(strMICCd) & "' "    'MICCODE
'
'            End If
'
'            res = SendQuery(gServer, SQL)
...
'                SQL = SQL & vbCrLf & "                  END) "
'                SQL = SQL & vbCrLf & " WHERE SPECIMENID = '" & lsSpecID & "'"
...
'            Len(Trim(GetText(frmInterface.vasTemp, iRow, 4))) <= 1 And Trim(GetText(frmInterface.vasTemp, iRow, 2)) <> "----" And _
'            Trim(GetText(frmInterface.vasTemp, iRow, 12)) <> "" Then
'
'            SQL = "SELECT count(*) FROM SENSITIVITY "
'            SQL = SQL & vbCrLf & " WHERE HID = '01' "                  'HID
'            SQL = SQL & vbCrLf & "   AND PID = '" & lsPid & "' "       'PID
'            SQL = SQL & vbCrLf & "   AND RECENO = '" & lsSpecNo & "' "    'RECENO
'            SQL = SQL & vbCrLf & "   AND SPECIMENCODE = '" & strSpcmCd & "' "   'SPECIMENCODE
'            SQL = SQL & vbCrLf & "   AND MICCODE = '" & UCase(strMICCd) & "' "    'MICCODE
'            SQL = SQL & vbCrLf & "   AND ANTICODE = '" & Trim(GetText(frmInterface.vasTemp, iRow, 12)) & "' "      'ANTICODE
'            res = db_select_Col(gServer, SQL)
'            If res > 0 And gReadBuf(0) = "0" Then
'                SQL = "INSERT INTO SENSITIVITY (HID,PID,RECENO,SPECIMENCODE,MICCODE,ANTICODE,RESULT,RESULTD,INPUT_UID,INPUT_DATETIME,COMMENT1,MICCLASSCODE,SEQNO,CULTURERECENO,EXAMCODE)"
'                SQL = SQL & vbCrLf & " Values ( "
'                SQL = SQL & vbCrLf & " '01', "                  'HID
'                SQL = SQL & vbCrLf & " '" & lsPid & "', "       'PID
'                SQL = SQL & vbCrLf & " '" & lsSpecNo & "', "    'RECENO
'                SQL = SQL & vbCrLf & " '" & strSpcmCd & "', "   'SPECIMENCODE
'                SQL = SQL & vbCrLf & " '" & strMICCd & "', "    'MICCODE
'                SQL = SQL & vbCrLf & " '" & Trim(GetText(frmInterface.vasTemp, iRow, 12)) & "', "                   'ANTICODE
'                SQL = SQL & vbCrLf & " '" & Trim(GetText(frmInterface.vasTemp, iRow, 5)) & "', "                    'RESULT
'                SQL = SQL & vbCrLf & " '" & Trim(GetText(frmInterface.vasTemp, iRow, 4)) & "', "                    'RESULTD    'R,S
'                SQL = SQL & vbCrLf & " '" & gUserID & "', "     'INPUT_UID
'                SQL = SQL & vbCrLf & " SYSDATE, "               'INPUT_DATETIME
'                SQL = SQL & vbCrLf & " '', "                    'COMMENT1
'                SQL = SQL & vbCrLf & " '', "                    'MICCLASSCODE
'                SQL = SQL & vbCrLf & " '" & i & "', "                    'SEQNO
'                SQL = SQL & vbCrLf & " '" & lsCulSpecNo & "', "                    'CULTURERECENO
'                SQL = SQL & vbCrLf & " '" & strExmnCD & "') "   'EXAMCODE
'            Else
'                SQL = "UPDATE SENSITIVITY SET RESULT = '" & Trim(GetText(frmInterface.vasTemp, iRow, 5)) & "', "
'                SQL = SQL & vbCrLf & "        RESULTD = '" & Trim(GetText(frmInterface.vasTemp, iRow, 4)) & "', "
...
'          " Where RECENO = '" & Trim(sExamCd) & "' " & vbCrLf & _
'          "   AND substr(EXAMCODE,1,5) = 'L4014' " & vbCrLf & _
...
On Error GoTo Err
    ...
    '-- 검사코드
    strExmnCD = Trim(GetText(frmInterface.vasRID, argSpcRow, 8))
    '-- 수치결과
    iResult = Trim(GetText(frmInterface.vasRID, argSpcRow, 10))
    '-- 문자결과
    sResult = Trim(GetText(frmInterface.vasRID, argSpcRow, 11))
    '-- 리마크
    sRemark = Trim(GetText(frmInterface.vasRID, argSpcRow, 12))
    
    sCnt = ""

    SQL = "SELECT EXAMCODE FROM EXAMRES "
    SQL = SQL & vbCrLf & " WHERE SPECIMENID = '" & lsID & "' "     '검체번호
    SQL = SQL & vbCrLf & "   AND EXAMCODE = '" & strExmnCD & "' "  '검사코드
    SQL = SQL & vbCrLf & "   AND EXAMSTATE = 'B' "                 '결과상태
    res = db_select_Col(gServer, SQL)
    ...
    If res > 0 And gReadBuf(0) <> "" Then
        '-- 문자결과
                       SQL = "SELECT RESCLASSCODE FROM EXAMMASTER "
        SQL = SQL & vbCrLf & " WHERE EXAMCODE = '" & strExmnCD & "' " '검사코드
        SQL = SQL & vbCrLf & "   AND SlipCode = 'X' "                 '위탁검사"
        SQL = SQL & vbCrLf & "   AND UseFlag  ='Y' "                  '사용유무"
        res = db_select_Col(gServer, SQL)
        
        ''      SQL = "Select examcode, examalias,"
        ''SQL = SQL & " case when resclasscode = '1' then '숫자'"
        ''SQL = SQL & "      when resclasscode=  '2' then '문자'"
        ''SQL = SQL & "      when resclasscode = '3' then '메모(별지참조)'"
        ''SQL = SQL & "      else '문자'  end"
        ''SQL = SQL & "  From exammaster"
        ''SQL = SQL & " Where slipcode = 'X' --위탁검사"
        ''SQL = SQL & "   and useflag ='Y' --사용유무"
        
        '문자형결과일 경우
        If gReadBuf(0) = "3" Then
            sResult = "별지참조"
        End If
        ...
        If sResult <> "" Then
            ...
            '-- 로컬저장
            SQL = " INSERT INTO SCL_RESULT (MAKEDATE,SPECIMENID,EXAMCODE,PID,PNAME,SEX,AGE,EXAMNAME,RECEDATETIME,RESULT1,RESULT2,REMARK,REFRANGE,REPORTDATE,STATUS) " & vbCrLf & _
                  " VALUES (" & vbCrLf & _
                  "'" & Format(CDate(frmInterface.dtpToday.Value), "yyyymmdd") & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 2)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 8)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 3)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 4)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 5)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 6)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 9)) & "', " & vbCrLf & _
                  "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 7)) & "', " & vbCrLf
                  
            If gReadBuf(0) = "1" Then       '수치결과
                SQL = SQL & "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 10)) & "', " & vbCrLf & _
                            "'', " & vbCrLf
            ElseIf gReadBuf(0) = "2" Then   '문자결과
                SQL = SQL & "'', " & vbCrLf & _
                            "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 11)) & "', " & vbCrLf
            ElseIf gReadBuf(0) = "3" Then   '메모(별지참조)
                SQL = SQL & "'', " & vbCrLf & _
                            "'별지참조', " & vbCrLf
            End If
            
            SQL = SQL & "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 12)) & "', " & vbCrLf & _
                        "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 13)) & "', " & vbCrLf & _
                        "'" & Trim(GetText(frmInterface.vasRID, argSpcRow, 14)) & "', " & vbCrLf & _
                        "'0') "
            ...
            SQL = SQL & vbCrLf & "                       THEN 'D'"
            SQL = SQL & vbCrLf & "                  END) "
            SQL = SQL & vbCrLf & " WHERE SPECIMENID = '" & lsID & "'"
            SQL = SQL & vbCrLf & "   AND EXAMCODE = '" & strExmnCD & "' "
            SQL = SQL & vbCrLf & "   AND NVL(RESEND,' ') || NVL(EXAMSTATE,' ') <> '1D' "
            SQL = SQL & vbCrLf & "   AND LABRECYN = 'Y' "
                                             '결과상태"
