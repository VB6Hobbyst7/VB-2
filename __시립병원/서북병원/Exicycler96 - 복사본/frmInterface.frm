   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            MaxCols         =   7
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":89A2
         End
...
'    SQL = SQL & "   AND substr(EXAMCODE,1,3) = 'L71'"
...
'            gPatient = chrSTX & CCur(i) & gPatient & CheckSum(CStr(i) & gPatient) & chrCR & chrLF
                        ...
                        'strOutput = intFrameNo & "P|1||PID0003||Jeon^Ji Suk1^^^^|" & vbCr & ETX
                        strSeq = strSeq + 1
                        intSndPhase = 3
                        intFrameNo = intFrameNo + 1
...
'            gOrder = chrSTX & CCur(i) & gOrder & CheckSum(CStr(i) & gOrder) & chrCR & chrLF
                        ...
                        strOutput = intFrameNo & "O|1|SID" & Trim(GetText(vasID, intCnt, 3)) & "^" & cboExam.Text & "^" & Han2EngOCX1.HanToEng(Trim(GetText(vasID, intCnt, 7))) & _
                                                  "^" & sCol & "^" & sRow & "||^^^" & Trim(GetText(vasID, intCnt, 4)) & "^" & cboExam.Text & "|||||||N|" & vbCr & ETX
                                                                   '& Trim(GetText(vasID, intCnt, 3)) &
                        'strOutput = intFrameNo & "O|1|SID0003^CNM-QL-ES^Jeon Ji Suk1 blood sample^B^3||^^^TID00_CNM^CNM|||||||N|" & vbCr & ETX
...
'                End Select
                ...
                If strTemp1 = "IPC_HBV Result" Then
                    strResult = strResult
                End If
                '6R|3|^^^TID00^HBV^HBV Ct|Undetermined|||N||F||||||
                If strTemp1 = "HBV Ct" Then
                    strResult = strResult
                End If
