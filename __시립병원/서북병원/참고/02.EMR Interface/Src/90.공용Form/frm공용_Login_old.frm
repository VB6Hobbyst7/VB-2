...
Begin VB.Form frm공용_Login_old 
   ...
   End
End
Attribute VB_Name = "frm공용_Login_old"
...
Public Sub MM_INITIAL()
    ...
    gstrJobMode = "1" '/작업모드(1.표준(DB,FTP 연결가능), 2.임시(ImageCapture만 가능))
    
    '/STEP2.DataBase 연결
    GoSub RTN_DB_CONNECT
    
    '/STEP3.FTP Server 연결
    GoSub RTN_FTP_SERVER
...
Private Sub txtUserPW_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then
        ...
        If OpenDB(gstrREG_DB_CONSTR) = True Then
            gstrQuy = "SELECT UID_1, USERKNAME "
            gstrQuy = gstrQuy & vbCrLf & "  FROM USERMASTER " '/HIS 사용자마스터
            gstrQuy = gstrQuy & vbCrLf & " WHERE UID_1 = '" & Trim(txtUserID) & "' "
            If ReadSQL(gstrQuy, ADR) = False Then Call CloseDB: End
                        
            If Not ADR Is Nothing Then
                'If Trim(txtUserPW) = Trim(ADR!PWD & "") Then
                    gtypUSER.USERID = Trim(ADR!UID_1 & "")
                    gtypUSER.USERNM = Trim(ADR!USERKNAME & "")
                    gtypUSER.USERPW = ""
