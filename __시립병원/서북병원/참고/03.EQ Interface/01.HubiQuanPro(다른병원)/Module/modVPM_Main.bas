...
'/기준/대상 DB 연결문자 변수
Public gstrJobMode          As String '/작업모드(1.표준(DB,FTP 연결가능), 2.임시(ImageCapture만 가능))
...
'''    If Trim(gtypEQ_INFO.EQIMGFILEPATH) = "" Then MsgBox "미전송 파일 저장 폴더 정보가 없습니다.", vbCritical, "확인": Exit Sub
'''    If Trim(gtypEQ_INFO.FTPIMGFILEPATH) = "" Then MsgBox "전송완료 파일 저장 폴더 정보가 없습니다.", vbCritical, "확인": Exit Sub
End Sub
