...
Public Function FUNC_HIS_RESULT_JUDGMENT()
    ...
    gstrQuy = ""
    gstrQuy = gstrQuy & vbCrLf & "SELECT VR_RESULT, VQ_RECENO "
    gstrQuy = gstrQuy & vbCrLf & "  From V_EXAMRES"
    gstrQuy = gstrQuy & vbCrLf & " WHERE VQ_RECEDATE  >= '" & strStartDate & "'"
    gstrQuy = gstrQuy & vbCrLf & "   AND VQ_RECEDATE  <= '" & strEndDate & "'"
    gstrQuy = gstrQuy & vbCrLf & "   AND VQ_RECENO < "
    gstrQuy = gstrQuy & vbCrLf & "              (SELECT MAX(VQ_RECENO)"
    gstrQuy = gstrQuy & vbCrLf & "                 From V_EXAMRES"
    gstrQuy = gstrQuy & vbCrLf & "                WHERE VQ_PID = '" & gtypPAT_RES.PATNO & "' "
    gstrQuy = gstrQuy & vbCrLf & "                  AND VR_EXAMCODE = '" & gtypPAT_RES.EXAMCD & "') "
    gstrQuy = gstrQuy & vbCrLf & "   AND VQ_PID       = '" & gtypPAT_RES.PATNO & "' "
    gstrQuy = gstrQuy & vbCrLf & "   AND VR_EXAMCODE  = '" & gtypPAT_RES.EXAMCD & "' "
    gstrQuy = gstrQuy & vbCrLf & "   AND NVL(VR_RESULT, ' ') <> ' '"
    gstrQuy = gstrQuy & vbCrLf & " ORDER BY VQ_RECEDATE DESC"
    If ReadSQL_HIS(gstrQuy, ADR_HIS) = False Then Call CloseDB_HIS: End

    If Not ADR_HIS Is Nothing Then
        strDBEFORRSLT = Trim(ADR_HIS!VR_RESULT & "")
        ADR_HIS.Close: Set ADR_HIS = Nothing
    End If
    '/이전데이터 불러오기--------------------------------------------------------------------------------------------
