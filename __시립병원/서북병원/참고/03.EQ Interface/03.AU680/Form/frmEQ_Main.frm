    ...
    For intY = 1 To Len(Text1)
        ...
        Select Case strOneByte
            ...
            Case chrETX
                ...
                If Mid(strOneLine, 1, 2) = "DB" Then
                    strStart = "Y"
                    gintOrderNo = 0
                ElseIf Mid(strOneLine, 1, 2) = "DE" Then
                    strStart = "N"
                End If
  
                
                If Mid(strOneLine, 1, 2) = "R " Or Mid(strOneLine, 1, 2) = "D " Or Mid(strOneLine, 1, 2) = "DQ " Then
                    strBuff = strOneLine & vbCrLf
                    txtBuff = ""
                    ReDim gstrOrder(11)
                    Erase gstrOrder
                    ReDim gstrOrder(11)
                    Call SUB_COMM_PART_AU680(strBuff)
                    
'                    If gintOrderNo > 0 Then
'                        intX = 1
'                        Do While gstrOrder(intX) <> ""
'                            SaveData "[Tx] : " & gstrOrder(intX)
'                            'MSComm1.Output = gstrOrder(intX)
'                            'txtBuff = txtBuff & vbCrLf & gstrOrder(intX)
'                            intX = intX + 1
'                        Loop
'                        SaveData "[Tx] : " & chrSTX & "SE" & chrETX
'                        strStart = ""
'                        strOneLine = ""
'                    Else
'
'                    End If
                ElseIf strStart = "Y" And (Mid(strOneLine, 1, 2) <> "DB" And Mid(strOneLine, 1, 2) <> "DE" And Mid(strOneLine, 1, 2) <> "RE") Then
                    txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & vbCrLf
                End If
        End Select
    Next intY
End Sub
...
Public Sub SUB_COMM_PART_AU680(argCOMM_BF As String)
    Dim strCrlf                     '/배열변수(CRLF 기준)
    Dim strPart                     '/배열변수(| 기준)
    Dim strBuff                     '/배열변수(바코드,DISKNO,POSNO)
    
    Dim intSignal   As Integer      '/신호배열순서
    ...
    For intSignal = 0 To UBound(strCrlf) - 1
        
        
    Select Case Mid(strCrlf(intSignal), 1, 2)
        
        Case "D " '/Result
            strEXSEQ = ""                           '/EXSEQ 매기는데 필요함
            strCrlf(intSignal) = Mid(strCrlf(intSignal), 3)   '/구분신호만 빼고 DATA만 취급
          
            '/ "003305 1015                        "
            '/  R   P  S   B
            gtypPAT_RES.DISKNO = Trim(Mid(strCrlf(intSignal), 1, 4)) '/DISKNO(LotNo 의 앞부분)
            gtypPAT_RES.POSNO = Trim(Mid(strCrlf(intSignal), 5, 2)) '/POSNO(LotNo 의 뒷부분)
            gtypPAT_RES.SAMPLENO = Trim(Mid(strCrlf(intSignal), 8, 4))
            gtypPAT_RES.BARCD = Trim(Mid(strCrlf(intSignal), 12, 24))  '/BARCD(검체번호(Barcode))
            ...
            '/검체번호(Barcode)별 검사회차
            If strEXSEQ <> "Y" Then
                gtypPAT_RES.EXSEQ = FUNC_GET_EXSEQ(gtypPAT_RES.BARCD)
                strEXSEQ = "Y"
            End If
            ...
            Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.EXSEQ) '/검사회차 넣어주기
            '/결과 처리하기-----------------------------------------------------------------------------------------------------------------/
            
            strBuff = Mid(strCrlf(intSignal), 36)
            ...
            Do While Len(strBuff) > 16
                gtypPAT_RES.EQCD = Mid(strBuff, 1, 3) '/EQCD(장비검사코드)
                
                
                gtypPAT_RES.EQRESULT = Trim(Mid(strBuff, 4, 6)) '/EQRESULT(장비원시결과)
                ...
                strBuff = Mid(strBuff, 18)
            Loop
            
        Case "R " '/Inquery Order
                strEXSEQ = "Y"
                strEXSEQ = ""
                strCrlf(intSignal) = Mid(strCrlf(intSignal), 3)
                gtypPAT_RES.DISKNO = Trim(Mid(strCrlf(intSignal), 1, 4)) '/DISKNO(LotNo 의 앞부분)
                gtypPAT_RES.POSNO = Trim(Mid(strCrlf(intSignal), 5, 2)) '/POSNO(LotNo 의 뒷부분)
                'gstrSampleType = Trim(Mid(strCrlf(intSignal), 7, 1))
                gtypPAT_RES.SAMPLENO = Trim(Mid(strCrlf(intSignal), 8, 4))
                gtypPAT_RES.BARCD = Trim(Mid(strCrlf(intSignal), 12, 20))  '/BARCD(검체번호(Barcode))
                
                If gstrSampleType = "" Then gstrSampleType = "N"
