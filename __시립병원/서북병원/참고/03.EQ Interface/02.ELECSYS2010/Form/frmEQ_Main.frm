        ...
        Select Case strOneByte
            ...
            Case chrETX
                If Mid(strOneLine, 2, 1) = "L" Then
                    ...
                    strBuff = txtBuff
                    txtBuff = ""
                    Call SUB_COMM_PART_ELECSYS(strBuff)
    ...
    Dim strBuff         As String
        ...
        Select Case strOneByte
            ...
            Case chrETX
                ...
                If Mid(strOneLine, 2, 1) = "H" Then: gstrOrderType = ""
                If Mid(strOneLine, 2, 1) = "Q" Then: gstrOrderType = "Q"
                If Mid(strOneLine, 2, 1) = "R" Then: gstrOrderType = "R"
            ...
            Case chrACK
                If gstrOrderType = "Q" Then
                    ...
                    If gintOrderNo > 4 Then
                        gintOrderNo = 0
                        gstrOrderType = ""
                    Else
                        MSComm1.Output = gstrOrder(gintOrderNo)
        ...
        Select Case Mid(strPart(0), 2)
            ...
            Case "O" '/정보(환자정보)
                ...
                For intLResultRow = 1 To sprLResult.DataRowCnt
                    If Trim(GET_CELL(sprLResult, 1, intLResultRow)) = gtypPAT_RES.BARCD And _
                       Trim(GET_CELL(sprLResult, 2, intLResultRow)) = gtypPAT_RES.EXSEQ And _
                       Trim(GET_CELL(sprLResult, 3, intLResultRow)) = gtypPAT_RES.SAMPLENO Then 'And _
                       Trim(GET_CELL(sprLResult, 4, intLResultRow)) = gtypPAT_RES.DISKNO And _
                       Trim(GET_CELL(sprLResult, 5, intLResultRow)) = gtypPAT_RES.POSNO Then
                ...
                If intLResultTarRow = 0 Then
                    ...
                    If gtypPAT_RES.PATSEX <> "" Or gtypPAT_RES.PATAGE <> "" Then
                        Call SET_CELL(sprLResult, 16, intLResultTarRow, gtypPAT_RES.PATSEX & "/" & gtypPAT_RES.PATAGE)
                    End If
                End If
                '/실시간 검사리스트 보여주기----------------------------------------------------------------------------------------------------/
                If strEXSEQ <> "Y" Then
...
'                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(검사결과(변형된 결과))
                ...
                Call FUNC_RESULT_LIMITCUTOFF                 '/결과 CUTOFF 설정값으로 변환
