...
Public Function FUNC_LOC_DELETE_PAT_RES(argBarcode As String, argSeqNo As String) As Boolean
    '/1.결과신호 받을 때
    '/2.처방내역 발생 때
    '/3.장비 특성에 따라 WHERE 절 조건을 달리한다. 단 BARCD,EXSEQ,EQCD는 유지한다.
    
    FUNC_LOC_DELETE_PAT_RES = False
...
On Error GoTo RTN_ERR
    ...
    ADC_LOC.BeginTrans
                       ...
                       gstrQuy = "DELETE "
    gstrQuy = gstrQuy & vbCrLf & "  FROM PAT_RES "
    gstrQuy = gstrQuy & vbCrLf & " WHERE BARCD     = '" & argBarcode & "'"
    gstrQuy = gstrQuy & vbCrLf & "   AND EXSEQ     =  " & Val(argSeqNo) & " "
    'gstrQuy = gstrQuy & vbCrLf & "   AND EQCD      = '" & gtypPAT_RES.EQCD & "' "
    gstrQuy = gstrQuy & vbCrLf & "   AND SAMPLENO  = '" & gtypPAT_RES.SAMPLENO & "' "
    If RunSQL_LOC(gstrQuy) = False Then Call CloseDB_LOC: Exit Function

    ADC_LOC.CommitTrans
    
    Call CloseDB_LOC
    
    FUNC_LOC_DELETE_PAT_RES = True
...
Public Function FUNC_RESULT_LIMITCUTOFF()
    Dim strResult_Buff  As String   '/결과값이 "< 4"  같이 문자가 섞여들어 올때 숫자만 추출
                                    '/CutOff 체크때문에 만듬
    '/수정해야됨
    '/ 결과에서 "<",">" 빼기
    If InStr(gtypPAT_RES.EQRESULT, "<") > 0 Then
       strResult_Buff = Trim(Mid(gtypPAT_RES.EQRESULT, InStr(gtypPAT_RES.EQRESULT, "<") + 1))
    ElseIf InStr(gtypPAT_RES.EQRESULT, ">") > 0 Then
        strResult_Buff = Trim(Mid(gtypPAT_RES.EQRESULT, InStr(gtypPAT_RES.EQRESULT, ">") + 1))
    End If
    ...
    If Not ADR_LOC Is Nothing Then
        If Trim(ADR_LOC!EQLIMITFLAG1 & "") <> "" Then
            Select Case Trim(ADR_LOC!EQLIMITFLAG1 & "")
                Case "0"
                    '/ 사용하지 않을떄는 그냥 넘어갑시다용
                Case "1"
                    'If strResult_Buff < Trim(ADR_LOC!EQLIMITVALUE1 & "") Then
                Case "2"
