    ...
    For intY = 1 To Len(Text1)
        ...
        Select Case strOneByte
            Case chrSTX
                strOneLine = ""
            Case chrETX
                If Mid(strOneLine, 2, 1) = "L" Then
                    txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & chrLF
                    
                    strBuff = txtBuff
                    txtBuff = ""
                    Call SUB_COMM_PART_NOVA(strBuff)
                    SaveData "[Tx] : " & strBuff
                Else
                    If strETBYN = "Y" Then
                        txtBuff = txtBuff & Mid(strOneLine, 2, Len(strOneLine) - 2) & chrLF
                        strETBYN = "N"
                    Else
                        txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & chrLF
                        
                    End If
                    'chrstx & strOneLine & vbcrlf   <--- 저장하면됨
                    strOneLine = ""
                End If
            Case chrETB
...
Private Sub mnuJobSub_Click(Index As Integer)
    Select Case Index
        Case 0: frmEQ_워크리스트.Show  'vbModal
...
Public Sub SUB_COMM_PART_NOVA(argCOMM_BF As String)
    Dim strCrlf '/배열변수(CRLF 기준)
    Dim strPart '/배열변수(| 기준)
    ...
    For intX = 0 To UBound(strCrlf) - 1
        ...
        Select Case Mid(strPart(0), 2)
            ...
            Case "O" '/정보(환자정보)
                ...
                '/strPart(4): Device Name(코드를 읽고 제품명이 출력된다)
                '/strPart(5): Lot No
                If Trim(strPart(15)) = "Cal" Then GoTo NEXT_DATA
...
'                gtypPAT_RES.POSNO = strLotNo(2) '/POSNO(LotNo 의 뒷부분)
            ...
            Case "R" '/검사결과정보
                ...
                'blnEQCDCheck =
                If FUNC_LOC_EQUIPCD_CHECK(gtypPAT_RES.EQCD) = False Then: GoTo NEXT_DATA  '/IF 에 등록된 장비검사코드가 아닌경우 결과 저장하지 않음
                ...
                'If gtypPAT_RES.EXAMCD <> "" Then
                    ...
                    '/받은 자료 Local 저장
                    If FUNC_LOC_SAVE_PAT_RES = True Then
                        If mnuJobModeAuto.Checked = True And Trim(gtypPAT_RES.BARCD) <> "" Then   '/전송방식이 자동전송이면...
                            Call FUNC_HIS_SAVE '/HIS에 결과 전송
                            Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS에 결과 전송
                        End If
                    End If
                ...
                'Else                                                                    '/Err 로그가 만들어 진다면 처리해줘야하는 부분
...
NEXT_DATA: '/다음 CASE 로 돌릴때 (좋은방식이 있을때 바꿀예정)
    Next intX
End Sub
