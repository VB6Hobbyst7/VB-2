...
On Error GoTo RTN_ERR
    ...
    If gtypPAT_RES.EXAMCD = "" Then GoTo RTN_ERR
...
With frmEQ_Main
    For intCol = gintEQ_StartCol To .sprLResult.MaxCols
        If GET_CELL(.sprLResult, intCol, argRow) <> "" Then
            ...
            Call SET_CELL(.sprLResult, 1, argRow, gtypPAT_RES.BARCD)
            Call SET_CELL(.sprLResult, 2, argRow, gtypPAT_RES.PATNM)
            Call SET_CELL(.sprLResult, 3, argRow, gtypPAT_RES.EXSEQ)
            Call SET_CELL(.sprLResult, 4, argRow, gtypPAT_RES.SAMPLENO)
            Call SET_CELL(.sprLResult, 5, argRow, gtypPAT_RES.DISKNO)
            Call SET_CELL(.sprLResult, 6, argRow, gtypPAT_RES.POSNO)
            Call SET_CELL(.sprLResult, 9, argRow, IIf(gtypPAT_RES.EXDT <> "", Format(gtypPAT_RES.EXDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.EXTM <> "", " " & Format(gtypPAT_RES.EXTM, "@@:@@:@@"), ""))
            Call SET_CELL(.sprLResult, 14, argRow, gtypPAT_RES.PATNO)
            If gtypPAT_RES.PATSEX <> "" Or gtypPAT_RES.PATAGE <> "" Then
                Call SET_CELL(.sprLResult, 15, argRow, gtypPAT_RES.PATSEX & "/" & gtypPAT_RES.PATAGE)
            End If
            '/환자정보 넣기
            
            gtypPAT_RES.EQCD = GET_CELL(.sprLResult, intCol, -999)
