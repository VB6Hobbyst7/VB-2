VERSION 5.00
Object = "{831FDD16-0C5C-11D2-A9FC-0000F8754DA1}#2.0#0"; "MSCOMCTL.OCX"
Object = "{FDAC2480-F4ED-4632-AA78-DCA210A74E49}#6.0#0"; "SPR32X60.ocx"
Object = "{648A5603-2C6E-101B-82B6-000000000014}#1.1#0"; "MSCOMM32.OCX"
Begin VB.Form frmEQ_Main 
   Caption         =   "Hi Interface EQ"
   ClientHeight    =   10170
   ClientLeft      =   1950
   ClientTop       =   1005
   ClientWidth     =   14985
   BeginProperty Font 
      Name            =   "±¼¸²Ã¼"
      Size            =   9
      Charset         =   129
      Weight          =   400
      Underline       =   0   'False
      Italic          =   0   'False
      Strikethrough   =   0   'False
   EndProperty
   Icon            =   "frmEQ_Main.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   ScaleHeight     =   10170
   ScaleWidth      =   14985
   Begin VB.ComboBox cboComport 
      Height          =   300
      Left            =   9420
      TabIndex        =   45
      Top             =   60
      Visible         =   0   'False
      Width           =   1455
   End
   Begin MSCommLib.MSComm MSComm2 
      Left            =   4260
      Top             =   0
      _ExtentX        =   1005
      _ExtentY        =   1005
      _Version        =   393216
      DTREnable       =   -1  'True
      InputLen        =   1
      RThreshold      =   1
      RTSEnable       =   -1  'True
   End
   Begin VB.TextBox txtBarcode 
      Height          =   435
      Left            =   6240
      TabIndex        =   43
      Top             =   60
      Width           =   3075
   End
   Begin VB.CommandButton Command2 
      Caption         =   "Command2"
      Height          =   675
      Left            =   10140
      TabIndex        =   42
      Top             =   4920
      Visible         =   0   'False
      Width           =   3255
   End
   Begin VB.TextBox Text1 
      Height          =   2535
      Left            =   8820
      MultiLine       =   -1  'True
      TabIndex        =   41
      Top             =   6000
      Visible         =   0   'False
      Width           =   4695
   End
   Begin VB.CommandButton Command1 
      Caption         =   "Command1"
      Height          =   495
      Left            =   11580
      TabIndex        =   38
      Top             =   60
      Visible         =   0   'False
      Width           =   1035
   End
   Begin VB.TextBox txtBuff 
      Height          =   1755
      Left            =   1920
      MultiLine       =   -1  'True
      TabIndex        =   37
      Top             =   6360
      Visible         =   0   'False
      Width           =   6195
   End
   Begin VB.TextBox txtSerialData 
      Height          =   5175
      Left            =   15180
      MultiLine       =   -1  'True
      ScrollBars      =   2  '¼öÁ÷
      TabIndex        =   3
      Top             =   4500
      Width           =   4995
   End
   Begin FPSpread.vaSpread sprDResult 
      Height          =   3075
      Left            =   5700
      TabIndex        =   7
      Top             =   1020
      Width           =   9255
      _Version        =   393216
      _ExtentX        =   16325
      _ExtentY        =   5424
      _StockProps     =   64
      BackColorStyle  =   1
      DisplayRowHeaders=   0   'False
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      MaxCols         =   11
      MaxRows         =   10
      SpreadDesigner  =   "frmEQ_Main.frx":263A
      UserResize      =   1
   End
   Begin VB.CommandButton cmdClear 
      Caption         =   "È­¸éÁ¤¸®(&C)"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   495
      Left            =   12660
      Style           =   1  '±×·¡ÇÈ
      TabIndex        =   6
      Top             =   60
      Width           =   1275
   End
   Begin VB.CommandButton cmdExit 
      Caption         =   "Á¾·á(&X)"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   495
      Left            =   13980
      Style           =   1  '±×·¡ÇÈ
      TabIndex        =   5
      Top             =   60
      Width           =   975
   End
   Begin MSCommLib.MSComm MSComm1 
      Left            =   2520
      Top             =   0
      _ExtentX        =   1005
      _ExtentY        =   1005
      _Version        =   393216
      CommPort        =   5
      DTREnable       =   -1  'True
      InputLen        =   1
      RThreshold      =   1
   End
   Begin MSComctlLib.ProgressBar prgPatient 
      Height          =   75
      Left            =   60
      TabIndex        =   2
      Top             =   600
      Width           =   14895
      _ExtentX        =   26273
      _ExtentY        =   132
      _Version        =   393216
      Appearance      =   0
      Scrolling       =   1
   End
   Begin MSComctlLib.StatusBar staCondition 
      Align           =   2  '¾Æ·¡ ¸ÂÃã
      Height          =   375
      Left            =   0
      TabIndex        =   0
      Top             =   9795
      Width           =   14985
      _ExtentX        =   26432
      _ExtentY        =   661
      _Version        =   393216
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         NumPanels       =   8
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            AutoSize        =   2
            Object.Width           =   5398
            MinWidth        =   3528
            Text            =   "Copyright ¨Ï 2010 Medimate Corp."
            TextSave        =   "Copyright ¨Ï 2010 Medimate Corp."
         EndProperty
         BeginProperty Panel2 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            AutoSize        =   1
            Object.Width           =   10927
         EndProperty
         BeginProperty Panel3 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Alignment       =   1
            Object.Width           =   1764
            MinWidth        =   1764
            Picture         =   "frmEQ_Main.frx":2D72
         EndProperty
         BeginProperty Panel4 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Alignment       =   1
            AutoSize        =   2
            Object.Width           =   1588
            MinWidth        =   1058
            Text            =   "Local DB"
            TextSave        =   "Local DB"
         EndProperty
         BeginProperty Panel5 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Alignment       =   1
            AutoSize        =   2
            Object.Width           =   1270
            MinWidth        =   1058
            Text            =   "HIS DB"
            TextSave        =   "HIS DB"
         EndProperty
         BeginProperty Panel6 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Alignment       =   1
            AutoSize        =   2
            Object.Width           =   1058
            MinWidth        =   1058
            Text            =   "COM"
            TextSave        =   "COM"
         EndProperty
         BeginProperty Panel7 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Style           =   6
            Alignment       =   1
            Object.Width           =   1940
            MinWidth        =   1940
            TextSave        =   "2011-12-12"
         EndProperty
         BeginProperty Panel8 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Style           =   5
            Alignment       =   1
            Object.Width           =   1764
            MinWidth        =   1764
            TextSave        =   "¿ÀÈÄ 8:52"
         EndProperty
      EndProperty
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
   End
   Begin FPSpread.vaSpread sprLResult 
      Height          =   5175
      Left            =   60
      TabIndex        =   4
      Top             =   4500
      Width           =   14895
      _Version        =   393216
      _ExtentX        =   26273
      _ExtentY        =   9128
      _StockProps     =   64
      BackColorStyle  =   1
      ColsFrozen      =   14
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   400
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      MaxCols         =   16
      MaxRows         =   20
      OperationMode   =   2
      SelectBlockOptions=   0
      SpreadDesigner  =   "frmEQ_Main.frx":32E9
   End
   Begin VB.Frame fraOption 
      Height          =   675
      Left            =   9360
      TabIndex        =   46
      Top             =   -60
      Width           =   1635
      Begin VB.OptionButton optOption 
         Caption         =   "¸®½ºÆ®·Î Àü¼Û"
         Height          =   180
         Index           =   1
         Left            =   60
         TabIndex        =   48
         Top             =   420
         Width           =   1515
      End
      Begin VB.OptionButton optOption 
         Caption         =   "Àåºñ·Î Àü¼Û"
         Height          =   180
         Index           =   0
         Left            =   60
         TabIndex        =   47
         Top             =   180
         Value           =   -1  'True
         Width           =   1335
      End
   End
   Begin VB.Label lblBarcode 
      Caption         =   "¹ÙÄÚµå"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   14.25
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   315
      Left            =   5100
      TabIndex        =   44
      Top             =   120
      Width           =   975
   End
   Begin VB.Line Line1 
      X1              =   60
      X2              =   5640
      Y1              =   1380
      Y2              =   1380
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Sample No"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   17
      Left            =   120
      TabIndex        =   40
      Top             =   1500
      Width           =   945
   End
   Begin VB.Label lblSAMPLENO 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   39
      Top             =   1500
      Width           =   900
   End
   Begin VB.Label lblDISKNOPOSNO 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   36
      Top             =   1740
      Width           =   900
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Rack/Pos"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   16
      Left            =   120
      TabIndex        =   35
      Top             =   1740
      Width           =   840
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "°Ë»çÈ¸Â÷"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   20
      Left            =   4260
      TabIndex        =   34
      Top             =   1080
      Width           =   780
   End
   Begin VB.Label lblEXSEQ 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "2"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9.75
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FF0000&
      Height          =   195
      Left            =   5160
      TabIndex        =   33
      Top             =   1080
      Width           =   120
   End
   Begin VB.Shape Shape1 
      BackColor       =   &H00FF8080&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      Height          =   255
      Index           =   5
      Left            =   9240
      Top             =   4200
      Width           =   255
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Delta"
      Height          =   180
      Index           =   15
      Left            =   9540
      TabIndex        =   32
      Top             =   4260
      Width           =   450
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Low"
      Height          =   180
      Index           =   14
      Left            =   7920
      TabIndex        =   31
      Top             =   4260
      Width           =   270
   End
   Begin VB.Shape Shape1 
      BackColor       =   &H0000FFFF&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      Height          =   255
      Index           =   4
      Left            =   8400
      Top             =   4200
      Width           =   255
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Panic"
      Height          =   180
      Index           =   13
      Left            =   10320
      TabIndex        =   30
      Top             =   4260
      Width           =   450
   End
   Begin VB.Shape Shape1 
      BackColor       =   &H000000FF&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      Height          =   255
      Index           =   2
      Left            =   10020
      Top             =   4200
      Width           =   255
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "High"
      Height          =   180
      Index           =   12
      Left            =   8700
      TabIndex        =   29
      Top             =   4260
      Width           =   360
   End
   Begin VB.Shape Shape1 
      BackColor       =   &H00FFFF80&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      Height          =   255
      Index           =   1
      Left            =   7620
      Top             =   4200
      Width           =   255
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "°á°ú Àü¼Û"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   11
      Left            =   120
      TabIndex        =   28
      Top             =   2460
      Width           =   885
   End
   Begin VB.Label lblSDDT 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   27
      Top             =   2460
      Width           =   900
   End
   Begin VB.Shape shpCon 
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      FillColor       =   &H00FF0000&
      FillStyle       =   0  '´Ü»ö
      Height          =   375
      Index           =   1
      Left            =   4800
      Top             =   180
      Width           =   135
   End
   Begin VB.Shape shpCon 
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      FillColor       =   &H000000FF&
      FillStyle       =   0  '´Ü»ö
      Height          =   375
      Index           =   0
      Left            =   60
      Top             =   60
      Width           =   135
   End
   Begin VB.Label lblRCDT 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   26
      Top             =   2220
      Width           =   900
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "°á°ú ¼ö½Å"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   10
      Left            =   120
      TabIndex        =   25
      Top             =   2220
      Width           =   885
   End
   Begin VB.Label lblORDGB 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   24
      Top             =   3180
      Width           =   900
   End
   Begin VB.Label lblORDDT 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   23
      Top             =   2940
      Width           =   900
   End
   Begin VB.Label lblSEXAGE 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   22
      Top             =   3900
      Width           =   900
   End
   Begin VB.Label lblPATNM 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   21
      Top             =   3660
      Width           =   900
   End
   Begin VB.Label lblPATNO 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   20
      Top             =   3420
      Width           =   900
   End
   Begin VB.Label lblEXDT 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      ForeColor       =   &H00000000&
      Height          =   180
      Left            =   1260
      TabIndex        =   19
      Top             =   1980
      Width           =   900
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "½Ç½Ã°£ °Ë»ç¸®½ºÆ®"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FFFFFF&
      Height          =   240
      Index           =   8
      Left            =   180
      TabIndex        =   18
      Top             =   4200
      Width           =   1620
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Ã³¹æ Á¾·ù"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   7
      Left            =   120
      TabIndex        =   17
      Top             =   3180
      Width           =   885
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Ã³¹æ ÀÏÀÚ"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   6
      Left            =   120
      TabIndex        =   16
      Top             =   2940
      Width           =   885
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "Ã³¹æ Àü¼Û"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   5
      Left            =   120
      TabIndex        =   15
      Top             =   1980
      Width           =   885
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "¼ºº°/¿¬·É"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   4
      Left            =   120
      TabIndex        =   14
      Top             =   3900
      Width           =   885
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "¼ö°ËÀÚ ¸í"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   3
      Left            =   120
      TabIndex        =   13
      Top             =   3660
      Width           =   885
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "º´·Ï ¹øÈ£"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   180
      Index           =   2
      Left            =   120
      TabIndex        =   12
      Top             =   3420
      Width           =   885
   End
   Begin VB.Label lblBARCD 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "1234567890"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9.75
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FF0000&
      Height          =   195
      Left            =   1260
      TabIndex        =   11
      Top             =   1080
      Width           =   1200
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "°ËÃ¼ ¹øÈ£"
      BeginProperty Font 
         Name            =   "±¼¸²Ã¼"
         Size            =   9.75
         Charset         =   129
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      Height          =   195
      Index           =   1
      Left            =   120
      TabIndex        =   10
      Top             =   1080
      Width           =   1020
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "°ËÃ¼¹øÈ£º° ¼¼ºÎÁ¤º¸"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FFFFFF&
      Height          =   240
      Index           =   0
      Left            =   180
      TabIndex        =   9
      Top             =   720
      Width           =   1815
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "°ËÃ¼¹øÈ£º° °Ë»ç°á°ú"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FFFFFF&
      Height          =   240
      Index           =   9
      Left            =   5820
      TabIndex        =   8
      Top             =   720
      Width           =   1815
   End
   Begin VB.Label lblÀåºñ¸í 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Åõ¸í
      Caption         =   "CLINITEK500"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   18
         Charset         =   0
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H00FFFFFF&
      Height          =   435
      Left            =   300
      TabIndex        =   1
      Top             =   60
      Width           =   2370
   End
   Begin VB.Shape Shape1 
      BackColor       =   &H00FFFFFF&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      BorderColor     =   &H00000000&
      FillColor       =   &H00FF8080&
      FillStyle       =   0  '´Ü»ö
      Height          =   495
      Index           =   3
      Left            =   60
      Shape           =   4  'µÕ±Ù »ç°¢Çü
      Top             =   60
      Width           =   4875
   End
   Begin VB.Shape shpDResult 
      BackColor       =   &H00808000&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      BorderColor     =   &H00000000&
      FillColor       =   &H00FFC0C0&
      FillStyle       =   5  'ÇÏÇâ ´ë°¢¼±
      Height          =   255
      Left            =   5700
      Shape           =   4  'µÕ±Ù »ç°¢Çü
      Top             =   720
      Width           =   9255
   End
   Begin VB.Shape Shape1 
      BackColor       =   &H00808000&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      BorderColor     =   &H00000000&
      FillColor       =   &H00FFC0C0&
      FillStyle       =   5  'ÇÏÇâ ´ë°¢¼±
      Height          =   255
      Index           =   0
      Left            =   60
      Shape           =   4  'µÕ±Ù »ç°¢Çü
      Top             =   720
      Width           =   5595
   End
   Begin VB.Shape shpLResult 
      BackColor       =   &H00808000&
      BackStyle       =   1  'Åõ¸íÇÏÁö ¾ÊÀ½
      BorderColor     =   &H00000000&
      FillColor       =   &H00FFC0C0&
      FillStyle       =   5  'ÇÏÇâ ´ë°¢¼±
      Height          =   255
      Left            =   60
      Shape           =   4  'µÕ±Ù »ç°¢Çü
      Top             =   4200
      Width           =   6915
   End
   Begin VB.Menu mnuFile 
      Caption         =   "File    "
      Begin VB.Menu mnuExit 
         Caption         =   "E&xit"
      End
   End
   Begin VB.Menu mnuSetting 
      Caption         =   "È¯°æ¼³Á¤    "
      Begin VB.Menu mnuSettingSub 
         Caption         =   "Åë½Å¼³Á¤"
         Index           =   0
      End
      Begin VB.Menu mnuSettingSub 
         Caption         =   "HIS DB Á¢¼ÓÁ¤º¸"
         Index           =   1
      End
      Begin VB.Menu mnuSettingSub 
         Caption         =   "ETC DB Á¢¼ÓÁ¤º¸"
         Index           =   2
      End
      Begin VB.Menu mnuSettingSub 
         Caption         =   "Åë½Å½ÅÈ£ ÃßÀû"
         Index           =   3
         Visible         =   0   'False
      End
   End
   Begin VB.Menu mnuJob 
      Caption         =   "ÀÛ¾÷    "
      Begin VB.Menu mnuJobSub 
         Caption         =   "WorkList ÀÛ¾÷"
         Index           =   0
      End
      Begin VB.Menu mnuJobSub 
         Caption         =   "°Ë»ç°á°ú °ü¸®"
         Index           =   1
      End
      Begin VB.Menu mnuJobSub 
         Caption         =   "-"
         Index           =   3
      End
      Begin VB.Menu mnuJobSub 
         Caption         =   "Àü¼Û¹æ½Ä"
         Index           =   4
         Begin VB.Menu mnuJobModeAuto 
            Caption         =   "ÀÚµ¿Àü¼Û[Auto]"
            Checked         =   -1  'True
         End
         Begin VB.Menu mnuJobModeManual 
            Caption         =   "¼öµ¿Àü¼Û[Manual]"
         End
      End
   End
   Begin VB.Menu mnuCode 
      Caption         =   "±âÃÊÄÚµå    "
      Begin VB.Menu mnuCodeSub 
         Caption         =   "Àåºñ°Ë»çÄÚµå °ü¸®"
         Index           =   0
      End
   End
   Begin VB.Menu mnuInfo 
      Caption         =   "Á¤º¸"
   End
End
Attribute VB_Name = "frmEQ_Main"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit

Dim lngMeHeight     As Long     '/Me.HeightÀÇ ÃÊ±â°ª
Dim lngMeWidth      As Long     '/Me.WidthÀÇ ÃÊ±â°ª
Dim strOneLine      As String
Dim strBarSignal    As String   '/mscomm2 ¿¡¼­ ¾¸
Private Type ConWhere   ' »ç¿ëÀÚ Á¤ÀÇ Çü½ÄÀ» ¸¸µì´Ï´Ù.
   Nm       As String
   Left     As Long
   Top      As Long
   Width    As Long
   Height   As Long
End Type
Dim CW()    As ConWhere

Public Function CHK_COMM_PORT() As Boolean
    CHK_COMM_PORT = False
    
On Error GoTo RTN_ERR_PORT

RE_CHK:

    MSComm1.CommPort = gtypEQ_INFO.SERIALPORT
    MSComm1.RTSEnable = gtypEQ_INFO.SERIALRTS
    MSComm1.DTREnable = gtypEQ_INFO.SERIALDTR
    MSComm1.Settings = gtypEQ_INFO.SERIALBAUD & "," & gtypEQ_INFO.SERIALPARITY & "," & gtypEQ_INFO.SERIALDATABIT & "," & gtypEQ_INFO.SERIALSTOPBIT

    If MSComm1.PortOpen = False Then MSComm1.PortOpen = True
    
    CHK_COMM_PORT = True
Exit Function

'/----------------------------------------------------------------------------------------------------/

RTN_ERR_PORT:
    If Err = 8002 Then      'Port
        If MsgBox("¢º¢º¢ºÅë½Å¼³Á¤ Info" & vbCrLf & vbCrLf & _
                  "MSComm Port Setting ÀÌ ¿Ã¹Ù¸£Áö ¾Ê½À´Ï´Ù." & vbCrLf & _
                  "(Àç)¼³Á¤ÇÏ°Ú½À´Ï±î?", vbQuestion + vbYesNo, "ÁúÀÇ") = vbNo Then
            
            MsgBox "¢º¢º¢ºÅë½Å¼³Á¤ Info" & vbCrLf & vbCrLf & _
                   "°è¼Ó ÁøÇàÇÒ °æ¿ì ÀÏºÎ ±â´ÉÀÌ Á¦ÇÑµË´Ï´Ù." & vbCrLf & _
                   "Á¤»óÀûÀÎ ÇÁ·Î±×·¥ ¿î¿ëÀ» À§ÇØ Àü»ê½Ç È¤Àº °ø±Þ¾÷Ã¼¿¡ ¿¬¶ôÁÖ½Ã±â ¹Ù¶ø´Ï´Ù.", vbInformation, "È®ÀÎ"
                
            Exit Function
        Else
            frmEQ°ø¿ë_Set_Port.Show vbModal
            
            GoTo RE_CHK
        End If
    Else
        Resume Next
    End If
End Function

Public Function FUNC_LOC_VIEW(ArgSection As Integer) As Boolean
    Dim strÃ³¹æÄÚµå     As String
    
    FUNC_LOC_VIEW = False
    
On Error GoTo RTN_ERR
    
    
    
'''    If ConnDB_LOC(gstrREG_DB_CONSTR) = True Then
'''        '/ÀåºñÄÚµåº° Ã³¹æÄÚµå °¡Á®¿À±â
'''        gstrQuy = "SELECT ORDCD "
'''        gstrQuy = gstrQuy & vbCrLf & "  FROM MM_EMR_EQORD "
'''        gstrQuy = gstrQuy & vbCrLf & " WHERE EQUIPCODE = '" & gtypEQ_INFO.EQUIPCODE & "' "
'''        If ReadSQL_LOC(gstrQuy, ADR_LOC) = False Then Call CloseDB_LOC: End
'''
'''        If Not ADR_LOC Is Nothing Then
'''            Do Until ADR_LOC.EOF
'''                strÃ³¹æÄÚµå = strÃ³¹æÄÚµå & ",'" & Trim(ADR_LOC!ORDCD & "") & "'"
'''
'''                ADR_LOC.MoveNext
'''            Loop
'''            ADR_LOC.Close: Set ADR_LOC = Nothing
'''
'''            strÃ³¹æÄÚµå = Mid(strÃ³¹æÄÚµå, 2)
'''        End If
    
    
    FUNC_LOC_VIEW = True

Exit Function

'/----------------------------------------------------------------------------------------------------/

RTN_ERR:

End Function

Public Sub SUB_COMM_PART_HUBIQUANPRO_BAR(argCOMM_BF As String)
    '/Patient ID °¡ ¹ÙÄÚµåÀÏ °æ¿ì·Î Ã³¸®ÇÑ´Ù.
    
    '/ÀÏ¹Ý°Ë»ç Sample
    'H|humasis|HUBI-QUAN pro|HP90003|46
    'P|110928-0001|20110928091712|P|CARDIAC 3/1|10-006
    'R1|CK-MB|0.00~5.00|MYO|0.00~100.00|TNI|0.00~0.40|
    'R2|CK-MB|>30.00|ng/mL| |
    'R2|MYO|>150.00|ng/mL| |
    'R2|TNI|7.00|ng/mL| |
    'L|1|N
    
    '/QC Sample
    'H|humasis|HUBI-QUAN pro|HP90003|
    'P|110928-0002|20110928095439|CARDIAC 3/1|10-006
    'R|CK-MB|ng/mL|10.53|Low|29.24|High
    'R|MYO|ng/mL|68.66|Low|>150.00|High
    'R|TNI|ng/mL|2.39|Low|6.87|High
    'L|1|N
    
    Dim strCrlf '/¹è¿­º¯¼ö(CRLF ±âÁØ)
    Dim strPart '/¹è¿­º¯¼ö(| ±âÁØ)
    Dim strLotNo '/Lot No
    Dim strEXSEQ            As String '/EXSEQ °ª Áõ°¡¿©ºÎ
    Dim intLResultRow       As Integer
    Dim intLResultTarRow    As Integer
    Dim intLResultTarCol    As Integer
    Dim intCol              As Integer
    Dim intLineCnt          As Integer
    
    strCrlf = Split(argCOMM_BF, vbCrLf)
    
    For intLineCnt = 0 To UBound(strCrlf) - 1
        strPart = Split(strCrlf(intLineCnt), "|")
        
        Select Case strPart(0)
            Case "H" '/HearderÁ¤º¸(°á°ú½ÅÈ£½ÃÀÛ)
                '/Ã³¸®¾ÈÇÔ
            
            Case "P" '/PatientÁ¤º¸(È¯ÀÚÁ¤º¸)
                '/strPart(0): P
                '/strPart(1): Test No(Barcode) YYDDMM-XXX1 ³¯Â¥º° SEQNO
                '/strPart(2): ³¯Â¥½Ã°£(°Ë»ç½ÃÀÛÀÏ½Ã?, °á°úÀü¼ÛÀÏ½Ã?) YYYYMMDDHHMMSS
                '/strPart(3): Patient ID (ÀÔ·ÂµÇÁö ¾ÊÀ¸¸é P ·Î Ç¥½ÃµÊ)
                '/strPart(4): Device Name(ÄÚµå¸¦ ÀÐ°í Á¦Ç°¸íÀÌ Ãâ·ÂµÈ´Ù)
                '/strPart(5): Lot No
                
                gtypPAT_RES.SAMPLENO = Trim(strPart(1)) '/Sample No(Test No)
                
                '/Patient ID °¡ ¹ÙÄÚµåÀÏ °æ¿ì
                gtypPAT_RES.BARCD = Trim(strPart(3)) '/BARCD(°ËÃ¼¹øÈ£(Barcode))
                '/Patient ID °¡ ¹ÙÄÚµåÀÏ °æ¿ì
                
                gtypPAT_RES.EXDT = Trim(Left(strPart(2), 8)) '/EXDT(°Ë»çÃ³¹æÀü¼ÛÀÏÀÚ(YYYYMMDD) HIEQ->ÀÇ·áÀåºñ)
                gtypPAT_RES.EXTM = Trim(Mid(strPart(2), 9)) '/EXTM(°Ë»çÃ³¹æÀü¼Û½Ã°£(24HHMMSS) HIEQ->ÀÇ·áÀåºñ)
                
                strLotNo = Split(strPart(5), "-")
                gtypPAT_RES.DISKNO = strLotNo(0) '/DISKNO(LotNo ÀÇ ¾ÕºÎºÐ)
                gtypPAT_RES.POSNO = strLotNo(1) '/POSNO(LotNo ÀÇ µÞºÎºÐ)
                
                Call FUNC_HIS_PATIENT '/HIS È¯ÀÚÁ¤º¸ °¡Á®¿À±â
                
                '/°ËÃ¼¹øÈ£/SampleNo/Rack/Pos°¡ Á¤ÀÇµÈ »óÅÂ¿¡¼­ ÁøÇàÇÒ °Í.
                If strEXSEQ <> "Y" Then
                    gtypPAT_RES.EXSEQ = FUNC_GET_EXSEQ(gtypPAT_RES.BARCD) '/°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷
                    strEXSEQ = "Y"
                End If
                
                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
                intLResultTarRow = 0
                For intLResultRow = 1 To sprLResult.DataRowCnt
                    If Trim(GET_CELL(sprLResult, 1, intLResultRow)) = gtypPAT_RES.BARCD And _
                       Trim(GET_CELL(sprLResult, 2, intLResultRow)) = gtypPAT_RES.EXSEQ And _
                       Trim(GET_CELL(sprLResult, 3, intLResultRow)) = gtypPAT_RES.SAMPLENO And _
                       Trim(GET_CELL(sprLResult, 4, intLResultRow)) = gtypPAT_RES.DISKNO And _
                       Trim(GET_CELL(sprLResult, 5, intLResultRow)) = gtypPAT_RES.POSNO Then
                       
                        intLResultTarRow = intLResultRow
                        Exit For
                    End If
                Next intLResultRow
            
                If intLResultTarRow = 0 Then
                    sprLResult.MaxRows = sprLResult.MaxRows + 1
                    intLResultTarRow = sprLResult.MaxRows
                    
                    Call SET_CELL(sprLResult, 1, intLResultTarRow, gtypPAT_RES.BARCD)
                    Call SET_CELL(sprLResult, 2, intLResultTarRow, gtypPAT_RES.EXSEQ)
                    Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.SAMPLENO)
                    Call SET_CELL(sprLResult, 4, intLResultTarRow, gtypPAT_RES.DISKNO)
                    Call SET_CELL(sprLResult, 5, intLResultTarRow, gtypPAT_RES.POSNO)
                    Call SET_CELL(sprLResult, 8, intLResultTarRow, IIf(gtypPAT_RES.EXDT <> "", Format(gtypPAT_RES.EXDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.EXTM <> "", " " & Format(gtypPAT_RES.EXTM, "@@:@@:@@"), ""))
                    Call SET_CELL(sprLResult, 13, intLResultTarRow, gtypPAT_RES.PATNO)
                    Call SET_CELL(sprLResult, 14, intLResultTarRow, gtypPAT_RES.PATNM)
                    If gtypPAT_RES.PATSEX <> "" Or gtypPAT_RES.PATAGE <> "" Then
                        Call SET_CELL(sprLResult, 15, intLResultTarRow, gtypPAT_RES.PATSEX & "/" & gtypPAT_RES.PATAGE)
                    End If
                End If
                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
                
            Case "R" '/°Ë»ç°á°úÁ¤º¸(QC)
                
            Case "R1" '/Âü°í¹üÀ§Á¤º¸(ÀÏ¹Ý°Ë»ç)
                '/Ã³¸®¾ÈÇÔ
            
            Case "R2" '/°Ë»ç°á°úÁ¤º¸(ÀÏ¹Ý°Ë»ç)
                '/strPart(0): R2
                '/strPart(1): Àåºñ°Ë»çÄÚµå
                '/strPart(2): Àåºñ°Ë»ç°á°ú
                '/strPart(3): °á°ú´ÜÀ§
                
                'R2|CK-MB|>30.00|ng/mL| |
            
                gtypPAT_RES.EQCD = strPart(1) '/EQCD(Àåºñ°Ë»çÄÚµå)
                
                gtypPAT_RES.EQRESULT = strPart(2) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
                gtypPAT_RES.RCDT = Format(Now, "YYYYMMDD") '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
                gtypPAT_RES.RCTM = Format(Now, "HHMMSS") '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
                gtypPAT_RES.STATEFLAG = "1" '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
                
                Call FUNC_HIS_ORDER_VIEW    '/Ã³¹æ³»¿ª Á¶È¸
                Call FUNC_HIS_RESULT_JUDGMENT   '/°á°ú ÆÇÁ¤
                
                gtypPAT_RES.SENDFLAG = "0"
                    
                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
                Call SET_CELL(sprLResult, 9, intLResultTarRow, IIf(gtypPAT_RES.RCDT <> "", Format(gtypPAT_RES.RCDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.RCTM <> "", " " & Format(gtypPAT_RES.RCTM, "@@:@@:@@"), "")) '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
                Call SET_CELL(sprLResult, 6, intLResultTarRow, IIf(gtypPAT_RES.STATEFLAG = "1", "°á°ú", "Ã³¹æ"))
                Call SET_CELL(sprLResult, 6, intLResultTarRow, IIf(gtypPAT_RES.SENDFLAG = "1", "¿Ï·á", "´ë±â"))
                Call SET_CELL(sprLResult, 11, intLResultTarRow, gtypPAT_RES.ORDDT) '/ORDDT(Ã³¹æÀÏÀÚ)
                Select Case gtypPAT_RES.ORDGB '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
                    Case "O": Call SET_CELL(sprLResult, 12, intLResultTarRow, "¿Ü·¡")
                    Case "I": Call SET_CELL(sprLResult, 12, intLResultTarRow, "ÀÔ¿ø")
                    Case "G": Call SET_CELL(sprLResult, 12, intLResultTarRow, "°ËÁø")
                End Select
                
                '/Àåºñ°Ë»çÇ×¸ñ Column Set/°á°ú
                For intCol = gintEQ_StartCol To sprLResult.MaxCols
                    If GET_CELL(sprLResult, intCol, -1000) = gtypPAT_RES.EQCD Then
                        Call SET_CELL(sprLResult, intCol, intLResultTarRow, gtypPAT_RES.Result) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
                        
                        sprLResult.Col = intCol
                        sprLResult.Row = intLResultTarRow
                        
                        If gtypPAT_RES.AFLAG = "L" Then
                            sprLResult.BackColor = &HFFFF&
                        End If
                        If gtypPAT_RES.AFLAG = "H" Then
                            sprLResult.BackColor = &HFFFF80
                        End If
                        If gtypPAT_RES.DFLAG = "D" Then
                            sprLResult.BackColor = &HFF8080
                        End If
                        If gtypPAT_RES.PFLAG = "P" Then
                            sprLResult.BackColor = &HFF&
                        End If

                        Exit For
                    End If
                Next intCol
                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
                
                '/ÇØ´ç Row·Î Focus ÀÌµ¿
                sprLResult.Col = 1
                sprLResult.Row = intLResultTarRow
                sprLResult.Action = ActionActiveCell
                
                '/ÇØ´ç Row ÀÚ·á ¼¼ºÎÁ¤º¸¿Í °Ë»ç°á°ú Ç¥½Ã
                Call sprLResult_LeaveRow(intLResultTarRow - 1, False, False, False, intLResultTarRow, False, False)
                
                '/¹ÞÀº ÀÚ·á Local ÀúÀå
                If FUNC_LOC_SAVE_PAT_RES = True Then
                    If mnuJobModeAuto.Checked = True Then '/Àü¼Û¹æ½ÄÀÌ ÀÚµ¿Àü¼ÛÀÌ¸é...
                        Call FUNC_HIS_SAVE '/HIS¿¡ °á°ú Àü¼Û
                        Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS¿¡ °á°ú Àü¼Û
                        
                    End If
                End If
            Case "L" '/Á¾·áÁ¤º¸
                '/Ã³¸®¾ÈÇÔ
    
                gtypPAT_RES.BARCD = ""
                gtypPAT_RES.EXSEQ = ""            '/EXSEQ(°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷)
                gtypPAT_RES.EQCD = ""             '/EQCD(Àåºñ°Ë»çÄÚµå)
                gtypPAT_RES.EXAMCD = ""           '/EXAMCD(Ã³¹æÄÚµå(HIS or LISÀÇ °Ë»çÄÚµå))
                gtypPAT_RES.EXDT = ""             '/EXDT(°Ë»çÃ³¹æÀü¼ÛÀÏÀÚ(YYYYMMDD) HIEQ->ÀÇ·áÀåºñ)
                gtypPAT_RES.EXTM = ""             '/EXTM(°Ë»çÃ³¹æÀü¼Û½Ã°£(24HHMMSS) HIEQ->ÀÇ·áÀåºñ)
                gtypPAT_RES.RCDT = ""             '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
                gtypPAT_RES.RCTM = ""             '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
                gtypPAT_RES.SDDT = ""             '/SDDT(°Ë»ç°á°úÀü¼ÛÀÏÀÚ(YYYYMMDD) HIEQ->HIS)
                gtypPAT_RES.SDTM = ""             '/SDTM(°Ë»ç°á°úÀü¼Û½Ã°£(24HHMMSS) HIEQ->HIS)
                gtypPAT_RES.Result = ""           '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
                gtypPAT_RES.EQRESULT = ""         '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
                gtypPAT_RES.AFLAG = ""            '/AFLAG(Abnormal(Á¤»óÂü°íÄ¡ ±âÁØ (H)High or (L)Low °ª Ç¥½Ã))
                gtypPAT_RES.PFLAG = ""            '/PFLAG(Panic)
                gtypPAT_RES.DFLAG = ""            '/DFLAG(Delta)
                gtypPAT_RES.SAMPLENO = ""         '/Sample No(AU2700, Uriscan µî¿¡ »ç¿ë)
                gtypPAT_RES.DISKNO = ""           '/DISKNO(µð½ºÅ©¹øÈ£ or ·º¹øÈ£)
                gtypPAT_RES.POSNO = ""            '/POSNO(À§Ä¡¹øÈ£)
                gtypPAT_RES.ORDDT = ""            '/ORDDT(Ã³¹æÀÏÀÚ)
                gtypPAT_RES.ORDGB = ""            '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
                gtypPAT_RES.PATNO = ""            '/PATNO(º´·Ï¹øÈ£)
                gtypPAT_RES.PATNM = ""            '/PATNM(¼ö°ËÀÚ¸í)
                gtypPAT_RES.PATSEX = ""           '/PATSEX(¼ºº°)
                gtypPAT_RES.PATAGE = ""           '/PATAGE(¿¬·É)
                gtypPAT_RES.SENDFLAG = ""         '/SENDFLAG(HIS Àü¼Û FLAG (0:´ë±â, 1:¿Ï·á))
                gtypPAT_RES.STATEFLAG = ""        '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
        End Select
    Next intLineCnt
End Sub

Public Sub SUB_MM_CANCEL()
    lblÀåºñ¸í = ""
    prgPatient.Max = 100
    prgPatient.Value = 100
    
    Call SUB_MM_KEY_CLEAR("1") '/°ËÃ¼¹øÈ£º° ¼¼ºÎÁ¤º¸
    Call SUB_MM_KEY_CLEAR("2") '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú
    Call SUB_MM_KEY_CLEAR("3") '/½Ç½Ã°£ °Ë»ç¸®½ºÆ®
    
    mnuSetting.Visible = False '/±¸¼º¸Þ´º ¾Èº¸ÀÌ±â

    txtSerialData.Visible = False
End Sub

Public Sub SUB_MM_INITIAL()
    '/Resize¸¦ À§ÇÑ ÃÊ±â Size Setting----------------------------------------------------------------------------------------------------/
    For intX = 0 To Me.Count - 1
        Select Case True
            Case TypeOf Me.Controls(intX) Is Timer
            Case TypeOf Me.Controls(intX) Is Menu
            Case TypeOf Me.Controls(intX) Is Line
            Case TypeOf Me.Controls(intX) Is MSComm
            Case TypeOf Me.Controls(intX) Is CommonDialog
            Case Else
                ReDim Preserve CW(intX)
                
                CW(intX).Nm = Me.Controls(intX).Name
                CW(intX).Left = Me.Controls(intX).Left
                CW(intX).Top = Me.Controls(intX).Top
                CW(intX).Width = Me.Controls(intX).Width
                CW(intX).Height = Me.Controls(intX).Height
        End Select
    Next intX
    
    '/Form Size Setting
    lngMeHeight = 10890
    lngMeWidth = 15150
    
    Me.Height = lngMeHeight
    Me.Width = lngMeWidth
    Me.Top = (Screen.Height - Me.Height) / 2
    Me.Left = (Screen.Width - Me.Width) / 2
    Me.Show
    '/Resize¸¦ À§ÇÑ ÃÊ±â Size Setting----------------------------------------------------------------------------------------------------/
    
    '/ÃÊ±â ÀÚ·á Setting----------------------------------------------------------------------------------------------------/
    GoSub ADD_ITEM
    '/ÃÊ±â ÀÚ·á Setting----------------------------------------------------------------------------------------------------/
    
    '/º¯µ¿ ÄÁÆ®·Ñ ÃÊ±âÈ­----------------------------------------------------------------------------------------------------/
    Call SUB_MM_CANCEL
    '/º¯µ¿ ÄÁÆ®·Ñ ÃÊ±âÈ­----------------------------------------------------------------------------------------------------/
    
    '/¿öÅ©¸®½ºÆ® ÀÛ¾÷¿©ºÎ(Y.»ç¿ëÇÔ, N.»ç¿ë¾ÈÇÔ)
    If gtypEQ_INFO.WORKLISTGB = "Y" Then
        mnuJobSub(0).Visible = True
    Else
        mnuJobSub(0).Visible = False
    End If
    
    '/ÀÛ¾÷¸ðµå(A.ÀÚµ¿, M.¼öµ¿)
    If gtypEQ_INFO.AUTOGB = "Y" Then
        mnuJobModeAuto.Checked = True
        staCondition.Panels.Item(3).Picture = LoadPicture(App.Path & "\Auto.jpg")
        mnuJobModeManual.Checked = False
    Else
        mnuJobModeManual.Checked = True
        staCondition.Panels.Item(3).Picture = LoadPicture(App.Path & "\Manual.jpg")
        mnuJobModeAuto.Checked = False
    End If
    
    Me.Caption = Me.Caption & " For " & App.FileDescription
    Me.Caption = Me.Caption & Space(10) & "(»ç¿ëÀÚ: " & gtypUSER.USERNM & " )"
    
    lblÀåºñ¸í = App.FileDescription

    '/ÀÛ¾÷ »óÅÂ Check----------------------------------------------------------------------------------------------------/
    If ConnDB_HIS = True Then
        Call CloseDB_HIS
        staCondition.Panels.Item(5).Enabled = True '/HISDB È°¼ºÈ­
    Else
        staCondition.Panels.Item(5).Enabled = False '/HISDB ºñÈ°¼ºÈ­
    End If
    
    If CHK_COMM_PORT = True Then
        staCondition.Panels.Item(6).Enabled = True '/COM Port È°¼ºÈ­
    Else
        staCondition.Panels.Item(6).Enabled = False '/COM Port ºñÈ°¼ºÈ­
    End If
    '/ÀÛ¾÷ »óÅÂ Check----------------------------------------------------------------------------------------------------/
Exit Sub

'/----------------------------------------------------------------------------------------------------/

ADD_ITEM:
    Dim intCnt   As Integer
    
    '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú Title Clear
    '/°Ë»ç¸í|Àåºñ°Ë»çÄÚµå|°á°ú|Wall
    sprDResult.ClearRange 1, -1, 2, -1, True
    sprDResult.ClearRange 4, -1, 5, -1, True
    sprDResult.ClearRange 7, -1, 8, -1, True
    sprDResult.ClearRange 10, -1, 11, -1, True
    
    If sprLResult.MaxCols > gintEQ_StartCol - 1 Then sprLResult.MaxCols = gintEQ_StartCol - 1
    
    If ConnDB_LOC = False Then
        MsgBox "¢º¢º¢ºLocal DataBase Info" & vbCrLf & vbCrLf & _
               "Local DataBase ¸¦ Á¢¼ÓÇÒ ¼ö ¾ø½À´Ï´Ù." & vbCrLf & _
               "Á¤»óÀûÀÎ ÇÁ·Î±×·¥ ¿î¿ëÀ» À§ÇØ Àü»ê½Ç È¤Àº °ø±Þ¾÷Ã¼¿¡ ¿¬¶ôÁÖ½Ã±â ¹Ù¶ø´Ï´Ù.", vbInformation, "È®ÀÎ"
        End
    Else
        frmEQ_Main.staCondition.Panels.Item(4).Enabled = True '/Main È­¸éÀÇ Local »óÅÂ È°¼ºÈ­

        gstrQuy = "SELECT * "
        gstrQuy = gstrQuy & vbCrLf & "  FROM EQ_MST "
        gstrQuy = gstrQuy & vbCrLf & " ORDER BY EQSEQ "
        If ReadSQL_LOC(gstrQuy, ADR_LOC) = False Then End
        
        If Not ADR_LOC Is Nothing Then
            Do Until ADR_LOC.EOF
                sprLResult.MaxCols = sprLResult.MaxCols + 1
                sprLResult.Col = sprLResult.MaxCols
                sprLResult.Row = -1
                sprLResult.BackColor = RGB(255, 255, 230)
                sprLResult.CellType = CellTypeStaticText
                sprLResult.TypeHAlign = TypeHAlignRight
                sprLResult.TypeVAlign = TypeVAlignCenter

                sprLResult.Row = -1000
                sprLResult.Text = Trim(ADR_LOC!EQCD & "")
                sprLResult.RowHidden = True

                sprLResult.Row = -999
                sprLResult.Text = Trim(ADR_LOC!EQNM & "")
                
                intCnt = intCnt + 1             '/½Ç°¡°£ °Ë»ç¸®½ºÆ® °Ë»çÇ×¸ñ ÀÐ±â Áõ°¡
        
                '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú Column
                Select Case intCnt
                    Case 1 To 10:  sprDResult.Col = 1
                    Case 11 To 20: sprDResult.Col = 4
                    Case 21 To 30: sprDResult.Col = 7
                    Case 31 To 40: sprDResult.Col = 10
                End Select
                
                '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú Row
                If (intCnt Mod 10) = 0 Then
                    sprDResult.Row = 10
                Else
                    sprDResult.Row = intCnt Mod 10
                End If
                sprDResult.Text = Trim(ADR_LOC!EQNM & "")
                
                ADR_LOC.MoveNext
            Loop
            
            ADR_LOC.Close: Set ADR_LOC = Nothing
        End If
        
        Call CloseDB_LOC
    End If
Return
End Sub

Public Sub SUB_MM_KEY_CLEAR(ArgSection As String)
    Select Case ArgSection
        Case "1" '/°ËÃ¼¹øÈ£º° ¼¼ºÎÁ¤º¸
            lblBARCD = ""
            lblEXSEQ = ""
            lblSAMPLENO = ""
            lblDISKNOPOSNO = ""
            lblEXDT = ""
            lblRCDT = ""
            lblSDDT = ""
            lblORDDT = ""
            lblORDGB = ""
            lblPATNO = ""
            lblPATNM = ""
            lblSEXAGE = ""
            
        Case "2" '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú
            '/(lCol As Long, lRow As Long, lCol2 As Long, lRow2 As Long, bDataOnly As Boolean)
            sprDResult.ClearRange 2, -1, 2, -1, True
            sprDResult.ClearRange 5, -1, 5, -1, True
            sprDResult.ClearRange 8, -1, 8, -1, True
            sprDResult.ClearRange 11, -1, 11, -1, True
            
        Case "3": '/½Ç½Ã°£ °Ë»ç¸®½ºÆ®
            If sprLResult.MaxRows > 0 Then sprLResult.MaxRows = 0
    End Select
End Sub

Public Sub SUB_MM_PRINT()
'''    Dim strFont1  As String
'''    Dim strFont2  As String
'''    Dim strHead1  As String
'''
'''    If sprVIEW.MaxRows = 0 Then MsgBox "Ãâ·ÂÇÒ ÀÚ·á°¡ ¾ø½À´Ï´Ù.", vbInformation, "È®ÀÎ": Exit Function
'''
'''    If MsgBox("Ãâ·ÂÇÏ°Ú½À´Ï±î?", vbQuestion + vbOKCancel, "Ãâ·Â¿©ºÎ") = vbCancel Then Exit Function
'''
'''    strFont1 = "/fn""±¼¸²Ã¼""/fz""15""/fb1/fi0/fu1/fk0/fs1"
'''    strFont2 = "/fn""±¼¸²Ã¼""/fz""10""/fb0/fi0/fu0/fk0/fs2"
'''
'''    strHead1 = "/f1/c" & "°Å·¡Ã³ ÄÚµå" & "/n/n/n"
'''
'''    With sprVIEW
'''        .PrintAbortMsg = "°Å·¡Ã³ ÄÚµå Ãâ·Â Áß..."
'''        .PrintHeader = strFont1 + strHead1 + strFont2
'''        .PrintFooter = "/c" & "PAGE : " & "/P"
'''        .PrintBorder = True
'''        .PrintGrid = True
'''        .PrintColHeaders = True
'''        .PrintRowHeaders = True
'''        .PrintColor = False
'''        .PrintMarginTop = 500
'''        .PrintMarginBottom = 500
'''        .PrintMarginLeft = 500
'''        .PrintMarginRight = 0
'''        .PrintType = PrintTypeAll
'''        .PrintShadows = False
'''        .PrintUseDataMax = False
'''        .Action = ActionSmartPrint
'''    End With
End Sub


Private Sub cboComport_Click()
On Error GoTo RTN_ERR_PORT
    '/mscomm2¼ÂÆÃ---------------------------------------------------------------------------------------------------
    If MSComm2.PortOpen = True Then MSComm2.PortOpen = False
    
    If cboComport.ListIndex > -1 Then
        'MSComm2.CommPort = 10
        MSComm2.CommPort = Mid(cboComport.Text, 4, 1)
        MSComm2.Settings = "9600,N,8,1"
        
        If MSComm2.PortOpen = False Then MSComm2.PortOpen = True
        If cboComport.Visible = True Then cboComport.Visible = False
        
    End If
    '/mscomm2¼ÂÆÃ---------------------------------------------------------------------------------------------------
Exit Sub
RTN_ERR_PORT:
    If Err = 8002 Then MsgBox "Port Á¤º¸¸¦ È®ÀÎÇØ ÁÖ¼¼¿ä"
End Sub


Private Sub cmdClear_Click()
    Call SUB_MM_KEY_CLEAR("1") '/°ËÃ¼¹øÈ£º° ¼¼ºÎÁ¤º¸
    Call SUB_MM_KEY_CLEAR("2") '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú
    Call SUB_MM_KEY_CLEAR("3") '/½Ç½Ã°£ °Ë»ç¸®½ºÆ®
End Sub

Private Sub cmdExit_Click()
    If frmEQ_¿öÅ©¸®½ºÆ®.Visible = True Then Unload frmEQ_¿öÅ©¸®½ºÆ®
    
    Unload Me
End Sub

Private Sub Command1_Click()
    MsgBox InStr(txtBuff, chrCR)
    Call SUB_COMM_PART_HUBIQUANPRO_BAR(txtBuff)
End Sub

Private Sub Command2_Click()
    Dim strOneByte  As String
    Dim strBuff     As String
    For intY = 1 To Len(Text1)
        
        strOneByte = Mid(Text1, intY, 1)
        
        If shpCon(0).FillColor = &HFF& Then
            shpCon(0).FillColor = &HFF0000
        Else
            shpCon(0).FillColor = &HFF&
        End If
    
        If shpCon(1).FillColor = &HFF& Then
            shpCon(1).FillColor = &HFF0000
        Else
            shpCon(1).FillColor = &HFF
        End If

        strOneLine = strOneLine & strOneByte    '/ÇÑ ¶óÀÎ ´ã´Â º¯¼ö
    
        Select Case strOneByte
            Case chrSTX
                strOneLine = ""
            Case chrETX
                txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & chrLF
                
                strBuff = txtBuff
                txtBuff = ""
                Call SUB_COMM_PART_CLINITEK500(strBuff)
                strOneLine = ""
            Case chrLF
                txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & chrLF
                strOneLine = ""
                    
            Case chrETB
                txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1)
                strETBYN = "Y"
            Case chrEOT
                strOneLine = ""
                'chrEOT <--- ÀúÀåÇØ¾ßµÊ
            Case chrENQ
                strOneLine = ""
                'chrENQ <--- ÀúÀåÇØ¾ßµÊ
        End Select
    Next intY
End Sub

Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    Dim ShiftDown, AltDown, CtrlDown, Txt
   
    ShiftDown = (Shift And vbShiftMask) > 0
    AltDown = (Shift And vbAltMask) > 0
    CtrlDown = (Shift And vbCtrlMask) > 0
   
    If KeyCode = vbKeyM Then   ' Å°ÀÇ Á¶ÇÕ »óÅÂ¸¦ Ãâ·ÂÇÕ´Ï´Ù.
        If mnuSetting.Visible = True Then
            mnuSetting.Visible = False
            
            lblBarcode.BackColor = &H80000005
            txtBarcode.BackColor = &H80000005
            txtBarcode.Enabled = True
            cboComport.Visible = False
            fraOption.Visible = True

        Else
            mnuSetting.Visible = True
                    
            lblBarcode.BackColor = &HFF&
            txtBarcode.BackColor = &H8080FF
            txtBarcode.Enabled = False
            cboComport.Visible = True
            fraOption.Visible = False
        End If
    End If
End Sub

Private Sub Form_Load()
    Call SUB_MM_INITIAL
    
    '/mscomm2¼ÂÆÃ---------------------------------------------------------------------------------------------------
    cboComport.AddItem "COM1", 0
    cboComport.AddItem "COM2", 1
    cboComport.AddItem "COM3", 2
    cboComport.AddItem "COM4", 3
    cboComport.ListIndex = 1
    '/mscomm2¼ÂÆÃ---------------------------------------------------------------------------------------------------
    
    DoEvents
    DoEvents
    DoEvents
End Sub

Private Sub Form_Resize()
    Dim intCnt  As Integer

On Error Resume Next
    '/object.Move Left, Top, Width, Height
    '/(((Me.Height - lngMeHeight) / 3) * 2) : ³ôÀÌ°¡ ´Ã¾î³ª´Â °³Ã¼ 3°³, µðÀÚÀÎ»ó ÇØ´ç °³Ã¼ À§¿¡ ´Ã¾î³­ °³Ã¼°¡ 2°³
    For intCnt = 0 To UBound(CW)
        Select Case CW(intCnt).Nm
            Case cmdClear.Name:     cmdClear.Move CW(intCnt).Left + (Me.Width - lngMeWidth), CW(intCnt).Top, CW(intCnt).Width, CW(intCnt).Height
            Case cmdExit.Name:      cmdExit.Move CW(intCnt).Left + (Me.Width - lngMeWidth), CW(intCnt).Top, CW(intCnt).Width, CW(intCnt).Height
            Case prgPatient.Name:   prgPatient.Move CW(intCnt).Left, CW(intCnt).Top, CW(intCnt).Width + (Me.Width - lngMeWidth), CW(intCnt).Height
            Case shpDResult.Name:   shpDResult.Move CW(intCnt).Left, CW(intCnt).Top, CW(intCnt).Width + (Me.Width - lngMeWidth), CW(intCnt).Height
            Case sprDResult.Name:   sprDResult.Move CW(intCnt).Left, CW(intCnt).Top, CW(intCnt).Width + (Me.Width - lngMeWidth), CW(intCnt).Height
            Case shpLResult.Name:   shpLResult.Move CW(intCnt).Left, CW(intCnt).Top, CW(intCnt).Width + (Me.Width - lngMeWidth), CW(intCnt).Height
            Case sprLResult.Name:   sprLResult.Move CW(intCnt).Left, CW(intCnt).Top, CW(intCnt).Width + (Me.Width - lngMeWidth), CW(intCnt).Height + (Me.Height - lngMeHeight)
        End Select
    Next intCnt

End Sub

Private Sub Form_Unload(Cancel As Integer)
    Call CloseDB_LOC
    Call CloseDB_HIS
    Call CloseDB_ETC
    
    If MSComm1.PortOpen = True Then MSComm1.PortOpen = False
    Set frmEQ_Main = Nothing
End Sub

Private Sub mnuCodeSub_Click(Index As Integer)
    Select Case Index
        Case 0:
            MsgBox "ÀÇ·áÀåºñ¿Í Åë½Å Áß¿¡ Àåºñ°Ë»çÄÚµå Á¤º¸¸¦ ¼öÁ¤ÇÏ¸é" & vbCrLf & _
                   "ÀÇµµµÇÁö ¾ÊÀº °á°ú¸¦ ÃÊ·¡ÇÒ ¼ö ÀÖ½À´Ï´Ù." & vbCrLf & vbCrLf & _
                   "Àåºñ°Ë»çÄÚµå Á¤º¸¸¦ ¼öÁ¤ÇÑ ÈÄ¿£ ÇÁ·Î±×·¥À» Àç ½ÇÇàÇÏ½Ê½Ã¿À", vbExclamation, "ÁÖÀÇ"
        
            frmEQ°ø¿ë_Àåºñ°Ë»çÄÚµå°ü¸®_Á¶È¸.Show vbModal
    End Select
End Sub

Private Sub mnuExit_Click()
    Unload Me
End Sub

Private Sub mnuInfo_Click()
    frmEQ°ø¿ë_Info.Show vbModal
End Sub

Private Sub mnuJobModeAuto_Click()
    If mnuJobModeAuto.Checked = False Then
        If MsgBox("°Ë»ç°á°ú¿¡ ´ëÇØ HIS(º´¿øÁ¤º¸½Ã½ºÅÛ)·ÎÀÇ Àü¼Û¹æ½ÄÀ» ÀÚµ¿Àü¼Û[Auto] (À¸)·Î ÇÏ°Ú½À´Ï±î?" & vbCrLf & vbCrLf & _
                  "(ÁÖÀÇ: ÀÇ·áÀåºñ¿Í Åë½ÅÁß ÀÏ¶§´Â Àü¼Û¹æ½ÄÀ» ¹Ù²ÙÁö ¸¶½Ê½Ã¿À!)", vbQuestion + vbOKCancel + vbDefaultButton2, "Àü¼Û¹æ½Ä º¯°æ È®ÀÎ") = vbCancel Then Exit Sub
        mnuJobModeAuto.Checked = True
        staCondition.Panels.Item(3).Picture = LoadPicture(App.Path & "\Auto.jpg")
        mnuJobModeManual.Checked = False
    End If
End Sub

Private Sub mnuJobModeManual_Click()
    If mnuJobModeManual.Checked = False Then
        If MsgBox("°Ë»ç°á°ú¿¡ ´ëÇØ HIS(º´¿øÁ¤º¸½Ã½ºÅÛ)·ÎÀÇ Àü¼Û¹æ½ÄÀ» ¼öµ¿Àü¼Û[Manual] (À¸)·Î ÇÏ°Ú½À´Ï±î?" & vbCrLf & vbCrLf & _
                  "(ÁÖÀÇ: ÀÇ·áÀåºñ¿Í Åë½ÅÁß ÀÏ¶§´Â Àü¼Û¹æ½ÄÀ» ¹Ù²ÙÁö ¸¶½Ê½Ã¿À!)", vbQuestion + vbOKCancel + vbDefaultButton2, "Àü¼Û¹æ½Ä º¯°æ È®ÀÎ") = vbCancel Then Exit Sub
        mnuJobModeManual.Checked = True
        staCondition.Panels.Item(3).Picture = LoadPicture(App.Path & "\Manual.jpg")
        mnuJobModeAuto.Checked = False
    End If
End Sub

Private Sub mnuJobSub_Click(Index As Integer)
    Select Case Index
        Case 0: frmEQ_¿öÅ©¸®½ºÆ®.Show
        Case 1: frmEQ_°Ë»ç°á°ú°ü¸®.Show vbModal
    End Select
End Sub

Private Sub mnuSettingSub_Click(Index As Integer)
    Select Case Index
        Case 0: frmEQ°ø¿ë_Set_Port.Show vbModal
        Case 1: gstrArgTemp1 = "HIS": frmEQ°ø¿ë_Set_DB.Show vbModal
        Case 2: gstrArgTemp1 = "ETC": frmEQ°ø¿ë_Set_DB.Show vbModal
        Case 3:
            txtSerialData.Left = 7800
            txtSerialData.Top = 2775
            If txtSerialData.Visible = False Then
                txtSerialData.Visible = True
            Else
                txtSerialData.Visible = False
            End If
        End Select
End Sub

Private Sub MSComm1_OnComm()
    Dim strOneByte      As String
    Dim strBuff         As String
    
    strOneByte = MSComm1.Input
        
        If shpCon(0).FillColor = &HFF& Then
            shpCon(0).FillColor = &HFF0000
        Else
            shpCon(0).FillColor = &HFF&
        End If
    
        If shpCon(1).FillColor = &HFF& Then
            shpCon(1).FillColor = &HFF0000
        Else
            shpCon(1).FillColor = &HFF
        End If

        strOneLine = strOneLine & strOneByte    '/ÇÑ ¶óÀÎ ´ã´Â º¯¼ö
    
        Select Case strOneByte
            Case chrSTX
                strOneLine = ""
            Case chrETX
                txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & chrLF
                
                strBuff = txtBuff
                txtBuff = ""
                Call SUB_COMM_PART_CLINITEK500(strBuff)
                SaveData "[Rx] : " & vbCrLf & strBuff
                strOneLine = ""
            Case chrLF
                txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1) & chrLF
                strOneLine = ""
                    
            Case chrETB
                txtBuff = txtBuff & Mid(strOneLine, 1, Len(strOneLine) - 1)
                strETBYN = "Y"
            Case chrEOT
                strOneLine = ""
                'chrEOT <--- ÀúÀåÇØ¾ßµÊ
            Case chrENQ
                strOneLine = ""
                'chrENQ <--- ÀúÀåÇØ¾ßµÊ
        End Select
End Sub

Public Sub SUB_COMM_PART_CLINITEK500(argCOMM_BF As String)
    Dim strCrlf '/¹è¿­º¯¼ö(CRLF ±âÁØ)
    Dim strPart '/¹è¿­º¯¼ö(| ±âÁØ)
    Dim strBuff '/¹è¿­º¯¼ö(¹ÙÄÚµå,DISKNO,POSNO)
    
    Dim strLotNo '/Lot No
    Dim strEXSEQ    As String
    
    Dim blnEQCDCheck    As Boolean
    
    Dim intLResultRow       As Integer
    Dim intLResultTarRow    As Integer
    Dim intLResultTarCol    As Integer
    Dim intCol              As Integer
    Dim intLineCnt          As Integer
    Dim strDataBuff         As String
    
    Dim intDataCnt          As Integer
    
    Dim intWorkList         As Integer
    
    strDataBuff = ""
    strCrlf = Split(argCOMM_BF, vbCrLf)

    '/ÃÊ±âÈ­ ÇØ¾ßÂ¡....
    gtypPAT_RES.BARCD = ""
    gtypPAT_RES.EXSEQ = ""            '/EXSEQ(°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷)
    gtypPAT_RES.EQCD = ""             '/EQCD(Àåºñ°Ë»çÄÚµå)
    gtypPAT_RES.EXAMCD = ""           '/EXAMCD(Ã³¹æÄÚµå(HIS or LISÀÇ °Ë»çÄÚµå))
    gtypPAT_RES.EXDT = ""             '/EXDT(°Ë»çÃ³¹æÀü¼ÛÀÏÀÚ(YYYYMMDD) HIEQ->ÀÇ·áÀåºñ)
    gtypPAT_RES.EXTM = ""             '/EXTM(°Ë»çÃ³¹æÀü¼Û½Ã°£(24HHMMSS) HIEQ->ÀÇ·áÀåºñ)
    gtypPAT_RES.RCDT = ""             '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
    gtypPAT_RES.RCTM = ""             '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
    gtypPAT_RES.SDDT = ""             '/SDDT(°Ë»ç°á°úÀü¼ÛÀÏÀÚ(YYYYMMDD) HIEQ->HIS)
    gtypPAT_RES.SDTM = ""             '/SDTM(°Ë»ç°á°úÀü¼Û½Ã°£(24HHMMSS) HIEQ->HIS)
    gtypPAT_RES.Result = ""           '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
    gtypPAT_RES.EQRESULT = ""         '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
    gtypPAT_RES.AFLAG = ""            '/AFLAG(Abnormal(Á¤»óÂü°íÄ¡ ±âÁØ (H)High or (L)Low °ª Ç¥½Ã))
    gtypPAT_RES.PFLAG = ""            '/PFLAG(Panic)
    gtypPAT_RES.DFLAG = ""            '/DFLAG(Delta)
    gtypPAT_RES.SAMPLENO = ""         '/Sample No(AU2700, Uriscan µî¿¡ »ç¿ë)
    gtypPAT_RES.DISKNO = ""           '/DISKNO(µð½ºÅ©¹øÈ£ or ·º¹øÈ£)
    gtypPAT_RES.POSNO = ""            '/POSNO(À§Ä¡¹øÈ£)
    gtypPAT_RES.ORDDT = ""            '/ORDDT(Ã³¹æÀÏÀÚ)
    gtypPAT_RES.ORDGB = ""            '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
    gtypPAT_RES.PATNO = ""            '/PATNO(º´·Ï¹øÈ£)
    gtypPAT_RES.PATNM = ""            '/PATNM(¼ö°ËÀÚ¸í)
    gtypPAT_RES.PATSEX = ""           '/PATSEX(¼ºº°)
    gtypPAT_RES.PATAGE = ""           '/PATAGE(¿¬·É)
    gtypPAT_RES.SENDFLAG = ""         '/SENDFLAG(HIS Àü¼Û FLAG (0:´ë±â, 1:¿Ï·á))
    gtypPAT_RES.STATEFLAG = ""        '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
    
    '/?Àç°ËÀÏ¶§ ±âÁ¸ ÀÚ·á Áßº¹ È®ÀÎÇÒ ¼ö ÀÖ´Â ¹æ¹ý
    For intDataCnt = 1 To UBound(strCrlf) - 1
        'strPart = Split(strCrlf(intX), "|")
                
        gtypPAT_RES.EXAMCD = ""
        strDataBuff = strCrlf(intDataCnt)
        Select Case intDataCnt
            Case 1
                
                '/Àåºñ¿¡ ¹ÙÄÚµåÀü¼Û ¾øÀÌ ´Ü¹æÇâÀ¸·Î ¹ÞÀ»¶§
                If frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList.MaxRows > 0 Then
                    For intWorkList = 1 To frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList.MaxRows
                        frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList.Row = intWorkList
                        frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList.Col = 1
                        If frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList.Value = 1 Then
                            frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList.Value = 0
                            gtypPAT_RES.BARCD = Trim(GET_CELL(frmEQ_¿öÅ©¸®½ºÆ®.sprWorkList, 3, intWorkList))
                            Exit For
                        End If
                    Next intWorkList
                End If
                
                If gtypPAT_RES.BARCD = "" Then gtypPAT_RES.BARCD = "No Barcode"
                
                gtypPAT_RES.PATNM = ""  '/PATNM(¼ö°ËÀÚ¸í)
                gtypPAT_RES.PATSEX = "" '/PATSEX(¼ºº°)
                gtypPAT_RES.PATAGE = "" '/PATAGE(¿¬·É)
                Call FUNC_HIS_PATIENT
                
                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
                    intLResultTarRow = 0
                    For intLResultRow = 1 To sprLResult.DataRowCnt
                        If Trim(GET_CELL(sprLResult, 1, intLResultRow)) = gtypPAT_RES.BARCD And _
                           Trim(GET_CELL(sprLResult, 3, intLResultRow)) = gtypPAT_RES.EXSEQ And _
                           Trim(GET_CELL(sprLResult, 4, intLResultRow)) = gtypPAT_RES.SAMPLENO And _
                           Trim(GET_CELL(sprLResult, 5, intLResultRow)) = gtypPAT_RES.DISKNO And _
                           Trim(GET_CELL(sprLResult, 6, intLResultRow)) = gtypPAT_RES.POSNO Then
    
                            intLResultTarRow = intLResultRow
                            Exit For
                        End If
                    Next intLResultRow
    
                    If intLResultTarRow = 0 Then
                        sprLResult.MaxRows = sprLResult.MaxRows + 1
                        intLResultTarRow = sprLResult.MaxRows
    
                        Call SET_CELL(sprLResult, 1, intLResultTarRow, gtypPAT_RES.BARCD)
                        Call SET_CELL(sprLResult, 2, intLResultTarRow, gtypPAT_RES.PATNM)
                        Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.EXSEQ)
                        Call SET_CELL(sprLResult, 4, intLResultTarRow, gtypPAT_RES.SAMPLENO)
                        Call SET_CELL(sprLResult, 5, intLResultTarRow, gtypPAT_RES.DISKNO)
                        Call SET_CELL(sprLResult, 6, intLResultTarRow, gtypPAT_RES.POSNO)
                        Call SET_CELL(sprLResult, 9, intLResultTarRow, IIf(gtypPAT_RES.EXDT <> "", Format(gtypPAT_RES.EXDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.EXTM <> "", " " & Format(gtypPAT_RES.EXTM, "@@:@@:@@"), ""))
                        Call SET_CELL(sprLResult, 14, intLResultTarRow, gtypPAT_RES.PATNO)
                        If gtypPAT_RES.PATSEX <> "" Or gtypPAT_RES.PATAGE <> "" Then
                            Call SET_CELL(sprLResult, 15, intLResultTarRow, gtypPAT_RES.PATSEX & "/" & gtypPAT_RES.PATAGE)
                        End If
                    End If
                    '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â---------------------------------------------------------------------------------------------------
        
'            Case 3              '/ "Color: STRAW        "
'                gtypPAT_RES.EQCD = Mid(strDataBuff, 1, 5) '/EQCD(Àåºñ°Ë»çÄÚµå)
'
'                If FUNC_LOC_EQUIPCD_CHECK(gtypPAT_RES.EQCD) = False Then: GoTo NEXT_DATA  '/IF ¿¡ µî·ÏµÈ Àåºñ°Ë»çÄÚµå°¡ ¾Æ´Ñ°æ¿ì °á°ú ÀúÀåÇÏÁö ¾ÊÀ½
'
'                gtypPAT_RES.EQRESULT = Trim(Mid(strDataBuff, 8)) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
'
'                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
'                gtypPAT_RES.EXDT = Format(Now, "YYYYMMDD")
'                gtypPAT_RES.EXTM = Format(Now, "HHMMSS")
'                gtypPAT_RES.RCDT = Format(Now, "YYYYMMDD") '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
'                gtypPAT_RES.RCTM = Format(Now, "HHMMSS") '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
'                gtypPAT_RES.STATEFLAG = "1" '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
'
'                Call FUNC_HIS_ORDER_VIEW    '/Ã³¹æ³»¿ª Á¶È¸
'                'If gtypPAT_RES.EXAMCD <> "" Then
'                    gtypPAT_RES.AFLAG = "" '/AFLAG(Abnormal(Á¤»óÂü°íÄ¡ ±âÁØ (H)High or (L)Low °ª Ç¥½Ã))
'                    gtypPAT_RES.PFLAG = "" '/PFLAG(Panic)
'                    gtypPAT_RES.DFLAG = "" '/DFLAG(Delta)
'                    'Call FUNC_HIS_RESULT_JUDGMENT '/°á°ú ÆÇÁ¤
'
'                    gtypPAT_RES.SENDFLAG = "0"
'
'                    '/Àåºñ°Ë»çÇ×¸ñ Column Set/°á°ú
'                    For intCol = gintEQ_StartCol To sprLResult.MaxCols
'                        If GET_CELL(sprLResult, intCol, -1000) = gtypPAT_RES.EQCD Then
'                            Call SET_CELL(sprLResult, intCol, intLResultTarRow, gtypPAT_RES.Result) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
'
'                            sprLResult.Col = intCol
'                            sprLResult.Row = intLResultTarRow
'
'                            If gtypPAT_RES.AFLAG = "L" Then
'                                sprLResult.BackColor = &HFFFF80
'                            End If
'                            If gtypPAT_RES.AFLAG = "H" Then
'                                sprLResult.BackColor = &HFFFF&
'                            End If
'                            If gtypPAT_RES.DFLAG = "D" Then
'                                sprLResult.BackColor = &HFF8080
'                            End If
'                            If gtypPAT_RES.PFLAG = "P" Then
'                                sprLResult.BackColor = &HFF&
'                            End If
'
'                            Exit For
'                        End If
'                    Next intCol
'
'                    '/ÇØ´ç Row·Î Focus ÀÌµ¿
'                    sprLResult.Col = 1
'                    sprLResult.Row = intLResultTarRow
'                    sprLResult.Action = ActionActiveCell
'
'                    If strEXSEQ <> "Y" Then
'                        gtypPAT_RES.EXSEQ = FUNC_GET_EXSEQ(gtypPAT_RES.BARCD) '/°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷
'                        strEXSEQ = "Y"
'                    End If
'
'                    Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.EXSEQ)
'                    '/¹ÞÀº ÀÚ·á Local ÀúÀå
'                    If FUNC_LOC_SAVE_PAT_RES = True Then
'                        If mnuJobModeAuto.Checked = True Then '/Àü¼Û¹æ½ÄÀÌ ÀÚµ¿Àü¼ÛÀÌ¸é...
'                            Call FUNC_HIS_SAVE '/HIS¿¡ °á°ú Àü¼Û
'                            Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS¿¡ °á°ú Àü¼Û
'                        End If
'                    End If
'                    '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
'                    Call SET_CELL(sprLResult, 10, intLResultTarRow, IIf(gtypPAT_RES.RCDT <> "", Format(gtypPAT_RES.RCDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.RCTM <> "", " " & Format(gtypPAT_RES.RCTM, "@@:@@:@@"), "")) '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
'                    Call SET_CELL(sprLResult, 7, intLResultTarRow, IIf(gtypPAT_RES.STATEFLAG = "1", "°á°ú", "Ã³¹æ"))
'                    Call SET_CELL(sprLResult, 8, intLResultTarRow, IIf(gtypPAT_RES.SENDFLAG = "1", "¿Ï·á", "´ë±â"))
'                    Call SET_CELL(sprLResult, 12, intLResultTarRow, gtypPAT_RES.ORDDT) '/ORDDT(Ã³¹æÀÏÀÚ)
'                    Select Case gtypPAT_RES.ORDGB '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
'                        Case "O": Call SET_CELL(sprLResult, 13, intLResultTarRow, "¿Ü·¡")
'                        Case "I": Call SET_CELL(sprLResult, 13, intLResultTarRow, "ÀÔ¿ø")
'                        Case "G": Call SET_CELL(sprLResult, 13, intLResultTarRow, "°ËÁø")
'                    End Select
'                'Else                                                                    '/Err ·Î±×°¡ ¸¸µé¾î Áø´Ù¸é Ã³¸®ÇØÁà¾ßÇÏ´Â ºÎºÐ
'
'                'End If
'            Case 4              '/"Clarity: CLEAR      "
'                gtypPAT_RES.EQCD = Mid(strDataBuff, 1, 7) '/EQCD(Àåºñ°Ë»çÄÚµå)
'
'                If FUNC_LOC_EQUIPCD_CHECK(gtypPAT_RES.EQCD) = False Then: GoTo NEXT_DATA  '/IF ¿¡ µî·ÏµÈ Àåºñ°Ë»çÄÚµå°¡ ¾Æ´Ñ°æ¿ì °á°ú ÀúÀåÇÏÁö ¾ÊÀ½
'
'                gtypPAT_RES.EQRESULT = Trim(Mid(strDataBuff, 10)) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
'
'                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
'                gtypPAT_RES.EXDT = Format(Now, "YYYYMMDD")
'                gtypPAT_RES.EXTM = Format(Now, "HHMMSS")
'                gtypPAT_RES.RCDT = Format(Now, "YYYYMMDD") '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
'                gtypPAT_RES.RCTM = Format(Now, "HHMMSS") '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
'                gtypPAT_RES.STATEFLAG = "1" '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
'
'                Call FUNC_HIS_ORDER_VIEW    '/Ã³¹æ³»¿ª Á¶È¸
'                'If gtypPAT_RES.EXAMCD <> "" Then
'                    gtypPAT_RES.AFLAG = "" '/AFLAG(Abnormal(Á¤»óÂü°íÄ¡ ±âÁØ (H)High or (L)Low °ª Ç¥½Ã))
'                    gtypPAT_RES.PFLAG = "" '/PFLAG(Panic)
'                    gtypPAT_RES.DFLAG = "" '/DFLAG(Delta)
'                    'Call FUNC_HIS_RESULT_JUDGMENT '/°á°ú ÆÇÁ¤
'
'                    gtypPAT_RES.SENDFLAG = "0"
'
'                    '/Àåºñ°Ë»çÇ×¸ñ Column Set/°á°ú
'                    For intCol = gintEQ_StartCol To sprLResult.MaxCols
'                        If GET_CELL(sprLResult, intCol, -1000) = gtypPAT_RES.EQCD Then
'                            Call SET_CELL(sprLResult, intCol, intLResultTarRow, gtypPAT_RES.Result) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
'
'                            sprLResult.Col = intCol
'                            sprLResult.Row = intLResultTarRow
'
'                            If gtypPAT_RES.AFLAG = "L" Then
'                                sprLResult.BackColor = &HFFFF80
'                            End If
'                            If gtypPAT_RES.AFLAG = "H" Then
'                                sprLResult.BackColor = &HFFFF&
'                            End If
'                            If gtypPAT_RES.DFLAG = "D" Then
'                                sprLResult.BackColor = &HFF8080
'                            End If
'                            If gtypPAT_RES.PFLAG = "P" Then
'                                sprLResult.BackColor = &HFF&
'                            End If
'
'                            Exit For
'                        End If
'                    Next intCol
'
'                    '/ÇØ´ç Row·Î Focus ÀÌµ¿
'                    sprLResult.Col = 1
'                    sprLResult.Row = intLResultTarRow
'                    sprLResult.Action = ActionActiveCell
'
'                    If strEXSEQ <> "Y" Then
'                        gtypPAT_RES.EXSEQ = FUNC_GET_EXSEQ(gtypPAT_RES.BARCD) '/°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷
'                        strEXSEQ = "Y"
'                    End If
'
'                    Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.EXSEQ)
'                    '/¹ÞÀº ÀÚ·á Local ÀúÀå
'                    If FUNC_LOC_SAVE_PAT_RES = True Then
'                        If mnuJobModeAuto.Checked = True Then '/Àü¼Û¹æ½ÄÀÌ ÀÚµ¿Àü¼ÛÀÌ¸é...
'                            Call FUNC_HIS_SAVE '/HIS¿¡ °á°ú Àü¼Û
'                            Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS¿¡ °á°ú Àü¼Û
'                        End If
'                    End If
'                    '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
'                    Call SET_CELL(sprLResult, 10, intLResultTarRow, IIf(gtypPAT_RES.RCDT <> "", Format(gtypPAT_RES.RCDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.RCTM <> "", " " & Format(gtypPAT_RES.RCTM, "@@:@@:@@"), "")) '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
'                    Call SET_CELL(sprLResult, 7, intLResultTarRow, IIf(gtypPAT_RES.STATEFLAG = "1", "°á°ú", "Ã³¹æ"))
'                    Call SET_CELL(sprLResult, 8, intLResultTarRow, IIf(gtypPAT_RES.SENDFLAG = "1", "¿Ï·á", "´ë±â"))
'                    Call SET_CELL(sprLResult, 12, intLResultTarRow, gtypPAT_RES.ORDDT) '/ORDDT(Ã³¹æÀÏÀÚ)
'                    Select Case gtypPAT_RES.ORDGB '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
'                        Case "O": Call SET_CELL(sprLResult, 13, intLResultTarRow, "¿Ü·¡")
'                        Case "I": Call SET_CELL(sprLResult, 13, intLResultTarRow, "ÀÔ¿ø")
'                        Case "G": Call SET_CELL(sprLResult, 13, intLResultTarRow, "°ËÁø")
'                    End Select
'                'Else                                                                    '/Err ·Î±×°¡ ¸¸µé¾î Áø´Ù¸é Ã³¸®ÇØÁà¾ßÇÏ´Â ºÎºÐ
'
'                'End If
            Case Else               '/"GLU         NEGATIVE","URO      0.2 E.U./dL"
                
                
                gtypPAT_RES.EQCD = Trim(Mid(strDataBuff, 1, 3)) '/EQCD(Àåºñ°Ë»çÄÚµå)
                
                If FUNC_LOC_EQUIPCD_CHECK(gtypPAT_RES.EQCD) = False Then: GoTo NEXT_DATA  '/IF ¿¡ µî·ÏµÈ Àåºñ°Ë»çÄÚµå°¡ ¾Æ´Ñ°æ¿ì °á°ú ÀúÀåÇÏÁö ¾ÊÀ½
                
                
                If gtypPAT_RES.EQCD <> "URO" Then
                    gtypPAT_RES.EQRESULT = Trim(Mid(strDataBuff, 5)) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
                Else
                    gtypPAT_RES.EQRESULT = Trim(Mid(strDataBuff, 5, 9)) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
                End If
                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
                gtypPAT_RES.EXDT = Format(Now, "YYYYMMDD")
                gtypPAT_RES.EXTM = Format(Now, "HHMMSS")
                gtypPAT_RES.RCDT = Format(Now, "YYYYMMDD") '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
                gtypPAT_RES.RCTM = Format(Now, "HHMMSS") '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
                gtypPAT_RES.STATEFLAG = "1" '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
    
                Call FUNC_HIS_ORDER_VIEW    '/Ã³¹æ³»¿ª Á¶È¸
                'If gtypPAT_RES.EXAMCD <> "" Then
                    gtypPAT_RES.AFLAG = "" '/AFLAG(Abnormal(Á¤»óÂü°íÄ¡ ±âÁØ (H)High or (L)Low °ª Ç¥½Ã))
                    gtypPAT_RES.PFLAG = "" '/PFLAG(Panic)
                    gtypPAT_RES.DFLAG = "" '/DFLAG(Delta)
                    'Call FUNC_HIS_RESULT_JUDGMENT '/°á°ú ÆÇÁ¤
    
                    gtypPAT_RES.SENDFLAG = "0"
    
                    '/Àåºñ°Ë»çÇ×¸ñ Column Set/°á°ú
                    For intCol = gintEQ_StartCol To sprLResult.MaxCols
                        If GET_CELL(sprLResult, intCol, -1000) = gtypPAT_RES.EQCD Then
                            Call SET_CELL(sprLResult, intCol, intLResultTarRow, gtypPAT_RES.Result) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
    
                            sprLResult.Col = intCol
                            sprLResult.Row = intLResultTarRow
    
                            If gtypPAT_RES.AFLAG = "L" Then
                                sprLResult.BackColor = &HFFFF80
                            End If
                            If gtypPAT_RES.AFLAG = "H" Then
                                sprLResult.BackColor = &HFFFF&
                            End If
                            If gtypPAT_RES.DFLAG = "D" Then
                                sprLResult.BackColor = &HFF8080
                            End If
                            If gtypPAT_RES.PFLAG = "P" Then
                                sprLResult.BackColor = &HFF&
                            End If
    
                            Exit For
                        End If
                    Next intCol
    
                    '/ÇØ´ç Row·Î Focus ÀÌµ¿
                    sprLResult.Col = 1
                    sprLResult.Row = intLResultTarRow
                    sprLResult.Action = ActionActiveCell
    
                    If strEXSEQ <> "Y" Then
                        gtypPAT_RES.EXSEQ = FUNC_GET_EXSEQ(gtypPAT_RES.BARCD) '/°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷
                        strEXSEQ = "Y"
                    End If
    
                    Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.EXSEQ)
                    '/¹ÞÀº ÀÚ·á Local ÀúÀå
                    If FUNC_LOC_SAVE_PAT_RES = True Then
                        If mnuJobModeAuto.Checked = True Then '/Àü¼Û¹æ½ÄÀÌ ÀÚµ¿Àü¼ÛÀÌ¸é...
                            Call FUNC_HIS_SAVE '/HIS¿¡ °á°ú Àü¼Û
                            Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS¿¡ °á°ú Àü¼Û
                        End If
                    End If
                    '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
                    Call SET_CELL(sprLResult, 10, intLResultTarRow, IIf(gtypPAT_RES.RCDT <> "", Format(gtypPAT_RES.RCDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.RCTM <> "", " " & Format(gtypPAT_RES.RCTM, "@@:@@:@@"), "")) '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
                    Call SET_CELL(sprLResult, 7, intLResultTarRow, IIf(gtypPAT_RES.STATEFLAG = "1", "°á°ú", "Ã³¹æ"))
                    Call SET_CELL(sprLResult, 8, intLResultTarRow, IIf(gtypPAT_RES.SENDFLAG = "1", "¿Ï·á", "´ë±â"))
                    Call SET_CELL(sprLResult, 12, intLResultTarRow, gtypPAT_RES.ORDDT) '/ORDDT(Ã³¹æÀÏÀÚ)
                    Select Case gtypPAT_RES.ORDGB '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
                        Case "O": Call SET_CELL(sprLResult, 13, intLResultTarRow, "¿Ü·¡")
                        Case "I": Call SET_CELL(sprLResult, 13, intLResultTarRow, "ÀÔ¿ø")
                        Case "G": Call SET_CELL(sprLResult, 13, intLResultTarRow, "°ËÁø")
                    End Select
                'Else                                                                    '/Err ·Î±×°¡ ¸¸µé¾î Áø´Ù¸é Ã³¸®ÇØÁà¾ßÇÏ´Â ºÎºÐ
    
                'End If
            
            End Select

        
                
                
                
'        Select Case Mid(strPart(0), 2)
'            Case "H" '/HearderÁ¤º¸(°á°ú½ÅÈ£½ÃÀÛ)
'                '/Ã³¸®¾ÈÇÔ
'
'            Case "O" '/Á¤º¸(È¯ÀÚÁ¤º¸)
'                '/strPart(0): P
'                '/strPart(1): Test No(Barcode) YYDDMM-XXX1 ³¯Â¥º° SEQNO
'                '/strPart(2): ³¯Â¥½Ã°£(°Ë»ç½ÃÀÛÀÏ½Ã?, °á°úÀü¼ÛÀÏ½Ã?) YYYYMMDDHHMMSS
'                '/strPart(3): Patient ID (ÀÔ·ÂµÇÁö ¾ÊÀ¸¸é P ·Î Ç¥½ÃµÊ)
'                '/strPart(4): Device Name(ÄÚµå¸¦ ÀÐ°í Á¦Ç°¸íÀÌ Ãâ·ÂµÈ´Ù)
'                '/strPart(5): Lot No
'
'                gtypPAT_RES.BARCD = Trim(strPart(2)) '/BARCD(°ËÃ¼¹øÈ£(Barcode))
'                strLotNo = Split(strPart(3), "^")
'                gtypPAT_RES.SAMPLENO = strLotNo(0)
'
'                If gtypPAT_RES.BARCD = "" And gtypPAT_RES.SAMPLENO <> "" Then: gtypPAT_RES.BARCD = gtypPAT_RES.SAMPLENO
'
''                '/DISKNO Áß '@' °¡ ¼¯¿© ÀÖÀ»¶§ »©ÁÖ°í ÀúÀåÇÕ½Ã´Ù - È¿ÁØ -
''                If InStr(1, strLotNo(1), "@") > 0 Then strLotNo(1) = Mid(strLotNo(1), InStr(1, strLotNo(1), "@") + 1)
''
''                gtypPAT_RES.DISKNO = strLotNo(1) '/DISKNO(LotNo ÀÇ ¾ÕºÎºÐ)
''                gtypPAT_RES.POSNO = strLotNo(2) '/POSNO(LotNo ÀÇ µÞºÎºÐ)
'
'                '/HIS È¯ÀÚÁ¤º¸ °¡Á®¿À±â
'                gtypPAT_RES.PATNM = ""  '/PATNM(¼ö°ËÀÚ¸í)
'                gtypPAT_RES.PATSEX = "" '/PATSEX(¼ºº°)
'                gtypPAT_RES.PATAGE = "" '/PATAGE(¿¬·É)
'                Call FUNC_HIS_PATIENT
'                '/HIS È¯ÀÚÁ¤º¸ °¡Á®¿À±â
'
'                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
'                intLResultTarRow = 0
'                For intLResultRow = 1 To sprLResult.DataRowCnt
'                    If Trim(GET_CELL(sprLResult, 1, intLResultRow)) = gtypPAT_RES.BARCD And _
'                       Trim(GET_CELL(sprLResult, 2, intLResultRow)) = gtypPAT_RES.EXSEQ And _
'                       Trim(GET_CELL(sprLResult, 3, intLResultRow)) = gtypPAT_RES.SAMPLENO And _
'                       Trim(GET_CELL(sprLResult, 4, intLResultRow)) = gtypPAT_RES.DISKNO And _
'                       Trim(GET_CELL(sprLResult, 5, intLResultRow)) = gtypPAT_RES.POSNO Then
'
'                        intLResultTarRow = intLResultRow
'                        Exit For
'                    End If
'                Next intLResultRow
'
'                If intLResultTarRow = 0 Then
'                    sprLResult.MaxRows = sprLResult.MaxRows + 1
'                    intLResultTarRow = sprLResult.MaxRows
'
'                    Call SET_CELL(sprLResult, 1, intLResultTarRow, gtypPAT_RES.BARCD)
'                    Call SET_CELL(sprLResult, 2, intLResultTarRow, gtypPAT_RES.EXSEQ)
'                    Call SET_CELL(sprLResult, 3, intLResultTarRow, gtypPAT_RES.SAMPLENO)
'                    Call SET_CELL(sprLResult, 4, intLResultTarRow, gtypPAT_RES.DISKNO)
'                    Call SET_CELL(sprLResult, 5, intLResultTarRow, gtypPAT_RES.POSNO)
'                    Call SET_CELL(sprLResult, 8, intLResultTarRow, IIf(gtypPAT_RES.EXDT <> "", Format(gtypPAT_RES.EXDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.EXTM <> "", " " & Format(gtypPAT_RES.EXTM, "@@:@@:@@"), ""))
'                    Call SET_CELL(sprLResult, 13, intLResultTarRow, gtypPAT_RES.PATNO)
'                    Call SET_CELL(sprLResult, 14, intLResultTarRow, gtypPAT_RES.PATNM)
'                    If gtypPAT_RES.PATSEX <> "" Or gtypPAT_RES.PATAGE <> "" Then
'                        Call SET_CELL(sprLResult, 15, intLResultTarRow, gtypPAT_RES.PATSEX & "/" & gtypPAT_RES.PATAGE)
'                    End If
'                End If
'                '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
'
'
'
'            Case "Q" '/¿À´õ RequestÁ¤º¸
'
'                strLotNo = Split(strPart(2), "^")
'
'                gtypPAT_RES.BARCD = strLotNo(1)
'                gtypPAT_RES.SAMPLENO = strLotNo(2)
'
'                '/DISKNO Áß '@' °¡ ¼¯¿© ÀÖÀ»¶§ »©ÁÖ°í ÀúÀåÇÕ½Ã´Ù¿ë - È¿ÁØ -
'                If InStr(1, strLotNo(3), "@") > 0 Then strLotNo(3) = Mid(strLotNo(3), InStr(1, strLotNo(3), "@") + 1)
'
'                gtypPAT_RES.DISKNO = strLotNo(3)
'                gtypPAT_RES.POSNO = strLotNo(4)
'
'                Call FUNC_HIS_PATIENT
'
'                '/¿À´õ¸¸µé±â ºÎºÐÀÌ ÀÖ¾î¾ßÇÔ
'                Call FUNC_HIS_ORDER_MAKE(gtypPAT_RES.BARCD)
'
'            Case "R" '/°Ë»ç°á°úÁ¤º¸
'                strBuff = Split(strPart(2), "^")
'                gtypPAT_RES.EQCD = strBuff(3) '/EQCD(Àåºñ°Ë»çÄÚµå)
'                'blnEQCDCheck =
'                If FUNC_LOC_EQUIPCD_CHECK(gtypPAT_RES.EQCD) = False Then: GoTo NEXT_DATA  '/IF ¿¡ µî·ÏµÈ Àåºñ°Ë»çÄÚµå°¡ ¾Æ´Ñ°æ¿ì °á°ú ÀúÀåÇÏÁö ¾ÊÀ½
'
'                If InStr(strPart(3), "^") > 0 Then
'                    strBuff = Split(strPart(3), "^")
'                    gtypPAT_RES.EQRESULT = strBuff(1) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
'                Else
'                    gtypPAT_RES.EQRESULT = strPart(3) '/EQRESULT(Àåºñ¿ø½Ã°á°ú)
'                End If
'                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
'                gtypPAT_RES.EXDT = Mid(strPart(11), 1, 8)
'                gtypPAT_RES.EXTM = Mid(strPart(11), 9, 6)
'                gtypPAT_RES.RCDT = Format(Now, "YYYYMMDD") '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
'                gtypPAT_RES.RCTM = Format(Now, "HHMMSS") '/RCTM(°Ë»ç°á°ú¼ö½Å½Ã°£(24HHMMSS) ÀÇ·áÀåºñ ->HIEQ)
'                gtypPAT_RES.STATEFLAG = "1" '/STATEFLAG(°á°úÁøÇà»óÅÂ (0:Ã³¹æ, 1:°á°ú))
'
'                Call FUNC_HIS_ORDER_VIEW    '/Ã³¹æ³»¿ª Á¶È¸
'                If gtypPAT_RES.EXAMCD <> "" Then
'                    gtypPAT_RES.AFLAG = "" '/AFLAG(Abnormal(Á¤»óÂü°íÄ¡ ±âÁØ (H)High or (L)Low °ª Ç¥½Ã))
'                    gtypPAT_RES.PFLAG = "" '/PFLAG(Panic)
'                    gtypPAT_RES.DFLAG = "" '/DFLAG(Delta)
'                    Call FUNC_HIS_RESULT_JUDGMENT '/°á°ú ÆÇÁ¤
'
'                    gtypPAT_RES.SENDFLAG = "0"
'
'                    '/Àåºñ°Ë»çÇ×¸ñ Column Set/°á°ú
'                    For intCol = gintEQ_StartCol To sprLResult.MaxCols
'                        If GET_CELL(sprLResult, intCol, -1000) = gtypPAT_RES.EQCD Then
'                            Call SET_CELL(sprLResult, intCol, intLResultTarRow, gtypPAT_RES.Result) '/RESULT(°Ë»ç°á°ú(º¯ÇüµÈ °á°ú))
'
'                            sprLResult.Col = intCol
'                            sprLResult.Row = intLResultTarRow
'
'                            If gtypPAT_RES.AFLAG = "L" Then
'                                sprLResult.BackColor = &HFFFF&
'                            End If
'                            If gtypPAT_RES.AFLAG = "H" Then
'                                sprLResult.BackColor = &HFFFF80
'                            End If
'                            If gtypPAT_RES.DFLAG = "D" Then
'                                sprLResult.BackColor = &HFF8080
'                            End If
'                            If gtypPAT_RES.PFLAG = "P" Then
'                                sprLResult.BackColor = &HFF&
'                            End If
'
'                            Exit For
'                        End If
'                    Next intCol
'
'                    '/ÇØ´ç Row·Î Focus ÀÌµ¿
'                    sprLResult.Col = 1
'                    sprLResult.Row = intLResultTarRow
'                    sprLResult.Action = ActionActiveCell
'
'                    If strEXSEQ <> "Y" Then
'                        gtypPAT_RES.EXSEQ = FUNC_GET_EXSEQ(gtypPAT_RES.BARCD) '/°ËÃ¼¹øÈ£(Barcode)º° °Ë»çÈ¸Â÷
'                        strEXSEQ = "Y"
'                    End If
'
'                    Call SET_CELL(sprLResult, 2, intLResultTarRow, gtypPAT_RES.EXSEQ)
'                    '/¹ÞÀº ÀÚ·á Local ÀúÀå
'                    If FUNC_LOC_SAVE_PAT_RES = True Then
'                        If mnuJobModeAuto.Checked = True Then '/Àü¼Û¹æ½ÄÀÌ ÀÚµ¿Àü¼ÛÀÌ¸é...
'                            Call FUNC_HIS_SAVE '/HIS¿¡ °á°ú Àü¼Û
'                            Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS¿¡ °á°ú Àü¼Û
'                        End If
'                    End If
'                    '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® º¸¿©ÁÖ±â----------------------------------------------------------------------------------------------------/
'                    Call SET_CELL(sprLResult, 9, intLResultTarRow, IIf(gtypPAT_RES.RCDT <> "", Format(gtypPAT_RES.RCDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.RCTM <> "", " " & Format(gtypPAT_RES.RCTM, "@@:@@:@@"), "")) '/RCDT(°Ë»ç°á°ú¼ö½ÅÀÏÀÚ(YYYYMMDD) ÀÇ·áÀåºñ ->HIEQ)
'                    Call SET_CELL(sprLResult, 6, intLResultTarRow, IIf(gtypPAT_RES.STATEFLAG = "1", "°á°ú", "Ã³¹æ"))
'                    Call SET_CELL(sprLResult, 6, intLResultTarRow, IIf(gtypPAT_RES.SENDFLAG = "1", "¿Ï·á", "´ë±â"))
'                    Call SET_CELL(sprLResult, 11, intLResultTarRow, gtypPAT_RES.ORDDT) '/ORDDT(Ã³¹æÀÏÀÚ)
'                    Select Case gtypPAT_RES.ORDGB '/ORDGB(Ã³¹æÁ¾·ù(O.¿Ü·¡, I.ÀÔ¿ø, G.°Ç°­°ËÁø))
'                        Case "O": Call SET_CELL(sprLResult, 12, intLResultTarRow, "¿Ü·¡")
'                        Case "I": Call SET_CELL(sprLResult, 12, intLResultTarRow, "ÀÔ¿ø")
'                        Case "G": Call SET_CELL(sprLResult, 12, intLResultTarRow, "°ËÁø")
'                    End Select
'
'
'                Else                                                                    '/Err ·Î±×°¡ ¸¸µé¾î Áø´Ù¸é Ã³¸®ÇØÁà¾ßÇÏ´Â ºÎºÐ
'
'                End If
'            Case "L" '/Á¾·áÁ¤º¸

'        End Select
        
NEXT_DATA: '/´ÙÀ½ CASE ·Î µ¹¸±¶§ (ÁÁÀº¹æ½ÄÀÌ ÀÖÀ»¶§ ¹Ù²Ü¿¹Á¤)
    Next intDataCnt
End Sub

Private Sub MSComm2_OnComm()
    Dim strOneByte  As String
    
    
    strOneByte = MSComm2.Input

    Select Case strOneByte
        Case chrSTX
            strBarSignal = ""
        Case chrETX
            If Mid(strBarSignal, 1, 3) = "AD" Then
                strBarSignal = ""
            Else
                MSComm2.Output = chrACK
                strBarSignal = ""
            End If
        Case Else
            strBarSignal = strBarSignal & strOneByte
            
    End Select
End Sub

Private Sub sprLResult_LeaveRow(ByVal Row As Long, ByVal RowWasLast As Boolean, ByVal RowChanged As Boolean, ByVal AllCellsHaveData As Boolean, ByVal NewRow As Long, ByVal NewRowIsLast As Long, Cancel As Boolean)
    If Cancel = True Then Exit Sub  '/ÀÓÀÇ·Î CancelÀ» True·Î ¸¸µé¾î È£ÃâÇÒ °æ¿ì Ã³¸®ÇÏÁö ¾Ê°Ô ÇÒ ¼ö ÀÖ´Ù.
    If NewRow = Row Then Exit Sub   '/Row°¡ º¯µ¿¾øÀ» ¶§¿¡´Â Ã³¸®ÇÏÁö ¾Ê´Â´Ù.
    If NewRow < 1 Then Exit Sub     '/¼±ÅÃÇÑ Row°¡ À¯È¿ÇÑ ³»¿ëÀÌÁö ¾ÊÀ¸¸é Ã³¸®ÇÏÁö ¾Ê´Â´Ù.
    
    Dim intLResultCol   As Integer
    Dim intCnt          As Integer
    Dim strResult       As String
    
    Call SUB_MM_KEY_CLEAR("1") '/°ËÃ¼¹øÈ£º° ¼¼ºÎÁ¤º¸
            
    lblBARCD = GET_CELL(sprLResult, 1, NewRow)
    lblPATNM = GET_CELL(sprLResult, 2, NewRow)
    lblEXSEQ = GET_CELL(sprLResult, 3, NewRow)
    lblSAMPLENO = GET_CELL(sprLResult, 4, NewRow)
    lblDISKNOPOSNO = GET_CELL(sprLResult, 5, NewRow) & "/" & GET_CELL(sprLResult, 6, NewRow)
    
    lblEXDT = GET_CELL(sprLResult, 9, NewRow)
    lblRCDT = GET_CELL(sprLResult, 10, NewRow)
    lblSDDT = GET_CELL(sprLResult, 11, NewRow)
    
    lblORDDT = GET_CELL(sprLResult, 12, NewRow)
    lblORDGB = GET_CELL(sprLResult, 13, NewRow)
    
    lblPATNO = GET_CELL(sprLResult, 14, NewRow)
    lblSEXAGE = GET_CELL(sprLResult, 15, NewRow)
    
    
    
    Call SUB_MM_KEY_CLEAR("2") '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú
    
    For intLResultCol = gintEQ_StartCol To sprLResult.MaxCols
        '/ÀÐ±â----------------------------------------------------------------------------------------------------/
        sprLResult.Col = intLResultCol
        sprLResult.Row = NewRow         '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® °Ë»ç°á°ú Row
        strResult = sprLResult.Text     '/½Ç½Ã°£ °Ë»ç¸®½ºÆ® °Ë»ç°á°ú °ª
        '/ÀÐ±â----------------------------------------------------------------------------------------------------/

        '/¾²±â----------------------------------------------------------------------------------------------------/
        intCnt = intCnt + 1             '/½Ç°¡°£ °Ë»ç¸®½ºÆ® °Ë»çÇ×¸ñ ÀÐ±â Áõ°¡

        '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú Column
        Select Case intCnt
            Case 1 To 10:  sprDResult.Col = 2
            Case 11 To 20: sprDResult.Col = 5
            Case 21 To 30: sprDResult.Col = 8
            Case 31 To 40: sprDResult.Col = 11
        End Select

        '/°ËÃ¼¹øÈ£º° °Ë»ç°á°ú Row
        If (intCnt Mod 10) = 0 Then
            sprDResult.Row = 10
        Else
            sprDResult.Row = intCnt Mod 10
        End If

        sprDResult.Text = strResult
        '/¾²±â----------------------------------------------------------------------------------------------------/
    Next intLResultCol
End Sub

Private Sub staCondition_PanelClick(ByVal Panel As MSComctlLib.Panel)
    If Panel = "COM" Then
        If txtSerialData.Visible = False Then
            txtSerialData.Visible = True
        Else
            txtSerialData.Visible = False
        End If
    End If
End Sub

Private Sub txtBarcode_KeyDown(KeyCode As Integer, Shift As Integer)
    '/¹ÙÄÚµå¹øÈ£ º¸³»±â (Àåºñ/½Ç½Ã°£ °Ë»ç¸®½ºÆ®)
    If KeyCode = 13 Then
        If optOption(0).Value = True Then
            If Len(txtBarcode) = 12 Then
                MSComm2.Output = chrSTX & txtBarcode & chrETX
                txtBarcode = ""
            Else
                txtBarcode = ""
            End If
        Else
            If Len(Trim(txtBarcode)) = 12 Then
                For intY = 1 To sprLResult.DataRowCnt
                    If GET_CELL(sprLResult, 1, intY) = "No Barcode" Then
                        Call SET_CELL(sprLResult, 1, intY, Trim(txtBarcode))
                        Call NOVA_SAVE(GET_CELL(sprLResult, 1, intY), intY)
                        Exit For
                    End If
                Next intY
                txtBarcode = ""
            Else
                txtBarcode = ""
            End If
        End If
    End If
        
End Sub
