...
Private Sub cmdExit_Click()
    If frmEQ_워크리스트.Visible = True Then Unload frmEQ_워크리스트
...
Private Sub MSComm1_OnComm()
    ...
    Dim strBuff         As String
        ...
        Select Case strOneByte
            ...
            Case chrETX
                '/신호의 종류 판별 strOrderType = Q : Request 신호
                Call SUB_COMM_PART_K4500(strOneLine)
...
'                        'sCode = Left(sCode, 3)             '장비코드
'                        sResult = Mid(lsData, 1, 5)
'
'
'                        'sResult = Trim(Right(sResult, 6))       '결과
'        '/--------
...
'
'
'                        SetText vasRes, lsEquipCode, j, colEquipCode    '장비코드
...
Public Sub SUB_COMM_PART_K4500(argCOMM_BF As String)
    Dim strCrlf '/배열변수(CRLF 기준)
    Dim strPart '/배열변수(| 기준)
    Dim strBuff '/배열변수(바코드,DISKNO,POSNO)
...
'    For intX = 0 To UBound(strCrlf) - 1
    ...
    Select Case Mid(argCOMM_BF, 1, 1)
        ...
        Case "D" '/Result
            strEXSEQ = ""                           '/EXSEQ 매기는데 필요함
'            strCrlf(intX) = Mid(strCrlf(intX), 3)   '/구분신호만 빼고 DATA만 취급
          
            '/ "003305 1015                        "
            '/  R   P  S   B
'            gtypPAT_RES.DISKNO = Trim(Mid(argCOMM_BF, 1, 4)) '/DISKNO(LotNo 의 앞부분)
'            gtypPAT_RES.POSNO = Trim(Mid(argCOMM_BF, 5, 2)) '/POSNO(LotNo 의 뒷부분)
'            gtypPAT_RES.SAMPLENO = Trim(Mid(argCOMM_BF, 8, 4))
            
            '/장비에서 바코드번호가 안나와서 비움
            gtypPAT_RES.BARCD = ""  '/BARCD(검체번호(Barcode))
            'gtypPAT_RES.BARCD = Trim(Mid(argCOMM_BF, 23, 12))  '/BARCD(검체번호(Barcode))
            ...
            intLResultTarRow = 0
            For intLResultRow = 1 To sprLResult.DataRowCnt
                If Trim(GET_CELL(sprLResult, 1, intLResultRow)) <> "" And _
                   Trim(GET_CELL(sprLResult, 1, intLResultRow)) <> "No Barcode" And _
                   Trim(GET_CELL(sprLResult, 2, intLResultRow)) = "" Then
                   
                    intLResultTarRow = intLResultRow
                    gtypPAT_RES.BARCD = Trim(GET_CELL(sprLResult, 1, intLResultRow))
            ...
            strBuff = Mid(argCOMM_BF, 53)
            
            Do While Len(strBuff) / 9 > 1
'            Do While Len(strBuff) > 16
                '/한줄신호에서 자릿수로 끊어야 하기 때문에 먼저 초기화 시켜줌
                gtypPAT_RES.EXAMCD = ""
                
                gtypPAT_RES.EQCD = Trim(Mid(strBuff, 1, 3)) '/EQCD(장비검사코드)
                
                
                gtypPAT_RES.EQRESULT = Trim(Mid(strBuff, 4, 5)) '/EQRESULT(장비원시결과)
'                gtypPAT_RES.EQRESULT = Mid(gtypPAT_RES.EQRESULT, 1, Len(gtypPAT_RES.EQRESULT) - 1)
                gtypPAT_RES.EQRESULT = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT)  '/RESULT(검사결과(변형된 결과))
                ...
                'If gtypPAT_RES.EXAMCD <> "" Then
                    ...
                    If FUNC_LOC_SAVE_PAT_RES = True Then
                        If mnuJobModeAuto.Checked = True And gtypPAT_RES.BARCD <> "" And gtypPAT_RES.Result <> "" Then  '/전송방식이 자동전송이면...
                            Call FUNC_HIS_SAVE '/HIS에 결과 전송
                            Call FUNC_LOC_SAVE_SEND(gtypPAT_RES.BARCD, gtypPAT_RES.EXSEQ, gtypPAT_RES.EQCD, gtypPAT_RES.SAMPLENO, gtypPAT_RES.DISKNO, gtypPAT_RES.POSNO, "1") '/HIS에 결과 전송
                        End If
                    End If
                    '/실시간 검사리스트 보여주기----------------------------------------------------------------------------------------------------/
                    Call SET_CELL(sprLResult, 10, intLResultTarRow, IIf(gtypPAT_RES.RCDT <> "", Format(gtypPAT_RES.RCDT, "@@@@-@@-@@"), "") & IIf(gtypPAT_RES.RCTM <> "", " " & Format(gtypPAT_RES.RCTM, "@@:@@:@@"), "")) '/RCDT(검사결과수신일자(YYYYMMDD) 의료장비 ->HIEQ)
                    Call SET_CELL(sprLResult, 7, intLResultTarRow, IIf(gtypPAT_RES.STATEFLAG = "1", "결과", "처방"))
                    Call SET_CELL(sprLResult, 8, intLResultTarRow, IIf(gtypPAT_RES.SENDFLAG = "1", "완료", "대기"))
                    Call SET_CELL(sprLResult, 12, intLResultTarRow, gtypPAT_RES.ORDDT) '/ORDDT(처방일자)
                    Select Case gtypPAT_RES.ORDGB '/ORDGB(처방종류(O.외래, I.입원, G.건강검진))
                        Case "O": Call SET_CELL(sprLResult, 13, intLResultTarRow, "외래")
                        Case "I": Call SET_CELL(sprLResult, 13, intLResultTarRow, "입원")
                        Case "G": Call SET_CELL(sprLResult, 13, intLResultTarRow, "검진")
                    End Select
                    '/결과 처리하기-----------------------------------------------------------------------------------------------------------------/
                'End If
                strBuff = Mid(strBuff, 10)
            Loop
        Case "R " '/Inquery Order
                strCrlf(intX) = Mid(strCrlf(intX), 3)
                gtypPAT_RES.DISKNO = Trim(Mid(strCrlf(intX), 1, 4)) '/DISKNO(LotNo 의 앞부분)
                gtypPAT_RES.POSNO = Trim(Mid(strCrlf(intX), 5, 2)) '/POSNO(LotNo 의 뒷부분)
                gtypPAT_RES.SAMPLENO = Trim(Mid(strCrlf(intX), 8, 4))
                gtypPAT_RES.BARCD = Trim(Mid(strCrlf(intX), 12, 24))  '/BARCD(검체번호(Barcode))
    ...
    For intX = 0 To UBound(strCrlf) - 1
        ...
        Select Case Mid(strPart(0), 2)
            ...
            Case "O" '/정보(환자정보)
                ...
                gtypPAT_RES.BARCD = Trim(strPart(2)) '/BARCD(검체번호(Barcode))
                strLotNo = Split(strPart(3), "^")
                gtypPAT_RES.SAMPLENO = strLotNo(0)
                
                '/DISKNO 중 '@' 가 섞여 있을때 빼주고 저장합시다용 - 효준 -
                If InStr(1, strLotNo(1), "@") > 0 Then strLotNo(1) = Mid(strLotNo(1), InStr(1, strLotNo(1), "@") + 1)
                
                gtypPAT_RES.DISKNO = strLotNo(1) '/DISKNO(LotNo 의 앞부분)
                gtypPAT_RES.POSNO = strLotNo(2) '/POSNO(LotNo 의 뒷부분)
            ...
            Case "R" '/검사결과정보
                ...
                gtypPAT_RES.EQCD = strBuff(3) '/EQCD(장비검사코드)
                
                If InStr(strPart(3), "^") > 0 Then
                    strBuff = Split(strPart(3), "^")
                    gtypPAT_RES.EQRESULT = strBuff(1) '/EQRESULT(장비원시결과)
                Else
                    gtypPAT_RES.EQRESULT = strPart(3) '/EQRESULT(장비원시결과)
                End If
                gtypPAT_RES.Result = FUNC_RESULT_CHANGE(gtypPAT_RES.EQCD, gtypPAT_RES.EQRESULT) '/RESULT(검사결과(변형된 결과))
...
Private Sub sprLResult_LeaveCell(ByVal Col As Long, ByVal Row As Long, ByVal NewCol As Long, ByVal NewRow As Long, Cancel As Boolean)
