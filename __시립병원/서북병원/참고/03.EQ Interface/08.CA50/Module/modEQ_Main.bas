...
Public Function FUNC_RESULT_CHANGE(argECCD As String, argEQRESULT As String)
    ...
    If Not IsNumeric(argEQRESULT) Then
        FUNC_RESULT_CHANGE = argEQRESULT
        Exit Function
    End If
...
'    FUNC_RESULT_CHANGE = gtypPAT_RES.EQRESULT
    ...
    If iFloat = 0 Then
        FUNC_RESULT_CHANGE = CStr(CCur(argEQRESULT))
    Else
        If Len(argEQRESULT) <= iFloat Then
            FUNC_RESULT_CHANGE = "0." & argEQRESULT
        Else
            FUNC_RESULT_CHANGE = CStr(CCur(Left(argEQRESULT, i - iFloat)) & "." & Right(argEQRESULT, iFloat))
        End If
    End If
