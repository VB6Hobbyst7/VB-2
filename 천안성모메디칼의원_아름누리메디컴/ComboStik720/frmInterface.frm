   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":FF93
            UserResize      =   2
         End
...
'    Debug.Print gAllExam
    ...
    '-- 검사대상자 가져오기
    SQL = ""
    SQL = SQL & " SELECT Distinct a.ENTERDATE, b.WAITSEQNO, c.SUJINNAME, a.CHARTNO, a.SUJINPART, b.MAP2SEQNO " & vbLf
    ...
    SQL = SQL & "  ORDER BY a.ENTERDATE, a.CHARTNO "
    ...
    Do Until RS.EOF
        If strWAITSEQNO <> Trim(RS.Fields(1) & "") Then
            intRow = intRow + 1
            ...
            SetText vasWorkList, RS.Fields(1) & "", intRow, colBarcode
            'SetText vasWorkList, "", intRow, colRack
            'SetText vasWorkList, "", intRow, colPos
            SetText vasWorkList, Trim(RS.Fields(3)), intRow, colPID
...
'                strResult = strResult & " " & Trim(Mid(strTemp1, 12, 4)) '-- 정량
...
'                strResult = Replace(strResult, "WBC/ul", "")
                ...
                If strIntBase = "p.H" Or strIntBase = "S.G" Then
                    strResult = Trim(Mid(strTemp1, 11))  '-- 정량
                
                ElseIf strIntBase = "URO" Then
                    strResult = Mid(strTemp1, 8, 4) '-- 정성
                    
                    If UCase(Trim(strResult)) = "NORM" Then
                        strResult = "+-"
                    Else
                        Select Case Trim(strResult)
                            Case "-":       strResult = "음성"
                            Case "norm":    strResult = "음성"
                            Case "+":       strResult = "1+"
                            Case "++":       strResult = "2+"
                            Case "+++":       strResult = "3+"
                            Case "++++":       strResult = "4+"
                ...
                SetRawData "[" & intCnt & "]" & strIntBase & "," & strResult
...
Private Sub txtBarcode_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        If Len(Trim(txtBarcode.Text)) = 12 Then
            Call GetSampleInfoB(Trim(txtBarcode.Text))
            vasID.RowHeight(-1) = 12
        Else
            MsgBox "바코드 길이가 12자리가 아닙니다", vbInformation, "알림"
            txtBarcode.Text = ""
            txtBarcode.SetFocus
        End If
    End If
...
Private Sub vasID_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    SQL = SQL & " GROUP BY SEQNO, EQUIPCODE, EXAMCODE, EXAMNAME, EQUIPRESULT, RESULT, SENDFLAG " & vbCrLf
