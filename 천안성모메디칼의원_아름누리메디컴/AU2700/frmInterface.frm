...
Begin VB.Form frmInterface 
   Caption         =   " OLYMPUS AU2700 Interface "
   ClientHeight    =   10680
   ClientLeft      =   345
   ClientTop       =   840
   ClientWidth     =   25560
   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":8715
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":90F4
         End
...
'    End If
    ...
    blnWrite = False
    vasPrint.MaxRows = vasRID.MaxRows
    vasPrint.MaxCols = vasRID.MaxCols
...
'    Next iRow
    ...
    Screen.MousePointer = 11
          ...
          SQL = " SELECT EXAMDATE as 접수일자, BARCODE AS 바코드번호, PID as 환자번호, PNAME AS 이름, SEQNO,EXAMCODE, RESULT,SENDFLAG " & vbCrLf
    SQL = SQL & "   FROM PAT_RES " & vbCrLf
    SQL = SQL & "  WHERE MID(EXAMDATE,1,8) = '" & Format(dtpExamDate, "YYYYMMDD") & "'" & vbCrLf
    SQL = SQL & "    AND EQUIPNO = '" & gEquip & "' " & vbCrLf
    SQL = SQL & " ORDER BY EXAMDATE,BARCODE,SEQNO,EXAMCODE "
        ...
        Do Until rs.EOF
            With vasRID
                For i = 1 To .DataRowCnt
                    ...
                    If Format(Trim(rs.Fields("접수일자")) & "", "####-##-## ##:##:##") = strDate And "" & Trim(rs("바코드번호")) = strChart Then
                        blnSame = True
                    End If
                    
                    If blnSame = True Then
                        For intCol = 7 To vasRID.MaxCols
                            If Trim(rs.Fields("EXAMCODE")) = gArrEquip(intCol - 6, 3) Then
                                SetText vasRID, Trim(rs.Fields("RESULT")) & "", .MaxRows, intCol
                                Exit For
                            End If
                        Next
                    End If
                ...
                If blnSame = False Then
                    ...
                    SetText vasRID, "0", .MaxRows, 1
                    SetText vasRID, Format(Trim(rs.Fields("접수일자")) & "", "####-##-## ##:##:##"), .MaxRows, 2
                    SetText vasRID, "" & Trim(rs.Fields("바코드번호")) & "", .MaxRows, 3
                    SetText vasRID, Trim(rs.Fields("환자번호")) & "", .MaxRows, 4
                    SetText vasRID, Trim(rs.Fields("이름")) & "", .MaxRows, 5
                    
                    Select Case Trim(rs.Fields("SENDFLAG")) & ""
                        Case "0": SetText vasRID, "에러", .MaxRows, 6
                                  SetBackColor vasRID, .MaxRows, .MaxRows, 1, 6, 202, 201, 112
                        Case "1": SetText vasRID, "결과", .MaxRows, 6
                        Case "2": SetText vasRID, "완료", .MaxRows, 6
                                  SetBackColor vasRID, .MaxRows, .MaxRows, 1, 6, 202, 255, 112
                        Case "3": SetText vasRID, "수정", .MaxRows, 6
                                  SetBackColor vasRID, .MaxRows, .MaxRows, 1, 6, 202, 245, 112
                    End Select
    ...
    Call SpreadSheetSort(vasRID, 5, 1)
...
Private Sub SetExamCode()
    ...
    With vasRID
        ...
        For i = 0 To UBound(gArrEquip) - 1
            ...
            Call SetText(vasRID, gArrEquip(i + 1, 4), 0, 6 + (i + 1))
            .ColWidth(6 + (i + 1)) = 6
...
Private Sub GetOrder(ByVal pBarNo As String)
    ...
    If Trim(strItems) = "" Then
        ...
        SetRawData "[Tx]" & STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(12 - Len(mOrder.BarNo)) & mOrder.BarNo & mOrder.Seq & "    E" & ETX
    Else
        ...
        comEqp.Output = STX & "S " & mOrder.RackNo & mOrder.TubePos & Space(1) & mOrder.Seq & mOrder.BarNo & Space(4) & "E" & strItems & ETX
        SetRawData "[Tx]" & STX & "S " & mOrder.RackNo & mOrder.TubePos & Space(1) & mOrder.Seq & mOrder.BarNo & Space(4) & "E" & strItems & ETX
    End If
        ...
        Select Case strType
            ...
            Case "D "    '## Result
                ...
                Do While Len(strTmp) >= 10
                    ...
                    'RPR
                    If strIntBase = "37" Then
                        If IsNumeric(strResult) Then
                            If Mid(strResult, 1, 1) = "-" Or CCur(strResult) < 0 Then
                                strResult = "0.01"
                            End If
                        End If
                    End If
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    SQL = "DELETE FROM PAT_RES " & vbCrLf & _
          "WHERE mid(EXAMDATE,1,8) = '" & Format(Now, "YYYYMMDD") & "' " & vbCrLf & _
          "  AND EQUIPNO = '" & gEquip & "' " & vbCrLf & _
    ...
    SQL = SQL & "VALUES("
    SQL = SQL & "'" & Trim(Format(Now, "YYYYMMDDHHMMSS")) & "', "
    SQL = SQL & "'" & gEquip & "', "
...
Private Sub vasRID_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    If Row < 1 Or Row > vasRID.DataRowCnt Then
        If Col < 6 Then
            Call SpreadSheetSort(vasRID, Col, 1)
        End If
        Exit Sub
    End If
    ...
    lsID = Mid(lsID, 2)
    strExamDate = Trim(GetText(vasRID, Row, 2))
    lblChangeBar.Caption = lsID
    lblBarcode(1).Caption = lsID
    lblChangePID.Caption = Trim(GetText(vasRID, Row, 4))
    ...
    SQL = "SELECT EQUIPCODE,EXAMCODE,EXAMNAME,EQUIPRESULT,RESULT,SEQNO,REFFLAG " & vbCrLf & _
          ...
          " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf & _
          "   AND BARCODE = '" & lsID & "' " & vbCrLf & _
          "   AND EXAMDATE = '" & strExamDate & "' " & vbCrLf & _
