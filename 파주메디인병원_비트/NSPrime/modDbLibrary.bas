...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                SQL = SQL & "   And LABODRDTE = '" & strHospDate & "'" & vbCr
                ...
                If Res = 1 Then
                    ...
                    SQL = SQL & "   And DATODRDTE ='" & strHospDate & "'" & vbCr
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_SSHWORK(ByVal asRow As Long, ByVal asRow1 As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    ...
    GetSampleInfoW_SSHWORK = -1
    strORQN = ""
    
    
    sBarcode = Trim(GetText(frmInterface.vasWorklist, asRow, colCHARTNO))
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, "0", asRow1, colCheckBox
                SetText .vasID, Trim(RS.Fields("접수일자")) & "", asRow1, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("바코드번호")) & "", asRow1, colBARCODE
                SetText .vasID, Trim(RS.Fields("챠트번호")) & "", asRow1, colCHARTNO
                SetText .vasID, Trim(RS.Fields("내원번호")) & "", asRow1, colPID
                SetText .vasID, Trim(RS.Fields("이름")) & "", asRow1, colPNAME
                SetText .vasID, Trim(RS.Fields("검체명")) & "", asRow1, colSPCNM
                SetText .vasID, Trim(RS.Fields("SEQ")) & "", asRow1, colPAGE
               
                Select Case Trim(Trim(RS.Fields("입외")) & "")
                    Case "A":   SetText .vasID, "외래", asRow1, colINOUT
                    Case "F":   SetText .vasID, "입원", asRow1, colINOUT
                    Case Else:  SetText .vasID, "", asRow1, colINOUT
                End Select
