...
Begin VB.Form frmInterface 
   BorderStyle     =   0  '없음
   Caption         =   " Sysmex CA1500_2 Interface "
   ClientHeight    =   10680
   ClientLeft      =   330
   ClientTop       =   825
   ClientWidth     =   15165
...
Private Sub Command16_Click()
    ...
    '-- 오더
    strBuffer = "R2210101 110713121500001204     1558200020B               040      050      060      300      620      "
    '-- 결과
    strBuffer = "D1210101U110715120300007201     1563200010B               041  138 042  587 044  133 046  207 047   79 051  516 056  299 057   45 061   54 062  372 066  217 067   85 301  822 302   48 307  822 "
        
        
    strBuffer = "D1210101U110718144000004701     1116196031B               041  107 042  940 044  103 046  172 047   71 051  231 056  249 057   50 "

    strBuffer = "D1210101U110718144500004801     1563200010B               041  207 042  322 044  209 046  213 047   70 051 1104*056  262 057   39 061   58 062  357 066  246 067   91 301******302 ----*306***** 307    0 "

        
strBuffer = "R2210101 110727103100001501"
strBuffer = "     1572200060B"
strBuffer = "   040      050      060"
strBuffer = "  300      "
strBuffer = ""

strBuffer = "D1210101U110727103200001501"
strBuffer = strBuffer & "     1572200060B"
strBuffer = strBuffer & "   041  123 042  697 044  12"
strBuffer = strBuffer & "0 046  262 047   69 051  335"
strBuffer = strBuffer & " 056  466 057   60 "
        
strBuffer = "D1210101U110727103200001501     1572200060B               041  123 042  697 044  120 046  262 047   69 051  335 056  466 057   60 "
        
        
'-- QC
strBuffer = "D1210101U111013144600000302     9903000121M               041  331 042  176 044  338 046   88 047  145 051  469 056  131 057  105 "


strBuffer = "D1210101U111013153700000502     1120022681B               041  102 042 1098 044   96 046   62 047   92 051  242 056  113 057   55 "
        
strBuffer = "D1210101U111024151900000201     9903000111M               041  107 042  982 044  101 046   64 047  164 051  259 056  136 057  122 "
strBuffer = "D1210101U111024152000000202     9903000121M               041  330 042  177 044  337 046   90 047  140 051  480 056  142 057  113 "
        
strBuffer = "D1210101U111024145200000101     1120035521B               041  110 042  920 044  104 046  289 047  197 051  247 056  447 057  146         "
        
        
strBuffer = "D1210101U111104180400001003     1120023501B               051  627 056  376 057   43 "
        
strBuffer = ""
        
'-- Err = 999 결과 : *
strBuffer = "D1210101U111106044300006307     1120043531B               041******042 ----*044  ---*046***** 047    0 "

'-- Err = 32 결과 : NCD ==> *
strBuffer = "D1210101U111107140900002701     1120080361B               041******042 ----*044  ---*046    1 047   67 051******056    2 057   50 "
        
        
    
strBuffer = "R2210101 111108171200000701     1120121451B       040      050      060      300      620" & vbCrLf & ""


strBuffer = "D1210101U111109114200007505     1120147151B               041  198*042  334*044  195*046   22 047   52 051 1049*056   23 057   26 "
...
'    Open App.Path & "\log\long.log" For Input As #3
    Open App.Path & "\log\1111.log" For Input As #3
    ...
    strBuffer = "D1210101U111121095300002205     1120000241B               041   98 042 1206 044   92 046  219 047   49 051  278 056  336 057   49 R221010"
    'strBuffer = "1 111121100900002409     1120474511B               040      050      060      300      620      "
    
    strBuffer = "D1210101U111227083100000401     1121625821B               041  325 042  184 044  332 046  179 047   82 "

   ' strBuffer = "R2210101 111227083600000407     1121626321B               040      050      060      300      620      "
...
Private Sub GetOrder(ByVal pBarNo As String)
    ...
    If InStr(pBarNo, "ERR") > 0 Then
        strItems = STX & "S2210101U" & Space(6) & Space(4) & mOrder.RackNo & mOrder.TubePos & pBarNo & "B" & _
                 Space(15) & ETX
    ...
    strItems = GetEquipExamCode_CA1500(gEquip, pBarNo)

    mOrder.Order = strItems

EndLine:

    MSComm1.Output = strItems
    Save_Raw_Data "[Tx]" & strItems
...
'            Save_Raw_Data "[Tx]" & ACK
            ...
            strTubePos = Format$(Mid$(strRcvBuf, 26, 2), "#")
            ...
            For i = 1 To Len(strTemp) Step 9
                ...
                Select Case strType
                    Case "1"    '## Time
                        ...
                        If strFlag = "*" Or InStr(strResult, "*") > 0 Then
                            If strIntBase = "051" Then  '--APTT
                                If IsNumeric(strResult) Then
                                    If strResult > 110 Then
                                        strResult = "NCD"
                                        blnNCD = True
                                    End If
                                Else
                                ...
                                End If
                            End If
                            If strIntBase = "041" Then  '--PT(sec)
                                If IsNumeric(strResult) Then
                                    If strResult > 58 Then
                                        strResult = "NCD"
                                        blnNCD = True
                                    End If
                                Else
                    ...
                    Case "2"    '## Activity percent/concentration
                        If (strIntBase = "062" Or strIntBase = "302") Then
                            strResult = Trim$(Format$(strResult, "@@@@"))
                        Else
                            strResult = Trim$(Format$(strResult, "@@@.@"))
                        End If
                    ...
                    Case "3"    '## Ratio
                        ...
                        End If
                    Case "4"    '## INR
                        'strResult = Trim$(Format$(strResult, "@0.@@"))
                        strResult = Trim$(Format$(strResult, "@.@@"))
                ...
                If res > 0 Then
                    ...
                    SetText vasID, strTubePos, gRow, colPos                  'Pos
                    ...
                    SetText vasRes, strMResult, lsResRow, 8                     '실제결과
                                            
                    Save_Local_One gRow, lsResRow, "1", strMResult
            ...
            '## DB에 결과저장
            If strState = "R" Or Mid(mResult.BarNo, 1, 2) = "99" Then
                gOrderExam = ""
