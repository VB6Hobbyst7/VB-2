...
'''
'''            '///////////////////// 이전결과가 문자가 섞였을때
'''            If IsNumeric(strBefoRslt) = False Then
'''                Do
...
On Error GoTo Err
    ...
    lsID = ""
    lsID = Trim(GetText(frmInterface.vasID, argSpcRow, 4))
    lsSpecNo = Trim(GetText(frmInterface.vasID, argSpcRow, 3))
    lsPid = Trim(GetText(frmInterface.vasID, argSpcRow, 7))
    ...
    strViewRslt = Trim(Format(GetDateFull, "mm")) & "/" & Trim(Format(GetDateFull, "dd")) & "보고"
    ...
    SQL = " Select equipcode, examcode, result, EQUIPRESULT, refflag, panicflag, deltaflag, ifgbn,'" & Trim(GetText(frmInterface.vasID, argSpcRow, 6)) & "'" & vbCrLf & _
          " From pat_res " & vbCrLf & _
          " Where equipno = '" & gEquip & "' " & vbCrLf & _
          " And examdate = '" & Format(CDate(frmInterface.dtpToday.Value), "yyyymmdd") & "'  " & vbCrLf & _
          " And barcode = '" & Trim(GetText(frmInterface.vasID, argSpcRow, 4)) & "' "
          
          '& vbCrLf & _
          " And diskno = '" & Trim(GetText(frmInterface.vasID, argSpcRow, 5)) & "' " & vbCrLf & _
          " And posno = '" & Trim(GetText(frmInterface.vasID, argSpcRow, 6)) & "' "
    ...
    For iRow = 1 To frmInterface.vasTemp.DataRowCnt
        ...
        If iRow = 1 Then
            ...
            SQL = SQL & vbCrLf & "   AND EXMN_CD = '" & Trim(GetText(frmInterface.vasTemp, iRow, 9)) & "' "                      '검사코드"
            ...
            If res > 0 Then
                ...
                If IsNumeric(sResult) Then
                    ...
                    strDecision = GetDecision(argSpcRow, strBarcode, Trim(GetText(frmInterface.vasTemp, iRow, 9)), sResult)
                ...
                SQL = SQL & vbCrLf & "   SET REAL_RSLT = '0', "                   '결과(장비결과)
                SQL = SQL & vbCrLf & "       VIEW_RSLT = '" & strViewRslt & "', "                   '결과(수정결과)"
...
''    ClearSpread frmInterface.vasTemp
''
''    SQL = " Select equipcode, examcode, result, EQUIPRESULT, refflag, panicflag, deltaflag, '" & Trim(GetText(frmInterface.vasID, argSpcRow, 6)) & "'" & vbCrLf & _
''          " From pat_res " & vbCrLf & _
''          " Where equipno = '" & gEquip & "' " & vbCrLf & _
''          " And examdate = '" & Format(CDate(frmInterface.dtpToday.Value), "yyyymmdd") & "'  " & vbCrLf & _
...
''        SQL = "SELECT RSLT_NO FROM SPSLHRRST "
''        SQL = SQL & vbCrLf & " WHERE SPCM_NO = '" & lsSpecNo & "' "                                             '검체번호"
''        SQL = SQL & vbCrLf & "   AND EXMN_CD = '" & Trim(GetText(frmInterface.vasTemp, iRow, 8)) & "' "                      '검사코드"
''        SQL = SQL & vbCrLf & "   AND PID = '" & lsPid & "' "                                                    '환자번호"
''        SQL = SQL & vbCrLf & "   AND RSLT_STAT < '2' "                                                          '결과상태"
''        res = db_select_Col(gServer, SQL)
''        If res > 0 Then
...
''                strBarcode = Trim(GetText(frmInterface.vasID, argSpcRow, colBarcode))
''                'strDecision = GetDecision(argSpcRow, strBarcode, iRow)
''                strDecision = GetDecision(argSpcRow, strBarcode, Trim(GetText(frmInterface.vasTemp, iRow, 8)), sResult)
''                strDecision = Split(strDecision, "|")
''            Else
''                strDecision = "||"
''                strDecision = Split(strDecision, "|")
...
'    Set rs_svr = Nothing
    ...
    AdoCmd_ORACLE.CommandType = adCmdText
    AdoCmd_ORACLE.CommandText = SQL
    AdoCmd_ORACLE.Parameters.Append AdoCmd_ORACLE.CreateParameter("FN_LABCVTBCNO", adVarChar, , 10, Trim(argPID))
    Set AdoRs_ORACLE = New ADODB.Recordset
    AdoRs_ORACLE.Open AdoCmd_ORACLE, , adOpenStatic, adLockBatchOptimistic
    
    sExamCd = AdoRs_ORACLE.Fields(0) & ""
