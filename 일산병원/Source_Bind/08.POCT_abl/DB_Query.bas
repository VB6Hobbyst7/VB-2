    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult1 <> "" And Mid(sResult1, 1, 3) <> "-99" And Trim(GetText(.vasTemp, iRow, 2)) <> "" And lsSpecNo <> "" Then
                ...
                '/////////// 혈액학, 생화학 장비만 사용 ( 다른장비들은 결과 입력상태(= 1)로 함)
'                If Mid(Trim(GetText(.vasTemp, iRow, 2)), 1, 2) = "L8" Or CInt(CODE_L8_cnt) > 0 Then
'                    Send_State = "1"
'                Else
'                    SQL_LOCAL = ""
'                    SQL_LOCAL = SQL_LOCAL & vbCrLf & "SELECT COUNT(EXAMCODE) FROM PAT_RES "
'                    SQL_LOCAL = SQL_LOCAL & vbCrLf & " WHERE (PANICFLAG <> '' OR  DELTAFLAG <> '' ) "
...
'                End If
                ...
                Send_State = "3" '/  <---------- 혈액학장비가 아니라서 상태가 1로만 들어감
    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If Mid(sResult1, 1, 3) = "-99" Then: sResult1 = ""
                ...
                If Trim(GetText(.vasTemp, iRow, 1)) = "IFCC" Or Trim(GetText(.vasTemp, iRow, 1)) = "eAg" Or Trim(GetText(.vasTemp, iRow, 1)) = "pO2" Or Trim(GetText(.vasTemp, iRow, 1)) = "pCO2" Or Trim(GetText(.vasTemp, iRow, 1)) = "pH" Then
                Else
                    If sResult1 <> "" Then
                        ...
                        Else
                            ...
                            If Trim(GetText(.vasTemp, iRow, 2)) <> "" Then
                                ...
                                res = SendQuery(gServer, SQL)
                                Debug.Print Trim(GetText(.vasTemp, iRow, 2))
                                If res = -1 Then
                                    SaveQuery SQL
                                    cn_Ser.RollbackTrans
                                    Exit Function
...
Function RsltState_Check(asSpecNo As String, asExamCode As String) As String '/// 결과 형태 : (그룹코드/멀티코드) : 상태가 중간보고 이하일때
    ...
    i = 0
    Do Until rs_CHANGE.EOF
        
        If InStr(1, gState_G, rs_CHANGE.Fields(i) & "") = 0 Then
            PRSC_CD_G = rs_CHANGE.Fields(i) & ""
            Exit Do
        End If
    ...
    For i = 0 To UBound(sExamCode_arr)
        adoComm.Parameters.Append adoComm.CreateParameter("EXMN_CD", adVarChar, , 10, Trim(sExamCode_arr(i)))
    Next i
    adoComm.Parameters.Append adoComm.CreateParameter("CD_DVSN", adVarChar, , Len("M"), Trim("M"))
    'adoComm.Parameters.Append adoComm.CreateParameter("RSLT_STAT", adVarChar, , 1, Trim("2"))
    ...
    Do Until rs_CHANGE.EOF
        ...
        If InStr(1, gState_M, rs_CHANGE.Fields(i) & "") = 0 Then
            PRSC_CD_M = rs_CHANGE.Fields(i) & ""
            Exit Do
        End If
...
'    End If
    ...
    For i = 0 To UBound(sExamCode_arr)
        adoComm.Parameters.Append adoComm.CreateParameter("EXMN_CD", adVarChar, , 10, Trim(sExamCode_arr(i)))
    Next i
    adoComm.Parameters.Append adoComm.CreateParameter("CD_DVSN", adVarChar, , Len("B"), Trim("B"))
    'adoComm.Parameters.Append adoComm.CreateParameter("RSLT_STAT", adVarChar, , 1, Trim("2"))
    ...
    i = 0
    Do Until rs_CHANGE.EOF
        If InStr(1, gState_B, rs_CHANGE.Fields(i) & "") = 0 Then
            PRSC_CD_B = rs_CHANGE.Fields(i) & ""
            Exit Do
        End If
