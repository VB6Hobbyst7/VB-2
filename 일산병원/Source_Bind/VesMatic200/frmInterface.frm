...
Begin VB.Form frmInterface 
   BorderStyle     =   0  '없음
   Caption         =   " Ves Matic 200 Interface "
   ClientHeight    =   10680
   ClientLeft      =   330
   ClientTop       =   825
   ClientWidth     =   15165
   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Top             =   360
         Width           =   14625
         Begin VB.CommandButton cmdRecieve 
            Caption         =   "결과받기"
            BeginProperty Font 
               Name            =   "굴림체"
               Size            =   9.75
            ...
            Width           =   2175
         End
         Begin VB.CommandButton cmdIFIni 
            Caption         =   "통신초기화"
            BeginProperty Font 
               Name            =   "굴림체"
               Size            =   9
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            MoveActiveOnFocus=   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":7881
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            MaxCols         =   7
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":82C9
         End
...
Dim blnSTXV As Boolean
Dim blnMTX  As Boolean
Dim blnCR   As Boolean
...
Private Sub cmdIFIni_Click()
'
'    '-- 신호끊김 방지???
'    MSComm1.Output = ACK & Chr(30) & Chr(31) & vbCr
'    Save_Raw_Data "[Mx]" & ACK & Chr(30) & Chr(31) & vbCr
    ...
    '마지막 결과 요구
    sRequest = ""
    sRequest = ">0002018300" & chrCR & "00"
    
    MSComm1.Output = sRequest
    Save_Raw_Data "[Tx]" & sRequest
...
strBuffer = strBuffer & ""
strBuffer = strBuffer & ">002301510111169117710908111805  28001290A5" & vbCr
strBuffer = strBuffer & "50"
strBuffer = ""
strBuffer = strBuffer & ">002301510111169090010908111700 118001290A3" & vbCr
strBuffer = strBuffer & "47"
'strBuffer = strBuffer & ">002301510111169098510908111659   5001290A1" & vbCr
'strBuffer = strBuffer & "48"


strBuffer = ">001801500211169410811116940701" & vbCr & "32"
strBuffer = ">002301510111200032010411110948  90001285B3" & vbCr & "53"
strBuffer = ">002301510111200151410411111038  75001285B6" & vbCr & "5C"
strBuffer = ">002301510111200222110411111734  10001285B5" & vbCr & "5B"
strBuffer = ">002301510111200222110411111734  10001285B5" & vbCr & "5B"
strBuffer = ">002301510111200397110511111904>140001285A4" & vbCr & "50"

strBuffer = ">002301510111201155010811111536   1001285A8" & vbCr & "4A"

strBuffer = ">0023015003112011607111201161111120115761" & vbCr & "25"

strBuffer = ">002301510111201149310811111454  55001285A3" & vbCr & "5B"

strBuffer = ">000D0150011120115501" & vbCr & "25"

strBuffer = ">002301510111201155010811111536   1001285A8" & vbCr & "25"

strBuffer = ">001B015101EVX010000811110916   000" & vbCr & "38"

strBuffer = ">000D01500111201381815" & vbCr

strBuffer = ">000D01500111201396015"

strBuffer = ">007001500A1120144431112013818111201396011120135411112013532111201394811120145191112013490111201448611120134781" & vbCr & "4B"

'-- Request
strBuffer = ">     0       0       1       8       0       1       5       0       0       2       1116941081              1116940701" & vbCr & "3       2"
'            STX & H-BLK & L-BLK & H-LEN & L-LEN & H-ADD & L-ADD & H-COM & L-COM &                 Data-1 &                 Data-n &       ETX &   H-CHK & L-CHK
'                                                                          Data-1: H-NUM & L-NUM & Barcode-1 & Terminator & Barcode-n & Terminator
            ...
            For i = 1 To lngBufLen
                ...
                Select Case BufChar
                    Case STXV
                        ...
                        strRecvData = strRecvData & BufChar
                    ...
                    Case Else
                        ...
                        If blnSTXV = True And blnCR = True Then
                            sEtxtoLen = sEtxtoLen + 1
                            Save_Raw_Data "[TO]" & sEtxtoLen
                            If sEtxtoLen = 2 Then
                                Save_Raw_Data "[TX]" & strRecvData
                                Call EditRcvData
...
End Sub
'
'Function VesMatic20(asData As String) As Integer
'    Dim sRack As String
'    Dim sPos As String
'    Dim sFlag As String
...
'    Dim sResult As String       '수치결과
'    Dim sResult1 As String      '문자결과
...
'    Dim iPos As Integer
'
'    Dim iBuffCnt As Integer
'
'    VesMatic20 = 0
'
'    If asData = "" Then Exit Function
'
'    'STX(>)와 ETX(CR)가 없으면 에러
'    If InStr(1, asData, ">") < 1 Then Exit Function
'
'    '결과인지 확인
'    If Mid(asData, 8, 2) <> "03" Then Exit Function
'
'    If Mid(asData, 2, 2) = "00" Then    '결과 첫번째 블럭(헤더 20바이트)
'        asData = Mid(asData, 10)   'COM 앞 잘라냄
'        giExamCnt = Asc(Mid(asData, 3, 1))
'
'        '결과 갯수
''        lblCnt.Caption = giExamCnt
'
'        sExamDate = Mid(asData, 6, 15) '05/11/200717:33 검사결과일시
'        sExamDate = Mid(sExamDate, 7, 4) & "/" & Mid(sExamDate, 4, 2) & "/" & Left(sExamDate, 2) & " " & Mid(sExamDate, 11, 5) & ":00"
'        If IsDate(gsDateTime) Then
'            If DateDiff("N", gsDateTime, sExamDate) > 0 Then    '같은 검사인 경우 무시
'                gsDateTime = sExamDate
'
'                WritePrivateProfileString "config", "EXAMDATE", gsDateTime, App.Path & "\Interface.ini"
'
'            Else    '같은 검사결과
'                giExamCnt = 0
'                gsData = ""
...
'            WritePrivateProfileString "config", "EXAMDATE", gsDateTime, App.Path & "\Interface.ini"
'        End If
'        asData = Mid(asData, 21)    'Header 자르고 데이타만
'        iBuffCnt = 30
'    Else    '결과 추가 블럭(헤더 없음)
'        If giExamCnt = 0 Then
'            gsData = ""
...
'            gsDataCnt = 0
'        End If
'        asData = Mid(asData, 10)   'COM 앞 자르고 데이타만
'        iBuffCnt = 10
'    End If
'
'
'    gsData = gsData & asData '결과만 담기
'
'    i = UBound(gsDataB)
'    For j = iBuffCnt To UBound(gsBuffB) - 3
'        i = i + 1
'        ReDim Preserve gsDataB(i)
'        gsDataB(i) = gsBuffB(j)
'    Next j
'
'    Do While Len(gsData) >= 40
'        sPos = Asc(Left(gsData, 1))
'        sFlag = Asc(Mid(gsData, 2, 1))
'        sBarCode = Trim(Mid(gsData, 3, 13))
'        sResult = Asc(Mid(gsData, 16, 1))
'
'        sResult = gsDataB(gsDataCnt + 16)
'
'        glRow = glRow + 1
'        gsDataCnt = gsDataCnt + 40
'
'        Connect_Server  '서버에 접속
'
'        sExamCode = ""
'        sSubcode = ""
'
'        SQL = " Select Coda, SubCoda From LaLaRi " & CR & _
'              " Where Lid = " & Trim(GetText(vasID, glRow, colBarcode)) & " " & CR & _
'              " And HCode = " & Trim(GetText(vasID, glRow, colPID)) & " " & CR & _
'              " And Coda IN (" & gAllExam & ") "
...
''                If sExamCode = Trim(gArrEquip(i, 2)) Then
''                    lCol = i
''                    Exit For
''                End If
''            Next i
...
'
'        'lCol = 1
'        vasID.SetText colPos, glRow, sPos
'
'        'SetText vasID, sResult, glRow, lCol + colResult - 1
'        vasID.SetText colResult, glRow, sResult
'        vasID.SetText colExamCode, glRow, sExamCode
'        vasID.SetText colSubCode, glRow, sSubcode
'        vasID.SetText colExamDate, glRow, gsDateTime
'
'        SetText vasID, "수신완료", glRow, colState
'
'        '로컬에 저장하기
'        Save_Local_One_1 glRow, lCol, "A"
'
'        giExamCnt = giExamCnt - 1
'
'        gsData = Mid(gsData, 41)
'    Loop
'
'    VesMatic20 = giExamCnt
'
'    If giExamCnt = 0 Then
'        gsData = ""
'        Timer1.Enabled = True
...
Private Sub EditRcvData()
    ...
    strRcvBuf = strRecvData
    strDeviceID = Mid$(strRcvBuf, 3, 2)

    strIntBase = "ESR"
    
    Dim intBarcnt As Integer
    Dim intBar    As Integer
    Dim strOrdRslt As String
    
    'STX(>)와 ETX(CR)가 없으면 에러
    If InStr(1, strRcvBuf, ">") < 1 Then Exit Sub
    
    If Trim(strRcvBuf) <> "" Then
        strOrdRslt = Mid(strRcvBuf, 7, 3)   '150:오더,151:결과
        
        '-- 오더
        If strOrdRslt = "150" Then
            MSComm1.Output = STX & "01" & vbCr
            Save_Raw_Data "[Rx]" & STX & "01" & vbCr
            ...
            MSComm1.Output = strRecvData
            Save_Raw_Data "[Rx]" & strRecvData
        ...
        Else
            ...
            strBarno = Mid$(strRcvBuf, 12, 10)
            If InStr(strBarno, MTX) > 0 Then
                strBarno = Mid(strBarno, 1, InStr(strBarno, MTX) - 1)
            End If
            If IsNumeric(strBarno) Then
                mResult.BarNo = strBarno
                strResult = Trim(Mid$(strRcvBuf, InStr(strRcvBuf, MTX) + 11, 4))
                If strResult = "0" Then strResult = "*"
                Call SetPatInfo(strBarno)
    ...
    If strResult <> "" Then
        ...
        If res > 0 Then
            ...
            SetText vasID, strRackNo, gRow, colRack                  'RackNo
...
'    MSComm1.Output = STX & strDeviceID & vbCr
