...
Function Insert_Data_QC(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        SQL = SQL & vbCrLf & " WHERE eqpm_cd in ('055','056','057') "   'VISTA Cardiac,HCG,ASO/RF QC
...
''                strBarcode = Mid(strBarcode, 1, 10)
'''                strDecision = GetDecision(argSpcRow, strBarcode, iRow)
''                strDecision = GetDecision(argSpcRow, strBarcode, Trim(GetText(frmInterface.vasTemp, iRow, 2)), sResult)
''                strDecision = Split(strDecision, "|")
''            Else
...
'    sExamCode = ""
    ...
    For i = 0 To UBound(gReadBuf)
        ...
        If gReadBuf(i) <> "" Then
            ...
            Select Case gReadBuf(i)
            Case "MMB":     strChannel = "0317^^" & Trim(gReadBuf(i))
            Case "CTNI":    strChannel = "2021^^" & Trim(gReadBuf(i))
            Case "PBNPM":   strChannel = "4015^^" & Trim(gReadBuf(i))
            Case "BHCG":    strChannel = "0802^^" & Trim(gReadBuf(i))
            Case "ASL":     strChannel = "0126^^" & Trim(gReadBuf(i))
            Case "RF":      strChannel = "1802^^" & Trim(gReadBuf(i))
            End Select
            
            If Trim(gReadBuf(i)) <> "990" Then
                strExamCode = strExamCode & "\^^^" & strChannel 'Trim(gReadBuf(i))
            End If
        Else
            Exit For
        End If
