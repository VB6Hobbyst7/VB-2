...
'검사항목 조회
    ...
    SQL = "SELECT equipcode, examcode, examname, resprec, seqno, testno, reflow, refhigh " & CR & _
          "  From equipexam " & CR & _
          " WHERE equipno = '" & gEquip & "' " & CR & _
          " group by examcode, equipcode, examname, resprec, seqno, testno, reflow, refhigh "
...
'    db_BeginTran gLocal
    ...
    If res = 1 Then
        SQL = "Update equipexam " & CR & _
              ...
              "    testno = '" & Trim(txtTestNo) & "', " & vbCrLf & _
              ...
              "  and examcode = '" & Trim(txtCode) & "' "
    ElseIf res = 0 Then
        SQL = "Insert Into equipexam (equipno,equipcode, examcode, examname, resprec, seqno, testno, reflow, refhigh) " & CR & _
              "Values ('" & gEquip & "', '" & Trim(txtEquipCode) & "', '" & Trim(txtCode) & "', '" & Trim(txtName.Text) & "', '" & Trim(txtDec) & "', " & liSeqNo & ", '" & Trim(txtTestNo) & "', '" & Trim(txtRefLow) & "', '" & Trim(txtRefHigh) & "') "
    End If
...
Private Sub vasList_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    txtTestNo = Trim(GetText(vasList, Row, 6))
