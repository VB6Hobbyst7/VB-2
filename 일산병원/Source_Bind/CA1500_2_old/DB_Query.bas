...
Function Insert_Data_QC(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        SQL = SQL & vbCrLf & " WHERE eqpm_cd = '031'"   'CA1500_2 QC
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult1 <> "" Then
                ...
                If IsNumeric(sCnt) = True Then
                    ...
                    SQL = SQL & vbCrLf & "  SET RSLT_VALU = '" & sResult2 & "', "                        '결과(수정결과)
                ...
                Else
                    ...
                    For k = 0 To UBound(varQcCode) - 1
                        If Trim(GetText(.vasTemp, iRow, 2)) <> "" And Trim(GetText(.vasTemp, iRow, 2)) = varQcCode(k) Then
                            ...
                            SQL = SQL & vbCrLf & "                      '" & sResult2 & "', '" & lsID & "', 'N', "
...
''    Set rs_DPRef = cn_Ser.Execute(SQL)
    ...
    '            If IsNumeric(strDestRslt) And IsNumeric(strBefoRslt) = True Then
                If IsNumeric(strDestRslt) And IsNumeric(strBefoRslt) = True And strBefoRslt > 0 And strDestRslt > 0 Then
                    strDelta = ""
...
Function Insert_Data(ByVal argSpcRow As Integer) As Integer
    ...
    Dim strWhereSQL     As String
    Dim strWhereSQL_G     As String
    Dim strWhereSQL_M     As String
    Dim strWhereSQL_B     As String
...
'''                End If
                If State_G <> "" Then
                    strWhereSQL_G = " EXMN_CD = ? "
                Else
                    strWhereSQL_G = ""
                End If
                If State_M <> "" Then
                    strWhereSQL_M = " EXMN_CD = ? "
                Else
                    strWhereSQL_M = ""
                End If
                If State_B <> "" Then
                    strWhereSQL_B = " EXMN_CD = ? "
                Else
                    strWhereSQL_B = ""
                End If
                
                If strWhereSQL_G <> "" Or strWhereSQL_M <> "" Or strWhereSQL_B <> "" Then
                    strWhereSQL = " AND ("
                End If
                
                If strWhereSQL_G <> "" Then
                    strWhereSQL = strWhereSQL & strWhereSQL_G & " OR "
                End If
                If strWhereSQL_M <> "" Then
                    strWhereSQL = strWhereSQL & strWhereSQL_M & " OR "
                End If
                If strWhereSQL_B <> "" Then
                    strWhereSQL = strWhereSQL & strWhereSQL_B & " OR "
                End If
                
                If strWhereSQL_G <> "" Or strWhereSQL_M <> "" Or strWhereSQL_B <> "" Then
                    strWhereSQL = Mid(strWhereSQL, 1, Len(strWhereSQL) - 3) & " )"
                End If
                ...
                If Send_State = "1" Then
                    ...
                    End If
                    If State_B <> "" Then
                        AdoCmd_ORACLE.Parameters.Append AdoCmd_ORACLE.CreateParameter("EXMN_CD", adVarChar, , 5, Trim(State_B))
                    End If
                ElseIf Send_State = "2" Then
