...
Begin VB.Form frmInterface 
   ...
   Begin FPSpreadADO.fpSpread spdResult 
      ...
      ShadowColor     =   14737632
      ShadowDark      =   12632256
      SpreadDesigner  =   "frmInterface.frx":CC62
      VisibleCols     =   3
      VisibleRows     =   10
      CellNoteIndicatorColor=   16576
...
Private Sub ReceiveProcess()
    ...
    If gComm.COMTYPE = "1" Then
        ...
        Select Case UCase(gHOSP.MACHNM)
            Case "ABL90":           Call Phase_Serial_ABL90
    ...
    ElseIf gComm.COMTYPE = "2" Then
        Select Case UCase(gHOSP.MACHNM)
            'Case "AQT90":           Call Phase_TCP_AQT90_ASTM
            Case "AQT90":           Call TCPRcvData_AQT90
    ...
    With frmInterface
        For intCnt = 0 To UBound(strRecvData)
            ...
            Select Case strType
                ...
                Case "QRD"
                    'MSH|^~\&|AQT90 FLEX^AQT90 FLEX|AQT90 FLEX^AQT90 FLEX|||20210413152509||QRY^A19|3|P|2.5|||AL|NE||
                    'QRD||R|I|1|||1^RD|1778614|DEM

                    strBarno = mGetP(strRcvBuf, 9, "|")
                    With mOrder
                        .BarNo = strBarno
                    End With
                    
                    Call GetOrder_AQT90(strBarno, gHOSP.RSTTYPE)
                    
                Case "QRF"
                    
                Case "PID"
                    'MSH|^~\&|AQT90 FLEX^AQT90 FLEX|AQT90 FLEX^AQT90 FLEX|||20210413151735||ORU^R31|1|P|2.5|||AL|NE||
                    'PID|1||1778638||^||||||||||||||
                    'PV1|1|U|||||||||||||||^||||||||||||||||||||||||||||
                    'ORC|NW|||||||||||||||
                    'OBR|1|||^|||||||O||||SDT_PATIENTSAMPLE^^^LLFA^^^P|^||||||20210413142628|||F|||||||||Anonymous||
                    'OBX|1|ST|^^^TnI^M||<0.010|ng/mL||N|||F|||20210413142628||^Anonymous||I393-838R0570N0014^^|20210413142628
                    'OBX|2|ST|^^^CKMB^M||2.0|ng/mL||N|||F|||20210413142628||^||I393-838R0570N0014^^|20210413142628
                ...
                Case "OBX"
                    strIntBase = mGetP(mGetP(strRcvBuf, 4, "|"), 4, "^")
...
Private Sub GetOrder_AQT90(ByVal pBarno As String, ByVal pType As String) ', _
                               ByVal pMaker As String, ByVal pMchNm As String, _
                               ByVal pModel As String, ByVal pSTime As String, _
                               ByVal pETime As String, ByVal pQryId As String)
    ...
    With frmInterface
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = ""
        strItems = GetEquipExamCode_AQT90(gHOSP.MACHCD, pBarno, intRow)
        ...
        Else
            ...
            '-- 오더문자열만들기
                      strSend = SB & "MSH|^~\&|RMED-X03RLDT825^|RMED-X03RLDT825^|||" & Format(Now, "yyyymmddhhmmss") & "||ADR^A19|1|P|2.5|||AL|SU||" & vbCr
            strSend = strSend & "MSA||1" & vbCr
            strSend = strSend & "QRD||R|I|1|||1^RD|" & mOrder.BarNo & "|DEM" & vbCr
            strSend = strSend & "PID|" & mOrder.BarNo & "||" & mOrder.BarNo & "||||" & vbCr
            strSend = strSend & "OBR|1|" & mOrder.PID & "||" & strItems & "^|||||||||" & vbCr
            strSend = strSend & EB & vbCr
...
Private Sub GetOrder_AQT90_ASTM(ByVal pBarno As String, ByVal pType As String) ', _
                               ByVal pMaker As String, ByVal pMchNm As String, _
                               ByVal pModel As String, ByVal pSTime As String, _
                               ByVal pETime As String, ByVal pQryId As String)
    ...
    With frmInterface
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = ""
        strItems = GetEquipExamCode_AQT90_ASTM(gHOSP.MACHCD, pBarno, intRow)
...
'strSend = ""
'strSend = strSend & SB & "MSH|^~\&|LIS^LIS|LIS^LIS|||20061208141100||ADR^A19|7|P|2.5|||AL|NE|US|8859/1"
'strSend = strSend & "MSA|AA|5|||"
'strSend = strSend & "QRD||R|I|1|||||1^RD|556677|DEM|"
'strSend = strSend & "PID|1||556677||Hansen^Peter||19540221|M||||||||||0|"
'strSend = strSend & "PV1|1||ICU||||||||||||||01^Petersen|||||||||||||||||||||||||||20061205|||||||||"


'H|\^&|||AQT90 FLEX^AQT90 FLEX||||||||1|20210413171011
'P|1||1778690||^|||||||||^|^^^|^|^||||||||
'O|1||Sample #^5|^^^||||||Anonymous|||||SDT_PATIENTSAMPLE^||||||||||F
'R|1|^^^CKMB^M|5.8|ng/mL|^^^|N||F||Anonymous|20210413160611|20210413160611
'L|1|N
'
...
'            strOutput = intFrameNo & "L|1|N" & vbCr & ETX
            ...
            '-- 오더문자열만들기
                      strSend = "H|\^&|||LIS^||||||||1|" & Format(Now, "yyyymmddhhmmss") & vbCr
            strSend = strSend & "P|1||" & mOrder.BarNo & "||||||||||||||||||||||" & vbCr
            'strSend = strSend & "O|1|" & mOrder.PID & "||" & strItems & "||||||||||||" & vbCr  '|||||||||||||||
            strSend = strSend & "O|1|" & mOrder.PID & "|" & mOrder.BarNo & "|" & strItems & "||" & Format(Now, "yyyymmddhhmmss") & "|||||||||||||||||" & vbCr  '|||||||||||||||
            strSend = strSend & "L|1|N" & vbCr & ""
        ...
        Else
            ...
            ElseIf strResType = 2 Then
                ...
                If IsNumeric(pIntResult) Then
                    ...
                    If Trim(RS_L.Fields("AMRLIMIT1")) & "" <> "" Then
                        If CCur(pIntResult) < CCur(Trim(RS_L.Fields("AMRLIMIT1"))) Then
                            strResult = Trim(RS_L.Fields("AMRRESULT1"))
                            blnChange = True
                        End If
                    End If
                    If Trim(RS_L.Fields("AMRLIMIT2")) & "" <> "" Then
                        If CCur(pIntResult) <= CCur(Trim(RS_L.Fields("AMRLIMIT2"))) Then
                            strResult = Trim(RS_L.Fields("AMRRESULT2"))
                            blnChange = True
                        End If
                    End If
                    If Trim(RS_L.Fields("AMRLIMIT3")) & "" <> "" Then
                        If CCur(pIntResult) > CCur(Trim(RS_L.Fields("AMRLIMIT3"))) Then
                            strResult = Trim(RS_L.Fields("AMRRESULT3"))
                            blnChange = True
                        End If
                    End If
                    If Trim(RS_L.Fields("AMRLIMIT4")) & "" <> "" Then
                        If CCur(pIntResult) >= CCur(Trim(RS_L.Fields("AMRLIMIT4"))) Then
                            strResult = Trim(RS_L.Fields("AMRRESULT4"))
                            blnChange = True
                        End If
                    End If
                                        
                    If blnChange = False Then
                        strChrResult = Trim(RS_L.Fields("AMRRESULT20"))
                    End If
...
Private Sub SerialRcvData_AQT90()
    '장비 수신 변수
    Dim strRcvBuf       As String   '수신한 Data
    Dim strType         As String   '수신한 Record Type
    Dim strBarno        As String   '수신한 바코드번호
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                Case "Q"    '## Request Information
                    ...
                    Call GetOrder_AQT90_ASTM(strBarno, gHOSP.RSTTYPE)
            ...
            Select Case strType
                ...
                Case "O"
                    ...
                    For j = 1 To spdOrder.DataRowCnt
                        If GetText(spdOrder, j, colBARCODE) = strBarno Then
                            blnSame = True
                            
                            '-- 결과정보
                            With mResult
                                .BarNo = strBarno
                                .RsltDate = GetText(spdOrder, j, colEXAMDATE)
                                .RsltTime = GetText(spdOrder, j, colEXAMTIME)
                                .RsltSeq = GetText(spdOrder, j, colSAVESEQ)
                            End With
...
Private Sub SerialRcvData_ABL90()
    '장비 수신 변수
    Dim strRcvBuf       As String   '수신한 Data
    Dim strType         As String   '수신한 Record Type
    Dim strBarno        As String   '수신한 바코드번호
...
Private Sub Phase_TCP_AQT90_ASTM()
...
Private Sub Phase_Serial_ABL90()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
