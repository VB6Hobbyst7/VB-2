...
On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        Select Case strOrdNm
            ...
            Case "COMMON":      strOrdCd = gAssayNM.COMMON_CD
        End Select
...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If lsID <> "" And strRegDate <> "" And sResult <> "" Then
                ...
                'cn_Ser.Execute "Exec Interface_SetPatientResult '" & strRegDate & "','" & lsPid & "','" & Trim$(strOrdCd) & "','" & Trim$(strEqpCd) & "','','" & sResult & "','','',0,0,0," & Res
                cn_Ser.Execute "Exec Interface_SetPatientResult02 '" & strRegDate & "'," & lsPid & ",'" & mGetP(strSubCodes, 1, "|") & "','" & mGetP(strSubCodes, 2, "|") & "','" & mGetP(strSubCodes, 3, "|") & "','" & sResult & "','','',0,0,0,'IMM0001','" & strReturn & "'"
...
'    @TestResult02 =
                ...
                'SetRawData "[SQL2]" & "'" & strRegDate & "'," & lsPid & ",'" & mGetP(strSubCodes, 1, "|") & "','" & mGetP(strSubCodes, 2, "|") & "','" & mGetP(strSubCodes, 3, "|") & "','" & sResult & "','','',0,0,0,'M010',"
                
                
'@LabRegDate = '2010-05-10'
'set @LabRegNo = 14
'set @OrderCode = '24067'        --또는 ‘’
'set @TestCode = '11002'         --또는 ‘’
'set @TestSubCode = '11002'
'set @TestResult01 = '10.1'          --또는 ‘Negative’
'set @TestResult02 = ''
'set @TestResultAbn = ''             --또는 정상‘’, 높음’H’, 낮음’L’, 이상’*’
'set @IsPanic = 0
'set @IsDelta = 0
'set @IsCritical = 0
