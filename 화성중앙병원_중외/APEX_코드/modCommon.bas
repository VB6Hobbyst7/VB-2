...
Public gAllTestCd_AT   As String   '-- 인터페이스에 등록된 전체검사코드
Public gAllTestCd_FD   As String   '-- 인터페이스에 등록된 전체검사코드
Public gAllTestCd_IN   As String   '-- 인터페이스에 등록된 전체검사코드
...
'''    gHEALTH.INITURL = Trim(strSetUp1)
    ...
    gHOSP.DOCNM = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "HOSP", "DOCNAME")         '-- 의사명
...
Public Sub GetRegSetup()
    ...
    gHOSP.BARLEN = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "HOSP", "BARLEN")        '-- 바코드길이
    gHOSP.DBCONCHK = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "HOSP", "DBCONCHK")    '-- DB연결체크
    gHOSP.DOCNM = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "HOSP", "DOCNAME")   '-- DB연결체크
    ...
    gB4C.IMPORT = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "PATH", "IMPORT")
    gB4C.EXPORT = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "PATH", "EXPORT")
    gB4C.IMAGE = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "PATH", "IMAGE")
    gB4C.ASSAYATNM = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "ATOPY")
    gB4C.ASSAYFDNM = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "FOOD")
    gB4C.ASSAYINNM = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "INHALANT")
    gB4C.ASSAY96NM = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "96M")
    gB4C.PRTNAME = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "PRTNAME")
    gB4C.ORGPRTNAME = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "ORGPRTNM")
    gB4C.ORDCODE = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "ASSAY", "ORDCODE")
...
Public Sub SetExamCode(ByVal SPD As Object)
    ...
    With SPD
        ...
        For i = 0 To UBound(gArrEQP) - 1
            ...
            '5 : 검사명으로 표시한다.
            Call SetText(SPD, Trim(gArrEQP(i + 1, 5)), 0, colSTATE + (i + 1))
            
            'If Trim(gArrEQP(i + 1, 5)) = "ATOPY" Or Trim(gArrEQP(i + 1, 5)) = "FOOD" Or Trim(gArrEQP(i + 1, 5)) = "INHALANT" Or Trim(gArrEQP(i + 1, 5)) = "96M" Then
            'If Trim(gArrEQP(i + 1, 5)) = "96M" Then
            '    .ColHidden = False
            'Else
            '    .ColHidden = True
            'End If
