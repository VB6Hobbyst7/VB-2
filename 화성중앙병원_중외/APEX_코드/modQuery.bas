...
On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        SQL = SQL & "   AND SAVESEQ     = " & strSaveSeq & vbCrLf
        SQL = SQL & "   AND BARCODE     = '" & strBarcode & "'                                      " & vbCrLf
        'SQL = SQL & "   AND EXAMCODE    = '" & gB4C.ORDCODE & "'                                    " & vbCrLf
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                '-- 결과저장용 키 가져오기
                If strOrdCd = "" And strTestCd <> "" Then
                    strOrdCd = GetSampleSubITEM(strBarcode, strTestCd)
                End If
...
Public Sub GetTestList()
    ...
    SQL = SQL & " ORDER BY a.GUBUN, a.SEQNO ASC, a.TESTNAME "
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        ...
        Do Until AdoRs_Local.EOF
            ...
            gArrEQP(i, 2) = AdoRs_Local.Fields("GUBUN").Value & ""
...
'-- 검사마스터 코드조회
Public Sub GetTestCodeList_Allergy(ByVal strGubun As String)
    Dim i       As Integer
    
    i = 0
    If strGubun = "ATOPY" Then
        gAllTestCd_AT = ""
    ElseIf strGubun = "FOOD" Then
        gAllTestCd_FD = ""
    ...
    SQL = SQL & "SELECT TESTCODE        " & vbCrLf
    SQL = SQL & "  FROM TESTMASTER      " & vbCrLf
    SQL = SQL & " WHERE GUBUN = '" & strGubun & "'" & vbCrLf
    SQL = SQL & " ORDER BY TESTCODE     " & vbCrLf
    AdoCn_Local.CursorLocation = adUseClient
    Set AdoRs_Local = AdoCn_Local.Execute(SQL, , 1)
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            ...
            If Trim(AdoRs_Local.Fields("TESTCODE").Value) <> "" Then
                If i = 1 Or gAllTestCd = "" Then
                    'gAllTestCd = "'" & AdoRs_Local.Fields("TESTCODE").Value & "'"
                    gAllTestCd_AT = "'" & mGetP(AdoRs_Local.Fields("TESTCODE").Value, 1, "|") & "'"
                Else
                    'gAllTestCd = gAllTestCd & ",'" & AdoRs_Local.Fields("TESTCODE").Value & "'"
                    gAllTestCd_AT = gAllTestCd_AT & ",'" & mGetP(AdoRs_Local.Fields("TESTCODE").Value, 1, "|") & "'"
                End If
            End If
            AdoRs_Local.MoveNext
        Loop
...
Public Sub GetTestMaster(ByVal SPD As Object)
    ...
    SQL = SQL & ", a.GUBUN          AS GUBUN        " & vbCrLf
...
'    SQL = SQL & "    ON a.RSLTCHANNEL   = c.RSLTCHANNEL             " & vbCrLf
    SQL = SQL & " WHERE a.EQUIPCD       = '" & gHOSP.MACHCD & "'    " & vbCrLf
    If frmTestSet.optGubun(0).Value = True Then
        SQL = SQL & "   AND a.GUBUN = 'ATOPY' " & vbCrLf
    ElseIf frmTestSet.optGubun(1).Value = True Then
        SQL = SQL & "   AND a.GUBUN = 'FOOD' " & vbCrLf
    ElseIf frmTestSet.optGubun(2).Value = True Then
        SQL = SQL & "   AND a.GUBUN = 'INHALANT' " & vbCrLf
    ElseIf frmTestSet.optGubun(3).Value = True Then
        SQL = SQL & "   AND a.GUBUN = '96M' " & vbCrLf
    End If
    SQL = SQL & " ORDER BY a.GUBUN, a.SEQNO "
...
On Error GoTo ErrHandle
    ...
    SQL = SQL & "     , a.SpecimenName          AS SpecimenName " & vbCrLf
    ...
    End If
    If frmInterface.fraJWINFO.Visible = True Then
        If frmInterface.optSch_JW(1).Value = True Then
            SQL = SQL & "   And a.IPDOPD = 1                    " & vbCrLf   '입원
        ElseIf frmInterface.optSch_JW(2).Value = True Then
            SQL = SQL & "   And a.IPDOPD = 0                    " & vbCrLf   '외래
        End If
    End If
    SQL = SQL & " GROUP BY a.RECEIPTDATE, a.IPDOPD, a.DEPTCODE, a.SPECIMENNUM, a.PTNO, a.RECEIPTNO, a.SpecimenName, a.SNAME,a.SEX,a.AGE,a.ORDERCODE " & vbCrLf
...
'                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    ...
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    SetText SPD, Trim(RS.Fields("SpecimenName")) & "", intRow, colSPECIMEN
                    SetText SPD, Trim(RS.Fields("DEPT")) & "", intRow, colDEPT
                    SetText SPD, IIf(Trim(RS.Fields("INOUT")) = "0", "외래", "입원"), intRow, colINOUT
...
Public Sub GetResultList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT SAVESEQ,EXAMDATE,EXAMTIME,HOSPDATE,BARCODE,PNAME,SENDFLAG,SENDDATE,DEPT,INOUT " & vbCr
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    'If cn_Server_Flag = True Then
        Select Case gEMR
            Case "JWINFO"
                    GetSampleInfo = GetSampleInfo_JWINFO(asRow, SPD)
...
On Error GoTo DBErr
    ...
    If gDBTYPE = "99" Then
        SetText SPD, "1", asRow, colCHECKBOX
        SetText SPD, Format(Now, "yyyy-mm-dd"), asRow, colHOSPDATE
        SetText SPD, mResult.BarNo, asRow, colBARCODE
        SetText SPD, asRow + 1000, asRow, colCHARTNO
        SetText SPD, "홍길동" & asRow, asRow, colPNAME
        SetText SPD, "51", asRow, colPAGE
        SetText SPD, "M", asRow, colPSEX
        SetText SPD, "Serum", asRow, colSPECIMEN
        SetText SPD, "EN", asRow, colDEPT
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                SetText SPD, Trim(RS.Fields("CHARTNO")) & "", asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                SetText SPD, Trim(RS.Fields("SpecimenName")) & "", asRow, colSPECIMEN
                SetText SPD, Trim(RS.Fields("DEPT")) & "", asRow, colDEPT
                SetText SPD, IIf(Trim(RS.Fields("INOUT")) = "0", "외래", "입원"), asRow, colINOUT
                ...
                '-- 화면에 표시
                For intCol = colSTATE + 1 To .MaxCols
                    If GetTestNm(Trim(RS.Fields("ITEM")), True) = gArrEQP(intCol - colSTATE, 5) Then
                        Call SetSPDOrder(SPD, SPD.MaxRows, SPD.MaxRows, intCol, intCol)
                        '-- 처방코드, 서브코드
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("ORDCODE")) & ""
...
DBErr:
    ...
    Call SetSQLData("에러", "GetSampleInfo_JWINFO" & vbCrLf & strErrMsg, "A")
