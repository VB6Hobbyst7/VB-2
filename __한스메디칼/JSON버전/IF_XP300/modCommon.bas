...
On Error GoTo onError
    ...
    '-- 레지스트리 주소 설정
    REG_MACH = REG_POSITION & "\" & gHOSP.HOSPNM & "\" & gMACH
...
Public Sub GetExeVersion()
    ...
    '-- HOSPITAl INFO GET
    strSetup = "":    strSetUp1 = ""
    Call GetPrivateProfileString("EXE", "PHASEFRAME", "", strSetup, 100, App.PATH & "\SANSOFT.ini")
    strSetUp1 = Trim(strSetup)
    strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
    gPhaseFrame = Trim(strSetUp1)
...
Public Sub GetRegSetup()
    ...
    '-- URL (JSON)
    gURL.LOGIN = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "URL", "LOGIN")
    gURL.WORKLIST = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "URL", "WORKLIST")
    gURL.PATLIST = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "URL", "PATLIST")
    gURL.PATSAVE = GetString(HKEY_CURRENT_USER, REG_MACH & "\" & "URL", "PATSAVE")
...
'   1(ColUserSortIndicatorAscending) - Ascending The  pointer appears when the column is sorted         :   오름차순
'           Ascending sort occurred. The BeforeUserSort and AfterUserSort events occurred.
'   2(ColUserSortIndicatorDescending) - Descending The  pointer appears when the column is sorted       :   내림차순
