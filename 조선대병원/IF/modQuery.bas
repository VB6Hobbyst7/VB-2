...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
        ...
        Case "CHOSUN"
                Call GetWorkList_CHOSUN(pFrom, pTo, SPD)
...
Public Sub GetWorkList_CHOSUN(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
'    SQL = SQL & " ORDER BY a.ENTERDATE, b.WAITSEQNO                 " & vbCr
    ...
    SQL = ""
    SQL = SQL & "Select DISTINCT M.PART_JUBSU_DATE   AS HOSPDATE         " & vbCr
    SQL = SQL & "     , M.PART_JUBSU_TIME                       " & vbCr
    SQL = SQL & "     , M.BUNHO             AS PID              " & vbCr
    SQL = SQL & "     , M.SUNAME            AS PNAME            " & vbCr
    SQL = SQL & "     , M.SPECIMEN_SER      AS BARCODE          " & vbCr
    SQL = SQL & "     , M.AGE               AS AGE              " & vbCr
    SQL = SQL & "     , M.SEX               AS SEX              " & vbCr
'    SQL = SQL & "     , M.SPECIMEN_CODE                         " & vbCr
    SQL = SQL & "     , M.GWA_NAME          AS INOUT            " & vbCr
    SQL = SQL & "     , M.HANGMOG_CODE      AS ITEM             " & vbCr
    'SQL = SQL & "     , E.GUMSA_NAME                            " & vbCr
    'SQL = SQL & "     , E.JANGBI_OUT_CODE                       " & vbCr
    'SQL = SQL & "     , E.JANGBI_CODE                           " & vbCr
    SQL = SQL & "     , R.LAB_NO            AS CHARTNO          " & vbCr
    'SQL = SQL & "     , R.CONFIRM_YN                            " & vbCr
    'SQL = SQL & "     , R.CPL_RESULT                            " & vbCr
    'SQL = SQL & "     , R.JANGBI_YN                             " & vbCr
    'SQL = SQL & "     , R.JANGBI_CODE                           " & vbCr
    SQL = SQL & "  From MEDI.CPL3020 R                          " & vbCr
    SQL = SQL & "     , MEDI.CPL2010 M                          " & vbCr
    SQL = SQL & "     , MEDI.CPL0101 E                          " & vbCr
    SQL = SQL & " Where M.PART_JUBSU_DATE Between to_date('" & pFrom & "','yyyymmdd')   " & vbCr
    SQL = SQL & "                             and to_date('" & pTo & "','yyyymmdd')     " & vbCr
    SQL = SQL & "   and NVL(R.CONFIRM_YN, 'N') = 'N'                " & vbCr
'    SQL = SQL & "   and E.JANGBI_OUT_CODE IN (" & gAllTestCd & ")   " & vbCr
    SQL = SQL & "   and R.JANGBI_CODE   = '" & gHOSP.MACHCD & "'    " & vbCr
    SQL = SQL & "   and E.JANGBI_OUT_CODE is Not Null               " & vbCr
    SQL = SQL & "   and R.SPECIMEN_SER  = M.SPECIMEN_SER            " & vbCr
    'SQL = SQL & "   and R.HANGMOG_CODE  = M.HANGMOG_CODE            " & vbCr
    'SQL = SQL & "   and R.SPECIMEN_CODE = M.SPECIMEN_CODE           " & vbCr
    SQL = SQL & "   and R.HANGMOG_CODE  = E.HANGMOG_CODE            " & vbCr
    SQL = SQL & "   and R.SPECIMEN_CODE = E.SPECIMEN_CODE           "
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, Trim(RS("BARCODE")) & "", intRow, colBARCODE
                    SetText SPD, Trim(RS.Fields("PID")) & "", intRow, colPID
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
...
'            SQL = SQL & "   AND (b.RESULT = '' OR b.RESULT IS NULL) " & vbCr
            ...
            SQL = ""
            SQL = SQL & "Select E.JANGBI_OUT_CODE   AS ITEM             " & vbCrLf
            SQL = SQL & "  From MEDI.CPL3020 R                          " & vbCrLf
            SQL = SQL & "     , MEDI.CPL2010 M                          " & vbCrLf
            SQL = SQL & "     , MEDI.CPL0101 E                          " & vbCrLf
            SQL = SQL & " Where R.SPECIMEN_SER  = '" & strBarcode & "'  " & vbCrLf
            SQL = SQL & "   and NVL(R.CONFIRM_YN, 'N') = 'N'            " & vbCrLf
            SQL = SQL & "   and R.JANGBI_CODE   = '" & gHOSP.MACHCD & "'" & vbCrLf
'            SQL = SQL & "   and E.JANGBI_OUT_CODE IN (" & gAllTestCd & ")   " & vbCr
            SQL = SQL & "   and E.JANGBI_OUT_CODE is Not Null           " & vbCrLf
            SQL = SQL & "   and R.SPECIMEN_SER  = M.SPECIMEN_SER        " & vbCrLf
            SQL = SQL & "   and R.HANGMOG_CODE  = M.HANGMOG_CODE        " & vbCrLf
            SQL = SQL & "   and R.SPECIMEN_CODE = M.SPECIMEN_CODE       " & vbCrLf
            SQL = SQL & "   and R.HANGMOG_CODE  = E.HANGMOG_CODE        " & vbCrLf
            SQL = SQL & "   and R.SPECIMEN_CODE = E.SPECIMEN_CODE       "
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        ...
        Case "CHOSUN"
                Call GetSampleInfo_CHOSUN(asRow, SPD)
...
'   Parameter : strSql(쿼리)
'Return Value : Qc검체번호
Public Function GetQcNO(ByVal stJangbi_Code As String, ByVal stQC As String, ByRef stQCNO As String, ByVal blnResultchk As Boolean, ByVal stTestItem As String) As String
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT MIN(A.SPECIMEN_SER) AS SPECIMEN_SER    " & vbCrLf
    SQL = SQL & "  FROM MEDI.CPL0820 A, MEDI.CPL0101 B                  " & vbCrLf
    SQL = SQL & " WHERE B.JANGBI_CODE       = '" & stJangbi_Code & "'   " & vbCrLf
    If stTestItem <> "" Then
        SQL = SQL & "   AND B.JANGBI_OUT_CODE   IN (" & stTestItem & ") " & vbCrLf
    End If
    SQL = SQL & "   AND A.QC                = '" & stQC & "'            " & vbCrLf
    SQL = SQL & "   AND A.ORDER_DATE        = TRUNC(SYSDATE)            " & vbCrLf
    SQL = SQL & "   AND B.JUNDAL_GUBUN      = A.JUNDAL_PART             " & vbCrLf
    SQL = SQL & "   AND B.HANGMOG_CODE      = A.HANGMOG_CODE            " & vbCrLf
    SQL = SQL & "   AND B.SPECIMEN_CODE     = A.QC                      " & vbCrLf
    SQL = SQL & "   AND A.CONFIRM_DATE IS NULL                          " & vbCrLf


    If blnResultchk Then
        SQL = SQL & " AND A.RESULT_YN = 'N' " & vbCrLf
    End If
    
    Call SetSQLData("GetQcNO", SQL, "A")
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            GetQcNO = Trim(RS.Fields("SPECIMEN_SER")) & ""
            Call SetSQLData("GetQcNO", GetQcNO, "A")
            RS.MoveNext
        Loop
    End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_CHOSUN(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    strChartNo = Trim(GetText(SPD, asRow, colCHARTNO))
    ...
    If Mid(strBarcode, 5, 2) = "99" Then
        ...
        SQL = ""
        SQL = SQL & "SELECT DISTINCT "
        SQL = SQL & "       A.ORDER_DATE        AS HOSPDATE " & vbCrLf
        SQL = SQL & "     , A.BUNHO             AS PID      " & vbCrLf
        SQL = SQL & "     , A.SUNAME            AS PNAME    " & vbCrLf
        SQL = SQL & "     , A.LOT_NO            AS CHARTNO  " & vbCrLf
        SQL = SQL & "     , A.HANGMOG_CODE                  " & vbCrLf
        SQL = SQL & "     , ''                  AS AGE      " & vbCrLf
        SQL = SQL & "     , ''                  AS SEX      " & vbCrLf
        SQL = SQL & "     , ''                  AS INOUT    " & vbCrLf
        SQL = SQL & "     , A.HANGMOG_CODE                  " & vbCrLf
        SQL = SQL & "     , A.HANGMOG_CODE                  " & vbCrLf
'        SQL = SQL & "     , B.SPECIMEN_CODE                 " & vbCrLf
'        SQL = SQL & "     , B.EMERGENCY                     " & vbCrLf
'        SQL = SQL & "     , A.JANGBI_CODE                   " & vbCrLf
        SQL = SQL & "     , B.JANGBI_OUT_CODE   AS ITEM     " & vbCrLf
'        SQL = SQL & "     , INS_CODE                        " & vbCrLf
        SQL = SQL & "  FROM MEDI.CPL0820 A, MEDI.CPL0101 B  " & vbCrLf
        SQL = SQL & " WHERE A.SPECIMEN_SER  = '" & strBarcode & "' " & vbCrLf
        '-- 2021.05.03 테이블 수정
        'SQL = SQL & "   AND A.JANGBI_CODE   = '" & gHOSP.MACHCD & "' " & vbCrLf
        SQL = SQL & "   AND B.JANGBI_CODE   = '" & gHOSP.MACHCD & "' " & vbCrLf
        SQL = SQL & "   AND A.JANGBI_CODE   = B.JANGBI_CODE     " & vbCrLf
        SQL = SQL & "   AND B.JUNDAL_GUBUN  = A.JUNDAL_PART     " & vbCrLf
        SQL = SQL & "   AND B.HANGMOG_CODE  = A.HANGMOG_CODE    " & vbCrLf
        SQL = SQL & "   AND B.SPECIMEN_CODE = A.QC              " & vbCrLf
        SQL = SQL & "   AND A.CONFIRM_DATE IS NULL              " & vbCrLf
        SQL = SQL & "   AND B.JANGBI_OUT_CODE IS NOT NULL       " & vbCrLf
        SQL = SQL & " ORDER BY B.JANGBI_OUT_CODE ASC            " & vbCrLf
    Else
        SQL = ""
        SQL = SQL & "Select DISTINCT "
        SQL = SQL & "       M.PART_JUBSU_DATE   AS HOSPDATE         " & vbCrLf
        SQL = SQL & "     , M.PART_JUBSU_TIME                       " & vbCrLf
        SQL = SQL & "     , M.BUNHO             AS PID              " & vbCrLf
        SQL = SQL & "     , M.SUNAME            AS PNAME            " & vbCrLf
        SQL = SQL & "     , M.AGE               AS AGE              " & vbCrLf
        SQL = SQL & "     , M.SEX               AS SEX              " & vbCrLf
    '    SQL = SQL & "     , M.SPECIMEN_CODE                         " & vbCr
        SQL = SQL & "     , M.GWA_NAME          AS INOUT            " & vbCrLf
    '    SQL = SQL & "     , M.HANGMOG_CODE                          " & vbCr
        'SQL = SQL & "     , E.GUMSA_NAME                            " & vbCr
        SQL = SQL & "     , R.HANGMOG_CODE                          " & vbCrLf
        SQL = SQL & "     , E.JANGBI_OUT_CODE   AS ITEM             " & vbCr
        'SQL = SQL & "     , E.JANGBI_CODE                           " & vbCr
        SQL = SQL & "     , R.LAB_NO            AS CHARTNO          " & vbCrLf
        'SQL = SQL & "     , R.CONFIRM_YN                            " & vbCr
        'SQL = SQL & "     , R.CPL_RESULT                            " & vbCr
        'SQL = SQL & "     , R.JANGBI_YN                             " & vbCr
        'SQL = SQL & "     , R.JANGBI_CODE                           " & vbCr
    ...
    '    SQL = SQL & "   and E.JANGBI_OUT_CODE IN (" & gAllTestCd & ")   " & vbCr
        '-- 2021.05.03 테이블 수정
        'SQL = SQL & "   and R.JANGBI_CODE   = '" & gHOSP.MACHCD & "'" & vbCrLf
        SQL = SQL & "   and E.JANGBI_CODE   = '" & gHOSP.MACHCD & "'" & vbCrLf
        SQL = SQL & "   and E.JANGBI_OUT_CODE is Not Null           " & vbCrLf
        SQL = SQL & "   and R.SPECIMEN_SER  = M.SPECIMEN_SER        " & vbCrLf
    '    SQL = SQL & "   and R.HANGMOG_CODE  = M.HANGMOG_CODE        " & vbCrLf
    '    SQL = SQL & "   and R.SPECIMEN_CODE = M.SPECIMEN_CODE       " & vbCrLf
            ...
            With SPD
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("HANGMOG_CODE")) & ""
