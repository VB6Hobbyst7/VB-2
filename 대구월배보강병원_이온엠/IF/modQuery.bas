...
'    SQL = SQL & "   AND O.H141_NOTYYN  = 'N'                                    " & vbCr
    ...
    SQL = SQL & "   AND O.H141_RSLTYN       <> 'Y'                  " & vbCrLf
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    Call CalAgeSex(Trim(RS.Fields("AGE")) & "1", Now)
...
On Error GoTo DBErr
    ...
    '코멘트에 결과를 넣으면 나머지 검사항목도 저장된것 처럼 나타나는 현상 제거
    'SQL = SQL & "   AND O.H141_RSLTYN       <> 'Y'                  " & vbCrLf
    SQL = SQL & "   AND O.H141_SEQNO  IN (SELECT H131_SEQNO FROM TB_H131_SPPRESULT "
    SQL = SQL & "                          WHERE H131_SPPTYPE = '" & gHOSP.PARTCD & "' "
    SQL = SQL & "                            AND (H131_RESULT = '' OR H131_RESULT IS NULL))"
