        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    '-- 서버저장
                    SQL = ""
                    'SQL = SQL & "Exec PR_IHIS_LISDIE_RESULT_CREATE " & strBarcode & "," & strTestCd & "," & gHOSP.MACHCD & "," & sResult & "," & gHOSP.USERID
                    SQL = SQL & "Exec PR_IHIS_LISDIE_RESULT_CREATE " & strBarcode & "," & strTestCd & "," & "" & "," & sResult & "," & gHOSP.USERID
                    
                    With AdoCmd
                        .CommandType = adCmdStoredProc
                        .Prepared = True
                        .CommandTimeout = 15
                        .CommandText = "PR_IHIS_LISDIE_RESULT_CREATE"
                        .CommandType = adCmdStoredProc
'                        .Parameters.Refresh
...
'                        Set AdoRs = Nothing
                        ...
                        Set Prm1 = .CreateParameter("I_SPECIMEN_SER", adVarChar, adParamInput, 4000, strBarcode)
                        .Parameters.Append Prm1
                        Set Prm2 = .CreateParameter("I_HANGMOG_CODE", adVarChar, adParamInput, 4000, strTestCd)
                        .Parameters.Append Prm2
                        'Set Prm3 = .CreateParameter("I_JANGBI_CODE", adVarChar, adParamInput, 4000, gHOSP.MACHCD)
                        Set Prm3 = .CreateParameter("I_JANGBI_CODE", adVarChar, adParamInput, 4000, "")
                        .Parameters.Append Prm3
                        Set prm4 = .CreateParameter("I_CPL_RESULT", adVarChar, adParamInput, 4000, sResult)
                        .Parameters.Append prm4
                        Set Prm5 = .CreateParameter("I_REFERENCE_F", adVarChar, adParamInput, 4000, "")
                        .Parameters.Append Prm5
                        Set Prm6 = .CreateParameter("I_REFERENCE_T", adVarChar, adParamInput, 4000, "")
                        .Parameters.Append Prm6
                        Set prm7 = .CreateParameter("I_USER_ID", adVarChar, adParamInput, 4000, gHOSP.USERID)
                        .Parameters.Append prm7
                        Set prm8 = .CreateParameter("IO_FLAG", adVarChar, adParamInputOutput, 4000, strErrMsg)
                        .Parameters.Append prm8
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_SUNGMO" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "SUNGMO"
                Call GetWorkList_SUNGMO(pFrom, pTo, SPD)
...
'    SQL = ""
'    SQL = SQL & "SELECT DISTINCT "
'    SQL = SQL & "  JUBSU_DATE       AS HOSPDATE         " & vbCrLf
...
'    SQL = SQL & ", ORDER_INPUT      AS DEPTNM           " & vbCrLf
'    SQL = SQL & ", COUNT(SUB_HANGMOG_CODE) as CNT       " & vbCrLf
'    'SQL = SQL & ", JUBSU_DATE                   " & vbCrLf
'    'SQL = SQL & ", JANGBI_CODE                  " & vbCrLf
'    SQL = SQL & "  FROM VW_IHIS_LISDIE_GUMSA_INFO_SUB   " & vbCrLf
'    SQL = SQL & " WHERE JUBSU_DATE BETWEEN '" & pFrom & "' AND '" & pTo & "' " & vbCrLf
'    'SQL = SQL & "   AND SPECIMEN_SER = '" & sBarcode & "'" & vbCrlf
'    SQL = SQL & "   AND SUB_HANGMOG_CODE IN (" & gAllTestCd & ")" & vbCrLf
'    SQL = SQL & " GROUP BY JUBSU_DATE,SPECIMEN_SER,BUNHO,SUNAME,AGE,SEX,ORDER_INPUT  " & vbCrLf
'    SQL = SQL & " ORDER BY JUBSU_DATE,BUNHO       "
    ...
    SQL = SQL & ", SEX                  AS SEX              " & vbCrLf
    SQL = SQL & ", ORDER_INPUT          AS DEPTNM           " & vbCrLf
    SQL = SQL & ", GUMSA_CODE                               " & vbCrLf
    SQL = SQL & ", COUNT(GUMSA_CODE)    AS CNT       " & vbCrLf
    SQL = SQL & "  FROM VW_IHIS_LISDIE_GUMSA_INFO   " & vbCrLf
    SQL = SQL & " WHERE JUBSU_DATE BETWEEN '" & pFrom & "' AND '" & pTo & "' " & vbCrLf
    'SQL = SQL & "   AND SPECIMEN_SER = '" & sBarcode & "'" & vbCrlf
    SQL = SQL & "   AND GUMSA_CODE IN (" & gAllTestCd & ")" & vbCrLf
    SQL = SQL & " GROUP BY JUBSU_DATE, SPECIMEN_SER,BUNHO,SUNAME,AGE,SEX,ORDER_INPUT,GUMSA_CODE  " & vbCrLf
    SQL = SQL & " ORDER BY JUBSU_DATE, GUMSA_CODE, SPECIMEN_SER, BUNHO       "
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_SUNGMO" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As Object) As String
    ...
    Select Case gEMR
        Case "SUNGMO"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT GUMSA_CODE as ITEM       " & vbCrLf
            SQL = SQL & "  FROM VW_IHIS_LISDIE_GUMSA_INFO           " & vbCrLf
            SQL = SQL & " WHERE SPECIMEN_SER = '" & strBarcode & "'     " & vbCrLf
            SQL = SQL & "   AND GUMSA_CODE IN (" & gAllTestCd & ")" & vbCrLf
            'SQL = SQL & " ORDER BY GUMSA_CODE"
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As Object) As Integer
    ...
    If cn_Server_Flag = True Then
        Select Case gEMR
            Case "SUNGMO"
                    Call GetSampleInfo_SUNGMO(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_SUNGMO(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    SQL = SQL & ", SEX              AS SEX              " & vbCrLf
'    SQL = SQL & ", ORDER_INPUT      AS DEPTNM           " & vbCrLf
'    SQL = SQL & ", SUB_HANGMOG_CODE AS ITEM             " & vbCrLf
'    SQL = SQL & "  FROM VW_IHIS_LISDIE_GUMSA_INFO_SUB   " & vbCrLf
'    SQL = SQL & " WHERE SPECIMEN_SER = '" & strBarcode & "'" & vbCrLf
'    SQL = SQL & "   AND SUB_HANGMOG_CODE IN (" & gAllTestCd & ")" & vbCrLf
    ...
    SQL = SQL & ", SEX                  AS SEX              " & vbCrLf
    SQL = SQL & ", ORDER_INPUT          AS DEPTNM           " & vbCrLf
    SQL = SQL & ", GUMSA_CODE           AS ITEM             " & vbCrLf
    SQL = SQL & "  FROM VW_IHIS_LISDIE_GUMSA_INFO           " & vbCrLf
    SQL = SQL & " WHERE SPECIMEN_SER = '" & strBarcode & "' " & vbCrLf
    SQL = SQL & "   AND GUMSA_CODE IN (" & gAllTestCd & ")  " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    'If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                    If Trim(RS.Fields("ITEM")) = gArrEQPNm(intCol - colSTATE, 6) Then
                        .Row = asRow
                        .Col = intCol
                        .BackColor = vbYellow
                        Call SetText(SPD, "◇", asRow, intCol)
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_SUNGMO" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
