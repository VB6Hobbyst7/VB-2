...
'한 장비채널에 검사코드가 1개이상 존재 (GLU-FBS, GLU-PP2..)
Public Function GetEquipExamCode_HITACHI7080(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim intIntBase      As Integer
    Dim strItems        As String           '전송할 검사항목
    Dim blnISE          As Boolean          'Na, K, Cl 검사여부
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" Then
                    ...
                    '-- 결과가 수치형이면
                    If IsNumeric(sResult) And InStr(sResult, "+") <= 0 Then
...
On Error GoTo ErrHandle
    ...
    strFYear = Mid(pFrom, 1, 4)
    strFMonth = Mid(pFrom, 5, 2)
    strFDay = Mid(pFrom, 7, 2)
    
    strTYear = Mid(pTo, 1, 4)
    strTMonth = Mid(pTo, 5, 2)
    strTDay = Mid(pTo, 7, 2)
    
    SQL = ""
    SQL = SQL & "Select DISTINCT "
    SQL = SQL & "       a.진료년                                                    " & vbCrLf
    SQL = SQL & "     , a.진료월                                                    " & vbCrLf
    SQL = SQL & "     , a.진료일                                                    " & vbCrLf
    ...
    SQL = SQL & " Where a.진료년 BETWEEN '" & strFYear & "' and '" & strTYear & "'  " & vbCrLf
    SQL = SQL & "   And a.진료월 BETWEEN '" & strFMonth & "' and '" & strTMonth & "'" & vbCrLf
    SQL = SQL & "   And a.진료일 BETWEEN '" & strFDay & "' and '" & strTDay & "'    " & vbCrLf
    ...
    SQL = SQL & " GROUP BY a.진료년,a.진료월,a.진료일,a.챠트번호,b.수진자명         " & vbCrLf   ', c.진료상태
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        Do Until RS.EOF
            With SPD
                strHDate = Trim(RS("진료년")) & Trim(RS("진료월")) & Trim(RS("진료일"))
                For i = 1 To SPD.DataRowCnt
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colBARCODE)
                    strChartNo = GetText(SPD, i, colCHARTNO)
                    If strHDate = strHospDate And Trim(RS("CHARTNO")) = strChartNo Then
                        blnSame = True
                    End If
                Next
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, strHDate, intRow, colHOSPDATE
...
'    Call SetCommStatus("R", pBarNo, frmMain.lstComStatus)
    ...
    intRow = -1
    With frmMain
        If gHOSP.MACHNM = "HITACHI7020" Then 'Or gHOSP.MACHNM = "HITACHI7080" Then
            For i = 1 To .spdOrder.DataRowCnt
                If gEMR = "MEDICHART" Then
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                strHDate = Trim(RS.Fields("진료년")) & Trim(RS.Fields("진료월")) & Trim(RS.Fields("진료일"))
                
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, strHDate, asRow, colHOSPDATE
