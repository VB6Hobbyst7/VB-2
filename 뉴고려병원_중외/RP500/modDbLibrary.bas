...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        strINOUT = Trim(GetText(.vasID, argSpcRow, colINOUT))
...
'''            strSrcfile = App.apth & "\" & Format(Now, "yyyymmdd")
'''
...
'''            Close #3
'''
'''                  SQL = "INSERT INTO QC_RP500 (QCDATE,QCDATA) Values ("
'''            SQL = SQL & vbCrLf & "'" & Format(Now, "yyyymmddhhmmss") & "',"
'''            SQL = SQL & vbCrLf & "'" & strBuffer & "')"
'''
'''            Call SetSQLData("QC저장", SQL)
'''
...
'''        Else
            ...
            For iRow = 1 To .vasTemp.DataRowCnt
                ...
                If sResult <> "" Then
                    ...
                    'SQL = SQL & vbCrLf & " Where ReceiptNo = '" & strChtNum & "' "
                    SQL = SQL & vbCrLf & "   AND OrderCode = '" & strExamCd & "'"
                    SQL = SQL & vbCrLf & "   And LabCode = '" & strExamCd & "'"
            ...
            If Res = 1 Then
                '-- 상태변경
                '-- OCS 상태변경
                If frmInterface.chkOCS.Value = "1" Then
                
'ResultDate     결과일자    CHAR        10
'ResultTime     결과시간    CHAR        8
'SignOut        결과 확인자 VARCHAR2    10


                                   SQL = "Update SLA_LabMaster "
                    SQL = SQL & vbCrLf & "   Set JStatus = '3' "
                    SQL = SQL & vbCrLf & " Where SPECIMENNUM = '" & lsID & "' "
                    'SQL = SQL & vbCrLf & " Where ReceiptNo = '" & strChtNum & "' "
                    SQL = SQL & vbCrLf & "   AND OrderCode = '" & strExamCd & "' "
                ...
                If frmInterface.chkOCS.Value = "1" Then
                    If strINOUT = "입원" Then
                                       SQL = "Update ORD_IPDORDER "
                        SQL = SQL & vbCrLf & "   Set OCSFLAG = '3' "    '결과완료
                        SQL = SQL & vbCrLf & " Where SPECIMENNUM = '" & lsID & "' "
                        SQL = SQL & vbCrLf & "   AND OrderCode = '" & strOrderCd & "' "
                        SQL = SQL & vbCrLf & "   And OCSFLAG < '3' "
                    Else
                                       SQL = "Update ORD_OPDORDER "
                        SQL = SQL & vbCrLf & "   Set OCSFLAG = '3' "    '결과완료
                        SQL = SQL & vbCrLf & " Where SPECIMENNUM = '" & lsID & "' "
                        SQL = SQL & vbCrLf & "   AND OrderCode = '" & strOrderCd & "' "
                        SQL = SQL & vbCrLf & "   And OCSFLAG < '3' "
                    End If
                    Call SetSQLData("상태저장_OCS", SQL)
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, IIf(Trim(RS.Fields("IPDOPD")) = "I", "입원", "외래"), asRow, colINOUT
