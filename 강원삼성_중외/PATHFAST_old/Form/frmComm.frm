...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Begin FPSpread.vaSpread spdResult1 
         ...
         MaxRows         =   1
         ScrollBarMaxAlign=   0   'False
         SpreadDesigner  =   "frmComm.frx":765D
         UserResize      =   0
      End
      ...
      Begin Threed.SSFrame SSFrame1 
         ...
         Begin FPSpread.vaSpread spdRstview 
            ...
            SpreadDesigner  =   "frmComm.frx":7BC5
            UserResize      =   0
         End
      End
      Begin Threed.SSPanel SSPanel1 
         ...
         Begin VB.ComboBox cboChk 
            Enabled         =   0   'False
            Height          =   300
            ItemData        =   "frmComm.frx":8204
            Left            =   3630
            List            =   "frmComm.frx":820E
            TabIndex        =   62
            Top             =   90
            Visible         =   0   'False
            Width           =   1095
         ...
         Begin VB.ComboBox cboComNm 
            Height          =   300
            ItemData        =   "frmComm.frx":821E
            Left            =   4755
            List            =   "frmComm.frx":8220
            TabIndex        =   61
            Top             =   90
            Visible         =   0   'False
            Width           =   1545
      ...
      Begin Threed.SSPanel SSPanel3 
         ...
         Begin VB.ComboBox cboRstgbn 
            Height          =   300
            Index           =   1
            ItemData        =   "frmComm.frx":8B12
            Left            =   2475
            List            =   "frmComm.frx":8B1F
            Style           =   2  '드롭다운 목록
            TabIndex        =   71
            Top             =   90
            Width           =   1770
      ...
      Begin FPSpread.vaSpread spdResult2 
         ...
         MaxRows         =   1
         ScrollBarMaxAlign=   0   'False
         SpreadDesigner  =   "frmComm.frx":8B49
         UserResize      =   0
      End
...
'@InstNo varchar(3),  장비코드
'@ConfirmYN char(1),  최종결과확인 유무
'@From  varchar(10),  일자구간1
'@To  varchar(10)     일자구간2
    ...
    CallForm = "clsCommon - Public Function f_subSet_WorkList() As ADODB.Recordset"
        ...
        If cboChk.ListIndex = 0 Then
            ...
            sqlDoc = " SELECT a.per_gumjin_date, a.per_gum_num, a.edpscode, a.result, a.send_date, a.per_name " & _
                    ...
                    " WHERE a.per_gumjin_date = '" & Mid(strBarno, 1, 8) & "'" & _
                    " AND a.per_gum_num = '" & Val(Mid(strBarno, 9)) & "' " & _
                    " AND a.result = '' " & _
        ...
        Else
            sqlDoc = " SELECT a.EnterDate, b.Status, b.waitseqno, b.MAP2SEQNO, b.DispDesc, b.RVALUEKIND, b.NORMLOW, b.NORMHIGH, b.NORMALVALUE, b.RVALUEKIND , " & _
                    ...
                    " WHERE a.waitseqno = '" & Val(Mid(strBarno, 9)) & "' " & _
                    ...
                    If Trim(strJinCd) <> "" Then
                        sqlDoc = sqlDoc & "    AND b.MAP2SEQNO in (" & strJinCd & ")" & vbLf
                    End If
                    sqlDoc = sqlDoc & " ORDER BY a.chartno "
        End If
...
'On Error GoTo ErrorTrap
    ...
    Else
        ...
        Do Until mAdoRs.EOF
            intIdx = 0
            With spdWorklist
                If strBarno <> mAdoRs.Fields("LID") Then
                    intRow = intRow + 1
                    ...
                    .SetText 4, intRow, mAdoRs.Fields("Hcode")
                    .SetText 5, intRow, mAdoRs("Serial")
                    .SetText 6, intRow, mAdoRs("PtName")
                    .SetText 7, intRow, mAdoRs("RORDER")
                    '-- 검사항목조회
                        ...
                        If Not itemX Is Nothing Then
                            ...
                            If intPos > 0 Then
                                ...
                                For tmpTestSeq1 = 1 To UBound(tmpTestcd) + 1
                                    ...
                                    Loop
                                Next
                                sqlDoc = "AP_INF_S_Update " & strBarno & ", " & strSerial & ", " & strRorder & ","
    ...
    With spdWorklist
        For introw1 = 1 To .maxrows
            ...
            If Trim$(varTmp) = "1" Then
                ...
                .GetText 4, introw1, varTmp:    strSPNo = Trim$(varTmp)
                ...
                intRow2 = f_funGet_SpreadRow(spdResult1, 1, strBarno)
                If intRow2 < 1 Then
...
'
                    ...
                    If Len(strBarno) <> 0 Then
                        Do Until mAdoRs.EOF
'                            Debug.Print strBarNo & "," & mAdoRs("CODA") & "," & mAdoRs("SUBCODA")
                            strEqpCd = f_funGet_CODE(mAdoRs("CODA") & "/" & mAdoRs("SUBCODA"))
                            Set itemX = lvwCuData.FindItem(strEqpCd, lvwTag, , lvwWhole)
                            If Not itemX Is Nothing Then
                                f_strTest(f_intIdx) = f_strTest(f_intIdx) + IIf(Mid$(itemX.tag, 1, 1) = "X", "13", itemX.tag)
                    ...
                    If blnFlag = True Then
                        ...
                        spdResult1.SetText 7, intRow2, strRorder
                    Else
                        spdResult1.maxrows = spdResult1.maxrows - 1
...
Private Sub psDataDefine(ByVal strdata As String, ByRef brChannel() As String, ByVal brspread As Object) ', ByVal brOst As String) ' ByRef brItemdeci() As String)
    ...
    Select Case Mid(sTemp, 3, 1)
        Case "H"
        Case "P"
            strWorkNo = mGetP(sTemp, 2, "|")
        Case "O"
            fNOVApHOX = Split(sTemp, "|")
            pSeq = fNOVApHOX(3)
            
            If Len(pSeq) > 0 Then
                With spdResult1
                
                    'sSeq = Val(fNOVApHOX_1(1))
                    sCol = 9
                    pGrid_Point = SeqSearch(spdResult1, pSeq, sCol)
...
'                    .GetText 6, pGrid_Point, varTmp:   strSerial = Trim$(varTmp)
            ...
            End If
        Case "R"
            fNOVApHOX_1 = Split(sTemp, "|")
            fNOVApHOX_2 = Split(Trim(fNOVApHOX_1(2)), "^")
            Channel_No = Trim(fNOVApHOX_2(3))
                        ...
                        For intCol = 10 To .MaxCols
                            ...
                            If Not itemX Is Nothing Then
                                For intIdx = 1 To .MaxCols
                                    If Len(fNOVApHOX_1(3)) > 0 Then
                                        If UCase(Channel_No) = Trim(itemX.tag) Then
                                            strRstval = Trim(fNOVApHOX_1(3))
                                            ...
                                            Dim p_slip  As String
                                            Dim p_wkno  As Integer
                                            Dim p_code  As String
                                            Dim p_serl  As Single
                                            Dim p_ecod  As Integer
                                            Dim p_emsg  As String
                                            Dim p_rslt  As String
                                            Dim sp_code As String
                                            Dim strSql  As String
                                            Dim sCheck  As Boolean
                                            Dim sstemp As String

                                            p_date = Mid(strBarno, 1, 8)
                                            p_slip = Mid(itemX, 1, 3)
                                            p_wkno = pName
                                            p_code = Mid(itemX, 4, 2)
                                            p_serl = Val(Mid(itemX, 6, 1))
                                            sstemp = itemX
                                            p_serl = Right(sstemp, 1)
                                            p_rslt = strRstval
                                            sp_code = p_slip & p_code
                                            strSql = ""
                                            ...
                                            If chkAuto.Value = vbChecked Then
                                                ...
                                                sqlDoc = "AP_INF_S_Update '" & strBarno & "', '" & pNo & "', '" & strRorder & "',"
                                                sqlDoc = sqlDoc & " '" & INS_CODE & "', '" & Mid(tmpTestCd1, 1, InStr(tmpTestCd1, "/") - 1) & "', '" & Mid(tmpTestCd1, InStr(tmpTestCd1, "/") + 1) & "',"
                                                sqlDoc = sqlDoc & " '" & strRstval & "'"
