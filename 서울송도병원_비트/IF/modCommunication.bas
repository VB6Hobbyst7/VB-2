...
'                SQL = SQL & "   And ResSubSeq = '" & mGetP(strSubCode, 3, "|") & "'             " & vbCr
                ...
                SQL = SQL & " WHERE RESSPMNUM = '" & strBarcode & "'" & vbCr
                SQL = SQL & "   AND RESLABCOD = '" & strTestCd & "'" & vbCr
                'SQL = SQL & "   AND RESREPTYP = 'N'" 'N : 미결과
                SQL = SQL & "   AND (RESREPTYP = 'N' or RESREPTYP = 'M' or RESREPTYP = '' or RESREPTYP IS NULL)"
        ...
        '상태 업데이트
              SQL = "Update RsbInf Set RsbItfYon = 'M' "            '-- 보고대기
        SQL = SQL & " WHERE RsbspmNum = '" & strBarcode & "'" & vbCr
        SQL = SQL & "   AND (RsbItfYon <> 'I' AND RsbItfYon <> 'F') " '최종(F)/중간(I)은 제외
