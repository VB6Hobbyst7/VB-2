...
'-- 검사코드로 검사명 조회
Public Function GetTestNm(ByVal pItem As String, Optional pFull As Boolean, Optional pGubun As String) As String
    Dim intRow          As Long
    ...
    If pGubun <> "" Then
        If pGubun = "FOOD" Then
            SQL = SQL & " AND GUBUN = 'FD'" & vbCr
        ElseIf pGubun = "INHALANT" Then
            SQL = SQL & " AND GUBUN = 'IN'" & vbCr
        End If
    End If
    
'    Call SetSQLData("검사명조회", SQL)
...
On Error GoTo RST
    ...
    SQL = SQL & " GROUP BY a.RECEIPTDATE, a.SPECIMENNUM, a.ORDERCODE, a.RECEIPTNO, a.IPDOPD, a.PTNO, a.SNAME, a.SEX, a.AGE " & vbCr
        ...
        Do Until RS.EOF
            With SPD
                ...
                For i = 1 To SPD.DataRowCnt
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colBARCODE)
                    strGubun = GetText(SPD, i, colRACKNO)
                    If Trim(RS("HOSPDATE")) = strHospDate And Trim(RS("BARCODE")) = strBarcode And Trim(RS("GUBUN")) = strGubun Then
                        blnSame = True
                    End If
                Next
                ...
                If blnSame = False Then
                    .MaxRows = .MaxRows + 1
                    intRow = .MaxRows
                    strBC = Trim(RS.Fields("BARCODE")) & ""
                    If Len(strBC) = 7 Then
                        strBC = "0" & strBC
                    End If
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", intRow, colHOSPDATE
                    SetText SPD, strBC, intRow, colBARCODE
                    If Trim(RS.Fields("GUBUN")) & "" = "L-5363" Then
                        SetText SPD, "FOOD", intRow, colRACKNO
                    ElseIf Trim(RS.Fields("GUBUN")) & "" = "L-5366" Then
                        SetText SPD, "INHALANT", intRow, colRACKNO
                    Else
                        SetText SPD, "처방오류", intRow, colRACKNO
                    End If
...
On Error GoTo RST
    ...
    SQL = SQL & "     , L.처방코드          AS ORDCD    " & vbCr
    SQL = SQL & "     , L.검사명칭          AS TESTNM   " & vbCr
    ...
    'SQL = SQL & "   AND L.검사종류 = '" & gHOSP.PARTCD & "'" & vbCr                                 '--> 검사종류를 사용시 '03'
    SQL = SQL & "   AND L.검사상태 < 5                                     " & vbCr
'    SQL = SQL & "   AND L.처방코드 + L.서브코드 IN (" & gAllTestCd & ")    " & vbCr
    SQL = SQL & "   AND L.처방코드 + L.서브코드 IN ('" & gFDOrdCd & "','" & gINOrdCd & "')    " & vbCr
    SQL = SQL & " GROUP BY J.접수일자, L.검체번호, A.챠트번호, J.접수번호, A.환자이름, A.환자성별, A.환자나이,L.처방코드,L.검사명칭 " & vbCr
        ...
        Do Until RS.EOF
            With SPD
                ...
                For i = 1 To SPD.DataRowCnt
                    strHospDate = GetText(SPD, i, colHOSPDATE)
                    strBarcode = GetText(SPD, i, colBARCODE)
                    strGubun = GetText(SPD, i, colPOSNO)
                    If Trim(RS("HOSPDATE")) = strHospDate And Trim(RS("BARCODE")) = strBarcode And Trim(RS("ORDCD")) = strGubun Then
                        blnSame = True
                    End If
                Next
                ...
                If blnSame = False Then
                    ...
                    If InStr(UCase(Trim(RS.Fields("TESTNM"))) & "", "FOOD") > 0 Then
                        SetText SPD, "FOOD", intRow, colRACKNO
                    ElseIf InStr(UCase(Trim(RS.Fields("TESTNM"))) & "", "INHAL") > 0 Then
                        SetText SPD, "INHALANT", intRow, colRACKNO
                    Else
                        SetText SPD, "처방오류", intRow, colRACKNO
                    End If
                
'                    SetText SPD, UCase(Trim(RS.Fields("TESTNM"))), intRow, colRACKNO
                    
                    SetText SPD, Trim(RS.Fields("ORDCD")) & "", intRow, colPOSNO
...
Public Sub GetResultList(ByVal pFrom As String, ByVal pTo As String, ByVal pDateType As Integer, ByVal pOpt As Integer)
    ...
    '-- 검사결과
    SQL = SQL & ",SEQNO,EXAMNAME,RESULT,REFFLAG,REFJUDGE" & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        ReDim Preserve strExcelData(RS.RecordCount, frmMain.spdROrder.MaxCols)
...
'                    End If
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    ...
                    If Trim(RS.Fields("EXAMNAME")) & "" = Trim(.Text) Then
                        'SetText frmMain.spdROrder, Trim(RS.Fields("RESULT")) & "(" & Trim(RS.Fields("REFJUDGE")) & ")", intRow, intCol
                        'SetText frmMain.spdROrder, Trim(RS.Fields("RESULT")) & "", intRow, intCol
                        SetText frmMain.spdROrder, Trim(RS.Fields("REFFLAG")) & "", intRow, intCol
                        strExcelData(intRow, intCol) = Trim(RS.Fields("REFJUDGE")) & ""
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    ...
    strGubun = Trim(GetText(SPD, asRow, colRACKNO))
...
'            SQL = SQL & "   AND i.IntLabCod IN (" & gAllTestCd & ")" & vbCr
        ...
        Case "JWINFO"
            ...
            If strGubun = "FOOD" Then
                SQL = SQL & "   AND ORDERCODE = 'L-5363'  " & vbCr
            ElseIf strGubun = "INHALANT" Then
                SQL = SQL & "   AND ORDERCODE = 'L-5366'  " & vbCr
            Else
...
'                        If Trim(RS.Fields("ITEM")) & "" = gFDOrdCd Then
'                            SetText SPD, "FOOD", asRow, colRACKNO
'                            blnFind = True
'                        ElseIf Trim(RS.Fields("ITEM")) & "" = gINOrdCd Then
'                            SetText SPD, "INHALANT", asRow, colRACKNO
'                            blnFind = True
'                        'Else
'                        '    SetText SPD, "처방오류", asRow, colRACKNO
'                        '    blnFind = True
'                        End If
'                    End If
'
...
'            SQL = SQL & "   And 처방코드+서브코드 in (" & gAllExam & ")"
    ...
    If SQL <> "" Then
        ...
        If Not RS.EOF = True And Not RS.BOF = True Then
            Do Until RS.EOF
                If strItems = "" Then
                    strItems = GetTestNm(Trim(RS.Fields("ITEM")) & "", False, strGubun)
                Else
                    strItems = strItems & "/" & GetTestNm(Trim(RS.Fields("ITEM")), False, strGubun)
                End If
                RS.MoveNext
            Loop
        End If
...
On Error GoTo DBErr
    ...
    If mResult.MnmCd = "FD" Then
        SQL = SQL & "   AND a.ORDERCODE = 'L-5363'" & vbCr
    ElseIf mResult.MnmCd = "IN" Then
        SQL = SQL & "   AND a.ORDERCODE = 'L-5366'" & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                If mResult.MnmCd = "FD" Then
                    SetText SPD, "FD", asRow, colRACKNO
                ElseIf mResult.MnmCd = "IN" Then
                    SetText SPD, "IN", asRow, colRACKNO
                End If
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    With frmMain
        Select Case gEMR
            ...
            Case Else
                sExamDate = GetText(.spdOrder, asRow1, colEXAMDATE) 'Format(.dtpToday, "yyyymmdd")
                If Trim(GetText(.spdOrder, asRow1, colSAVESEQ)) = "" Then
                    Exit Function
                End If
