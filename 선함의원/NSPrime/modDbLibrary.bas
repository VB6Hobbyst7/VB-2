...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        lsChtid = Trim(GetText(.vasID, argSpcRow, colCHARTNO))
...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            strSN = Trim(GetText(.vasTemp, iRow, 10))
            ...
            '-- 검사결과저장
            If sResult <> "" Then
                      SQL = "update req_result set "
                SQL = SQL & " result = '" & sResult & "'"
                SQL = SQL & ",result_type = case when fun_isnumeric('" & sResult & "') = 1 then 'N' else 'C' end " & vbCr       '-- 검사결과가 수치형일경우 'N'으로 문자형일경우 'C'로 처리."
                SQL = SQL & ",result_num = case when fun_isnumeric('" & sResult & "') = 1 then '" & sResult & "' end " & vbCr   '-- 검사결과가 수치형일경우 처리"
                SQL = SQL & ",result_text = case when fun_isnumeric('" & sResult & "') = 2 then '" & sResult & "' end " & vbCr  '-- 검사결과가 문자형일경우 처리"
                SQL = SQL & ",result_date = to_char(sysdate,'yyyymmdd')" & vbCr     '* 결과입력일자"
                SQL = SQL & ",result_time = to_char(sysdate,'hh24miss') " & vbCr    '* 결과입력시간"
                SQL = SQL & ",interface_date = sysdate " & vbCr                     '* Interface전송 일시"
                SQL = SQL & ",progress_code = '04'" & vbCr
                SQL = SQL & ",lab_yn ='N'" & vbCr
                SQL = SQL & "  where req_no = '" & lsID & "'" & vbCr            '/* 의뢰번호 */"
                SQL = SQL & "    and item_code = '" & strExamCd & "'" & vbCr    '/* 검사코드 */"
                SQL = SQL & "    and labseq = '" & strSN & "'"              '/* 검사Seq */"
...
'      -- 테이블 사용
    ...
    SQL = SQL & "   AND SPNO = '" & sBarcode & "' " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("ORCD")) & "',"
                
                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                SetText .vasID, Trim(RS.Fields("OKDT")) & "", asRow, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("SPNO")) & "", asRow, colBARCODE
                SetText .vasID, Trim(RS.Fields("WKNO")) & "", asRow, colCHARTNO
                SetText .vasID, Trim(RS.Fields("PAID")) & "", asRow, colPID
                SetText .vasID, Trim(RS.Fields("PANM")) & "", asRow, colPNAME
                SetText .vasID, Trim(RS.Fields("SEXS")) & "", asRow, colPSEX
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_SQLLAB(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
...
'    SQL = SQL & " ORDER BY OKDT "
    ...
    SQL = "  select                      " '* Worklist 오더 조회"
    SQL = SQL & "    req_no                    " '* 의뢰번호
    SQL = SQL & "  , req_date                  " '* 의뢰일자
    SQL = SQL & "  , pat_name                  " '* 환자명
    SQL = SQL & "  , chart_no                  " '* 차트번호
    SQL = SQL & "  , id_no                     " '* 생년월일
    SQL = SQL & "  , sex                       " '* 성별
    SQL = SQL & "  , age                       " '* 나이
    SQL = SQL & "  , cust_name                 " '* 거래처명
    SQL = SQL & "  , item_code                 " '* 검사코드
    SQL = SQL & "  , item_name                 " '* 검사명
    SQL = SQL & "  , sample_code               " '* 검체코드
    SQL = SQL & "  , sample_name               " '* 검체명
    SQL = SQL & "  , labseq                    " '* 검사Seq
    SQL = SQL & "  , result                    " '* 검사결과
    SQL = SQL & "  , result_text               " '* 검사결과(문자형)
    SQL = SQL & "  , result_num                " '* 검사결과(숫자형)
    SQL = SQL & "  , comt                      " '* Commenet
    SQL = SQL & "  , work_no                   " '* 작업번호
    SQL = SQL & "  , work_date                 " '* 작업일자
    SQL = SQL & "  , dept_code                 " '* 영업소코드
    SQL = SQL & "  , dept_name                 " '* 영업소명
    SQL = SQL & "  , lmp_date                  " '* LMP일자
    SQL = SQL & "  , gest_week                 " '* 임신주수(주)
    SQL = SQL & "  , gest_day                  " '* 임신주수(일)
    SQL = SQL & "  , usg_week                  " '* USG주수(주)
    SQL = SQL & "  , usg_day                   " '* USG주수(일)
    SQL = SQL & "  , nt                        " '* NT(mm)
    SQL = SQL & "  , crl                       " '* CRL
    SQL = SQL & "  , weight                    " '* 체중
    SQL = SQL & "  , urine_vol                 " '* 축뇨량
    SQL = SQL & "  , progress_code             "
    SQL = SQL & "   from v_l020101             "
    SQL = SQL & " where REQ_NO = '" & sBarcode & "'" & vbCr
    SQL = SQL & "   and progress_code < '05' " & vbCr
    SQL = SQL & "   AND item_code IN (" & gAllExam & ")" & vbCr
    SQL = SQL & "   AND (result = '' OR result is null)"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("item_code")) & "',"
                
                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                SetText .vasID, Trim(RS.Fields("req_date")) & "", asRow, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("req_no")) & "", asRow, colBARCODE
                SetText .vasID, Trim(RS.Fields("chart_no")) & "", asRow, colCHARTNO
                SetText .vasID, Trim(RS.Fields("labseq")) & "", asRow, colSN
                SetText .vasID, Trim(RS.Fields("pat_name")) & "", asRow, colPNAME
                SetText .vasID, Trim(RS.Fields("sex")) & "", asRow, colPSEX
                
                '-- 화면에 표시
                For intCol = colState + 1 To .vasID.MaxCols
                    If Trim(RS.Fields("item_code")) = gArrEquip(intCol - colState, 3) Then
                        .vasID.Row = asRow
                        .vasID.Col = intCol
                        .vasID.BackColor = vbYellow
                        Exit For
