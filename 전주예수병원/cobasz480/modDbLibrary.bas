...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If iRow = .vasTemp.DataRowCnt Then
                '-- 결과저장
                sResult = GetRemark(strEqpCd, sResult)
                
                If sResult <> "" Then
                    
                                   SQL = "Update SPXMSTDT  "
                    SQL = SQL & vbCrLf & "   Set RSLTTEXT = '" & sResult & "' "
                    SQL = SQL & vbCrLf & " Where PATHNO = '" & lsID & "' "
...
Function GetRemark(ByVal strChannel As String, ByVal strResult As String) As String
    GetRemark = ""
...
'08EGFR03 -21
'07KRAS - no codon
'07KRAS01 - codon 12/13
'07KRAS02 - codon 61
'07KRAS03 - codon 12/13,61

    Dim strEGFRResultN As String
    Dim strEGFRResultP As String
    Dim strEGFRDesc    As String
    Dim strEGFRCmnt    As String
    
    Dim strKRASResultN As String
    Dim strKRASResultP As String
    Dim strKRASDesc    As String
    Dim strKRASCmnt    As String
    
    strEGFRResultN = ""
    strEGFRResultN = strEGFRResultN & "<분석 결과>" & vbCr
    strEGFRResultN = strEGFRResultN & "상기환자의 EGFR gene의 exon 18~21에 대해 검사한 결과 돌연변이가 검출되지 않았습니다." & vbCr
    
    strEGFRDesc = ""
    strEGFRDesc = strEGFRDesc & "<분석 방법>" & vbCr
    strEGFRDesc = strEGFRDesc & "1. DNA extraction from sample tissue" & vbCr
    strEGFRDesc = strEGFRDesc & "2. Real-time PCR with DNA" & vbCr
    strEGFRDesc = strEGFRDesc & "3. cobas z480 Analyzer" & vbCr
    strEGFRDesc = strEGFRDesc & "4. Anaysis of Data" & vbCr
     
    strEGFRCmnt = ""
    strEGFRCmnt = strEGFRCmnt & "<Comment>" & vbCr
    strEGFRCmnt = strEGFRCmnt & "EGFR(Epidermal growth factor receptor)의 tyrosine kinase domain의 돌연변이는 비소세포성 폐암(Non-small cell lung cancer)에서 EGR-TKIs(EGFR tyrosine kinase inhibitor)에 대한 반응과 연관되어 있다고 연구 보고되어 있습니다. 본 검사는 EGFR 유전자의 Exon 18번 ~ 21번에 대하여 실시간 중합효소 연쇄 반응을 통하여 돌연변이의 유무를 확인하는 검사입니다."
    
    
    
    
    strKRASResultN = ""
    strKRASResultN = strKRASResultN & "<분석 결과>" & vbCr
    strKRASResultN = strKRASResultN & "상기환자의 K-ras gene의 codon 12/13, 61에 대해 검사한 결과 돌연변이가 검출되지 않았습니다." & vbCr
    
    strKRASDesc = ""
    strKRASDesc = strKRASDesc & "<분석 방법>" & vbCr
    strKRASDesc = strKRASDesc & "1. DNA extraction from sample tissue" & vbCr
    strKRASDesc = strKRASDesc & "2. Real-time PCR with DNA" & vbCr
    strKRASDesc = strKRASDesc & "3. cobas z480 Analyzer" & vbCr
    strKRASDesc = strKRASDesc & "4. Anaysis of Data" & vbCr
     
    strKRASCmnt = ""
    strKRASCmnt = strKRASCmnt & "<Comment>" & vbCr
    strKRASCmnt = strKRASCmnt & "Ras gene은 신호전달체계의 중요한 조절인자로 알려져 있고, Ras gene의 point mutation에 의해 단일 핵산이 대치되면 지속적인 신호전달체계를 활성화시켜 세포의 성장과 분화를 지속시켜 종양을 발생시키는 것으로 연구, 보고되어 있습니다. 본 검사는 KRAS 유전자의 codon 12/13번과 codon 61번 대하여 실시간 중합효소 연쇄 반응을 통하여 돌연변이 유무를 확인하는 검사입니다."
    
    Select Case strChannel
        Case "08EGFR":      GetRemark = ""
                            GetRemark = GetRemark & strEGFRResultN & vbCr
                            GetRemark = GetRemark & strEGFRDesc & vbCr
                            GetRemark = GetRemark & strEGFRCmnt
                            
        Case "08EGFR00":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 EGFR gene의 exon 18~21에 대해 검사한 결과 " & strResult & " 이 검출되었습니다."
                            GetRemark = GetRemark & strEGFRDesc & vbCr
                            GetRemark = GetRemark & strEGFRCmnt
        
        Case "08EGFR01":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 EGFR gene의 exon 18~21에 대해 검사한 결과 " & strResult & " 이 검출되었습니다."
                            GetRemark = GetRemark & strEGFRDesc & vbCr
                            GetRemark = GetRemark & strEGFRCmnt
        
        Case "08EGFR07":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 EGFR gene의 exon 18~21에 대해 검사한 결과 " & strResult & " 이 검출되었습니다."
                            GetRemark = GetRemark & strEGFRDesc & vbCr
                            GetRemark = GetRemark & strEGFRCmnt
                            
        Case "08EGFR03":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 EGFR gene의 exon 18~21에 대해 검사한 결과 " & strResult & " 이 검출되었습니다."
                            GetRemark = GetRemark & strEGFRDesc & vbCr
                            GetRemark = GetRemark & strEGFRCmnt
        
        Case "07KRAS":      GetRemark = ""
                            GetRemark = GetRemark & strKRASResultN & vbCr
                            GetRemark = GetRemark & strKRASDesc & vbCr
                            GetRemark = GetRemark & strKRASCmnt
                            
        Case "07KRAS01":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 K-ras gene의 codon 12/13, 61에 대해 검사한 결과 " & strResult & " 의 돌연변이가 검출되었습니다."
                            GetRemark = GetRemark & strKRASDesc & vbCr
                            GetRemark = GetRemark & strKRASCmnt
                            
        Case "07KRAS02":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 K-ras gene의 codon 12/13, 61에 대해 검사한 결과 " & strResult & " 의 돌연변이가 검출되었습니다."
                            GetRemark = GetRemark & strKRASDesc & vbCr
                            GetRemark = GetRemark & strKRASCmnt
                            
        Case "07KRAS03":    GetRemark = ""
                            GetRemark = GetRemark & "상기환자의 K-ras gene의 codon 12/13, 61에 대해 검사한 결과 " & strResult & " 의 돌연변이가 검출되었습니다."
                            GetRemark = GetRemark & strKRASDesc & vbCr
                            GetRemark = GetRemark & strKRASCmnt
    End Select
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_GESUS(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
...
'      -- 테이블 사용
          SQL = "SELECT DISTINCT DIAGDATE as 접수일자, PATHNO as 바코드번호, PATNO as 내원번호, MEDDEPT as ITEM " & vbCr
    SQL = SQL & vbCrLf & "  FROM SPXMSTDT " & vbCr
    SQL = SQL & vbCrLf & " WHERE PATHNO = '" & sBarcode & "'"
'    SQL = SQL & vbCrLf & "   AND MEDDEPT IN (" & gAllExam & ") " & vbCr
    If frmInterface.chkSaveAll.Value = "0" Then
        SQL = SQL & vbCrLf & "   AND (RSLTTEXT = '' OR RSLTTEXT IS NULL) " & vbLf
    End If
    SQL = SQL & "  ORDER BY DIAGDATE "
