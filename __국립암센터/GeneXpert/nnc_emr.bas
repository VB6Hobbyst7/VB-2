...
'''    Save_Raw_Data "[asProc]" & asProc
'''    Save_Raw_Data "[asSpcno]" & asSpcno
    
    sParam = Select_Param(asProc, asSpcno)
    'Call SaveXML_Data(sParam)
'''    Save_Raw_Data "[Param]" & sParam
    
    sRetStr = Online_XML_Qry(asProc, sParam)
    
'''    Save_Raw_Data "[sRetStr]" & sRetStr
...
Public Function Online_XML_Qry(ByVal asStrDiv As String, ByVal asParam As String) As String
    ...
    Save_Raw_Data "[Use Proc => " & strDiv & " ]" & sParam
    send = oSOAP.wsLISInterface(strDiv, sParam)
    Save_Raw_Data "[Return Proc => " & strDiv & " ]" & send
    Online_XML_Qry = send
    Set oSOAP = Nothing
    DoEvents
    Exit Function
...
ErrHandle:
    If oSOAP.FaultString <> "" Then
        ...
        Save_Raw_Data Format(Time, "hh:nn:ss") & "[SOAP]" & oSOAP.FaultString & vbCrLf & oSOAP.Detail & vbCrLf
    End If
    If Trim(Err.Description) <> "" Then
        Debug.Print Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf
        Save_Raw_Data Format(Time, "hh:nn:ss") & "[ERROR]" & Err.Description & vbCrLf
    End If
End Function
...
'''                 "</Table>"
    ...
    Save_Raw_Data "[Select_Param]" & Select_Param
