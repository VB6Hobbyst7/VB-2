...
Begin VB.Form frmIISRapidLab744 
   BackColor       =   &H00DBE6E6&
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISRapidLab744.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISRapidLab744.frx":04F2
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISRapidLab744.frx":0D42
      TextTip         =   2
   End
   ...
   End
End
Attribute VB_Name = "frmIISRapidLab744"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISRapidLab744.frm
'   작성자  : 오세원
'   내  용  : RapidLab 744 장비폼
'   버  전  :
'   병  원  :
'-----------------------------------------------------------------------------'
...
Private Sub EditRcvData()
    ...
    Dim objAccInfo   As clsIISIntInfo    '인터페이스 검체정보 클래스
    ...
    Dim strIntName   As String   '장비기준 검사명
    ...
    Dim i            As Long
    Dim j            As Long
    Dim mIntLibBuff  As Variant
    
    strRcvBuf = mIntLib.Buffers(1).Buffers
    strRcvBuf = Replace(strRcvBuf, vbLf, "")
'    strRcvBuf = Replace(strRcvBuf, vbLf, "")
    mIntLibBuff = Split(strRcvBuf, vbCr)
    strIDRecord = Trim(mIntLibBuff(2))
    
    'MsgBox strIDRecord
    
    If strIDRecord = "SAMPLE RESULTS" Then
        'MsgBox "1"
        '## WorkNo 조회
        'MsgBox mIntLibBuff(7)
        Pos1 = InStr(mIntLibBuff(7), "#")
        If Pos1 > 0 Then
            strWorkNo = Mid(mIntLibBuff(7), Pos1 + 1)
        Else
            '## NOTE: WorkNo가 전송되지 않은 에러처리
            Exit Sub
        End If
        
'MsgBox "strWorkNo:" & strWorkNo
        ...
        Else
            If optSeq(0).Value = True Then
                For i = 1 To tblReady.DataRowCnt
                    ...
                    If CStr(vWorkNo) = Val(strWorkNo) Then
       ...
       ' MsgBox strBarNo
        ...
        '## 결과저장
        For i = 1 To 3
            strIntBase = Trim(Mid(mIntLibBuff(9 + i), 1, 3))
            strIntName = Mid(strIntBase, 1, Len(strIntBase) - 1)
            strIntResult = Trim(Mid(mIntLibBuff(9 + i), 4, 6))
...
'            MsgBox strIntResult
            ...
            Call objIntInfo.IntResults.Add(strIntBase, strIntName, strIntResult, strResult, strWorkNo)
        Next
...
'                'na
'                Pos1 = 0:   Pos2 = 0
'                Pos1 = InStr(mIntLibBuff(10 + i), strIntBase)
'                If Pos1 > 0 Then
'                    strIntResult = Trim(Mid(mIntLibBuff(10 + i), 4, 6))
'                    strResult = strIntResult
'
'                    Call objIntInfo.IntResults.Add(strIntBase, objIntNm.IntNm, strIntResult, strResult, strWorkNo)
'                    i = 0
'                    strTemp2 = strIntBase
'                   ' i = i + 1
'                Else
...
Private Sub SetComplete2(ByVal pAccInfo As clsIISAccInfo, ByVal strSpcPos As String)
    ...
    Dim i           As Long
'    Dim objIntInfo   As clsIISIntInfo
...
'For Each objIntInfo In pAccInfo.IISInfos
'    strSpcPos = objIntInfo.SpcPos
'Next
        .Col = TCompleteEnum.ccNo:      .Text = pAccInfo.SpcPos      'strSpcPos
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISRapidLab744", "GetEqpComm", strErrMsg, Now
