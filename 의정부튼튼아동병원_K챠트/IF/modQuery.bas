...
'                            SetText frmWorkList.spdWork, Trim(RS.Fields("검체명")) & "", .MaxRows, colSPCNM
         ...
         Case "KCHART"
            ...
            SQL = SQL & "  J.접수일자 AS 접수일자," & vbCrLf
            SQL = SQL & "  L.검체번호 AS 바코드번호," & vbCrLf
            SQL = SQL & "  A.챠트번호 AS 챠트번호, " & vbCrLf
            SQL = SQL & "  J.접수번호 AS 내원번호," & vbCrLf
            SQL = SQL & "  '입원' AS 입외, " & vbCrLf
            SQL = SQL & "  L.진료검사ID AS R, " & vbCrLf
            SQL = SQL & "  L.진료지원ID AS P, " & vbCrLf
            SQL = SQL & "  A.환자이름 AS 이름, " & vbCrLf
            SQL = SQL & "  A.환자성별 AS 성별, " & vbCrLf
            SQL = SQL & "  A.환자나이  AS 나이 " & vbCrLf
'            SQL = SQL & "  L.처방코드 + L.서브코드 AS ITEM " & vbCrLf
            ...
            SQL = SQL & "  Where J.접수일자 Between '000000" & pFrom & "' and '" & pTo & "235959'" & vbCrLf
                ...
                Do Until RS.EOF
                    ...
                    With frmWorkList.spdWork
                        ...
                        If blnSame = False Then
                            ...
                            SetText frmWorkList.spdWork, Trim(RS.Fields("R")) & "", .MaxRows, colKEY1     '진료검사ID
                            SetText frmWorkList.spdWork, Trim(RS.Fields("P")) & "", .MaxRows, colKEY2      '진료지원ID
...
'            SQL = SQL & ", L.LABSMPNAM as 검체명"
        ...
        Case "KCHART"
            ...
            SQL = SQL & "  INNER JOIN TB_진료지원 J ON (L.진료지원ID = J.진료지원ID) " & vbCrLf
            SQL = SQL & "  INNER JOIN TB_진료일반 A ON (J.진료일자   = A.진료일자 AND J.챠트번호 = A.챠트번호 AND J.진료번호 = A.진료번호) " & vbCrLf
           ' SQL = SQL & "  Where J.접수일자 Between '" & pFrDt & "000000' and '" & pToDt & "235959'" & vbCrLf
            SQL = SQL & "  Where L.검체번호 = '" & strBarcode & "'" & vbCrLf
'            SQL = SQL & "    AND L.검사종류 = '" & gHOSP.LABCD & "'" & vbCrLf
            SQL = SQL & "    AND L.검사상태 < 5 " & vbCrLf
            SQL = SQL & "    AND L.처방코드 + L.서브코드 IN (" & gAllTestCd & ")" & vbCrLf
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        SetText SPD, "1", .MaxRows, colCHECKBOX
                        SetText SPD, Trim(RS.Fields("접수일자")) & "", asRow, colHOSPDATE
                        SetText SPD, Trim(RS.Fields("바코드번호")) & "", asRow, colBARCODE
                        SetText SPD, Trim(RS.Fields("챠트번호")) & "", asRow, colCHARTNO
                        SetText SPD, Trim(RS.Fields("내원번호")) & "", asRow, colPID
                        ...
                        'SetText SPD, Trim(RS.Fields("R")) & "", asRow, colKEY1     '진료검사ID
                        'SetText SPD, Trim(RS.Fields("P")) & "", asRow, colKEY2      '진료지원ID

                       ' SetText SPD, Trim(RS.Fields("R")) & "", asRow, colRACKNO     '진료검사ID
                       ' SetText SPD, Trim(RS.Fields("P")) & "", asRow, colPOSNO      '진료지원ID

'                        mResult.RefL = ""
'                        mResult.RefH = ""
'                        If Trim(RS.Fields("성별")) & "" = "M" Then
'                            If Trim(RS.Fields("참고치0")) & "" <> "" Then
'                                mResult.RefL = Trim(RS.Fields("참고치0")) & ""
'                                mResult.RefH = Trim(RS.Fields("참고치1")) & ""
'                            End If
'                        Else
'                            If Trim(RS.Fields("성별")) & "" = "F" Then
'                                If Trim(RS.Fields("참고치2")) & "" <> "" Then
'                                    mResult.RefL = Trim(RS.Fields("참고치2")) & ""
'                                    mResult.RefH = Trim(RS.Fields("참고치3")) & ""
'                                Else
'                                    mResult.RefL = Trim(RS.Fields("참고치0")) & ""
'                                    mResult.RefH = Trim(RS.Fields("참고치1")) & ""
'                                End If
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    With frmMain
        ...
        If DBExec(AdoCn_Local, SQL) Then
            ...
            SQL = SQL & ",'" & mResult.Key1 & "'"
            SQL = SQL & ",'" & mResult.Key2 & "'"
