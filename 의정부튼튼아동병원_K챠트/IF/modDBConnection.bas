...
'MS JetDB Make
Global AdoCn_Local_Send     As ADODB.Connection
Global AdoRs_Local_Send     As ADODB.Recordset
Global AdoCmd_Local_Send    As ADODB.Command
Global AdoParm_Local_Send   As ADODB.Parameter
...
'-- 전송용 MDB 연결
Public Function DbConnect_Local_Send() As Boolean
    Dim DB_Name         As String
    Dim UserName        As String
    Dim Password        As String
    Dim blnWinNTAuth    As Boolean

    Set AdoCn_Local_Send = New ADODB.Connection

On Error GoTo ConnectError

    DB_Name = "C:\INTERFACE\WORKLIST.mdb"
    UserName = "admin"
    Password = "admin"

    If (DB_Name = "") Or (UserName = "") Then
        DbConnect_Local_Send = False
        Set AdoCn_Local = Nothing
        Exit Function
    End If
    Dim i As Integer
