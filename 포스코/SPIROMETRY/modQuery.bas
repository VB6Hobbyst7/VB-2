        ...
        With frmMain.spdTest
            ...
            Do Until AdoRs_Local.EOF
               ...
               ' Call SetText(frmMain.spdTest, IIf(AdoRs_Local.Fields("QCTemp").Value & "" = "1", "사용", "미사용"), intRow, colLUseResSpec)
                ...
                gArrEQP(intRow, 19) = ""    '델타구분으로 사용 (ACK : DELTAGBN)
                gArrEQP(intRow, 20) = ""    '이전결과로  사용 (ACK : BEFORERESULT)
...
Public Sub GetTestList_Title()
    ...
    SQL = SQL & "  FROM EQPMASTER " & vbCr
    SQL = SQL & " WHERE EQUIPCD = '" & gHOSP.MACHCD & "'" & vbCr
    'SQL = SQL & "   AND (RIGHT(TESTNAME,2) <> 'Pr' AND RIGHT(TESTNAME,1) <> '%' AND RIGHT(TESTNAME,2) <> 'Bs')" & vbCr
    SQL = SQL & "   AND (RIGHT(TESTNAME,2) <> 'Pr' AND RIGHT(TESTNAME,2) <> 'Bs')" & vbCr
    SQL = SQL & " ORDER BY SEQNO "
    ...
    Set AdoRs_Local = AdoCn_Local.Execute(SQL, , 1)
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        ReDim Preserve gArrEQP_Title(AdoRs_Local.RecordCount + 2, 20)
        Do Until AdoRs_Local.EOF
            intRow = intRow + 1
            gArrEQP_Title(intRow, 1) = AdoRs_Local.Fields("SEQNO").Value & ""
            gArrEQP_Title(intRow, 2) = AdoRs_Local.Fields("TESTCODE").Value & ""
            gArrEQP_Title(intRow, 3) = AdoRs_Local.Fields("SENDCHANNEL").Value & ""
            gArrEQP_Title(intRow, 4) = AdoRs_Local.Fields("RSLTCHANNEL").Value & ""
            gArrEQP_Title(intRow, 5) = AdoRs_Local.Fields("ABBRNAME").Value & ""
            gArrEQP_Title(intRow, 6) = AdoRs_Local.Fields("RESPREC").Value & ""
            gArrEQP_Title(intRow, 7) = AdoRs_Local.Fields("REFLOW").Value & ""
            gArrEQP_Title(intRow, 8) = AdoRs_Local.Fields("REFHIGH").Value & ""
            gArrEQP_Title(intRow, 9) = AdoRs_Local.Fields("RESULTTYPE").Value & ""
            gArrEQP_Title(intRow, 10) = AdoRs_Local.Fields("CUTOFFUSE").Value & ""
            gArrEQP_Title(intRow, 11) = AdoRs_Local.Fields("COLCOMP").Value & "" & AdoRs_Local.Fields("COLIN").Value
            gArrEQP_Title(intRow, 12) = AdoRs_Local.Fields("COLOUT").Value & ""
            gArrEQP_Title(intRow, 13) = AdoRs_Local.Fields("COMOUT").Value & ""
            gArrEQP_Title(intRow, 14) = AdoRs_Local.Fields("COHCOMP").Value & "" & AdoRs_Local.Fields("COHIN").Value
            gArrEQP_Title(intRow, 15) = AdoRs_Local.Fields("COHOUT").Value & ""
            gArrEQP_Title(intRow, 16) = ""    '처방코드로 사용(ACK : SPECIMENCD)
            gArrEQP_Title(intRow, 17) = ""    'SUB코드로 사용 (ACK : LABSEQ)
            gArrEQP_Title(intRow, 18) = ""    'SUB코드로 사용 (ACK : PARTGBN)
            
            gArrEQP_Title(intRow, 19) = ""    '델타구분으로 사용 (ACK : DELTAGBN)
            gArrEQP_Title(intRow, 20) = ""    '이전결과로  사용 (ACK : BEFORERESULT)
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
'        Case "ACK"                          'ACK LIS
'                Call GetWorkList_ACK(pFrom, pTo, SPD)
'
'        Case "AMIS"                         '아미스
...
On Error GoTo RST
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT"
    SQL = SQL & "       a.per_gumjin_date   AS HOSPDATE                             " & vbCr
    SQL = SQL & "     , a.per_name          AS PNAME                                " & vbCr
    SQL = SQL & "     , a.per_jikbun        AS BARCODE                              " & vbCr
    SQL = SQL & "     , a.per_jupsu_date                                            " & vbCr
    SQL = SQL & "     , a.per_jupsu_time                                            " & vbCr
    SQL = SQL & "     , COUNT(b.meditem)    AS CNT                                  " & vbCr
    SQL = SQL & "  FROM TB_JUPSU a                                                  " & vbCr
    SQL = SQL & "     , TB_JUPSU_ITEM b                                             " & vbCr
    SQL = SQL & "     , BAG_CODEVALUE c                                             " & vbCr
    SQL = SQL & " WHERE a.com_code = '1'                                            " & vbCr
    SQL = SQL & "   AND a.per_gumjin_date BETWEEN '" & pFrom & "' AND '" & pTo & "' " & vbCr
    SQL = SQL & "   AND a.per_jupsu_date        = b.per_jupsu_date                  " & vbCr
    SQL = SQL & "   AND a.per_ssn               = b.per_ssn                         " & vbCr
    SQL = SQL & "   AND a.per_gumjin_date       = b.per_gumjin_date                 " & vbCr
    SQL = SQL & "   AND b.meditem               IN ( " & gAllTestCd & ")            " & vbCr
    SQL = SQL & "   AND 'P00' || a.jupsu_gubun  = c.gumsacode                       " & vbCr
    SQL = SQL & "   AND c.codegubun = 'P00'                                         " & vbCr
    SQL = SQL & "   AND (b.RESULT = '' OR b.RESULT IS NULL)                         " & vbCr
    SQL = SQL & " GROUP BY a.per_gumjin_date, a.per_name, a.per_jikbun, a.per_jupsu_date, a.per_jupsu_time " & vbCr
    SQL = SQL & " ORDER BY a.per_jupsu_date, a.per_jupsu_time                       " & vbCr
...
Public Sub GetWorkList_ACK(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo RST
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       M.LABDATE   as HOSPDATE, M.PARTGBN, M.LABSEQ, M.SPECIMENCD" & vbCr
    SQL = SQL & "     , M.NAME      as PNAME" & vbCr
    SQL = SQL & "     , M.REGNO     as PID" & vbCr
    SQL = SQL & "     , M.AGE       as AGE" & vbCr
    SQL = SQL & "     , M.SEX       as Sex" & vbCr
    SQL = SQL & "     , M.IDLEFT + M.IDRIGHT AS PJUMIN, M.DEPTCD, M.JUBSUGBN" & vbCr
    SQL = SQL & "     , M.MEDICALREMARK as BARCODE" & vbCr
    ...
    'SQL = SQL & " WHERE M.MEDICALREMARK = '" & strBarcode & "'" & vbCr
    SQL = SQL & " WHERE M.LABDATE  BETWEEN '" & pFrom & "' and '" & pTo & "'    " & vbCr
    ...
    SQL = SQL & "   And R.TESTITEMSEQ   IN (" & gAllTestCd & ")" & vbCr
...
Public Sub GetStatistics_ACK(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread, ByVal sGubun As String)
    Dim RS          As ADODB.Recordset
    Dim strFromMon  As String
    Dim strToMon    As String
    Dim intRow      As Integer
...
On Error GoTo RST
    ...
    If sGubun = 1 Then
        SQL = ""
        SQL = SQL & "select r.labdate as stadate,r.testitemseq,t.printnm,count(r.result) as cnt"
        SQL = SQL & "  from c_result r, testitem t"
        SQL = SQL & " where r.labdate between '" & pFrom & "' and '" & pTo & "'"
        SQL = SQL & "   and r.machinecd = '!'"
        SQL = SQL & "   and r.result <> ''"
        SQL = SQL & "   and r.partgbn = '01'"
        SQL = SQL & "   and r.SPECIMENCD = '002'"
        SQL = SQL & "   and t.partcd = 'C'"
        SQL = SQL & "   and r.partgbn = t.partgbn"
        SQL = SQL & "   and r.SPECIMENCD = t.SPECIMENCD"
        SQL = SQL & "   and r.testitemseq = t.testitemseq"
        SQL = SQL & " group by r.labdate, t.regservercd,t.printnm,r.testitemseq"
        'SQL = SQL & "  having substring(r.labdate,1,6) = '" & strFromMon & "'"
        SQL = SQL & " order by t.printnm"
    
    ElseIf sGubun = 2 Then
'        SQL = ""
'        SQL = SQL & "select labdate,testitemseq,count(*) from c_result"
'        SQL = SQL & " where labdate BETWEEN '" & pFrom & "' and '" & pTo & "'" & vbCr
'        SQL = SQL & "   and machinecd = '!'" & vbCr
'        SQL = SQL & "   and result <> ''" & vbCr
'        SQL = SQL & " group by labdate,testitemseq" & vbCr
        
        strFromMon = Mid(pFrom, 1, 6) & "01"
        strToMon = Mid(pTo, 1, 6) & "31"
        
        SQL = ""
        SQL = SQL & "select substring(r.labdate,1,6) as stadate,r.testitemseq,t.printnm,count(r.result) as cnt"
        SQL = SQL & "  from c_result r, testitem t"
        SQL = SQL & " where r.labdate between '" & strFromMon & "' and '" & strToMon & "'"
        SQL = SQL & "   and r.machinecd = '!'"
        SQL = SQL & "   and r.result <> ''"
        SQL = SQL & "   and r.partgbn = '01'"
        SQL = SQL & "   and r.SPECIMENCD = '002'"
        ...
        SQL = SQL & "   and r.SPECIMENCD = t.SPECIMENCD"
        SQL = SQL & "   and r.testitemseq = t.testitemseq"
        SQL = SQL & " group by substring(r.labdate,1,6), t.regservercd,t.printnm,r.testitemseq"
        SQL = SQL & "  having substring(r.labdate,1,6) = '" & strFromMon & "'"
        SQL = SQL & " order by t.printnm"
        ...
        Do Until RS.EOF
            With SPD
                ...
                If sGubun = 1 Then  '일계
                    '-- Header
                    'SetText SPD, "No", 0, 1
                    'SetText SPD, "검사항목", 0, 2
                    If intRow = 1 Then
                        intCol = intCol + 1
                        .MaxCols = intCol
                            SetText SPD, Trim(RS.Fields("STADATE")) & "", 0, intRow
                    Else
                        blnSame = False
                        For iCnt = 3 To .MaxCols
                            If Trim(RS.Fields("STADATE")) & "" = GetText(SPD, 0, iCnt) Then
                                blnSame = True
                                Exit For
                            End If
                        Next
                    ...
                    End If
                    '-- Detail
                    SetText SPD, intRow, intRow, 1
                    SetText SPD, Trim(RS.Fields("PRINTNM")) & "", intRow, 2
...
Public Sub GetResultList(ByVal pFrom As String, ByVal pTo As String, ByVal pDateType As Integer, ByVal pOpt As Integer)
    ...
    '-- 접수일자
    Else
        SQL = SQL & " WHERE MID(HOSPDATE,1,8) Between '" & pFrom & "' AND '" & pTo & "'" & vbCr
    End If
...
'                    End If
    ...
    frmMain.spdROrder.RowHeight(-1) = 20
...
'-- 장비결과 조회
Public Sub SetDeleteList(ByVal pFrom As String, ByVal pTo As String)
    ...
    SQL = ""
    SQL = SQL & "DELETE FROM PATRESULT_BEST " & vbCrLf
    SQL = SQL & " WHERE MID(EXAMDATE,1,8) Between '" & pFrom & "' AND '" & pTo & "'" & vbCr
...
'            End If
        ...
        Case "ONITGUM"
            SQL = ""
            SQL = SQL & "SELECT MEDITEM      AS ITEM                    " & vbCrLf
            SQL = SQL & "  FROM TB_JUPSU_ITEM                           " & vbCrLf
            SQL = SQL & " WHERE PER_GUMJIN_DATE = '" & strRegDate & "'  " & vbCrLf
            SQL = SQL & "   AND PER_JUPSU_DATE  = '" & strRegDate & "'  " & vbCrLf
            '채용검진
            If frmMain.chkCYGJ.Value = "1" Then
                SQL = SQL & "   AND blood_no    = '" & strBarcode & "'  " & vbCrLf
            Else
                SQL = SQL & "   AND per_jikbun  = '" & strBarcode & "'  " & vbCrLf
            End If
            SQL = SQL & "   AND MEDITEM         IN (" & gAllTestCd & ") " & vbCrLf
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
'        Case "ACK"
'                Call GetSampleInfo_ACK(asRow, SPD)
'
'        Case "AMIS"
...
On Error GoTo DBErr
    ...
    strPatID = Trim(GetText(SPD, asRow, colPID))
    strChartNo = Trim(GetText(SPD, asRow, colCHARTNO))
    strDate = Format(frmMain.dtpCYGJ.Value, "yyyymmdd")
    ...
    If frmMain.chkCYGJ.Value = "1" Then
        SQL = ""
        SQL = SQL & "SELECT a.per_name              AS PNAME                            " & vbCrLf
        SQL = SQL & "     , a.blood_no              AS BARCODE                          " & vbCrLf
        SQL = SQL & "     , b.meditem               AS ITEM                             " & vbCrLf
        SQL = SQL & "     , a.per_jupsu_date            " & vbCrLf
        SQL = SQL & "     , a.per_ssn                   " & vbCrLf
        SQL = SQL & "     , a.group_no                  " & vbCrLf
'        SQL = SQL & "     , seq_no                      " & vbCrLf
        SQL = SQL & "     , MAX(a.per_gumjin_date)  AS HOSPDATE                         " & vbCrLf
        SQL = SQL & "  FROM TB_JUPSU a                                                  " & vbCrLf
        SQL = SQL & "     , TB_JUPSU_ITEM b                                             " & vbCrLf
        SQL = SQL & "     , BAG_CODEVALUE c                                             " & vbCrLf
        SQL = SQL & " WHERE a.com_code = '1'                                            " & vbCrLf
        SQL = SQL & "   AND a.blood_no              = '" & strBarcode & "'              " & vbCrLf
        SQL = SQL & "   AND a.per_gumjin_date       = '" & strDate & "'                 " & vbCrLf
        SQL = SQL & "   AND a.per_ssn               = b.per_ssn                         " & vbCrLf
        SQL = SQL & "   AND a.per_jupsu_date        = b.per_jupsu_date                  " & vbCrLf
        SQL = SQL & "   AND a.per_ssn               = b.per_ssn                         " & vbCrLf
        SQL = SQL & "   AND a.per_gumjin_date       = b.per_gumjin_date                 " & vbCrLf
        SQL = SQL & "   AND b.meditem               IN ( " & gAllTestCd & ")            " & vbCrLf
        SQL = SQL & "   AND 'P00' || a.jupsu_gubun  = c.gumsacode                       " & vbCrLf
        SQL = SQL & "   AND c.codegubun = 'P00'                                         " & vbCrLf
        SQL = SQL & " GROUP BY a.per_name,  a.blood_no, b.meditem,a.per_jupsu_date,a.per_ssn,a.group_no  " & vbCrLf
    Else
        SQL = ""
        SQL = SQL & "SELECT a.per_name              AS PNAME                            " & vbCrLf
        SQL = SQL & "     , a.per_jikbun            AS BARCODE                          " & vbCrLf
        SQL = SQL & "     , b.meditem               AS ITEM                             " & vbCrLf
        SQL = SQL & "     , MAX(a.per_gumjin_date)  AS HOSPDATE                         " & vbCrLf
        SQL = SQL & "  FROM TB_JUPSU a                                                  " & vbCrLf
        SQL = SQL & "     , TB_JUPSU_ITEM b                                             " & vbCrLf
        SQL = SQL & "     , BAG_CODEVALUE c                                             " & vbCrLf
        SQL = SQL & " WHERE a.com_code = '1'                                            " & vbCrLf
        SQL = SQL & "   AND a.per_jikbun            = '" & strBarcode & "'              " & vbCrLf
        SQL = SQL & "   AND a.per_ssn               = b.per_ssn                         " & vbCrLf
        SQL = SQL & "   AND a.per_jupsu_date        = b.per_jupsu_date                  " & vbCrLf
        SQL = SQL & "   AND a.per_ssn               = b.per_ssn                         " & vbCrLf
        SQL = SQL & "   AND a.per_gumjin_date       = b.per_gumjin_date                 " & vbCrLf
        ...
        SQL = SQL & "   AND 'P00' || a.jupsu_gubun  = c.gumsacode                       " & vbCrLf
        SQL = SQL & "   AND c.codegubun = 'P00'                                         " & vbCrLf
        SQL = SQL & " GROUP BY a.per_name,  a.per_jikbun, b.meditem                     " & vbCrLf
    End If
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                '채용검진
                If frmMain.chkCYGJ.Value = "1" Then
                    strKey = Trim(RS.Fields("per_jupsu_date")) & "|" & Trim(RS.Fields("per_ssn")) & "|" & Trim(RS.Fields("group_no")) & "|"
                    SetText SPD, strKey, asRow, colKEY1
                End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_ACK(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    SQL = SQL & "     , M.RESULTENDYN, R.SUBMCD, R.TESTITEMSEQ as ITEM, R.RESULTDATE, R.RESULTTIME, R.ROUTINECD, R.MACHINECD, R.RESULT, R.DELTAGBN, R.REFGBN, R.PANICGBN, R.DELTAMARK, R.REFMARK, R.PANICMARK, R.CMCODE " & vbCr
    ...
    SQL = SQL & "     , M.AGE       as AGE" & vbCr
    SQL = SQL & "     , M.SEX       as SEX" & vbCr
    SQL = SQL & "     , M.IDLEFT + M.IDRIGHT AS PJUMIN " & vbCr
    'SQL = SQL & "     , M.DEPTCD, M.JUBSUGBN" & vbCr
    SQL = SQL & "     , M.MEDICALREMARK as BARCODE" & vbCr
    SQL = SQL & "     , M.RESULTENDYN, R.SUBMCD " & vbCr
    SQL = SQL & "     , R.TESTITEMSEQ as ITEM " & vbCr
    
    SQL = SQL & "     , R.DELTAGBN  " & vbCr
    SQL = SQL & "     , R.BEFORERESULT " & vbCr
    
    'SQL = SQL & "     , R.RESULTDATE, R.RESULTTIME, R.ROUTINECD, R.MACHINECD, R.RESULT, R.DELTAGBN, R.REFGBN, R.PANICGBN, R.DELTAMARK, R.REFMARK, R.PANICMARK, R.CMCODE " & vbCr
    
    Select Case gHOSP.PARTCD
        Case "H": SQL = SQL & "  FROM H_JUBSU M, H_RESULT R " & vbCr   '혈액학
        Case "C": SQL = SQL & "  FROM C_JUBSU M, C_RESULT R " & vbCr   '생화학
        Case "J": SQL = SQL & "  FROM J_JUBSU M, J_RESULT R " & vbCr   '수탁
        Case "M": SQL = SQL & "  FROM M_JUBSU M, M_RESULT R " & vbCr   '미생물
        Case "S": SQL = SQL & "  FROM S_JUBSU M, S_RESULT R " & vbCr   '면역학
        Case "U": SQL = SQL & "  FROM U_JUBSU M, U_RESULT R " & vbCr   '뇨화학
    End Select
    ...
    SQL = SQL & "   AND M.SPECIMENCD    = R.SPECIMENCD" & vbCr
    SQL = SQL & "   And R.TESTITEMSEQ   IN (" & gAllTestCd & ")" & vbCr
    SQL = SQL & "   AND R.CONFIRM       <> 'Y' " & vbCr
    SQL = SQL & "   AND (R.RESULT = '' OR R.RESULT IS NULL) " & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                mResult.PARTGBN = Trim(RS.Fields("PARTGBN")) & ""
                mResult.SPECIMENCD = Trim(RS.Fields("SPECIMENCD")) & ""
                mResult.PAge = Trim(RS.Fields("AGE")) & ""
                ...
                Case "2", "4"
                    mResult.PSex = "F"
                Case Else
                    mResult.PSex = Trim(RS.Fields("SEX")) & ""
                End Select
                
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
                SetText SPD, Trim(RS.Fields("BARCODE")), asRow, colBARCODE
                SetText SPD, Trim(RS.Fields("LABSEQ")), asRow, colSEQNO
                SetText SPD, Trim(RS.Fields("PID")) & "", asRow, colPID
                SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                SetText SPD, mResult.PSex, asRow, colPSEX
                SetText SPD, mResult.PAge, asRow, colPAGE
                SetText SPD, Trim(RS.Fields("PJUMIN")) & "", asRow, colPJUMIN
                ...
                With mOrder
                    ...
                    .OrdDate = Trim(RS.Fields("HOSPDATE")) & ""
                End With
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        '-- SPECIMENCD
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("SPECIMENCD")) & ""
                        '-- LABSEQ
                        gArrEQP(intCol - colSTATE, 17) = Trim(RS.Fields("LABSEQ")) & ""
                        '-- PARTGBN
                        gArrEQP(intCol - colSTATE, 18) = Trim(RS.Fields("PARTGBN")) & ""
                        
                        '-- DELTAGBN
                        gArrEQP(intCol - colSTATE, 19) = Trim(RS.Fields("DELTAGBN")) & ""
                        '-- BEFORERESULT
                        gArrEQP(intCol - colSTATE, 20) = Trim(RS.Fields("BEFORERESULT")) & ""
...
'-- 검사자 정보 가져오기
Function GetTestComplete_ACK(ByVal pBarcode As String) As Boolean
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = SQL & " WHERE M.MEDICALREMARK = '" & pBarcode & "'" & vbCr
    SQL = SQL & "   AND M.LABDATE       = R.LABDATE         " & vbCr
    SQL = SQL & "   AND M.PARTGBN       = R.PARTGBN         " & vbCr
    SQL = SQL & "   AND M.LABSEQ        = R.LABSEQ          " & vbCr
    SQL = SQL & "   AND M.SPECIMENCD    = R.SPECIMENCD      " & vbCr
...
Public Function getMaxTestNum(ByVal strDate As String) As Long
    ...
    SQL = SQL & " WHERE MID(EXAMDATE,1,8) = '" & Mid(strDate, 1, 8) & "' " & vbCrLf
...
'-- 오늘 검사한 날짜의 Max + 1 번호를 가져온다 (바코드번호)
Public Function getMaxTestNum_Bar(ByVal strDate As String, ByVal strBarCd As String) As Long

    getMaxTestNum_Bar = 1
    
          SQL = "SELECT MAX(SAVESEQ) as SEQ FROM PATRESULT  "
    SQL = SQL & " WHERE MID(EXAMDATE,1,8) = '" & Mid(strDate, 1, 8) & "' " & vbCrLf
    SQL = SQL & "   AND BARCODE    = '" & strBarCd & "' " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        If Trim(RS.Fields("SEQ") & "") = "" Then
            getMaxTestNum_Bar = 1
        Else
            getMaxTestNum_Bar = Trim(RS.Fields("SEQ")) + 1
        End If
    Else
        getMaxTestNum_Bar = 1
    End If
    
    If getMaxTestNum_Bar >= 99999 Then
        getMaxTestNum_Bar = 99999
    End If
