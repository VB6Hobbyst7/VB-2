    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 서버저장
                SQL = ""
                SQL = SQL + "UPDATE TB_JUPSU_ITEM                           " & vbCr
                SQL = SQL + "   SET RESULT          = '" & sResult & "'     " & vbCr
                SQL = SQL + " WHERE PER_JUPSU_DATE  = '" & strDate & "'     " & vbCr
                SQL = SQL + "   AND PER_JIKBUN      = '" & strBarcode & "'  " & vbCr
                SQL = SQL + "   AND MEDITEM         = '" & strTestCd & "'   " & vbCr
...
'        Call SetSQLData("로컬결과조회", strExamDate & "," & strHospDate & "," & strBarcode)
