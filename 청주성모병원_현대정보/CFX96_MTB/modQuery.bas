        ...
        For i = 1 To UBound(varRcvData) Step 19
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, varRcvData(i + 6) & "", intRow, colSPECIMEN
...
On Error GoTo DBErr
    ...
    Call DisplayNode_Info(App.PATH & "\Xml\" & strXmlName)
...
'        Next
        ...
        'For i = 1 To UBound(varRcvData)
            With SPD
                ...
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, XmlSelect.PRCPDD, asRow, colHOSPDATE
                'SetText SPD, varRcvData(2) & "", asRow, colINOUT
                'SetText SPD, varRcvData(1) & "", asRow, colBARCODE
                SetText SPD, XmlSelect.PID, asRow, colPID
                SetText SPD, XmlSelect.PATNM, asRow, colPNAME
                SetText SPD, XmlSelect.SEX, asRow, colPSEX
                SetText SPD, XmlSelect.AGE, asRow, colPAGE
                SetText SPD, XmlSelect.SPCNM, asRow, colSPECIMEN
                ...
                '오더정보에 저장
                With mOrder
                    .BarNo = XmlSelect.BCNO
                    .PID = XmlSelect.PID
                    .PNAME = XmlSelect.PATNM
                    .Count = CStr(intTestCnt)
                    .NoOrder = False
                End With
                
                '-- 화면에 표시
                If Trim(XmlSelect.TESTCD) <> "" Then
                    varTstCode = Split(XmlSelect.TESTCD, "▦")
                    For j = 0 To UBound(varTstCode) '- 1
                        gPatOrdCd = gPatOrdCd & "'" & Trim(varTstCode(j)) & "',"
