...
Begin VB.Form frmMain 
   ...
   Begin VB.Frame fraHidden 
      ...
      Begin FPSpread.vaSpread spdComStatus 
         ...
         SpreadDesigner  =   "frmMain.frx":1102
         UserResize      =   0
         TextTip         =   2
         ScrollBarTrack  =   3
         ShowScrollTips  =   3
   ...
   Begin FPSpread.vaSpread spdOrder 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":1571
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   ...
   Begin VB.PictureBox picComm 
      ...
      Begin FPSpread.vaSpread spdExcel 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         SpreadDesigner  =   "frmMain.frx":350A
      End
   ...
   Begin FPSpread.vaSpread spdResult 
      ...
      ScrollBarExtMode=   -1  'True
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":69B4
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   ...
   Begin FPSpread.vaSpread spdWork 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":76D9
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
...
'''                    intCnt = intCnt + 1
'''
'''                    strAllStr = strAllStr & CStr(intCnt) & Space(5 - Len(CStr(intCnt))) & strBC & Space(20 - Len(CStr(intCnt))) & "CoV" & Space(147)
'''
'''                    'sSender:= sSender + PadRightStr(IntToStr(i),' ', 5) + PadRightStr(Bcd,' ', 20)+ PadRightStr(IfCd,' ', 150);
'''
'''                    '-- 진행상태(Order) 표시
'''                    Call SetText(spdOrder, "오더전송", intRow, colSTATE)
'''                    Call SetText(spdOrder, "0", intRow, colCHECKBOX)
'''
'''                    'strFileName = "nimbus_mtb.lis"
'''
'''                End If
'''            End If
'''        Next
...
'''    End If
    ...
    With spdOrder
        '-- OPEN
        strFileName = "LIS.nimbus"
        If Dir$(gComm.ORDPATH & "\" & strFileName, vbNormal) <> "" Then
            Kill gComm.ORDPATH & "\" & strFileName
        End If
        lngFIleNum = FreeFile
        Open gComm.ORDPATH & "\" & strFileName For Append As #lngFIleNum
        strCFXFile = ""
        ...
        For iCnt = 1 To spdOrder.MaxRows '+ 2
            'If iCnt = 48 Then
            '    Exit For
            'End If
            ...
            If GetText(spdOrder, iCnt, colCHECKBOX) = "1" Then
                ...
                strPNM = GetText(spdOrder, iCnt, colPNAME)
                strPID = GetText(spdOrder, iCnt, colCHARTNO)
                
                If iCnt = 1 Then
                    strCFXFile = strCFXFile & "Row,Column,*Target Name,*Sample Name,Sample No,Patient Id" & vbNewLine
                End If
                strCFXFile = strCFXFile & Chr(64 + j) & "," & K & ",," & strPNM & ",," & strBarno & "" & vbNewLine
                j = j + 1
                If j = 9 Then
                    j = 1
                    K = K + 1
                ...
                Call SetText(spdOrder, "오더전송", iCnt, colSTATE)
            End If
            
            If iCnt >= spdOrder.MaxRows Then
                strCFXFile = strCFXFile & Chr(64 + j) & "," & K & ",," & "NC" & ",," & "NC" & "" & vbNewLine
                j = j + 1
                If j = 9 Then
                    j = 1
                    K = K + 1
                    ...
                    End If
                End If
                strCFXFile = strCFXFile & Chr(64 + j) & "," & K & ",," & "PC" & ",," & "PC" & "" & vbNewLine
            End If
        Next
...
Private Sub FileRcvData_CFX96_COV19()
    Dim RS_L            As ADODB.Recordset
    '장비 수신 변수
    Dim strRcvBuf       As String   '수신한 Data
    Dim strType         As String   '수신한 Record Type
...
On Error GoTo ErrHandle
    ...
    With frmMain
        For intRow = 3 To spdExcel.MaxRows
            intSmpNo = 0
            intIntNo = 0
            intRstNo = 0
            For intCol = 1 To spdExcel.MaxCols
                ...
                If UCase(GetText(spdExcel, 1, intCol)) = "환자 ID" Then
                    intSmpNo = intCol
                End If
            ...
            If strBarno = "" Then
                strBarno = GetText(spdExcel, intRow, intSmpNo_NP)
            End If
            ...
            For intC = 6 To .spdExcel.MaxCols Step 2
                strIntBase = GetText(spdExcel, 2, intC)
                If InStr(strIntBase, "해석") > 0 Then
                    strIntBase = "COVID19"
                    strResult = GetText(spdExcel, intRow, intC)
                
                    'If strIntBase = "COVID19" Then
                        If strResult = "-" Then
                            strResult = "Negative"
                        '2021-04-29 : "SARS-CoV-2" 추가
                        ElseIf strResult = "+" Or strResult = "SARS-CoV-2" Then '"SARS-CoV-2 inconclusive" 'Invalid
                            strResult = "Positive"
                        ElseIf strResult = "Negative Control(-)" Or strResult = "Positive Control(+)" Then
                            strResult = strResult
                        ElseIf strResult = "SARS-CoV-2 inconclusive" Then
                            strResult = "inconclusive"
                        ElseIf strResult = "Invalid" Then
                            strResult = "Invalid"
                        Else
                            strResult = "미결정" 'Undecided
                        End If
                    'End If
                
                Else
                    strResult = GetText(spdExcel, intRow, intC + 1)
...
ErrHandle:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_FileRcvData_CFX96_COV19" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
