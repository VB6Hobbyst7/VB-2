...
'    If Right(m_BufferString, 1) = vbCr Then
        Debug.Print sBuffer
        
'        SCKRCV_DATA = sBuffer
        
    strTmp = Split(sBuffer, vbCr)
    
    For intpCnt = 0 To UBound(strTmp)
        Select Case Mid(strTmp(intpCnt), 2, 3)
        Case "MSH"
                strMSH = Trim(mGetP1(strTmp(intpCnt), 10, "|"))
                strDate = Trim(mGetP1(strTmp(intpCnt), 7, "|"))
        End Select
    Next
        
                       
'                       strSndBuffer = "MSH|^~\&|Mindray|BS-400|||20100608142704||ACK^R01|1|P|2.3.1||||0||ASCII|||" & vbCr
'MSH|^~\&|Mindray|BS-220|||20110103130741||ORU^R01|35|P|2.3.1||||0||ASCII|||
'                       strSndBuffer = Chr(11) & "MSH|^~\&|Mindray|BS-220|||20100608142704||ACK^R01|1|P|2.3.1||||0||ASCII|||" & Chr(13)
''                       strSndBuffer = Chr(11) & "MSH|^~\&|||Mindray|BS-220|20110103130741||ACK^R01|2|P|2.3.1|" & Chr(13)
'        strSndBuffer = strSndBuffer & "MSA|AA|1|Message accepted|||0|" & Chr(13)
''        strSndBuffer = strSndBuffer & "MSA|AE|2||||100|" & Chr(13)
'        strSndBuffer = strSndBuffer & Chr(28) & vbCr

'                       strSndBuffer = Chr(11) & "MSH|^~\&|LIS||||" & strDate & "||ACK^R01|" & strMSH & "|P|2.3.1||||||UNICODE" & Chr(10)
'        strSndBuffer = strSndBuffer & "MSA|AA|1" & Chr(10)
'        strSndBuffer = strSndBuffer & Chr(28) & vbCr
        
                       strSndBuffer = Chr(11) & "MSH|^~\&|Mindray|BS-400|||" & strDate & "||ACK^R01|" & strMSH & "|P|2.3.1||||0||ASCII|||" & Chr(13)
        strSndBuffer = strSndBuffer & "MSA|AA|" & strMSH & "|Message accepted|||0|" & Chr(13)
'        strSndBuffer = strSndBuffer & "" & vbCr
        strSndBuffer = strSndBuffer & Chr(28) & vbCr

''MSH|^~\&|LIS-Server||Mindray|BS-200|'+sNow+'||ACK^R01|'+MsgSeq+'|P|2.3.1||||0||ASCII|||'+CR+
'      //              'MSA|AA|'+MsgSeq+'|Message accepted|||0|'+ CR + EB + CR;
 

        
        ProcSendMessage strSndBuffer
        
        Call frmComm.ComReceive(sBuffer)
...
'       3.pDelimiter : 구분자
'-----------------------------------------------------------------------------'
Public Function mGetP1(ByVal pText As String, ByVal pPosition As Integer, _
    ...
    mGetP1 = Mid$(pText, intPos1, intPos2 - intPos1)
    Exit Function
