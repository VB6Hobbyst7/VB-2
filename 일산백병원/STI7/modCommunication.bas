...
Public Sub FileRcvData_CFX96()
    ...
    Dim strMultiInf     As String
    Dim strSGrp         As String
    Dim strMGrp         As String
    Dim strInfections   As String
    ...
    With frmMain
        For intCnt = 1 To UBound(strRecvData)
            ...
            Else
                If InStr(strInfections, strTarget) <= 0 Then
                    strInfections = strInfections & strTarget & ":" & strResult & ","
                    If InStr(strResult, "+") > 0 Then
                        strMGrp = strMGrp & strTarget & ":" & strResult & ","
                    End If
                End If
            End If
        Next
        ...
        '마지막 ',' 제거
        If Len(strInfections) > 0 Then
            strInfections = Mid(strInfections, 1, Len(strInfections) - 1)
        End If
        
        '-- 음성일 경우
        If strTotFlag = "Negative" Then
            strFlag = strInfections & vbNewLine
            strFlag = strFlag & "STI-7 Single infection : N/A" & vbNewLine
            strFlag = strFlag & "STI-7 Multi infection : N/A"
        ...
        ElseIf strTotFlag = "Positive" Then
            If Len(strMGrp) > 0 Then
                ...
                '마지막 ',' 제거
                If Len(strMGrp) > 0 Then
                    strMGrp = Mid(strMGrp, 1, Len(strMGrp) - 1)
                End If
                
                strFlag = strInfections & vbNewLine
                If InStr(strMGrp, ",") > 0 Then
                    strFlag = strFlag & "STI-7 Single infection : N/A" & vbNewLine
                    strFlag = strFlag & "STI-7 Multi infection : " & strMGrp
                Else
                    strSGrp = strMGrp
                    strFlag = strFlag & "STI-7 Single infection : " & strSGrp & vbNewLine
                    strFlag = strFlag & "STI-7 Multi infection : N/A"
                End If
            End If
        End If
        ...
        '결과를 붙임
        strFlag = strResult & vbNewLine & strFlag
                ...
                If Not RS_L.EOF = True And Not RS_L.BOF = True Then
                    ...
                    For intCol = colSTATE + 1 To .spdOrder.MaxCols
                        If lsTestCode = gArrEQP(intCol - colSTATE, 2) Then
                            ...
                            '-- IC
                            SetText .spdOrder, strICVal, gRow, colSTI7
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Dim prm(112)        As New ADODB.Parameter
    Dim i               As Integer

    Dim varData         As Variant
    Dim varMData        As Variant
    ...
    Dim j               As Integer
    Dim k               As Integer
    Dim intINData_Invalid       As Integer
    Dim intINSData      As Integer
    Dim intINMData      As Integer
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strJubNo <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                With AdoCmd
                    ...
                    Set prm(4) = .CreateParameter("@RESULT_RESID", adChar, adParamInput, 5, Mid(strDoctor, 1, 5))   'Resident
                    Set prm(5) = .CreateParameter("@RESULT_STAFF", adChar, adParamInput, 5, Mid(strDoctor, 1, 5))   'Staff1
                    Set prm(6) = .CreateParameter("@RESULT_STAFF2", adChar, adParamInput, 5, "")                    'Staff2
                    Set prm(7) = .CreateParameter("@RESULT_DATE", adChar, adParamInput, 8, Format(strRstDate, "yyyymmdd")) '결과일
                    ...
                    Set prm(79) = .CreateParameter("@M501T", adVarChar, adParamInput, 100, "")      '-- HPV type - (1행,1열)
                    Set prm(80) = .CreateParameter("@M502T", adVarChar, adParamInput, 100, "")      '-- HPV type - (1행,2열)
                    Set prm(81) = .CreateParameter("@M503T", adVarChar, adParamInput, 100, "")      '-- HPV type - (1행,3열)
                    Set prm(82) = .CreateParameter("@M504T", adVarChar, adParamInput, 100, "")      '-- HPV type - (1행,4열)
                    Set prm(83) = .CreateParameter("@M505T", adVarChar, adParamInput, 100, "")      '-- HPV type - (1행,2열)
                    Set prm(84) = .CreateParameter("@M506T", adVarChar, adParamInput, 100, "")      '-- HPV type - (2행,1열)
                    Set prm(85) = .CreateParameter("@M507T", adVarChar, adParamInput, 100, "")      '-- HPV type - (2행,2열)
                    Set prm(86) = .CreateParameter("@M508T", adVarChar, adParamInput, 100, "")      '-- HPV type - (2행,3열)
                    Set prm(87) = .CreateParameter("@M509T", adVarChar, adParamInput, 100, "")      '-- HPV type - (2행,4열)
                    Set prm(88) = .CreateParameter("@M510T", adVarChar, adParamInput, 100, "")     '-- HPV type - (2행,5열)
                    ...
                    For i = 0 To UBound(varData)
                        If varData(i) <> "" Then
                            If i = 0 Then
                                intINData_Invalid = 0
                                intINData = 0
                                If UCase(varData(i)) = "NEGATIVE" Then
                                    intINData = 1
                                ElseIf UCase(varData(i)) = "POSITIVE" Then
                                    intINData = 0
                                ElseIf UCase(varData(i)) = "INVALID" Then
                                    intINData = 0
                                    intINData_Invalid = 1
                                End If
                                Set prm(103) = .CreateParameter("@M523", adSmallInt, adParamInput, 1, intINData)                '--STI - Negative
                                Set prm(104) = .CreateParameter("@M524", adSmallInt, adParamInput, 1, intINData_Invalid)        '--STI - Insufficient Sample

                            ElseIf i = 1 Then
                                varRData = Split(varData(i), ",")
                                For j = 0 To UBound(varRData)
                                    Select Case mGetP(varRData(j), 1, ":")
'                                        Case "UU": Set prm(106) = .CreateParameter("@M530T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - UU
'                                        Case "UP": Set prm(107) = .CreateParameter("@M531T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - UP
'                                        Case "MG": Set prm(108) = .CreateParameter("@M527T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - MG
'                                        Case "MH": Set prm(109) = .CreateParameter("@M528T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - MH
'                                        Case "NG": Set prm(110) = .CreateParameter("@M526T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - NG
'                                        Case "CT": Set prm(111) = .CreateParameter("@M525T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - CT
'                                        Case "TV": Set prm(112) = .CreateParameter("@M529T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - TV
                                        
                                        Case "CT": Set prm(105) = .CreateParameter("@M525T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - CT
                                        Case "NG": Set prm(106) = .CreateParameter("@M526T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - NG
                                        Case "MG": Set prm(107) = .CreateParameter("@M527T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - MG
                                        Case "MH": Set prm(108) = .CreateParameter("@M528T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - MH
                                        Case "TV": Set prm(109) = .CreateParameter("@M529T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - TV
                                        Case "UU": Set prm(110) = .CreateParameter("@M530T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - UU
                                        Case "UP": Set prm(111) = .CreateParameter("@M531T", adChar, adParamInput, 10, mGetP(varRData(j), 2, ":"))      '--STI - UP
                            ...
                            ElseIf i = 2 Then
                                intINData = 0
                                If InStr(varData(i), "Single") > 0 Then
                                    If Trim(mGetP(varData(i), 2, ":")) = "N/A" Then
                                        intINData = 0
                                        intINSData = 0
                                    Else
                                        intINData = 1
                                    ...
                                    End If
                                End If
                                Set prm(101) = .CreateParameter("@M521", adSmallInt, adParamInput, 1, intINData)      '--STI - Single infection
                            ElseIf i = 3 Then
                                intINData = 0
                                If InStr(varData(i), "Multi") > 0 Then
                                    If Trim(mGetP(varData(i), 2, ":")) = "N/A" Then
                                        intINData = 0
                                        intINMData = 0
                                    Else
                                    ...
                                    End If
                                End If
                                Set prm(102) = .CreateParameter("@M522", adSmallInt, adParamInput, 1, intINData)      '--STI - Multiple infection
                            End If
                        End If
                    Next
                    
                    Set prm(112) = .CreateParameter("@TAT_SAYU", adChar, adParamInput, 3, "")

                    'Set prm(102) = .CreateParameter("@M521", adSmallInt, adParamInput, 1, intINSData)      '--STI - Single infection
                    'Set prm(103) = .CreateParameter("@M522", adSmallInt, adParamInput, 1, intINMData)      '--STI - Multiple infection
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strJubNo <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                With AdoCmd
                    ...
                    '-- 2018.02.06 수정
                    'Set prm(90) = .CreateParameter("@M503", adSmallInt, adParamInput, 1, 0)             '--DNA-Chip Insufficient Sample
                    If UCase(sResult1) = "INVALID" Then
                        Set prm(90) = .CreateParameter("@M503", adSmallInt, adParamInput, 1, 1)             '--DNA-Chip Insufficient Sample
                    Else
                        Set prm(90) = .CreateParameter("@M503", adSmallInt, adParamInput, 1, 0)             '--DNA-Chip Insufficient Sample
                    End If
