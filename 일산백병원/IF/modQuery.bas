...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
        ...
        Case "WHITE"                        '일산뱍병원
                Call GetWorkList_WHITE(pFrom, pTo, SPD)
...
Public Sub GetWorkList_WHITE(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
'    Dim RS          As ADODB.Recordset
'    Dim i           As Integer
'    Dim iCnt        As Long
'    Dim intRow      As Long
...
On Error GoTo RST
    ...
    '내원정보에 연번과 검사결과에 처방번호로 매칭
    '처방번호, 접수번호, 검사코드, 순번 로 매칭
    
    SQL = ""
    SQL = SQL & "SELECT DISTINCT"
    SQL = SQL & "       R.접수일                as HOSPDATE " & vbCr
'    SQL = SQL & "       M.내원일                as HOSPDATE " & vbCr
    SQL = SQL & "     , R.처방번호              as BARCODE  " & vbCr
    SQL = SQL & "     , P.등록번호              as PID      " & vbCr
    SQL = SQL & "     , R.접수번호              as CHARTNO  " & vbCr
    SQL = SQL & "     , P.성명                  as PNAME    " & vbCr
    SQL = SQL & "     , M.외래입원              as INOUT    " & vbCr
    SQL = SQL & "     , COUNT(R.검사코드)       as CNT      " & vbCr
    SQL = SQL & "  FROM DBinfor..내원정보 M, DBinfor..환자정보 P, DBsupport..검사결과 R     " & vbCr
    SQL = SQL & " WHERE M.연번      = R.처방번호                                            " & vbCr
    SQL = SQL & "   AND M.등록번호  = P.등록번호                                            " & vbCr
    SQL = SQL & "   AND R.처방번호  = R.처방번호                                            " & vbCr
    SQL = SQL & "   AND R.접수일 BETWEEN '" & pFrom & "' and '" & pTo & "'                  " & vbCr
    SQL = SQL & "   AND R.검사코드 IN (" & gAllTestCd & ")                                  " & vbCr
    SQL = SQL & "   AND (R.결과 = '' OR R.결과 IS NULL)                                     " & vbCr
    SQL = SQL & " GROUP BY R.접수일, R.처방번호, P.등록번호, R.접수번호, P.성명, M.외래입원 " & vbCr
'    SQL = SQL & " ORDER BY R.접수일, R.접수번호                                            " & vbCr
...
'            SQL = SQL & "   AND L.LABCHTNUM  = M.MANCHTNUM                  " & vbCr                    '-- 챠트번호
        ...
        Case "EMRLAB"
            ...
            AdoCn_Local.CursorLocation = adUseClient
            Set RS = New ADODB.Recordset
            RS.Open AdoCn_Local.Execute(SQL, sqlRet)
...
'            SQL = SQL & "   And 처방코드+서브코드 in (" & gAllExam & ")"
    ...
    If SQL <> "" Then
        ...
        Set RS = New ADODB.Recordset
        Set RS = AdoCn_Local.Execute(SQL, , 1)
        If Not RS.EOF = True And Not RS1.BOF = True Then
            Do Until RS1.EOF
                If strItems = "" Then
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        ...
        Case "WHITE"
                Call GetSampleInfo_WHITE(asRow, SPD)
...
'    SQL = SQL & "       M.내원일                as HOSPDATE " & vbCr
    ...
    SQL = SQL & "     , P.성명                  as PNAME     " & vbCr
    SQL = SQL & "     , M.외래입원              as INOUT     " & vbCr
    SQL = SQL & "     , R.검사코드              as ITEM      " & vbCr
    SQL = SQL & "     , R.순번                  as ITEMSEQ   " & vbCr
    SQL = SQL & "  FROM DBinfor..내원정보 M, DBinfor..환자정보 P, DBsupport..검사결과 R     " & vbCr
    SQL = SQL & " WHERE M.연번      = R.처방번호                                            " & vbCr
    SQL = SQL & "   AND M.등록번호  = P.등록번호                                            " & vbCr
    SQL = SQL & "   AND R.처방번호  = R.처방번호                                            " & vbCr
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_WHITE(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim RS1             As ADODB.Recordset
    Dim strRegDate      As String
    Dim strOrgBarcode   As String
    Dim strBarcode      As String
...
On Error GoTo DBErr
    ...
    '-- Record Count 가져옴
    AdoCn.CursorLocation = adUseClient
    SQL = "p_interfacequery '2','" & strBarcode & "'"
    Set RS = AdoCn.Execute("Exec p_interfacequery '2','" & strBarcode & "'", sqlRet)
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            gPatOrdCd = gPatOrdCd & "'" & Trim(RS.Fields("ORD_CODE")) & "_" & Trim(RS.Fields("LELMT_SEQ")) & "',"
            RS.MoveNext
        Loop
    End If
    RS.Close
    ...
    Call SetSQLData("RCV", gPatOrdCd, "A")
    
    '-- Record Count 가져옴
    AdoCn.CursorLocation = adUseClient
    SQL = "Exec p_interfacequery '1','" & strBarcode & "'"
    Set RS = AdoCn.Execute("Exec p_interfacequery '1','" & strBarcode & "'", sqlRet)
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                intTestCnt = intTestCnt + 1
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, Trim(RS.Fields("SAMPLE_DATE")) & "", asRow, colHOSPDATE
                SetText SPD, strBarcode, asRow, colBARCODE
                SetText SPD, Trim(RS.Fields("CHART_NO")), asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("SAMPLE_SEQ")) & "", asRow, colPID
                SetText SPD, Trim(RS.Fields("PAT_NAME")) & "", asRow, colPNAME
                SetText SPD, Trim(RS.Fields("PART")) & "", asRow, colINOUT
