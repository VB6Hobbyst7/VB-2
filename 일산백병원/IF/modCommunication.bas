...
Function SaveTransData_WHITE(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
...
'    Dim k               As Integer
    ...
    Dim strSampSeq      As String
    Dim intSampSeq      As Integer
    
    Dim strInsData      As String
    
On Error GoTo ErrHandle
    
    
    With frmMain
        SaveTransData_WHITE = -1
        intRow = 0
        ...
        strBarcode = Trim(GetText(.spdOrder, argSpcRow, colBARCODE))
        strHospDate = Trim(GetText(.spdOrder, argSpcRow, colHOSPDATE))
        strPart = Trim(GetText(.spdOrder, argSpcRow, colINOUT))
        strChartNo = Trim(GetText(.spdOrder, argSpcRow, colCHARTNO))
        strSampSeq = Trim(GetText(.spdOrder, argSpcRow, colPID))
        
        If IsNumeric(strSampSeq) Then
            intSampSeq = strSampSeq
        'Else
        '    Exit Function
        End If
...
'        RS_L.Close
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                With AdoCmd
                    .CommandTimeout = 15
                    .CommandText = "p_l3a01interface1"
                    .CommandType = adCmdStoredProc
                    
                    
                    Set prm1 = .CreateParameter("Action", adChar, adParamInput, 1, "U")      '호출 조건
                    .Parameters.Append prm1

                    Set prm2 = .CreateParameter("Sample_Date", adChar, adParamInput, 8, strHospDate)       '검사 접수 일자
                    .Parameters.Append prm2

                    Set prm3 = .CreateParameter("Part", adChar, adParamInput, 2, strPart)    '검사 부서
                    .Parameters.Append prm3

                    Set prm4 = .CreateParameter("Chart_No", adChar, adParamInput, 7, strChartNo)    '환자 등록 번호
                    .Parameters.Append prm4

                    Set prm5 = .CreateParameter("Sample_Seq", adInteger, adParamInput, 4, intSampSeq)    '검체 번호
                    .Parameters.Append prm5

                    Set prm6 = .CreateParameter("Ord_Code", adChar, adParamInput, 8, mGetP(strTestCd, 1, "_"))  '오더 코드
                    .Parameters.Append prm6

                    Set prm7 = .CreateParameter("Lelmt_Seq", adInteger, adParamInput, 5, mGetP(strTestCd, 2, "_"))  '오더 코드 Sequence
                    .Parameters.Append prm7

                    Set prm8 = .CreateParameter("Result", adVarChar, adParamInput, 12, sResult)    '검사 결과 값
                    .Parameters.Append prm8

                    Set prm9 = .CreateParameter("q_ref", adChar, adParamInput, 1, "")    '
                    .Parameters.Append prm9

                    Set prm10 = .CreateParameter("q_pan", adChar, adParamInput, 1, "")    '
                    .Parameters.Append prm10

                    Set prm11 = .CreateParameter("q_del", adChar, adParamInput, 1, "")    '
                    .Parameters.Append prm11

                    Set prm12 = .CreateParameter("q_cri", adChar, adParamInput, 1, "")    '
                    .Parameters.Append prm12

                    Set prm13 = .CreateParameter("USER_ID", adChar, adParamInput, 5, gHOSP.USERID)     '
                    .Parameters.Append prm13

                    Set prm14 = .CreateParameter("GOODS_CODE", adChar, adParamInput, 2, gHOSP.MACHCD)    '
                ...
                strInsData = ""
                strInsData = strInsData & "U" & vbCrLf
                strInsData = strInsData & strHospDate & vbCrLf
                strInsData = strInsData & strPart & vbCrLf
                strInsData = strInsData & strChartNo & vbCrLf
                strInsData = strInsData & intSampSeq & vbCrLf
                strInsData = strInsData & mGetP(strTestCd, 1, "_") & vbCrLf
                strInsData = strInsData & mGetP(strTestCd, 2, "_") & vbCrLf
                strInsData = strInsData & sResult & vbCrLf
                strInsData = strInsData & "" & vbCrLf
                strInsData = strInsData & "" & vbCrLf
                strInsData = strInsData & "" & vbCrLf
                strInsData = strInsData & "" & vbCrLf
                strInsData = strInsData & gHOSP.USERID & vbCrLf
                strInsData = strInsData & gHOSP.MACHCD & vbCrLf
                
                Call SetSQLData("결과저장", strInsData)
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Select Case gEMR
        ...
        Case "EMRLAB"
            SaveTransData = SaveTransData_EMRLAB(argSpcRow)
        ...
        Case "WHITE"
            SaveTransData = SaveTransData_WHITE(argSpcRow)
...
Function SaveTransData_EMRLAB(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim blnSave         As Boolean
    Dim intRow          As Integer
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            strJudge = Trim(GetText(.vasTemp, intRow, 7))     '판정
