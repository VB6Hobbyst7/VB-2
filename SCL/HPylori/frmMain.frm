...
Begin VB.Form frmMain 
   ...
   Begin FPSpread.vaSpread spdResult 
      ...
      SpreadDesigner  =   "frmMain.frx":1369
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
      ...
      Begin MSComctlLib.ImageList imlStatus 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":1FC1
               Key             =   "RUN"
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":255B
               Key             =   "NOT"
            EndProperty
            BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":2AF5
               Key             =   "STOP"
            EndProperty
            BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":308F
               Key             =   "LST"
            EndProperty
            BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":3921
               Key             =   "ITM"
            EndProperty
   ...
   Begin FPSpread.vaSpread spdOrder 
      ...
      ScrollBarExtMode=   -1  'True
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":74F3
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdWork 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":93EA
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
...
Private Sub Excel_Open()
    ...
    '이전버전용
    Y = spdExcel.GetExcelSheetList(CFXFile.Filename, List, ListCount, "Report.txt", handle, True)
    If Y = True Then
...
'R|1|^MTB-RIF^^MTB^Xpert MTB-RIF Assay G4^6^MTB^|NOT DETECTED^|||||F||<None>|20190911141454|20190911155618|Cepheid-642628D^820753^723731^602896890^57901^20210620
                    ...
                    If intRCnt = 0 Then
                        ...
                        '                                        Reagent Lot ID
                        '                                              Expiration Date
                        'Cepheid-642628D^842865^889218^521465888^25417^20211017
                        ...
                        Select Case strMachNum
                            ...
                            Case "888634", "889218", "889219", "889221"
                                mResult.EqpCd = "E23"
                        End Select
...
'''                    End If
                ...
                Case "L"
                    ...
                    ReDim Preserve mCmt.CARBAR_CMTCD(mResult.RsltSeq) As String
                    ReDim Preserve mCmt.CARBAR_CMT(mResult.RsltSeq) As String
                    ReDim Preserve mCmt.MTBRIF_CMTCD(mResult.RsltSeq) As String
                    ReDim Preserve mCmt.MTBRIF_CMT(mResult.RsltSeq) As String
                    ReDim Preserve mCmt.CMNTCD(mResult.RsltSeq) As String
                    
                    mCmt.CARBAR_CMTCD(mResult.RsltSeq) = mResult.CARBAR_CMTCD
                    mCmt.CARBAR_CMT(mResult.RsltSeq) = mResult.CARBAR_CMT
                    mCmt.MTBRIF_CMTCD(mResult.RsltSeq) = mResult.MTBRIF_CMTCD
                    mCmt.MTBRIF_CMT(mResult.RsltSeq) = mResult.MTBRIF_CMT
                    mCmt.CMNTCD(mResult.RsltSeq) = mResult.CMNTCD
            End Select
        Next
...
Private Sub FileRcvData_CFX96()
    ...
    Dim intMTB          As Integer
    Dim intBAC          As Integer
    Dim intIC           As Integer
    Dim intRstNo        As Integer
    Dim intPName        As Integer

On Error GoTo ErrHandle

    
    With frmMain
        If spdExcel.MaxRows < 4 Then
            Exit Sub
        End If
        ...
        For intCol = 1 To spdExcel.MaxCols
            ...
            If UCase(GetText(spdExcel, intRow, intCol)) = "샘플 NO" Then
                intNo = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "환자 ID" Then
                intSmpNo = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "WELL" Then
                intWellNo = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "MTB" Then
                intMTB = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "이름" Then
                intPName = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "MYCOBACTERIA" Then 'Mycobacteria
                intBAC = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "IC" Then
                intIC = intCol
            End If
            If UCase(GetText(spdExcel, intRow, intCol)) = "자동   해석" Then
                intRstNo = intCol
            End If
        Next
        
        For intRow = 2 To spdExcel.MaxRows Step 3
            strRackNo = GetText(spdExcel, intRow, intWellNo)
            strBarno = GetText(spdExcel, intRow, intNo)
            If strBarno = "" Then
                strBarno = GetText(spdExcel, intRow, intPName)
            End If
            ...
            For intCol = 1 To 4
                ...
                strIntResult = ""
                If intCol = 1 Then
                    strIntBase = GetText(spdExcel, intRow, 5)
                    strResult = GetText(spdExcel, intRow, 6)
                    'strIntResult = GetText(spdExcel, intRow, 7)
                ElseIf intCol = 2 Then
                    strIntBase = GetText(spdExcel, intRow + 1, 5)
                    strResult = GetText(spdExcel, intRow + 1, 6)
                    'strIntResult = GetText(spdExcel, intRow, intCol + 1)
                ElseIf intCol = 3 Then
                    strIntBase = GetText(spdExcel, intRow + 2, 5)
                    strResult = GetText(spdExcel, intRow + 2, 6)
                   ' strIntResult = GetText(spdExcel, intRow, intCol + 1)
                ElseIf intCol = 4 Then
                    strIntBase = "자동해석" 'GetText(spdExcel, 2, intCol)   '자동   해석
                    strResult = GetText(spdExcel, intRow, 8)
                    'strIntResult = GetText(spdExcel, intRow, intCol + 1)
                Else
                    strResult = ""
                    strIntResult = ""
...
ErrHandle:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_FileRcvData_CFX96" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
        ...
        With frmMain
            ...
            For intCol = colSTATE + 1 To .spdOrder.MaxCols
                If strAbbrName = gArrEQP(intCol - colSTATE, 6) Then
                    ...
                    If UCase(pResult) = "DETECTED" Or UCase(pResult) = "POS" Or UCase(pResult) = "POSITIVE" Then
                        SetBackColor .spdOrder, gRow, gRow, 1, .spdOrder.MaxCols, 255, 0, 0
                        'SetForeColor .spdOrder, gRow, gRow, 1, .spdOrder.MaxCols, 255, 255, 0
                        strJudge = "H"
                    Else
                        blnPos = False
                        For J = colITEMS To spdOrder.MaxCols
                            spdOrder.Row = gRow
                            spdOrder.Col = J
                            If spdOrder.BackColor = vbRed Then
                                blnPos = True
                                Exit For
                            End If
                        Next
                        If blnPos = False Then
                            .spdOrder.BlockMode = True
                            .spdOrder.Row = gRow
                            .spdOrder.Row2 = gRow
                            .spdOrder.Col = colEXAMDATE
                            .spdOrder.Col2 = .spdOrder.MaxCols
                            .spdOrder.BackColor = vbWhite
...
Private Sub spdOrder_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        If colBARCODE = sCol Then
            ...
            Else
                strSaveSeq = Trim(GetText(spdOrder, sRow, colSAVESEQ))
                '정보수정
                SQL = ""
                ...
                mResult.CARBAR_CMTCD = mCmt.CARBAR_CMTCD(strSaveSeq)
                mResult.CARBAR_CMT = mCmt.CARBAR_CMT(strSaveSeq)
                mResult.MTBRIF_CMTCD = mCmt.MTBRIF_CMTCD(strSaveSeq)
                mResult.MTBRIF_CMT = mCmt.MTBRIF_CMT(strSaveSeq)
                mResult.CMNTCD = mCmt.CMNTCD(strSaveSeq)
            End If
