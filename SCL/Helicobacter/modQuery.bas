...
On Error GoTo ErrHandle
    ...
    strFooter = strFooter & ". 관련질환과 의의: 발열, 기침, 호흡곤란, 두통, 오한, 인후통, 호흡부전, 패혈성 쇼크, 다발성 장기 부전 등의 원인인 2019-nCoV를 검출함" & vbCrLf
...
'        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        SQL = SQL & "   AND EXAMCODE IN ('L4811') " & vbCrLf
...
'                        strRdRpVal = sResult
'                    Else
                        '아래와같이 여러개의 결과 나올 경우를 처리했으나 그렇게 나올일은 없다네
                        'pResult = sResult
...
'        End If
        ...
        If pSPCID <> "" And pResult <> "" Then
            ...
            sParam = sParam & "<QID><![CDATA[PG_SRL.INTERFACE_I01]]></QID>" & vbCrLf
            sParam = sParam & "<QTYPE><![CDATA[Package]]></QTYPE>" & vbCrLf
            sParam = sParam & "<USERID><![CDATA[" & "LIS_PROD" & "]]></USERID>" & vbCrLf          'HIS
            sParam = sParam & "<EXECTYPE><![CDATA[NONQUERY]]></EXECTYPE>" & vbCrLf
            sParam = sParam & "<TABLENAME><![CDATA[]]></TABLENAME>" & vbCrLf
            sParam = sParam & "<P0><![CDATA[" & vbTab & pSPCID & vbTab & "]]></P0>" & vbCrLf        '바코드
            sParam = sParam & "<P1><![CDATA[" & vbTab & pTSTCD & vbTab & "]]></P1>" & vbCrLf        '검사코드
            sParam = sParam & "<P2><![CDATA[" & vbTab & pResult & vbTab & "]]></P2>" & vbCrLf       '검사결과
            sParam = sParam & "<P3><![CDATA[" & vbTab & "" & vbTab & "]]></P3>" & vbCrLf
            sParam = sParam & "<P4><![CDATA[" & vbTab & pEQUIPCD & vbTab & "]]></P4>" & vbCrLf      '장비코드
            sParam = sParam & "<P5><![CDATA[" & vbTab & "" & vbTab & "]]></P5>" & vbCrLf
            sParam = sParam & "<P6><![CDATA[" & pCNT & "]]></P6>" & vbCrLf                          '검사수
            sParam = sParam & "<P7><![CDATA[" & "" & "]]></P7>" & vbCrLf                           '1 : 접수, 2 : 입력, 3 : 최종보고
            sParam = sParam & "<P8><![CDATA[" & "2" & "]]></P8>" & vbCrLf
            sParam = sParam & "<P9><![CDATA[" & gHOSP.USERID & "]]></P9>" & vbCrLf
            sParam = sParam & "<P10><![CDATA[]]></P10>" & vbCrLf
            sParam = sParam & "<P11><![CDATA[]]></P11>" & vbCrLf
            sParam = sParam & "</Table>" & vbCrLf
            sParam = sParam & "</NewDataSet>" & vbCrLf
        
        
'<Table>
'<QID><![CDATA[PG_SRL.INTERFACE_I01]]></QID>
'<QTYPE><![CDATA[Package]]></QTYPE>
'<USERID><![CDATA[LIS_PROD]]></USERID>
'<EXECTYPE><![CDATA[NONQUERY]]></EXECTYPE>
'<TABLENAME><![CDATA[]]></TABLENAME>
'<P00><![CDATA[  21070137669 ]]></P00>
'<P01><![CDATA[  L4701   ]]></P01>
'<P02><![CDATA[  Negative    ]]></P02>
'<P03><![CDATA[      ]]></P03>
'<P04><![CDATA[  CFX96   ]]></P04>
'<P05><![CDATA[      ]]></P05>
'<P06><![CDATA[1]]></P06>
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    Select Case gEMR
        Case "SCL"
                Call GetWorkList_SCL(pFrom, pTo, SPD)
...
Public Sub GetWorkList_SCL(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    Set RS = adoExecQuery50P("SLRTRM50P", pFrom, gHOSP.MACHCD, Val(frmWorkList.txtFromWN.Text), Val(frmWorkList.txtToWN.Text), Space$(5), "0", "")
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        ...
        Do Until RS.EOF
            With SPD
                If strKeyno <> RS.Fields("BARCODENO") And InStr(gAllTestCd, Trim(RS("ITEMCODE"))) > 0 Then
                    intRow = intRow + 1
                    ...
                    .SetText colSEQNO, intRow, Trim(RS.Fields("WORKNO"))
                    .SetText colHOSPDATE, intRow, Trim(RS("ORDDATE"))
                    .SetText colBARCODE, intRow, Trim(RS("BARCODENO"))
                    .SetText colPID, intRow, Trim(RS.Fields("WORKNO"))
                    .SetText colPNAME, intRow, Trim(RS("PNAME"))
                    
                    strNames = GetText(SPD, intRow, colITEMS)
                    strNames = GetTestNm(Trim(RS("ITEMCODE")))
                    Call SetText(SPD, strNames, intRow, colITEMS)
                    
                    '-- 검사코드 확인 테스트용
                    '.SetText colState, intRow, Trim(rs("ITEMCODE")) & Trim(rs("ITEMNAME"))
                    strKeyno = RS("BARCODENO")
...
ReSearch:
    ...
    sRcvData = Online_XML("PG_SRL.INTERFACE_S14", "", "GETQUERY", "", "", pFrom, pTo, strTestCds) ' "PKG_MSE_LM_INTERFACE.PC_MSE_ORDER_SELECT"
        ...
        For i = 0 To UBound(XmlWork.ACPT_DTE)
            If XmlWork.SPC_NO(i) = "" Then
                ...
                If SPD.MaxRows > 0 Then
                    'Call frmWorkList.cmdPC_Click
                    'Call frmWorkList.cmdNC_Click
                End If
...
'</NewDataSet>
    ...
    sRes = Online_XML("PG_SRL.INTERFACE_S06", strBarcode, "GETQUERY", "", "") ' "PKG_MSE_LM_INTERFACE.PC_MSE_ORDER_SELECT"
...
'
    If XmlInfo.PT_NO <> "" Then
        With SPD
            .ReDraw = False
            intTestCnt = intTestCnt + 1
            If XmlInfo.PATNAME <> "" Then
                SetText SPD, "1", asRow, colCHECKBOX
            Else
                SetText SPD, "0", asRow, colCHECKBOX
            End If
            'SetText SPD, XmlInfo.SPCACPTDT(i), asRow, colHOSPDATE
            SetText SPD, XmlInfo.SPC_NO, asRow, colBARCODE
            SetText SPD, XmlInfo.PT_NO, asRow, colPID
            SetText SPD, XmlInfo.PATNAME, asRow, colPNAME
            SetText SPD, XmlInfo.SEX, asRow, colPSEX
            'SetText SPD, XmlInfo.AGE(i), asRow, colPAGE
'            SetText SPD, XmlInfo.SPCNM(i), asRow, colSPECIMEN
                                                             
'            오더정보에 저장
                With mOrder
                    .BarNo = XmlInfo.SPC_NO
                    .PID = XmlInfo.PT_NO
                    .PNAME = XmlInfo.PATNAME
                    .Count = CStr(intTestCnt)
                    .NoOrder = False
                End With
                ...
                For intNodeLen = 0 To xNode.childNodes.Length - 1
                    For i = 0 To xNode.childNodes.Item(intNodeLen).childNodes.Length - 1
                        ...
                        Select Case UCase(xNode.childNodes.Item(intNodeLen).childNodes.Item(i).baseName)
                            ...
                            Case "PT_NM":               XmlWork.PATNAME(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                            Case "TST_DTE":             XmlWork.ACPT_DTE(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                            Case "GNL_ITEM_CD":         XmlWork.TST_CD(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                            Case "TRANS_YN":            XmlWork.TST_STAT(J) = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
            ...
            If xNode.nodeType = NODE_ELEMENT Then
                For intNodeLen = 0 To xNode.childNodes.Length - 1
                    For i = 0 To xNode.childNodes.Item(intNodeLen).childNodes.Length - 1
                        ...
                        Select Case UCase(xNode.childNodes.Item(intNodeLen).childNodes.Item(i).baseName)
                            Case "SPC_NO":          XmlInfo.SPC_NO = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                            Case "PTNO":            XmlInfo.PT_NO = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
...
'로그인
Public Sub DisplayNode_Login(asPath As String, asCnt As Integer)
        ...
        For Each xNode In Child_Node
            If xNode.nodeType = NODE_ELEMENT Then
                For intNodeLen = 0 To xNode.childNodes.Length - 1
                    For i = 0 To xNode.childNodes.Item(intNodeLen).childNodes.Length - 1
                        Select Case UCase(xNode.childNodes.Item(intNodeLen).childNodes.Item(i).baseName)
                            Case "WK_ID":       XmlLogIN.WK_ID = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                            Case "WK_PW":       XmlLogIN.WK_PW = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                            Case "WK_NM":       XmlLogIN.WK_NM = xNode.childNodes.Item(intNodeLen).childNodes.Item(i).nodeTypedValue
                        End Select
                    Next
                    J = J + 1
                Next
