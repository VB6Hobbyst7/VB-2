...
'                    SQL = SQL & "   And MASTERCODE  = 'LH1P01'    " & vbCrLf  '마스터코드 LH1P01
                    ...
                    SQL = SQL & "   Set STATUS     = '4'                    " & vbCrLf '검사상태 [결과등록(3:결과미확인, 4:부분전송)]
                    SQL = SQL & "     , RESULTDATE = TRUNC(SYSDATE)         " & vbCrLf
                    SQL = SQL & " Where SPECNO     = '" & strBarcode & "'   " & vbCrLf '검체번호
                    SQL = SQL & "   And STATUS     <= '4'                   " & vbCrLf '검사상태 [3:검체접수]
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String _
    ...
    Select Case gEMR
        Case "TWIN"                         '투윈정보
                Call GetWorkList_TWIN(pFrom, pTo, SPD, pSave)
...
Public Sub GetWorkList_TWIN(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pSave As Boolean = False)
    Dim RS          As ADODB.Recordset
    Dim i           As Integer
    Dim strHospDate As String
    Dim strBarcode  As String
...
On Error GoTo ErrHandle
    ...
    SQL = SQL & "     , COUNT(A.SUBCODE)                        AS CNT          " & vbCrLf
    ...
    SQL = SQL & " WHERE TO_CHAR(C.JOBDATE,'yyyy-mm-dd') between  '" & pFrom & "' AND '" & pTo & "'  " & vbCrLf
    'SQL = SQL & " Where A.SPECNO = '" & strBarcode & "'                         " & vbCrLf
    SQL = SQL & "   AND A.SUBCODE IN (" & gAllTestCd & ")                       " & vbCrLf
    If pSave = False Then
        SQL = SQL & "   AND C.STATUS  <= '4'                                    " & vbCrLf '검사상태
    End If
    SQL = SQL & "   And C.SPECNO  = A.SPECNO                                    " & vbCrLf
    SQL = SQL & "   And A.MASTERCODE = B.MASTERCODE                             " & vbCrLf
    SQL = SQL & " GROUP BY C.JOBDATE, C.SPECNO, C.PTNO, C.JOBNO, C.GBIO, C.SNAME, C.SEX, C.AGE"
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_TWIN" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    ...
    If Not RS1.EOF = True And Not RS1.BOF = True Then
        Do Until RS1.EOF
            With SPD
                ...
                '-- 화면에 표시
                For intCol = colSTATE + 1 To .MaxCols
                    If GetTestNm(Trim(RS1.Fields("itemcd"))) = gArrEQP(intCol - colSTATE, 6) Then
                        Call SetSPDOrder(SPD, SPD.MaxRows, SPD.MaxRows, intCol, intCol)
                        Exit For
                    End If
                Next
