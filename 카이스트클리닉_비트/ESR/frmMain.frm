...
'    End If
    ...
    Select Case UCase(gHOSP.MACHNM)
        Case "ESR2010":         Call Phase_Serial_ESR
...
On Error GoTo RST
    ...
    With frmMain
        ...
        For i = 0 To Pos - 1
            strRcvBuf = Mid(RcvBuffer, (i * 5) + 1, 5)
            strIntBase = "ESR"
            strSeq = Trim(Mid(strRcvBuf, 2, 1))
            strSeq = Asc(strSeq)
            strResult = Trim(Mid(strRcvBuf, 3, 1))
            strResult = Asc(strResult)
            
            '-- 같은 포지션 찾기
            blnSame = False
            For ii = 1 To spdOrder.MaxRows
                If Trim(strSeq) = Trim(GetText(spdOrder, ii, colSEQNO)) Then
                    spdOrder.Row = ii
                    spdOrder.Col = colSTATE
...
'    Next i
    ...
    tmrESR2010.Interval = 3000
    tmrESR2010.Enabled = True
...
Private Sub tmrESR2010_Timer()
    
    lblTestDate.Caption = Format(Now, "yyyy-mm-dd")
    Call SerialRcvData_ESR
    RcvBuffer = ""
    tmrESR2010.Enabled = False
