...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        If pSPCID <> "" Then
            ...
            sParam = sParam & "<QID><![CDATA[PKG_LAB.INTERFACE_I02]]></QID>"
            sParam = sParam & "<QTYPE><![CDATA[Package]]></QTYPE>" & vbCrLf
            sParam = sParam & "<USERID><![CDATA[" & gHOSP.USERID & "]]></USERID>"
            sParam = sParam & "<EXECTYPE><![CDATA[NONQUERY]]></EXECTYPE>" & vbCrLf
            sParam = sParam & "<TABLENAME><![CDATA[]]></TABLENAME>" & vbCrLf
            sParam = sParam & "<P0><![CDATA[" & vbTab & pSPCID & vbTab & "]]></P0>"
            sParam = sParam & "<P1><![CDATA[" & vbTab & pTSTCD & vbTab & "]]></P1>"
            sParam = sParam & "<P2><![CDATA[" & vbTab & pResult & vbTab & "]]></P2>"
            sParam = sParam & "<P3><![CDATA[" & vbTab & "" & vbTab & "]]></P3>"
            sParam = sParam & "<P4><![CDATA[" & vbTab & pEQUIPCD & vbTab & "]]></P4>"
            sParam = sParam & "<P5><![CDATA[" & vbTab & "" & vbTab & "]]></P5>"
            sParam = sParam & "<P6><![CDATA[" & vbTab & pCNT & vbTab & "]]></P6>"
            sParam = sParam & "<P7><![CDATA[" & vbTab & "" & vbTab & "]]></P7>"
            sParam = sParam & "<P8><![CDATA[" & vbTab & "" & vbTab & "]]></P8>"
            sParam = sParam & "</Table>" & vbCrLf
            sParam = sParam & "</NewDataSet>" & vbCrLf
...
ReSearch:
    ...
    strTestCds = Replace(gAllTestCd, "'", "") 'gHOSP.ORDCODE
    
    
    sRcvData = Online_XML(gXml_TERM_SELECT, strBarcode, "GETQUERY", "", "", pFrom, pTo, strTestCds) ' "PKG_MSE_LM_INTERFACE.PC_MSE_ORDER_SELECT"
    ...
    'Kill App.PATH & "\XML\" & strXmlName
    'MsgBox UBound(XmlWork.ACPT_DTE)
    ...
    If UBound(XmlWork.ACPT_DTE) >= 0 Then
        For i = 0 To UBound(XmlWork.ACPT_DTE)
            ...
            With SPD
                ...
                'If GetSampleTest_HDINFO(XmlWork.SPC_NO(i)) > 0 Then
...
'
                'If InStr(UCase(XmlWork.IFREQCDLIST(i)), "COVID19") > 0 Then
                    For J = 1 To SPD.DataRowCnt
                        strHospDate = GetText(SPD, J, colHOSPDATE)
                        strBarcode = GetText(SPD, J, colBARCODE)
...
'    MsgBox strBarcode
    ...
    sRes = Online_XML(gXml_BAR_SELECT, strBarcode, "GETQUERY", "", "") ' "PKG_MSE_LM_INTERFACE.PC_MSE_ORDER_SELECT"
...
'''    sRcvData = sRes
'''    If InStr(1, sRcvData, "<?xml version") > 0 Then
'''        varRcvData = Split(sRcvData, "<Table>")
'''    End If
'''
'''    strXmlName = gHOSP.MACHNM & "_" & Format(CDate(Now), "yyyymmdd") & "_" & strBarcode & ".xml"
'''
