...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Tab(0).Control(14)=   "SSFrame1"
      Tab(0).Control(14).Enabled=   0   'False
      Tab(0).Control(15)=   "FrameError"
      Tab(0).Control(15).Enabled=   0   'False
      Tab(0).Control(16)=   "UserPanel1"
      Tab(0).Control(16).Enabled=   0   'False
      Tab(0).Control(17)=   "Crpt"
      Tab(0).Control(17).Enabled=   0   'False
      Tab(0).Control(18)=   "spdResult1"
      Tab(0).Control(18).Enabled=   0   'False
      Tab(0).Control(19)=   "cmdSel(0)"
      Tab(0).Control(19).Enabled=   0   'False
      Tab(0).Control(20)=   "cmdSel(1)"
...
Private Sub cmdSearch_Click()
    ...
    Dim sqlRet      As Integer
    Dim sqlDoc      As String
    Dim intRow      As Integer, intRow1      As Integer, i      As Integer
    ...
    Dim barnochk(100) As String
    Dim barnoflag As String
    
    Screen.MousePointer = vbHourglass
    spdResult1.ReDraw = False
    
     With spdResult1     'vbGreen색(오더전송상태)의 환자 목록은 안지움
         For intRow = 1 To .MaxRows
            .GetText 0, intRow, varTmp
            If Trim(varTmp) = "검사중" Or Trim(varTmp) = "완료" Then
                intRow1 = intRow1 + 1
                .Col = 3: .Row = intRow
                barnochk(intRow1) = .Text
            Else
                .Col = 3: .Row = intRow
                .Text = ""
            End If
        ...
        For intRow = .MaxRows To 1 Step -1
            ...
            Do Until mAdoRs.EOF
                ...
                With spdResult1
                    ...
                    If strBarno <> mAdoRs.Fields("BCID") And barnoflag = "1" Then
                        intRow = intRow + 1
                        If intRow > .MaxRows Then .MaxRows = .MaxRows + 1: .RowHeight(.MaxRows) = 12
                        ...
                        Do Until mAdoRs1.EOF
                            strEqpCd = f_funGet_CODE(mAdoRs1("CODA") & "/" & mAdoRs1("SUBCODA"))
...
Private Sub cmdAppend_Click(Index As Integer)
    ...
    Dim strRstval       As String, intCnt       As Integer
...
AgainInput:
    ...
    If Len(sNo) > 0 And spdResult1.MaxRows > 0 Then
        If Not IsNumeric(sNo) Then
            MsgBox "숫자만 입력하세요.!", vbCritical, INS_NAME
            GoTo AgainInput
        End If
...
Private Sub ComReceive(ByRef RecData As String)
    ...
    strRec = ""
    If InStr(strReceiveTmp, "") = 0 Then
        strRec = RecData
        strReceiveTmp = ""
    Else
        strRec = strReceiveTmp & RecData
    End If
    ...
    For intIdx = 1 To Len(strRec)
        ...
        sStartCheck = InStr(f_strBuffer, "")
        sEndCheck = InStr(f_strBuffer, "")
        If sStartCheck <> 0 And sEndCheck <> 0 Then
            If Len(strRec) > 121 Then
                strReceiveTmp = Mid(strRec, 122)
            Else
                strReceiveTmp = ""
            End If
...
Dim sqlDoc  As String
Dim intCol As Integer, i As Integer, y As Integer
Dim tmpTstCd As String
Dim sSeq, varTmp, strBarno, strDate, strTime As String
Dim intIdx      As Integer
Dim strRorder   As String
Dim intPos          As String, strTestcd    As String, strTestRst   As String
Dim Resulttmp(18) As String
    
    On Error Resume Next

    If Mid(strdata, 4, 1) = "U" Then    'sample distinction code ( Analysis data : U, QC : C )
    
        strTestRst = Mid(strdata, 36, 90)
        'strDta = "D1U201101140000000000000013000000001681039300118003480088500300003390039900219000570072400037000100012100453001360007420066200592" 'D1U070409"
        'D1U1702210000000000004000000S007700392001310037800964003340034700120002120007200716000160000600055005210017410116003980{ETX}
        '00700 04590 01450 04210 09170 03160 03440 01790 02770 00700 06530 00190 00050 00460 04580 01360 01040 02830
        For i = 1 To Len(strTestRst) Step 5
            y = y + 1
            Resulttmp(y) = Mid(strTestRst, i, 5)
'            Debug.Print Resulttmp(y)
        Next i

        pGrid_Point = 0
    
        With spdResult1
            'sSeq = Val(Mid(strdata, 13, 16))
            ...
            If pGrid_Point > 0 Then
                For intCol = pIntcol To .MaxCols
                    ...
                    'If .BackColor = OrderColor Then
                        ...
                        Set itemX = lvwCuData.FindItem(Trim$(varTmp), lvwSubItem, , lvwWhole)
                        If Not itemX Is Nothing Then
                            For intIdx = 1 To UBound(Resulttmp)

                                Channel_No = intIdx
                                If Trim(Channel_No) = Trim(itemX.Tag) Then
                                    
                                    strRstval = Trim(IIf(Mid(Resulttmp(intIdx), 1, 1) = "*", "", Resulttmp(intIdx)))
                                   
                                    If strRstval <> "" Then
                                        Select Case intIdx
                                        Case 2, 20       '소수 둘째 자리
                                            strRstval = Val(Mid(strRstval, 1, 2)) & "." & Mid(strRstval, 3, 3)
                                        Case 8       '정수
                                            strRstval = Val(Mid(strRstval, 1, 5))
                                        Case Else    '소수 첫째 자리
                                            strRstval = Val(Mid(strRstval, 1, 3)) & "." & Mid(strRstval, 4, 2)
                                        End Select
...
Private Sub Command1_Click()
    ...
    strDta = "D1U201101140000000000000013000000001681039300118003480088500300003390039900219000570072400037000100012100453001360007420066200592" 'D1U070409"
             'D1U1702210000000000004000000S007700392001310037800964003340034700120002120007200716000160000600055005210017410116003980{ETX}
'   strDta = "D1U9709300000000000005111111S1234F5678F9123F4567F8901F2345F6789F****F1234F1234F5678F4321F9876F****F1234F1234F1234F1234F"
    Call ComReceive(strDta)
...
'    Me.Show
    ...
    Open App.Path + "\" + "Sysmex_KX21.log" For Append As #1
