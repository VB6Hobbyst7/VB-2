...
On Error GoTo RST
    ...
    '조회내용
    'LID     , Hcode    , Serial        , PtName, ROrder    , Orderdate , ErYN
    '접수번호, 병록번호 , 원무일련번호  , 환자명, 처방순번  , 처방일자  , 응급여부(0:일반,1:응급)
        ...
        Do Until RS.EOF
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, Trim(RS.Fields("SERIAL")) & "", intRow, colCHARTNO
                    SetText SPD, Trim(RS.Fields("PTNAME")) & "", intRow, colPNAME
                    SetText SPD, IIf(Trim(RS.Fields("ERYN")) & "" = "0", "일반", "응급"), intRow, colINOUT
                    SetText SPD, CStr(iCnt), intRow, colOCNT
...
'    End If
    ...
    SQL = "EXEC AP_INF_L_GetOrder '" & gHOSP.MACHCD & "', '" & strBarcode & "'"
    ...
    'Coda,      SubCoda,    Sys_Code,       ROrder,     Lid,            Serial,         HCode,          PtName
    '검사코드,  검사부코드, 장비코드(무시), 처방순번,   접수(검체)번호, 원무일련번호,   환자차트번호,   환자성명
...
'                SetText SPD, Trim(RS.Fields("ORDERDATE")) & "", asRow, colHOSPDATE
                ...
                SetText SPD, Trim(RS.Fields("SERIAL")) & "", asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("PTNAME")) & "", asRow, colPNAME
                'SetText SPD, IIf(Trim(RS.Fields("ERYN")) & "" = "0", "일반", "응급"), asRow, colINOUT
                ...
                Call SetSQLData("바코드조회", gPatOrdCd)
            End With
            DoEvents
