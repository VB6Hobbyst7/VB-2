...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                '-- 결과업데이트
                         SQL = "Update TW_HSP_OCS.TWEXAM_RESULTC "
                SQL = SQL & "   Set STATUS     = '4',"                  '검사상태
                SQL = SQL & "       RESULT     = '" & sResult & "',"  '검사결과
                SQL = SQL & "       RESULTDATE = TRUNC(SYSDATE) "       '검사전송시간
                SQL = SQL & " Where SPECNO     = '" & lsID & "'"        '검체번호
                '-- HGB
                If strEqpCd = "HR06" Then
                    SQL = SQL & "   And MASTERCODE IN ('HR01', 'HR06') "
                '-- HCT
                ElseIf strEqpCd = "HR07" Then
                    SQL = SQL & "   And MASTERCODE IN ('HR01', 'HR07') "
                Else
                    SQL = SQL & "   And MASTERCODE = 'HR01'"
                End If
                SQL = SQL & "   And SUBCODE = '" & strEqpCd & "'"    '검사코드
                
                'SQL = SQL & "   And MASTERCODE = '" & Mid(strEqpCd, 1, 4) & "'"
                'SQL = SQL & "   And SUBCODE = '" & Mid(strEqpCd, 5) & "'"    '검사코드
...
'    SQL = SQL & " WHERE QC_BAR_NO = '" & argPID & "'" & vbCrLf
    'SQL = SQL & "   AND ITEM_CD IN (" & gAllExam & ")" & vbCrLf
          ...
          SQL = " SELECT DISTINCT L.처방코드 + L.서브코드 AS EXAMCODE " & vbCrLf
    SQL = SQL & "   FROM TB_진료검사 L " & vbCrLf
    SQL = SQL & "  Where L.검체번호 = '" & argPID & "'" & vbCrLf
    SQL = SQL & "    AND L.검사종류 = '" & gDept_Code & "'" & vbCrLf
    SQL = SQL & "    AND L.검사상태 < 5 " & vbCrLf
    SQL = SQL & "    AND L.처방코드 + L.서브코드 IN (" & gAllExam & ")" & vbCrLf
    SQL = SQL & "  ORDER BY L.처방코드 + L.서브코드"
