...
Begin VB.Form frmIISArchitect 
   ...
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16773087
      SpreadDesigner  =   "frmIISArchitect.frx":04CB
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16773087
      SpreadDesigner  =   "frmIISArchitect.frx":0D37
      TextTip         =   2
   End
End
...
Private Sub EditRcvData()
    ...
    Dim strRack      As String   '수신한 Rack
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        Select Case strType
            ...
            Case "Q"    '## Request Information
                ...
                strRack = Trim$(mGetP(mGetP(strRcvBuf, 3, "|"), 3, "^"))
                strPos = Trim$(mGetP(mGetP(strRcvBuf, 3, "|"), 4, "^"))
                
                If InStr(strOrgBarNo, "?") > 0 Then
                    strOrgBarNo = mIntLib.GetBarcodeNo("RFM", strRack, strPos)
            ...
            Case "O"    '## Order
                ...
                strOrgBarNo = mGetP(strTemp, 1, "^")
                strBarNo = Mid(strOrgBarNo, 1, 10)
                strSeg = mGetP(strTemp, 2, "^")
                strRack = mGetP(strTemp, 2, "^")
            ...
            Case "R"    '## Result
                ...
                If strIntBase = "628" Or strIntBase = "131" Or strIntBase = "301" Or strIntBase = "291" Then
                    Select Case strFlag
                        Case "F"    '## 정량
                ...
                If blnSave = True Then
                    If objIntNms.ExistIntBase(strIntBase) Then
                        Call objIntInfo.IntResults.Add(strIntBase, objIntNms.GetIntNm(strIntBase), _
                             strResult, strResult, strPos & "/" & strSeg)
                        mIntLib.State = "R"
                    End If
                End If
                '-- Edit by Sewon,Oh
                'Approved patient results =  Collated
                'Approved patient results <> Single5
                '## DB에 결과저장
...
Private Sub GetOrder(ByVal pBarNo As String)
    Dim objAccInfo As clsIISAccInfo     '접수내역 클래스Item
    ...
    Select Case mIntLib.SndPhase
        ...
        Case 2  '## Patient
            'strOutput = mIntLib.GetFrameNo & "P|1" & vbCr & ETX
                                           ...
                                           '2P|1|1234567890             |    1234567890         |    1234567890         ||||||||||0000-1111
            strOutput = mIntLib.GetFrameNo & "P|1|" & Mid(mOrder.OrgBarNo, 5, 4) & "|" & Mid(mOrder.OrgBarNo, 5, 4) & "|" & Mid(mOrder.OrgBarNo, 5, 4) & "||||||||||" & Mid(mOrder.OrgBarNo, 5, 4) & vbCr & ETX

            mIntLib.SndPhase = 4

        Case 3  '## No Order
            'strOutput = mIntLib.GetFrameNo & "Q|1|^" & mOrder.BarNo & "^" & mOrder.RackNo & "^" & mOrder.PosNo & "||^^^ALL||||||||X" & vbCr & ETX
            strOutput = mIntLib.GetFrameNo & "Q|1|^???????????^" & mOrder.RackNo & "^" & mOrder.PosNo & "||^^^ALL||||||||X" & vbCr & ETX
        ...
        Case 4  '## Order
            With mOrder
                If .IsSending = False Then  '## 최초 보낼때
                    ...
                    'strOutput = "O|1|" & .OrgBarNo & "||" & .GetOrder & "|R||||||N||||||||||||||Q"
                    '-- Using Sample Cup
                    '2Q|1|^Z1500000011^50030^1||^^^ALL||||||||O
                    strOutput = "O|1|" & .OrgBarNo & "^" & .RackNo & "^" & .PosNo & "||" & .GetOrder & "|||||||N||||||||||||||Q"
                    'strOutput = "O|1|???????????^" & .RackNo & "^" & .PosNo & "||" & .GetOrder & "|||||||N||||||||||||||Q"
