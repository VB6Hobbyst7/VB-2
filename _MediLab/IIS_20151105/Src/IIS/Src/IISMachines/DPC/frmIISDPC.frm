...
Begin VB.Form frmIISDPC 
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISDPC.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16773087
      SpreadDesigner  =   "frmIISDPC.frx":04FA
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16773087
      SpreadDesigner  =   "frmIISDPC.frx":0D3C
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISDPC"
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        Select Case strType
            ...
            Case "Q"    '## Request Information
                ...
                ElseIf OPTVERSION = "1.3" Then      '## Version 1.3
                    ...
                    With mOrder
                        ...
                        .Pos = mGetP(strTemp, 6, "^")
                        .RackType = mGetP(strTemp, 8, "^")
                        .ConType = mGetP(strTemp, 9, "^")
                    End With
                End If
            ...
            Case "O"    '## Order
                ...
                With objIntInfo
                    .BarNo = strBarNo
                    .SpcPos = strPos & "/" & Mid$(strRackNo, 2)
                End With

            Case "R"    '## Result
                strIntBase = Format$(mGetP(Replace$(mGetP(strRcvBuf, 3, "|"), "^^^", ""), 1, "/"), "00")
...
Private Sub SendOrder()
    ...
    Select Case mIntLib.SndPhase
        ...
        Case 0      '## 최초 오더 전송인 경우
            ...
            '## Order
            '## NOTE: 버전1.3의 경우 접수정보가 없을때 꼭 확인할것!
            If mOrder.NoOrder = False Then
                '## 접수정보가 있는경우: 검체유형, 응급유무는 수신한것을 따름!
                If OPTVERSION = "1.0" Then      '## Version 1.0
                    strOutput = strOutput & "O|1|0^" & mOrder.BarNo & "^" & mOrder.SpcType & "^" & _
                                mOrder.RackNo & "^" & mOrder.Pos & "|" & mOrder.Kind & "|" & _
                                mOrder.GetOrder & "|" & mOrder.Priority & "||||||N||^^||||||^^^^||||||O" & vbCr
                ElseIf OPTVERSION = "1.3" Then  '## Version 1.3
                    strOutput = strOutput & "O|1|" & Trim$(mOrder.BarNo) & "|" & mOrder.GetInstSpcId & _
                                "|" & mOrder.GetOrder & "|" & mOrder.GetPriority & "||||||A||||" & _
            ...
            Else
                ...
                If OPTVERSION = "1.0" Then      '## Version 1.0
                    strOutput = strOutput & "O|1|0^" & mOrder.BarNo & "^" & mOrder.SpcType & "^" & _
                                mOrder.RackNo & "^" & mOrder.Pos & "|" & mOrder.Kind & "||" & _
                                mOrder.Priority & "||||||N||^^||||||^^^^||||||O" & vbCr
                ElseIf OPTVERSION = "1.3" Then  '## Version 1.3
                    strOutput = strOutput & "O|1|" & Trim$(mOrder.BarNo) & "|" & mOrder.GetInstSpcId & _
                                "||" & mOrder.GetPriority & "||||||A||||" & mOrder.GetSampleType & _
                                "||||||||||O" & vbCr
                End If
            End If
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    With tblReady
        ...
        .Col = TReadyEnum.ccNo:     .Text = mOrder.Pos & "/" & Mid$(mOrder.RackNo, 2)
...
Private Sub SetComplete2(ByVal pAccInfo As clsIISAccInfo)
    ...
    With tblComplete
        ...
        .Col = TCompleteEnum.ccNo:      .Text = pAccInfo.DeptNm 'pAccInfo.SpcPos
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISDPC", "GetEqpComm", strErrMsg, Now
