...
Function SaveTransData_JBUNIV(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        strPatNm = Trim(GetText(SPD, argSpcRow, colPNAME))
        strChartNo = Trim(GetText(SPD, argSpcRow, colCHARTNO))
        strSpecimen = Trim(GetText(SPD, argSpcRow, colSPECIMEN))
...
''                strJudge = RsLocal.Fields("REFJUDGE").Value & ""
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If sResult <> "" Then
                    strInCD = strInCD & strTestCd & "^" & strSpecimen & "▦"
                    strInVal = strInVal & sResult & "▦"
...
'SetRawData strInVal
                ...
                With AdoCmd
                    ...
                    .CommandTimeout = 15
                    .CommandText = "pkg_ack_exlab.exlabrsltsave"
                    .CommandType = adCmdStoredProc
                
                    Set prm1 = .CreateParameter("in_hospcd", adVarChar, adParamInput, 2000, "A")
                    .Parameters.Append prm1
                    Set prm2 = .CreateParameter("in_bcno", adVarChar, adParamInput, 2000, strBarcode)
                    .Parameters.Append prm2
                    Set prm3 = .CreateParameter("in_testinfo", adVarChar, adParamInput, 2000, strInCD)
                    .Parameters.Append prm3          '검사코드^검체코드▦검사코드^검체코드▦검사코드^검체코드
                    Set prm4 = .CreateParameter("in_totalcnt", adVarChar, adParamInput, 2000, CStr(intTotCnt))
                    .Parameters.Append prm4
                    Set prm5 = .CreateParameter("in_pid", adVarChar, adParamInput, 2000, strPatID)
                    .Parameters.Append prm5
                    Set prm6 = .CreateParameter("in_rsltinfo", adVarChar, adParamInput, 2000, strInVal)
                    .Parameters.Append prm6        '검사결과▦검사결과
                    Set prm7 = .CreateParameter("in_rstflg", adVarChar, adParamInput, 2000, "3")        '상태: 1(결과저장), 2(중간보고), 3(최종보고)
                    .Parameters.Append prm7
                    Set prm8 = .CreateParameter("in_custcd", adVarChar, adParamInput, 2000, gHOSP.MACHCD)
                    .Parameters.Append prm8     '장비코드
                    Set prm9 = .CreateParameter("in_id", adVarChar, adParamInput, 2000, gHOSP.USERID)
                    .Parameters.Append prm9         '사용자아이디
                    Set prm10 = .CreateParameter("in_ip", adVarChar, adParamInput, 2000, CStr("192.1.106.165"))
                    .Parameters.Append prm10    '아이피주소
                
                    SetRawData "[prm2]" & strBarcode
                    SetRawData "[prm3]" & strInCD
                    SetRawData "[prm4]" & CStr(intTotCnt)
                    SetRawData "[prm5]" & strPatID
                    SetRawData "[prm6]" & strInVal
                    SetRawData "[prm7]" & "1"
                    SetRawData "[prm8]" & gHOSP.MACHCD
                    SetRawData "[prm9]" & gHOSP.USERID
                    SetRawData "[prm10]" & CStr("192.1.106.165")
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "SaveTransData_JBUNIV" & vbNewLine & vbNewLine
...
Public Sub GetTestListName()
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT GUBUN,SEQNO,SENDCHANNEL,RSLTCHANNEL,ABBRNAME,TESTNAME " & vbCrLf
'    SQL = SQL & " ,RESPRECUSE,RESPREC,REFMLOW,REFMHIGH,REFFLOW,REFFHIGH " & vbCrLf
    SQL = SQL & "  FROM EQPMASTER " & vbCr
    SQL = SQL & " WHERE EQUIPCD = '" & gHOSP.MACHCD & "'" & vbCr
    SQL = SQL & " ORDER BY GUBUN,SEQNO "
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        ...
        Do Until AdoRs_Local.EOF
            ...
            gArrEQPNm(intRow, 7) = AdoRs_Local.Fields("TESTNAME").Value & ""
...
'-- 검사마스터 조회
Public Sub GetTestMaster(ByVal SPD As Object, Optional ByVal pGubun As Integer)
    Dim intRow          As Long
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        With SPD
            ...
            Do Until AdoRs_Local.EOF
                ...
                Call SetText(SPD, AdoRs_Local.Fields("GUBUN").Value & "", intRow, colLGUBUN)
                ...
                Call SetText(SPD, AdoRs_Local.Fields("EXAMTYPE").Value & "", intRow, colLTYPE)
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    ...
    Select Case gEMR
        ...
        Case "JBUNIV"
                Call GetWorkList_JBUNIV(pFrom, pTo, SPD)
...
Public Sub GetWorkList_JBUNIV(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    pBegin = frmMain.txtStartNum
    pEnd = frmMain.txtStopNum


    SQL = ""
    SQL = SQL & " SELECT "
    SQL = SQL & "     j011m.colldt                              AS HOSPDATE " & vbCrLf
    SQL = SQL & "   , j011m.bcno                                AS BARCODE  " & vbCrLf
    'SQL = SQL & "   , j010m.bcprtno AS CHARTNO" & vbCr
'    SQL = SQL & "   , r010m.WKYMD||r010m.WKGRPCD||r010m.WKNO    AS FLWKNO   " & vbCrLf
    SQL = SQL & "   , r010m.WKNO                                AS CHARTNO  " & vbCrLf
    SQL = SQL & "   , j011m.regno                               AS PID      " & vbCrLf
    SQL = SQL & "   , j010m.patnm                               AS PNAME    " & vbCrLf
    SQL = SQL & "   , j010m.age                                 AS AGE      " & vbCrLf
    SQL = SQL & "   , j010m.sex                                 AS SEX      " & vbCrLf
    SQL = SQL & "   , j011m.IOGBN                               AS INOUT    " & vbCrLf
    SQL = SQL & "   , j010m.DEPTCD                              AS DEPT     " & vbCrLf
    SQL = SQL & "   , j010m.WARDNO                              AS WARD     " & vbCrLf
    SQL = SQL & "   , j010m.ROOMNO                              AS ROOM     " & vbCrLf
'    SQL = SQL & "   , f72m.TESTCD                               AS ITEM     " & vbCrLf
    SQL = SQL & "   , r010m.SPCCD                               AS SPCCD    " & vbCrLf
    SQL = SQL & "  FROM LJ011M j011m                                        " & vbCrLf
    SQL = SQL & "       INNER JOIN LJ010M j010m                             " & vbCrLf
    SQL = SQL & "               ON j011m.bcno               = j010m.bcno    " & vbCrLf
    SQL = SQL & "              AND j011m.regno              = j010m.regno   " & vbCrLf
    SQL = SQL & "       INNER JOIN LR010M r010m                             " & vbCrLf
    SQL = SQL & "               ON j011m.bcno               = r010m.bcno    " & vbCrLf
    SQL = SQL & "              AND j011m.regno              = r010m.regno   " & vbCrLf
    SQL = SQL & "              AND NVL(r010m.rstflg,'0')    = '0'           " & vbCrLf
    SQL = SQL & "       INNER JOIN LF072M f72m                              " & vbCrLf
    SQL = SQL & "               ON f72m.eqcd        = '" & gHOSP.MACHCD & "'" & vbCrLf
'    SQL = SQL & "              AND f72m.testcd     IN (" & gAllTestCd & ")  " & vbCrLf
    SQL = SQL & "              AND f72m.testcd     IN ('52004','52219')  " & vbCrLf
    SQL = SQL & "              AND r010m.testcd     = f72m.testcd           " & vbCrLf
    SQL = SQL & " WHERE j011m.colldt BETWEEN '" & pFrom & "' AND '" & pTo & "'" & vbCrLf
    SQL = SQL & "   AND r010m.wkno between '" & pBegin & "' AND '" & pEnd & "'" & vbCrLf
    SQL = SQL & "   AND j011m.spcflg            = '4'                       " & vbCrLf
    SQL = SQL & "   AND NVL(j011m.rstflg, '0')  = '0'                       " & vbCrLf
    SQL = SQL & " UNION                                                     " & vbCrLf
    SQL = SQL & " SELECT "
    SQL = SQL & "     j011m.colldt                              AS HOSPDATE " & vbCrLf
    SQL = SQL & "   , j011m.bcno                                AS BARCODE  " & vbCrLf
    'SQL = SQL & "   , j010m.bcprtno AS CHARTNO" & vbCr
'    SQL = SQL & "   , r010m.FLWKNO                              AS FLWKNO   " & vbCrLf
    SQL = SQL & "   , r010m.WKNO                                AS CHARTNO  " & vbCrLf
    SQL = SQL & "   , j011m.regno                               AS PID      " & vbCrLf
    SQL = SQL & "   , j010m.patnm                               AS PNAME    " & vbCrLf
    SQL = SQL & "   , j010m.age                                 AS AGE      " & vbCrLf
...
'    SQL = SQL & "              AND f72m.testcd     IN (" & gAllTestCd & ")  " & vbCrLf
    ...
    SQL = SQL & "   AND j011m.spcflg            = '4'                       " & vbCrLf
    SQL = SQL & "   AND NVL(j011m.rstflg, '0')  = '0'                       " & vbCrLf
   ' SQL = SQL & " ORDER BY j011m.colldt                                     " & vbCrLf
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        Case "JBUNIV"
                Call GetSampleInfo_JBUNIV(asRow, SPD) '
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_JBUNIV(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    SQL = SQL & "              AND f72m.testcd     IN (" & gAllTestCd & ")  " & vbCrLf
    ...
    If mResult.Kind <> "" Then
        If mResult.Kind = "IN" Then
            SQL = SQL & "              AND f72m.testcd     IN ('52219')  " & vbCrLf
        Else
            SQL = SQL & "              AND f72m.testcd     IN ('52004')  " & vbCrLf
        End If
    Else
        SQL = SQL & "              AND f72m.testcd     IN ('52004','52219')  " & vbCrLf
    End If
...
'    SQL = SQL & " WHERE j011m.colldt BETWEEN '" & pFrom & "' AND '" & pTo & "'" & vbCrLf
'    SQL = SQL & "   AND r010m.wkno between '" & pBegin & "' AND '" & pEnd & "'" & vbCrLf
'    SQL = SQL & " WHERE j011m.bcno              = '" & strBarcode & "'      " & vbCrLf
    SQL = SQL & " WHERE j011m.bcno              = fn_ack_get_bcno_normal(" & mOrder.OrgBarNo & ")" & vbCrLf
    SQL = SQL & "   AND j011m.spcflg            = '4'                       " & vbCrLf
    SQL = SQL & "   AND NVL(j011m.rstflg, '0')  = '0'                       " & vbCrLf
    SQL = SQL & " UNION                                                     " & vbCrLf
    SQL = SQL & " SELECT "
...
'    SQL = SQL & " WHERE j011m.bcno              = '" & strBarcode & "'      " & vbCrLf
    ...
    SQL = SQL & "   AND j011m.spcflg            = '4'                       " & vbCrLf
    SQL = SQL & "   AND NVL(j011m.rstflg, '0')  = '0'                       " & vbCrLf
    'SQL = SQL & " ORDER BY FLWKNO                                           " & vbCrLf
...
'                SetText SPD, Trim(Rs.Fields("patient_kind")) & "", asRow, colINOUT
                ...
                '수정해야함
                If Trim(RS.Fields("ITEM")) & "" = "52219" Then
                    SetText SPD, "IN", asRow, colINOUT
                ElseIf Trim(RS.Fields("ITEM")) & "" = "52004" Then
                    SetText SPD, "FD", asRow, colINOUT
                End If
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        Call SetText(SPD, "◇", asRow, intCol)
                        '-- 검체코드
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("SPCCD")) & ""
                        Exit For
                    End If
                Next
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_JBUNIV" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
