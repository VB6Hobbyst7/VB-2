...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            strSpcCd = Trim(GetText(.vasTemp, iRow, 15))    '검체코드
            ...
            If lsPid <> "" And sResult <> "" Then
                strInCD = strInCD & strEqpCd & "^" & strSpcCd & "▦"
        ...
        If strInCD <> "" And strInVal <> "" Then
            'strInCD = Mid(strInCD, 1, Len(strInCD) - 1)
            'strInVal = Mid(strInVal, 1, Len(strInVal) - 1)
            ...
            With cmdSQL
                ...
                '-- 결과저장 = pkg_ack_exlab.exlabrsltsave
                '  SpUpload.ProcedureName:= 'pkg_ack_exlab.exlabrsltsave';
                '  SpUpload.parameters.CreateParameter('in_hospcd', ftString, pdInput, 100, '');    //'A'
                '  SpUpload.parameters.CreateParameter('in_bcno', ftString, pdInput, 100, '');
                '  SpUpload.parameters.CreateParameter('in_testinfo', ftString, pdInput, 100, '');  //검사코드^검체코드▦검사코드^검체코드▦검사코드^검체코드
                '  SpUpload.parameters.CreateParameter('in_totalcnt', ftString, pdInput, 100, '');
                '  SpUpload.parameters.CreateParameter('in_pid', ftString, pdInput, 100, '');
                '  SpUpload.parameters.CreateParameter('in_rsltinfo', ftString, pdInput, 100, '');  //검사결과▦검사결과
                '  SpUpload.parameters.CreateParameter('in_rstflg', ftString, pdInput, 100, '');    //상태: 1(결과저장), 2(중간보고), 3(최종보고)
                '  SpUpload.parameters.CreateParameter('in_custcd', ftString, pdInput, 100, '');   //장비코드
                '  SpUpload.parameters.CreateParameter('in_id', ftString, pdInput, 100, '');      //사용자아이디            gIFUser
                '  SpUpload.parameters.CreateParameter('in_ip', ftString, pdInput, 100, '');      //아이피주소
                
                
                Set prm1 = .CreateParameter("in_hospcd", adVarChar, adParamInput, 100, "A"):  .Parameters.Append prm1
                Set prm2 = .CreateParameter("in_bcno", adVarChar, adParamInput, 100, lsID): .Parameters.Append prm2
                Set prm3 = .CreateParameter("in_testinfo", adVarChar, adParamInput, 100, strInCD): .Parameters.Append prm3        '검사코드^검체코드▦검사코드^검체코드▦검사코드^검체코드
                Set prm4 = .CreateParameter("in_totalcnt", adVarChar, adParamInput, 100, CStr(intTotCnt)):  .Parameters.Append prm4
                Set prm5 = .CreateParameter("in_pid", adVarChar, adParamInput, 100, lsPid):  .Parameters.Append prm5
                Set prm6 = .CreateParameter("in_rsltinfo", adVarChar, adParamInput, 100, strInVal):  .Parameters.Append prm6         '검사결과▦검사결과
                Set prm7 = .CreateParameter("in_rstflg", adVarChar, adParamInput, 100, "1"):  .Parameters.Append prm7               '상태: 1(결과저장), 2(중간보고), 3(최종보고)
                Set prm8 = .CreateParameter("in_custcd", adVarChar, adParamInput, 100, gEquipCode):  .Parameters.Append prm8        '장비코드
                Set prm9 = .CreateParameter("in_id", adVarChar, adParamInput, 100, gIFUser):  .Parameters.Append prm9               '사용자아이디
                Set prm10 = .CreateParameter("in_ip", adVarChar, adParamInput, 100, CStr(frmInterface.Winsock1.LocalIP)):  .Parameters.Append prm10    '아이피주소
                
                SetRawData "[prm2]" & lsID
                SetRawData "[prm3]" & strInCD
                SetRawData "[prm4]" & CStr(intTotCnt)
                SetRawData "[prm5]" & lsPid
                SetRawData "[prm6]" & strInVal
                SetRawData "[prm7]" & "1"
                SetRawData "[prm8]" & gEquipCode
                SetRawData "[prm9]" & gIFUser
                SetRawData "[prm10]" & CStr(frmInterface.Winsock1.LocalIP)
