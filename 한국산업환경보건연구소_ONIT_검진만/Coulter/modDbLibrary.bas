...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        strPerGumDate = Trim(GetText(.vasID, argSpcRow, colHOSPDATE))
...
'                SQL = SQL & "   And OKFL <> 'Y' "   '-- 결과확정유무
                ...
                SQL = SQL & " Where PER_GUMJIN_DATE = '" & strPerGumDate & "'"
                SQL = SQL & "   And PER_GUM_NUM = " & Val(lsID) & ""
                SQL = SQL & "   And EDPSCODE = '" & strEqpCd & "'"
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_GSEN(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
...
'      -- 테이블 사용
    SQL = ""
    SQL = SQL & "Select DISTINCT a.ORDT as 접수일자,'0',b.PANM as 이름,a.SPNO as 바코드번호,a.PAID as 챠트번호,a.OIFL,'0',b.SEXS as 성별,b.AGES as 나이,a.WKNO as 내원번호,a.ORCD as ITEM,a.ORQN as ITEMSEQ " & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, IIf(Trim(RS.Fields("OIFL")) = "I", "입원", "외래"), asRow, colINOUT
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_ONIT(ByVal asRow As Long) As Integer
    Dim sGumDate    As String
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    ...
    GetSampleInfoW_ONIT = -1
    strORQN = ""
    
    
    sGumDate = Trim(GetText(frmInterface.vasID, asRow, colHOSPDATE))
    sBarcode = Val(Trim(GetText(frmInterface.vasID, asRow, colBARCODE)))
    
    If sBarcode = "" Or sGumDate = "" Then
        Exit Function
    End If
...
'      -- 테이블 사용
    ...
    SQL = ""
    SQL = SQL + vbLf + "SELECT DISTINCT PER_GUMJIN_DATE as 접수일자, PER_NAME as 이름, PER_GUM_NUM as 바코드번호, EDPSCODE as ITEM "
    SQL = SQL + vbLf + "  FROM ONIT..GUMJIN_INTERFACE "
    SQL = SQL + vbLf + " WHERE PER_GUMJIN_DATE = '" & sGumDate & "' "
    SQL = SQL + vbLf + "   AND PER_GUM_NUM = '" & sBarcode & "' "
    SQL = SQL + vbLf + "   AND EDPSCODE IN (" & gAllExam & ") "
    SQL = SQL + vbLf + "   AND RESULT = ''  OR RESULT IS NULL "
    SQL = SQL + vbLf + " ORDER BY 접수일자, 이름"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("ITEM")) & "',"
                'strORQN = strORQN & Trim(RS.Fields("ITEM")) & ","
