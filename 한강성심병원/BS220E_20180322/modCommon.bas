...
'    gQCEquip = Trim(frmInterface.txtTemp)
    ...
    db_tmp = ""
    Call GetPrivateProfileString("config", "TPort", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gPort = Trim(frmInterface.txtTemp)
...
Public Sub SetEditData(argSQL As String)
'argSQL의 내용을 파일로 저장
    Dim FilNum
    Dim sFileName As String
...
Public Function getMaxTestNum(ByVal strDate As String) As String
...
'''
'''
'''          SQL = "SELECT TOP 1 SAVESEQ as SEQ, MAX(EXAMDATE) "
'''    SQL = SQL & "  FROM PATRESULT "
'''    SQL = SQL & " GROUP BY SAVESEQ"
'''    SQL = SQL & " ORDER BY EXAMDATE "
'''
...
'''            getMaxTestNum = 1
'''        Else
'''            If InStr(Trim(gReadBuf(0)), "-") > 0 Then
'''                getMaxTestNum = mGetP(Trim(gReadBuf(0)), 1, "-") + 1
'''            Else
...
'    End If
          ...
          SQL = "SELECT SAVESEQ FROM PATRESULT  "
    SQL = SQL & " WHERE MID(EXAMDATE,1,8) = '" & strDate & "' " & vbCrLf
    'SQL = SQL & " ORDER BY SAVESEQ  "
    ...
    For i = 0 To UBound(gReadBuf)
        If gReadBuf(i) <> "" Then
            strTmp = mGetP(Trim(gReadBuf(i)), 1, "-")
            If Val(strTmp) > Val(strTmp1) Then
                getMaxTestNum = strTmp
            End If
            strTmp1 = strTmp
'            strTmp = strTmp & "'" & Trim(gReadBuf(i)) & "',"
       ' Else
       '     Exit For
        End If
    Next
    
    If getMaxTestNum <> "" Then
        getMaxTestNum = getMaxTestNum + 1
    Else
        getMaxTestNum = 1
    End If
...
'-- 오늘 검사한 날짜의 Max + 1 번호를 가져온다
Public Function getMaxNum(ByVal strDate As String) As String
    Dim strTmp As String
    Dim strTmp1 As String
    Dim i As Integer
    ...
    If Res > 0 Then
        If Trim(gReadBuf(0)) = "" Then
            getMaxNum = 1
        Else
            getMaxNum = Trim(gReadBuf(0)) + 1
        End If
    End If

    If getMaxNum >= 99999 Then
        getMaxNum = 99999
    End If
