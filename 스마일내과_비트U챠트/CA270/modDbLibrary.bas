...
Function SaveTransDataW_BITUCHART(ByVal argSpcRow As Integer) As Integer
    Dim iRow            As Integer
    Dim lsID            As String
...
'                SQL = SQL & "   AND (RESREPTYP = 'N' OR RESREPTYP = '' OR RESREPTYP IS NULL " 'N : 미결과
                
                
'                '-- 결과업데이트
                      SQL = "Update ResInf "
                SQL = SQL & "   Set ResRltVal = '" & sResult & "'" & vbCrLf   '검사결과
'                SQL = SQL & "      ,ResRepTyp = 'I' " & vbCrLf
'                SQL = SQL & "      ,ResStatus = '4' " & vbCrLf
                SQL = SQL & " Where ResOcmNum = '" & Space(10 - Len(lsPid)) & lsPid & "'" & vbCrLf            '10
'                SQL = SQL & "   And ResOdrSeq = '" & mGetP(strSubCodes, 1, "|") & "'" & vbCrLf
'                SQL = SQL & "   And ResSeq    = '" & mGetP(strSubCodes, 2, "|") & "'" & vbCrLf
'                SQL = SQL & "   And ResSubSeq = '" & mGetP(strSubCodes, 3, "|") & "'" & vbCrLf
                SQL = SQL & "   And RESLABCOD = '" & strEqpCd & "'"    '검사코드
                SQL = SQL & "   And (ResRltVal = '' OR ResRltVal IS NULL)"
...
'        '-- 상태업데이트
'              SQL = "Update OspInf "
'        SQL = SQL & "   Set OspChkstt  = 'M' " & vbCrLf                 '검사상태(중간보고)
'        SQL = SQL & " Where OspChkstt <> 'F' " & vbCrLf
'        SQL = SQL & "   And OspOcmNum = '" & Space(10 - Len(lsPid)) & lsPid & "'" & vbCrLf            '10
'        SQL = SQL & "   And OspOdrSeq = '" & mGetP(strSubCodes, 1, "|") & "'" & vbCrLf
'        SQL = SQL & "   And OspSeq    = '" & mGetP(strSubCodes, 2, "|") & "'" & vbCrLf
'        SQL = SQL & "   And OSPCHTNUM = '" & Space(14 - Len(strChtNum)) & strChtNum & "'"                 '14
'
...
Function SaveTransDataW_BIT(ByVal argSpcRow As Integer) As Integer
    Dim iRow            As Integer
    Dim lsID            As String
    Dim strDate         As String
...
On Error GoTo ErrHandle
    ...
    With frmInterface
        SaveTransDataW_BIT = -1
        
        lsID = Trim(GetText(.vasID, argSpcRow, colCHARTNO))
        lsPid = Trim(GetText(.vasID, argSpcRow, colPID))
        strExamDate = Trim(GetText(.vasID, argSpcRow, colEXAMDATE))
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                SQL = SQL & "   And LABODRDTE = '" & Format(Mid(strExamDate, 1, 8), "####-##-##") & "'" & vbCr
                SQL = SQL & "   And LABCHTNUM = '" & lsID & "'" & vbCr  '챠트번호
                ...
                If Res = 1 Then
                    ...
                    SQL = SQL & "   And DATODRDTE ='" & Format(Mid(strExamDate, 1, 8), "####-##-##") & "'" & vbCr
                    SQL = SQL & "   And DATCHTNUM = '" & lsID & "'" & vbCr  '챠트번호
...
'            Else
                If sResult <> "" Then
                    ...
                    With cmdSQL
                        .CommandTimeout = 15
                        '-- 서울의료원
                        '.CommandText = "PR_CPL_CPL0891_INSERT"
                        '-- 인천성모
                        .CommandText = "PR_IHIS_LISDIE_RESULT_CREATE"
                        .CommandType = adCmdStoredProc
                        
                        Set prm1 = .CreateParameter("I_SPECIMEN_SER", adVarChar, adParamInput, 4000, lsID)
                        .Parameters.Append prm1
                        Set prm2 = .CreateParameter("I_HANGMOG_CODE", adVarChar, adParamInput, 4000, strEqpCd)
                        .Parameters.Append prm2
                        Set prm3 = .CreateParameter("I_JANGBI_CODE", adVarChar, adParamInput, 4000, gEquipCode)
                        ...
                        Set prm7 = .CreateParameter("I_USER_ID", adVarChar, adParamInput, 4000, gUserID)
                        ...
                        SetSQLData "결과저장", lsID & "," & strEqpCd & "," & gEquipCode & "," & sResult & ",'',''," & gUserID & "," & strErrMsg
...
'                End If
'                SQL = SQL & " , ANALYZERCODE= '" & gMachCD & "'" 'GEMINI = 43
...
'      -- 테이블 사용
          SQL = "SELECT DISTINCT a.RECEIPTDATE as 접수일자, a.SPECIMENNUM as 바코드번호, a.IPDOPD, a.RECEIPTNO as 챠트번호, a.PTNO as 내원번호, a.SNAME as 이름, a.LABCODE as ITEM, a.ORDERCODE"
...
'    SQL = SQL & "   AND HANGMOG_CODE IN (" & gAllExam & ")"
    ...
    '-- 뷰 사용(국제성모)
          SQL = "SELECT DISTINCT JUBSU_DATE as 접수일자, BUNHO as 내원번호, SUNAME as 이름, AGE as 나이, SEX as 성별, " & vbCr
    SQL = SQL & "       ORDER_INPUT as 부서,ORDER_DATE,SUB_HANGMOG_CODE as ITEM, JANGBI_CODE " & vbCr
    SQL = SQL & "  FROM VW_IHIS_LISDIE_GUMSA_INFO_SUB " & vbCr
    SQL = SQL & " WHERE SPECIMEN_SER = '" & sBarcode & "'" & vbCr
    SQL = SQL & "   AND SUB_HANGMOG_CODE IN (" & gAllExam & ")"
...
'    바코드로 조회SQL = SQL & "  ORDER BY R.SPCMNO"
    SQL = ""
    ...
    SQL = SQL & "   AND O.ORDERCODE = '" & gOrderCD & "'"
...
Function GetSampleInfoW_BIT(ByVal asRow As Long) As Integer
    ...
    If sBarcode = "" Or Mid(sBarcode, 1, 7) = "NoOrder" Then
        Exit Function
    End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_BITUCHART(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim sDate      As String
    ...
    GetSampleInfoW_BITUCHART = -1
    
    sBarcode = Trim(GetText(frmInterface.vasID, asRow, colCHARTNO))
    sDate = Trim(GetText(frmInterface.vasID, asRow, colHOSPDATE))
    If sDate = "" Then
        sFrDt = Format(frmInterface.dtpStartDt.Value, "yyyymmdd")
        sToDt = Format(frmInterface.dtpStopDt.Value, "yyyymmdd")
    End If
...
'''    SQL = SQL & "  FROM RsbInf M, ResInf R, ospinf O, PBSINF P, LabMst E" & vbCrLf
'''    SQL = SQL & " WHERE M.Rsbacpnum = '" & sBarcode & "'" & vbCrLf
'''    SQL = SQL & "   And M.RsbAckStt <> 'A' " & vbCrLf
'''    SQL = SQL & "   And O.OspChkStt <> 'F' " & vbCrLf
'''    SQL = SQL & "   And (R.ResRepTyp Is Null or R.ResRepTyp <> 'F') " & vbCrLf
'''    SQL = SQL & "   And (R.ResRltVal is Null or R.ResRltVal = '') " & vbCrLf
...
'''    SQL = SQL & "   and R.ResLabcod = E.LabCod" & vbCrLf
    ...
    SQL = SQL & "        R.RESOCMNUM AS 바코드번호, O.OCMCHTNUM AS 차트번호,R.RESOCMNUM AS 내원번호, '' AS 입외," & vbCrLf
    SQL = SQL & "        '' AS R, '' AS P, P.PBSPATNAM AS 이름, P.PBSSEXTYP AS 성별,'' AS 나이, R.RESLABCOD AS ITEM" & vbCrLf
    SQL = SQL & "        ,R.ResOdrSeq, R.ResSeq, R.ResSubSeq "
    ...
    If sDate <> "" Then
        SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & sDate & "000000" & "' AND '" & sDate & "235959" & "'" & vbCrLf
    Else
        SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & sFrDt & "000000" & "' AND '" & sToDt & "235959" & "'" & vbCrLf
    End If
    
    SQL = SQL & "   AND LTRIM(O.OCMCHTNUM) = '" & sBarcode & "'" & vbCrLf   '& Space(10 - Len(lsPid)) & lsPid
'    SQL = SQL & "   AND O.OCMACPDTM = '" & sDate & "'" & vbCrLf
    ...
    With frmInterface
        Do Until RS.EOF
            ...
            'SetText .vasID, Trim(RS.Fields("차트번호")), asRow, colCHARTNO         '챠트번호(결과상태 저장시 필요)
            'SetText .vasID, Trim(RS.Fields("내원번호")), asRow, colPID             '등록번호(결과     저장시 필요)
            'SetText .vasID, Trim(RS.Fields("이름")), asRow, colPNAME           '환자명
