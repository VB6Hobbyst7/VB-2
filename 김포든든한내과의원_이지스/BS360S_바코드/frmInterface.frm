...
Public Sub TCPRcvData_BS360S()
    ...
    Dim strType         As String   '수신한 Record Type
    Dim strBarno        As String   '수신한 바코드번호
    Dim strPID          As String   '수신한 바코드번호
    Dim strSeq          As String   '수신한 Sequence
    Dim strRackNo       As String   '수신한 Rack Or Disk No
    Dim strTubePos      As String   '수신한 Tube Position
    Dim strIntBase      As String   '수신한 장비기준 검사명
            ...
            Select Case strType
                Case "MSH"
                    ...
                    Select Case mOrder.MSHMessageType
                        '-- 검사결과수신 ACK
                        Case "ORU^R01"  '==> ACK^R01
                                           strSndBuffer = SB & "MSH|^~\&|||" & mOrder.BSMaker & "|" & mOrder.BSMchNm & "|" & mOrder.BSDtTm & "||ACK^R01|" & mOrder.MSHMessageID & "|" & mOrder.MSHProduct & "|" & mOrder.MSHHL7Version & "||||0||ASCII|||" & vbCr
                            strSndBuffer = strSndBuffer & "MSA|AA|" & mOrder.MSHMessageID & "|Message accepted|||0|" & vbCr
                            strSndBuffer = strSndBuffer & EB & vbCr
                        ...
                        '-- 오더요청수신
                        Case "QRY^Q02"  '==> QCK^Q02
                                           strSndBuffer = SB & "MSH|^~\&|||" & mOrder.BSMaker & "|" & mOrder.BSMchNm & "|" & mOrder.BSDtTm & "||QCK^Q02|" & mOrder.MSHMessageID & "|" & mOrder.MSHProduct & "|" & mOrder.MSHHL7Version & "||||0||ASCII|||" & vbCr
                            strSndBuffer = strSndBuffer & "MSA|AA|" & mOrder.MSHMessageID & "|Message accepted|||0|" & vbCr
                            strSndBuffer = strSndBuffer & "ERR|0|" & vbCr & EB & vbCr
                            strSndBuffer = strSndBuffer & "QAK|SR|OK|" & vbCr
                            strSndBuffer = strSndBuffer & EB & vbCr
                ...
                Case "QRD"
'[Rx]MSH|^~\&|||||20210729080525||QRY^Q02|1|P|2.3.1||||||ASCII|||
'QRD|20210729080525|R|D|1|||RD|00195808|OTH|||T|
'QRF||||||RCT|COR|ALL||
'
                    With mOrder
                        .QRDQryTime = mGetP(strRcvBuf, 2, "|")
                        ...
                        .BarNo = mGetP(strRcvBuf, 9, "|")
                        strOrgBarno = mOrder.BarNo
                        If IsNumeric(mOrder.BarNo) Then
                            strBarno = Val(mOrder.BarNo)
                            mOrder.BarNo = Val(mOrder.BarNo)
                        End If
                    End With
                    
                    If mOrder.MSHMessageType = "QRY^Q02" Then
                        Call GetOrder_BS360S(strBarno, gHOSP.RSTTYPE)
                    End If
...
'                    Call GetOrder_BS360S(strBarno, gHOSP.RSTTYPE)
                ...
                Case "OBR"
                    'MSH|^~\&|||||20210714115402||ORU^R01|4|P|2.3.1||||2||ASCII|||
                    'OBR|1|ALB|ALB|^|Serum|20210714082158|20210714081530|20210714083006|||1|1|Level-1|26481|20230731||M|4.400000|0.400000|4.390989|g/dL|||||||||||||||||||||||||||
                    
                    'MSH|^~\&|||||20210714124358||ORU^R01|43|P|2.3.1||||0||ASCII|||
                    'PID|2||||주교숙|||O|||||||||||||||||||||||
                    'OBR|2|193145|2|^|N|20210714122630|20210714122630|||1^2||||20210714122630|Serum|||||||||||||||||||||||||||||||||
                    '
                    If blnQC = True Then
                        'OBR|2|ALP|ALP|^|Serum|20210721081722|20210721075525|20210721082710|||1|1|Level-1|26481|20230731||M|122.000000|11.000000|143.657059|IU/L|||||||||||||||||||||||||||s
                        
                        strBarno = mGetP(strRcvBuf, 14, "|")
                        'strBarno = mGetP(strRcvBuf, 15, "|")
                        'strPID = mGetP(strRcvBuf, 14, "|")
                        ...
                        strIntBase = mGetP(strRcvBuf, 3, "|")
                        'strResult = Trim$(mGetP(strRcvBuf, 19, "|"))
                        strResult = Trim$(mGetP(strRcvBuf, 21, "|"))
                        strIntResult = strResult
...
ErrHandle:
    ...
    Call SetSQLData("에러", strErrMsg, "A")
...
'              TMaster.FBarCode:= TokenStr(sData,'|',9);
'              TMaster.FSampleNo:= TokenStr(sData,'|',5);
            ...
            strSend = strSend & "QRF||||||RCT|COR|ALL||" & vbCr
            ...
            'DSP|11||1^'+TMaster.FSampleNo+'|||'+ CR +                           //Sample Position
            ...
            strSend = strSend & "DSP|21||" & strOrgBarno & "|||" & vbCr      'barcode
