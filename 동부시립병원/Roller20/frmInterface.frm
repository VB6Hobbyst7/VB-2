...
'''    strType = Trim$(Mid$(strRcvBuf, 1, 1))
'''
'''    If strType = "R" Then
'''        '## 바코드번호 조회
'''        strBarNo = Mid$(Trim$(Mid$(strRcvBuf, 4, 15)), 1, SPCLEN)
'''        '-- RackNo/Pos 추가 s.w,oh (2007.03.28)
'''        strPos = Mid$(Trim$(Mid$(strRcvBuf, 19, 2)), 1, SPCLEN)
'''        strRackNo = Mid$(Trim$(Mid$(strRcvBuf, 21, 2)), 1, SPCLEN)
'''
...
'''        End With
'''
'''        '## 1.0.3: 이상대(2005-08-29)
'''        '   - 장비쪽에 "-004"가 전송되면 NF로 표시하도록 수정
'''        '## 검사결과 조회
'''        strIntBase = "ESR"
'''        strResult = Trim$(Mid$(strRcvBuf, 25, 4))
'''        If strResult = "-004" Then
'''            strResult = "NF"
'''        Else
...
'    varRcvBuf = Split(strRcvBuf, vbCrLf)
    ...
    If strType = "R" Then
        ...
        '-- RackNo/Pos 추가 s.w,oh (2007.03.28)
        strTubePos = Trim$(Mid$(strRcvBuf, 19, 2))
        strRackNo = Trim$(Mid$(strRcvBuf, 21, 2))
