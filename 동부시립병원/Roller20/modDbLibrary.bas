...
'      -- 테이블 사용
    ...
    SQL = SQL & "  FROM "
    SQL = SQL & "(SELECT * "
    SQL = SQL & "   FROM man.검사검체1V"
    SQL = SQL & "  WHERE 인수일자 ='2015-08-28'"
    SQL = SQL & "    AND 처방전코드='221'"
    SQL = SQL & "    AND 처리구분코드 IN ('P','E')) a,"
    SQL = SQL & "(SELECT * "
    SQL = SQL & "   FROM man.V_검사기품목 "
    SQL = SQL & "  WHERE   검사기코드='stago') b"
    SQL = SQL & " WHERE a.품목코드= b.품목코드"
