        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    'CBC는 ESR하고 바코드가 같아서 상태업데이트 하면 안된다.
'                    '-- 상태변경
'                    SQL = ""
...
On Error GoTo ErrHandle
    ...
    SQL = SQL & " GROUP BY R.RESULTDATE,P.PATID,P.PATNAME,R.SPCMNO          " & vbCrLf
...
Public Function GetEquipExamCode_XN550(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim strSendCH       As String
    Dim strCBC      As String
    Dim strDIFF     As String
    
    GetEquipExamCode_XN550 = ""
    strExamCode = ""
    ...
    If Trim(argEquipCode) = "" Or gPatOrdCd = "" Then
        ...
        GetEquipExamCode_XN550 = strExamCode
    ...
    If strExamCode <> "" Then
        GetEquipExamCode_XN550 = Mid(strExamCode, 1, Len(strExamCode) - 1)
    End If
...
'    End If
    ...
    SQL = SQL & "     , P.PRSNIDPRE         AS JUMIN1   " & vbCrLf
    SQL = SQL & "     , PRSNIDPOST          AS JUMIN2   " & vbCrLf
    SQL = SQL & "  From RESULTOFNUM R, REGISTINFOS L, PATMST P  " & vbCrLf
    SQL = SQL & " Where R.SPCMNO        = '" & strBarcode & "'  " & vbCrLf
    SQL = SQL & "   And R.ACPTDATE      =   L.ACPTDATE          " & vbCrLf
    SQL = SQL & "   And R.ACPTSEQ       =   L.ACPTSEQ           " & vbCrLf
    ...
    SQL = SQL & "   And R.RESULTITEMCODE IN (" & gAllTestCd & ")" & vbCrLf
'    SQL = SQL & "   And (R.TEXTRESULTVAL = '' OR R.TEXTRESULTVAL IS NULL)   " & vbCrLf    '
    SQL = SQL & "   And R.RESULTFLAG      <= 1                                 " & vbCrLf
    SQL = SQL & " ORDER BY R.RESULTDATE,P.PATID                             " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                '성별/나이
                Call CalAgeSex(Trim(RS.Fields("JUMIN1")) & "" & Trim(RS.Fields("JUMIN2")), Now)
