...
Public Sub Init_fpSpread_Detail()
    With frmSubMonitor.spdDetail
...
'CCY On Error GoTo ErrorHandler
    ...
    strQry = strQry + "      AND TO_CHAR(B.DT_TIME,'yyyy') > '2011' "
...
'MsgBox Now & "-" & CDate(mainFrm.fpSpread_Tot_DtVPN.Text) & "====" & DateDiff("n", CDate(mainFrm.fpSpread_Tot_DtVPN.Text), Now)
    ...
    'MsgBox "aaaa=" & mainFrm.fpSpread_Tot_DtVPN.ActiveRow
    'mainFrm.fpSpread_Tot_DtVPN.ActiveRow = mainFrm.fpSpread_Tot_DtVPN.RecordCount
...
'MsgBox Now & "-" & CDate(mainFrm.fpSpread_Tot_DtCDMA.Text) & "====" & DateDiff("n", CDate(mainFrm.fpSpread_Tot_DtCDMA.Text), Now)
...
'    strQry = "SELECT STATION_NAME, TO_CHAR(OBS_TIME,'yyyy/mm/dd hh24:mi:ss') OBS_TIME " + vbCrLf
'    strQry = strQry + "FROM( " + vbCrLf
'    strQry = strQry + "SELECT STATION_NAME, C.OBS_TIME " + vbCrLf
'    strQry = strQry + "FROM WRN.T_WRN_STATION A, WRN.T_WRN_TW_BUOY B, (SELECT STATION_ID, MAX(OBS_TIME) AS OBS_TIME FROM WRN.T_WRN_TW_BUOY GROUP BY STATION_ID) C " + vbCrLf
...
'            mainFrm.fpSpread_Tot_Tw.DataSource = dtRs.DataSource
'        Else
...
'MsgBox Now & "-" & CDate(mainFrm.fpSpread_Tot_Tw.Text) & "====" & DateDiff("n", CDate(mainFrm.fpSpread_Tot_Tw.Text), Now)
...
'Call LogWrite(strQry)
    ...
    If Not (dtRs.EOF Or dtRs.BOF) Then
'        If AdoDBConn.Errors.Count = 0 Then
'            mainFrm.fpSpread_Tot_Ag.DataSource = dtRs.DataSource
'        Else
'            AdoDBConn.Errors.Clear
'        End If
        Dim cnt As Integer
...
'MsgBox Now & "-" & CDate(mainFrm.fpSpread_Tot_Ag.Text) & "====" & DateDiff("n", CDate(mainFrm.fpSpread_Tot_Ag.Text), Now)
