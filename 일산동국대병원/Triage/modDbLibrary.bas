...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_DONGGUK(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
'    Dim strTestCd   As String
...
'    strORQN = ""
    ...
    ispcno$ = sBarcode '"12020356983" '"12020152330" ' "12020356983" ''varTmp
    rv = sl_d_60_sel_spcno&(ispcno$, pt_no$(), patname$(), Sex$(), Age$(), gnl_item_cd$(), bl_gth_dte$(), dept$(), wd_no$(), tst_cd$())
    
    Call SetSQLData("바코드조회", ispcno$ & ",rv:" & rv)
...
'        .GetText 5, intRow, varTmp: sEquipCode = varTmp
        ...
        With frmInterface
            ...
            SetText .vasID, bl_gth_dte(0), asRow, colHOSPDATE
            SetText .vasID, ispcno, asRow, colBARCODE
            SetText .vasID, pt_no(0), asRow, colPID
            SetText .vasID, patname(0), asRow, colPNAME
            SetText .vasID, Sex(0), asRow, colPSEX
            SetText .vasID, Age(0), asRow, colPAGE
                        
            '-- 화면에 표시
            For intTstCnt = 0 To UBound(tst_cd)
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(tst_cd(intTstCnt)) & "',"
                For intCol = colState + 1 To .vasID.MaxCols
                    If tst_cd(intTstCnt) = gArrEquip(intCol - colState, 3) Then
                        .vasID.Row = asRow
                        .vasID.Col = intCol
                        .vasID.BackColor = vbYellow
                        Exit For
