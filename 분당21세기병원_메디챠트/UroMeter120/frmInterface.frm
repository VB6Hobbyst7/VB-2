   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboChk 
            Height          =   315
            ItemData        =   "frmInterface.frx":B7D6
            Left            =   3720
            List            =   "frmInterface.frx":B7E3
            TabIndex        =   52
            Top             =   -30
            Visible         =   0   'False
            Width           =   825
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":B7F9
         End
         ...
         Begin FPSpread.vaSpread vasWorkList 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":F554
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":1007B
            UserResize      =   2
         End
         Begin VB.Label Label1 
            Appearance      =   0  '평면
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String)
    ...
    intRow = 0
          ...
          SQL = "Select Distinct a.챠트번호, a.진료년, a.진료월, a.진료일, c.진료상태, b.수진자명, b.주민등록번호" & vbCr
    SQL = SQL & "  From TB_검사항목 a, TB_인적사항 b, tb_진료기본 c " & vbCr
    
    'SQL = SQL & " Where a.진료년 Between '" & strFYear & "' And '" & strTYear & "'" & vbCr
    'SQL = SQL & "   And a.진료월 Between '" & strFMonth & "' And '" & strTMonth & "'" & vbCr
    'SQL = SQL & "   And a.진료일 Between '" & strFDay & "' And '" & strTDay & "'" & vbCr
    
    SQL = SQL & " Where a.진료년 + a.진료월 + a.진료일" & " >= '" & strFYear + strFMonth + strFDay & "'" & vbCr
    SQL = SQL & "   And a.진료년 + a.진료월 + a.진료일" & " <= '" & strTYear + strTMonth + strTDay & "'" & vbCr
    
    
    SQL = SQL & "   And a.처방번호 > 0 " & vbCr
    SQL = SQL & "   And c.진료상태 in ('1','5','6','7','8','9') " & vbCr
    SQL = SQL & "   And a.처방코드 + a.서브코드 in (" & gAllExam & ")" & vbCr
    ...
    SQL = SQL & "   And (a.검사결과 is null or a.검사결과 = '')     " & vbCr
    SQL = SQL & " Order By a.진료년, a.진료월, a.진료일, b.수진자명 "
    
    '결과없는것만
    'If chkOld.Value Then
    '      SQL.Text:= SQL.Text + '   And (a.검사결과 is null or a.검사결과 = '''')';
    'End If
...
'    SetRawData "[SQL1]" & SQL
    ...
    Do Until RS.EOF
        ...
        SetText vasWorkList, Trim(RS.Fields(0)) & "", intRow, colBarcode
        'SetText vasWorkList, Trim(RS.Fields(6)), intRow, colPID
        SetText vasWorkList, Trim(RS.Fields(5)), intRow, colPName
        
        'SetText vasWorkList, Trim(RS.Fields(6)), intRow, colSex
        'SetText vasWorkList, Trim(RS.Fields(7)), intRow, colAge
...
Private Sub Command16_Click()
    ...
    strBuffer = strBuffer & "~NOV/28/2009 11:12:10" & vbCrLf
    strBuffer = strBuffer & "Name:             Sex:" & vbCrLf
    strBuffer = strBuffer & "Ward:             Age:" & vbCrLf
    strBuffer = strBuffer & "Strip Name (10)      (0002)" & vbCrLf
    strBuffer = strBuffer & "BLD       -        neg" & vbCrLf
    strBuffer = strBuffer & "BIL       -        neg" & vbCrLf
    strBuffer = strBuffer & "URO    norm    0.1mg/dl" & vbCrLf
    
    strBuffer = strBuffer & "KET       -        neg" & vbCrLf
    strBuffer = strBuffer & "PRO       -        neg" & vbCrLf
    strBuffer = strBuffer & "NIT       -        neg" & vbCrLf
    strBuffer = strBuffer & "GLU       -        neg" & vbCrLf
    strBuffer = strBuffer & "pH                 7.0" & vbCrLf
    strBuffer = strBuffer & "S.G              1.015" & vbCrLf
    strBuffer = strBuffer & "LEU       -    25WBC/ul" & vbCrLf
    strBuffer = strBuffer & "(SN=60158391)" & vbCrLf
    strBuffer = strBuffer & "ID(             )" & vbCrLf
    strBuffer = strBuffer & "OP(00000000) LOT(000000) ~" & vbCrLf
    ...
    For intCnt = 1 To UBound(fLCURO)
        ...
        Select Case intCnt
            ...
            Case 4 To 13    '## Result
                ...
                strResult = strResult & " " & Trim(Mid(strTemp1, 15, 12)) '-- 정량
                ...
                If strResult <> "" Then
                    ...
                    SQL = SQL & "   AND EXAMCODE + SUBCODE in (" & gOrderExam & ") "
'                    SetRawData "[SQLn]" & SQL
                    ...
                    If Res > 0 Then
                        ...
                        End If
                        SetRawData "[lsSubCode]" & lsSubCode
...
'    sExamDate = Format(dtpToday, "yyyymmdd")
    ...
    SQL = SQL & "EXAMDATE,EQUIPNO,BARCODE,DISKNO,POSNO," & vbCrLf & _
                ...
                "EQUIPRESULT,RESULT,EXAMNAME,SENDFLAG,EXAMUID,SUBCODE) " & vbCrLf
    SQL = SQL & "VALUES("
