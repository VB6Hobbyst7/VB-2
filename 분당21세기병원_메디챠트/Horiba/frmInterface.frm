   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C320
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":CE4F
         End
         Begin VB.Label Label1 
            Appearance      =   0  'Æò¸é
            AutoSize        =   -1  'True
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String)
    ...
    intRow = 0
          ...
          SQL = "Select Distinct a.Ã­Æ®¹øÈ£, a.Áø·á³â, a.Áø·á¿ù, a.Áø·áÀÏ, c.Áø·á»óÅÂ, b.¼öÁøÀÚ¸í, b.ÁÖ¹Îµî·Ï¹øÈ£" & vbCr
    SQL = SQL & "  From TB_°Ë»çÇ×¸ñ a, TB_ÀÎÀû»çÇ× b, tb_Áø·á±âº» c " & vbCr
'    SQL = SQL & " Where a.Áø·á³â Between '" & strFYear & "' And '" & strTYear & "'" & vbCr
'    SQL = SQL & "   And a.Áø·á¿ù Between '" & strFMonth & "' And '" & strTMonth & "'" & vbCr
'    SQL = SQL & "   And a.Áø·áÀÏ Between '" & strFDay & "' And '" & strTDay & "'" & vbCr
    
    SQL = SQL & " Where a.Áø·á³â + a.Áø·á¿ù + a.Áø·áÀÏ" & " >= '" & strFYear + strFMonth + strFDay & "'" & vbCr
    SQL = SQL & "   And a.Áø·á³â + a.Áø·á¿ù + a.Áø·áÀÏ" & " <= '" & strTYear + strTMonth + strTDay & "'" & vbCr
    
    SQL = SQL & "   And a.Ã³¹æ¹øÈ£ > 0 " & vbCr
    SQL = SQL & "   And c.Áø·á»óÅÂ in ('1','5','6','7','8','9') " & vbCr
    SQL = SQL & "   And a.Ã³¹æÄÚµå + a.¼­ºêÄÚµå in (" & gAllExam & ")" & vbCr
    SQL = SQL & "   And (a.°Ë»ç°á°ú is null or a.°Ë»ç°á°ú = '')   " & vbCr
    SQL = SQL & "   And a.Áø·á³â = c.Áø·á³â                         " & vbCr
    SQL = SQL & "   And a.Áø·á¿ù = c.Áø·á¿ù                         " & vbCr
    SQL = SQL & "   And a.Áø·áÀÏ = c.Áø·áÀÏ                         " & vbCr
    SQL = SQL & "   And a.Ã­Æ®¹øÈ£ = c.Ã­Æ®¹øÈ£                     " & vbCr
    SQL = SQL & "   And a.Ã­Æ®¹øÈ£ = b.Ã­Æ®¹øÈ£                     " & vbCr
    SQL = SQL & "   And (a.°Ë»ç°á°ú is null or a.°Ë»ç°á°ú = '')     " & vbCr
    SQL = SQL & " Order By a.Áø·á³â, a.Áø·á¿ù, a.Áø·áÀÏ, b.¼öÁøÀÚ¸í "
    
    '°á°ú¾ø´Â°Í¸¸
    'If chkOld.Value Then
    '      SQL.Text:= SQL.Text + '   And (a.°Ë»ç°á°ú is null or a.°Ë»ç°á°ú = '''')';
    'End If
