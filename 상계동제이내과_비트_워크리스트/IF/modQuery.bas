...
'            SQL = SQL & "        ,M.RsbBarCod AS BARCODE "
...
'            SQL = SQL & "   and m.workarea = '" & gHOSP.LABCD & "'  " & vbCr
        ...
        Case "BIT"
'            SQL = ""
'            SQL = SQL & "SELECT R.ResLabCod AS ITEM" & vbCr
'            SQL = SQL & "  FROM RsbInf M, ResInf R, ospinf O, PBSINF P, LabMst E" & vbCr
'            SQL = SQL & " WHERE M.RsbBarCod = '" & strBarcode & "'" & vbCr
'            SQL = SQL & "   AND O.OCMCOMSTT NOT IN ('CN', 'CR', 'VC')" & vbCr
'            SQL = SQL & "   AND R.RESLABCOD IN (" & gAllTestCd & ")" & vbCr
'            SQL = SQL & "   And M.RsbAckStt <> 'A' " & vbCr
'            SQL = SQL & "   And O.OspChkStt <> 'F' " & vbCr
'            SQL = SQL & "   And (R.ResRepTyp Is Null or R.ResRepTyp <> 'F') " & vbCr
'            SQL = SQL & "   And (R.ResRltVal is Null or R.ResRltVal = '') " & vbCr
'            SQL = SQL & "   And R.ResStatus <> '5' " & vbCr
'            SQL = SQL & "   And M.RSBACPNUM = R.ResRsbAcp" & vbCr
'            SQL = SQL & "   And R.ResOcmNum = O.OspOcmNum" & vbCr
'            SQL = SQL & "   and R.ResOdrSeq = O.OspOdrSeq" & vbCr
'            SQL = SQL & "   and R.ResSeq    = O.OspSeq" & vbCr
'            SQL = SQL & "   and O.OSPCHTNUM = P.PBSCHTNUM" & vbCr
'            SQL = SQL & "   and R.ResLabcod = E.LabCod" & vbCr
                        
'            SQL = ""
'            SQL = SQL & " SELECT Distinct R.RESLABCOD AS ITEM" & vbLf
'            SQL = SQL & "   FROM RESINF R, PBSINF P " & vbLf
'            SQL = SQL & "  WHERE R.RESCHTNUM = P.PBSCHTNUM  " & vbLf
'            'SQL = SQL & "    AND R.RESSPMNUM = '" & strBarcode & "'" & vbLf
'            SQL = SQL & "    AND LTRIM(RTRIM(R.RESOCMNUM)) = '" & strBarcode & "'" & vbLf
'            SQL = SQL & "    AND R.RESLABCOD IN (" & gAllTestCd & ") "
            
            SQL = ""
            SQL = SQL & " SELECT DISTINCT R.ResLabCod AS ITEM " & vbCr
            SQL = SQL & "   FROM RESINF AS R, OCMINF AS O, PBSINF AS P, LABMST AS E, ODRINF AS W" & vbCr
...
'            SQL = ""
'            SQL = SQL & "SELECT DISTINCT SUBSTRING(O.OCMACPDTM,1,8) AS HOSPDATE"
'            SQL = SQL & "      ,P.PbsPatNam AS PNAME "
'            SQL = SQL & "      ,O.OSPCHTNUM AS CHARTNO"
'            SQL = SQL & "      ,R.ResOcmNum AS PID"
'            SQL = SQL & "      ,P.PBSSEXTYP AS SEX"
'            SQL = SQL & "      ,R.ResLabCod AS ITEM"
'            SQL = SQL & "      ,R.ResOdrSeq, R.ResSeq, R.ResSubSeq " & vbCr
'            SQL = SQL & "  FROM RsbInf M, ResInf R, ospinf O, PBSINF P, LabMst E" & vbCr
'            SQL = SQL & " WHERE M.RsbBarCod = '" & strBarcode & "'" & vbCr
'            SQL = SQL & "   AND O.OCMCOMSTT NOT IN ('CN', 'CR', 'VC')" & vbCr
...
'            SQL = ""
'            SQL = SQL & " SELECT Distinct R.RESCHTNUM AS , P.PBSPATNAM, P.PBSRESNUM " & vbLf
'            SQL = SQL & "   FROM RESINF R, PBSINF P " & vbLf
'            SQL = SQL & "  WHERE R.RESCHTNUM = P.PBSCHTNUM  " & vbLf
'            SQL = SQL & "    AND R.RESSPMNUM = '" & strBarcode & "'"
                    ...
                    With SPD
                        ...
                        For intCol = colSTATE + 1 To .MaxCols
                            If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                                ...
                                '-- 결과저장용 SEQ
                                gArrEQP(intCol - colSTATE, 7) = Trim(RS.Fields("ResOdrSeq")) & "|" & Trim(RS.Fields("ResSeq")) & "|" & Trim(RS.Fields("ResSubSeq"))   '결과저장용 번호's
