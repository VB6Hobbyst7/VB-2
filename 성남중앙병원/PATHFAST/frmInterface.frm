   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":B558
         End
         Begin FPSpread.vaSpread vasWorkList 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":F274
            UserResize      =   2
         End
         ...
         Begin VB.ComboBox cboChk 
            ...
            EndProperty
            Height          =   300
            ItemData        =   "frmInterface.frx":FB46
            Left            =   2340
            List            =   "frmInterface.frx":FB50
            TabIndex        =   45
            Top             =   -90
            Visible         =   0   'False
            Width           =   765
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":FB60
            UserResize      =   2
         End
...
Private Sub cmdRSch_Click()
    ...
    'SELECT 처음 '' 는 체크박스
          SQL = " SELECT '', DISKNO, POSNO, PID, PNAME, PSEX, PAGE, COUNT(*), COUNT(*), SENDFLAG " & vbCrLf
...
'    End If
    SQL = SQL & "  GROUP BY DISKNO, POSNO, PID, PNAME, PSEX, PAGE, SENDFLAG "
...
'    SQL = SQL & "   ORDER BY prex.acp_dt, prex.smp_no " & vbCr
    ...
    '챠트번호,성명,진료일자,진료시간,검사대분류코드,검사대분류명,검사코드,검사명칭,검사순서,검사 결과값
'          SQL = "SELECT a.cht_id, e.pt1_nm ,a.jindt ,a.jintm ,a.daecode ,d.dae_name ,b.won_code ,c.WON_NAME ,b.seq ,b.kg1" & vbNewLine
'    SQL = SQL & "  FROM ocs1.snjocsdb.dbo.INTERFACEREADY a "                                '-- 대기자
'    SQL = SQL & "  LEFT JOIN ocs1.snjocsdb.dbo.gum_kg1 b on a.cht_id = b.cht_id"            '-- 검사코드
'    SQL = SQL & "                                       and a.jindt  = b.jindt"
'    SQL = SQL & "                                       and a.jintm  = b.jintm"
'    SQL = SQL & "                                       and a.daecode = b.dae_code"
'    SQL = SQL & "  LEFT JOIN ocs1.snjocsdb.dbo.woncode c on c.jin_code = 1"                 '-- 검사명칭
'    SQL = SQL & "                                       and c.dae_code = b.dae_code"
'    SQL = SQL & "                                       and c.won_code = b.won_code"
'    SQL = SQL & "                                       and c.seq = 1"
'    SQL = SQL & "  LEFT JOIN ocs1.snjocsdb.dbo.daecode d on d.jin_code = 1"                 '-- 검사대분류
'    SQL = SQL & "                                       and d.dae_code = b.dae_code"
'    SQL = SQL & "  LEFT JOIN won1.snjwondb.dbo.pt1m00 e on e.pt1_cd = a.cht_id"             '-- 성명
'    SQL = SQL & " WHERE a.cht_id  <> 0  "
'    SQL = SQL & " ORDER BY b.cht_id ,b.jindt ,b.jintm,b.dae_code,b.won_code,b.seq"
    
'          SQL = "SELECT DISTINCT a.cht_id, e.pt1_nm ,a.jindt ,a.jintm ,a.dae_code ,d.dae_name ,b.won_code ,c.WON_NAME ,b.seq ,b.kg1" & vbNewLine
          SQL = "SELECT DISTINCT a.c_num, a.cht_id, e.pt1_nm ,a.jindt ,a.jintm ,a.daecode as dae_code ,d.dae_name " & vbNewLine
    SQL = SQL & "  FROM ocs1.snjocsdb.dbo.INTERFACEREADY a " & vbNewLine                                '-- 대기자
    SQL = SQL & "  LEFT JOIN ocs1.snjocsdb.dbo.gum_kg1 b on a.cht_id = b.cht_id" & vbNewLine            '-- 검사코드
    SQL = SQL & "                               and a.jindt  = b.jindt" & vbCr
    SQL = SQL & "                               and a.jintm  = b.jintm" & vbCr
    SQL = SQL & "                               and a.daecode = b.dae_code" & vbCr
    SQL = SQL & "  LEFT JOIN ocs1.snjocsdb.dbo.woncode c on c.jin_code = 1" & vbNewLine                 '-- 검사명칭
    SQL = SQL & "                               and c.dae_code = b.dae_code" & vbCr
    SQL = SQL & "                               and c.won_code = b.won_code" & vbCr
    SQL = SQL & "                               and c.seq = 1" & vbCr
    SQL = SQL & "  LEFT JOIN ocs1.snjocsdb.dbo.daecode d on d.jin_code = 1" & vbNewLine                 '-- 검사대분류
    SQL = SQL & "                               and d.dae_code = b.dae_code" & vbCr
    SQL = SQL & "  LEFT JOIN won1.snjwondb.dbo.pt1m00 e on e.pt1_cd = a.cht_id" & vbNewLine             '-- 성명
    SQL = SQL & " WHERE a.cht_id  <> 0  " & vbNewLine
    SQL = SQL & "   AND a.jindt BETWEEN '" & pFrDt & "' AND '" & pToDt & "'" & vbCr
'    SQL = SQL & " ORDER BY b.cht_id ,b.jindt ,b.jintm,b.dae_code,b.won_code,b.seq"
    SQL = SQL & "   AND c.dae_code = '7' " & vbCr
    SQL = SQL & "   AND c.won_code in (" & gAllExam & ")" & vbCr
   ' SQL = SQL & "   AND (b.kg1 = '' or b.kg1 = null)"
    SQL = SQL & " ORDER BY a.c_num, a.cht_id ,e.pt1_nm,a.jindt ,a.jintm,a.daecode,d.dae_name " & vbCr
    
    
    'SQL = "select a.rst_dt,a.dept_cd,a.seq,b.name"
    'SQL = SQL & " from lab_qc..s2qcs001 a "
    'SQL = SQL & "  left join test..int b on a.seq = b.seq "


    SQL = ""
    SQL = SQL & "select DISTINCT a.c_num, a.cht_id, e.pt1_nm ,a.jindt ,a.jintm ,b.dae_code as dae_code ,d.dae_name" & vbCr
    SQL = SQL & "  from ocs1.snjocsdb.dbo.INTERFACEREADY a,ocs1.snjocsdb.dbo.gum_kg1 b , ocs1.snjocsdb.dbo.woncode c,ocs1.snjocsdb.dbo.daecode d,won1.snjwondb.dbo.pt1m00 e" & vbCr
    SQL = SQL & " Where a.cht_id <> 0" & vbCr
    SQL = SQL & "   AND a.jindt BETWEEN '" & pFrDt & "' AND '" & pToDt & "'" & vbCr
    SQL = SQL & "   AND c.dae_code = '7'" & vbCr
    SQL = SQL & "   AND c.won_code in (" & gAllExam & ")" & vbCr
    SQL = SQL & "   AND c.jin_code = '1' " & vbCr
    SQL = SQL & "   AND c.jin_code = d.jin_code" & vbCr
    SQL = SQL & "   and c.dae_code = d.dae_code" & vbCr
    SQL = SQL & "   and e.pt1_cd = a.cht_id" & vbCr
    SQL = SQL & "   and a.cht_id = b.cht_id" & vbCr
    SQL = SQL & "   and c.won_code =b.won_code" & vbCr
    SQL = SQL & "   and c.dae_code = b.dae_code" & vbCr
    SQL = SQL & "   and a.jindt  = b.jindt" & vbCr
    SQL = SQL & "   and a.jintm  = b.jintm" & vbCr
    ...
    Do Until RS.EOF
        ...
        SetText vasWorkList, Trim(RS.Fields("jindt")), intRow, 3
        SetText vasWorkList, Trim(RS.Fields("jintm")), intRow, 4
        SetText vasWorkList, Trim(RS.Fields("cht_id")), intRow, 5
        SetText vasWorkList, Trim(RS.Fields("pt1_nm")), intRow, 6
        SetText vasWorkList, Trim(RS.Fields("dae_code")), intRow, 7
        SetText vasWorkList, Trim(RS.Fields("dae_name")), intRow, 8
    
'        SetText vasWorkList, Trim(RS.Fields("rst_dt")), intRow, 3
'        SetText vasWorkList, "1037", intRow, 4
'        SetText vasWorkList, "12345", intRow, 5
'        SetText vasWorkList, "홍길동", intRow, 6
'        SetText vasWorkList, "111", intRow, 7
'        SetText vasWorkList, "전해질", intRow, 8
...
'''Dim strDt, strTm, strID, strDC, strWC As String
''    Set RS = cn_Ser.Execute(SQL, , 1)
'''    i = 0
''    Do Until RS.EOF
'''        i = i + 1
...
''            SetText vasWorkList, "1", intRow, colCheckBox
''            SetText vasWorkList, CStr(intRow), intRow, colSeqNo
''    '        SetText vasWorkList, Trim(RS.Fields("jin_dt")), intRow, 3
''    '        SetText vasWorkList, Trim(RS.Fields("jin_tm")), intRow, 4
''    '        SetText vasWorkList, Trim(RS.Fields("cht_id")), intRow, 5
''    '        SetText vasWorkList, Trim(RS.Fields("pt1_nm")), intRow, 6
''    '        SetText vasWorkList, Trim(RS.Fields("dae_code")), intRow, 7
''    '        SetText vasWorkList, Trim(RS.Fields("dae_name")), intRow, 8
'''        Else
'''            If strDt <> Trim(RS.Fields("jin_dt")) & "" Or strTm = Trim(RS.Fields("jin_tm")) & "" Or strID = Trim(RS.Fields("cht_id")) & "" Or strDC = Trim(RS.Fields("dae_code")) & "" Or strWC = Trim(RS.Fields("won_code")) & "" Then
''
''
'''        End If
''
'''        strDt = Trim(RS.Fields("jin_dt")) & ""
'''        strTm = Trim(RS.Fields("jin_tm")) & ""
'''        strID = Trim(RS.Fields("cht_id")) & ""
'''        strDC = Trim(RS.Fields("dae_code")) & ""
'''        strWC = Trim(RS.Fields("won_code")) & ""
''
''        SetText vasWorkList, Trim(RS.Fields("rst_dt")), intRow, 3
''        SetText vasWorkList, "1037", intRow, 4
''        SetText vasWorkList, "12345", intRow, 5
...
strBuffer = strBuffer & "=================" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "SAMPLE RESULTS" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "PATIENT 'S NAME" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "-----------#0007" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "Na "" 141.0 mmol/L     " & vbCr
strBuffer = strBuffer & "K ""   3.73 mmol/L     " & vbCr
strBuffer = strBuffer & "Cl^ 107.0 mmol/L  $" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "CORRELATIONS ON" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "JUN-06-06; 13:02" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
...
strBuffer = strBuffer & "" & vbCr
    ...
    strBuffer = "1H|@^\|||PATHFAST01^0903A0859^01.00.00.00|||||||P|1|20110721160813" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "2P|1|||||||U||||||||||||||||||||||||||" & vbCr
    strBuffer = strBuffer & "90" & vbCrLf
    strBuffer = strBuffer & "3O|1|00287300^1^||^^^06^NTproBNP^1061202237|||||||||||1||||||||||F|||||" & vbCr
    strBuffer = strBuffer & "4A" & vbCrLf
    strBuffer = strBuffer & "4R|1|^^^06^NTproBNP^1061202237|11066^F|pg/mL||A@H||F||Administrator||20110721155157|" & vbCr
    strBuffer = strBuffer & "B1" & vbCrLf
    strBuffer = strBuffer & "5C|1|I|DF^2H^^40.0^20110713173333|I" & vbCr
    strBuffer = strBuffer & "3C" & vbCrLf
    strBuffer = strBuffer & "6L|1|N" & vbCr
    strBuffer = strBuffer & "09" & vbCrLf
    strBuffer = strBuffer & "" & vbCrLf
    
    strBuffer = ""
    strBuffer = strBuffer & "1H|@^\|||PATHFAST01^0802A0741^01.00.00.00|||||||P|1|20101129135826" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "2P|1|||||||U||||||||||||||||||||||||||" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "3O|1|100009123028^1^||^^^01^cTnI^1011111264|||||||||||1||||||||||F|||||" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "4R|1|^^^01^cTnI^1011111264|0.171^F|ng/mL||A||F||Administrator||20101129134202|" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "5C|1|I|DF^^^40.0^20110528200517|I" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "6P|2|||||||U||||||||||||||||||||||||||" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "7O|1|100009123028^2^||^^^03^CK-MB^1031105236|||||||||||1||||||||||F|||||" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "0R|1|^^^03^CK-MB^1031105236|11.4^F|ng/mL||A@H||F||Administrator||20101129134202|" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    strBuffer = strBuffer & "1C|1|I|DF^H^^40.0^20110528193902|I" & vbCr
    strBuffer = strBuffer & "77" & vbCrLf
    ...
    strBuffer = ""
    strBuffer = strBuffer & "1H|@^\|||PATHFAST01^1505C2341^01.00.00.00|||||||P|1|20160128082837" & vbCr
    strBuffer = strBuffer & "7B" & vbCrLf
    strBuffer = strBuffer & "2P|1|||||||U||||||||||||||||||||||||||" & vbCr
    strBuffer = strBuffer & "90" & vbCrLf
    strBuffer = strBuffer & "3O|1|^1^||^^^06^NTproBNP^1061604468|||||||||||2||||||||||F|||||" & vbCr
    strBuffer = strBuffer & "C3" & vbCrLf
    strBuffer = strBuffer & "4R|1|^^^06^NTproBNP^1061604468|439^F|pg/mL||N||F||Administrator||20160119203124|" & vbCr
    strBuffer = strBuffer & "DE" & vbCrLf
    strBuffer = strBuffer & "5C|1|I|^^^^20160119141141|I" & vbCr
    strBuffer = strBuffer & "73" & vbCrLf
    strBuffer = strBuffer & "6P|2|||||||U||||||||||||||||||||||||||" & vbCr
    strBuffer = strBuffer & "95" & vbCrLf
    strBuffer = strBuffer & "7O|1|^1^||^^^06^NTproBNP^1061604468|||||||||||2||||||||||F|||||" & vbCr
    strBuffer = strBuffer & "C7" & vbCrLf
    strBuffer = strBuffer & "0R|1|^^^06^NTproBNP^1061604468|54.2^F|pg/mL||N||F||Administrator||20160119170221|" & vbCr
    strBuffer = strBuffer & "04" & vbCrLf
    strBuffer = strBuffer & "1C|1|I|^^^^20160119141141|I" & vbCr
...
Function GetExamCode() As Integer
    ...
    For i = 1 To vasCode.DataRowCnt
        ...
        'vasWorkList.MaxCols = vasWorkList.MaxCols + 1
        'SetText vasWorkList, Trim(GetText(vasCode, i, 1)), 0, vasWorkList.MaxCols - 1
...
'   기능 : 장비로부 수신한 데이터 편집
'-----------------------------------------------------------------------------'
Private Sub EditRcvDataElectro()
    ...
    If InStr(strBuffer, "CAL") > 0 Then Exit Sub     'CALIBRATION
    If InStr(strBuffer, "SLOPE") > 0 Then Exit Sub  'SLOPE
        ...
        Select Case intCnt
            Case 7    '## Sample No.
                ...
                For ii = 1 To vasWorkList.DataRowCnt
                    If Trim(GetText(vasWorkList, ii, colSeqNo)) = Val(strSeq) Then
                        intRow = ii
                        gRow = intRow
                        vasWorkList.Row = gRow
                        vasWorkList.Col = colBarcode
                        strBarno = Trim(vasWorkList.Text)
                        Exit For
                    End If
                Next
            ...
            Case 10, 11, 12 '## Result
                '## 장비기준 검사명, 결과
                If intCnt = 12 Then
                    strIntBase = Trim(mGetP(strRcvBuf, 1, "^"))
                Else
                    strIntBase = Trim(mGetP(strRcvBuf, 1, """"))
                End If
...
'''                    If vasWorkList.Value = "1" Then
'''                        'vasWorkList.Col = 9
'''                        'If vasWorkList.Value = "" Then
'''                            vasWorkList.Col = 5
'''                            strBarno = vasWorkList.Text
'''                            vasWorkList.Col = 3
'''                            strDT = vasWorkList.Text
'''                            vasWorkList.Col = 4
'''                            strTM = vasWorkList.Text
'''                            gRow = i
'''                            Exit For
'''                        'End If
'''                    End If
'''                Next
                ...
                '3O|1|^1^||^^^03^CK-MB^1031801655|||||||||||1||||||||||F|||||

                strBarno = mGetP(mGetP(strRcvBuf, 3, "|"), 2, "^")
                For i = 1 To vasWorkList.DataRowCnt
                    vasWorkList.Row = i
                    vasWorkList.Col = colSeqNo
                    If Trim(vasWorkList.Text) = Trim(strBarno) Then
                        'vasWorkList.Col = 9
                        'If vasWorkList.Value = "" Then
                            vasWorkList.Col = 5
                            strBarno = vasWorkList.Text
...
'                End If
                ...
                '-- 바코드번호에 존재하는 검사코드 가져오기(인수 : 장비코드,바코드번호)
                gOrderExam = GetOrderExamCode_NN(gEquip, Trim(strBarno), strDT, strTM)
            ...
            Case "R"
                ...
                If strResult <> "" Then
                    ...
                    If Res > 0 Then
                        ...
                        '-- Work List
                        SetText vasWorkList, "Result", gRow, 9                 '11 진행상태
                        ...
                        '-- 메인결과 List
'                        For i = 9 To vasWorkList.MaxCols - 1
'                            vasWorkList.Row = 0
'                            vasWorkList.Col = i
'                            If vasWorkList.Value = strIntBase Then
'                                SetText vasWorkList, strResult, gRow, i          '결과
'                                Exit For
'                            End If
'                        Next

                       ' SetText vasWorkList,strResult,gRow,
                    ...
                    Else
                        ...
                        If Res > 0 Then
                            ...
                            '-- Work List
                            SetText vasWorkList, "Result", gRow, 9                 '진행상태
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    '-- seq 찾아오기
          SQL = "SELECT seq " & vbNewLine
    SQL = SQL & "  FROM snjocsdb..gum_kg1 " & vbNewLine
    SQL = SQL & " WHERE jindt  = '" & Trim(GetText(vasWorkList, asRow1, 3)) & "'" & vbNewLine
    SQL = SQL & "   AND jintm  = '" & Trim(GetText(vasWorkList, asRow1, 4)) & "'" & vbNewLine
    SQL = SQL & "   AND cht_id = '" & Trim(GetText(vasWorkList, asRow1, 5)) & "'" & vbNewLine
    SQL = SQL & "   AND dae_code = '" & Trim(GetText(vasWorkList, asRow1, 7)) & "'" & vbNewLine
    SQL = SQL & "   AND won_code = '" & Trim(GetText(vasRes, asRow2, colExamCode)) & "'"
    
    Set RS = cn_Ser.Execute(SQL, , 1)
    
    Do Until RS.EOF
        strSeq = Trim(RS.Fields("seq")) & ""
        RS.MoveNext
    Loop
    ...
    SQL = "DELETE FROM PATRESULT " & vbCrLf & _
          ...
          "  AND EQUIPNO = '" & gEquip & "' " & vbCrLf & _
          "  AND BARCODE = '" & Trim(GetText(vasWorkList, asRow1, 5)) & "' " & vbCrLf & _
    ...
    SQL = SQL & "'" & gEquip & "', "
    SQL = SQL & "'" & Trim(GetText(vasWorkList, asRow1, 5)) & "', "
    SQL = SQL & "'" & Trim(GetText(vasWorkList, asRow1, 3)) & "', "
    SQL = SQL & "'" & Trim(GetText(vasWorkList, asRow1, 4)) & "', "
    SQL = SQL & "'" & Trim(GetText(vasWorkList, asRow1, 5)) & "', "
    SQL = SQL & "'" & Trim(GetText(vasWorkList, asRow1, 6)) & "', "
    SQL = SQL & "'" & Trim(GetText(vasWorkList, asRow1, 7)) & "', "
    SQL = SQL & "'" & strSeq & "', "
...
Private Sub vasRID_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    lblPname(1).Caption = Trim(GetText(vasRID, Row, 5))
    lblRrow.Caption = Row
    'Local에서 불러오기
    ClearSpread vasRRes
...
Private Sub vasWorkList_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    lsID = Trim(GetText(vasWorkList, Row, 5))
    lblChangeBar.Caption = lsID
    ...
    If KeyAscii = vbKeyReturn Then
        With vasWorkList
            If .ActiveCol = colSeqNo Then
                strSeq = GetText(vasWorkList, .ActiveRow, .ActiveCol)
                If Not IsNumeric(strSeq) Then
                    MsgBox "숫자만 입력이 가능합니다"
                    Exit Sub
                End If
                For intRow = .ActiveRow + 1 To .MaxRows
                    Call SetText(vasWorkList, strSeq + 1, intRow, colSeqNo)
                Next
            End If
        End With
    End If
