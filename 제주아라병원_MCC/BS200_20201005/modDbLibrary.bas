        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                If IsNumeric(sResult) Then
                          SQL = "Update MCCSI.H7LIS_BCODE_ORD " & vbCr
                    SQL = SQL & "   Set RESULT_VAL = '" & sResult & "'" & vbCr
                    SQL = SQL & "      ,RESULT_CD = '10' " & vbCr   '10:수치,20:서술
                    'SQL = SQL & "      ,RESULT_TYPE = '30'"         '10:접수, 20:진행, 30:완료, 50:취소
                Else
                          SQL = "Update MCCSI.H7LIS_BCODE_ORD " & vbCr
                    SQL = SQL & "   Set RESULT_NM = '" & sResult & "'" & vbCr
                    SQL = SQL & "      ,RESULT_CD = '20' " & vbCr   '10:수치,20:서술
                    'SQL = SQL & "      ,RESULT_TYPE = '30'"         '10:접수, 20:진행, 30:완료, 50:취소
                End If
            
                'SQL = SQL & " Where BCODE_NO = '" & lsID & "'" & vbCr
                SQL = SQL & " WHERE ORD_YMD = '" & strHospDate & "'" & vbCr
                SQL = SQL & "   And RECEPT_NO = '" & strChtNum & "'" & vbCr
                SQL = SQL & "   And ORD_CD = '" & strEqpCd & "'" & vbCr
                SQL = SQL & "   And RESULT_TYPE = '20' "            '10:접수, 20:진행, 30:완료, 50:취소
...
'    sBarcode = Trim(GetText(frmInterface.vasID, asRow, colBARCODE))
    ...
    sOrdDate = Trim(GetText(frmInterface.vasID, asRow, colHOSPDATE))
...
'    SQL = SQL & " WHERE BCODE_NO = '" & sBarcode & "'" & vbCr
    SQL = SQL & " WHERE RECEPT_NO = '" & sBarcode & "'" & vbCr
    SQL = SQL & "   AND ORD_YMD = '" & sOrdDate & "'" & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                SetText .vasID, Trim(RS.Fields("ORD_YMD")) & "", asRow, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("BCODE_NO")) & "", asRow, colBARCODE
                ...
                SetText .vasID, Trim(RS.Fields("AGE")) & "", asRow, colPAGE
