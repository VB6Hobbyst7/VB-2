...
Begin VB.Form frmIISExicycler 
   ...
   Begin FPSpread.vaSpread tblExcelResult 
      Height          =   225
      Left            =   13500
      TabIndex        =   44
      Top             =   6330
      ...
      SpreadDesigner  =   "frmIISExicycler.frx":0000
   End
   Begin FPSpread.vaSpread tblexcel 
      ...
      EndProperty
      RetainSelBlock  =   0   'False
      SpreadDesigner  =   "frmIISExicycler.frx":45B3
   End
   Begin VB.OptionButton optDev 
      BackColor       =   &H00DBE6E6&
      Caption         =   "H1N1"
      Height          =   285
      Index           =   2
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISExicycler.frx":8B66
   End
   ...
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISExicycler.frx":907F
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISExicycler.frx":9820
   End
   ...
   End
End
Attribute VB_Name = "frmIISExicycler"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISExicycler.frm
'   작성자  : 오세원
'   내  용  : Exicycler 장비폼
'   작성일  : 2010-07-01
'   버  전  : 1.0.0
...
'On Error GoTo ErrRoutine
    ...
    For intIdx = 0 To FileMultiReader.ListCount - 1
        ...
        Call tblExcelResult.IsExcelFile(strSrcfile)
        
        y = tblExcelResult.GetExcelSheetList(strSrcfile, List, ListCount, "reportt.txt", handle, True)
        If y = True Then
            z = tblExcelResult.ImportExcelSheet(handle, 0)
        End If
...
'        lngBufLen = Len(strBuffer)
        ...
        With tblExcelResult
            For i = 1 To .MaxRows
                ...
                If Trim(varTmp) = "Sample ID" And mIntLib.Phase = 0 Then
                    mIntLib.Phase = 1
                    .GetText 7, i, varTmp3
                Else
                    BufChar = ""
                    BufChar = Mid$(Trim(varTmp), 1, 1)
                    
                    If Trim(varTmp) <> "" Then
                        If mIntLib.Phase = "1" Then
                            Select Case BufChar
                                Case "P"
                                    If InStr(Trim(varTmp3), "MTB") > 0 Then
                                        '.GetText 7, i, varTmp1:   varTmp = varTmp & "|MTB|" & Trim(varTmp1)
                                        .GetText 5, i, varTmp1
                                        If UCase(varTmp1) = "DETECTED" Then
                                            varTmp1 = "Positive"
                                        Else
                                            varTmp1 = "Negative"
                                        End If
                                        varTmp = varTmp & "|MTB|" & Trim(varTmp1)
                                    Else
                                        '.GetText 7, i, varTmp1:   varTmp = varTmp & "|NTM|" & Trim(varTmp1)
                                        .GetText 5, i, varTmp1
                                        If UCase(varTmp1) = "DETECTED" Then
                                            varTmp1 = "Positive"
                                        Else
                                            varTmp1 = "Negative"
                                        End If
                                        varTmp = varTmp & "|NTM|" & Trim(varTmp1)
                                    End If
                                    
                                    Call mIntLib.AddBuffer(varTmp)
                                    Call EditRcvData
                                    
                                    mIntLib.ClearBuffer
                                Case "H"
                                    '-- 2014.07.05 osw수정 : 결과값 치환형태 변경, 엑셀형식은 변경되지 않음(추후 변경됨)
                                    '   아래와 같이 변경됨
                                    '   LSE87A 장비값이 '-'일때 'Negative(<60)' 으로 결과변환
                                    '   LSE87B 장비값이 '-'일때 'Negative(<10)' 으로 결과변환
                                
                                    '-- Copy = 7
                                    .GetText 7, i, varTmp1
                                    If Trim(varTmp1) = "-" Then
                                        .GetText 9, i, varTmp1
                                        'varTmp = varTmp & "|" & mGetP(Trim(varTmp1), 1, "E+") & " X 10(" & Val(Right(Trim(varTmp1), 2)) - 1 & ")"
                                        varTmp = varTmp & "|" & "-"
                                    Else
                                        varTmp = varTmp & "|" & mGetP(Trim(varTmp1), 1, "E+") & " X 10(" & Val(Right(Trim(varTmp1), 2)) & ")"
                                    End If
                                ...
                                Case "N"
                                    ...
                                    For j = i + 1 To .MaxRows
                                        .GetText 2, j, varTmp2
                                        If varTmp = varTmp2 Then
                                            .GetText 7, i, varTmp1
                                            .GetText 7, j, varTmp2
                                            varTmp = varTmp & "|" & Trim(varTmp1) & "|" & Trim(varTmp2)
                                            Exit For
                                        End If
                                    Next
                                    
                                    If Trim(varTmp2) <> "" Then
                                        Call mIntLib.AddBuffer(varTmp)
                                        Call EditRcvData
                                    End If
...
Private Sub cmdMakeWS_Click()
    ...
    AlloFile.InitDir = "C:\ExiDiagData\Order\"
    
    AlloFile.Filter = "Excel(*.xls)|*.xls"


    If optDev(0).Value = True Then
        AlloFile.FileName = optDev(0).Caption & "_" & Format(Now, "yyyymmdd") & "_" & EMPNM
    ElseIf optDev(1).Value = True Then
        AlloFile.FileName = optDev(1).Caption & "_" & Format(Now, "yyyymmdd") & "_" & EMPNM
    Else
        AlloFile.FileName = optDev(2).Caption & "_" & Format(Now, "yyyymmdd") & "_" & EMPNM
    End If
    
    AlloFile.ShowSave

    Call tblexcel.ExportToExcel(AlloFile.FileName, "", "")
...
'        strTmp = " " & Chr(9)
'        For intCnt = 1 To 12
'            strTmp = strTmp & intCnt & Chr(9)
'        Next
'        strTmp = strTmp & "A" & Chr(9)
'        .BlockMode = False
'        lngRows = .MaxRows
'    End With
...
'    tblexcel.SaveTabFileU (AlloFile.FileName)
...
Private Sub Form_Load()
    ...
    txtFileNm.Text = "Exi_" & Format(Now, "yyyymmdd")
...
'    End If
    ...
    With tblexcel
        For intCnt = 2 To 13
            .SetText intCnt, 1, intCnt - 1
        Next
        For intCnt = 2 To 9
            .SetText 1, intCnt, Chr(intCnt + 63)
        Next
    End With
    ...
    FileMultiReader.Path = "C:\ExiDiagData\Result\"
...
Private Sub EditRcvData()
    ...
    For Each objBuffer In mIntLib.Buffers
        ...
        strType = Mid(mGetP(strRcvBuf, 1, "|"), 1, 1)
        strBarNo = Mid(mGetP(strRcvBuf, 1, "|"), 2)
        
        strBarNo = Mid(strBarNo, 1, 2) & "000" & Mid(strBarNo, 3)
        strIntBase = mGetP(strRcvBuf, 2, "|")
        
        If Len(strBarNo) < 11 Then GoTo EndLine
        Set objIntInfo = New clsIISIntInfo
        With objIntInfo
            .BarNo = strBarNo
        End With
        ...
        ElseIf strType = "H" Then
            For i = 0 To 1
                If i = 0 Then
                    strIntBase = "Copy"
                    strResult = mGetP(strRcvBuf, 2, "|")
                Else
                    strIntBase = "IU"
                    strResult = mGetP(strRcvBuf, 3, "|")
                End If
                strIntResult = strResult 'mGetP(strRcvBuf, 3, ";")
...
Private Sub GetOrder(ByVal pBarNo As String)
    ...
    If Not (objAccInfo Is Nothing) Then
        '## tblReady, tblresult, Label에 정보표시
        If SetReady(objAccInfo) = True Then
            Call SetLabel(objAccInfo)
            Call SetResult(objAccInfo)
        End If
...
'       - pAccInfo : 접수내역 클래스
'-----------------------------------------------------------------------------'
Private Function SetReady(ByVal pAccInfo As clsIISAccInfo) As Boolean
...
'LMI205A ==> PCR for M tbc
'LMI205B ==> PCR for NTM (nontuberculous mycobacteria
'LSE87A ==> Copy/ml
'LSE87B ==> IU/ml
'LIM300A ==> H1N1 (Real time PCR)
'LIM300B ==> Influenza-A (RT-PCR)
    SetReady = True

    For Each objResult In pAccInfo.Results
        'Debug.Print objResult.TestCd & " ==> " & objResult.TestNm
        If optDev(0).Value = True Then          'PCR
            If InStr(Trim(objResult.TestCd), "LMI205") <= 0 Then
                MsgBox "검사구분을 확인하십시요", vbOKOnly + vbCritical, Me.Caption
                SetReady = False
                Exit Function
            End If
        ElseIf optDev(1).Value = True Then      'HBV
            If InStr(Trim(objResult.TestCd), "LSE87") <= 0 Then
                MsgBox "검사구분을 확인하십시요", vbOKOnly + vbCritical, Me.Caption
                SetReady = False
                Exit Function
            End If
        Else                                    'H1N1
            If InStr(Trim(objResult.TestCd), "LIM300") <= 0 Then
                MsgBox "검사구분을 확인하십시요", vbOKOnly + vbCritical, Me.Caption
                SetReady = False
                Exit Function
            End If
...
'        End If
        ...
        .Col = TReadyEnum.ccBarNo:  .Text = pAccInfo.GetBarNo: strBarNo = Mid(pAccInfo.GetBarNo, 1, 2) & Mid(pAccInfo.GetBarNo, 6, 6)
        ...
        If optDev(0).Value = True Then          'PCR
            For intRow = 2 To 13
                ...
                Else
                    intCol2 = 2
                End If
                For intCol = intCol2 To 9
                    tblexcel.GetText intRow, intCol, varTmp
                    If Trim(varTmp) = "" Then
                        tblexcel.SetText intRow, intCol, "P" & strBarNo
                        Exit Function
                    End If
                Next intCol
            Next intRow
        ElseIf optDev(1).Value = True Then      'HBV
            For intRow = 2 To 13
                ...
                For intCol = intCol2 To 9
                    tblexcel.GetText intRow, intCol, varTmp
                    If Trim(varTmp) = "" Then
                        tblexcel.SetText intRow, intCol, "H" & strBarNo
                        Exit Function
                    End If
                Next intCol
            Next intRow
        ...
        Else                                    'H1N1
            For intRow = 2 To 13
                ...
                For intCol = intCol2 To 9
                    tblexcel.GetText intRow, intCol, varTmp
                    If Trim(varTmp) = "" Then
                        tblexcel.SetText intRow, intCol, "N" & strBarNo
                        tblexcel.SetText intRow + 6, intCol, "N" & strBarNo
                        Exit Function
                    End If
                Next intCol
            Next intRow
