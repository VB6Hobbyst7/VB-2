...
Begin VB.Form frmIISPFA100 
   ...
   Begin FPSpread.vaSpread tblComplete 
      ...
      SpreadDesigner  =   "frmIISPFA100.frx":0000
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISPFA100.frx":07AF
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISPFA100"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISPFA100.frm
'   작성자  : 오세원
'   내  용  : PFA100 장비폼
'   작성일  : 2010-05-31
'   버  전  :
'       1. 1.0.0: 오세원(2010-05-31)
'   병  원  :
'       1. 안동성소병원
'-----------------------------------------------------------------------------'
...
Private Sub EditRcvData()
    ...
    Dim strIntResult   As String   '수신한 결과값
    ...
    Dim vRcvBuf     As Variant
    
    If Len(Buffer) < 50 Then
        Buffer = ""
        Exit Sub
    End If
    
    strRcvBuf = Buffer 'mIntLib.Buffers(1).Buffers
    strRcvBuf = Mid(strRcvBuf, InStr(strRcvBuf, "PFA-100"))
    strRcvBuf = Replace(strRcvBuf, vbLf, "")
    vRcvBuf = Split(strRcvBuf, vbCr)
    
    strType = vRcvBuf(0)
    
    If strType = "PFA-100" Then
        '## 바코드번호 조회
        strBarNo = Mid$(Trim$(Mid$(vRcvBuf(3), 7)), 1, SPCLEN)
...
'            .SpcPos = strPos & "/" & strRackNo
        ...
        '## 검사결과 조회
        strIntBase = Mid$(Trim$(Mid$(vRcvBuf(4), 21)), 1)
        strIntResult = Mid$(Trim$(Mid$(vRcvBuf(5), 11)), 1, 3)
...
On Error GoTo Errors
    ...
    With MSComm
        ...
        If .PortOpen Then
            ...
            Error.SetLog App.EXEName, "frmIISTEST1", "GetEqpComm", strErrMsg, Now
