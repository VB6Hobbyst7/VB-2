...
Begin VB.Form frmIISElecsys1010 
   BackColor       =   &H00DBE6E6&
   Caption         =   "Elecsys 1010"
   ClientHeight    =   9180
   ClientLeft      =   165
   ClientTop       =   450
   ClientWidth     =   15240
   ...
   Begin FPSpread.vaSpread tblReady 
      ...
      SpreadDesigner  =   "frmIISElecsys1010.frx":0000
   End
   Begin FPSpread.vaSpread tblComplete 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISElecsys1010.frx":04EF
   End
   ...
   Begin FPSpread.vaSpread tblResult 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   13697023
      SpreadDesigner  =   "frmIISElecsys1010.frx":0D31
      TextTip         =   2
   End
End
Attribute VB_Name = "frmIISElecsys1010"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'-----------------------------------------------------------------------------'
'   파일명  : frmIISElecsys1010.frm
'   작성자  : 이상대
'   내  용  : Elecsys 1010 장비폼
'   작성일  : 2004-07-02
'   버  전  :
'   병  원  :
'       1. 동국대 경주병원
...
Private Sub EditRcvData()
    ...
    Dim strDiskNo    As String   '수신한 Rack Or Disk No
        ...
        Select Case strType
            ...
            Case "Q"    '## Request Information
                ...
                With mOrder
                    .ClsClear
                    .BarNo = strBarNo
                    .Seq = mGetP(strRcvBuf, 3, "^")
                    .DiskNo = mGetP(strRcvBuf, 4, "^")
                    .TubePos = mGetP(strRcvBuf, 5, "^")
                End With
            ...
            Case "O"    '## Order
                ...
                strDiskNo = mGetP(strTemp, 2, "^")
                strTubePos = Format$(mGetP(strTemp, 3, "^"), "##")
                
                '## Elecsys 1010장비는 Disk No가 항상 "0"이어서 검체위치는 Tube Position 사용
            ...
            Case "C"    '## Comment
                ...
                If strFlag <> "N" Then
                    strTemp = mGetP(strRcvBuf, 4, "|")
                    strComm = "[Flag]: " & mGetP(strTemp, 1, "^") & ", " & mGetP(strTemp, 2, "^")
        ...
        Case 4  '## Order
            With mOrder
                If .NoOrder = True Then
                    '## 접수정보가 없을경우
                    strOutput = mIntLib.GetFrameNo & "O|1|" & .BarNo & "|" & .Seq & "^" & .DiskNo & _
                                "^" & .TubePos & "^^SAMPLE^NORMAL|ALL" & _
                                "|R||||||C||||||||||||||Q" & vbCr & ETX
                    mIntLib.SndPhase = 5
                Else
                    If .IsSending = False Then  '## 최초 보낼때
                        strOutput = "O|1|" & .BarNo & "|" & .Seq & "^" & .DiskNo & "^" & .TubePos & _
...
Private Sub SetReady(ByVal pAccInfo As clsIISAccInfo)
    ...
    With tblReady
        ...
        .Col = TReadyEnum.ccNo:     .Text = Format$(mOrder.TubePos, "##")
...
Private Sub SetComplete2(ByVal pAccInfo As clsIISAccInfo)
    ...
    With tblComplete
        If pAccInfo.RcvCnt = 1 Then
            ...
            End If
        Else
            '## 전송된 항목이 해당검체의 첫번째 항목이 아니면 tblComplete Spread에서 같은
            '## 바코드번호, WorkNo를 찾어 같은 Row에 결과를 입력
            For i = .DataRowCnt To 1 Step -1
                Call .GetText(TCompleteEnum.ccNo, i, vTubePos)
                Call .GetText(TCompleteEnum.ccBarNo, i, vBarNo)
                
                If CStr(vBarNo) = pAccInfo.GetBarNo And CStr(vTubePos) = pAccInfo.SpcPos Then
                    .Row = i
                    Exit For
                End If
            Next i
