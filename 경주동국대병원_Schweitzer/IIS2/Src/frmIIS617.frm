...
Begin VB.Form frmIIS617 
   BackColor       =   &H00DBE6E6&
   BorderStyle     =   4  '고정 도구 창
   Caption         =   "QC Reject 사유 관리"
   ClientHeight    =   8925
   ClientLeft      =   4080
   ClientTop       =   285
   ClientWidth     =   11175
...
Private Sub Form_Activate()
    ...
    frmIIS600.tvwMenu.Nodes("IIS617").Selected = True
End Sub

Private Sub Form_Unload(Cancel As Integer)
    Set mTemplate = Nothing
    Set frmIIS617 = Nothing
End Sub

Private Sub cmdSave_Click()
    Dim itmX        As ListItem
    Dim strCode     As String       'QC Reject 사유 코드
    Dim strContent  As String       'QC Reject 사유

    '## QC Reject 사유 코드
    If txtCode.Text = "" Then
        MsgBox "QC Reject 사유 코드를 입력하세요.", vbInformation, "정보"
        txtCode.SetFocus
        Exit Sub
    End If
    ...
    If txtContent.Text = "" Then
        MsgBox "QC Reject 사유를 입력하세요.", vbInformation, "정보"
        txtContent.SetFocus
        Exit Sub
    End If
    ...
    With mTemplate
        ...
        '# QC Reject 사유 코드가 있으면 Update, 없으면 Insert
...
Private Sub cmdDelete_Click()
    ...
    Set itmX = lvwCodeList.FindItem(strCode, lvwText)
    If itmX Is Nothing Then
        MsgBox "존재하지 않는 QC Reject 사유 코드 입니다.", vbInformation, "정보"
        With txtCode
            .SetFocus
            .SelStart = 0
            .SelLength = Len(.Text)
...
Private Sub txtContent_LostFocus()
    ...
    If Len(strContent) >= 4000 Then
        MsgBox "QC Reject 사유는 4000자 이하로 입력해야 합니다.", vbInformation, "정보"
        txtContent.SetFocus
    End If
End Sub
...
On Error GoTo Errors
    With lvwCodeList
        ...
        Set Rs = mTemplate.GetTemplates(CQCREJECTCD)
...
Errors:
    Set Rs = Nothing
    Set itmX = Nothing
    Error.SetLog App.EXEName, "frmIIS617", "GetCodeList", Err.Description, Now
