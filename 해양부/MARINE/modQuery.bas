...
'-- 조위VPN리스트 찾아온다.
Public Function Get_VPNList(Optional ByVal pDT_TS_ID As String, Optional ByVal pNameSort As Boolean = False) As ADODB.Recordset
    Dim pAdoRS      As ADODB.Recordset
    Dim pCallForm   As String
    
    pCallForm = "Public Function Get_VPNList(Optional ByVal pDT_TS_ID As String) As ADODB.Recordset"
    
On Error GoTo ErrorRoutine

'CREATE TABLE RTDB.DT_MAX_DT_TIME //조위관측소 VPN 54개 정도
'(
'    DT_TS_ID   NUMBER NOT NULL,
'    DT_TIME Not Date
')

'CREATE TABLE RTDB.TIDAL_STATION
'(
...
'    DEFAULT_TIDE      VARCHAR2 (30),
'    OBS_POST_ID       VARCHAR2 (11) NOT NULL,
'    COLLECTION_TYPE   CHAR (1) DEFAULT 'V' NOT NULL,
'    VPN_IP            VARCHAR2 (17),
'    KORDI_MODELING    CHAR (1) DEFAULT 'N' NOT NULL,
'    FTP_USER          VARCHAR2 (50),
'    FTP_PW            VARCHAR2 (50),
'    FTP_PORT          NUMBER,
'    FTP_REMOTEDIR     VARCHAR2 (200),
'    FTP_LOCALDIR      VARCHAR2 (200),
'    FTP_PASSIVE       NUMBER,
'    COLLECTION_FILE VARCHAR2(10)
')

    SQL = ""
    SQL = SQL & "SELECT a.DT_TS_ID, a.DT_TIME, b.TS_NAME        " & vbCrLf
    SQL = SQL & "  FROM RTDB.DT_MAX_DT_TIME a                   " & vbCrLf
    SQL = SQL & "     , RTDB.TIDAL_STATION b                    " & vbCrLf
    SQL = SQL & " WHERE a.DT_TS_ID = b.TS_ID                    " & vbCrLf
    If pDT_TS_ID <> "" Then
        SQL = SQL & "   AND a.DT_TS_ID = '" & pDT_TS_ID & "'    " & vbCrLf
    End If
    If pNameSort = True Then
        SQL = SQL & " Order By b.TS_NAME"
    Else
        SQL = SQL & " ORDER BY a.DT_TIME DESC, b.TS_NAME" & vbCrLf
    End If
    'Call SetRawData(SQL)
    ...
    If GetRecordset(AdoCn, SQL, pAdoRS, pCallForm) Then
        Set Get_VPNList = pAdoRS
    Else
        Set Get_VPNList = Nothing
    End If
...
'-- 부이VPN리스트 찾아온다.
Public Function Get_VPNList2(Optional ByVal pSTATION_ID As String, Optional ByVal pNameSort As Boolean = False) As ADODB.Recordset
    Dim pAdoRS      As ADODB.Recordset
    Dim pCallForm   As String
    
    pCallForm = "Public Function Get_VPNList2(Optional ByVal pSTATION_ID As String) As ADODB.Recordset"
    
On Error GoTo ErrorRoutine

'CREATE TABLE RTDB.BUOY_MAX_DT_TIME //조위관측소 VPN 54개 정도
'(
'    STATION_ID   NUMBER NOT NULL,
'    OBS_TIME Not Date
')

'CREATE TABLE RTDB.T_WRN_STATION
'(
'    STATION_ID
'    STATION_LON
'    STATION_LAT
...
')
    ...
    SQL = ""
    SQL = SQL & "SELECT a.STATION_ID, a.OBS_TIME, b.STATION_NAME" & vbCrLf
    SQL = SQL & "  FROM WRN.BUOY_MAX_DT_TIME a                 " & vbCrLf
    SQL = SQL & "     , WRN.T_WRN_STATION b                    " & vbCrLf
    SQL = SQL & " WHERE a.STATION_ID = b.STATION_ID             " & vbCrLf
    If pSTATION_ID <> "" Then
        SQL = SQL & "   AND a.STATION_ID = '" & pSTATION_ID & "'  " & vbCrLf
    End If
    
    If pNameSort = True Then
        SQL = SQL & " Order By b.STATION_NAME"
    Else
        SQL = SQL & " ORDER BY a.OBS_TIME DESC " & vbCrLf
    End If
    ...
    If GetRecordset(AdoCn2, SQL, pAdoRS, pCallForm) Then
        Set Get_VPNList2 = pAdoRS
        Set pAdoRS = Nothing
    Else
        Set Get_VPNList2 = Nothing
    End If
...
'-- 부이리스트 찾아온다.
Public Function Get_BUOYList_Detail(Optional ByVal pSTATION_ID As String, _
                             ...
                             Optional ByVal pCount As Integer) As ADODB.Recordset
    ...
    If pDT_F_HOUR = "1" Then
        strFrom = Format(pDT_F_TIME, "YYYYMMDD") & "000000"
    Else
        strFrom = Format(pDT_F_TIME, "YYYYMMDD") & Format(pDT_F_HOUR - 1, "00") & "5959"
    End If
    
    If pDT_T_HOUR = "1" Then
        strTo = Format(pDT_T_TIME, "YYYYMMDD") & "000000"
    Else
        strTo = Format(pDT_T_TIME, "YYYYMMDD") & Format(pDT_T_HOUR - 1, "00") & "5959"
    End If
    
    pCallForm = "Public Function Get_BUOYList_Detail() As ADODB.Recordset"
    
'CREATE TABLE WRN.BUOY_MAX_DT_TIME //종합해양 관측부이
'(
'    STATION_ID   VARCHAR2 (20) NOT NULL,
'    OBS_TIME Not Date
')

'CREATE TABLE WRN.T_WRN_STATION //종합해양 관측부이
'(
'    STATION_ID     VARCHAR2 (20) NOT NULL,
'    STATION_LON    NUMBER,
'    STATION_LAT    NUMBER,
'    STATION_NAME   VARCHAR2 (20),
'    OBS_POST_ID    VARCHAR2 (11),
'    EQUIP_ID       VARCHAR2 (20),
'    STATION_GU VARCHAR2(4)
')
...
'''    SQL = SQL & "  FROM WRN.BUOY_MAX_DT_TIME a, WRN.T_WRN_STATION b     " & vbCrLf
'''    SQL = SQL & " WHERE a.STATION_ID = b.STATION_ID                     " & vbCrLf
'''    If pSTATION_ID <> "" Then
'''        SQL = SQL & "   AND a.STATION_ID = '" & pSTATION_ID & "'" & vbCrLf
'''    End If
'''    If pDT_F_TIME <> "" And pDT_T_TIME <> "" Then
'''        SQL = SQL & "   AND a.OBS_TIME BETWEEN TO_DATE('" & strFrom & "','YYYYMMDDHH24MISS') AND TO_DATE('" & strTo & "','YYYYMMDDHH24MISS')" & vbCrLf
'''    End If
'''    If pCount > 0 Then
'''        SQL = SQL & "   AND ROWNUM <= " & pCount & vbCrLf
'''    End If
'''
'''    SQL = SQL & " ORDER BY a.STATION_ID, a.OBS_TIME, b.STATION_NAME" & vbCrLf
    

    SQL = ""
    SQL = SQL & "SELECT  b.STATION_NAME                    " & vbCrLf
    ...
    SQL = SQL & ", a.AIR_TEMP"
    SQL = SQL & ", a.AIR_PRES"
    SQL = SQL & ", a.BUOY_ORIENTATION"
    SQL = SQL & ", a.WH"
    SQL = SQL & ", a.WP"
    SQL = SQL & ", a.CURRENT_S"
    SQL = SQL & ", a.CURRENT_D"
    SQL = SQL & ", a.W_TEMP"
    SQL = SQL & ", a.CONDUCTIVITY"
    SQL = SQL & ", a.SAL"
    SQL = SQL & ", a.EQUIP_ID"
    SQL = SQL & ", a.WIND_D_RAW"
    SQL = SQL & ", a.VISIBILITY"
    SQL = SQL & ", a.BATTERY"
    SQL = SQL & ", a.REFERENCE"
    SQL = SQL & ", a.TRACK_SEQ"
    SQL = SQL & ", a.MAX_WAVE_PERIOD"
    SQL = SQL & ", a.MAX_WAVE_HEIGHT"
    SQL = SQL & ", a.WAVE_DIRECT"
    SQL = SQL & ", a.ORIGINAL_WIND_D"
    SQL = SQL & "  FROM WRN.T_WRN_TW_BUOY a                     " & vbCrLf
    SQL = SQL & "     , WRN.T_WRN_STATION b                     " & vbCrLf
    SQL = SQL & " WHERE a.STATION_ID    = b.STATION_ID          " & vbCrLf
'    SQL = SQL & "   AND a.STATION_ID    = '" & pStationID & "'  " & vbCrLf
    If pSTATION_ID <> "" Then
        SQL = SQL & "   AND a.STATION_ID = '" & pSTATION_ID & "'" & vbCrLf
    End If
    If pDT_F_TIME <> "" And pDT_T_TIME <> "" Then
...
'        SQL = SQL & "   AND ROWNUM <= " & pCount & vbCrLf
'    End If
    SQL = SQL & " ORDER BY a.OBS_TIME                           " & vbCrLf
...
'    Call SetRawData(SQL)
    ...
    If GetRecordset(AdoCn2, SQL, pAdoRS, pCallForm) Then
        Set Get_BUOYList_Detail = pAdoRS
    Else
        Set Get_BUOYList_Detail = Nothing
    End If
...
'-- 부이리스트 찾아온다.
Public Function Get_BUOYList(Optional ByVal pSTATION_ID As String) As ADODB.Recordset
    Dim pAdoRS      As ADODB.Recordset
    Dim pCallForm   As String
    
    pCallForm = "Public Function Get_BUOYList(Optional ByVal pSTATION_ID As String) As ADODB.Recordset"
...
On Error GoTo ErrorRoutine
    ...
    If GetRecordset(AdoCn2, SQL, pAdoRS, pCallForm) Then
        Set Get_BUOYList = pAdoRS
    Else
        Set Get_BUOYList = Nothing
    End If
...
'-- 조위관측자료수집로그 찾아온다.
Public Function Get_JOUYList(Optional ByVal pDT_TS_ID As String, Optional ByVal pDT_TIME As String, Optional ByVal pCount As Integer) As ADODB.Recordset
    
    Dim pAdoRS      As ADODB.Recordset
    Dim pCallForm   As String
    
    pCallForm = "Public Function Get_JOUYList() As ADODB.Recordset"
    
'CREATE TABLE RTDB.LOG_DT 조위관측소()
'(
'    DT_TS_ID      NUMBER NOT NULL,
'    DT_TIME       DATE NOT NULL,
'    LOG_ID        CHAR (4) NOT NULL,
'    REG_DATE      DATE NOT NULL,
'    LOG_CONTENT   VARCHAR2 (500) NOT NULL,
'    OBS_POST_ID   VARCHAR2 (11) DEFAULT '000000' NOT NULL
')

On Error GoTo ErrorRoutine

    SQL = ""
    SQL = SQL & "SELECT a.DT_TS_ID, b.TS_NAME, a.DT_TIME, a.REG_DATE, a.LOG_CONTENT " & vbCrLf
    SQL = SQL & "  FROM RTDB.LOG_DT a                                               " & vbCrLf
    SQL = SQL & "     , RTDB.TIDAL_STATION b                                        " & vbCrLf
    SQL = SQL & " WHERE a.DT_TS_ID = b.TS_ID                                        " & vbCrLf
    SQL = SQL & "   AND ROWNUM <= " & pCount
    If pDT_TS_ID <> "" Then
        SQL = SQL & "   AND a.DT_TS_ID    = '" & pDT_TS_ID & "'                     " & vbCrLf
    End If
    If pDT_TIME <> "" Then
        'SQL = SQL & "   AND a.DT_TIME    <= '" & Format(pDT_TIME, "YY/MM/DD") & "'   " & vbCrLf
        SQL = SQL & "   AND a.DT_TIME    >= '" & Format(pDT_TIME, "YY/MM/DD") & "'   " & vbCrLf
    End If
    
    SQL = SQL & " ORDER BY a.DT_TIME DESC                                           " & vbCrLf
...
'    Call SetRawData(SQL)
    ...
    If GetRecordset(AdoCn, SQL, pAdoRS, pCallForm) Then
        Set Get_JOUYList = pAdoRS
    Else
        Set Get_JOUYList = Nothing
    End If
...
'-- 조위관측자료수집로그 찾아온다.
Public Function Get_DTList(Optional ByVal pDT_TS_ID As String, Optional ByVal pDT_TIME As String, Optional ByVal pCount As Integer) As ADODB.Recordset
    
    Dim pAdoRS      As ADODB.Recordset
    Dim pCallForm   As String
    
    pCallForm = "Public Function Get_DTList() As ADODB.Recordset"
    
On Error GoTo ErrorRoutine

    SQL = ""
    SQL = SQL & "SELECT a.DT_TS_ID"
    SQL = SQL & ", b.TS_NAME"
    SQL = SQL & ", a.DT_TIME"
    SQL = SQL & ", a.DT_WSPEED"
    SQL = SQL & ", a.DT_WMSPEED"
    SQL = SQL & ", a.DT_WDIR"
    SQL = SQL & ", a.DT_ATEMP"
    SQL = SQL & ", a.DT_APRESS"
    SQL = SQL & ", a.DT_TIDE"
    ...
    SQL = SQL & ", a.DT_SAL"
    SQL = SQL & ", a.DT_WTEMP"
    SQL = SQL & ", a.DT_VISIBILITY"
    SQL = SQL & ", a.REFERENCE"
    SQL = SQL & ", a.DT_TIDE3"
    SQL = SQL & ", a.DT_CONDUCTIVITY"
    SQL = SQL & ", a.DT_REF_TIME"
    SQL = SQL & ", a.DT_FLAG"
    SQL = SQL & ", a.REG_DATE"
    SQL = SQL & ", a.TRACK_SEQ"
    SQL = SQL & ", a.DT_SIGNIFI_WAVE"
    SQL = SQL & ", a.DT_MAX_WAVE"
    SQL = SQL & ", a.DT_SIGNIFI_WAVE_PERIOD"
    SQL = SQL & ", a.DT_MAX_WAVE_PERIOD"
    SQL = SQL & ", a.DT_INSOLATION"
    SQL = SQL & ", a.DT_LTIDE"
    SQL = SQL & ", a.DT_LTIDE2"
    SQL = SQL & "  FROM RTDB.DT a                             " & vbCrLf
    SQL = SQL & "     , RTDB.TIDAL_STATION b                                        " & vbCrLf
    SQL = SQL & " WHERE a.DT_TS_ID = b.TS_ID                                        " & vbCrLf
    SQL = SQL & "   AND ROWNUM <= " & pCount
    If pDT_TS_ID <> "" Then
...
'    Call SetRawData(SQL)
    ...
    If GetRecordset(AdoCn, SQL, pAdoRS, pCallForm) Then
        Set Get_DTList = pAdoRS
    Else
        Set Get_DTList = Nothing
    End If
...
'-- 조위관측자료수집로그 찾아온다.
Public Function Get_DTList_Detail(Optional ByVal pDT_TS_ID As String, _
                             Optional ByVal pDT_F_TIME As String, _
                             Optional ByVal pDT_F_HOUR As String, _
                             Optional ByVal pDT_T_TIME As String, _
                             Optional ByVal pDT_T_HOUR As String, _
    ...
    pCallForm = "Public Function Get_DTList_Detail() As ADODB.Recordset"
...
On Error GoTo ErrorRoutine
    ...
    'If pCount <> "" Then
'        SQL = SQL & "   AND ROWNUM <= " & CStr(pCount) & vbCrLf
    'End If
    ...
    If pDT_F_TIME <> "" Then
     '   SQL = SQL & "   AND a.DT_TIME     = '" & Format(pDT_TIME, "YY/MM/DD") & "'    " & vbCrLf
        SQL = SQL & "   AND a.DT_TIME BETWEEN TO_DATE('" & strFrom & "','YYYYMMDDHH24MISS') AND TO_DATE('" & strTo & "','YYYYMMDDHH24MISS')" & vbCrLf
    End If
    SQL = SQL & " ORDER BY a.DT_TIME DESC                         " & vbCrLf
...
'    Call SetRawData(SQL)
    ...
    If GetRecordset(AdoCn, SQL, pAdoRS, pCallForm) Then
        Set Get_DTList_Detail = pAdoRS
    Else
        Set Get_DTList_Detail = Nothing
    End If
...
'-- 조위관측자료수집로그 찾아온다.
Public Function Get_JOUYList_Detail(Optional ByVal pDT_TS_ID As String, _
                             Optional ByVal pDT_F_TIME As String, _
                             Optional ByVal pDT_F_HOUR As String, _
                             Optional ByVal pDT_T_TIME As String, _
                             Optional ByVal pDT_T_HOUR As String, _
    ...
    pCallForm = "Public Function Get_JOUYList_Detail() As ADODB.Recordset"
...
On Error GoTo ErrorRoutine
    ...
    SQL = ""
    SQL = SQL & "SELECT a.DT_TS_ID, b.TS_NAME, a.DT_TIME, a.REG_DATE, a.LOG_CONTENT  " & vbCrLf
    SQL = SQL & "  FROM RTDB.LOG_DT a, RTDB.TIDAL_STATION b                                                 " & vbCrLf
    SQL = SQL & " WHERE a.DT_TS_ID = b.TS_ID                                                                " & vbCrLf
    SQL = SQL & "   AND a.DT_TS_ID = '" & pDT_TS_ID & "'                                                    " & vbCrLf
    If pDT_F_TIME <> "" And pDT_T_TIME <> "" Then
          'AND a.DT_TIME BETWEEN TO_DATE('20200415010000','YYYYMMDDHH24MISS') AND TO_DATE('20200417235959','YYYYMMDDHH24MISS')
        SQL = SQL & "   AND a.DT_TIME BETWEEN TO_DATE('" & strFrom & "','YYYYMMDDHH24MISS') AND TO_DATE('" & strTo & "','YYYYMMDDHH24MISS')" & vbCrLf
    End If
'    If pCount > 0 Then
'        SQL = SQL & "   AND ROWNUM <= " & pCount & vbCrLf
...
'    Call SetRawData(SQL)
    ...
    If GetRecordset(AdoCn, SQL, pAdoRS, pCallForm) Then
        Set Get_JOUYList_Detail = pAdoRS
    Else
        Set Get_JOUYList_Detail = Nothing
    End If
...
'-- 해양부이VIEW리스트 찾아온다.
Public Function Get_BOUYViewList(Optional ByVal pStationID As String, Optional ByVal pOBSTIME As String, Optional ByVal pCount As String) As ADODB.Recordset
    
    Dim pAdoRS      As ADODB.Recordset
    Dim pCallForm   As String
    
    pCallForm = "Public Function Get_BOUYViewList() As ADODB.Recordset"
    
'CREATE TABLE WRN.T_WRN_TW_BUOY
'(
'    STATION_ID         VARCHAR2 (20) NOT NULL,
'    BUOY_LON           VARCHAR2 (20),
'    BUOY_LAT           VARCHAR2 (20),
...
'    BATTERY            NUMBER,
'    REFERENCE          NUMBER,
'    TRACK_SEQ          VARCHAR2 (50),
'    MAX_WAVE_PERIOD    NUMBER,
'    MAX_WAVE_HEIGHT    NUMBER,
'    WAVE_DIRECT        NUMBER,
'    ORIGINAL_WIND_D    NUMBER,
'    CONSTRAINT FK_TW_BUOY_STATION_EQUIP FOREIGN KEY (STATION_ID, EQUIP_ID) REFERENCES WRN.T_WRN_EQUIP_META (STATION_ID, EQUIP_ID)
')

On Error GoTo ErrorRoutine

    pOBSTIME = Format(pOBSTIME, "YYYYMMDDhhmmss")
    ...
    SQL = SQL & " WHERE a.STATION_ID    = b.STATION_ID          " & vbCrLf
    SQL = SQL & "   AND a.STATION_ID    = '" & pStationID & "'  " & vbCrLf
    SQL = SQL & "   AND a.OBS_TIME      <= TO_DATE('" & pOBSTIME & "','YYYYMMDDHH24MISS')" & vbCrLf
    
    'SQL = SQL & "   AND a.OBS_TIME      = '" & Format(pOBSTIME, "YYYYMMDDhhmmss") & "'    " & vbCrLf
    ...
    If GetRecordset(AdoCn2, SQL, pAdoRS, pCallForm) Then
        Set Get_BOUYViewList = pAdoRS
    Else
        Set Get_BOUYViewList = Nothing
    End If
