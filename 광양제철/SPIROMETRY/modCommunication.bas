...
Type IntfData
    ...
    DeltaGBN    As String
    BeforeRslt  As String
    
    'SPIROMETRY
    TestDate    As String
    PRace       As String '(1:?, 2:?,3:Asian Org.)
    PRacePer    As String
    PHeight     As String
    PWeight     As String
    AtmPress    As String
...
'for SPIROMETRY
Public mSTXNo               As Integer
Public isSTX                As Boolean
...
'                            mOrder.BarNo = pBarno
        ...
        Call SetText(.spdOrder, mResult.PSex, intRow, colPSEX)
        Call SetText(.spdOrder, mResult.PAge, intRow, colPAGE)
    
        Call SetText(.spdOrder, mResult.PHeight, intRow, colRACKNO) '신장
        Call SetText(.spdOrder, mResult.PWeight, intRow, colPOSNO)  '체중
        Call SetText(.spdOrder, mResult.Roomtemp, intRow, colINOUT)   '온도
        Call SetText(.spdOrder, mResult.Humidity, intRow, colCHARTNO)   '습도
        Call SetText(.spdOrder, mResult.AtmPress, intRow, colPID)   '기압
        Call SetText(.spdOrder, mResult.PRace & ". (" & mResult.PRacePer & ")", intRow, colPJUMIN)    'Race
...
'        End If
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,SAMPLETYPE,INOUT,PANICVALUE,REFJUDGE,DELTAVALUE " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
...
'        Call SetSQLData("로컬결과조회", SQL)
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strHospDate <> "" And strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                'If strJumin <> "" And Len(strJumin) = 13 Then
                '    SQL = SQL & "   AND IDLEFT  = '" & Mid(strJumin, 1, 6) & "'" & vbCr
                '    SQL = SQL & "   AND IDRIGHT = '" & Mid(strJumin, 7, 7) & "'" & vbCr
                'End If
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        strSaveSeq = .lblSeq(0).Caption  ' Trim(GetText(.spdOrder, argSpcRow, colSAVESEQ))
        strExamDate = Trim(GetText(.spdOrder, argSpcRow, colEXAMDATE))
        ...
        SQL = SQL & "   AND MID(EXAMDATE,1,8)   = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        SQL = SQL & "   AND BARCODE             = '" & strBarcode & "' " & vbCr                       '바코드
        SQL = SQL & "   AND EXAMCODE            <> 'N/A' " & vbCr                       '바코드
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 서버저장
                SQL = ""
                SQL = SQL + "UPDATE TB_JUPSU_ITEM                           " & vbCrLf
                SQL = SQL + "   SET RESULT          = '" & sResult & "'     " & vbCrLf
                SQL = SQL + " WHERE PER_JUPSU_DATE  = '" & strDate & "'     " & vbCrLf
                SQL = SQL + "   AND MEDITEM         = '" & strTestCd & "'   " & vbCrLf
...
Function SaveTransData_ONITGUMJIN(ByVal Index As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        SaveTransData_ONITGUMJIN = -1
        intRow = 0
        blnSave = False
        
        strExamDate = Mid(.txtTestDate(Index).Text, 1, 10)
        strExamDate = Format(strExamDate, "yyyymmdd")
        strHospDate = .txtHospDate(Index).Text
        strBarcode = Trim(.txtPatID(Index).Text)
...
'MsgBox varKey(2)
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT HOSPDATE, EQUIPCODE, EXAMCODE, RESULT " & vbCr
        SQL = SQL & "  FROM PATRESULT_BEST              " & vbCr
        SQL = SQL & " WHERE EQUIPNO             = '" & gHOSP.MACHCD & "'" & vbCr                '장비코드
        SQL = SQL & "   AND MID(EXAMDATE,1,8)   = '" & strExamDate & "'" & vbCr      '검사일
        SQL = SQL & "   AND HOSPDATE            = '" & strHospDate & "'" & vbCr      '
        SQL = SQL & "   AND BARCODE             = '" & strBarcode & "' " & vbCr                 '바코드
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            ...
            Do Until RS_L.EOF
                intRow = intRow + 1
                Call SetText(.vasTemp, RS_L.Fields("HOSPDATE").Value & "", intRow, 1)
                Call SetText(.vasTemp, RS_L.Fields("EQUIPCODE").Value & "", intRow, 2)
                Call SetText(.vasTemp, RS_L.Fields("EXAMCODE").Value & "", intRow, 3)
                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 4)
                RS_L.MoveNext
            Loop
        End If
...
'MsgBox "333"
        '-- 서버로 결과값 저장하기
        For intRow = 1 To .vasTemp.DataRowCnt
            strDate = Trim(GetText(.vasTemp, intRow, 1))
            strTestCd = Trim(GetText(.vasTemp, intRow, 3))     '검사코드
            sResult = Trim(GetText(.vasTemp, intRow, 4))       '결과(수정결과)
...
'MsgBox sResult
            ...
            If strBarcode <> "" And strDate <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 서버저장
                SQL = ""
                SQL = SQL + "UPDATE TB_JUPSU_ITEM                           " & vbCrLf
                SQL = SQL + "   SET RESULT          = '" & sResult & "'     " & vbCrLf
                ...
                SQL = SQL + "   AND MEDITEM         = '" & strTestCd & "'   " & vbCrLf
                '채용검진
                'strKey = Trim(RS.Fields("per_jupsu_date")) & "|" & Trim(RS.Fields("per_ssn")) & "|" & Trim(RS.Fields("group_no")) & "|" & Trim(RS.Fields("seq_no")) & "|"
                If frmMain.chkCYGJ.Value = "1" Then
                    SQL = SQL + "   AND per_jupsu_date  = '" & varKey(0) & "'" & vbCrLf
                    SQL = SQL + "   AND per_ssn         = '" & varKey(1) & "'" & vbCrLf
                    SQL = SQL + "   AND group_no        = '" & varKey(2) & "'" & vbCrLf
'                    SQL = SQL + "   AND seq_no          = '" & varKey(3) & "'" & vbCrLf
                Else
                    SQL = SQL + "   AND PER_JIKBUN  = '" & strBarcode & "'  " & vbCrLf
                End If
...
Function SetJudge(asResult As String, asEquipCode As String)
    ...
    Select Case gEMR
        Case "ACK"                         '아미스
                SetJudge = SetJudge_ACK(asResult, asEquipCode)
...
Function SetPanic(asResult As String, asEquipCode As String)

    Select Case gEMR
        Case "ACK"                         '아미스
                SetPanic = SetPanic_ACK(asResult, asEquipCode)
    End Select
    
End Function

Function SetDelta(asResult As String, asEquipCode As String, asDeltaGbn As String, asBeforeResullt As String)

    Select Case gEMR
        Case "ACK"                         '아미스
                SetDelta = SetDelta_ACK(asResult, asEquipCode, asDeltaGbn, asBeforeResullt)
    End Select
    
End Function


Function SetJudge_ACK(asResult As String, asEquipCode As String) As String
    Dim RSJ         As ADODB.Recordset
...
On Error GoTo ErrorTrap
    ...
    If strSex = "M" Then
        SQL = "Select REFHIGHM as MAX, REFLOWM as MIN "
    Else
        SQL = "Select REFHIGHF as MAX, REFLOWF as MIN "
    End If
'    SQL = SQL & "    , PANICLOW, PANICHIGH  "
...
Function GetTestRst_ACK(pBarCd As String, pTestCd As String) As Boolean
    Dim RSJ         As ADODB.Recordset
    Dim sqlRet      As Integer
    Dim sqlDoc      As String
    Dim strAge      As String
...
On Error GoTo ErrorTrap
    ...
    GetTestRst_ACK = False
    
    SQL = ""
    SQL = SQL & "SELECT R.RESULT " & vbCr
    Select Case gHOSP.PARTCD
    ...
    End Select
    SQL = SQL & " WHERE M.MEDICALREMARK = '" & pBarCd & "'" & vbCr
    ...
    SQL = SQL & "   And R.TESTITEMSEQ   = '" & pTestCd & "'" & vbCr
    SQL = SQL & "   AND R.CONFIRM       <> 'Y' " & vbCr
    
    Call SetSQLData("계산결과", SQL, "A")

    Set RSJ = AdoCn.Execute(SQL)
    Do Until RSJ.EOF
        If Trim(RSJ.Fields("RESULT")) <> "" Then
            GetTestRst_ACK = True
        Else
            GetTestRst_ACK = False
        End If
...
'select top 1 result from c_result
' where LABDATE < '20190208'
'   and  PARTGBN = '01'
'   and SPECIMENCD = '002'
'   and regno = '00287438'
'   and testitemseq = '080'
        ...
        If IsNumeric(asResult) Then
            ...
            If IsNumeric(strDeltaMIN) And IsNumeric(strDeltaMAX) Then
                Select Case asDeltaGbn
                    ...
                    Case "4"    '기간당 변화비율 ((오늘결과 - 이전결과) / 이전결과 ) * 100
                            strDelta = ((CCur(asResult) - CCur(asBeforeResullt)) / CCur(asBeforeResullt)) * 100
