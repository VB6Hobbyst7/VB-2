...
Public Function mCalP(ByVal pText As String) As String
    
    Dim strOne      As String
    Dim intVal(10)  As Integer
    Dim strYSJ(10)  As String
    ...
    For i = 1 To Len(pText)
        strOne = Mid(pText, i, 1)
        If IsNumeric(strOne) Then
            strVal = strVal & CStr(strOne)
        Else
            If strOne = "+" Or strOne = "-" Or strOne = "*" Or strOne = "/" Or strOne = "(" Or strOne = ")" Then
                strYSJ(j) = strOne
                j = j + 1
            Else
                If strOne = "." Then
...
Public Sub SetWorkData(argSQL As String, argCnt As Integer)
    Dim FilNum
    Dim sFilename   As String
    Dim FindFile    As String
    ...
    If Dir(App.PATH & "\WorkList", vbDirectory) <> "WorkList" Then
        MkDir (App.PATH & "\WorkList")
    End If
    
    sFilename = "WL_" & Format(CDate(Now), "yyyymmddhhmmss") & "_" & CStr(argCnt)
    
    Open App.PATH & "\WorkList\" & sFilename & ".txt" For Append As FilNum
...
ErrHandle:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SetXMLData" & vbNewLine & vbNewLine
...
Public Sub SetSPDOrder(ByVal vasTable As Object, ByVal argRow1 As Integer, ByVal argRow2 As Integer, ByVal argCol1 As Integer, ByVal argCol2 As Integer)
    
    Call SetText(vasTable, "◇", argRow1, argCol1)    '◇▣◈
    Call SetText(vasTable, "◇", argRow1, argCol1)    '◇▣◈
    ...
    With vasTable
        ...
        .GridShowHoriz = False
        .GridShowVert = False
        .SetCellBorder argCol1, argRow1, argCol2, argRow2, CellBorderIndexOutline, vbRed, CellBorderStyleFineDot
