    ...
    With frmInterface
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                strRsltDate = Format(Now, "yyyy-mm-dd")
                strRsltTime = Format(Time, "HH:MM:SS")
                
                If strChartNo <> "" And strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" Then
                    '-- 결과저장
                    SQL = ""
                    SQL = SQL & "Update SLA_LabResult                           " & vbCrLf
                    SQL = SQL & "   Set Result          = '" & sResult & "'     " & vbCrLf
                    SQL = SQL & "     , NormalFlag      = '0'                   " & vbCrLf
                    SQL = SQL & "     , PanicFlag       = '0'                   " & vbCrLf
                    SQL = SQL & "     , DeltaFlag       = '0'                   " & vbCrLf
                    SQL = SQL & "     , TransFlag       = '1'                   " & vbCrLf  '1:저장만/2:완료
                    SQL = SQL & "     , ResultID        = '" & gHOSP.USERID & "'" & vbCrLf
                    SQL = SQL & "     , ResultDate      = '" & strRsltDate & "' " & vbCrLf
                    SQL = SQL & "     , ResultTime      = '" & strRsltTime & "' " & vbCrLf
                    SQL = SQL & " Where ReceiptNo       = '" & strChartNo & "'  " & vbCrLf
                    ...
                    '-- 상태변경
                    SQL = ""
                    SQL = SQL & "Update SLA_LabMaster                           " & vbCrLf
                    SQL = SQL & "   Set JStatus         = '2'                   " & vbCrLf  '진행상태 flag  : 0.병동Barcode출력,    1.접수,  2.보류,  3.결과,  4.SignOut
                    SQL = SQL & " Where ReceiptNo       = '" & strChartNo & "'  " & vbCrLf
                    SQL = SQL & "   And SpecimenNum     = '" & strBarcode & "'  " & vbCrLf
                    SQL = SQL & "   And OrderCode       = '" & strOrdCd & "'    " & vbCrLf
                    SQL = SQL & "   And JStatus         IN ('0','1','2')        " & vbCrLf
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    SQL = SQL & "     , RESUPDDTM = '" & Format(Now, "yyyymmddhhmm") & "'           " & vbCrLf
                    SQL = SQL & "     , RESREPTYP = 'M'                                             " & vbCrLf  'M : 보고대기, N : 미결과, F : 보고
                    SQL = SQL & " WHERE LTRIM(RTRIM(ResOcmNum))         = '" & strBarcode & "'      " & vbCrLf  '내원번호
...
On Error GoTo ErrHandle
    ...
    SQL = ""
    SQL = SQL & "Select DISTINCT "
    SQL = SQL & "       a.RECEIPTDATE                           AS HOSPDATE     " & vbCrLf  '접수일자 YYYY-MM-DD
    SQL = SQL & "     , DECODE(a.IPDOPD,'1','입원','0','외래')  AS INOUT        " & vbCrLf  '1:입원, 0:외래
    SQL = SQL & "     , a.DEPTCODE                              AS DEPT         " & vbCrLf
    SQL = SQL & "     , a.SPECIMENNUM                           AS BARCODE      " & vbCrLf  '검체번호(Number(8)) nnnnnnnn
    SQL = SQL & "     , a.PTNO                                  AS PID          " & vbCrLf
    SQL = SQL & "     , a.RECEIPTNO                             AS CHARTNO      " & vbCrLf  '접수번호(Order별 부여:Number(8)) nnnnnnnn (PK)
    SQL = SQL & "     , a.SNAME                                 AS PNAME        " & vbCrLf
    SQL = SQL & "     , a.SEX                                   AS SEX          " & vbCrLf
    SQL = SQL & "     , a.AGE                                   AS AGE          " & vbCrLf
    SQL = SQL & "     , COUNT(a.LABCODE)                        AS CNT          " & vbCrLf
    SQL = SQL & "  From SLA_LabMaster a, "                                                  '임상병리 접수 마스터
    SQL = SQL & "       SLA_LabResult b                                         " & vbCrLf  '임상병리 결과 테이블
    SQL = SQL & " Where a.RECEIPTNO     =   b.RECEIPTNO                         " & vbCrLf  'PK
    SQL = SQL & "   And a.ORDERCODE     =   b.ORDERCODE                         " & vbCrLf  'PK
    SQL = SQL & "   And b.LABCODE       IN (" & gAllTestCd & ")                 " & vbCrLf  'PK
    SQL = SQL & "   And a.RECEIPTDATE   =   b.RECEIPTDATE                       " & vbCrLf  '의미없음
    SQL = SQL & "   And a.SPECIMENNUM   =   b.SPECIMENNUM                       " & vbCrLf  '의미없음
    SQL = SQL & "   And a.RECEIPTDATE Between '" & pFrom & "' And '" & pTo & "' " & vbCrLf
    SQL = SQL & "   And a.STAT          IN ('0','1','9')                        " & vbCrLf  '응급 flag      : 0.일반,   1.응급,  9.외부검사
    If pSave = False Then
        SQL = SQL & "   And a.JSTATUS       IN ('0','1','2')                    " & vbCrLf  '진행상태 flag  : 0.병동Barcode출력,    1.접수,  2.보류,  3.결과,  4.SignOut
        '검사결과가 없는것만
        SQL = SQL & "   AND (b.RESULT = '' OR b.RESULT IS NULL)                 " & vbCrLf  'Varchar2(500)
    End If
...
On Error GoTo ErrHandle
    ...
    'OcmInf..OcmComStt : 접수상태
    '첫번째 :   W(대기), H(보류), T(완료),   P(진료완료), C(취소),
    '두번째 :   N(정상), R(예약), C(컨설트), T(트랜스퍼), i(입원전환), N(간호접수대기), S(컨설트, 트랜스퍼 보냄), F(컨설트완료)

    SQL = ""
    SQL = SQL & "Select DISTINCT "
    SQL = SQL & "      W.OcmAcpDtm          AS HOSPDATE                             " & vbCrLf  '접수일시,입원일시 : CHAR(12) 년월일시 까지 저장됨
    SQL = SQL & "    , ''                   AS INOUT                                " & vbCrLf
    SQL = SQL & "    , ''                   AS DEPT                                 " & vbCrLf
    SQL = SQL & "    , R.ResOcmNum          AS BARCODE                              " & vbCrLf  '내원번호 : CHAR(10) 내원시 발본되는 일련번호   PK
    SQL = SQL & "    , ''                   AS PID                                  " & vbCrLf
    SQL = SQL & "    , W.OcmChtNum          AS CHARTNO                              " & vbCrLf
    SQL = SQL & "    , ''                   AS PJUMIN                               " & vbCrLf  '차트번호 : CHAR(14) 최대 14자리…앞에 공백으로 반드시
    ...
    SQL = SQL & " Where W.OcmNum    = R.ResOcmNum                                   " & vbCrLf  '내원번호
    SQL = SQL & "   And W.OcmNum    = O.OdrOcmNum                                   " & vbCrLf
    SQL = SQL & "   And W.OcmChtNum = P.PbsChtNum                                   " & vbCrLf  '차트번호
   ''SQL = SQL & "   And W.OcmDepCod = D.DepCod                                      " & vbCrLf  '진료과   : DepMst(DepCod)
    SQL = SQL & "   And O.OdrSeq    = R.ResOdrSeq                                   " & vbCrLf
    SQL = SQL & "   And R.ResLabCod = E.LabCod                                      " & vbCrLf
    SQL = SQL & "   And O.Odrdelflg = 'N'                                           " & vbCrLf
    SQL = SQL & "   And W.OcmComStt Not In ('CN','CR','VC')                         " & vbCrLf  '접수상태 : 접수취소 (CN), 예약취소 (CR), VC : 입원취소
...
Function GetSampleSubITEM(ByVal pBarcode As String, ByVal pTestCd As String, Optional ByVal pRegDate As String, Optional ByVal pChartNo As String) As String
    ...
    Select Case gEMR
        Case "JWINFO"
                ...
                If pChartNo <> "" Then
                    SQL = SQL & "   And RECEIPTNO   = '" & pChartNo & "'    " & vbCrLf
                End If
...
On Error GoTo DBErr
    ...
    strBarcode = Trim(GetText(SPD, asRow, colBARCODE))
    strPatID = Trim(GetText(SPD, asRow, colPID))
    strChartNo = Trim(GetText(SPD, asRow, colCHARTNO))  '접수번호(Order별 부여:Number(8)) nnnnnnnn (PK)
    ...
    SQL = SQL & "     , a.SEX                                   AS SEX          " & vbCrLf
    SQL = SQL & "     , a.AGE                                   AS AGE          " & vbCrLf
    SQL = SQL & "     , a.ORDERCODE                             AS ORDCODE      " & vbCrLf  'Order 코드(CHAR(8))
    SQL = SQL & "     , b.LABCODE                               AS ITEM         " & vbCrLf  '검사코드(CHAR(10))
    ...
    SQL = SQL & "   And a.RECEIPTDATE   = b.RECEIPTDATE                         " & vbCrLf  '의미없음
    SQL = SQL & "   And a.SPECIMENNUM   = b.SPECIMENNUM                         " & vbCrLf  '의미없음
    SQL = SQL & "   And a.RECEIPTNO     = '" & strChartNo & "'                  " & vbCrLf
