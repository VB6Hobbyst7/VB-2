   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboChk 
            ...
            EndProperty
            Height          =   300
            ItemData        =   "frmInterface.frx":C201
            Left            =   2340
            List            =   "frmInterface.frx":C20B
            TabIndex        =   45
            Top             =   -90
            Visible         =   0   'False
            Width           =   765
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C21B
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":CD21
         End
...
'
    strBuffer = ""
    strBuffer = strBuffer & "1H|\^&|||Analyzer|||||||||20111022124736" & vbCr
    strBuffer = strBuffer & "BB" & vbCr
    strBuffer = strBuffer & "2Q|1|*512836*||||||||||N" & vbCr
    strBuffer = strBuffer & "6F" & vbCr
    strBuffer = strBuffer & "3L|1" & vbCr
    strBuffer = strBuffer & "3C" & vbCr
    strBuffer = strBuffer & "" & vbCrLf
        
    strBuffer = ""
    strBuffer = strBuffer & "1H|\^&|||Analyzer|||||||||20111022130953" & vbCr
    strBuffer = strBuffer & "B9" & vbCr
    strBuffer = strBuffer & "2P|1|72225|||홍종은^||18000101|M||||||" & vbCr
    strBuffer = strBuffer & "3C" & vbCr
    strBuffer = strBuffer & "3C|1|I||G" & vbCr
    strBuffer = strBuffer & "37" & vbCr
    strBuffer = strBuffer & "4O|1|*512827*||^^^01||||||||||||01" & vbCr
    strBuffer = strBuffer & "ED" & vbCr
    strBuffer = strBuffer & "5C|1|I||G" & vbCr
    strBuffer = strBuffer & "39" & vbCr
    strBuffer = strBuffer & "6R|1|^^^01|4.0|g/dl||N||||||20111022130953" & vbCr
    strBuffer = strBuffer & "18" & vbCr
    strBuffer = strBuffer & "7L|1" & vbCr
    strBuffer = strBuffer & "40" & vbCr
    strBuffer = strBuffer & "" & vbCrLf
    
    strBuffer = ":n     1   1                              8  1    51   2   129   3   205   5    49   8    89  13   7.7  14   4.7  15   1.2 "
...
'1H|\^&|||Host|||||||||20111022125934
'14
'
'2P|1|290797|||박선환^^|||F||||||
'46
'
'3O|1|*512836*||^^^09\^^^10\^^^05\^^^06\^^^61\^^^63\^^^62\^^^08||||||||||||01
'FA
'
'2011-10-22 12:59:35
'4L|1
'3D
    ...
    If gASTMFormat = "1" Then
        Select Case intSndPhase
            Case 1  '## Header
                strOutput = intFrameNo & "H|\^&|||Host|||||||||" & Format(Now, "yyyymmddhhmmss") & vbCr & ETX
                intSndPhase = 2
                intFrameNo = intFrameNo + 1
            Case 2  '## Patient
                strOutput = intFrameNo & "P|1|" & mOrder.RackNo & "|||" & mOrder.TubePos & "^^|||F||||||" & vbCr & ETX
                intSndPhase = 4
                intFrameNo = intFrameNo + 1
            ...
            Case 4  '## Order
                If mOrder.NoOrder = True Then
                    ...
                    '## 접수정보가 없을경우
                    strOutput = intFrameNo & "O|1|" & mOrder.BarNo & "||R||||||C||||||||||||||Q" & vbCr & ETX
                    intSndPhase = 5
...
Private Sub GetOrder(ByVal pBarNo As String)
   ...
   ' MsgBox "5"
    ...
    'gRow = GetSampleInfoW(intRow)
    gRow = GetSampleInfoW(intRow, pBarNo)
    
    '-- 바코드번호에 존재하는 검사코드 가져오기(인수 : 장비코드,바코드번호)
    gOrderExam = GetOrderTestCode(gEquip, pBarNo)
    
    '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
    strItems = GetGetEquipExamCode_IMOLA(gEquip, pBarNo, intRow)
...
Private Sub SetPatInfoQry(ByVal pBarNo As String)
    Dim i           As Integer
    Dim intRow      As Long
    Dim strItems    As String
    Dim tmpBarNo    As String
...
On Error Resume Next
    ...
    strRcvBuf = strBuffer
    strType = Mid$(strRcvBuf, 1, 1)         ' get Frame of RcvBuffer.
    ...
    Select Case strType
        ...
        Case ";"    'SPE  전송
            strFunction = Mid(strRcvBuf, 2, 39)
            ...
            For i = 1 To vasID.DataRowCnt
                If Trim(GetText(vasID, i, colPos)) = Val(strTubePos) Then
                    strBarno = Trim(GetText(vasID, i, colBarcode))
                    gRow = i
                    'strDate = Trim(GetText(vasID, i, colOrdDate))
                    Exit For
                End If
            Next
            ...
            '-- 환자정보 가져오기(검사코드[gOrderExam] 포함)
            Call GetOrder(strBarno)
            
            '-- 바코드번호에 존재하는 검사코드 가져오기(인수 : 장비코드,바코드번호)
            'gOrderExam = GetOrderExamCode(gEquip, strSeq, strDate)

            strSendData = String$(37, "0")
        ...
        Case "1", "2", "3", "4", "5", "6", "7", "8", "9"    'FR1 to FR9(검사항목 25개 이상일 경우 처리)
            ...
            If strFunc <> "@" And strFunc <> "M" Then
                ...
                For i = 0 To UBound(varResponse) - 1
                    strExamCodeR = strExamCodeR & "'" & mGetP(varResponse(i), 6, vbTab) & "',"
                Next
    
                If strExamCodeR <> "" Then
                    strExamCodeR = Mid(strExamCodeR, 1, Len(strExamCodeR) - 1)
                End If
                
            
                For i = 44 To Len(strRcvBuf) Step 10
                    strIntBase = Trim(Mid(strRcvBuf, i, 5))
                    strResult = Trim(Mid(strRcvBuf, i + 5, 5))
                    ...
                    If strResult <> "" Then
                        ...
                        SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' "
                        SQL = SQL & "   AND EQUIPCODE = '" & strIntBase & "' "
                        SQL = SQL & "   AND EXAMCODE in (" & strExamCodeR & ") "
                        Res = GetDBSelectColumn(gLocal, SQL)
