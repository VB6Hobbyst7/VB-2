    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" And strSeqS <> "" Then
                ...
                '-- 상태변경1
                      SQL = "UPDATE scrprexh SET"
                SQL = SQL & "       smp_stus = decode(smp_stus, '0','1',smp_stus)"
                SQL = SQL & " WHERE hos_org_no = '" & gGINUS_Parm.HCD & "'" & vbCr
                SQL = SQL & "   AND smp_no     = '" & lsID & "'" & vbCr
                SQL = SQL & "   AND cd         = '" & strEqpCd & "'" & vbCr
                ...
                '-- 상태변경2
                      SQL = "UPDATE mosxpslh SET"
                SQL = SQL & "       prcp_stus_cd = '4'"
                SQL = SQL & " WHERE hos_org_no = '" & gGINUS_Parm.HCD & "'" & vbCr
                SQL = SQL & "   AND smp_no     = '" & lsID & "'" & vbCr
                SQL = SQL & "   AND prcp_stus_cd <> '6'"
                SQL = SQL & "   AND prcp_typ_cd IN ('O','C')"
...
Function GetOrderTestCode(argEquipCode As String, argPID As String) As String
    Dim i           As Integer
    Dim sExamCode   As String
    Dim strExamCode As String
    Dim sExamCd     As String
...
'    For i = 0 To UBound(varResponse) - 1
    Do Until RS.EOF
        ...
        GetOrderTestCode = GetOrderTestCode & "'" & Trim(RS.Fields("cd")) & "',"
        RS.MoveNext
    Loop
    
    
    If GetOrderTestCode <> "" Then
        GetOrderTestCode = Mid(GetOrderTestCode, 1, Len(GetOrderTestCode) - 1)
    End If
    
    ' SetRawData "[GetOrderTestCode]" & GetOrderTestCode
...
'-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기
Function GetGetEquipExamCode_IMOLA(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim i As Integer
    Dim strExamCode As String
    Dim sBarcode     As String
    
    GetGetEquipExamCode_IMOLA = ""
    ...
    GetGetEquipExamCode_IMOLA = Mid(strExamCode, 2)
