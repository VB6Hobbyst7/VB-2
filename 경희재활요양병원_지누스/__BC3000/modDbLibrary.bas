...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Function GetOrderExamCode_GINUSDLL(argEquipCode As String, argPID As String) As String
    Dim i           As Integer
    Dim sExamCode   As String
    Dim strExamCode As String
    Dim sExamCd     As String
    ...
    For i = 0 To UBound(varResponse) - 1
        If mGetP(varResponse(i), 6, vbTab) = argEquipCode Then
            GetOrderExamCode_GINUSDLL = GetOrderExamCode_GINUSDLL & "'" & mGetP(varResponse(i), 6, vbTab) & "',"
            Exit For
        End If
    Next
    
    If GetOrderExamCode_GINUSDLL <> "" Then
        GetOrderExamCode_GINUSDLL = Mid(GetOrderExamCode_GINUSDLL, 1, Len(GetOrderExamCode_GINUSDLL) - 1)
    End If
