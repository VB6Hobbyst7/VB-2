...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        strSVC = Trim(GetText(.vasID, argSpcRow, colRack))
        strOrdDate = Trim(GetText(.vasID, argSpcRow, colOrdDate))
...
'        cn_Ser.BeginTrans
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If strSVC = "검진" Then
                If sResult <> "" Then
                    '-- 판정 참고치 가져오기
                          SQL = "Select Res1_Low, Res1_High "
                    SQL = SQL & "  From mresult001 "
                    SQL = SQL & " Where res1_date  = '" & strOrdDate & "'"
                    SQL = SQL & "   AND res1_gumno = '" & lsID & "'"
                    SQL = SQL & "   AND res1_gum_code = '" & strEqpCd & "'"
                    Set RS_SVR = cn_Ser.Execute(SQL)
                    Do Until RS_SVR.EOF
                        strRefVal = ""
                        If sResult < Trim(RS_SVR.Fields("Res1_Low")) & "" Then
                            strRefVal = "L"
                        ElseIf sResult > Trim(RS_SVR.Fields("Res1_High")) & "" Then
                            strRefVal = "H"
                        End If
                    ...
                    Set RS_SVR = Nothing
                          ...
                          SQL = "Update mresult001 Set "
                    SQL = SQL & "res1_result   = '" & sResult & "',"
                    SQL = SQL & "res1_rfm_name = '" & sResult & "',"
                    SQL = SQL & "res1_Pan      = '" & strRefVal & "'"
                    SQL = SQL & " where res1_date   = '" & strOrdDate & "'"
                    SQL = SQL & " and res1_gumno    = '" & lsID & "'"
                    SQL = SQL & " and res1_gum_code = '" & strEqpCd & "'"
            ...
            Else
                If sResult <> "" Then
                    '-- 결과용 SEQ 찾아오기
                    strSeqS = GetOrderExamCode_TwoServer(strEqpCd, lsID, strSVC, strOrdDate)
                    If strSeqS <> "" Then
                       '-- 검사결과 저장하기
...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Function GetOrderExamCode_XN1000(argEquipCode As String, argPID As String, argSVC As String, Optional argDate As String) As String
    Dim i           As Integer
    Dim sExamCode   As String
    Dim strExamCode As String
    Dim sExamCd     As String
    ...
    If argSVC = "검진" Then
        ...
        SQL = SQL & "  FROM mresult001"
        SQL = SQL & " WHERE res1_date  = '" & argDate & "'"
        SQL = SQL & "   AND res1_gumno = '" & argPID & "'"
        SQL = SQL & "   AND res1_gum_code in (" & gAllExam & ")" & vbNewLine
        SQL = SQL & "   AND (res1_result is null or res1_result = '')" & vbNewLine
        
        Set RS_SVR = cn_Ser.Execute(SQL)
        Do Until RS_SVR.EOF
            GetOrderExamCode_XN1000 = GetOrderExamCode_XN1000 & "'" & Trim(RS_SVR.Fields(0)) & "',"
            RS_SVR.MoveNext
        Loop
    ...
    Else
        ...
        strRequest = strRequest & "hos_org_no" + vbTab + gGINUS_Parm.HCD + vbTab
        strRequest = strRequest & "smp_no" + vbTab + argPID + vbTab
        strRequest = strRequest & "mach_cd" + vbTab + "01" + vbTab + vbCr
        ...
        For i = 0 To UBound(varResponse) - 1
            If mGetP(varResponse(i), 6, vbTab) = argEquipCode Then
                GetOrderExamCode_XN1000 = GetOrderExamCode_XN1000 & "'" & mGetP(varResponse(i), 6, vbTab) & ","
                Exit For
            End If
        Next
    End If
        
    If GetOrderExamCode_XN1000 <> "" Then
        GetOrderExamCode_XN1000 = Mid(GetOrderExamCode_XN1000, 1, Len(GetOrderExamCode_XN1000) - 1)
    End If
    
    SetRawData "[GetOrderExamCode_XN1000]" & GetOrderExamCode_XN1000
    
End Function

'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Function GetOrderExamCode_TwoServer(argEquipCode As String, argPID As String, argSVC As String, Optional argDate As String) As String
    Dim i           As Integer
    Dim sExamCode   As String
    Dim strExamCode As String
    Dim sExamCd     As String
    ...
    If argSVC = "검진" Then
        ...
        Set RS_SVR = cn_Ser.Execute(SQL)
        Do Until RS_SVR.EOF
            GetOrderExamCode_TwoServer = GetOrderExamCode_TwoServer & "'" & Trim(RS_SVR.Fields(0)) & "',"
            RS_SVR.MoveNext
        Loop
        
        If GetOrderExamCode_TwoServer <> "" Then
            GetOrderExamCode_TwoServer = Mid(GetOrderExamCode_TwoServer, 1, Len(GetOrderExamCode_TwoServer) - 1)
        End If
    Else
        '-- 검사항목 가져오기
                     strRequest = "jobs" + vbTab + "Q" + vbTab
        ...
        For i = 0 To UBound(varResponse) - 1
            If mGetP(varResponse(i), 6, vbTab) = argEquipCode Then
                GetOrderExamCode_TwoServer = GetOrderExamCode_TwoServer & mGetP(varResponse(i), 3, vbTab) & "|" & mGetP(varResponse(i), 4, vbTab) & "|" & mGetP(varResponse(i), 5, vbTab)
                Exit For
            End If
        Next
    End If
