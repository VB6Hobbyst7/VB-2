...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    Dim sVzvMCmnt  As String
    Dim sVzvGCmnt  As String
    Dim sMycoMCmnt  As String
    Dim sMycoGCmnt  As String
    Dim sPYGCmnt    As String
    
    sPYGCmnt = ": 소견없음"
    
    sVzvMCmnt = ""
    sVzvMCmnt = sVzvMCmnt & "*** VZV (Varicella Zoster Virus) Ab%0D"
    sVzvMCmnt = sVzvMCmnt & "Primary infection 혹은 reactivation 경우 VZV IgM 양성이며,%0D"
    sVzvMCmnt = sVzvMCmnt & "VZV IgG 양성은 과거감염여부 및 면역상태 평가에 유용합니다.%0D"
    sVzvMCmnt = sVzvMCmnt & "위 검사결과는 진단검사의학과 전문의가 판독, 검증하였습니다.%0D"
    sVzvMCmnt = sVzvMCmnt & "* 판독의: 문희원"
    
    sVzvGCmnt = sVzvMCmnt
    
    sMycoMCmnt = ""
    sMycoMCmnt = sMycoMCmnt & "*** Mycoplasma pneumoniae Ab IgG , IgM%0D"
    sMycoMCmnt = sMycoMCmnt & "IgG 가 4배 이상 증가하거나 IgM 이 양성일 경우 급성 감염을%0D"
    sMycoMCmnt = sMycoMCmnt & "의심할 수 있으나 clinical correlation 이 필요 합니다.%0D"
    sMycoMCmnt = sMycoMCmnt & "* 판독의 : 문희원"
    
    sMycoGCmnt = sMycoMCmnt
...
On Error Resume Next
    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If InStr(UCase(sResult), "POSITIVE") > 0 Then
                Select Case strEqpCd
                    Case "VZV-M":   sComment = sVzvMCmnt
                    Case "VZV-G":   sComment = sVzvGCmnt
                    Case "MYCO-M":  sComment = sMycoMCmnt
                    Case "MYCO-G":  sComment = sMycoGCmnt
                    Case "PY-G":    sComment = sPYGCmnt
                    Case Else:      sComment = ""
                End Select
            End If
            
            sParam = ""
            sParam = sParam & "http://" & gKNUH.IP & gKNUH.RSTURL & "&"
