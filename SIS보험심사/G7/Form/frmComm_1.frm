...
Private Sub ComReceive(ByRef RecData() As Byte)
    ...
    For intIdx = 1 To Len(strRec)
        ...
        If Mid$(strRec, intIdx, 1) = Chr(13) Then
        'If Len(f_strBuffer) = 82 Then
            sRstText = f_strBuffer
            'f_strBuffer = ""
            '------------------------------<<< fGseven() 배열 Clear 한다.         >>>----------
            For Loop_Count = 1 To 100: fGseven(Loop_Count) = "": Next Loop_Count
            '------------------------------<<< fGseven() 배열에 구분하여 넣는다.  >>>----------
                                
            If Mid(sRstText, 1, 2) <> "99" Then
                fGseven(1) = Mid(sRstText, 3, 8)
               ...
               ' fGseven(12) = "M0306090021"
                Max_Arary_Cnt = lvwWorkList.ListItems.Count
            Else
                GoTo Rst
            End If
                            
            Set itemX = lvwWorkList.FindItem(Trim(fGseven(12)), lvwTag, , lvwWhole)
            
            If Not itemX Is Nothing Then
                If Not itemX Is Nothing Then
                    With itemX
                        For ii = 1 To lvwWorkList.ColumnHeaders.Count - 1
                            itemX.SubItems(ii) = Trim(fGseven(7))
                            ...
                            With Result
                                .MSG_SID = fGseven(12)
                                .MSG_TYPE = "G"
                                .MSG_RESULT = Trim(fGseven(7))
                            End With
                            Set Result = Nothing
                            
                            Set Result1 = New clsResult
                            With Result1
                                .Rst_Sid = Trim(fGseven(12))
                                .Rst_Eid = INS_CODE
                                .Rst_Type = "G"
                                .Rst_Test = fCellItem(ii)
                                .Rst_Values = Trim(fGseven(7))
                                .Rst_Tag = Trim(fGseven(12))
                                .Rst_Error = ""
                            End With
...
'            Call .Set_EqpResultsql(Result.Rst_Tag, Result.Rst_Values, Result.Rst_Error) '서버에 등록
            ...
            Call .Set_EqpResultsql(Result.Rst_Sid, Result.Rst_Tag, Result.Rst_Values, Result.Rst_Eid, Result.Rst_Error)   '서버에 등록
            '결과 완료테이블에 추가
            ...
            End If
            '결과값 등록
            itemX.SubItems(lvwComplete.ColumnHeaders(COL_KEY & Result.Rst_Tag).SubItemIndex) = Result.Rst_Values
            Set itemS = itemX.ListSubItems(lvwComplete.ColumnHeaders(COL_KEY & Result.Rst_Tag).SubItemIndex)
...
'            Set itemS = itemX.ListSubItems(lvwComplete.ColumnHeaders(COL_KEY & Result.Rst_Test).SubItemIndex)
        ...
        Else '검체번호가 없는것은 미등록 테이블에
            ...
            If itemX Is Nothing Then
                ...
                End With
            End If
            itemX.SubItems(lvwError.ColumnHeaders(COL_KEY & Result.Rst_Tag).SubItemIndex) = Result.Rst_Values
            Set itemS = itemX.ListSubItems(lvwError.ColumnHeaders(COL_KEY & Result.Rst_Tag).SubItemIndex)
...
Private Sub txtWorkSpcno_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        ...
        With objWorkList
            '.SetAdoCn AdoCn_Jet
            Call .SetAdoCn(AdoCn_SQL)
            Set mAdoRs = .Get_WorkList(UCase(Trim(txtWorkSpcno.Text)))
        End With
