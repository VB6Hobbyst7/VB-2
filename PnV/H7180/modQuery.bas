...
Function SaveTransData_PNV(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
...
On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            ...
            sHeader = ""
            sHeader = sHeader & "X-LAB-SECURITY: AGASGBggFASVfg42ASFV5255GGSAVNJJKPQOWDKVM4fiFHDoWFmqSGHYASDksapqmdm2DASFASfyomsFASGAS==" & vbCrLf
            ...
            sBody = "id=" & strPatID & "&LabResultList=["
            sSTV = ""
            ...
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    '-- 결과저장
                    sSTV = sSTV & "{"
                    sSTV = sSTV & """SerialNo" & """:""" & strTestCd & """, "
                    sSTV = sSTV & """Value" & """:""" & sResult & """, "
                    sSTV = sSTV & """Type" & """:" & sResult & ", "
                    sSTV = sSTV & """Index" & """:" & "1" & ", "
        ...
        If sSTV <> "" Then
            sSTV = Mid(sSTV, 1, Len(sSTV) - 1)
        
            sBody = sBody & sSTV & "]"
            
            'SetRawData "[결과저장]" & sBody

            sRcvData = OpenURLWithIE2(sURL, sHeader, sBody, frmInterface.Inet1)
            
            If InStr(sRcvData, "err_msg") > 0 Then
                SaveTransData_PNV = -1
            Else
                SaveTransData_PNV = 1
            End If
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_PNV" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
Public Sub GetWorkList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pFromNo As String = "", Optional ByVal pToNo As String = "", Optional ByVal pSave As Boolean = False)
    ...
    Select Case gEMR
        Case "PNV"
                Call GetWorkList_PNV(pFrom, pTo, SPD)
...
Public Sub GetWorkList_PNV(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
                ...
                With SPD
                   ...
                   ' If strAllergyID > 10 Then
                        ...
                        If blnSame = False Then
                            ...
                            SetText SPD, Trim(mGetP(varPetData(8), 2, ":")), .MaxRows, colHOSPDATE
                            SetText SPD, Trim(mGetP(varPetData(6), 2, ":")), .MaxRows, colCHARTNO
                            SetText SPD, Trim(mGetP(varPetData(0), 2, ":")), .MaxRows, colBARCODE
                            SetText SPD, Trim(mGetP(varPetData(0), 2, ":")), .MaxRows, colPID
                            SetText SPD, Trim(mGetP(varPetData(7), 2, ":")), .MaxRows, colPNAME
                            SetText SPD, Trim(mGetP(varPetData(9), 2, ":")), .MaxRows, colSPECIMEN 'colBREED       '-- 품종
                            SetText SPD, Trim(mGetP(varPetData(2), 2, ":")), .MaxRows, colPJUMIN 'colDOB         '-- 생년월일
                            SetText SPD, strAssayNm, .MaxRows, colINOUT 'colASSAYNM                             '-- 검사구분(명)

                            Select Case Trim(mGetP(varPetData(1), 2, ":"))
                                Case "1": SetText SPD, "F", .MaxRows, colPSEX
                                Case "2": SetText SPD, "FS", .MaxRows, colPSEX
                                Case "3": SetText SPD, "M", .MaxRows, colPSEX
                                Case "4": SetText SPD, "MN", .MaxRows, colPSEX
                                Case "5": SetText SPD, "모름", .MaxRows, colPSEX

                            End Select

                            '-- 품종
                            Select Case Trim(mGetP(varPetData(4), 2, ":")) & ""
                                Case "1":   SetText SPD, "개", .MaxRows, colPAGE
                                Case "2":   SetText SPD, "고양이", .MaxRows, colPAGE
                            End Select
                        End If
                        blnSame = False
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_PNV" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "PNV"
            SaveTransData = SaveTransData_PNV(argSpcRow, SPD)
