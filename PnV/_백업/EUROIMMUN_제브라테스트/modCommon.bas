...
'    End With
    ...
    ' MultiByteToWideChar( CP_UTF8 , 0, utf8, -1, strWide2, 1024);.
...
'## 파일오픈
                    ...
                    STM.WriteText Converted, adWriteChar

                    STM.SaveToFile App.Path & "\Log\label.pdf", adSaveCreateNotExist
                    STM.Close
                    Set STM = Nothing
                    
    
    'D:\프로젝트\VB\PnV\EUROIMMUN\Log
    FilNum = FreeFile
    
    Open App.Path & "\Log\label.pdf" For Append As FilNum
    Print #FilNum, Converted
    Close FilNum
...
'argSQL의 내용을 파일로 저장
    ...
    Open App.Path & "\Log\" & sFileName & ".pdf" For Append As FilNum
