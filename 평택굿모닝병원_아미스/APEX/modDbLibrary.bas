...
On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        ClearSpread .vasTemp
              ...
              SQL = "SELECT EQUIPCODE,EXAMCODE,RESULT,EQUIPRESULT,REFFLAG,PANICVALUE,DELTAVALUE,PSEX,SEQNO,PAGE,PID,DISKNO,POSNO,EXAMSUBCODE,REFVALUE " & vbCrLf
...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            strRefVal = Trim(GetText(.vasTemp, iRow, 15))    '판정값
            ...
            If IsNumeric(strRefVal) Then
                sResult = sResult & " (" & strRefVal & ")"
            End If
...
'                SQL = SQL & ""
                ...
                '-- 결과저장
                      SQL = " Update resultofnum Set" & vbCr
                SQL = SQL & " resultindate = to_char(sysdate,'yyyymmdd')" & vbCr
                SQL = SQL & " , resultintime = to_char(sysdate,'HH24MI')" & vbCr
                SQL = SQL & " , resultinid = '" & gUserID & "'" & vbCr
                ...
                '-- 일반결과 판정
                If strEqpCd <> "XA01" Then
                    sOldResult = Replace(sOldResult, "<", "")
                    If sOldResult > gAssayNM.HIGH Then
                        SQL = SQL & " , highlowflag = 'H' " & vbCr
                    End If
                End If
                
                '-- IGE결과 판정
                If strEqpCd = "XA01" Then
                    sOldResult = Trim(Replace(sOldResult, "≤", ""))
                    sOldResult = Trim(Replace(sOldResult, ">", ""))
                    If sOldResult > gAssayNM.IGEHIGH Then
                        SQL = SQL & " , highlowflag = 'H' " & vbCr
                    End If
                End If
                
                SQL = SQL & " , ANALYZERCODE= '" & gMachCD & "'" 'APEX = 41
                
                SQL = SQL & " where spcmno = '" & lsID & "'" & vbCr
                SQL = SQL & " and resultitemcode = '" & strEqpCd & "'" & vbCr
                SQL = SQL & " and resultflag < '2' " & vbCr
