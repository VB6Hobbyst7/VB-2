...
Public gAllTestCd_Real   As String   '-- 인터페이스에 등록된 전체검사코드
...
'TST_STAT:          상태\
'ACPT_DTETM:        접수일시
'DOCTORNOTE_YN:     DOCTORNOTE_YN
'SND_ARVL_NO_CNTE:  SND_ARVL_NO_CNTE
'DEXM_YENU:         연번호
    ...
    DOCTORNOTE_YN()     As String
    SND_ARVL_NO_CNTE()  As String
    DEXM_YENU()         As String
End Type

Public XmlWork  As XML_WorkList
...
Public Sub GetSetup()
    ...
    strSetup = "":    strSetUp1 = ""
    Call GetPrivateProfileString("HOSP", "IMPORTPATH", "", strSetup, 100, App.PATH & "\INI\" & gMACH & ".ini")
    strSetUp1 = Trim(strSetup)
    strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
    gHOSP.IMPPATH = Trim(strSetUp1)
    
    strSetup = "":    strSetUp1 = ""
    Call GetPrivateProfileString("HOSP", "EXPORTPATH", "", strSetup, 100, App.PATH & "\INI\" & gMACH & ".ini")
    strSetUp1 = Trim(strSetup)
    strSetUp1 = Mid(strSetUp1, 1, Len(strSetUp1) - 1)
    gHOSP.EXPPATH = Trim(strSetUp1)
