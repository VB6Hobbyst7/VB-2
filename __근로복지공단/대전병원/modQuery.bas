...
Public Function GetEquipExamCode_ADVIA2120(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim strSendCH       As String
    Dim iAffected       As Integer
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            strSendCH = Trim(AdoRs_Local.Fields("SENDCHANNEL").Value & "")
            If strSendCH <> "" Then
                strExamCode = strExamCode & Right("000" & Trim(strSendCH), 3)
...
Function SaveTransData_KCWH(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
            ...
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    With AdoCmd
                        ...
                        Set Prm13 = .CreateParameter("in_sUserIp", adVarChar, adParamInput, 4000, frmInterface.wSck.LocalIP)
                        .Parameters.Append Prm13
                        strSavePara = strSavePara & "in_sUserIp:" & frmInterface.wSck.LocalIP & vbNewLine
                        ...
                        Call SetSQLData("결과저장", strSavePara, "A")
                        .Execute
...
ErrHandle:
    ...
    strErrMSG = ""
    strErrMSG = strErrMSG & "위    치 : " & gHOSP.MACHNM & "_SaveTransData_KCWH" & vbNewLine & vbNewLine
    strErrMSG = strErrMSG & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMSG = strErrMSG & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMSG
    frmErrMsg.Show
...
Public Sub GetWorkList_UBCARE(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As fpSpread)
    Dim RS          As ADODB.Recordset
...
Public Sub GetWorkList_KCWH(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, Optional ByVal pSave As Boolean = False)
    Dim RS          As ADODB.Recordset
    Dim Prm1        As New ADODB.Parameter
    Dim Prm2        As New ADODB.Parameter
    Dim Prm3        As New ADODB.Parameter
        ...
        Do Until RS.EOF
            With SPD
                For i = 1 To SPD.DataRowCnt
                    ...
                    If Trim(RS("ORDDT")) = strHospDate And Trim(RS("BCNO")) = strBarcode And _
                        Trim(RS("PT_NO")) = strPID And Trim(RS("LABNO")) = strChartNo Then
                        blnSame = True
                        Exit For
                    End If
                Next
...
ErrHandle:
    ...
    strErrMSG = ""
    strErrMSG = strErrMSG & "위    치 : " & gHOSP.MACHNM & "_GetWorkList_KCWH" & vbNewLine & vbNewLine
    strErrMSG = strErrMSG & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMSG = strErrMSG & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMSG
...
Function SaveTransData(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Select Case gEMR
        Case "KCWH"
            SaveTransData = SaveTransData_KCWH(argSpcRow, SPD)
...
Public Sub SetUpdateStatus(ByVal SPD As Object, ByVal pRow As Integer, ByVal pRes As Integer)
    ...
    If pRes = -1 Then
        ...
        Call SetBackColor(SPD, pRow, pRow, colEXAMDATE, colSTATE, 234, 247, 0) '255,255,0 : Yellow
    ...
    Else
        Call SetBackColor(SPD, pRow, pRow, colEXAMDATE, colSTATE, 202, 255, 112)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_KCWH(ByVal asRow As Long, ByVal SPD As Object) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                '-- 화면에 표시
                For intCol = colSTATE + 1 To .MaxCols
                    If GetTestNm(Trim(RS.Fields("TCLSCD"))) = gArrEQP(intCol - colSTATE, 6) Then
                        Call SetSPDOrder(SPD, SPD.MaxRows, SPD.MaxRows, intCol, intCol)
                        '-- 처방코드, 서브코드
                        'gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("ORDCODE")) & ""
...
DBErr:
    ...
    strErrMSG = ""
    strErrMSG = strErrMSG & "위    치 : " & gHOSP.MACHNM & "_GetSampleInfo_KCWH" & vbNewLine & vbNewLine
    strErrMSG = strErrMSG & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMSG = strErrMSG & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMSG
    frmErrMsg.Show
