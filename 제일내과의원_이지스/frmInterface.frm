...
Begin VB.Form frmInterface 
   ...
   Begin FPSpreadADO.fpSpread spdOrder 
      ...
      ShadowColor     =   14737632
      ShadowDark      =   12632256
      SpreadDesigner  =   "frmInterface.frx":BB72
      UnitType        =   0
      VisibleCols     =   3
      VisibleRows     =   10
      ScrollBarTrack  =   3
   ...
   Begin FPSpreadADO.fpSpread spdResult 
      ...
      ShadowColor     =   14737632
      ShadowDark      =   12632256
      SpreadDesigner  =   "frmInterface.frx":C806
      VisibleCols     =   3
      VisibleRows     =   10
      CellNoteIndicatorColor=   16576
...
Private Sub ReceiveProcess()
    ...
    ElseIf gComm.COMTYPE = "2" Then
        Select Case UCase(gHOSP.MACHNM)
            Case "BA200":           Call TCPRcvData_BA200
        ...
        For intCnt = 0 To UBound(strRecvData)
            ...
            Select Case strType
                Case "MSH"
                    ...
                    mOrder.MsgTime = mGetP(strRcvBuf, 7, "|")
                    mOrder.MsgType = mGetP(strRcvBuf, 9, "|")
...
'
                        ...
                        strSndBuffer = strSndBuffer & SB & "MSH|^~\&|Modulab|Systelab|BA400|Biosystems|" & mOrder.MsgTime & "||RSP^K11^RSP_K11|" & mOrder.MsgSeq & "|P|2.5.1|||ER|NE||UNICODE UTF-8|||LAB-27^IHE" & vbCr
                    ...
                    ElseIf mOrder.MsgType = "OUL^R22^OUL_R22" Then
                        ...
                        ':MSH|^~\&|Modulab|Systelab|BA200|Biosystems|20210617103231||ACK^R22^ACK|d5948c30-0f95-444e-ae33-7f0a8a579858|P|2.5.1|||ER|NE||UNICODE UTF-8|||LAB-28^IHEMSA|AA|d5948c30-0f95-444e-ae33-7f0a8a579858
                    
                    '--?? 오더 잘받았다는 응답 (오더건수만큼 온다) >> ORL^O34^ORL_O34
    ...
    With frmInterface
        For i = 1 To .spdOrder.DataRowCnt
            If GetText(spdOrder, i, colCHECKBOX) = "1" And GetText(spdOrder, i, colSTATE) = "" Then
                ...
                mOrder.Jumin = Trim(GetText(.spdOrder, i, colPJUMIN))
                ...
                Else
                    ...
                    With mOrder
                        strSend = ""
                        strSend = strSend & SB & "MSH|^~\&|Modulab|Systelab|BA400|Biosystems|" & strDtTm & "||OML^O33^OML_O33|69F2746D24014F21AD7139756F64CAD8|P|2.5.1|||ER|AL||UNICODE UTF-8|||LAB-28^IHE" & vbCr
                        strSend = strSend & "PID|||" & .PID & "(" & .PNAME & "-" & .BarNo & ")" & "||" & .PNAME & "||" & mOrder.Jumin & "|" & mPatient.SEX & vbCr
                        strSend = strSend & "SPM|1|" & .BarNo & "-" & .PID & "||SER|||||||P||||||" & strDtTm & "|" & strDtTm & vbCr
                        strSend = strSend & "SAC|||" & .PID & vbCr
                        varItems = Split(strItems, "|")
                        For j = 0 To UBound(varItems)
                            If varItems(j) <> "" Then
                                strSend = strSend & "ORC|NW||||||||" & strDtTm & vbCr
                                strSend = strSend & "TQ1|||||||||R" & vbCr
                                strSend = strSend & "OBR||""""||" & varItems(j) & "^" & varItems(j) & "^BA400" & vbCr
                            End If
                        Next
                    End With
                    strSend = strSend & EB & vbCr
...
Private Sub wSCK_DataArrival(ByVal bytesTotal As Long)
    ...
    If InStr(strText, SB) > 0 And InStr(strText, EB) > 0 Then
        Call ReceiveProcess
        pBuffer = ""
    End If
