...
Begin VB.Form frmInterface 
   Caption         =   " RapidLab 348EX Interface "
   ClientHeight    =   11040
   ClientLeft      =   345
   ClientTop       =   840
   ClientWidth     =   21735
   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox cboChk 
            ...
            EndProperty
            Height          =   300
            ItemData        =   "frmInterface.frx":C22F
            Left            =   2340
            List            =   "frmInterface.frx":C239
            TabIndex        =   45
            Top             =   -90
            Visible         =   0   'False
            Width           =   765
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C249
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":CD4F
         End
...
strBuffer = strBuffer & "" & vbCr
    ...
    strBuffer = "SMP_EDIT_DATA&aMOD348iIIDB228rTYPESAMPLErSEQ1rDATE21Jan16rTIME13:43rDEVICESYRINGEmpH7.266LmPCO265.8mmHgHmPO231.2mmHgLmNa+148mmol/LHmK+3.90mmol/LmCl-108mmol/LmHct48%cCa++mmol/LcHCO3act29.3mmol/LctCO231.3mmol/LcO2SAT49.9%cBE(vt)0.2mmol/LcBE(vv)2.3mmol/LctHb(est)16.4g/dLcAnGap14.5mmol/LmBP774mmHg&96        "
...
'    SetRawData "[RxT]" & strBuffer
    
    fLCURO = Split(strBuffer, vbCr)
 
 
    strIDRecord = Trim$(mGetP(strBuffer, 1, FS))
    
    If strIDRecord = "SMP_NEW_DATA" Or strIDRecord = "SMP_EDIT_DATA" Then
        '## WorkNo 조회
        Pos1 = InStr(strBuffer, "rSEQ")
        If Pos1 > 0 Then
            Pos2 = InStr(Mid$(strBuffer, Pos1), FS)
            strSeq = Format$(mGetP(Mid$(strBuffer, Pos1, Pos2), 2, GS), "#####")
            strSeq = Val(strSeq)
        Else
        ...
        x1 = 1
        Do While InStr(x1, strBuffer, FS & "m") <> 0
            x1 = InStr(x1, strBuffer, FS & "m")
            x2 = InStr(x1, strBuffer, GS)
    
    '        AssayNm = Mid(MsgBuf, x1 + 2, x2 - (x1 + 2))
            'Ca++의 경우 장비검사코드가 동일하기 때문에 Measured & Calibrated 의 구분이 필요...
            strIntBase = Mid(strBuffer, x1 + 1, x2 - (x1 + 1))
            x2 = x2 + 1
            x1 = InStr(x2, strBuffer, GS)
            strResult = Mid(strBuffer, x2, x1 - x2)
...
'                        lsSubCode = Trim(gReadBuf(3))
        ...
        x1 = 1
        Do While InStr(x1, strBuffer, FS & "c") <> 0
            x1 = InStr(x1, strBuffer, FS & "c")
            x2 = InStr(x1, strBuffer, GS)
...
'                End If
'
'                If GetWorkList_Barcode(strNewBC, .ActiveRow) Then
'                    '-- 로컬 저장
