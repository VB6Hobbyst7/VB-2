...
Begin VB.Form frmInterface 
   Caption         =   " RapiChem M744 Interface "
   ClientHeight    =   11040
   ClientLeft      =   345
   ClientTop       =   840
   ClientWidth     =   21735
   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasWorkList 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":B71B
            UserResize      =   2
         End
         ...
         Begin VB.ComboBox cboChk 
            ...
            EndProperty
            Height          =   300
            ItemData        =   "frmInterface.frx":C2D9
            Left            =   2340
            List            =   "frmInterface.frx":C2E3
            TabIndex        =   45
            Top             =   -90
            Visible         =   0   'False
            Width           =   765
         ...
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C2F3
            UserResize      =   2
         End
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":CDF9
         End
...
Private Sub cmdCClear_Click()
    ...
    For intRow = vasWorkList.MaxRows To 1 Step -1
        vasWorkList.Row = intRow
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String)
    ...
    SQL = SQL & "   ORDER BY prex.acp_dt, prex.smp_no " & vbCr
          ...
          SQL = "SELECT p.patid ,p.patname ,p.sex  ,p.prsnidpre||prsnidpost prsnid  ,r.resultflag  ,w.resultstate  ,w.rsvacptstate  ,r.ORDERCODE"
...
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "-----------#0032" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "Na"" 141.4 mmol/L     " & vbCr
strBuffer = strBuffer & "K""   4.25 mmol/L     " & vbCr
strBuffer = strBuffer & "Cl^ 101.1 mmol/L" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "JAN-26-16; 15:37" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "" & vbCr
...
strBuffer = ""
strBuffer = strBuffer & "  All RESULTS" & vbCr
strBuffer = strBuffer & "Na""    K""   Cl^" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "JAN -25" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "#0035" & vbCr
strBuffer = strBuffer & "142.5  4.82 104.4" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "#0036" & vbCr
strBuffer = strBuffer & "139.2  3.73 100.7" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "JAN -26" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "#0001" & vbCr
strBuffer = strBuffer & "140.8  3.72 104.2" & vbCr
strBuffer = strBuffer & "" & vbCr
strBuffer = strBuffer & "#0002" & vbCr
strBuffer = strBuffer & "139.7  3.79 105.8" & vbCr
strBuffer = strBuffer & "JAN-27-16; 11:40" & vbCr
...
Private Sub EditRcvDataASTM()
    ...
    fLCURO = Split(strBuffer, vbCr)
 
'    If Trim(fLCURO(2)) <> "SAMPLE RESULTS" Then
'        If Trim(fLCURO(2)) <> "DAILY RESULTS" Then
'            Exit Sub
'        End If
'    End If
    
    If Trim(fLCURO(2)) = "SAMPLE RESULTS" Then
        For intCnt = 7 To UBound(fLCURO)
            strTemp1 = fLCURO(intCnt)
            Debug.Print fLCURO(intCnt)
            ...
            Select Case intCnt
                Case 7    '## Order
                    ...
                    For i = 1 To vasWorkList.DataRowCnt
                        ...
                        vasWorkList.Col = colCheckBox
                        If vasWorkList.Value = "1" Then
                            SetText vasWorkList, strSeq, gRow, colSeqNo
                            vasWorkList.Col = colBarcode
                    ...
                    If gRow <= 0 Then
                        ...
                        SetText vasWorkList, strSeq, gRow, colSeqNo
                        SetText vasWorkList, strSeq, gRow, colBarcode
                    End If
                    
                    strState = "O"
                    
                    
                Case 10 To 12    '## Result
                    '## 장비기준 검사명, 결과, Abnormal Flag
                    strIntBase = Mid(strTemp1, 1, 4)
                    strIntBase = Trim(strIntBase)
                    
                    strResult = Mid(strTemp1, 5, 6)
...
'   기능 : 장비로부 수신한 데이터 편집
'-----------------------------------------------------------------------------'
Private Sub EditRcvDataASTM_Daily()
    Dim strRcvBuf    As String   '수신한 Data
    Dim strType      As String   '수신한 Record Type
    Dim strBarno     As String   '수신한 바코드번호
    Dim strSeq       As String   '수신한 Sequence
...
'    End If
    ...
    For intCnt = 1 To UBound(fLCURO)
        ...
        If i = 1 Then
            strIntBase = "Na"""
            strResult = Mid(strTemp1, 1, 6)
            strResult = Trim(strResult)
        ElseIf i = 2 Then
            strIntBase = "K"""
            strResult = Mid(strTemp1, 7, 6)
            strResult = Trim(strResult)
        Else
            strIntBase = "Cl^"
            strResult = Mid(strTemp1, 13, 6)
            strResult = Trim(strResult)
        End If
...
Function SetLocalUpdate(ByVal strNewBC As String, ByVal iRow As Long)
          
    If Trim(GetText(vasWorkList, iRow, colSeqNo)) = "" Then
    '    MsgBox "바코드 번호 변경 실패", vbOKOnly + vbCritical, Me.Caption
        Exit Function
    End If
    
    'If MsgBox(Trim(GetText(vasWorkList, iRow, colSeqNo)) & "를 " & strNewBC & "로 수정하시겠습니까?", vbYesNo + vbInformation, Me.Caption) = vbYes Then
              SQL = "UPDATE PATRESULT SET "
        SQL = SQL & " BARCODE = '" & strNewBC & "', "
        SQL = SQL & " PID = '" & Trim(GetText(vasWorkList, iRow, colPID)) & "', "
        SQL = SQL & " PNAME = '" & Trim(GetText(vasWorkList, iRow, colPName)) & "'" & vbCrLf
        SQL = SQL & " WHERE EXAMDATE = '" & Format(dtpToday, "YYYYMMDD") & "' " & vbCrLf
        SQL = SQL & "   AND EQUIPNO = '" & gEquip & "' " & vbCrLf
        SQL = SQL & "   AND BARCODE = '" & Trim(GetText(vasWorkList, iRow, colSeqNo)) & "' "
...
Private Sub txtBarcode_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        '-- 바코드 번호 유효성 체크
        If Len(txtBarcode.Text) <> 9 Then
            MsgBox "바코드 길이가 틀립니다", vbOKOnly + vbCritical, Me.Caption
            Exit Sub
        End If
        ...
        SQL = SQL & " and r.spcmno = w.spcmno"
        SQL = SQL & " and r.OrderCode = w.OrderCode"
        SQL = SQL & " and r.spcmno = '" & Trim(txtBarcode.Text) & "'"
        SQL = SQL & " and r.resultitemcode in (" & gAllExam & ")"
        ...
        If Not (RS.EOF Or RS.BOF) Then
            ...
            Do Until RS.EOF
                ...
                SetText vasWorkList, "1", intRow, colCheckBox
                SetText vasWorkList, Trim(txtBarcode.Text), intRow, colSeqNo
                SetText vasWorkList, Trim(txtBarcode.Text), intRow, colBarcode
                SetText vasWorkList, Trim(RS.Fields("patid")), intRow, colPID
                SetText vasWorkList, Trim(RS.Fields("patname")), intRow, colPName
                SetText vasWorkList, Trim(RS.Fields("sex")), intRow, colSex
...
'Debug.Print "F:" & vasWorkList.ActiveRow
'On Error GoTo Rst:
        ...
        With vasWorkList
            ...
            If .ActiveCol = colBarcode Then
                ...
                If Not IsNumeric(strNewBC) Then
                    MsgBox "숫자만 입력하세요", vbOKOnly + vbInformation, Me.Caption
                    Exit Sub
                End If
                
                If Len(strNewBC) > 9 Then
                    Exit Sub
                End If
                ...
                For i = 1 To .MaxRows
                    ...
                    .Col = colBarcode
                    If i <> intRow Then
                        If Trim(.Text) = strNewBC Then
                            MsgBox "같은 바코드가 리스트에 있습니다.", vbOKOnly + vbInformation, Me.Caption
                            '.SelLength = Len(.Text)
                            '.SelStart = 0
                            '.Text = ""
                            Exit Sub
                ...
                '-- 로컬 저장
                Call SetLocalUpdate(strNewBC, intRow)
                
                If GetWorkList_Barcode(strNewBC, intRow) Then
                    '-- 로컬 저장
                    'Call SetLocalUpdate(strNewBC, .ActiveRow)
                End If
                
                vasWorkList.RowHeight(-1) = 12
                
                vasWorkList.Row = intRow + 1
                vasWorkList.Col = colBarcode
...
On Error GoTo Rst
    ...
    Do Until RS.EOF
        ...
        SetText vasWorkList, "1", intRow, colCheckBox
        SetText vasWorkList, pBunNo, intRow, colSeqNo
        SetText vasWorkList, pBunNo, intRow, colBarcode
        SetText vasWorkList, Trim(RS.Fields("patid")), intRow, colPID
        SetText vasWorkList, Trim(RS.Fields("patname")), intRow, colPName
        SetText vasWorkList, Trim(RS.Fields("sex")), intRow, colSex
        RS.MoveNext
    ...
    If GetWorkList_Barcode = False Then
        ...
        SetText vasWorkList, pBunNo, intRow, colSeqNo
        SetText vasWorkList, pBunNo, intRow, colBarcode
        SetText vasWorkList, "", intRow, colPID
        SetText vasWorkList, "", intRow, colPName
        SetText vasWorkList, "", intRow, colSex
    End If
