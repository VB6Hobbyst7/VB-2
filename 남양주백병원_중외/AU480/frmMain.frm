...
Private Sub GetOrder(ByVal pBarNo As String, ByVal pType As String)
    ...
    With frmMain
        ...
        If Trim(strItems) = "" Or mOrder.SendCnt = 0 Then
            ...
            '-- 진행상태(Order) 표시
            Call SetText(frmMain.spdOrder, "오더없음", intRow, colSTATE)
            strOrder = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.OrgBarNo)) & mOrder.OrgBarNo & "    E" & ETX
        ...
        Else
            ...
            '-- 진행상태(Order) 표시
            Call SetText(frmMain.spdOrder, "오더전송", intRow, colSTATE)
            strOrder = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.OrgBarNo)) & mOrder.OrgBarNo & "    E" & strItems & ETX
        End If
...
'''                        End If
                        ...
                        '-- CREA
'                        If strIntBase = gCalCH.CREA Then
'                            'GFR 계산
'                            strGFR = ""
'                            strCREA = strResult
...
'                        End If
                        ...
                        'TC
                        If strIntBase = gCalCH.TC Then
                            strTC = strResult
                        End If
                        
                        'TG
                        If strIntBase = gCalCH.TG Then
                            strTG = strResult
                        End If
                        
                        'HDL
                        If strIntBase = gCalCH.HDL Then
                            strHDL = strResult
                        End If
                        
                        'LDL 계산
                        If strTC <> "" And strTG <> "" And strHDL <> "" And IsNumeric(strTC) And IsNumeric(strTG) And IsNumeric(strHDL) Then
                            strLDL = strTC - ((strTG / 5) + strHDL)
                            If strLDL < 0 Then
                                strLDL = "0"
                            End If
                            strTC = ""
