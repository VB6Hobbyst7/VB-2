...
'On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        strPatNm = Trim(GetText(.vasID, argSpcRow, colPNAME))
...
'            strUID = gUserID
            'ORDSEQNO,NORLOW,NORHIGH,SLIPCD,ORDDATE,ORDTYPE "
            
            strTemp = GetTestSeq_GSAM(argSpcRow, strEqpCd)
            ...
            If sResult <> "" Then
                If iRow = 1 Then
                          SQL = "Update SLSPCMDT Set "
                    SQL = SQL & "  procstat = 'E'"
                    SQL = SQL & " WHERE spcid    = '" & lsID & "'"
                    SQL = SQL & "   AND procstat IN ('A','B','C')"
                                   
                    SetSQLData "검사결과저장", SQL
                    ...
                    End If
                          ...
                          SQL = "Update SLSPEXDT Set "
                    SQL = SQL & "  procstat = 'E'"
                    SQL = SQL & " where spcid    = '" & lsID & "'"
'                    SQL = SQL & "   and ordseqno = '" & strSeqNo & "'"
'                    SQL = SQL & "   and examcode = '" & strEqpCd & "'"
                    SQL = SQL & "   AND procstat IN ('A','B','C')"
                    SetSQLData "검사결과저장", SQL
                    ...
                    End If
                          ...
                          SQL = "Update SLXWORKT Set "
                    SQL = SQL & "  procstat = 'E'"
                    SQL = SQL & " ,repttech = '" & strUID & "'" '검사자ID
                    SQL = SQL & " where spcid    = '" & lsID & "'"
                    SQL = SQL & "   AND procstat IN ('A','B','C')"
                    SetSQLData "검사결과저장", SQL
                    ...
                    End If
                          ...
                          SQL = "Update MDEXMORT Set "
                    SQL = SQL & "  procstat = 'E'"
                    SQL = SQL & " where spcid1 = '" & lsID & "'"
                    SQL = SQL & "   AND procstat IN ('A','B','C')"
                    SetSQLData "검사결과저장", SQL
                ...
                With cmdSQL
                    ...
                    Set prm1 = .CreateParameter("IN_SPCID", adVarChar, adParamInput, 4000, lsID)
                    .Parameters.Append prm1
                    Set prm2 = .CreateParameter("IN_EXAMCODE", adVarChar, adParamInput, 4000, strEqpCd)
                    ...
                    Set prm6 = .CreateParameter("IN_EQIPCODE", adVarChar, adParamInput, 4000, gEquipCode)
                    .Parameters.Append prm6
                    Set prm7 = .CreateParameter("IN_EDITID", adVarChar, adParamInput, 4000, strUID)
                    .Parameters.Append prm7
                    Set prm8 = .CreateParameter("IN_EDITIP", adVarChar, adParamInput, 4000, gIP)
                    .Parameters.Append prm8
                    Set prm9 = .CreateParameter("OUT_ERR", adVarChar, adParamOutput, 4000, strErr)
                    .Parameters.Append prm9

                    SetSQLData "검사결과저장", lsID & "," & strEqpCd & "," & sResult & "," & sResult & "," & "" & "," & gEquipCode & "," & strUID & "," & gIP
                    SetSQLData "검사결과저장", "StrErr:" & strErr
...
Function GetSampleInfoW_SUNGMO(ByVal asRow As Long) As Integer
    ...
    With frmInterface
        Do Until RS.EOF
            ...
            Call SetRawData("[정보조회결과-BSDATE]" & Trim(RS.Fields("BSDATE")) & "")
            Call SetRawData("[정보조회결과-SAMPLE]" & Trim(RS.Fields("SAMPLE")) & "")
            Call SetRawData("[정보조회결과-HOSPNO]" & Trim(RS.Fields("HOSPNO")) & "")
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_GSAM(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    ...
    Select Case Mid(sBarcode, 1, 1)
        Case "1", "2", "3", "4", "5" '안양
            strPatTableName = "ORAA1.APPATBAT P"
        Case Else
            '지샘
            strPatTableName = "ORAA1.APPATBAT @goraa1 P"
    End Select
  
  
  
          SQL = "SELECT W.SPCID,                                  " & vbCr
    SQL = SQL & "      to_char(W.ORDDATE, 'yyyymmdd') AS ODT,   " & vbCr
    SQL = SQL & "      W.ORDSEQNO,                                " & vbCr
    SQL = SQL & "      W.EXAMCODE,                                " & vbCr
    SQL = SQL & "      to_char(W.WORKNO),                         " & vbCr
    SQL = SQL & "      to_char(W.AREANO),                         " & vbCr
    SQL = SQL & "      W.RSLTTEXT,                                " & vbCr
    SQL = SQL & "      S.SPCCODE ,                                " & vbCr
    SQL = SQL & "      W.MEDDEPT,                                 " & vbCr
    SQL = SQL & "      S.PATNO,                                   " & vbCr
    SQL = SQL & "      P.PATNAME,                                 " & vbCr
    SQL = SQL & "      P.SEX,                                     " & vbCr
    SQL = SQL & "      to_char(P.BIRTdate, 'yyyymmdd') AS BIR,    " & vbCr
    SQL = SQL & "      W.EXECID                                   " & vbCr '접수자 ID
    SQL = SQL & " FROM SLXWORKT W,                                " & vbCr
    SQL = SQL & "      SLSPCMDT S,                                " & vbCr
    SQL = SQL & strPatTableName & vbCr                            '샘병원 DB링크
    SQL = SQL & "WHERE W.SPCID    = '" & sBarcode & "'            " & vbCr
    If frmInterface.chkSave.Value <> "1" Then
        SQL = SQL & "  AND W.PROCSTAT = 'E'                       " & vbCr
    End If
    SQL = SQL & "  AND S.SPCID    = W.SPCID                       " & vbCr
    SQL = SQL & "  AND W.PATNO    = P.PATNO                       " & vbCr
    SQL = SQL & "ORDER BY W.WORKNO, W.SPCID, W.EXAMCODE           "
    ...
    With frmInterface
        Do Until RS.EOF
            ...
            SetText .vasID, Trim(RS.Fields("ODT")) & "", asRow, colHOSPDATE
            SetText .vasID, Trim(RS.Fields("SPCID")) & "", asRow, colBARCODE
            SetText .vasID, Trim(RS.Fields("PATNO")) & "", asRow, colCHARTNO
            SetText .vasID, Trim(RS.Fields("PATNAME")) & "", asRow, colPNAME
            SetText .vasID, Trim(RS.Fields("SEX")) & "", asRow, colPSEX
            'gUserID = Trim(RS.Fields("SEX")) & ""
            gUserID = Trim(RS.Fields("EXECID")) & ""
            RS.MoveNext
        Loop
...
'-- 검사자 정보 가져오기
Function GetTestSeq_GSAM(ByVal asRow As Long, ByVal asTestCd As String) As String
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    ...
    End If
          ...
          SQL = "SELECT ORDSEQNO,NORLOW,NORHIGH,WORKCODE,EXECDATE,RSLTTYPE,ORDDATE "
    SQL = SQL & " FROM SLXWORKT                                 " & vbCr
    SQL = SQL & "WHERE SPCID    = '" & sBarcode & "'            " & vbCr
    'SQL = SQL & "  AND PROCSTAT = 'E'                           " & vbCr
    SQL = SQL & "  AND EXAMCODE = '" & asTestCd & "' " & vbCr
