...
Public Const colBARCODE = 5     '병록번호
Public Const colCHARTNO = 6     '바코드번호
Public Const colPID = 7         '워크번호
...
Type OcsData
    ...
    FAcDt   As String   '접수일자
    FAcTm   As String   '접수시간
    FRqDt   As String   '의뢰일자
    FRqTm   As String   '의뢰시간
...
'    strEqpCd = frmInterface.txtTemp
    ...
    db_tmp = ""
    Call GetPrivateProfileString("config", "OrdCd", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gOrdCd = Trim(frmInterface.txtTemp)
...
'    gQCEquip = Trim(frmInterface.txtTemp)
    ...
    db_tmp = ""
    Call GetPrivateProfileString("DATABASE", "ordtable", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gDB_Parm.OrdTable = Trim(frmInterface.txtTemp)
    
    db_tmp = ""
    Call GetPrivateProfileString("DATABASE", "rsttable", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gDB_Parm.RstTable = Trim(frmInterface.txtTemp)
    ...
    db_tmp = ""
    Call GetPrivateProfileString("Server", "VerifyID", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gVerifyID = Trim(frmInterface.txtTemp)
...
Public Function GetNextWkno() As String
    ...
    '-- 결과업데이트
          SQL = "SELECT MAX(WNIFWKNO) + 1 From acawnifh  " & vbCr
    SQL = SQL & " WHERE WNIFDATE = '" & Format(Now, "yyyymmdd") & "' "
    
    'Call SetSQLData("워크번호 가져오기", SQL)
...
Public Function GetNextSpcid() As String
    ...
    '-- 결과업데이트
    SQL = "SELECT SMPL_SEQN.NEXTVAL  FROM dual"
