...
On Error GoTo ErrHandle
    ...
    SQL = SQL & " ( WNIFDPCD, WNIFHPNO, WNIFDATE, WNIFSLIP, WNIFITEM                    " & vbCr    '1
    SQL = SQL & " , WNIFOITP, WNIFWKNO, WNIFIDNO, WNIFNAME, WNIFRRNF                    " & vbCr    '2
    SQL = SQL & " , WNIFRSEX, WNIFRRNS, WNIFWARD, WNIFROOM, WNIFDEPT                    " & vbCr    '3
    
    SQL = SQL & " , WNIFDOCT, WNIFMDDT, WNIFACDT, WNIFACTM, WNIFRQDT                    " & vbCr    '4 WNIFACDT, WNIFACTM, WNIFRQDT
    SQL = SQL & " , WNIFRQTM, WNIFLSDT, WNIFRPDT, WNIFRPTM, WNIFRPMT                    " & vbCr    '5 WNIFRQTM
    SQL = SQL & " , WNIFRPMD, WNIFSMDT, WNIFSMTM, WNIFSMPL, WNIFSMGB                    " & vbCr
    SQL = SQL & " , WNIFSMYR, WNIFSMSN, WNIFSMS1, WNIFSMS2, WNIFSTAT                    " & vbCr    '-- WNIFSMYR (7자리로 변경)
    SQL = SQL & " , WNIFQCHK, WNIFSPCL, WNIFCONT, WNIFODNU, WNIFTYPE                    " & vbCr
    SQL = SQL & " , WNIFJSNU, WNIFJSCD, WNIFDOID, WNIFDONM, WNIFTRFG                    " & vbCr
    SQL = SQL & " , WNIFPRDT, WNIFPRTM, WNIFMEMO, WNIFECT1, WNIFECT2                    " & vbCr
    ...
    SQL = SQL & ",'" & mOCS.FRqDt & "'" & vbCr  '20
    
    'SQL = SQL & ",'" & mOCS.FAcTm & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FRqTm & "'" & vbCr
    SQL = SQL & ",'" & Format(Now, "yyyymmdd") & "'" & vbCr
    SQL = SQL & ",'" & Format(Now, "yyyymmdd") & "'" & vbCr
    ...
    SQL = SQL & ",'" & Mid(mOCS.FBarCode, 3, 7) & "'" & vbCr
...
Function GetSampleInfoW_ILSIN(ByVal asRow As Long) As Integer
    ...
    '-- 바코드 번호로 오더 조회
          SQL = "Select ptbsname             " & vbCr
    SQL = SQL & " ,substr(ptbsrrns,1,1) JNO1 " & vbCr
    SQL = SQL & " ,substr(ptbsrrns,2,6) JNO2 " & vbCr
    ...
    SQL = SQL & " ,ipslacdt                 " & vbCr    '접수일자
    SQL = SQL & " ,ipslactm                 " & vbCr    '접수시간
    SQL = SQL & " ,ipslhpdt                 " & vbCr    '
    SQL = SQL & " ,ipslhptm                 " & vbCr    '
    
    SQL = SQL & " ,ipslgndt                 " & vbCr    '의뢰일자
    SQL = SQL & " ,ipslgntm                 " & vbCr    '의뢰시간
    
    SQL = SQL & "  From pmcptbsm, ocsipslh  " & vbCr
    SQL = SQL & " Where ipslidno = '" & sBarcode & "'   " & vbCr    '챠트번호
    SQL = SQL & "   And ipslmddt IN ('" & Format(Now - 1, "yyyymmdd") & "','" & Format(Now, "yyyymmdd") & "') " & vbCr  '처방일자
'    SQL = SQL & "   And ipslmddt IN ('" & Format(Now, "yyyymmdd") & "','" & Format(Now, "yyyymmdd") & "') " & vbCr
    SQL = SQL & "   And ipslcode = '" & gOrdCd & "'" & vbCr '처방코드 'SUEBGAG'
        ...
        If Not RS.EOF = True And Not RS.BOF = True Then
            Do Until RS.EOF
                ...
                With mOCS
                    ...
                    .FAcDt = Trim(RS.Fields("ipslacdt")) & ""   '접수일자
                    .FAcTm = Trim(RS.Fields("ipslactm")) & ""   '접수시간
                    
                    .FRqDt = Trim(RS.Fields("ipslgndt")) & ""   '의뢰일자
                    .FRqTm = Trim(RS.Fields("ipslgntm")) & ""   '의뢰시간
