               ...
               ' Call SetText(frmMain.spdTest, IIf(AdoRs_Local.Fields("QCTemp").Value & "" = "1", "사용", "미사용"), intRow, colLUseResSpec)
                ...
                If Trim(AdoRs_Local.Fields("TESTCODE").Value) <> "" Then
                    If intRow = 1 Or gAllTestCd = "" Then
                        ...
                        gAllTestCds = Trim(AdoRs_Local.Fields("TESTCODE").Value) & Space(12 - Len(Trim(AdoRs_Local.Fields("TESTCODE").Value)))
                    Else
                        gAllTestCd = gAllTestCd & ",'" & AdoRs_Local.Fields("TESTCODE").Value & "'"
                        gAllTestCds = gAllTestCds & Trim(AdoRs_Local.Fields("TESTCODE").Value) & Space(12 - Len(Trim(AdoRs_Local.Fields("TESTCODE").Value)))
                    End If
                End If
...
On Error GoTo RST
    ...
    SQL = SQL & " ORDER BY READING_YMD,PTNT_NO,BCODE_NO                         " & vbCr
...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        ...
        Case "ILSIN"
                If GetSampleInfo_ILSIN(asRow, SPD) = -1 Then
                    GetSampleInfo = -1
                    Exit Function
                End If
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_ILSIN(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = ""
    SQL = SQL & "Select ptbsname                    " & vbCr
    SQL = SQL & "     , substr(ptbsrrns,1,1) as JNO1   " & vbCr
    SQL = SQL & "     , substr(ptbsrrns,2,6) as JNO2   " & vbCr
    SQL = SQL & "     , ptbsrrnf JNO                " & vbCr
    SQL = SQL & "     , ipslward                    " & vbCr
    SQL = SQL & "     , ipslroom                    " & vbCr
    SQL = SQL & "     , ipsldept                    " & vbCr
    SQL = SQL & "     , ipsldoct                    " & vbCr
    SQL = SQL & "     , ipslgnus                    " & vbCr  '//의사사번 - 보고자.
    SQL = SQL & "     , ipslmddt                    " & vbCr
    SQL = SQL & "     , ipslacdt                    " & vbCr
    SQL = SQL & "     , ipslactm                    " & vbCr
    SQL = SQL & "     , ipslhpdt                    " & vbCr
    SQL = SQL & "     , ipslhptm                    " & vbCr
    SQL = SQL & "  From pmcptbsm, ocsipslh          " & vbCr
    SQL = SQL & " Where ipslidno = '" & strBarcode & "'" & vbCr
    SQL = SQL & "   And ipslmddt IN ('" & Format(Now - 1, "yyyymmdd") & "','" & Format(Now, "yyyymmdd") & "') " & vbCr
'    SQL = SQL & "   And ipslmddt IN ('" & Format(Now, "yyyymmdd") & "','" & Format(Now, "yyyymmdd") & "') " & vbCr
    SQL = SQL & "   And ipslcode = '" & gHOSP.LABCD & "'" & vbCr  'SUEBGAG
    SQL = SQL & "   And ipslidno = ptbsidno        " & vbCr
    SQL = SQL & "   And ipslflag = 'O'           " & vbCr
    SQL = SQL & "   And ipslstat = '0'           " & vbCr
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With mOCS
                ...
                .FAcDt = Trim(RS.Fields("ipslacdt")) & ""
                .FAcTm = Trim(RS.Fields("ipslactm")) & ""
            ...
            With SPD
                ...
                intTestCnt = intTestCnt + 1
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, mOCS.FMdDt, asRow, colHOSPDATE
                SetText SPD, strBarcode, asRow, colBARCODE
                SetText SPD, mOCS.FWorkNo, asRow, colPID
                'SetText SPD, TmOCS.FBarCode, asRow, colCHARTNO
                SetText SPD, mOCS.FPNM, asRow, colPNAME
                
                SetText SPD, mOCS.FDocID, asRow, colINOUT
                SetText SPD, mOCS.FAcDt, asRow, colRACKNO
                SetText SPD, mOCS.FAcTm, asRow, colPOSNO
...
Public Function GetNextWkno() As String

    GetNextWkno = ""
    
    SQL = ""
    SQL = SQL & "SELECT MAX(WNIFWKNO) + 1 as MAXWNIFWKNO "
    SQL = SQL & "  From acawnifh  " & vbCr
    SQL = SQL & " WHERE WNIFDATE = '" & Format(Now, "yyyymmdd") & "' "
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            GetNextWkno = Trim(RS.Fields("MAXWNIFWKNO").Value)
            RS.MoveNext
        Loop
    End If
    ...
    Call SetSQLData("GetNextWkno", GetNextWkno)
    
End Function


Public Function GetNextSpcid() As String

    GetNextSpcid = ""
    
    '-- 결과업데이트
    SQL = ""
    SQL = SQL & "SELECT SMPL_SEQN.NEXTVAL as NEXTVAL" & vbCr
    SQL = SQL & "  FROM dual "
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            GetNextSpcid = Trim(RS.Fields("NEXTVAL").Value)
            RS.MoveNext
        Loop
    End If
    ...
    Call SetSQLData("GetNextWkno", GetNextSpcid)
...
On Error GoTo ErrHandle
    ...
    SQL = SQL & " , WNIFJSNU, WNIFJSCD, WNIFDOID, WNIFDONM, WNIFTRFG                    " & vbCr
    SQL = SQL & " , WNIFPRDT, WNIFPRTM, WNIFMEMO, WNIFECT1, WNIFECT2                    " & vbCr
    SQL = SQL & " , WNIFECT3, WNIFCHRT, WNIFSLID, WNIFBARC, WNIFACUS )        " & vbCr
    SQL = SQL & " Values (" & vbCr
    SQL = SQL & " 'LA'" & vbCr
    SQL = SQL & ",'01'" & vbCr
    ...
    SQL = SQL & ",'" & mOCS.FPNM & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FJNO & "'" & vbCr   '10
    SQL = SQL & ",'" & mOCS.FJNO1 & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FJNO2 & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FWard & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FRoom & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FDept & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FDoct & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FMdDt & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FAcDt & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FAcTm & "'" & vbCr
    SQL = SQL & ",'" & mOCS.FAcDt & "'" & vbCr  '20
    SQL = SQL & ",'" & mOCS.FAcTm & "'" & vbCr
    SQL = SQL & ",'" & Format(Now, "yyyymmdd") & "'" & vbCr
    SQL = SQL & ",'" & Format(Now, "yyyymmdd") & "'" & vbCr
    SQL = SQL & ",'" & Format(Now, "hhmm") & "'" & vbCr
    ...
    SQL = SQL & ",'004'" & vbCr
    SQL = SQL & ",''" & vbCr
    SQL = SQL & ",'" & Format(Now, "yy") & "'" & vbCr   '30
    SQL = SQL & ",'" & Mid(mOCS.FBarCode, 3, 6) & "'" & vbCr
    SQL = SQL & ",1" & vbCr
    SQL = SQL & ",1" & vbCr
    SQL = SQL & ",'2'" & vbCr
    SQL = SQL & ",'-'" & vbCr
