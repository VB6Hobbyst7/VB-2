...
Begin VB.Form frmComm 
   ...
   Begin TabDlg.SSTab tabWork 
      ...
      Tab(0).Control(5)=   "cmdStartNo"
      Tab(0).Control(5).Enabled=   0   'False
      Tab(0).Control(6)=   "cmdPosNo"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).Control(7)=   "cmdRackNo"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).Control(8)=   "cmdWordQuery"
      Tab(0).Control(8).Enabled=   0   'False
      Tab(0).Control(9)=   "cmdEot"
      ...
      Tab(0).Control(16)=   "SSPanel1"
      Tab(0).Control(16).Enabled=   0   'False
      Tab(0).Control(17)=   "SSFrame1"
      Tab(0).Control(17).Enabled=   0   'False
      Tab(0).Control(18)=   "fraError"
      ...
      Tab(0).Control(22)=   "UserPanel1"
      Tab(0).Control(22).Enabled=   0   'False
      Tab(0).Control(23)=   "spdResult1"
      Tab(0).Control(23).Enabled=   0   'False
      Tab(0).Control(24)=   "cmdSel(0)"
      Tab(0).Control(24).Enabled=   0   'False
      Tab(0).Control(25)=   "cmdSel(1)"
      Tab(0).Control(25).Enabled=   0   'False
      Tab(0).Control(26)=   "chkAuto"
      Tab(0).Control(26).Enabled=   0   'False
      Tab(0).Control(27)=   "optDiv(0)"
      Tab(0).Control(27).Enabled=   0   'False
      Tab(0).Control(28)=   "optDiv(1)"
      ...
      Begin FPSpread.vaSpread spdWorklist 
         ...
         RetainSelBlock  =   0   'False
         ScrollBarMaxAlign=   0   'False
         SpreadDesigner  =   "frmComm.frx":76E1
         UserResize      =   0
      End
      ...
      Begin Threed.SSFrame SSFrame1 
         ...
         Begin FPSpread.vaSpread spdRstview 
            ...
            SpreadDesigner  =   "frmComm.frx":7C32
            UserResize      =   0
         End
      End
      Begin Threed.SSPanel SSPanel1 
         ...
         Begin VB.ComboBox cboChk 
            Enabled         =   0   'False
            Height          =   300
            ItemData        =   "frmComm.frx":827F
            Left            =   3630
            List            =   "frmComm.frx":8289
            TabIndex        =   59
            Top             =   90
            Visible         =   0   'False
            Width           =   1095
         ...
         Begin VB.ComboBox cboComNm 
            Height          =   300
            ItemData        =   "frmComm.frx":8299
            Left            =   4755
            List            =   "frmComm.frx":829B
            TabIndex        =   58
            Top             =   90
            Visible         =   0   'False
            Width           =   1545
      ...
      Begin Threed.SSPanel SSPanel3 
         ...
         Begin VB.ComboBox cboRstgbn 
            Height          =   300
            Index           =   1
            ItemData        =   "frmComm.frx":8B8D
            Left            =   2460
            List            =   "frmComm.frx":8B9A
            Style           =   2  '드롭다운 목록
            TabIndex        =   68
            Top             =   90
            Visible         =   0   'False
      ...
      Begin FPSpread.vaSpread spdResult2 
         ...
         RetainSelBlock  =   0   'False
         ScrollBarMaxAlign=   0   'False
         SpreadDesigner  =   "frmComm.frx":8BC4
         UserResize      =   0
      End
...
On Error GoTo ErrorTrap
    ...
    'strDate = Mid(strDate, 1, 4) & "-" & Mid(strDate, 5, 2) & "-" & Mid(strDate, 7, 2) & " 00:00:00"
                   
             sqlDoc = "Select 접수일자,병록번호,검체번호,성명, 결과" & vbCr
'    sqlDoc = sqlDoc & "  From 검사검체3v" & vbCr
    sqlDoc = sqlDoc & "  From 검사검체5v" & vbCr
'    sqlDoc = sqlDoc & " Where 접수일자 = '" & strDate & "'" & vbCr
    sqlDoc = sqlDoc & " Where 접수일자 = TO_DATE('" & strDate & "', 'yyyymmdd')" & vbCr
    sqlDoc = sqlDoc & "   and 품목코드 in ('b1030z','b1030','b1030h','b103h')" & vbCr
    sqlDoc = sqlDoc & "   and 인수자코드 in ('lb13','lb14','lb15','lb16','lb17','lb18','lb19','lb1a','lb1b','lb1c','lb1d','lb1e') " & vbCr
    sqlDoc = sqlDoc & "   and (DC구분 = '' or   DC구분 is null)" & vbCr
    If optDiv(0).Value = True Then
        sqlDoc = sqlDoc & "   and (결과 = '' or 결과 is null)"
    Else
        sqlDoc = sqlDoc & "   and (결과 <> '' or 결과 is not null)"
    End If
    
    
    sqlDoc = "SELECT 접수일자, 검체번호, 병록번호, 성명, 과코드, 처리구분코드, 품목코드, 결과 FROM 검사검체1V"
    sqlDoc = sqlDoc & vbCrLf & " WHERE 접수일자 BETWEEN '" & strDate & "' AND '" & strDate & "' "
    sqlDoc = sqlDoc & vbCrLf & "AND 품목코드 IN ('b1030z','b1030','b1030h','b103h') "
    If optDiv(0).Value = True Then
        sqlDoc = sqlDoc & vbCrLf & "AND 처리구분코드 = 'P' "
    Else
        sqlDoc = sqlDoc & vbCrLf & "AND 처리구분코드 IN ('R', 'E') "
    End If
...
On Error GoTo ErrorTrap
    ...
    CallForm = "clsCommon - Public Function f_subSet_WorkList() As ADODB.Recordset"
        ...
        Set AdoRs_SQL = New ADODB.Recordset
                 ...
                 sqlDoc = "Select 접수번호, 품목코드, 검체번호, 접수일자, 처방전코드, 병록번호, 접수일자, 입외구분, 성명, 나이, 성별코드, 생년월일, 처리구분코드, 과코드"
        sqlDoc = sqlDoc & "  From 검사검체1v"
        sqlDoc = sqlDoc & " Where 검체번호 = '" & strPid & "'"
        sqlDoc = sqlDoc & "   and 품목코드 in ('b1030z','b1030','b1030h','b103h')" & vbCr
        sqlDoc = sqlDoc & "   and 처리구분코드 <> 'R'"
...
'On Error GoTo ErrorTrap
    ...
    If RecordChk = False Then
        MsgBox mskOrdDate.Text & "일의 검사 대상자가 없습니다.", vbOKOnly + vbInformation, Me.Caption
    Else
        strBarno = ""
        intRow = 1
        Do Until mAdoRs.EOF
            
            '접수일자,병록번호,검체번호,성명, 결과
            With spdWorklist
                If strBarno <> mAdoRs.Fields("검체번호") Then
                    intRow = intRow + 1
                    If intRow > .maxrows Then .maxrows = .maxrows + 1:  .RowHeight(.maxrows) = 13
                    
                    .SetText 1, intRow, mAdoRs.Fields("접수일자") & ""
                    .SetText 2, intRow, mAdoRs.Fields("병록번호") & ""
                    .SetText 3, intRow, mAdoRs.Fields("검체번호") & ""
                    .SetText 4, intRow, mAdoRs.Fields("성명") & ""
                    .SetText 5, intRow, mAdoRs.Fields("결과") & ""
                    .SetText 6, intRow, mAdoRs.Fields("처리구분코드") & ""
                    
                    '-- 검사상세
                    Set mAdoRs1 = f_subSet_WorkList_Barcode(Trim(mAdoRs.Fields("검체번호")))
                    If RecordChk = True Then
                        Do Until mAdoRs1.EOF
                            .SetText 7, pGrid_Point, mAdoRs1("품목코드") & ""
                            mAdoRs1.MoveNext
                        Loop
                    End If
...
'            .GetText 7, intRow, varTmp:   pNo = Trim$(varTmp)
'            .GetText 8, intRow, varTmp:   strSugaCode = Trim$(varTmp)

            .GetText 2, intRow, varTmp:   strBarno = Trim$(varTmp)
            .GetText 3, intRow, varTmp:   pName = Trim$(varTmp)
            .GetText 7, intRow, varTmp:   strDate = Trim$(varTmp)
            .GetText 6, intRow, varTmp:   pNo = Trim$(varTmp)
            .GetText 8, intRow, varTmp:   strSugaCode = Trim$(varTmp)
                ...
                For intCol = 9 To .MaxCols
                    ...
                    If Trim$(varTmp) <> "" Then
                        ...
                        If Not itemX Is Nothing Then
                            ...
                            strSeqNum = ""
                            If strSugaCode = "b1030z" Then
                                strSeqNum = "38"
                            ElseIf strSugaCode = "b1030" Then
                                strSeqNum = "54"
                            ElseIf strSugaCode = "b1030h" Then
                                strSeqNum = "55"
                            ElseIf strSugaCode = "b103h" Then
                                strSeqNum = "67"
                            End If

                            'sTxt = INS_CODE & "," & strBarno & "," & "1,1," & strRstval & "," & vbLf
                            sTxt = INS_CODE & "," & strBarno & "," & "1," & strSeqNum & "," & strRstval & "," & vbLf

                            tTxt = tTxt & sTxt
                            
                            objSpd.Row = intRow
                            objSpd.Col = 2
                            objSpd.BackColor = vbCyan
                            objSpd.Col = 3
                            objSpd.BackColor = vbCyan
                            objSpd.Col = 4
                            objSpd.BackColor = vbCyan
    ...
    Open App.Path & "\" & INS_CODE & ".dat" For Output As #2
    ...
    If chkRCP.Value = 1 And chkRSH.Value = 1 Then
        Ret = WinExec("rsh.exe med -l lab ./exp_interface.sh clbdvt02m3 h01", 2)
    End If
    ...
    Do While Not adoRS.EOF
        ...
        If strSpcno <> Trim$(adoRS(0) & "") + Trim$(adoRS(6) & "") Then
            ...
            If intRow > .maxrows Then .maxrows = .maxrows + 1:  .RowHeight(.maxrows) = 13
                ...
                .SetText 4, intRow, Trim$(adoRS("NAME") & "")
                '.SetText 5, intRow, Trim$(adoRS(0) & "")
                '.SetText 6, intRow, Trim$(adoRS(0) & "")
                .SetText 7, intRow, Trim$(adoRS("PNO") & "")
                .SetText 8, intRow, Trim$(adoRS("RSTVAL") & "")
            End If
...
'            Timer1.Enabled = True
            ...
            For intIdx = 1 To Len(strDta)
                strBuff = Mid$(strDta, intIdx, 1)
                
                'Debug.Print strBuff & ":" & Asc(strBuff) & "###"
...
Public Function GetChkSum(ByVal pMsg As String) As String
    ...
    For i = 1 To Len(pMsg)
        ...
        'Debug.Print lngChkSum & ">>" & Mid("0" & Hex(lngChkSum), Len(Hex(lngChkSum)), 2)
    Next
    ...
    For intIdx = 1 To Len(strRec)
        ...
        Select Case strBuff
            ...
            Case vbCr  '-- etx
                    f_strBuffer = f_strBuffer + strBuff
                    'strTmp = f_strBuffer
                    sStxCheck = InStr(f_strBuffer, ">")
                    sEtxCheck = InStr(f_strBuffer, vbCr)
                    If sStxCheck <> 0 And sEtxCheck <> 0 Then
                        sEtxtoLen = sEtxtoLen + 1
                        If sEtxtoLen = 4 Then
                            Print #1, "[TX]" & f_strBuffer;
                            Call psDataDefine(f_strBuffer, fChannel(), spdResult1)
                            sEtxtoLen = 0
                            f_strBuffer = ""
                        End If
...
'    On Error Resume Next
  ...
  '      MsgBox "1"
    ...
    Else
        Exit Sub
    End If
    comEQP.Output = ACK & "01" & vbCr
    Debug.Print "[1ST]" & ACK & "01" & vbCr
    Print #1, vbNewLine & "[1ST]" & ACK & "01" & vbCr;
    
    If InStr(1, strdata, ">") < 1 Then Exit Sub
    
    If Trim(strdata) <> "" Then
        strTestcd = Mid(strdata, 7, 3)   '150:오더,151:결과
        
        '-- 오더
        If strTestcd = "150" Then
            comEQP.Output = sTemp
            Debug.Print "[2ND]" & sTemp
            Print #1, vbNewLine & "[2ND]" & sTemp;

            strRcvBuf = Mid(strdata, 12)
            varBarCd = Split(strRcvBuf, "")
            
            'Print #1, vbNewLine & "[O_Bar]" & varBarCd
            
            For ii = 0 To UBound(varBarCd) - 1
                strBarno = varBarCd(ii)
                    ...
                    If RecordChk = True Then
                        Do Until mAdoRs.EOF
                            ...
                            With spdResult1
                                ...
                                .SetText 2, pGrid_Point, strBarno
                                .SetText 3, pGrid_Point, mAdoRs("성명") & ""
                                .SetText 4, pGrid_Point, IIf(Trim(mAdoRs("성별코드")) = "M", "남", "여") & "/" & Format(Now, "yyyy") - Format(Trim(mAdoRs("생년월일")), "yyyy") - 1
                                If mAdoRs("입외구분") = "1" Then
                                    .SetText 5, intRow, "입원"
                                ElseIf mAdoRs("입외구분") = "2" Then
                                    .SetText 5, intRow, "외래"
                                Else
                                    .SetText 5, intRow, "퇴원"
                                End If
                                .SetText 6, pGrid_Point, mAdoRs("병록번호") & ""
                                .SetText 7, pGrid_Point, mAdoRs("접수일자") & ""
                                .SetText 8, pGrid_Point, mAdoRs("품목코드") & ""
                            End With
                            mAdoRs.MoveNext
                        Loop
                    Else
                        With spdResult1
                            ...
                            .SetText 2, pGrid_Point, strBarno
                            .SetText 3, pGrid_Point, "오더없음"
                            .SetText 4, pGrid_Point, ""
                            .SetText 5, pGrid_Point, ""
                            .SetText 6, pGrid_Point, ""
                            .SetText 7, pGrid_Point, ""
                        End With
            ...
            comEQP.Output = sTemp
            Print #1, vbNewLine & "[RT]" & sTemp;

        '-- 결과
        Else
            
            strBarno = Mid$(strdata, 12, 10)
            If InStr(strBarno, "") > 0 Then
                strBarno = Mid(strBarno, 1, InStr(strBarno, "") - 1)
            End If
Result:
            Print #1, vbNewLine & "[R_Bar]" & strBarno
            'strBarno = "4910701"
            If IsNumeric(strBarno) Then
                strRstval = Trim(Mid$(strdata, InStr(strdata, "") + 11, 4))
                If Len(strBarno) > 0 And Len(strBarno) = 7 Then
                    intRow = 0
                    Set mAdoRs = f_subSet_WorkList_Barcode(strBarno)
                    ...
                    If RecordChk = True Then
                        Do Until mAdoRs.EOF
                            ...
                            With spdResult1
                                ...
                                .SetText 4, pGrid_Point, IIf(Trim(mAdoRs("성별코드")) = "M", "남", "여") & "/" & Format(Now, "yyyy") - Format(Trim(mAdoRs("생년월일")), "yyyy") - 1
                                If mAdoRs("입외구분") = "1" Then
                                    .SetText 5, pGrid_Point, "입원"
                                ElseIf mAdoRs("입외구분") = "2" Then
                                    .SetText 5, pGrid_Point, "외래"
                                Else
                                    .SetText 5, pGrid_Point, "퇴원"
                                End If
                                .SetText 6, pGrid_Point, mAdoRs("병록번호") & ""
                                .SetText 7, pGrid_Point, mAdoRs("접수일자") & ""
                                .SetText 8, pGrid_Point, mAdoRs("품목코드") & ""
                    ...
                    With spdResult1
                        ...
                        .GetText 7, pGrid_Point, varTmp:   strDate = Trim$(varTmp)
                        .GetText 6, pGrid_Point, varTmp:   pNo = Trim$(varTmp)
                        .GetText 8, pGrid_Point, varTmp:   strSugaCode = Trim$(varTmp)
                                        ...
                                        If Len(strRstval) > 0 Then
                                            If Channel_No = Trim(itemX.tag) Then
                                                ...
                                                If chkAuto.Value = vbChecked Then
                                                    ...
                                                    '서버 서비스 실행 rsh med -l lab ./exp_interface.sh clbdvt02m3 esr
...
'    strTmp = strTmp & "" & vbCr
    ...
    strTmp = strTmp & "      VES MATIC 60      "
    strTmp = strTmp & "  ************************"
    strTmp = strTmp & "  1 = 4965861......   3 "
    strTmp = strTmp & "  2 = 4966418......   5 "
    strTmp = strTmp & "  3 = 4965805......   8 "
    strTmp = strTmp & "  4 = 4965888......   6 "
    strTmp = strTmp & "  5 = 4965939......   4 "
    strTmp = strTmp & "  6 = 4965914......   3 "
    strTmp = strTmp & "  7 = 4965817......   5 "
    strTmp = strTmp & "  8 = 4965904......   7 "
    strTmp = strTmp & "  9 = 4966476......   5 "
    strTmp = strTmp & " 10 = 4965980......  16 "
    strTmp = strTmp & " 11 = 4965868......   6 "
    strTmp = strTmp & " 12 = 4965877......  16 "
    strTmp = strTmp & " 13 = 4966021......  13 "
    strTmp = strTmp & " 14 = 4966004......   6 "
    strTmp = strTmp & " 15 = 4965797......  11 "
    strTmp = strTmp & " 16 = 4965956......   9 "
    strTmp = strTmp & " 17 = 4965931......  30 "
    strTmp = strTmp & " 18 = 4965895......   8 "
    strTmp = strTmp & " 19 = 4965882......  10 "
    strTmp = strTmp & " 20 = 4965992......   5 "
    strTmp = strTmp & " 21 = 4965829......  20 "
    strTmp = strTmp & " 22 = 4965849......  17 "
    strTmp = strTmp & " 23 = 4965985......  19 "
    strTmp = strTmp & " 24 = 4965968......   5 "
    strTmp = strTmp & " 25 = 4966006......   9 "
    strTmp = strTmp & " 26 = 4965974......   8 "
    strTmp = strTmp & " 27 = 4965851......   5 "
    strTmp = strTmp & " 28 = 4965950......   5 "
    strTmp = strTmp & " 29 = 4965825......   6 "
    strTmp = strTmp & " 30 = 4965812......   6"
...
'    Me.Show
    ...
    Open App.Path + "\Log\" + Format$(Now, "YYYYMMDD") + "_" + REG_INSNAME & ".log" For Append As #1
...
'                Next
            ...
            ElseIf aCOL = 9 Then
                .GetText 2, aROW, varTmp:   strBarno = Trim$(varTmp)
                .GetText 3, aROW, varTmp:   pName = Trim$(varTmp)
                .GetText 7, aROW, varTmp:   strDate = Trim$(varTmp)
                .GetText 6, aROW, varTmp:   pNo = Trim$(varTmp)
                .GetText 8, aROW, varTmp:   strSugaCode = Trim$(varTmp)
                .GetText 9, aROW, varTmp:   strRstval = Trim$(varTmp)
...
Private Sub spdWorklist_KeyPress(KeyAscii As Integer)
    Dim aROW    As Integer, aCOL   As Integer
    Dim varChk  As Variant, varBar As Variant, varNum As Variant
    Dim iRow    As Integer, iCnt   As Integer
...
'                .SetText 4, intRow, mAdoRs.Fields("성명") & ""
'                .SetText 5, intRow, mAdoRs.Fields("결과") & ""
'                .SetText 6, pGrid_Point, mAdoRs1("품목코드") & ""
                
                .GetText 1, aROW, varTmp:   strDate = Trim$(varTmp)
                .GetText 2, aROW, varTmp:   strBarno = Trim$(varTmp)
                .GetText 3, aROW, varTmp:   pNo = Trim$(varTmp)
                .GetText 4, aROW, varTmp:   pName = Trim$(varTmp)
                .GetText 5, aROW, varTmp:   strRstval = Trim$(varTmp)
                .GetText 6, aROW, varTmp:   strSugaCode = Trim$(varTmp)
                
                Set itemX = lvwCuData.FindItem("ESR", lvwSubItem, , lvwWhole)
                If Not itemX Is Nothing Then
                    strTestcd = itemX.Text
...
'        .Col = 4
'
'        YesNo = MsgBox(Trim(.Text) & " 환자를 지우시겠습니까?", vbExclamation + vbYesNo, INS_NAME)
'        If vbYes = YesNo Then
...
Private Sub Timer1_Timer()
    ...
    If f_strBuffer <> "" And Len(f_strBuffer) Then
        Call psDataDefine(f_strBuffer, fChannel(), spdResult1)
