    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                SQL = SQL & "   And (ResRepTyp  = '' OR ResRepTyp = 'M') " '최종(F)/중간(I)은 제외
...
'          SQL = "SELECT R.ResLabCod AS EXAMCODE" & vbCr
...
'    SQL = SQL & " WHERE SUBSTRING(O.OCMACPDTM,1,8) = '" & argOrdDt & "'" & vbCr
'    SQL = SQL & "   AND R.RESLABCOD IN (" & gAllExam & ") " & vbCr
'    SQL = SQL & "   AND LTRIM(O.OcmChtNum) = '" & argPID & "'" & vbCr
'    SQL = SQL & "   AND LTRIM(R.ResOcmNum) = '" & argInNum & "'" & vbCr
...
'        SQL = SQL & "   AND (R.RESREPTYP IS NULL OR R.RESREPTYP <> 'F')" & vbCr
