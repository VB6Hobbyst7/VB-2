...
type
  EMatricSingluraError = class(EMathError);

procedure PolyFit(const x, y: TArray<Double>; var coefs: array of Double;
  var correl_coef: Double; const npoints, nterms: Integer);
...
procedure square(const x: matrix; const y: array of Double; var a: matrix;
  var g: array of Double; const nrow, ncol: Integer);
var
  i, k, l: Integer;
begin
...
var
  ...
  begin { procedure gausj2 }
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              error := true;
              Raise EMatricSingluraError.Create
                ('Error in GaussJordan2: matrix is singular');
            end;
            if index[k, 2] < 0 then
      ...
      for m := 0 to ncol - 1 do
        b[icol, m] := b[icol, m] / pivot;
      if nv > 0 then
        for m := 0 to nv - 1 do
          w[icol, m] := w[icol, m] / pivot;
      ...
      begin
        ...
        begin
          ...
          b[n, icol] := 0.0;
          for m := 0 to ncol - 1 do
            b[n, m] := b[n, m] - b[icol, m] * t;
          if nv > 0 then
            for m := 0 to nv - 1 do
              w[n, m] := w[n, m] - w[icol, m] * t;
        end;
      end;
...
begin { gauss-jordan main routine }
  ...
  try
    ...
    for k := 0 to ncol - 1 do
      ...
      begin
        error := true;
        Raise EMatricSingluraError.Create('Error in GaussJordan: matrix is singular');
      end;
...
procedure PolyFit(const x, y: TArray<Double>; { was Real }
...
begin
  ...
  if correl_coef <> 0 then
    correl_coef := srs / correl_coef;
  if correl_coef >= 1 then
    correl_coef := 0.0
  else
