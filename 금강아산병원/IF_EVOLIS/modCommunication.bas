...
Function SaveTransData_ASAN(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        '-- 서버로 결과값 저장하기
        For intRow = 1 To .vasTemp.DataRowCnt
            strEqpcd = Trim(GetText(.vasTemp, intRow, 1))       '검사채널
            strTestCd = Trim(GetText(.vasTemp, intRow, 3))      '검사코드
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            strRefVal = Trim(GetText(.vasTemp, intRow, 7))      'PANIC
            ...
            If IsNumeric(sResult) Then
                ...
                sResult = mGetP(sResult, 1, ".") & "." & Mid(mGetP(sResult, 2, "."), 1, 2)
                sResult = Format(sResult, "#0.00")
            Else
                If InStr(sResult, "Negative") > 0 Then
                    sResult3 = mGetP(mGetP(sResult, 2, "("), 1, ")")
                    sResult3 = Format(sResult3, "#0.00")
                    sResult = "Negative(" & sResult3 & ")"
                End If
                
                If InStr(sResult, "Positive") > 0 Then
                    sResult3 = mGetP(mGetP(sResult, 2, "("), 1, ")")
                    sResult3 = Format(sResult3, "#0.00")
                    sResult = "Positive(" & sResult3 & ")"
                End If
                
                If InStr(sResult, "Equivocal") > 0 Then
                    sResult3 = mGetP(mGetP(sResult, 2, "("), 1, ")")
                    sResult3 = Format(sResult3, "#0.00")
                    sResult = "Equivocal(" & sResult3 & ")"
                End If
            ...
            'Else
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    ...
                    With AdoCmd
                        ...
                        Set prm1 = .CreateParameter("r_hocd", adChar, adParamInput, 2, gHOSP.MACHCD)    '장비코드
                        .Parameters.Append prm1
                        Set prm2 = .CreateParameter("r_paid", adChar, adParamInput, 8, strPatID)        '환자번호
                        .Parameters.Append prm2
                        Set prm3 = .CreateParameter("r_spno", adChar, adParamInput, 14, strBarcode)     '검체번호
                        .Parameters.Append prm3
                        Set prm4 = .CreateParameter("r_orcd", adChar, adParamInput, 7, strTestCd)       '검사코드
                        .Parameters.Append prm4
                        Set prm5 = .CreateParameter("r_rslt", adVarChar, adParamInput, 30, sResult)     '검사결과
                        .Parameters.Append prm5
                        If strRefVal = "Y" Then
                            Set prm6 = .CreateParameter("r_okfl", adChar, adParamInput, 1, "N")             '확정여부(Y / N)
                        Else
                            Set prm6 = .CreateParameter("r_okfl", adChar, adParamInput, 1, "Y")             '확정여부(Y / N)
                        End If
                        .Parameters.Append prm6
                        Set prm7 = .CreateParameter("r_maid", adChar, adParamInput, 7, Mid(strEqpcd, 1, 7))      '장비채널?
                        .Parameters.Append prm7
                        Set prm8 = .CreateParameter("s_eryn", adVarChar, adParamOutput, 4000, 0)        '오류여부(Y / N)
                        .Parameters.Append prm8
                        Set prm9 = .CreateParameter("s_retn", adVarChar, adParamOutput, 4000, 0)        '반환값
                        .Parameters.Append prm9
                        
                        strErr = ""
                        strErr = strErr & .Parameters("s_eryn").Value & ":" & .Parameters("s_eryn").Value & vbNewLine
                        strErr = strErr & "r_hocd : " & gHOSP.MACHCD & vbNewLine
                        strErr = strErr & "r_paid : " & strPatID & vbNewLine
                        strErr = strErr & "r_spno : " & strBarcode & vbNewLine
                        strErr = strErr & "r_orcd : " & strTestCd & vbNewLine
                        strErr = strErr & "r_rslt : " & sResult & vbNewLine
                        strErr = strErr & "r_okfl : " & "N" & vbNewLine
                        strErr = strErr & "r_maid : " & strEqpcd
                        
                        Call SetSQLData("저장값", strErr, "A")
...
ErrHandle:
    SaveTransData_ASAN = -1
    Call SetSQLData("저장오류1", Err.Description)
    
End Function

Function SaveTransDataR_ASAN(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
