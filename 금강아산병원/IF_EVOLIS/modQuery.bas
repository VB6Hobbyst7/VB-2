...
'                gArrEQP(intRow, 12) = AdoRs_Local.Fields("COLOUT").Value & ""
                ...
                'panic으로 사용
                gArrEQP(intRow, 10) = AdoRs_Local.Fields("CUTOFFUSE").Value & ""
                gArrEQP(intRow, 11) = AdoRs_Local.Fields("COLCOMP").Value & ""
                gArrEQP(intRow, 12) = AdoRs_Local.Fields("COLIN").Value & ""
...
'                            SetText SPD, IIf(Trim(RS.Fields("INOUT")) & "" = "10", "입원", "외래"), intRow, colINOUT
        ...
        Case "ASAN"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT OKDT  as HOSPDATE"
            SQL = SQL & ", SPNO as BARCODE"
            SQL = SQL & ", WKNO as CHARTNO"
            SQL = SQL & ", PAID as PID"
            SQL = SQL & ", PANM as PNAME"
            SQL = SQL & ", SEXS as SEX" & vbCr
            'SQL = SQL & ", ORCD as ITEM " & vbCr
            SQL = SQL & "  FROM V_AFISINFO " & vbCr
            SQL = SQL & " WHERE OKFL = 'N'" & vbCr
            SQL = SQL & "   AND OKDT BETWEEN  TO_DATE('" & pFrom & "','yyyymmdd') AND TO_DATE('" & pTo & "','yyyymmdd') + 1" & vbCr
            SQL = SQL & "   AND ORCD IN (" & gAllTestCd & ")" & vbCr
            SQL = SQL & "   AND HOCD = '" & gHOSP.MACHCD & "'" & vbCr
                ...
                Do Until RS.EOF
                    ...
                    With SPD
                        ...
                        For i = 1 To SPD.DataRowCnt
                            ...
                            '    blnSame = True
                            'End If
                            If Trim(RS("BARCODE")) = strBarcode Then
                                blnSame = True
                            End If
                        Next
...
'            SQL = SQL & "   and m.workarea = '" & gHOSP.LABCD & "'  " & vbCr
        ...
        Case "ASAN"
            ...
            SQL = SQL & "   AND HOCD = '" & gHOSP.MACHCD & "'" & vbCrLf
...
On Error GoTo DBErr
    ...
    Select Case gOCS
        Case "ASAN"
            'OKFL as Y/N결과확정여부, OKDT as 결과확정DATE, OKID as 확정자
            SQL = ""
            SQL = SQL & "SELECT DISTINCT OKDT  as HOSPDATE"
            SQL = SQL & ", SPNO as BARCODE"
            SQL = SQL & ", WKNO as CHARTNO"
...
'                        If Trim(RS.Fields("ITEM")) = "L9018" Then
'                            SetText SPD, Trim(RS.Fields("BARCODE")) & "_" & asRow, asRow, colBARCODE
'                            mOrder.BarNo = Trim(RS.Fields("BARCODE")) & "_" & asRow
'                        End If
