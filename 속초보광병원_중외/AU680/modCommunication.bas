...
Public Sub GetOrder(ByVal pBarNo As String, ByVal pType As String)
    ...
    With frmMain
        ...
        If Trim(strItems) = "" Then
            ...
            frmMain.comEqp.Output = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.BarNo)) & mOrder.BarNo & "    E" & ETX
        ...
        Else
            ...
            SetRawData "[Tx]" & STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.BarNo)) & mOrder.BarNo & Space(4) & "E" & strItems & ETX
            frmMain.comEqp.Output = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.BarNo)) & mOrder.BarNo & Space(4) & "E" & strItems & ETX
        End If
        ...
        For intCnt = 1 To UBound(strRecvData)
            ...
            Select Case strType
                ...
                Case "D "    '## Result
                        ...
                        strSEX = GetText(frmMain.spdOrder, gRow, colPSEX)
                        strAGE = GetText(frmMain.spdOrder, gRow, colPAGE)
                        strGFR = ""
                        strCREA = ""
                        ...
                        Do While Len(strTmp) >= 10
                            ...
                            '-- RPR 결과가  '-' 값이면 0.0 으로 변환
                            'If strIntBase = "24" Then
                            If strIntBase = "56" Then
                                If Mid(strResult, 1, 1) = "-" Then
                                    strResult = "0.0"
                                End If
                            End If
                            
                            '-- CRP 결과가  '-' 값이면 0.01 으로 변환
                            'If strIntBase = "22" Then
                            If strIntBase = "54" Then
                                If Mid(strResult, 1, 1) = "-" Then
                                    strResult = "0.01"
                                End If
                            End If
                            
                            '-- AST 결과가  '1' 또는 '2' 이면 3 으로 변환
                            'If strIntBase = "07" Then
                            If strIntBase = "37" Then
                                If strResult = "1" Or strResult = "2" Then
                                    strResult = "3"
                                End If
                            End If
                            
                            '-- ALT 결과가  '1' 또는 '2' 이면 3 으로 변환
                            'If strIntBase = "08" Then
                            If strIntBase = "38" Then
                                If strResult = "1" Or strResult = "2" Then
                                    strResult = "3"
                                End If
                            End If
                            
                            '-- LIP 결과가  '0' ~'3.4' 이면 <3.5 으로 변환
                            'If strIntBase = "25" Then
                            If strIntBase = "25" Then
                                If strResult > "0" And strResult < "3.5" Then
                                    strResult = "<3.5"
                                End If
                            End If
                            
                            '-- RA 결과가  '0' ~'4.9' 이면 <5 으로 변환
'                            If strIntBase = "23" Then
'                                If strResult > "0" And strResult < "5" Then
'                                    strResult = "<5.0"
'                                End If
'                            End If
                            ...
                            '-- CREA
                            'If strIntBase = "01" Then
                            If strIntBase = "31" Then
                                'GFR 계산
                                strGFR = ""
                                strCREA = strResult
                                '소수점 한자리만 사용 1.26 ==> 1.2
                                strCREA = mGetP(strCREA, 1, ".") & "." & Mid(mGetP(strCREA, 2, "."), 1, 1)
                                
                                If CCur(strCREA) > 0 Then
                                    If strSEX = "M" Then
                                        strGFR = 175 * (strCREA ^ -1.154) * (strAGE ^ -0.203)
                                    ElseIf strSEX = "F" Then
                                        strGFR = 175 * (strCREA ^ -1.154) * (strAGE ^ -0.203) * 0.742
                                    End If
                                    
                                    If strGFR <> "" Then
                                        'strGFR = Format(strGFR, "##0.00")
                                        strGFR = mGetP(strGFR, 1, ".")
                                        'strGFR = Format(strGFR, "##0")
                                    End If
                                Else
                                    strGFR = "Error"
                                End If
