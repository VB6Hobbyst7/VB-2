        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" Then
                    ...
                    SQL = SQL & " Where SPECIMENNUM  = '" & strBarcode & "'  " & vbCrLf
                    SQL = SQL & "   AND OrderCode    = '" & strOrdCd & "'    " & vbCrLf
'                    SQL = SQL & "   AND Receiptno   = '" & strChartNo & "'    " & vbCrLf
                    SQL = SQL & "   AND Receiptdate = '" & strHospDate & "'    " & vbCrLf
...
Public Sub GetResultList(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object)
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT SAVESEQ,EXAMDATE,EXAMTIME,HOSPDATE,BARCODE,PNAME,SENDFLAG,SENDDATE,CHARTNO,PID " & vbCr
    '-- 검사결과
...
'-- 장비결과 조회
Public Sub GetResultList_Barcode(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As Object, ByVal pBarNo As String, ByVal pAll As String)
    Dim RS          As ADODB.Recordset
    Dim i           As Integer
    Dim iCnt        As Long
    Dim intRow      As Long
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT SAVESEQ,EXAMDATE,EXAMTIME,HOSPDATE,BARCODE,PNAME,SENDFLAG,SENDDATE,CHARTNO,PID " & vbCrLf
    SQL = SQL & ",SEQNO,EXAMNAME,RESULT,PREVRESULT,REFJUDGE" & vbCrLf
    SQL = SQL & "  FROM PATRESULT " & vbCrLf
    If pAll = "1" Then
        SQL = SQL & " WHERE BARCODE  = '" & pBarNo & "'" & vbCrLf
    Else
        SQL = SQL & " WHERE EXAMDATE Between '" & pFrom & "' AND '" & pTo & "'" & vbCrLf
        SQL = SQL & "   AND BARCODE  = '" & pBarNo & "'" & vbCrLf
    End If
