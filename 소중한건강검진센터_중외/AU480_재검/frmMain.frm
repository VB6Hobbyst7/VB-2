...
Begin VB.Form frmMain 
      ...
      Begin MSComctlLib.ImageList imlStatus 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":3688
               Key             =   "NOF"
            EndProperty
            BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":37E2
               Key             =   "ON"
               Object.Tag             =   "OFF"
            EndProperty
            BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":40BC
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   Begin FPSpread.vaSpread spdWork 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":6FA6
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdOrder 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":91BA
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
...
Private Sub chkReRun_Click()
    
    If chkReRun.Value = "1" Then
        lblHospInfo(2).Caption = "-- 재검모드 --"
        shpStatus.BackColor = &HFF80FF
    Else
        lblHospInfo(2).Caption = "-- 일반모드 --"
        shpStatus.BackColor = &HACFFEF
    End If
End Sub
...
Private Function GetOrder_ReRun(ByVal pBarNo As String, ByVal pType As String) As Boolean

    Dim i           As Integer
    Dim intRrRow    As Long
    Dim intRow      As Long
    Dim strItems    As String
    Dim strOrder    As String

    GetOrder_ReRun = False
    ...
    With frmMain
        Select Case pType
            '-- 바코드 사용
            Case "0"
                For i = 1 To frmRerun.spdResult.DataRowCnt
                    If Trim(GetText(frmRerun.spdResult, i, colBARCODE)) = pBarNo Then
                        intRrRow = i
                        Exit For
                    End If
                Next i
            ...
            Case "1"
                For i = 1 To frmRerun.spdResult.DataRowCnt
                    If Val(Trim(GetText(frmRerun.spdResult, i, colSEQNO))) = Val(mOrder.Seq) Then
                        pBarNo = Trim(GetText(frmRerun.spdResult, i, colBARCODE))
                        mOrder.BarNo = pBarNo
                        intRrRow = i
                        Exit For
                    End If
            ...
            Case "2"
                For i = 1 To frmRerun.spdResult.DataRowCnt
                    If Trim(GetText(frmRerun.spdResult, i, colRACKNO)) = mOrder.RackNo And Trim(GetText(frmRerun.spdResult, i, colPOSNO)) = mOrder.TubePos Then
                        pBarNo = Trim(GetText(frmRerun.spdResult, i, colBARCODE))
                        intRrRow = i
                        Exit For
                    End If
            ...
            Case "3"
                For i = 1 To frmRerun.spdResult.DataRowCnt
                    If GetText(frmRerun.spdResult, i, colCHECKBOX) = "1" Then
                        pBarNo = Trim(GetText(frmRerun.spdResult, i, colBARCODE))
                        mOrder.BarNo = pBarNo
                        intRrRow = i
                        Exit For
        ...
        '## 검사자 정보 가져오기 ##
        '-- 재검 환자정보 화면표시
        For i = colHOSPDATE To colSTATE
            Call SetText(frmMain.spdOrder, GetText(frmRerun.spdResult, intRrRow, i), intRow, i)
        Next
        
        strItems = ""
        mOrder.Order = ""
        '-- 재검 환자정보 오더채널
        For i = colSTATE + 1 To frmRerun.spdResult.MaxCols
            frmRerun.spdResult.Row = intRrRow
            frmRerun.spdResult.Col = i
            If frmRerun.spdResult.Value <> "0" Then
                'Debug.Print gArrEQP(i - colSTATE, 3) '오더채널
                mOrder.SendCnt = mOrder.SendCnt + 1
                strItems = strItems & Format(gArrEQP(i - colSTATE, 3), "000") & "0"
            End If
        Next
        ...
        If Trim(strItems) = "" Or mOrder.SendCnt = 0 Then
            ...
            '-- 진행상태(Order) 표시
            Call SetText(frmRerun.spdResult, "오더없음", intRrRow, colSTATE)
            strOrder = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.BarNo)) & mOrder.BarNo & "    E" & ETX
            Call DeleteRow(frmRerun.spdResult, intRrRow, intRrRow)
            frmRerun.spdResult.MaxRows = frmRerun.spdResult.MaxRows - 1
            
            '-- 진행상태(Order) 표시
            Call SetText(frmRerun.spdResult, "오더없음", intRow, colSTATE)
            strOrder = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.BarNo)) & mOrder.BarNo & "    E" & ETX
        ...
        Else
            ...
            '-- 진행상태(Order) 표시
            Call SetText(frmRerun.spdResult, "오더전송", intRrRow, colSTATE)
            strOrder = STX & "S " & mOrder.RackNo & mOrder.TubePos & mOrder.Seq & Space(20 - Len(mOrder.BarNo)) & mOrder.BarNo & "    E" & strItems & ETX
            Call DeleteRow(frmRerun.spdResult, intRrRow, intRrRow)
            frmRerun.spdResult.MaxRows = frmRerun.spdResult.MaxRows - 1
        ...
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                Case "R "    '## Inquiry Order
                    ...
                    '재검모드일 경우
                    If gRerunMode = True Then
                        If GetOrder_ReRun(strBarno, gHOSP.RSTTYPE) = False Then
                            '재검 창에서 못찾으면 원래화면에서 찾는다.
