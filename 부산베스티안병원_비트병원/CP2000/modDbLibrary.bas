...
On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        Call GetSampleInfoW_BITDLL(argSpcRow)
...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
       ...
       ' cn_Ser.BeginTrans
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            '-- 결과저장용 SEQ
            'Trim(LabResultObject.GetDataValue(iNumber, "RESODRSEQ")) & "|" & Trim(LabResultObject.GetDataValue(iNumber, "RESSEQ")) & "|" & Trim(LabResultObject.GetDataValue(iNumber, "RESSUBSEQ"))
...
'                SQL = SQL & "       ResItfVal = '" & sResult & "'," & vbCrLf   '인터페이스결과
...
'                SQL = SQL & "   And RESLABCOD = '" & strEqpCd & "'"    '검사코드
                ...
                'I : 중간보고, F : 최종보고
                strLOG = mOrder.LBQACPNUM & "," & _
                         mOrder.LBQOCMNUM & "," & _
                         mGetP(strSubCodes, 1, "|") & "," & _
                         mGetP(strSubCodes, 2, "|") & "," & _
                         mGetP(strSubCodes, 3, "|") & "," & _
                         strEqpCd & "," & _
                         sResult & "," _
                         & "I" & "," & _
                         gUserID & "," & _
                ...
                If LabResultObject.UpdateLabResult(mOrder.LBQACPNUM, mOrder.LBQOCMNUM, mGetP(strSubCodes, 1, "|"), mGetP(strSubCodes, 2, "|"), mGetP(strSubCodes, 3, "|"), strEqpCd, sResult, "I", gUserID, Format(Now, "yyyymmddhhmm")) Then
                    SaveTransDataW = 1
                End If
            
                Call SetSQLData("결과저장", strLOG)
                'I
                '18421205
            End If
        Next iRow
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_BITDLL(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol      As Integer
    Dim i           As Integer
...
'On Error GoTo RST
    ...
    If LabResultObject.GetLabStandByListByBarCode(sBarcode) Then
        With frmInterface
        
'            ReDim Preserve mOrder.B1530_RESODRSEQ(LabResultObject.GetRowCount - 1)
'            ReDim Preserve mOrder.B1530_RESSEQ(LabResultObject.GetRowCount - 1)
'            ReDim Preserve mOrder.B1530_RESSUBSEQ(LabResultObject.GetRowCount - 1)
'
'            ReDim Preserve mOrder.B1540_RESODRSEQ(LabResultObject.GetRowCount - 1)
'            ReDim Preserve mOrder.B1540_RESSEQ(LabResultObject.GetRowCount - 1)
'            ReDim Preserve mOrder.B1540_RESSUBSEQ(LabResultObject.GetRowCount - 1)
'
'            ReDim Preserve mOrder.B1540A_RESODRSEQ(LabResultObject.GetRowCount - 1)
'            ReDim Preserve mOrder.B1540A_RESSEQ(LabResultObject.GetRowCount - 1)
'            ReDim Preserve mOrder.B1540A_RESSUBSEQ(LabResultObject.GetRowCount - 1)
            
            For iNumber = 0 To LabResultObject.GetRowCount - 1
                
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(LabResultObject.GetDataValue(iNumber, "RESLABCOD")) & "',"
                
                SetText .vasID, "1", asRow, colCheckBox
                SetText .vasID, Mid(LabResultObject.GetDataValue(iNumber, "LBQACPDTM"), 1, 8), asRow, colHOSPDATE
                SetText .vasID, LabResultObject.GetDataValue(iNumber, "RSBBARCOD"), asRow, colBARCODE
                SetText .vasID, Trim(LabResultObject.GetDataValue(iNumber, "LBQCHTNUM")), asRow, colCHARTNO
                SetText .vasID, LabResultObject.GetDataValue(iNumber, "LBQPATNAM"), asRow, colPNAME
                ...
                If strJumin <> "" And Len(strJumin) >= 12 Then
                    ...
                    ElseIf Mid(strJumin, 7, 1) = "4" Then
                        ...
                        strAge = Format(Now, "yyyy") - Val("20" & Mid(strJumin, 1, 2))
                    End If
                    SetText .vasID, strSex, asRow, colPSEX
                    SetText .vasID, strAge, asRow, colPAGE
                End If
                
                strSaveCode = Trim(LabResultObject.GetDataValue(iNumber, "RESODRSEQ")) & "|" & Trim(LabResultObject.GetDataValue(iNumber, "RESSEQ")) & "|" & Trim(LabResultObject.GetDataValue(iNumber, "RESSUBSEQ"))
                Call SetSQLData("오더조회", strSaveCode)
                '-- 화면에 표시
                For intCol = colState + 1 To .vasID.MaxCols
                    If Trim(LabResultObject.GetDataValue(iNumber, "RESLABCOD")) = gArrEquip(intCol - colState, 3) Then
                        .vasID.Row = asRow
                        .vasID.Col = intCol
                        .vasID.BackColor = vbYellow
                        
                        Call SetSQLData("오더조회", gArrEquip(intCol - colState, 7))
                        '-- 결과저장용 SEQ
                        gArrEquip(intCol - colState, 7) = strSaveCode
...
'RST1:
                ...
                '        검사접수서브시퀀스 (ResSubSeq)
                '        검사결과값
                '        검사결과타입( F: 최종보고, I: 중간보고)
                '        검사담당자아이디
                '        검사일시( [Ex] "200504211530")
                With mOrder
                    .BarNo = sBarcode
...
'                    If Trim(LabResultObject.GetDataValue(iNumber, "RESLABCOD")) = "B1530" Then
'                        .B1530_RESODRSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESODRSEQ"))
'                        .B1530_RESSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESSEQ"))
'                        .B1530_RESSUBSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESSUBSEQ"))
'                    ElseIf Trim(LabResultObject.GetDataValue(iNumber, "RESLABCOD")) = "B1540" Then
'                        .B1540_RESODRSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESODRSEQ"))
'                        .B1540_RESSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESSEQ"))
'                        .B1540_RESSUBSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESSUBSEQ"))
'                    ElseIf Trim(LabResultObject.GetDataValue(iNumber, "RESLABCOD")) = "B1540A" Then
'                        .B1540A_RESODRSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESODRSEQ"))
'                        .B1540A_RESSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESSEQ"))
'                        .B1540A_RESSUBSEQ(iNumber) = Trim(LabResultObject.GetDataValue(iNumber, "RESSUBSEQ"))
'                    End If
                    ...
                    strLOG = mOrder.LBQACPNUM & "," & _
                         ...
                         mOrder.RESSEQ & "," & _
                         mOrder.RESSUBSEQ & "," & _
                         Trim(LabResultObject.GetDataValue(iNumber, "RESLABCOD"))
                    
                    Call SetSQLData("오더조회", strLOG)
                End With
            Next
