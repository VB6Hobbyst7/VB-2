...
Public intMode As Integer             '신규입력모드=1, 조회/수정모드=2
...
'        Exit Sub
    ...
    '비만관련 항목 초기화/치료프로그램 초기화 및 각종 화면 컨트롤 초기화
    '----------------------------------------------------------------------
    '2005-01-27 류진선
    '왼쪽위의 전회대비값을 표시하는 컨트롤 초기화
    '치료프로그램 콤보박스, 사용자입력 처방칼로기, Notice/Memo컨트롤 초기화
    '----------------------------------------------------------------------
    Call InitialControl
    '처방입력하는 스프레드(치료종목/출력물) 초기화
    '======================
    Call InitialSpread4 '치료종목
    Call InitialSpread5 '출력물
    '======================
    
    '변화도(표) 탭을 누를것으로 함.(왼쪽 아래)
    Call imgViewHistory_Click(0)

    'imgViewHistory_Click(0)에서 Call Bottom0(0)를 부름 혹시 삭제해도 될수도 있음 확인해 볼것.
    '2004-12-09 류진선 imgViewHistory_Click(0)에서 Bottom0(0)를 호출함.
    'Call Bottom0(0)
    
    '치료종목/출력물(오른쪽 하단) 탭을 누른것으로 함.
    Call imgTab_Click(0)

'2005-01-27 류진선
'==========> 여기까지 컨트롤 초기화/여기서부터 데이터 보여주기

    '가장 최근 입력값으로 보여주기
    
    '왼쪽 상단 비만관련 데이터 (체중/비만도/허리둘레/WHR/BMI/체지방률/RMR/TEE)보여주기
    Call ShowObesityRecord
    
    
    '오른쪽 상단 칼로리처방 데이터 초기화
    Call InitialTreatCalory
    intMode = 1
    cmdPreTreat.Visible = True
    '오늘일자의 Treat가 있으면 해당 진료데이터 불러오기
    Call ShowTodayTreat
    
    '다이어트를 하도록 하는 처방칼로리 컨트롤 초기화
...
Private Sub imgValuation_Click()
    Set imgTreat.Picture = LoadPicture(App.Path & IMG_평가_ON)
