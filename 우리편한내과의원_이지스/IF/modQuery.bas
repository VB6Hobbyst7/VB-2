...
Public Function GetEquipExamCode_HITACHI7020(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            ...
            'HbA1c 추가
            ':q    161 34                              3 19 105.3  20  5.22  44   6.7
            If intIntBase <> 99 Then
                If intIntBase = 44 Then
                    Mid$(strItems, 19, 1) = "1"
                    Mid$(strItems, 20, 1) = "1"
                    mOrder.SendCnt = mOrder.SendCnt + 1
                Else
    ...
    With frmMain
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                    If strJudge = "" Then
                        strJudge = SetJudge(sResult, strEqpcd)
                    End If
...
Function SetJudge_LOCAL(asResult As String, asEquipCode As String)
    ...
    SQL = ""
    SQL = SQL & "SELECT REFMLOW, REFMHIGH                     " & vbCr
    SQL = SQL & "  FROM EQPMASTER                           " & vbCr
    SQL = SQL & " WHERE EQUIPCD     = '" & gHOSP.MACHCD & "'" & vbCr
    ...
    If Not RS_L.EOF = True And Not RS_L.BOF = True Then
        If IsNumeric(Trim(RS_L.Fields("REFMLOW")) & "") = True And IsNumeric(Trim(RS_L.Fields("REFMHIGH")) & "") = True Then
            sLVal = Trim(RS_L.Fields("REFMLOW")) & ""
            sHVal = Trim(RS_L.Fields("REFMHIGH")) & ""
