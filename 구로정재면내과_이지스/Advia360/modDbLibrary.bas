...
'''                    SQL = SQL & "       RESULT_CD = '10' " & vbCr
'''                Else
'''                          SQL = "Update MCCSI.H7LIS_BCODE_ORD " & vbCr
'''                    SQL = SQL & "   Set RESULT_NM = '" & sResult & "', " & vbCr
'''                    SQL = SQL & "       RESULT_CD = '20' " & vbCr
'''                End If
'''
'''                SQL = SQL & " Where BCODE_NO = '" & lsID & "'" & vbCr
'''                SQL = SQL & "   And ORD_CD = '" & strEqpCd & "'" & vbCr
'''                SQL = SQL & "   And RESULT_TYPE = '20' "
                
                strHL = f_subSet_RefVal(strEqpCd, "", sResult)
                ...
                If IsNumeric(sResult) Then
                    ...
                    SQL = SQL & " ,vfy_ymd    = '" & Format(Now, "yyyymmdd") & "'" & vbCr   '보고일자
                   ' SQL = SQL & " ,hl_gb      = '" & strHL & "'"
                    SQL = SQL & " where recept_no = '" & lsID & "'" & vbCr
                    SQL = SQL & "   And ord_cd = '" & strEqpCd & "'" & vbCr
                    
                    'SQL = SQL & "   And sts_cd   = 'A' "
                    SQL = SQL & "   AND sts_cd  IN ('A','R') " & vbCr             ' A:접수 R:결과"
                    
                    'SQL = SQL & "   And ord_no = '" & strOrd_Cd & "'" & vbCr
                    'SQL = SQL & "   And ord_seq_no = '" & strOrd_Seq_No & "'" & vbCr
