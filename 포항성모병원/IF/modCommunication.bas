...
Function SaveTransData_POHANG(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 서버저장
                SQL = ""
                SQL = SQL & "Update exam_resultc "
                SQL = SQL & "   Set result       = '" & sResult & "'    " & vbCr  '검사결과
                SQL = SQL & "     , status       = 'Y'                  " & vbCr
                SQL = SQL & "     , resultdate   = sysdate              " & vbCr
                SQL = SQL & " Where specno       = '" & strBarcode & "' " & vbCr  '검체번호
                SQL = SQL & "   And subcode      = '" & strTestCd & "'  " & vbCr  '검사코드
                SQL = SQL & "   And status      <> 'V'                  " & vbCr
                ...
                '-- 상태업데이트
                SQL = ""
                SQL = SQL & "Update exam_specmst "
                SQL = SQL & "   Set status  = '04'                  " & vbCr
                SQL = SQL & " Where specno = '" & strBarcode & "'   " & vbCr '검체번호
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Select Case gEMR
        ...
        Case "POHANG"
            SaveTransData = SaveTransData_POHANG(argSpcRow)
