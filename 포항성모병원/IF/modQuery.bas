...
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    ...
    Select Case gEMR
        ...
        Case "POHANG"                      '포항성모
                Call GetSampleInfo_POHANG(asRow, SPD)
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_POHANG(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
On Error GoTo DBErr
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       m.sname     AS PNAME                " & vbCr
    SQL = SQL & "     , m.pano      AS PID                  " & vbCr
    SQL = SQL & "     , m.specno    AS BARCODE              " & vbCr
    SQL = SQL & "     , r.subcode   AS ITEM                 " & vbCr
    SQL = SQL & "  FROM exam_specmst m                      " & vbCr
    SQL = SQL & "     , exam_resultc r                      " & vbCr
    SQL = SQL & " WHERE m.SpecNo = r.SpecNo                 " & vbCr
    SQL = SQL & "   AND m.specno = '" & strBarcode & "'     " & vbCr
    SQL = SQL & "   AND r.subcode IN (" & gAllTestCd & ")   " & vbCr
    SQL = SQL & "   AND r.status <> 'V'                     " & vbCr
