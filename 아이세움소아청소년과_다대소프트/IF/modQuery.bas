...
Public Function GetEquipExamCode_PPC125(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim strSendCH       As String
    Dim iAffected       As Integer
    
    GetEquipExamCode_PPC125 = ""
    strExamCode = ""
    mOrder.Count = 0
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            ...
            If strSendCH <> "" Then
                ...
                'mOrder.Items(mOrder.Count) = "^^^" & strSendCH
                strExamCode = strExamCode & "\^^^" & strSendCH
            End If
            AdoRs_Local.MoveNext
        Loop
    ...
    If strExamCode <> "" Then
        GetEquipExamCode_PPC125 = Mid(strExamCode, 2)
'        GetEquipExamCode_PPC125 = "^^^ALB\^^^ALP"
'        GetEquipExamCode_PPC125 = "^^^ALB"
    End If
...
'        SQL = SQL & "   AND SAVESEQ     = " & strSaveSeq & vbCrLf
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                'MsgBox strBarcode & "," & strTestCd & "," & sResult & "," & strOrdCd & "," & strTestCd & "," & strTestCdSub
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" And strTestCdSub <> "" Then
                    ...
                    SQL = SQL & "   And 검사상태              IN ('0','1','2')        " & vbCrLf
        ...
        If Not RsLocal.EOF = True And Not RsLocal.BOF = True Then
            Do Until RsLocal.EOF
                ...
                If strBarcode <> "" And strTestCd <> "" And sResult <> "" And strOrdCd <> "" Then
                    ...
                    SQL = SQL & "   and 진료번호        = '" & strOrdCd & "'                            " & vbCrLf
                    SQL = SQL & "   and 처방번호        = '" & strTestCdSub & "'                        " & vbCrLf
...
'    Call SetCommStatus("R", pBarNo, frmMain.lstComStatus)
    ...
    With frmMain
        Select Case pType
            ...
            Case "1"
                For i = 1 To .spdOrder.DataRowCnt
                    If Val(GetText(frmMain.spdOrder, i, colSEQNO)) = Val(mOrder.Seq) Then
                        intRow = i
                        Exit For
                    End If
                Next i
...
'                   3 : 체크된것중 제일 위에 것
'-----------------------------------------------------------------------------'
Public Sub SetPatInfo_PPC125(ByVal pBarNo As String, ByVal pType As String)
...
On Error GoTo DBErr
    ...
    SQL = SQL & "       (a.진료년 + a.진료월 + a.진료일)    AS HOSPDATE     " & vbCrLf
    SQL = SQL & "     , a.챠트번호                          AS CHARTNO      " & vbCrLf
    SQL = SQL & "     , a.진료번호                          AS ORDCODE      " & vbCrLf  '해당 일자에 환자가 진료를 여러 번 봤을 경우 증가하는 진료인덱스
    SQL = SQL & "     , a.처방번호                          AS SUBCODE      " & vbCrLf  '각 검사처방 별 부여되는 처방 인덱스
    SQL = SQL & "     , c.진료상태                          AS STATE        " & vbCrLf
    SQL = SQL & "     , b.수진자명                          AS PNAME        " & vbCrLf
    SQL = SQL & "     , b.주민등록번호                      AS PJUMIN       " & vbCrLf
