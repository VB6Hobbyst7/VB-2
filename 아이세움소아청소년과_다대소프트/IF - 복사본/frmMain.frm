...
Begin VB.Form frmMain 
      ...
      Begin MSComctlLib.ImageList imlStatus 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":1B61
               Key             =   "RUN"
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":20FB
               Key             =   "NOT"
            EndProperty
            BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":2695
               Key             =   "STOP"
            EndProperty
            BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":2C2F
               Key             =   "LST"
            EndProperty
            BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":34C1
               Key             =   "ITM"
            EndProperty
            BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":361B
               Key             =   "ERR"
            EndProperty
            BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":3775
               Key             =   "NOF"
            EndProperty
            BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":38CF
               Key             =   "ON"
               Object.Tag             =   "OFF"
            EndProperty
            BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":41A9
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   Begin FPSpread.vaSpread spdOrder 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":7093
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdWork 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":902B
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
...
'    End If
    ...
    Select Case UCase(gHOSP.MACHNM)
        Case "PPC125":          Call Phase_Serial_PPC125
    ...
    With frmMain
        ...
        Select Case gHOSP.HOSPNM
            Case "YUMIZEN"
                ...
                mOrder.Order = strItems
            Case Else
                strItems = GetEquipExamCode_PPC125(gHOSP.MACHCD, pBarNo, intRow)
                mOrder.Order = strItems
        End Select
...
Private Sub SendTCPData(ByVal pSendData As Variant)
...
Private Sub SerialRcvData_PPC125()
    '장비 수신 변수
    Dim strRcvBuf       As String   '수신한 Data
    Dim strType         As String   '수신한 Record Type
    Dim strBarno        As String   '수신한 바코드번호
    ...
    With frmMain
        For intCnt = 1 To UBound(strRData)
            ...
            Select Case strType
                ...
                '2P|1||3||9876543210|||N||||||0^Y

                Case "P"    '## Patient
                    '2P|1|||||||N||||||0^Y
                    '2P|1||3||190918003|||N||||||0^Y
                    
                    strBarno = mGetP(strRcvBuf, 6, "|")
                    
                    If strBarno = "" Then
                        strBarno = "오더없음_" & Format(Now, "hhmmss")
                    End If
                    
                    blnSameBar = False
                    
                    For i = 1 To spdOrder.MaxRows
                        If Trim(GetText(spdOrder, i, colBARCODE)) = Trim(strBarno) And Trim(GetText(spdOrder, i, colEXAMDATE)) <> "" Then
                            blnSameBar = True
                            Exit For
                        End If
                    Next
                    ...
                    If blnSameBar = False Then
                        'If Trim(strBarno) <> Trim(strOldBarno) Then
                            ...
                            '-- 결과환자정보
                            Call SetPatInfo_PPC125(strBarno, gHOSP.RSTTYPE)
                        'End If
                    End If
                ...
                Case "O"
'''                    '3O|1|^0001^^^|SAGES 200^||S|20190820091013|||||||||Serum||||||||||O
'''                    '3O|1|1^0001^^^|SAGES 200^||S|20190902115639|||||||||1||||||||||O
...
'''                        'strOldBarno = strBarno
'''
'''                        '먼저 그리드에 같은 Seq 가 있는지 찾는다.
'''                        'blnSameSeq = False
'''                        blnSameSeq = True
'''                        For J = 1 To .spdOrder.MaxRows
'''                            If Val(GetText(spdOrder, J, colSEQNO)) = Val(strSeq) Then
'''                                mResult.RsltSeq = GetText(spdOrder, J, colSAVESEQ)
'''                                If GetText(spdOrder, J, colSAVESEQ) = "" Then
'''                                    blnSameSeq = False
'''                                    Exit For
'''                                End If
'''                            End If
...
'''                    strState = "O"
                ...
                Case "L"
                    ...
                    Call spdOrder_Click(colBARCODE, gRow)
...
Private Sub Phase_Serial_PPC125()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
        ...
        Select Case intPhase
            Case 1      '## Estabilshment Phase
                Select Case BufChar
                    ...
                    Case ACK
                        If strState = "Q" Then
                            Call SendOrder_PPC125
                            'Call SendOrder_PPC125_Batch
                        End If
                End Select
            Case 2      '## Transfer Phase
                Select Case BufChar
...
Private Sub SendOrder_PPC125_Batch()
    Dim strOutput   As String     '송신할 데이터
    Dim intRow      As Integer
    Dim intDestRow  As Integer
    Dim blnOrder    As Boolean
        ...
        Select Case intSndPhase
            ...
            Case 3  '## No Order
                ...
                If mOrder.IsSending = False Then
                    ...
                    strOutput = "O|1|" & mOrder.BarNo & "||" & "^^^AST" & "|R||||||A||||" & "Serum"
                    If Len(strOutput) > 230 Then
                        mOrder.IsSending = True
                        mOrder.Order = Mid$(strOutput, 231)
                        strOutput = intFrameNo & Mid$(strOutput, 1, 230) & vbCr & ETB
...
Private Sub SendOrder_YUMIZEN()
    ...
    Select Case intSndPhase
        ...
        Case 3  '## Order
            ...
            '## 최초 보낼때
            If mOrder.IsSending = False Then
                strOutput = "O|" & mOrder.SendCnt & "|" & mOrder.BarNo & "||" & mOrder.Order & "|R|" & Format(Now, "yyyymmddhhmmss") & "|||||N||||||||||||||Q|||||"
                If Len(strOutput) > 230 Then
                    mOrder.IsSending = True
                    mOrder.Order = Mid$(strOutput, 231)
                    strOutput = intFrameNo & Mid$(strOutput, 1, 230) & vbCr & ETB
...
'1H|\^&|||SAGES^3.6^11052213||||||||E-1394-97|20190820104300
'2P|1|||||||N||||||0^Y
'3O|1|^0001^^^|SAGES 200^||S|20190820104300|||||||||Serum||||||||||O
Private Sub SendOrder_PPC125()
    Dim strOutput   As String     '송신할 데이터
    Dim intRow      As Integer
    Dim intCnt      As Integer
    Dim strOrder    As String
    ...
    Select Case intSndPhase
        Case 1  '## Header
                                    '1H|\^&[CR][ETX]E5[CR][LF] PPC 125 [ACK]
        ...
        Case 2  '## Patient
                                    '2P|1|||||||||||||^[CR][ETX]E9[CR][LF] PPC 125 [ACK]
                                    '<STX><FN>P|1||123456||ZhangDongdong|||M||||||40^Y<CR><ETX>[Checksum] <CR><LF>
            'strOutput = intFrameNo & "P|1||" & mOrder.PID & "|||||||||||^" & vbCr & ETX
            'strOutput = intFrameNo & "P|1||" & mOrder.PID & "|||||||||||^" & vbCr & ETX
            
            'P|1||||ZhangDongdong|||M||||||40^Y
            strOutput = intFrameNo & "P|1||" & mOrder.BarNo & "^" & mOrder.BarNo & "||" & mOrder.BarNo & "|||||||||^" & vbCr & ETX
            intSndPhase = 3
            intFrameNo = intFrameNo + 1

        Case 3  '## Order
                           '3O|1|3977777^4^1^1^N||^^^AST|R|20090119100534||||||||
                           
                           '3O|1|128123^1^1^1^N||^^^ALT|R|20090119100417|||||||| |1||||||||||O[CR][ETX]F9[CR][LF]
            
            mOrder.SendCnt = mOrder.SendCnt + 1
            strOrder = mGetP(mOrder.Order, mOrder.SendCnt, "\")
            
            '## 최초 보낼때
            If mOrder.IsSending = False Then
                
                strOutput = "O|" & mOrder.SendCnt & "|" & mOrder.Seq & "^" & mOrder.BarNo & "^^^N||" & strOrder & "|R|" & Format(Now, "yyyymmddhhmmss") & "|||||||||||||||||||"
                If Len(strOutput) > 230 Then
                    mOrder.IsSending = True
                    mOrder.Order = Mid$(strOutput, 231)
                    strOutput = intFrameNo & Mid$(strOutput, 1, 230) & vbCr & ETB
            ...
            'mOrder.Order = Mid(mOrder.Order, Len(strOrder) + 1)
...
'    strText = Replace(strText, vbLf, "")
    ...
    If Len(pBuffer) > 0 Then
        ...
        Select Case UCase(gHOSP.MACHNM)
            Case "PPC125":          Call Phase_TCP_PPC125
            Case "PPC300N":         Call Phase_TCP_PPC300N
