...
'    sBirth = ""
    sBirth = sBirth & Mid(asPNRN, 1, 2) '& "/" & Mid(asPNRN, 5, 2) & "/" & Mid(asPNRN, 7, 2)
...
Public Sub CalAgeSex1(ByRef asBirth As String, ByRef asCurDate As String)
    Dim sBirth As String
    Dim sStart As String
    ...
    If Mid(asBirth, 1, 1) = "_" Or Mid(asBirth, 1, 1) = "" Then
        Exit Sub
    End If
        
    sBirth = SeperatorCls(asBirth)
    sBirth = Left(sBirth, 4) & "/" & Mid(sBirth, 5, 2) & "/" & Mid(sBirth, 7, 2)
