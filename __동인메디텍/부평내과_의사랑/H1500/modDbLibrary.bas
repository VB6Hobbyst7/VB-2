...
'-- 주민번호로 나이와 성별을 찾아온다.
'-- 인수 :  주민번호,현재날자
Public Function CalAgeSex(ByRef asPNRN As String, ByVal asCurDate As String) As String
...
Function GetSampleList(ByVal pFrDT As String, ByVal pToDT As String) As Integer
    ...
    '날자로 환자정보 불러오기
          'SQL = "SELECT DiSTINCT CHARTNO, PATNAME, PATSEX, PATAGE,COMPANY,HOSPCODE,PATJUMIN,PATNO,COMMDATE,EXAMNO,EXAMID,IOFLAG, BARCODE   "
          SQL = "SELECT DiSTINCT CHARTNO, PATNAME, PATSEX, PATAGE,COMPANY,HOSPCODE,PATJUMIN,PATNO,BARCODE,PATJUMIN  "
    SQL = SQL & vbCrLf & "  FROM PAT_RES "
    SQL = SQL & vbCrLf & " WHERE EXAMTYPE = 'H' "
    ...
    Do While Not RS.EOF
        ...
        strBarcode = Trim(RS.Fields(8).Value & "")
        strDOB = Trim(RS.Fields(9).Value & "")
        ...
        SetText frmInterface.vasID, strBarcode, asRow, colBarcode
        
        If strDOB <> "" And Len(strDOB) >= 13 Then
            strDOB = CalAgeSex(strDOB, Format(Now, "yyyy/mm/dd"))
        End If
        
        SetText frmInterface.vasID, strDOB, asRow, colPos    '주민번호 ==> DOB
...
'        SetText frmInterface.vasID, Trim(RS.Fields(7).Value & ""), asRow, 15
        ...
        i = GetOrderCBCDIFF(gEquip, strBarcode)
        
        If i = 1 Then
            SetText frmInterface.vasID, "C", asRow, colRack
        ElseIf i = 2 Then
            SetText frmInterface.vasID, "CD", asRow, colRack
        Else
            SetText frmInterface.vasID, "", asRow, colRack
        End If
...
Function GetOrderCBCDIFF(argEquipCode As String, argPID As String) As Integer
    Dim rs_svr          As ADODB.Recordset
    Dim GetOrderCBC     As String
    Dim GetOrderDIF     As String
    ...
    Set rs_svr = cn.Execute(SQL)
    Do Until rs_svr.EOF
        If Trim(rs_svr.Fields(0)) = "b1010" Or Trim(rs_svr.Fields(0)) = "b1060" Or Trim(rs_svr.Fields(0)) = "b1050" Then
            GetOrderCBC = "CBC"
        End If
        '                               neu                               eo                           baso
        If Trim(rs_svr.Fields(0)) = "b10191" Or Trim(rs_svr.Fields(0)) = "b109021" Or Trim(rs_svr.Fields(0)) = "b109021" Then
            GetOrderDIF = "DIF"
        End If
    ...
    If GetOrderCBC <> "" And GetOrderDIF <> "" Then
        GetOrderCBCDIFF = 2
    ElseIf GetOrderDIF <> "" Then
        GetOrderCBCDIFF = 2
    ElseIf GetOrderCBC <> "" Then
        GetOrderCBCDIFF = 1
    Else
        GetOrderCBCDIFF = 0
    End If
