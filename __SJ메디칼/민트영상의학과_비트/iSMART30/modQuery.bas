...
'
'SELECT L.LABSERIAL, L.LABATTEND as 내원번호, L.LABCHTNUM as 챠트번호, L.LABODRDTE as 접수일자, M.MANADMFOR as 입외,
'       M.MANRESNUM as 주민번호, M.MANPATNAM as 이름, L.LABINSNUM as 처방순서,L.LABSMPNAM as 검체명, L.LABBARCOD as 바코드번호, L.LABODRCOD as ITEM, L.LABODRSTP as SEQ
'  FROM ME_LABDAT L, ME_DAT D, ME_MAN M
' WHERE L.LABODRDTE between  '2020-12-21' AND '2020-12-21'
'   AND L.LABKEYNUM = D.DATKEYNUM
'   AND L.LABATTEND = D.DATATTEND
'   AND L.LABATTEND = M.MANATTEND
'   AND L.LABCHTNUM = D.DATCHTNUM
'   AND L.LABCHTNUM = M.MANCHTNUM
'   AND L.LABODRDTE = D.DATODRDTE
'   AND L.LABODRCOD IN ('B1050','B1040','B1010','B1010A','B1020','B1020A','B1020B','B1020C','B1060','B1091-C','B1091F','B1091E','B1091-B','B1091-A','B1091D','B1091C','B1091B-1','B1060B','B1091A','B1060C','RDW','B1220','B1060A')
'   AND (L.LABCANCEL = '' OR L.LABCANCEL IS NULL)
'   AND (L.LABRESULT = ''  OR L.LABRESULT IS NULL)
'   AND L.LABENDDEP < '3'
' ORDER BY L.LABODRDTE, L.LABATTEND, L.LABBARCOD desc, L.LABINSNUM, L.LABODRCOD
'
    ...
    SQL = SQL & "   AND L.LABKEYNUM = D.DATKEYNUM"
    SQL = SQL & "   AND L.LABATTEND = D.DATATTEND"
    SQL = SQL & "   AND L.LABATTEND = M.MANATTEND"
    SQL = SQL & "   AND L.LABCHTNUM = D.DATCHTNUM"
    SQL = SQL & "   AND L.LABCHTNUM = M.MANCHTNUM"
    SQL = SQL & "   AND L.LABODRDTE = D.DATODRDTE"
...
'''    SQL = SQL & "   AND L.LABKEYNUM = D.DATTTEND                   " & vbCrLf      '-- 내원번호
'''    SQL = SQL & "   AND LKEYNUM                   " & vbCrLf      '-- 테이블연결키값
'''    SQL = SQL & "   AND L.LABATTEND = D.DATATTEND                   " & vbCrLf      '-- 내원번호
''''    SQL = SQL & "   AND L.LABATTEND = M.MANA.LABCHTNUM = D.DATCHTNUM                   " & vbCrLf      '-- 챠트번호
''''    SQL = SQL & "   AND L.LABCHTNUM = M.MANCHTNUM                   " & vbCrLf      '-- 챠트번호
'''    SQL = SQL & "   AND L.LABODRDTE = D.DATODRDTE                   " & vbCrLf      '-- 처방일자
