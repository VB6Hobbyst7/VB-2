    ...
    Select Case gOCS
        ...
        Case "KOMAIN"
         ...
         Case "BIT"
            ...
            SQL = ""
            SQL = SQL & " SELECT DISTINCT R.RESOCMNUM AS 내원번호, O.OCMCHTNUM AS 챠트번호, SUBSTRING(O.OCMACPDTM,1,8) AS 접수일자, P.PBSPATNAM AS 이름, P.PBSSEXTYP AS 성별" & vbCrLf
            SQL = SQL & "   FROM RESINF AS R, OCMINF AS O, PBSINF AS P, LABMST AS E, ODRINF AS W" & vbCrLf
            SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & pFrom & "000000" & "' AND '" & pTo & "235959" & "'" & vbCrLf
...
'            End If
            SQL = SQL & " ORDER BY 접수일자, 차트번호, 내원번호"
...
'            SQL = SQL & "   AND a.STS_CD    = b.STS_CD " & vbCr
         ...
         Case "BIT"
           ...
           ' SQL = SQL & " ORDER BY L.LABODRDTE, L.LABATTEND, L.LABBARCOD Desc, L.LABINSNUM, L.LABODRCOD "
            ...
            SQL = ""
            SQL = SQL & " SELECT DISTINCT E.LABCOD AS ITEM " & vbCrLf
            SQL = SQL & "   FROM RESINF AS R, OCMINF AS O, PBSINF AS P, LABMST AS E, ODRINF AS W" & vbCrLf
            'SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & pFrom & "000000" & "' AND '" & pTo & "235959" & "'" & vbCrLf
            SQL = SQL & " WHERE O.OCMCOMSTT NOT IN ('CN', 'CR', 'VC')" & vbCrLf
            SQL = SQL & "   AND R.RESLABCOD IN (" & gAllTestCd & ")" & vbCrLf
            SQL = SQL & "   AND SUBSTRING(O.OCMACPDTM,1,8) = '" & strRegDate & "'" & vbCr
            SQL = SQL & "   AND O.OCMCHTNUM = '" & strChartNo & "'" & vbCr
            SQL = SQL & "   AND R.RESOCMNUM = O.OCMNUM" & vbCrLf
            SQL = SQL & "   AND O.OCMCHTNUM = P.PBSCHTNUM" & vbCrLf
            SQL = SQL & "   AND R.RESOCMNUM = W.ODROCMNUM" & vbCrLf
            SQL = SQL & "   AND R.RESLABCOD *= W.ODRCOD" & vbCrLf
...
'''            'SQL = SQL & " ORDER BY L.LABODRDTE, L.LABATTEND, L.LABBARCOD Desc, L.LABINSNUM, L.LABODRCOD "
            ...
            SQL = ""
            SQL = SQL & " SELECT DISTINCT R.RESOCMNUM AS PID, O.OCMCHTNUM AS CHARTNO, SUBSTRING(O.OCMACPDTM,1,8) AS HOSPDATE, P.PBSPATNAM AS PNAME, R.RESLABCOD AS ITEM, P.PBSSEXTYP AS 성별" & vbCrLf
            SQL = SQL & "   FROM RESINF AS R, OCMINF AS O, PBSINF AS P, LABMST AS E, ODRINF AS W" & vbCrLf
            'SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & pFrom & "000000" & "' AND '" & pTo & "235959" & "'" & vbCrLf
            SQL = SQL & " WHERE O.OCMCOMSTT NOT IN ('CN', 'CR', 'VC')" & vbCrLf
            SQL = SQL & "   AND R.RESLABCOD IN (" & gAllTestCd & ")" & vbCrLf
