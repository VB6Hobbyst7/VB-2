   ...
   Begin TabDlg.SSTab stInterface 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin FPSpread.vaSpread vasRes 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":C28A
         End
         Begin FPSpread.vaSpread vasID 
            ...
            RetainSelBlock  =   0   'False
            ScrollBars      =   2
            SpreadDesigner  =   "frmInterface.frx":FFED
            UserResize      =   2
         End
         Begin VB.Label Label1 
            Appearance      =   0  'Æò¸é
...
Const colInNum = 3      '³»¿ø¹øÈ£
Const colBarcode = 4    'Ã­Æ®¹øÈ£(=¹ÙÄÚµå)

Const colSeqno = 5      'SeqNo
Const colRack = 6       'Rack
Const colPos = 7        'Pos

Const colPName = 8      'È¯ÀÚ¸í
Const colPSex = 9       '¼ºº°
Const colOCnt = 10
Const colRCnt = 11
Const colState = 12
...
Private Sub GetWorkList(ByVal pFrDt As String, ByVal pToDt As String, Optional pBarNo As String)
    ...
    '-- BIT
          SQL = " SELECT DISTINCT '', SUBSTRING(O.OCMACPDTM,1,8)  AS Á¢¼öÀÏ½Ã,'Áø·á', R.RESOCMNUM AS ³»¿ø¹øÈ£, O.OCMCHTNUM AS Â÷Æ®¹øÈ£,  P.PBSPATNAM AS È¯ÀÚ¸í, '','', P.PBSBIRDTE  AS »ý³â¿ùÀÏ,'Á¢¼ö'"
    ...
    SQL = SQL & " WHERE O.OCMACPDTM BETWEEN '" & Format(strDate, "yyyymmdd000000") & "' AND '" & Format(strDate, "yyyymmdd235959") & "'"
    SQL = SQL & "   AND O.OCMCOMSTT NOT IN ('CN', 'CR', 'VC')"
    SQL = SQL & "   AND R.RESLABCOD IN (" & gAllExam & ")"
    SQL = SQL & "   AND (R.RESREPTYP IS NULL OR R.RESREPTYP <> 'F') "          '--  'I':Áß°£ 'F' ¿Ï·á"
    ...
    SQL = SQL & " ORDER BY Á¢¼öÀÏ½Ã, ³»¿ø¹øÈ£"
    Res = GetDBSelectVas(gServer, SQL, vasWorkList, vasWorkList.DataRowCnt + 1)
    
    
    
'          SQL = "SELECT Distinct A.REQUEST_DATE, A.EXAM_NO, A.CHART_NO, A.PERSON_NAME, A.PERSONAL_ID "
          
          SQL = "SELECT Distinct '', A.REQUEST_DATE AS Á¢¼öÀÏ½Ã, '°ËÁø', A.EXAM_NO AS ³»¿ø¹øÈ£, A.CHART_NO AS Â÷Æ®¹øÈ£, A.PERSON_NAME AS È¯ÀÚ¸í, A.PERSONAL_ID "
    ...
    Res = GetDBSelectVas(gServer, SQL, vasWorkList)
    ...
    Do Until RS.EOF
        ...
        strDate = Mid(RS("REQUEST_DATE"), 1, 4) & "-" & Mid(RS("REQUEST_DATE"), 5, 2) & "-" & Mid(RS("REQUEST_DATE"), 7, 2)
        
        SetText vasWorkList, "1", intRow, colCheckBox
        SetText vasWorkList, strDate, intRow, colOrdDate
        SetText vasWorkList, Trim(RS.Fields("EXAM_NO")) & "", intRow, colSeqno
        SetText vasWorkList, Trim(RS.Fields("CHART_NO")) & "", intRow, colBarcode
        SetText vasWorkList, "0", intRow, colRack
        SetText vasWorkList, txtNum, intRow, colPos
        SetText vasWorkList, Trim(RS.Fields("PERSON_NAME")) & "", intRow, colPName
        
        If Mid(RS.Fields("PERSONAL_ID") & "", 8, 1) = " " Then
            SetText vasWorkList, "", intRow, colPSex
        Else
            Select Case Mid(RS("PERSONAL_ID") & "", 7, 1)
                Case 1, 3
                    SetText vasWorkList, "M", intRow, colPSex
                Case 2, 4
                    SetText vasWorkList, "F", intRow, colPSex
            End Select
        End If
...
'       - pBarNo : ¹ÙÄÚµå¹øÈ£
'-----------------------------------------------------------------------------'
Private Sub GetOrder(ByVal pBarNo As String, ByVal pDate As String, ByVal pSeqNo As String)

'                Call SetText(vasID, strDate, gRow, colOrdDate)
'                Call SetText(vasID, strSeq, gRow, colSeqNo)
    ...
    Call SetText(vasID, pDate, intRow, colOrdDate)
    Call SetText(vasID, pSeqNo, intRow, colSeqno)
    ...
    gOrderExam = GetOrderExamCode(gEquip, pSeqNo, pDate)
...
'                Call SetText(vasID, strSeq, gRow, colSeqNo)
                ...
                Call GetOrder(strBarno, strDate, strSeq)
...
'''                    '-- LDL =  TC-HDL-(TG/5)
'''                    If strTC <> "" And strHDL <> "" And strTG <> "" Then
'''                        '½ÇÁ¦ Ã¤³ÎÀº "14"¹øÀÌ³ª °è»ê°ªÀ¸·Î Ã³¸®ÇÏ¿© ÀÓ½Ã Ã¤³ÎÀ» ºÎ¿©ÇÑ´Ù.
'''                        strIntBase = "94"
'''                        strItemX = itemX: strItemTag = itemX.Tag
'''                        itemX = "1011": itemX.Tag = "94"
'''
'''                        intCol = 24
'''                        strResult = Format(strTC - strHDL - (strTG / 5), "##0")
'''                        strTG = "": strHDL = "": strTC = ""
'''                        blnItemX = True
'''                        GoTo Rst
'''                    End If
'''
'''                    '-- GLB = TP - ALB
'''                    If strTP <> "" And strALB <> "" Then
'''                        strIntBase = "91"
'''                        strItemX = itemX: strItemTag = itemX.Tag
'''                        itemX = "1051": itemX.Tag = "91"
'''                        intCol = 10
'''                        strResult = Format(strTP - strALB, "##.0")
'''                        strChannel = "91"
'''                        strTP = "": strALB = ""
'''
'''                        blnItemX = True
...
Function SetLocalDB(ByVal asRow1 As Long, ByVal asRow2 As Long, asSend As String, Optional asEquipResult As String = "")
    ...
    SQL = "DELETE FROM PATRESULT " & vbCrLf & _
          "WHERE EXAMDATE = '" & Format(Trim(GetText(vasID, asRow1, colOrdDate)), "yyyymmdd") & "' " & vbCrLf & _
          "  AND EQUIPNO = '" & gEquip & "' " & vbCrLf & _
          "  AND BARCODE = '" & Trim(GetText(vasID, asRow1, colSeqno)) & "' " & vbCrLf & _
...
'    SQL = SQL & "'" & Trim(Format(dtpExamDate.Value, "YYYYMMDD")) & "', "
    ...
    SQL = SQL & "'" & gEquip & "', "
    SQL = SQL & "'" & Trim(GetText(vasID, asRow1, colSeqno)) & "', "
...
Private Sub vasID_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    lsID = Trim(GetText(vasID, Row, colSeqno))
    ...
    SQL = "SELECT EQUIPCODE, EXAMCODE, EXAMNAME, EQUIPRESULT, RESULT, SEQNO, SENDFLAG " & vbCrLf & _
          ...
          "  AND EXAMDATE = '" & Format(Trim(GetText(vasID, Row, colOrdDate)), "yyyymmdd") & "' " & vbCrLf & _
...
'            SQL = SQL & "   AND POSNO     = '" & Trim(GetText(vasID, iRow, colPos)) & "' " & vbCrLf
            SQL = SQL & "   AND EXAMDATE  = '" & Format(lblDate.Caption, "YYYYMMDD") & "' "
