...
'-- QC 결과 리스트 조회
Public Function GetQCResult_Detail(ByVal pEqpCD As String, ByVal pQCChannel As String, ByVal pAnalyID As String, ByVal pResult As String) As String
    Dim i               As Integer
...
On Error GoTo Rst
    ...
    If strQCTmp <> "" Then
        ...
        strQCVal = strQCVal & "1" & "|"             ' run           // 1,2,3,4
        ...
        strQCVal = strQCVal & strQCTmp              ' MethodID, InstrumentID, ReagentID, UnitID, TemperatureID
...
'                            SetText frmWorkList.spdWork, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
         ...
         Case "EASYS"
            ...
            SQL = SQL & " GROUP BY a.ACC_YMD, a.RECEPT_NO, a.PTNT_NO, a.PTNT_NM " & vbCr
            ...
            If Not RS.EOF = True And Not RS.BOF = True Then
                Do Until RS.EOF
                    With SPD
                        ...
                        If Trim(RS.Fields("PID")) & "" = "01" Then 'Serum
                            mOrder.SPCCD = "1"
                        ElseIf Trim(RS.Fields("PID")) & "" = "04" Then 'Urine
                            mOrder.SPCCD = "2"
                        Else
