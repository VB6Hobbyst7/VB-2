...
On Error GoTo RST
    ...
    SQL = SQL & " GROUP BY a.RECEIPTDATE, a.RECEIPTTIME, a.SPECIMENNUM, a.RECEIPTNO, a.IPDOPD, a.PTNO, a.SNAME " & vbCr
    SQL = SQL & " ORDER BY a.RECEIPTDATE, a.RECEIPTTIME, a.PTNO "
        ...
        Do Until RS.EOF
            With SPD
                ...
                For i = 1 To SPD.DataRowCnt
                    ...
                    strPatID = GetText(SPD, i, colPID)
                    If Trim(RS("HOSPDATE")) = strHospDate And Trim(RS("PID")) = strPatID Then
                        blnSame = True
                    End If
                Next
...
Public Sub GetWorkList_EHEALTH(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
'    SQL = SQL & " ORDER BY R.SPCMNO                                             " & vbCr
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       b.OBODORDT          as HOSPDATE " & vbCr    '입력일
    'SQL = SQL & "     , a.APATMRNO          as BARCODE  " & vbCr                 '등록번호
    SQL = SQL & "     , b.OBODMRNO          as BARCODE  " & vbCr    '등록번호
    SQL = SQL & "     , b.OBODCASE          as CHARTNO  " & vbCr    '내원번호
    SQL = SQL & "     , b.OBODORNO          as PID      " & vbCr    'ORDER NUMBER
    'SQL = SQL & "     , b.OBODORSQ      as ORDERCODE"                 'ORDER SEQUENCE
    SQL = SQL & "     , b.OBODIOGB          as INOUT    " & vbCr    '입/외 I=입원/O=외래
    SQL = SQL & "     , a.APATNAME          as PNAME    " & vbCr    '환자성명
    SQL = SQL & "     , a.APATPSEX          as SEX      " & vbCr    '성별(M/F)
    SQL = SQL & "     , a.APATJMN1          as JUMIN1   " & vbCr    '주민번호(년월일)
    SQL = SQL & "     , a.APATJMN2          as JUMIN2   " & vbCr    '주민번호(년월일)
    SQL = SQL & "     , COUNT(b.OBODCODE)   as CNT      " & vbCr
    ...
    SQL = SQL & "   AND b.OBODORDT between '" & pFrom & "' AND '" & pTo & "'" & vbCr    '입력일
    SQL = SQL & "   AND RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) IN (" & gAllTestCd & ") " & vbCr    '검사코드 + '|' + OBSUSUBC
    SQL = SQL & "   AND b.OBODSTAT = 'AC' " & vbCr                                      '필수 기본 = 'OE', 채혈시 = 'AC'
    SQL = SQL & " GROUP BY b.OBODORDT, b.OBODMRNO, b.OBODCASE, b.OBODORNO, b.OBODIOGB, a.APATNAME, a.APATPSEX, a.APATJMN1, a.APATJMN2" & vbCr
    SQL = SQL & " Order by b.OBODORDT, b.OBODMRNO" & vbCr
...
'            SQL = SQL & "   AND L.LABCHTNUM  = M.MANCHTNUM                  " & vbCr                    '-- 챠트번호
        ...
        Case "EHEALTH"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT "
            SQL = SQL & "       RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) AS ITEM" & vbCr
            SQL = SQL & "  FROM ABPATMST a"                 '환자기본정보
            SQL = SQL & "      ,OBODRMTM b"                 '처방내역 Table
            SQL = SQL & "      ,OBSURSTM c " & vbCr         '검사결과(수치결과) Table
            SQL = SQL & " WHERE a.APATMRNO = b.OBODMRNO " & vbCr                                '등록번호,고객번호
            ...
            SQL = SQL & "   AND b.OBODORSQ = c.OBSUORSQ " & vbCr                                'ORDER SEQUENCE
            SQL = SQL & "   AND (c.OBSURSLT IS NULL OR c.OBSURSLT = '')" & vbCr                 '검사결과
            SQL = SQL & "   AND b.OBODMRNO = '" & strBarcode & "'          " & vbCr
            SQL = SQL & "   AND RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) IN (" & gAllTestCd & ") " & vbCr    '검사코드 + '|' + OBSUSUBC
            SQL = SQL & "   AND b.OBODSTAT = 'AC' " & vbCr                                      '필수 기본 = 'OE', 채혈시 = 'AC'
...
On Error GoTo DBErr
    ...
    'SQL = SQL & "   AND a.PTNO     = '" & strPatID & "'" & vbCr
    SQL = SQL & "   AND b.LABCODE IN (" & gAllTestCd & ")   " & vbCr
            ...
            With SPD
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        '-- RECEIPTNO
                        gArrEQP(intCol - colSTATE, 17) = Trim(RS.Fields("CHARTNO")) & ""
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_EHEALTH(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    SQL = SQL & " ORDER BY R.SPCMNO                                 " & vbCr
    ...
    SQL = SQL & "     , b.OBODCASE          as CHARTNO  " & vbCr    '내원번호
    SQL = SQL & "     , b.OBODORNO          as PID      " & vbCr    'ORDER NUMBER
    SQL = SQL & "     , b.OBODORSQ          as ORDCODE  " & vbCr    'ORDER SEQUENCE
    SQL = SQL & "     , b.OBODIOGB          as INOUT    " & vbCr    '입/외 I=입원/O=외래
    SQL = SQL & "     , a.APATNAME          as PNAME    " & vbCr    '환자성명
    SQL = SQL & "     , a.APATPSEX          as SEX      " & vbCr    '성별(M/F)
    SQL = SQL & "     , a.APATJMN1          as JUMIN1   " & vbCr    '주민번호(년월일)
