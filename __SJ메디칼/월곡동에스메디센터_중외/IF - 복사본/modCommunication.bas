                ...
                Case "3"
                    For i = 1 To .spdOrder.DataRowCnt
                        ...
                        If GetText(frmMain.spdOrder, i, colCHECKBOX) = "1" Then
                            mResult.BarNo = Trim(GetText(frmMain.spdOrder, i, colBARCODE))
                            mResult.PatNo = Trim(GetText(frmMain.spdOrder, i, colPID))
                            mResult.ChartNo = Trim(GetText(frmMain.spdOrder, i, colCHARTNO))
                            intRow = i
                            Exit For
                        End If
                    Next i
...
'                   3 : 체크된것중 제일 위에 것
'-----------------------------------------------------------------------------'
Public Sub SetPatInfo_CA270W(ByVal pBarno As String, ByVal pType As String)
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 오더코드찾기
                'If Trim(strOrdCd) = "" Then
                    strSubCode = ""
                    strOrdCd = ""
                    
                    SQL = ""
                    SQL = SQL & "Select ReceiptNo,OrderCode From SLA_LabResult " & vbCr
                    SQL = SQL & " Where PTNO = '" & strPatID & "' " & vbCr
                    SQL = SQL & "   AND RECEIPTDATE = '" & strHospDate & "' " & vbCr
                    SQL = SQL & "   And LabCode = '" & strTestCd & "'       " & vbCr
                    ...
                    If Not RS.EOF = True And Not RS.BOF = True Then
                        Do Until RS.EOF
                            strSubCode = Trim(RS("ReceiptNo") & "")
                            strOrdCd = Trim(RS("OrderCode") & "")
                            
                            Call SetSQLData("오더찾기", strSubCode & "," & strOrdCd & "," & strTestCd, "A")
                ...
                If strSubCode = "" And strOrdCd = "" Then
                    SQL = ""
                    SQL = SQL & "Select ReceiptNo,OrderCode,LabCode From SLA_LabResult " & vbCr
                    SQL = SQL & " Where PTNO = '" & strPatID & "' " & vbCr
                    SQL = SQL & "   AND RECEIPTDATE = '" & strHospDate & "' " & vbCr
                    
                    Select Case strTestCd
                    Case "B1091N1", "LY%", "LYM%"   'LYM%
                            strTestCd = "('B1091N1','LY%','LYM%')"
                    Case "B1091N2", "MID%"          'MXD%
                            strTestCd = "('B1091N2','MID%')"
                    Case "B1091N3", "SEG"           'GRA%
                            strTestCd = "('B1091N3','SEG')"
                    Case Else
                            strTestCd = "('" & strTestCd & "')"
                    End Select
                    
                    SQL = SQL & "   And LabCode IN " & strTestCd & vbCr
                ...
                If strSubCode <> "" And strOrdCd <> "" And strTestCd <> "" Then
                    '-- 결과저장
                          SQL = "Update SLA_LabResult                       " & vbCr
                    SQL = SQL & "   Set Result      = '" & sResult & "'     " & vbCr
                    SQL = SQL & "      ,NormalFlag  = '0'                   " & vbCr
                    ...
                    SQL = SQL & "      ,ResultTime  = '" & Trim(Format(Time, "HH:MM:SS")) & "'" & vbCr
                    'SQL = SQL & " Where SPECIMENNUM = '" & strBarcode & "'  " & vbCr
                    SQL = SQL & " Where ReceiptNo = '" & strSubCode & "' " & vbCr
                    SQL = SQL & "   AND RECEIPTDATE = '" & strHospDate & "' " & vbCr
                    If strOrdCd <> "" Then
...
Function SaveTransData_BRAIN(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim blnSave         As Boolean
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        strExamDate = Trim(GetText(.spdOrder, argSpcRow, colEXAMDATE))
        strHospDate = Trim(GetText(.spdOrder, argSpcRow, colHOSPDATE))
        strHospDate = Mid(strHospDate, 1, 8)
        strBarcode = Trim(GetText(.spdOrder, argSpcRow, colBARCODE))
        strPatID = Trim(GetText(.spdOrder, argSpcRow, colPID))
        strChartNo = Trim(GetText(.spdOrder, argSpcRow, colCHARTNO))
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                '-- 결과저장
'                SQL = ""
'                SQL = SQL & "UPDATE AspsLabws" & vbCr
'                SQL = SQL & "   Set sLabws_result   = '" & sResult & "'     " & vbCr
'                SQL = SQL & " Where sLabws_date     = '" & strHospDate & "' " & vbCr
'                SQL = SQL & "   And sLabws_cnt      = '" & strBarcode & "'  " & vbCr
'                SQL = SQL & "   And sLabws_momu     = '" & strTestCd & "'   " & vbCr
'                SQL = SQL & "   And sLabws_sCnt     = '" & strSubCode & "'  " & vbCr
...
'
'                SQL = ""
'                SQL = SQL & "UPDATE AspsLabw" & vbCr
'                SQL = SQL & "   Set sLabw_status    = '2'                   " & vbCr
'                SQL = SQL & " Where sLabw_date      = '" & strHospDate & "' " & vbCr
'                SQL = SQL & "   And sLabw_cnt       = '" & strBarcode & "'  " & vbCr
'                SQL = SQL & "   And sLabw_cham      = '" & strChartNo & "'  " & vbCr
'                SQL = SQL & "   And sLabw_status    < '2'                   " & vbCr
'                SQL = SQL & "   And sLabw_Hid       = '" & gHOSP.HOSPCD & "'" & vbCr
'
'                Call SetSQLData("상태변경", SQL, "A")
'                AdoCn.Execute SQL
                
                SQL = ""
                SQL = SQL & "UPDATE oslabws " & vbCr
                SQL = SQL & "   SET slabws_result = '" & sResult & "'" & vbCr
                SQL = SQL & "  FROM oslabws INNER JOIN oslabs     ON slabws_slab    = slabs_key             " & vbCr
                SQL = SQL & "                                    AND slabws_scnt    = slabs_cnt             " & vbCr
                SQL = SQL & "               INNER JOIN ospecislab ON specis_date    = slabws_date           " & vbCr
                SQL = SQL & "                                    AND specis_cnt     = slabws_cnt            " & vbCr
                SQL = SQL & "               INNER JOIN Ospecimen  ON speci_date     = specis_date           " & vbCr
                SQL = SQL & "                                    AND speci_seqno    = specis_seqno          " & vbCr
                SQL = SQL & "                                    AND speci_date     = '" & strHospDate & "' " & vbCr
                SQL = SQL & "                                    AND speci_seqno    = '" & strPatID & "'    " & vbCr
                SQL = SQL & "                                    AND slabs_rcode    IN ('" & strTestCd & "')" & vbCr
...
Function SaveTransData_BUSANUNIV(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim blnSave         As Boolean
...
'                SQL = SQL & "," & gHOSP.MACHCD
                ...
                With AdoCmd
                    .CommandTimeout = 15
                    .CommandText = "ext_all_objects.xpr_gaon_labrslt"
                    .CommandType = adCmdStoredProc
                    
                    
                    Set prm1 = .CreateParameter("P_BCD_SGTIN", adVarChar, adParamInput, 4000, strBarcode)      '바코드번호
                    .Parameters.Append prm1

                    Set prm2 = .CreateParameter("P_BCD_CODE", adVarChar, adParamInput, 4000, strTestCd)       '처방코드
                    .Parameters.Append prm2

                    Set prm3 = .CreateParameter("P_LABRSLTVAL", adVarChar, adParamInput, 4000, sResult)    '결과값
                    .Parameters.Append prm3
...
Function SaveTransData_EHEALTH(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
    Dim strTime         As String
    ...
    With frmMain
        ...
        For intRow = 1 To .vasTemp.DataRowCnt
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                SQL = SQL & "       OBSURSLT = '" & sResult & "'" & vbCr '수치형결과
                SQL = SQL & "     , OBSUENDT = '" & strDate & "'" & vbCr
                SQL = SQL & "     , OBSUENTM = '" & strTime & "'" & vbCr
                SQL = SQL & "     , OBSUENID = '" & gHOSP.USERID & "'" & vbCr
                SQL = SQL & " Where OBSUMRNO = '" & strBarcode & "'" & vbCr
                SQL = SQL & "   And OBSUCASE = '" & strChartNo & "'" & vbCr
                SQL = SQL & "   And OBSUORNO = '" & strPatID & "'" & vbCr
                SQL = SQL & "   And OBSUORSQ = '" & strOrdCd & "'" & vbCr
                SQL = SQL & "   And OBSUCODE = '" & Trim(mGetP(strTestCd, 1, "|")) & "'" & vbCr
                SQL = SQL & "   And OBSUSUBC = '" & Trim(mGetP(strTestCd, 2, "|")) & "'"
                
                
                'SQL = SQL & "   And RTRIM(c.OBSUCODE) + '|' + RTRIM(c.OBSUSUBC) = '" & strTestCd & "'" & vbCr
                
                Call SetSQLData("결과저장", SQL, "A")
                AdoCn.Execute SQL
                
                '--남자
                If Trim(mGetP(strTestCd, 2, "|")) = "001" Then
                    SQL = ""
                    SQL = SQL & "Update OBSURSTM Set "
                    SQL = SQL & "       OBSURSLT = '" & sResult & "'" & vbCr '수치형결과
                    SQL = SQL & "     , OBSUENDT = '" & strDate & "'" & vbCr
...
Function SaveTransData(ByVal argSpcRow As Integer) As Integer
    ...
    Select Case gEMR
        ...
        Case "BRAIN"
            SaveTransData = SaveTransData_BRAIN(argSpcRow)
        ...
        Case "BUSANUNIV"
            SaveTransData = SaveTransData_BUSANUNIV(argSpcRow)
        
        Case "EHEALTH"
            SaveTransData = SaveTransData_EHEALTH(argSpcRow)
