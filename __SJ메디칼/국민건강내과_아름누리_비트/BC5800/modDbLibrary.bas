...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    With frmInterface
        ...
        strGumNum = Trim(GetText(.vasID, argSpcRow, colGumNum))

        '-- Local에서 환자별로 결과값 가져오기
        ClearSpread .vasTemp
        
              SQL = "SELECT EQUIPCODE,EXAMCODE,RESULT,EQUIPRESULT,REFFLAG,PANICVALUE,DELTAVALUE,PSEX,SEQNO,PAGE,PID,RECENO " & vbCrLf
        SQL = SQL & "  FROM PATRESULT " & vbCrLf
        SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf                                           '장비코드
        SQL = SQL & "   AND EXAMDATE = '" & strDate & "'  " & vbCrLf                                        '검사일
        SQL = SQL & "   AND BARCODE = '" & lsID & "' " & vbCrLf       '바코드
        SQL = SQL & "   AND PID = '" & strInNum & "' " & vbCrLf       '내원번호
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                If strGumNum <> "" Then
                    GumEqpCd = ""
                    Call GetPrivateProfileString("eqpcd", strEqpCd, "", GumEqpCd, 100, App.Path & "\Interface.ini")
                    frmInterface.txtTemp = Trim(GumEqpCd)
...
'    strInNum = Trim(GetText(frmInterface.vasID, asRow, colInNum))
'    strGumNum = Trim(GetText(frmInterface.vasID, asRow, colGumNum))
'    If sBarcode = "" Then
'        Exit Function
'    End If
'
...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Function GetOrderExamCode(argEquipCode As String, argPID As String, _
                          Optional argOrdDt As String, Optional argInNum As String, Optional argGumNum As String) As String
    ...
    If Trim(argInNum) = "" And Trim(argGumNum) = "" Then
        Exit Function
    End If
    
    
    If Trim(argInNum) <> "" Then
              SQL = "SELECT R.ResLabCod AS EXAMCODE" & vbCrLf
        SQL = SQL & "  FROM DRBITPACK..RESINF AS R, DRBITPACK..OCMINF AS O, DRBITPACK..PBSINF AS P, DRBITPACK..LABMST AS E, DRBITPACK..ODRINF AS W" & vbCrLf
        SQL = SQL & " WHERE SUBSTRING(O.OCMACPDTM,1,8) = '" & argOrdDt & "'" & vbCrLf
        SQL = SQL & "   AND R.RESLABCOD IN (" & gAllExam & ") " & vbCrLf
        SQL = SQL & "   AND LTRIM(O.OcmChtNum) = '" & argPID & "'" & vbCrLf
        SQL = SQL & "   AND LTRIM(R.ResOcmNum) = '" & argInNum & "'" & vbCrLf
        SQL = SQL & "   AND R.RESOCMNUM = O.OCMNUM" & vbCrLf
        SQL = SQL & "   AND O.OCMCHTNUM = P.PBSCHTNUM" & vbCrLf
        SQL = SQL & "   AND R.RESOCMNUM = W.ODROCMNUM" & vbCrLf
        SQL = SQL & "   AND R.RESLABCOD = W.ODRCOD" & vbCrLf
    ...
    Else
              SQL = "SELECT Distinct B.EXAM_CODE AS EXAMCODE" & vbCrLf
        SQL = SQL & "  FROM MEDITOLISS..TOTAL A, MEDITOLISS..TOTRES B" & vbCrLf
        SQL = SQL + " WHERE A.REQUEST_DATE = '" & argOrdDt & "'" & vbCrLf
        SQL = SQL & "   And B.EXAM_CODE in (" & gAllExam & ") " & vbCrLf
        SQL = SQL & "   And A.CHART_NO = '" & argPID & "'" & vbCrLf
        SQL = SQL & "   And A.EXAM_NO = '" & argGumNum & "'" & vbCrLf
        SQL = SQL & "   AND B.EXAM_PART = 'H'" & vbCrLf
        '-- 저장미포함
        If frmInterface.chkSaveAll.Value = "0" Then
    ...
    SetRawData "[GetOrderExamCode]" & SQL
