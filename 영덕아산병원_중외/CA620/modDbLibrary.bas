...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                With cmdSQL
                    .CommandTimeout = 15
                    .CommandText = "setAfisInfo2"
                    .CommandType = adCmdStoredProc
                    
                    
                    Set prm1 = .CreateParameter("r_hocd", adChar, adParamInput, 2, gEquipCode)
                    .Parameters.Append prm1
                    
                    Set prm2 = .CreateParameter("r_paid", adChar, adParamInput, 8, lsPid)
                    .Parameters.Append prm2
                    
                    Set prm3 = .CreateParameter("r_spno", adChar, adParamInput, 14, lsID)
                    .Parameters.Append prm3
                    
                    Set prm4 = .CreateParameter("r_orcd", adChar, adParamInput, 7, strExamCd)
                    .Parameters.Append prm4
                    
                    Set prm5 = .CreateParameter("r_rslt", adVarChar, adParamInput, 30, sResult)
                    .Parameters.Append prm5
                    
                    Set prm6 = .CreateParameter("r_okfl", adChar, adParamInput, 1, "N")
                    .Parameters.Append prm6
                    
                    Set prm7 = .CreateParameter("r_maid", adChar, adParamInput, 7, strEqpCd)
                    .Parameters.Append prm7
                    
                    
                    Set prm8 = .CreateParameter("s_eryn", adVarChar, adParamOutput, 4000, 0)
                    .Parameters.Append prm8
                    
                    Set prm9 = .CreateParameter("s_retn", adVarChar, adParamOutput, 4000, 0)
                    .Parameters.Append prm9
                    
                    .Execute
                    
                    strErr = .Parameters("s_eryn").Value
                    If UCase(strErr) = "Y" Then
                        '-- 저장실패
                        strErr = ""
                        strErr = strErr & .Parameters("s_eryn").Value & ":" & .Parameters("s_eryn").Value & vbNewLine
                        strErr = strErr & "r_hocd : " & gEquipCode & vbNewLine
                        strErr = strErr & "r_paid : " & lsID & vbNewLine
                        strErr = strErr & "r_spno : " & lsChtid & vbNewLine
                        strErr = strErr & "r_orcd : " & strExamCd & vbNewLine
                        strErr = strErr & "r_rslt : " & sResult & vbNewLine
                        strErr = strErr & "r_okfl : " & "N" & vbNewLine
                        strErr = strErr & "r_maid : " & strEqpCd
                        
                        Call SetSQLData("저장오류", strErr)
                        Set cmdSQL = Nothing
                        SaveTransDataW = -1
                    Else
                        '-- 저장성공
...
Function GetGetEquipExamCode_CA620(argEquipCode As String, argPID As String, argRow As Long) As String
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Dim i As Integer
Dim sExamCode As String
...
Dim strIntBase As String
    ...
    GetGetEquipExamCode_CA620 = ""
    
    If Trim(argEquipCode) = "" Then
        Exit Function
    End If
    
    sBarcode = Trim(GetText(frmInterface.vasID, argRow, colBARCODE))   '2 샘플 바코드 번호
    ...
    GetGetEquipExamCode_CA620 = strExamCode
