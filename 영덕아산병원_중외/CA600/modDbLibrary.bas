...
'        SQL = SQL & "   AND POSNO = '" & Trim(GetText(.vasID, argSpcRow, colPOSNO)) & "' "                    'POS 번호(진료지원ID)
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If sResult <> "" Then
                ...
                If Res = 1 Then
                    '-- OrderCode 찾아오기
                    strOrderCd = GetSampleInfoORD_JWINFO(lsID, strEqpCd)
...
'      -- 테이블 사용
          SQL = "SELECT DISTINCT a.RECEIPTDATE, a.SPECIMENNUM, a.IPDOPD, a.RECEIPTNO, a.PTNO, a.SNAME, b.LABCODE, a.ORDERCODE"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetOrderExamCode = GetOrderExamCode & "'" & Trim(RS.Fields("LABCODE")) & "',"
                
                SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                SetText .vasID, Trim(RS.Fields("RECEIPTDATE")) & "", asRow, colHOSPDATE
                SetText .vasID, Trim(RS.Fields("SPECIMENNUM")) & "", asRow, colBARCODE
                SetText .vasID, Trim(RS.Fields("RECEIPTNO")) & "", asRow, colCHARTNO
                SetText .vasID, Trim(RS.Fields("PTNO")) & "", asRow, colPID
                SetText .vasID, Trim(RS.Fields("SNAME")) & "", asRow, colPNAME
...
'-- 검사자 정보 가져오기
Function GetSampleInfoORD_JWINFO(ByVal sBarcode As String, ByVal pLabCode As String) As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    Dim pFrDt   As String
...
'      -- 테이블 사용
          SQL = "SELECT DISTINCT a.ORDERCODE"
    SQL = SQL & vbCrLf & "  FROM SLA_LabMaster a,SLA_LabResult b "
    SQL = SQL & vbCrLf & " WHERE a.SPECIMENNUM = '" & sBarcode & "'"
    SQL = SQL & vbCrLf & "   AND a.RECEIPTNO = b.RECEIPTNO "
    SQL = SQL & vbCrLf & "   AND a.OrderCode = b.OrderCode "
    SQL = SQL & vbCrLf & "   AND b.LABCODE = '" & pLabCode & "'"
    SQL = SQL & vbCrLf & "   AND a.JSTATUS < '3'" & vbLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                GetSampleInfoORD_JWINFO = Trim(RS.Fields("ORDERCODE")) & ""
                RS.MoveNext
            Loop
