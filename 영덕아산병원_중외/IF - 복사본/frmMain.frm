...
'   기능 : 오더정보 전송
'-----------------------------------------------------------------------------'
Private Sub SendOrder_XT1800i(ByVal pFlag As Boolean)
    Dim strOutput   As String     '송신할 데이터
    
    
    If mOrder.Order = "" Then
    
    Else
        comEqp.Output = mOrder.Order
        SetRawData "[Tx]" & mOrder.Order
        mOrder.Order = ""
    End If
...
Private Sub SerialRcvData_XT1800i()
    Dim RS_L            As ADODB.Recordset
    Dim strRcvBuf       As String   '수신한 Data
    Dim strType         As String   '수신한 Record Type
    Dim strBarno        As String   '수신한 바코드번호
    ...
    Dim i               As Integer
    Dim strQCTemp       As String
    Dim strDisCode1     As String
    Dim strDisCode2     As String
    Dim strMode         As String
'    Dim strRst(1 To 32) As String       '결과 임시 저장
    ...
    With frmMain
        ...
        Case "R"
            ...
            Call GetOrder_XT1800i(strBarno, gHOSP.RSTTYPE)
            
            strState = "Q"
        
        Case "D"
            strDisCode2 = Mid(strRcvBuf, 2, 1)
            strBarno = Mid$(strRcvBuf, 33, 15)
            ...
            Select Case strDisCode2
                Case "1"
                    ...
                    strTubePos = Format$(Mid$(strRcvBuf, 68, 2), "#")
                ...
                Case "2"
                    For i = 1 To 32
                        strIntBase = Format(i, "00")
                        strResult = ""
                        Select Case i
                            Case 1  'WBC
                                strResult = Trim(Mid(strRcvBuf, 48, 5))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@@")), "0.00")
                                End If
                            Case 2  'RBC
                                strResult = Trim(Mid(strRcvBuf, 54, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@.@@")), "0.00")
                                End If
                            Case 3 To 7 'HGB, HCT, MCV, MCH, MCHC
                                strResult = Trim(Mid(strRcvBuf, 59 + (i - 3) * 5, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@@")), "0.00")
                                End If
                            Case 8  'PLT
                                strResult = Trim(Mid(strRcvBuf, 84, 4))
                                If strResult <> "" Then
                                    strResult = Trim(Val(Format(strResult, "@@@@")))
                                End If
                            Case 9 To 13 'LYMPH%, MONO%, NEUT%, EO%, BASO%
                                strResult = Trim(Mid(strRcvBuf, 89 + (i - 9) * 5, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@@")), "0.00")
                                End If
                            Case 14 To 18 'LYMPH#, MONO#, NEUT#, EO#, BASO# ????????????????????????????
                                strResult = Trim(Mid(strRcvBuf, 114 + (i - 14) * 6, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@@")), "0.00")
                                End If
                            Case 19 To 23   'RDW-CV(%), RDW-SD(fL), PDW(fL), MPV(fL), P-LCR
                                strResult = Trim(Mid(strRcvBuf, 144 + (i - 14) * 5, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@@")), "0.0")
                                End If
                            Case 24     'RET %
                                'strResult = Trim(Mid(strRcvBuf, 169, 4))
                            Case 25     'RET #
                                strResult = Trim(Mid(strRcvBuf, 174, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@@")), "0.00")
                                End If
                            Case 26 To 29   'IRF, LFR, MFR, HFR ????????????????????????????
                                strResult = Trim(Mid(strRcvBuf, 179 + (i - 26) * 5, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@@.@")), "0.0")
                                End If
                            Case 30     'PCT
                                strResult = Trim(Mid(strRcvBuf, 199, 4))
                                If strResult <> "" Then
                                    strResult = Format(Val(Format(strResult, "@@.@@")), "0.00")
                                End If
                            Case Else
...
Private Sub GetOrder_XT1800i(ByVal pBarno As String, ByVal pType As String)
    ...
    Dim strOrdBuf   As String
        
    intRow = -1
    strOrder1 = ""
    strOrder2 = ""
    strOrdBuf = String$(35, "0")
    ...
    With frmMain
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = GetEquipExamCode_XT1800i(gHOSP.MACHCD, pBarno, intRow)

        '-- 지금 보내는 오더 만들기 (S11) ------------
        'Yes Order
        'strOrder1 = STX & "S11" & Space(8) & "000" & mBarNo & "00" & mRackNo & mPos & "1" & Space(145) & String$(18, "0") & strItems & String$(17, "0") & ETX
        'No Order
        'strOrder1 = STX & "S11" & Space(8) & "000" & mBarNo & "00" & mRackNo & mPos & "1" & Space(145) & String$(51, "0") & String$(17, "0") & ETX
        
        strOrder1 = STX 'Chr$(2)
        strOrder1 = strOrder1 & "S11"
        strOrder1 = strOrder1 & Format(Now, "YYYYMMDD")
        strOrder1 = strOrder1 & Space$(3)
        strOrder1 = strOrder1 & Right(Space(15) & mOrder.BarNo, 15)
        strOrder1 = strOrder1 & Space$(2)
        strOrder1 = strOrder1 & Format(mOrder.RackNo, "000000")
        strOrder1 = strOrder1 & Format(mOrder.TubePos, "00")
        strOrder1 = strOrder1 & "1"
        strOrder1 = strOrder1 & Space$(16)      '-- Patient No (16)
        strOrder1 = strOrder1 & Space$(40)
        strOrder1 = strOrder1 & Space$(1)       '--- SEX (1) : M:1, F:2
        strOrder1 = strOrder1 & Space$(8)
        strOrder1 = strOrder1 & Space(20)       '--- Doctor (20)
        strOrder1 = strOrder1 & Space(20)       '--- Ward (20)
        strOrder1 = strOrder1 & Space(40)       '--- Sample Comment (40)
        strOrder1 = strOrder1 & Space$(18)
        
        'Order 만들기
        If InStr(strItems, "C") > 0 Then    'CBC
            For i = 1 To 8
                Mid(strOrdBuf, i, 1) = "1"
            Next i
            For i = 19 To 23
                Mid(strOrdBuf, i, 1) = "1"
            Next i
            Mid(strOrdBuf, 33, 1) = "1"
        End If
        
        If InStr(strItems, "D") > 0 Then      'DIFF
            For i = 9 To 18
                Mid(strOrdBuf, i, 1) = "1"
            Next i
        End If
        
        If InStr(strItems, "R") > 0 Then      'RETI
            For i = 26 To 31
                Mid(strOrdBuf, i, 1) = "1"
            Next i
        End If
        
        If InStr(strItems, "N") > 0 Then      'NRBC
            Mid(strOrdBuf, 34, 1) = "1"
            Mid(strOrdBuf, 35, 1) = "1"
        End If
        
        strOrder1 = strOrder1 & strOrdBuf
        strOrder1 = strOrder1 & Space$(15)
        strOrder1 = strOrder1 & ETX 'Chr(3)
        '-----------------------------------------------
        
        '-- 두번째 보내는 오더 만들기 (S12) ------------
        'strOrder2 = STX & "S2" & strInfoCd & Space(8) & "000" & mBarNo & "00" & mRackNo & mPos & "1" & Space(116) & String(97, "0") & ETX
        
        strOrder2 = STX 'Chr$(2)
        strOrder2 = strOrder2 & "S21"
        strOrder2 = strOrder2 & Format(Now, "YYYYMMDD")
        strOrder2 = strOrder2 & Space$(3)
        strOrder2 = strOrder2 & Right(Space(15) & mOrder.BarNo, 15)
        strOrder2 = strOrder2 & Space$(2)
        strOrder2 = strOrder2 & Format(mOrder.RackNo, "000000")
        strOrder2 = strOrder2 & Format(mOrder.TubePos, "00")
        strOrder2 = strOrder2 & "1"
        strOrder2 = strOrder2 & Space(16)
        strOrder2 = strOrder2 & Space$(100)
        strOrder2 = strOrder2 & Space$(97)
        strOrder2 = strOrder2 & ETX 'Chr(3)
        
        mOrder.Order = strOrder2
        '-----------------------------------------------
        
        '-- 오더 전송 (S11)
        comEqp.Output = strOrder1
...
'검체번호에 존재하는 장비번호 해당하는 수가코드 가져오기
'한 장비 번호에 검사코드가 1개이상 존재
Private Function GetEquipExamCode_XT1800i(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim i As Integer
    Dim sExamCode As String
    Dim strExamCode As String
    Dim sSpecNo     As String
    ...
    Dim strRETI     As String
    Dim strNRBC     As String
    
    GetEquipExamCode_XT1800i = ""
    ...
    If Not AdoRs_Local.EOF = True And Not AdoRs_Local.BOF = True Then
        Do Until AdoRs_Local.EOF
            Select Case Mid(Trim(AdoRs_Local.Fields("SENDCHANNEL").Value & ""), 1, 1)
                Case "C": strCBC = "C"
                Case "D": strDIFF = "D"
                Case "R": strRETI = "R"
                Case "N": strNRBC = "N"
    ...
    strExamCode = strCBC & strDIFF & strRETI & strNRBC
    
    GetEquipExamCode_XT1800i = strExamCode
