        ...
        If .optBarSeq(0).Value = True Then
            For i = 1 To .spdOrder.DataRowCnt
                If IsNumeric(pBarno) Then
                    ...
                    If CCur(Trim(GetText(frmMain.spdOrder, i, colBARCODE))) = CCur(Trim(pBarno)) Then
...
'        End If
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,SAMPLETYPE,INOUT,PANICVALUE,REFJUDGE " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
