...
'        SetText frmInterface.vasID, Trim(gReadBuf(4)), asRow, colIOFlag
        Select Case Trim(gReadBuf(4))
            Case "1": SetText frmInterface.vasID, "외래", asRow, colIOFlag
            Case "2": SetText frmInterface.vasID, "입원", asRow, colIOFlag
...
'-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기
Function GetGetEquipExamCode_ActDiff5AL(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim i As Integer
    Dim j As Integer
    Dim strExamCode  As String
    Dim sBarcode     As String
    
    GetGetEquipExamCode_ActDiff5AL = ""
    j = 0
    ...
    GetGetEquipExamCode_ActDiff5AL = strExamCode
