...
'                    SQL = SQL & "   And STS_CD     = 'A'                    " & vbCrLf
                    ...
                    SQL = ""
                    SQL = SQL & "UPDATE DBsupport..검사결과              " & vbCr
                    SQL = SQL & "   SET 처리상태   = '완료'              " & vbCr
                    SQL = SQL & "     , 결과       = '" & sResult & "'   " & vbCr
                    SQL = SQL & "     , 판정       = '" & strJudge & "'  " & vbCr   '판정입력(▲ : 참고치 이상 ::   ▼  : 참고치 이하)
                    If strJudge = "H" Then
                        SQL = SQL & "     , 판정       = '▲'  " & vbCr   '판정입력(▲ : 참고치 이상 ::   ▼  : 참고치 이하)
                    ElseIf strJudge = "L" Then
                        SQL = SQL & "     , 판정       = '▼'  " & vbCr   '판정입력(▲ : 참고치 이상 ::   ▼  : 참고치 이하)
                    End If
                    SQL = SQL & " WHERE 처방번호   = '" & strBarcode & "'" & vbCr
                    SQL = SQL & "   AND 접수번호   = '" & strChartNo & "'" & vbCr
                    
                    SQL = SQL & "   And 검사코드   = '" & strTestCd & "' " & vbCr
                    SQL = SQL & "   And 순번       = '" & strTestCdSub & "'" & vbCr
                    'SQL = SQL & "   And 검사코드   = '" & Mid(strTestCd, 1, Len(strTestCd) - 2) & "' " & vbCr
                    'SQL = SQL & "   And 순번       = '" & Right(strTestCdSub, 2) & "'" & vbCr
            ...
            With SPD
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        gArrEQP(intCol - colSTATE, 16) = Trim(RS.Fields("ITEMSEQ")) & ""
