...
'한 장비채널에 검사코드가 1개이상 존재 (GLU-FBS, GLU-PP2..)
Public Function GetEquipExamCode_HITACHI7060(argEquipCode As String, argPID As String, Optional intRow As Long) As String
    Dim strExamCode     As String
    Dim intIntBase      As Integer
    Dim strItems        As String           '전송할 검사항목
    Dim blnISE          As Boolean          'Na, K, Cl 검사여부

    strItems = String$(53, "0")
    GetEquipExamCode_HITACHI7060 = strItems
    strExamCode = ""
    blnISE = False
    mOrder.SendCnt = 0
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "SaveTransData_EASYS" & vbNewLine & vbNewLine
...
Function SaveTransData_EMRLAB(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    Dim RsLocal         As ADODB.Recordset
...
'                    SQL = SQL & "   And STS_CD     = 'A'                    " & vbCrLf
                    ...
                    SQL = ""
                    SQL = SQL & "UPDATE DBsupport..검사결과              " & vbCrLf
                    SQL = SQL & "   SET 처리상태   = '완료'              " & vbCrLf
                    SQL = SQL & "     , 결과       = '" & sResult & "'   " & vbCrLf
                    'SQL = SQL & "     , 판정       = '" & strJudge & "'  " & vbCrLf   '판정입력(▲ : 참고치 이상 ::   ▼  : 참고치 이하)
                    If strJudge = "H" Then
                        SQL = SQL & "     , 판정       = '▲'  " & vbCrLf   '판정입력(▲ : 참고치 이상 ::   ▼  : 참고치 이하)
                    ElseIf strJudge = "L" Then
                        SQL = SQL & "     , 판정       = '▼'  " & vbCrLf   '판정입력(▲ : 참고치 이상 ::   ▼  : 참고치 이하)
                    End If
                    SQL = SQL & " WHERE 처방번호   = '" & strBarcode & "'" & vbCrLf
'                    SQL = SQL & "   AND 접수번호   = '" & strOrdCd & "'" & vbCrLf
                    
                    '단독코드
                    'If gHOSP.MACHNM = "HITACHI7060" Or gHOSP.MACHNM = "ISMART300" Or gHOSP.MACHNM = "VISION" Then
                    If gHOSP.MACHNM = "HITACHI7060" Or gHOSP.MACHNM = "VISION" Then
                        SQL = SQL & "   And 검사코드   = '" & strTestCd & "' " & vbCrLf
                        SQL = SQL & "   And 순번       = '" & strTestCdSub & "'" & vbCrLf
                    Else
                        SQL = SQL & "   And 검사코드   = '" & Mid(strTestCd, 1, Len(strTestCd) - 2) & "' " & vbCrLf
                        SQL = SQL & "   And 순번       = " & Val(Right(strTestCd, 2)) & vbCrLf
                    End If
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "SaveTransData_EMRLAB" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show vbModal
...
'                Call GetWorkList_EASYS(pFrom, pTo, SPD)
        ...
        Case "EMRLAB"
                Call GetWorkList_EMRLAB(pFrom, pTo, SPD)
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetWorkList_MSINFOTEC" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show vbModal
        ...
        Do Until RS.EOF
            ...
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
                    '환자 나이
                    strJumin = Trim(RS.Fields("AGE")) & ""
                    If strJumin <> "" And Len(strJumin) = 8 Then
                        strAge = CalAge(strJumin, Format(Date, "yyyy/mm/dd"))
                    End If
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetWorkList_EASYS" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show vbModal
...
Public Sub GetWorkList_EMRLAB(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
On Error GoTo ErrHandle
    ...
    SQL = ""
    SQL = SQL & "SELECT DISTINCT"
    SQL = SQL & "       R.접수일                as HOSPDATE " & vbCrLf
'    SQL = SQL & "       M.내원일                as HOSPDATE " & vbCrlf
    SQL = SQL & "     , R.처방번호              as BARCODE  " & vbCrLf
    SQL = SQL & "     , P.등록번호              as PID      " & vbCrLf
    SQL = SQL & "     , R.접수번호              as CHARTNO  " & vbCrLf
    SQL = SQL & "     , P.성명                  as PNAME    " & vbCrLf
    SQL = SQL & "     , P.남녀                  as SEX      " & vbCrLf
    SQL = SQL & "     , M.외래입원              as INOUT    " & vbCrLf
    SQL = SQL & "     , COUNT(R.검사코드)       as CNT      " & vbCrLf
    SQL = SQL & "  FROM DBinfor..내원정보 M, DBinfor..VIEW_환자정보 P, DBsupport..검사결과 R    " & vbCrLf  'Dbinfor..VIEW_환자정보
    SQL = SQL & " WHERE M.연번      = R.처방번호                                                " & vbCrLf
    SQL = SQL & "   AND M.등록번호  = P.등록번호                                                " & vbCrLf
    SQL = SQL & "   AND R.처방번호  = R.처방번호                                                " & vbCrLf
    SQL = SQL & "   AND R.접수일 BETWEEN '" & pFrom & "' and '" & pTo & "'                      " & vbCrLf
    SQL = SQL & "   AND R.검사코드 IN (" & gAllTestCd & ")                                      " & vbCrLf
'    SQL = SQL & "   AND R.순번 IN (" & gAllTestCd & ")                                      " & vbCrLf
    SQL = SQL & "   AND (R.결과 = '' OR R.결과 IS NULL)                                         " & vbCrLf
    SQL = SQL & " GROUP BY R.접수일, R.처방번호, P.등록번호, R.접수번호, P.성명, P.남녀, M.외래입원     " & vbCrLf
'    SQL = SQL & " ORDER BY R.접수일, R.접수번호                                            " & vbCrlf
        ...
        Do Until RS.EOF
            ...
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
                    SetText SPD, Trim(RS.Fields("PNAME")) & "", intRow, colPNAME
                    SetText SPD, IIf(Trim(RS.Fields("SEX")) & "" = "남", "M", "F"), intRow, colPSEX
                    SetText SPD, IIf(Trim(RS.Fields("INOUT")) & "" = "I", "입원", "외래"), intRow, colINOUT
                    SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
...
ErrHandle:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetWorkList_EMRLAB" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show vbModal
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    ...
    Select Case gEMR
        Case "EMRLAB"
            SQL = ""
            SQL = SQL & "SELECT DISTINCT R.검사항목       as ITEM           " & vbCr
            SQL = SQL & "  FROM DBinfor..내원정보 M, DBinfor..VIEW_환자정보 P, DBsupport..검사결과 R     " & vbCr
            SQL = SQL & " WHERE M.연번      = R.처방번호                    " & vbCr
            SQL = SQL & "   AND M.등록번호  = P.등록번호                    " & vbCr
            SQL = SQL & "   AND R.처방번호  = R.처방번호                    " & vbCr
            SQL = SQL & "   AND R.접수일    = '" & strRegDate & "'          " & vbCr
            SQL = SQL & "   AND R.처방번호  = '" & strBarcode & "'          " & vbCr
            SQL = SQL & "   AND R.검사코드 IN (" & gAllTestCd & ")          " & vbCr
            'SQL = SQL & " ORDER BY R.검사항목                               " & vbCr
...
'                Call GetSampleInfo_EASYS(asRow, SPD)
        ...
        Case "EMRLAB"
                Call GetSampleInfo_EMRLAB(asRow, SPD)
...
On Error GoTo DBErr
    ...
    SQL = SQL & "   AND a.SUTAK_CD  = ''                    " & vbCrLf
    SQL = SQL & "   AND a.ORD_CD IN (" & gAllTestCd & ")    " & vbCrLf
    SQL = SQL & " ORDER BY a.ACC_YMD,a.RECEPT_NO,a.ORD_CD   " & vbCrLf
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                '환자 성별
                mPatient.SEX = Trim(RS.Fields("SEX")) & ""
                
                '환자 나이
                strJumin = Trim(RS.Fields("AGE")) & ""
                If strJumin <> "" And Len(strJumin) = 8 Then
                    mPatient.AGE = CalAge(strJumin, Format(Date, "yyyy/mm/dd"))
                End If
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_EASYS" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
'-- 검사자 정보 가져오기 [이지스]
Function GetSampleInfo_EMRLAB(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    SQL = SQL & "       M.내원일                as HOSPDATE " & vbCrlf
    ...
    SQL = SQL & "     , P.남녀                  as SEX       " & vbCrLf
    SQL = SQL & "     , M.외래입원              as INOUT     " & vbCrLf
    SQL = SQL & "     , R.검사코드              as ITEM      " & vbCrLf
    SQL = SQL & "     , R.순번                  as ITEMSEQ   " & vbCrLf
    SQL = SQL & "  FROM DBinfor..내원정보 M, DBinfor..VIEW_환자정보 P, DBsupport..검사결과 R     " & vbCrLf
    SQL = SQL & " WHERE M.연번      = R.처방번호                                            " & vbCrLf
    SQL = SQL & "   AND M.등록번호  = P.등록번호                                            " & vbCrLf
    SQL = SQL & "   AND R.처방번호  = R.처방번호                                            " & vbCrLf
    SQL = SQL & "   AND R.처방번호 = '" & strBarcode & "'                                   " & vbCrLf
    SQL = SQL & "   AND R.검사코드 IN (" & gAllTestCd & ")                                  " & vbCrLf
...
'                SetText SPD, Trim(RS.Fields("CHARTNO")), asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                SetText SPD, IIf(Trim(RS.Fields("SEX")) & "" = "남", "M", "F"), asRow, colPSEX
                SetText SPD, IIf(Trim(RS.Fields("INOUT")) & "" = "I", "입원", "외래"), asRow, colINOUT
                ...
                For intCol = colSTATE + 1 To .MaxCols
                    If Trim(RS.Fields("ITEM")) = gArrEQP(intCol - colSTATE, 2) Then
                        ...
                        gArrEQP(intCol - colSTATE, 17) = Trim(RS.Fields("ITEMSEQ")) & ""
                ...
                If gHOSP.MACHNM = "HITACHI7060" Or gHOSP.MACHNM = "ISMART300" Then
                    gPatTest(intTestCnt) = Trim(RS.Fields("ITEM"))
                Else
                    gPatTest(intTestCnt) = Trim(RS.Fields("ITEM")) & Trim(RS.Fields("ITEMSEQ"))
                End If
            End With
            DoEvents
...
DBErr:
    ...
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "GetSampleInfo_EMRLAB" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
