...
Function SaveTransDataW(ByVal argSpcRow As Integer) As Integer
    ...
    Dim sHeader$, sDetail$, sLog$, sRtn$
    
On Error GoTo ErrHandle

    Set objRESULT = CreateObject("IFAMIS.CAMIS01")

    sHeader = ""
    sHeader = "flag" & Chr(124) & "spcnNo" & Chr(124) & "patno" & Chr(124) & "ordrCd" & Chr(124) & "examCd" & Chr(124) & "equipRsltVal" & Chr(124) & "equipErrCd" & Chr(124) & "dgiEquipCd" & Chr(124) & "lvlNo" & Chr(124) & "lotNo" & Chr(124) & "dltnMthCd" & Chr(124) & "gnrlDltnMgnfVal"
        
    With frmInterface
        SaveTransDataW = -1
        
        lsID = Trim(GetText(.vasID, argSpcRow, colBARCODE))
        lsPid = Trim(GetText(.vasID, argSpcRow, colCHARTNO))
...
'        strOrd_Cd = Trim(GetText(.vasID, argSpcRow, colPAGE))
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            sDetail = ""
            '         "flag  |spcnNo      |patno        |ordrCd          |examCd          |equipRsltVal   |equipErrCd|dgiEquipCd  |  lvlNo  |  lotNo  |  dltnMthCd  |  gnrlDltnMgnfVal"
            sDetail = "N" & "|" & lsID & "|" & lsPid & "|" & strOrderCd & "|" & strEqpCd & "|" & sResult & "||" & gEquipCode & "||||" & "" 'ETX
            
            If sResult <> "" Then
                sRtn = objRESULT.GetResult(gUseSvr, gRstSvcID, sHeader, sDetail, gEquipCode)
                
                'Call SetSQLData("결과저장", SQL)
                       sLog = "서버:" & gUseSvr & vbCr
                sLog = sLog & "서비스:" & gRstSvcID & vbCr
                sLog = sLog & "Header" & sHeader & vbCr
                sLog = sLog & "Detail" & sDetail & vbCr
                sLog = sLog & "장비코드:" & gEquipCode
                                
                Call SetSQLData("결과저장", sLog)
...
'''          SQL = " SELECT DISTINCT C.JOBDATE,C.PTNO,C.JOBNO, DECODE(C.GBIO,'I','입원','O','외래') AS GBIO,C.SNAME,C.SEX,C.AGE " & vbCrLf
...
'''    SQL = SQL & " Where C.SPECNO   = '" & sBarcode & "'" & vbCrLf   ' 검체번호
'''    SQL = SQL & "   And B.EQUCODE1 = '" & gEquipCode & "'" & vbCrLf ' 장비코드
'''    SQL = SQL & "   AND C.STATUS   <= '3' " & vbCrLf                 ' 검사상태(4 : 부분완료)
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_SCL(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim strDate     As String
    ...
    Set adoRS2 = adoExecQuery51P("SLRTRM51P", Trim(sBarcode), gEquipCode, "")

    SetSQLData "바코드조회 : ", strRecordStatus

    Select Case strRecordStatus
        Case "R"
            frmInterface.StatusBar1.Panels(2).Text = Trim(sBarcode) & " 바코드 오류 ! 바코드번호를 확인하세요."
            adoRS2.Close: Set adoRS2 = Nothing
        Case "M"
            frmInterface.StatusBar1.Panels(2).Text = Trim(sBarcode) & " 장비코드 오류 !  "
            adoRS2.Close: Set adoRS2 = Nothing
        Case "Y", "N", " "
            frmInterface.StatusBar1.Panels(2).Text = Trim(sBarcode) & " 검사진행."

            If Not adoRS2.EOF = True And Not adoRS2.BOF = True Then
                With frmInterface
                    Do Until adoRS2.EOF
                        GetOrderExamCode = GetOrderExamCode & "'" & Trim(adoRS2.Fields("ITEMCODE")) & Trim(adoRS2.Fields("DCODE")) & "',"
                        
                        SetText .vasID, "1", .vasID.MaxRows, colCheckBox
                        'SetText .vasID, Trim(adoRS2.Fields("acc_ymd")) & "", asRow, colHOSPDATE
                        'SetText .vasID, Trim(adoRS2.Fields("recept_no")) & "", asRow, colBARCODE
                        SetText .vasID, Trim(adoRS2.Fields("WORKNO")) & "", asRow, colPID
                        SetText .vasID, Trim(adoRS2.Fields("PNAME")) & "", asRow, colPNAME
                        'SetText .vasID, Trim(adoRS2.Fields("ord_seq_no")) & "", asRow, colPSEX
                        'SetText .vasID, Trim(adoRS2.Fields("ord_no")) & "", asRow, colPAGE
                        
                        '-- 화면에 표시
                        For intCol = colState + 1 To .vasID.MaxCols
                            If Trim(adoRS2.Fields("ITEMCODE")) & Trim(adoRS2.Fields("DCODE")) = gArrEquip(intCol - colState, 3) Then
                                .vasID.Row = asRow
                                .vasID.Col = intCol
                                .vasID.BackColor = vbYellow
                                Exit For
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_ASAN(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim strDate     As String
    Dim adoRS2      As ADODB.Recordset
    Dim GetOrderExamCode    As String
    ...
    Dim i%, j%
    Dim sRtn$, sErr$, sLog$
    Dim objORDER As Object
    Dim sOption$
    Dim vRtn As Variant
    
    On Error GoTo ErrHandler
    
    GetSampleInfoW_ASAN = -1
    ...
    Set objORDER = CreateObject("IFAMIS.CAMIS01")
    
    '== 인수 Description ==
    '1. 개발 : D , 운영 : P
    '2. Order Service ID : WSPSS3500R
    '3. 검체번호 & "|" & 장비코드
    '4. 장비코드
    '5  에러메세지
    
    sRtn = objORDER.GetOrder(gUseSvr, gOdrSvcID, sBarcode & Chr(124) & gEquipCode, gEquipCode, sErr)
    
    Set objORDER = Nothing
    
'sRtn = "|||52242982|홍길동|M|19800426|170000276501|20170122H20CBCO00186|CV|||AC0092||Y|20170125160655|X|||"
 '||||||||||34668492|황은희|F|19600118|170021076801|20170420H20CBCI00002|HEM|14899||AC0094||Y|20170420172118|X|||0|접수|||||||||||||

'오더 조회시 데이터 내용 확인 부탁 드립니다.

'처음 3개 필드는 무시하시구요 그 다음 필드부터 내용 입니다
'4  patno             병원등록번호(환자코드)
'5  patNm             환자성명
'6  sexCd             성별
'7  patBirthdt        생년월일
...
'12 ordrCd            처방코드
'13 examCd            검사코드
'14 markRsltVal       표시결과값
'15 valdYn            Y 결과값던짐
'16 rsltRportDt       보고일자
'17 dltnMthCd         희석방법코드
'18 gnrlDltnMgnfVal   일반희석배율값
    
    
    If sErr <> "" Then
        Call SetSQLData("오더조회", sBarcode & sErr)
        Exit Function
    End If
    
           sLog = "서버:" & gUseSvr & vbCr
    sLog = sLog & "서비스:" & gOdrSvcID & vbCr
    sLog = sLog & "조회정보" & sBarcode & Chr(124) & gEquipCode & vbCr
    sLog = sLog & "장비코드" & gEquipCode & sErr & vbCr
    sLog = sLog & "반환값:" & sRtn
    Call SetSQLData("오더조회", sLog)
    
    ' 반환값이 ESR 하나일것이므로 필요없을듯...
'    vRtn = Split(sRtn, Chr(124))
'    For i = 0 To UBound(vRtn)
'    Next
    ...
    If sRtn <> "" Then
        With frmInterface
            ...
            GetOrderExamCode = GetOrderExamCode & "'" & Trim(mGetP(sRtn, 13, Chr(124))) & "',"
            
            SetText .vasID, "1", .vasID.MaxRows, colCheckBox
            SetText .vasID, strDate, asRow, colHOSPDATE
'                        SetText .vasID, Trim(mGetP(sRtn, 8, Chr(124))), asRow, colBARCODE
            SetText .vasID, Trim(mGetP(sRtn, 9 + 7, Chr(124))), asRow, colCHARTNO
            SetText .vasID, Trim(mGetP(sRtn, 10 + 7, Chr(124))), asRow, colPID
            SetText .vasID, Trim(mGetP(sRtn, 11 + 7, Chr(124))), asRow, colINOUT
            SetText .vasID, Trim(mGetP(sRtn, 5 + 7, Chr(124))), asRow, colPNAME
            SetText .vasID, Trim(mGetP(sRtn, 6 + 7, Chr(124))), asRow, colPSEX
            SetText .vasID, Trim(mGetP(sRtn, 7 + 7, Chr(124))), asRow, colPAGE
            
            '-- 화면에 표시
            For intCol = colState + 1 To .vasID.MaxCols
                If Trim(mGetP(sRtn, 13 + 7, Chr(124))) = gArrEquip(intCol - colState, 3) Then
                    .vasID.Row = asRow
                    .vasID.Col = intCol
                    .vasID.BackColor = vbYellow
                    Exit For
...
'    SQL = SQL & "   AND a.sts_cd   = 'A'"                                                                           ' A:접수 R:결과"
...
'      -- 테이블 사용
          SQL = "SELECT DISTINCT a.RECEIPTDATE as 접수일자, a.SPECIMENNUM as 바코드번호, a.IPDOPD, a.RECEIPTNO as 챠트번호, a.PTNO as 내원번호, a.SNAME as 이름, b.LABCODE as ITEM, a.ORDERCODE"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                '    SetText .vasID, Trim(RS.Fields("챠트번호")) & "", asRow, colBARCODE
                'Else
                    SetText .vasID, Trim(RS.Fields("바코드번호")) & "", asRow, colBARCODE
                'End If
                ...
                SetText .vasID, IIf(Trim(RS.Fields("IPDOPD")) = 1, "입원", "외래"), asRow, colINOUT
