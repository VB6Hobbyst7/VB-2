...
Public gUseSvr As String
Public gOdrSvcID As String
Public gRstSvcID As String
...
Public Const colEXAMDATE = 3    '검사(결과)일자
Public Const colHOSPDATE = 4    'NA
Public Const colBARCODE = 5     '검체번호
Public Const colCHARTNO = 6     '검사번호
Public Const colPID = 7         '진료과
Public Const colINOUT = 8       '병동
Public Const colDISKNO = 9
Public Const colPOSNO = 10
Public Const colPNAME = 11
Public Const colPSEX = 12
Public Const colPAGE = 13       '생년월일
...
'    gQCEquip = Trim(frmInterface.txtTemp)
    ...
    '==  UseSvr 관련 설정    ==============================================================================
    db_tmp = ""
    Call GetPrivateProfileString("Server", "UseSvr", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gUseSvr = Trim(frmInterface.txtTemp)
    
    db_tmp = ""
    Call GetPrivateProfileString("Server", "OdrSvcID", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gOdrSvcID = Trim(frmInterface.txtTemp)
        
    db_tmp = ""
    Call GetPrivateProfileString("Server", "RstSvcID", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gRstSvcID = Trim(frmInterface.txtTemp)
...
'argSQL의 내용을 파일로 저장
    ...
    sFileName = Format(CDate(frmInterface.dtpToday), "yyyy-mm-dd") & strName
