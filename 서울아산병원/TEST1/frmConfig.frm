...
Private Sub cmdConfirm_Click()
    ...
    Else
        ...
        If optSvc(0).Value = True Then
            gUseSvr = "D"
        ElseIf optSvc(1).Value = True Then
            gUseSvr = "P"
        Else
            gUseSvr = ""
        End If
        
        gOdrSvcID = Trim(txtOdrSvcID.Text)
        gRstSvcID = Trim(txtRstSvcID.Text)
        
        Call WritePrivateProfileString("Server", "UseSvr", gUseSvr, App.Path & "\Interface.ini")
        Call WritePrivateProfileString("Server", "OdrSvcID", gOdrSvcID, App.Path & "\Interface.ini")
        Call WritePrivateProfileString("Server", "RstSvcID", gRstSvcID, App.Path & "\Interface.ini")
...
Private Sub Form_Load()
    ...
    txtOdrSvcID = gOdrSvcID
    txtRstSvcID = gRstSvcID
    If gUseSvr = "D" Then
        optSvc(0).Value = True
    ElseIf gUseSvr = "P" Then
        optSvc(1).Value = True
    Else
        optSvc(2).Value = True
    End If
