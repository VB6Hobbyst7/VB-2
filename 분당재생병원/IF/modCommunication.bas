...
Public Sub Phase_TCP_ALEREi()
    ...
    Call frmMain.TCPRcvData_ALEREi
...
Function SaveTransData_BDJS(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
...
'        End If
        ...
        .vasTemp.MaxRows = 0
              ...
              SQL = "SELECT EQUIPCODE,ORDERCODE,EXAMCODE,EXAMSUBCODE,EQUIPRESULT,RESULT,SEQNO " & vbCr
        SQL = SQL & "  FROM PATRESULT " & vbCr
        SQL = SQL & " WHERE EQUIPNO = '" & gHOSP.MACHCD & "'" & vbCr                      '장비코드
        SQL = SQL & "   AND SAVESEQ = " & strSaveSeq & vbCr                               '저장번호
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & Mid(strExamDate, 1, 8) & "'" & vbCr  '검사일
        ...
        If Not RS_L.EOF = True And Not RS_L.BOF = True Then
            ...
            Do Until RS_L.EOF
                ...
                Call SetText(.vasTemp, RS_L.Fields("EQUIPRESULT").Value & "", intRow, 5)
                Call SetText(.vasTemp, RS_L.Fields("RESULT").Value & "", intRow, 6)
                Call SetText(.vasTemp, RS_L.Fields("SEQNO").Value & "", intRow, 7)
                RS_L.MoveNext
            Loop
        End If
        ...
        '-- 서버로 결과값 저장하기
        For intRow = 1 To .vasTemp.DataRowCnt
            strOrdCd = Trim(GetText(.vasTemp, intRow, 2))      '검사코드
            strTestCd = Trim(GetText(.vasTemp, intRow, 3))      '검사코드
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            intSeq = Trim(GetText(.vasTemp, intRow, 7))         '결과순번
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                With AdoCmd
                    .CommandTimeout = 15
                    .CommandText = "SP_LAB_IF_COM.InsertEqupRslt"
                    .CommandType = adCmdStoredProc
                    
                    
                    Set prm1 = .CreateParameter("i_SpcNo", adVarChar, adParamInput, 4000, strBarcode)      '검체번호
                    .Parameters.Append prm1
                    Set prm2 = .CreateParameter("i_LabCd", adVarChar, adParamInput, 4000, strTestCd)       '검사코드
                    .Parameters.Append prm2
                    Set prm3 = .CreateParameter("i_ExamEqupCd", adVarChar, adParamInput, 4000, gHOSP.MACHCD)    '장비코드
                    .Parameters.Append prm3
                    Set prm4 = .CreateParameter("i_RetEqupExamCd", adVarChar, adParamInput, 4000, strOrdCd)    '반환장비검사코드
                    .Parameters.Append prm4
                    Set prm5 = .CreateParameter("i_EqupRslt", adVarChar, adParamInput, 4000, sResult)    '장비결과
                    .Parameters.Append prm5
                    Set prm6 = .CreateParameter("i_FlagCdLst", adVarChar, adParamInput, 4000, "")    'Flag코드리스트
                    .Parameters.Append prm6
                    Set prm7 = .CreateParameter("i_ErrCdLst", adVarChar, adParamInput, 4000, "")    '오류코드리스트
                    .Parameters.Append prm7
                    Set prm8 = .CreateParameter("i_RackNo", adVarChar, adParamInput, 4000, "")    'Rack번호
                    .Parameters.Append prm8
                    Set prm9 = .CreateParameter("i_PosNo", adVarChar, adParamInput, 4000, "")    'Position번호
                    .Parameters.Append prm9
                    Set prm10 = .CreateParameter("i_EqupSeq", adNumeric, adParamInput, 100, intSeq)    '장비Seq
                    .Parameters.Append prm10
                    Set prm11 = .CreateParameter("i_Rmk", adVarChar, adParamInput, 4000, "")    '비고
                    .Parameters.Append prm11
                    Set prm12 = .CreateParameter("i_CommCdYn", adVarChar, adParamInput, 4000, "Y")    '전송코드여부 'Y'
                    .Parameters.Append prm12
                    Set prm13 = .CreateParameter("i_ExamStrYmd", adVarChar, adParamInput, 4000, Format(Now, "yyyymmdd"))   '검사시작일자
                    .Parameters.Append prm13
                    Set prm14 = .CreateParameter("i_ExamStrHms", adVarChar, adParamInput, 100, Format(Now, "hhmmss"))    '검사시작시분초
                    .Parameters.Append prm14
                    Set prm15 = .CreateParameter("i_ExamID", adVarChar, adParamInput, 100, gHOSP.USERID)     '검사자
                ...
                Call SetSQLData("결과저장", strBarcode & "," & strTestCd & "," & gHOSP.MACHCD & "," & strOrdCd & "," & sResult & "," & intSeq)
...
Function SaveTransDataR_BDJS(ByVal argSpcRow As Integer) As Integer
    Dim RS_L            As ADODB.Recordset
    Dim intRow          As Integer
    Dim strDate         As String
...
'        End If
        ...
        '-- 서버로 결과값 저장하기
        For intRow = 1 To .vasTemp.DataRowCnt
            strOrdCd = Trim(GetText(.vasTemp, intRow, 2))       '처방코드(IA,IB)
            strTestCd = Trim(GetText(.vasTemp, intRow, 3))      '검사코드
            sResult1 = Trim(GetText(.vasTemp, intRow, 5))       '결과(장비결과)
            sResult2 = Trim(GetText(.vasTemp, intRow, 6))       '결과(수정결과)
            intSeq = Trim(GetText(.vasTemp, intRow, 7))         '결과순번
            ...
            If strBarcode <> "" And strTestCd <> "" And sResult <> "" Then
                ...
                Call SetSQLData("결과저장", strBarcode & "," & strTestCd & "," & gHOSP.MACHCD & "," & strOrdCd & "," & sResult & "," & intSeq, "A")
                
                                            '1814302178,    X3B000087_004,      LM833,                                          ,Negative,2
