...
'                        End If
        ...
        Case "JWINFO"
            ...
            SQL = SQL & "   AND a.SPECIMENNUM = b.SPECIMENNUM" & vbCr
            SQL = SQL & "   AND a.JSTATUS < '3'" & vbCr
            SQL = SQL & " GROUP BY a.RECEIPTDATE, a.SPECIMENNUM, a.RECEIPTNO, a.IPDOPD, a.PTNO, a.SNAME,b.LABCODE" & vbCr
            SQL = SQL & " ORDER BY a.RECEIPTDATE,a.SPECIMENNUM "
        ...
        Case "BDJS"
            ...
            AdoCmd.CommandText = "SP_LAB_IF_COM.SelectGnlRslt"
            AdoCmd.CommandType = adCmdStoredProc
            
            'MsgBox gHOSP.MACHCD
            Set prm1 = AdoCmd.CreateParameter("i_ExamEqupCd", adVarChar, adParamInput, 4000, gHOSP.MACHCD)
            AdoCmd.Parameters.Append prm1
            
            'MsgBox strBarcode
            Set prm2 = AdoCmd.CreateParameter("i_SpcNo", adVarChar, adParamInput, 4000, strBarcode)
            ...
            If Not rs.EOF = True And Not rs.BOF = True Then
                Do Until rs.EOF
                    With SPD
                        ...
                        SetText SPD, "1", asRow, colCHECKBOX
                        'SetText SPD, Trim(rs.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
                        SetText SPD, Trim(rs.Fields("SpcNo")), asRow, colBARCODE
                        SetText SPD, Trim(rs.Fields("ExamSlipCd")) & "", asRow, colSEQNO
                        SetText SPD, Trim(rs.Fields("PatId")) & "", asRow, colPID
                        mOrder.PID = Trim(rs.Fields("PatId")) & ""
                        SetText SPD, Trim(rs.Fields("PatNm")) & "", asRow, colPNAME
                        mOrder.PName = Trim(rs.Fields("PatNm")) & ""
                        SetText SPD, Trim(rs.Fields("AGE")) & "", asRow, colPAGE
                        SetText SPD, Trim(rs.Fields("SEX")) & "", asRow, colPSEX
                        SetText SPD, CStr(intTestCnt), asRow, colOCNT
                                                
                                                
                        strLCd = Trim(rs.Fields("LabCd")) & ""
                        '-- 화면에 표시
                        For intCol = colSTATE + 1 To .MaxCols
                            If strLCd = gArrEQP(intCol - colSTATE, 2) Then
                                .Row = asRow
                                .Col = intCol
                                .BackColor = vbYellow
                                Call SetText(SPD, "◇", asRow, intCol)
                            ...
                            End If
                        Next
                        gPatOrdCd = gPatOrdCd & "'" & strLCd & "',"
                        '반환장비검사코드
                        gPatTstCd = gPatTstCd & strLCd & "," & Trim(rs.Fields("RetEqupExamCd")) & "|"
                    End With
                    DoEvents
    ...
    Call SetSQLData("검사코드", gPatOrdCd & "//" & gPatTstCd)
