...
Begin VB.Form frmMain 
      ...
      Begin MSComctlLib.ImageList imlStatus 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":1B6F
               Key             =   "RUN"
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":2109
               Key             =   "NOT"
            EndProperty
            ...
            BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":2C3D
               Key             =   "LST"
            EndProperty
            BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":34CF
               Key             =   "ITM"
            EndProperty
            BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":3629
               Key             =   "ERR"
            EndProperty
            ...
            BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":38DD
               Key             =   "ON"
               Object.Tag             =   "OFF"
            EndProperty
            BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmMain.frx":41B7
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   Begin FPSpread.vaSpread spdOrder 
      ...
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":70A1
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
   Begin FPSpread.vaSpread spdWork 
      ...
      SelectBlockOptions=   0
      ShadowColor     =   16777215
      SpreadDesigner  =   "frmMain.frx":9047
      ScrollBarTrack  =   3
      ShowScrollTips  =   3
   End
...
Private Sub cmdSend_Click()
    ...
    Call SendWsckData(txtSend.Text)
...
Private Sub comEQP_OnComm()
    ...
    Select Case comEqp.CommEvent
        Case comEvReceive
            ...
            Select Case UCase(gHOSP.MACHNM)
                        Case "PPC610H":         Call Phase_Serial_PPC610H
                        Case "UROMETER720":     Call Phase_Serial_UROMETER720
                        Case "HORIBA":          Call Phase_Serial_HORIBA
                        Case "ACCESS2":         Call Phase_Serial_ACCESS2
...
Private Sub GetOrder_PPC300N(ByVal pBarNo As String, ByVal pType As String)
    Dim i           As Integer
    Dim intRow      As Long
    Dim strItems    As String
    Dim strOrder    As String
    ...
    With frmMain
        ...
        '-- 장비수신정보 화면표시
        Call SetText(.spdOrder, mOrder.QRDSampleBarcode, intRow, colBARCODE)
        Call SetText(.spdOrder, mOrder.Seq, intRow, colSEQNO)
        Call SetText(.spdOrder, mOrder.RackNo, intRow, colRACKNO)
        Call SetText(.spdOrder, mOrder.TubePos, intRow, colPOSNO)
        ...
        '-- 로컬테이블에서 검사항목에 해당하는 검사채널 찾아오기 (intRow = 기존 검사했던 바코드가 다시 올라올 경우 위치를 못찾는다.)
        strItems = ""
        'strItems = GetEquipExamCode_PPC300N(gHOSP.MACHCD, pBarNo, intRow)
        strItems = strItems & "DSP|29||Fe^^^|||" & vbCr
        strItems = strItems & "DSP|30||UIBC^^^|||" & vbCr
...
'''
'''            strSend = strSend & "QRD|" & mOrder.BSDtTm & "|R|D|" & mOrder.Seq & "|||RD|" & mOrder.BSQryId & "|OTH|||T|" & vbCr
'''
'''            '                           QRF|||                                                          '|||RCT|COR|ALL||
'''
'''
...
'''            strSend = strSend & "DSP|3||" & mOrder.PNAME & "|||" & vbCr      'Patient Name - 바코드번호
...
'''            strSend = strSend & EB & vbCr
            ...
            mOrder.MSHSysDateTime = Format(Now, "yyyymmddhhmmss")
            mOrder.QRDQryTime = mOrder.MSHSysDateTime
            
            mOrder.SPCID = mOrder.PID ' & " " & Format(Now, "mmddhhmm")
            mOrder.SPCID = mOrder.BarNo ' & " " & Format(Now, "mmddhhmm")
            
            'mOrder.SPCID = CStr(intSndPhase)
            mOrder.Seq = GetText(spdOrder, intRow, colSEQNO)
            'mOrder.SPCID = mOrder.PID
            
'''                      'strSend = SB & "MSH|^~\&|||" & mOrder.MSHCorpName & "|" & mOrder.MSHDeviceModel & "|" & mOrder.MSHSysDateTime & "||DSR^Q03|" & mOrder.MSHMessageID & "|" & mOrder.MSHProduct & "|" & mOrder.MSHHL7Version & "||||||" & mOrder.MSHChrEncoding & "|||" & vbCr
'''                      strSend = SB & "MSH|^~\&|||" & mOrder.MSHCorpName & "|" & mOrder.MSHDeviceModel & "|" & mOrder.MSHSysDateTime & "||DSR^Q03|1|" & mOrder.MSHProduct & "|" & mOrder.MSHHL7Version & "||||||" & mOrder.MSHChrEncoding & "|||" & vbCr
'''            strSend = strSend & "MSA|AA|" & CStr(intSndPhase) & "|Message accepted|||0|" & vbCr
'''            strSend = strSend & "ERR|0|" & vbCr
'''            strSend = strSend & "QAK|SR|OK|" & vbCr
'''            strSend = strSend & "QRD|" & mOrder.QRDQryTime & "|R|D|1|||RD|" & mOrder.QRDSampleBarcode & "|OTH|||T|" & vbCr
'''            'strSend = strSend & "QRF|" & mOrder.QRFProduct & "|" & mOrder.QRFWherStartDtTm & "|" & mOrder.QRFWherEndDtTm & "|||RCT|COR|ALL||" & vbCr
'''            strSend = strSend & "QRF|" & mOrder.QRFProduct & "|19000101000000|" & mOrder.QRFWherEndDtTm & "|||RCT|COR|ALL||" & vbCr
'''            strSend = strSend & "DSP|1||" & mOrder.PID & "|||" & vbCr    '-- Patient ID
'''            'strSend = strSend & "DSP|1||" & mOrder.SPCID & "|||" & vbCr      '-- Patient ID
'''
'''            strSend = strSend & "DSP|2|||||" & vbCr                         'Bed No
'''            strSend = strSend & "DSP|3||" & mOrder.PNAME & "|||" & vbCr      'Patient Name
'''            'strSend = strSend & "DSP|3||" & mOrder.PNAME & " " & mOrder.PID & "|||" & vbCr      'Patient Name
'''            strSend = strSend & "DSP|4|||||" & vbCr                         'Birth Date
'''            strSend = strSend & "DSP|5|||||" & vbCr                         'Gender
'''            strSend = strSend & "DSP|6|||||" & vbCr                         'Blood Type
'''            strSend = strSend & "DSP|7|||||" & vbCr
...
'''            strSend = strSend & "DSP|10|||||" & vbCr
'''            'strSend = strSend & "DSP|11|||||" & vbCr
'''            strSend = strSend & "DSP|11||1^" & mOrder.QRDNum & "|||" & vbCr
'''            strSend = strSend & "DSP|12|||||" & vbCr
'''            strSend = strSend & "DSP|13|||||" & vbCr
'''            strSend = strSend & "DSP|14|||||" & vbCr
'''            strSend = strSend & "DSP|15|||||" & vbCr                        'Patient Type
...
'''            strSend = strSend & "DSP|20|||||" & vbCr
'''
'''            strSend = strSend & "DSP|21||" & mOrder.QRDSampleBarcode & "|||" & vbCr                       'barcode
'''            'strSend = strSend & "DSP|21||" & mOrder.PID & "|||" & vbCr                       'barcode
'''
'''            'strSend = strSend & "DSP|22||" & mOrder.Seq & "|||" & vbCr                'sample id
'''            strSend = strSend & "DSP|22||" & mOrder.QRDNum & "|||" & vbCr                'sample id
'''            'strSend = strSend & "DSP|22||" & mOrder.QRDSampleBarcode & "|||" & vbCr                'sample id
'''
''''            mOrder.QRDNum = mOrder.QRDNum + 1
'''
'''
''''DSP|21||9275|||
''''DSP|22||1|||
''''DSP|23||20190906094253|||
'''
...
'''
''''            strSend = strSend & strItems
'''            strSend = strSend & "DSP|29||ALT^^^|||" & vbCr
'''            strSend = strSend & "DSP|30||AST^^^|||" & vbCr
'''
''''            blnLast = True
''''            For i = 1 To spdOrder.DataRowCnt
''''                If Trim(GetText(spdOrder, i, colCHECKBOX)) = "1" And Trim(GetText(spdOrder, i, colSTATE)) = "" Then
...
'''            intSndPhase = intSndPhase + 1
                      ...
                      'strSend = SB & "MSH|^~\&|||" & mOrder.MSHCorpName & "|" & mOrder.MSHDeviceModel & "|" & mOrder.MSHSysDateTime & "||DSR^Q03|1|" & mOrder.MSHProduct & "|" & mOrder.MSHHL7Version & "||||||" & mOrder.MSHChrEncoding & "|||" & vbCr
                      strSend = SB & "MSH|^~\&|||" & mOrder.MSHCorpName & "|" & mOrder.MSHDeviceModel & "|" & mOrder.MSHSysDateTime & "||DSR^Q03|1|P|2.3.1||||0||ASCII|||" & vbCr
            'strSend = strSend & "MSA|AA|" & CStr(intSndPhase) & "|Message accepted|||0|" & vbCr
            strSend = strSend & "MSA|AA|2|Message accepted|||0|" & vbCr
            strSend = strSend & "ERR|0|" & vbCr
            strSend = strSend & "QAK|SR|OK|" & vbCr
            strSend = strSend & "QRD|" & mOrder.QRDQryTime & "|R|D|1|||RD|" & mOrder.QRDSampleBarcode & "|OTH|||T|" & vbCr
            'strSend = strSend & "QRF|" & mOrder.QRFProduct & "|19000101000000|" & mOrder.QRFWherEndDtTm & "|||RCT|COR|ALL||" & vbCr
            'strSend = strSend & "QRF|" & mOrder.QRFProduct & "|" & mOrder.QRFWherStartDtTm & "|" & mOrder.QRFWherEndDtTm & "|||RCT|COR|ALL||" & vbCr
            strSend = strSend & "QRF|" & mOrder.QRFProduct & "|" & Format(Now, "yyyymmdd000000") & "|" & Format(Now, "yyyymmdd235959") & "|||RCT|COR|ALL||" & vbCr
            strSend = strSend & "DSP|1||" & mOrder.QRDNum & "|||" & vbCr     '-- Patient ID
            strSend = strSend & "DSP|2||" & mOrder.QRDSampleBarcode & "|||" & vbCr                         'Bed No
            strSend = strSend & "DSP|3||SERUM|||" & vbCr      'Patient Name
            strSend = strSend & "DSP|4|||||" & vbCr                         'Birth Date
            strSend = strSend & "DSP|5|||||" & vbCr                         'Gender
            strSend = strSend & "DSP|6|||||" & vbCr                         'Blood Type
            strSend = strSend & "DSP|7|||||" & vbCr
            ...
            strSend = strSend & "DSP|9|||||" & vbCr
            strSend = strSend & "DSP|10|||||" & vbCr
            strSend = strSend & "DSP|11||Laboratory|||" & vbCr
            strSend = strSend & "DSP|12||Server|||" & vbCr
            strSend = strSend & "DSP|13||123|||" & vbCr
            strSend = strSend & "DSP|14|||||" & vbCr
            strSend = strSend & "DSP|15||2019-09-10|||" & vbCr                        'Patient Type
            strSend = strSend & "DSP|16||N|||" & vbCr
            strSend = strSend & "DSP|17||2|||" & vbCr                        'Pay type
            strSend = strSend & "DSP|18||42^ALB^-0.2779^10.0^g/l^35.0-55.0|||" & vbCr
            strSend = strSend & "DSP|19||41^TP^0.3927^44.5^g/l^60.0-85.0|||" & vbCr
            strSend = strSend & "DSC|-1|" & vbCr
            strSend = strSend & EB & vbCr
            ...
            Select Case strType
                Case "MSH"
                    ...
                    Select Case mOrder.MSHMessageType
                        ...
                        Case "ACK^Q03"
                            '-- 최초이후전송
                            Call GetOrder_PPC300N(strBarno, gHOSP.RSTTYPE)
...
'                            .RsltSeq = getMaxTestNum(Format(frmMain.dtpToday, "yyyymmdd"))
                ...
                Case "QRD"
                    'QRD|20190910155033|R|D|1|||RD|1234567890|OTH|||T|
                    With mOrder
                ...
                Case "QRF"
                    ...
                    '-- 최초오더전송
                    intSndPhase = 1
                    strBarno = mOrder.QRDSampleBarcode
...
Private Sub TCPRcvData_PPC300N_단방향()
    Dim RS_L            As ADODB.Recordset
            ...
            Select Case strType
                Case "MSH"
                    ...
                    Select Case mOrder.BSMType
                        Case "ORU^R01"  '==> ACK^R01
                                           strSend = SB & "MSH|^~\&|||" & mOrder.BSMaker & "|" & mOrder.BSMchNm & "|" & mOrder.BSDtTm & "||ACK^R01|" & mOrder.BSMType & "|P|2.3.1||||0||ASCII|||" & vbCr
                            strSend = strSend & "MSA|AA|" & mOrder.BSMType & "|Message accepted|||0|" & vbCr
                            strSend = strSend & EB & vbCr
                        ...
                        '-- 오더 준비
                        Case "QRY^Q02"  '==> QCK^Q02
                                           strSend = SB & "MSH|^~\&|||" & mOrder.BSMaker & "|" & mOrder.BSMchNm & "|" & mOrder.BSDtTm & "||QCK^Q02|" & mOrder.BSMType & "|P|2.3.1||||0||ASCII|||" & vbCr
                            strSend = strSend & "MSA|AA|" & mOrder.BSMType & "|Message accepted|||0|" & vbCr
                            strSend = strSend & "ERR|0|" & vbCr & EB & vbCr
                            strSend = strSend & "QAK|SR|OK|" & vbCr
                            strSend = strSend & EB & vbCr
...
'
                Case "QRD"
                    ...
                    mOrder.BSQryId = mGetP(strRcvBuf, 9, "|")
                    mOrder.BarNo = mOrder.BSQryId
                    mOrder.Seq = mGetP(strRcvBuf, 5, "|")
                ...
                Case "QRF"
                    ...
                    Call GetOrder_PPC300N(mOrder.BSQryId, gHOSP.RSTTYPE)
...
Private Function ResultProcess(ByVal pBarNo As String, ByVal pIntBase As String, ByVal pResult As String, Optional ByVal pIntResult As String) As Boolean
    Dim RS_L            As ADODB.Recordset
    Dim strOrderCode    As String   '처방코드
    Dim strTestName     As String   '검사코드
    Dim strAbbrName     As String   '검사코드
...
Private Sub SerialRcvData_PPC610H()
    Dim RS_L            As ADODB.Recordset
...
RST:
    strErrMsg = ""
    strErrMsg = strErrMsg & "위    치 : " & gHOSP.MACHNM & "_SerialRcvData_PPC610H" & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류번호 : " & Err.Number & vbNewLine & vbNewLine
    strErrMsg = strErrMsg & "오류내용 : " & Err.Description & vbNewLine
    frmErrMsg.txtErr = vbNewLine & strErrMsg
    frmErrMsg.Show
...
'    End With
    ...
    'If blnOrder = True Then
        Select Case intSndPhase
            ...
            Case 2  '## Patient
                strOutput = intFrameNo & "P|1|" & mOrder.ChartNo & vbCr & ETX
                intSndPhase = 3
                intFrameNo = intFrameNo + 1
...
Private Sub Phase_Serial_PPC610H()
    Dim Buffer      As Variant
    Dim BufChar     As String
    Dim lngBufLen   As Long
    Dim i           As Long
