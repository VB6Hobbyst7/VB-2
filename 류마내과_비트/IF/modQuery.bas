...
On Error GoTo ErrHandle
    ...
    SQL = SQL & "    , substring(W.OcmAcpDtm,1,8) AS HOSPDATE                   " & vbCrLf
...
'SQL = ""
'SQL = SQL & "select distinct ltrim(rtrim(a.ospchtnum)) as pid "
'SQL = SQL & "              , ltrim(rtrim(b.pbspatnam)) as pname"
'SQL = SQL & "              , substring(a.ospodrdtm, 1, 8) as hospdate"
'SQL = SQL & "              , a.ospocmnum as barcode "
'SQL = SQL & "              , a.ospchtnum as chartno "
'SQL = SQL & "from dbo.ospinf a, dbo.pbsinf b, dbo.resinf c, dbo.OcmInf d"
'SQL = SQL & "where a.ospodrdtm between '" & pFrom & "' and '" & pTo & "'   " & vbCrLf
'SQL = SQL & "and a.ospcsmdep = 'LAB'"
'SQL = SQL & "and a.ospdelflg = 'N'"
'SQL = SQL & "and a.ospcanyon = 'N'"
'SQL = SQL & "and a.OspOcmNum = d.OcmNum"
'SQL = SQL & "and d.OcmComStt In ('PN', 'TN', 'OE', 'VN','HN','WN')"
'SQL = SQL & "and d.OcmRsuYon In ('Y', '', 'N')"
'SQL = SQL & "and ltrim(rtrim(b.pbschtnum)) = ltrim(rtrim(a.ospchtnum))"
'SQL = SQL & "and c.resocmnum = a.ospocmnum"
'SQL = SQL & "and c.resodrseq = a.ospodrseq"
'SQL = SQL & "and c.resseq = a.ospseq"
'SQL = SQL & "and upper(ltrim(rtrim(c.reslabcod))) in (" & UCase(gAllTestCd) & ")           " & vbCrLf
'SQL = SQL & "and ltrim(rtrim(isnull(c.resrltval, ''))) = ''"
'SQL = SQL & "order by a.ospchtnum, ptnm"
...
On Error GoTo DBErr
    ...
    SQL = SQL & "      and O.OdrSeq = R.ResOdrSeq                           " & vbCrLf
    SQL = SQL & "      and R.ResLabCod = E.LabCod                           " & vbCrLf
    SQL = SQL & "      and Upper(LTRIM(RTRIM(R.ResLabCod))) IN (" & UCase(gAllTestCd) & ")" & vbCrLf 'Upper(LTRIM(RTRIM(RESLABCOD)))
    SQL = SQL & "      and W.OcmComStt Not In ('CN', 'CR','VC')             " & vbCrLf
