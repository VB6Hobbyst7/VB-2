...
'-----------------------  출처: https://jinwookoh.tistory.com/52 [Programming Note]
Private Declare Function MultiByteToWideChar Lib "kernel32" (ByVal CodePage As Long, ByVal dwFlags As Long, ByVal lpMultiByteStr As Long, ByVal cchMultiByte As Long, ByVal lpWideCharStr As Long, ByVal cchWideChar As Long) As Long
Private Declare Function WideCharToMultiByte Lib "kernel32" (ByVal CodePage As Long, ByVal dwFlags As Long, ByVal lpWideCharStr As Long, ByVal cchWideChar As Long, ByVal lpMultiByteStr As Long, ByVal cchMultiByte As Long, ByVal lpDefaultChar As Long, ByVal lpUsedDefaultChar As Long) As Long
Private Declare Function GetACP Lib "kernel32" () As Long
Private Const CP_ACP = 0 ' default to ANSI code page
...
'출처: https://tiston.tistory.com/entry/VB6-EUC-KR-Encode-URL [?]
...
'''                    STM.Charset = "utf-8"
'''
'''                    strHeader = "MSH|^~\&||||Roche Host|||ORM^O01||" & Format(Now, "yyyymmddhhmmss") & "|P||||ER|ER||8859/1|" & vbCr
'''                    strHeader = strHeader & "PID|1||" & vPID & "||" & vPatNm & "||30|F" & vbCr
'''                    strHeader = strHeader & "ORC|NW|" & vBarNo & "|||||^^^^^R||" & Format(Now, "yyyymmddhhmmss") & vbCr
'''                    'strHeader = strHeader & "OBR||531455510078||TEST|||20140720003156||||A||||||||||||||||||"
'''                    strTestCd = ""
'''
'''                    If CStr(vSTAT) = "일반" Then
''''                            strSPC = mGetP(strRecord, 1, "#")
''''                            strRecord = mGetP(strRecord, 2, "#")
'''                        'OBR||531455510078||TEST|||20140720003156||||A||||||||||||||||||
''''                        If strSPC = "1A" Or strSPC = "1H" Then
'''                        k = UBound(varRecord)
'''                        For J = 0 To k
'''                            strSPC = mGetP(varRecord(J), 1, "#")
'''                            strRecord = mGetP(varRecord(J), 2, "#")
'''                            If J = 0 Then
'''                                If J = k Then
'''                                    strBody = "OBR||" & vBarNo & "||" & strRecord & "|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||"
'''                                Else
'''                                    strBody = "OBR||" & vBarNo & "||" & strRecord & "|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||" & vbCr
'''                                End If
'''                            Else
'''                                If J = k Then
'''                                    strBody = strBody & "OBR||" & vBarNo & "||" & strRecord & "|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||"
'''                                Else
'''                                    strBody = strBody & "OBR||" & vBarNo & "||" & strRecord & "|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||" & vbCr
'''                                End If
'''                            End If
'''                            strTestCd = strTestCd & "'" & strRecord & "',"
'''                        Next
'''                    Else
'''                        strBody = "OBR||" & vBarNo & "||ARCH2|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||"
'''                    End If
'''
'''                    If CStr(vSTAT) = "응급" Then
'''                        strBody = "OBR||" & vBarNo & "||STAT|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||"
'''                    '-- 외주
'''                    ElseIf CStr(vSTAT) = "외주" Then
'''                        strBody = "OBR||" & vBarNo & "||SEND|||" & Format(Now, "yyyymmddhhmmss") & "||||A||||||||||||||||||"
'''                    End If
'''
'''                    STM.WriteText strHeader & strBody
'''
...
'''                    STM.Close
'''                    Set STM = Nothing
Function UTF82String(ByRef data As String) As String
    Dim STM
...
'    STM.Type = 2
    ...
    STM.SaveToFile data, adSaveCreateNotExist
...
Private Function ConvertUtf8ToString(ByRef data As String) As String
    ...
    ' close up and return
    objStream.Close
    ConvertUtf8ToString = strTmp
   
End Function


Private Function ConvertToUtf8_URLEncode(ByRef sText As String) As String
...
Private Function ConvertToUtf8(ByRef sText As String) As String
    ...
    WideCharToMultiByte 65001, 0, StrPtr(sText), Len(utf8bytes(0)), 0, 0, 0, 0
...
Private Function LenH(sText As String) As String
   
    LenH = LenB(StrConv(sText, vbFromUnicode))
    
End Function

Private Function UTF8_Encode(ByRef sStr() As Byte) As String
...
Private Function ChangeChr(pStr As String)
    Dim i       As Integer
    Dim srData  As String
    
    For i = 1 To Len(pStr)
        If Len(Hex(Asc(Mid$(pStr, i, 1)))) = 4 Then
            srData = srData & "%" & Mid$(Hex(Asc(Mid(pStr, i, 1))), 1, 2) & "%" & Mid$(Hex(Asc(Mid(pStr, i, 1))), 3, 2)
        Else
            srData = srData & "%" & Hex(Asc(Mid$(pStr, i, 1)))
        End If
    Next
...
'    strCmnt = ""
'    strCmnt = strCmnt & "1) 검출한계 이하의 바이러스 농도%0D"
'    strCmnt = strCmnt & "2) 너무 이르거나(잠복기) 늦은 시기의 검체%0D"
'    strCmnt = strCmnt & "3) 바이러스 변이 혹은 억제 물질의 존재%0D"
'    strCmnt = strCmnt & "4) 검사전 항바이러스제 투여%0D"
'    strCmnt = strCmnt & "5) 부적절한 검체의 질(부적절 취급 혹은 운송 포함)"
...
'    strCmnt = ""
    ...
    strCmnt = ""
    strCmnt = strCmnt & "위음성이 가능한 원인" & vbCrLf
    strCmnt = strCmnt & "1) 검출한계 이하의 바이러스 농도" & vbCrLf
    strCmnt = strCmnt & "2) 너무 이르거나(잠복기) 늦은 시기의 검체" & vbCrLf
    strCmnt = strCmnt & "3) 바이러스 변이 혹은 억제 물질의 존재" & vbCrLf
    strCmnt = strCmnt & "4) 검사전 항바이러스제 투여" & vbCrLf
    strCmnt = strCmnt & "5) 부적절한 검체의 질(부적절 취급 혹은 운송 포함)"
    
    'strCmnt = StrConv(strCmnt, vbWide)
    'buf = StrConv(strCmnt, vbFromUnicode)
    
    'strCmnt = ConvertToUtf8(strCmnt)
    
    
'    buf = StrConv(strCmnt, vbFromUnicode)
   ' strCmnt = UTF8_Encode(buf)

    'strCmnt = UrlEncode(strCmnt)
    
    
    'Delphi Cmt:= HTTPEncode(UTF8Encode(WideString(Cmt)));
    strCmnt = "%EC%9C%84%EC%9D%8C%EC%84%B1%EC%9D%B4+%EA%B0%80%EB%8A%A5%ED%95%9C+%EC%9B%90%EC%9D%B8%0D%0A1)+%EA%B2%80%EC%B6%9C%ED%95%9C%EA%B3%84+%EC%9D%B4%ED%95%98%EC%9D%98+%EB%B0%94%EC%9D%B4%EB%9F%AC%EC%8A%A4+%EB%86%8D%EB%8F%84%0D%0A2)+%EB%84%88%EB%AC%B4+%EC%9D%B4%EB%A5%B4%EA%B1%B0%EB%82%98(%EC%9E%A0%EB%B3%B5%EA%B8%B0)+%EB%8A%A6%EC%9D%80+%EC%8B%9C%EA%B8%B0%EC%9D%98+%EA%B2%80%EC%B2%B4%0D%0A3)+%EB%B0%94%EC%9D%B4%EB%9F%AC%EC%8A%A4+%EB%B3%80%EC%9D%B4+%ED%98%B9%EC%9D%80+%EC%96%B5%EC%A0%9C+%EB%AC%BC%EC%A7%88%EC%9D%98+%EC%A1%B4%EC%9E%AC%0D%0A4)+%EA%B2%80%EC%82%AC%EC%A0%84+%ED%95%AD%EB%B0%94%EC%9D%B4%EB%9F%AC%EC%8A%A4%EC%A0%9C+%ED%88%AC%EC%97%AC%0D%0A5)+%EB%B6%80%EC%A0%81%EC%A0%88%ED%95%9C+%EA%B2%80%EC%B2%B4%EC%9D%98+%EC%A7%88(%EB%B6%80%EC%A0%81%EC%A0%88+%EC%B7%A8%EA%B8%89+%ED%98%B9%EC%9D%80+%EC%9A%B4%EC%86%A1+%ED%8F%AC%ED%95%A8)"
...
'        End If
                        ...
                        'Else
                            If UCase(sResult) <> "POSITIVE" Then
                                If UCase(sResult) = "NEGATIVE" Then
                                    '2020-07-13수정
                                    'strAllResult = strAllResult & strTestCd & "" & sResult & "" & strCmnt & "" & strDate & ""
                                    strAllResult = strAllResult & strTestCd & "" & sResult & "" & strDate & ""
                                Else
                                    strAllResult = strAllResult & strTestCd & "" & sResult & "" & strDate & ""
                                End If
        ...
        If strAllResult <> "" Then
            ...
            sParam = sParam & "testcont=" & strCmnt & "&"                                   '시행부서 코멘트 내용
            sParam = sParam & "testcontcd=01&"
            sParam = sParam & "execdeptcd=I1"
...
'http://emr013.cmcnu.or.kr/cmcnu/.live?submit_id=TXLII00101&business_id=li&ex_interface=94055501|013&bcno=O370R12B0&result=LPD327Negative20200720152407&instcd=013&eqmtcd=I19&userid=94055501&
'testcont=%EC%9C%84%EC%9D%8C%EC%84%B1%EC%9D%B4+%EA%B0%80%EB%8A%A5%ED%95%9C+%EC%9B%90%EC%9D%B8%0D%0A1)+%EA%B2%80%EC%B6%9C%ED%95%9C%EA%B3%84+%EC%9D%B4%ED%95%98%EC%9D%98+%EB%B0%94%EC%9D%B4%EB%9F%AC%EC%8A%A4+%EB%86%8D%EB%8F%84%0D%0A2)+%EB%84%88%EB%AC%B4+%EC%9D%B4%EB%A5%B4%EA%B1%B0%EB%82%98(%EC%9E%A0%EB%B3%B5%EA%B8%B0)+%EB%8A%A6%EC%9D%80+%EC%8B%9C%EA%B8%B0%EC%9D%98+%EA%B2%80%EC%B2%B4%0D%0A3)+%EB%B0%94%EC%9D%B4%EB%9F%AC%EC%8A%A4+%EB%B3%80%EC%9D%B4+%ED%98%B9%EC%9D%80+%EC%96%B5%EC%A0%9C+%EB%AC%BC%EC%A7%88%EC%9D%98+%EC%A1%B4%EC%9E%AC%0D%0A4)+%EA%B2%80%EC%82%AC%EC%A0%84+%ED%95%AD%EB%B0%94%EC%9D%B4%EB%9F%AC%EC%8A%A4%EC%A0%9C+%ED%88%AC%EC%97%AC%0D%0A5)+%EB%B6%80%EC%A0%81%EC%A0%88%ED%95%9C+%EA%B2%80%EC%B2%B4%EC%9D%98+%EC%A7%88(%EB%B6%80%EC%A0%81%EC%A0%88+%EC%B7%A8%EA%B8%89+%ED%98%B9%EC%9D%80+%EC%9A%B4%EC%86%A1+%ED%8F%AC%ED%95%A8)
'&testcontcd=01&execdeptcd=I1
            

            
            
'http://emr013edu.cmcnu.or.kr/cmcnu/.live?
'submit_id=TXLII00101&
'business_id=li&
'instcd=013&
'result=LPD327Negative20200710&
'ex_interface=10404936|013&
'userid=10404936&
'eqmtcd=I19&
'bcno=I370G0010&
'testcont=test comment &
'testcontcd=01&
'execdeptcd=I1
...
'    Call SetSQLData("바코드조회", "gPatOrdCd :" & gPatOrdCd & vbNewLine, "A")
