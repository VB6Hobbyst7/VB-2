...
Public Function GetUserNm(ByVal PID As String) As String
    ...
    SQL = SQL & "       usrb.USERNM as USERNM " & vbCr       '-- 사용자성명
    ...
    If AdoRs.BOF = False Then
        GetUserNm = AdoRs.Fields("USERNM").Value & ""
    Else
        GetUserNm = ""
    End If
...
'                Debug.Print Trim(RS.Fields("EXAMDATE"))
                If strSaveSeq <> Trim(RS.Fields("SAVESEQ")) & "" Or strExamDate <> Trim(RS.Fields("EXAMDATE")) & "" Then
                    ...
                    '-- 처방의 추가
                    SetText frmMain.spdROrder, Trim(RS.Fields("PANICVALUE")) & "", intRow, colKEY1
...
On Error GoTo DBErr
    ...
    SQL = SQL & ",  (CASE WHEN ACPT.PRCPGENRFLAG NOT IN ('O','S') THEN ( SELECT COM.FN_ZS_GETUSERNM(A.RGSTRID, A.PRCPDD) AS ORDDRNM"
    SQL = SQL & "                                                         FROM EMR.MMOHIPRC A"
    SQL = SQL & "                                                        Where A.INSTCD = ACPT.INSTCD"
    SQL = SQL & "                                                          AND A.ORDDD  = ACPT.ORDDD"
    SQL = SQL & "                                                          AND A.PID    = ACPT.PID"
    SQL = SQL & "                                                          AND A.PRCPDD = ACPT.PRCPDD"
    SQL = SQL & "                                                          AND A.PRCPNO = ACPT.PRCPNO"
    SQL = SQL & "                                                          AND A.PRCPHISTCD = 'O'"
    SQL = SQL & "                                                          AND ROWNUM < = 1 )"
    SQL = SQL & "        ELSE (SELECT COM.FN_ZS_GETUSERNM(A.RGSTRID, A.PRCPDD) AS ORDDRNM"
    SQL = SQL & "                FROM EMR.MMOHOPRC A"
    SQL = SQL & "               Where A.INSTCD = ACPT.INSTCD"
    SQL = SQL & "                 AND A.ORDDD  = ACPT.ORDDD"
    SQL = SQL & "                 AND A.PID    = ACPT.PID"
    SQL = SQL & "                 AND A.PRCPDD = ACPT.PRCPDD"
    ...
    If Not AdoRs.EOF = True And Not AdoRs.BOF = True Then
        Do Until AdoRs.EOF
            With SPD
                ...
                '-- 처방의 추가
                'SetText SPD, Trim(AdoRs.Fields("ORDDRNM")) & "", asRow, colRCNT
                mResult.DoctNm = Trim(AdoRs.Fields("ORDDRNM")) & ""
...
'Public Const colRCNT = 19       '결과갯수                       ==> 검체명
            ...
            '-- 처방의 추가
            'SQL = SQL & ",'" & Trim(GetText(.spdOrder, asRow1, colRCNT)) & "'"
            SQL = SQL & ",'" & mResult.DoctNm & "'"
