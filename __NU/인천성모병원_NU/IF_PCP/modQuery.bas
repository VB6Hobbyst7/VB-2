...
Function SaveTransData_NU(ByVal argSpcRow As Integer, ByVal SPD As Object) As Integer
    ...
    Dim strNPRslt       As String
    Dim strERslt        As String
    Dim strRdrpRslt     As String
    Dim strICARslt      As String
...
On Error GoTo ErrHandle
    ...
    With frmMain
        ...
        strSeq = Trim(GetText(.spdOrder, argSpcRow, colSEQNO))
        ...
        If Trim(strBarcode) = "" Then
            If strSeq = "NC" Then
                ...
                For i = colSTATE + 1 To .spdOrder.MaxCols
                    If GetText(.spdOrder, 0, i) = "E" Then
                        strERslt = GetText(.spdOrder, argSpcRow, i)
                    ElseIf GetText(.spdOrder, 0, i) = "RdRP" Then
                        strRdrpRslt = GetText(.spdOrder, argSpcRow, i)
                    ElseIf GetText(.spdOrder, 0, i) = "Covid19" Then
                        strNPRslt = GetText(.spdOrder, argSpcRow, i)
                    End If
                Next
                varTestCode = Replace(gAllTestCd, "'", "")
                varTestCode = Split(varTestCode, ",")
                
                strAllResult = ""
                For i = 0 To UBound(varTestCode)
                    If Mid(varTestCode(i), 1, 3) = "LPD" Then
                        strAllResult = strAllResult & Trim(varTestCode(i)) & "" & strNPRslt & "" & Format(Now, "yyyymmddhhmmss") & ""
                    End If
                Next
                
                sParam = ""
                sParam = sParam & "submit_id=TXLQI00102&"
                sParam = sParam & "business_id=li&"
                sParam = sParam & "ex_interface=" & frmMain.txtTestID.Text & "|" & gHOSP.HOSPCD & "&"       '사용자ID|기관코드
                sParam = sParam & "ifid=NC_COV_SD&"                                                                '
            ...
            ElseIf strSeq = "PC" Then
                ...
                For i = colSTATE + 1 To .spdOrder.MaxCols
                    ...
                    ElseIf GetText(.spdOrder, 0, i) = "Covid19" Then
                        strNPRslt = GetText(.spdOrder, argSpcRow, i)
                    '-- 2021-01-04 : QC 추가 ==> 코드 = LPD331, 컨트롤 = PC, 결과값 = ICA
                    ElseIf GetText(.spdOrder, 0, i) = "ICA" Then
                        strICARslt = GetText(.spdOrder, argSpcRow, i)
                    End If
                Next
                ...
                '선생님 요구사항대로 하드코딩 한다.
                For i = 0 To UBound(varTestCode)
                    If Mid(varTestCode(i), 1, 6) = "LPD328" Then
                        strAllResult = strAllResult & Trim(varTestCode(i)) & "" & strRdrpRslt & "" & Format(Now, "yyyymmddhhmmss") & ""
                    ElseIf Mid(varTestCode(i), 1, 6) = "LPD329" Then
                        strAllResult = strAllResult & Trim(varTestCode(i)) & "" & strERslt & "" & Format(Now, "yyyymmddhhmmss") & ""
                    '-- 2021-01-04 : QC 추가 ==> 코드 = LPD331, 컨트롤 = PC, 결과값 = ICA
                    ElseIf Mid(varTestCode(i), 1, 6) = "LPD331" Then
                        strAllResult = strAllResult & Trim(varTestCode(i)) & "" & strICARslt & "" & Format(Now, "yyyymmddhhmmss") & ""
                    'ElseIf Mid(varTestCode(i), 1, 3) = "LPD" Then
                    '    strAllResult = strAllResult & Trim(varTestCode(i)) & "" & "Positive" & "" & Format(Now, "yyyymmddhhmmss") & ""
                    End If
                Next
        ...
        '정도관리 결과저장
'        If Trim(strBarcode) = "NC" Then
'            'gAllTestCd = 'ZZZZZ','YYYYY','XXXXX','LPD336','LPD331','LPD329','LPD328'
'            varTestCode = Replace(gAllTestCd, "'", "")
'            varTestCode = Split(varTestCode, ",")
'
'            strAllResult = ""
'            For i = 0 To UBound(varTestCode)
'                If Mid(varTestCode(i), 1, 3) = "LPD" Then
'                    strAllResult = strAllResult & Trim(varTestCode(i)) & "" & "NC" & "" & Format(Now, "yyyymmddhhmmss") & ""
'                End If
'            Next
'
'            sParam = ""
...
'            Exit Function
'
'        ElseIf Trim(strBarcode) = "PC" Then
'            varTestCode = Replace(gAllTestCd, "'", "")
'            varTestCode = Split(varTestCode, ",")
'
'            strAllResult = ""
'            For i = 0 To UBound(varTestCode)
'                If Mid(varTestCode(i), 1, 3) = "LPD" Then
'                    strAllResult = strAllResult & Trim(varTestCode(i)) & "" & "PC" & "" & Format(Now, "yyyymmddhhmmss") & ""
'                End If
'            Next
'
'            sParam = ""
...
'                If XmlSelectS.BCNO(i) = "O675J28N0" Then
'                    MsgBox XmlSelectS.BCNO(i)
'
'                    MsgBox XmlSelectS.IFREQCDLIST(i)
'
'                    MsgBox XmlSelectS.TCLSCDLIST(i)
'
'                End If
                
                If InStr(UCase(XmlSelectS.IFREQCDLIST(i)), "PCP") > 0 Then
                    'XmlSelectS.TCLSCDLIST(i) = Replace(XmlSelectS.TCLSCDLIST(i), "▦", "")
                    'LPD329-▦
                    'LPD331-▦LPD331-▦
                    varTestCd = Split(XmlSelectS.TCLSCDLIST(i), "▦")
                    
                    blnAdd = False
                    
                    For m = 0 To UBound(varTestCd) - 1
                        XmlSelectS.TCLSCDLIST(i) = varTestCd(m)
                        
                        If XmlSelectS.TCLSCDLIST(i) <> "" And blnAdd = False Then
                            If InStr(1, gAllTestCd, XmlSelectS.TCLSCDLIST(i)) > 0 Then
                                blnAdd = True
