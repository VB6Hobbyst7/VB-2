...
On Error GoTo ErrHandle
    ...
    With frmInterface
        ...
        strName = Trim(GetText(.vasID, argSpcRow, colPNAME))
        ...
        strRegDate = Format(strRegDate, "yyyymmddhhmmss")
        ...
        For iRow = 1 To 30 '.vasTemp.DataRowCnt
            ...
            Select Case sResult1
                Case 0 To 0.34:         sResult = "  -   /   -  "
                Case Else:              sResult = sResult1 & " / " & sResult2
            End Select

            If strEqpCd <> "" Then
                If Mid(lsID, 1, 1) = "Q" Then
                    '검사코드,결과값,코멘트,결과보고일자,결과상태 ex) LIA196013201202274LIA1960252012
                    'strAllResult = strAllResult & strEqpCd & "" & sResult & "" & Format(Now, "yyyymmddhhmmss") & ""
                    strAllResult = strAllResult & strEqpCd & "" & sResult & "" & strRegDate & ""
                Else
                    '검사코드,결과값,코멘트,결과보고일자,결과상태 ex) LIA196013201202274LIA1960252012
                    '-- 결과상태 : 결과입력 1, 중간보고 2, 최종보고 4
                    strAllResult = strAllResult & strEqpCd & "" & sResult & "" & Format(Now, "yyyymmdd") & "" & "1" & ""
                End If
            End If
            
        Next iRow
        
        SetRawData "[strAllResult]" & strAllResult
        ...
        If strAllResult <> "" Then
            If Mid(lsID, 1, 1) = "Q" Then
                ...
                'sParam = sParam & "ifid=" & strName & "&"                                   '물질코드   'NU기준자료에서 정도관리->정도관리기초정보관리->QC물질관리 화면 들어가서 지금 하는 검사 물질코드 찾으신다음에 맨 끝에 IF연동코드 여기에 저코드가 있어야되요
            ...
            SetRawData "[서버저장 : URL]" & NUAPI.APIURL & sParam
            SetRawData "[서버저장 : Param]" & sParam & vbNewLine & "[서버저장 : Return]" & sRcvData
        ...
        strAllResult = ""
        '-- 서버로 결과값 저장하기
        For iRow = 31 To .vasTemp.DataRowCnt
            strEqpCd = Trim(GetText(.vasTemp, iRow, 2))
...
On Error GoTo ErrorTrap
    ...
    If Mid(sBarcode, 1, 1) <> "Q" Then
        ...
        Call SetSQLData("바코드조회", NUAPI.APIURL & sParam & vbNewLine & sRcvData)
    ...
    Else
        ...
        sParam = sParam & "business_id=lis&"                                        'business_id
        sParam = sParam & "ex_interface=" & NUAPI.UID & "|" & NUAPI.HOSPCD & "&"    '사용자ID|기관코드
        sParam = sParam & "bcno=" & sBarcode & "&"                       'QC바코드
        sParam = sParam & "eqmtcd=" & NUAPI.INSTCD & "&"                            '장비코드
        sParam = sParam & "instcd=" & NUAPI.HOSPCD & "&"                            '기관코드
        sParam = sParam & "userid=" & NUAPI.UID                                     '사용자ID
        ...
        If UBound(varRcvData) >= 0 Then
            ...
            With frmInterface.vasID
                ...
                .SetText colHOSPDATE, intRow, Format(Mid(varRcvData(1), 1, 8), "####-##-##")
                '.SetText colIO, intRow, varRcvData(2) & ""
                .SetText colBARCODE, intRow, varRcvData(2) & ""
                '.SetText colPID, intRow, varRcvData(4) & ""
                
                '.SetText colPNAME, intRow, varRcvData(7) & ""
                If varRcvData(7) & "" = "41" Then
                    .SetText colPNAME, intRow, "QCINNEG"
                ElseIf varRcvData(7) & "" = "42" Then
                    .SetText colPNAME, intRow, "QCINPOS"
                ElseIf varRcvData(7) & "" = "SJ" Then
                    .SetText colPNAME, intRow, "QCFDNEG"
                ElseIf varRcvData(7) & "" = "SK" Then
                    .SetText colPNAME, intRow, "QCFDPOS"
                End If
                
                'QCFDNEG
                'QCFDPOS
                
                '.SetText colPSEX, intRow, varRcvData(7) & ""
                '.SetText colPSEX, intRow, mGetP(varRcvData(6) & "", 1, "/")
                '.SetText colPAGE, intRow, mGetP(varRcvData(6) & "", 2, "/")
                '.SetText colER, intRow, varRcvData(7) & ""
                '.SetText colWORKNO, intRow, varRcvData(8) & ""
                ...
                strTestCds = varRcvData(3) & ""
                strTestCds = Replace(strTestCds, "▦", "")
                
                If InStr(varRcvData(3) & "", "LIM305") > 0 Then
                    .SetText colASSAYNM, intRow, "Inhalant"
                ElseIf InStr(varRcvData(3) & "", "LIM306") > 0 Then
                    .SetText colASSAYNM, intRow, "Food"
                End If
