...
'
'        Case "BRAIN"
'                Call GetWorkList_BRAIN(pFrom, pTo, SPD)
'
'        Case "BIT"                          '비트
...
Public Sub GetWorkList_BRAIN(ByVal pFrom As String, ByVal pTo As String, ByVal SPD As vaSpread)
    Dim RS          As ADODB.Recordset
    Dim blnSame     As Boolean
...
'    SQL = ""
'    SQL = SQL & "Select DISTINCT "
'    SQL = SQL & "       a.slabw_date            AS HOSPDATE     " & vbCr '-- 접수일
'    SQL = SQL & "     , a.slabw_cnt             AS BARCODE      " & vbCr '-- 접수번호(바코드)
'    SQL = SQL & "     , a.slabw_cham            AS PNAME        " & vbCr '-- 이름
'    SQL = SQL & "     , c.cham_whanja           AS CHARTNO      " & vbCr '-- 차트번호
'    SQL = SQL & "     , COUNT(c.sLabws_momu)    AS CNT          " & vbCr
'    SQL = SQL & "  From AspCham c   "
'    SQL = SQL & "     , AspsLabw a  "
'    SQL = SQL & "     , AspsLabws b " & vbCr
'    SQL = SQL & " Where b.slabws_date BETWEEN '" & pFrom & "' AND '" & pTo & "' " & vbCr
'    SQL = SQL & "   And b.slabws_momu  IN (" & gAllTestCd & ")                   " & vbCr
'    SQL = SQL & "   And a.slabw_hid     = '" & gHOSP.HOSPCD & "'                " & vbCr    '-- 병원코드(150000)
'    SQL = SQL & "   And a.slabw_status IN (0,1)                                 " & vbCr    '-- 0:대기, 1:진행
'    SQL = SQL & "   And a.slabw_hid     = b.slabws_hid                          " & vbCr
'    SQL = SQL & "   And a.slabw_date    = b.slabws_date                         " & vbCr
'    SQL = SQL & "   And a.slabw_cnt     = b.slabws_cnt                          " & vbCr
'    SQL = SQL & "   And c.cham_Key      = a.slabw_cham                          " & vbCr
'    SQL = SQL & " Group By a.slabw_date, a.slabw_cnt, a.slabw_cham, a.slabw_status, c.cham_whanja " & vbCr
'    SQL = SQL & " Order By a.slabw_date , a.slabw_cnt                           " & vbCr
   
    SQL = ""
    SQL = SQL & "SELECT DISTINCT "
    SQL = SQL & "       SLABW_INOUT             AS INOUT    " & vbCr
    SQL = SQL & "     , SLABWS_DATE             AS HOSPDATE " & vbCr
    SQL = SQL & "     , CHAM_KEY                AS BARCODE  " & vbCr
    SQL = SQL & "     , CHAM_WHANJA             AS PNAME    " & vbCr
    SQL = SQL & "     , CHAM_JUMIN1             AS JUMIN1   " & vbCr
    SQL = SQL & "     , CHAM_JUMIN2             AS JUMIN2   " & vbCr
    SQL = SQL & "     , SPECI_DATE              AS CHARTNO " & vbCr
    SQL = SQL & "     , SPECI_SEQNO             AS PID      " & vbCr '결과저장 Key
    'SQL = SQL & "     , COUNT(E.slabs_rcode)    AS CNT      " & vbCr
    SQL = SQL & "  FROM BRWONMU..WCHAM A                                                " & vbCr
    SQL = SQL & "       INNER JOIN OSLABW B     ON A.CHAM_KEY = B.SLABW_CHAM            " & vbCr
    
    If frmMain.optSch(0).Value = True Then
        SQL = SQL & "                               AND B.slabw_status = '0'                " & vbCr
    ElseIf frmMain.optSch(1).Value = True Then
        SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
    ElseIf frmMain.optSch(2).Value = True Then
        SQL = SQL & "                               AND B.slabw_status = '2'                " & vbCr
    Else
        SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
    End If
    
    SQL = SQL & "       INNER JOIN OSLABWS C    ON B.SLABW_DATE = C.SLABWS_DATE         " & vbCr
    SQL = SQL & "                               AND B.slabw_dept = C.slabws_dept        " & vbCr
    SQL = SQL & "                               AND B.slabw_cnt = C.slabws_cnt          " & vbCr
    SQL = SQL & "                               AND B.slabw_slab = C.slabws_slab        " & vbCr
    SQL = SQL & "                               AND C.slabws_date >= '" & pFrom & "'    " & vbCr
    SQL = SQL & "                               AND C.slabws_date <= '" & pTo & "'      " & vbCr
    SQL = SQL & "       INNER JOIN OSLABS E     ON C.SLABWS_SCNT = E.SLABS_CNT          " & vbCr
    SQL = SQL & "                               AND C.slabws_slab = E.slabs_key         " & vbCr
    SQL = SQL & "                               AND E.slabs_rcode IN (" & gAllTestCd & ")" & vbCr
    SQL = SQL & "                               AND E.slabs_use  = 1                    " & vbCr
    SQL = SQL & "       INNER JOIN Ospecislab F ON B.slabw_cnt = F.specis_cnt           " & vbCr
    SQL = SQL & "                               AND B.slabw_date = F.specis_date        " & vbCr
    SQL = SQL & "                               AND B.slabw_dept = F.specis_dept        " & vbCr
    SQL = SQL & "                               AND F.specis_deleted = 0                " & vbCr
    SQL = SQL & "       INNER JOIN OSPECIMEN S  ON A.cham_key = S.SPECI_CHAM            " & vbCr
    SQL = SQL & "                               AND F.specis_date = S.speci_date        " & vbCr
    SQL = SQL & "                               AND F.specis_seqno = S.speci_seqno      " & vbCr
    'SQL = SQL & " GROUP BY SLABW_INOUT, SLABWS_DATE, CHAM_KEY, CHAM_WHANJA, SPECI_DATE, SPECI_SEQNO " & vbCr
    SQL = SQL & " ORDER BY SPECI_DATE, SPECI_SEQNO                                      " & vbCr
...
'                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "", intRow, colCHARTNO
                    ...
                    SetText SPD, Trim(RS.Fields("CHARTNO")) & "" & Trim(RS.Fields("PID")), intRow, colCHARTNO
                    
                    SetText SPD, IIf(Trim(RS.Fields("INOUT")) & "" = "0", "입원", "외래"), intRow, colINOUT
                    'SetText SPD, Trim(RS.Fields("CNT")) & "", intRow, colOCNT
                    SetText SPD, GetSampleITEM(intRow, SPD), intRow, colITEMS
                    If gWORKPOS = "P" Then
                        SetText SPD, frmWorkList.txtSeqNo.Text, intRow, colSEQNO
                    ...
                    Else
                        ...
                        frmMain.txtSeqNo.Text = frmMain.txtSeqNo.Text + 1
                    End If
                    frmMain.txtSeqNo.Text = Format(frmMain.txtSeqNo.Text, "000")
                End If
            End With
...
'    sParam = sParam & "enddd=" & sSch2 & "&"                                    '종료작업일자
    ...
    'SERVERIP + "/himed2/.live?submit_id=TRLII00123&business_id=lis&instcd=053&startdd=" + argStartDate + "&enddd=" + argEnddate + "&testcd=" + argLisCode + "";
...
'★ 검사장비& 바코드번호로 오더조회
'http://emr001edu.cmcnu.or.kr/cmcnu/.live?submit_id=TRLII00101&business_id=li&bcno=I26I100Z0&instcd=015&eqmtcd=H13
        ...
        For i = 1 To UBound(varRcvData) Step 14
            With SPD
                ...
                If blnSame = False Then
                    ...
                    SetText SPD, "1", intRow, colCHECKBOX
                    SetText SPD, Mid(varRcvData(i), 1, 8), intRow, colHOSPDATE
...
'    SQL = SQL & "   AND L.LABCHTNUM = M.MANCHTNUM       " & vbCr                    '-- 챠트번호
    ...
    SQL = SQL & " ORDER BY L.LABODRDTE, L.LABCHTNUM, L.LABBARCOD                       " & vbCr
...
Function GetSampleITEM(ByVal asRow As Long, ByVal SPD As vaSpread) As String
    ...
    Select Case gEMR
        ...
        Case "BRAIN"
'            SQL = ""
'            SQL = SQL & "Select sLabws_momu AS ITEM                         " & vbCr '--검사코드"
'            SQL = SQL & "  From AspsLabws                                   " & vbCr
'            SQL = SQL & " where sLabws_cnt  = '" & strBarcode & "'          " & vbCr
'            SQL = SQL & "   and sLabws_date = '" & strRegDate & "'          " & vbCr
'            SQL = SQL & "   and slabws_momu IN (" & gAllTestCd & ")         " & vbCr
'            SQL = SQL & "   and sLabws_hid  = '" & gHOSP.HOSPCD & "'        " & vbCr
'            SQL = SQL & " Order By sLabws_momu                              " & vbCr
        
            SQL = ""
            SQL = SQL & "SELECT DISTINCT E.slabs_rcode    AS ITEM      " & vbCr
            SQL = SQL & "  FROM BRWONMU..WCHAM A                                                " & vbCr
            SQL = SQL & "       INNER JOIN OSLABW B     ON A.CHAM_KEY = B.SLABW_CHAM            " & vbCr
            SQL = SQL & "                               AND A.CHAM_KEY = '" & strBarcode & "'   " & vbCr
            'SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
            ...
            SQL = SQL & "                               AND B.slabw_cnt = C.slabws_cnt          " & vbCr
            SQL = SQL & "                               AND B.slabw_slab = C.slabws_slab        " & vbCr
            SQL = SQL & "                               AND C.slabws_date = '" & strRegDate & "'" & vbCr
            SQL = SQL & "       INNER JOIN OSLABS E     ON C.SLABWS_SCNT = E.SLABS_CNT          " & vbCr
            SQL = SQL & "                               AND C.slabws_slab = E.slabs_key         " & vbCr
            SQL = SQL & "                               AND E.slabs_rcode IN (" & gAllTestCd & ")" & vbCr
            SQL = SQL & "                               AND E.slabs_use  = 1                    " & vbCr
            SQL = SQL & " ORDER BY E.slabs_rcode                                     " & vbCr
...
'
'        Case "BUSANUNIV"
'                Call GetSampleInfo_BUSANUNIV(asRow, SPD)
'
'        Case "EMEDI"
'                Call GetSampleInfo_AMIS(asRow, SPD)
'
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_BUSANUNIV(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim RS1             As ADODB.Recordset
    Dim strRegDate      As String
    Dim strOrgBarcode   As String
    Dim strBarcode      As String
...
On Error GoTo DBErr
    ...
    AdoCmd.CommandText = "ext_all_objects.xpr_gaon_patlist"
    AdoCmd.CommandType = adCmdStoredProc

    Set prm1 = AdoCmd.CreateParameter("P_ODRDTE", adVarChar, adParamInput, 4000, strDate)
    AdoCmd.Parameters.Append prm1
    Set prm2 = AdoCmd.CreateParameter("P_BCD_SGTIN", adVarChar, adParamInput, 4000, strBarcode)
    AdoCmd.Parameters.Append prm2

    SQL = ""
    SQL = SQL & "Exec ext_all_objects.xpr_gaon_patlist "
    SQL = SQL & "  '" & strDate & "'"
    SQL = SQL & " ,'" & strBarcode & "'"
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                'intTestCnt = intTestCnt + 1
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, Trim(RS.Fields("bcd_coddte")) & "", asRow, colHOSPDATE
                SetText SPD, Trim(RS.Fields("bcd_patnum")) & "", asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("bcd_patnum")) & "", asRow, colPID
                SetText SPD, Trim(RS.Fields("bcd_pattyp")) & "", asRow, colINOUT
                SetText SPD, Trim(RS.Fields("bcd_patnam")) & "", asRow, colPNAME
                SetText SPD, Trim(RS.Fields("bcd_patsex")) & "", asRow, colPSEX
                SetText SPD, Trim(RS.Fields("bcd_patage")) & "", asRow, colPAGE
                'SetText SPD, Trim(RS.Fields("bcd_status")) & "", asRow, colSTATE '접수
                ...
                With mOrder
                    .BarNo = strOrgBarcode
                    .PID = Trim(RS.Fields("bcd_patnum")) & ""
                    .PName = Trim(RS.Fields("bcd_patnam")) & ""
                    .Count = CStr(intTestCnt)
                    .NoOrder = False
                End With
    ...
    Call GetSampleInfoTest_BUSANUNIV(asRow, SPD)
...
'-- 검사자 오더정보 가져오기
Function GetSampleInfoTest_BUSANUNIV(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim RS1             As ADODB.Recordset
    Dim strRegDate      As String
    Dim strOrgBarcode   As String
    Dim strBarcode      As String
...
'    Screen.MousePointer = 11
    ...
    AdoCmd.CommandTimeout = 15
    AdoCmd.CommandText = "ext_all_objects.xpr_gaon_odrlist"
    AdoCmd.CommandType = adCmdStoredProc
    Set prm1 = AdoCmd.CreateParameter("P_BCD_SGTIN", adVarChar, adParamInput, 4000, strBarcode)
    AdoCmd.Parameters.Append prm1
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                '-- 화면에 표시
                For intCol = colSTATE + 1 To frmMain.spdOrder.MaxCols
                    If Trim(RS.Fields("bcd_code")) = gArrEQP(intCol - colSTATE, 2) Then
                        .Row = asRow
                        .Col = intCol
                        .BackColor = vbYellow
                        Call SetText(SPD, "◇", asRow, intCol)

                        gPatOrdCd = gPatOrdCd & "'" & Trim(RS.Fields("bcd_code")) & "',"
                        Exit For
                    End If
                Next
            End With
...
'    sParam = sParam & "bcno=" & sBarcode                                                        '바코드
    ...
    '양지
    ''submit_id=TRLII90101&business_id=li&bcno=E46KY00W0&instcd=204&eqmtcd=C03&
'    sParam = ""
'    sParam = sParam & "submit_id=TRLII90101&"                                   'submit ID
'    sParam = sParam & "business_id=li&"                                         'business_id
'    sParam = sParam & "bcno=" & strBarcode                                      '
'    sParam = sParam & "instcd=" & gHOSP.HOSPCD & "&"                            '기관코드
...
'sRcvData = sRcvData & "<root><worklist><bcno><![CDATA[3010700060]]></bcno><patnm><![CDATA[이난희]]></patnm><prgstno><![CDATA[720922-2******]]></prgstno><pid><![CDATA[000102480]]></pid><sex><![CDATA[F]]></sex><age><![CDATA[46]]></age><spcnm><![CDATA[Throat swab]]></spcnm><spccd><![CDATA[023]]></spccd><tclscd><![CDATA[VB6012A]]></tclscd><spcstat><![CDATA[4]]></spcstat><rsltstat><![CDATA[-]]></rsltstat><workno><![CDATA[20181217I20005]]></workno><testcd><![CDATA[VB6012A]]></testcd><execprcpuniqno><![CDATA[2002642756]]></execprcpuniqno><spcacptdt><![CDATA[20181217101911]]></spcacptdt><prcpdd><![CDATA[20181217]]></prcpdd><retestyn><![CDATA[N]]></retestyn><testlrgcd><![CDATA[I]]></testlrgcd><orddeptcd><![CDATA[IM]]></orddeptcd></worklist><message><type>info</type><code>info</code><msg>정상적으로 처리되었습니다.</msg><description></description></message>"
...
'CDATA[20190219140638]]></spcacptdt><acptflag><![
'CDATA[입원]]></acptflag><bcno><![
'CDATA[I46KZ0730]]></bcno><pid><![
'CDATA[19005172]]></pid><patnm><![
'CDATA[박주연]]></patnm><sexage><![
'CDATA[M/35]]></sexage><erprcpflag><![
'CDATA[N]]></erprcpflag><workno><![
'CDATA[20190219C300118]]></workno><tsectnm><![
'CDATA[특수화학]]></tsectnm><ifreqcdlist><![
'CDATA[pO2▦K+▦pCO2▦TCO2▦BE(B)▦Glu▦HCO3-▦Hct▦pH▦THbc▦SO2c▦Na+▦]]></ifreqcdlist><tclscdlist><![
'CDATA[LCAM10003▦LCAM103▦LCAM10002▦LCAM101▦LCA104▦LCAM105▦LCA103▦LCAM104▦LCAM10001▦LCA109▦LCA106▦LCAM102▦]]></tclscdlist><urinextrvol><![
'CDATA[ ]]></urinextrvol><retestyn><![CDATA[N▦N▦N▦N▦N▦N▦N▦N▦N▦N▦N▦N▦]]></retestyn><rsltstat><![
'CDATA[LCAM10003-▦LCAM103-▦LCAM10002-▦LCAM101-▦LCA104-▦LCAM105-▦LCA103-▦LCAM104-▦LCAM10001-▦LCA109-▦LCA106-▦LCAM102-▦]]></rsltstat><orddeptnm><![CDATA[ME]]></orddeptnm><orddrnm><![CDATA[이해리]]></orddrnm><wardcd><![CDATA[61W]]></wardcd></worklist><resultKM error="no" type="status" clear="true" description="info||정상 처리되었습니다." updateinstance="true" source="1550554749773"/>
'</spcworklist></root>
    ...
    If UBound(varRcvData) >= 0 Then
        ...
        For i = 1 To UBound(varRcvData) 'Step 19
            With SPD
                SetText SPD, "1", asRow, colCHECKBOX
                SetText SPD, Mid(varRcvData(1), 1, 8), asRow, colHOSPDATE
                ...
                strSex = mGetP(varRcvData(6) & "", 1, "/")
                strAge = mGetP(varRcvData(6) & "", 2, "/")
                
                SetText SPD, strSex, asRow, colPSEX
                SetText SPD, strAge, asRow, colPAGE
                'SetText SPD, varRcvData(3) & "", asRow, colPJUMIN
                SetText SPD, varRcvData(8) & "", asRow, colCHARTNO
...
'                    gPatOrdCd = "'" & varRcvData(11) & "',"
'                End If
'                    If Trim(varRcvData(9) & "") = "-" Then
'                        gPatOrdCd = gPatOrdCd & "'" & "VB6012A" & "',"
'                    Else
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_BRAIN(ByVal asRow As Long, ByVal SPD As vaSpread) As Integer
    Dim strRegDate      As String
    Dim strBarcode      As String
    Dim strPatID        As String
    Dim strChartNo      As String
...
'    SQL = SQL & "     , a.slabw_cham            AS PNAME        " & vbCr '-- 이름
'    SQL = SQL & "     , c.cham_whanja           AS CHARTNO      " & vbCr '-- 차트번호
'    SQL = SQL & "     , c.sLabws_momu           AS ITEM         " & vbCr
'    SQL = SQL & "     , c.slabws_scnt           AS SUBCODE      " & vbCr '-- 검사순번? (Sub코드로 사용) "
'    SQL = SQL & "  From AspCham c   "
'    SQL = SQL & "     , AspsLabw a  "
'    SQL = SQL & "     , AspsLabws b " & vbCr
'    SQL = SQL & " Where sLabws_cnt = '" & strBarcode & "'                       " & vbCr
'    SQL = SQL & "   And b.slabws_date = '" & strRegDate & "' "
'    SQL = SQL & "   And b.slabws_momu  IN (" & gAllTestCd & ")                   " & vbCr
'    SQL = SQL & "   And a.slabw_hid     = '" & gHOSP.HOSPCD & "'                " & vbCr    '-- 병원코드(150000)
'    SQL = SQL & "   And a.slabw_status IN (0,1)                                 " & vbCr    '-- 0:대기, 1:진행
'    SQL = SQL & "   And a.slabw_hid     = b.slabws_hid                          " & vbCr
...
'    SQL = SQL & " Order By a.slabw_date , a.slabw_cnt                           " & vbCr
    ...
    SQL = SQL & "     , SPECI_DATE              AS CHARTNO " & vbCr
    SQL = SQL & "     , SPECI_SEQNO             AS PID      " & vbCr '결과저장 Key
    SQL = SQL & "     , E.slabs_rcode           AS ITEM      " & vbCr
    SQL = SQL & "  FROM BRWONMU..WCHAM A                                                " & vbCr
    SQL = SQL & "       INNER JOIN OSLABW B     ON A.CHAM_KEY = B.SLABW_CHAM            " & vbCr
'    SQL = SQL & "                               AND B.slabw_status = '1'                " & vbCr
    If frmMain.optSch(0).Value = True Then
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        Do Until RS.EOF
            With SPD
                ...
                SetText SPD, Trim(RS.Fields("HOSPDATE")) & "", asRow, colHOSPDATE
                SetText SPD, Trim(RS.Fields("BARCODE")), asRow, colBARCODE
                SetText SPD, Trim(RS.Fields("CHARTNO")) & "" & Trim(RS.Fields("PID")), asRow, colCHARTNO
                SetText SPD, Trim(RS.Fields("PNAME")) & "", asRow, colPNAME
                SetText SPD, IIf(Trim(RS.Fields("INOUT")) & "" = "0", "입원", "외래"), asRow, colINOUT
...
'Public Function Get_ResultList_ELP(ByVal strPrgCd As String, ByVal pIdx As Integer, Optional ByVal pBarcode As String) As ADODB.Recordset
...
'    If pIdx = 1 Then
'        strSQL = ""
'        strSQL = strSQL & "Select 'ELP' as TYPE, Id, Data_analisi, Seq, Programma, PT, NumFraz,             " & vbNewLine
'        strSQL = strSQL & "       Nome_1,Nome_2,Nome_3,Nome_4,Nome_5,Nome_6,Nome_7,Nome_8,Nome_9,Nome_10,   " & vbNewLine
'        strSQL = strSQL & "       Fraz_1,Fraz_2,Fraz_3,Fraz_4,Fraz_5,Fraz_6,Fraz_7,Fraz_8,Fraz_9,Fraz_10,   " & vbNewLine
'        strSQL = strSQL & "       Nome_Picco_1, Nome_Picco_2, Val_Picco_1, Val_Picco_2, Coord_Picco,        " & vbNewLine
'        strSQL = strSQL & "       Nome_Picco_3, Nome_Picco_4, Val_Picco_3, Val_Picco_4,                     " & vbNewLine
'        strSQL = strSQL & "       Nome_Picco_5, Nome_Picco_6, Val_Picco_5, Val_Picco_6,                     " & vbNewLine
'        strSQL = strSQL & "       Nome_Picco_7, Nome_Picco_8, Val_Picco_7, Val_Picco_8,                     " & vbNewLine
'        strSQL = strSQL & "       Paz_Noto,ScalaMan,Curva,Rapp1,Rapp2,Commento1                             " & vbNewLine
'        strSQL = strSQL & "  From Anagrafica                                                                " & vbNewLine
'        strSQL = strSQL & " Where substring(ID,1,2) <> 'CO'                                                 " & vbNewLine
'        strSQL = strSQL & "   And substring(ID,2,1) <> '?'                                                  " & vbNewLine
'        strSQL = strSQL & "   And Programma = '" & strPrgCd & "'                                            " & vbNewLine
'        strSQL = strSQL & "   And substring(Id, 13,2) = 'EL'                                                " & vbNewLine
'        strSQL = strSQL & "   And Data_analisi = '" & Format(dtpResult.Value, "yyyy-mm-dd") & "'            " & vbNewLine
'        strSQL = strSQL & "  Order By Seq "
'
'        Set AdoRs = New ADODB.Recordset
'        If Get_Recordset(AdoCn, strSQL, AdoRs, "") Then
'            Set Get_ResultList_ELP = AdoRs
'        Else
'            Set Get_ResultList_ELP = Nothing
'        End If
'        Set AdoRs = Nothing
'
'    Else
'        strSQL = ""
'        strSQL = strSQL & "Select substring(Id, 13,3) as TYPE,Id,Data_analisi,Seq,Programma, PT, NumFraz,   " & vbNewLine
'        strSQL = strSQL & "       Nome_1,Nome_2,Nome_3,Nome_4,Nome_5,Nome_6,Nome_7,Nome_8,Nome_9,Nome_10,   " & vbNewLine
'        strSQL = strSQL & "       Fraz_1,Fraz_2,Fraz_3,Fraz_4,Fraz_5,Fraz_6,Fraz_7,Fraz_8,Fraz_9,Fraz_10,   " & vbNewLine
'        strSQL = strSQL & "       Nome_Picco_1, Nome_Picco_2, Val_Picco_1, Val_Picco_2, Coord_Picco,        " & vbNewLine
'        strSQL = strSQL & "       Nome_Picco_3, Nome_Picco_4, Val_Picco_3, Val_Picco_4,                     " & vbNewLine
...
'        strSQL = strSQL & "       Paz_Noto,ScalaMan,Curva,Rapp1,Rapp2,Commento1                             " & vbNewLine
'        strSQL = strSQL & "  From Anagrafica                                                                " & vbNewLine
'        strSQL = strSQL & " Where Programma = '" & strPrgCd & "'                                            " & vbNewLine
'        strSQL = strSQL & "   And substring(Id, 1,11) = '" & Mid(pBarcode, 1, 11) & "'                      " & vbNewLine
'        strSQL = strSQL & "   And Data_analisi = '" & Format(dtpResult.Value, "yyyy-mm-dd") & "'            " & vbNewLine
'        strSQL = strSQL & "   And substring(Id, 13,2) <> 'EL'                                               " & vbNewLine
'        strSQL = strSQL & "   And substring(Id, 13,2) <> 'Re'                                               " & vbNewLine
'        strSQL = strSQL & "  Order By Seq "
'
'        Set AdoRs = New ADODB.Recordset
'        If Get_Recordset(AdoCn, strSQL, AdoRs, "") Then
