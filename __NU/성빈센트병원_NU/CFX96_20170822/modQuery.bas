...
'AS CHARTNO, ACPT.acptstatcd as STATCD , ACPT.pid AS PID, ACPT.prcpgenrflag AS INOUT, ACPT.ORDDEPTCD AS RACKNO, DEPT.deptengabbr
...
'ACPT.acptdd BETWEEN :V00002 AND :V00003     AND ACPT.testcd IN (:V00004)     AND ACPT.acptstatcd = 0     AND ACPT.instcd
...
'''            SQL = SQL & "select /*+ leading(acpt) */"
'''            SQL = SQL & "    acpt.instcd , --기관기호"
'''            SQL = SQL & "    acpt.acptdd , --접수일자"
'''            SQL = SQL & "    acpt.acptno , --접수번호"
'''            SQL = SQL & "    acpt.acptitemno , --접수항목번호"
'''            SQL = SQL & "    acpt.PTNO , --병리번호"
'''            SQL = SQL & "    acpt.acptstatcd     --접수상태코드(0:접수(440),1:취소(100,240),2:예비결과(710),3:최종결과(730), 4:수정결과(740))"
'''            SQL = SQL & "    acpt.PID , --등록번호"
'''            SQL = SQL & "    acpt.TESTCD , --검사코드"
'''            SQL = SQL & "    test.testengnm , --영문검사명"
'''            SQL = SQL & "    spcm.SPCNM , --검체명"
'''            SQL = SQL & "    acpt.prcpgenrflag , --입원 / 외래구분"
'''            SQL = SQL & "    acpt.orddeptcd , --진료과코드"
'''            SQL = SQL & "    dept.deptengabbr , --진료과명"
'''            SQL = SQL & "    acpt.prcpdd,        --처방일자,"
'''            SQL = SQL & "    acpt.execprcpuniqno , --실시처방유일번호"
'''            SQL = SQL & "    acpt.prcpno , --처방번호"
'''            SQL = SQL & "    ptbs.hngnm , --환자명"
'''            SQL = SQL & "    ptbs.sex , --성별"
'''            SQL = SQL & "    ptbs.brthdd , --생일"
'''            SQL = SQL & "    com.fn_zz_getage(ptbs.rrgstno1, ptbs.rrgstno2, acpt.acptdd, 'A', ptbs.brthdd) as age  -- 접수일자기준 나이"
'''            SQL = SQL & "from lis.lpjmacpt acpt, lis.lpcmtest test, lis.lpcmspcm spcm, pam.pmcmptbs ptbs, com.zsdddept dept"
'''            SQL = SQL & "where acpt.instcd = ? --'성빈센트병원(017)는 고정입니다."
'''            SQL = SQL & "and acpt.acptdd between  ? and ? "
'''            SQL = SQL & "and acpt.testcd = ? -- 검사 처방코드 PMO12040 고정 GFX96 에서 다른처방 처리시 in 절로 변경"
'''            SQL = SQL & "and acpt.acptstatcd = '0' --접수상태코드(0:접수,1:취소,2:예비결과,3:최종결과, 4:수정결과 고정"
'''            SQL = SQL & "and acpt.instcd          = test.instcd"
'''            SQL = SQL & "and acpt.testcd          = test.testcd"
'''            SQL = SQL & "and acpt.instcd          = spcm.instcd"
'''            SQL = SQL & "and acpt.spccd           = spcm.spccd"
'''            SQL = SQL & "and acpt.instcd          = ptbs.instcd"
'''            SQL = SQL & "and acpt.pid             = ptbs.pid"
'''            SQL = SQL & "and acpt.instcd          = dept.instcd"
'''            SQL = SQL & "and acpt.orddeptcd       = dept.deptcd"
...
'-- 검사자 정보 가져오기
Function GetSampleInfo(ByVal asRow As Long, ByVal SPD As vaSpread, Optional ByVal pBarno As String = "") As Integer
    Dim strBarcode      As String
    Dim strRegDate      As String
    Dim lngExamNo       As Long
...
On Error GoTo DBErr
    ...
    Select Case gOCS
        ...
        Case "NU"
            SQL = ""
'            SQL = SQL & "SELECT DISTINCT"
'            SQL = SQL & " A.ACPTDD AS HOSPDATE"             '접수일자
'            SQL = SQL & ", A.ACPTNO AS BARCODE"             '접수번호
'            SQL = SQL & ", A.ACPTITEMNO AS SEQ"             '접수항목번호
'            SQL = SQL & ", A.ptno AS CHARTNO"               '병리번호
'            SQL = SQL & ", A.pid AS PID"                    '등록번호
'            SQL = SQL & ", A.testcd AS ITEM"                '검사코드
'            SQL = SQL & ", T.testengnm AS TESTNM"           '영문검사명
'            SQL = SQL & ", S.spcnm AS SPCNM"                '검체명
'            SQL = SQL & ", A.prcpgenrflag AS INOUT"         '입원/외래구분
'            SQL = SQL & ", A.ORDDEPTCD AS RACKNO"           '진료과코드
'            SQL = SQL & ", D.deptengabbr AS POSNO"          '진료과명
'            SQL = SQL & ", A.PRCPDD AS KEY1"                '처방일자',
'            SQL = SQL & ", A.EXECPRCPUNIQNO AS KEY2"        '실시처방유일번호
'            SQL = SQL & ", A.prcpno AS JUMIN"               '처방번호
'            SQL = SQL & ", P.hngnm AS PNAME"                '환자명
'            SQL = SQL & ", P.SEX AS SEX"                    '성별
'           ' SQL = SQL & ", P.BRTHDD AS AGE"                 '나이
'            SQL = SQL & ", com.fn_zz_getage(ptbs.rrgstno1, ptbs.rrgstno2, acpt.acptdd, 'A', ptbs.brthdd) as age " ' -- 접수일자기준 나이"
'
'            SQL = SQL & "  FROM lis.lpjmacpt A, lis.lpcmtest T, lis.lpcmspcm S, pam.pmcmptbs P, com.zsdddept D" & vbCr
'            SQL = SQL & " WHERE A.instcd= ? " & vbCr  '성빈센트병원(instcd = 017)는 고정입니다.
'            SQL = SQL & "   AND A.ptno = ? " & vbCr
'            SQL = SQL & "   AND A.testcd IN (?) " & vbCr
'            SQL = SQL & "   AND A.acptstatcd IN ('0')" & vbCr 'acptstatcd는 고정입니다
'            SQL = SQL & "   AND A.instcd          = T.instcd" & vbCr
'            SQL = SQL & "   AND A.testcd          = T.testcd" & vbCr
'            SQL = SQL & "   AND A.instcd          = S.instcd" & vbCr
'            SQL = SQL & "   AND A.spccd           = S.spccd" & vbCr
'            SQL = SQL & "   AND A.instcd          = P.instcd" & vbCr
'            SQL = SQL & "   AND A.pid             = P.pid" & vbCr
'            SQL = SQL & "   AND A.instcd          = D.instcd" & vbCr
'            SQL = SQL & "   AND A.orddeptcd       = D.deptcd" & vbCr
'            SQL = SQL & "   AND A.prcpdd BETWEEN D.valifromdd AND D.valitodd"
            ...
            SQL = SQL & "  FROM lis.lpjmacpt ACPT, lis.lpcmtest TEST, lis.lpcmspcm SPCM, pam.pmcmptbs PTBS, com.zsdddept DEPT" & vbCr
'            SQL = SQL & " WHERE ACPT.instcd= '" & gHOSP.HOSPCD & "'" & vbCr                        '성빈센트병원(instcd = 017)는 고정입니다.
'            SQL = SQL & "   AND ACPT.acptdd BETWEEN '" & pFrom & "' AND '" & pTo & "'" & vbCr
'            SQL = SQL & "   AND ACPT.testcd IN (" & gAllTestCd & ") " & vbCr                       '검사처방코드 : PMO12040 고정 GFX96 에서 다른처방 처리시 in 절로 변경
'            SQL = SQL & "   AND ACPT.acptstatcd = '0' " & vbCr                                     '접수상태코드 : (0:접수,1:취소,2:예비결과,3:최종결과, 4:수정결과 고정
            ...
            'AdoCmd.Parameters.Append AdoCmd.CreateParameter("A.acptstatcd", adVarChar, adParamInput, 1000, "0")
            ...
            If Not AdoRs.EOF = True And Not AdoRs.BOF = True Then
                Do Until AdoRs.EOF
                    With SPD
                        ...
                        SetText SPD, Trim(AdoRs.Fields("rsltrgstno")) & "", asRow, colPJUMIN   '처방번호
                        
                        SetText SPD, Trim(AdoRs.Fields("SEX")) & "", asRow, colPSEX
                        SetText SPD, Trim(AdoRs.Fields("AGE")) & "", asRow, colPAGE
                        'SetText SPD, Trim(AdoRs.Fields("TESTNM")) & "", asRow, colOCNT
                        'SetText SPD, Trim(AdoRs.Fields("SPCNM")) & "", asRow, colRCNT
                        SetText SPD, IIf(Trim(AdoRs.Fields("INOUT")) & "" = "I", "입원", "외래"), asRow, colINOUT
                        
'                        SetText SPD, "1", asRow, colCHECKBOX
'                        SetText SPD, "20170717", asRow, colHOSPDATE
'                        'SetText SPD, "A1234567890", asRow, colBARCODE
'                        SetText SPD, "852456", asRow, colSEQNO    '접수항목번호
'                        SetText SPD, "34567890", asRow, colCHARTNO
'                        SetText SPD, "23456789", asRow, colPID
'                        SetText SPD, "홍길동", asRow, colPNAME
'                        SetText SPD, "ICU", asRow, colRACKNO  '진료과코드
'                        SetText SPD, "중환자실", asRow, colPOSNO    '진료과명
'                        SetText SPD, "20170716", asRow, colKEY1       '처방일자
'                        SetText SPD, "98765432109876543210", asRow, colKEY2      '실시처방유일번호
'                        SetText SPD, "345678", asRow, colPJUMIN   '처방번호
'                        SetText SPD, "F", asRow, colPSEX
'                        SetText SPD, "19710311", asRow, colPAGE
'                        SetText SPD, "HPV", asRow, colOCNT
'                        SetText SPD, "SERUM", asRow, colRCNT
...
'-- 검사자 정보 가져오기
Function GetSampleInfo_Save(ByVal asRow As Long, ByVal SPD As vaSpread, Optional ByVal pBarno As String = "") As Integer
    Dim strBarcode      As String
    Dim strRegDate      As String
    Dim lngExamNo       As Long
    Dim intTestCnt      As Integer
