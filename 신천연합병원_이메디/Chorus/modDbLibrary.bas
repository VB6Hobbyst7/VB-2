    ...
    With frmInterface
        ...
        For iRow = 1 To .vasTemp.DataRowCnt
            ...
            If lsPid <> "" And sResult <> "" Then
                '--결과업데이트
                SQL = ""
                SQL = SQL & "Update RESULTOFNUM"
                SQL = SQL & "   Set TEXTRESULTVAL = '" & sResult & "',"
                SQL = SQL & "       RESULTINDATE  = '" & Format(Now, "yyyymmdd") & "',"
                SQL = SQL & "       RESULTINTIME  = '" & Format(Now, "hhmm") & "',"
                SQL = SQL & "       RESULTFLAG    = 1 "
                SQL = SQL & " Where SPCMNO = '" & lsID & "'"
                SQL = SQL & "   And RESULTITEMCODE = '" & strExamCd & "'"
                '-- 2019.12.23 추가
                SQL = SQL & "   And RESULTFLAG    <= 1 "
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_EMEDI(ByVal asRow As Long) As Integer
    Dim sDate As String
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
...
'      -- 테이블 사용
    SQL = ""
    SQL = SQL & "Select DISTINCT R.RESULTDATE as 접수일자, P.PATID as 내원번호, P.PATNAME as 이름, R.SPCMNO as 바코드번호,R.RESULTITEMCODE as ITEM "
    SQL = SQL & "  From RESULTOFNUM R, REGISTINFOS L, PATMST P "
    SQL = SQL & " Where R.RESULTDATE = '" & Format(Now, "yyyymmdd") & "'"
    SQL = SQL & "   And R.SPCMNO = '" & sBarcode & "'"
    SQL = SQL & "   And R.ACPTDATE = L.ACPTDATE"
    SQL = SQL & "   And R.ACPTSEQ = L.ACPTSEQ"
