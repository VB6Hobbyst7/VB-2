...
Public Function Get_Savdir()
    Dim objFSO      As FileSystemObject
    
    Set objFSO = New FileSystemObject
    With objFSO
        If Not .FolderExists(App.Path & "\" & BACK) Then
            Call .CreateFolder(App.Path & "\" & BACK)
        End If
    End With
    Set objFSO = Nothing
    
    Get_Savdir = App.Path & "\" & BACK
    
End Function

Public Function SaveFile(fName As String, strdata As String) As String
  ...
  FileName = Get_Savdir & fName
  With objFile
    If .FileExists(FileName) Then
      Call .DeleteFile(FileName)
    End If
    Set logFile = .OpenTextFile(FileName, ForWriting, True)
  End With
...
Function FindAge(ByVal strBirthDate As String, ByVal strAgeType As String, _
    ...
    strBirthDate = Replace(strBirthDate, "-", "")
