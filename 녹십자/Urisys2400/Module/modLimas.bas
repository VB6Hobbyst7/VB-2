...
Global Const REG_POSITION   As String = "Software\SWIT_INTERFACE\GCLAB\" & REG_INSNAME
...
'반환값 = DateDiff("단위", 날짜1, 날짜2(나중 시간))
'
'
'
'단위에는 d(일:day),w(평일, weekday), ww(주:week), m(월), y(년), q(분기), h(시), m(분), s(초)
'
'[출처] vb DateDiff 일수 및 시간 계산|작성자 고동석
...
'    Call Connect_FTP
'
'    If Len(GetString(HKEY_CURRENT_USER, REG_SRV_FTP, KEY_HOSTIP)) = 0 Then
'        frmSetFTP.Show vbModal
'    End If
...
'
    ...
    End If
'
    If Len(GetString(HKEY_CURRENT_USER, REG_ORACLEDB, REG_SERVICE)) = 0 Then
...
Public Sub ShutDown()
  ...
  If Not AdoCn_SQL Is Nothing Then
      If AdoCn_SQL.state <> adStateClosed Then AdoCn_SQL.Close
