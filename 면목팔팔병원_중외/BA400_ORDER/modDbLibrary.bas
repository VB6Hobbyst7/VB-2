...
Function GetSampleInfoW_JWINFO(ByVal asRow As Long) As Integer
    ...
    sBarcode = Trim(GetText(frmInterface.vasID, asRow, colBARCODE))
    strRcpDate = Trim(GetText(frmInterface.vasID, asRow, colHOSPDATE))
    strPtNo = Trim(GetText(frmInterface.vasID, asRow, colPID))
...
'      -- 바코드 사용
          SQL = "SELECT DISTINCT a.RECEIPTDATE as 접수일자, a.SPECIMENNUM as 바코드번호, a.IPDOPD, a.PTNO as 내원번호, a.SNAME as 이름, b.LABCODE as ITEM, a.ORDERCODE, a.SEX as SEX"
...
'                SetText .vasID, Trim(RS.Fields("챠트번호")) & "", asRow, colCHARTNO
                SetText .vasID, Trim(RS.Fields("내원번호")) & "", asRow, colPID
                SetText .vasID, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                SetText .vasID, Trim(RS.Fields("SEX")) & "", asRow, colPSEX   'ORDERCODE
...
'-- 검사자 정보 가져오기
Function GetSampleInfoW_JWINFO_Work(ByVal asRow As Long) As Integer
    Dim sBarcode    As String
    Dim GetOrderExamCode As String
    Dim intCol     As Integer
    Dim strTestCd   As String
    ...
    GetSampleInfoW_JWINFO_Work = -1
    
    sBarcode = Trim(GetText(frmInterface.vasWork, asRow, colBARCODE))
    strRcpDate = Trim(GetText(frmInterface.vasWork, asRow, colHOSPDATE))
    strPtNo = Trim(GetText(frmInterface.vasWork, asRow, colPID))
    ...
    If Not RS.EOF = True And Not RS.BOF = True Then
        With frmInterface
            Do Until RS.EOF
                ...
                SetText .vasWork, "1", asRow, colCheckBox
                SetText .vasWork, Trim(RS.Fields("접수일자")) & "", asRow, colHOSPDATE
                SetText .vasWork, Trim(RS.Fields("바코드번호")) & "", asRow, colBARCODE
                SetText .vasWork, Trim(RS.Fields("내원번호")) & "", asRow, colPID
                SetText .vasWork, Trim(RS.Fields("이름")) & "", asRow, colPNAME
                SetText .vasWork, Trim(RS.Fields("SEX")) & "", asRow, colPSEX
                SetText .vasWork, IIf(Trim(RS.Fields("IPDOPD")) = 1, "입원", "외래"), asRow, colINOUT
    ...
    frmInterface.vasWork.RowHeight(-1) = 12
