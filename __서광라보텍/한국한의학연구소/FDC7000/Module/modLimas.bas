...
Global Const REG_POSITION   As String = "Software\디딤정보\한국한의학연구소\" & REG_INSNAME
...
Public Function getSvrcInfo(ByVal strSrcNm As String, ByVal strArg1 As String, ByVal strArg2 As String) As String
    ...
    If fbput(ByVal sendBuf.bufptr, ByVal fbget_fldkey("S_TYPE1"), ByVal strText1, 0) = -1 Then
        ErrMsg = tmaxerrdesc(gettperrno())
        MsgBox "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg, vbOKOnly + vbCritical, "TMAX 에러"
        GoTo memoryfree
    End If
    
    If fbput(ByVal sendBuf.bufptr, ByVal fbget_fldkey("S_TYPE2"), ByVal strText2, 0) = -1 Then
        ErrMsg = tmaxerrdesc(gettperrno())
        MsgBox "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg, vbOKOnly + vbCritical, "TMAX 에러"
        GoTo memoryfree
    End If
    ...
    '-- S_TYPE3를 '2'로 넘겨야 함
    If fbput(ByVal sendBuf.bufptr, ByVal fbget_fldkey("S_TYPE3"), "2", 0) = -1 Then
        ErrMsg = tmaxerrdesc(gettperrno())
        MsgBox "Tmax Error Number : " & gettperrno() & vbNewLine & ErrMsg, vbOKOnly + vbCritical, "TMAX 에러"
        GoTo memoryfree
    End If
