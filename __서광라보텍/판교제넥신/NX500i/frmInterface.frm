...
Begin VB.Form frmInterface 
   ...
   Begin VB.Frame FrmHideControl 
      ...
      Begin VB.Frame Frame4 
         ...
         Begin FPSpread.vaSpread vasPrint 
            ...
            EndProperty
            MaxCols         =   9
            SpreadDesigner  =   "frmInterface.frx":3C60
         End
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin FPSpread.vaSpread vasRes 
         ...
         RetainSelBlock  =   0   'False
         ScrollBars      =   2
         SpreadDesigner  =   "frmInterface.frx":7E57
      End
   End
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VB.Frame Frame5 
            ...
            Begin VB.CheckBox chkAll 
               ...
               Width           =   705
            End
            Begin VB.OptionButton optBeast 
               Caption         =   "Rat"
               BeginProperty Font 
                  Name            =   "굴림체"
                  Size            =   9
...
Private Sub cmdExcelExport_Click()
    ...
    If chkAll.Value = "0" Then
        If optBeast(0).Value = True Then
            If optSex(0).Value = True Then
                  SQL = "SELECT SEQNO, EXAMNAME, REFLOW_MIM as LOW,REFHIGH_MIM as HIGH " & vbCrLf
            ElseIf optSex(1).Value = True Then
                  SQL = "SELECT SEQNO, EXAMNAME, REFLOW_MIF as LOW,REFHIGH_MIF as HIGH " & vbCrLf
            End If
            SQL = SQL & "  From EQPMASTER " & vbCrLf
            SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf
            SQL = SQL & " ORDER BY SEQNO * 10 "
        ElseIf optBeast(1).Value = True Then
            If optSex(0).Value = True Then
                  SQL = "SELECT SEQNO, EXAMNAME,REFLOW_MCM as LOW,REFHIGH_MCM as HIGH " & vbCrLf
            ElseIf optSex(1).Value = True Then
                  SQL = "SELECT SEQNO, EXAMNAME,REFLOW_MCF as LOW,REFHIGH_MCF as HIGH " & vbCrLf
            End If
            SQL = SQL & "  From EQPMASTER " & vbCrLf
            SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf
            SQL = SQL & " ORDER BY SEQNO * 10 "
        ElseIf optBeast(2).Value = True Then
            If optSex(0).Value = True Then
                  SQL = "SELECT SEQNO, EXAMNAME,REFLOW_RSM as LOW,REFHIGH_RSM as HIGH " & vbCrLf
            ElseIf optSex(1).Value = True Then
                  SQL = "SELECT SEQNO, EXAMNAME,REFLOW_RSF as LOW,REFHIGH_RSF as HIGH " & vbCrLf
            End If
            SQL = SQL & "  From EQPMASTER " & vbCrLf
            SQL = SQL & " WHERE EQUIPNO = '" & gEquip & "' " & vbCrLf
            SQL = SQL & " ORDER BY SEQNO * 10 "
        ...
        If vasID.Value = 1 Then
            If blnWrite = False Then
                For j = 1 To vasID.MaxCols
                    'If j = 3 Or j = 4 Or j = 5 Or j = 6 Or j = colPNAME Or j = colPSEX Or j >= 17 Then
                    If j = colEXAMDATE Or j = colBARCODE Or j = colPNAME Or j = colPSEX Or j >= 17 Then
                        If j = colEXAMDATE Then k = 1
                        If j = colBARCODE Then k = 2
                        If j = colPNAME Then k = 3
                        If j = colPSEX Then k = 4
            ...
            For j = 1 To vasID.MaxCols
                ...
                If j = colEXAMDATE Or j = colBARCODE Or j = colPNAME Or j = colPSEX Or j >= 17 Then
                    ...
                    SetText vasPrint, Trim(GetText(vasID, iRow, j)), iRow + 1, k
                End If
            Next
        End If
    Next iRow
    
    If SQL <> "" And blnWrite = True And blnRef = False Then
        Set RS_ref = cn.Execute(SQL)
        Do Until RS_ref.EOF
            For i = colState + 1 To vasID.MaxCols
                If Trim(GetText(vasID, 0, i)) = Trim(RS_ref.Fields("EXAMNAME")) Then
                    'Debug.Print RS_ref.Fields("LOW") & "-" & RS_ref.Fields("HIGH")
                    SetText vasPrint, RS_ref.Fields("LOW") & "-" & RS_ref.Fields("HIGH"), 1, i - 12
                    Exit For
                End If
            Next
            RS_ref.MoveNext
    ...
    Else
        ...
        Else
            'MsgBox "있다"
            If MsgBox("같은 엑셀파일이 있습니다." & vbNewLine & "덮어쓰시겠습니까?", vbYesNo + vbInformation) = vbYes Then
                SaveExcel sFileName, vasPrint
                MsgBox "엑셀 저장완료", vbOKOnly + vbInformation, Me.Caption
            End If
        End If
...
Rst:
    ...
    If Err.Number = 32755 Then
        MsgBox "엑셀 저장취소", vbOKOnly + vbCritical, Me.Caption
    End If
...
Private Sub cmdRsltSearch_Click()
    ...
    If chkAll.Value = "0" Then
        If optBeast(0).Value = True Then
            SQL = SQL & "    AND PNAME = 'Mouse(ICR)'"
        ElseIf optBeast(1).Value = True Then
            SQL = SQL & "    AND PNAME = 'Mouse(C57BL/6)'"
        ElseIf optBeast(2).Value = True Then
            SQL = SQL & "    AND PNAME = 'Rat'"
        End If
...
'    stInterface.Tab = 0
    ...
    tmrSign.Interval = 1000
    tmrSign.Enabled = True
    
'    cboBeast.Clear
'    cboBeast.AddItem "All"
'    cboBeast.AddItem "Mouse(ICR)"
'    cboBeast.AddItem "Mouse(C57BL/6)"
'    cboBeast.AddItem "Rat"
'    cboBeast.ListIndex = 0
...
'    Call SetText(vasID, mResult.RackNo, intRow, colDISKNO)
    ...
    Call SetText(vasID, mResult.Kind, intRow, colPNAME)
    Call SetText(vasID, mResult.Sex, intRow, colPSEX)
    
'    If optBeast(0).Value = True Then
'        Call SetText(vasID, optBeast(0).Caption, intRow, colPNAME)
'    ElseIf optBeast(1).Value = True Then
'        Call SetText(vasID, optBeast(1).Caption, intRow, colPNAME)
'    ElseIf optBeast(2).Value = True Then
'        Call SetText(vasID, optBeast(2).Caption, intRow, colPNAME)
'    End If
...
Private Sub EditRcvData_NX500()
    ...
    ElseIf UCase(strType) = "N" Then
        ...
        strSeq = Trim(Mid(strRcvBuf, 23, 3))
        If optBeast(0).Value = True Then
            strKind = optBeast(0).Caption
        ElseIf optBeast(1).Value = True Then
            strKind = optBeast(1).Caption
        ElseIf optBeast(2).Value = True Then
            strKind = optBeast(2).Caption
        End If
        
        If optSex(0).Value = True Then
            strSex = Mid(optSex(0).Caption, 1, 1)
        ElseIf optSex(1).Value = True Then
            strSex = Mid(optSex(1).Caption, 1, 1)
        End If
...
Private Sub vasID_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyDelete Then
        ...
        lsDate = Trim(GetText(vasID, iRow, colEXAMDATE))
...
'        End If
        ...
        SQL = SQL & "   AND MID(EXAMDATE,1,8) = '" & lsDate & "' "
        Res = SendQuery(gLocal, SQL)
