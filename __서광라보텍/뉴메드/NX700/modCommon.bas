...
'    strEqpCd = frmInterface.txtTemp
    ...
    db_tmp = ""
    Call GetPrivateProfileString("Server", "INUNIT", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gINUnit = Trim(frmInterface.txtTemp)

    db_tmp = ""
    Call GetPrivateProfileString("Server", "SAVEDT", "", db_tmp, 100, App.Path & "\Interface.ini")
    frmInterface.txtTemp = Trim(db_tmp)
    gSaveDt = Trim(frmInterface.txtTemp)
